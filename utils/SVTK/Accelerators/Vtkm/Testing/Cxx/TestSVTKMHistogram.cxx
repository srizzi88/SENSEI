...
void MakeStatDistDataSet(svtkDataSet* dataset)
{
  ...
  // Due to the fact that SetScalars API on svtkDataSetAttributes would add the array
  // and replace it with the existing scalars, we need to add data as point arrays, then
  // switch active scalars when needed.
  AddArrayToSVTKData("p_poission", pd, poisson, nVerts);
  AddArrayToSVTKData("p_normal", pd, normal, nVerts);
  AddArrayToSVTKData("p_chiSquare", pd, chiSquare, nVerts);
  AddArrayToSVTKData("p_uniform", pd, uniform, nVerts);

  // Set cell scalars
  AddArrayToSVTKData("c_poission", cd, poisson, nCells);
  AddArrayToSVTKData("c_normal", cd, normal, nCells);
  AddArrayToSVTKData("c_chiSquare", cd, chiSquare, nCells);
  AddArrayToSVTKData("c_uniform", cd, uniform, nCells);

  // Add a simple data to test custom data range and center bins around min and max
  static double simpleData[6] = { 0, 2, 4, 6, 8, 10 };
  AddArrayToSVTKData("simple_data", pd, simpleData, 6);
}

void VerifyHistogram(svtkmHistogram* filter, bool verifyPointData)
{
  svtkTable* table = svtkTable::SafeDownCast(filter->GetOutput());
  assert(table != nullptr);
  svtkDoubleArray* binExtents =
    svtkDoubleArray::SafeDownCast(table->GetRowData()->GetArray("bin_extents"));
  if (binExtents == nullptr)
  {
    std::cout << "bin_extents is missing from the result svtkTable" << std::endl;
    return;
  }
  ...
  svtkDataArray* binValues =
    svtkDataArray::SafeDownCast(table->GetRowData()->GetAbstractArray("bin_values"));
  assert(binValues != nullptr);
  double sum = 0;
  svtkIdType numberOfBins = static_cast<svtkIdType>(filter->GetNumberOfBins());
  for (svtkIdType i = 0; i < numberOfBins; i++)
  {
...
void testHistogramWithPointData(svtkmHistogram* filter, bool isPointType)
{
  int fieldAsso =
    isPointType ? svtkDataObject::FIELD_ASSOCIATION_POINTS : svtkDataObject::FIELD_ASSOCIATION_CELLS;
...
int TestSVTKMHistogram(int, char*[])
{
  svtkNew<svtkPolyData> ds;
  MakeStatDistDataSet(ds);

  svtkNew<svtkmHistogram> filter;
  // Run Data
  ...
  filter->SetInputArrayToProcess(0, 0, 0, svtkDataObject::FIELD_ASSOCIATION_POINTS, "simple_data");
  filter->Update();
  svtkTable* sdTable = svtkTable::SafeDownCast(filter->GetOutput());
  assert(sdTable != nullptr);
  svtkDoubleArray* binExtents =
    svtkDoubleArray::SafeDownCast(sdTable->GetRowData()->GetArray("bin_extents"));
  double binExtentsResult[4] = { 2, 4, 6, 8 };
  double binValuesResult[4] = { 2, 1, 1, 2 };
  for (svtkIdType i = 0; i < binExtents->GetNumberOfTuples(); i++)
  {
  ...
  svtkDataArray* binValues =
    svtkDataArray::SafeDownCast(sdTable->GetRowData()->GetAbstractArray("bin_values"));
  assert(binValues != nullptr);
  for (svtkIdType i = 0; i < binValues->GetNumberOfTuples(); i++)
  {
