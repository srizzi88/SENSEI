...
int TestBoxPlot(int, char*[])
{
  ...
  svtkNew<svtkChartBox> chart;
  view->GetScene()->AddItem(chart);

  // Creates a svtkPlotBox input table
  // The svtkPlotBox object will display 4 (arbitrary) box plots
  int numParam = 5;
  svtkNew<svtkTable> inputBoxPlotTable;

  for (int i = 0; i < numParam; i++)
  {
    char num[10];
    snprintf(num, sizeof(num), "P%d", i);
    svtkNew<svtkIntArray> arrIndex;
  ...
  svtkPlotBox::SafeDownCast(chart->GetPlot(0))->SetColumnColor("P1", rgb);
