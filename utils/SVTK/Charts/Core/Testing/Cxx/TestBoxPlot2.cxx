...
int TestBoxPlot2(int, char*[])
{
  ...
  svtkSmartPointer<svtkChartBox> chart = svtkSmartPointer<svtkChartBox>::New();
  view->GetScene()->AddItem(chart);

  // Creates a svtkPlotBox input table
  int numberOfColumns = 5;
  svtkSmartPointer<svtkTable> inputBoxPlotTable = svtkSmartPointer<svtkTable>::New();

  for (int i = 0; i < numberOfColumns; ++i)
  {
    char num[10];
    snprintf(num, sizeof(num), "Run %d", i + 1);
    svtkSmartPointer<svtkIntArray> arrIndex = svtkSmartPointer<svtkIntArray>::New();
  ...
  svtkSmartPointer<svtkComputeQuartiles> quartiles = svtkSmartPointer<svtkComputeQuartiles>::New();
  quartiles->SetInputData(svtkStatisticsAlgorithm::INPUT_DATA, inputBoxPlotTable);
  quartiles->Update();

  svtkSmartPointer<svtkLookupTable> lookup = svtkSmartPointer<svtkLookupTable>::New();
  ...
  chart->GetPlot(0)->AddObserver(svtkCommand::ErrorEvent, errorObserver);
  ...
  svtkTable* outTable = quartiles->GetOutput();
