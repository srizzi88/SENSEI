...
int TestContextItemStacking(int, char*[])
{
  ...
  svtkNew<svtkBlockItem> rootItem;
  rootItem->SetDimensions(0, 350, 50, 50);

  int i = 0;
  int s = 120;
  int step = s / 3;
  svtkNew<svtkBlockItem> test1;
  ...
  test2->SetLabel("2");
  i += step;
  svtkNew<svtkBlockItem> test3;
  test3->SetDimensions(i, i, s, s);
  test3->SetLabel("3");
  i += step;
  svtkNew<svtkBlockItem> test4;
  test4->SetDimensions(i, i, s, s);
  test4->SetLabel("4");
  i += step;
  svtkNew<svtkBlockItem> test41;
  test41->SetDimensions(i, i, s, s);
  test41->SetLabel("4.1");
  i += step;
  svtkNew<svtkBlockItem> test411;
  test411->SetDimensions(i, i, s, s);
  test411->SetLabel("4.1.1");
  i += step;
  svtkNew<svtkBlockItem> test42;
  test42->SetDimensions(i, i, s, s);
  test42->SetLabel("4.2");
  i += step;
  svtkNew<svtkBlockItem> test5;
  test5->SetDimensions(i, i, s, s);
  test5->SetLabel("5");

  // Build up our multi-level scene
  svtkIdType index1 = rootItem->AddItem(test1);
  svtkIdType index2 = rootItem->AddItem(test2);
  svtkIdType index3 = rootItem->AddItem(test3);
  svtkIdType index4 = rootItem->AddItem(test4);
  svtkIdType index41 = test4->AddItem(test41);
  svtkIdType index411 = test41->AddItem(test411);
  svtkIdType index42 = test4->AddItem(test42);
  svtkIdType index5 = rootItem->AddItem(test5);
