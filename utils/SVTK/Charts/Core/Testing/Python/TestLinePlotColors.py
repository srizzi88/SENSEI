...
class TestLinePlotColors(svtk.test.Testing.svtkTest):
    def testLinePlot(self):
        ...
        line0 = chart.AddPlot(svtk.svtkChart.LINE)
        line0.SetInputData(table, 0, 1)
        line0.SetColor(50, 50, 50, 255)
        line0.SetWidth(3.0)
        line0.GetPen().SetLineType(svtk.svtkPen.SOLID_LINE)
        line0.SetMarkerStyle(svtk.svtkPlotPoints.CIRCLE)
        line0.SetScalarVisibility(1)
        line0.SetLookupTable(lut)
        line0.SelectColorArray(1)

        line1 = chart.AddPlot(svtk.svtkChart.LINE)
        line1.SetInputData(table, 0, 2)
        line1.GetPen().SetLineType(svtk.svtkPen.NO_PEN)
        line1.SetMarkerStyle(svtk.svtkPlotPoints.PLUS)
        line1.SetColor(150, 100, 0, 255)

        line2 = chart.AddPlot(svtk.svtkChart.LINE)
        line2.SetInputData(table, 0, 3)
        line2.SetColor(100, 100, 100, 255)
        line2.SetWidth(3.0)
        line2.GetPen().SetLineType(svtk.svtkPen.DASH_LINE)
        line2.SetMarkerStyle(svtk.svtkPlotPoints.SQUARE)
...
if __name__ == "__main__":
    svtk.test.Testing.main([(TestLinePlotColors, 'test')])
