...
svtkAxis::svtkAxis()
{
  ...
  this->TickLength = 5;
  this->LabelProperties = svtkTextProperty::New();
  ...
  this->Notation = svtkAxis::STANDARD_NOTATION;
  this->Behavior = svtkAxis::AUTO;
  ...
  this->GridPen = svtkPen::New();
  this->GridPen->SetColor(242, 242, 242);
  this->GridPen->SetWidth(1.0);
  this->TickPositions = svtkSmartPointer<svtkDoubleArray>::New();
  this->TickScenePositions = svtkSmartPointer<svtkFloatArray>::New();
  this->TickLabels = svtkSmartPointer<svtkStringArray>::New();
  this->UsingNiceMinMax = false;
  this->TickMarksDirty = true;
  this->MaxLabel[0] = this->MaxLabel[1] = 0.0;
  this->Resized = true;
  this->SetPosition(svtkAxis::LEFT);
  this->TickLabelAlgorithm = svtkAxis::TICK_SIMPLE;
...
void svtkAxis::SetPosition(int position)
{
...
//-----------------------------------------------------------------------------
void svtkAxis::SetPoint1(const svtkVector2f& pos)
{
...
//-----------------------------------------------------------------------------
void svtkAxis::SetPoint1(float x, float y)
{
  this->SetPoint1(svtkVector2f(x, y));
}

//-----------------------------------------------------------------------------
svtkVector2f svtkAxis::GetPosition1()
{
  return this->Position1;
}

//-----------------------------------------------------------------------------
void svtkAxis::SetPoint2(const svtkVector2f& pos)
{
...
//-----------------------------------------------------------------------------
void svtkAxis::SetPoint2(float x, float y)
{
  this->SetPoint2(svtkVector2f(x, y));
}

//-----------------------------------------------------------------------------
svtkVector2f svtkAxis::GetPosition2()
{
  return this->Position2;
}

//-----------------------------------------------------------------------------
void svtkAxis::SetNumberOfTicks(int numberOfTicks)
{
...
void svtkAxis::Update()
{
  ...
  if ((this->Behavior == svtkAxis::AUTO || this->Behavior == svtkAxis::FIXED) && this->TickMarksDirty)
  {
  ...
  if (this->Resized && (this->Behavior == svtkAxis::AUTO || this->Behavior == svtkAxis::FIXED))
  {
    this->RecalculateTickSpacing();
    this->Resized = false;
  }
  ...
  svtkIdType n = this->TickPositions->GetNumberOfTuples();
  this->TickScenePositions->SetNumberOfTuples(n);
  for (svtkIdType i = 0; i < n; ++i)
  {
    int iPos = svtkContext2D::FloatToInt(
...
//-----------------------------------------------------------------------------
bool svtkAxis::Paint(svtkContext2D* painter)
{
  // This is where everything should be drawn, or dispatched to other methods.
  svtkDebugMacro(<< "Paint event called in svtkAxis.");
  ...
  if (!this->Title.empty() && this->TitleVisible)
  {
    ...
    // Draw the axis label
    if (this->Position == svtkAxis::LEFT)
    {
      // Draw the axis label
      x = svtkContext2D::FloatToInt(this->Point1[0] - this->MaxLabel[0] - 10);
      y = svtkContext2D::FloatToInt(this->Point1[1] + this->Point2[1]) / 2;
    }
    else if (this->Position == svtkAxis::RIGHT)
    {
      // Draw the axis label
      x = svtkContext2D::FloatToInt(this->Point1[0] + this->MaxLabel[0] + 10);
      y = svtkContext2D::FloatToInt(this->Point1[1] + this->Point2[1]) / 2;
    }
    else if (this->Position == svtkAxis::BOTTOM)
    {
      x = svtkContext2D::FloatToInt(this->Point1[0] + this->Point2[0]) / 2;
      y = svtkContext2D::FloatToInt(this->Point1[1] - this->MaxLabel[1] - 10);
    }
    else if (this->Position == svtkAxis::TOP)
    {
      x = svtkContext2D::FloatToInt(this->Point1[0] + this->Point2[0]) / 2;
      y = svtkContext2D::FloatToInt(this->Point1[1] + this->MaxLabel[1] + 10);
    }
    else if (this->Position == svtkAxis::PARALLEL)
    {
      x = svtkContext2D::FloatToInt(this->Point1[0]);
      y = svtkContext2D::FloatToInt(this->Point1[1] - this->MaxLabel[1] - 15);
    }
  ...
  svtkStdString* tickLabel = this->TickLabels->GetPointer(0);
  svtkIdType numMarks = this->TickScenePositions->GetNumberOfTuples();
  ...
  if (this->Position == svtkAxis::LEFT || this->Position == svtkAxis::PARALLEL ||
    this->Position == svtkAxis::BOTTOM)
  {
  ...
  svtkVector2i tileScale(1);
  if (!this->Scene)
  {
    svtkWarningMacro("svtkAxis needs a svtkContextScene to determine window "
  ...
  svtkRectf minLabelRect(0, 0, 0, 0);
  svtkRectf maxLabelRect(0, 0, 0, 0);
  ...
  if (this->RangeLabelsVisible)
  {
    ...
    else if (this->Position == svtkAxis::TOP || this->Position == svtkAxis::BOTTOM)
    {
    ...
    this->Position == svtkAxis::RIGHT)
  {
    ...
    for (svtkIdType i = 0; i < numMarks; ++i)
    {
      ...
      if (this->LabelsVisible)
      {
        ...
        svtkRectf boundsRect(bounds[0], bounds[1], bounds[2], bounds[3]);
  ...
  {
    svtkWarningMacro("Unknown position encountered in the paint call: " << this->Position);
  }
...
//-----------------------------------------------------------------------------
void svtkAxis::SetMinimum(double minimum)
{
  ...
  this->TickMarksDirty = true;
  this->Modified();
  this->InvokeEvent(svtkChart::UpdateRange);
}

//-----------------------------------------------------------------------------
void svtkAxis::SetUnscaledMinimum(double minimum)
{
...
//-----------------------------------------------------------------------------
void svtkAxis::SetMinimumLimit(double lowest)
{
...
//-----------------------------------------------------------------------------
void svtkAxis::SetUnscaledMinimumLimit(double lowest)
{
...
//-----------------------------------------------------------------------------
void svtkAxis::SetMaximum(double maximum)
{
...
//-----------------------------------------------------------------------------
void svtkAxis::SetUnscaledMaximum(double maximum)
{
...
//-----------------------------------------------------------------------------
void svtkAxis::SetMaximumLimit(double highest)
{
...
//-----------------------------------------------------------------------------
void svtkAxis::SetUnscaledMaximumLimit(double highest)
{
...
//-----------------------------------------------------------------------------
void svtkAxis::SetRange(double minimum, double maximum)
{
...
//-----------------------------------------------------------------------------
void svtkAxis::SetRange(double* range)
{
  if (range)
  {
...
//-----------------------------------------------------------------------------
void svtkAxis::SetUnscaledRange(double minimum, double maximum)
{
  bool rangeModified = false;
...
//-----------------------------------------------------------------------------
void svtkAxis::SetUnscaledRange(double* range)
{
  if (range)
  {
...
//-----------------------------------------------------------------------------
void svtkAxis::GetRange(double* range)
{
  if (range)
  {
...
//-----------------------------------------------------------------------------
void svtkAxis::GetUnscaledRange(double* range)
{
  if (range)
  {
...
//-----------------------------------------------------------------------------
void svtkAxis::SetTitle(const svtkStdString& title)
{
  if (this->Title != title)
  {
    this->Title = title;
...
//-----------------------------------------------------------------------------
svtkStdString svtkAxis::GetTitle()
{
  return this->Title;
}

//-----------------------------------------------------------------------------
void svtkAxis::SetPrecision(int precision)
{
...
//-----------------------------------------------------------------------------
void svtkAxis::SetLabelFormat(const std::string& fmt)
{
  svtkDebugMacro(<< this->GetClassName() << " (" << this << "): setting LabelFormat to " << fmt);
...
//-----------------------------------------------------------------------------
void svtkAxis::SetLogScale(bool logScale)
{
...
//-----------------------------------------------------------------------------
void svtkAxis::SetNotation(int notation)
{
...
//-----------------------------------------------------------------------------
void svtkAxis::AutoScale()
{
  if (this->Behavior != svtkAxis::AUTO)
  {
    return;
  }
  ...
  if (this->TickLabelAlgorithm == svtkAxis::TICK_SIMPLE)
  {
...
//-----------------------------------------------------------------------------
void svtkAxis::RecalculateTickSpacing()
{
  // Calculate the min and max, set the number of ticks and the tick spacing,
  // discard the min and max in this case. TODO: Refactor the function called.
  if (this->Behavior == svtkAxis::AUTO || this->Behavior == svtkAxis::FIXED)
  {
    double min = this->Minimum;
    double max = this->Maximum;
    if (this->TickLabelAlgorithm == svtkAxis::TICK_SIMPLE)
...
//-----------------------------------------------------------------------------
svtkDoubleArray* svtkAxis::GetTickPositions()
{
  return this->TickPositions;
}

//-----------------------------------------------------------------------------
svtkFloatArray* svtkAxis::GetTickScenePositions()
{
  return this->TickScenePositions;
}

//-----------------------------------------------------------------------------
svtkStringArray* svtkAxis::GetTickLabels()
{
  return this->TickLabels;
}

//-----------------------------------------------------------------------------
bool svtkAxis::SetCustomTickPositions(svtkDoubleArray* positions, svtkStringArray* labels)
{
  if (!positions && !labels)
  {
    this->CustomTickLabels = false;
...
//-----------------------------------------------------------------------------
svtkRectf svtkAxis::GetBoundingRect(svtkContext2D* painter)
{
  bool vertical = false;
  if (this->Position == svtkAxis::LEFT || this->Position == svtkAxis::RIGHT ||
    this->Position == svtkAxis::PARALLEL)
  {
    vertical = true;
  }
  ...
  float tallest = 0.0;
  svtkRectf bounds(0, 0, 0, 0);
  if (this->LabelsVisible)
  {
    for (svtkIdType i = 0; i < this->TickLabels->GetNumberOfTuples(); ++i)
    {
      painter->ApplyTextProp(this->LabelProperties);
  ...
  if (this->RangeLabelsVisible)
  {
    ...
    svtkStdString minLabel =
      this->GenerateSprintfLabel(this->UnscaledMinimum, this->RangeLabelFormat);
    svtkStdString maxLabel =
      this->GenerateSprintfLabel(this->UnscaledMaximum, this->RangeLabelFormat);
...
//-----------------------------------------------------------------------------
void svtkAxis::UpdateLogScaleActive(bool alwaysUpdateMinMaxFromUnscaled)
{
    ...
    if (needUpdate || alwaysUpdateMinMaxFromUnscaled)
    {
      ...
      { // Both unscaled min & max are negative, logs must be swapped
        ...
        if (this->UnscaledMaximumLimit >= 0)
        {
          ...
          this->MinimumLimit = -svtkMath::Inf();
          this->NonLogUnscaledMaxLimit = this->UnscaledMaximumLimit;
...
//-----------------------------------------------------------------------------
void svtkAxis::GenerateTickLabels(double min, double max)
{
  ...
  {
    if (this->TickLabelAlgorithm == svtkAxis::TICK_WILKINSON_EXTENDED)
    {
      // Now calculate the tick labels, and positions within the axis range
      ...
      svtkNew<svtkAxisExtended> tickPositionExtended;
      ...
      svtkVector3d values = tickPositionExtended->GenerateExtendedTickLabels(min, max, 4, scaling);
      min = values[0];
      max = values[1];
    ...
    if (this->LogScaleActive)
    {
      ...
      n = svtkContext2D::FloatToInt(range / pow(10.0, this->TickInterval));
    }
    ...
    else
    {
      ...
      n = svtkContext2D::FloatToInt(range / this->TickInterval);
    }
    for (int i = 0; i <= n; ++i)
    {
      double value = 0.0;
...
//-----------------------------------------------------------------------------
void svtkAxis::GenerateTickLabels()
{
  this->TickLabels->SetNumberOfTuples(0);
  for (svtkIdType i = 0; i < this->TickPositions->GetNumberOfTuples(); ++i)
  {
...
//-----------------------------------------------------------------------------
svtkStdString svtkAxis::GenerateSimpleLabel(double val)
{
  ...
  { // Use the C++ style stream format specification:
    ...
    ostr << val;
    result = svtkStdString(ostr.str());
  }

  // Strip out leading zeros on the exponent:
  svtksys::RegularExpression regExp("[Ee][+-]");
  if (regExp.find(result))
  {
    svtkStdString::iterator it = result.begin() + regExp.start() + 2;
  ...
  {
    svtksys::RegularExpression regExp2("[+-]?[0-9]+\\.[0-9]+");
    if (regExp2.find(result))
    {
      svtkStdString tmp(result);
      long num = std::lround(stof(regExp2.match(0)));
      result = std::to_string(num);
      svtkStdString::iterator it = tmp.begin();
...
void svtkAxis::GenerateLabelFormat(int notation, double n)
{
  std::ostringstream ostr;
  ostr.imbue(std::locale::classic());
...
//-----------------------------------------------------------------------------
svtkStdString svtkAxis::GenerateSprintfLabel(double value, const std::string& format)
{
  // Use the C-style printf specification:
  ...
  svtkStdString result = svtkStdString(buffer);

  return result;
}

//-----------------------------------------------------------------------------
double svtkAxis::NiceMinMax(double& min, double& max, float pixelRange, float tickPixelSpacing)
{
  ...
  int maxTicks = svtkContext2D::FloatToInt(pixelRange / tickPixelSpacing);
  if (maxTicks == 0)
  {
  ...
  double niceTickSpacing = svtkAxis::NiceNumber(normTickSpacing, true);
...
//-----------------------------------------------------------------------------
double svtkAxis::CalculateNiceMinMax(double& min, double& max)
{
  ...
  if (max < min)
  {
    niceTickSpacing = svtkAxis::NiceMinMax(max, min, pixelRange, tickPixelSpacing);
  }
  else
  {
    niceTickSpacing = svtkAxis::NiceMinMax(min, max, pixelRange, tickPixelSpacing);
  }
...
//-----------------------------------------------------------------------------
double svtkAxis::NiceNumber(double n, bool roundUp)
{
  if (roundUp)
  {
    if (n <= 1.0)
...
//-----------------------------------------------------------------------------
double svtkAxis::LogScaleTickMark(double number, bool roundUp, bool& niceValue, int& order)
{
...
//-----------------------------------------------------------------------------
void svtkAxis::GenerateLogSpacedLinearTicks(int order, double min, double max)
{
...
//-----------------------------------------------------------------------------
void svtkAxis::GenerateLogScaleTickMarks(int order, double min, double max, bool detailLabels)
{
...
//-----------------------------------------------------------------------------
inline bool svtkAxis::InRange(double value)
{
...
//-----------------------------------------------------------------------------
void svtkAxis::PrintSelf(ostream& os, svtkIndent indent)
{
