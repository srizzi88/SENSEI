...
#include "svtkMath.h" // for SVTK_DBL_EPSILON
...
svtkStandardNewMacro(svtkAxisExtended);

svtkAxisExtended::svtkAxisExtended()
{
  this->FontSize = 0;
...
// and j
double svtkAxisExtended::SimplicityMax(int qIndex, int qLength, int j)
{
  int v = 1;
  ++qIndex;
  return 1.0 - (qIndex - 1.0) / (qLength - 1.0) - j + v;
...
// more preferable
double svtkAxisExtended::Coverage(double dmin, double dmax, double lmin, double lmax)
{
  double coverage =
...
double svtkAxisExtended::CoverageMax(double dmin, double dmax, double span)
{
...
// user given value
double svtkAxisExtended::Density(int k, double m, double dmin, double dmax, double lmin, double lmax)
{
...
// (user given)
double svtkAxisExtended::DensityMax(int k, double m)
{
  if (k >= m)
  {
    return 2 - (k - 1) / (m - 1);
...
double svtkAxisExtended::FormatLegibilityScore(double n, int format)
{
  switch (format)
  {
    case 1:
...
int svtkAxisExtended::FormatStringLength(int format, double n, int precision)
{
...
double svtkAxisExtended::Legibility(
...
svtkVector3d svtkAxisExtended::GenerateExtendedTickLabels(
  double dmin, double dmax, double m, double scaling)
{
      ...
      while (k < INF)
      {
        ...
        while (z < INF)
        {
          ...
          for (int start = minStart; start <= maxStart; ++start)
          {
            ...
            // svtkVector<double,4> l = this->Legibility(lmin, lmax, lstep, scaling);

            svtkVector<int, 3> legibilityIndex;
  ...
  ans[1] = bestLmax;
  ans[2] = bestLstep;
  // svtkVector3d answers(bestLmin, bestLmax, bestLstep);
  // return Sequence(bestLmin, bestLmax, bestLstep);
  return ans;
}

void svtkAxisExtended::PrintSelf(ostream& os, svtkIndent indent)
{
