...
//-----------------------------------------------------------------------------
class svtkChartXYPrivate
{
public:
  svtkChartXYPrivate()
  {
    this->Colors = svtkSmartPointer<svtkColorSeries>::New();
    this->Clip = svtkSmartPointer<svtkContextClip>::New();
    this->Borders[0] = 60;
    this->Borders[1] = 50;
    this->Borders[2] = 20;
    this->Borders[3] = 20;
  }
  svtkPlot* GetPlotByColumn(svtkIdType columnId)
  {
    std::vector<svtkPlot*>::iterator it = this->plots.begin();
  ...
  std::vector<svtkPlot*> plots;                   // Charts can contain multiple plots of data
  std::vector<svtkContextTransform*> PlotCorners; // Stored by corner...
  std::vector<svtkAxis*> axes;                    // Charts can contain multiple axes
  svtkSmartPointer<svtkColorSeries> Colors;        // Colors in the chart
  svtkSmartPointer<svtkContextClip> Clip;          // Colors in the chart
  int Borders[4];
  svtkTimeStamp TransformCalculatedTime;
};

//-----------------------------------------------------------------------------
svtkStandardNewMacro(svtkChartXY);

//-----------------------------------------------------------------------------
svtkChartXY::svtkChartXY()
{
  this->ChartPrivate = new svtkChartXYPrivate;
  ...
  svtkSmartPointer<svtkContextTransform> corner = svtkSmartPointer<svtkContextTransform>::New();
  ...
  for (int i = 0; i < 4; ++i)
  {
    this->ChartPrivate->axes.push_back(svtkAxis::New());
  ...
  this->ChartPrivate->axes[svtkAxis::LEFT]->SetPosition(svtkAxis::LEFT);
  this->ChartPrivate->axes[svtkAxis::BOTTOM]->SetPosition(svtkAxis::BOTTOM);
  this->ChartPrivate->axes[svtkAxis::RIGHT]->SetPosition(svtkAxis::RIGHT);
  this->ChartPrivate->axes[svtkAxis::TOP]->SetPosition(svtkAxis::TOP);

  // Set up the x and y axes - should be configured based on data
  this->ChartPrivate->axes[svtkAxis::LEFT]->SetTitle("Y Axis");
  this->ChartPrivate->axes[svtkAxis::BOTTOM]->SetTitle("X Axis");

  grid1->SetXAxis(this->ChartPrivate->axes[svtkAxis::BOTTOM]);
  grid1->SetYAxis(this->ChartPrivate->axes[svtkAxis::LEFT]);
  grid2->SetXAxis(this->ChartPrivate->axes[svtkAxis::TOP]);
  grid2->SetYAxis(this->ChartPrivate->axes[svtkAxis::RIGHT]);

  // Then the legend is drawn
  this->Legend = svtkSmartPointer<svtkChartLegend>::New();
...
void svtkChartXY::Update()
{
  ...
  if (this->AnnotationLink)
  {
    ...
    svtkSelection* selection =
      svtkSelection::SafeDownCast(this->AnnotationLink->GetOutputDataObject(2));
    // Two major selection methods - row based or plot based.
    if (this->SelectionMethod == svtkChart::SELECTION_ROWS)
    {
      svtkSelectionNode* node = selection->GetNumberOfNodes() > 0 ? selection->GetNode(0) : nullptr;
      svtkIdTypeArray* idArray =
        node ? svtkArrayDownCast<svtkIdTypeArray>(node->GetSelectionList()) : nullptr;
      std::vector<svtkPlot*>::iterator it = this->ChartPrivate->plots.begin();
    ...
    else if (this->SelectionMethod == svtkChart::SELECTION_PLOTS)
    {
      for (unsigned int i = 0; i < selection->GetNumberOfNodes(); ++i)
      {
        ...
        svtkPlot* selectionPlot =
          svtkPlot::SafeDownCast(node->GetProperties()->Get(svtkSelectionNode::PROP()));
    ...
    else if (this->SelectionMethod == svtkChart::SELECTION_COLUMNS)
    {
      // Retrieve all the selected plots
      std::vector<svtkPlot*> selectedPlots;
      for (unsigned int i = 0; i < selection->GetNumberOfNodes(); ++i)
      {
        svtkSelectionNode* node = selection->GetNode(i);
        svtkIdTypeArray* selectedColumns =
          svtkArrayDownCast<svtkIdTypeArray>(node->GetSelectionList());
        svtkIdType* ptr = reinterpret_cast<svtkIdType*>(selectedColumns->GetVoidPointer(0));
        for (svtkIdType j = 0; j < selectedColumns->GetNumberOfTuples(); ++j)
        {
          svtkPlot* selectedPlot = this->ChartPrivate->GetPlotByColumn(ptr[j]);
          if (selectedPlot)
          {
      ...
      {
        svtkPlot* plot = *it;
        svtkIdTypeArray* plotSelection = nullptr;
        ...
        if (isSelected)
        {
          ...
          {
            plotSelection = svtkIdTypeArray::New();
            ownPlotSelection = true;
            for (svtkIdType j = 0; j < column->GetNumberOfTuples(); ++j)
            {
  ...
  if (this->AutoAxes)
  {
    svtkTuple<bool, 4> visibilities(false);
    for (int i = 0; i < static_cast<int>(this->ChartPrivate->PlotCorners.size()); ++i)
    {
      int visible = 0;
      for (svtkIdType j = 0; j < this->ChartPrivate->PlotCorners[i]->GetNumberOfItems(); ++j)
      {
        if (svtkPlot::SafeDownCast(this->ChartPrivate->PlotCorners[i]->GetItem(j))->GetVisible())
        {
          ++visible;
        }
      }
...
//-----------------------------------------------------------------------------
bool svtkChartXY::Paint(svtkContext2D* painter)
{
  ...
  svtkVector2i geometry(0, 0);
  bool recalculateTransform = false;
  if (this->LayoutStrategy == svtkChart::FILL_SCENE)
  {
    geometry = svtkVector2i(this->GetScene()->GetSceneWidth(), this->GetScene()->GetSceneHeight());
    if (geometry.GetX() != this->Geometry[0] || geometry.GetY() != this->Geometry[1])
    {
      recalculateTransform = true;
    }
    this->SetSize(svtkRectf(0.0, 0.0, geometry.GetX(), geometry.GetY()));
  }
  ...
  // shift-scale factors, have been updated, we give the svtkPlot instances an
  ...
  {
    painter->GetPen()->SetLineType(svtkPen::NO_PEN);
  ...
  if (this->DrawSelectionPolygon)
  {
    ...
    const svtkContextPolygon& polygon = this->SelectionPolygon;

    // draw each line segment
    for (svtkIdType i = 0; i < polygon.GetNumberOfPoints() - 1; i++)
    {
      const svtkVector2f& a = polygon.GetPoint(i);
      const svtkVector2f& b = polygon.GetPoint(i + 1);
    ...
    {
      const svtkVector2f& start = polygon.GetPoint(0);
      const svtkVector2f& end = polygon.GetPoint(polygon.GetNumberOfPoints() - 1);
  ...
  if (this->Title)
  {
    ...
    svtkAxis* topAxis = this->ChartPrivate->axes[svtkAxis::TOP];
    if (topAxis->GetVisible())
    {
      svtkRectf bounds = topAxis->GetBoundingRect(painter);
...
//-----------------------------------------------------------------------------
void svtkChartXY::CalculateBarPlots()
{
  // Calculate the width, spacing and offsets for the bar plot - they are grouped
  size_t n = this->ChartPrivate->plots.size();
  std::vector<svtkPlotBar*> bars;
  for (size_t i = 0; i < n; ++i)
  {
    svtkPlotBar* bar = svtkPlotBar::SafeDownCast(this->ChartPrivate->plots[i]);
  ...
  if (!bars.empty())
  {
    ...
    float barWidth = 0.1;
    svtkPlotBar* bar = bars[0];
    if (!bar->GetUseIndexForXSeries())
    {
      svtkTable* table = bar->GetData()->GetInput();
      if (table)
      {
        svtkDataArray* x = bar->GetData()->GetInputArrayToProcess(0, table);
...
//-----------------------------------------------------------------------------
void svtkChartXY::RecalculatePlotTransforms()
{
  for (int i = 0; i < int(this->ChartPrivate->PlotCorners.size()); ++i)
  {
    if (this->ChartPrivate->PlotCorners[i]->GetNumberOfItems())
    {
      svtkAxis* xAxis = nullptr;
      svtkAxis* yAxis = nullptr;
      // Get the appropriate axes, and recalculate the transform.
      switch (i)
      {
        case 0:
        {
          xAxis = this->ChartPrivate->axes[svtkAxis::BOTTOM];
          yAxis = this->ChartPrivate->axes[svtkAxis::LEFT];
          break;
        }
        case 1:
          xAxis = this->ChartPrivate->axes[svtkAxis::BOTTOM];
          yAxis = this->ChartPrivate->axes[svtkAxis::RIGHT];
          break;
        case 2:
          xAxis = this->ChartPrivate->axes[svtkAxis::TOP];
          yAxis = this->ChartPrivate->axes[svtkAxis::RIGHT];
          break;
        case 3:
          xAxis = this->ChartPrivate->axes[svtkAxis::TOP];
          ...
          break;
        default:
          svtkWarningMacro("Error: default case in recalculate plot transforms.");
      }
      ...
      for (svtkIdType j = 0; j < this->ChartPrivate->PlotCorners[i]->GetNumberOfItems(); ++j)
      {
        svtkPlot* plot = svtkPlot::SafeDownCast(this->ChartPrivate->PlotCorners[i]->GetItem(j));
        if (plot)
        {
...
//-----------------------------------------------------------------------------
int svtkChartXY::GetPlotCorner(svtkPlot* plot)
{
  svtkAxis* x = plot->GetXAxis();
  svtkAxis* y = plot->GetYAxis();
  if (x == this->ChartPrivate->axes[svtkAxis::BOTTOM] &&
    y == this->ChartPrivate->axes[svtkAxis::LEFT])
  {
    return 0;
  }
  else if (x == this->ChartPrivate->axes[svtkAxis::BOTTOM] &&
    y == this->ChartPrivate->axes[svtkAxis::RIGHT])
  {
    return 1;
  }
  else if (x == this->ChartPrivate->axes[svtkAxis::TOP] &&
    y == this->ChartPrivate->axes[svtkAxis::RIGHT])
  {
    return 2;
  }
...
//-----------------------------------------------------------------------------
void svtkChartXY::SetPlotCorner(svtkPlot* plot, int corner)
{
  if (corner < 0 || corner > 3)
  {
    svtkWarningMacro("Invalid corner specified, should be between 0 and 3: " << corner);
    return;
  }
  ...
  if (corner == 0)
  {
    plot->SetXAxis(this->ChartPrivate->axes[svtkAxis::BOTTOM]);
    plot->SetYAxis(this->ChartPrivate->axes[svtkAxis::LEFT]);
  }
  else if (corner == 1)
  {
    plot->SetXAxis(this->ChartPrivate->axes[svtkAxis::BOTTOM]);
    plot->SetYAxis(this->ChartPrivate->axes[svtkAxis::RIGHT]);
  }
  else if (corner == 2)
  {
    plot->SetXAxis(this->ChartPrivate->axes[svtkAxis::TOP]);
    plot->SetYAxis(this->ChartPrivate->axes[svtkAxis::RIGHT]);
  }
  else if (corner == 3)
  {
...
//-----------------------------------------------------------------------------
void svtkChartXY::RecalculatePlotBounds()
{
  ...
  std::vector<svtkPlot*>::iterator it;
  ...
  for (int i = 0; i < 4; ++i)
  {
    svtkAxis* axis = this->ChartPrivate->axes[i];
    ...
    if (this->AdjustLowerBoundForLogPlot && axis->GetLogScale() &&
      (range[0] <= 0.0 || svtkMath::IsNan(range[0])))
    {
      if (range[1] <= 0.0 || svtkMath::IsNan(range[1]))
      {
    ...
    if (axis->GetBehavior() == svtkAxis::AUTO && initialized[i])
    {
...
//-----------------------------------------------------------------------------
void svtkChartXY::ReleasePlotSelections()
{
  std::vector<svtkPlot*>::iterator it = this->ChartPrivate->plots.begin();
  for (; it != this->ChartPrivate->plots.end(); ++it)
  {
    ...
    svtkNew<svtkIdTypeArray> emptySelectionArray;
...
//-----------------------------------------------------------------------------
bool svtkChartXY::UpdateLayout(svtkContext2D* painter)
{
  ...
  svtkVector2i hiddenAxisBorder = tileScale * this->HiddenAxisBorder;

  // Axes
  if (this->LayoutStrategy == svtkChart::FILL_SCENE || this->LayoutStrategy == svtkChart::FILL_RECT)
  {
    for (int i = 0; i < 4; ++i)
    {
      int border = 0;
      ...
      if (axis->GetVisible())
      {
        ...
        if (i == svtkAxis::TOP || i == svtkAxis::BOTTOM)
        { // Horizontal axes
  ...
  if (this->DrawAxesAtOrigin)
  {
    ...
    svtkVector2f origin(0.0);
    svtkTransform2D* transform = this->ChartPrivate->PlotCorners[0]->GetTransform();
    ...
    this->ChartPrivate->axes[svtkAxis::BOTTOM]->SetPoint1(this->Point1[0], origin[1]);
    this->ChartPrivate->axes[svtkAxis::BOTTOM]->SetPoint2(this->Point2[0], origin[1]);
    this->ChartPrivate->axes[svtkAxis::LEFT]->SetPoint1(origin[0], this->Point1[1]);
    this->ChartPrivate->axes[svtkAxis::LEFT]->SetPoint2(origin[0], this->Point2[1]);
  }
  else
  {
    if (this->LayoutStrategy == svtkChart::AXES_TO_RECT)
    {
...
//-----------------------------------------------------------------------------
int svtkChartXY::GetLegendBorder(svtkContext2D* painter, int axisPosition)
{
  ...
  int padding = 10;
  svtkVector2i legendSize(0, 0);
  svtkVector2i legendAlignment(
  ...
  if (axisPosition == svtkAxis::LEFT && legendAlignment.GetX() == svtkChartLegend::LEFT)
  {
    return legendSize.GetX() + padding * tileScale.GetX();
  }
  else if (axisPosition == svtkAxis::RIGHT && legendAlignment.GetX() == svtkChartLegend::RIGHT)
  {
    return legendSize.GetX() + padding * tileScale.GetX();
  }
  else if ((axisPosition == svtkAxis::TOP || axisPosition == svtkAxis::BOTTOM) &&
    (legendAlignment.GetX() == svtkChartLegend::LEFT ||
      legendAlignment.GetX() == svtkChartLegend::RIGHT))
  {
    return 0;
  }
  else if (axisPosition == svtkAxis::TOP && legendAlignment.GetY() == svtkChartLegend::TOP)
  {
    return legendSize.GetY() + padding * tileScale.GetY();
  }
  else if (axisPosition == svtkAxis::BOTTOM && legendAlignment.GetY() == svtkChartLegend::BOTTOM)
  {
    return legendSize.GetY() + padding * tileScale.GetY();
  }
  else
...
//-----------------------------------------------------------------------------
void svtkChartXY::SetLegendPosition(const svtkRectf& rect)
{
  // Put the legend in the top corner of the chart
  svtkVector2f pos(0, 0);
  int padding = 5;
  svtkVector2i legendAlignment(
    this->Legend->GetHorizontalAlignment(), this->Legend->GetVerticalAlignment());

  if (legendAlignment[0] == svtkChartLegend::CUSTOM || legendAlignment[1] == svtkChartLegend::CUSTOM)
  {
    return;
  }
  ...
  {
    // Non-inline legends.
    if (legendAlignment.GetX() == svtkChartLegend::LEFT)
    {
      pos.SetX(this->Point1[0] - this->ChartPrivate->Borders[svtkAxis::LEFT] + padding);
    }
    else if (legendAlignment.GetX() == svtkChartLegend::RIGHT)
    {
      pos.SetX(
        this->Point2[0] + this->ChartPrivate->Borders[svtkAxis::RIGHT] - rect.GetWidth() - padding);
    }
    else if (legendAlignment.GetX() == svtkChartLegend::CENTER)
    {
      pos.SetX(
        ((this->Point2[0] - this->Point1[0]) / 2.0) - (rect.GetWidth() / 2.0) + this->Point1[0]);
      // Check for the special case where the legend is on the top or bottom
      if (legendAlignment.GetY() == svtkChartLegend::TOP)
      {
        pos.SetY(
          this->Point2[1] + this->ChartPrivate->Borders[svtkAxis::TOP] - rect.GetHeight() - padding);
      }
      else if (legendAlignment.GetY() == svtkChartLegend::BOTTOM)
      {
        pos.SetY(this->Point1[1] - this->ChartPrivate->Borders[svtkAxis::BOTTOM] + padding);
      }
    }
    // Vertical alignment
    if (legendAlignment.GetX() != svtkChartLegend::CENTER)
    {
      if (legendAlignment.GetY() == svtkChartLegend::TOP)
      {
        pos.SetY(this->Point2[1] - rect.GetHeight());
    ...
    if (legendAlignment.GetY() == svtkChartLegend::CENTER)
    {
      pos.SetY(
        ((this->Point2[1] - this->Point1[1]) / 2.0) - (rect.GetHeight() / 2.0) + this->Point1[1]);
    }
...
//-----------------------------------------------------------------------------
svtkPlot* svtkChartXY::AddPlot(int type)
{
  ...
  svtkColor3ub color = this->ChartPrivate->Colors->GetColorRepeating(
    static_cast<int>(this->ChartPrivate->plots.size()));
  switch (type)
  {
    case LINE:
    {
      svtkPlotLine* line = svtkPlotLine::New();
    ...
    case POINTS:
    {
      svtkPlotPoints* points = svtkPlotPoints::New();
    ...
    case BAR:
    {
      svtkPlotBar* bar = svtkPlotBar::New();
    ...
    case FUNCTIONALBAG:
    {
      svtkPlotFunctionalBag* bag = svtkPlotFunctionalBag::New();
    ...
    case STACKED:
    {
      svtkPlotStacked* stacked = svtkPlotStacked::New();
    ...
    case BAG:
    {
      svtkPlotBag* bag = svtkPlotBag::New();
      bag->SetParent(this);
      bag->GetBrush()->SetColor(color.GetData());
      plot = bag;
      break;
    ...
    case AREA:
    {
      svtkPlotArea* area = svtkPlotArea::New();
      area->SetParent(this);
...
//-----------------------------------------------------------------------------
svtkIdType svtkChartXY::AddPlot(svtkPlot* plot)
{
  if (plot == nullptr)
  {
    return -1;
  ...
  svtkIdType plotIndex = static_cast<svtkIdType>(this->ChartPrivate->plots.size() - 1);
...
//-----------------------------------------------------------------------------
bool svtkChartXY::RemovePlot(svtkIdType index)
{
  if (index < static_cast<svtkIdType>(this->ChartPrivate->plots.size()))
  {
...
//-----------------------------------------------------------------------------
svtkPlot* svtkChartXY::GetPlot(svtkIdType index)
{
  if (static_cast<svtkIdType>(this->ChartPrivate->plots.size()) > index)
  {
...
//-----------------------------------------------------------------------------
svtkIdType svtkChartXY::GetPlotIndex(svtkPlot* plot)
{
  int corner = this->GetPlotCorner(plot);
  return corner >= 0 && corner < 4 ? this->ChartPrivate->PlotCorners[corner]->GetItemIndex(plot)
                                   : static_cast<svtkIdType>(-1);
}

//-----------------------------------------------------------------------------
svtkIdType svtkChartXY::RaisePlot(svtkPlot* plot)
{
  svtkIdType plotIndex = this->GetPlotIndex(plot);
  int corner = this->GetPlotCorner(plot);
...
//-----------------------------------------------------------------------------
svtkIdType svtkChartXY::StackPlotAbove(svtkPlot* plot, svtkPlot* under)
{
  svtkIdType plotIndex = this->GetPlotIndex(plot);
  svtkIdType underIndex = this->GetPlotIndex(under);
  int corner = this->GetPlotCorner(plot);
...
//-----------------------------------------------------------------------------
svtkIdType svtkChartXY::LowerPlot(svtkPlot* plot)
{
  svtkIdType plotIndex = this->GetPlotIndex(plot);
  int corner = this->GetPlotCorner(plot);
  if (corner < 0 || corner >= 4)
...
//-----------------------------------------------------------------------------
svtkIdType svtkChartXY::StackPlotUnder(svtkPlot* plot, svtkPlot* above)
{
  svtkIdType plotIndex = this->GetPlotIndex(plot);
  svtkIdType aboveIndex = this->GetPlotIndex(above);
  int corner = this->GetPlotCorner(plot);
...
//-----------------------------------------------------------------------------
void svtkChartXY::SetShowLegend(bool visible)
{
...
//-----------------------------------------------------------------------------
svtkChartLegend* svtkChartXY::GetLegend()
{
  return this->Legend;
}

//-----------------------------------------------------------------------------
void svtkChartXY::SetTooltip(svtkTooltipItem* tooltip)
{
...
//-----------------------------------------------------------------------------
svtkTooltipItem* svtkChartXY::GetTooltip()
{
  return this->Tooltip;
}

//-----------------------------------------------------------------------------
svtkIdType svtkChartXY::GetNumberOfPlots()
{
  return static_cast<svtkIdType>(this->ChartPrivate->plots.size());
}

//-----------------------------------------------------------------------------
svtkAxis* svtkChartXY::GetAxis(int axisIndex)
{
  if (axisIndex < 4)
  {
...
//-----------------------------------------------------------------------------
void svtkChartXY::SetAxis(int axisIndex, svtkAxis* axis)
{
  if ((axisIndex < 4) && (axisIndex >= 0))
  {
    svtkAxis* old_axis = this->ChartPrivate->axes[axisIndex];
    ...
    svtkPlotGrid* grid1 = static_cast<svtkPlotGrid*>(this->ChartPrivate->Clip->GetItem(0));
    svtkPlotGrid* grid2 = static_cast<svtkPlotGrid*>(this->ChartPrivate->Clip->GetItem(1));
    switch (axisIndex)
    {
...
//-----------------------------------------------------------------------------
svtkIdType svtkChartXY::GetNumberOfAxes()
{
  return 4;
}

//-----------------------------------------------------------------------------
void svtkChartXY::RecalculateBounds()
{
  // Ensure that the bounds are recalculated
  this->PlotTransformValid = false;
  if (this->Scene)
...
//-----------------------------------------------------------------------------
void svtkChartXY::SetSelectionMethod(int method)
{
  if (method == this->SelectionMethod)
  {
    return;
  }
  if (method == svtkChart::SELECTION_PLOTS)
  {
...
//-----------------------------------------------------------------------------
void svtkChartXY::RemovePlotSelections()
{
  std::vector<svtkPlot*>::iterator it = this->ChartPrivate->plots.begin();
  for (; it != this->ChartPrivate->plots.end(); ++it)
  {
...
//-----------------------------------------------------------------------------
bool svtkChartXY::Hit(const svtkContextMouseEvent& mouse)
{
  if (!this->Interactive)
  {
    return false;
...
//-----------------------------------------------------------------------------
bool svtkChartXY::MouseEnterEvent(const svtkContextMouseEvent&)
{
...
//-----------------------------------------------------------------------------
bool svtkChartXY::MouseMoveEvent(const svtkContextMouseEvent& mouse)
{
  ...
  if (mouse.GetButton() == this->Actions.Pan())
  {
    ...
    svtkAxis* xAxis = this->ChartPrivate->axes[svtkAxis::BOTTOM];
    svtkAxis* yAxis = this->ChartPrivate->axes[svtkAxis::LEFT];
    svtkTransform2D* transform = this->ChartPrivate->PlotCorners[0]->GetTransform();
    ...
    if (this->ChartPrivate->PlotCorners.size() == 2)
    {
      ...
      screenPos = svtkVector2d(mouse.GetScreenPos().Cast<double>().GetData());
      lastScreenPos = svtkVector2d(mouse.GetLastScreenPos().Cast<double>().GetData());
      pos = svtkVector2d(0.0, 0.0);
      last = svtkVector2d(0.0, 0.0);
      yAxis = this->ChartPrivate->axes[svtkAxis::RIGHT];
  ...
  else if (mouse.GetButton() == this->Actions.ZoomAxis())
  {
    ...
    svtkAxis* axes[] = { this->ChartPrivate->axes[svtkAxis::BOTTOM],
      this->ChartPrivate->axes[svtkAxis::LEFT], this->ChartPrivate->axes[svtkAxis::TOP],
      this->ChartPrivate->axes[svtkAxis::RIGHT] };

    for (int i = 0; i < 4; i++)
    {
      svtkAxis* axis = axes[i];
      if (!axis)
      {
        continue;
      }
  ...
  {
    // Iterate through each corner, and check for a nearby point
    std::vector<svtkContextTransform*>::iterator it = this->ChartPrivate->PlotCorners.begin();
    for (; it != this->ChartPrivate->PlotCorners.end(); ++it)
    {
      svtkContextTransform* plotCorner = *it;
      if (!plotCorner)
      {
        continue;
      }
      ...
      svtkTransform2D* transform = plotCorner->GetTransform();
      transform->InverseTransformPoints(mouse.GetPos().GetData(), position.GetData(), 1);
      for (int j = 0; j < items; ++j)
      {
        svtkPlot* plot = svtkPlot::SafeDownCast(plotCorner->GetItem(j));
        if (!plot || plot->IsA("svtkPlotBar"))
        {
          continue;
        }
        svtkIdTypeArray* selectionArray = plot->GetSelection();
        ...
        {
          svtkDebugMacro("Move event (Click and Drag) found more than one point to update.");
        }
        svtkIdType index = selectionArray->GetValue(0);
        if (this->DragPointAlongX)
        {
          svtkDataArray* xArray = plot->GetData()->GetInputArrayToProcess(0, plot->GetInput());
          xArray->SetVariantValue(index, position.GetX());
        }
        if (this->DragPointAlongY)
        {
          svtkDataArray* yArray = plot->GetData()->GetInputArrayToProcess(1, plot->GetInput());
...
//-----------------------------------------------------------------------------
int svtkChartXY::LocatePointInPlot(const svtkVector2f& position, const svtkVector2f& tolerance,
...
//-----------------------------------------------------------------------------
bool svtkChartXY::LocatePointInPlots(const svtkContextMouseEvent& mouse, int invokeEvent)
{
  size_t n = this->ChartPrivate->plots.size();
  svtkVector2i pos(mouse.GetScreenPos());
  if (pos[0] > this->Point1[0] && pos[0] < this->Point2[0] && pos[1] > this->Point1[1] &&
    pos[1] < this->Point2[1] && n)
  {
    ...
    for (size_t i = 0; i < this->ChartPrivate->PlotCorners.size(); ++i)
    {
      ...
      if (items)
      {
        ...
        svtkTransform2D* transform = this->ChartPrivate->PlotCorners[i]->GetTransform();
        transform->InverseTransformPoints(mouse.GetPos().GetData(), position.GetData(), 1);
        // Use a tolerance of +/- 5 pixels
        svtkVector2f tolerance(std::fabs(5 * (1.0 / transform->GetMatrix()->GetElement(0, 0))),
        ...
        for (int j = items - 1; j >= 0; --j)
        {
          ...
          if (seriesIndex >= 0)
          {
            ...
            if (invokeEvent >= 0)
            {
              ...
              if (invokeEvent == svtkCommand::SelectionChangedEvent)
              {
                // Construct a new selection with the selected point in it.
                svtkNew<svtkIdTypeArray> selectionIds;
                selectionIds->InsertNextValue(seriesIndex);
                plot->SetSelection(selectionIds);

                if (this->AnnotationLink)
                {
                  svtkChartXY::MakeSelection(this->AnnotationLink, selectionIds, plot);
                }
              }
            }
            return true;
...
//-----------------------------------------------------------------------------
void svtkChartXY::SetTooltipInfo(const svtkContextMouseEvent& mouse, const svtkVector2d& plotPos,
...
//-----------------------------------------------------------------------------
bool svtkChartXY::MouseLeaveEvent(const svtkContextMouseEvent&)
{
  this->DrawNearestPoint = false;
...
//-----------------------------------------------------------------------------
bool svtkChartXY::MouseButtonPressEvent(const svtkContextMouseEvent& mouse)
{
  if (this->Tooltip)
  {
    this->Tooltip->SetVisible(false);
  ...
  else if (mouse.GetButton() == this->Actions.ClickAndDrag())
  {
    ...
    this->DragPoint = this->LocatePointInPlots(mouse, svtkCommand::SelectionChangedEvent);
    this->InvokeEvent(svtkCommand::SelectionChangedEvent);
    return true;
  }
...
//-----------------------------------------------------------------------------
bool svtkChartXY::MouseButtonReleaseEvent(const svtkContextMouseEvent& mouse)
{
  // Iterate through each corner, and check for a nearby point
  for (size_t i = 0; i < this->ChartPrivate->PlotCorners.size(); ++i)
  {
  ...
  if (isActionSelectInvalid || isActionSelectPolygonInvalid || isActionPan)
  {
    ...
    {
      this->LocatePointInPlots(mouse, svtkCommand::InteractionEvent);
    }
    if (mouse.GetButton() == this->ActionsClick.Select())
    {
      this->LocatePointInPlots(mouse, svtkCommand::SelectionChangedEvent);
      this->InvokeEvent(svtkCommand::SelectionChangedEvent);
    }
    if (mouse.GetButton() != this->ActionsClick.Notify() &&
    ...
    mouse.GetButton() == this->Actions.SelectPolygon())
  {
    ...
    int selectionMode = svtkChartXY::GetMouseSelectionMode(mouse, this->SelectionMode);
    ...
    // plot.
    svtkNew<svtkIdTypeArray> oldSelection;
    svtkNew<svtkIdTypeArray> accumulateSelection;
    if (this->SelectionMethod == svtkChart::SELECTION_ROWS)
    {
        ...
        if (items)
        {
          ...
          for (int j = 0; j < items; ++j)
          {
            ...
            if (plot && plot->GetVisible() && plot->GetSelectable())
            {
              ...
              svtkChartXY::BuildSelection(nullptr, svtkContextScene::SELECTION_ADDITION,
    ...
    else if (this->SelectionMethod == svtkChart::SELECTION_COLUMNS)
    {
      if (this->AnnotationLink)
      {
        ...
        svtkSelection* selection =
          svtkSelection::SafeDownCast(this->AnnotationLink->GetOutputDataObject(2));
        svtkSelectionNode* node =
          selection->GetNumberOfNodes() > 0 ? selection->GetNode(0) : nullptr;
        if (node)
        {
          oldSelection->DeepCopy(svtkArrayDownCast<svtkIdTypeArray>(node->GetSelectionList()));
        }
      }
      svtkNew<svtkIdTypeArray> plotSelection;
      // We are performing plot based selections.
      for (size_t i = 0; i < this->ChartPrivate->PlotCorners.size(); ++i)
      {
        int items = static_cast<int>(this->ChartPrivate->PlotCorners[i]->GetNumberOfItems());
            ...
            if (plot && plot->GetVisible() && plot->GetSelectable())
            {
              ...
              if (selected)
              {
                ...
                if (plotSelection->GetNumberOfTuples() != column->GetNumberOfTuples())
                {
                  ...
                  for (svtkIdType k = 0; k < column->GetNumberOfTuples(); ++k)
                  {
      ...
      svtkIdType* ptrSelection =
        reinterpret_cast<svtkIdType*>(accumulateSelection->GetVoidPointer(0));
  ...
  else if (mouse.GetButton() == this->Actions.Zoom())
  {
    ...
    svtkVector2f point2(mouse.GetPos());

    this->ZoomInAxes(this->ChartPrivate->axes[svtkAxis::BOTTOM],
      this->ChartPrivate->axes[svtkAxis::LEFT], this->MouseBox.GetData(), point2.GetData());
    this->ZoomInAxes(this->ChartPrivate->axes[svtkAxis::TOP],
      this->ChartPrivate->axes[svtkAxis::RIGHT], this->MouseBox.GetData(), point2.GetData());
...
void svtkChartXY::ZoomInAxes(svtkAxis* x, svtkAxis* y, float* originf, float* maxf)
{
  svtkNew<svtkTransform2D> transform;
  this->CalculateUnscaledPlotTransform(x, y, transform);
  svtkVector2d origin(originf[0], originf[1]);
  svtkVector2d max(maxf[0], maxf[1]);
  svtkVector2d torigin;
...
//-----------------------------------------------------------------------------
bool svtkChartXY::MouseWheelEvent(const svtkContextMouseEvent&, int delta)
{
  if (this->Tooltip)
  {
    this->Tooltip->SetVisible(false);
...
//-----------------------------------------------------------------------------
bool svtkChartXY::KeyPressEvent(const svtkContextKeyEvent& key)
{
...
//-----------------------------------------------------------------------------
bool svtkChartXY::RemovePlotFromCorners(svtkPlot* plot)
{
...
//-----------------------------------------------------------------------------
inline void svtkChartXY::TransformBoxOrPolygon(bool polygonMode, svtkTransform2D* transform,
  const svtkVector2f& mousePosition, svtkVector2f& min, svtkVector2f& max, svtkContextPolygon& polygon)
{
  if (polygonMode)
  {
    svtkNew<svtkTransform2D> inverseTransform;
...
//-----------------------------------------------------------------------------
void svtkChartXY::PrintSelf(ostream& os, svtkIndent indent)
{
...
//-----------------------------------------------------------------------------
void svtkChartXY::MakeSelection(svtkAnnotationLink* link, svtkIdTypeArray* selectionIds, svtkPlot* plot)
{
  ...
  if (plot)
  {
    ...
    svtkSelection* selection = link->GetCurrentSelection();
    svtkSmartPointer<svtkSelectionNode> node;
    for (unsigned int i = 0; i < selection->GetNumberOfNodes(); ++i)
    {
      svtkSelectionNode* tmp = selection->GetNode(i);
      svtkPlot* selectionPlot =
        svtkPlot::SafeDownCast(tmp->GetProperties()->Get(svtkSelectionNode::PROP()));
    ...
    if (!node)
    {
      node = svtkSmartPointer<svtkSelectionNode>::New();
      selection->AddNode(node.GetPointer());
      node->SetContentType(svtkSelectionNode::INDICES);
      node->SetFieldType(svtkSelectionNode::POINT);
      node->GetProperties()->Set(svtkSelectionNode::PROP(), plot);
      node->GetProperties()->Set(svtkSelectionNode::SOURCE(), plot->GetInput());
    }
...
//-----------------------------------------------------------------------------
void svtkChartXY::MinusSelection(svtkIdTypeArray* selection, svtkIdTypeArray* oldSelection)
{
  // We rely on the selection id arrays being sorted.
  std::vector<svtkIdType> output;
  svtkIdType* ptrSelection = static_cast<svtkIdType*>(selection->GetVoidPointer(0));
  svtkIdType* ptrOldSelection = static_cast<svtkIdType*>(oldSelection->GetVoidPointer(0));
  svtkIdType oldSize = oldSelection->GetNumberOfTuples();
  svtkIdType size = selection->GetNumberOfTuples();
  svtkIdType iOld = 0;
  ...
  selection->SetNumberOfTuples(static_cast<svtkIdType>(output.size()));
  ptrSelection = static_cast<svtkIdType*>(selection->GetVoidPointer(0));
  for (std::vector<svtkIdType>::iterator it = output.begin(); it != output.end();
       ++it, ++ptrSelection)
  {
    *ptrSelection = *it;
  }
...
//-----------------------------------------------------------------------------
void svtkChartXY::AddSelection(svtkIdTypeArray* selection, svtkIdTypeArray* oldSelection)
{
  // Add all unique array indices to create a new combined array.
  svtkIdType* ptrSelection = static_cast<svtkIdType*>(selection->GetVoidPointer(0));
  svtkIdType* ptrOldSelection = static_cast<svtkIdType*>(oldSelection->GetVoidPointer(0));
  std::vector<svtkIdType> output(selection->GetNumberOfTuples() + oldSelection->GetNumberOfTuples());
  std::vector<svtkIdType>::iterator it;
  ...
  ptrSelection = static_cast<svtkIdType*>(selection->GetVoidPointer(0));
  for (std::vector<svtkIdType>::iterator i = output.begin(); i != it; ++i, ++ptrSelection)
  {
    *ptrSelection = *i;
  }
}
...
//-----------------------------------------------------------------------------
void svtkChartXY::ToggleSelection(svtkIdTypeArray* selection, svtkIdTypeArray* oldSelection)
{
  // We rely on the selection id arrays being sorted.
  std::vector<svtkIdType> output;
  svtkIdType* ptrSelection = static_cast<svtkIdType*>(selection->GetVoidPointer(0));
...
//-----------------------------------------------------------------------------
void svtkChartXY::BuildSelection(svtkAnnotationLink* link, int selectionMode,
  svtkIdTypeArray* plotSelection, svtkIdTypeArray* oldSelection, svtkPlot* plot)
{
  ...
  {
    case svtkContextScene::SELECTION_ADDITION:
      AddSelection(plotSelection, oldSelection);
      break;
    case svtkContextScene::SELECTION_SUBTRACTION:
      MinusSelection(plotSelection, oldSelection);
      break;
    case svtkContextScene::SELECTION_TOGGLE:
      ToggleSelection(plotSelection, oldSelection);
      break;
    case svtkContextScene::SELECTION_DEFAULT:
    default:
...
//-----------------------------------------------------------------------------
int svtkChartXY::GetMouseSelectionMode(const svtkContextMouseEvent& mouse, int selectionMode)
{
  // Mouse modifiers override the current selection mode.
  if (mouse.GetModifiers() & svtkContextMouseEvent::SHIFT_MODIFIER &&
    mouse.GetModifiers() & svtkContextMouseEvent::CONTROL_MODIFIER)
  {
    return svtkContextScene::SELECTION_TOGGLE;
  }
  else if (mouse.GetModifiers() & svtkContextMouseEvent::CONTROL_MODIFIER)
  {
    return svtkContextScene::SELECTION_ADDITION;
  }
  else if (mouse.GetModifiers() & svtkContextMouseEvent::SHIFT_MODIFIER)
  {
    return svtkContextScene::SELECTION_SUBTRACTION;
  }
