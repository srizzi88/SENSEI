...
svtkStandardNewMacro(svtkChartXYZ);

//-----------------------------------------------------------------------------
svtkChartXYZ::svtkChartXYZ()
  : Geometry(0, 0, 10, 10)
  , IsX(false)
  , Angle(0)
{
  ...
  {
    svtkNew<svtkAxis> axis;
    this->Axes[i] = axis;
  }
}

//-----------------------------------------------------------------------------
svtkChartXYZ::~svtkChartXYZ() = default;

//-----------------------------------------------------------------------------
void svtkChartXYZ::SetAngle(double angle)
{
  this->Angle = angle;
}

//-----------------------------------------------------------------------------
void svtkChartXYZ::SetAroundX(bool IsX_)
{
  this->IsX = IsX_;
}

//-----------------------------------------------------------------------------
void svtkChartXYZ::SetAutoRotate(bool b)
{
  this->AutoRotate = b;
}

//-----------------------------------------------------------------------------
void svtkChartXYZ::SetDecorateAxes(bool b)
{
  this->DrawAxesDecoration = b;
}

//-----------------------------------------------------------------------------
void svtkChartXYZ::SetAnnotationLink(svtkAnnotationLink* link)
{
  if (this->Link != link)
  {
    this->Link = link;
...
//-----------------------------------------------------------------------------
svtkAxis* svtkChartXYZ::GetAxis(int axis)
{
...
//-----------------------------------------------------------------------------
void svtkChartXYZ::SetAxis(int axisIndex, svtkAxis* axis)
{
...
//-----------------------------------------------------------------------------
void svtkChartXYZ::SetAxisColor(const svtkColor4ub& color)
{
...
//-----------------------------------------------------------------------------
svtkColor4ub svtkChartXYZ::GetAxisColor()
{
  return this->AxisPen->GetColorObject();
}

//-----------------------------------------------------------------------------
void svtkChartXYZ::SetGeometry(const svtkRectf& bounds)
{
  this->Geometry = bounds;

  this->Axes[0]->SetPoint1(svtkVector2f(this->Geometry.GetX(), this->Geometry.GetY()));
  this->Axes[0]->SetPoint2(
    svtkVector2f(this->Geometry.GetX() + this->Geometry.GetWidth(), this->Geometry.GetY()));

  this->Axes[1]->SetPoint1(svtkVector2f(this->Geometry.GetX(), this->Geometry.GetY()));
  this->Axes[1]->SetPoint2(
    svtkVector2f(this->Geometry.GetX(), this->Geometry.GetY() + this->Geometry.GetHeight()));

  // Z is faked, largely to get valid ranges and rounded numbers...
  this->Axes[2]->SetPoint1(svtkVector2f(this->Geometry.GetX(), 0));
  if (this->IsX)
  {
    this->Axes[2]->SetPoint2(svtkVector2f(this->Geometry.GetX(), this->Geometry.GetHeight()));
  }
  else
  {
    this->Axes[2]->SetPoint2(svtkVector2f(this->Geometry.GetX(), this->Geometry.GetWidth()));
  }
}

//-----------------------------------------------------------------------------
void svtkChartXYZ::RecalculateBounds()
{
  if (this->Plots.empty())
  {
    return;
  ...
  double bounds[] = { SVTK_DOUBLE_MAX, SVTK_DOUBLE_MIN, SVTK_DOUBLE_MAX, SVTK_DOUBLE_MIN,
    SVTK_DOUBLE_MAX, SVTK_DOUBLE_MIN };

  // Need to calculate the bounds in three dimensions and set up the axes.
  for (unsigned int i = 0; i < this->Plots.size(); ++i)
  {
    std::vector<svtkVector3f> const& points = this->Plots[i]->GetPoints();
    for (unsigned int j = 0; j < points.size(); ++j)
    {
      const svtkVector3f& v = points[j];
      for (int k = 0; k < 3; ++k)
      {
        if (v[k] < bounds[2 * k])
        {
...
//-----------------------------------------------------------------------------
void svtkChartXYZ::PrintSelf(ostream& os, svtkIndent indent)
{
  Superclass::PrintSelf(os, indent);
}

//-----------------------------------------------------------------------------
void svtkChartXYZ::Update()
{
  if (this->Link)
  {
    svtkSelection* selection = svtkSelection::SafeDownCast(this->Link->GetOutputDataObject(2));
...
//-----------------------------------------------------------------------------
bool svtkChartXYZ::Paint(svtkContext2D* painter)
{
  if (!this->Visible)
    return false;
...
//-----------------------------------------------------------------------------
void svtkChartXYZ::DrawAxes(svtkContext3D* context)
{
  context->PushMatrix();
  ...
  svtkVector3f box[4];
  box[0] = svtkVector3f(0, 0, 0);
  box[1] = svtkVector3f(0, 1, 0);
  box[2] = svtkVector3f(1, 1, 0);
  box[3] = svtkVector3f(1, 0, 0);
  ...
  context->DrawLine(box[2], box[3]);
  context->DrawLine(box[3], box[0]);
  context->DrawLine(svtkVector3f(0, 0, 0), svtkVector3f(0, 0, 1));
  context->DrawLine(svtkVector3f(1, 0, 0), svtkVector3f(1, 0, 1));
  context->DrawLine(svtkVector3f(0, 1, 0), svtkVector3f(0, 1, 1));
  context->DrawLine(svtkVector3f(1, 1, 0), svtkVector3f(1, 1, 1));
}

//-----------------------------------------------------------------------------
void svtkChartXYZ::ComputeDataBounds()
{
...
//-----------------------------------------------------------------------------
void svtkChartXYZ::DrawAxesLabels(svtkContext2D* painter)
{
  svtkContext3D* context = painter->GetContext3D();

  // set up text property
  svtkNew<svtkTextProperty> textProperties;
...
//-----------------------------------------------------------------------------
void svtkChartXYZ::GetOffsetForAxisLabel(int axis, float* bounds, float* offset)
{
  offset[0] = 0;
  offset[1] = 0;
...
//-----------------------------------------------------------------------------
void svtkChartXYZ::DrawTickMarks(svtkContext2D* painter)
{
  svtkContext3D* context = painter->GetContext3D();
  float bounds[4];
  ...
  for (int axis = 0; axis < 3; ++axis)
  {
    ...
    std::vector<svtkVector3f> tickPoints;
    int currentTick = 0;
...
//-----------------------------------------------------------------------------
void svtkChartXYZ::DetermineWhichAxesToLabel()
{
...
//-----------------------------------------------------------------------------
bool svtkChartXYZ::Hit(const svtkContextMouseEvent& svtkNotUsed(mouse))
{
...
//-----------------------------------------------------------------------------
bool svtkChartXYZ::MouseButtonPressEvent(const svtkContextMouseEvent& mouse)
{
...
//-----------------------------------------------------------------------------
bool svtkChartXYZ::MouseMoveEvent(const svtkContextMouseEvent& mouse)
{
  if (mouse.GetButton() == svtkContextMouseEvent::LEFT_BUTTON)
  {
...
//-----------------------------------------------------------------------------
bool svtkChartXYZ::MouseWheelEvent(const svtkContextMouseEvent&, int delta)
{
...
//-----------------------------------------------------------------------------
void svtkChartXYZ::ZoomAxes(int delta)
{
  float scaling = pow(2.0f, delta / 10.0f);
...
//-----------------------------------------------------------------------------
bool svtkChartXYZ::Rotate(const svtkContextMouseEvent& mouse)
{
...
//-----------------------------------------------------------------------------
bool svtkChartXYZ::Pan(const svtkContextMouseEvent& mouse)
{
  // Figure out how much the mouse has moved in plot coordinates
  svtkVector2d screenPos(mouse.GetScreenPos().Cast<double>().GetData());
  svtkVector2d lastScreenPos(mouse.GetLastScreenPos().Cast<double>().GetData());
...
//-----------------------------------------------------------------------------
bool svtkChartXYZ::Zoom(const svtkContextMouseEvent& mouse)
{
...
//-----------------------------------------------------------------------------
bool svtkChartXYZ::Spin(const svtkContextMouseEvent& mouse)
{
  // Figure out how much the mouse has moved in plot coordinates
  svtkVector2d screenPos(mouse.GetScreenPos().Cast<double>().GetData());
  svtkVector2d lastScreenPos(mouse.GetLastScreenPos().Cast<double>().GetData());

  double newAngle = svtkMath::DegreesFromRadians(atan2(screenPos[1], screenPos[0]));
  double oldAngle = svtkMath::DegreesFromRadians(atan2(lastScreenPos[1], lastScreenPos[0]));
...
//-----------------------------------------------------------------------------
bool svtkChartXYZ::KeyPressEvent(const svtkContextKeyEvent& key)
{
...
void svtkChartXYZ::CalculateTransforms()
{
  ...
  svtkVector3f mtranslation = -1.0 * translation;
  ...
  svtkVector3d cube[8];
  svtkVector3d transformedCube[8];

  cube[0] = svtkVector3d(0, 0, 0);
  cube[1] = svtkVector3d(0, 0, 1);
  cube[2] = svtkVector3d(0, 1, 0);
  cube[3] = svtkVector3d(0, 1, 1);
  cube[4] = svtkVector3d(1, 0, 0);
  cube[5] = svtkVector3d(1, 0, 1);
  cube[6] = svtkVector3d(1, 1, 0);
  cube[7] = svtkVector3d(1, 1, 1);
  ...
  svtkNew<svtkPlane> face1;
  svtkMath::Cross((transformedCube[2] - transformedCube[0]).GetData(),
    (transformedCube[1] - transformedCube[0]).GetData(), norm1);
  svtkMath::Normalize(norm1);
  face1->SetNormal(norm1);
  face1->SetOrigin(transformedCube[3].GetData());
  this->BoundingCube->AddItem(face1);

  svtkNew<svtkPlane> face2;
  svtkMath::Cross((transformedCube[5] - transformedCube[4]).GetData(),
    (transformedCube[6] - transformedCube[4]).GetData(), norm2);
  svtkMath::Normalize(norm2);
  ...
  svtkNew<svtkPlane> face3;
  svtkMath::Cross((transformedCube[1] - transformedCube[0]).GetData(),
    (transformedCube[4] - transformedCube[0]).GetData(), norm3);
  svtkMath::Normalize(norm3);
  face3->SetNormal(norm3);
  face3->SetOrigin(transformedCube[5].GetData());
  this->BoundingCube->AddItem(face3);

  svtkNew<svtkPlane> face4;
  svtkMath::Cross((transformedCube[6] - transformedCube[2]).GetData(),
    (transformedCube[3] - transformedCube[2]).GetData(), norm4);
  svtkMath::Normalize(norm4);
  ...
  svtkNew<svtkPlane> face5;
  svtkMath::Cross((transformedCube[4] - transformedCube[0]).GetData(),
    (transformedCube[2] - transformedCube[0]).GetData(), norm5);
  svtkMath::Normalize(norm5);
  face5->SetNormal(norm5);
  face5->SetOrigin(transformedCube[6].GetData());
  this->BoundingCube->AddItem(face5);

  svtkNew<svtkPlane> face6;
  svtkMath::Cross((transformedCube[3] - transformedCube[1]).GetData(),
    (transformedCube[5] - transformedCube[1]).GetData(), norm6);
  svtkMath::Normalize(norm6);
...
//-----------------------------------------------------------------------------
void svtkChartXYZ::ScaleDownAxes()
{
  float point[3];
  int sceneWidth = this->Scene->GetSceneWidth();
  int sceneHeight = this->Scene->GetSceneHeight();
...
//-----------------------------------------------------------------------------
void svtkChartXYZ::InitializeFutureBox()
{
  double scale[3] = { 300, 300, 300 };
  for (int i = 0; i < 3; ++i)
  {
...
//-----------------------------------------------------------------------------
bool svtkChartXYZ::CheckForSceneResize()
{
  ...
  if (this->SceneWidth != currentWidth || this->SceneHeight != currentHeight)
  {
    ...
    if (this->SceneWidth > 0)
    {
      ...
      svtkVector2f axisPt = this->Axes[0]->GetPosition1();
      axisPt[0] += dx;
      axisPt[1] += dy;
...
//-----------------------------------------------------------------------------
void svtkChartXYZ::InitializeAxesBoundaryPoints()
{
  int currentPoint = 0;
  for (int i = 0; i < 2; ++i)
  {
...
//-----------------------------------------------------------------------------
double svtkChartXYZ::CalculateNiceMinMax(double& min, double& max, int axis)
{
  ...
  return svtkAxis::NiceMinMax(min, max, pixelRange, 30.0f);
}

//-----------------------------------------------------------------------------
void svtkChartXYZ::RecalculateTransform()
{
  this->CalculatePlotTransform(this->Axes[0], this->Axes[1], this->Axes[2], this->PlotTransform);
}

//-----------------------------------------------------------------------------
bool svtkChartXYZ::CalculatePlotTransform(
  svtkAxis* x, svtkAxis* y, svtkAxis* z, svtkTransform* transform)
{
...
//-----------------------------------------------------------------------------
svtkIdType svtkChartXYZ::AddPlot(svtkPlot3D* plot)
{
  if (plot == nullptr)
  {
    return -1;
  ...
  svtkIdType plotIndex = static_cast<svtkIdType>(this->Plots.size() - 1);
...
//-----------------------------------------------------------------------------
void svtkChartXYZ::SetFitToScene(bool b)
{
  this->FitToScene = b;
}

//-----------------------------------------------------------------------------
void svtkChartXYZ::GetClippingPlaneEquation(int i, double* planeEquation)
{
  int n = this->BoundingCube->GetNumberOfItems();
  if (i >= 0 && i < n)
  {
    // Get the plane
    svtkPlane* plane = this->BoundingCube->GetItem(i);
