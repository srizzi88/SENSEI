/*=========================================================================
  ...
  Module:    svtkControlPointsItem.cxx
...
class svtkControlPointsAddPointItem : public svtkPlot
{
public:
  svtkTypeMacro(svtkControlPointsAddPointItem, svtkPlot);
  static svtkControlPointsAddPointItem* New();

  svtkControlPointsItem* ControlPointsItem;
...
protected:
  svtkControlPointsAddPointItem() = default;
  ~svtkControlPointsAddPointItem() override = default;
...
private:
  svtkControlPointsAddPointItem(const svtkControlPointsAddPointItem&) = delete;
  void operator=(const svtkControlPointsAddPointItem&) = delete;
};

svtkStandardNewMacro(svtkControlPointsAddPointItem);

//-----------------------------------------------------------------------------
svtkControlPointsItem::svtkControlPointsItem()
{
  ...
  this->SelectedPointPen = svtkPen::New();
  ...
  this->SelectedPointBrush = svtkBrush::New();
  ...
  this->Callback->SetCallback(svtkControlPointsItem::CallComputePoints);
...
//-----------------------------------------------------------------------------
void svtkControlPointsItem::PrintSelf(ostream& os, svtkIndent indent)
{
...
//-----------------------------------------------------------------------------
void svtkControlPointsItem::GetBounds(double bounds[4])
{
...
//-----------------------------------------------------------------------------
void svtkControlPointsItem::ResetBounds()
{
  this->Bounds[0] = 0.;
  this->Bounds[1] = -1.;
  this->Bounds[2] = 0.;
...
//-----------------------------------------------------------------------------
void svtkControlPointsItem::ComputeBounds()
{
  double oldBounds[4];
...
//-----------------------------------------------------------------------------
void svtkControlPointsItem::ComputeBounds(double* bounds)
{
  bounds[0] = bounds[2] = SVTK_DOUBLE_MAX;
  bounds[1] = bounds[3] = -SVTK_DOUBLE_MAX;
...
//-----------------------------------------------------------------------------
bool svtkControlPointsItem::Paint(svtkContext2D* painter)
{
...
//-----------------------------------------------------------------------------
void svtkControlPointsItem::StartChanges()
{
  ++this->StartedChanges;
  if (this->StartedChanges == 1)
  {
    this->InvokeEvent(svtkCommand::StartEvent);
    this->emitEvent(svtkCommand::StartEvent);
  }
}

//-----------------------------------------------------------------------------
void svtkControlPointsItem::EndChanges()
{
  --this->StartedChanges;
  assert(this->StartedChanges >= 0);
  if (this->StartedChanges == 0)
  {
    this->emitEvent(svtkCommand::EndEvent);
    this->InvokeEvent(svtkCommand::EndEvent);
  }
}

//-----------------------------------------------------------------------------
void svtkControlPointsItem::StartInteraction()
{
  ++this->StartedInteractions;
  this->emitEvent(svtkCommand::StartInteractionEvent);
}

//-----------------------------------------------------------------------------
void svtkControlPointsItem::StartInteractionIfNotStarted()
{
...
//-----------------------------------------------------------------------------
void svtkControlPointsItem::Interaction()
{
  assert(this->StartedInteractions > 0);
  this->emitEvent(svtkCommand::InteractionEvent);
}

//-----------------------------------------------------------------------------
void svtkControlPointsItem::EndInteraction()
{
  --this->StartedInteractions;
  assert(this->StartedInteractions >= 0);
  this->emitEvent(svtkCommand::EndInteractionEvent);
}

//-----------------------------------------------------------------------------
int svtkControlPointsItem::GetInteractionsCount() const
{
  return this->StartedInteractions;
}

//-----------------------------------------------------------------------------
// void svtkControlPointsItem::emitEvent(unsigned long event, void* params);

//-----------------------------------------------------------------------------
void svtkControlPointsItem::CallComputePoints(
  svtkObject* svtkNotUsed(sender), unsigned long event, void* receiver, void* svtkNotUsed(params))
{
  svtkControlPointsItem* item = reinterpret_cast<svtkControlPointsItem*>(receiver);
  switch (event)
  {
...
//-----------------------------------------------------------------------------
void svtkControlPointsItem::ComputePoints()
{
  ...
  {
    svtkIdTypeArray* oldSelection = this->Selection;
    this->Selection = svtkIdTypeArray::New();
    for (svtkIdType i = 0; i < selectedPointCount; ++i)
    {
...
//-----------------------------------------------------------------------------
bool svtkControlPointsItem::Hit(const svtkContextMouseEvent& mouse)
{
  svtkVector2f vpos = mouse.GetPos();
  double pos[2];
...
//-----------------------------------------------------------------------------
bool svtkControlPointsItem::ClampValidDataPos(double pos[2])
{
...
//-----------------------------------------------------------------------------
bool svtkControlPointsItem::ClampValidScreenPos(double pos[2])
{
...
//-----------------------------------------------------------------------------
void svtkControlPointsItem::DrawUnselectedPoints(svtkContext2D* painter)
{
  const int count = this->GetNumberOfPoints();
  for (svtkIdType i = 0; i < count; ++i)
  {
    svtkIdType idx = this->Selection ? this->Selection->LookupValue(i) : -1;
    if (idx != -1)
    {
      continue;
    }
...
//-----------------------------------------------------------------------------
void svtkControlPointsItem::DrawSelectedPoints(svtkContext2D* painter)
{
  const int count = this->Selection ? this->Selection->GetNumberOfTuples() : 0;
  for (svtkIdType i = 0; i < count; ++i)
  {
    svtkIdType index = this->Selection->GetValue(i);
    assert(index != -1);
...
//-----------------------------------------------------------------------------
void svtkControlPointsItem::DrawPoint(svtkContext2D* painter, svtkIdType index)
{
  assert(index != -1);
  double point[4];
  ...
  svtkSmartPointer<svtkTransform2D> translation = svtkSmartPointer<svtkTransform2D>::New();
      ...
      this->GetCurrentPoint() == index))
  {
    ...
    painter->SetTransform(translation);
    svtkStdString label = this->GetControlPointLabel(index);
    ...
    if (bounds[1].GetX() != 0.0f && bounds[1].GetY() != 0.0f)
    {
      ...
      bounds[0] = svtkVector2f(-5 / scale[0], -3 / scale[1]);
      bounds[1] = svtkVector2f(bounds[1].GetX() + 10 / scale[0], bounds[1].GetY() + 10 / scale[1]);
...
//-----------------------------------------------------------------------------
void svtkControlPointsItem::SelectPoint(double* currentPoint)
{
  svtkIdType pointId = this->FindPoint(currentPoint);
  if (pointId == -1)
  {
    svtkErrorMacro(<< "try to select a point that doesn't exist");
    return;
  }
...
//-----------------------------------------------------------------------------
void svtkControlPointsItem::SelectPoint(svtkIdType pointId)
{
...
//-----------------------------------------------------------------------------
void svtkControlPointsItem::SelectAllPoints()
{
...
//-----------------------------------------------------------------------------
void svtkControlPointsItem::DeselectPoint(double* point)
{
  // make sure the point belongs to the list of points
  svtkIdType pointId = this->FindPoint(point);
  if (pointId == -1)
  {
    svtkErrorMacro(<< "try to deselect a point that doesn't exist");
    return;
  }
...
//-----------------------------------------------------------------------------
void svtkControlPointsItem::DeselectPoint(svtkIdType pointId)
{
  svtkIdType selectionPointId = this->Selection ? this->Selection->LookupValue(pointId) : -1;
  if (selectionPointId == -1)
  {
    // svtkErrorMacro(<< "Point:" << pointId << " was not selected");
    return;
  }
...
//-----------------------------------------------------------------------------
void svtkControlPointsItem::DeselectAllPoints()
{
...
//-----------------------------------------------------------------------------
void svtkControlPointsItem::ToggleSelectPoint(double* currentPoint)
{
  // make sure the point belongs to the list of points
  svtkIdType pointId = this->FindPoint(currentPoint);
  if (pointId == -1)
  {
    svtkErrorMacro(<< "try to toggle a point that doesn't exist");
    return;
  }
...
//-----------------------------------------------------------------------------
void svtkControlPointsItem::ToggleSelectPoint(svtkIdType pointId)
{
  svtkIdType selectionId = this->Selection ? this->Selection->LookupValue(pointId) : -1;
  if (selectionId != -1)
  {
    this->DeselectPoint(pointId);
    return;
...
//-----------------------------------------------------------------------------
bool svtkControlPointsItem::SelectPoints(const svtkVector2f& min, const svtkVector2f& max)
{
...
//-----------------------------------------------------------------------------
svtkIdType svtkControlPointsItem::GetNumberOfSelectedPoints() const
{
  return this->Selection ? this->Selection->GetNumberOfTuples() : 0;
}

//-----------------------------------------------------------------------------
svtkIdType svtkControlPointsItem::GetCurrentPoint() const
{
  return this->CurrentPoint;
}
/*
//-----------------------------------------------------------------------------
svtkIdType svtkControlPointsItem::FindPoint(double* pos, double tolerance)
{
...
//-----------------------------------------------------------------------------
bool svtkControlPointsItem::IsOverPoint(double* pos, svtkIdType pointId)
{
...
//-----------------------------------------------------------------------------
svtkIdType svtkControlPointsItem::FindPoint(double* posData)
{
  double pos[2];
...
//-----------------------------------------------------------------------------
svtkIdType svtkControlPointsItem::GetControlPointId(double* point)
{
  const int numberOfPoints = this->GetNumberOfPoints();
  for (svtkIdType i = 0; i < numberOfPoints; ++i)
  {
...
//-----------------------------------------------------------------------------
void svtkControlPointsItem ::GetControlPointsIds(
  svtkIdTypeArray* points, bool excludeFirstAndLast) const
{
  ...
  svtkIdType pointId = excludeFirstAndLast ? 1 : 0;
  for (svtkIdType i = 0; i < numberOfPoints; ++i)
  {
...
//-----------------------------------------------------------------------------
void svtkControlPointsItem::AddPointId(svtkIdType addedPointId)
{
  assert(addedPointId != -1);
  // offset all the point ids
  const int selectionCount = this->GetNumberOfSelectedPoints();
  for (svtkIdType i = 0; i < selectionCount; ++i)
  {
    svtkIdType pointId = this->Selection->GetValue(i);
...
//-----------------------------------------------------------------------------
svtkIdType svtkControlPointsItem::RemovePoint(double* point)
{
  return this->RemovePointId(this->GetControlPointId(point));
}

//-----------------------------------------------------------------------------
svtkIdType svtkControlPointsItem::RemovePoint(svtkIdType pointId)
{
  double point[4];
  this->GetControlPoint(pointId, point);
...
//-----------------------------------------------------------------------------
svtkIdType svtkControlPointsItem::RemovePointId(svtkIdType pointId)
{
  ...
  const svtkIdType selectionCount = this->GetNumberOfSelectedPoints();
  for (svtkIdType i = 0; i < selectionCount; ++i)
  {
    svtkIdType selectedPointId = this->Selection->GetValue(i);
...
//-----------------------------------------------------------------------------
void svtkControlPointsItem::SetCurrentPoint(svtkIdType index)
{
  ...
  this->InvokeEvent(
    svtkControlPointsItem::CurrentPointChangedEvent, reinterpret_cast<void*>(this->CurrentPoint));
  this->GetScene()->SetDirty(true);
}

//-----------------------------------------------------------------------------
bool svtkControlPointsItem::MouseButtonPressEvent(const svtkContextMouseEvent& mouse)
{
  this->MouseMoved = false;
  this->PointToToggle = -1;
  this->PointToDelete = -1;
  ...
  svtkIdType pointUnderMouse = this->FindPoint(pos);
  ...
  if (mouse.GetButton() == svtkContextMouseEvent::LEFT_BUTTON)
  {
    ...
    else if (pointUnderMouse == -1 && this->GetNumberOfSelectedPoints() <= 1 && !this->StrokeMode)
    {
      ...
      svtkIdType addedPoint = this->AddPoint(pos);
  ...
  if (mouse.GetButton() == svtkContextMouseEvent::RIGHT_BUTTON && pointUnderMouse != -1)
  {
  ...
  if (mouse.GetButton() == svtkContextMouseEvent::MIDDLE_BUTTON && pointUnderMouse != -1)
  {
...
//-----------------------------------------------------------------------------
bool svtkControlPointsItem::MouseDoubleClickEvent(const svtkContextMouseEvent& mouse)
{
  if (mouse.GetButton() == svtkContextMouseEvent::RIGHT_BUTTON)
  {
  ...
  if (mouse.GetButton() == svtkContextMouseEvent::LEFT_BUTTON && this->CurrentPoint != -1)
  {
    this->InvokeEvent(
      svtkControlPointsItem::CurrentPointEditEvent, reinterpret_cast<void*>(this->CurrentPoint));
  }
  return res;
}

//-----------------------------------------------------------------------------
bool svtkControlPointsItem::MouseMoveEvent(const svtkContextMouseEvent& mouse)
{
  svtkVector2f mousePos = mouse.GetPos();
  ...
  if (mouse.GetButton() == svtkContextMouseEvent::LEFT_BUTTON)
  {
    ...
    {
      svtkVector2f deltaPos = mouse.GetPos() - mouse.GetLastPos();
      ...
      if (svtkIdTypeArray* points = this->GetSelection())
      {
  ...
  if (mouse.GetButton() == svtkContextMouseEvent::RIGHT_BUTTON)
  {
    ...
    pos[0] = mousePos[0];
    pos[1] = mousePos[1];
    svtkIdType pointUnderCursor = this->FindPoint(pos);
  ...
  if (mouse.GetButton() == svtkContextMouseEvent::MIDDLE_BUTTON)
  {
  ...
  if (mouse.GetButton() == svtkContextMouseEvent::RIGHT_BUTTON && this->CurrentPoint == -1)
  {
    return false;
  }
...
//-----------------------------------------------------------------------------
void svtkControlPointsItem::SetCurrentPointPos(const svtkVector2f& newPos)
{
  svtkIdType movedPoint = this->SetPointPos(this->CurrentPoint, newPos);
  // If the moved point was not CurrentPoint then make it current.
  this->SetCurrentPoint(movedPoint);
}

//-----------------------------------------------------------------------------
svtkIdType svtkControlPointsItem::SetPointPos(svtkIdType point, const svtkVector2f& newPos)
{
  if (point == -1)
  {
    return point;
...
//-----------------------------------------------------------------------------
void svtkControlPointsItem::MoveCurrentPoint(const svtkVector2f& translation)
{
  this->MovePoint(this->CurrentPoint, translation);
}

//-----------------------------------------------------------------------------
svtkIdType svtkControlPointsItem::MovePoint(svtkIdType pointId, const svtkVector2f& translation)
{
  double point[4];
  this->GetControlPoint(pointId, point);
  return this->SetPointPos(
    pointId, svtkVector2f(point[0] + translation.GetX(), point[1] + translation.GetY()));
}

//-----------------------------------------------------------------------------
void svtkControlPointsItem::MovePoints(const svtkVector2f& translation, svtkIdTypeArray* pointIds)
{
  assert(pointIds);
  this->StartChanges();
  // don't support 'switch' mode yet
  // svtkIdTypeArray* addedSelection = svtkIdTypeArray::New();
  ...
  int step = tX < 0.f ? 1 : -1;
  for (svtkIdType i = start; i != end; i += step)
  {
    svtkIdType pointId = pointIds->GetValue(i);
    double currentPoint[4] = { 0.0, 0.0, 0.0, 0.0 };
    this->GetControlPoint(pointId, currentPoint);
    svtkVector2f newPos(currentPoint[0] + tX, currentPoint[1] + tY);
    // svtkIdType newIdx =
...
//-----------------------------------------------------------------------------
void svtkControlPointsItem::MovePoints(const svtkVector2f& translation, bool dontMoveFirstAndLast)
{
  svtkNew<svtkIdTypeArray> points;
  this->GetControlPointsIds(points, dontMoveFirstAndLast);
  this->MovePoints(translation, points);
}

//-----------------------------------------------------------------------------
void svtkControlPointsItem::SpreadPoints(float factor, svtkIdTypeArray* pointIds)
{
  assert(pointIds);
  ...
  double point[4];
  svtkIdType minPointId = pointIds->GetValue(0);
  this->GetControlPoint(minPointId, point);
  min[0] = point[0];
  min[1] = point[1];
  svtkIdType maxPointId = pointIds->GetValue(pointIds->GetNumberOfTuples() - 1);
  ...
  // Left part
  svtkIdType start = 0;
  svtkIdType end = pointIds->GetNumberOfTuples();
  svtkIdType step = 1;
  svtkIdType median = -1; // not needed when factor >= 0
  if (factor < 0.f)
  {
    // search median
    for (svtkIdType i = 0; i < end; ++i)
    {
      svtkIdType pointId = pointIds->GetValue(i);
      this->GetControlPoint(pointId, point);
      if (point[0] > center[0])
  ...
  for (i = start; i != end; i += step)
  {
    ...
    svtkVector2f newPos(std::min(point[0] + tX, center[0]), point[1]);
    this->SetPointPos(pointId, newPos);
  }
  // Right part
  ...
  for (i = start; i != end; i += step)
  {
    ...
    svtkVector2f newPos(std::max(point[0] + tX, center[0]), point[1]);
    this->SetPointPos(pointId, newPos);
  }
  this->EndChanges();
}
...
//-----------------------------------------------------------------------------
void svtkControlPointsItem::SpreadPoints(float factor, bool dontSpreadFirstAndLast)
{
  svtkNew<svtkIdTypeArray> points;
...
//-----------------------------------------------------------------------------
svtkVector2f svtkControlPointsItem::GetCenterOfMass(svtkIdTypeArray* pointIDs) const
{
  double average[4] = { 0., 0., 0., 0. };
  const svtkIdType pointCount = pointIDs->GetNumberOfTuples();
  for (svtkIdType i = 0; i < pointCount; ++i)
  {
    double point[4];
  ...
  return svtkVector2f(average[0], average[1]);
}

//-----------------------------------------------------------------------------
void svtkControlPointsItem::Stroke(const svtkVector2f& newPos)
{
  double pos[2];
  pos[0] = newPos[0];
  pos[1] = newPos[1];
  ...
  {
    svtkIdType lastPointId = this->CurrentPoint;
...
//-----------------------------------------------------------------------------
void svtkControlPointsItem::EditPoint(float svtkNotUsed(tX), float svtkNotUsed(tY)) {}

//-----------------------------------------------------------------------------
bool svtkControlPointsItem::MouseButtonReleaseEvent(const svtkContextMouseEvent& mouse)
{
  ...
  if (mouse.GetButton() == svtkContextMouseEvent::RIGHT_BUTTON && this->PointToToggle != -1)
  {
  ...
  if (mouse.GetButton() == svtkContextMouseEvent::MIDDLE_BUTTON && this->PointToDelete != -1)
  {
...
//-----------------------------------------------------------------------------
bool svtkControlPointsItem::KeyPressEvent(const svtkContextKeyEvent& key)
{
  bool move = key.GetInteractor()->GetAltKey() != 0 ||
    key.GetInteractor()->GetKeySym() == svtkStdString("plus") ||
    key.GetInteractor()->GetKeySym() == svtkStdString("minus");
  ...
  if (current)
  {
    if (key.GetInteractor()->GetKeySym() == svtkStdString("Right") ||
      key.GetInteractor()->GetKeySym() == svtkStdString("Up"))
    {
      this->SetCurrentPoint(std::min(this->GetNumberOfPoints() - 1, this->GetCurrentPoint() + 1));
    }
    else if (key.GetInteractor()->GetKeySym() == svtkStdString("Left") ||
      key.GetInteractor()->GetKeySym() == svtkStdString("Down"))
    {
      this->SetCurrentPoint(std::max(0, static_cast<int>(this->GetCurrentPoint()) - 1));
    }
    else if (key.GetInteractor()->GetKeySym() == svtkStdString("End"))
    {
      this->SetCurrentPoint(this->GetNumberOfPoints() - 1);
    }
    else if (key.GetInteractor()->GetKeySym() == svtkStdString("Home"))
    {
  ...
  else if (select)
  {
    ...
    {
      svtkIdType newCurrentPointId = this->GetNumberOfPoints() - 1;
      for (svtkIdType pointId = this->CurrentPoint; pointId < newCurrentPointId; ++pointId)
      {
        this->SelectPoint(pointId);
      }
    ...
    {
      svtkIdType newCurrentPointId = 0;
      for (svtkIdType pointId = this->CurrentPoint; pointId > newCurrentPointId; --pointId)
      {
        this->SelectPoint(pointId);
      }
  ...
  {
    svtkVector2f translate(0, 0);
    if (key.GetInteractor()->GetKeySym() == std::string("Up"))
    {
      translate = translate + svtkVector2f(0., 1.);
    }
    ...
    if (key.GetInteractor()->GetKeySym() == std::string("Right"))
    {
      translate = translate + svtkVector2f(1., 0.);
    }
    ...
    else if (key.GetInteractor()->GetKeySym() == std::string("plus"))
    {
      ...
      if (svtkIdTypeArray* pointIds = this->GetSelection())
      {
  ...
  if (key.GetInteractor()->GetKeySym() == svtkStdString("space"))
  {
    this->ToggleSelectPoint(this->GetCurrentPoint());
  }
  else if (key.GetInteractor()->GetKeySym() == svtkStdString("Escape"))
  {
    this->DeselectAllPoints();
  }
...
//-----------------------------------------------------------------------------
bool svtkControlPointsItem::KeyReleaseEvent(const svtkContextKeyEvent& key)
{
  if (key.GetInteractor()->GetKeySym() == std::string("Delete") ||
    key.GetInteractor()->GetKeySym() == std::string("BackSpace"))
  {
    svtkIdType removedPoint = this->RemovePoint(this->GetCurrentPoint());
...
//-----------------------------------------------------------------------------
bool svtkControlPointsItem::GetEndPointsMovable()
{
  return (this->GetEndPointsXMovable() || this->GetEndPointsYMovable());
}

//-----------------------------------------------------------------------------
bool svtkControlPointsItem::IsEndPointPicked()
{
  ...
  svtkIdTypeArray* selection = this->GetSelection();
...
//-----------------------------------------------------------------------------
bool svtkControlPointsItem::IsPointRemovable(svtkIdType pointId)
{
...
//-----------------------------------------------------------------------------
svtkStdString svtkControlPointsItem::GetControlPointLabel(svtkIdType pointId)
{
  svtkStdString result;
  if (this->LabelFormat)
  {
...
//-----------------------------------------------------------------------------
svtkPlot* svtkControlPointsItem::GetAddPointItem()
{
