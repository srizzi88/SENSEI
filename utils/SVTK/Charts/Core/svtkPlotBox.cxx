...
class svtkPlotBox::Private : public std::vector<std::vector<double> >
{
public:
  Private() = default;
};
...
//-----------------------------------------------------------------------------
svtkStandardNewMacro(svtkPlotBox);

//-----------------------------------------------------------------------------
svtkPlotBox::svtkPlotBox()
{
  this->Storage = new svtkPlotBox::Private();
...
//-----------------------------------------------------------------------------
bool svtkPlotBox::Paint(svtkContext2D* painter)
{
  // This is where everything should be drawn, or dispatched to other methods.
  svtkDebugMacro(<< "Paint event called in svtkPlotBox.");
  ...
  if (this->Storage->empty() || this->Storage->at(0).size() != 5)
  {
    svtkErrorMacro(
      << "Input table must contain 5 rows per column. These rows hold min, quartile 1, median, "
         "quartile 2 and max. Use svtkComputeQuartiles to create a proper table.");
    return false;
  }

  svtkChartBox* parent = svtkChartBox::SafeDownCast(this->Parent);

  int nbCols = static_cast<int>(this->Storage->size());
  for (int i = 0; i < nbCols; i++)
  {
    svtkStdString colName = parent->GetVisibleColumns()->GetValue(i);
    int index;
...
//-----------------------------------------------------------------------------
void svtkPlotBox::DrawBoxPlot(int i, unsigned char* rgba, double x, svtkContext2D* painter)
{
...
//-----------------------------------------------------------------------------
svtkStringArray* svtkPlotBox::GetLabels()
{
  if (this->Labels)
  {
    return this->Labels;
...
//-----------------------------------------------------------------------------
bool svtkPlotBox::PaintLegend(svtkContext2D* painter, const svtkRectf& rec, int)
{
  if (this->Storage->empty() || this->Storage->at(0).size() < 5)
  {
    return false;
...
//-----------------------------------------------------------------------------
void svtkPlotBox::SetInputData(svtkTable* table)
{
...
//-----------------------------------------------------------------------------
svtkIdType svtkPlotBox::GetNearestPoint(const svtkVector2f& point, const svtkVector2f& tol,
  ...
  if (!this->LegacyRecursionFlag)
  {
    ...
    if (ret != -1)
    {
      SVTK_LEGACY_REPLACED_BODY(svtkPlotBox::GetNearestPoint(const svtkVector2f& point,
        ...
        "SVTK 9.0",
        svtkPlotBox::GetNearestPoint(const svtkVector2f& point, const svtkVector2f& tol,
...
//-----------------------------------------------------------------------------
bool svtkPlotBox::UpdateTableCache(svtkTable* table)
{
...
//-----------------------------------------------------------------------------
void svtkPlotBox::SetLookupTable(svtkScalarsToColors* lut)
{
...
//-----------------------------------------------------------------------------
svtkScalarsToColors* svtkPlotBox::GetLookupTable()
{
...
//-----------------------------------------------------------------------------
void svtkPlotBox::SetColumnColor(const svtkStdString& colName, double* rgb)
{
  if (this->LookupTable == nullptr)
  {
    this->CreateDefaultLookupTable();
...
//-----------------------------------------------------------------------------
void svtkPlotBox::CreateDefaultLookupTable()
{
...
//-----------------------------------------------------------------------------
void svtkPlotBox::PrintSelf(ostream& os, svtkIndent indent)
{
