...
int TestCategoricalColors(int svtkNotUsed(argc), char* svtkNotUsed(argv)[])
{
  bool res = true;
  ...
  svtkColorSeries* palettes = svtkColorSeries::New();
#if 0
  svtkIdType numSchemes = palettes->GetNumberOfColorSchemes();
  for (svtkIdType i = 0; i < numSchemes; ++ i)
  {
  ...
  svtkDoubleArray* data = svtkDoubleArray::New();
  ...
  svtkUnsignedCharArray* color = lut->MapScalars(data, SVTK_RGBA, 0);
  unsigned char* cval;
  for (svtkIdType i = 0; i < color->GetNumberOfTuples(); ++i)
  {
