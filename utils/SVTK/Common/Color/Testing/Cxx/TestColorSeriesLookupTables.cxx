...
std::string DisplayOrdinalLUTAsString(svtkLookupTable* lut)
{
  svtkIdType tv = lut->GetNumberOfTableValues();
  double dR[2];
...
//-----------------------------------------------------------------------------
//! Display the contents of the lookup table.
std::string DisplayCategoricalLUTAsString(svtkLookupTable* lut)
{
  svtkIdType tv = lut->GetNumberOfTableValues();
  double dR[2];
  lut->GetTableRange(dR);
...
std::pair<bool, std::string> CompareOrdinalLUTs(svtkLookupTable* lut1, svtkLookupTable* lut2)
{
  ...
  else
  {
    ...
    {
      svtkIdType tv = lut1->GetNumberOfTableValues();
      double dR[2];
...
std::pair<bool, std::string> CompareCategoricalLUTs(svtkLookupTable* lut1, svtkLookupTable* lut2)
{
  std::pair<bool, std::string> res(true, "");
  if (lut1->GetNumberOfTableValues() != lut2->GetNumberOfTableValues())
  {
  ...
  else
  {
    ...
    else
    {
      ...
      svtkSmartPointer<svtkDoubleArray> data = svtkSmartPointer<svtkDoubleArray>::New();
      for (std::vector<double>::const_iterator p = indices.begin(); p != indices.end(); ++p)
      {
        data->InsertNextValue(*p);
      }
      svtkUnsignedCharArray* color1 = lut1->MapScalars(data, SVTK_RGBA, 0);
      svtkUnsignedCharArray* color2 = lut2->MapScalars(data, SVTK_RGBA, 0);
      unsigned char* cval1;
      unsigned char* cval2;
      for (svtkIdType i = 0; i < color1->GetNumberOfTuples(); ++i)
      {
...
int TestColorSeriesLookupTables(int svtkNotUsed(argc), char* svtkNotUsed(argv)[])
{
  std::string line(
    "-----------------------------------------------------------------------------\n");
  bool res = true;
  svtkSmartPointer<svtkColorSeries> colorSeries = svtkSmartPointer<svtkColorSeries>::New();
  ...
  svtkSmartPointer<svtkLookupTable> lut2 = svtkSmartPointer<svtkLookupTable>::New();

  // These next two tables will be categorical tables.
  svtkSmartPointer<svtkLookupTable> lut3 = svtkSmartPointer<svtkLookupTable>::New();
  svtkSmartPointer<svtkLookupTable> lut4 = svtkSmartPointer<svtkLookupTable>::New();
  // For the annotation just use a letter of the alphabet.
  svtkSmartPointer<svtkVariantArray> values = svtkSmartPointer<svtkVariantArray>::New();
  std::string str = "abcdefghijklmnopqrstuvwxyz";
  for (size_t i = 0; i < 10; ++i)
  {
    values->InsertNextValue(svtkVariant(str.substr(i, 1)));
  }
