 ...
 *
 *
 * The svtkColorSeries stores palettes of colors. There are several default
 ...
 * <a
 * href="http://htmlpreview.github.io/?https://github.com/lorensen/SVTKExamples/blob/master/src/Python/Visualization/SVTKColorSeriesPatches.html">SVTKColorSeriesPatches</a>.
 * Use
 * <a
 * href="https://lorensen.github.io/SVTKExamples/site/Python/Visualization/ColorSeriesPatches/">ColorSeriesPatches</a>
...
#ifndef svtkColorSeries_h
#define svtkColorSeries_h
...
class SVTKCOMMONCOLOR_EXPORT svtkColorSeries : public svtkObject
{
public:
  svtkTypeMacro(svtkColorSeries, svtkObject);
  void PrintSelf(ostream& os, svtkIndent indent) override;

  /**
   * Create a new svtkColorSeries with the SPECTRUM color scheme.
   */
  static svtkColorSeries* New();
  ...
  virtual int SetColorSchemeByName(const svtkStdString& schemeName);
  //@}
   ...
   */
  virtual svtkStdString GetColorSchemeName() const;

  /**
   * Set the name of the current color scheme
   */
  virtual void SetColorSchemeName(const svtkStdString& scheme);
   ...
   */
  svtkColor3ub GetColor(int index) const;

  /**
   * Get the color at the specified index. If the index is out of range then
   * the call wraps around, i.e. uses the mod operator.
   */
  svtkColor3ub GetColorRepeating(int index) const;

  /**
   * Set the color at the specified index. Does nothing if the index is out of
   * range.
   */
  virtual void SetColor(int index, const svtkColor3ub& color);

  /**
   * Adds the color to the end of the list.
   */
  virtual void AddColor(const svtkColor3ub& color);

  /**
   * Inserts the color at the specified index in the list.
   */
  virtual void InsertColor(int index, const svtkColor3ub& color);
   ...
   */
  virtual void DeepCopy(svtkColorSeries* chartColors);
   ...
   */
  void BuildLookupTable(svtkLookupTable* lkup, int lutIndexing = CATEGORICAL);
   ...
   */
  SVTK_NEWINSTANCE
  svtkLookupTable* CreateLookupTable(int lutIndexing = CATEGORICAL);

protected:
  svtkColorSeries();
  ~svtkColorSeries() override;
  ...
  /// The color scheme being used.
  svtkStdString ColorSchemeName;

private:
  svtkColorSeries(const svtkColorSeries&) = delete;
  void operator=(const svtkColorSeries&) = delete;
};

#endif // svtkColorSeries_h
