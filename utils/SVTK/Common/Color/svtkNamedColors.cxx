...
class svtkNamedColorsDataStore
{
public:
  svtkNamedColorsDataStore() { this->Init(); }

  virtual ~svtkNamedColorsDataStore() = default;
...
public:
  ...
  std::map<svtkStdString, svtkColor4ub>* GetColorMap() { return &this->colorMap; }

  //-----------------------------------------------------------------------------
  // Get the color by name.
  // The name is treated as being case-insensitive.
  // The color is returned as svtkColor4ub class.
  // Black is returned if the color is not found.
  void GetColor(const svtkStdString& name, svtkColor4ub& color)
  {
    for (int i = 0; i < 3; ++i)
    {
      color[i] = 0;
    ...
    if (!name.empty())
    {
      svtkStdString s = this->ToLowercase(name);
      std::map<svtkStdString, svtkColor4ub>::iterator p = this->colorMap.find(s);
  ...
  // The color is returned as svtkColor4ub class.
  // Black is returned if the color is not found.
  void GetColor(const svtkStdString& name, svtkColor4d& color)
  {
    svtkColor4ub c;
  ...
  // Get the color by name.
  // The name is treated as being case-insensitive.
  // The color is returned as svtkColor3ub class.
  // Black is returned if the color is not found.
  void GetColor(const svtkStdString& name, svtkColor3ub& color)
  {
    svtkColor4ub c;
    this->GetColor(name, c);
  ...
  // Get the color by name.
  // The name is treated as being case-insensitive.
  // The color is returned as svtkColor3d class.
  // Black is returned if the color is not found.
  void GetColor(const svtkStdString& name, svtkColor3d& color)
  {
    svtkColor4d c;
    this->GetColor(name, c);
    for (int i = 0; i < color.GetSize(); ++i)
  ...
  // The name is treated as being case-insensitive.
  // The color is a svtkColor4ub class.
  // No color is set if the name is empty or the color vector is not of size 4.
  void SetColor(const svtkStdString& name, const svtkColor4ub& color)
  {
  ...
  // Set the color by name.
  // The name is treated as being case-insensitive.
  // The color is a svtkColor4d class.
  // No color is set if the name is empty or the color vector is not of size 4.
  void SetColor(const svtkStdString& name, const svtkColor4d& color)
  {
    if (!name.empty() && color.GetSize() == 4)
    {
      svtkStdString s = this->ToLowercase(name);
      ...
      this->colorMap[s] = svtkColor4ub(r, g, b, a);
    }
  }
  ...
  // Set the color by name.
  // The name is treated as being case-insensitive.
  // The color is a svtkColor3ub class.
  // No color is set if the name is empty or the color vector is not of size 4.
  void SetColor(const svtkStdString& name, const svtkColor3ub& color)
  {
    svtkColor4ub c;
    for (int i = 0; i < 3; ++i)
    {
  ...
  // Set the color by name.
  // The name is treated as being case-insensitive.
  // The color is a svtkColor3d class.
  // No color is set if the name is empty or the color vector is not of size 4.
  void SetColor(const svtkStdString& name, const svtkColor3d& color)
  {
    svtkColor4d c;
    for (int i = 0; i < 3; ++i)
    {
  ...
  // The name is treated as being case-insensitive.
  void RemoveColor(const svtkStdString& name)
  {
    if (!name.empty())
    {
      svtkStdString s = this->ToLowercase(name);
  ...
  bool ColorExists(const svtkStdString& name)
  {
    svtkStdString s = this->ToLowercase(name);
  ...
  std::vector<svtkStdString> GetColorNames()
  {
    std::vector<svtkStdString> colorNames;
    for (std::map<svtkStdString, svtkColor4ub>::const_iterator p = this->GetColorMap()->begin();
  ...
  std::vector<std::vector<svtkStdString> > GetSynonyms()
  {
    std::vector<svtkStdString> cn = this->GetColorNames();
    std::map<svtkStdString, std::vector<svtkStdString> > synonyms;
    for (std::vector<svtkStdString>::const_iterator p = cn.begin(); p != cn.end(); ++p)
    {
      svtkColor4ub vu;
      this->GetColor(*p, vu);
      std::vector<svtkStdString> duplicates;
      for (std::map<svtkStdString, svtkColor4ub>::const_iterator q = this->GetColorMap()->begin();
      ...
      if (!duplicates.empty())
      {
        ...
        for (std::vector<svtkStdString>::const_iterator r = duplicates.begin();
             r != duplicates.end(); ++r)
        {
    ...
    std::vector<std::vector<svtkStdString> > retVec;
    for (std::map<svtkStdString, std::vector<svtkStdString> >::const_iterator p = synonyms.begin();
         p != synonyms.end(); ++p)
    {
      std::vector<svtkStdString> vstr;
      vstr.push_back(p->first);
      for (std::vector<svtkStdString>::const_iterator q = p->second.begin(); q != p->second.end();
           ++q)
      {
        vstr.push_back(*q);
      }
...
public:
  ...
  void Init()
  {
    ...
    for (size_t i = 0; i < colorTableSz; ++i)
    {
      ...
      svtkStdString s = colorTable[i];
      size_t idx = s.find(',');
      svtkStdString name = s.substr(0, idx);
      svtkStdString t = this->ToLowercase(name);
      name = t;
      size_t start = idx + 1;
      idx = s.find(',', start);
      ...
      color.push_back(static_cast<unsigned char>(atoi(t.c_str())));
      this->colorMap[name] = svtkColor4ub(color[0], color[1], color[2], color[3]);
    }
  }
...
private:
  ...
  svtkStdString ToLowercase(const svtkStdString& str)
  {
    svtkStdString s;
  ...
  std::map<svtkStdString, svtkColor4ub> colorMap;
};

//------------------------------------------------------------------------------
// Helper class for parsing a string which defines a RGB or RGBA color.
class svtkColorStringParser
{
public:
  svtkColorStringParser(svtkNamedColors* namedColors)
  {
  ...
  bool Parse(const svtkStdString& colorString);

  const svtkColor4ub& GetColor() const { return this->Color; }

private:
  void RGBAFuncStringToRGBA(svtkStdString color, svtkStdString::size_type pos, unsigned int argCount);

  void HexStringToRGBA(svtkStdString color);
...
bool svtkColorStringParser::Parse(const svtkStdString& colorString)
{
  svtkStdString color = this->ToLowercase(colorString);

  // Skip heading spaces.
  svtkStdString::size_type pos = color.find_first_not_of(" \t");
  this->StateGood = (pos != svtkStdString::npos);
  ...
  if (this->StateGood)
  {
    if (color[pos] == '#')
    {
      ...
      svtkStdString::size_type last = color.find_last_not_of(" \t");
...
void svtkColorStringParser::HexStringToRGBA(svtkStdString color)
{
  // Check if it is valid hexadecimal representation.
  svtkStdString::size_type pos = color.find_first_not_of("0123456789abcdefABCDEF");
  this->StateGood = (pos == svtkStdString::npos);
  ...
  if (this->StateGood)
  {
    ...
    {
      color.resize(6);
      svtkStdString::size_type i = 3, j = 6;
      do
      {
        color[--j] = color[--i];
        color[--j] = color[i];
...
// If the parsed string is not valid set `StateGood` to false.
void svtkColorStringParser::RGBAFuncStringToRGBA(
  svtkStdString color, svtkStdString::size_type pos, unsigned int argCount)
{
  // Parse '('.
  pos = color.find_first_not_of(" \t", pos);
  this->StateGood = (pos != svtkStdString::npos && color[pos] == '(');

  // Check if percentage values are used.
  bool usePercentage = (color.find_first_of("%", pos + 1) != svtkStdString::npos);

  // Count and replace delimiters.
  unsigned int delimiterCount = 0;
  for (svtkStdString::size_type i = pos + 1; (delimiterCount < argCount) && (i < color.size()); ++i)
  {
    if (color[i] == ',')
    {
      color[i] = ' ';
...
//-----------------------------------------------------------------------------
svtkStandardNewMacro(svtkNamedColors);

//----------------------------------------------------------------------------
svtkNamedColors::svtkNamedColors()
{
  this->Colors = new svtkNamedColorsDataStore;
  this->Parser = new svtkColorStringParser(this);
}
...
void svtkNamedColors::PrintSelf(ostream& os, svtkIndent indent)
{
  this->Superclass::PrintSelf(os, indent);
  for (std::map<svtkStdString, svtkColor4ub>::const_iterator p = this->Colors->GetColorMap()->begin();
...
//-----------------------------------------------------------------------------
svtkStdString svtkNamedColors::GetColorNames()
{
  std::vector<svtkStdString> cnv = this->Colors->GetColorNames();
  svtkStdString colorNames;
  // Get the last element in the vector.
  std::vector<svtkStdString>::iterator lastItr = cnv.end();
  --lastItr;
  // Now just iterate through the map getting the color names.
  for (std::vector<svtkStdString>::const_iterator p = cnv.begin(); p != cnv.end(); ++p)
  {
    colorNames += *p;
    if (p != lastItr)
    {
...
//-----------------------------------------------------------------------------
void svtkNamedColors::GetColorNames(svtkStringArray* colorNames)
{
  std::vector<svtkStdString> cnv = this->Colors->GetColorNames();
  // Now just iterate through the map getting the color names.
  for (std::vector<svtkStdString>::const_iterator p = cnv.begin(); p != cnv.end(); ++p)
...
//-----------------------------------------------------------------------------
svtkStdString svtkNamedColors::GetSynonyms()
{
  svtkStdString synonyms;
  std::vector<std::vector<svtkStdString> > syn = this->Colors->GetSynonyms();
  std::vector<std::vector<svtkStdString> >::const_iterator synLast = syn.end();
  --synLast;
  for (std::vector<std::vector<svtkStdString> >::const_iterator p = syn.begin(); p != syn.end(); ++p)
  {
    // Get the last element in the vector.
    std::vector<svtkStdString>::const_iterator strLast = p->end();
    --strLast;
    for (std::vector<svtkStdString>::const_iterator q = p->begin(); q != p->end(); ++q)
    {
      synonyms += *q;
      if (q != strLast)
      {
...
//-----------------------------------------------------------------------------
int svtkNamedColors::GetNumberOfColors()
{
  return static_cast<int>(this->Colors->GetColorMap()->size());
}

//-----------------------------------------------------------------------------
void svtkNamedColors::ResetColors()
{
...
//-----------------------------------------------------------------------------
bool svtkNamedColors::ColorExists(const svtkStdString& name)
{
  return this->Colors->ColorExists(name);
}

//-----------------------------------------------------------------------------
svtkColor4ub svtkNamedColors::GetColor4ub(const svtkStdString& name)
{
  svtkColor4ub color;
...
//-----------------------------------------------------------------------------
void svtkNamedColors::GetColor(
  const svtkStdString& name, unsigned char& r, unsigned char& g, unsigned char& b, unsigned char& a)
{
  svtkColor4ub rgba;
...
//-----------------------------------------------------------------------------
void svtkNamedColors::GetColor(const svtkStdString& name, unsigned char rgba[4])
{
  svtkColor4ub color;
  this->Colors->GetColor(name, color);
  for (int i = 0; i < color.GetSize(); ++i)
...
//-----------------------------------------------------------------------------
void svtkNamedColors::GetColor(const svtkStdString& name, svtkColor4ub& rgba)
{
  this->Colors->GetColor(name, rgba);
}

//-----------------------------------------------------------------------------
svtkColor4d svtkNamedColors::GetColor4d(const svtkStdString& name)
{
  svtkColor4d color;
  this->Colors->GetColor(name, color);
  return color;
...
//-----------------------------------------------------------------------------
void svtkNamedColors::GetColor(const svtkStdString& name, double& r, double& g, double& b, double& a)
{
  svtkColor4d rgba;
  this->Colors->GetColor(name, rgba);
  r = rgba[0];
...
//-----------------------------------------------------------------------------
void svtkNamedColors::GetColor(const svtkStdString& name, double rgba[4])
{
  svtkColor4d color;
  this->Colors->GetColor(name, color);
  for (int i = 0; i < color.GetSize(); ++i)
...
//-----------------------------------------------------------------------------
void svtkNamedColors::GetColor(const svtkStdString& name, svtkColor4d& rgba)
{
  this->Colors->GetColor(name, rgba);
}

//-----------------------------------------------------------------------------
svtkColor3ub svtkNamedColors::GetColor3ub(const svtkStdString& name)
{
  svtkColor3ub rgb;
...
//-----------------------------------------------------------------------------
svtkColor3d svtkNamedColors::GetColor3d(const svtkStdString& name)
{
  svtkColor3d rgb;
  this->Colors->GetColor(name, rgb);
  return rgb;
...
//-----------------------------------------------------------------------------
void svtkNamedColors::GetColor(const svtkStdString& name, double& r, double& g, double& b)
{
  svtkColor3d rgb;
  this->Colors->GetColor(name, rgb);
  r = rgb[0];
...
//-----------------------------------------------------------------------------
void svtkNamedColors::GetColorRGB(const svtkStdString& name, double rgb[3])
{
  svtkColor3d color;
  this->Colors->GetColor(name, color);
  for (int i = 0; i < color.GetSize(); ++i)
...
//-----------------------------------------------------------------------------
void svtkNamedColors::GetColor(const svtkStdString& name, svtkColor3ub& rgb)
{
  this->Colors->GetColor(name, rgb);
}

//-----------------------------------------------------------------------------
void svtkNamedColors::GetColor(const svtkStdString& name, svtkColor3d& rgb)
{
  this->Colors->GetColor(name, rgb);
}

//-----------------------------------------------------------------------------
void svtkNamedColors::SetColor(const svtkStdString& name, const unsigned char& r,
...
//-----------------------------------------------------------------------------
void svtkNamedColors::SetColor(const svtkStdString& name, const unsigned char rgba[4])
{
  svtkColor4ub v;
...
//-----------------------------------------------------------------------------
void svtkNamedColors::SetColor(const svtkStdString& name, const svtkColor4ub& rgba)
{
  this->Colors->SetColor(name, rgba);
}

//-----------------------------------------------------------------------------
void svtkNamedColors::SetColor(const svtkStdString& name, const svtkColor3ub& rgb)
{
  this->Colors->SetColor(name, rgb);
}

//-----------------------------------------------------------------------------
void svtkNamedColors::SetColor(
  const svtkStdString& name, const double& r, const double& g, const double& b, const double& a)
{
  svtkColor4d v;
  v.SetRed(r);
  v.SetGreen(g);
...
//-----------------------------------------------------------------------------
void svtkNamedColors::SetColor(const svtkStdString& name, const double rgba[4])
{
  svtkColor4d v;
  for (int i = 0; i < v.GetSize(); ++i)
  {
...
//-----------------------------------------------------------------------------
void svtkNamedColors::SetColor(const svtkStdString& name, const svtkColor4d& rgba)
{
  svtkColor4d v;
  for (int i = 0; i < v.GetSize(); ++i)
  {
...
//-----------------------------------------------------------------------------
void svtkNamedColors::SetColor(const svtkStdString& name, const svtkColor3d& rgb)
{
  this->Colors->SetColor(name, rgb);
}

//-----------------------------------------------------------------------------
void svtkNamedColors::RemoveColor(const svtkStdString& name)
{
  if (!name.empty())
  {
...
//-----------------------------------------------------------------------------
svtkColor4ub svtkNamedColors::HTMLColorToRGBA(const svtkStdString& colorString)
{
...
//-----------------------------------------------------------------------------
svtkColor3ub svtkNamedColors::HTMLColorToRGB(const svtkStdString& colorString)
{
  svtkColor4ub color4ub = this->HTMLColorToRGBA(colorString);
  svtkColor3ub color3ub;
...
//-----------------------------------------------------------------------------
void svtkNamedColors::SetColor(const svtkStdString& name, const svtkStdString& htmlString)
{
  if (!name.empty())
  {
    svtkColor4ub noColor(0, 0, 0, 0);
    svtkColor4ub color = this->HTMLColorToRGBA(htmlString);
    if (color != noColor)
    {
...
//-----------------------------------------------------------------------------
svtkStdString svtkNamedColors::RGBToHTMLColor(const svtkColor3ub& rgb)
{
  std::stringstream ss;
...
//-----------------------------------------------------------------------------
svtkStdString svtkNamedColors::RGBAToHTMLColor(const svtkColor4ub& rgba)
{
  std::stringstream ss;
  ss.precision(3);
