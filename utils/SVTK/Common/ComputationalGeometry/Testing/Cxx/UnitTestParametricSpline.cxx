...
int TestErrors();
int TestMisc();
int TestSetPoints(svtkPoints*, bool closed = false);
void GeneratePoints(int, svtkSmartPointer<svtkPoints>&);
void GenerateRandomPoints(int, svtkSmartPointer<svtkPoints>&);
...
void GeneratePoints(int npts, svtkSmartPointer<svtkPoints>& points)
{
  points->SetNumberOfPoints(npts);
  double deltaX = svtkMath::Pi() * 2.0 / (npts - 1);
  double deltaZ = 2.0 / (npts - 1);
  for (int n = 0; n < npts; ++n)
  {
    double x[3];
    x[0] = -svtkMath::Pi() + n * deltaX;
    x[1] = std::sin(x[0]);
    x[2] = -1.0 + n * deltaZ;
...
int TestSetPoints(svtkPoints* points, bool closed)
{
  int status = 0;

  svtkSmartPointer<svtkParametricSpline> pspline = svtkSmartPointer<svtkParametricSpline>::New();
  if (closed)
  {
    pspline->ClosedOn();
  }
  ...
  svtkPoints* knots = svtkPoints::New(SVTK_DOUBLE);
  ...
  for (int n = 0; n < npts; ++n)
  {
    ...
    if (n > 0)
    {
      length += std::sqrt(svtkMath::Distance2BetweenPoints(x, xm1));
    }
    xm1[0] = x[0];
    xm1[1] = x[1];
    xm1[2] = x[2];
  ...
  for (int n = 0; n < npts; ++n)
  {
    ...
    if (!svtkMathUtilities::FuzzyCompare(x[0], result[0], tolerance) ||
      !svtkMathUtilities::FuzzyCompare(x[1], result[1], tolerance) ||
      !svtkMathUtilities::FuzzyCompare(x[2], result[2], tolerance))
    {
...
int TestErrors()
{
  ...
  svtkSmartPointer<svtkParametricSpline> pspline = svtkSmartPointer<svtkParametricSpline>::New();
  pspline->AddObserver(svtkCommand::ErrorEvent, errorObserver);
...
int TestSetGet()
{
  ...
  if (className != "svtkCardinalSpline")
  {
  ...
  className = pspline->GetXSpline()->GetClassName();
  if (className != "svtkKochanekSpline")
  {
    std::cout << "GetXSpline: Expected "
              << "svtkKochanekSpline"
              << " but got " << className << std::endl;
