...
svtkStandardNewMacro(svtkKochanekSpline);
...
// DefaultTension = 0,
// DefaultContinuity = 0.
svtkKochanekSpline::svtkKochanekSpline()
{
...
double svtkKochanekSpline::Evaluate(double t)
{
  int index = 0;
  double* intervals;
  double* coefficients;
...
void svtkKochanekSpline::Compute()
{
  double *ts, *xs;
  double* coefficients;
  ...
  if (size < 2)
  {
    svtkErrorMacro("Spline requires at least 2 points. # of points is: " << size);
    return;
  }
...
#define SVTK_EPSILON .0001

//----------------------------------------------------------------------------
// Compute the coefficients for a 1D spline
void svtkKochanekSpline::Fit1D(int size, double* x, double* y, double tension, double bias,
  ...
  else // curve is open
  {
    switch (leftConstraint)
    {
      ...
      case 3:
        ...
        if ((leftValue > (-2.0 + SVTK_EPSILON)) || (leftValue < (-2.0 - SVTK_EPSILON)))
        {
          coefficients[0][1] =
    ...
    switch (rightConstraint)
    {
      ...
      case 3:
        ...
        if ((rightValue > (-2.0 + SVTK_EPSILON)) || (rightValue < (-2.0 - SVTK_EPSILON)))
        {
...
//----------------------------------------------------------------------------
void svtkKochanekSpline::DeepCopy(svtkSpline* s)
{
  svtkKochanekSpline* spline = svtkKochanekSpline::SafeDownCast(s);
  ...
  // Now do superclass
  this->svtkSpline::DeepCopy(s);
}

//----------------------------------------------------------------------------
void svtkKochanekSpline::PrintSelf(ostream& os, svtkIndent indent)
{
