/*=========================================================================
  ...
  Module:    svtkParametricSpline.cxx
...
svtkStandardNewMacro(svtkParametricSpline);

//----------------------------------------------------------------------------
svtkParametricSpline::svtkParametricSpline()
{
  this->MinimumU = 0;
  this->MaximumU = 1.0;
  this->JoinU = 0;
  ...
  this->XSpline = svtkCardinalSpline::New();
  this->YSpline = svtkCardinalSpline::New();
  this->ZSpline = svtkCardinalSpline::New();
...
//----------------------------------------------------------------------------
void svtkParametricSpline::SetNumberOfPoints(svtkIdType numPts)
{
  if (!this->Points)
  {
...
//----------------------------------------------------------------------------
void svtkParametricSpline::SetPoint(svtkIdType index, double x, double y, double z)
{
  if (this->Points)
  {
...
//----------------------------------------------------------------------------
void svtkParametricSpline::SetPoints(svtkPoints* pts)
{
  if (pts != this->Points)
  {
...
//----------------------------------------------------------------------------
void svtkParametricSpline::SetXSpline(svtkSpline* s)
{
  if (s != this->XSpline)
  {
...
//----------------------------------------------------------------------------
void svtkParametricSpline::SetYSpline(svtkSpline* s)
{
  if (s != this->YSpline)
  {
...
//----------------------------------------------------------------------------
void svtkParametricSpline::SetZSpline(svtkSpline* s)
{
  if (s != this->ZSpline)
  {
...
//----------------------------------------------------------------------------
void svtkParametricSpline::Evaluate(double U[3], double Pt[3], double*)
{
...
//----------------------------------------------------------------------------
double svtkParametricSpline::EvaluateScalar(double u[3], double*, double*)
{
  // make sure everything has been set up
  if (this->InitializeTime < this->GetMTime())
  {
...
int svtkParametricSpline::Initialize()
{
  // Check to make sure splines are available
  if (!this->XSpline || !this->YSpline || !this->ZSpline)
  {
    svtkErrorMacro("Please specify splines");
    return 0;
  }
  if (!this->Points)
  {
    svtkErrorMacro("Please specify points");
    return 0;
  }
  ...
  svtkIdType npts = this->Points->GetNumberOfPoints();

  if (npts < 1)
  {
    svtkErrorMacro("Please specify at least one point.");
    return 0;
  }
  if (npts < 2)
  {
  ...
  if (this->ParameterizeByLength)
  {
    ...
    for (i = 1; i < npts; ++i)
    {
      ...
      // if ( len <= 0.0 )
      //  {
      //  svtkErrorMacro("Spline must have non-coincident points");
      //  return 0; //failure
      //  }
      this->Length += len;
      xPrev[0] = x[0];
    ...
    if (this->Closed)
    {
      ...
      this->ClosedLength = this->Length + sqrt(svtkMath::Distance2BetweenPoints(x, xPrev));
    }
  }
  else
  {
...
//----------------------------------------------------------------------------
void svtkParametricSpline::PrintSelf(ostream& os, svtkIndent indent)
{
