...
svtkUniformGrid* GetGrid(double* origin, double* spacing, int* ndim)
{
  ...
  svtkDoubleArray* pntData = svtkDoubleArray::New();
  ...
  {
    svtkCell* myCell = grd->GetCell(cellIdx);

    svtkPoints* cellPoints = myCell->GetPoints();
...
// data-set.
bool DataSetsEqual(svtkImageData* img, svtkStructuredGrid* sg)
{
  // 0. Check dimensions
  int imgdim[3];
  int sgdim[3];
  ...
  {
    svtkDataArray* array1 = img->GetPointData()->GetArray(dataArrayIdx);
    svtkDataArray* array2 = sg->GetPointData()->GetArray(dataArrayIdx);
  ...
  {
    svtkDataArray* array1 = img->GetCellData()->GetArray(dataArrayIdx);
    svtkDataArray* array2 = sg->GetCellData()->GetArray(dataArrayIdx);
    if (array1->GetNumberOfComponents() != array2->GetNumberOfComponents())
    {
      return false;
    }
...
int TestImageDataToStructuredGrid(int, char*[])
{
  ...
  int ndim[3] = { 10, 10, 1 };
  svtkUniformGrid* img1 = GetGrid(origin, spacing, ndim);

  svtkImageToStructuredGrid* myFilter = svtkImageToStructuredGrid::New();
  myFilter->SetInputData(img1);
  img1->Delete();
  myFilter->Update();
  svtkStructuredGrid* sg1 = myFilter->GetOutput();
