...
// This test checks the SplitExtent method of svtkThreadedImageAlgorithm.
...
// A simple subclass of svtkThreadedImageAlgorithm to test
class ThreadedImageAlgorithmTester : public svtkThreadedImageAlgorithm
{
public:
  static ThreadedImageAlgorithmTester* New();
  svtkTypeMacro(ThreadedImageAlgorithmTester, svtkThreadedImageAlgorithm);
  void SetSplitPath(const int path[3], int len);
  bool TestSplitExtent(int extent[6], svtkIdType pieces);
};

svtkStandardNewMacro(ThreadedImageAlgorithmTester);
...
bool ThreadedImageAlgorithmTester::TestSplitExtent(int extent[6], svtkIdType pieces)
{
  bool success = true;

  int size[3] = { extent[1] - extent[0] + 1, extent[3] - extent[2] + 1, extent[5] - extent[4] + 1 };

  svtkIdType n = this->SplitExtent(nullptr, extent, 0, pieces);
  ...
  {
    int split[6];
    svtkIdType m = this->SplitExtent(split, extent, i, pieces);
...
{
  svtkNew<ThreadedImageAlgorithmTester> tester;
            ...
            for (int xsize = 1; xsize <= 100; xsize += 99)
            {
              for (int ysize = 1; ysize <= 92; ysize += 13)
              {
                for (int zsize = 1; zsize <= 10; zsize++)
                {
                  ...
                  svtkIdType maxpieces = xsize * ysize * zsize;
                  maxpieces = (maxpieces > 100 ? 100 : maxpieces);
                  svtkIdType inc = 1 + maxpieces / 5;
                  for (svtkIdType pieces = 1; pieces <= maxpieces; pieces += inc)
                  {
