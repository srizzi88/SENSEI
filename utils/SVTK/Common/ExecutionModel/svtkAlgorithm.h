...
/**
 * @class   svtkAlgorithm
 * @brief   Superclass for all sources, filters, and sinks in SVTK.
 *
 * svtkAlgorithm is the superclass for all sources, filters, and sinks
 * in SVTK.  It defines a generalized interface for executing data
 ...
 * controlled by instances of svtkExecutive.  Every svtkAlgorithm
 * instance has an associated svtkExecutive when it is used in a
 * pipeline.  The executive is responsible for data flow.
 */

#ifndef svtkAlgorithm_h
#define svtkAlgorithm_h
...
class SVTKCOMMONEXECUTIONMODEL_EXPORT svtkAlgorithm : public svtkObject
{
public:
  static svtkAlgorithm* New();
  svtkTypeMacro(svtkAlgorithm, svtkObject);
   ...
   * output precision: svtkAppendPolyData, svtkCleanPolyData, svtkClipPolyData,
   * svtkConnectivityFilter, svtkDecimatePolylineFilter, svtkDecimatePro, svtkDelaunay3D,
   * svtkFeatureEdges, svtkGlyph3D, svtkHedgeHog, svtkMaskPoints, svtkPolyDataConnectivityFilter,
   * svtkSmoothPolyDataFilter, svtkStaticCleanPolyData, svtkThresholdPoints, svtkTubeFilter,
   * svtkAppendPoints, svtkTransformFilter, svtkTransformPolyDataFilter,
   * svtkLinearToQuadraticCellsFilter, svtkProcrustesAlignmentFilter,
   * svtkAdaptiveSubdivisionFilter, svtkBoundedPointSource, svtkArcSource, svtkConeSource,
   * svtkCubeSource, svtkCylinderSource, svtkDiskSource, svtkEllipseArcSource,
   * svtkEllipticalButtonSource, svtkFrustumSource, svtkGlyphSource2D, svtkLineSource,
   * svtkOutlineSource, svtkParametricFunctionSource, svtkPlaneSource, svtkPlatonicSolidSource,
   * svtkPointSource, svtkRectangularButtonSource, svtkRegularPolygonSource, svtkSphereSource,
   * svtkSuperquadricSource, svtkTessellatedBoxSource, svtkTextSource, svtkTexturedSphereSource,
   * svtkImageToPoints, svtkDepthImageToPointCloud.
   ...
   */
  virtual void SetExecutive(svtkExecutive* executive);
   ...
   * svtkExecutive::ComputePipelineMTime() for details.
   */
   ...
   * 0 on failure. When can be either svtkExecutive::BeforeForward or
   * svtkExecutive::AfterForward.
   */
  virtual int ModifyRequest(svtkInformation* request, int when);
   ...
   */
  svtkInformation* GetInputPortInformation(int port);
   ...
   */
  svtkInformation* GetOutputPortInformation(int port);
   ...
   */
  svtkSetMacro(AbortExecute, svtkTypeBool);
  svtkGetMacro(AbortExecute, svtkTypeBool);
  svtkBooleanMacro(AbortExecute, svtkTypeBool);
  //@}
   ...
   */
  SVTK_LEGACY(void SetProgress(double));
   ...
   * set to 1.0. This is useful when the svtkAlgorithm instance is used as an
   * internal algorithm to solve only a part of a whole problem.
   *
   * If calling on a internal svtkAlgorithm, make sure you take into
   * consideration that values set of the outer svtkAlgorithm as well since the
   * outer svtkAlgorithm itself may be nested in another algorithm.
   *
   * @note SetProgressShiftScale does not modify the MTime of the algorithm.
   */
  void SetProgressShiftScale(double shift, double scale);
  svtkGetMacro(ProgressShift, double);
  svtkGetMacro(ProgressScale, double);
  //@}
  ...
  svtkGetStringMacro(ProgressText);
  //@}
   ...
   */
  svtkGetMacro(ErrorCode, unsigned long);
  //@}

  // left public for performance since it is used in inner loops
  svtkTypeBool AbortExecute;

  /**
   * Keys used to specify input port requirements.
   * \ingroup InformationKeys
   */
  static svtkInformationIntegerKey* INPUT_IS_OPTIONAL();
  /**
   * \ingroup InformationKeys
   */
  static svtkInformationIntegerKey* INPUT_IS_REPEATABLE();
  /**
   * \ingroup InformationKeys
   */
  static svtkInformationInformationVectorKey* INPUT_REQUIRED_FIELDS();
  /**
   * \ingroup InformationKeys
   */
  static svtkInformationStringVectorKey* INPUT_REQUIRED_DATA_TYPE();
  /**
   * \ingroup InformationKeys
   */
  static svtkInformationInformationVectorKey* INPUT_ARRAYS_TO_PROCESS();
  /**
   * \ingroup InformationKeys
   */
  static svtkInformationIntegerKey* INPUT_PORT();
  /**
   * \ingroup InformationKeys
   */
  static svtkInformationIntegerKey* INPUT_CONNECTION();
   ...
   * \ingroup InformationKeys
   */
  static svtkInformationIntegerKey* CAN_PRODUCE_SUB_EXTENT();
   ...
   * \ingroup InformationKeys
   */
  static svtkInformationIntegerKey* CAN_HANDLE_PIECE_REQUEST();
   ...
   * stored. See svtkDataObject::FieldAssociations for detail.
   */
  ...
  virtual void SetInputArrayToProcess(int idx, svtkInformation* info);
  //@}
   ...
   */
  svtkInformation* GetInputArrayInformation(int idx);
   ...
   * the given port.
   */
  svtkDataObject* GetOutputDataObject(int port);

  /**
   * Get the data object that will contain the algorithm input for the given
   * port and given connection.
   */
  svtkDataObject* GetInputDataObject(int port, int connection);
   ...
   */
  virtual void SetInputConnection(int port, svtkAlgorithmOutput* input);
   ...
   */
  virtual void AddInputConnection(int port, svtkAlgorithmOutput* input);
  virtual void AddInputConnection(svtkAlgorithmOutput* input);
  //@}
   ...
   */
  virtual void RemoveInputConnection(int port, svtkAlgorithmOutput* input);
   ...
   * method creates a svtkTrivialProducer instance and sets that as the
   * input-connection for the given port. It is safe to call this method repeatedly
   * with the same input data object. The MTime of the svtkAlgorithm will not
   * change unless the data object changed.
   */
  virtual void SetInputDataObject(int port, svtkDataObject* data);
  virtual void SetInputDataObject(svtkDataObject* data) { this->SetInputDataObject(0, data); }
   ...
   */
  virtual void AddInputDataObject(int port, svtkDataObject* data);
  virtual void AddInputDataObject(svtkDataObject* data) { this->AddInputDataObject(0, data); }
   ...
   */
  svtkAlgorithmOutput* GetOutputPort(int index);
  svtkAlgorithmOutput* GetOutputPort() { return this->GetOutputPort(0); }
   ...
   */
  svtkAlgorithmOutput* GetInputConnection(int port, int index);

  /**
   * Returns the algorithm and the output port index of
   * that algorithm connected to a port-index pair.
   */
  svtkAlgorithm* GetInputAlgorithm(int port, int index, int& algPort);

  /**
   * Returns the algorithm connected to a port-index pair.
   */
  svtkAlgorithm* GetInputAlgorithm(int port, int index);

  /**
   * Equivalent to GetInputAlgorithm(0, 0).
   */
  svtkAlgorithm* GetInputAlgorithm() { return this->GetInputAlgorithm(0, 0); }

  /**
   * Returns the executive associated with a particular input
   * connection.
   */
  svtkExecutive* GetInputExecutive(int port, int index);

  /**
   * Equivalent to GetInputExecutive(0, 0)
   */
  svtkExecutive* GetInputExecutive() { return this->GetInputExecutive(0, 0); }
   ...
   */
  svtkInformation* GetInputInformation(int port, int index);

  /**
   * Equivalent to GetInputInformation(0, 0)
   */
  svtkInformation* GetInputInformation() { return this->GetInputInformation(0, 0); }
   ...
   * the information objects passed as arguments instead.
   */
  svtkInformation* GetOutputInformation(int port);
   ...
   * Update(int port, svtkInformationVector* requests) for details.
   */
  virtual svtkTypeBool Update(svtkInformation* requests);
  ...
  //======================================================================
  // The following block of code is to support old style SVTK applications. If
   ...
   */
  int UpdateExtentIsEmpty(svtkInformation* pinfo, svtkDataObject* output);
  int UpdateExtentIsEmpty(svtkInformation* pinfo, int extentType);
  //@}
   ...
   */
  static void SetDefaultExecutivePrototype(svtkExecutive* proto);
   ...
   * be 0.
   */
  int* GetUpdateExtent() SVTK_SIZEHINT(6) { return this->GetUpdateExtent(0); }
  int* GetUpdateExtent(int port) SVTK_SIZEHINT(6);
   ...
   */
  void SetProgressObserver(svtkProgressObserver*);
  svtkGetObjectMacro(ProgressObserver, svtkProgressObserver);
  //@}

protected:
  svtkAlgorithm();
  ~svtkAlgorithm() override;

  // Keys used to indicate that input/output port information has been
  // filled.
  static svtkInformationIntegerKey* PORT_REQUIREMENTS_FILLED();
   ...
   */
  virtual int FillInputPortInformation(int port, svtkInformation* info);
   ...
   */
  virtual int FillOutputPortInformation(int port, svtkInformation* info);
   ...
   */
  int GetInputArrayAssociation(int idx, svtkInformationVector** inputVector);
   ...
   */
  int GetInputArrayAssociation(int idx, int connection, svtkInformationVector** inputVector);
  int GetInputArrayAssociation(int idx, svtkDataObject* input);
  //@}
   ...
   */
  svtkDataArray* GetInputArrayToProcess(int idx, svtkInformationVector** inputVector);
  svtkDataArray* GetInputArrayToProcess(
   ...
   * on the port with the appropriate connection id substituted.
   */
  svtkDataArray* GetInputArrayToProcess(int idx, int connection, svtkInformationVector** inputVector);
  svtkDataArray* GetInputArrayToProcess(
  ...
  svtkDataArray* GetInputArrayToProcess(int idx, svtkDataObject* input, int& association);
  //@}
   ...
   * only reasonable during the REQUEST_DATA pass
   */
  svtkAbstractArray* GetInputAbstractArrayToProcess(int idx, svtkInformationVector** inputVector);
  svtkAbstractArray* GetInputAbstractArrayToProcess(
    int idx, svtkInformationVector** inputVector, int& association);
  //@}
  ...
  svtkAbstractArray* GetInputAbstractArrayToProcess(int idx, svtkDataObject* input, int& association);
  //@}
   ...
   * svtkDataObject::GetActiveFieldInformation)
   */
  svtkInformation* GetInputArrayFieldInformation(int idx, svtkInformationVector** inputVector);

  /**
   * Create a default executive.
   * If the DefaultExecutivePrototype is set, a copy of it is created
   * in CreateDefaultExecutive() using NewInstance().
   * Otherwise, svtkStreamingDemandDrivenPipeline is created.
   */
  virtual svtkExecutive* CreateDefaultExecutive();
   ...
   * reading or writing the file.
   */
  svtkSetMacro(ErrorCode, unsigned long);
   ...
   */
  virtual void SetNthInputConnection(int port, int index, svtkAlgorithmOutput* input);
  ...
  static svtkExecutive* DefaultExecutivePrototype;

  /**
   * These methods are used by subclasses to implement methods to
   * set data objects directly as input. Internally, they create
   * a svtkTrivialProducer that has the data object as output and
   * connect it to the algorithm.
   */
  void SetInputDataInternal(int port, svtkDataObject* input)
  {
    this->SetInputDataObject(port, input);
  }
  void AddInputDataInternal(int port, svtkDataObject* input)
  {
    this->AddInputDataObject(port, input);
  }

  svtkProgressObserver* ProgressObserver;

private:
  svtkExecutive* Executive;
  svtkInformationVector* InputPortInformation;
  svtkInformationVector* OutputPortInformation;
  svtkAlgorithmInternals* AlgorithmInternal;
  static void ConnectionAdd(
    svtkAlgorithm* producer, int producerPort, svtkAlgorithm* consumer, int consumerPort);
  static void ConnectionRemove(
    svtkAlgorithm* producer, int producerPort, svtkAlgorithm* consumer, int consumerPort);
  static void ConnectionRemoveAllInput(svtkAlgorithm* consumer, int port);
  static void ConnectionRemoveAllOutput(svtkAlgorithm* producer, int port);

private:
  svtkAlgorithm(const svtkAlgorithm&) = delete;
  void operator=(const svtkAlgorithm&) = delete;
