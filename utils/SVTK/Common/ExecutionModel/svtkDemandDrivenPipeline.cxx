/*=========================================================================
  ...
  Module:    svtkDemandDrivenPipeline.cxx
...
svtkStandardNewMacro(svtkDemandDrivenPipeline);

svtkInformationKeyMacro(svtkDemandDrivenPipeline, DATA_NOT_GENERATED, Integer);
svtkInformationKeyMacro(svtkDemandDrivenPipeline, RELEASE_DATA, Integer);
svtkInformationKeyMacro(svtkDemandDrivenPipeline, REQUEST_DATA, Request);
svtkInformationKeyMacro(svtkDemandDrivenPipeline, REQUEST_DATA_NOT_GENERATED, Request);
svtkInformationKeyMacro(svtkDemandDrivenPipeline, REQUEST_DATA_OBJECT, Request);
svtkInformationKeyMacro(svtkDemandDrivenPipeline, REQUEST_INFORMATION, Request);

//----------------------------------------------------------------------------
svtkDemandDrivenPipeline::svtkDemandDrivenPipeline()
{
...
//----------------------------------------------------------------------------
void svtkDemandDrivenPipeline::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
int svtkDemandDrivenPipeline::ComputePipelineMTime(svtkInformation* request,
  ...
  svtkMTimeType* mtime)
{
  ...
  if (!this->SharedInputInformation)
  {
    ...
    for (int i = 0; i < this->Algorithm->GetNumberOfInputPorts(); ++i)
    {
      ...
      {
        svtkInformation* info = inInfoVec[i]->GetInformationObject(j);
...
//----------------------------------------------------------------------------
svtkTypeBool svtkDemandDrivenPipeline::ProcessRequest(
  svtkInformation* request, svtkInformationVector** inInfoVec, svtkInformationVector* outInfoVec)
{
  ...
  if (this->Algorithm && request->Has(REQUEST_DATA_OBJECT()))
  {
    ...
    if (this->PipelineMTime > this->DataObjectTime.GetMTime())
    {
      ...
      svtkLogF(TRACE, "%s execute-data-object", svtkLogIdentifier(this->Algorithm));
      ...
      for (int i = 0; result && i < outInfoVec->GetNumberOfInformationObjects(); ++i)
      {
        ...
        if (!info->Get(svtkDataObject::DATA_OBJECT()))
        {
          result = 0;
        }
      }
  ...
  if (this->Algorithm && request->Has(REQUEST_INFORMATION()))
  {
...

    {
      ...
      svtkLogF(TRACE, "%s execute-information", svtkLogIdentifier(this->Algorithm));
  ...
  if (this->Algorithm && request->Has(REQUEST_DATA()))
  {
    ...
    if (this->NeedToExecuteData(outputPort, inInfoVec, outInfoVec))
    {
      ...
      svtkLogF(TRACE, "%s execute-data", svtkLogIdentifier(this->Algorithm));
...
//----------------------------------------------------------------------------
void svtkDemandDrivenPipeline::ResetPipelineInformation(int, svtkInformation*) {}

//----------------------------------------------------------------------------
svtkTypeBool svtkDemandDrivenPipeline::Update()
{
  return this->Superclass::Update();
}

//----------------------------------------------------------------------------
svtkTypeBool svtkDemandDrivenPipeline::Update(int port)
{
...
//----------------------------------------------------------------------------
int svtkDemandDrivenPipeline::UpdatePipelineMTime()
{
  // The algorithm should not invoke anything on the executive.
...
//----------------------------------------------------------------------------
int svtkDemandDrivenPipeline::UpdateDataObject()
{
  // The algorithm should not invoke anything on the executive.
...
//----------------------------------------------------------------------------
int svtkDemandDrivenPipeline::UpdateInformation()
{
  // The algorithm should not invoke anything on the executive.
  ...
  if (!this->InfoRequest)
  {
    this->InfoRequest = svtkInformation::New();
    this->InfoRequest->Set(REQUEST_INFORMATION());
    // The request is forwarded upstream through the pipeline.
    this->InfoRequest->Set(svtkExecutive::FORWARD_DIRECTION(), svtkExecutive::RequestUpstream);
    // Algorithms process this request after it is forwarded.
    this->InfoRequest->Set(svtkExecutive::ALGORITHM_AFTER_FORWARD(), 1);
  }
...
//----------------------------------------------------------------------------
int svtkDemandDrivenPipeline::UpdateData(int outputPort)
{
  // The algorithm should not invoke anything on the executive.
  ...
  {
    svtkErrorMacro("UpdateData given output port index " << outputPort << " on an algorithm with "
...
//----------------------------------------------------------------------------
int svtkDemandDrivenPipeline::ExecuteDataObject(
  svtkInformation* request, svtkInformationVector** inInfo, svtkInformationVector* outInfo)
{
  // Invoke the request on the algorithm.
  int result = this->CallAlgorithm(request, svtkExecutive::RequestDownstream, inInfo, outInfo);
...
//----------------------------------------------------------------------------
int svtkDemandDrivenPipeline::ExecuteInformation(
  svtkInformation* request, svtkInformationVector** inInfoVec, svtkInformationVector* outInfoVec)
{
  ...
  // Invoke the request on the algorithm.
  return this->CallAlgorithm(request, svtkExecutive::RequestDownstream, inInfoVec, outInfoVec);
}

//----------------------------------------------------------------------------
int svtkDemandDrivenPipeline::ExecuteData(
  svtkInformation* request, svtkInformationVector** inInfo, svtkInformationVector* outInfo)
{
  this->ExecuteDataStart(request, inInfo, outInfo);
  // Invoke the request on the algorithm.
  //   svtkMTimeType mTimeBefore = this->Algorithm->GetMTime();
  int result = this->CallAlgorithm(request, svtkExecutive::RequestDownstream, inInfo, outInfo);
  //   if (mTimeBefore != this->Algorithm->GetMTime())
  //     {
  //     svtkWarningMacro(<< this->Algorithm->GetClassName()
...
//----------------------------------------------------------------------------
void svtkDemandDrivenPipeline::ExecuteDataStart(
  svtkInformation* request, svtkInformationVector** inInfo, svtkInformationVector* outputs)
{
  int i;
  ...
  this->CallAlgorithm(request, svtkExecutive::RequestDownstream, inInfo, outputs);
  ...
  {
    svtkInformation* outInfo = outputs->GetInformationObject(i);
  ...
  // Pass the svtkDataObject's field data from the first input to all
  // outputs.
  if (this->GetNumberOfInputPorts() > 0)
  {
    svtkDataObject* input = this->GetInputData(0, 0, inInfo);
  ...
  this->Algorithm->InvokeEvent(svtkCommand::StartEvent, nullptr);
...
//----------------------------------------------------------------------------
void svtkDemandDrivenPipeline::ExecuteDataEnd(
  svtkInformation* request, svtkInformationVector** inInfoVec, svtkInformationVector* outputs)
{
  ...
  this->Algorithm->InvokeEvent(svtkCommand::EndEvent, nullptr);
...
//----------------------------------------------------------------------------
void svtkDemandDrivenPipeline::MarkOutputsGenerated(
  svtkInformation*, svtkInformationVector** /* inInfoVec */, svtkInformationVector* outputs)
{
...
//----------------------------------------------------------------------------
int svtkDemandDrivenPipeline::CheckDataObject(int port, svtkInformationVector* outInfoVec)
{
  // Check that the given output port has a valid data object.
  svtkInformation* outInfo = outInfoVec->GetInformationObject(port);
  svtkDataObject* data = outInfo->Get(svtkDataObject::DATA_OBJECT());
  svtkInformation* portInfo = this->Algorithm->GetOutputPortInformation(port);
  if (const char* dt = portInfo->Get(svtkDataObject::DATA_TYPE_NAME()))
  {
    ...
    if (!data || incorrectdata)
    {
      if (data)
      {
        svtkDebugMacro(<< "CHECKDATAOBJECT Replacing " << data->GetClassName());
      }
      // Try to create an instance of the correct type.
      data = svtkDataObjectTypes::NewDataObject(dt);
      this->SetOutputData(port, data, outInfo);
      if (data)
      {
        svtkDebugMacro(<< "CHECKDATAOBJECT Created " << dt);
        data->FastDelete();
      }
    }
    if (!data)
...
//----------------------------------------------------------------------------
int svtkDemandDrivenPipeline::InputCountIsValid(svtkInformationVector** inInfoVec)
{
...
//----------------------------------------------------------------------------
int svtkDemandDrivenPipeline::InputCountIsValid(int port, svtkInformationVector** inInfoVec)
{
  ...
  {
    svtkErrorMacro("Input port " << port << " of algorithm " << this->Algorithm->GetClassName()
...
//----------------------------------------------------------------------------
int svtkDemandDrivenPipeline::InputTypeIsValid(svtkInformationVector** inInfoVec)
{
...
//----------------------------------------------------------------------------
int svtkDemandDrivenPipeline::InputTypeIsValid(int port, svtkInformationVector** inInfoVec)
{
...
//----------------------------------------------------------------------------
int svtkDemandDrivenPipeline::InputTypeIsValid(int port, int index, svtkInformationVector** inInfoVec)
{
  if (!inInfoVec[port])
  {
    return 0;
  }
  svtkInformation* info = this->Algorithm->GetInputPortInformation(port);
  svtkDataObject* input = this->GetInputData(port, index, inInfoVec);

  // Enforce required type, if any.
  if (info->Has(svtkAlgorithm::INPUT_REQUIRED_DATA_TYPE()) &&
    info->Length(svtkAlgorithm::INPUT_REQUIRED_DATA_TYPE()) > 0)
  {
    // The input cannot be nullptr unless the port is optional.
    if (!input && !info->Get(svtkAlgorithm::INPUT_IS_OPTIONAL()))
    {
      svtkErrorMacro("Input for connection index "
        << index << " on input port index " << port << " for algorithm "
        << this->Algorithm->GetClassName() << "(" << this->Algorithm << ") is nullptr, but a "
        << info->Get(svtkAlgorithm::INPUT_REQUIRED_DATA_TYPE(), 0) << " is required.");
      return 0;
    }
    ...
    if (input)
    {
      int size = info->Length(svtkAlgorithm::INPUT_REQUIRED_DATA_TYPE());
      for (int i = 0; i < size; ++i)
      {
        if (input->IsA(info->Get(svtkAlgorithm::INPUT_REQUIRED_DATA_TYPE(), i)))
        {
          foundMatch = true;
        }
      }
...
//----------------------------------------------------------------------------
int svtkDemandDrivenPipeline::InputFieldsAreValid(svtkInformationVector** inInfoVec)
{
...
//----------------------------------------------------------------------------
int svtkDemandDrivenPipeline::InputFieldsAreValid(int port, svtkInformationVector** inInfoVec)
{
...
//----------------------------------------------------------------------------
int svtkDemandDrivenPipeline::InputFieldsAreValid(
  int port, int index, svtkInformationVector** inInfoVec)
{
  svtkInformation* info = this->Algorithm->GetInputPortInformation(port);
  svtkInformationVector* fields = info->Get(svtkAlgorithm::INPUT_REQUIRED_FIELDS());
  ...
  {
    svtkInformation* field = fields->GetInformationObject(i);

    // Decide which kinds of fields to check.
    int checkPoints = 1;
    int checkCells = 1;
    int checkFields = 1;
    if (field->Has(svtkDataObject::FIELD_ASSOCIATION()))
    {
      switch (field->Get(svtkDataObject::FIELD_ASSOCIATION()))
      {
    ...
    // Point and cell data arrays only exist in svtkDataSet instances.
    svtkDataSet* dataSet = svtkDataSet::SafeDownCast(input);
        ...
        this->FieldArrayExists(input->GetFieldData(), field)))
    {
      /* TODO: Construct more descriptive error message from field
         requirements. */
      svtkErrorMacro("Required field not found in input.");
      result = 0;
    }
  }
...
//----------------------------------------------------------------------------
int svtkDemandDrivenPipeline::DataSetAttributeExists(
  svtkDataSetAttributes* dsa, svtkInformation* field)
{
  if (field->Has(svtkDataObject::FIELD_ATTRIBUTE_TYPE()))
  {
    // A specific attribute must match the requirements.
    int attrType = field->Get(svtkDataObject::FIELD_ATTRIBUTE_TYPE());
...
//----------------------------------------------------------------------------
int svtkDemandDrivenPipeline::FieldArrayExists(svtkFieldData* data, svtkInformation* field)
{
...
//----------------------------------------------------------------------------
int svtkDemandDrivenPipeline::ArrayIsValid(svtkAbstractArray* array, svtkInformation* field)
{
  ...
  if (const char* name = field->Get(svtkDataObject::FIELD_NAME()))
  {
  ...
  if (field->Has(svtkDataObject::FIELD_ARRAY_TYPE()))
  {
    int arrayType = field->Get(svtkDataObject::FIELD_ARRAY_TYPE());
  ...
  if (field->Has(svtkDataObject::FIELD_NUMBER_OF_COMPONENTS()))
  {
    int arrayNumComponents = field->Get(svtkDataObject::FIELD_NUMBER_OF_COMPONENTS());
  ...
  // field data (not point or cell data).
  if (field->Has(svtkDataObject::FIELD_NUMBER_OF_TUPLES()))
  {
    int arrayNumTuples = field->Get(svtkDataObject::FIELD_NUMBER_OF_TUPLES());
...
//----------------------------------------------------------------------------
int svtkDemandDrivenPipeline::InputIsOptional(int port)
{
  if (svtkInformation* info = this->Algorithm->GetInputPortInformation(port))
  {
    return info->Get(svtkAlgorithm::INPUT_IS_OPTIONAL());
  }
  return 0;
}

//----------------------------------------------------------------------------
int svtkDemandDrivenPipeline::InputIsRepeatable(int port)
{
  if (svtkInformation* info = this->Algorithm->GetInputPortInformation(port))
  {
    return info->Get(svtkAlgorithm::INPUT_IS_REPEATABLE());
  }
  return 0;
}

//----------------------------------------------------------------------------
svtkDataObject* svtkDemandDrivenPipeline::NewDataObject(const char* type)
{
  return svtkDataObjectTypes::NewDataObject(type);
}

//----------------------------------------------------------------------------
int svtkDemandDrivenPipeline ::NeedToExecuteData(
  int outputPort, svtkInformationVector** inInfoVec, svtkInformationVector* outInfoVec)
{
  ...
  if (outputPort >= 0)
  {
    ...
    svtkInformation* info = outInfoVec->GetInformationObject(outputPort);
...
//----------------------------------------------------------------------------
int svtkDemandDrivenPipeline::SetReleaseDataFlag(int port, int n)
{
...
//----------------------------------------------------------------------------
int svtkDemandDrivenPipeline::GetReleaseDataFlag(int port)
{
