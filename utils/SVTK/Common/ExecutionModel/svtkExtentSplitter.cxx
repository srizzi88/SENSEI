...
svtkStandardNewMacro(svtkExtentSplitter);

//----------------------------------------------------------------------------
struct svtkExtentSplitterExtent
{
  int extent[6];
};

struct svtkExtentSplitterSource
{
  int extent[6];
  int priority;
};

struct svtkExtentSplitterSubExtent
{
  int extent[6];
  int source;
};

class svtkExtentSplitterInternals
{
public:
  typedef std::queue<svtkExtentSplitterExtent> QueueType;
  typedef std::map<int, svtkExtentSplitterSource> SourcesType;
  typedef std::vector<svtkExtentSplitterSubExtent> SubExtentsType;
  SourcesType Sources;
  QueueType Queue;
...
//----------------------------------------------------------------------------
svtkExtentSplitter::svtkExtentSplitter()
{
  this->Internal = new svtkExtentSplitterInternals;
  this->PointMode = 0;
}
...
//----------------------------------------------------------------------------
void svtkExtentSplitter::PrintSelf(ostream& os, svtkIndent indent)
{
  ...
  else
  {
    ...
    for (svtkExtentSplitterInternals::SourcesType::const_iterator src =
  ...
  else
  {
    ...
    for (svtkExtentSplitterInternals::SubExtentsType::const_iterator i =
...
//----------------------------------------------------------------------------
void svtkExtentSplitter::AddExtentSource(
  int id, int priority, int x0, int x1, int y0, int y1, int z0, int z1)
{
  // Add the source.
  svtkExtentSplitterSource& source = this->Internal->Sources[id];
  source.extent[0] = x0;
  source.extent[1] = x1;
  source.extent[2] = y0;
  source.extent[3] = y1;
...
//----------------------------------------------------------------------------
void svtkExtentSplitter::AddExtentSource(int id, int priority, int* extent)
{
...
//----------------------------------------------------------------------------
void svtkExtentSplitter::RemoveExtentSource(int id)
{
  // Remove the source.
...
//----------------------------------------------------------------------------
void svtkExtentSplitter::RemoveAllExtentSources()
{
...
//----------------------------------------------------------------------------
void svtkExtentSplitter::AddExtent(int x0, int x1, int y0, int y1, int z0, int z1)
{
  // Queue the extent.
  svtkExtentSplitterExtent e;
  e.extent[0] = x0;
  e.extent[1] = x1;
  e.extent[2] = y0;
  e.extent[3] = y1;
...
//----------------------------------------------------------------------------
void svtkExtentSplitter::GetSubExtent(int index, int* extent)
{
  if (index < 0 || index >= this->GetNumberOfSubExtents())
  {
    svtkErrorMacro("SubExtent index " << index << " is out of range [0,"
...
//----------------------------------------------------------------------------
int svtkExtentSplitter::GetSubExtentSource(int index)
{
  if (index < 0 || index >= this->GetNumberOfSubExtents())
  {
    svtkErrorMacro("SubExtent index " << index << " is out of range [0,"
...
//----------------------------------------------------------------------------
int svtkExtentSplitter::ComputeSubExtents()
{
  // Assume success.
  int result = 1;

  svtkExtentSplitterInternals::SubExtentsType subExtents;
  int bestPriority;
  ...
  while (!this->Internal->Queue.empty())
  {
    ...
    svtkExtentSplitterExtent e = this->Internal->Queue.front();
    ...
    subExtents.clear();
    bestPriority = -1;
    svtkExtentSplitterSubExtent se;
    for (svtkExtentSplitterInternals::SourcesType::const_iterator src =
           this->Internal->Sources.begin();
         src != this->Internal->Sources.end(); ++src)
    {
...
//----------------------------------------------------------------------------
void svtkExtentSplitter::SplitExtent(int* extent, int* subextent)
{
...
//----------------------------------------------------------------------------
int svtkExtentSplitter::IntersectExtents(const int* extent1, const int* extent2, int* result)
{
...
//----------------------------------------------------------------------------
int svtkExtentSplitter::Min(int a, int b)
{
  return (a < b) ? a : b;
}

//----------------------------------------------------------------------------
int svtkExtentSplitter::Max(int a, int b)
{
  return (a > b) ? a : b;
}
