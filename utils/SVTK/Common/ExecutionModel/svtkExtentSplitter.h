 ...
 *
 * svtkExtentSplitter splits each input extent into non-overlapping
...
#ifndef svtkExtentSplitter_h
#define svtkExtentSplitter_h
...
class SVTKCOMMONEXECUTIONMODEL_EXPORT svtkExtentSplitter : public svtkObject
{
public:
  svtkTypeMacro(svtkExtentSplitter, svtkObject);
  void PrintSelf(ostream& os, svtkIndent indent) override;
  static svtkExtentSplitter* New();
   ...
   * ComputeSubExtents.
   */
  int* GetSubExtent(int index) SVTK_SIZEHINT(6);
   ...
   */
  svtkGetMacro(PointMode, svtkTypeBool);
  svtkSetMacro(PointMode, svtkTypeBool);
  svtkBooleanMacro(PointMode, svtkTypeBool);
  //@}

protected:
  svtkExtentSplitter();
  ~svtkExtentSplitter() override;
  ...
  svtkExtentSplitterInternals* Internal;

  // On if reading only all points (but not always all cells) is
  // necessary.  Used for reading volumes of planar slices storing
  // only point data.
  svtkTypeBool PointMode;

private:
  svtkExtentSplitter(const svtkExtentSplitter&) = delete;
  void operator=(const svtkExtentSplitter&) = delete;
};
