/*=========================================================================
  ...
  Module:    svtkExtentTranslator.cxx
...
svtkStandardNewMacro(svtkExtentTranslator);

svtkInformationKeyMacro(svtkExtentTranslator, DATA_SPLIT_MODE, Integer);

// Subclass svtkInformationIntegerRequestKey to set the DataKey.
class svtkInformationSplitModeRequestKey : public svtkInformationIntegerRequestKey
{
public:
  svtkInformationSplitModeRequestKey(const char* name, const char* location)
    : svtkInformationIntegerRequestKey(name, location)
  {
    this->DataKey = svtkExtentTranslator::DATA_SPLIT_MODE();
  }
};
svtkInformationKeySubclassMacro(
  svtkExtentTranslator, UPDATE_SPLIT_MODE, SplitModeRequest, IntegerRequest);

//----------------------------------------------------------------------------
svtkExtentTranslator::svtkExtentTranslator()
{
  this->Piece = 0;
  ...
  this->SplitMode = svtkExtentTranslator::BLOCK_MODE;
...
//----------------------------------------------------------------------------
void svtkExtentTranslator::SetSplitPath(int len, int* sp)
{
...
//----------------------------------------------------------------------------
int svtkExtentTranslator::PieceToExtent()
{
...
//----------------------------------------------------------------------------
int svtkExtentTranslator::PieceToExtentByPoints()
{
  return this->PieceToExtentThreadSafe(this->Piece, this->NumberOfPieces, this->GhostLevel,
    this->WholeExtent, this->Extent, this->SplitMode, 1);
}

int svtkExtentTranslator::PieceToExtentThreadSafe(int piece, int numPieces, int ghostLevel,
...
//----------------------------------------------------------------------------
int svtkExtentTranslator::SplitExtent(int piece, int numPieces, int* ext, int splitMode)
{
  int numPiecesInFirstHalf;
  unsigned long size[3];
  int splitAxis;
  svtkLargeInteger mid;
...
//----------------------------------------------------------------------------
int svtkExtentTranslator::SplitExtentByPoints(int piece, int numPieces, int* ext, int splitMode)
{
  int numPiecesInFirstHalf;
...
//----------------------------------------------------------------------------
void svtkExtentTranslator::PrintSelf(ostream& os, svtkIndent indent)
{
  ...
  os << indent << "SplitMode: ";
  if (this->SplitMode == svtkExtentTranslator::BLOCK_MODE)
  {
    os << "Block\n";
  }
  else if (this->SplitMode == svtkExtentTranslator::X_SLAB_MODE)
  {
    os << "X Slab\n";
  }
  else if (this->SplitMode == svtkExtentTranslator::Y_SLAB_MODE)
  {
    os << "Y Slab\n";
  }
  else if (this->SplitMode == svtkExtentTranslator::Z_SLAB_MODE)
  {
    os << "Z Slab\n";
  }
  else
