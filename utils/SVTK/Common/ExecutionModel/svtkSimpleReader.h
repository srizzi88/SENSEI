...
=========================================================================*/
/**
 * @class   svtkSimpleReader
 * @brief   Superclass for algorithms that are not time or parallel aware
 *
 */

#ifndef svtkSimpleReader_h
#define svtkSimpleReader_h
...
class SVTKCOMMONEXECUTIONMODEL_EXPORT svtkSimpleReader : public svtkReaderAlgorithm
{
public:
  svtkTypeMacro(svtkSimpleReader, svtkReaderAlgorithm);
   ...
   * will return svtkMath::NaN() if no time value is present
   * in the file.
   */
   ...
   */
  virtual int ReadMetaDataSimple(const std::string& /*fname*/, svtkInformation* /*metadata*/)
  {
    return 1;
  }
   ...
   */
  virtual int ReadMeshSimple(const std::string& fname, svtkDataObject* output) = 0;
   ...
   */
  virtual int ReadPointsSimple(const std::string& fname, svtkDataObject* output) = 0;
   ...
   */
  virtual int ReadArraysSimple(const std::string& fname, svtkDataObject* output) = 0;

protected:
  svtkSimpleReader();
  ~svtkSimpleReader() override;
...
private:
  svtkSimpleReader(const svtkSimpleReader&) = delete;
  void operator=(const svtkSimpleReader&) = delete;

  svtkSimpleReaderInternal* Internal;
};
