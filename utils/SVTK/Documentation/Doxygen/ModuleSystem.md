...
SVTK 9.0 introduces a new build system compared to previous versions. This
...
"building". Scanning uses the [svtk_module_scan][] function to search the
[svtk.module][] files for metadata, gathers the set of modules to build and
returns them to the caller. That list of modules is eventually passed to
[svtk_module_build][] which sorts the modules for their build order and then
...
[svtk.module]: @ref module-parse-module
[svtk_module_build]: @ref svtk_module_build

# Modules

Modules are described by [svtk.module][] files. These files are "scanned" using
the [svtk_module_scan][] function. They provide all the information necessary for
the module system to:

  - provide cache variables for selecting the module (e.g.,
    `SVTK_MODULE_ENABLE_ModuleName`);
...
The [svtk.module][] files are read and "parsed", but not executed directly. This
...
be passed manually to [svtk_module_scan][] or discovered by using the
[svtk_module_find_modules][] convenience function.
...
arguments to the `svtk_module_scan` function. Modules passed to
...
The next selector is the `SVTK_MODULE_ENABLE_` variable for the module. This is
...
their setting, but a project may set the `_svtk_module_group_default_${group}`
variable to change this default value.

After all of the above logic, if a module is still marked as `DEFAULT`, the
`WANT_BY_DEFAULT` argument to [svtk_module_scan][] is used to determine whether
...
or not. The module system will add a `SVTK_MODULE_ENABLE_${module}` compilation
...
named `SVTK_MODULE_ENABLE_Namespace_Target`.
...
argument to the [svtk_module_build][] function. If the directory is available and
...
After scanning is complete, [svtk_module_scan][] returns a list of modules and
...
When using [svtk_module_build][], the `PROVIDES_MODULES` and `PROVIDES_KITS` from
...
The [svtk_module_build][] function is where the decision to build with or without
...
targets not built under [svtk_module_build][] also end up at a sensible location.
...
library metadata. For example, SVTK suffixes its library filenames with a version
...
[svtk_module_build][] also offers arguments to aid in installing module
...
argument to [svtk_module_build][]. Setting this to `OFF` will suppress the
installation of:
...
export set when [svtk_module_build][] is called, but the export set is considered
...
directories will be searched for. It defaults to `Testing` due to SVTK's
conventions.

The module system, due to SVTK's usage of it, has convenience parameters for
...
Each is provided in the testing subdirectory as `_svtk_build_${name}`, so the
`TEST_DATA_TARGET` argument is available as `_svtk_build_TEST_DATA_TARGET`.
...
There are two main functions provided for this:
  ...
  - [svtk_module_add_module][]
  - [svtk_module_add_executable][]
...
to the `LIBRARY_NAME` for the module. The [svtk_module_build][]
...
[svtk_module_build][] `BUILD_WITH_KITS` argument, it will be built as an
[`OBJECT`][cmake-OBJECT] library and the kit machinery in [svtk_module_build][]
...
[svtk_module_add_module]: @ref svtk_module_add_module
[svtk_module_add_executable]: @ref svtk_module_add_executable
...
`add_library`, [svtk_module_add_module][] classifies its source files explicitly:
...
specified to [svtk_module_build][] and may be added to a subdirectory of this
...
installed header layout, [svtk_module_install_headers][] should be used.
...
installed via [svtk_module_install_headers][] are not. This is a known limitation
at the moment.
...
There are also a couple of convenience parameters that use SVTK's file naming
...
[svtk_module_install_headers]: @ref svtk_module_install_headers

## Executables

Executables may be created using [svtk_module_add_executable][]. The first
...
differences, if any) is:
  ...
  - [set_target_properties][cmake-set_target_properties] becomes
    [svtk_module_set_properties][]
  - [set_property(TARGET)][cmake-set_property] becomes
    [svtk_module_set_property][]
  - [get_property(TARGET)][cmake-get_property] becomes
    [svtk_module_get_property][]
  - [add_dependencies][cmake-add_dependencies] becomes [svtk_module_depend][]
...
[svtk_module_set_properties]: @ref svtk_module_set_properties
[svtk_module_set_property]: @ref svtk_module_set_property
[svtk_module_get_property]: @ref svtk_module_get_property
[svtk_module_depend]: @ref svtk_module_depend
[svtk_module_include]: @ref svtk_module_include
[svtk_module_definitions]: @ref svtk_module_definitions
[svtk_module_compile_options]: @ref svtk_module_compile_options
[svtk_module_compile_features]: @ref svtk_module_compile_features
[svtk_module_link]: @ref svtk_module_link
[svtk_module_link_options]: @ref svtk_module_link_options
...
The main assumption is that all modules passed to a single [svtk_module_build][]
...
`PACKAGE` argument for [svtk_module_build][]. These are done so that the
...
[svtk_module_build][]:
  ...
  - `<PACKAGE>-svtk-module-properties.cmake`: Properties for the targets exported
    into the build.
...
[svtk_module_export_find_packages][] function. This function writes a file named
...
found using [svtk_module_find_package][] are required during the
...
This file should be included after the `<PACKAGE>-svtk-module-properties.cmake`
file generated by the [svtk_module_build][] call so that it can use the module
...
[svtk_module_export_find_packages]: @ref svtk_module_export_find_packages
[svtk_module_find_package]: @ref svtk_module_find_package
...
Kits are described in [svtk.kit][] files which act much like [svtk.module][]
files. However, they only have `NAME`, `LIBRARY_NAME`, and `DESCRIPTION` fields.
These all act just like they do in the `svtk.module` context. These files may
either be passed manually to [svtk_module_scan][] or discovered by using the
[svtk_module_find_kits][] convenience function.

Before a module may be a member of a kit, a [svtk.kit][] must declare it and be
...
Code which uses modules may call the [svtk_module_autoinit][] function to use
...
[svtk_module_autoinit]: @ref svtk_module_autoinit
[autoinit]: @ref module-autoinit

<a name="wrapping"></a>
## Wrapping

SVTK comes with support for wrapping its classes into other languages.
Currently, SVTK supports wrapping its classes for use in the Python and Java
languages. In order to wrap a set of modules for a language, a separate
function is used for each language.

All languages read the headers of classes with a `__SVTK_WRAP__` preprocessor
definition defined. This may be used to hide methods or other details from the
wrapping code if wanted.

### Python

For Python, the [svtk_module_wrap_python][] function must be used. This function
...
`SVTK::PythonWrapped` has a basename of `PythonWrapped`) must be used. The
...
[svtk_module_python_default_destination][] function is used to determine a
default if one is not passed.

The Python wrappers define a `__SVTK_WRAP_PYTHON__` preprocessor definition when
reading code which may be used to hide methods or other details from the Python
wrapping code.

[svtk_module_wrap_python]: @ref svtk_module_wrap_python
[svtk_module_python_default_destination]: @ref svtk_module_python_default_destination

### Java

For Java, the [svtk_module_wrap_java][] function must be used. This function
...
`_svtk_module_java_files` properties containing a list of `.java` sources
...
The Java wrappers define a `__SVTK_WRAP_JAVA__` preprocessor definition when
reading code which may be used to hide methods or other details from the Java
wrapping code.

[svtk_module_wrap_java]: @ref svtk_module_wrap_java
...
The primary API is [svtk_module_third_party][] which creates a
`SVTK_MODULE_USE_EXTERNAL_Namespace_Target` option for the module to switch
...
[svtk_module_build][] function. Arguments passed under the `INTERNAL` and
`EXTERNAL` arguments to this command are then passed on to
[svtk_module_third_party_internal][] or [svtk_module_third_party_external][],
respectively, depending on the `SVTK_MODULE_USE_EXTERNAL_Namespace_Target`
option.

Note that third party modules (marked as such by adding the `THIRD_PARTY`
keyword to a `svtk.module` file) may not be part of a kit, be wrapped, or
participate in autoinit.

[svtk_module_third_party]: @ref svtk_module_third_party
[svtk_module_third_party_internal]: @ref svtk_module_third_party_internal
[svtk_module_third_party_external]: @ref svtk_module_third_party_external
...
[svtk_module_find_package][] (except `PRIVATE`), information about the found
...
using the `_svtk_module_log` variable. This variable is a list of "domains" to
...
rather than exposing `_svtk_module_log` directly.

## Control variables

These variables do not follow the API convention and are used if set:

  - `_svtk_module_warnings`: If enabled, "strict" warnings are generated. These
    are not strictly problems, but may be used as linting for improving usage of
    the module system.
  - `_svtk_module_log`: A list of "domains" to output debugging information.
  - `_svtk_module_group_default_${group}`: used to set a non-`DEFAULT` default
    for group settings.

Some mechanisms use global properties instead:

  - `_svtk_module_autoinit_include`: The file that needs to be included in order
    to make the `SVTK_MODULE_AUTOINIT` symbol available for use in the
    [autoinit][] support.
