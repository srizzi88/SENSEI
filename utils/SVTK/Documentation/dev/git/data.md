...
This page documents how to add test data while developing SVTK with [Git][].
...
2.  Edit the corresponding `CMakeLists.txt` file:
    ...
    and add the test in a `svtk_add_test_...` call (which references
...
3.  For tests not using such a call, reference the data file in an
    ...
    using a path relative to the test directory:
        ...
        ExternalData_add_test("${_svtk_build_TEST_DATA_TARGET}"
          NAME ${_svtk_build_test}Cxx-MyTest
          COMMAND <SVTK_MODULE_NAME>CxxTests MyTest
...
Copy the data file into your local source tree.
    ...
    $ cp ../SVTK-build/Testing/Temporary/MyTest.png Some/Module/Testing/Data/Baseline
...
your build tree the `SVTKData` target must be built.  One may build the
target directly, e.g. `make SVTKData`, to obtain the data without a
complete build.  The output will be something like

    -- Fetching ".../ExternalData/SHA512/..."
    -- [download 100% complete]
    -- Downloaded object: "SVTK-build/ExternalData/Objects/SHA512/..."

The downloaded files appear in `SVTK-build/ExternalData` by default.
...
your local disk outside all build trees, e.g. `/home/user/.ExternalData`:
    ...
    $ cmake -DExternalData_OBJECT_STORES=/home/user/.ExternalData ../SVTK
...
A SVTK test data file is not stored in the main source tree under version
...
[DATA{} references](#add-test).  SVTK [sets](/CMake/svtkExternalData.cmake)
...
The script pushes the matching data objects to your SVTK GitLab fork.
For example:
...
for `refs/data/` in your SVTK GitLab fork, fetches them, erases the refs
from your fork, and uploads them to a location that we
[tell ExternalData to search](/CMake/svtkExternalData.cmake) at build time.
...
For SVTK, that location is currently
`http://www.svtk.org/files/ExternalData/SHA512/XXXX` where `XXXX` is the
...
single machine to contribute changes directly to upstream SVTK.  When
working in an external branch of SVTK, perhaps during a long-term topic
development effort, data objects need to be published separately.

The workflow for adding data to an external branch of SVTK is the same
...
directory at the top of the SVTK source tree by the
...
the data, e.g.
    ...
    cmake ../SVTK "-DExternalData_URL_TEMPLATES=https://username.github.io/SVTK/ExternalData/%(algo)/%(hash)

In this example we assume the files are published on a [Github Pages][]
`gh-pages` branch in `username`'s fork of SVTK.
