Develop SVTK with Git
====================

This page documents how to develop SVTK through [Git][].
...
Here we document procedures used by the SVTK development community.
...
5.  (Optional, but highly recommended.)
    [Register](https://open.cdash.org/register.php) with the SVTK project
    ...
    "Subscribe to this project" on the right of SVTK.
...
SVTK development uses a [branchy workflow][] based on topic branches.
...
[SVTK GitLab][] repository page and using the "Fork" button in the upper right.

[SVTK GitLab]: https://gitlab.kitware.com/svtk/svtk
...
It should be of the form:
    ...
    https://gitlab.kitware.com/<username>/svtk/merge_requests/new

Follow these steps:

1.  In the "**Source branch**" box select the `<username>/svtk` repository
    and the `my-topic` branch.

2.  In the "**Target branch**" box select the `svtk/svtk` repository and
...
page.  It is of the form:
    ...
    $ git fetch https://gitlab.kitware.com/$username/svtk.git $branch
...
SVTK has a [buildbot](http://buildbot.net) instance watching for merge requests
...
[SVTK project on open.cdash.org](https://open.cdash.org/index.php?project=SVTK).

Otherwise, `Expected`, `Superbuild`, or `Experimental` builds can be
directly accesssed from within Kitware at the following sites:

  * [svtk-expected](https://buildbot.kitware.com/builders?category=svtk-expected)
  * [svtk-superbuild](https://buildbot.kitware.com/builders?category=svtk-superbuild)
  * [svtk-experimental](https://buildbot.kitware.com/builders?category=svtk-experimental)
