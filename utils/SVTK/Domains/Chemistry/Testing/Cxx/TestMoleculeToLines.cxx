...
{
  char* fileName = svtkTestUtilities::ExpandDataFileName(argc, argv, "Data/porphyrin.cml");

  // read molecule from cml file
  svtkNew<svtkCMLMoleculeReader> reader;
  reader->SetFileName(fileName);
  reader->Update();
  delete[] fileName;
  svtkMolecule* molecule = reader->GetOutput();

  // convert
  svtkNew<svtkMoleculeToLinesFilter> converter;
  converter->SetInputConnection(reader->GetOutputPort());
  converter->Update();
  svtkPolyData* poly = converter->GetOutput();
