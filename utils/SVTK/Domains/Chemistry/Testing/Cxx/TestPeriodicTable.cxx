...
int TestPeriodicTable(int, char*[])
{
  ...
  {
    cout << "svtkPeriodicTable::GetAtomicNumber cannot parse numeric "
  ...
  {
    cout << "svtkPeriodicTable does not return 0 for invalid numeric strings. "
  ...
  {
    cout << "svtkPeriodicTable did not return 0 for an invalid string: "
  ...
  for (unsigned short i = 0; i <= pTab->GetNumberOfElements(); ++i)
  {
    ...
    if (pTab->GetAtomicNumber(name) != i)
    {
      cout << "Element name failed roundtrip: Name: \"" << name << "\" atomic number: " << i
           << "svtkPeriodicTable::GetAtomicNumber(\"" << name
    ...
    if (pTab->GetAtomicNumber(symbol) != i)
    {
      cout << "Element symbol failed roundtrip: Symbol: \"" << symbol << "\" atomic number: " << i
           << " svtkPeriodicTable::GetAtomicNumber(\"" << symbol
  ...
  // Test the svtkStdString variant.
  ...
  // Check color API.
  svtkColor3f color = pTab->GetDefaultRGBTuple(6);
  svtkColor3f expectedColor(0.5, 0.5, 0.5);
  ...
  if (!svtkMathUtilities::FuzzyCompare(radius, 0.82f, 0.01f))
  {
    cout << "Incorrect radius: " << setprecision(8) << radius << endl;
    ++errors;
  }
  radius = pTab->GetVDWRadius(56);
  if (!svtkMathUtilities::FuzzyCompare(radius, 2.7f, 0.01f))
  {
    cout << "Incorrect radius: " << setprecision(8) << radius << endl;
    ++errors;
  }
