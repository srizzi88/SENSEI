...
// svtkOpenGLMoleculeMapper. If you change this class please
// also check that class for impacts.
svtkObjectFactoryNewMacro(svtkMoleculeMapper);

//----------------------------------------------------------------------------
svtkMoleculeMapper::svtkMoleculeMapper()
  : RenderAtoms(true)
  ...
  , RenderLattice(true)
{
  ...
  svtkNew<svtkTransform> cylXform;
  cylXform->RotateWXYZ(90, 0.0, 0.0, 1.0);
  svtkNew<svtkTransformPolyDataFilter> cylXformFilter;
  ...
  this->AtomGlyphMapper->AddObserver(svtkCommand::StartEvent, cb);
  this->AtomGlyphMapper->AddObserver(svtkCommand::EndEvent, cb);
  this->AtomGlyphMapper->AddObserver(svtkCommand::ProgressEvent, cb);

  this->BondGlyphMapper->AddObserver(svtkCommand::StartEvent, cb);
  this->BondGlyphMapper->AddObserver(svtkCommand::EndEvent, cb);
  this->BondGlyphMapper->AddObserver(svtkCommand::ProgressEvent, cb);
  ...
  this->SetInputArrayToProcess(
    0, 0, 0, svtkDataObject::FIELD_ASSOCIATION_VERTICES, "Atomic Numbers");
}
...
//----------------------------------------------------------------------------
void svtkMoleculeMapper::SetInputData(svtkMolecule* input)
{
  this->SetInputDataInternal(0, input);
}

//----------------------------------------------------------------------------
svtkMolecule* svtkMoleculeMapper::GetInput()
{
  return svtkMolecule::SafeDownCast(this->GetExecutive()->GetInputData(0, 0));
}

//----------------------------------------------------------------------------
void svtkMoleculeMapper::UseBallAndStickSettings()
{
...
//----------------------------------------------------------------------------
void svtkMoleculeMapper::UseVDWSpheresSettings()
{
  this->SetRenderAtoms(true);
  this->SetRenderBonds(true);
  this->SetAtomicRadiusType(VDWRadius);
...
//----------------------------------------------------------------------------
void svtkMoleculeMapper::UseLiquoriceStickSettings()
{
  this->SetRenderAtoms(true);
  this->SetRenderBonds(true);
...
//----------------------------------------------------------------------------
void svtkMoleculeMapper::UseFastSettings()
{
  this->SetRenderAtoms(true);
  this->SetRenderBonds(true);
  this->SetAtomicRadiusType(UnitRadius);
...
//----------------------------------------------------------------------------
const char* svtkMoleculeMapper::GetAtomicRadiusTypeAsString()
{
...
//----------------------------------------------------------------------------
const char* svtkMoleculeMapper::GetBondColorModeAsString()
{
...
//----------------------------------------------------------------------------
void svtkMoleculeMapper::GetSelectedAtomsAndBonds(
  svtkSelection* selection, svtkIdTypeArray* atomIds, svtkIdTypeArray* bondIds)
{
  ...
  const svtkIdType numAtoms = this->GetInput()->GetNumberOfAtoms();
  const svtkIdType numBonds = this->GetInput()->GetNumberOfBonds();
  const svtkIdType numAtomsAndBonds = numAtoms + numBonds;
...
//----------------------------------------------------------------------------
void svtkMoleculeMapper::Render(svtkRenderer* ren, svtkActor* act)
{
...
//----------------------------------------------------------------------------
void svtkMoleculeMapper::GlyphRender(svtkRenderer* ren, svtkActor* act)
{
...
//----------------------------------------------------------------------------
void svtkMoleculeMapper::UpdateGlyphPolyData()
{
  svtkMolecule* molecule = this->GetInput();
...
void svtkMoleculeMapper::UpdateAtomGlyphPolyData()
{
  this->AtomGlyphPolyData->Initialize();

  svtkMolecule* molecule = this->GetInput();

  svtkAbstractArray* inputColorArray = this->GetInputAbstractArrayToProcess(0, molecule);
  svtkAbstractArray* colorArray = nullptr;
  svtkUnsignedCharArray* singleColorArray = nullptr;
  switch (this->AtomColorMode)
  {
    case SingleColor:
    {
      colorArray = svtkUnsignedCharArray::New();
      colorArray->SetNumberOfComponents(3);
      svtkIdType size = 3 * molecule->GetNumberOfAtoms();
      colorArray->Allocate(size);
      colorArray->SetName("Colors");
      singleColorArray = svtkArrayDownCast<svtkUnsignedCharArray>(colorArray);
  ...
  svtkNew<svtkUnsignedShortArray> atomicNbWithoutGhostArray;
  svtkUnsignedShortArray* atomicNbFullArray = molecule->GetAtomicNumberArray();
  svtkNew<svtkPoints> points;
  svtkPoints* allPoints = molecule->GetAtomicPositionArray();
  svtkUnsignedCharArray* ghosts = molecule->GetAtomGhostArray();
  ...
  const svtkIdType numAtoms = points->GetNumberOfPoints();

  if (colorArray != nullptr)
  {
    if (colorArray->GetNumberOfTuples() != numAtoms)
    {
      svtkErrorMacro("Color array size does not match number of atoms.");
    }
  ...
  svtkNew<svtkFloatArray> scaleFactors;
  ...
  {
    default:
      svtkWarningMacro(<< "Unknown radius type: " << this->AtomicRadiusType
                      << ". Falling back to 'VDWRadius' (" << VDWRadius << ").");
      SVTK_FALLTHROUGH;
    case VDWRadius:
      for (svtkIdType i = 0; i < numAtoms; ++i)
      {
    ...
    case CustomArrayRadius:
    {
      svtkDataArray* allRadii = molecule->GetVertexData()->GetArray(this->AtomicRadiusArrayName);
      if (!allRadii)
      {
        svtkWarningMacro("AtomicRadiusType set to CustomArrayRadius, but no "
                        "array named "
      ...
      {
        svtkWarningMacro("'radii' array contains "
...
void svtkMoleculeMapper::UpdateBondGlyphPolyData()
{
  this->BondGlyphPolyData->Initialize();

  svtkMolecule* molecule = this->GetInput();
  const svtkIdType numBonds = molecule->GetNumberOfBonds();

  // For selection ID offset:
  const svtkIdType numAtoms = molecule->GetNumberOfAtoms();

  // Create arrays
  svtkNew<svtkPoints> cylCenters;
  svtkNew<svtkFloatArray> cylScales;
  svtkNew<svtkFloatArray> orientationVectors;
  // Since svtkHardwareSelector won't distinguish between the internal instance
  // mappers of svtkMoleculeMapper, use a custom selection ID range. This also
  // fixes the issue of bonds that are colored-by-atom, as these are rendered
  // as two glyphs.
  svtkNew<svtkIdTypeArray> selectionIds;

  // Setup arrays
  // svtkPoints uses three components by default
  ...
  svtkIdType numCylinders = numBonds;
  ...
  svtkAbstractArray* atomColorArray = this->GetInputAbstractArrayToProcess(0, molecule);
  svtkUnsignedCharArray* singleColorArray = nullptr;
  svtkAbstractArray* cylColors = nullptr;
  ...
  {
    singleColorArray = svtkUnsignedCharArray::New();
  ...
  for (svtkIdType bondInd = 0; bondInd < numBonds; ++bondInd)
  {
    ...
    // Extract bond info
    svtkBond bond = molecule->GetBond(bondInd);
    ...
    // TODO svtkVector scalar multiplication
    ...
    if (this->UseMultiCylindersForBonds)
    {
      switch (bondOrder)
      {
        ...
        case 2:
          ...
          break;
        case 3:
          // TODO svtkVector scalar multiplication, negation
          // delta = bondVec.Cross(unitZ).Normalized() * deltaLength;
          // initialDisp = -delta;
          delta = bondVec.Cross(unitZ).Normalized();
          delta[0] *= deltaLength;
    ...
    for (unsigned short iter = 0; iter < bondOrder; ++iter)
    {
      ...
      switch (this->BondColorMode)
      {
        ...
        case DiscreteByAtom:
          ...
          // TODO svtkVector subtraction, scalar multiplication
          ...
          // Add cylinder for begin atom:
          // TODO svtkVector addition, scalar multiplication
          // halfCylinderCenter = cylinderCenter + (bondVec * quarterLength);
          halfCylinderCenter[0] = cylinderCenter[0] + (bondVec[0] * quarterLength);
          halfCylinderCenter[1] = cylinderCenter[1] + (bondVec[1] * quarterLength);
          halfCylinderCenter[2] = cylinderCenter[2] + (bondVec[2] * quarterLength);
      ...
      {
        // TODO svtkVector in-place addition
...
//----------------------------------------------------------------------------
void svtkMoleculeMapper::UpdateLatticePolyData()
{
  ...
  svtkNew<svtkUnsignedCharArray> latticeColors;
...
//----------------------------------------------------------------------------
void svtkMoleculeMapper::ReleaseGraphicsResources(svtkWindow* w)
{
  this->AtomGlyphMapper->ReleaseGraphicsResources(w);
  this->BondGlyphMapper->ReleaseGraphicsResources(w);
}

double* svtkMoleculeMapper::GetBounds()
{
  svtkMolecule* input = this->GetInput();
  if (!input)
  {
...
//----------------------------------------------------------------------------
int svtkMoleculeMapper::FillInputPortInformation(int svtkNotUsed(port), svtkInformation* info)
{
...
//----------------------------------------------------------------------------
void svtkMoleculeMapper::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
void svtkMoleculeMapper::SetMapScalars(bool map)
{
