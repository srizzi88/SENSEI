/*=========================================================================
  ...
  Module:    svtkVASPTessellationReader.cxx
...
svtkStandardNewMacro(svtkVASPTessellationReader);
...
//------------------------------------------------------------------------------
void svtkVASPTessellationReader::PrintSelf(std::ostream& os, svtkIndent indent)
{
  this->Superclass::PrintSelf(os, indent);
}

//------------------------------------------------------------------------------
svtkVASPTessellationReader::svtkVASPTessellationReader()
  : FileName(nullptr)
...
//------------------------------------------------------------------------------
int svtkVASPTessellationReader::RequestData(
  svtkInformation*, svtkInformationVector**, svtkInformationVector* outInfos)
{
  svtkInformation* outInfo0 = outInfos->GetInformationObject(0);
  svtkInformation* outInfo1 = outInfos->GetInformationObject(1);

  svtkMolecule* molecule = svtkMolecule::SafeDownCast(outInfo0->Get(svtkDataObject::DATA_OBJECT()));
  assert(molecule);

  svtkUnstructuredGrid* voronoi =
    svtkUnstructuredGrid::SafeDownCast(outInfo1->Get(svtkDataObject::DATA_OBJECT()));
  assert(voronoi);
  ...
  {
    molecule->GetInformation()->Set(svtkDataObject::DATA_TIME_STEP(), time);
    voronoi->GetInformation()->Set(svtkDataObject::DATA_TIME_STEP(), time);
  }
  else
  {
    molecule->Initialize();
...
//------------------------------------------------------------------------------
int svtkVASPTessellationReader::RequestInformation(
  svtkInformation*, svtkInformationVector**, svtkInformationVector* outInfos)
{
  svtksys::ifstream in(this->FileName);
  if (!in)
  ...
  if (!times.empty())
  {
    ...
    {
      svtkInformation* outInfo = outInfos->GetInformationObject(port);
      outInfo->Set(svtkSDDP::TIME_RANGE(), timeRange, 2);
      outInfo->Set(svtkSDDP::TIME_STEPS(), &times[0], static_cast<int>(times.size()));
    }
  }
...
//------------------------------------------------------------------------------
int svtkVASPTessellationReader::FillOutputPortInformation(int port, svtkInformation* info)
{
  switch (port)
  {
    case 0:
...
//------------------------------------------------------------------------------
bool svtkVASPTessellationReader::NextTimeStep(std::istream& in, double& time)
{
  std::string line;
...
//------------------------------------------------------------------------------
size_t svtkVASPTessellationReader::SelectTimeStepIndex(svtkInformation* info)
{
  if (!info->Has(svtkSDDP::TIME_STEPS()) || !info->Has(svtkSDDP::UPDATE_TIME_STEP()))
  {
    return 0;
  }

  double* times = info->Get(svtkSDDP::TIME_STEPS());
  int nTimes = info->Length(svtkSDDP::TIME_STEPS());
  double t = info->Get(svtkSDDP::UPDATE_TIME_STEP());
...
//------------------------------------------------------------------------------
bool svtkVASPTessellationReader::ReadTimeStep(
  std::istream& in, svtkMolecule* molecule, svtkUnstructuredGrid* voronoi)
{
  ...
  {
    svtkErrorMacro("Unexpected EOF while reading lattice info.");
    return false;
  }
  if (!this->LatticeParser->find(line))
  {
    svtkErrorMacro("Error parsing lattice info from line: " << line);
    return false;
  }
  ...
  if (!std::getline(in, line))
  {
    svtkErrorMacro("Unexpected EOF while parsing number of atoms.");
    return false;
  }
  if (!this->AtomCountParser->find(line))
  {
    svtkErrorMacro("Error parsing number of atoms from line: " << line);
    return false;
  }
  svtkIdType nAtoms;
  if (!parse(this->AtomCountParser->match(1), nAtoms))
  {
    svtkErrorMacro("Error parsing number atoms '" << this->AtomCountParser->match(1)
                                                 << "'' from line: " << line);
    return false;
  }
  ...
  if (!std::getline(in, line))
  {
    svtkErrorMacro("Unexpected EOF while reading atomic number list.");
    return false;
  }
  if (!parseCommaSepList(line, atomicNumbers))
  {
    svtkErrorMacro("Error while parsing atomic number list: " << line);
    return false;
  }
  ...
  {
    svtkErrorMacro("Error: expected " << nAtoms
                                     << " atomic numbers, but only "
                                        "parsed "
                                     << atomicNumbers.size());
    return false;
  }
  svtkVector3f pos(0.f);
  ...
  svtkNew<svtkUnsignedShortArray> tessAtomicNumbers;
  tessAtomicNumbers->SetName("Atomic Numbers");
  tessAtomicNumbers->Allocate(nAtoms);
  svtkNew<svtkIdTypeArray> tessAtomIds;
  ...
  std::set<svtkIdType> uniquePointIds;
  // parse as doubles for locator API, but store as floats:
  std::vector<std::vector<double> > pointData;
  std::vector<std::vector<svtkIdType> > faceData;

  for (svtkIdType atomEntry = 0; atomEntry < nAtoms; ++atomEntry)
  {
    ...
    while (line.empty() || line.find_first_not_of(" \t") == std::string::npos)
    {
      if (!std::getline(in, line))
      {
        svtkErrorMacro("Unexpected EOF while reading atom entry " << atomEntry);
        return false;
      }
    }
    ...
    {
      svtkErrorMacro("Error parsing atom position/radius specification: " << line);
      return false;
    }
    svtkIdType atomId;
    ...
    if (atomId >= nAtoms)
    {
      svtkErrorMacro("Found entry for atom with id " << atomId
                                                    << ", but "
                                                       "only "
                                                    << nAtoms << " atoms exist.");
      return false;
    }
    svtkAtom atom = molecule->GetAtom(atomId);
    ...
    if (!std::getline(in, line))
    {
      svtkErrorMacro("Unexpected EOF while reading voronoi points for atom " << atomId);
      return false;
    }
    ...
    if (!std::getline(in, line))
    {
      svtkErrorMacro("Unexpected EOF while reading voronoi faces for atom " << atomId);
      return false;
    }
    ...
    for (size_t i = 0; i < pointData.size(); ++i)
    {
      ...
      if (p.size() != 3)
      {
        svtkErrorMacro("Error: Tessellation point " << i << " for atom " << atomId << " has "
                                                   << p.size()
                                                   << " components. "
    ...
    {
      const std::vector<svtkIdType>& face = faceData[faceId];
      faceStream.push_back(static_cast<svtkIdType>(face.size()));
      for (std::vector<svtkIdType>::const_iterator it = face.begin(), itEnd = face.end();
           it != itEnd; ++it)
      {
        // Convert the local point id into the dataset point id:
        svtkIdType datasetId = pointIds[*it];
    ...
    voronoi->InsertNextCell(SVTK_POLYHEDRON, static_cast<svtkIdType>(pointIds.size()),
      pointIds.empty() ? nullptr : &pointIds[0], static_cast<svtkIdType>(faceData.size()),
