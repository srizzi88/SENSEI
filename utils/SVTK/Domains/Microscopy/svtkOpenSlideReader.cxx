/*=========================================================================
  ...
  Module:    svtkOpenSlideReader.cxx
...
svtkStandardNewMacro(svtkOpenSlideReader);

void svtkOpenSlideReader::ExecuteInformation()
{
  ...
  {
    svtkErrorWithObjectMacro(this, "File could not be opened by openslide");
    return;
  }
...
void svtkOpenSlideReader::ExecuteDataWithInformation(svtkDataObject* output, svtkInformation* outInfo)
{
  int inExtent[6];

  svtkStreamingDemandDrivenPipeline::GetUpdateExtent(outInfo, inExtent);

  svtkImageData* data = this->AllocateOutputData(output, outInfo);

  if (this->openslide_handle == nullptr)
  {
    svtkErrorWithObjectMacro(this, "File could not be read by openslide");
    return;
  }
  ...
  // SVTK extents have origin at top left and y axis looking downwards
...
//----------------------------------------------------------------------------
int svtkOpenSlideReader::CanReadFile(const char* fname)
{
...
svtkOpenSlideReader::~svtkOpenSlideReader()
{
...
//----------------------------------------------------------------------------
void svtkOpenSlideReader::PrintSelf(ostream& os, svtkIndent indent)
{
