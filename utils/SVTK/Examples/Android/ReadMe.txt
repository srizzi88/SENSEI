Welcome to the android examples for SVTK. Currently the android support is at
a very early stage but hopefully these examples can give you a head start on
developing your own android applications.

Currently there are three examples here.  NativeSVTK is built around the
...
SVTK widgets or other OpenGL elements. This is best for applications that
...
look very much like the regular SVTK code you are used to.

The second example is JavaSVTK.  In this example the OpenGL context is
...
is handled by SVTK.  In the JavaSVTK example we forward keyboard and
motion events to a SVTK interactor for processing but you could choose
...
jni/main.cxx while the Java/C++ interface API is defined in JavaSVTKLib.java
The bulk of the actual application logic is in JavaSVTKView.java
...
To build SVTK and these examples follow the steps below.
...
* Create a binary directory for SVTK. Typically I will create svtkandroid next to my svtk source tree.

* Run cmake on svtkandroid with -DSVTK_ANDROID_BUILD=ON, if you use the gui add a
...
That should build SVTK and the android examples. To try running an example you
...
cd into CMakeExternals/Build/svtk-android/Examples/Android/ExampleName/bin
