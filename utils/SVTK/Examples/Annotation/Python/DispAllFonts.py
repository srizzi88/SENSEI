...
if sys.hexversion < 0x03000000:
    ...
    # for Python3
    import tkinter
from svtk.tk.svtkTkRenderWindowInteractor import \
     svtkTkRenderWindowInteractor
import string
...
# svtkTkRenderWindowInteractor is a Tk widget that we can render into.
# It has a GetRenderWindow method that returns a svtkRenderWindow.
# This can then be used to create a svtkRenderer and etc. We can also
# specify a svtkRenderWindow to be used when creating the widget by
...
svtkw = svtkTkRenderWindowInteractor(root, rw=renWin, width=800)


# Once the SVTK widget has been created it can be inserted into a whole
...
# update the corresponding SVTK objects by calling the SetFontSize
...
# propagate this GUI setting to the corresponding SVTK object.
...
# Finally we pack the SVTK widget and the sliders on top of each other
# (side='top') inside the main root widget.
svtkw.Initialize()
size_slider.pack(side="top", fill="both")
svtkw.pack(side="top", fill='both', expand=1)
