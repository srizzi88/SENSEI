...
- (void)initializeSVTKSupport
{
  ...
  if (ren && cocoaRenWin && renWinInt)
  {
    ...
    // instruct svtk to not associate the NSOpenGLContext it creates with our NSView.
...
{
  svtkRenderer* ren = [self renderer];
  svtkRenderWindow* renWin = [self renderWindow];
  svtkRenderWindowInteractor* renWinInt = [self renderWindowInteractor];
...
{
  svtkCocoaRenderWindow* renderWindow = [self renderWindow];
  if (renderWindow)
  {
    // DPI has been hardcoded to 72 but in order to cater for text rendering discrepancies with
    // svtkTextActors for instance we must adjust the scaling factor per NSWindow.
