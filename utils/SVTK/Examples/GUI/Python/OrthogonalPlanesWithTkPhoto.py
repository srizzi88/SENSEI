...
if sys.hexversion < 0x03000000:
    ...
    from tkinter import *
import svtk.tk
import svtk.tk.svtkLoadPythonTkWidgets
import svtk.tk.svtkTkImageViewerWidget
from svtk.tk.svtkTkPhotoImage import *
from svtk.util.misc import *


class SampleViewer:
    def __init__ ( self ):
        self.Tk = Tk = tkinter.Tk();
        Tk.title ( 'Python Version of svtkImageDataToTkPhoto' );

        # Image pipeline
        reader = svtkVolume16Reader ()
        reader.SetDataDimensions ( 64, 64 )
        reader.SetDataByteOrderToLittleEndian ( )
        reader.SetFilePrefix ( svtkGetDataRoot() + '/Data/headsq/quarter'  )
        reader.SetImageRange ( 1, 93 )
        reader.SetDataSpacing ( 3.2, 3.2, 1.5 )
        reader.Update ()

        self.cast = cast = svtkImageCast()
        cast.SetInputConnection( reader.GetOutputPort() )
        cast.SetOutputScalarType ( reader.GetOutput().GetScalarType() )
        cast.ClampOverflowOn()

        # Make the image a little bigger
        self.resample = resample = svtkImageResample ()
        ...
        tphoto = self.tphoto = self.tphoto = svtkTkPhotoImage ();
        cphoto = self.cphoto = svtkTkPhotoImage ();
        sphoto = self.sphoto = svtkTkPhotoImage ();
        reader.Update()
