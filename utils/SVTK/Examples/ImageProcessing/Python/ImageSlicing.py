...
coronal = svtk.svtkMatrix4x4()
coronal.DeepCopy((1, 0, 0, center[0],
                  0, 0, 1, center[1],
                  0,-1, 0, center[2],
                  0, 0, 0, 1))

sagittal = svtk.svtkMatrix4x4()
sagittal.DeepCopy((0, 0,-1, center[0],
                   1, 0, 0, center[1],
                   0,-1, 0, center[2],
                   0, 0, 0, 1))

oblique = svtk.svtkMatrix4x4()
...
color = svtk.svtkImageMapToColors()
color.SetLookupTable(table)
color.SetInputConnection(reslice.GetOutputPort())

# Display the image
actor = svtk.svtkImageActor()
actor.GetMapper().SetInputConnection(color.GetOutputPort())

renderer = svtk.svtkRenderer()
renderer.AddActor(actor)

window = svtk.svtkRenderWindow()
window.AddRenderer(renderer)

# Set up the interaction
interactorStyle = svtk.svtkInteractorStyleImage()
interactor = svtk.svtkRenderWindowInteractor()
