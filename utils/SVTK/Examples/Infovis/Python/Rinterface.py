...
# Python example script that uses the svtkRIinterface to create an instance of the
# R interpreter and pass it some data, modify the data in R, and pass the result
# back to SVTK.

# SVTK must be built with SVTK_USE_GNU_R turned on for this example to work!
...
if __name__ == "__main__":
  ...
  # Create an instance of the R interpreter.  Note, rinterface is not a SVTK pipeline object.
  rinterface = svtkRInterface()

  # Create an array of 10 doubles in SVTK and fill it with some data
  darray = svtkDoubleArray()
  for d in range(0, 10):
  ...
  rinterface.AssignSVTKDataArrayToRVariable(darray, "d")
  ...
  # Copy matrix d from R back to SVTK as an array of doubles in bdarray.
  bdarray = rinterface.AssignRVariableToSVTKDataArray("d")

  # Display the contents of bdarray.
  print("\n\nContents of bdarray copied to SVTK from R\n\n")
