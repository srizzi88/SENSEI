...
class svtkBoxWidgetCallback : public svtkCommand
{
public:
  static svtkBoxWidgetCallback* New() { return new svtkBoxWidgetCallback; }
  void Execute(svtkObject* caller, unsigned long, void*) override
  {
    svtkBoxWidget* widget = reinterpret_cast<svtkBoxWidget*>(caller);
    if (this->Mapper)
    {
      svtkPlanes* planes = svtkPlanes::New();
      widget->GetPlanes(planes);
      this->Mapper->SetClippingPlanes(planes);
      planes->Delete();
    }
  }
  void SetMapper(svtkSmartVolumeMapper* m) { this->Mapper = m; }

protected:
  svtkBoxWidgetCallback() { this->Mapper = nullptr; }

  svtkSmartVolumeMapper* Mapper;
};
...
int main(int argc, char* argv[])
{
  ...
  // SetDesiredUpdateRate - the svtkRenderWindow divides it
  ...
  // Read the data
  svtkAlgorithm* reader = nullptr;
  svtkImageData* input = nullptr;
  if (dirname)
  {
    svtkDICOMImageReader* dicomReader = svtkDICOMImageReader::New();
  ...
  {
    svtkXMLImageDataReader* xmlReader = svtkXMLImageDataReader::New();
  ...
  {
    svtkMetaImageReader* metaReader = svtkMetaImageReader::New();
  ...
  svtkBoxWidget* box = svtkBoxWidget::New();
  if (clip)
  {
    ...
    box->InsideOutOn();
    box->PlaceWidget();
    svtkBoxWidgetCallback* callback = svtkBoxWidgetCallback::New();
    callback->SetMapper(mapper);
    box->AddObserver(svtkCommand::InteractionEvent, callback);
    callback->Delete();
    box->EnabledOn();
  ...
  switch (blendType)
  {
    ...
    case 6:
      ...
      break;
    default:
      svtkGenericWarningMacro("Unknown blend type.");
      break;
  }
