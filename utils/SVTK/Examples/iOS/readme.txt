The iOS build of SVTK is in a beta state. Please do try it out but understand
it is very new.

To build a SVTK application for iOS you will need a Mac/Macbook that has Xcode on
it and the iOS dev kit. What follows are the basic steps:

* Make a directory for the host binaries (e.g. svtkios). I typically create this
* next to the SVTK source tree.

The end result of your SVTK build will be a svtk framework that includes header
...
directory in svtkios ala cd svtkios; mkdir install

* Run cmake on svtkios with -DSVTK_IOS_BUILD=ON, if you use the gui add a
* boolean entry with that name prior to configuring and set it on.

* Change the SVTK_INSTALL_PREFIX to where you want the framework written.
...
<yourBinDir>/CMakeExternals/Build/svtk-ios-device-armv7/Examples/iOS

* likely need to update a couple setting in the project to find the svtk fraework
...
* If you built SVTK to run on the actual device, make sure you have all
