...
//------------------------------------------------------------------------------
svtkUniformGrid* GetGrid(double origin[3], double h[3], int ndims[3])
{
...
//------------------------------------------------------------------------------
svtkOverlappingAMR* GetAMRDataSet(const int description)
{
  ...
  svtkOverlappingAMR* amrDataSet = svtkOverlappingAMR::New();
  ...
  svtkUniformGrid* gridPtr = nullptr;
  switch (description)
  {
    case SVTK_XY_PLANE:
      // Root block
      ...
      box = svtkAMRBox(origin, ndims, spacing, amrDataSet->GetOrigin(), description);
  ...
  svtkAMRUtilities::BlankCells(amrDataSet);
  return (amrDataSet);
}
...
{
  int rc = 0;
  svtkOverlappingAMR* myAMR = GetAMRDataSet(dataDescription);
  assert("pre: AMR dataset is nullptr" && (myAMR != nullptr));

  svtkUniformGrid* root = myAMR->GetDataSet(0, 0);
...
int TestAMRBlanking(int, char*[])
{
  ...
  // XYZ test
  status = AMRVisibilityTests::TestAMRVisibility(SVTK_XYZ_GRID);
  AMRVisibilityTests::CheckTestStatus(status, "TestAMRVisibility-SVTK_XYZ_GRID");
  rc += status;

  // XY PLANE
  status = AMRVisibilityTests::TestAMRVisibility(SVTK_XY_PLANE);
  AMRVisibilityTests::CheckTestStatus(status, "TestAMRVisibility-SVTK_XY_PLANE");
  rc += status;

  // XZ PLANE
  status = AMRVisibilityTests::TestAMRVisibility(SVTK_XZ_PLANE);
  AMRVisibilityTests::CheckTestStatus(status, "TestAMRVisibility-SVTK_XZ_PLANE");
  rc += status;

  // YZ PLANE
  status = AMRVisibilityTests::TestAMRVisibility(SVTK_YZ_PLANE);
  AMRVisibilityTests::CheckTestStatus(status, "TestAMRVisibility-SVTK_YZ_PLANE");
  rc += status;
  return (rc);
}
