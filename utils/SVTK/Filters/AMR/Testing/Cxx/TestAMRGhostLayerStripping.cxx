...
// configuration using the svtkAMRGaussianPulseSource which has a known structure.
...
//------------------------------------------------------------------------------
// Debugging utilities. Must link svtkIOXML to work
#ifdef DEBUG_ON
...
//------------------------------------------------------------------------------
void WriteUnGhostedGrids(const int dimension, svtkOverlappingAMR* amr)
{
...
void ComputeCellCenter(svtkUniformGrid* grid, svtkIdType cellIdx, double centroid[3])
{
  ...
  svtkCell* myCell = grid->svtkImageData::GetCell(cellIdx);
...
//------------------------------------------------------------------------------
void GeneratePulseField(const int dimension, svtkUniformGrid* grid)
{
...
//------------------------------------------------------------------------------
svtkUniformGrid* GetGhostedGrid(
  const int dimension, svtkUniformGrid* refGrid, int ghost[6], const int NG)
{
...
//------------------------------------------------------------------------------
svtkOverlappingAMR* GetGhostedDataSet(const int dimension, const int NG, svtkOverlappingAMR* inputAMR)
{
  svtkOverlappingAMR* ghostedAMR = svtkOverlappingAMR::New();
  ...
  // Copy the root grid
  svtkUniformGrid* rootGrid = svtkUniformGrid::New();
  rootGrid->DeepCopy(inputAMR->GetDataSet(0, 0));
  svtkAMRBox box(rootGrid->GetOrigin(), rootGrid->GetDimensions(), rootGrid->GetSpacing(),
    ghostedAMR->GetOrigin(), rootGrid->GetGridDescription());
  ghostedAMR->SetAMRBox(0, 0, box);
  ghostedAMR->SetDataSet(0, 0, rootGrid);
  rootGrid->Delete();

  // Knowing the AMR configuration returned by svtkAMRGaussingPulseSource
  ...
  for (int i = 0; i < 2; ++i)
  {
    svtkUniformGrid* grid = inputAMR->GetDataSet(1, i);
    svtkUniformGrid* ghostedGrid = GetGhostedGrid(dimension, grid, ghost[i], NG);
    box = svtkAMRBox(ghostedGrid->GetOrigin(), ghostedGrid->GetDimensions(),
...
//------------------------------------------------------------------------------
svtkOverlappingAMR* GetAMRDataSet(const int dimension, const int refinementRatio)
{
  svtkAMRGaussianPulseSource* amrGPSource = svtkAMRGaussianPulseSource::New();
  amrGPSource->SetDimension(dimension);
  amrGPSource->SetRefinementRatio(refinementRatio);
  amrGPSource->Update();
  svtkOverlappingAMR* myAMR = svtkOverlappingAMR::New();
...
bool CheckFields(svtkUniformGrid* grid)
{
  ...
  svtkDoubleArray* pulseArray = svtkArrayDownCast<svtkDoubleArray>(CD->GetArray("Gaussian-Pulse"));
  ...
  for (; cellIdx < grid->GetNumberOfCells(); ++cellIdx)
  {
    ...
    if (!svtkMathUtilities::FuzzyCompare(val, pulses[cellIdx], 1e-9))
    {
    ...
    if (!svtkMathUtilities::FuzzyCompare(centroid[0], centers[cellIdx * 3]) ||
      !svtkMathUtilities::FuzzyCompare(centroid[1], centers[cellIdx * 3 + 1]) ||
      !svtkMathUtilities::FuzzyCompare(centroid[2], centers[cellIdx * 3 + 2]))
    {
...
//------------------------------------------------------------------------------
bool AMRDataSetsAreEqual(svtkOverlappingAMR* computed, svtkOverlappingAMR* expected)
{
  ...
  for (; levelIdx < computed->GetNumberOfLevels(); ++levelIdx)
  {
    ...
    {
      svtkUniformGrid* computedGrid = computed->GetDataSet(levelIdx, dataIdx);
      svtkUniformGrid* expectedGrid = expected->GetDataSet(levelIdx, dataIdx);
...
int TestGhostStripping(const int dimension, const int refinementRatio, const int NG)
{
  ...
  svtkOverlappingAMR* amrData = GetAMRDataSet(dimension, refinementRatio);
  assert("pre: amrData should not be nullptr!" && (amrData != nullptr));
  if (svtkAMRUtilities::HasPartiallyOverlappingGhostCells(amrData))
  {
    ++rc;
  ...
  svtkOverlappingAMR* ghostedAMRData = GetGhostedDataSet(dimension, NG, amrData);
  ...
  if (NG == refinementRatio)
  {
    ...
    if (svtkAMRUtilities::HasPartiallyOverlappingGhostCells(ghostedAMRData))
    {
      ++rc;
  ...
  svtkOverlappingAMR* strippedAMRData = svtkOverlappingAMR::New();
...
//------------------------------------------------------------------------------
int TestAMRGhostLayerStripping(int svtkNotUsed(argc), char* svtkNotUsed(argv)[])
{
  int rc = 0;
  int DIM0 = 2;
  int NDIM = 3;
