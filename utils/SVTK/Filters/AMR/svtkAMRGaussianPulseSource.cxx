/*=========================================================================
 ...
 Module:    svtkAMRGaussianPulseSource.cxx
...
svtkStandardNewMacro(svtkAMRGaussianPulseSource);

//------------------------------------------------------------------------------
svtkAMRGaussianPulseSource::svtkAMRGaussianPulseSource()
{
...
//------------------------------------------------------------------------------
svtkAMRGaussianPulseSource::~svtkAMRGaussianPulseSource() = default;

//------------------------------------------------------------------------------
void svtkAMRGaussianPulseSource::PrintSelf(ostream& os, svtkIndent indent)
{
  this->Superclass::PrintSelf(os, indent);
}

//------------------------------------------------------------------------------
void svtkAMRGaussianPulseSource::ComputeCellCenter(
  svtkUniformGrid* grid, svtkIdType cellIdx, double centroid[3])
{
...
//------------------------------------------------------------------------------
void svtkAMRGaussianPulseSource::GeneratePulseField(svtkUniformGrid* grid)
{
...
//------------------------------------------------------------------------------
svtkUniformGrid* svtkAMRGaussianPulseSource::RefinePatch(svtkUniformGrid* parent, int patchExtent[6])
{
  ...
  assert("pre: patchExtent must be within the parent extent!" &&
    svtkStructuredExtent::Smaller(patchExtent, ext));
  ...
  svtkIdType minIdx = svtkStructuredData::ComputePointIdForExtent(ext, minIJK);
  ...
  svtkIdType maxIdx = svtkStructuredData::ComputePointIdForExtent(ext, maxIJK);
...
//------------------------------------------------------------------------------
svtkUniformGrid* svtkAMRGaussianPulseSource::GetGrid(double origin[3], double h[3], int ndim[3])
{
  svtkUniformGrid* grid = svtkUniformGrid::New();
  grid->Initialize();
...
//------------------------------------------------------------------------------
void svtkAMRGaussianPulseSource::Generate2DDataSet(svtkOverlappingAMR* amr)
{
  ...
  svtkUniformGrid* grid = this->GetGrid(origin, h, ndim);
  svtkAMRBox box(grid->GetOrigin(), grid->GetDimensions(), grid->GetSpacing(), origin,
  ...
  svtkUniformGrid* refinedPatch = nullptr;
  for (int patchIdx = 0; patchIdx < 2; ++patchIdx)
  {
    ...
    box = svtkAMRBox(refinedPatch->GetOrigin(), refinedPatch->GetDimensions(),
...
//------------------------------------------------------------------------------
void svtkAMRGaussianPulseSource::Generate3DDataSet(svtkOverlappingAMR* amr)
{
  assert("pre: input AMR dataset is nullptr" && (amr != nullptr));
...
//------------------------------------------------------------------------------
int svtkAMRGaussianPulseSource::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** svtkNotUsed(inputVector), svtkInformationVector* outputVector)
{
  svtkInformation* info = outputVector->GetInformationObject(0);
  svtkOverlappingAMR* output =
    svtkOverlappingAMR::SafeDownCast(info->Get(svtkDataObject::DATA_OBJECT()));
  ...
  switch (this->Dimension)
  {
    ...
    case 3:
      ...
      break;
    default:
      svtkErrorMacro("Dimensions must be either 2 or 3!");
  }

  svtkAMRUtilities::BlankCells(output);
  return 1;
}
