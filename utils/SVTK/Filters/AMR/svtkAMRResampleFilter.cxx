/*=========================================================================
 ...
 Module:    svtkAMRToUniformGrid.cxx
...
svtkStandardNewMacro(svtkAMRResampleFilter);

//-----------------------------------------------------------------------------
svtkAMRResampleFilter::svtkAMRResampleFilter()
{
  ...
  this->ROI = svtkMultiBlockDataSet::New();
...
//-----------------------------------------------------------------------------
void svtkAMRResampleFilter::PrintSelf(std::ostream& oss, svtkIndent indent)
{
  this->Superclass::PrintSelf(oss, indent);
}

//-----------------------------------------------------------------------------
int svtkAMRResampleFilter::FillInputPortInformation(int svtkNotUsed(port), svtkInformation* info)
{
...
//-----------------------------------------------------------------------------
int svtkAMRResampleFilter::FillOutputPortInformation(int svtkNotUsed(port), svtkInformation* info)
{
  assert("pre: information object is nullptr" && (info != nullptr));
...
//-----------------------------------------------------------------------------
int svtkAMRResampleFilter::RequestUpdateExtent(svtkInformation*, svtkInformationVector** inputVector,
  svtkInformationVector* svtkNotUsed(outputVector))
{
  ...
  if (this->DemandDrivenMode == 1)
  {
    ...
    info->Set(svtkCompositeDataPipeline::LOAD_REQUESTED_BLOCKS(), 1);

    // Tell reader which blocks this process requires
    info->Set(svtkCompositeDataPipeline::UPDATE_COMPOSITE_INDICES(), &this->BlocksToLoad[0],
...
//-----------------------------------------------------------------------------
int svtkAMRResampleFilter::RequestInformation(svtkInformation* svtkNotUsed(rqst),
  svtkInformationVector** inputVector, svtkInformationVector* svtkNotUsed(outputVector))
{
  ...
  if (this->DemandDrivenMode == 1 &&
    input->Has(svtkCompositeDataPipeline::COMPOSITE_DATA_META_DATA()))
  {
    //    this->AMRMetaData = svtkOverlappingAMR::New();
    this->AMRMetaData = svtkOverlappingAMR::SafeDownCast(
...
//-----------------------------------------------------------------------------
int svtkAMRResampleFilter::RequestData(svtkInformation* svtkNotUsed(rqst),
...
//-----------------------------------------------------------------------------
bool svtkAMRResampleFilter::FoundDonor(double q[3], svtkUniformGrid*& donorGrid, int& cellIdx)
{
  ...
  if (status == 1)
  {
    cellIdx = svtkStructuredData::ComputeCellId(donorGrid->GetDimensions(), ijk);
    return true;
  }
  return false;
}
...
//-----------------------------------------------------------------------------
void svtkAMRResampleFilter::InitializeFields(svtkFieldData* f, svtkIdType size, svtkCellData* src)
{
  ...
  for (int arrayIdx = 0; arrayIdx < src->GetNumberOfArrays(); ++arrayIdx)
  {
    ...
    //      int myIndex = -1;
    //      svtkDataArray *arrayPtr = f->GetArray(
...
//-----------------------------------------------------------------------------
void svtkAMRResampleFilter::CopyData(
  svtkFieldData* target, svtkIdType targetIdx, svtkCellData* src, svtkIdType srcIdx)
{
  ...
  for (; arrayIdx < src->GetNumberOfArrays(); ++arrayIdx)
  {
    ...
    svtkDataArray* srcArray = src->GetArray(arrayIdx);
...
//-----------------------------------------------------------------------------
void svtkAMRResampleFilter::ComputeCellCentroid(
  svtkUniformGrid* g, const svtkIdType cellIdx, double c[3])
{
  ...
  svtkCell* myCell = g->GetCell(cellIdx);
...
void svtkAMRResampleFilter::TransferToCellCenters(svtkUniformGrid* g, svtkOverlappingAMR* amrds)
{
  ...
  for (; cellIdx < g->GetNumberOfCells(); ++cellIdx)
  {
    ...
    for (; level < amrds->GetNumberOfDataSets(level); ++level)
    {
      ...
      {
        int donorCellIdx = -1;
        svtkUniformGrid* donorGrid = amrds->GetDataSet(level, dataIdx);
...
//-----------------------------------------------------------------------------
bool svtkAMRResampleFilter::SearchForDonorGridAtLevel(double q[3], svtkOverlappingAMR* amrds,
  unsigned int level, unsigned int& donorGridId, int& donorCellIdx)
{
  ...
  svtkTimerLog::MarkStartEvent(oss.str().c_str());
  ...
  for (donorGridId = 0; donorGridId < amrds->GetNumberOfDataSets(level); ++donorGridId)
  {
    ...
    if (amrds->GetAMRInfo()->FindCell(q, level, donorGridId, donorCellIdx))
    {
      assert("pre: donorCellIdx is invalid" && (donorCellIdx >= 0)); //  &&
      // (donorCellIdx < donorGrid->GetNumberOfCells()) );
      svtkTimerLog::MarkEndEvent(oss.str().c_str());
      return true;
    } // END if
...
//-----------------------------------------------------------------------------
int svtkAMRResampleFilter::ProbeGridPointInAMR(double q[3], unsigned int& donorLevel,
  unsigned int& donorGridId, svtkOverlappingAMR* amrds, unsigned int maxLevel, bool hadDonorGrid)
{
  assert("pre: AMR dataset is nullptr" && amrds != nullptr);

  svtkUniformGrid* currentGrid = nullptr;
  int currentCellIdx = -1;
  int donorCellIdx = -1;
  unsigned int currentLevel = 0;
  unsigned int currentGridId = 0;
  svtkUniformGrid* donorGrid = hadDonorGrid ? amrds->GetDataSet(donorLevel, donorGridId) : nullptr;
  ...
  for (int level = startLevel; level != endLevel; level += incLevel)
  {
    ...
    else if (currentGrid != nullptr)
    {
      ...
      // svtkErrorMacro("Could not find point in an unblanked cell.");
...
//-----------------------------------------------------------------------------
bool svtkAMRResampleFilter::SearchGridAncestors(
  double q[3], svtkOverlappingAMR* amrds, unsigned int& level, unsigned int& gridId, int& cellId)
{
  assert("pre: AMR dataset is nullptr" && (amrds != nullptr));
  ...
  for (; level > 0; --level)
  {
    ...
    if (numParents > 1)
    {
      svtkDebugMacro("Number of parents: " << numParents << " - Only processing 1 route");
    }
    gridId = parents[0];
...
//-----------------------------------------------------------------------------
void svtkAMRResampleFilter::SearchGridDecendants(double q[3], svtkOverlappingAMR* amrds,
...
//-----------------------------------------------------------------------------
int svtkAMRResampleFilter::ProbeGridPointInAMRGraph(double q[3], unsigned int& donorLevel,
  unsigned int& donorGridId, svtkOverlappingAMR* amrds, unsigned int maxLevel, bool useCached)
{
  assert("pre: AMR dataset is nullptr" && amrds != nullptr);

  int donorCellIdx = -1;

  svtkUniformGrid* donorGrid = nullptr;
  // STEP 0: Check the previously cached donor-grid
  if (useCached)
  {
    if (!amrds->GetAMRInfo()->FindCell(q, donorLevel, donorGridId, donorCellIdx))
...
//-----------------------------------------------------------------------------
void svtkAMRResampleFilter::TransferToGridNodes(svtkUniformGrid* g, svtkOverlappingAMR* amrds)
{
  ...
  svtkPointData* PD = g->GetPointData();
  ...
  if (this->AMRMetaData)
  {
    for (pIdx = 0; pIdx < g->GetNumberOfPoints(); ++pIdx)
    {
      ...
      {
        useCached = true;
        svtkUniformGrid* amrGrid = amrds->GetDataSet(donorLevel, donorGridId);
  ...
  else
  {
    for (pIdx = 0; pIdx < g->GetNumberOfPoints(); ++pIdx)
    {
      ...
      if (donorCellIdx != -1)
      {
        useCached = true;
        this->AverageLevel += donorLevel;
        svtkUniformGrid* donorGrid = amrds->GetDataSet(donorLevel, donorGridId);
...
//-----------------------------------------------------------------------------
void svtkAMRResampleFilter::TransferSolution(svtkUniformGrid* g, svtkOverlappingAMR* amrds)
{
  assert("pre: uniform grid is nullptr" && (g != nullptr));
  assert("pre: AMR data-structure is nullptr" && (amrds != nullptr));
...
//-----------------------------------------------------------------------------
void svtkAMRResampleFilter::ExtractRegion(
  svtkOverlappingAMR* amrds, svtkMultiBlockDataSet* mbds, svtkOverlappingAMR* svtkNotUsed(metadata))
{
...
//-----------------------------------------------------------------------------
void svtkAMRResampleFilter::ComputeAMRBlocksToLoad(svtkOverlappingAMR* metadata)
{
...
//-----------------------------------------------------------------------------
void svtkAMRResampleFilter::GetDomainParameters(svtkOverlappingAMR* amr, double domainMin[3],
...
//-----------------------------------------------------------------------------
void svtkAMRResampleFilter::SnapBounds(const double* svtkNotUsed(h0[3]), const double domainMin[3],
  const double domainMax[3], const int* svtkNotUsed(dims[3]), bool outside[6])
{
  int i, j;
  for (i = 0, j = 0; i < 3; ++i)
  {
...
//-----------------------------------------------------------------------------
void svtkAMRResampleFilter::ComputeLevelOfResolution(
  const int N[3], const double h0[3], const double L[3], const double rf)
{
  ...
  for (int i = 0; i < 3; ++i)
  {
    ...
    int currentLevel = svtkMath::Floor(0.5 + (log(c1) / log(rf)));
...
//-----------------------------------------------------------------------------
void svtkAMRResampleFilter::AdjustNumberOfSamplesInRegion(
...
//-----------------------------------------------------------------------------
void svtkAMRResampleFilter::ComputeAndAdjustRegionParameters(svtkOverlappingAMR* amrds, double h[3])
{
  assert("pre: AMR dataset is nullptr" && (amrds != nullptr));
...
//-----------------------------------------------------------------------------
void svtkAMRResampleFilter::GetRegion(double h[3])
{
...
//-----------------------------------------------------------------------------
bool svtkAMRResampleFilter::GridsIntersect(double* g1, double* g2)
{
...
//-----------------------------------------------------------------------------
bool svtkAMRResampleFilter::IsBlockWithinBounds(double* grd)
{
  ...
  for (unsigned int block = 0; block < this->ROI->GetNumberOfBlocks(); ++block)
  {
    if (this->IsRegionMine(block))
    {
      svtkUniformGrid* blk = svtkUniformGrid::SafeDownCast(this->ROI->GetBlock(block));
...
//-----------------------------------------------------------------------------
int svtkAMRResampleFilter::GetRegionProcessId(const int regionIdx)
{
  if (!this->IsParallel())
  {
    return 0;
...
//-----------------------------------------------------------------------------
bool svtkAMRResampleFilter::IsRegionMine(const int regionIdx)
{
  if (!this->IsParallel())
  {
    return true;
...
//-----------------------------------------------------------------------------
bool svtkAMRResampleFilter::IsParallel()
{
...
//-----------------------------------------------------------------------------
svtkUniformGrid* svtkAMRResampleFilter::GetReferenceGrid(svtkOverlappingAMR* amrds)
{
  assert("pre:AMR dataset is nullptr" && (amrds != nullptr));
  ...
  for (unsigned int l = 0; l < numLevels; ++l)
  {
    ...
    {
      svtkUniformGrid* refGrid = amrds->GetDataSet(l, dataIdx);
      if (refGrid != nullptr)
      {
        return (refGrid);
      }
