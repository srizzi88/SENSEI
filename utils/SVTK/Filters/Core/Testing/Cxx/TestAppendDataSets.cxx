...
//////////////////////////////////////////////////////////////////////////////
void CreateDataset(svtkPointSet* dataset, int numberOfPoints,
  ...
  const std::vector<DataArrayInfo>& cellArrayInfo)
{
  ...
  auto datasetPD = svtkPolyData::SafeDownCast(dataset);
  auto datasetUG = svtkUnstructuredGrid::SafeDownCast(dataset);
  if (datasetPD)
  {
  ...
  for (svtkIdType i = 0; i < numberOfCells; ++i)
  {
    ...
    if (datasetPD)
    {
      datasetPD->InsertNextCell(SVTK_VERTEX, 1, &pointId);
    }
    if (datasetUG)
    {
      datasetUG->InsertNextCell(SVTK_VERTEX, 1, &pointId);
    }
  }
...
//////////////////////////////////////////////////////////////////////////////
int PrintAndCheck(const std::vector<svtkDataSet*>& inputs, svtkDataSet* output, int fieldType)
{
...
//////////////////////////////////////////////////////////////////////////////
int AppendDatasetsAndCheckMergedArrayLengths(svtkAppendDataSets* append)
{
  append->MergePointsOn();
  append->Update();
  svtkDataSet* output = append->GetOutput();
...
// Returns 1 on success, 0 otherwise
//////////////////////////////////////////////////////////////////////////////
int AppendDatasetsAndPrint(const std::vector<svtkDataSet*>& inputs, const char* expectedDataSetType)
{
  svtkNew<svtkAppendDataSets> append;
  for (size_t inputIndex = 0; inputIndex < inputs.size(); ++inputIndex)
  {
    append->AddInputData(inputs[inputIndex]);
  }
  append->SetOutputDataSetType(svtkDataObjectTypes::GetTypeIdFromClassName(expectedDataSetType));
  append->Update();
  svtkDataSet* output = append->GetOutput();
...
bool TestToleranceModes(svtkDataSet* dataset1, svtkDataSet* dataset2)
{
...
bool TestToleranceModes()
{
  ...
  {
    std::cerr << "Failed testing tolerance mode for 'svtkPolyData'\n";
    return false;
  }

  svtkNew<svtkUnstructuredGrid> ugrid1;
  ugrid1->Allocate(3, 10);
  ugrid1->SetPoints(points1);
  ugrid1->InsertNextCell(SVTK_LINE, 2, ptIds);

  svtkNew<svtkUnstructuredGrid> ugrid2;
  ugrid2->Allocate(3, 10);
  ugrid2->SetPoints(points2);
  ugrid2->InsertNextCell(SVTK_LINE, 2, ptIds);

  if (!TestToleranceModes(ugrid1, ugrid2))
  {
    std::cerr << "Failed testing tolerance mode for 'svtkUnstructuredGrid'\n";
    return false;
  }
...
int TestAppendDataSets(int, char*[])
{
  ...
  std::vector<svtkDataSet*> inputs(2, static_cast<svtkDataSet*>(nullptr));
  inputs[0] = d1;
  inputs[1] = d2;
  if (!AppendDatasetsAndPrint(inputs, "svtkPolyData"))
  {
    std::cerr << "svtkAppendDataSets failed with no active scalars\n";
    return EXIT_FAILURE;
  }
  ...
  if (!AppendDatasetsAndPrint(inputs, "svtkPolyData"))
  {
    std::cerr << "svtkAppendDataSets failed with active scalar 'A' in D1, active scalar 'B' in D2\n";
    return EXIT_FAILURE;
  }
  ...
  if (!AppendDatasetsAndPrint(inputs, "svtkPolyData"))
  {
    std::cerr << "svtkAppendDataSets failed with active scalar 'B' in D1, active scalar 'A' in D2\n";
    return EXIT_FAILURE;
  }
  ...
  if (!AppendDatasetsAndPrint(inputs, "svtkPolyData"))
  {
    std::cerr << "svtkAppendDataSets failed with active scalar 'A' in D1, active scalar 'A' in D2\n";
    return EXIT_FAILURE;
  }
  ...
  if (!AppendDatasetsAndPrint(inputs, "svtkPolyData"))
  {
    std::cerr << "svtkAppendDataSets failed with active scalar 'B' in D1, active scalar 'B' in D2\n";
    return EXIT_FAILURE;
  }
  ...
  if (!AppendDatasetsAndPrint(inputs, "svtkPolyData"))
  {
    std::cerr << "svtkAppendDataSets failed with no common array names and no active scalars\n";
    return EXIT_FAILURE;
  }
  ...
  if (!AppendDatasetsAndPrint(inputs, "svtkPolyData"))
  {
    std::cerr << "svtkAppendDataSets failed with scalar arrays with nullptr names\n";
    return EXIT_FAILURE;
  }
  ...
  if (!AppendDatasetsAndPrint(inputs, "svtkPolyData"))
  {
    std::cerr << "svtkAppendDataSets failed with scalar arrays with 2 components\n";
    return EXIT_FAILURE;
  }
  ...
  {
    std::cerr
      << "svtkAppendDataSets failed with scalar arrays with same name but different components\n";
    return EXIT_FAILURE;
  }
  ...
  if (!AppendDatasetsAndPrint(inputs, "svtkPolyData"))
  {
    std::cerr << "svtkAppendDataSets failed with deep copied datasets\n";
    return EXIT_FAILURE;
  }
  ...
  svtkNew<svtkUnstructuredGrid> d9;
  ...
  inputs[0] = d9;
  inputs[1] = d8;
  if (!AppendDatasetsAndPrint(inputs, "svtkUnstructuredGrid"))
  {
    std::cerr << "svtkAppendDataSets failed with unstructured grid and polydata\n";
    return EXIT_FAILURE;
  }
