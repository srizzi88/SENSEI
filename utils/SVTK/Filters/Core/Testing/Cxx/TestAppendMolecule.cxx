...
void InitSimpleMolecule(svtkMolecule* molecule)
{
  NB_OF_MOL++;
  svtkAtom h1 = molecule->AppendAtom(1, 0.5, 1.5, -NB_OF_MOL);
  svtkAtom h2 = molecule->AppendAtom(1, 0.5, 1.5, NB_OF_MOL);
  molecule->AppendBond(h1, h2, 1);
}

void AddAtomData(svtkMolecule* molecule)
{
  svtkNew<svtkDoubleArray> data;
  data->SetName("Data");
  data->SetNumberOfComponents(1);
  svtkIdType size = molecule->GetNumberOfAtoms();
  ...
  svtkNew<svtkStringArray> stringData;
...
int CheckMolecule(svtkMolecule* molecule, int nbAtoms, int nbBonds, int nbOfAtomArrays,
  int nbOfBondArrays, svtkDoubleArray* values, int nbGhostAtoms, int nbGhostBonds)
{
  ...
  svtkDataArray* resultData = molecule->GetAtomData()->GetArray("Data");
  if (!resultData)
  {
  ...
  svtkUnsignedShortArray* bondOrderArray = molecule->GetBondOrdersArray();
  if (!bondOrderArray)
  {
  ...
  int nbOfGhosts = 0;
  for (svtkIdType id = 0; id < nbAtoms; id++)
  {
  ...
  nbOfGhosts = 0;
  for (svtkIdType id = 0; id < nbBonds; id++)
  {
...
int TestAppendMolecule(int, char*[])
{
  ...
  svtkNew<svtkMolecule> simpleMolecule1;
  InitSimpleMolecule(simpleMolecule1);

  svtkNew<svtkMolecule> simpleMolecule2;
  InitSimpleMolecule(simpleMolecule2);

  svtkNew<svtkMoleculeAppend> appender;
  appender->AddInputData(simpleMolecule1);
  appender->AddInputData(simpleMolecule2);
  appender->Update();
  svtkMolecule* resultMolecule = appender->GetOutput();
  ...
  svtkAtom firstAtom2 = fullMolecule2->GetAtom(0);
  svtkAtom firstAtom3 = fullMolecule3->GetAtom(0);

  svtkAtom ghostAtom2 =
    fullMolecule2->AppendAtom(firstAtom3.GetAtomicNumber(), firstAtom3.GetPosition());
  svtkBond ghostBond2 = fullMolecule2->AppendBond(firstAtom2, ghostAtom2, 1);

  svtkAtom ghostAtom3 =
    fullMolecule3->AppendAtom(firstAtom2.GetAtomicNumber(), firstAtom2.GetPosition());
  svtkBond ghostBond3 = fullMolecule3->AppendBond(firstAtom3, ghostAtom3, 1);
  ...
  // APPEND 1 with 2
  svtkNew<svtkMoleculeAppend> appender2;
  appender2->AddInputData(fullMolecule1);
  appender2->AddInputData(fullMolecule2);
  appender2->Update();
  svtkMolecule* resultFullMolecule = appender2->GetOutput();
  ...
  svtkNew<svtkDoubleArray> expectedResultValues;
