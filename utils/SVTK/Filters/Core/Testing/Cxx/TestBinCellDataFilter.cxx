...
=========================================================================*/
// This tests the svtkBinCellDataFilter class.
...
svtkSmartPointer<svtkUnstructuredGrid> ConstructDelaunay3DSphere(
  svtkIdType numberOfPoints, svtkMersenneTwister* seq, bool sampleShellOnly)
{
  ...
  svtkSmartPointer<svtkPointSource> source = svtkSmartPointer<svtkPointSource>::New();
  source->SetNumberOfPoints(numberOfPoints);
  source->SetCenter(0., 0., 0.);
  source->SetRadius(1.);
  source->SetDistributionToUniform();
  source->SetOutputPointsPrecision(svtkAlgorithm::DOUBLE_PRECISION);
  ...
  svtkUnstructuredGrid* ug = delaunay3D->GetOutput();
  svtkSmartPointer<svtkDoubleArray> radius = svtkSmartPointer<svtkDoubleArray>::New();
...
int TestBinCellDataFilter(int, char*[])
{
  ...
  svtkNew<svtkMersenneTwister> seq;
  seq->InitializeSequence(0, 0);

  const svtkIdType numberOfSourcePoints = 1.e4;
  const svtkIdType numberOfInputPoints = 1.e1;

  svtkSmartPointer<svtkUnstructuredGrid> sourceGrid =
    ConstructDelaunay3DSphere(numberOfSourcePoints, seq, false);
  svtkSmartPointer<svtkUnstructuredGrid> inputGrid =
    ConstructDelaunay3DSphere(numberOfInputPoints, seq, true);

  svtkNew<svtkCellLocator> locator;

  svtkNew<svtkBinCellDataFilter> binDataFilter;
  ...
  {
    svtkIdTypeArray* binnedData = svtkIdTypeArray::SafeDownCast(
    ...
    for (svtkIdType i = 0; i < binnedData->GetNumberOfTuples(); i++)
    {
      ...
      for (svtkIdType j = 1; j < binDataFilter->GetNumberOfBins(); j++)
      {
    ...
    svtkIdType expectedBins[18][4] = { { 0, 0, 223, 257 }, { 432, 2268, 1660, 137 },
  ...
  binDataFilter->SetCellOverlapMethod(svtkBinCellDataFilter::CELL_POINTS);
  binDataFilter->Update();

  {
    ...
    svtkIdType expectedBins[18][4] = { { 0, 0, 430, 533 }, { 445, 2837, 2388, 258 },
