...
// svtkExecutionTimer to svtkDelaunay2D so that I can watch
...
int TestExecutionTimer(int svtkNotUsed(argc), char* svtkNotUsed(argv)[])
{
  ...
  svtkSmartPointer<svtkExecutionTimer> timer = svtkSmartPointer<svtkExecutionTimer>::New();
  timer->SetFilter(delaunay2D);

  delaunay2D->Update();

  std::cout << "TestExecutionTimer: Filter under inspection (" << timer->GetFilter()->GetClassName()
            << ") execution time: " << std::fixed << std::setprecision(8)
            << timer->GetElapsedCPUTime() << " sec (CPU), " << timer->GetElapsedWallClockTime()
            << " sec (wall clock)\n";
