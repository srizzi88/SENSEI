...
int TestTransposeTable(int, char*[])
{
  ...
  svtkNew<svtkStringArray> colName;
  ...
  svtkNew<svtkDoubleArray> colDouble;
  ...
  svtkNew<svtkFloatArray> colFloat;
  ...
  svtkNew<svtkTransposeTable> filter;
  filter->SetInputData(table);
  filter->Update();

  svtkTable* outTable = filter->GetOutput();
  ...
  for (int i = 0; i < table->GetNumberOfRows(); i++)
  {
    std::stringstream ss;
    ss << i;
    svtkAbstractArray* col = outTable->GetColumnByName(ss.str().c_str());
