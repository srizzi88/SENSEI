...
int TestUnstructuredGridQuadricDecimation(int, char*[])
{
  ...
  const svtkIdType numberOfOriginalPoints = 1.e4;

  // # of decimation tests to perform
  const svtkIdType numberOfTests = 4;
  ...
  svtkPolyData* pd = cleaner->GetOutput();
  ...
  const svtkIdType numberOfOriginalTetras = delaunay3D->GetOutput()->GetNumberOfCells();

  for (svtkIdType test = 0; test < numberOfTests; test++)
  {
    // Decimate the tetrahedral mesh.
    svtkSmartPointer<svtkUnstructuredGridQuadricDecimation> decimate =
      svtkSmartPointer<svtkUnstructuredGridQuadricDecimation>::New();
