...
static svtkSmartPointer<svtkImageData> MakeImageData(unsigned int dim);
...
int UnitTestMaskPoints(int, char*[])
{
  ...
  svtkSmartPointer<svtkMaskPoints> mask0 = svtkSmartPointer<svtkMaskPoints>::New();
  mask0->Print(print0);
  ...
  // Try with image data
  svtkSmartPointer<svtkMaskPoints> mask1 = svtkSmartPointer<svtkMaskPoints>::New();
  mask1->SetInputData(MakeImageData(10));
  mask1->GenerateVerticesOn();
  mask1->SetOutputPointsPrecision(svtkAlgorithm::DEFAULT_PRECISION);
  mask1->RandomModeOn();
  ...
  mask0->AddObserver(svtkCommand::ErrorEvent, errorObserver);
