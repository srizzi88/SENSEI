...
static svtkSmartPointer<svtkPointData> MakePointData(unsigned int numberOfPoints);
static svtkSmartPointer<svtkCellData> MakeCellData(unsigned int numberOfCells);
...
int UnitTestMergeFilter(int, char*[])
{
  ...
  svtkSmartPointer<svtkMergeFilter> merge0 = svtkSmartPointer<svtkMergeFilter>::New();
  merge0->Print(print0);
  ...
  merge0->AddObserver(svtkCommand::WarningEvent, warningObserver);
  merge0->Update();
  ...
  // Create a PolyData
  svtkSmartPointer<svtkDataObjectGenerator> dog1 = svtkSmartPointer<svtkDataObjectGenerator>::New();
  dog1->SetProgram("PD1");
  dog1->Update();
  svtkSmartPointer<svtkPolyData> polyData = svtkPolyData::SafeDownCast(dog1->GetOutput());

  svtkSmartPointer<svtkPolyData> polyData2 = svtkPolyData::SafeDownCast(dog1->GetOutput());

  svtkSmartPointer<svtkPointData> pointData = MakePointData(polyData2->GetNumberOfPoints());
  svtkSmartPointer<svtkCellData> cellData = MakeCellData(polyData2->GetNumberOfCells());
  ...
  svtkSmartPointer<svtkMergeFilter> merge1 = svtkSmartPointer<svtkMergeFilter>::New();
  ...
  if (merge1->GetGeometry() != svtkDataSet::SafeDownCast(polyData))
  {
  ...
  merge1->AddObserver(svtkCommand::WarningEvent, warningObserver);
  svtkSmartPointer<svtkPointData> pointData2 = MakePointData(100);
  svtkSmartPointer<svtkCellData> cellData2 = MakeCellData(100);
...
svtkSmartPointer<svtkPointData> MakePointData(unsigned int numberOfPoints)
{
  svtkSmartPointer<svtkPointData> pointData = svtkSmartPointer<svtkPointData>::New();
  svtkSmartPointer<svtkFloatArray> scalarsArray = svtkSmartPointer<svtkFloatArray>::New();
  scalarsArray->SetNumberOfTuples(numberOfPoints);
  scalarsArray->SetNumberOfComponents(1);
  scalarsArray->SetName("Scalars:floatArray");
  svtkSmartPointer<svtkFloatArray> normalsArray = svtkSmartPointer<svtkFloatArray>::New();
  normalsArray->SetNumberOfTuples(numberOfPoints);
  normalsArray->SetNumberOfComponents(3);
  normalsArray->SetName("Normals:floatArray");
  svtkSmartPointer<svtkFloatArray> vectorsArray = svtkSmartPointer<svtkFloatArray>::New();
  vectorsArray->SetNumberOfTuples(numberOfPoints);
  vectorsArray->SetNumberOfComponents(3);
  vectorsArray->SetName("Vectors:floatArray");
  svtkSmartPointer<svtkFloatArray> tcoordsArray = svtkSmartPointer<svtkFloatArray>::New();
  tcoordsArray->SetNumberOfTuples(numberOfPoints);
  tcoordsArray->SetNumberOfComponents(2);
  tcoordsArray->SetName("Tcoords:floatArray");
  svtkSmartPointer<svtkFloatArray> tensorsArray = svtkSmartPointer<svtkFloatArray>::New();
...
svtkSmartPointer<svtkCellData> MakeCellData(unsigned int numberOfCells)
{
  svtkSmartPointer<svtkCellData> cellData = svtkSmartPointer<svtkCellData>::New();
  svtkSmartPointer<svtkFloatArray> scalarsArray = svtkSmartPointer<svtkFloatArray>::New();
