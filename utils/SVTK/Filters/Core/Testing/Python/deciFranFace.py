...
class deciFranFace(svtk.test.Testing.svtkTest):
    ...
    def testDeciFranFace(self):
        ...
        pnm1 = svtk.svtkPNGReader()
        pnm1.SetFileName(SVTK_DATA_ROOT + "/Data/fran_cut.png")
        ...
        for topology in boundaryVertexDeletion:
            for accumulate in accumulates:
                ...
                deci.update({idx: svtk.svtkDecimatePro()})
                ...
                mapper.update({idx: svtk.svtkPolyDataMapper()})
                mapper[idx].SetInputConnection(deci[idx].GetOutputPort())
                actor.update({idx: svtk.svtkActor()})
...
if __name__ == "__main__":
     svtk.test.Testing.main([(deciFranFace, 'test')])
