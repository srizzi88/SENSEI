...
class skinOrder(svtk.test.Testing.svtkTest):
    ...
    def testSkinOrder(self):
        ...
        for idx, order in enumerate(orders):
            reader.append(svtk.svtkVolume16Reader())
            reader[idx].SetDataDimensions(RESOLUTION, RESOLUTION)
            reader[idx].SetFilePrefix(SVTK_DATA_ROOT + '/Data/headsq/quarter')
...
if __name__ == "__main__":
     svtk.test.Testing.main([(skinOrder, 'test')])
