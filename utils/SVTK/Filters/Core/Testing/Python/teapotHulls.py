...
byuReader = svtk.svtkBYUReader()
byuReader.SetGeometryFileName(SVTK_DATA_ROOT + "/Data/teapot.g")

byuMapper = svtk.svtkPolyDataMapper()
byuMapper.SetInputConnection(byuReader.GetOutputPort())

i = 0
while i < 9:
    idx = str(i)
    exec("byuActor" + idx + " = svtk.svtkActor()")
    eval("byuActor" + idx).SetMapper(byuMapper)

    ren1.AddActor(eval("byuActor" + idx))

    exec("hull" + idx + " = svtk.svtkHull()")
    eval("hull" + idx).SetInputConnection(byuReader.GetOutputPort())

    exec("hullMapper" + idx + " = svtk.svtkPolyDataMapper()")
    eval("hullMapper" + idx).SetInputConnection(
      eval("hull" + idx).GetOutputPort())

    exec("hullActor" + idx + " = svtk.svtkActor()")
