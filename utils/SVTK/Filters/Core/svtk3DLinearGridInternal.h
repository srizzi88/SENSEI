/*=========================================================================
  ...
  Module:    svtk3DLinearGridInternal.h
...
=========================================================================*/
/**
 * @class   svtk3DLinearGridInternal
 * @brief   fast access and processing of 3D linear grids
 *
 * svtk3DLinearGridInternal provides fast access and processing of 3D linear cells
 * contained in a svtkUnstructuredGrid: tetrahedra, hexahedra, voxels,
 ...
 * non-3D linear cells contained in a svtkUnstructuredGrids is allowed although
 ...
 *
 * @sa
 * svtk3DLinearGridInternalPlaneCutter svtk3DLinearGridInternalCrinkleCutter svtkContour3DLinearGrid
 */

#ifndef svtk3DLinearGridInternal_h
#define svtk3DLinearGridInternal_h
...
struct BaseCell
{
  ...
  // Set up the case table. This is done by accessing standard SVTK cells and
  ...
  void BuildCases(int numCases, const svtkIdType** edges, int** cases, unsigned short* caseArray);
};
...
void BaseCell::BuildCases(
  int numCases, const svtkIdType** edges, int** cases, unsigned short* caseArray)
{
...
// Load and transform svtkTetra case table. The case tables are repackaged for
// efficiency (e.g., support the GetCase() method).
void TetraCell::BuildCases()
{
  const svtkIdType** edges = new const svtkIdType*[this->NumEdges];
  int numCases = std::pow(2, this->NumVerts);
  int** cases = new int*[numCases];
  for (int i = 0; i < this->NumEdges; ++i)
  {
    edges[i] = svtkTetra::GetEdgeArray(i);
  }
  for (int i = 0; i < numCases; ++i)
  {
    cases[i] = svtkTetra::GetTriangleCases(i);
  }
...
void HexahedronCell::BuildCases()
{
  ...
  for (int i = 0; i < this->NumEdges; ++i)
  {
    edges[i] = svtkHexahedron::GetEdgeArray(i);
  }
  for (int i = 0; i < numCases; ++i)
  {
    cases[i] = svtkHexahedron::GetTriangleCases(i);
  }
...
void WedgeCell::BuildCases()
{
  ...
  for (int i = 0; i < this->NumEdges; ++i)
  {
    edges[i] = svtkWedge::GetEdgeArray(i);
  }
  for (int i = 0; i < numCases; ++i)
  {
    cases[i] = svtkWedge::GetTriangleCases(i);
  }
...
void PyramidCell::BuildCases()
{
  ...
  for (int i = 0; i < this->NumEdges; ++i)
  {
    edges[i] = svtkPyramid::GetEdgeArray(i);
  }
  for (int i = 0; i < numCases; ++i)
  {
    cases[i] = svtkPyramid::GetTriangleCases(i);
  }
...
// the MC cases (svtkMarchingCubesTriangleCases) are specified for the
...
void VoxelCell::BuildCases()
{
  ...
  const svtkIdType** edges = new const svtkIdType*[this->NumEdges];
  constexpr svtkIdType voxEdges[12][2] = { { 0, 1 }, { 1, 3 }, { 2, 3 }, { 0, 2 }, { 4, 5 },
  ...
  for (hexCase = 0; hexCase < numCases; ++hexCase)
  {
    ...
    cases[voxCase] = svtkHexahedron::GetTriangleCases(hexCase);
  }
...
struct CellIter
{
  ...
  svtkSmartPointer<svtkCellArrayIterator> ConnIter;
  ...
  CellIter(svtkIdType numCells, unsigned char* types, svtkCellArray* cellArray)
    : Copy(false)
    , Cell(nullptr)
    , NumVerts(0)
    , Cases(nullptr)
    ...
    , Types(types)
    , CellArray(cellArray)
    , ConnIter(svtk::TakeSmartPointer(cellArray->NewIterator()))
  {
  ...
  CellIter& operator=(const CellIter& cellIter)
  {
    ...
    {
      this->ConnIter = svtk::TakeSmartPointer(this->CellArray->NewIterator());
  ...
  const svtkIdType* Initialize(svtkIdType cellId)
  {
    ...
    svtkIdType dummy;
    const svtkIdType* conn;
    this->ConnIter->GetCurrentCell(dummy, conn);
    return conn;
  }

  const svtkIdType* Next()
  {
  ...
  unsigned char GetCellType(svtkIdType cellId) { return this->Types[cellId]; }

  // Method for random access of cell, no caching
  const svtkIdType* GetCellIds(svtkIdType cellId)
  {
    this->Cell = this->GetCell(this->Types[cellId]);
    this->NumVerts = this->Cell->NumVerts;
    this->Cases = this->Cell->Cases;
...
#endif // svtk3DLinearGridInternal_h
// SVTK-HeaderTest-Exclude: svtk3DLinearGridInternal.h
