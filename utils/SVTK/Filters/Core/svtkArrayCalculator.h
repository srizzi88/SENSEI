 ...
 *
 * svtkArrayCalculator performs operations on vectors or scalars in field
 * data arrays.  It uses svtkFunctionParser to do the parsing and to
...
#ifndef svtkArrayCalculator_h
#define svtkArrayCalculator_h

#include "svtkDataObject.h"        // For attribute types
...
#define SVTK_ATTRIBUTE_MODE_USE_VERTEX_DATA 3
#define SVTK_ATTRIBUTE_MODE_USE_EDGE_DATA 4
#endif

class SVTKFILTERSCORE_EXPORT svtkArrayCalculator : public svtkPassInputTypeAlgorithm
{
public:
  svtkTypeMacro(svtkArrayCalculator, svtkPassInputTypeAlgorithm);
  void PrintSelf(ostream& os, svtkIndent indent) override;

  static svtkArrayCalculator* New();
   ...
   * Initial value is SVTK_DOUBLE.
   */
  svtkGetMacro(ResultArrayType, int);
  svtkSetMacro(ResultArrayType, int);
  //@}
   ...
   */
  svtkGetMacro(CoordinateResults, svtkTypeBool);
  svtkSetMacro(CoordinateResults, svtkTypeBool);
  svtkBooleanMacro(CoordinateResults, svtkTypeBool);
  //@}
   ...
   */
  svtkGetMacro(ResultNormals, bool);
  svtkSetMacro(ResultNormals, bool);
  svtkBooleanMacro(ResultNormals, bool);
  //@}
   ...
   */
  svtkGetMacro(ResultTCoords, bool);
  svtkSetMacro(ResultTCoords, bool);
  svtkBooleanMacro(ResultTCoords, bool);
  //@}
   ...
   *
   * @deprecated Replaced By GetAttributeType and SetAttributeType as of SVTK 8.1.
   */
#ifndef SVTK_LEGACY_REMOVE
  SVTK_LEGACY(void SetAttributeMode(int newMode);)
  SVTK_LEGACY(int GetAttributeMode();)
  SVTK_LEGACY(void SetAttributeModeToDefault()) { this->SetAttributeType(DEFAULT_ATTRIBUTE_TYPE); }
  SVTK_LEGACY(void SetAttributeModeToUsePointData())
  {
    this->SetAttributeType(svtkDataObject::POINT);
  }
  SVTK_LEGACY(void SetAttributeModeToUseCellData()) { this->SetAttributeType(svtkDataObject::CELL); }
  SVTK_LEGACY(void SetAttributeModeToUseVertexData())
  {
    this->SetAttributeType(svtkDataObject::VERTEX);
  }
  SVTK_LEGACY(void SetAttributeModeToUseEdgeData()) { this->SetAttributeType(svtkDataObject::EDGE); }
  SVTK_LEGACY(const char* GetAttributeModeAsString());
#endif
  //@}
   ...
   * for svtkDataSets).  By default the filter uses Point/Vertex/Row data depending
   * on the input data type.  The input value for this function should be one of the
   * constants in svtkDataObject::AttributeTypes or DEFAULT_ATTRIBUTE_TYPE for 'default behavior'.
   */
  svtkSetMacro(AttributeType, int);
  svtkGetMacro(AttributeType, int);
  void SetAttributeTypeToDefault() { this->SetAttributeType(DEFAULT_ATTRIBUTE_TYPE); }
  void SetAttributeTypeToPointData() { this->SetAttributeType(svtkDataObject::POINT); }
  void SetAttributeTypeToCellData() { this->SetAttributeType(svtkDataObject::CELL); }
  void SetAttributeTypeToEdgeData() { this->SetAttributeType(svtkDataObject::EDGE); }
  void SetAttributeTypeToVertexData() { this->SetAttributeType(svtkDataObject::VERTEX); }
  void SetAttributeTypeToRowData() { this->SetAttributeType(svtkDataObject::ROW); }
  //@}
  ...
  svtkGetMacro(NumberOfScalarArrays, int);
  svtkGetMacro(NumberOfVectorArrays, int);
  //@}
   ...
   */
  //@}
  svtkSetMacro(IgnoreMissingArrays, bool);
  svtkGetMacro(IgnoreMissingArrays, bool);
  svtkBooleanMacro(IgnoreMissingArrays, bool);

  /**
   * Returns the output of the filter downcast to a svtkDataSet or nullptr if the
   * cast fails.
   */
  svtkDataSet* GetDataSetOutput();

protected:
  svtkArrayCalculator();
  ~svtkArrayCalculator() override;

  int FillInputPortInformation(int, svtkInformation*) override;

  // Do the bulk of the work
  int ProcessDataObject(svtkDataObject* input, svtkDataObject* output);
   ...
   */
  int GetAttributeTypeFromInput(svtkDataObject* input);
  ...
  svtkFunctionParser* FunctionParser;

  svtkTypeBool ReplaceInvalidValues;
  double ReplacementValue;
  bool IgnoreMissingArrays;

  svtkTypeBool CoordinateResults;
  bool ResultNormals;
  bool ResultTCoords;
...
private:
  svtkArrayCalculator(const svtkArrayCalculator&) = delete;
  void operator=(const svtkArrayCalculator&) = delete;
};
