...
svtkStandardNewMacro(svtkDecimatePro);

#define SVTK_TOLERANCE 1.0e-05
#define SVTK_MAX_TRIS_PER_VERTEX SVTK_CELL_SIZE
#define SVTK_RECYCLE_VERTEX SVTK_DOUBLE_MAX

#define SVTK_SIMPLE_VERTEX 1
#define SVTK_BOUNDARY_VERTEX 2
#define SVTK_INTERIOR_EDGE_VERTEX 3
#define SVTK_CORNER_VERTEX 4
#define SVTK_CRACK_TIP_VERTEX 5
#define SVTK_EDGE_END_VERTEX 6
#define SVTK_NON_MANIFOLD_VERTEX 7
#define SVTK_DEGENERATE_VERTEX 8
#define SVTK_HIGH_DEGREE_VERTEX 9

#define SVTK_STATE_UNSPLIT 0
#define SVTK_STATE_SPLIT 1
#define SVTK_STATE_SPLIT_ALL 2
...
svtkDecimatePro::svtkDecimatePro()
{
  this->Neighbors = svtkIdList::New();
  this->Neighbors->Allocate(SVTK_MAX_TRIS_PER_VERTEX);
  this->V = new svtkDecimatePro::VertexArray(SVTK_MAX_TRIS_PER_VERTEX + 1);
  this->T = new svtkDecimatePro::TriArray(SVTK_MAX_TRIS_PER_VERTEX + 1);
  this->EdgeLengths = svtkPriorityQueue::New();
  this->EdgeLengths->Allocate(SVTK_MAX_TRIS_PER_VERTEX);

  this->InflectionPoints = svtkDoubleArray::New();
  this->TargetReduction = 0.90;
  this->FeatureAngle = 15.0;
  this->PreserveTopology = 0;
  this->MaximumError = SVTK_DOUBLE_MAX;
  this->AbsoluteError = SVTK_DOUBLE_MAX;
...
//
int svtkDecimatePro::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  svtkIdType i, ptId, numPts, numTris, collapseId;
  svtkPoints* inPts;
  svtkPoints* newPts;
  ...
  svtkIdType totalEliminated, numRecycles, numPops;
  svtkIdType ncells;
  svtkIdType pt1, pt2, cellId, fedges[2];
  svtkIdType* cells;
  ...
  svtkPointData* meshPD = nullptr;
  svtkIdType *map, numNewPts, totalPts;
  svtkIdType newCellPts[3];
  int abortExecute = 0;

  svtkDebugMacro(<< "Executing progressive decimation...");
  ...
  {
    svtkErrorMacro(<< "No data to decimate!");
    return 1;
  }
  ...
  {
    this->Error =
      (this->MaximumError >= SVTK_DOUBLE_MAX ? SVTK_DOUBLE_MAX : this->MaximumError * max);
  }
  else
  {
    this->Error = (this->AbsoluteError >= SVTK_DOUBLE_MAX ? SVTK_DOUBLE_MAX : this->AbsoluteError);
  }
  this->Tolerance = SVTK_TOLERANCE * input->GetLength();
  ...
  this->SplitState = SVTK_STATE_UNSPLIT;

  // Lets check to make sure there are only triangles in the input.
  {
    const svtkIdType cellSize = input->GetPolys()->IsHomogeneous();
    if (cellSize != 3)
    {
      svtkErrorMacro("DecimatePro does not accept polygons that are not triangles.");
  ...
  else
  {
    ...
    output->GetPointData()->PassData(input->GetPointData());
    output->GetCellData()->PassData(input->GetCellData());
    // svtkWarningMacro(<<"Reduction == 0: passing data through unchanged");
    return 1;
  }
  ...
  {
    this->VertexError = svtkDoubleArray::New();
    this->VertexError->Allocate(numPts, static_cast<svtkIdType>(0.25 * numPts));
    for (i = 0; i < numPts; i++)
    {
  ...
  {
    svtkDebugMacro(<< "Pre-splitting mesh");
    this->SplitState = SVTK_STATE_SPLIT;
    this->SplitMesh();
  }
  ...
  for (ptId = 0; ptId < npts && !abortExecute; ptId++)
  {
    if (!(ptId % 10000))
    {
      svtkDebugMacro(<< "Inserting vertex #" << ptId);
       ...
       numPops++)
  {
    ...
    {
      svtkDebugMacro(<< "Deleting vertex #" << numPops);
    ...
    if (ncells > 0)
    {
      ...
      {
        numRecycles++;
        this->Insert(ptId, SVTK_RECYCLE_VERTEX);
      }
  ...
  {
    if (this->Mesh->GetCellType(cellId) == SVTK_TRIANGLE) // non-null element
    {
...
static double ComputeEdgeError(double x[3], double x1[3], double x2[3])
{
  double projDist = svtkLine::DistanceToLine(x, x1, x2);
  double edgeLength = svtkMath::Distance2BetweenPoints(x1, x2);
...
static double ComputeSingleTriangleError(double x[3], double x1[3], double x2[3])
{
  return svtkTriangle::TriangleArea(x, x1, x2);
}
...
static double ComputeSimpleError(double x[3], double normal[3], double point[3])
{
  double dist = svtkPlane::DistanceToPlane(x, normal, point);
  return dist * dist;
}
...
//
void svtkDecimatePro::SplitMesh()
{
  svtkIdType ptId, fedges[2];
  int type;
  svtkIdType* cells;
  svtkIdType ncells;

  this->CosAngle = cos(svtkMath::RadiansFromDegrees(this->SplitAngle));
  for (ptId = 0; ptId < this->Mesh->GetNumberOfPoints(); ptId++)
  {
    ...
    if (ncells > 0 &&
      ((type = this->EvaluateVertex(ptId, ncells, cells, fedges)) == SVTK_CORNER_VERTEX ||
        type == SVTK_INTERIOR_EDGE_VERTEX || type == SVTK_NON_MANIFOLD_VERTEX))
    {
...
#define SVTK_FEATURE_ANGLE(tri1, tri2) svtkMath::Dot(this->T->Array[tri1].n, this->T->Array[tri2].n)
//----------------------------------------------------------------------------
...
//
int svtkDecimatePro::EvaluateVertex(
  svtkIdType ptId, svtkIdType numTris, svtkIdType* tris, svtkIdType fedges[2])
{
  svtkIdType numNei, numFEdges;
  svtkIdType numVerts;
  svtkDecimatePro::LocalTri t;
  svtkDecimatePro::LocalVertex sn;
  svtkIdType startVertex, nextVertex, numNormals;
  int i, j, vtype;
  ...
  {
    return SVTK_HIGH_DEGREE_VERTEX;
  }
  ...
  if (nextVertex == startVertex && numNei == 1)
  {
    ...
    {
      vtype = SVTK_NON_MANIFOLD_VERTEX;
    }
    else // remove last vertex addition
    {
      this->V->MaxId -= 1;
      vtype = SVTK_SIMPLE_VERTEX;
    }
  }
  //
  ...
  for (i = 0; i < this->T->GetNumberOfTriangles(); i++)
  {
    ...
    this->T->Array[i].area = svtkTriangle::TriangleArea(this->X, x1, x2);
    svtkTriangle::TriangleCenter(this->X, x1, x2, center);
    ...
    //
    if (svtkMath::Normalize(normal) != 0.0)
    {
      numNormals++;
      for (j = 0; j < 3; j++)
      {
  ...
  //
  if (vtype == SVTK_SIMPLE_VERTEX) // first edge
  {
    if ((this->V->Array[0].FAngle = SVTK_FEATURE_ANGLE(0, this->T->MaxId)) <= this->CosAngle)
    {
      fedges[numFEdges++] = 0;
    }
  }
  ...
  {
    if ((this->V->Array[i + 1].FAngle = SVTK_FEATURE_ANGLE(i, i + 1)) <= this->CosAngle)
    {
      if (numFEdges >= 2)
      {
        numFEdges++;
  ...
  //  Final classification
  //
  if (vtype == SVTK_SIMPLE_VERTEX && numFEdges > 0)
  {
    if (numFEdges == 1)
    {
      vtype = SVTK_EDGE_END_VERTEX;
    }
    else if (numFEdges == 2)
    {
      vtype = SVTK_INTERIOR_EDGE_VERTEX;
    }
    else
    {
      vtype = SVTK_CORNER_VERTEX;
    }
  }
  else if (vtype == SVTK_BOUNDARY_VERTEX)
  {
    if (numFEdges != 2)
    {
      vtype = SVTK_CORNER_VERTEX;
...
//
void svtkDecimatePro::SplitVertex(
  svtkIdType ptId, int type, svtkIdType numTris, svtkIdType* tris, int insert)
{
  svtkIdType id, fedge1, fedge2, i, j;
  svtkIdType tri, veryFirst;
  int numSplitTris;
  const svtkIdType* verts;
  svtkIdType nverts;
  double error;
  svtkIdType startTri, p[2];
  int maxGroupSize;
  svtkPointData* meshPD = this->Mesh->GetPointData();

  //
  // On an interior edge split along the edge
  //
  if (type == SVTK_INTERIOR_EDGE_VERTEX) // when edge splitting is on
  {
  ...
  //
  else if (type == SVTK_CORNER_VERTEX)
  {
  ...
  {
    svtkIdList* triangles = svtkIdList::New();
    svtkIdList* cellIds = svtkIdList::New();
    svtkIdList* group = svtkIdList::New();

    triangles->Allocate(SVTK_MAX_TRIS_PER_VERTEX);
    cellIds->Allocate(5, 10);
    group->Allocate(SVTK_MAX_TRIS_PER_VERTEX);
    ...
    if (type == SVTK_NON_MANIFOLD_VERTEX || type == SVTK_HIGH_DEGREE_VERTEX)
    {
      ; // use maxGroupSize
    }
    else
  ...
  int type, svtkIdType fedges[2], svtkIdType& pt1, svtkIdType& pt2, svtkIdList* CollapseTris)
{
  ...
  switch (type)
  {
    case SVTK_SIMPLE_VERTEX:
    case SVTK_EDGE_END_VERTEX:
    case SVTK_INTERIOR_EDGE_VERTEX:
      if (type == SVTK_INTERIOR_EDGE_VERTEX)
      {
        dist2 = svtkMath::Distance2BetweenPoints(this->X, this->V->Array[fedges[0]].x);
        this->EdgeLengths->Insert(dist2, fedges[0]);

        dist2 = svtkMath::Distance2BetweenPoints(this->X, this->V->Array[fedges[1]].x);
      ...
      else // Compute the edge lengths
      {
        ...
        {
          dist2 = svtkMath::Distance2BetweenPoints(this->X, this->V->Array[i].x);
    ...
    case SVTK_BOUNDARY_VERTEX: //--------------------------------------------
      CollapseTris->SetNumberOfIds(1);
      // Compute the edge lengths
      dist2 = svtkMath::Distance2BetweenPoints(this->X, this->V->Array[0].x);
      e2dist2 = svtkMath::Distance2BetweenPoints(this->X, this->V->Array[this->V->MaxId].x);
...
//
int svtkDecimatePro::IsValidSplit(int index)
{
  svtkIdType fedges[2];
  int i, sign;
  svtkIdType nverts = this->V->MaxId + 1, j;
  double *x, val, sPt[3], v21[3], sN[3];
  svtkIdType l1[SVTK_MAX_TRIS_PER_VERTEX], l2[SVTK_MAX_TRIS_PER_VERTEX];
  svtkIdType n1, n2;
  ...
  for (j = 0; j < (nverts - 3); j++)
  {
    ...
    svtkMath::Cross(v21, this->Normal, sN);
    ...
    for (sign = 0, i = 0; i < n1; i++) // first loop
    {
      if (!(l1[i] == fedges[0] || l1[i] == fedges[1]))
      {
        ...
        val = svtkPlane::Evaluate(sN, sPt, x);
...
//
void svtkDecimatePro::SplitLoop(
  svtkIdType fedges[2], svtkIdType& n1, svtkIdType* l1, svtkIdType& n2, svtkIdType* l2)
{
  svtkIdType i;
  svtkIdType* loop;
  svtkIdType* count;
...
int svtkDecimatePro::CollapseEdge(int type, svtkIdType ptId, svtkIdType collapseId, svtkIdType pt1,
  svtkIdType pt2, svtkIdList* CollapseTris)
{
  svtkIdType i, numDeleted = CollapseTris->GetNumberOfIds();
  svtkIdType ntris = this->T->MaxId + 1;
  svtkIdType nverts = this->V->MaxId + 1;
  svtkIdType tri[2];
  svtkIdType verts[SVTK_MAX_TRIS_PER_VERTEX + 1];
  ...
  // type == SVTK_CRACK_TIP_VERTEX || type == SVTK_SIMPLE_VERTEX
  if (numDeleted == 2)
  {
    if (type == SVTK_CRACK_TIP_VERTEX) // got to seal the crack first
    {
      this->NumMerges++;
  ...
  else if (numDeleted == 1) // e.g., SVTK_BOUNDARY_VERTEX
  {
    // delete one triangle
    this->Mesh->RemoveReferenceToCell(pt1, tri[0]);
    this->Mesh->RemoveReferenceToCell(collapseId, tri[0]);
  ...
  {
    svtkErrorMacro(<< "invalid numDeleted count");
  }
...
void svtkDecimatePro::GetInflectionPoints(double* inflectionPoints)
{
  svtkIdType i;
...
double* svtkDecimatePro::GetInflectionPoints()
{
...
svtkIdType svtkDecimatePro::GetNumberOfInflectionPoints()
{
...
//----------------------------------------------------------------------------
void svtkDecimatePro::InitializeQueue(svtkIdType numPts)
{
  if (!this->PreserveTopology && this->Splitting)
  {
    numPts = static_cast<svtkIdType>(numPts * 1.25);
  }

  this->Queue = svtkPriorityQueue::New();
  this->Queue->Allocate(numPts, static_cast<svtkIdType>(0.25 * numPts));
}

//----------------------------------------------------------------------------
int svtkDecimatePro::Pop(double& error)
{
  svtkIdType ptId;
  ...
  if (this->NumberOfRemainingTris > 0 && this->Split && this->SplitState == SVTK_STATE_UNSPLIT)
  {
    svtkDebugMacro(<< "Splitting this->Mesh");
  ...
  if (this->NumberOfRemainingTris > 0 && this->Split && this->SplitState != SVTK_STATE_SPLIT_ALL)
  {
    svtkDebugMacro(<< "Final splitting attempt");

    this->SplitState = SVTK_STATE_SPLIT_ALL;
    for (ptId = 0; ptId < this->Mesh->GetNumberOfPoints(); ptId++)
    {
      this->Insert(ptId);
    }
...
void svtkDecimatePro::Insert(svtkIdType ptId, double error)
{
  int type, simpleType;
  svtkIdType* cells;
  svtkIdType fedges[2];
  ...
  if (error < -this->Tolerance)
  {
    ...
    if (ncells > 0)
    {
      ...
      if (type == SVTK_SIMPLE_VERTEX || type == SVTK_EDGE_END_VERTEX || type == SVTK_CRACK_TIP_VERTEX)
      {
        simpleType = 1;
        error = ComputeSimpleError(this->X, this->Normal, this->Pt);
      }

      else if ((type == SVTK_INTERIOR_EDGE_VERTEX) ||
        (type == SVTK_BOUNDARY_VERTEX && this->BoundaryVertexDeletion))
      {
        simpleType = 1;
      ...
      else if (this->SplitState == SVTK_STATE_SPLIT && type != SVTK_DEGENERATE_VERTEX)
      {
  ...
  else if (error >= SVTK_RECYCLE_VERTEX)
  {
    // see whether to split it, otherwise it isn't inserted yet
    if (this->SplitState == SVTK_STATE_SPLIT_ALL)
    {
      this->Mesh->GetPoint(ptId, this->X);
      this->Mesh->GetPointCells(ptId, ncells, cells);
      if (ncells > 0)
...
void svtkDecimatePro::DistributeError(double error)
{
  svtkIdType i;
  svtkIdType nverts = this->V->MaxId + 1;
...
//----------------------------------------------------------------------------
void svtkDecimatePro::DeleteQueue()
{
  if (this->Queue)
  {
    this->Queue->Delete();
...
//----------------------------------------------------------------------------
double svtkDecimatePro::DeleteId(svtkIdType id)
{
  return this->Queue->DeleteId(id);
}

//----------------------------------------------------------------------------
void svtkDecimatePro::Reset()
{
  this->Queue->Reset();
}

//----------------------------------------------------------------------------
void svtkDecimatePro::PrintSelf(ostream& os, svtkIndent indent)
{
