...
=========================================================================*/
/**
 * @class   svtkDelaunay2D
 * @brief   create 2D Delaunay triangulation of input points
 *
 * svtkDelaunay2D is a filter that constructs a 2D Delaunay triangulation from
 * a list of input points. These points may be represented by any dataset of
 * type svtkPointSet and subclasses. The output of the filter is a polygonal
 ...
 * can use the svtkTransformFilter to transform the points into and
 ...
 * svtkTransformFilter method.  Specifying a transform directly also allows
 ...
 * input. This input is an instance of svtkPolyData which contains
 ...
 *
 * @sa
 * svtkDelaunay3D svtkTransformFilter svtkGaussianSplatter
 */

#ifndef svtkDelaunay2D_h
#define svtkDelaunay2D_h
...
#define SVTK_DELAUNAY_XY_PLANE 0
#define SVTK_SET_TRANSFORM_PLANE 1
#define SVTK_BEST_FITTING_PLANE 2

class SVTKFILTERSCORE_EXPORT svtkDelaunay2D : public svtkPolyDataAlgorithm
{
public:
  svtkTypeMacro(svtkDelaunay2D, svtkPolyDataAlgorithm);
   ...
   */
  void SetSourceData(svtkPolyData*);
   ...
   */
  svtkSetClampMacro(Offset, double, 0.75, SVTK_DOUBLE_MAX);
   ...
   * subclass of svtkAbstractTransform (thus it does not need to be a
   ...
   * triangulation. When the SVTK_DELAUNAY_XY_PLANE is set, the z-coordinate
   * is simply ignored. When SVTK_SET_TRANSFORM_PLANE is set, then a transform
   ...
   */
  svtkSetClampMacro(ProjectionPlaneMode, int, SVTK_DELAUNAY_XY_PLANE, SVTK_BEST_FITTING_PLANE);
  svtkGetMacro(ProjectionPlaneMode, int);
  //@}

  /**
   * This method computes the best fit plane to a set of points represented
   * by a svtkPointSet. The method constructs a transform and returns it on
   * successful completion (null otherwise). The user is responsible for
   * deleting the transform instance.
   */
  static svtkAbstractTransform* ComputeBestFittingPlane(svtkPointSet* input);

protected:
  svtkDelaunay2D();
  ~svtkDelaunay2D() override;
...
private:
  svtkPolyData* Mesh; // the created mesh
  double* Points;    // the raw points in double precision
  void SetPoint(svtkIdType id, double* x)
  {
    svtkIdType idx = 3 * id;
    this->Points[idx] = x[0];
  ...
  int* RecoverBoundary(svtkPolyData* source);
  int RecoverEdge(svtkPolyData* source, svtkIdType p1, svtkIdType p2);
  void FillPolygons(svtkCellArray* polys, int* triUse);
...
private:
  svtkDelaunay2D(const svtkDelaunay2D&) = delete;
  void operator=(const svtkDelaunay2D&) = delete;
};
