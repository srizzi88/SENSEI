...
svtkStandardNewMacro(svtkFlyingEdges2D);

// This templated class is the heart of the algorithm. Templated across
// scalar type T. svtkFlyingEdges2D populates the information in this class
// and then invokes ContourImage() to actually initiate executions.
template <class T>
class svtkFlyingEdges2DAlgorithm
{
public:
  ...
  void InterpolateAxesEdge(double value, T* s0, int ijk0[3], T* s1, int ijk1[3], svtkIdType vId)
  {
  ...
  {
  public:
    Pass1(svtkFlyingEdges2DAlgorithm<TT>* algo, double value)
    {
      this->Algo = algo;
      this->Value = value;
    }
    svtkFlyingEdges2DAlgorithm<TT>* Algo;
    double Value;
  ...
  {
  public:
    Pass2(svtkFlyingEdges2DAlgorithm<TT>* algo) { this->Algo = algo; }
    svtkFlyingEdges2DAlgorithm<TT>* Algo;
    void operator()(svtkIdType row, svtkIdType end)
    {
      for (; row < end; ++row)
  ...
  {
  public:
    Pass4(svtkFlyingEdges2DAlgorithm<TT>* algo, double value)
    {
      this->Algo = algo;
      this->Value = value;
    }
  ...
  static void ContourImage(svtkFlyingEdges2D* self, T* scalars, svtkPoints* newPts,
...
template <class T>
const unsigned char svtkFlyingEdges2DAlgorithm<T>::VertMap[4][2] = {
  { 0, 1 },
  { 2, 3 },
  { 0, 2 },
  { 1, 3 },
...
template <class T>
const unsigned char svtkFlyingEdges2DAlgorithm<T>::VertOffsets[4][2] = {
  { 0, 0 },
  { 1, 0 },
  { 0, 1 },
  { 1, 1 },
...
template <class T>
const unsigned char svtkFlyingEdges2DAlgorithm<T>::EdgeCases[16][5] = {
  { 0, 0, 0, 0, 0 },
  { 1, 0, 2, 0, 0 },
  { 1, 3, 0, 0, 0 },
  { 1, 3, 2, 0, 0 },
...
template <class T>
svtkFlyingEdges2DAlgorithm<T>::svtkFlyingEdges2DAlgorithm()
  : XCases(nullptr)
...
template <class T>
void svtkFlyingEdges2DAlgorithm<T>::InterpolateEdge(double value, T* s, int ijk[3],
  unsigned char edgeNum, unsigned char edgeUses[12], svtkIdType* eIds)
{
...
template <class T>
void svtkFlyingEdges2DAlgorithm<T>::GeneratePoints(
  double value, unsigned char loc, T* sPtr, int ijk[3], unsigned char* edgeUses, svtkIdType* eIds)
{
...
template <class T>
void svtkFlyingEdges2DAlgorithm<T>::ProcessXEdge(double value, T* inPtr, svtkIdType row)
{
  ...
  for (svtkIdType i = 0; i < nxcells; ++i, ++ePtr)
  {
    ...
    edgeCase =
      (s0 < value ? svtkFlyingEdges2DAlgorithm::Below : svtkFlyingEdges2DAlgorithm::LeftAbove);
    edgeCase |=
      (s1 < value ? svtkFlyingEdges2DAlgorithm::Below : svtkFlyingEdges2DAlgorithm::RightAbove);

    this->SetXEdge(ePtr, edgeCase);

    // if edge intersects contour
    if (edgeCase == svtkFlyingEdges2DAlgorithm::LeftAbove ||
      edgeCase == svtkFlyingEdges2DAlgorithm::RightAbove)
    {
...
// work.
template <class T>
void svtkFlyingEdges2DAlgorithm<T>::ProcessYEdges(svtkIdType row)
{
...
// algorithm.
template <class T>
void svtkFlyingEdges2DAlgorithm<T>::GenerateOutput(double value, T* rowPtr, svtkIdType row)
{
  svtkIdType* eMD0 = this->EdgeMetaData + row * 5;
  svtkIdType* eMD1 = this->EdgeMetaData + (row + 1) * 5;
...
// svtkFlyingEdges2D class with the templated algorithm class. It also invokes
// the three passes of the Flying Edges algorithm.
//
template <class T>
void svtkFlyingEdges2DAlgorithm<T>::ContourImage(svtkFlyingEdges2D* self, T* scalars,
  ...
  int* updateExt)
{
  ...
  svtkFlyingEdges2DAlgorithm<T> algo;
...
svtkFlyingEdges2D::svtkFlyingEdges2D()
{
...
svtkMTimeType svtkFlyingEdges2D::GetMTime()
{
...
//
int svtkFlyingEdges2D::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  {
    svtkTemplateMacro(svtkFlyingEdges2DAlgorithm<SVTK_TT>::ContourImage(
...
//----------------------------------------------------------------------------
int svtkFlyingEdges2D::FillInputPortInformation(int, svtkInformation* info)
{
...
//----------------------------------------------------------------------------
void svtkFlyingEdges2D::PrintSelf(ostream& os, svtkIndent indent)
{
