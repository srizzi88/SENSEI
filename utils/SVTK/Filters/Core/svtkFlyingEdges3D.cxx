...
svtkStandardNewMacro(svtkFlyingEdges3D);

//----------------------------------------------------------------------------
namespace
{
// This templated class implements the heart of the algorithm.
// svtkFlyingEdges3D populates the information in this class and
// then invokes Contour() to actually initiate execution.
template <class T>
class svtkFlyingEdges3DAlgorithm
{
public:
  ...
  {
  public:
    svtkFlyingEdges3DAlgorithm<TT>* Algo;
    double Value;
    Pass1(svtkFlyingEdges3DAlgorithm<TT>* algo, double value)
    {
      this->Algo = algo;
      this->Value = value;
    }
  ...
  {
  public:
    Pass2(svtkFlyingEdges3DAlgorithm<TT>* algo) { this->Algo = algo; }
    svtkFlyingEdges3DAlgorithm<TT>* Algo;
    void operator()(svtkIdType slice, svtkIdType end)
    {
      for (; slice < end; ++slice)
  ...
  {
  public:
    Pass4(svtkFlyingEdges3DAlgorithm<TT>* algo, double value)
    {
      this->Algo = algo;
      this->Value = value;
    }
  ...
  static void Contour(svtkFlyingEdges3D* self, svtkImageData* input, svtkDataArray* inScalars,
...
template <class T>
const unsigned char svtkFlyingEdges3DAlgorithm<T>::EdgeMap[12] = { 0, 5, 1, 4, 2, 7, 3, 6, 8, 9, 10,
  11 };

//----------------------------------------------------------------------------
// Map MC edges numbering to use the saner FlyingEdges edge numbering scheme.
template <class T>
const unsigned char svtkFlyingEdges3DAlgorithm<T>::VertMap[12][2] = {
  { 0, 1 },
  { 2, 3 },
  { 4, 5 },
  { 6, 7 },
...
template <class T>
const unsigned char svtkFlyingEdges3DAlgorithm<T>::VertOffsets[8][3] = {
  { 0, 0, 0 },
  { 1, 0, 0 },
  { 0, 1, 0 },
  { 1, 1, 0 },
...
template <class T>
svtkFlyingEdges3DAlgorithm<T>::svtkFlyingEdges3DAlgorithm()
  : XCases(nullptr)
...
template <class T>
void svtkFlyingEdges3DAlgorithm<T>::CountBoundaryYZInts(
...
// volume.
template <class T>
void svtkFlyingEdges3DAlgorithm<T>::ComputeBoundaryGradient(svtkIdType ijk[3],
...
template <class T>
void svtkFlyingEdges3DAlgorithm<T>::InterpolateEdge(double value, svtkIdType ijk[3], T const* const s,
  const int incs[3], unsigned char edgeNum, unsigned char const* const edgeUses, svtkIdType* eIds)
{
...
template <class T>
void svtkFlyingEdges3DAlgorithm<T>::GeneratePoints(double value, unsigned char loc, svtkIdType ijk[3],
  T const* const sPtr, const int incs[3], unsigned char const* const edgeUses, svtkIdType* eIds)
{
...
// trimming).
template <class T>
void svtkFlyingEdges3DAlgorithm<T>::ProcessXEdge(
  double value, T const* const inPtr, svtkIdType row, svtkIdType slice)
{
  ...
  for (svtkIdType i = 0; i < nxcells; ++i, ++ePtr)
  {
    ...
    if (s0 >= value)
    {
      edgeCase = svtkFlyingEdges3DAlgorithm::LeftAbove;
    }
    else
    {
      edgeCase = svtkFlyingEdges3DAlgorithm::Below;
    }
    if (s1 >= value)
    {
      edgeCase |= svtkFlyingEdges3DAlgorithm::RightAbove;
    }

    this->SetXEdge(ePtr, edgeCase);

    // if edge intersects contour
    if (edgeCase == svtkFlyingEdges3DAlgorithm::LeftAbove ||
      edgeCase == svtkFlyingEdges3DAlgorithm::RightAbove)
    {
...
// information.
template <class T>
void svtkFlyingEdges3DAlgorithm<T>::ProcessYZEdges(svtkIdType row, svtkIdType slice)
{
...
// algorithm.
template <class T>
void svtkFlyingEdges3DAlgorithm<T>::GenerateOutput(
...
// interfaces the svtkFlyingEdges3D class with the templated algorithm
// class. It also invokes the three passes of the Flying Edges algorithm.
template <class T>
void svtkFlyingEdges3DAlgorithm<T>::Contour(svtkFlyingEdges3D* self, svtkImageData* input,
  svtkDataArray* inScalars, int extent[6], svtkIdType* incs, T* scalars, svtkPolyData* output,
  svtkPoints* newPts, svtkCellArray* newTris, svtkDataArray* newScalars, svtkFloatArray* newNormals,
  svtkFloatArray* newGradients)
{
  ...
  svtkFlyingEdges3DAlgorithm<T> algo;
...
svtkFlyingEdges3D::svtkFlyingEdges3D()
{
...
svtkMTimeType svtkFlyingEdges3D::GetMTime()
{
...
//----------------------------------------------------------------------------
int svtkFlyingEdges3D::RequestUpdateExtent(svtkInformation* svtkNotUsed(request),
...
//----------------------------------------------------------------------------
int svtkFlyingEdges3D::RequestData(
  svtkInformation* request, svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  {
    svtkTemplateMacro(svtkFlyingEdges3DAlgorithm<SVTK_TT>::Contour(this, input, inScalars, exExt, incs,
      (SVTK_TT*)ptr, output, newPts, newTris, newScalars, newNormals, newGradients));
  }
...
//----------------------------------------------------------------------------
int svtkFlyingEdges3D::FillInputPortInformation(int, svtkInformation* info)
{
...
//----------------------------------------------------------------------------
void svtkFlyingEdges3D::PrintSelf(ostream& os, svtkIndent indent)
{
