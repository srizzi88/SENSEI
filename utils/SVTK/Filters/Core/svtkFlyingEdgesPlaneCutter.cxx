/*=========================================================================
  ...
  Module:    svtkFlyingEdgesPlaneCutter.cxx
...
svtkStandardNewMacro(svtkFlyingEdgesPlaneCutter);
svtkCxxSetObjectMacro(svtkFlyingEdgesPlaneCutter, Plane, svtkPlane);

//----------------------------------------------------------------------------
namespace
{
// This templated class implements the heart of the algorithm.
// svtkFlyingEdgesPlaneCutter populates the information in this class and
// then invokes Contour() to actually initiate execution.
template <class T>
class svtkFlyingEdgesPlaneCutterAlgorithm
{
public:
  ...
  // Setup algorithm
  svtkFlyingEdgesPlaneCutterAlgorithm();
  ...
  void ProcessXEdge(double xL[3], double xR[3], svtkIdType row, svtkIdType slice); // PASS 1
  void ProcessYZEdges(svtkIdType row, svtkIdType slice);                           // PASS 2
  void GenerateOutput(T* inPtr, svtkIdType row, svtkIdType slice);                 // PASS 4
  ...
  void InterpolateAxesEdge(double t, svtkIdType vId, const int incs[3], double x0[3], double x1[3],
    const double s0, const double s1, svtkIdType ijk0[3], svtkIdType ijk1[3])
  {
  ...
  void InterpolateEdge(svtkIdType ijk[3], T const* const s, const int incs[3], double x[3],
  ...
  void GeneratePoints(unsigned char loc, svtkIdType ijk[3], const T* sPtr, const int incs[3],
    double x[3], const double sV, unsigned char const* const edgeUses, svtkIdType* eIds);
  ...
  {
  public:
    svtkFlyingEdgesPlaneCutterAlgorithm<TT>* Algo;
    Pass1(svtkFlyingEdgesPlaneCutterAlgorithm<TT>* algo) { this->Algo = algo; }
  ...
  {
  public:
    Pass2(svtkFlyingEdgesPlaneCutterAlgorithm<TT>* algo) { this->Algo = algo; }
    svtkFlyingEdgesPlaneCutterAlgorithm<TT>* Algo;
    void operator()(svtkIdType slice, svtkIdType end)
    {
      for (; slice < end; ++slice)
  ...
  {
  public:
    Pass4(svtkFlyingEdgesPlaneCutterAlgorithm<TT>* algo) { this->Algo = algo; }
    svtkFlyingEdgesPlaneCutterAlgorithm<TT>* Algo;
    void operator()(svtkIdType slice, svtkIdType end)
    {
      svtkIdType row;
  ...
  static void Contour(svtkFlyingEdgesPlaneCutter* self, svtkImageData* input, svtkDataArray* inScalars,
    int extent[6], svtkIdType* incs, T* scalars, svtkPolyData* output, svtkPoints* newPts,
    svtkCellArray* newTris, svtkDataArray* newScalars, svtkDataArray* newNormals);
};

//----------------------------------------------------------------------------
// Map MC edges numbering to use the saner FlyingEdges edge numbering scheme.
template <class T>
const unsigned char svtkFlyingEdgesPlaneCutterAlgorithm<T>::EdgeMap[12] = { 0, 5, 1, 4, 2, 7, 3, 6,
  8, 9, 10, 11 };

//----------------------------------------------------------------------------
// Map MC edges numbering to use the saner FlyingEdges edge numbering scheme.
template <class T>
const unsigned char svtkFlyingEdgesPlaneCutterAlgorithm<T>::VertMap[12][2] = { { 0, 1 }, { 2, 3 },
...
template <class T>
const unsigned char svtkFlyingEdgesPlaneCutterAlgorithm<T>::VertOffsets[8][3] = { { 0, 0, 0 },
...
template <class T>
svtkFlyingEdgesPlaneCutterAlgorithm<T>::svtkFlyingEdgesPlaneCutterAlgorithm()
  : XCases(nullptr)
...
template <class T>
void svtkFlyingEdgesPlaneCutterAlgorithm<T>::CountBoundaryYZInts(
...
template <class T>
void svtkFlyingEdgesPlaneCutterAlgorithm<T>::InterpolateEdge(svtkIdType ijk[3], T const* const s,
  ...
  svtkIdType* eIds)
{
  ...
  double sV0 = svtkPlane::Evaluate(this->Normal, this->Center, x0);
  double sV1 = svtkPlane::Evaluate(this->Normal, this->Center, x1);
...
template <class T>
void svtkFlyingEdgesPlaneCutterAlgorithm<T>::GeneratePoints(unsigned char loc, svtkIdType ijk[3],
  ...
  unsigned char const* const edgeUses, svtkIdType* eIds)
{
  ...
  for (int i = 0; i < 3; ++i)
  {
    if (edgeUses[i * 4])
    {
      ...
      sV1 = svtkPlane::Evaluate(this->Normal, this->Center, x1);
      double t = -sV0 / (sV1 - sV0);
...
// trimming).
//
// This method varies significantly from the first pass of svtkFlyingEdges3D
// because the evaluation of the x-edges is performed by intersection against
// the cutting plane.
template <class T>
void svtkFlyingEdgesPlaneCutterAlgorithm<T>::ProcessXEdge(
  double xL[3], double xR[3], svtkIdType row, svtkIdType slice)
{
  ...
  svtkIdType numInts;
  double a, b;
  double vL = svtkPlane::Evaluate(this->Normal, this->Center, xL);
  double vR = svtkPlane::Evaluate(this->Normal, this->Center, xR);
  if (vL >= 0.0 && vR >= 0.0)
  { // also catches the case where vL=vR==0.0
    numInts = 0;
    std::fill_n(ePtr, nxcells, svtkFlyingEdgesPlaneCutterAlgorithm::BothAbove);
    minInt = nxcells;
    maxInt = 0;
  }
  ...
  {
    numInts = 0;
    std::fill_n(ePtr, nxcells, svtkFlyingEdgesPlaneCutterAlgorithm::Below);
    minInt = nxcells;
    maxInt = 0;
  }
  else
  { // volume edge intersects plane
    ...
    a = fabs(vL);
    b = fabs(vR);
    minInt = static_cast<svtkIdType>((a * nxcells) / (a + b));
    minInt = minInt >= nxcells ? nxcells - 1 : minInt;
    maxInt = minInt + 1;
    if (vL < 0.0) //&& vR >= 0.0
    {
      std::fill_n(ePtr, minInt, svtkFlyingEdgesPlaneCutterAlgorithm::Below);
      this->SetXEdge(ePtr + minInt, svtkFlyingEdgesPlaneCutterAlgorithm::RightAbove);
      std::fill_n(ePtr + maxInt, nxcells - maxInt, svtkFlyingEdgesPlaneCutterAlgorithm::BothAbove);
    }
    else // vL >= 0.0 && vR < 0.0
    {
      std::fill_n(ePtr, minInt, svtkFlyingEdgesPlaneCutterAlgorithm::BothAbove);
      this->SetXEdge(ePtr + minInt, svtkFlyingEdgesPlaneCutterAlgorithm::LeftAbove);
      std::fill_n(ePtr + maxInt, nxcells - maxInt, svtkFlyingEdgesPlaneCutterAlgorithm::Below);
    }
  } // volume edge intersects plane
...
// information.
template <class T>
void svtkFlyingEdgesPlaneCutterAlgorithm<T>::ProcessYZEdges(svtkIdType row, svtkIdType slice)
{
...
// algorithm.
template <class T>
void svtkFlyingEdgesPlaneCutterAlgorithm<T>::GenerateOutput(
  T* rowPtr, svtkIdType row, svtkIdType slice)
{
  // Grab the edge meta data surrounding the voxel row.
  svtkIdType* eMD[4];
  eMD[0] = this->EdgeMetaData + (slice * this->Dims[1] + row) * 6; // this x-edge
  ...
  for (i = xL; i < xR; ++i)
  {
    ...
    if (numTris > 0)
    {
      ...
      if (this->CaseIncludesAxes(eCase) || loc != Interior)
      {
        ...
        sV = svtkPlane::Evaluate(this->Normal, this->Center, x);
...
// interfaces the svtkFlyingEdgesPlaneCutter class with the templated algorithm
// class. It also invokes the three passes of the Flying Edges algorithm.
template <class T>
void svtkFlyingEdgesPlaneCutterAlgorithm<T>::Contour(svtkFlyingEdgesPlaneCutter* self,
  ...
  svtkDataArray* newNormals)
{
  ...
  svtkFlyingEdgesPlaneCutterAlgorithm<T> algo;
...
svtkFlyingEdgesPlaneCutter::svtkFlyingEdgesPlaneCutter()
{
...
svtkMTimeType svtkFlyingEdgesPlaneCutter::GetMTime()
{
...
//----------------------------------------------------------------------------
int svtkFlyingEdgesPlaneCutter::RequestUpdateExtent(svtkInformation* svtkNotUsed(request),
...
//----------------------------------------------------------------------------
int svtkFlyingEdgesPlaneCutter::RequestData(
  svtkInformation* request, svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  {
    svtkDebugMacro(<< "Cutting requires 3D data");
    return 0;
  }
  ...
  {
    svtkDebugMacro("No scalars for cutting.");
    return 0;
  }
  ...
  svtkDataArray* newScalars = inScalars->NewInstance();
  ...
  {
    svtkTemplateMacro(svtkFlyingEdgesPlaneCutterAlgorithm<SVTK_TT>::Contour(this, input, inScalars,
      exExt, incs, (SVTK_TT*)ptr, output, newPts, newTris, newScalars, newNormals));
  }
...
//----------------------------------------------------------------------------
int svtkFlyingEdgesPlaneCutter::FillInputPortInformation(int, svtkInformation* info)
{
...
//----------------------------------------------------------------------------
void svtkFlyingEdgesPlaneCutter::PrintSelf(ostream& os, svtkIndent indent)
{
