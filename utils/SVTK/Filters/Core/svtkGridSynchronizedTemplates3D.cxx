/*=========================================================================
  ...
  Module:    svtkGridSynchronizedTemplates3D.cxx
...
svtkStandardNewMacro(svtkGridSynchronizedTemplates3D);
...
svtkGridSynchronizedTemplates3D::svtkGridSynchronizedTemplates3D()
{
...
//----------------------------------------------------------------------------
void svtkGridSynchronizedTemplates3D::SetInputMemoryLimit(long svtkNotUsed(limit))
{
  svtkErrorMacro(<< "This filter no longer supports a memory limit.");
  svtkErrorMacro(<< "This filter no longer initiates streaming.");
  svtkErrorMacro(<< "Please use a .... after this filter to achieve similar functionality.");
}
...
svtkMTimeType svtkGridSynchronizedTemplates3D::GetMTime()
{
...
//----------------------------------------------------------------------------
static void svtkGridSynchronizedTemplates3DInitializeOutput(int* ext, int precision,
  svtkStructuredGrid* input, svtkPolyData* o, svtkFloatArray* scalars, svtkFloatArray* normals,
  ...
  int i, int j, int k, int inExt[6], int incY, int incZ, T* sc, PointsType* pt, double g[3])
{
  ...
  NtNi2[1] = &(NtNi[1][0]);
  NtNi2[2] = &(NtNi[2][0]);
  if (svtkMath::InvertMatrix(NtN2, NtNi2, 3, tmpIntArray, tmpDoubleArray) == 0)
  {
    svtkGenericWarningMacro("Cannot compute gradient of grid");
    return;
  }
...
//----------------------------------------------------------------------------
#define SVTK_CSP3PA(i2, j2, k2, s, p, grad, norm)                                                   \
  if (NeedGradients)                                                                               \
  {                                                                                                \
    if (!g0)                                                                                       \
    {                                                                                              \
...
{
public:
  CellVisibility(svtkStructuredGrid* structuredGrid)
    : MASKED_CELL_VALUE(svtkDataSetAttributes::HIDDENCELL | svtkDataSetAttributes::REFINEDCELL)
    ...
    , InputCellGhostArray(nullptr)
  {
    ...
    else if (this->InputStructuredGrid->HasAnyBlankPoints())
    {
      ...
      // svtkStructuredGrid::IsCellVisible;
  ...
  bool operator()(svtkIdType id) const
  {
    switch (Procedure)
    {
      case CELLS:
...
private:
  ...
  svtkStructuredGrid* InputStructuredGrid;
...
template <class T, class PointsType>
void ContourGrid(svtkGridSynchronizedTemplates3D* self, int* exExt, T* scalars,
  svtkStructuredGrid* input, svtkPolyData* output, PointsType*, svtkDataArray* inScalars,
  bool outputTriangles)
{
  ...
  svtkGridSynchronizedTemplates3DInitializeOutput(exExt, self->GetOutputPointsPrecision(), input,
            ...
            if (v0 ^ v1)
            {
              ...
              if (*isect2Ptr == -1)
              {
                ...
                *isect2Ptr = newPts->InsertNextPoint(x);
                SVTK_CSP3PA(i + 1, j, k, s1, p1, grad, norm);
            ...
            if (v0 ^ v2)
            {
              ...
              if (*(isect2Ptr + 1) == -1)
              {
                ...
                *(isect2Ptr + 1) = newPts->InsertNextPoint(x);
                SVTK_CSP3PA(i, j + 1, k, s2, p2, grad, norm);
            ...
            if (v0 ^ v3)
            {
              ...
              if (*(isect2Ptr + 2) == -1)
              {
                ...
                *(isect2Ptr + 2) = newPts->InsertNextPoint(x);
                SVTK_CSP3PA(i, j, k + 1, s3, p3, grad, norm);
...
template <class T>
void ContourGrid(svtkGridSynchronizedTemplates3D* self, int* exExt, T* scalars,
  svtkStructuredGrid* input, svtkPolyData* output, svtkDataArray* inScalars, bool outputTriangles)
{
  switch (input->GetPoints()->GetData()->GetDataType())
  {
    svtkTemplateMacro(ContourGrid(self, exExt, scalars, input, output, static_cast<SVTK_TT*>(nullptr),
...
void svtkGridSynchronizedTemplates3D::ThreadedExecute(
  svtkStructuredGrid* input, svtkInformationVector** inputVector, svtkInformation* outInfo)
{
  ...
  {
    svtkErrorMacro(<< "3D structured contours requires 3D data");
    return;
  }
  ...
  if (inScalars->GetNumberOfComponents() == 1)
  {
    ...
    {
      svtkTemplateMacro(ContourGrid(this, exExt, static_cast<SVTK_TT*>(scalars), input, output,
        inScalars, this->GenerateTriangles != 0));
    } // switch
  }
  else // multiple components - have to convert
  {
    svtkDoubleArray* image = svtkDoubleArray::New();
  ...
  svtkDebugMacro(<< "Produced: " << output->GetNumberOfPoints() << " points, "
...
//----------------------------------------------------------------------------
int svtkGridSynchronizedTemplates3D::RequestUpdateExtent(svtkInformation* svtkNotUsed(request),
...
//----------------------------------------------------------------------------
int svtkGridSynchronizedTemplates3D::FillInputPortInformation(int, svtkInformation* info)
{
...
//----------------------------------------------------------------------------
void svtkGridSynchronizedTemplates3D::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
int svtkGridSynchronizedTemplates3D::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
