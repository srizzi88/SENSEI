...
int svtkHull::AddPlane(double A, double B, double C)
{
  double* tmpPointer;
  int i;
  ...
  if (norm == 0.0)
  {
    svtkErrorMacro(<< "Zero length vector not allowed for plane normal!");
    return -SVTK_INT_MAX;
  }
  A /= norm;
  B /= norm;
  ...
  if ((this->NumberOfPlanes + 1) >= this->PlanesStorageSize)
  {
    ...
    if (!this->Planes)
    {
      svtkErrorMacro(<< "Unable to allocate space for planes");
...
// of three doubles.
int svtkHull::AddPlane(double plane[3])
{
...
// with this method.
void svtkHull::SetPlane(int i, double A, double B, double C)
{
  double norm;
  ...
  {
    svtkErrorMacro(<< "Invalid index in SetPlane");
    return;
  }
...
void svtkHull::SetPlane(int i, double plane[3])
{
  this->SetPlane(i, plane[0], plane[1], plane[2]);
}

int svtkHull::AddPlane(double A, double B, double C, double D)
{
  int i, j;
...
int svtkHull::AddPlane(double plane[3], double D)
{
  int i, j;
...
void svtkHull::SetPlane(int i, double A, double B, double C, double D)
{
  if (i >= 0 && i < this->NumberOfPlanes)
  {
...
void svtkHull::SetPlane(int i, double plane[3], double D)
{
  this->SetPlane(i, plane[0], plane[1], plane[2], D);
}

void svtkHull::SetPlanes(svtkPlanes* planes)
{
  ...
  {
    int i, idx;
    svtkPoints* points = planes->GetPoints();
    svtkDataArray* normals = planes->GetNormals();
    if (points && normals)
    {
...
void svtkHull::AddCubeFacePlanes()
{
...
void svtkHull::AddCubeEdgePlanes()
{
...
void svtkHull::AddCubeVertexPlanes()
{
...
// planes, and so on.
void svtkHull::AddRecursiveSpherePlanes(int level)
{
  int numTriangles;
  double* points;
  int* triangles;
  ...
  if (level < 0)
  {
    svtkErrorMacro(<< "Cannot have a level less than 0!");
    return;
  }

  if (level > 10)
  {
    svtkErrorMacro(<< "Cannot have a level greater than 10!");
    return;
  }
...
// set of planes.
int svtkHull::RequestData(svtkInformation* svtkNotUsed(request), svtkInformationVector** inputVector,
  svtkInformationVector* outputVector)
{
  ...
  if (numPoints < 3)
  {
    svtkErrorMacro(<< "There must be >= 3 points in the input data!!!\n");
    return 1;
  }
  ...
  {
    svtkErrorMacro(<< "There must be >= 4 planes!!!\n");
    return 1;
  }
...
// the hull.
void svtkHull::ComputePlaneDistances(svtkPolyData* input)
{
  svtkIdType i;
  int j;
  double coord[3];
...
void svtkHull::ClipPolygonsFromPlanes(
  svtkPoints* outPoints, svtkCellArray* outPolys, const double* bounds)
{
  int i, j, k, q;
  ...
  pnts = static_cast<svtkIdType*>(malloc(sizeof(svtkIdType) * oldSize));
  ...
  for (i = 0; i < this->NumberOfPlanes; i++)
  {
    ...
    if (vertCount > 0)
    {
      ...
      {
        svtkIdType* tmpPointer =
          static_cast<svtkIdType*>(realloc(pnts, vertCount * sizeof(svtkIdType)));
        if (tmpPointer == nullptr)
        {
          svtkErrorMacro(<< "Unable to allocate space for PointIds");
        }
        pnts = tmpPointer;
        oldSize = vertCount;
      }
...
void svtkHull::CreateInitialPolygon(double* verts, int i, const double* bounds)
{
...
void svtkHull::GenerateHull(
  svtkPolyData* pd, double xmin, double xmax, double ymin, double ymax, double zmin, double zmax)
{
  double bounds[6];
  bounds[0] = xmin;
  bounds[1] = xmax;
...
void svtkHull::GenerateHull(svtkPolyData* pd, double* bounds)
{
...
// Print the object
void svtkHull::PrintSelf(ostream& os, svtkIndent indent)
{
  int i;
