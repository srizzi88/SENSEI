 ...
 *
 * svtkHull is a filter which will produce an n-sided convex hull given a
 ...
 * svtkPolyData, until the planes just touch the svtkPolyData. Then,
 ...
 *
 * The output of this filter can be used in combination with svtkLODActor
...
class SVTKFILTERSCORE_EXPORT svtkHull : public svtkPolyDataAlgorithm
{
public:
  static svtkHull* New();
  svtkTypeMacro(svtkHull, svtkPolyDataAlgorithm);
   ...
   * a svtkPlanes* argument, where it is set to the D value defined there.
   */
   ...
   */
  void SetPlanes(svtkPlanes* planes);

  //@{
  /**
   * Get the number of planes in the current set of planes.
   */
  svtkGetMacro(NumberOfPlanes, int);
  //@}
   ...
   * instance of svtkPolyData into which the points and cells defining
   ...
   */
  void GenerateHull(svtkPolyData* pd, double* bounds);
  void GenerateHull(
...
protected:
  ...
  void ComputePlaneDistances(svtkPolyData* input);

  // Internal method used to create the actual polygons from the set
  // of planes
  void ClipPolygonsFromPlanes(svtkPoints* points, svtkCellArray* polys, const double* bounds);
...
private:
  svtkHull(const svtkHull&) = delete;
  void operator=(const svtkHull&) = delete;
};
