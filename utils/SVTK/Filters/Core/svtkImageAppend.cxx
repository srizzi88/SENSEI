...
svtkStandardNewMacro(svtkImageAppend);

//----------------------------------------------------------------------------
svtkImageAppend::svtkImageAppend()
{
  this->AppendAxis = 0;
  this->Shifts = nullptr;
...
//----------------------------------------------------------------------------
void svtkImageAppend::ReplaceNthInputConnection(int idx, svtkAlgorithmOutput* input)
{
...
void svtkImageAppend::SetInputData(int idx, svtkDataObject* input)
{
  this->SetInputDataInternal(idx, input);
}

//----------------------------------------------------------------------------
svtkDataObject* svtkImageAppend::GetInput(int idx)
{
...
int svtkImageAppend::RequestInformation(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  unionExt[0] = unionExt[2] = unionExt[4] = SVTK_INT_MAX;
  unionExt[1] = unionExt[3] = unionExt[5] = -SVTK_INT_MAX;
  ...
  inExt = inInfo->Get(svtkStreamingDemandDrivenPipeline::WHOLE_EXTENT());
  ...
  {
    inInfo = inputVector[0]->GetInformationObject(0);
    inInfo->Get(svtkStreamingDemandDrivenPipeline::WHOLE_EXTENT(), outExt);
    max = tmp - 1;
...
//----------------------------------------------------------------------------
void svtkImageAppend::InternalComputeInputUpdateExtent(
...
//----------------------------------------------------------------------------
int svtkImageAppend::RequestUpdateExtent(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
...
//----------------------------------------------------------------------------
static void svtkImageAppendGetContinuousIncrements(int wExtent[6], int sExtent[6], svtkIdType nComp,
  bool forCells, svtkIdType& incX, svtkIdType& incY, svtkIdType& incZ)
{
  // I can not use the one in svtkImageData, since that assumes point scalars
...
template <class T>
void svtkImageAppendExecute(svtkImageAppend* self, int id, int inExt[6], svtkImageData* inData,
  T* inPtr, int outExt[6], svtkImageData* outData, T* outPtr, svtkIdType numComp, bool forCells,
  int nArrays)
{
  int idxR, idxY, idxZ;
  int maxX, maxY, maxZ;
  ...
  svtkImageAppendGetContinuousIncrements(
...
//----------------------------------------------------------------------------
void svtkImageAppend::InitOutput(int outExt[6], svtkImageData* outData)
{
  int idxY, idxZ;
  int maxY, maxZ;
  svtkIdType outIncY, outIncZ;
  int rowLength;
  int typeSize;
  ...
  svtkPointData* pd = outData->GetPointData();
  for (int arrayIdx = 0; arrayIdx < pd->GetNumberOfArrays(); arrayIdx++)
  {
    svtkDataArray* array = pd->GetArray(arrayIdx);
    if (!array)
    {
      continue;
    }

    typeSize = svtkDataArray::GetDataTypeSize(array->GetDataType());
    outPtrZ = static_cast<unsigned char*>(outData->GetArrayPointerForExtent(array, outExt));
...
void svtkImageAppend::ThreadedRequestData(svtkInformation* svtkNotUsed(request),
  ...
  svtkImageData*** inData, svtkImageData** outData, int outExt[6], int id)
{
  ...
  for (idx1 = 0; idx1 < this->GetNumberOfInputConnections(0); ++idx1)
  {
    ...
    if (inData[0][idx1] != nullptr)
    {
      ...
      cOutExt[this->AppendAxis * 2] = inExt[this->AppendAxis * 2] + this->Shifts[idx1];
      ...
      if (inExt[0] <= inExt[1] && inExt[2] <= inExt[3] && inExt[4] <= inExt[5])
      {
        ...
        for (ai = 0; ai < inData[0][idx1]->GetPointData()->GetNumberOfArrays(); ai++)
        {
          ...
          {
            svtkTemplateMacro(
              svtkImageAppendExecute(this, id, inExt, inData[0][idx1], static_cast<SVTK_TT*>(inPtr),
                cOutExt, outData[0], static_cast<SVTK_TT*>(outPtr), numComp, false, nArrays));
            default:
        ...
        for (ai = 0; ai < inData[0][idx1]->GetCellData()->GetNumberOfArrays(); ai++)
        {
          ...
          svtkIdType cellId;
          cellId = svtkStructuredData::ComputeCellIdForExtent(inExt, c_in);
          inPtr = inArray->GetVoidPointer(cellId * numComp);
          cellId = svtkStructuredData::ComputeCellIdForExtent(outExt, c_out);
          ...
          switch (inArray->GetDataType())
          {
            svtkTemplateMacro(
              svtkImageAppendExecute(this, id, inExt, inData[0][idx1], static_cast<SVTK_TT*>(inPtr),
                cOutExt, outData[0], static_cast<SVTK_TT*>(outPtr), numComp, true, nArrays));
            default:
              svtkErrorMacro(<< "Execute: Unknown ScalarType");
              return;
          }
...
//----------------------------------------------------------------------------
int svtkImageAppend::FillInputPortInformation(int i, svtkInformation* info)
{
...
//----------------------------------------------------------------------------
void svtkImageAppend::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
void svtkImageAppend::AllocateOutputData(svtkImageData* output, svtkInformation*, int* uExtent)
{
  output->SetExtent(uExtent);

  // compute number of cells and points in the uExtent
  svtkIdType numpts = 1;
  svtkIdType numcells = 1;
  for (int i = 0; i < 3; i++)
  {
    if (uExtent[i * 2 + 1] >= uExtent[i * 2])
    {
      svtkIdType dim = uExtent[i * 2 + 1] - uExtent[i * 2];
      numpts = numpts * (dim + 1);
      if (dim != 0)
      {
  ...
  svtkImageData* in = svtkImageData::SafeDownCast(this->GetInputDataObject(0, 0));

  svtkDataSetAttributes *ifd, *ofd;
  ifd = in->GetPointData();
...
//----------------------------------------------------------------------------
svtkImageData* svtkImageAppend::AllocateOutputData(svtkDataObject* output, svtkInformation* outInfo)
{
...
//----------------------------------------------------------------------------
void svtkImageAppend::CopyAttributeData(svtkImageData* svtkNotUsed(input),
  svtkImageData* svtkNotUsed(output), svtkInformationVector** svtkNotUsed(inputVector))
{
