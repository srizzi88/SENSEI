...
=========================================================================*/
/**
 * @class   svtkMaskFields
 * @brief   Allow control of which fields get passed
 * to the output
 *
 * svtkMaskFields is used to mark which fields in the input dataset
 ...
 * svtkSplitField svtkMergeFields svtkAssignAttribute
 */

#ifndef svtkMaskFields_h
#define svtkMaskFields_h
...
class SVTKFILTERSCORE_EXPORT svtkMaskFields : public svtkDataSetAlgorithm
{
public:
  svtkTypeMacro(svtkMaskFields, svtkDataSetAlgorithm);
  void PrintSelf(ostream& os, svtkIndent indent) override;

  /**
   * Create a new svtkMaskFields.
   */
  static svtkMaskFields* New();
   ...
   * @verbatim
   * maskFields->CopyFieldOff(svtkMaskFields::CELL_DATA, "foo");
   * @endverbatim
  ...
  /**
   * Turn on/off the copying of the attribute or specified by svtkDataSetAttributes:AttributeTypes.
   * During the copying/passing, the following rules are followed for each
   * array:
   * 1. If the copy flag for an array is set (on or off), it is applied
   * This overrides rule 2.
   ...
   * @verbatim
   * maskFields->CopyAttributeOff(svtkMaskFields::POINT_DATA, svtkDataSetAttributes::SCALARS);
   * @endverbatim
...
private:
  svtkMaskFields(const svtkMaskFields&) = delete;
  void operator=(const svtkMaskFields&) = delete;
};
