...
svtkStandardNewMacro(svtkMassProperties);

//----------------------------------------------------------------------------
// TODO: replace with std::cbrt() when C++11 is minimum requirement.
static inline double svtkCubeRoot(double x)
{
  if (x > 0.0)
  {
...
svtkMassProperties::svtkMassProperties()
{
...
svtkMassProperties::~svtkMassProperties() = default;
...
int svtkMassProperties::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* svtkNotUsed(outputVector))
{
  ...
  svtkIdType cellId, numCells, numPts, numIds;
  double p[3];
  ...
  {
    svtkErrorMacro(<< "No data to measure...!");
    return 1;
  }
  ...
  {
    if (input->GetCellType(cellId) != SVTK_TRIANGLE)
    {
      svtkWarningMacro(<< "Input data type must be SVTK_TRIANGLE not " << input->GetCellType(cellId));
      continue;
    }
    ...
    {
      svtkErrorMacro(<< "Unpredicted situation...!");
      return 1;
    }
  ...
  this->NormalizedShapeIndex = (sqrt(surfacearea) / svtkCubeRoot(this->Volume)) / 2.199085233;

  return 1;
}

//----------------------------------------------------------------------------
void svtkMassProperties::PrintSelf(ostream& os, svtkIndent indent)
{
  this->Superclass::PrintSelf(os, indent);

  svtkPolyData* input = svtkPolyData::SafeDownCast(this->GetInput(0));
  if (!input)
  {
    return;
  }
