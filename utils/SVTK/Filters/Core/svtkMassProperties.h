...
=========================================================================*/
/**
 * @class   svtkMassProperties
 * @brief   estimate volume, area, shape index of triangle mesh
 *
 * svtkMassProperties estimates the volume, the surface area, and the
 ...
 *
 * @warning
 * Currently only triangles are processed. Use svtkTriangleFilter to convert
 * any strips or polygons to triangles. If multiple closed objects are
 * defined consider using svtkMultiObjectMassProperties. Alternatively,
 * svtkPolyDataConnectivityFilter can be used to extract connected regions
 * (i.e., objects) one at a time, and then each object can be processed by
 * this filter.
 *
 * @sa
 * svtkTriangleFilter svtkMultiObjectMassProperties
 */

#ifndef svtkMassProperties_h
#define svtkMassProperties_h

#include "svtkFiltersCoreModule.h" // For export macro
#include "svtkPolyDataAlgorithm.h"

class SVTKFILTERSCORE_EXPORT svtkMassProperties : public svtkPolyDataAlgorithm
{
public:
  /**
   * Constructs with initial values of zero.
   */
  static svtkMassProperties* New();

  svtkTypeMacro(svtkMassProperties, svtkPolyDataAlgorithm);
...
private:
  svtkMassProperties(const svtkMassProperties&) = delete;
  void operator=(const svtkMassProperties&) = delete;
};
