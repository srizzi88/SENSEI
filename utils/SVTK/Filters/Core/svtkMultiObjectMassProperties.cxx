/*=========================================================================
  ...
  Module:    svtkMultiObjectMassProperties.cxx
...
=========================================================================*/
#include "svtkMultiObjectMassProperties.h"
...
svtkStandardNewMacro(svtkMultiObjectMassProperties);
...
struct ComputeProperties
{
  ...
  svtkSMPThreadLocalObject<svtkPolygon> Polygon;
  svtkSMPThreadLocalObject<svtkIdList> Triangles;

  ComputeProperties(
    svtkPolyData* mesh, double center[3], unsigned char* orient, double* areas, double* volumes)
    : Mesh(mesh)
    , Orient(orient)
    , Areas(areas)
    , Volumes(volumes)
  ...
  void Initialize()
  {
    svtkPolygon*& polygon = this->Polygon.Local();
    polygon->PointIds->Allocate(128);
    polygon->Points->Allocate(128);

    svtkIdList*& tris = this->Triangles.Local();
  ...
  void operator()(svtkIdType polyId, svtkIdType endPolyId)
  {
    ...
    svtkIdType numTris;
    svtkPolygon*& poly = this->Polygon.Local();
    svtkIdList*& tris = this->Triangles.Local();
    int i;
  ...
  static void Execute(svtkIdType numPolys, svtkPolyData* output, double center[3],
    unsigned char* orient, double* areas, double* volumes)
  {
    ComputeProperties compute(output, center, orient, areas, volumes);
    svtkSMPTools::For(0, numPolys, compute);
  }
};
...
svtkMultiObjectMassProperties::svtkMultiObjectMassProperties()
{
  ...
  this->CellNeighbors = svtkIdList::New();
  this->Wave = nullptr;
  this->Wave2 = nullptr;
}
...
int svtkMultiObjectMassProperties::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  svtkIdType numPolys = input->GetPolys()->GetNumberOfCells();
  ...
  {
    svtkWarningMacro(<< "Skipping some non-poly cells");

    // Pass points through, can always use svtkCleanPolyData to eliminate
    // unused points.
    ...
    for (polyId = 0, cellId = 0; cellId < numCells; ++cellId)
    {
      ...
      if (cellType == SVTK_TRIANGLE || cellType == SVTK_QUAD || cellType == SVTK_POLYGON)
      {
  ...
  int idx;
  svtkIdType* objectIds;
  svtkIdTypeArray* objectIdsArray =
    static_cast<svtkIdTypeArray*>(output->GetCellData()->GetArray("ObjectIds", idx));
  if (!this->SkipValidityCheck || objectIdsArray == nullptr ||
    objectIdsArray->GetDataType() != SVTK_ID_TYPE)
  {
    performValidityCheck = true;
    this->ObjectIds = objectIdsArray = svtkIdTypeArray::New();
  ...
  this->ObjectValidity = svtkUnsignedCharArray::New();
  ...
  svtkDoubleArray* polyAreas = svtkDoubleArray::New();
  ...
  svtkDoubleArray* polyVolumes = svtkDoubleArray::New();
  ...
  this->ObjectAreas = svtkDoubleArray::New();
  ...
  this->ObjectVolumes = svtkDoubleArray::New();
...
void svtkMultiObjectMassProperties::TraverseAndMark(
  svtkPolyData* output, svtkIdType* objectIds, svtkDataArray* valid, unsigned char* orient)
{
  svtkIdType i, j, k, numIds, polyId, npts, p0, p1, numNei, neiId;
  const svtkIdType* pts;
...
//----------------------------------------------------------------------------
void svtkMultiObjectMassProperties::PrintSelf(ostream& os, svtkIndent indent)
{
