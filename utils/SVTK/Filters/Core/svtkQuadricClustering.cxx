/*=========================================================================
  ...
  Module:    svtkQuadricClustering.cxx
...
svtkStandardNewMacro(svtkQuadricClustering);

//----------------------------------------------------------------------------
// PIMPLd STL set for keeping track of inserted cells
struct svtkQuadricClusteringIdTypeHash
{
  size_t operator()(svtkIdType val) const { return static_cast<size_t>(val); }
};
class svtkQuadricClusteringCellSet
  : public std::unordered_set<svtkIdType, svtkQuadricClusteringIdTypeHash>
{
};
typedef svtkQuadricClusteringCellSet::iterator svtkQuadricClusteringCellSetIterator;
...
svtkQuadricClustering::svtkQuadricClustering()
{
  ...
  this->FeatureEdges = svtkFeatureEdges::New();
  this->FeatureEdges->FeatureEdgesOff();
  this->FeatureEdges->BoundaryEdgesOn();
  this->FeaturePoints = svtkPoints::New();
...
//----------------------------------------------------------------------------
int svtkQuadricClustering::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  svtkPolyData* input = nullptr;
  if (inInfo)
  {
    input = svtkPolyData::SafeDownCast(inInfo->Get(svtkDataObject::DATA_OBJECT()));
  }
  svtkPolyData* output = svtkPolyData::SafeDownCast(outInfo->Get(svtkDataObject::DATA_OBJECT()));

  svtkTimerLog* tlog = nullptr;
  ...
  if (this->Debug)
  {
    tlog = svtkTimerLog::New();
    tlog->StartTimer();
  }

  // Lets limit the number of divisions based on
  // the number of points in the input.
  // (To minimize chance of overflow, force math in svtkIdType type,
  // which is sometimes bigger than int, and never smaller.)
  svtkIdType target = input->GetNumberOfPoints();
  svtkIdType numDiv = static_cast<svtkIdType>(this->NumberOfXDivisions) * this->NumberOfYDivisions *
  ...
  {
    tlog->StopTimer();
    svtkDebugMacro(<< "Execution took: " << tlog->GetElapsedTime() << " seconds.");
    tlog->Delete();
  }
...
//----------------------------------------------------------------------------
void svtkQuadricClustering::StartAppend(double* bounds)
{
  // If there are duplicate triangles. remove them
  if (this->PreventDuplicateCells)
  {
    this->CellSet = new svtkQuadricClusteringCellSet;
    this->NumberOfBins =
  ...
  if (this->OutputTriangleArray)
  {
    this->OutputTriangleArray->Delete();
    this->OutputTriangleArray = nullptr;
    // svtkWarningMacro("Array already created.  Did you call EndAppend?");
  }
  if (this->OutputLines)
  {
    this->OutputLines->Delete();
  ...
  this->OutputTriangleArray = svtkCellArray::New();
  this->OutputLines = svtkCellArray::New();
  ...
  this->NumberOfBinsUsed = 0;
  delete[] this->QuadricArray;
  this->QuadricArray = new svtkQuadricClustering::PointQuadric[this->NumberOfDivisions[0] *
    this->NumberOfDivisions[1] * this->NumberOfDivisions[2]];
  if (this->QuadricArray == nullptr)
  {
    svtkErrorMacro("Could not allocate quadric grid.");
    return;
  }

  svtkInformation* inInfo = this->GetExecutive()->GetInputInformation(0, 0);
  svtkInformation* outInfo = this->GetExecutive()->GetOutputInformation(0);
  svtkPolyData* input = nullptr;
  if (inInfo)
  {
    input = svtkPolyData::SafeDownCast(inInfo->Get(svtkDataObject::DATA_OBJECT()));
...
//----------------------------------------------------------------------------
void svtkQuadricClustering::Append(svtkPolyData* pd)
{
  svtkCellArray *inputPolys, *inputStrips, *inputLines, *inputVerts;
  svtkPoints* inputPoints = pd->GetPoints();

  // Check for mis-use of the Append methods.
  if (this->OutputTriangleArray == nullptr || this->OutputLines == nullptr)
  {
    svtkDebugMacro("Missing Array:  Did you call StartAppend?");
    return;
  }
...
//----------------------------------------------------------------------------
void svtkQuadricClustering::AddPolygons(
  svtkCellArray* polys, svtkPoints* points, int geometryFlag, svtkPolyData* input, svtkPolyData* output)
{
  ...
  for (polys->InitTraversal(); polys->GetNextCell(numPts, ptIds);)
  {
    ...
    for (svtkIdType j = 0; j < numPts - 2; j++) // creates triangles; assumes poly is convex
    {
...
//----------------------------------------------------------------------------
void svtkQuadricClustering::AddStrips(svtkCellArray* strips, svtkPoints* points, int geometryFlag,
  svtkPolyData* input, svtkPolyData* output)
{
  const svtkIdType* ptIds = nullptr;
  svtkIdType numPts = 0;
  double pts[3][3];
...
//----------------------------------------------------------------------------
inline void svtkQuadricClustering::InitializeQuadric(double quadric[9])
{
  quadric[0] = 0.0;
  quadric[1] = 0.0;
  quadric[2] = 0.0;
...
void svtkQuadricClustering::AddTriangle(svtkIdType* binIds, double* pt0, double* pt1, double* pt2,
  int geometryFlag, svtkPolyData* input, svtkPolyData* output)
{
  ...
  svtkTriangle::ComputeQuadric(pt0, pt1, pt2, quadric4x4);
  ...
  if (geometryFlag)
  {
    ...
    if (binIds[0] != binIds[1] && binIds[0] != binIds[2] && binIds[1] != binIds[2])
    {
      if (this->PreventDuplicateCells)
      {
        svtkIdType minIdx = (binIds[0] < binIds[1] ? (binIds[0] < binIds[2] ? 0 : 2)
        ...
        svtkIdType idx = binIds[minIdx] + this->NumberOfBins * binIds[midIdx] +
...
//----------------------------------------------------------------------------
void svtkQuadricClustering::AddEdges(
  svtkCellArray* edges, svtkPoints* points, int geometryFlag, svtkPolyData* input, svtkPolyData* output)
{
  svtkIdType numCells;
  const svtkIdType* ptIds = nullptr;
  svtkIdType numPts = 0;
...
// only the quadric is affected.
void svtkQuadricClustering::AddEdge(svtkIdType* binIds, double* pt0, double* pt1, int geometryFlag,
  svtkPolyData* input, svtkPolyData* output)
{
  svtkIdType edgePtIds[2];
  double length2, tmp;
  double d[3];
...
//----------------------------------------------------------------------------
void svtkQuadricClustering::AddVertices(
  svtkCellArray* verts, svtkPoints* points, int geometryFlag, svtkPolyData* input, svtkPolyData* output)
{
  svtkIdType numCells;
  const svtkIdType* ptIds = nullptr;
  svtkIdType numPts = 0;
...
// only the quadric is affected.
void svtkQuadricClustering::AddVertex(
  svtkIdType binId, double* pt, int geometryFlag, svtkPolyData* input, svtkPolyData* output)
{
  double q[9];
...
//----------------------------------------------------------------------------
void svtkQuadricClustering::AddQuadric(svtkIdType binId, double quadric[9])
{
...
//----------------------------------------------------------------------------
svtkIdType svtkQuadricClustering::HashPoint(double point[3])
{
  svtkIdType xBinCoord = static_cast<svtkIdType>((point[0] - this->Bounds[0]) * this->XBinStep);
  if (xBinCoord < 0)
  {
    xBinCoord = 0;
  }
  ...
  svtkIdType yBinCoord = static_cast<svtkIdType>((point[1] - this->Bounds[2]) * this->YBinStep);
  if (yBinCoord < 0)
  {
    yBinCoord = 0;
  }
  ...
  svtkIdType zBinCoord = static_cast<svtkIdType>((point[2] - this->Bounds[4]) * this->ZBinStep);
  if (zBinCoord < 0)
  {
    zBinCoord = 0;
  }
...
//----------------------------------------------------------------------------
void svtkQuadricClustering::EndAppend()
{
  svtkInformation* inInfo = this->GetExecutive()->GetInputInformation(0, 0);
  svtkInformation* outInfo = this->GetExecutive()->GetOutputInformation(0);
  svtkPolyData* input = nullptr;
  ...
  outputPoints = svtkPoints::New();
  for (svtkIdType i = 0; !abortExecute && i < numBuckets; i++)
  {
    if (cstep > step)
    {
      cstep = 0;
      svtkDebugMacro(<< "Finding point in bin #" << i);
...
//----------------------------------------------------------------------------
void svtkQuadricClustering::ComputeRepresentativePoint(
  double quadric[9], svtkIdType binId, double point[3])
{
  ...
  svtkIdType x = binId % this->NumberOfDivisions[0];
  svtkIdType y = (binId / this->NumberOfDivisions[0]) % this->NumberOfDivisions[1];
  svtkIdType z = binId / this->SliceSize;
  ...
  //
#define SVTK_SVTHRESHOLD 1.0E-3
  ...
  for (int i = 0; i < 3; i++)
  {
    for (int j = 0; j < 3; j++)
    {
      if (i == j)
      {
        if (fabs(w[i] / maxW) > SVTK_SVTHRESHOLD)
        {
...
#undef SVTK_SVTHRESHOLD

  svtkMath::Transpose3x3(U, UT);
  svtkMath::Transpose3x3(VT, V);
  svtkMath::Multiply3x3(W, UT, tempMatrix);
  svtkMath::Multiply3x3(V, tempMatrix, tempMatrix);
  svtkMath::Multiply3x3(A, cellCenter, tempVector);
  for (int i = 0; i < 3; i++)
  {
    tempVector[i] = b[i] - tempVector[i];
  }
  svtkMath::Multiply3x3(tempMatrix, tempVector, tempVector);
  ...
  double deltaMag = svtkMath::Norm(tempVector);
...
//----------------------------------------------------------------------------
void svtkQuadricClustering::SetNumberOfDivisions(int div0, int div1, int div2)
{
...
//----------------------------------------------------------------------------
void svtkQuadricClustering::SetNumberOfXDivisions(int num)
{
  ...
  if (num < 1)
  {
    svtkErrorMacro("You cannot use less than one division.");
    return;
  }
  this->Modified();
...
//----------------------------------------------------------------------------
void svtkQuadricClustering::SetNumberOfYDivisions(int num)
{
...
//----------------------------------------------------------------------------
void svtkQuadricClustering::SetNumberOfZDivisions(int num)
{
...
//----------------------------------------------------------------------------
int* svtkQuadricClustering::GetNumberOfDivisions()
{
  static int divs[3];
...
//----------------------------------------------------------------------------
void svtkQuadricClustering::GetNumberOfDivisions(int divs[3])
{
...
//----------------------------------------------------------------------------
void svtkQuadricClustering::SetDivisionOrigin(double x, double y, double z)
{
...
//----------------------------------------------------------------------------
void svtkQuadricClustering::SetDivisionSpacing(double x, double y, double z)
{
  ...
  if (x <= 0)
  {
    svtkErrorMacro(<< "Spacing (x) should be larger than 0.0, setting to 1.0");
    x = 1.0;
  }
  if (y <= 0)
  {
    svtkErrorMacro(<< "Spacing (y) should be larger than 0.0, setting to 1.0");
    y = 1.0;
  }
  if (z <= 0)
  {
    svtkErrorMacro(<< "Spacing (z) should be larger than 0.0, setting to 1.0");
    z = 1.0;
  }
  this->Modified();
...
//----------------------------------------------------------------------------
void svtkQuadricClustering::EndAppendUsingPoints(svtkPolyData* input, svtkPolyData* output)
{
  svtkIdType outPtId;
  svtkPoints* inputPoints;
  svtkPoints* outputPoints;
  svtkIdType numPoints, numBins;
  svtkIdType binId;
  ...
  for (svtkIdType i = 0; i < numBins; ++i)
  {
    minError[i] = SVTK_DOUBLE_MAX;
  }
...
void svtkQuadricClustering::EndAppendVertexGeometry(svtkPolyData* input, svtkPolyData* output)
{
  svtkCellArray *inVerts, *outVerts;
  svtkIdType* tmp = nullptr;
  svtkIdType tmpLength = 0;
  svtkIdType tmpIdx;
  double pt[3];
  const svtkIdType* ptIds = nullptr;
  svtkIdType numPts = 0;
  svtkIdType outPtId;
  svtkIdType binId, cellId, outCellId;

  inVerts = input->GetVerts();
  outVerts = svtkCellArray::New();
...
void svtkQuadricClustering::AppendFeatureQuadrics(svtkPolyData* pd, svtkPolyData* output)
{
  svtkPolyData* input = svtkPolyData::New();
  svtkPoints* edgePts;
  svtkCellArray* edges;
  svtkIdType binId;
  double featurePt[3];
  ...
  if (edges && edges->GetNumberOfCells() && edgePts)
  {
    ...
    if (this->UseFeaturePoints)
    {
      ...
      for (svtkIdType i = 0; i < this->FeaturePoints->GetNumberOfPoints(); i++)
      {
...
void svtkQuadricClustering::FindFeaturePoints(
  svtkCellArray* edges, svtkPoints* edgePts, double svtkNotUsed(angle))
{
  svtkIdType pointIds[2];
  const svtkIdType* cellPts = nullptr;
  svtkIdType numCellPts;
  svtkIdList* pointIdList = svtkIdList::New();
  svtkIdType numPts = edgePts->GetNumberOfPoints();
  svtkIdType numEdges = edges->GetNumberOfCells();
  svtkIdType edgeCount;
  svtkIdType** pointTable = new svtkIdType*[numPts];
  double featurePoint[3];
  double featureEdges[2][3];
  double point1[3], point2[3];
  const svtkIdType* cellPointIds;
  double radAngle = svtkMath::RadiansFromDegrees(this->FeaturePointsAngle);

  this->FeaturePoints->Allocate(numPts);

  for (svtkIdType i = 0; i < numPts; i++)
  {
    pointTable[i] = new svtkIdType[4];
    pointTable[i][1] = 0;
  }
  ...
  for (svtkIdType i = 0; i < numPts; i++)
  {
    ...
    else if (pointTable[i][1] == 2)
    {
      for (int j = 0; j < 2; j++)
      {
        ...
        svtkMath::Normalize(featureEdges[j]);
      }
      if (acos(svtkMath::Dot(featureEdges[0], featureEdges[1])) < radAngle)
      {
        edgePts->GetPoint(pointTable[i][0], featurePoint);
      }
    }
...
//----------------------------------------------------------------------------
int svtkQuadricClustering::FillInputPortInformation(int port, svtkInformation* info)
{
...
//----------------------------------------------------------------------------
void svtkQuadricClustering::PrintSelf(ostream& os, svtkIndent indent)
{
