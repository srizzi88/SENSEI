/*=========================================================================
  ...
  Module:    svtkQuadricDecimation.cxx
...
#include "svtkQuadricDecimation.h"
...
svtkStandardNewMacro(svtkQuadricDecimation);

//----------------------------------------------------------------------------
svtkQuadricDecimation::svtkQuadricDecimation()
{
  this->Edges = svtkEdgeTable::New();
  this->EdgeCosts = svtkPriorityQueue::New();
  this->EndPoint1List = svtkIdList::New();
  this->EndPoint2List = svtkIdList::New();
  this->ErrorQuadrics = nullptr;
  this->VolumeConstraints = nullptr;
  this->TargetPoints = svtkDoubleArray::New();
...
void svtkQuadricDecimation::SetPointAttributeArray(svtkIdType ptId, const double* x)
{
  int i;
...
void svtkQuadricDecimation::GetPointAttributeArray(svtkIdType ptId, double* x)
{
  int i;
...
//----------------------------------------------------------------------------
int svtkQuadricDecimation::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  svtkIdType numTris = input->GetNumberOfPolys();
  svtkIdType edgeId, i;
  int j;
  double cost;
  double* x;
  svtkCellArray* polys;
  svtkDataArray* attrib;
  svtkPoints* points;
  svtkPointData* pointData;
  svtkIdType endPtIds[2];
  svtkIdList* outputCellList;
  svtkIdType npts;
  const svtkIdType* pts;
  svtkIdType numDeletedTris = 0;
  ...
  {
    svtkErrorMacro("Nothing to decimate");
    return 1;
  }

  if (input->GetPolys()->GetMaxCellSize() > 3)
  {
    svtkErrorMacro("Can only decimate triangles");
    return 1;
  }

  polys = svtkCellArray::New();
  points = svtkPoints::New();
  pointData = svtkPointData::New();
  outputCellList = svtkIdList::New();
  ...
  this->ErrorQuadrics = new svtkQuadricDecimation::ErrorQuadric[numPts];
  ...
  svtkDebugMacro(<< "Computing Edges");
  ...
  this->CollapseCellIds = svtkIdList::New();
  ...
  int abort = 0;
  while (
    !abort && edgeId >= 0 && cost < SVTK_DOUBLE_MAX && this->ActualReduction < this->TargetReduction)
  {
    if (!(this->NumberOfEdgeCollapses % 10000))
    {
      svtkDebugMacro(<< "Collapsing edge#" << this->NumberOfEdgeCollapses);
    ...
    svtkDebugMacro(<< "Cost: " << cost << " Edge: " << endPtIds[0] << " " << endPtIds[1]);
  ...
  svtkDebugMacro(<< "Number Of Edge Collapses: " << this->NumberOfEdgeCollapses
                << " Cost: " << cost);
  ...
  for (i = 0; i < this->Mesh->GetNumberOfCells(); i++)
  {
    if (this->Mesh->GetCell(i)->GetCellType() != SVTK_EMPTY_CELL)
    {
  ...
  if (this->AttributeErrorMetric)
  {
    if (nullptr != (attrib = output->GetPointData()->GetNormals()))
    {
      ...
      {
        svtkMath::Normalize(attrib->GetTuple3(i));
      }
    }
...
//----------------------------------------------------------------------------
void svtkQuadricDecimation::InitializeQuadrics(svtkIdType numPts)
{
  svtkPolyData* input = this->Mesh;
  double* QEM;
  svtkIdType ptId;
  int i, j;
  svtkCellArray* polys;
  ...
  for (polys->InitTraversal(); polys->GetNextCell(npts, pts);)
  {
    ...
    if (this->AttributeErrorMetric)
    {
      ...
      if (svtkMath::LUFactorLinearSystem(A, index, 4))
      {
        for (i = 0; i < this->NumberOfComponents; i++)
        {
          x[3] = 0;
          ...
          svtkMath::LUSolveLinearSystem(A, index, x, 4);
      ...
      {
        svtkErrorMacro(<< "Unable to factor attribute matrix!");
      }
    }
...
void svtkQuadricDecimation::AddBoundaryConstraints()
{
  svtkPolyData* input = this->Mesh;
  double* QEM;
  svtkIdType cellId;
  ...
  for (cellId = 0; cellId < input->GetNumberOfCells(); cellId++)
  {
    ...
    for (i = 0; i < 3; i++)
    {
      ...
      if (cellIds->GetNumberOfIds() == 0)
      {
        ...
        // triangle
        c = svtkMath::Dot(e0, e1) / (e0[0] * e0[0] + e0[1] * e0[1] + e0[2] * e0[2]);
        for (j = 0; j < 3; j++)
        {
          n[j] = e1[j] - c * e0[j];
        }
...
//----------------------------------------------------------------------------
void svtkQuadricDecimation::AddQuadric(svtkIdType oldPtId, svtkIdType newPtId)
{
  int i;
...
//----------------------------------------------------------------------------
void svtkQuadricDecimation::FindAffectedEdges(svtkIdType p1Id, svtkIdType p2Id, svtkIdList* edges)
{
  svtkIdType ncells;
  svtkIdType *cells, edgeId;
  svtkIdType npts;
  const svtkIdType* pts;
  svtkIdType i, j;
...
void svtkQuadricDecimation::UpdateEdgeData(svtkIdType pt0Id, svtkIdType pt1Id)
{
  svtkIdList* changedEdges = svtkIdList::New();
  svtkIdType i, edgeId, edge[2];
  double cost;
...
//----------------------------------------------------------------------------
double svtkQuadricDecimation::ComputeCost(svtkIdType edgeId, double* x)
{
  ...
  norm = svtkMath::Norm(A[0]);
  normTemp = svtkMath::Norm(A[1]);
  norm = norm > normTemp ? norm : normTemp;
  normTemp = svtkMath::Norm(A[2]);
  norm = norm > normTemp ? norm : normTemp;

  if (fabs(svtkMath::Determinant3x3(A)) / (norm * norm * norm) > errorNumber)
  {
    // it would be better to use the normal of the matrix to test singularity??
    svtkMath::LinearSolve3x3(A, b, x);
    svtkMath::Multiply3x3(A, x, temp);
  ...
  else
  {
    ...
    svtkMath::Multiply3x3(A, v, temp2);
    if (svtkMath::Dot(temp2, temp2) > errorNumber)
    {
      svtkMath::Multiply3x3(A, pt1, temp);
      for (i = 0; i < 3; i++)
        temp[i] = b[i] - temp[i];
      c = svtkMath::Dot(temp2, temp) / svtkMath::Dot(temp2, temp2);
      for (i = 0; i < 3; i++)
        x[i] = pt1[i] + c * v[i];
    }
    else
...
//----------------------------------------------------------------------------
double svtkQuadricDecimation::ComputeCost2(svtkIdType edgeId, double* x)
{
  ...
  solveOk = svtkMath::SolveLinearSystem(
...
int svtkQuadricDecimation::CollapseEdge(svtkIdType pt0Id, svtkIdType pt1Id)
{
  int j, numDeleted = 0;
  svtkIdType i, cellId;
  svtkIdType npts;
...
int svtkQuadricDecimation::TrianglePlaneCheck(
...
int svtkQuadricDecimation::IsGoodPlacement(svtkIdType pt0Id, svtkIdType pt1Id, const double* x)
{
  svtkIdType ncells, i;
  svtkIdType ptId, *cells;
  svtkIdType npts;
  const svtkIdType* pts;
...
void svtkQuadricDecimation::ComputeNumberOfComponents()
{
  svtkPointData* pd = this->Mesh->GetPointData();
  int i, j;
  ...
  if (pd->GetScalars() != nullptr && this->ScalarsAttribute)
  {
    ...
    svtkDebugMacro("scalars " << this->NumberOfComponents << " " << this->AttributeScale[0]);
  }
  ...
  if (pd->GetVectors() != nullptr && this->VectorsAttribute)
  {
    ...
    svtkDebugMacro("vectors " << this->NumberOfComponents << " " << this->AttributeScale[1]);
  }
  ...
  if (pd->GetNormals() != nullptr && this->NormalsAttribute)
  {
    ...
    svtkDebugMacro("normals " << this->NumberOfComponents << " " << this->AttributeScale[2]);
  }
  ...
  if (pd->GetTCoords() != nullptr && this->TCoordsAttribute)
  {
    ...
    svtkDebugMacro("tcoords " << this->NumberOfComponents << " " << this->AttributeScale[3]);
  }
  ...
  {
    svtkDataArray* inTensors = pd->GetTensors();
    ...
    svtkDebugMacro("tensors " << this->NumberOfComponents << " " << this->AttributeScale[4]);
  }
  this->AttributeComponents[4] = this->NumberOfComponents;

  svtkDebugMacro("Number of components: " << this->NumberOfComponents);
}

//----------------------------------------------------------------------------
void svtkQuadricDecimation::PrintSelf(ostream& os, svtkIndent indent)
{
