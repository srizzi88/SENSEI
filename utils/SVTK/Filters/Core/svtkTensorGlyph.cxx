...
svtkStandardNewMacro(svtkTensorGlyph);

// Construct object with scaling on and scale factor 1.0. Eigenvalues are
// extracted, glyphs are colored with input scalar data, and logarithmic
// scaling is turned off.
svtkTensorGlyph::svtkTensorGlyph()
{
  this->Scaling = 1;
  ...
  this->SetInputArrayToProcess(
    0, 0, 0, svtkDataObject::FIELD_ASSOCIATION_POINTS, svtkDataSetAttributes::TENSORS);

  // by default, process active point scalars
  this->SetInputArrayToProcess(
    1, 0, 0, svtkDataObject::FIELD_ASSOCIATION_POINTS, svtkDataSetAttributes::SCALARS);
}

//----------------------------------------------------------------------------
svtkTensorGlyph::~svtkTensorGlyph() = default;

//----------------------------------------------------------------------------
int svtkTensorGlyph::RequestUpdateExtent(svtkInformation* svtkNotUsed(request),
...
//----------------------------------------------------------------------------
int svtkTensorGlyph::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  // get the info objects
  svtkInformation* inInfo = inputVector[0]->GetInformationObject(0);
  ...
  int j;
  svtkPoints* sourcePts;
  svtkDataArray* sourceNormals;
  svtkCellArray *sourceCells, *cells;
  svtkPoints* newPts;
  ...
  svtkDebugMacro(<< "Generating tensor glyphs");
  ...
  {
    svtkErrorMacro(<< "No data to glyph!");
    return 1;
  }

  pts = new svtkIdType[source->GetMaxCellSize()];
  ...
  svtkPointData* pd = this->GetSource()->GetPointData();
  ...
  for (inPtId = 0; inPtId < numPts; inPtId++)
  {
    ...
    else // use tensor columns as eigenvectors
    {
      ...
      w[0] = svtkMath::Normalize(xv);
      w[1] = svtkMath::Normalize(yv);
      w[2] = svtkMath::Normalize(zv);
    }
    ...
    for (dir = 0; dir < numDirs; dir++)
    {
      ...
      // old behaviour of svtkTensorGlyphs (which only used one dir),
  ...
  svtkDebugMacro(<< "Generated " << numPts << " tensor glyphs");
  //
...
//----------------------------------------------------------------------------
void svtkTensorGlyph::SetSourceConnection(int id, svtkAlgorithmOutput* algOutput)
{
  if (id < 0)
  {
...
//----------------------------------------------------------------------------
void svtkTensorGlyph::SetSourceData(svtkPolyData* source)
{
  this->SetInputData(1, source);
}

//----------------------------------------------------------------------------
svtkPolyData* svtkTensorGlyph::GetSource()
{
...
//----------------------------------------------------------------------------
int svtkTensorGlyph::FillInputPortInformation(int port, svtkInformation* info)
{
  if (port == 1)
  {
...
//----------------------------------------------------------------------------
void svtkTensorGlyph::PrintSelf(ostream& os, svtkIndent indent)
{
