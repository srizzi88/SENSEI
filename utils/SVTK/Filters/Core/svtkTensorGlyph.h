 ...
 * of tensor
 *
 * svtkTensorGlyph is a filter that copies a geometric representation
 ...
 * additional capability over the svtkGlyph3D object. That is, the
 ...
 *
 * @sa
 * svtkGlyph3D svtkPointLoad svtkHyperStreamline
 */

#ifndef svtkTensorGlyph_h
#define svtkTensorGlyph_h

#include "svtkFiltersCoreModule.h" // For export macro
#include "svtkPolyDataAlgorithm.h"

class SVTKFILTERSCORE_EXPORT svtkTensorGlyph : public svtkPolyDataAlgorithm
{
public:
  svtkTypeMacro(svtkTensorGlyph, svtkPolyDataAlgorithm);
   ...
   */
  svtkSetMacro(ThreeGlyphs, svtkTypeBool);
  svtkGetMacro(ThreeGlyphs, svtkTypeBool);
  svtkBooleanMacro(ThreeGlyphs, svtkTypeBool);
  //@}
   ...
   */
  svtkSetMacro(ExtractEigenvalues, svtkTypeBool);
  svtkBooleanMacro(ExtractEigenvalues, svtkTypeBool);
  svtkGetMacro(ExtractEigenvalues, svtkTypeBool);
  //@}
   ...
   */
  svtkSetMacro(ColorGlyphs, svtkTypeBool);
  svtkGetMacro(ColorGlyphs, svtkTypeBool);
  svtkBooleanMacro(ColorGlyphs, svtkTypeBool);
  //@}
   ...
   */
  svtkSetClampMacro(ColorMode, int, COLOR_BY_SCALARS, COLOR_BY_EIGENVALUES);
   ...
   */
  svtkSetMacro(ClampScaling, svtkTypeBool);
  svtkGetMacro(ClampScaling, svtkTypeBool);
  svtkBooleanMacro(ClampScaling, svtkTypeBool);
  //@}
   ...
   */
  svtkSetMacro(MaxScaleFactor, double);
  svtkGetMacro(MaxScaleFactor, double);
  //@}

protected:
  svtkTensorGlyph();
  ~svtkTensorGlyph() override;
  ...
  svtkTypeBool ExtractEigenvalues; // Boolean controls eigenfunction extraction
  svtkTypeBool ColorGlyphs;        // Boolean controls coloring with input scalar data
  int ColorMode;                  // The coloring mode to use for the glyphs.
  svtkTypeBool ClampScaling;       // Boolean controls whether scaling is clamped.
  double MaxScaleFactor;          // Maximum scale factor (ScaleFactor*eigenvalue)
  svtkTypeBool ThreeGlyphs;        // Boolean controls drawing 1 or 3 glyphs
  svtkTypeBool Symmetric;          // Boolean controls drawing a "mirror" of each glyph
  double Length;                  // Distance, in x, from the origin to the end of the glyph
private:
  svtkTensorGlyph(const svtkTensorGlyph&) = delete;
  void operator=(const svtkTensorGlyph&) = delete;
};
