...
svtkStandardNewMacro(svtkTubeFilter);

// Construct object with radius 0.5, radius variation turned off, the number
// of sides set to 3, and radius factor of 10.
svtkTubeFilter::svtkTubeFilter()
{
  this->Radius = 0.5;
  this->VaryRadius = SVTK_VARY_RADIUS_OFF;
  ...
  this->GenerateTCoords = SVTK_TCOORDS_OFF;
...
{
  IdPointsEqual(svtkPoints* points)
    : Points(points)
  {
  }

  bool operator()(svtkIdType id1, svtkIdType id2) const
  {
    double p1[3], p2[3];
...
int svtkTubeFilter::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  svtkIdType npts = 0;
  const svtkIdType* ptsOrig = nullptr;
  svtkIdType offset = 0;
  ...
  if (inScalars)
  {
    ...
    {
      if (this->VaryRadius == SVTK_VARY_RADIUS_BY_SCALAR)
      {
        svtkWarningMacro(<< "Scalar range is zero!");
      }
      range[1] = range[0] + 1.0;
    }
    if (this->VaryRadius == SVTK_VARY_RADIUS_BY_ABSOLUTE_SCALAR)
    {
      ...
      if (range[0] < 0.0)
      {
        svtkWarningMacro(<< "Scalar values fall below zero when using absolute radius values!");
      }
    }
  }
  if (inVectors)
  ...
  //
  this->Theta = 2.0 * svtkMath::Pi() / this->NumberOfSides;
  ...
  for (inLines->InitTraversal(); inLines->GetNextCell(npts, ptsOrig) && !abort; inCellId++)
  {
    ...
    std::vector<svtkIdType> ptsCopy(ptsOrig, ptsOrig + npts);
    svtkIdType* pts = ptsCopy.data();

    // remove degenerate lines to avoid warnings
    npts = static_cast<svtkIdType>(std::unique(pts, pts + npts, IdPointsEqual(inPts)) - pts);
    if (npts < 2)
    {
      continue; // skip tubing this polyline
    }
...
int svtkTubeFilter::GeneratePoints(svtkIdType offset, svtkIdType npts, const svtkIdType* pts,
  svtkPoints* inPts, svtkPoints* newPts, svtkPointData* pd, svtkPointData* outPD,
  svtkFloatArray* newNormals, svtkDataArray* inScalars, double range[2], svtkDataArray* inVectors,
  double maxSpeed, svtkDataArray* inNormals)
{
  svtkIdType j;
  int i, k;
  double p[3];
  ...
  for (j = 0; j < npts; j++)
  {
    if (j == 0) // first point
    {
      ...
      svtkMath::Normalize(startCapNorm);
    }
    ...
    {
      svtkDebugMacro(<< "Using alternate bevel vector");
    ...
    */
    ...
    if (inScalars && this->VaryRadius == SVTK_VARY_RADIUS_BY_SCALAR)
    {
      sFactor = 1.0 +
        ((this->RadiusFactor - 1.0) * (inScalars->GetComponent(pts[j], 0) - range[0]) /
          (range[1] - range[0]));
    }
    else if (inVectors && this->VaryRadius == SVTK_VARY_RADIUS_BY_VECTOR)
    {
      sFactor = sqrt((double)maxSpeed / svtkMath::Norm(inVectors->GetTuple(pts[j])));
      if (sFactor > this->RadiusFactor)
      {
        sFactor = this->RadiusFactor;
      }
    }
    else if (inScalars && this->VaryRadius == SVTK_VARY_RADIUS_BY_ABSOLUTE_SCALAR)
    {
      sFactor = inScalars->GetComponent(pts[j], 0);
      if (sFactor < 0.0)
      {
        svtkWarningMacro(<< "Scalar value less than zero, skipping line");
        return 0;
      }
    }
...
void svtkTubeFilter::GenerateStrips(svtkIdType offset, svtkIdType npts,
  ...
  svtkCellArray* newStrips)
{
  ...
  if (this->Capping)
  {
    svtkIdType startIdx = offset + npts * this->NumberOfSides;
    svtkIdType idx;
...
void svtkTubeFilter::GenerateTextureCoords(svtkIdType offset, svtkIdType npts, const svtkIdType* pts,
  svtkPoints* inPts, svtkDataArray* inScalars, svtkFloatArray* newTCoords)
{
  ...
  double s0, s;
  if (this->GenerateTCoords == SVTK_TCOORDS_FROM_SCALARS)
  {
  ...
  {
    int ik;
    svtkIdType startIdx = offset + npts * numSides;
...
svtkIdType svtkTubeFilter::ComputeOffset(svtkIdType offset, svtkIdType npts)
{
  if (this->SidesShareVertices)
  {
...
const char* svtkTubeFilter::GetVaryRadiusAsString()
{
  if (this->VaryRadius == SVTK_VARY_RADIUS_OFF)
  {
    return "VaryRadiusOff";
  }
  else if (this->VaryRadius == SVTK_VARY_RADIUS_BY_SCALAR)
  {
    return "VaryRadiusByScalar";
  }
  else if (this->VaryRadius == SVTK_VARY_RADIUS_BY_ABSOLUTE_SCALAR)
  {
...
const char* svtkTubeFilter::GetGenerateTCoordsAsString()
{
...
void svtkTubeFilter::PrintSelf(ostream& os, svtkIndent indent)
{
