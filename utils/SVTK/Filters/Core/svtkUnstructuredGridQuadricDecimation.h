/*=========================================================================
  ...
  Module:    svtkUnstructuredGridQuadricDecimation.h
...
/**
 * @class   svtkUnstructuredGridQuadricDecimation
 * @brief   reduce the number of
 * tetrahedra in a mesh
 *
 *
 *
 * svtkUnstructuredGridQuadricDecimation is a class that simplifies
 * tetrahedral meshes using randomized multiple choice edge
 * collapses. The input to this filter is a svtkUnstructuredGrid object
...
#ifndef svtkUnstructuredGridQuadricDecimation_h
#define svtkUnstructuredGridQuadricDecimation_h

#include "svtkFiltersCoreModule.h" // For export macro
#include "svtkUnstructuredGridAlgorithm.h"

class SVTKFILTERSCORE_EXPORT svtkUnstructuredGridQuadricDecimation
  : public svtkUnstructuredGridAlgorithm
{
public:
  svtkTypeMacro(svtkUnstructuredGridQuadricDecimation, svtkUnstructuredGridAlgorithm);
  void PrintSelf(ostream& os, svtkIndent indent) override;
  static svtkUnstructuredGridQuadricDecimation* New();
   ...
   */
  svtkSetMacro(TargetReduction, double);
   ...
   */
  svtkSetMacro(NumberOfTetsOutput, int);
  svtkGetMacro(NumberOfTetsOutput, int);
  //@}
   ...
   */
  svtkSetMacro(NumberOfEdgesToDecimate, int);
  svtkGetMacro(NumberOfEdgesToDecimate, int);
  //@}
   ...
   */
  svtkSetMacro(NumberOfCandidates, int);
  svtkGetMacro(NumberOfCandidates, int);
  //@}
   ...
   */
  svtkSetMacro(AutoAddCandidates, int);
  svtkGetMacro(AutoAddCandidates, int);
  //@}
   ...
   * Default is 0.4.
   */
  svtkSetMacro(AutoAddCandidatesThreshold, double);
  svtkGetMacro(AutoAddCandidatesThreshold, double);
  //@}
   ...
   */
  svtkSetMacro(BoundaryWeight, double);
  svtkGetMacro(BoundaryWeight, double);
  //@}
...
protected:
  ...
  ~svtkUnstructuredGridQuadricDecimation() override;
...
private:
  svtkUnstructuredGridQuadricDecimation(const svtkUnstructuredGridQuadricDecimation&) = delete;
  void operator=(const svtkUnstructuredGridQuadricDecimation&) = delete;
};
