...
#include "svtkStaticPointLocator2D.h"
#include "svtkStreamingDemandDrivenPipeline.h"
#include "svtkTransform.h"

#include <vector>

svtkStandardNewMacro(svtkVoronoi2D);
svtkCxxSetObjectMacro(svtkVoronoi2D, Transform, svtkAbstractTransform);
...
double EvaluateTheta(double x[2], double tileX[2])
{
  ...
  return (t >= 0 ? t : (2.0 * svtkMath::Pi() + t));
}
...
struct VVertex
{
  svtkIdType PointId; // generating point id (across from tile edge)
  ...
  VVertex(svtkIdType ptId, double tileX[2], double x[2])
    : PointId(ptId)
    , Val(0.0)
  {
...
struct VTile
{
  svtkIdType NPts;                   // total number of points in dataset
  svtkIdType PointId;                // generating tile point id (in tile)
  double TileX[2];                  // generating tile point - x-y coordinates
  VertexRingType Verts;             // ordered loop of vertices (ordered in theta)
  svtkStaticPointLocator2D* Locator; // locator
  ...
  // order.
  void Initialize(svtkIdType ptId, const double x[2])
  {
  ...
  void Initialize(
    svtkIdType ptId, const double x[2], svtkPoints* pts, svtkIdType nPts, const svtkIdType* p)
  {
    // The generating tile point
    this->PointId = ptId;
  ...
  // debugging.
  void PopulatePolyData(svtkPoints* centers, svtkCellArray* tile, svtkDoubleArray* radii)
  {
    svtkIdType nPts = static_cast<svtkIdType>(this->Verts.size());
  ...
  void UpdateFlowerFootprint()
  {
    ...
    xMin[0] = xMin[1] = SVTK_FLOAT_MAX;
    xMax[0] = xMax[1] = SVTK_FLOAT_MIN;
    VertexRingIterator tPtr;
    for (tPtr = this->Verts.begin(); tPtr != this->Verts.end(); ++tPtr)
    {
      r = sqrt((tPtr->X[0] - this->TileX[0]) * (tPtr->X[0] - this->TileX[0]) +
  ...
  // and unit normal.
  int ClipTile(svtkIdType ptId, const double p[3], double svtkNotUsed(tol))
  {
    // Create half-space
  ...
  svtkIdType NextSpiralBucket(int ij[2])
  {
...
// and is mentioned in comments elsewhere.
  ...
  bool BuildTile(svtkIdList* pIds, const double* pts, double tol, svtkIdType maxClips)
  {
    const double* v;
    int ij[2];
    svtkIdType bucket, i, ptId, nPts, numClips = 0, numClipAttempts = 0;
    svtkIdType prevNumClips, numPts = this->NPts;
    ;
...
{
  svtkIdType NumberOfTiles;
  svtkIdType NumberOfPoints;
  std::vector<svtkIdType> LocalTiles;
  std::vector<TileVertex> LocalPoints;
  std::vector<svtkIdType> LocalScalars; // cell scalars
  VTile Tile;
  svtkIdType Offset;
...
struct VoronoiTiles
{
  const double* Points;
  svtkIdType NPts;
  svtkStaticPointLocator2D* Locator;
  double PaddedBounds[4]; // the domain over which Voronoi is calculated
  double Bounds[4];       // locator bounds
  int Divisions[2];
  double H[2];
  ...
  VoronoiTiles(svtkIdType npts, double* points, svtkStaticPointLocator2D* loc, double padding,
    double tol, svtkPolyData* output, int scalarMode, svtkIdType maxClips)
    : Points(points)
    , NPts(npts)
    , Locator(loc)
    , Tol(tol)
    ...
    , NumThreadsUsed(0)
  {
    ...
    this->Scalars = static_cast<svtkIdTypeArray*>(output->GetCellData()->GetScalars());
  ...
  void operator()(svtkIdType ptId, svtkIdType endPtId)
  {
    svtkIdList*& pIds = this->PIds.Local();
    LocalDataType& localData = this->LocalData.Local();
    svtkIdType& numTiles = localData.NumberOfTiles;
    svtkIdType& numPoints = localData.NumberOfPoints;
    std::vector<svtkIdType>& lTiles = localData.LocalTiles;
    std::vector<TileVertex>& lPoints = localData.LocalPoints;
    std::vector<svtkIdType>& lScalars = localData.LocalScalars;
    ...
    for (; ptId < endPtId; ++ptId, x += 3)
    {
      ...
      if (tile.BuildTile(pIds, this->Points, this->Tol, this->MaxClips))
      {
        ...
        svtkIdType i, nPts = static_cast<svtkIdType>(tile.Verts.size());
        ...
        if (this->ScalarMode == svtkVoronoi2D::POINT_IDS)
        {
  ...
  void Reduce()
  {
    ...
    svtkIdType totalTiles = 0;
    svtkIdType connSize = 0;
    svtkIdType totalPoints = 0;
    svtkIdType offset = totalPoints;
    this->NumThreadsUsed = 0;
    svtkSMPThreadLocal<LocalDataType>::iterator ldItr;
    svtkSMPThreadLocal<LocalDataType>::iterator ldEnd = this->LocalData.end();
    for (ldItr = this->LocalData.begin(); ldItr != ldEnd; ++ldItr)
    {
      totalTiles += (*ldItr).NumberOfTiles;
      connSize += static_cast<svtkIdType>((*ldItr).LocalTiles.size());
    ...
    std::vector<svtkIdType>::iterator sItr, sEnd;
    svtkIdType* scalars = nullptr;
    if (this->Scalars)
    {
      this->Scalars->SetNumberOfTuples(totalTiles);
      scalars = static_cast<svtkIdType*>(this->Scalars->GetVoidPointer(0));
    }

    svtkIdType threadId = 0;
    for (ldItr = this->LocalData.begin(); ldItr != ldEnd; ++ldItr)
    {
      // Points
      ...
      this->Tiles->AppendLegacyFormat((*ldItr).LocalTiles.data(),
        static_cast<svtkIdType>((*ldItr).LocalTiles.size()), (*ldItr).Offset);

      // Scalars if requested
      if (scalars != nullptr)
      {
        if (this->ScalarMode == svtkVoronoi2D::THREAD_IDS)
        {
        ...
        else // if ( this->ScalarMode == svtkVoronoi2D::POINT_IDS )
        {
  ...
  static int Execute(svtkStaticPointLocator2D* loc, svtkIdType numPts, double* points, double padding,
    double tol, svtkPolyData* output, int sMode, svtkIdType pointOfInterest, svtkIdType maxClips)
  {
    VoronoiTiles vt(numPts, points, loc, padding, tol, output, sMode, maxClips);
    if (pointOfInterest < 0 || pointOfInterest >= numPts)
    {
      svtkSMPTools::For(0, numPts, vt);
    }
    else
    {
      svtkSMPTools::For(pointOfInterest, pointOfInterest + 1, vt);
    }
...
//------------------------------------------------------------------------------
// Construct object
svtkVoronoi2D::svtkVoronoi2D()
{
  this->Padding = 0.01;
  this->Tolerance = 0.00001; // currently hidden
  this->Locator = svtkStaticPointLocator2D::New();
  ...
  this->MaximumNumberOfTileClips = SVTK_ID_MAX;
  this->GenerateVoronoiFlower = false;
  this->NumberOfThreadsUsed = 0;
  this->Spheres = svtkSpheres::New();
...
//------------------------------------------------------------------------------
int svtkVoronoi2D::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  svtkDebugMacro(<< "Generating 2D Voronoi Tessellation");

  // Initialize; check input
  svtkIdType pId, numPts;
  svtkPoints* inPoints;
  if ((inPoints = input->GetPoints()) == nullptr || (numPts = inPoints->GetNumberOfPoints()) < 1)
  {
    svtkDebugMacro("Cannot tessellate; need at least 1 input point");
    return 1;
  }
  ...
  else if (this->ProjectionPlaneMode == SVTK_BEST_FITTING_PLANE)
  {
    ...
    this->SetTransform(svtkDelaunay2D::ComputeBestFittingPlane(input));
    tPoints = svtkPoints::New();
    tPoints->SetDataTypeToDouble();
    this->Transform->TransformPoints(inPoints, tPoints);
  }
  else if (inPoints->GetDataType() == SVTK_DOUBLE)
  ...
  svtkPolyData* tInput = svtkPolyData::New();
  ...
  svtkCellArray* tiles = svtkCellArray::New();
  output->SetPoints(newPts);
  output->SetPolys(tiles);
  if (this->GenerateScalars != NONE)
  {
    svtkIdTypeArray* ts = svtkIdTypeArray::New();
  ...
  svtkDebugMacro(<< "Produced " << output->GetNumberOfCells() << " tiles and "
  ...
  if (this->GenerateVoronoiFlower && this->PointOfInterest >= 0 && this->PointOfInterest < numPts)
  {
    ...
    // tile.
    svtkPoints* fPts = svtkPoints::New();
    fPts->SetDataTypeToDouble();
    svtkCellArray* fVerts = svtkCellArray::New();
    ...
    // Now update the svtkSpheres implicit function, and create a third output
    ...
    svtkInformation* outInfo3 = outputVector->GetInformationObject(2);
    svtkPolyData* output3 = svtkPolyData::SafeDownCast(outInfo3->Get(svtkDataObject::DATA_OBJECT()));

    svtkPoints* centers = svtkPoints::New();
    centers->SetDataTypeToDouble();
    svtkCellArray* singleTile = svtkCellArray::New();
...
int svtkVoronoi2D::FillInputPortInformation(int, svtkInformation* info)
{
...
svtkMTimeType svtkVoronoi2D::GetMTime()
{
...
//----------------------------------------------------------------------------
void svtkVoronoi2D::PrintSelf(ostream& os, svtkIndent indent)
{
