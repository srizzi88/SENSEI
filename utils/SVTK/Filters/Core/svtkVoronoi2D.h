 ...
 *
 * svtkVoronoi2D is a filter that constructs a 2D Voronoi tessellation of a
 * list of input points. The points are assumed to lie in a plane. These
 * points may be represented by any dataset of type svtkPointSet and
 ...
 * the svtkTransformFilter to transform the points into and out of the x-y
 * plane or you can specify a transform to svtkVoronoi2D directly.  In the
 ...
 * as would be required when using the svtkTransformFilter method.  Specifying
 ...
 * points close to a specified position. A svtkStaticPointLocator2D is used
 ...
 *
 * @sa
 * svtkDelaunay2D svtkTransformFilter svtkStaticPointLocator2D
 */

#ifndef svtkVoronoi2D_h
#define svtkVoronoi2D_h

#include "svtkFiltersCoreModule.h" // For export macro
#include "svtkPolyDataAlgorithm.h"

class svtkStaticPointLocator2D;
class svtkAbstractTransform;
class svtkPointSet;
class svtkSpheres;

class SVTKFILTERSCORE_EXPORT svtkVoronoi2D : public svtkPolyDataAlgorithm
{
public:
  //@{
  /**
   ...
   */
  static svtkVoronoi2D* New();
  svtkTypeMacro(svtkVoronoi2D, svtkPolyDataAlgorithm);
   ...
   */
  svtkSetClampMacro(Padding, double, 0.001, 0.25);
   ...
   * any subclass of svtkAbstractTransform (thus it does not need to be a
   ...
   * tessellation. When the SVTK_XY_PLANE is set, the z-coordinate is simply
   * ignored. When SVTK_SET_TRANSFORM_PLANE is set, then a transform must be
   * supplied and the points are transformed using it. Finally, if
   * SVTK_BEST_FITTING_PLANE is set, then the filter computes a best fitting
   * plane and projects the points onto it.
   */
  svtkSetClampMacro(ProjectionPlaneMode, int, XY_PLANE, BEST_FITTING_PLANE);
  svtkGetMacro(ProjectionPlaneMode, int);
   ...
   * MaximumNumberOfTileClips=SVTK_ID_MAX and therefore automatically limited
   * by the algorithm).
   */
  svtkSetClampMacro(PointOfInterest, svtkIdType, -1, SVTK_ID_MAX);
  svtkGetMacro(PointOfInterest, svtkIdType);
  svtkSetClampMacro(MaximumNumberOfTileClips, svtkIdType, 1, SVTK_ID_MAX);
  svtkGetMacro(MaximumNumberOfTileClips, svtkIdType);
  //@}
   ...
   */
  svtkStaticPointLocator2D* GetLocator() { return this->Locator; }
  //@}
   ...
   */
  svtkSetMacro(GenerateVoronoiFlower, svtkTypeBool);
  svtkGetMacro(GenerateVoronoiFlower, svtkTypeBool);
  svtkBooleanMacro(GenerateVoronoiFlower, svtkTypeBool);
  //@}

  //@{
  /**
   * Return the Voronoi flower (a collection of spheres) for the point of
   * interest in the form of a svtkSpheres implicit function. This is valid
   ...
   * purposes.
   */
  svtkGetObjectMacro(Spheres, svtkSpheres);
  //@}
...
protected:
  ...
  svtkIdType PointOfInterest;
  svtkIdType MaximumNumberOfTileClips;
  svtkTypeBool GenerateVoronoiFlower;
...
private:
  svtkVoronoi2D(const svtkVoronoi2D&) = delete;
  void operator=(const svtkVoronoi2D&) = delete;
};
