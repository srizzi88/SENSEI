...
int TestLagrangianParticle(int, char*[])
{
  ...
  std::unique_ptr<svtkLagrangianParticle> part2(part.NewParticle(particleCounter));
  particleCounter++;
  std::unique_ptr<svtkLagrangianParticle> part3(part2->CloneParticle());
  ...
  part.SetTermination(svtkLagrangianParticle::PARTICLE_TERMINATION_SURF_TERMINATED);
  if (part.GetTermination() != svtkLagrangianParticle::PARTICLE_TERMINATION_SURF_TERMINATED)
  {
    std::cerr << "Incorrect Termination" << std::endl;
    return EXIT_FAILURE;
  }

  part.SetInteraction(svtkLagrangianParticle::SURFACE_INTERACTION_TERMINATED);
  if (part.GetInteraction() != svtkLagrangianParticle::SURFACE_INTERACTION_TERMINATED)
  {
  ...
  part.PrintSelf(std::cout, svtkIndent(0));

  particleCounter = 0;
  svtkLagrangianParticle part4(nvar, seedId, particleCounter, seedId, 0, pd, 8, 17);
  part4.SetThreadedData(data.get());
  particleCounter++;
  std::unique_ptr<svtkLagrangianParticle> part5(svtkLagrangianParticle::NewInstance(
