/*=========================================================================
  ...
  Module:    svtkLagrangianParticle.h
 ...
 *
 *
 * Particle to inject and integrate in the svtkLagrangianParticleTracker.
 * This class does NOT inherit from svtkObject in order to increase performance
 * and reduce memory usage.
 *
 * @sa
 * svtkLagrangianParticleTracker svtkLagrangianBasicIntegrationModel
 * svtkLagrangianMatidaIntegrationModel
 */

#ifndef svtkLagrangianParticle_h
#define svtkLagrangianParticle_h

#include "svtkFiltersFlowPathsModule.h" // For export macro
#include "svtkNew.h"                    // For svtkNew
#include "svtkSystemIncludes.h"         // For PrintSelf signature and svtkType
...
class SVTKFILTERSFLOWPATHS_EXPORT svtkLagrangianParticle
{
public:
  /**
   ...
   * of svtkLagrangianBasicIntegrationModel::GetNumberOfIndependantVariable()
   ...
   */
  svtkLagrangianParticle(int numberOfVariables, svtkIdType seedId, svtkIdType particleId,
    svtkIdType seedArrayTupleIndex, double integrationTime, svtkPointData* seedData, int weightsSize,
   ...
   */
  static svtkLagrangianParticle* NewInstance(int numberOfVariables, svtkIdType seedId,
    ...
    svtkIdType numberOfSteps = 0, double previousIntegrationTime = 0);
   ...
   */
  svtkLagrangianParticle* NewParticle(svtkIdType particleId);

  /**
   * method to create an exact clone of a particle.
   */
  svtkLagrangianParticle* CloneParticle();

  /**
   * Destructor.
   */
  virtual ~svtkLagrangianParticle();
   ...
   * To be used with svtkInitialValueProblemSolver::ComputeNextStep.
  ...
  //@{
  /**
   * Get/Set a pointer to a svtkLagrangianThreadedData that is considered to be local to the thread.
   ...
   * that you may need. This is set by the svtkLagrangianParticleTracker and can be
   * initialized/finalized in the model
   */
  inline svtkLagrangianThreadedData* GetThreadedData() { return this->ThreadedData; }
  inline void SetThreadedData(svtkLagrangianThreadedData* threadedData)
  {
   ...
   */
  virtual void SetParentId(svtkIdType parentId);
  virtual svtkIdType GetParentId();
  //@}
   ...
   */
  svtkDataSet* GetLastDataSet();

  /**
   * Get the locator used to find the last traversed cell
   */
  svtkAbstractCellLocator* GetLastLocator();

  /**
   * Get the last intersected surface cell id.
   */
  svtkIdType GetLastSurfaceCellId();

  /**
   * Get the dataset containing the last intersected surface cell
   */
  svtkDataSet* GetLastSurfaceDataSet();

  /**
   * Set the last dataset and last cell id
   */
  void SetLastCell(svtkAbstractCellLocator* locator, svtkDataSet* dataset, svtkIdType cellId,
    double lastCellPosition[3]);

  /**
   * Set the last surface dataset and last surface cell id
   */
  void SetLastSurfaceCell(svtkDataSet* dataset, svtkIdType cellId);

  /**
   * Get particle current number of steps.
   */
  virtual svtkIdType GetNumberOfSteps();
...
protected:
  svtkLagrangianParticle(const svtkLagrangianParticle&) = delete;
  svtkLagrangianParticle() = delete;
  void operator=(const svtkLagrangianParticle&) = delete;
  ...
  svtkLagrangianThreadedData* ThreadedData = nullptr;

  svtkIdType Id;
  svtkIdType ParentId;
  svtkIdType SeedId;
  svtkIdType NumberOfSteps;
  svtkIdType SeedArrayTupleIndex;
  svtkPointData* SeedData;

  svtkAbstractCellLocator* LastLocator;
  ...
  int Interaction;
  int UserFlag;
  svtkDataSet* LastSurfaceDataSet;
  svtkIdType LastSurfaceCellId;
...
#endif
// SVTK-HeaderTest-Exclude: svtkLagrangianParticle.h
