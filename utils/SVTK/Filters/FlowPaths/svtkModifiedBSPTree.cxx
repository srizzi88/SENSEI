/*=========================================================================
  ...
  Module:    svtkModifiedBSPTree.cxx
...
//
//----------------------------------------------------------------------------
svtkStandardNewMacro(svtkModifiedBSPTree);
//----------------------------------------------------------------------------
//
//
enum
...
//////////////////////////////////////////////////////////////////////////////
svtkModifiedBSPTree::svtkModifiedBSPTree()
{
...
//---------------------------------------------------------------------------
void svtkModifiedBSPTree::FreeSearchStructure()
{
  delete this->mRoot;
  this->mRoot = nullptr;
  this->Level = 0;
...
public:
  ...
  //
  Sorted_cell_extents_Lists(svtkIdType nCells)
  {
    for (int i = 0; i < 3; i++)
    {
...
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
void svtkModifiedBSPTree::BuildLocator()
{
...
//---------------------------------------------------------------------------
void svtkModifiedBSPTree::BuildLocatorIfNeeded()
{
  if (this->LazyEvaluation)
  {
...
//---------------------------------------------------------------------------
void svtkModifiedBSPTree::ForceBuildLocator()
{
  //
...
//---------------------------------------------------------------------------
void svtkModifiedBSPTree::BuildLocatorInternal()
{
  //
  svtkIdType numCells;
  if (!this->DataSet || (numCells = this->DataSet->GetNumberOfCells()) < 1)
  {
    svtkDebugMacro(<< "No Cells to divide");
    numCells = 0;
  }
  svtkDebugMacro(<< "Creating BSPTree for " << numCells << " cells");
  ...
  //
  svtkDebugMacro(<< "Beginning Subdivision");
  //
  if (numCells > 0)
  {
  ...
  (void)av_depth;
  svtkDebugMacro(<< "BSP Tree Statistics \n"
...
//
void svtkModifiedBSPTree::Subdivide(BSPNode* node, Sorted_cell_extents_Lists* lists,
  svtkDataSet* dataset, svtkIdType nCells, int depth, int maxlevel, svtkIdType maxCells, int& MaxDepth)
{
  //
  ...
  if ((nCells > maxCells) && (depth < maxlevel))
  {
    ...
    int Daxis;
    svtkIdType TargetCount = (3 * nCells) / 4;
    //
    for (svtkIdType k, j = 0; j < nCells && !found && !abort; j++)
    {
      // for each axis..
      ...
      for (Daxis = node->mAxis, k = 0; k < 3; Daxis = (Daxis + 1) % 3, k++)
      {
        ...
        else
        {
          ...
          if (j >= TargetCount)
          {
            // svtkDebugMacro("Aborted node division : excessive overlap : Cell count = " << nCells);
            abort = true;
            break;
          }
        }
    ...
    if (found)
    {
      ...
      svtkIdType Cmin_l[3] = { 0, 0, 0 }, Cmin_m[3] = { 0, 0, 0 }, Cmin_r[3] = { 0, 0, 0 };
      svtkIdType Cmax_l[3] = { 0, 0, 0 }, Cmax_m[3] = { 0, 0, 0 }, Cmax_r[3] = { 0, 0, 0 };
      ...
      {
        svtkWarningMacro("Error count in min lists");
      }
      ...
      {
        svtkWarningMacro("Error count in max lists");
      }
      ...
      {
        // svtkDebugMacro(<<"Child 0 or 2 empty : Aborting subdivision for node " << Cmin_l[0] << " "
      ...
      else
      {
        ...
        {
          svtkWarningMacro(<< "Child 0 Empty ! - this shouldn't happen");
        }
        delete left;
        ...
        {
          svtkWarningMacro(<< "Child 2 Empty ! - this shouldn't happen");
        }
        delete right;
        //
        npn += 1; // Parent node
  ...
  for (int i = 0; i < 6; i++)
  {
    node->sorted_cell_lists[i] = new svtkIdType[nCells];
  }
  //
  for (int i = 0; i < 3; i++)
  {
...
//---------------------------------------------------------------------------
void svtkModifiedBSPTree::GenerateRepresentation(int level, svtkPolyData* pd)
{
  nodestack ns;
  boxlist bl;
  BSPNode* node;
  ...
  svtkAppendPolyData* polys = svtkAppendPolyData::New();
  size_t s = bl.size();
...
//---------------------------------------------------------------------------
void svtkModifiedBSPTree::GenerateRepresentationLeafs(svtkPolyData* pd)
{
...
//---------------------------------------------------------------------------
int svtkModifiedBSPTree::IntersectWithLine(const double p1[3], const double p2[3], double tol,
  ...
  double& t, double x[3], double pcoords[3], int& subId, svtkIdType& cellId)
{
  ...
  while (!ns.empty())
  {
    ...
    {
      svtkIdType cell_ID = node->sorted_cell_lists[axis][i];
      //
...
//---------------------------------------------------------------------------
int svtkModifiedBSPTree::IntersectWithLine(
  const double p1[3], const double p2[3], const double tol, svtkPoints* points, svtkIdList* cellIds)
{
  //
  BSPNode *node, *Near, *Mid, *Far;
  ...
  svtkSmartPointer<svtkPoints> tempPoints;
  svtkSmartPointer<svtkIdList> tempIds;
  if (points)
  {
    tempPoints = svtkSmartPointer<svtkPoints>::New();
  }
  if (cellIds)
  {
    tempIds = svtkSmartPointer<svtkIdList>::New();
  }
  int icount = 0;
  //
  // OK, lets walk the tree and find intersections
...
//---------------------------------------------------------------------------
int svtkModifiedBSPTree::IntersectCellInternal(svtkIdType cell_ID, const double p1[3],
...
//////////////////////////////////////////////////////////////////////////////
//---------------------------------------------------------------------------
bool svtkModifiedBSPTree_Inside(double bounds[6], double point[3]);
//---------------------------------------------------------------------------
svtkIdType svtkModifiedBSPTree::FindCell(
  double x[3], double, svtkGenericCell* cell, double pcoords[3], double* weights)
{
  //
  this->BuildLocatorIfNeeded();
  //
  ...
  while (!ns.empty())
  {
    ...
    { // a leaf, so test the cells
      for (int i = 0; i < node->num_cells; i++)
      {
        ...
        //
        if (svtkModifiedBSPTree_Inside(CellBounds[cell_ID], x))
        {
...
//---------------------------------------------------------------------------
bool svtkModifiedBSPTree::InsideCellBounds(double x[3], svtkIdType cell_ID)
{
  //
  this->BuildLocatorIfNeeded();
  //
  return svtkModifiedBSPTree_Inside(this->CellBounds[cell_ID], x);
}
//----------------------------------------------------------------------------
svtkIdListCollection* svtkModifiedBSPTree::GetLeafNodeCellInformation()
{
  if (!this->mRoot)
  {
    return nullptr;
  ...
  this->BuildLocatorIfNeeded();
  //
  svtkIdListCollection* LeafCellsList = svtkIdListCollection::New();
  nodestack ns;
  ...
  while (!ns.empty())
  {
    ...
    { // a leaf
      svtkSmartPointer<svtkIdList> newList = svtkSmartPointer<svtkIdList>::New();
...
//----------------------------------------------------------------------------
void svtkModifiedBSPTree::PrintSelf(ostream& os, svtkIndent indent)
{
