...
=========================================================================*/
/**
 * @class   svtkModifiedBSPTree
 * @brief   Generate axis aligned BBox tree for raycasting and other Locator based searches
 *
 *
 * svtkModifiedBSPTree creates an evenly balanced BSP tree using a top down
...
#ifndef svtkModifiedBSPTree_h
#define svtkModifiedBSPTree_h

#include "svtkAbstractCellLocator.h"
#include "svtkFiltersFlowPathsModule.h" // For export macro
#include "svtkSmartPointer.h"           // required because it is nice
...
class SVTKFILTERSFLOWPATHS_EXPORT svtkModifiedBSPTree : public svtkAbstractCellLocator
{
public:
  //@{
  /**
   * Standard Type-Macro
   */
  svtkTypeMacro(svtkModifiedBSPTree, svtkAbstractCellLocator);
   ...
   */
  static svtkModifiedBSPTree* New();

  // Re-use any superclass signatures that we don't override.
  using svtkAbstractCellLocator::FindCell;
   ...
   */
  virtual void GenerateRepresentationLeafs(svtkPolyData* pd);
   ...
   * For each intersection found, the svtkPoints and CellIds objects
   * have the relevant information added in order of intersection increasing
   * from ray start to end. If either svtkPoints or CellIds are nullptr
  ...
  virtual int IntersectWithLine(const double p1[3], const double p2[3], const double tol,
    svtkPoints* points, svtkIdList* cellIds);
   ...
   */
  svtkIdType FindCell(
    double x[3], double tol2, svtkGenericCell* GenCell, double pcoords[3], double* weights) override;

  bool InsideCellBounds(double x[3], svtkIdType cell_ID) override;
   ...
   */
  svtkIdListCollection* GetLeafNodeCellInformation();

protected:
  svtkModifiedBSPTree();
  ~svtkModifiedBSPTree() override;
  //
  ...
  void Subdivide(BSPNode* node, Sorted_cell_extents_Lists* lists, svtkDataSet* dataSet,
    svtkIdType nCells, int depth, int maxlevel, svtkIdType maxCells, int& MaxDepth);
  ...
  virtual int IntersectCellInternal(svtkIdType cell_ID, const double p1[3], const double p2[3],
...
private:
  svtkModifiedBSPTree(const svtkModifiedBSPTree&) = delete;
  void operator=(const svtkModifiedBSPTree&) = delete;
};
...
public:
  ...
  BSPNode(void)
  {
    ...
    for (int i = 0; i < 3; i++)
    {
      this->Bounds[i * 2] = SVTK_FLOAT_MAX;
      this->Bounds[i * 2 + 1] = -SVTK_FLOAT_MAX;
    }
  }
  // Destructor
  ~BSPNode(void)
...
protected:
  ...
  svtkIdType* sorted_cell_lists[6];
  ...
  //
  friend class svtkModifiedBSPTree;
  friend class svtkParticleBoxTree;

public:
  static bool SVTKFILTERSFLOWPATHS_EXPORT RayMinMaxT(const double bounds[6], const double origin[3],
    const double dir[3], double& rTmin, double& rTmax);
  static int SVTKFILTERSFLOWPATHS_EXPORT getDominantAxis(const double dir[3]);
};
