...
static void TestPolyData(svtkPolyData* data, int num, svtkRenderWindow* renwin,
  const double minBoxPoint[3], const double maxBoxPoint[3])
{
  // Set up test of normal box.
  SVTK_CREATE(svtkBoxClipDataSet, clipper1);
  ...
  SVTK_CREATE(svtkBoxClipDataSet, clipper3);
  ...
  SVTK_CREATE(svtkDataSetSurfaceFilter, surface3);
  ...
  SVTK_CREATE(svtkBoxClipDataSet, clipper4);
...
int BoxClipPolyData(int, char*[])
{
  ...
  SVTK_CREATE(svtkDoubleArray, trianglePointsArray);
  ...
  SVTK_CREATE(svtkPolyData, triangles);
  ...
  // Test lines.
  SVTK_CREATE(svtkPolyData, sphereNoNormals);
  sphereNoNormals->CopyStructure(sphere->GetOutput());

  SVTK_CREATE(svtkPlane, plane);
  plane->SetOrigin(0.0, 0.0, 0.0);
  plane->SetNormal(0.0, 0.0, 1.0);

  SVTK_CREATE(svtkCutter, cutter);
  ...
  // Test verts.
  SVTK_CREATE(svtkPolyData, verts);
  svtkPoints* vertsPoints = sphereNoNormals->GetPoints();
  verts->SetPoints(vertsPoints);

  SVTK_CREATE(svtkCellArray, vertsCells);
