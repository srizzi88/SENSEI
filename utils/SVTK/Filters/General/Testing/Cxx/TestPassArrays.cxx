...
int TestPassArrays(int svtkNotUsed(argc), char* svtkNotUsed(argv)[])
{
  SVTK_CREATE(svtkPassArrays, pass);
  ...
  for (int type = 0; type < 3; type++)
  {
    for (int removeArrays = 0; removeArrays <= 1; removeArrays++)
    {
      for (int useFieldTypes = 0; useFieldTypes <= 1; useFieldTypes++)
      {
        ...
        pass->Update();
        svtkPolyData* out = svtkPolyData::SafeDownCast(pass->GetOutput());
        std::cerr << "... done" << std::endl;

        std::cerr << "Checking output ..." << std::endl;
        svtkFieldData* outAttrib = out->GetAttributesAsFieldData(type);
        svtkIntArray* out1 = svtkArrayDownCast<svtkIntArray>(outAttrib->GetAbstractArray("column1"));
        svtkIntArray* out2 = svtkArrayDownCast<svtkIntArray>(outAttrib->GetAbstractArray("column2"));
        if (useFieldTypes)
        {
          if (!out1 || !out2)
          {
