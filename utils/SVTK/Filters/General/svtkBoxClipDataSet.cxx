...
svtkStandardNewMacro(svtkBoxClipDataSet);
svtkCxxSetObjectMacro(svtkBoxClipDataSet, Locator, svtkIncrementalPointLocator);
//----------------------------------------------------------------------------
svtkBoxClipDataSet::svtkBoxClipDataSet()
{
...
int svtkBoxClipDataSet::GetNumberOfOutputs()
{
...
svtkMTimeType svtkBoxClipDataSet::GetMTime()
{
...
//----------------------------------------------------------------------------
svtkUnstructuredGrid* svtkBoxClipDataSet::GetClippedOutput()
{
...
// Clip by box
//
int svtkBoxClipDataSet::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  svtkDataArray* scalars = this->GetInputArrayToProcess(0, inputVector);
  ...
  for (cellId = 0; cellId < numCells && !abort; cellId++)
  {
    ...
    if (this->GenerateClippedOutput)
    {
      ...
      {
        svtkErrorMacro(<< "Do not support cells of dimension " << cell->GetCellDimension());
      }
    }
    else
    {
...
void svtkBoxClipDataSet::CreateDefaultLocator()
{
  if (this->Locator == nullptr)
  {
...
//
void svtkBoxClipDataSet::SetBoxClip(const double* n0, const double* o0, const double* n1,
...
void svtkBoxClipDataSet::SetBoxClip(
...
//----------------------------------------------------------------------------
int svtkBoxClipDataSet::FillInputPortInformation(int, svtkInformation* info)
{
...
//----------------------------------------------------------------------------
void svtkBoxClipDataSet::PrintSelf(ostream& os, svtkIndent indent)
{
...
//-----------------------------------------------------------------------------
// InterpolateEdge: Interpolate the data in a svtkDataSetAttributes along a line
//
// This method works very much like svtkDataSetAttributes::InterpolateEdge
...
// placed in the output arrays.  This is necessary because svtkBoxClipDataSet
// will continually cut (and consequently interpolate) the input until it
// fits within the bounds.
void svtkBoxClipDataSet::InterpolateEdge(
  svtkDataSetAttributes* attributes, svtkIdType toId, svtkIdType fromId1, svtkIdType fromId2, double t)
{
  int numArrays = attributes->GetNumberOfArrays();
  for (int i = 0; i < numArrays; i++)
  {
    svtkAbstractArray* array = attributes->GetAbstractArray(i);
...
//
void svtkBoxClipDataSet::MinEdgeF(
  const unsigned int* id_v, const svtkIdType* cellIds, unsigned int* edgF)
{
...
//
void svtkBoxClipDataSet::WedgeToTetra(
  const svtkIdType* wedgeId, const svtkIdType* cellIds, svtkCellArray* newCellArray)
{
  int i;
  int id;
  svtkIdType xmin;
  svtkIdType tab[4];
  svtkIdType tabpyram[5];

  const svtkIdType vwedge[6][4] = {
    { 0, 4, 3, 5 },
    { 1, 4, 3, 5 },
    { 2, 4, 3, 5 },
    { 3, 0, 1, 2 },
...
//
void svtkBoxClipDataSet::PyramidToTetra(
  ...
  svtkIdType typeobj, svtkIdType npts, const svtkIdType* cellIds, svtkCellArray* newCellArray)
{
  ...
  const svtkIdType triPassThrough[3] = { 0, 1, 2 };
  svtkIdType tri[3];
  svtkIdType line[2];
  ...
  switch (typeobj)
  {
    ...
    case SVTK_POLYGON: // 7 (Convex case)
      tri[0] = 0;
    ...
    {
      const svtkIdType tetra[4] = { 0, 1, 2, 3 };
    ...
    case SVTK_BIQUADRATIC_QUADRATIC_WEDGE:
      ...
      {
        svtkErrorMacro(<< " This cell is not a wedge\n");
        return;
      }
      break;

    case SVTK_PYRAMID: // Create 2 tetrahedra
    case SVTK_QUADRATIC_PYRAMID:
      if (npts == 5)
      {
        // note: the first element vpyram[][0] is the smallest index of pyramid
        const svtkIdType vpyram[8][4] = {
          { 0, 1, 2, 4 },
          { 0, 2, 3, 4 },
          { 1, 2, 3, 4 },
          { 1, 3, 0, 4 },
      ...
      {
        svtkErrorMacro(<< " This cell is not a pyramid\n");
        return;
      }
      break;
  }
...
//
void svtkBoxClipDataSet::CreateTetra(
  svtkIdType npts, const svtkIdType* cellIds, svtkCellArray* newCellArray)
{
  svtkIdType tabp[5];
  svtkIdType tab[3][4];
  ...
  if (npts == 6)
  {
    // SVTK_WEDGE: Create 3 tetrahedra
    // first tetrahedron
    const svtkIdType vwedge[6][4] = {
      { 0, 4, 3, 5 },
      { 1, 4, 3, 5 },
  ...
  {
    // SVTK_PYRAMID: Create 2 tetrahedra
...
//----------------------------------------------------------------------------
//
void svtkBoxClipDataSet::ClipBox(svtkPoints* newPoints, svtkGenericCell* cell,
...
{
  svtkIdType cellType = cell->GetCellType();
  svtkIdList* cellIds = cell->GetPointIds();
  svtkCellArray* arraytetra = svtkCellArray::New();
  svtkPoints* cellPts = cell->GetPoints();
  svtkIdType npts = cellPts->GetNumberOfPoints();
  std::vector<svtkIdType> cellptId(npts);
  svtkIdType iid[4];
  const svtkIdType* v_id = nullptr;
  svtkIdType *verts, v1, v2;
  svtkIdType ptId;
  svtkIdType tab_id[6];
  svtkIdType ptstetra = 4;
  ...
  for (idtetranew = 0; idtetranew < totalnewtetra; idtetranew++)
  {
    ...
    {
      // Tetrahedron inside.
      svtkIdType newCellId = tets->InsertNextCell(4, iid);
    ...
    svtkIdType newCellId = cellarray->InsertNextCell(4, iid);
    unsigned int planes;
    ...
    for (planes = 0; planes < 6; planes++)
    {
      ...
      svtkCellArray* newcellArray = svtkCellArray::New();
      int edgeNum;
          ...
          case 4: // We have two wedges
            switch (edges_inter)
            {
              ...
              case 1356:
                ...
                break;
              default:
                svtkErrorMacro(<< "Intersection not found: Num_inter = " << num_inter
...
//
void svtkBoxClipDataSet::ClipHexahedron(svtkPoints* newPoints, svtkGenericCell* cell,
  svtkIncrementalPointLocator* locator, svtkCellArray* tets, svtkPointData* inPD, svtkPointData* outPD,
  svtkCellData* inCD, svtkIdType cellId, svtkCellData* outCD)
{
  svtkIdType idcellnew;
  svtkIdType cellType = cell->GetCellType();
  svtkIdList* cellIds = cell->GetPointIds();
  svtkCellArray* arraytetra = svtkCellArray::New();
  svtkPoints* cellPts = cell->GetPoints();
  ...
  for (idtetranew = 0; idtetranew < totalnewtetra; idtetranew++)
  {
    ...
    if (allInside)
    {
      svtkIdType newCellId = tets->InsertNextCell(4, iid); // Tetrahedron inside.
      outCD->CopyData(inCD, cellId, newCellId);
      continue;
    }
    ...
    for (planes = 0; planes < 6; planes++)
    {
      svtkIdType totalnewcells = cellarray->GetNumberOfCells();
      svtkCellArray* newcellArray = svtkCellArray::New();
...
//
void svtkBoxClipDataSet::ClipBoxInOut(svtkPoints* newPoints, svtkGenericCell* cell,
  ...
  svtkPointData** outPD, svtkCellData* inCD, svtkIdType cellId, svtkCellData** outCD)
{
  ...
  for (idtetranew = 0; idtetranew < totalnewtetra; idtetranew++)
  {
    ...
    svtkCellArray* cellarray = svtkCellArray::New();
    svtkCellArray* cellarrayout = svtkCellArray::New();
...
//
void svtkBoxClipDataSet::ClipHexahedronInOut(svtkPoints* newPoints, svtkGenericCell* cell,
  svtkIncrementalPointLocator* locator, svtkCellArray** tets, svtkPointData* inPD,
  svtkPointData** outPD, svtkCellData* inCD, svtkIdType cellId, svtkCellData** outCD)
{
  svtkIdType cellType = cell->GetCellType();
...
//-------------------------------------------------------
void svtkBoxClipDataSet::ClipBox2D(svtkPoints* newPoints, svtkGenericCell* cell,
  svtkIncrementalPointLocator* locator, svtkCellArray* tets, svtkPointData* inPD, svtkPointData* outPD,
  svtkCellData* inCD, svtkIdType cellId, svtkCellData* outCD)
{
  svtkIdType cellType = cell->GetCellType();
  svtkIdList* cellIds = cell->GetPointIds();
  svtkCellArray* arraytriangle = svtkCellArray::New();
  svtkPoints* cellPts = cell->GetPoints();
  svtkIdType npts = cellPts->GetNumberOfPoints();
  std::vector<svtkIdType> cellptId(npts);
  svtkIdType iid[3];
...
void svtkBoxClipDataSet::ClipBoxInOut2D(svtkPoints* newPoints, svtkGenericCell* cell,
  svtkIncrementalPointLocator* locator, svtkCellArray** tets, svtkPointData* inPD,
  svtkPointData** outPD, svtkCellData* inCD, svtkIdType cellId, svtkCellData** outCD)
{
  svtkIdType cellType = cell->GetCellType();
...
void svtkBoxClipDataSet::ClipHexahedron2D(svtkPoints* newPoints, svtkGenericCell* cell,
  svtkIncrementalPointLocator* locator, svtkCellArray* tets, svtkPointData* inPD, svtkPointData* outPD,
  svtkCellData* inCD, svtkIdType cellId, svtkCellData* outCD)
{
  svtkIdType cellType = cell->GetCellType();
...
void svtkBoxClipDataSet::ClipHexahedronInOut2D(svtkPoints* newPoints, svtkGenericCell* cell,
  svtkIncrementalPointLocator* locator, svtkCellArray** tets, svtkPointData* inPD,
  svtkPointData** outPD, svtkCellData* inCD, svtkIdType cellId, svtkCellData** outCD)
{
  svtkIdType cellType = cell->GetCellType();
...
void svtkBoxClipDataSet::ClipBox1D(svtkPoints* newPoints, svtkGenericCell* cell,
  svtkIncrementalPointLocator* locator, svtkCellArray* lines, svtkPointData* inPD, svtkPointData* outPD,
  svtkCellData* inCD, svtkIdType cellId, svtkCellData* outCD)
{
  svtkIdType cellType = cell->GetCellType();
  svtkIdList* cellIds = cell->GetPointIds();
  svtkCellArray* arrayline = svtkCellArray::New();
  svtkPoints* cellPts = cell->GetPoints();
  svtkIdType npts = cellPts->GetNumberOfPoints();
  std::vector<svtkIdType> cellptId(npts);
  svtkIdType iid[2];
...
void svtkBoxClipDataSet::ClipBoxInOut1D(svtkPoints* newPoints, svtkGenericCell* cell,
...
void svtkBoxClipDataSet::ClipHexahedron1D(svtkPoints* newPoints, svtkGenericCell* cell,
  svtkIncrementalPointLocator* locator, svtkCellArray* lines, svtkPointData* inPD, svtkPointData* outPD,
  svtkCellData* inCD, svtkIdType cellId, svtkCellData* outCD)
{
  svtkIdType cellType = cell->GetCellType();
...
void svtkBoxClipDataSet::ClipHexahedronInOut1D(svtkPoints* newPoints, svtkGenericCell* cell,
  svtkIncrementalPointLocator* locator, svtkCellArray** lines, svtkPointData* inPD,
  svtkPointData** outPD, svtkCellData* inCD, svtkIdType cellId, svtkCellData** outCD)
{
  svtkIdType cellType = cell->GetCellType();
...
void svtkBoxClipDataSet::ClipBox0D(svtkGenericCell* cell, svtkIncrementalPointLocator* locator,
  svtkCellArray* verts, svtkPointData* inPD, svtkPointData* outPD, svtkCellData* inCD, svtkIdType cellId,
  svtkCellData* outCD)
{
  svtkIdType cellType = cell->GetCellType();
  svtkIdList* cellIds = cell->GetPointIds();
  svtkCellArray* arrayvert = svtkCellArray::New();
  svtkPoints* cellPts = cell->GetPoints();
  svtkIdType npts = cellPts->GetNumberOfPoints();
  std::vector<svtkIdType> cellptId(npts);
  svtkIdType iid;
...
void svtkBoxClipDataSet::ClipBoxInOut0D(svtkGenericCell* cell, svtkIncrementalPointLocator* locator,
...
void svtkBoxClipDataSet::ClipHexahedron0D(svtkGenericCell* cell, svtkIncrementalPointLocator* locator,
  svtkCellArray* verts, svtkPointData* inPD, svtkPointData* outPD, svtkCellData* inCD, svtkIdType cellId,
  svtkCellData* outCD)
{
  svtkIdType cellType = cell->GetCellType();
...
void svtkBoxClipDataSet::ClipHexahedronInOut0D(svtkGenericCell* cell,
  svtkIncrementalPointLocator* locator, svtkCellArray** verts, svtkPointData* inPD,
  svtkPointData** outPD, svtkCellData* inCD, svtkIdType cellId, svtkCellData** outCD)
{
  svtkIdType cellType = cell->GetCellType();
  svtkIdList* cellIds = cell->GetPointIds();
