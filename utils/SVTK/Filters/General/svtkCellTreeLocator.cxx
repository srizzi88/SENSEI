/*=========================================================================
  ...
  Module:    svtkCellTreeLocator.cxx
...
svtkStandardNewMacro(svtkCellTreeLocator);
...
// There are 3 svtkCellTreeNode in x,y,z directions.  In contrast, svtkModifiedBSPTree class stores
...
inline void svtkCellTreeLocator::svtkCellTreeNode::MakeNode(unsigned int left, unsigned int d,
...
//----------------------------------------------------------------------------
inline void svtkCellTreeLocator::svtkCellTreeNode::SetChildren(unsigned int left)
{
  this->Index = GetDimension() | (left << 2); // In index 2 LSBs (Least Significant Bits) store the
                                              // dimension. MSBs store the position
}
//----------------------------------------------------------------------------
inline bool svtkCellTreeLocator::svtkCellTreeNode::IsNode() const
{
  return (this->Index & 3) != 3; // For a leaf 2 LSBs in index is 3
}
//----------------------------------------------------------------------------
inline unsigned int svtkCellTreeLocator::svtkCellTreeNode::GetLeftChildIndex() const
{
  return (this->Index >> 2);
}
//----------------------------------------------------------------------------
inline unsigned int svtkCellTreeLocator::svtkCellTreeNode::GetRightChildIndex() const
{
  return (this->Index >> 2) +
    1; // Right child node is adjacent to the Left child node in the data structure
}
//----------------------------------------------------------------------------
inline unsigned int svtkCellTreeLocator::svtkCellTreeNode::GetDimension() const
{
  return this->Index & 3;
}
//----------------------------------------------------------------------------
inline const float& svtkCellTreeLocator::svtkCellTreeNode::GetLeftMaxValue() const
{
  return this->LeftMax;
}
//----------------------------------------------------------------------------
inline const float& svtkCellTreeLocator::svtkCellTreeNode::GetRightMinValue() const
{
  return this->RightMin;
}
//----------------------------------------------------------------------------
inline void svtkCellTreeLocator::svtkCellTreeNode::MakeLeaf(unsigned int start, unsigned int size)
{
  this->Index = 3;
  this->Sz = size;
  this->St = start;
...
//----------------------------------------------------------------------------
bool svtkCellTreeLocator::svtkCellTreeNode::IsLeaf() const
{
  return this->Index == 3;
}
//----------------------------------------------------------------------------
unsigned int svtkCellTreeLocator::svtkCellTreeNode::Start() const
{
  return this->St;
}
//----------------------------------------------------------------------------
unsigned int svtkCellTreeLocator::svtkCellTreeNode::Size() const
{
  return this->Sz;
}
//----------------------------------------------------------------------------
...
class svtkCellPointTraversal
{
private:
  const svtkCellTreeLocator::svtkCellTree& m_ct;
  unsigned int m_stack[CELLTREE_MAX_DEPTH];
  unsigned int* m_sp; // stack pointer
  const float* m_pos; // 3-D coordinates of the points
  svtkCellPointTraversal(const svtkCellPointTraversal&) = delete;
  void operator=(svtkCellPointTraversal&) = delete;

protected:
  friend class svtkCellTreeLocator::svtkCellTree;
  friend class svtkCellTreeLocator::svtkCellTreeNode;
  friend class svtkCellTreeBuilder;

public:
  svtkCellPointTraversal(const svtkCellTreeLocator::svtkCellTree& ct, const float* pos)
    : m_ct(ct)
    , m_pos(pos)
  {
  ...
  const svtkCellTreeLocator::svtkCellTreeNode*
         ...
         // contain in the data domain
  {
    while (true)
    {
      ...
      const svtkCellTreeLocator::svtkCellTreeNode* n = &this->m_ct.Nodes.front() + *(--this->m_sp);
...
private:
  ...
  void Split(unsigned int index, float min[3], float max[3])
  {
    ...
    float plane = SVTK_FLOAT_MIN;    // bad value in case it doesn't get setx
    unsigned int dim = SVTK_INT_MAX; // bad value in case it doesn't get set
    ...
    svtkCellTreeLocator::svtkCellTreeNode child[2];
...
public:
  ...
  void Build(svtkCellTreeLocator* ctl, svtkCellTreeLocator::svtkCellTree& ct, svtkDataSet* ds)
  {
    const svtkIdType size = ds->GetNumberOfCells();
    double cellBounds[6];
    ...
    svtkCellTreeLocator::svtkCellTreeNode root;
    ...
    std::vector<svtkCellTreeLocator::svtkCellTreeNode>::iterator ni = ct.Nodes.begin();
    std::vector<svtkCellTreeLocator::svtkCellTreeNode>::iterator nn = ct.Nodes.begin() + 1;
...
public:
  ...
  std::vector<svtkCellTreeLocator::svtkCellTreeNode> m_nodes;
};

//----------------------------------------------------------------------------

typedef std::stack<svtkCellTreeLocator::svtkCellTreeNode*,
  std::vector<svtkCellTreeLocator::svtkCellTreeNode*> >
  nodeStack;

svtkCellTreeLocator::svtkCellTreeLocator()
{
...
svtkCellTreeLocator::~svtkCellTreeLocator()
{
  ...
  this->svtkCellTreeLocator::FreeSearchStructure();
}

//----------------------------------------------------------------------------
void svtkCellTreeLocator::BuildLocatorIfNeeded()
{
  if (this->LazyEvaluation)
  {
    ...
    {
      this->Modified();
      svtkDebugMacro(<< "Forcing BuildLocator");
      this->ForceBuildLocator();
    }
  }
}
//----------------------------------------------------------------------------
void svtkCellTreeLocator::ForceBuildLocator()
{
  //
  ...
  if ((this->Tree) && this->UseExistingSearchStructure)
  {
    ...
    svtkDebugMacro(<< "BuildLocator exited - UseExistingSearchStructure");
    return;
  }
  this->BuildLocatorInternal();
}
//----------------------------------------------------------------------------
void svtkCellTreeLocator::BuildLocatorInternal()
{
  this->FreeSearchStructure();
  if (!this->DataSet || (this->DataSet->GetNumberOfCells() < 1))
  {
    svtkErrorMacro(<< " No Cells in the data set\n");
    return;
  }
  //
  ...
  //
  this->Tree = new svtkCellTree;
  svtkCellTreeBuilder builder;
...
void svtkCellTreeLocator::BuildLocator()
{
  if (this->LazyEvaluation)
  {
    return;
...
//----------------------------------------------------------------------------
svtkIdType svtkCellTreeLocator::FindCell(
  double pos[3], double, svtkGenericCell* cell, double pcoords[3], double* weights)
{
  ...
  svtkCellPointTraversal pt(*(this->Tree), _pos);

  // bool found = false;

  while (const svtkCellTreeNode* n = pt.Next())
  {
...
int svtkCellTreeLocator::IntersectWithLine(const double p1[3], const double p2[3], double tol,
  double& t, double x[3], double pcoords[3], int& subId, svtkIdType& cellId, svtkGenericCell* cell)
{
...
int svtkCellTreeLocator::IntersectWithLine(const double p1[3], const double p2[3], double tol,
  double& t, double x[3], double pcoords[3], int& subId, svtkIdType& cellIds)
{
  //
  svtkCellTreeNode *node, *near, *far;
  ...
  nodeStack ns;
  double closest_intersection = SVTK_FLOAT_MAX;
  bool HIT = false;
  ...
  //
  svtkCellTreeNode* n = &this->Tree->Nodes.front();
  ns.push(n);
  while (!ns.empty())
  {
    node = ns.top();
    ...
    {
      svtkIdType cell_ID = this->Tree->Leaves[node->Start() + i];
      //
...
//----------------------------------------------------------------------------
bool svtkCellTreeLocator::RayMinMaxT(
...
//----------------------------------------------------------------------------
int svtkCellTreeLocator::getDominantAxis(const double dir[3])
{
...
//----------------------------------------------------------------------------
void svtkCellTreeLocator::Classify(const double origin[3], const double dir[3], double& rDist,
  svtkCellTreeNode*& near, svtkCellTreeNode*& parent, svtkCellTreeNode*& far, int& mustCheck)
{
  ...
  if (tOriginToDivPlane2 > 0) // origin is right of the rmin
  {
    ...
    rDist = (tDivDirection) ? tOriginToDivPlane2 / tDivDirection : SVTK_FLOAT_MAX;
  }
  else if (tOriginToDivPlane < 0) // origin is left of the lm
  {
    ...
    rDist = (tDivDirection) ? tOriginToDivPlane / tDivDirection : SVTK_FLOAT_MAX;
  }
  ...
  else
  {
    ...
    if (tDivDirection < 0)
    {
      ...
      rDist = (tDivDirection) ? 0 / tDivDirection : SVTK_FLOAT_MAX;
    }
    else
    {
      far = &this->Tree->Nodes.at(parent->GetLeftChildIndex());
...
//----------------------------------------------------------------------------
int svtkCellTreeLocator::IntersectCellInternal(svtkIdType cell_ID, const double p1[3],
...
//----------------------------------------------------------------------------
void svtkCellTreeLocator::FreeSearchStructure()
{
  delete this->Tree;
  this->Tree = nullptr;
...
typedef std::pair<svtkBoundingBox, int> boxLevel;
typedef std::vector<boxLevel> boxlist;
// For testing bounds of the tree we need node/box
typedef std::pair<svtkCellTreeLocator::svtkCellTreeNode*, boxLevel> nodeBoxLevel;
typedef std::stack<nodeBoxLevel, std::vector<nodeBoxLevel> > nodeinfostack;
//---------------------------------------------------------------------------
static void SplitNodeBox(
  svtkCellTreeLocator::svtkCellTreeNode* n, svtkBoundingBox& b, svtkBoundingBox& l, svtkBoundingBox& r)
{
  double minpt[3], maxpt[3];
  // create a box for left node
  svtkBoundingBox ll(b);
...
//---------------------------------------------------------------------------
static void AddBox(svtkPolyData* pd, double* bounds, int level)
{
  svtkPoints* pts = pd->GetPoints();
  svtkCellArray* lines = pd->GetLines();
  svtkIntArray* levels = svtkArrayDownCast<svtkIntArray>(pd->GetPointData()->GetArray(0));
  double x[3];
  svtkIdType cells[8], ids[2];
  //
  x[0] = bounds[0];
  x[1] = bounds[2];
  x[2] = bounds[4];
...
//---------------------------------------------------------------------------
void svtkCellTreeLocator::GenerateRepresentation(int level, svtkPolyData* pd)
{
  this->BuildLocatorIfNeeded();
  //
  nodeinfostack ns;
  boxlist bl;
  //
  svtkCellTreeNode* n0 = &this->Tree->Nodes.front();
  // create a box for the root
  float* DataBBox = this->Tree->DataBBox;
  svtkBoundingBox lbox, rbox,
  ...
  while (!ns.empty())
  {
    ...
    else if (n0->IsNode())
    {
      ...
      svtkCellTreeNode* n1 = &this->Tree->Nodes.at(n0->GetLeftChildIndex());
      svtkCellTreeNode* n2 = &this->Tree->Nodes.at(n0->GetLeftChildIndex() + 1);
      ns.pop();
...
//---------------------------------------------------------------------------
void svtkCellTreeLocator::FindCellsWithinBounds(double* bbox, svtkIdList* cells)
{
  this->BuildLocatorIfNeeded();
  //
  nodeinfostack ns;
  double cellBounds[6];
  svtkBoundingBox TestBox(bbox);
  //
  svtkCellTreeNode* n0 = &this->Tree->Nodes.front();
  // create a box for the root
  float* DataBBox = this->Tree->DataBBox;
  ...
  {
    n0 = ns.top().first;
    svtkBoundingBox& nodebox = ns.top().second.first;
    if (TestBox.Intersects(nodebox))
    {
      if (n0->IsLeaf())
      {
        ...
        {
          svtkIdType cell_ID = this->Tree->Leaves[n0->Start() + i];
          double* boundsPtr = cellBounds;
          if (this->CellBounds)
          {
            boundsPtr = this->CellBounds[cell_ID];
...
void svtkCellTreeLocator::PrintSelf(ostream& os, svtkIndent indent)
{
