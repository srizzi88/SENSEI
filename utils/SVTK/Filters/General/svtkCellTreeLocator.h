 ...
 *
 * @sa
 * svtkLocator svtkCellLocator svtkModifiedBSPTree
 */

#ifndef svtkCellTreeLocator_h
#define svtkCellTreeLocator_h
...
class SVTKFILTERSGENERAL_EXPORT svtkCellTreeLocator : public svtkAbstractCellLocator
{
public:
  class svtkCellTree;
  class svtkCellTreeNode;

  svtkTypeMacro(svtkCellTreeLocator, svtkAbstractCellLocator);
   ...
   * paper
   */
  static svtkCellTreeLocator* New();
  ...
  */
   ...
   * Return intersection point (if any) AND the cell which was intersected by
   * the finite line. The cell is returned as a cell id and as a generic cell.
   * This function is a modification from the svtkModifiedBSPTree class using the
   ...
   */
  class SVTKFILTERSGENERAL_EXPORT svtkCellTree
  {
  public:
    std::vector<svtkCellTreeNode> Nodes;
    std::vector<unsigned int> Leaves;
    friend class svtkCellPointTraversal;
    friend class svtkCellTreeNode;
   ...
   */
  class SVTKFILTERSGENERAL_EXPORT svtkCellTreeNode
  {
  public:
  protected:
    unsigned int Index;
...
protected:
  ...
  void Classify(const double origin[3], const double dir[3], double& rDist, svtkCellTreeNode*& near,
    svtkCellTreeNode*& mid, svtkCellTreeNode*& far, int& mustCheck);

  // From svtkModifiedBSPTRee
...
private:
  svtkCellTreeLocator(const svtkCellTreeLocator&) = delete;
  void operator=(const svtkCellTreeLocator&) = delete;
};
