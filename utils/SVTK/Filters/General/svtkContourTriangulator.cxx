/*=========================================================================
  ...
  Module:    svtkContourTriangulator.cxx
...
svtkStandardNewMacro(svtkContourTriangulator);

//----------------------------------------------------------------------------
svtkContourTriangulator::svtkContourTriangulator()
{
...
//----------------------------------------------------------------------------
svtkContourTriangulator::~svtkContourTriangulator() = default;

//----------------------------------------------------------------------------
void svtkContourTriangulator::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
int svtkContourTriangulator::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  this->TriangulationError = !svtkContourTriangulator::TriangulateContours(
  ...
  {
    svtkErrorMacro("Triangulation failed, output might have holes.");
  }
...
// 7) Triangulate polygons with svtkPolygon::Triangulate()
// 8) Add triangles for each point removed in Step 3
//
// In other words, this routine does a lot of work to process the contours
// so that svtkPolygon can be used to triangulate them (svtkPolygon only does
...
// are O(n log n).  The svtkDelaunay2D filter will go into infinite recursion
...
// A poly type that is just a vector of svtkIdType
typedef std::vector<svtkIdType> svtkCCSPoly;
...
typedef std::vector<size_t> svtkCCSPolyGroup;

// Extra info for each edge in a poly
typedef std::vector<svtkIdType> svtkCCSPolyEdges;

// A cell array, but as an STL vector
typedef std::vector<svtkIdType> svtkCCSCellArray;
...
#define SVTK_CCS_POLYGON_TOLERANCE 1e-5

// Take a set of lines, join them tip-to-tail to create polygons
void svtkCCSMakePolysFromLines(svtkPolyData* data, svtkIdType firstLine, svtkIdType endLine,
  bool oriented, std::vector<svtkCCSPoly>& newPolys, std::vector<size_t>& incompletePolys);

// Finish any incomplete polygons by trying to join loose ends
void svtkCCSJoinLooseEnds(std::vector<svtkCCSPoly>& polys, std::vector<size_t>& incompletePolys,
  svtkPoints* points, const double normal[3]);

// Check for polygons that contain multiple loops, and split the loops apart.
// Returns the number of splits made.
int svtkCCSSplitAtPinchPoints(std::vector<svtkCCSPoly>& polys, svtkPoints* points,
  std::vector<svtkCCSPolyGroup>& polyGroups, std::vector<svtkCCSPolyEdges>& polyEdges,
...
int svtkCCSVectorProgression(const double p[3], const double p1[3], const double p2[3],
  const double p3[3], const double normal[3]);

// Compute polygon bounds.  Poly must have at least one point.
double svtkCCSPolygonBounds(const svtkCCSPoly& poly, svtkPoints* points, double bounds[6]);
...
void svtkCCSFindTrueEdges(std::vector<svtkCCSPoly>& newPolys, svtkPoints* points,
  std::vector<svtkCCSPolyEdges>& polyEdges, svtkCCSCellArray& originalEdges);

// Reverse a polygon that has been simplified with svtkCSSFindTrueEdges
void svtkCCSReversePoly(svtkCCSPoly& poly, svtkCCSPolyEdges& edges, svtkCCSCellArray& originalEdges);

// Set sense to 1 if the poly's normal matches the specified normal, and
// zero otherwise. Returns zero if poly is degenerate.
int svtkCCSCheckPolygonSense(
  svtkCCSPoly& polys, svtkPoints* points, const double normal[3], bool& sense);
...
// each triangle.
void svtkCCSInsertTriangle(svtkCellArray* polys, const svtkCCSPoly& poly, const size_t trids[3],
  const svtkCCSPolyEdges& polyEdges, const svtkCCSCellArray& originalEdges);
...
void svtkCCSMakeHoleyPolys(std::vector<svtkCCSPoly>& polys, svtkPoints* points,
  std::vector<svtkCCSPolyGroup>& polyGroups, std::vector<svtkCCSPolyEdges>& polyEdges,
  svtkCCSCellArray& originalEdges, const double normal[3], bool oriented);
...
int svtkCCSCutHoleyPolys(std::vector<svtkCCSPoly>& polys, svtkPoints* points,
  std::vector<svtkCCSPolyGroup>& polyGroups, std::vector<svtkCCSPolyEdges>& polyEdges,
  const double normal[3]);

// Compute the normal of a polygon
double svtkCCSPolygonNormal(const svtkCCSPoly& poly, svtkPoints* points, double normal[3]);

// Compute the quality of a triangle
double svtkCCSTriangleQuality(
...
int svtkCCSTriangulate(const svtkCCSPoly& poly, svtkPoints* points, const svtkCCSPolyEdges& polyEdges,
  const svtkCCSCellArray& originalEdges, svtkCellArray* polys, const double normal[3]);
  ...
  const svtkCCSCellArray& originalEdges, svtkCellArray* polys, const double normal[3])
{
  ...
  else if (n == 3)
  {
    ...
    svtkCCSInsertTriangle(polys, poly, trids, polyEdges, originalEdges);
  }
  ...
  else
  {
    ...
    for (j = 0; j < n; j++)
    {
      ...
      double q = svtkCCSTriangleQuality(ppoint, point, npoint, normal);
      if (q > maxq)
      {
        maxi = i;
        maxq = q;
    ...
    for (;;)
    {
      ...
      if (verts[i].second > 0)
      {
        ...
        if (concave)
        {
          ...
          svtkMath::Cross(v, normal, u);
          double d = svtkMath::Dot(ppoint, u);

          size_t jj = (j + 1 != n ? j + 1 : 0);
          double x[3];
          points->GetPoint(poly[verts[jj].first], x);
          bool side = (svtkMath::Dot(x, u) < d);
          ...
          for (; foundEar && jj != k; jj = (jj + 1 != n ? jj + 1 : 0))
          {
            ...
            y[2] = x[2];
            points->GetPoint(poly[verts[jj].first], x);
            if (side ^ (svtkMath::Dot(x, u) < d))
            {
              side = !side;
              foundNegative = true;
              double s, t;
              foundEar = (svtkLine::Intersection(ppoint, npoint, x, y, s, t) == 0);
            }
          }
        ...
        else
        {
          ...
          double kq = svtkCCSTriangleQuality(point, ppoint, npoint, normal);
          ...
          double jq = svtkCCSTriangleQuality(ppoint, npoint, point, normal);
      ...
      for (int endIdx = 0; endIdx < 2; endIdx++)
      {
        ...
        if (!matches.empty())
        {
          ...
          if (endPts[endIdx] != lineEndPts[endIdx])
          {
            ...
            svtkCCSPoly::iterator pit = poly.end();
            const svtkIdType* ptsIt = pts + completePoly;
            const svtkIdType* ptsEnd = pts + npts - 1;
            if (endIdx == 1)
            {
  ...
  svtkPoints* points, const double normal[3])
{
  ...
  const double tol = SVTK_CCS_POLYGON_TOLERANCE;
  ...
  {
    svtkCCSPoly& poly1 = polys[incompletePolys[n - 1]];
    svtkIdType pt1 = poly1[poly1.size() - 1];
    double p1[3], p2[3];
    points->GetPoint(pt1, p1);

    double dMin = SVTK_DOUBLE_MAX;
    size_t iMin = 0;

    for (size_t i = 0; i < n; i++)
    {
      svtkCCSPoly& poly2 = polys[incompletePolys[i]];
      svtkIdType pt2 = poly2[0];
      ...
      double pc[4];
      svtkMath::Cross(normal, v, pc);
      pc[3] = -svtkMath::Dot(pc, pm);
...
int svtkCCSSplitAtPinchPoints(std::vector<svtkCCSPoly>& polys, svtkPoints* points,
  ...
  const double normal[3], bool oriented)
{
  svtkPoints* tryPoints = svtkPoints::New();
  tryPoints->SetDataTypeToDouble();

  svtkIncrementalOctreePointLocator* locator = svtkIncrementalOctreePointLocator::New();

  int splitCount = 0;

  for (size_t i = 0; i < polys.size(); i++)
  {
    svtkCCSPoly& poly = polys[i];
    size_t n = poly.size();

    double bounds[6];
    double tol = SVTK_CCS_POLYGON_TOLERANCE;
    tol *= sqrt(svtkCCSPolygonBounds(poly, points, bounds));
    ...
    {
      double point[3];
      svtkIdType firstId = poly[idx2];
      points->GetPoint(firstId, point);

      svtkIdType vertIdx = 0;
      if (!locator->InsertUniquePoint(point, vertIdx))
      {
        ...
        if ((idx2 > idx1 + 2 - unique) && (n + idx1 > idx2 + 2 - unique))
        {
          if (oriented)
          {
            ...
            if (svtkCCSVectorProgression(point, p1, p2, p3, normal) > 0)
            {
              foundMatch = 1;
              break;
            }
    ...
    if (foundMatch)
    {
      ...
      svtkCCSPoly& oldPoly = polys[i];
      svtkCCSPolyEdges& oldEdges = polyEdges[i];
      svtkCCSPoly newPoly1(m + unique);
      svtkCCSPolyEdges newEdges1(m + unique);
      svtkCCSPoly newPoly2(n - m + unique);
      svtkCCSPolyEdges newEdges2(n - m + unique);
  ...
  const double p3[3], const double normal[3])
{
  ...
  svtkMath::Cross(v2, v1, w1);
  svtkMath::Cross(v2, v3, w2);
  double s1 = svtkMath::Dot(w1, normal);
  double s2 = svtkMath::Dot(w2, normal);
  ...
  if (s1 != 0 && s2 != 0)
  {
    ...
    double c1 = svtkMath::Dot(v2, v1);
    double l1 = svtkMath::Norm(v1);
    double c2 = svtkMath::Dot(v2, v3);
    double l2 = svtkMath::Norm(v3);
...
void svtkCCSFindTrueEdges(std::vector<svtkCCSPoly>& polys, svtkPoints* points,
  std::vector<svtkCCSPolyEdges>& polyEdges, svtkCCSCellArray& originalEdges)
{
  // Tolerance^2 for angle to see if line segments are parallel
  const double atol2 = (SVTK_CCS_POLYGON_TOLERANCE * SVTK_CCS_POLYGON_TOLERANCE);

  for (size_t polyId = 0; polyId < polys.size(); polyId++)
  {
    svtkCCSPoly& oldPoly = polys[polyId];
    size_t n = oldPoly.size();
    polyEdges.push_back(svtkCCSPolyEdges());
    ...
    double bounds[6];
    double tol2 = svtkCCSPolygonBounds(oldPoly, points, bounds) * atol2;

    // The new poly
    svtkCCSPoly newPoly;
    svtkCCSPolyEdges& newEdges = polyEdges[polyId];
    svtkIdType cornerPointId = 0;
    svtkIdType oldOriginalId = -1;
    ...
    std::vector<svtkIdType> partialEdge;
    int cellCount = 0;
    ...
    for (size_t j = 0; j < n; j++)
    {
      ...
      v2[1] = p2[1] - p1[1];
      v2[2] = p2[2] - p1[2];
      l2 = svtkMath::Dot(v2, v2);

      // Dot product is |v1||v2|cos(theta)
      double c = svtkMath::Dot(v1, v2);
      ...
      if (m <= 3 || (l1 > tol2 && (c < 0 || l1 < tol2 || l2 < tol2 || s2 > l1 * l2 * atol2)))
      {
        ...
        if (cellCount > 1)
        {
          ...
          newEdges.push_back(static_cast<svtkIdType>(countLocation));
        }
        else if (cellCount == 0)
        {
    ...
    {
      svtkIdType pointId = partialEdge[ii];
      if (pointId != oldOriginalId)
      {
        if (cellCount == 1)
        {
...
void svtkCCSReversePoly(svtkCCSPoly& poly, svtkCCSPolyEdges& edges, svtkCCSCellArray& originalEdges)
{
  ...
  for (size_t i = 0; i < edges.size(); i++)
  {
    if (edges[i] >= 0)
    {
      svtkIdType* pts = &originalEdges[edges[i]];
      svtkIdType npts = *pts++;
...
// svtkCCSFindTrueEdges was called.
  ...
  const svtkCCSPolyEdges& polyEdges, const svtkCCSCellArray& originalEdges)
{
  ...
  for (int vert = 0; vert < 3; vert++)
  {
    ...
    svtkIdType edgeLoc = polyEdges[currId];
    if (edgeLoc >= 0)
    {
...
int svtkCCSCheckPolygonSense(
  svtkCCSPoly& poly, svtkPoints* points, const double normal[3], bool& sense)
{
  // Compute the normal
  ...
  // Check the normal
  double d = svtkMath::Dot(pnormal, normal);
...
// by calling svtkCCSPrepareForPolyInPoly() on outerPoly.

int svtkCCSPolyInPoly(const svtkCCSPoly& outerPoly, const svtkCCSPoly& innerPoly, svtkPoints* points,
  const double normal[3], const double* pp, const double bounds[6], double tol2)
{
  ...
  for (size_t jj = 0; jj < m; jj++)
  {
    ...
    if (svtkPolygon::PointInPolygon(p, static_cast<int>(n), const_cast<double*>(pp),
          const_cast<double*>(bounds), const_cast<double*>(normal)))
    {
      ...
      for (size_t ii = 0; ii < n; ii++)
      {
        ...
        if (svtkLine::DistanceToLine(p, q1, q2, t, dummy) < tol2)
        {
          pointOnEdge = 1;
          break;
        }
...
void svtkCCSPrepareForPolyInPoly(
  const svtkCCSPoly& outerPoly, svtkPoints* points, double* pp, double bounds[6], double& tol2)
{
  size_t n = outerPoly.size();
  ...
  tol2 = (svtkCCSPolygonBounds(outerPoly, points, bounds) *
    (SVTK_CCS_POLYGON_TOLERANCE * SVTK_CCS_POLYGON_TOLERANCE));
}
...
void svtkCCSMakeHoleyPolys(std::vector<svtkCCSPoly>& newPolys, svtkPoints* points,
  std::vector<svtkCCSPolyGroup>& polyGroups, std::vector<svtkCCSPolyEdges>& polyEdges,
  svtkCCSCellArray& originalEdges, const double normal[3], bool oriented)
{
  ...
  svtkCCSBitArray polyReversed;
  svtkCCSBitArray innerPolys;
  ...
  for (size_t i = 0; i < numNewPolys; i++)
  {
    ...
    bool sense = 0;
    if (svtkCCSCheckPolygonSense(newPolys[i], points, normal, sense))
    {
      polyReversed.set(i, !sense);
    }

    // Precompute some values needed for poly-in-poly checks
    svtkCCSPrepareForPolyInPoly(newPolys[i], points, pp, bounds, tol2);
    ...
    for (size_t j = 0; j < numNewPolys; j++)
    {
      if (j != i && newPolys[j].size() >= 3)
      {
        ...
        svtkCCSPolyGroup& pg = polyGroups[j];
        if (std::find(pg.begin(), pg.end(), i) == pg.end())
        {
          if (svtkCCSPolyInPoly(newPolys[i], newPolys[j], points, normal, pp, bounds, tol2))
          {
            // Add to group
  ...
  if (!oriented)
  {
    ...
    while (!outerPolyStack.empty())
    {
      ...
      {
        svtkCCSReversePoly(newPolys[j], polyEdges[j], originalEdges);
      ...
      if (polyGroups[j].size() > 1)
      {
        ...
        for (size_t k = 1; k < polyGroups[j].size(); k++)
        {
          ...
          else
          {
            ...
            {
              svtkCCSReversePoly(newPolys[jj], polyEdges[jj], originalEdges);
...
int svtkCCSCheckCut(const std::vector<svtkCCSPoly>& polys, svtkPoints* points, const double normal[3],
  const svtkCCSPolyGroup& polyGroup, size_t outerPolyId, size_t innerPolyId, svtkIdType outerIdx,
  svtkIdType innerIdx)
{
  svtkIdType ptId1 = polys[outerPolyId][outerIdx];
  svtkIdType ptId2 = polys[innerPolyId][innerIdx];
  ...
  {
    const svtkCCSPoly& poly = polys[polyId];
    size_t n = poly.size();
    ...
    if (svtkCCSVectorProgression(r, r1, r2, r3, normal) > 0)
    {
      return 0;
    }
  ...
  double pc[4];
  svtkMath::Cross(normal, w, pc);
  pc[3] = -svtkMath::Dot(pc, p1);

  for (size_t i = 0; i < polyGroup.size(); i++)
  {
    const svtkCCSPoly& poly = polys[polyGroup[i]];
    size_t n = poly.size();

    double q1[3];
    svtkIdType qtId1 = poly[n - 1];
    ...
    for (size_t j = 0; j < n; j++)
    {
      ...
      if (ptId1 != qtId1 && ptId1 != qtId2 && ptId2 != qtId1 && ptId2 != qtId2)
      {
        ...
        if ((c1 ^ c2) || v1 * v1 < tol2 || v2 * v2 < tol2)
        {
          ...
          w[1] = q2[1] - q1[1];
          w[2] = q2[2] - q1[2];
          if (svtkMath::Dot(w, w) > 0)
          {
            double qc[4];
            svtkMath::Cross(w, normal, qc);
            qc[3] = -svtkMath::Dot(qc, q1);
            ...
            if ((d1 ^ d2))
            {
              ...
              if (svtkMath::Distance2BetweenPoints(p, q) > tol2)
              {
                return 0;
              }
            }
...
double svtkCCSCutQuality(
  const svtkCCSPoly& outerPoly, const svtkCCSPoly& innerPoly, size_t i, size_t j, svtkPoints* points)
{
  size_t n = outerPoly.size();
  size_t m = innerPoly.size();
...
void svtkCCSFindSharpestVerts(
  const svtkCCSPoly& poly, svtkPoints* points, const double normal[3], size_t verts[2])
{
  double p1[3], p2[3];
  ...
  for (size_t j = 0; j < n; j++)
  {
    ...
    v2[1] = p2[1] - p1[1];
    v2[2] = p2[2] - p1[2];
    l2 = sqrt(svtkMath::Dot(v2, v2));

    svtkMath::Cross(v1, v2, v);
    double b = svtkMath::Dot(v, normal);

    if (b < 0 && l1 * l2 > 0)
    {
      // Dot product is |v1||v2|cos(theta), range [-1, +1]
      double val = svtkMath::Dot(v1, v2) / (l1 * l2);
      if (val < minVal[0])
      {
        minVal[1] = minVal[0];
        minVal[0] = val;
...
// Used by svtkCCSCutHoleyPolys.

int svtkCCSFindCuts(const std::vector<svtkCCSPoly>& polys, const svtkCCSPolyGroup& polyGroup,
  size_t outerPolyId, size_t innerPolyId, svtkPoints* points, const double normal[3],
  size_t cuts[2][2], size_t exhaustive)
{
  const svtkCCSPoly& outerPoly = polys[outerPolyId];
  const svtkCCSPoly& innerPoly = polys[innerPolyId];
  size_t innerSize = innerPoly.size();
  // Find the two sharpest points on the inner poly
  size_t verts[2];
  svtkCCSFindSharpestVerts(innerPoly, points, normal, verts);

  // A list of cut locations according to quality
  typedef std::pair<double, size_t> svtkCCSCutLoc;
  std::vector<svtkCCSCutLoc> cutlist(outerPoly.size());
  ...
  for (cutId = 0; cutId < 2; cutId++)
  {
    ...
    for (size_t i = 0; i < count && !foundCut; i++)
    {
      ...
      {
        double q = svtkCCSCutQuality(outerPoly, innerPoly, kk, j, points);
        cutlist[kk].first = q;
      ...
      for (size_t lid = 0; lid < cutlist.size(); lid++)
      {
        ...
        if (cutId > 0)
        {
          ...
          double u, v;
          if (svtkLine::Intersection(p1, p2, q1, q2, u, v) == 2)
          {
            continue;
          }
        }
        ...
        if (svtkCCSCheckCut(polys, points, normal, polyGroup, outerPolyId, innerPolyId,
              static_cast<svtkIdType>(k), static_cast<svtkIdType>(j)))
        {
          cuts[cutId][0] = k;
          cuts[cutId][1] = j;
          foundCut = 1;
...
// ---------------------------------------------------
// Helper for svtkCCSCutHoleyPolys.  Change a polygon and a hole
// into two separate polygons by making two cuts between them.

void svtkCCSMakeCuts(std::vector<svtkCCSPoly>& polys, std::vector<svtkCCSPolyEdges>& polyEdges,
  size_t outerPolyId, size_t innerPolyId, svtkPoints* points, const size_t cuts[2][2])
{
  double q[3], r[3];
  for (size_t bb = 0; bb < 2; bb++)
  {
    svtkIdType ptId1 = polys[outerPolyId][cuts[bb][0]];
    svtkIdType ptId2 = polys[innerPolyId][cuts[bb][1]];
    points->GetPoint(ptId1, q);
    points->GetPoint(ptId2, r);
  }

  svtkCCSPoly& outerPoly = polys[outerPolyId];
  svtkCCSPoly& innerPoly = polys[innerPolyId];

  svtkCCSPolyEdges& outerEdges = polyEdges[outerPolyId];
  svtkCCSPolyEdges& innerEdges = polyEdges[innerPolyId];
  ...
  svtkCCSPoly poly1(n2);
  svtkCCSPolyEdges edges1(n2);
  ...
  svtkCCSPoly poly2(m2);
  svtkCCSPolyEdges edges2(m2);
  ...
  const double normal[3])
{
  ...
  {
    svtkCCSPolyGroup& polyGroup = polyGroups[groupId];
    ...
    if (polyGroup.size() > 1)
    {
      ...
      for (int exhaustive = 0; exhaustive < 2 && !madeCut; exhaustive++)
      {
        for (size_t j = 1; j < polyGroup.size(); j++)
        {
          ...
          {
            svtkCCSMakeCuts(polys, polyEdges, outerPolyId, innerPolyId, points, cuts);
            madeCut = 1;
            break;
          }
        }
      ...
      if (polyGroup.size() > 1)
      {
        svtkCCSPoly& poly1 = polys[outerPolyId];
        double* pp = new double[3 * poly1.size()];
        double bounds[6];
        double tol2;
        svtkCCSPrepareForPolyInPoly(poly1, points, pp, bounds, tol2);

        size_t ii = 1;
        while (ii < polyGroup.size())
        {
          if (svtkCCSPolyInPoly(poly1, polys[polyGroup[ii]], points, normal, pp, bounds, tol2))
          {
...
//#define SVTK_CCS_SHOW_FAILED_POLYS

int svtkContourTriangulator::TriangulateContours(svtkPolyData* data, svtkIdType firstLine,
  svtkIdType numLines, svtkCellArray* polys, const double normal[3])
{
  int triangulationFailure = 0;
  ...
  std::vector<svtkCCSPoly> newPolys;
  ...
  if (normal == nullptr)
  {
    ...
    {
      double n[3];
      double norm2 = svtkCCSPolygonNormal(newPolys[i], points, n);
      if (norm2 > maxnorm2)
      {
        maxnorm2 = norm2;
  ...
  svtkCCSJoinLooseEnds(newPolys, incompletePolys, points, normal);
  ...
  std::vector<svtkCCSPolyEdges> polyEdges;
  polyEdges.reserve(100);
  svtkCCSCellArray originalEdges;
  originalEdges.reserve(200);
  svtkCCSFindTrueEdges(newPolys, points, polyEdges, originalEdges);
  ...
  size_t numNewPolys = newPolys.size();
  std::vector<svtkCCSPolyGroup> polyGroups(numNewPolys);
  for (size_t i = 0; i < numNewPolys; i++)
  {
  ...
  svtkCCSMakeHoleyPolys(newPolys, points, polyGroups, polyEdges, originalEdges, normal, oriented);
  ...
  if (!svtkCCSCutHoleyPolys(newPolys, points, polyGroups, polyEdges, normal))
  {
  ...
  svtkCCSSplitAtPinchPoints(newPolys, points, polyGroups, polyEdges, normal, oriented);
...
// ---------------------------------------------------
int svtkContourTriangulator::TriangulatePolygon(
  svtkIdList* polygon, svtkPoints* points, svtkCellArray* triangles)
{
  svtkIdType n = polygon->GetNumberOfIds();
  std::vector<svtkCCSPoly> polys(1);
  svtkCCSPoly& poly = polys[0];
  poly.resize(n);
  ...
  svtkCCSCellArray originalEdges;
  std::vector<svtkCCSPolyEdges> polyEdges;
  svtkCCSFindTrueEdges(polys, points, polyEdges, originalEdges);
  svtkCCSPolyEdges& edges = polyEdges[0];

  int success = 1;
  double normal[3];
  if (svtkCCSPolygonNormal(poly, points, normal))
  {
    success = svtkCCSTriangulate(poly, points, edges, originalEdges, triangles, normal);
  }
  return success;
}
