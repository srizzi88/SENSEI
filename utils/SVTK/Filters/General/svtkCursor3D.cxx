...
svtkStandardNewMacro(svtkCursor3D);

// Construct with model bounds = (-1,1,-1,1,-1,1), focal point = (0,0,0),
// all parts of cursor visible, and wrapping off.
svtkCursor3D::svtkCursor3D()
{
  svtkPoints* pts;

  this->Focus = svtkPolyData::New();
  pts = svtkPoints::New();
  pts->SetDataTypeToDouble();
  pts->svtkPoints::InsertPoint(0, 0.0, 0.0, 0.0);
  this->Focus->SetPoints(pts);
  pts->Delete();
  svtkCellArray* vert = svtkCellArray::New();
...
int svtkCursor3D::RequestData(svtkInformation* svtkNotUsed(request),
...
void svtkCursor3D::SetModelBounds(
...
void svtkCursor3D::SetFocalPoint(double x[3])
{
...
void svtkCursor3D::PrintSelf(ostream& os, svtkIndent indent)
{
