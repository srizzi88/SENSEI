/*=========================================================================
  ...
  Module:    svtkDiscreteFlyingEdges2D.cxx
...
=========================================================================*/
#include "svtkDiscreteFlyingEdges2D.h"
...
svtkStandardNewMacro(svtkDiscreteFlyingEdges2D);

// This templated class is the heart of the algorithm. Templated across
// scalar type T. svtkDiscreteFlyingEdges2D populates the information in this class
// and then invokes ContourImage() to actually initiate executions.
template <class T>
class svtkDiscreteFlyingEdges2DAlgorithm
{
public:
  ...
  svtkDiscreteFlyingEdges2DAlgorithm();

  // The three passes of the algorithm.
  void ProcessXEdge(double value, T* inPtr, svtkIdType row);   // PASS 1
  void ProcessYEdges(svtkIdType row);                          // PASS 2
  void GenerateOutput(double value, T* inPtr, svtkIdType row); // PASS 4
  ...
  void CountBoundaryYInts(unsigned char loc, unsigned char* edgeUses, svtkIdType* eMD)
  {
    if (loc == 2)
    { //+x boundary
      eMD[1] += edgeUses[3];
  ...
  struct GenerateLinesImpl
  {
    ...
    void operator()(CellStateT& state, const unsigned char* edges, int numLines, svtkIdType* eIds,
      svtkIdType& lineId)
    {
  ...
  void GenerateLines(
    unsigned char eCase, unsigned char numLines, svtkIdType* eIds, svtkIdType& lineId)
  {
  ...
  void InterpolateAxesEdge(double svtkNotUsed(value), T* svtkNotUsed(s0), int ijk0[3],
    T* svtkNotUsed(s1), int ijk1[3], svtkIdType vId)
  {
    double t = 0.5;
  ...
  void InterpolateEdge(double value, T* s, int ijk[3], unsigned char edgeNum,
    unsigned char edgeUses[4], svtkIdType* eIds);

  // Produce the output points on the pixel axes for this pixel cell.
  void GeneratePoints(
    double value, unsigned char loc, T* sPtr, int ijk[3], unsigned char* edgeUses, svtkIdType* eIds);

  // Helper function to set up the point ids on pixel edges.
  unsigned char InitPixelIds(
    unsigned char* ePtr0, unsigned char* ePtr1, svtkIdType* eMD0, svtkIdType* eMD1, svtkIdType* eIds)
  {
  ...
  void AdvancePixelIds(unsigned char eCase, svtkIdType* eIds)
  {
  ...
  {
  public:
    Pass1(svtkDiscreteFlyingEdges2DAlgorithm<TT>* algo, double value)
    {
      this->Algo = algo;
      this->Value = value;
    }
    svtkDiscreteFlyingEdges2DAlgorithm<TT>* Algo;
    double Value;
  ...
  {
  public:
    Pass2(svtkDiscreteFlyingEdges2DAlgorithm<TT>* algo) { this->Algo = algo; }
    svtkDiscreteFlyingEdges2DAlgorithm<TT>* Algo;
    void operator()(svtkIdType row, svtkIdType end)
    {
      for (; row < end; ++row)
  ...
  {
  public:
    Pass4(svtkDiscreteFlyingEdges2DAlgorithm<TT>* algo, double value)
    {
      this->Algo = algo;
      this->Value = value;
    }
  ...
  static void ContourImage(svtkDiscreteFlyingEdges2D* self, T* scalars, svtkPoints* newPts,
    svtkDataArray* newScalars, svtkCellArray* newLines, svtkImageData* input, int* updateExt);
};

//----------------------------------------------------------------------------
// Specify the points that define each edge.
template <class T>
const unsigned char svtkDiscreteFlyingEdges2DAlgorithm<T>::VertMap[4][2] = {
  { 0, 1 },
  { 2, 3 },
  { 0, 2 },
  { 1, 3 },
...
template <class T>
const unsigned char svtkDiscreteFlyingEdges2DAlgorithm<T>::VertOffsets[4][2] = {
  { 0, 0 },
  { 1, 0 },
  { 0, 1 },
  { 1, 1 },
...
template <class T>
const unsigned char svtkDiscreteFlyingEdges2DAlgorithm<T>::EdgeCases[16][5] = {
  { 0, 0, 0, 0, 0 },
  { 1, 0, 2, 0, 0 },
  { 1, 3, 0, 0, 0 },
  { 1, 3, 2, 0, 0 },
...
template <class T>
svtkDiscreteFlyingEdges2DAlgorithm<T>::svtkDiscreteFlyingEdges2DAlgorithm()
  : XCases(nullptr)
...
template <class T>
void svtkDiscreteFlyingEdges2DAlgorithm<T>::InterpolateEdge(double svtkNotUsed(value),
  T* svtkNotUsed(s), int ijk[3], unsigned char edgeNum, unsigned char edgeUses[12], svtkIdType* eIds)
{
...
template <class T>
void svtkDiscreteFlyingEdges2DAlgorithm<T>::GeneratePoints(
  double value, unsigned char loc, T* sPtr, int ijk[3], unsigned char* edgeUses, svtkIdType* eIds)
{
...
template <class T>
void svtkDiscreteFlyingEdges2DAlgorithm<T>::ProcessXEdge(double value, T* inPtr, svtkIdType row)
{
  svtkIdType nxcells = this->Dims[0] - 1;
  svtkIdType minInt = nxcells, maxInt = 0;
  unsigned char edgeCase;
  svtkIdType* eMD = this->EdgeMetaData + row * 5;
  ...
  for (svtkIdType i = 0; i < nxcells; ++i, ++ePtr)
  {
    ...
    if (s0 != labelValue)
    {
      edgeCase = (s1 != value ? svtkDiscreteFlyingEdges2DAlgorithm::BothOutside
                              : svtkDiscreteFlyingEdges2DAlgorithm::LeftOutside);
    }
    else // s0 == labelValue
    {
      edgeCase = (s1 != value ? svtkDiscreteFlyingEdges2DAlgorithm::RightOutside
                              : svtkDiscreteFlyingEdges2DAlgorithm::BothInside);
    }

    this->SetXEdge(ePtr, edgeCase);

    // if edge intersects contour
    if (edgeCase == svtkDiscreteFlyingEdges2DAlgorithm::LeftOutside ||
      edgeCase == svtkDiscreteFlyingEdges2DAlgorithm::RightOutside)
    {
...
// work.
template <class T>
void svtkDiscreteFlyingEdges2DAlgorithm<T>::ProcessYEdges(svtkIdType row)
{
  ...
  // And metadata
  svtkIdType* eMD0 = this->EdgeMetaData + row * 5;
  svtkIdType* eMD1 = this->EdgeMetaData + (row + 1) * 5;
  ...
  svtkIdType xL = ((eMD0[3] < eMD1[3]) ? eMD0[3] : eMD1[3]);
  svtkIdType xR = ((eMD0[4] > eMD1[4]) ? eMD0[4] : eMD1[4]);
  if (xInts)
  {
    if (xL > 0)
    {
...
// algorithm.
template <class T>
void svtkDiscreteFlyingEdges2DAlgorithm<T>::GenerateOutput(double value, T* rowPtr, svtkIdType row)
{
  svtkIdType* eMD0 = this->EdgeMetaData + row * 5;
  svtkIdType* eMD1 = this->EdgeMetaData + (row + 1) * 5;
  ...
  svtkIdType lineId = eMD0[2];
  svtkIdType eIds[4]; // the ids of generated points
...
// svtkDiscreteFlyingEdges2D class with the templated algorithm class. It also invokes
// the three passes of the Flying Edges algorithm.
//
template <class T>
void svtkDiscreteFlyingEdges2DAlgorithm<T>::ContourImage(svtkDiscreteFlyingEdges2D* self, T* scalars,
  svtkPoints* newPts, svtkDataArray* newScalars, svtkCellArray* newLines, svtkImageData* input,
  int* updateExt)
{
  double value, *values = self->GetValues();
  svtkIdType numContours = self->GetNumberOfContours();
  svtkIdType vidx, row, *eMD;
  svtkIdType numOutXPts, numOutYPts, numOutLines, numXPts = 0, numYPts = 0, numLines = 0;
  svtkIdType startXPts, startYPts, startLines;
  ...
  svtkDiscreteFlyingEdges2DAlgorithm<T> algo;
  ...
  algo.EdgeMetaData = new svtkIdType[algo.Dims[1] * 5];
...
svtkDiscreteFlyingEdges2D::svtkDiscreteFlyingEdges2D()
{
...
svtkMTimeType svtkDiscreteFlyingEdges2D::GetMTime()
{
...
//
int svtkDiscreteFlyingEdges2D::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  svtkDebugMacro(<< "Executing 2D Flying Edges");
  ...
  {
    svtkTemplateMacro(svtkDiscreteFlyingEdges2DAlgorithm<SVTK_TT>::ContourImage(
      this, (SVTK_TT*)scalars, newPts, newScalars, newLines, input, ext));
  } // switch
...
//----------------------------------------------------------------------------
int svtkDiscreteFlyingEdges2D::FillInputPortInformation(int, svtkInformation* info)
{
...
//----------------------------------------------------------------------------
void svtkDiscreteFlyingEdges2D::PrintSelf(ostream& os, svtkIndent indent)
{
