/*=========================================================================
  ...
  Module:    svtkDiscreteFlyingEdges3D.cxx
...
=========================================================================*/
#include "svtkDiscreteFlyingEdges3D.h"
...
svtkStandardNewMacro(svtkDiscreteFlyingEdges3D);

//----------------------------------------------------------------------------
namespace
{
// This templated class implements the heart of the algorithm.
// svtkDiscreteFlyingEdges3D populates the information in this class and
// then invokes Contour() to actually initiate execution.
template <class T>
class svtkDiscreteFlyingEdges3DAlgorithm
{
public:
  ...
  // A table to map old edge ids (as defined from svtkMarchingCubesCases) into
  ...
  svtkIdType* EdgeMetaData;

  // Internal variables used by the various algorithm methods. Interfaces SVTK
  // image data in a form more convenient to the algorithm.
  T* Scalars;
  svtkIdType Dims[3];
  svtkIdType NumberOfEdges;
  svtkIdType SliceOffset;
  int Min0;
  int Max0;
  int Inc0;
  int Min1;
  ...
  // Setup algorithm
  svtkDiscreteFlyingEdges3DAlgorithm();

  // The three main passes of the algorithm.
  void ProcessXEdge(double value, T const* const inPtr, svtkIdType row, svtkIdType slice); // PASS 1
  void ProcessYZEdges(svtkIdType row, svtkIdType slice);                                   // PASS 2
  void GenerateOutput(double value, T* inPtr, svtkIdType row, svtkIdType slice);           // PASS 4
  ...
  void CountBoundaryYZInts(unsigned char loc, unsigned char* edgeCases, svtkIdType* eMD[4]);
  ...
  struct GenerateTrisImpl
  {
    ...
    void operator()(
      CellStateT& state, const unsigned char* edges, int numTris, svtkIdType* eIds, svtkIdType& triId)
    {
      ...
      auto connRange = svtk::DataArrayValueRange<1>(conn);
  ...
  void GenerateTris(unsigned char eCase, unsigned char numTris, svtkIdType* eIds, svtkIdType& triId)
  {
  ...
  void ComputeGradient(unsigned char loc, svtkIdType ijk[3], T const* const s0_start,
  ...
  void InterpolateAxesEdge(double t, unsigned char loc, T const* const s, const int incs[3],
    svtkIdType vId, svtkIdType ijk0[3], svtkIdType ijk1[3], float g0[3])
  {
    ...
    {
      svtkIdType v0 = ijk0[0] + ijk0[1] * incs[1] + ijk0[2] * incs[2];
      svtkIdType v1 = ijk1[0] + ijk1[1] * incs[1] + ijk1[2] * incs[2];
      this->Arrays.InterpolateEdge(v0, v1, t, vId);
    }
  }

  // Compute the gradient on a point which may be on the boundary of the volume.
  void ComputeBoundaryGradient(svtkIdType ijk[3], T const* const s0_start, T const* const s0_end,
  ...
  void InterpolateEdge(double value, svtkIdType ijk[3], T const* const s, const int incs[3],
    unsigned char edgeNum, unsigned char const* const edgeUses, svtkIdType* eIds);

  // Produce the output points on the voxel axes for this voxel cell.
  void GeneratePoints(double value, unsigned char loc, svtkIdType ijk[3], T const* const sPtr,
    const int incs[3], unsigned char const* const edgeUses, svtkIdType* eIds);

  // Helper function to set up the point ids on voxel edges.
  unsigned char InitVoxelIds(unsigned char* ePtr[4], svtkIdType* eMD[4], svtkIdType* eIds)
  {
  ...
  void AdvanceVoxelIds(unsigned char eCase, svtkIdType* eIds)
  {
  ...
  {
  public:
    svtkDiscreteFlyingEdges3DAlgorithm<TT>* Algo;
    double Value;
    Pass1(svtkDiscreteFlyingEdges3DAlgorithm<TT>* algo, double value)
    {
      this->Algo = algo;
      this->Value = value;
    }
  ...
  {
  public:
    Pass2(svtkDiscreteFlyingEdges3DAlgorithm<TT>* algo) { this->Algo = algo; }
    svtkDiscreteFlyingEdges3DAlgorithm<TT>* Algo;
    void operator()(svtkIdType slice, svtkIdType end)
    {
      for (; slice < end; ++slice)
      {
        for (svtkIdType row = 0; row < (this->Algo->Dims[1] - 1); ++row)
        {
  ...
  {
  public:
    Pass4(svtkDiscreteFlyingEdges3DAlgorithm<TT>* algo, double value)
    {
      this->Algo = algo;
      this->Value = value;
    }
    ...
    {
      svtkIdType row;
      svtkIdType* eMD0 = this->Algo->EdgeMetaData + slice * 6 * this->Algo->Dims[1];
      svtkIdType* eMD1 = eMD0 + 6 * this->Algo->Dims[1];
      TT *rowPtr, *slicePtr = this->Algo->Scalars + slice * this->Algo->Inc2;
      for (; slice < end; ++slice)
      {
  ...
  static void Contour(svtkDiscreteFlyingEdges3D* self, svtkImageData* input, svtkDataArray* inScalars,
    int extent[6], svtkIdType* incs, T* scalars, svtkPolyData* output, svtkPoints* newPts,
    svtkCellArray* newTris, svtkDataArray* newScalars, svtkFloatArray* newNormals,
...
template <class T>
const unsigned char svtkDiscreteFlyingEdges3DAlgorithm<T>::EdgeMap[12] = { 0, 5, 1, 4, 2, 7, 3, 6, 8,
  9, 10, 11 };

//----------------------------------------------------------------------------
// Map MC edges numbering to use the saner FlyingEdges edge numbering scheme.
template <class T>
const unsigned char svtkDiscreteFlyingEdges3DAlgorithm<T>::VertMap[12][2] = {
  { 0, 1 },
  { 2, 3 },
  { 4, 5 },
  { 6, 7 },
...
template <class T>
const unsigned char svtkDiscreteFlyingEdges3DAlgorithm<T>::VertOffsets[8][3] = {
  { 0, 0, 0 },
  { 1, 0, 0 },
  { 0, 1, 0 },
  { 1, 1, 0 },
...
// svtkVoxel::Contour() method.
template <class T>
svtkDiscreteFlyingEdges3DAlgorithm<T>::svtkDiscreteFlyingEdges3DAlgorithm()
  : XCases(nullptr)
...
template <class T>
void svtkDiscreteFlyingEdges3DAlgorithm<T>::CountBoundaryYZInts(
  unsigned char loc, unsigned char* edgeUses, svtkIdType* eMD[4])
{
  switch (loc)
  {
    case 2: //+x boundary
...
// volume.
template <class T>
void svtkDiscreteFlyingEdges3DAlgorithm<T>::ComputeBoundaryGradient(svtkIdType ijk[3],
...
template <class T>
void svtkDiscreteFlyingEdges3DAlgorithm<T>::InterpolateEdge(double svtkNotUsed(value),
  svtkIdType ijk[3], T const* const s, const int incs[3], unsigned char edgeNum,
  unsigned char const* const edgeUses, svtkIdType* eIds)
{
  ...
  svtkIdType ijk0[3], ijk1[3], vId = eIds[edgeNum];
...
template <class T>
void svtkDiscreteFlyingEdges3DAlgorithm<T>::GeneratePoints(double value, unsigned char loc,
  svtkIdType ijk[3], T const* const sPtr, const int incs[3], unsigned char const* const edgeUses,
  svtkIdType* eIds)
{
  ...
  for (int i = 0; i < 3; ++i)
  {
    if (edgeUses[i * 4])
    {
      ...
      svtkIdType ijk1[3] = { ijk[0], ijk[1], ijk[2] };
      ++ijk1[i];
...
// trimming).
template <class T>
void svtkDiscreteFlyingEdges3DAlgorithm<T>::ProcessXEdge(
  double value, T const* const inPtr, svtkIdType row, svtkIdType slice)
{
  svtkIdType nxcells = this->Dims[0] - 1;
  svtkIdType minInt = nxcells, maxInt = 0;
  svtkIdType* edgeMetaData;
  ...
  svtkIdType inc0 = this->Inc0;

  for (svtkIdType i = 0; i < nxcells; ++i, ++ePtr)
  {
    s0 = s1;
    ...
    if (s0 != labelValue)
    {
      edgeCase = (s1 != value ? svtkDiscreteFlyingEdges3DAlgorithm::BothOutside
                              : svtkDiscreteFlyingEdges3DAlgorithm::LeftOutside);
    }
    else // s0 == labelValue
    {
      edgeCase = (s1 != value ? svtkDiscreteFlyingEdges3DAlgorithm::RightOutside
                              : svtkDiscreteFlyingEdges3DAlgorithm::BothInside);
    }

    this->SetXEdge(ePtr, edgeCase);

    // if edge intersects contour
    if (edgeCase == svtkDiscreteFlyingEdges3DAlgorithm::LeftOutside ||
      edgeCase == svtkDiscreteFlyingEdges3DAlgorithm::RightOutside)
    {
...
// information.
template <class T>
void svtkDiscreteFlyingEdges3DAlgorithm<T>::ProcessYZEdges(svtkIdType row, svtkIdType slice)
{
  ...
  svtkIdType xL = eMD[0][4], xR = eMD[0][5];
  svtkIdType i;
  if (xInts)
  {
    for (i = 1; i < 4; ++i)
  ...
  ePtr[2] += xL;
  ePtr[3] += xL;
  const svtkIdType dim0Wall = this->Dims[0] - 2;
...
// algorithm.
template <class T>
void svtkDiscreteFlyingEdges3DAlgorithm<T>::GenerateOutput(
  double value, T* rowPtr, svtkIdType row, svtkIdType slice)
{
  // Grab the edge meta data surrounding the voxel row.
  svtkIdType* eMD[4];
  eMD[0] = this->EdgeMetaData + (slice * this->Dims[1] + row) * 6; // this x-edge
  ...
  svtkIdType triId = eMD[0][3];
  svtkIdType eIds[12]; // the ids of generated points
...
// interfaces the svtkDiscreteFlyingEdges3D class with the templated algorithm
// class. It also invokes the three passes of the Flying Edges algorithm.
template <class T>
void svtkDiscreteFlyingEdges3DAlgorithm<T>::Contour(svtkDiscreteFlyingEdges3D* self,
  svtkImageData* input, svtkDataArray* inScalars, int extent[6], svtkIdType* incs, T* scalars,
  svtkPolyData* output, svtkPoints* newPts, svtkCellArray* newTris, svtkDataArray* newScalars,
  svtkFloatArray* newNormals, svtkFloatArray* newGradients)
{
  double value, *values = self->GetValues();
  svtkIdType numContours = self->GetNumberOfContours();
  svtkIdType vidx, row, slice, *eMD, zInc;
  svtkIdType numOutXPts, numOutYPts, numOutZPts, numOutTris;
  svtkIdType numXPts = 0, numYPts = 0, numZPts = 0, numTris = 0;
  svtkIdType startXPts, startYPts, startZPts, startTris;
  startXPts = startYPts = startZPts = startTris = 0;

  // This may be subvolume of the total 3D image. Capture information for
  // subsequent processing.
  svtkDiscreteFlyingEdges3DAlgorithm<T> algo;
  ...
  algo.EdgeMetaData = new svtkIdType[algo.NumberOfEdges * 6];
  ...
  for (vidx = 0; vidx < numContours; vidx++)
  {
    ...
    svtkSMPTools::For(0, algo.Dims[2], pass1);
    ...
    Pass2<T> pass2(&algo);
    svtkSMPTools::For(0, algo.Dims[2] - 1, pass2);
    ...
    svtkIdType totalPts = numOutXPts + numOutYPts + numOutZPts;
    if (totalPts > 0)
    {
      ...
      if (newScalars)
      {
        svtkIdType numPrevPts = newScalars->GetNumberOfTuples();
        svtkIdType numNewPts = totalPts - numPrevPts;
      ...
      svtkSMPTools::For(0, algo.Dims[2] - 1, pass4);
...
svtkDiscreteFlyingEdges3D::svtkDiscreteFlyingEdges3D()
{
...
svtkMTimeType svtkDiscreteFlyingEdges3D::GetMTime()
{
...
//----------------------------------------------------------------------------
int svtkDiscreteFlyingEdges3D::RequestUpdateExtent(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  if (this->ComputeGradients || this->ComputeNormals)
  {
    ...
    inInfo->Set(svtkStreamingDemandDrivenPipeline::UPDATE_NUMBER_OF_GHOST_LEVELS(), ghostLevels + 1);
  }
...
//----------------------------------------------------------------------------
int svtkDiscreteFlyingEdges3D::RequestData(
  svtkInformation* request, svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  svtkDebugMacro(<< "Executing 3D flying edges");
  ...
  int exExt[6];
  inInfo->Get(svtkStreamingDemandDrivenPipeline::UPDATE_EXTENT(), exExt);
  for (int i = 0; i < 3; i++)
  {
  ...
  {
    svtkDebugMacro(<< "3D structured contours requires 3D data");
    return 0;
  }
  ...
  {
    svtkDebugMacro("No scalars for contouring.");
    return 0;
  }
  ...
  {
    svtkErrorMacro("Scalars have " << numComps
                                  << " components. "
  ...
  svtkCellArray* newTris = svtkCellArray::New();
  ...
  {
    svtkTemplateMacro(svtkDiscreteFlyingEdges3DAlgorithm<SVTK_TT>::Contour(this, input, inScalars,
      exExt, incs, (SVTK_TT*)ptr, output, newPts, newTris, newScalars, newNormals, newGradients));
  }
  ...
  if (newNormals)
  {
    ...
    output->GetPointData()->SetActiveAttribute(idx, svtkDataSetAttributes::NORMALS);
    newNormals->Delete();
  }
  ...
  if (newGradients)
  {
    ...
    output->GetPointData()->SetActiveAttribute(idx, svtkDataSetAttributes::VECTORS);
    newGradients->Delete();
  }
...
//----------------------------------------------------------------------------
int svtkDiscreteFlyingEdges3D::FillInputPortInformation(int, svtkInformation* info)
{
...
//----------------------------------------------------------------------------
void svtkDiscreteFlyingEdges3D::PrintSelf(ostream& os, svtkIndent indent)
{
