/*=========================================================================
  ...
  Module:    svtkDiscreteMarchingCubes.cxx
...
svtkDiscreteMarchingCubes::~svtkDiscreteMarchingCubes() = default;

//
// Contouring filter specialized for volumes and "short int" data values.
//
template <class T>
void svtkDiscreteMarchingCubesComputeGradient(svtkDiscreteMarchingCubes* self, T* scalars,
  int dims[3], svtkIncrementalPointLocator* locator, svtkDataArray* newCellScalars,
  svtkDataArray* newPointScalars, svtkCellArray* newPolys, double* values, int numValues)
{
  double s[8], value;
  int i, j, k, pts[8][3], xp, yp, zp, *x1, *x2;
  svtkIdType sliceSize, rowSize;
  ...
  svtkIdType jOffset, kOffset, idx;
  svtkIdType ptIds[3];
  int extent[6];
...
// Contouring filter specialized for volumes and "short int" data values.
//
int svtkDiscreteMarchingCubes::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  svtkFloatArray* newCellScalars;
  svtkFloatArray* newPointScalars;
  ...
  estimatedSize *= dims[1]; // The "*=" ensures coercion to svtkIdType,
  estimatedSize *= dims[2]; // which might be wider than "int"
  estimatedSize = static_cast<svtkIdType>(pow(static_cast<double>(estimatedSize), .75));
  ...
  {
    newCellScalars = svtkFloatArray::New();
  ...
  {
    newPointScalars = svtkFloatArray::New();
  ...
  if (inScalars->GetNumberOfComponents() == 1)
  {
    ...
    {
      svtkTemplateMacro(svtkDiscreteMarchingCubesComputeGradient(this, static_cast<SVTK_TT*>(scalars),
  ...
  {
    svtkIdType dataSize = dims[0];
    dataSize *= dims[1]; // The "*=" ensures coercion to svtkIdType,
    ...
    svtkDiscreteMarchingCubesComputeGradient(this, scalars, dims, this->Locator, newCellScalars,
