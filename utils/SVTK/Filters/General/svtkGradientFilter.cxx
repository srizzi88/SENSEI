...
svtkStandardNewMacro(svtkGradientFilter);
...
template <class data_type>
void ComputePointGradientsUG(svtkDataSet* structure, svtkDataArray* array, data_type* gradients,
...
int GetCellParametricData(
  svtkIdType pointId, double pointCoord[3], svtkCell* cell, int& subId, double parametricCoord[3]);

template <class data_type>
void ComputeCellGradientsUG(svtkDataSet* structure, svtkDataArray* array, data_type* gradients,
  int numberOfInputComponents, data_type* vorticity, data_type* qCriterion, data_type* divergence);

// Functions for image data and structured grids
template <class Grid, class data_type>
void ComputeGradientsSG(Grid output, svtkDataArray* array, data_type* gradients,
  int numberOfInputComponents, int fieldAssociation, data_type* vorticity, data_type* qCriterion,
  data_type* divergence);

bool svtkGradientFilterHasArray(svtkFieldData* fieldData, svtkDataArray* array)
{
...
void GetGridEntityCoordinate(
  svtkDataSet* grid, int fieldAssociation, svtkIdType index, double coords[3])
{
  if (fieldAssociation == svtkDataObject::FIELD_ASSOCIATION_POINTS)
  {
    grid->GetPoint(index, coords);
  }
  else
  {
    svtkCell* cell = grid->GetCell(index);
    double pcoords[3];
...
template <class data_type>
void Fill(svtkDataArray* array, data_type svtkNotUsed(data), int replacementValueOption)
{
  switch (replacementValueOption)
  {
    case svtkGradientFilter::Zero:
      array->Fill(0);
      return;
    case svtkGradientFilter::NaN:
      array->Fill(svtkMath::Nan());
      return;
    case svtkGradientFilter::DataTypeMin:
      array->Fill(std::numeric_limits<data_type>::min());
      return;
    case svtkGradientFilter::DataTypeMax:
      array->Fill(std::numeric_limits<data_type>::max());
      return;
  }
}
template <class data_type>
int GetOutputDataType(data_type svtkNotUsed(data))
{
...
//-----------------------------------------------------------------------------
svtkGradientFilter::svtkGradientFilter()
{
  ...
  this->ContributingCellOption = svtkGradientFilter::All;
  this->ReplacementValueOption = svtkGradientFilter::Zero;
  this->SetInputScalars(
    svtkDataObject::FIELD_ASSOCIATION_POINTS_THEN_CELLS, svtkDataSetAttributes::SCALARS);
}
...
//-----------------------------------------------------------------------------
void svtkGradientFilter::PrintSelf(ostream& os, svtkIndent indent)
{
...
//-----------------------------------------------------------------------------
void svtkGradientFilter::SetInputScalars(int fieldAssociation, const char* name)
{
  if ((fieldAssociation != svtkDataObject::FIELD_ASSOCIATION_POINTS) &&
  ...
  {
    svtkErrorMacro("Input Array must be associated with points or cells.");
    return;
  }
...
//-----------------------------------------------------------------------------
void svtkGradientFilter::SetInputScalars(int fieldAssociation, int fieldAttributeType)
{
  if ((fieldAssociation != svtkDataObject::FIELD_ASSOCIATION_POINTS) &&
    (fieldAssociation != svtkDataObject::FIELD_ASSOCIATION_CELLS) &&
    (fieldAssociation != svtkDataObject::FIELD_ASSOCIATION_POINTS_THEN_CELLS))
...
//-----------------------------------------------------------------------------
int svtkGradientFilter::RequestUpdateExtent(svtkInformation* svtkNotUsed(request),
...
//-----------------------------------------------------------------------------
int svtkGradientFilter::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  if (input->GetNumberOfCells() == 0)
  {
    ...
    {
      svtkWarningMacro("Cannot compute gradient for datasets without cells");
    }
  ...
  if (array == nullptr)
  {
    svtkErrorMacro("No input array. If this dataset is part of a composite dataset"
      << " check to make sure that all non-empty blocks have this array.");
    return 0;
  }
  if (array->GetNumberOfComponents() == 0)
  {
    svtkErrorMacro("Input array must have at least one component.");
    return 0;
  }
  ...
  {
    svtkWarningMacro("Input array must have exactly three components with "
  ...
  if (svtkGradientFilterHasArray(input->GetPointData(), array))
  {
    fieldAssociation = svtkDataObject::FIELD_ASSOCIATION_POINTS;
  }
  else if (svtkGradientFilterHasArray(input->GetCellData(), array))
  {
    fieldAssociation = svtkDataObject::FIELD_ASSOCIATION_CELLS;
  }
  else
  {
    svtkErrorMacro("Input arrays do not seem to be either point or cell arrays.");
    return 0;
  }
  ...
  if (output->IsA("svtkImageData") || output->IsA("svtkStructuredGrid") ||
    output->IsA("svtkRectilinearGrid"))
  {
...
//-----------------------------------------------------------------------------
int svtkGradientFilter::ComputeUnstructuredGridGradient(svtkDataArray* array, int fieldAssociation,
  ...
  svtkDataSet* output)
{
  ...
  svtkSmartPointer<svtkDataArray> gradients = nullptr;
  if (this->ComputeGradient)
  {
    gradients.TakeReference(svtkDataArray::CreateDataArray(arrayType));
    gradients->SetNumberOfComponents(3 * numberOfInputComponents);
    gradients->SetNumberOfTuples(array->GetNumberOfTuples());
    switch (arrayType)
    {
      svtkFloatingPointTemplateMacro(
        Fill(gradients, static_cast<SVTK_TT>(0), this->ReplacementValueOption));
    }
  ...
  svtkSmartPointer<svtkDataArray> divergence = nullptr;
  if (computeDivergence)
  {
    divergence.TakeReference(svtkDataArray::CreateDataArray(arrayType));
    divergence->SetNumberOfTuples(array->GetNumberOfTuples());
    switch (arrayType)
    {
      svtkFloatingPointTemplateMacro(
        Fill(divergence, static_cast<SVTK_TT>(0), this->ReplacementValueOption));
    }
  ...
  svtkSmartPointer<svtkDataArray> vorticity;
  if (computeVorticity)
  {
    vorticity.TakeReference(svtkDataArray::CreateDataArray(arrayType));
    vorticity->SetNumberOfComponents(3);
    vorticity->SetNumberOfTuples(array->GetNumberOfTuples());
    switch (arrayType)
    {
      svtkFloatingPointTemplateMacro(
        Fill(vorticity, static_cast<SVTK_TT>(0), this->ReplacementValueOption));
    }
  ...
  svtkSmartPointer<svtkDataArray> qCriterion;
  if (computeQCriterion)
  {
    qCriterion.TakeReference(svtkDataArray::CreateDataArray(arrayType));
    qCriterion->SetNumberOfTuples(array->GetNumberOfTuples());
    switch (arrayType)
    {
      svtkFloatingPointTemplateMacro(
        Fill(qCriterion, static_cast<SVTK_TT>(0), this->ReplacementValueOption));
    }
  ...
  if (this->ContributingCellOption == svtkGradientFilter::DataSetMax)
  {
    int maxDimension = input->IsA("svtkPolyData") == 1 ? 2 : 3;
    for (svtkIdType i = 0; i < input->GetNumberOfCells(); i++)
    {
  ...
  if (fieldAssociation == svtkDataObject::FIELD_ASSOCIATION_POINTS)
  {
    if (!this->FasterApproximation)
    {
      ...
      { // ok to use template macro here since we made the output arrays ourselves
        svtkFloatingPointTemplateMacro(ComputePointGradientsUG(input, array,
          (gradients == nullptr ? nullptr : static_cast<SVTK_TT*>(gradients->GetVoidPointer(0))),
          numberOfInputComponents,
          (vorticity == nullptr ? nullptr : static_cast<SVTK_TT*>(vorticity->GetVoidPointer(0))),
          (qCriterion == nullptr ? nullptr : static_cast<SVTK_TT*>(qCriterion->GetVoidPointer(0))),
          (divergence == nullptr ? nullptr : static_cast<SVTK_TT*>(divergence->GetVoidPointer(0))),
    ...
    else // this->FasterApproximation
    {
      ...
      svtkSmartPointer<svtkDataArray> cellGradients = nullptr;
      if (gradients)
      {
        cellGradients.TakeReference(svtkDataArray::CreateDataArray(arrayType));
      ...
      svtkSmartPointer<svtkDataArray> cellDivergence = nullptr;
      if (divergence)
      {
        cellDivergence.TakeReference(svtkDataArray::CreateDataArray(arrayType));
        cellDivergence->SetName(divergence->GetName());
        cellDivergence->SetNumberOfTuples(input->GetNumberOfCells());
      }
      svtkSmartPointer<svtkDataArray> cellVorticity = nullptr;
      if (vorticity)
      {
        cellVorticity.TakeReference(svtkDataArray::CreateDataArray(arrayType));
      ...
      svtkSmartPointer<svtkDataArray> cellQCriterion = nullptr;
      if (qCriterion)
      {
        cellQCriterion.TakeReference(svtkDataArray::CreateDataArray(arrayType));
      ...
      switch (arrayType)
      { // ok to use template macro here since we made the output arrays ourselves
        svtkFloatingPointTemplateMacro(ComputeCellGradientsUG(input, array,
          (cellGradients == nullptr ? nullptr
                                    : static_cast<SVTK_TT*>(cellGradients->GetVoidPointer(0))),
          numberOfInputComponents,
          (vorticity == nullptr ? nullptr : static_cast<SVTK_TT*>(cellVorticity->GetVoidPointer(0))),
          (qCriterion == nullptr ? nullptr
                                 : static_cast<SVTK_TT*>(cellQCriterion->GetVoidPointer(0))),
          (divergence == nullptr ? nullptr
                                 : static_cast<SVTK_TT*>(cellDivergence->GetVoidPointer(0)))));
      }

      // We need to convert cell Array to points Array.
      svtkSmartPointer<svtkDataSet> dummy;
      ...
      svtkNew<svtkCellDataToPointData> cd2pd;
  ...
  else // fieldAssociation == svtkDataObject::FIELD_ASSOCIATION_CELLS
  {
    // We need to convert cell Array to points Array.
    svtkSmartPointer<svtkDataSet> dummy;
    dummy.TakeReference(input->NewInstance());
    ...
    cd2pd->SetContributingCellOption(this->ContributingCellOption);
    cd2pd->Update();
    svtkDataArray* pointScalars = cd2pd->GetOutput()->GetPointData()->GetScalars();
    pointScalars->Register(this);

    switch (arrayType)
    { // ok to use template macro here since we made the output arrays ourselves
      svtkFloatingPointTemplateMacro(ComputeCellGradientsUG(input, pointScalars,
        (gradients == nullptr ? nullptr : static_cast<SVTK_TT*>(gradients->GetVoidPointer(0))),
        numberOfInputComponents,
        (vorticity == nullptr ? nullptr : static_cast<SVTK_TT*>(vorticity->GetVoidPointer(0))),
        (qCriterion == nullptr ? nullptr : static_cast<SVTK_TT*>(qCriterion->GetVoidPointer(0))),
...
//-----------------------------------------------------------------------------
int svtkGradientFilter::ComputeRegularGridGradient(svtkDataArray* array, int fieldAssociation,
  bool computeVorticity, bool computeQCriterion, bool computeDivergence, svtkDataSet* output)
{
  ...
  if (svtkStructuredGrid* structuredGrid = svtkStructuredGrid::SafeDownCast(output))
  {
    switch (arrayType)
    { // ok to use template macro here since we made the output arrays ourselves
      svtkFloatingPointTemplateMacro(ComputeGradientsSG(structuredGrid, array,
        (gradients == nullptr ? nullptr : static_cast<SVTK_TT*>(gradients->GetVoidPointer(0))),
  ...
  else if (svtkImageData* imageData = svtkImageData::SafeDownCast(output))
  {
    switch (arrayType)
    { // ok to use template macro here since we made the output arrays ourselves
      svtkFloatingPointTemplateMacro(ComputeGradientsSG(imageData, array,
        (gradients == nullptr ? nullptr : static_cast<SVTK_TT*>(gradients->GetVoidPointer(0))),
        numberOfInputComponents, fieldAssociation,
        (vorticity == nullptr ? nullptr : static_cast<SVTK_TT*>(vorticity->GetVoidPointer(0))),
        (qCriterion == nullptr ? nullptr : static_cast<SVTK_TT*>(qCriterion->GetVoidPointer(0))),
  ...
  else if (svtkRectilinearGrid* rectilinearGrid = svtkRectilinearGrid::SafeDownCast(output))
  {
    switch (arrayType)
    { // ok to use template macro here since we made the output arrays ourselves
      svtkFloatingPointTemplateMacro(ComputeGradientsSG(rectilinearGrid, array,
        (gradients == nullptr ? nullptr : static_cast<SVTK_TT*>(gradients->GetVoidPointer(0))),
        numberOfInputComponents, fieldAssociation,
        (vorticity == nullptr ? nullptr : static_cast<SVTK_TT*>(vorticity->GetVoidPointer(0))),
        (qCriterion == nullptr ? nullptr : static_cast<SVTK_TT*>(qCriterion->GetVoidPointer(0))),
  ...
  else if (fieldAssociation == svtkDataObject::FIELD_ASSOCIATION_CELLS)
  {
    if (gradients)
    {
      output->GetCellData()->AddArray(gradients);
  ...
  {
    svtkErrorMacro("Bad fieldAssociation value " << fieldAssociation << endl);
  }
...
//-----------------------------------------------------------------------------
int svtkGradientFilter::GetOutputArrayType(svtkDataArray* array)
{
  int retType = SVTK_DOUBLE;
  switch (array->GetDataType())
  {
    svtkTemplateMacro(retType = GetOutputDataType(static_cast<SVTK_TT>(0)));
  }
  return retType;
}
...
void ComputePointGradientsUG(svtkDataSet* structure, svtkDataArray* array, data_type* gradients,
  ...
  int highestCellDimension, int contributingCellOption)
{
  svtkNew<svtkIdList> currentPoint;
  currentPoint->SetNumberOfIds(1);
  svtkNew<svtkIdList> cellsOnPoint;

  svtkIdType numpts = structure->GetNumberOfPoints();
  ...
  const int maxCellDimension = structure->IsA("svtkPolyData") ? 2 : 3;

  for (svtkIdType point = 0; point < numpts; point++)
  {
    ...
    svtkIdType numCellNeighbors = cellsOnPoint->GetNumberOfIds();
    ...
    if (contributingCellOption == svtkGradientFilter::Patch)
    {
      highestCellDimension = 0;
      for (svtkIdType neighbor = 0; neighbor < numCellNeighbors; neighbor++)
      {
    ...
    svtkIdType numValidCellNeighbors = 0;

    // Iterate on all cells and find all points connected to current point
    // by an edge.
    for (svtkIdType neighbor = 0; neighbor < numCellNeighbors; neighbor++)
    {
      svtkCell* cell = structure->GetCell(cellsOnPoint->GetId(neighbor));
