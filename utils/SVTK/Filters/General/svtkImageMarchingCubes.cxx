/*=========================================================================
  ...
  Module:    svtkImageMarchingCubes.cxx
...
svtkStandardNewMacro(svtkImageMarchingCubes);

//----------------------------------------------------------------------------
// Description:
// Construct object with initial range (0,1) and single contour value
// of 0.0. ComputeNormal is on, ComputeGradients is off and ComputeScalars is on.
svtkImageMarchingCubes::svtkImageMarchingCubes()
{
...
svtkMTimeType svtkImageMarchingCubes::GetMTime()
{
...
//----------------------------------------------------------------------------
template <class T>
int svtkImageMarchingCubesGetTypeSize(T*)
{
  return sizeof(T);
}

//----------------------------------------------------------------------------
int svtkImageMarchingCubes::RequestUpdateExtent(svtkInformation* svtkNotUsed(request),
...
//----------------------------------------------------------------------------
int svtkImageMarchingCubes::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  svtkDemandDrivenPipeline* inputExec =
    svtkDemandDrivenPipeline::SafeDownCast(svtkExecutive::PRODUCER()->GetExecutive(inInfo));

  svtkIdType numContours = this->ContourValues->GetNumberOfContours();
  double* values = this->ContourValues->GetValues();

  svtkDebugMacro("Starting Execute Method");
  ...
  {
    svtkTemplateMacro(temp = svtkImageMarchingCubesGetTypeSize(static_cast<SVTK_TT*>(nullptr)));
    default:
      svtkErrorMacro(<< "Could not determine input scalar type.");
      return 1;
  }
  ...
  {
    svtkWarningMacro("Execute: Need " << minSlicesPerChunk * (temp / 1024) << " KB to load "
                                     << minSlicesPerChunk << " slices.\n");
    this->NumberOfSlicesPerChunk = minSlicesPerChunk;
  }
  svtkDebugMacro("Execute: NumberOfSlicesPerChunk = " << this->NumberOfSlicesPerChunk);
  ...
  svtkIdType estimatedSize = static_cast<svtkIdType>(extent[1] - extent[0] + 1);
  estimatedSize *= static_cast<svtkIdType>(extent[3] - extent[2] + 1);
  estimatedSize *= static_cast<svtkIdType>(extent[5] - extent[4] + 1);
  estimatedSize = static_cast<svtkIdType>(pow(1.0 * estimatedSize, 0.75));
  estimatedSize = (estimatedSize / 1024) * 1024; // multiple of 1024
  if (estimatedSize < 1024)
  {
    estimatedSize = 1024;
  }
  svtkDebugMacro(<< "Estimated number of points/triangles: " << estimatedSize);
  this->Points = svtkPoints::New();
  this->Points->Allocate(estimatedSize, estimatedSize / 2);
  this->Triangles = svtkCellArray::New();
  this->Triangles->AllocateEstimate(estimatedSize, 1);
  if (this->ComputeScalars)
  {
    this->Scalars = svtkFloatArray::New();
  ...
  {
    this->Gradients = svtkFloatArray::New();
  ...
  for (int chunkMin = zMin, chunkMax; chunkMin < zMax; chunkMin = chunkMax)
  {
    ...
    if (svtkDataObject::GetGlobalReleaseDataFlag() ||
      inInfo->Has(svtkStreamingDemandDrivenPipeline::RELEASE_DATA()))
    {
      inData->ReleaseData();
    }
  }
  ...
  svtkDebugMacro(<< "Created: " << this->Points->GetNumberOfPoints() << " points, "
  ...
  svtkImageTransform::TransformPointSet(inData, output);
...
template <class T>
void svtkImageMarchingCubesComputePointGradient(
...
template <class T>
int svtkImageMarchingCubesMakeNewPoint(svtkImageMarchingCubes* self, int idx0, int idx1, int idx2,
  int inc0, int inc1, int inc2, T* ptr, int edge, int* imageExtent, double value)
{
  ...
  if (self->NeedGradients)
  {
    ...
    svtkImageMarchingCubesComputePointGradient(ptr, g, inc0, inc1, inc2, b0, b1, b2);
    ...
    svtkImageMarchingCubesComputePointGradient(ptrB, gB, inc0, inc1, inc2, b0, b1, b2);
...
template <class T>
void svtkImageMarchingCubesHandleCube(svtkImageMarchingCubes* self, int cellX, int cellY, int cellZ,
  svtkImageData* inData, T* ptr, int numContours, double* values)
{
      ...
      while (*edge > -1)
      {
        for (ii = 0; ii < 3; ++ii, ++edge) // insert triangle
        {
          ...
          if (pointIds[ii] == -1)
          {
            ...
            pointIds[ii] = svtkImageMarchingCubesMakeNewPoint(
...
//----------------------------------------------------------------------------
template <class T>
void svtkImageMarchingCubesMarch(svtkImageMarchingCubes* self, svtkImageData* inData, T* ptr,
  ...
  for (idx2 = chunkMin; idx2 < chunkMax; ++idx2)
  {
    ...
    for (idx1 = min1; idx1 < max1; ++idx1)
    {
      ...
      for (idx0 = min0; idx0 < max0; ++idx0)
      {
        ...
        svtkImageMarchingCubesHandleCube(self, idx0, idx1, idx2, inData, ptr0, numContours, values);
...
void svtkImageMarchingCubes::March(
  svtkImageData* inData, int chunkMin, int chunkMax, int numContours, double* values)
{
  void* ptr = inData->GetScalarPointer();

  switch (inData->GetScalarType())
  {
    svtkTemplateMacro(svtkImageMarchingCubesMarch(
      this, inData, static_cast<SVTK_TT*>(ptr), chunkMin, chunkMax, numContours, values));
    default:
      svtkErrorMacro(<< "Unknown output ScalarType");
      return;
  }
}
...
// One 2d array of cubes is stored. (z dimension is ignored).
void svtkImageMarchingCubes::InitializeLocator(int min0, int max0, int min1, int max1)
{
  // Free old memory
  ...
  // 5 non shared edges.
  svtkIdType size = 5;
  size *= static_cast<svtkIdType>(this->LocatorDimX);
  size *= static_cast<svtkIdType>(this->LocatorDimY);
  this->LocatorPointIds = new svtkIdType[size];
...
void svtkImageMarchingCubes::DeleteLocator()
{
  // Free old memory
  delete[] this->LocatorPointIds;
  this->LocatorPointIds = nullptr;
...
void svtkImageMarchingCubes::IncrementLocatorZ()
{
  svtkIdType* ptr = this->LocatorPointIds;
...
void svtkImageMarchingCubes::AddLocatorPoint(int cellX, int cellY, int edge, svtkIdType ptId)
{
  // Get the correct position in the array.
  svtkIdType* ptr = this->GetLocatorPointer(cellX, cellY, edge);
  *ptr = ptId;
}

//----------------------------------------------------------------------------
// This method gets a point from the locator.
svtkIdType svtkImageMarchingCubes::GetLocatorPoint(int cellX, int cellY, int edge)
{
  // Get the correct position in the array.
  svtkIdType* ptr = this->GetLocatorPointer(cellX, cellY, edge);
  return *ptr;
...
svtkIdType* svtkImageMarchingCubes::GetLocatorPointer(int cellX, int cellY, int edge)
{
  ...
  return this->LocatorPointIds + edge +
    (cellX + cellY * static_cast<svtkIdType>(this->LocatorDimX)) * 5;
}

//----------------------------------------------------------------------------
int svtkImageMarchingCubes::FillInputPortInformation(int, svtkInformation* info)
{
...
//----------------------------------------------------------------------------
void svtkImageMarchingCubes::PrintSelf(ostream& os, svtkIndent indent)
{
