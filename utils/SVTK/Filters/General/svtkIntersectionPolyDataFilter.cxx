/*=========================================================================
  ...
  Module:    svtkIntersectionPolyDataFilter.cxx
...
typedef std::multimap<svtkIdType, svtkIdType> IntersectionMapType;
...
typedef std::multimap<svtkIdType, CellEdgeLineType> PointEdgeMapType;
typedef PointEdgeMapType::iterator PointEdgeMapIteratorType;

//----------------------------------------------------------------------------
// Private implementation to hide STL.
//----------------------------------------------------------------------------
class svtkIntersectionPolyDataFilter::Impl
{
public:
  Impl();
  virtual ~Impl();
  ...
  static int FindTriangleIntersections(
    svtkOBBNode* node0, svtkOBBNode* node1, svtkMatrix4x4* transform, void* arg);

  // Runs the split mesh for the designated input surface
  int SplitMesh(int inputIndex, svtkPolyData* output, svtkPolyData* intersectionLines);

protected:
  // Split cells into polygons created by intersection lines
  svtkCellArray* SplitCell(svtkPolyData* input, svtkIdType cellId, const svtkIdType* cellPts,
    IntersectionMapType* map, svtkPolyData* interLines, int inputIndex, int numCurrCells);

  // Function to add point to check edge list for remeshing step
  int AddToPointEdgeMap(int index, svtkIdType ptId, double x[3], svtkPolyData* mesh, svtkIdType cellId,
    svtkIdType edgeId, svtkIdType lineId, const svtkIdType triPts[3]);

  // Function to add information about the new cell data
  void AddToNewCellMap(
    int inputIndex, int interPtCount, int interPts[3], svtkPolyData* interLines, int numCurrCells);

  // Function inside SplitCell to get the smaller triangle loops
  int GetLoops(svtkPolyData* pd, std::vector<simPolygon>* loops);

  // Get individual polygon loop of splitting cell
  int GetSingleLoop(svtkPolyData* pd, simPolygon* loop, svtkIdType nextCell,
    std::vector<bool>& interPtBool, std::vector<bool>& lineBool);

  // Follow a loop orientation to iterate around a split polygon
  int FollowLoopOrientation(svtkPolyData* pd, simPolygon* loop, svtkIdType* nextCell,
    svtkIdType nextPt, svtkIdType prevPt, svtkIdList* pointCells);

  // Set the loop orientation based on CW CCW geometric test
  void SetLoopOrientation(svtkPolyData* pd, simPolygon* loop, svtkIdType* nextCell, svtkIdType nextPt,
    svtkIdType prevPt, svtkIdList* pointCells);

  // Get the loop orientation is already given
  int GetLoopOrientation(svtkPolyData* pd, svtkIdType cell, svtkIdType ptId1, svtkIdType ptId2);

  // Orient the triangle based on the transform for remeshing
  void Orient(
    svtkPolyData* pd, svtkTransform* transform, svtkPolyData* boundary, svtkPolygon* boundarypoly);

  // Checks to make sure multiple lines are not added to the same triangle
  // that needs to re-triangulated
  int CheckLine(svtkPolyData* pd, svtkIdType ptId1, svtkIdType ptId2);

  // Gets a transform to the XY plane for three points comprising a triangle
  int GetTransform(svtkTransform* transform, svtkPoints* points);

public:
  svtkPolyData* Mesh[2];
  svtkOBBTree* OBBTree1;

  // Stores the intersection lines.
  svtkCellArray* IntersectionLines;

  svtkIdTypeArray* SurfaceId;
  svtkIdTypeArray* NewCellIds[2];

  // Cell data that indicates in which cell each intersection
  // lies. One array for each output surface.
  svtkIdTypeArray* CellIds[2];
  ...
  svtkIdTypeArray* PointCellIds[2];
  svtkIntArray* BoundaryPoints[2];

  // Merging filter used to convert intersection lines from "line
  // soup" to connected polylines.
  svtkPointLocator* PointMerger;
  ...
  // svtkPolyData to hold current splitting cell. Used to double check area
  // of small area cells
  svtkPolyData* SplittingPD;
  int TransformSign;
  double Tolerance;
  double RelativeSubtriangleArea;

  // Pointer to overarching filter
  svtkIntersectionPolyDataFilter* ParentFilter;
...
//----------------------------------------------------------------------------
svtkIntersectionPolyDataFilter::Impl::Impl()
  : OBBTree1(nullptr)
  ...
  , PointMerger(nullptr)
{
  ...
  this->SplittingPD = svtkPolyData::New();
...
//----------------------------------------------------------------------------
int svtkIntersectionPolyDataFilter::Impl ::FindTriangleIntersections(
  svtkOBBNode* node0, svtkOBBNode* node1, svtkMatrix4x4* transform, void* arg)
{
  svtkIntersectionPolyDataFilter::Impl* info =
    reinterpret_cast<svtkIntersectionPolyDataFilter::Impl*>(arg);

  // Set up local structures to hold Impl array information
  svtkPolyData* mesh0 = info->Mesh[0];
  svtkPolyData* mesh1 = info->Mesh[1];
  svtkOBBTree* obbTree1 = info->OBBTree1;
  svtkCellArray* intersectionLines = info->IntersectionLines;
  svtkIdTypeArray* intersectionSurfaceId = info->SurfaceId;
  svtkIdTypeArray* intersectionCellIds0 = info->CellIds[0];
  svtkIdTypeArray* intersectionCellIds1 = info->CellIds[1];
  svtkPointLocator* pointMerger = info->PointMerger;
  double tolerance = info->Tolerance;

  // The number of cells in OBBTree
  int numCells0 = node0->Cells->GetNumberOfIds();

  for (svtkIdType id0 = 0; id0 < numCells0; id0++)
  {
    svtkIdType cellId0 = node0->Cells->GetId(id0);
    int type0 = mesh0->GetCellType(cellId0);

    // Make sure the cell is a triangle
    if (type0 == SVTK_TRIANGLE)
    {
      svtkIdType npts0;
      const svtkIdType* triPtIds0;
      mesh0->GetCellPoints(cellId0, npts0, triPtIds0);
      double triPts0[3][3];
      for (svtkIdType id = 0; id < npts0; id++)
      {
      ...
      if (obbTree1->TriangleIntersectsNode(node1, triPts0[0], triPts0[1], triPts0[2], transform))
      {
        ...
        for (svtkIdType id1 = 0; id1 < numCells1; id1++)
        {
          ...
          if (type1 == SVTK_TRIANGLE)
          {
            ...
            double surfaceid[2];
            int intersects = svtkIntersectionPolyDataFilter::TriangleTriangleIntersection(triPts0[0],
            ...
            if (intersects)
            {
              svtkIdType lineId = intersectionLines->GetNumberOfCells();

              svtkIdType ptId0, ptId1;
              int unique[2];
              ...
              {
                svtkSmartPointer<svtkPolyData> lineTest = svtkSmartPointer<svtkPolyData>::New();
              ...
              if (addline)
              {
                ...
                for (svtkIdType edgeId = 0; edgeId < 3; edgeId++)
                {
...
//----------------------------------------------------------------------------
int svtkIntersectionPolyDataFilter::Impl ::SplitMesh(
  int inputIndex, svtkPolyData* output, svtkPolyData* intersectionLines)
{
  svtkPolyData* input = this->Mesh[inputIndex];
  ...
  // Process points
  //
  svtkIdType inputNumPoints = input->GetPoints()->GetNumberOfPoints();
  ...
  // using a svtkPointLocator. However, some lines may have an endpoint
  ...
  //
  svtkSmartPointer<svtkPolyData> splitLines = svtkSmartPointer<svtkPolyData>::New();
  ...
  for (svtkIdType ptId = 0; ptId < inputNumPoints; ptId++)
  {
    double pt[3];
  ...
  // data as we go.
  for (svtkIdType id = 0; id < splitLines->GetNumberOfPoints(); id++)
  {
    double pt[3];
    splitLines->GetPoint(id, pt);
    svtkIdType newPtId = output->GetPoints()->InsertNextPoint(pt);

    // Retrieve the cell ID from splitLines
    svtkIdType cellId = this->PointCellIds[inputIndex]->GetValue(id);
  ...
  if (input->GetPolys()->GetNumberOfCells() > 0)
  {
    ...
    svtkIdType newId = output->GetNumberOfCells();
    ...
    svtkSmartPointer<svtkIdList> edgeNeighbors = svtkSmartPointer<svtkIdList>::New();
    svtkIdType nptsX = 0;
    const svtkIdType* pts = nullptr;
    svtkSmartPointer<svtkIdList> cellsToCheck = svtkSmartPointer<svtkIdList>::New();
    for (cells->InitTraversal(); cells->GetNextCell(nptsX, pts); cellIdX++)
    {
      if (nptsX != 3)
      {
        svtkGenericWarningMacro(<< "svtkIntersectionPolyDataFilter only works"
      ...
      for (svtkIdType ptId = 0; ptId < nptsX; ptId++)
      {
        svtkIdType pt0Id = pts[ptId];
        svtkIdType pt1Id = pts[(ptId + 1) % nptsX];
        edgeNeighbors->Reset();
        input->GetCellEdgeNeighbors(cellIdX, pt0Id, pt1Id, edgeNeighbors);
        for (svtkIdType nbr = 0; nbr < edgeNeighbors->GetNumberOfIds(); nbr++)
        {
          svtkIdType nbrId = edgeNeighbors->GetId(nbr);
      ...
      else
      {
        ...
        svtkCellArray* splitCells = this->SplitCell(
          input, cellIdX, pts, intersectionMap, splitLines, inputIndex, numCurrCells);
        if (splitCells == nullptr)
        {
          svtkDebugWithObjectMacro(this->ParentFilter, << "Error in splitting cell!");
          return 0;
        }
        ...
        for (subCellId = 0; splitCells->GetNextCell(npts, ptIds); subCellId++)
        {
          ...
          svtkTriangle::ComputeNormal(pt0, pt1, pt2, subCellNormal);
          svtkMath::Normalize(subCellNormal);

          if (svtkMath::Dot(normal, subCellNormal) > 0)
          {
...
svtkCellArray* svtkIntersectionPolyDataFilter::Impl ::SplitCell(svtkPolyData* input, svtkIdType cellId,
  const svtkIdType* cellPts, IntersectionMapType* map, svtkPolyData* interLines, int inputIndex,
  int numCurrCells)
{
  // Copy down the SurfaceID array that tells which surface the point belongs
  // to
  svtkIdTypeArray* surfaceMapper;
  surfaceMapper = svtkIdTypeArray::SafeDownCast(interLines->GetPointData()->GetArray("SurfaceID"));

  // Array to keep track of which points are on the boundary of the cell
  svtkSmartPointer<svtkIdTypeArray> cellBoundaryPt = svtkSmartPointer<svtkIdTypeArray>::New();
  ...
  svtkSmartPointer<svtkPoints> points = svtkSmartPointer<svtkPoints>::New();
  svtkSmartPointer<svtkPointLocator> merger = svtkSmartPointer<svtkPointLocator>::New();
  ...
  for (int i = 0; i < 3; i++)
  {
    ...
    {
      svtkGenericWarningMacro(<< "invalid point read 1");
    }
  ...
  svtkSmartPointer<svtkCellArray> interceptlines = svtkSmartPointer<svtkCellArray>::New();
  ...
  // This maps the point IDs for the svtkPolyData passed to
  // svtkDelaunay2D back to the original IDs in interLines. NOTE: The
  // point IDs from the cell are not stored here.
  std::map<svtkIdType, svtkIdType> ptIdMap;
  ...
  {
    svtkIdType lineId = iterLower->second;
    svtkIdType nLinePts;
    const svtkIdType* linePtIds;
    ...
    for (svtkIdType i = 0; i < nLinePts; i++)
    {
      std::map<svtkIdType, svtkIdType>::iterator location = ptIdMap.find(linePtIds[i]);
      ...
      if (location == ptIdMap.end())
      {
        ...
        {
          svtkGenericWarningMacro(<< "invalid point read 2");
        }
  ...
  svtkSmartPointer<svtkIdList> nbrCellIds = svtkSmartPointer<svtkIdList>::New();
  for (svtkIdType i = 0; i < 3; i++)
  {
    // Get Points belonging to each edge of this cell
    svtkIdType edgePtId0 = cellPts[i];
    svtkIdType edgePtId1 = cellPts[(i + 1) % 3];

    double edgePt0[3], edgePt1[3];
    if (edgePtId0 >= input->GetNumberOfPoints())
    {
      svtkGenericWarningMacro(<< "invalid point read 3");
    }
    if (edgePtId1 >= input->GetNumberOfPoints())
    {
      svtkGenericWarningMacro(<< "invalid point read 4");
    }
    ...
    for (svtkIdType j = 0; j < nbrCellIds->GetNumberOfIds(); j++)
    {
      svtkIdType nbrCellId = nbrCellIds->GetId(j);
      ...
      while (iterLower != iterUpper)
      {
        ...
        const svtkIdType* linePtIds;
        interLines->GetLines()->GetCellAtId(lineId, nLinePts, linePtIds);
        for (svtkIdType k = 0; k < nLinePts; k++)
        {
          if (linePtIds[k] >= interLines->GetNumberOfPoints())
          {
            svtkGenericWarningMacro(<< "invalid point read 5");
          }
          ...
          {
            svtkIdType mappedPtId = ptIterLower->second;
            ...
            {
              svtkIdType checkCellId = cellIterLower->second;
  ...
  std::map<svtkIdType, svtkIdType> reverseIdMap;
  std::map<svtkIdType, svtkIdType> reverseLineIdMap;
  std::map<svtkIdType, svtkIdType>::iterator iter = ptIdMap.begin();
  ...
  svtkTriangle::TriangleCenter(p0, p1, p2, c);
  svtkTriangle::ComputeNormal(p0, p1, p2, n);
  svtkMath::Perpendiculars(n, v0, v1, 0.0);

  // For each point on an edge, compute it's relative angle about n.
  svtkSmartPointer<svtkIdTypeArray> edgePtIdList = svtkSmartPointer<svtkIdTypeArray>::New();
  svtkSmartPointer<svtkIdTypeArray> interPtIdList = svtkSmartPointer<svtkIdTypeArray>::New();
  edgePtIdList->Allocate(points->GetNumberOfPoints());
  svtkSmartPointer<svtkDoubleArray> angleList = svtkSmartPointer<svtkDoubleArray>::New();
  angleList->Allocate(points->GetNumberOfPoints());
  bool* interPtBool = new bool[points->GetNumberOfPoints()];

  for (svtkIdType ptId = 0; ptId < points->GetNumberOfPoints(); ptId++)
  {
    double x[3];
    ...
    if (cellBoundaryPt->GetValue(ptId))
    {
      ...
      double d[3];
      svtkMath::Subtract(x, c, d);
      angleList->InsertNextValue(atan2(svtkMath::Dot(d, v0), svtkMath::Dot(d, v1)));
      if (ptId > 2)
      {
        // Intersection Point!
  ...
  // a consistent order.
  svtkSortDataArray::Sort(angleList, edgePtIdList);

  svtkSmartPointer<svtkPolyData> checkPD = svtkSmartPointer<svtkPolyData>::New();
  ...
  svtkCellArray* splitCells = svtkCellArray::New();
  svtkSmartPointer<svtkPolyData> interpd = svtkSmartPointer<svtkPolyData>::New();
  interpd->SetPoints(points);
  interpd->SetLines(interceptlines);
  interpd->BuildLinks();

  svtkSmartPointer<svtkPolyData> fullpd = svtkSmartPointer<svtkPolyData>::New();
  ...
  svtkSmartPointer<svtkPolyData> transformedpd = svtkSmartPointer<svtkPolyData>::New();
  ...
  if (interPtIdList->GetNumberOfTuples() > 0 && interceptlines->GetNumberOfCells() > 0)
  {
    ...
    {
      svtkCellArray* polys;
      svtkSmartPointer<svtkPolyData> newpd = svtkSmartPointer<svtkPolyData>::New();
      ...
      svtkSmartPointer<svtkPolyData> boundary = svtkSmartPointer<svtkPolyData>::New();
      svtkSmartPointer<svtkPolygon> boundaryPoly = svtkSmartPointer<svtkPolygon>::New();
      ...
      svtkSmartPointer<svtkDelaunay2D> del2D = svtkSmartPointer<svtkDelaunay2D>::New();
      svtkSmartPointer<svtkTriangleFilter> triangulator = svtkSmartPointer<svtkTriangleFilter>::New();

      svtkSmartPointer<svtkCellArray> triangulatedPolyCells = svtkSmartPointer<svtkCellArray>::New();
      if (success)
      {
        for (svtkIdType i = 0; i < idList->GetNumberOfIds(); i += 3)
        {
          svtkIdType t[3] = { idList->GetId(i), idList->GetId(i + 1), idList->GetId(i + 2) };
      ...
      else
      {
        ...
        if (polys->GetNumberOfCells() != newpd->GetNumberOfPoints() - 2)
        {
          ...
          {
            svtkSmartPointer<svtkDelaunay2D> del2Doffset = svtkSmartPointer<svtkDelaunay2D>::New();
            ...
            del2Doffset->SetProjectionPlaneMode(SVTK_SET_TRANSFORM_PLANE);
      ...
      for (polys->InitTraversal(); polys->GetNextCell(npts, ptIds);)
      {
        ...
        {
          svtkGenericWarningMacro(<< "Invalid point ID!!!");
        }
  ...
  else // Not (intersection lines and new points)
  {
    ...
    svtkCellArray* polys = del2D->GetOutput()->GetPolys();
...
//----------------------------------------------------------------------------
int svtkIntersectionPolyDataFilter::Impl ::AddToPointEdgeMap(int index, svtkIdType ptId, double x[3],
  svtkPolyData* mesh, svtkIdType cellId, svtkIdType edgeId, svtkIdType lineId,
  const svtkIdType triPtIds[3])
{
  int value = -1;
  svtkIdType edgePtId0 = triPtIds[edgeId];
  svtkIdType edgePtId1 = triPtIds[(edgeId + 1) % 3];
  double pt0[3], pt1[3];
  ...
  dist = svtkLine::DistanceToLine(x, pt0, pt1, t, closestPt);
...
void svtkIntersectionPolyDataFilter::Impl::AddToNewCellMap(
  int inputIndex, int interPtCount, int interPts[3], svtkPolyData* interLines, int numCurrCells)
{
  svtkIdList** cellIds;
  cellIds = new svtkIdList*[interPtCount];
  for (int i = 0; i < interPtCount; i++)
  {
    cellIds[i] = svtkIdList::New();
    svtkSmartPointer<svtkIdList> temp = svtkSmartPointer<svtkIdList>::New();
...
int svtkIntersectionPolyDataFilter::Impl ::GetLoops(svtkPolyData* pd, std::vector<simPolygon>* loops)
{
  svtkSmartPointer<svtkIdList> pointCells = svtkSmartPointer<svtkIdList>::New();
  svtkSmartPointer<svtkIdList> cellPoints = svtkSmartPointer<svtkIdList>::New();
  simPoint nextPt;
  svtkIdType nextCell;
  ...
  for (svtkIdType lineId = 0; lineId < pd->GetNumberOfCells(); lineId++)
  {
    if (lineBool[lineId] == false)
    {
      svtkDebugWithObjectMacro(this->ParentFilter, << "LINE FALSE: Find extra loop/s");
...
int svtkIntersectionPolyDataFilter::Impl ::GetSingleLoop(svtkPolyData* pd, simPolygon* loop,
  svtkIdType nextCell, std::vector<bool>& interPtBool, std::vector<bool>& lineBool)
{
  int intertype = 0;
  svtkSmartPointer<svtkIdList> pointCells = svtkSmartPointer<svtkIdList>::New();
  svtkSmartPointer<svtkIdList> cellPoints = svtkSmartPointer<svtkIdList>::New();
  ...
  svtkIdType nextPt = loop->points.front().id;
  svtkIdType startPt = nextPt;
  interPtBool[nextPt] = true;
  pd->GetCellPoints(nextCell, cellPoints);

  simPoint newpoint;
  svtkIdType prevPt = nextPt;
  ...
  while (nextPt != startPt)
  {
    ...
    {
      svtkSmartPointer<svtkPolyData> currentpd = svtkSmartPointer<svtkPolyData>::New();
      svtkSmartPointer<svtkCellArray> currentcells = svtkSmartPointer<svtkCellArray>::New();
...
int svtkIntersectionPolyDataFilter::Impl ::FollowLoopOrientation(svtkPolyData* pd, simPolygon* loop,
  svtkIdType* nextCell, svtkIdType nextPt, svtkIdType prevPt, svtkIdList* pointCells)
{
  // Follow the orientation of this loop
  int foundcell = 0;
  double newcell = 0;
  double minangle = SVTK_DOUBLE_MAX;
  for (svtkIdType i = 0; i < pointCells->GetNumberOfIds(); i++)
  {
    svtkIdType cellId = pointCells->GetId(i);
    if (*nextCell != cellId)
    {
      ...
      if (neworient == loop->orientation)
      {
        ...
        svtkSmartPointer<svtkIdList> specialCellPoints = svtkSmartPointer<svtkIdList>::New();
        ...
        edge1[2] = 0.;
        edge2[2] = 0.;
        svtkMath::Normalize(edge1);
        svtkMath::Normalize(edge2);
        double dot = svtkMath::Dot(edge1, edge2);
        if (dot > 1.0)
        {
          dot = 1.0;
        }
        ...
        double angle = svtkMath::DegreesFromRadians(acos(dot));
        if (angle < minangle)
        {
          minangle = angle;
          newcell = cellId;
  ...
  if (foundcell == 0)
  {
    svtkWarningWithObjectMacro(this->ParentFilter, << "No cell with correct orientation found");
    return 0;
  }
...
void svtkIntersectionPolyDataFilter::Impl ::SetLoopOrientation(svtkPolyData* pd, simPolygon* loop,
  svtkIdType* nextCell, svtkIdType nextPt, svtkIdType prevPt, svtkIdList* pointCells)
{
...
int svtkIntersectionPolyDataFilter::Impl::GetLoopOrientation(
  svtkPolyData* pd, svtkIdType cell, svtkIdType ptId1, svtkIdType ptId2)
{
  ...
  if (fabs(area) < 1e-10)
  {
    ...
    svtkDebugWithObjectMacro(this->ParentFilter, << "Very Small Area Triangle");
    svtkDebugWithObjectMacro(
      this->ParentFilter, << "Double check area with more accurate transform");
    svtkSmartPointer<svtkPoints> testPoints = svtkSmartPointer<svtkPoints>::New();
    svtkSmartPointer<svtkPolyData> testPD = svtkSmartPointer<svtkPolyData>::New();
    svtkSmartPointer<svtkCellArray> testCells = svtkSmartPointer<svtkCellArray>::New();
    ...
    svtkSmartPointer<svtkTransform> newTransform = svtkSmartPointer<svtkTransform>::New();
    ...
    svtkSmartPointer<svtkTransformPolyDataFilter> newTransformer =
      svtkSmartPointer<svtkTransformPolyDataFilter>::New();
    ...
    svtkDebugWithObjectMacro(this->ParentFilter, << "Area was: " << area);
    area = 0;
    area = area + (pt1[0] * pt2[1]) - (pt2[0] * pt1[1]);
    area = area + (pt2[0] * pt3[1]) - (pt3[0] * pt2[1]);
    area = area + (pt3[0] * pt1[1]) - (pt1[0] * pt3[1]);
    svtkDebugWithObjectMacro(this->ParentFilter, << "Corrected area is: " << area);
  }
  if (area < 0)
  {
    orientation = -1;
...
void svtkIntersectionPolyDataFilter::Impl ::Orient(
  svtkPolyData* pd, svtkTransform* transform, svtkPolyData* boundary, svtkPolygon* boundarypoly)
{
...
int svtkIntersectionPolyDataFilter::Impl::CheckLine(
  svtkPolyData* pd, svtkIdType ptId1, svtkIdType ptId2)
{
  svtkSmartPointer<svtkIdList> pointCells1 = svtkSmartPointer<svtkIdList>::New();
  svtkSmartPointer<svtkIdList> pointCells2 = svtkSmartPointer<svtkIdList>::New();
...
int svtkIntersectionPolyDataFilter::Impl::GetTransform(svtkTransform* transform, svtkPoints* points)
{
  ...
  else
  {
    ...
    double dot = svtkMath::Dot(zaxis, normal);
    if (dot > 1.0)
    {
      dot = 1.0;
    }
    ...
    rotationAngle = svtkMath::DegreesFromRadians(acos(dot));
  }
...
//----------------------------------------------------------------------------
svtkStandardNewMacro(svtkIntersectionPolyDataFilter);

//----------------------------------------------------------------------------
svtkIntersectionPolyDataFilter::svtkIntersectionPolyDataFilter()
  : SplitFirstOutput(1)
  , SplitSecondOutput(1)
{
...
//----------------------------------------------------------------------------
svtkIntersectionPolyDataFilter::~svtkIntersectionPolyDataFilter() = default;

//----------------------------------------------------------------------------
void svtkIntersectionPolyDataFilter::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
int svtkIntersectionPolyDataFilter::TriangleTriangleIntersection(double p1[3], double q1[3],
  ...
  double pt2[3], double surfaceid[2], double tolerance)
{
  ...
  svtkTriangle::ComputeNormal(p1, q1, r1, n1);
  svtkTriangle::ComputeNormal(p2, q2, r2, n2);
  double s1 = -svtkMath::Dot(n1, p1);
  double s2 = -svtkMath::Dot(n2, p2);

  // Compute signed distances of points p1, q1, r1 from supporting
  // plane of second triangle.
  double dist1[3];
  dist1[0] = svtkMath::Dot(n2, p1) + s2;
  dist1[1] = svtkMath::Dot(n2, q1) + s2;
  dist1[2] = svtkMath::Dot(n2, r1) + s2;

  // If signs of all points are the same, all the points lie on the
  // same side of the supporting plane, and we can exit early.
  if ((dist1[0] * dist1[1] > tolerance) && (dist1[0] * dist1[2] > tolerance))
  {
    // svtkDebugMacro(<<"Same side supporting plane 1!");
    return 0;
  }
  // Do the same for p2, q2, r2 and supporting plane of first
  // triangle.
  double dist2[3];
  dist2[0] = svtkMath::Dot(n1, p2) + s1;
  dist2[1] = svtkMath::Dot(n1, q2) + s1;
  dist2[2] = svtkMath::Dot(n1, r2) + s1;

  // If signs of all points are the same, all the points lie on the
  // same side of the supporting plane, and we can exit early.
  if ((dist2[0] * dist2[1] > tolerance) && (dist2[0] * dist2[2] > tolerance))
  {
    // svtkDebugMacro(<<"Same side supporting plane 2!");
    return 0;
  }
  ...
  double n1n2 = svtkMath::Dot(n1, n2);
  ...
  for (int i = 0; i < 3; i++)
  {
    ...
    double val1 = svtkPlane::IntersectWithLine(pts1[id1], pts1[id2], n2, p2, t, x);
    if (val1 == 1 || (t > (0 - tolerance) && t < (1 + tolerance)))
    {
      ...
      t1[index1++] = svtkMath::Dot(x, v) - svtkMath::Dot(p, v);
    }

    double val2 = svtkPlane::IntersectWithLine(pts2[id1], pts2[id2], n1, p1, t, x);
    if (val2 == 1 || (t > (0 - tolerance) && t < (1 + tolerance)))
    {
      ...
      t2[index2++] = svtkMath::Dot(x, v) - svtkMath::Dot(p, v);
    }
  }
  ...
  {
    // svtkDebugMacro(<<"Only one edge intersecting!");
    return 0;
  }

  // Check for NaNs
  if (svtkMath::IsNan(t1[0]) || svtkMath::IsNan(t1[1]) || svtkMath::IsNan(t2[0]) ||
    svtkMath::IsNan(t2[1]))
  {
    // svtkWarningMacro(<<"NaNs!");
    return 0;
  }
...
void svtkIntersectionPolyDataFilter::CleanAndCheckSurface(
  svtkPolyData* pd, double stats[2], double tolerance)
{
  int badEdges = 0;
  int freeEdges = 0;
  svtkSmartPointer<svtkCleanPolyData> cleaner = svtkSmartPointer<svtkCleanPolyData>::New();
  svtkSmartPointer<svtkIntArray> bad = svtkSmartPointer<svtkIntArray>::New();
  svtkSmartPointer<svtkIntArray> freeedge = svtkSmartPointer<svtkIntArray>::New();
  svtkSmartPointer<svtkIdList> edgeneighbors = svtkSmartPointer<svtkIdList>::New();
...
void svtkIntersectionPolyDataFilter::CleanAndCheckInput(svtkPolyData* pd, double tolerance)
{
  svtkSmartPointer<svtkCleanPolyData> cleaner = svtkSmartPointer<svtkCleanPolyData>::New();
  svtkSmartPointer<svtkTriangleFilter> triangulator = svtkSmartPointer<svtkTriangleFilter>::New();
  svtkSmartPointer<svtkPolyDataNormals> normaler = svtkSmartPointer<svtkPolyDataNormals>::New();

  // svtkDebugMacro(<<"Cleaning");
  cleaner->SetInputData(pd);
  cleaner->ToleranceIsAbsoluteOn();
  cleaner->SetAbsoluteTolerance(tolerance);
  cleaner->Update();
  // svtkDebugMacro(<<"Triangulating");
  triangulator->SetInputData(cleaner->GetOutput());
  triangulator->Update();
  // svtkDebugMacro(<<"Getting Normals");
  ...
  svtkMath::Cross(v1, v2, cellNorm);

  double arrayNormal[3];
  // svtkDebugMacro(<<"Getting Normal Array");
  normaler->GetOutput()->GetCellData()->GetNormals("Normals")->GetTuple(0, arrayNormal);
}

//----------------------------------------------------------------------------
int svtkIntersectionPolyDataFilter::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  svtkInformation* outIntersectionInfo = outputVector->GetInformationObject(0);
  svtkInformation* outPolyDataInfo0 = outputVector->GetInformationObject(1);
  svtkInformation* outPolyDataInfo1 = outputVector->GetInformationObject(2);
  ...
  {
    svtkDebugMacro(<< "Empty input data");
    return 1;
  }

  if (this->CheckInput)
  {
    svtkDebugMacro(<< "Checking Input 0");
    this->CleanAndCheckInput(input0, this->Tolerance);
    svtkDebugMacro(<< "Checking Input 1");
    this->CleanAndCheckInput(input1, this->Tolerance);
  }

  svtkPolyData* outputIntersection =
    svtkPolyData::SafeDownCast(outIntersectionInfo->Get(svtkDataObject::DATA_OBJECT()));
  svtkSmartPointer<svtkPoints> outputIntersectionPoints = svtkSmartPointer<svtkPoints>::New();
  outputIntersection->SetPoints(outputIntersectionPoints);

  svtkPolyData* outputPolyData0 =
    svtkPolyData::SafeDownCast(outPolyDataInfo0->Get(svtkDataObject::DATA_OBJECT()));

  svtkPolyData* outputPolyData1 =
    svtkPolyData::SafeDownCast(outPolyDataInfo1->Get(svtkDataObject::DATA_OBJECT()));

  // Set up new poly data for the inputs to build cells and links.
  svtkSmartPointer<svtkPolyData> mesh0 = svtkSmartPointer<svtkPolyData>::New();
  mesh0->DeepCopy(input0);

  svtkSmartPointer<svtkPolyData> mesh1 = svtkSmartPointer<svtkPolyData>::New();
  mesh1->DeepCopy(input1);

  // Find the triangle-triangle intersections between mesh0 and mesh1
  svtkSmartPointer<svtkOBBTree> obbTree0 = svtkSmartPointer<svtkOBBTree>::New();
  ...
  svtkSmartPointer<svtkOBBTree> obbTree1 = svtkSmartPointer<svtkOBBTree>::New();
  ...
  // intersections.
  svtkIntersectionPolyDataFilter::Impl* impl = new svtkIntersectionPolyDataFilter::Impl();
  ...
  svtkSmartPointer<svtkPointLocator> pointMerger = svtkSmartPointer<svtkPointLocator>::New();
  ...
  obbTree0->IntersectWithOBBTree(
    obbTree1, nullptr, svtkIntersectionPolyDataFilter::Impl::FindTriangleIntersections, impl);

  int rawLines = outputIntersection->GetNumberOfLines();

  for (int i = 0; i < 2; i++)
  {
    for (svtkIdType interCellId = 0; interCellId < rawLines; interCellId++)
    {
      impl->NewCellIds[i]->InsertTuple2(interCellId, -1, -1);
    }
  }

  svtkDebugMacro(<< "LINEPTSBEFORE " << outputIntersection->GetNumberOfPoints());
  ...
  svtkSmartPointer<svtkPolyData> tmpLines = svtkSmartPointer<svtkPolyData>::New();
  tmpLines->DeepCopy(outputIntersection);
  tmpLines->BuildLinks();

  svtkSmartPointer<svtkCleanPolyData> lineCleaner = svtkSmartPointer<svtkCleanPolyData>::New();
  ...
  svtkSmartPointer<svtkPointLocator> linePtMapper = svtkSmartPointer<svtkPointLocator>::New();
  linePtMapper->SetDataSet(outputIntersection);
  linePtMapper->BuildLocator();
  double newpt[3];
  svtkIdType mapPtId = 0;
  for (svtkIdType ptId = 0; ptId < tmpLines->GetNumberOfPoints(); ptId++)
  {
  ...
  svtkDebugMacro(<< "LINEPTSAFTER " << outputIntersection->GetNumberOfPoints());
  ...
  {
    svtkGenericWarningMacro(<< "No Intersection between objects ");
  ...
  impl->BoundaryPoints[0] = svtkIntArray::New();
  impl->BoundaryPoints[1] = svtkIntArray::New();
...
//----------------------------------------------------------------------------
int svtkIntersectionPolyDataFilter::FillInputPortInformation(int port, svtkInformation* info)
{
