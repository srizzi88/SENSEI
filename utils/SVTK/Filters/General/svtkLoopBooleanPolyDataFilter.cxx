/*=========================================================================
  ...
  Module:    svtkLoopBooleanPolyDataFilter.cxx
...
=========================================================================*/
/** @file svtkLoopBooleanPolyDataFilter.cxx
...
class svtkLoopBooleanPolyDataFilter::Impl
{
public:
  Impl();
  virtual ~Impl();
  ...
  void PerformBoolean(svtkPolyData* output, int booleanOperation);
  void ThresholdRegions(svtkPolyData** surfaces);

protected:
  int RunLoopFind(svtkIdType interPt, svtkIdType nextCell, bool* usedPt, simLoop* loop);

  int RunLoopTest(svtkIdType interPt, svtkIdType nextCell, simLoop* loop, bool* usedPt);

  int GetCellOrientation(svtkPolyData* pd, svtkIdType cellId, svtkIdType p0, svtkIdType p1, int index);
...
public:
  ...
  svtkPolyData* Mesh[2];
  svtkPolyData* IntersectionLines;

  svtkIntArray* BoundaryPointArray[2];
  svtkIntArray* BoundaryCellArray[2];
  svtkIntArray* BooleanArray[2];
  svtkIntArray* NewCellIds[2];

  svtkIdType* Checked[2];
  svtkIdType* CheckedCarefully[2];
  svtkIdType* PointMapper[2];
  svtkIdType* ReversePointMapper[2];

  svtkIdList* CheckCells;
  svtkIdList* CheckCells2;
  svtkIdList* CheckCellsCareful;
  svtkIdList* CheckCellsCareful2;

  // Pointer to overarching filter
  svtkLoopBooleanPolyDataFilter* ParentFilter;
};

svtkLoopBooleanPolyDataFilter::Impl::Impl()
  : CheckCells(nullptr)
  , CheckCells2(nullptr)
  ...
  , CheckCellsCareful2(nullptr)
{
  for (int i = 0; i < 2; i++)
  {
    this->Mesh[i] = svtkPolyData::New();

    this->BooleanArray[i] = svtkIntArray::New();
    this->BoundaryPointArray[i] = svtkIntArray::New();
    this->BoundaryCellArray[i] = svtkIntArray::New();
    this->NewCellIds[i] = svtkIntArray::New();
  ...
  this->IntersectionLines = svtkPolyData::New();
  this->CheckCells = svtkIdList::New();
  this->CheckCells2 = svtkIdList::New();
  this->CheckCellsCareful = svtkIdList::New();
  this->CheckCellsCareful2 = svtkIdList::New();
...
int svtkLoopBooleanPolyDataFilter::Impl::FindRegion(
  int inputIndex, int fillnumber, int start, int fill)
{
  svtkDebugWithObjectMacro(this->ParentFilter, << "Finding region with fill " << fillnumber
                                              << " of mesh " << inputIndex << " with cellID "
                                              << this->CheckCells->GetId(0));

  // Id List to store neighbor cells for each set of nodes and a cell
  svtkSmartPointer<svtkIdList> neighbors = svtkSmartPointer<svtkIdList>::New();
  svtkSmartPointer<svtkIdList> tmp = svtkSmartPointer<svtkIdList>::New();

  svtkIdType numCheckCells;
  ...
  while ((numCheckCells = this->CheckCells->GetNumberOfIds()) > 0)
  {
    ...
    {
      svtkIdType cellId = this->CheckCells->GetId(c);
      ...
      if (this->Checked[inputIndex][cellId] == 0)
      {
        ...
        for (int i = 0; i < npts; i++)
        {
          ...
          svtkIdType numNeighbors = neighbors->GetNumberOfIds();
...
int svtkLoopBooleanPolyDataFilter::Impl::FindRegionTipToe(int inputIndex, int fillnumber, int fill)
{
  // Id List to store neighbor cells for each set of nodes and a cell
  svtkSmartPointer<svtkIdList> tmp = svtkSmartPointer<svtkIdList>::New();
  svtkSmartPointer<svtkIdList> neighborIds = svtkSmartPointer<svtkIdList>::New();
  ...
  while ((numCheckCells = this->CheckCellsCareful->GetNumberOfIds()) > 0)
  {
    ...
    {
      neighborIds->Reset();
      svtkIdType cellId = this->CheckCellsCareful->GetId(c);
      // Get the three points of the cell
      const svtkIdType* pts = nullptr;
      svtkIdType npts = 0;
      this->Mesh[inputIndex]->GetCellPoints(cellId, npts, pts);
      ...
      if (this->CheckedCarefully[inputIndex][cellId] == 0)
      {
        ...
        svtkDebugWithObjectMacro(this->ParentFilter, << "Checking edges of cell " << cellId);
        for (int i = 0; i < npts; i++)
        {
          svtkIdType p1 = pts[i];
          svtkIdType p2 = pts[(i + 1) % (npts)];
          ...
          if (numNeighbors == 1)
          {
            ...
            svtkIdType neighbor = neighbors->GetId(0);
            ...
            else
            {
              ...
              svtkSmartPointer<svtkIdList> bLinesOne = svtkSmartPointer<svtkIdList>::New();
              svtkSmartPointer<svtkIdList> bLinesTwo = svtkSmartPointer<svtkIdList>::New();

              svtkIdType bPt1 = PointMapper[inputIndex][p1];
              this->IntersectionLines->GetPointCells(bPt1, bLinesOne);

              svtkIdType bPt2 = PointMapper[inputIndex][p2];
              ...
              {
                svtkDebugWithObjectMacro(this->ParentFilter, << "False positive! " << neighbor);
                neighborIds->InsertNextId(neighbor);
              }
              else
              {
          ...
          {
            svtkDebugWithObjectMacro(this->ParentFilter, << "NumNei is not 1");
            svtkDebugWithObjectMacro(this->ParentFilter, << "Number of Neighbors " << numNeighbors);
            svtkDebugWithObjectMacro(this->ParentFilter, << "Cell is " << cellId);
            for (int k = 0; k < numNeighbors; k++)
            {
              svtkDebugWithObjectMacro(this->ParentFilter, << "Id!!! " << neighbors->GetId(k));
            }
          }
        }

        svtkIdType numIds = neighborIds->GetNumberOfIds();
        if (numIds > 0)
        {
          // Add all Ids in current list to global list of Ids
          for (int k = 0; k < numIds; k++)
          {
            svtkIdType neighborId = neighborIds->GetId(k);
...
void svtkLoopBooleanPolyDataFilter::Impl::Initialize()
{
  for (int i = 0; i < 2; i++)
  {
    if (this->Mesh[i]->GetNumberOfPoints() == 0 || this->Mesh[i]->GetNumberOfCells() == 0)
    {
      svtkGenericWarningMacro(<< "Mesh has zero points or cells and "
                             << "cannot run filter");
      return;
    }
    ...
    this->Checked[i] = new svtkIdType[numPolys];
    this->CheckedCarefully[i] = new svtkIdType[numPolys];
    this->PointMapper[i] = new svtkIdType[numPolys];
    this->ReversePointMapper[i] = new svtkIdType[numLinePts];
...
// lines
void svtkLoopBooleanPolyDataFilter::Impl::GetBooleanRegions(
  int inputIndex, std::vector<simLoop>* loops)
{
  svtkSmartPointer<svtkPolyData> tmpPolyData = svtkSmartPointer<svtkPolyData>::New();
  ...
  for (loopit = loops->begin(); loopit != loops->end(); ++loopit)
  {
    ...
    for (cellit = loopcells.begin(); cellit != loopcells.end(); ++cellit)
    {
      simLine nextLine;
      nextLine = *cellit;
      svtkIdType nextCell = nextLine.id;
      svtkIdType p1 = nextLine.pt1;
      svtkIdType p2 = nextLine.pt2;
      svtkIdType outputCellId0 = this->NewCellIds[inputIndex]->GetComponent(nextCell, 0);
      svtkIdType outputCellId1 = this->NewCellIds[inputIndex]->GetComponent(nextCell, 1);
      // If the cell has not been given an orientation from the flood fill
      // algorithm and it has an id from svtkIntersectionPolyDataFilter
      if (outputCellId0 != -1)
      {
...
int svtkLoopBooleanPolyDataFilter::Impl::GetCellOrientation(
  svtkPolyData* pd, svtkIdType cellId, svtkIdType p0, svtkIdType p1, int index)
{
  svtkDebugWithObjectMacro(this->ParentFilter, << "CellId: " << cellId);
  svtkIdType npts;
  ...
  svtkIdType cellPtId0 = this->ReversePointMapper[index][p0];
  svtkIdType cellPtId1 = this->ReversePointMapper[index][p1];
  double points[3][3];
  svtkIdType cellPtId2 = 0;
  ...
  svtkSmartPointer<svtkPoints> cellPts = svtkSmartPointer<svtkPoints>::New();
  ...
  svtkSmartPointer<svtkPolyData> cellPD = svtkSmartPointer<svtkPolyData>::New();
  cellPD->SetPoints(cellPts);

  svtkSmartPointer<svtkCellArray> cellLines = svtkSmartPointer<svtkCellArray>::New();
  for (int j = 0; j < npts; j++)
  {
    int spot1 = j;
    int spot2 = (j + 1) % 3;
  ...
  svtkTriangle::ComputeNormal(points[0], points[1], points[2], normal);
  ...
  svtkDebugWithObjectMacro(this->ParentFilter, << "ROTATION ANGLE " << rotationAngle);
  transform->RotateWXYZ(rotationAngle, rotationAxis[0], rotationAxis[1], rotationAxis[2]);

  svtkTriangle::TriangleCenter(points[0], points[1], points[2], center);
  ...
  svtkSmartPointer<svtkPolyData> transPD = svtkSmartPointer<svtkPolyData>::New();
  ...
  {
    svtkDebugWithObjectMacro(this->ParentFilter, << "Line pts are " << p0 << " and " << p1);
    svtkDebugWithObjectMacro(
...
void svtkLoopBooleanPolyDataFilter::Impl::ResetCheckArrays()
{
  for (int i = 0; i < 2; i++)
  {
    int numPolys = this->Mesh[i]->GetNumberOfCells();
    for (svtkIdType cellId = 0; cellId < numPolys; cellId++)
    {
...
void svtkLoopBooleanPolyDataFilter::Impl::SetBoundaryArrays()
{
  // Variables used in the function
  // Point locator to find points on mesh that are the points on the boundary
  // lines
  svtkSmartPointer<svtkPointLocator> pointLocator1 = svtkSmartPointer<svtkPointLocator>::New();
  svtkSmartPointer<svtkPointLocator> pointLocator2 = svtkSmartPointer<svtkPointLocator>::New();
  ...
  for (svtkIdType pointId = 0; pointId < numPoints; pointId++)
  {
    ...
    // Find point on mesh
    svtkIdType bp1 = pointLocator1->FindClosestPoint(pt);
    this->PointMapper[0][bp1] = pointId;
    this->ReversePointMapper[0][pointId] = bp1;
    this->BoundaryPointArray[0]->InsertValue(bp1, 1);
    svtkSmartPointer<svtkIdList> bpCellIds1 = svtkSmartPointer<svtkIdList>::New();
    ...
    svtkIdType bp2 = pointLocator2->FindClosestPoint(pt);
    this->PointMapper[1][bp2] = pointId;
    this->ReversePointMapper[1][pointId] = bp2;
    this->BoundaryPointArray[1]->InsertValue(bp2, 1);
    svtkSmartPointer<svtkIdList> bpCellIds2 = svtkSmartPointer<svtkIdList>::New();
...
void svtkLoopBooleanPolyDataFilter::Impl::SetCheckArrays()
{
  for (int i = 0; i < 2; i++)
  {
    // Get the number of Polys for scalar allocation
...
svtkStandardNewMacro(svtkLoopBooleanPolyDataFilter);

//-----------------------------------------------------------------------------
svtkLoopBooleanPolyDataFilter::svtkLoopBooleanPolyDataFilter()
...
//-----------------------------------------------------------------------------
svtkLoopBooleanPolyDataFilter::~svtkLoopBooleanPolyDataFilter() = default;

//-----------------------------------------------------------------------------
int svtkLoopBooleanPolyDataFilter::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  svtkDebugMacro(<< "Intersection is Done!!!");

  svtkLoopBooleanPolyDataFilter::Impl* impl = new svtkLoopBooleanPolyDataFilter::Impl();
  ...
  {
    svtkWarningMacro(<< "No intersections!");
    ...
    else
    {
      ...
      {
        svtkDebugMacro(<< "Only returning first surface");
        outputSurface->DeepCopy(impl->Mesh[0]);
      }
      else if (this->NoIntersectionOutput == 2)
      {
        svtkDebugMacro(<< "Only returning second surface");
        outputSurface->DeepCopy(impl->Mesh[1]);
      }
      else
      {
        svtkDebugMacro(<< "Keeping both");
  ...
  svtkDebugMacro(<< "Initializing");
  impl->Initialize();
  svtkDebugMacro(<< "Setting Bound Arrays");
  impl->SetBoundaryArrays();
  svtkDebugMacro(<< "Setting Check Arrays");
  impl->SetCheckArrays();

  // Determine the intersection type and obtain the intersection loops
  // to give to Boolean Region finding
  svtkDebugMacro(<< "Determining Intersection Type");
  ...
  svtkDebugMacro(<< "DONE WITH 1");
  impl->GetBooleanRegions(1, &loops);
  svtkDebugMacro(<< "DONE WITH 2");
  ...
  // manifold surfaces!
  svtkDebugMacro(<< "SURFACE 1 BAD TRI MIN: " << badtri1[0] << " MAX: " << badtri1[1]);
  svtkDebugMacro(<< "SURFACE 1 FREE EDGE MIN: " << freeedge1[0] << " MAX: " << freeedge1[1]);
  svtkDebugMacro(<< "SURFACE 2 BAD TRI MIN: " << badtri2[0] << " MAX: " << badtri2[1]);
  svtkDebugMacro(<< "SURFACE 2 FREE EDGE MIN: " << freeedge2[0] << " MAX: " << freeedge2[1]);

  double fullbadtri[2], fullfreeedge[2], dummy[2];
  svtkIntersectionPolyDataFilter::CleanAndCheckSurface(outputSurface, dummy, this->Tolerance);
  ...
  svtkDebugMacro(<< "FULL SURFACE BAD TRI MIN: " << fullbadtri[0]);
  svtkDebugMacro(<< " MAX: " << fullbadtri[1]);
  svtkDebugMacro(<< "FULL SURFACE FREE EDGE MIN: " << fullfreeedge[0]);
  svtkDebugMacro(<< " MAX: " << fullfreeedge[1]);
...
//-----------------------------------------------------------------------------
void svtkLoopBooleanPolyDataFilter::PrintSelf(ostream& os, svtkIndent indent)
{
...
//-----------------------------------------------------------------------------
int svtkLoopBooleanPolyDataFilter::FillInputPortInformation(int port, svtkInformation* info)
{
...
void svtkLoopBooleanPolyDataFilter::Impl::DetermineIntersection(std::vector<simLoop>* loops)
{
  int numInterPts = this->IntersectionLines->GetNumberOfPoints();
  bool* usedPt;
  usedPt = new bool[numInterPts];
  for (svtkIdType interPt = 0; interPt < numInterPts; interPt++)
  {
  ...
  for (svtkIdType interPt = 0; interPt < numInterPts; interPt++)
  {
    if (usedPt[interPt] == false)
    {
      ...
      svtkIdType nextCell = cellIds->GetId(0);
      ...
      if (caseId != -1)
      {
        ...
        {
          svtkIdType nextPt = caseId;
          svtkDebugWithObjectMacro(this->ParentFilter, << "End point of open loop is " << nextPt);
          newloop.endPt = nextPt;
          newloop.loopType = 2;
          nextCell = cellIds->GetId(1);
          svtkIdType newId = this->RunLoopFind(interPt, nextCell, usedPt, &newloop);
  ...
  svtkDebugWithObjectMacro(this->ParentFilter, << "Number Of Loops: " << loops->size());

  delete[] usedPt;
}

// Function for if the intersection is soft closed or open
int svtkLoopBooleanPolyDataFilter::Impl::RunLoopFind(
  svtkIdType interPt, svtkIdType nextCell, bool* usedPt, simLoop* loop)
{
  svtkIdType prevPt = interPt;
  svtkIdType nextPt = interPt;
  ...
  while (nextPt != loop->cells.front().pt1)
  {
    ...
    if (cellIds->GetNumberOfIds() > 2)
    {
      ...
      svtkDebugWithObjectMacro(this->ParentFilter, << "Next cell is " << nextCell);
    }
    else if (cellIds->GetNumberOfIds() < 2)
    {
      svtkDebugWithObjectMacro(
  ...
  loop->endPt = nextPt;
  loop->loopType = 0;
  svtkDebugWithObjectMacro(this->ParentFilter, << "Start and End Point are " << nextPt);

  return -1;
}

// Tests an orientation in a specified region
int svtkLoopBooleanPolyDataFilter::Impl::RunLoopTest(
  svtkIdType interPt, svtkIdType nextCell, simLoop* loop, bool* usedPt)
{
  // This test is only if the intersection has soft closed loops
  svtkDebugWithObjectMacro(this->ParentFilter, << "Running Loop Test to find right loop");
  svtkIdType stopCell = nextCell;
  svtkIdType prevPt = interPt;
  svtkIdType nextPt = interPt;
  svtkSmartPointer<svtkIdList> pointIds = svtkSmartPointer<svtkIdList>::New();
  svtkSmartPointer<svtkPolyData> tmpPolyData = svtkSmartPointer<svtkPolyData>::New();
  ...
  svtkSmartPointer<svtkIdList> cellIds = svtkSmartPointer<svtkIdList>::New();
  IntersectionLines->GetPointCells(nextPt, cellIds);
  svtkDebugWithObjectMacro(this->ParentFilter,
    << "Number of cells should be more than two!! " << cellIds->GetNumberOfIds());
  for (svtkIdType i = 0; i < cellIds->GetNumberOfIds(); i++)
  {
    int numRegionsFound = 0;
    svtkIdType cellId = cellIds->GetId(i);
    svtkDebugWithObjectMacro(this->ParentFilter, << "Testing cell " << cellId);
    ...
    {
      svtkDebugWithObjectMacro(this->ParentFilter, << "Bad One");
    }
    if (cellId != stopCell && usedPt[nextPt] != true)
    {
      ...
      newline.pt2 = nextPt;
      loop->cells.push_back(newline);
      svtkDebugWithObjectMacro(this->ParentFilter, << "Cell id is: " << cellId);
      for (cellit = loop->cells.begin(); cellit != loop->cells.end(); ++cellit)
      {
        ...
        svtkIdType p1 = nextLine.pt1;
        svtkIdType p2 = nextLine.pt2;
        svtkDebugWithObjectMacro(this->ParentFilter, << "Line cell is " << nextCell);
        svtkIdType outputCellId0 = this->NewCellIds[input]->GetComponent(nextCell, 0);
        svtkIdType outputCellId1 = this->NewCellIds[input]->GetComponent(nextCell, 1);
        if (outputCellId0 != -1)
        {
      ...
      {
        svtkDebugWithObjectMacro(this->ParentFilter, << "Legitimate Loop found");
        return cellId;
      }
    }
  }
...
void svtkLoopBooleanPolyDataFilter::Impl::PerformBoolean(svtkPolyData* output, int booleanOperation)
{
  // svtkSmartPointer<svtkThreshold> thresholder =
  //  svtkSmartPointer<svtkThreshold>::New();
  // svtkSmartPointer<svtkDataSetSurfaceFilter> surfacer =
  //  svtkSmartPointer<svtkDataSetSurfaceFilter>::New();

  svtkPolyData* surfaces[4];
  for (int i = 0; i < 4; i++)
  {
    surfaces[i] = svtkPolyData::New();
  }
  ...
  if (this->IntersectionCase == 2)
  {
    svtkSmartPointer<svtkPolyData> tmp = svtkSmartPointer<svtkPolyData>::New();
    int numCells[4];
    std::list<svtkIdType> nocellregion;
    for (int i = 0; i < 4; i++)
    {
...
void svtkLoopBooleanPolyDataFilter::Impl::ThresholdRegions(svtkPolyData** surfaces)
{
  svtkPoints* points[4];
  svtkCellArray* cells[4];
  svtkIntArray* boundaryPoints[4];
  svtkIntArray* boundaryCells[4];
  svtkIntArray* booleanCells[4];

  for (int i = 0; i < 4; i++)
  {
    points[i] = svtkPoints::New();
    cells[i] = svtkCellArray::New();
    boundaryPoints[i] = svtkIntArray::New();
    boundaryCells[i] = svtkIntArray::New();
    booleanCells[i] = svtkIntArray::New();
  }
  ...
  for (int i = 0; i < 2; i++)
  {
    ...
    for (int j = 0; j < numCells; j++)
    {
      ...
      if (value < 0)
      {
        svtkSmartPointer<svtkIdList> newPointIds = svtkSmartPointer<svtkIdList>::New();
        newPointIds->SetNumberOfIds(3);
        for (int k = 0; k < npts; k++)
        {
          double pt[3];
          this->Mesh[i]->GetPoint(pts[k], pt);
          svtkIdType newId = points[3 * i]->InsertNextPoint(pt);
          newPointIds->SetId(k, newId);
          boundaryPoints[3 * i]->InsertValue(newId, this->BoundaryPointArray[i]->GetValue(pts[k]));
        }
        svtkIdType cellId = cells[3 * i]->InsertNextCell(newPointIds);
      ...
      if (value > 0)
      {
        ...
        for (int k = 0; k < npts; k++)
        {
          double pt[3];
          this->Mesh[i]->GetPoint(pts[k], pt);
          svtkIdType newId = points[i + 1]->InsertNextPoint(pt);
          newPointIds->SetId(k, newId);
          boundaryPoints[i + 1]->InsertValue(newId, this->BoundaryPointArray[i]->GetValue(pts[k]));
        }
        svtkIdType cellId = cells[i + 1]->InsertNextCell(newPointIds);
