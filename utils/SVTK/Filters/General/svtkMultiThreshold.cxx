...
svtkStandardNewMacro(svtkMultiThreshold);
...
static int svtkMultiThresholdLimitErrorCount = 0;

static const char* svtkMultiThresholdSetOperationNames[] = { "AND", "OR", "XOR", "WOR", "NAND" };

// Note the inverted ordering... index this with (-component-1) since the enum values are negative.
static const char* svtkMultiThresholdNormNames[] = {
  "L1Norm",
  "L2Norm",
  "LInfinityNorm",
};

static double svtkMultiThresholdSingleComponentNorm(
  svtkDataArray* arr, svtkIdType tuple, int component)
{
  return arr->GetTuple(tuple)[component];
}

static double svtkMultiThresholdL1ComponentNorm(
  svtkDataArray* arr, svtkIdType tuple, int svtkNotUsed(component))
{
...
static double svtkMultiThresholdL2ComponentNorm(
  svtkDataArray* arr, svtkIdType tuple, int svtkNotUsed(component))
{
  double* x = arr->GetTuple(tuple);
  double norm = 0.;
...
static double svtkMultiThresholdLinfComponentNorm(
  svtkDataArray* arr, svtkIdType tuple, int svtkNotUsed(component))
{
  double* x = arr->GetTuple(tuple);
  double norm = 0.;
...
void svtkMultiThreshold::NormKey::ComputeNorm(
  svtkIdType cellId, svtkCell* cell, svtkDataArray* array, double cellNorm[2]) const
{
  if (!array)
  {
    cellNorm[0] = cellNorm[1] = svtkMath::Nan();
    return;
  }

  if (this->Association == svtkDataObject::FIELD_ASSOCIATION_POINTS)
  {
    svtkIdList* ptIds = cell->GetPointIds();
...
void svtkMultiThreshold::Set::PrintNodeName(ostream& os)
{
  os << "set" << this->Id;
}

#define SVTK_MULTITHRESH_ABOVE_BOTTOM(val)                                                          \
  (this->EndpointClosures[0] == CLOSED ? (val) >= this->EndpointValues[0]                          \
                                       : (val) > this->EndpointValues[0])

#define SVTK_MULTITHRESH_BELOW_TOP(val)                                                             \
  (this->EndpointClosures[1] == CLOSED ? (val) <= this->EndpointValues[1]                          \
                                       : (val) < this->EndpointValues[1])

#define SVTK_MULTITHRESH_ABOVE_TOP(val)                                                             \
  (this->EndpointClosures[1] == CLOSED ? (val) > this->EndpointValues[1]                           \
                                       : (val) >= this->EndpointValues[1])

#define SVTK_MULTITHRESH_BELOW_BOTTOM(val)                                                          \
  (this->EndpointClosures[0] == CLOSED ? (val) < this->EndpointValues[0]                           \
                                       : (val) <= this->EndpointValues[0])

#define SVTK_MULTITHRESH_IS_IN_INTERVAL(val)                                                        \
  (SVTK_MULTITHRESH_ABOVE_BOTTOM(val) && SVTK_MULTITHRESH_BELOW_TOP(val))

#define SVTK_MULTITHRESH_SPANS_INTERVAL(val1, val2)                                                 \
  (SVTK_MULTITHRESH_ABOVE_TOP(val2) && SVTK_MULTITHRESH_BELOW_BOTTOM(val1))

int svtkMultiThreshold::Interval::Match(double cellNorm[2])
{
  if (this->Norm.Association == svtkDataObject::FIELD_ASSOCIATION_POINTS)
  {
    if (this->Norm.AllScalars)
    {
      return SVTK_MULTITHRESH_IS_IN_INTERVAL(cellNorm[0]) &&
        SVTK_MULTITHRESH_IS_IN_INTERVAL(cellNorm[1]);
    }
    else
    {
      return (SVTK_MULTITHRESH_IS_IN_INTERVAL(cellNorm[0]) ||
        SVTK_MULTITHRESH_IS_IN_INTERVAL(cellNorm[1]) ||
        SVTK_MULTITHRESH_SPANS_INTERVAL(cellNorm[0], cellNorm[1]));
    }
  }
  else
  {
...
void svtkMultiThreshold::Interval::PrintNode(ostream& os)
{
  ...
  {
    os << svtkMultiThresholdNormNames[-this->Norm.Component - 1] << "(";
  }
  os << (this->Norm.Association == svtkDataObject::FIELD_ASSOCIATION_POINTS ? "point " : "cell ");
  if (this->Norm.Type >= 0)
  {
    os << svtkDataSetAttributes::GetAttributeTypeAsString(this->Norm.Type);
  }
  else
  {
...
void svtkMultiThreshold::BooleanSet::PrintNode(ostream& os)
{
  os << "  set" << this->Id << " [shape=rect,";
  if (this->OutputId >= 0)
  {
  ...
  os << "label=\"" << svtkMultiThresholdSetOperationNames[this->Operator] << "\"]" << endl;
}

void svtkMultiThreshold::PrintSelf(ostream& os, svtkIndent indent)
{
...
int svtkMultiThreshold::AddIntervalSet(double xmin, double xmax, int omin, int omax, int assoc,
  const char* arrayName, int component, int allScalars)
{
  if (!arrayName)
  {
    svtkWarningMacro("You passed a null array name.");
    return -1;
  }
...
{
  if (attribType < 0 || attribType >= svtkDataSetAttributes::NUM_ATTRIBUTES)
  {
    svtkWarningMacro("You passed an invalid attribute type (" << attribType << ")");
    return -1;
  }
...
int svtkMultiThreshold::AddBooleanSet(int operation, int numInputs, int* inputs)
{
  if (operation < svtkMultiThreshold::AND || operation > svtkMultiThreshold::NAND)
  {
    svtkErrorMacro("Invalid operation (" << operation << ")");
    return -1;
  }

  if (numInputs < 1)
  {
    svtkErrorMacro("Operators require at least one operand. You passed " << numInputs << ".");
    return -1;
  }
  ...
  for (i = 0; i < numInputs; ++i)
  {
    ...
    if (inId < 0 || inId >= sId)
    {
      svtkErrorMacro("Input " << i << " is invalid (" << inId << ").");
      return -1;
    }
  }
...
int svtkMultiThreshold::OutputSet(int setId)
{
  if (setId < 0 || setId >= (int)this->Sets.size())
  {
    svtkWarningMacro("Cannot output " << setId << " because there is no set with that label.");
    return -1;
  }
...
svtkMultiThreshold::svtkMultiThreshold()
{
  this->NextArrayIndex = 0;
  this->NumberOfOutputs = 0;
}
...
int svtkMultiThreshold::AddIntervalSet(NormKey& nk, double xmin, double xmax, int omin, int omax)
{
  if (xmin > xmax)
  {
    svtkWarningMacro("Intervals must be specified with ascending values (xmin <= xmax)");
    return -1;
  }
  ...
  {
    svtkWarningMacro("One of the interval endpoints is not a number.");
    return -1;
  }

  if (xmin == xmax && (omin == OPEN || omax == OPEN))
  {
    svtkWarningMacro("An open interval with equal endpoints will always be empty. I won't help you "
                    "waste my time.");
    return -1;
  }

  if (nk.Association != svtkDataObject::FIELD_ASSOCIATION_POINTS &&
    nk.Association != svtkDataObject::FIELD_ASSOCIATION_CELLS)
  {
    svtkWarningMacro(
  ...
  svtkMultiThreshold::Interval* interval = new svtkMultiThreshold::Interval;
  interval->Norm = nk;
  ...
  if (nk.Component >= 0)
  {
    nk.NormFunction = interval->Norm.NormFunction = svtkMultiThresholdSingleComponentNorm;
  }
  else if (nk.Component == -1)
  {
    nk.NormFunction = interval->Norm.NormFunction = svtkMultiThresholdL1ComponentNorm;
  }
  else if (nk.Component == -2)
  {
    nk.NormFunction = interval->Norm.NormFunction = svtkMultiThresholdL2ComponentNorm;
  }
  else // ( nk.Component == -3 )
  {
    nk.NormFunction = interval->Norm.NormFunction = svtkMultiThresholdLinfComponentNorm;
  }
...
int svtkMultiThreshold::RequestData(
  svtkInformation* svtkNotUsed(req), svtkInformationVector** inputs, svtkInformationVector* output)
{
  int i;
  if (this->Sets.empty())
  {
  ...
  svtkMultiThresholdLimitErrorCount = 0;

  // I. Create multiblock output and 1 child dataset for each set to be output
  svtkInformation* iinfo = inputs[0]->GetInformationObject(0);
  svtkInformation* oinfo = output->GetInformationObject(0);

  int updateNumPieces = oinfo->Get(svtkStreamingDemandDrivenPipeline::UPDATE_NUMBER_OF_PIECES());
  int updatePiece = oinfo->Get(svtkStreamingDemandDrivenPipeline::UPDATE_PIECE_NUMBER());

  svtkPointSet* in = svtkPointSet::SafeDownCast(iinfo->Get(svtkDataObject::DATA_OBJECT()));
  svtkMultiBlockDataSet* omesh =
    svtkMultiBlockDataSet::SafeDownCast(oinfo->Get(svtkDataObject::DATA_OBJECT()));
  if (!omesh)
  {
    return 0;
  }
  ...
  std::vector<svtkUnstructuredGrid*> outv; // vector of output datasets
  ...
  svtkIdType inCell;
  svtkCellData* inCellData = in->GetCellData();
  ...
  std::vector<svtkDataArray*> NormArrays;
  i = 0;
  for (aacn = this->IntervalRules.begin(); aacn != this->IntervalRules.end(); ++aacn, ++i)
  {
    svtkDataArray* arr = this->GetInputArrayToProcess(aacn->first.InputArrayIndex, inputs);
    if (!arr)
    {
      svtkErrorMacro("Input array for norm " << i << " is null");
      return 0;
    }
...
int svtkMultiThreshold::FillInputPortInformation(int svtkNotUsed(port), svtkInformation* info)
{
  info->Set(svtkAlgorithm::INPUT_REQUIRED_DATA_TYPE(), "svtkPointSet");
  return 1;
}

void svtkMultiThreshold::UpdateDependents(int id, std::set<int>& unresolvedOutputs,
  TruthTreeValues& setStates, svtkCellData* inCellData, svtkIdType inCell, svtkGenericCell* cell,
  std::vector<svtkUnstructuredGrid*>& outv)
{
       ...
       dit != this->DependentSets[id].end(); ++dit)
  {
    ...
    if (!bset)
    {
      if (++svtkMultiThresholdLimitErrorCount > 5)
      {
        svtkErrorMacro("Set " << id << " has a dependent set (" << (*dit)
        ...
        { // we just checked the last input... only now can we determine the output
               ...
               ++inIt)
          {
            ...
            else if (setStates[*inIt] != EXCLUDE)
            {
              if (++svtkMultiThresholdLimitErrorCount > 5)
              {
                svtkErrorMacro("Boolean set " << (*dit) << " (XOR) had indeterminate input ("
                                             << (*inIt) << ") on final pass");
              }
            }
          }
        ...
        { // we just checked the last input... only now can we determine the output
               ...
               ++inIt)
          {
            ...
            else if (setStates[*inIt] != EXCLUDE)
            {
              if (++svtkMultiThresholdLimitErrorCount > 5)
              {
                svtkErrorMacro("Boolean set " << (*dit) << " (WOR) had indeterminate input ("
                                             << (*inIt) << ") on final pass");
              }
            }
          }
...
void svtkMultiThreshold::PrintGraph(ostream& os)
{
