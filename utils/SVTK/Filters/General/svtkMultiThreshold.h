...
/**
 * @class   svtkMultiThreshold
 * @brief   Threshold cells within multiple intervals
 *
 * This filter can be substituted for a chain of several svtkThreshold filters
 * and can also perform more sophisticated subsetting operations.
 * It generates a svtkMultiBlockDataSet as its output.
 * This multiblock dataset contains a svtkUnstructuredGrid for each thresholded
 * subset you request.
 ...
 * times and then appending the results, as can be required with svtkThreshold
 ...
 *
 * Another advantage this filter provides over svtkThreshold is the ability
 ...
 * svtkMultiThreshold provides \f$L_1\f$, \f$L_2\f$, and \f$L_{\infty}\f$ norms.
 *
 ...
 * To accomplish this with a svtkMultiThreshold filter,
 * <pre>
 * svtkMultiThreshold* thr;
 * int intervalSets[3];
 *
 * intervalSets[0] = thr->AddIntervalSet( svtkMath::NegInf(), 320., svtkMultiThreshold::CLOSED,
 * svtkMultiThreshold::OPEN, svtkDataObject::FIELD_ASSOCIATION_POINTS, "T", 0, 1 ); intervalSets[1] =
 * thr->AddIntervalSet( 101., svtkMath::Inf(), svtkMultiThreshold::OPEN, svtkMultiThreshold::CLOSED,
 *     svtkDataObject::FIELD_ASSOCIATION_CELLS, "P", 0, 1 );
 * intervalSets[2] = thr->AddIntervalSet( svtkMath::NegInf(), 0.1, svtkMultiThreshold::CLOSED,
 * svtkMultiThreshold::OPEN, svtkDataObject::FIELD_ASSOCIATION_POINTS, "s", 0, 1 );
 *
 * int intermediate = thr->AddBooleanSet( svtkMultiThreshold::OR, 2, &intervalSets[1] );
 *
 * int intersection[2];
 * intersection[0] = intervalSets[0];
 * intersection[1] = intermediate;
 * int outputSet = thr->AddBooleanSet( svtkMultiThreshold::AND, 2, intersection );
 *
...
#ifndef svtkMultiThreshold_h
#define svtkMultiThreshold_h

#include "svtkFiltersGeneralModule.h" // For export macro
#include "svtkMath.h"                 // for Inf() and NegInf()
...
class SVTKFILTERSGENERAL_EXPORT svtkMultiThreshold : public svtkMultiBlockDataSetAlgorithm
{
public:
  svtkTypeMacro(svtkMultiThreshold, svtkMultiBlockDataSetAlgorithm);
  static svtkMultiThreshold* New();
   ...
   * with xmin set to svtkMath::NegInf() and xmax set to \a a.
   * Similarly, if you wish to include all cells with values above some number \a a,
   * call with xmin set to \a a and xmax set to svtkMath::Inf().
   ...
   * - \a center is invalid,
   * - \a xmin == \a xmax and \a omin and/or \a omax are svtkMultiThreshold::OPEN, or
   * - \a xmin > \a xmax.
   * - \a xmin or \a xmax is not a number (i.e., IEEE NaN). Having both \a xmin and \a xmax equal
   NaN is allowed.
   * svtkMath provides a portable way to specify IEEE infinities and Nan.
   ...
   * @param omin Whether the interval should be open or closed at \a xmin. Use
   svtkMultiThreshold::OPEN or svtkMultiThreshold::CLOSED.
   * @param omax Whether the interval should be open or closed at \a xmax. Use
   svtkMultiThreshold::OPEN or svtkMultiThreshold::CLOSED.
   * @param assoc One of svtkDataObject::FIELD_ASSOCIATION_CELLS or
   svtkDataObject::FIELD_ASSOCIATION_POINTS indicating whether
   ...
   * One of svtkDataSetAttributes::SCALARS, VECTORS, TENSORS, NORMALS, TCOORDS, or GLOBALIDS.
   ...
   * @param allScalars When \a center is svtkDataObject::FIELD_ASSOCIATION_POINTS, must all scalars
  ...
  /// components.
  typedef double (*TupleNorm)(svtkDataArray* arr, svtkIdType tuple, int component);
  ...
  {
  public:
    int Association; // svtkDataObject::FIELD_ASSOCIATION_POINTS or
                     // svtkDataObject::FIELD_ASSOCIATION_CELLS
    int Type;        // -1 => use Name, otherwise: svtkDataSetAttributes::{SCALARS, VECTORS, TENSORS,
    ...
    int AllScalars;   // For Association == svtkDataObject::FIELD_ASSOCIATION_POINTS, must all points
                      // be in the interval?
    int InputArrayIndex;    // The number passed to svtkAlgorithm::SetInputArrayToProcess()
    ...
    /// svtkMultiThreshold::IntervalRules map.
...
protected:
  ...
  /**
   * We accept any mesh that is descended from svtkPointSet.
   * In the future, it is possible to accept more types but this would require
   * us to generate a new svtkPoints object for each output mesh.
   */
  ...
  void UpdateDependents(int id, std::set<int>& unresolvedOutputs, TruthTreeValues& setStates,
    svtkCellData* inCellData, svtkIdType cellId, svtkGenericCell* cell,
  ...
  svtkMultiThreshold(const svtkMultiThreshold&) = delete;
  void operator=(const svtkMultiThreshold&) = delete;
};

inline int svtkMultiThreshold::AddLowpassIntervalSet(
  double xmax, int assoc, const char* arrayName, int component, int allScalars)
{
  return this->AddIntervalSet(
    svtkMath::NegInf(), xmax, CLOSED, CLOSED, assoc, arrayName, component, allScalars);
}

inline int svtkMultiThreshold::AddHighpassIntervalSet(
  double xmin, int assoc, const char* arrayName, int component, int allScalars)
{
  return this->AddIntervalSet(
    xmin, svtkMath::Inf(), CLOSED, CLOSED, assoc, arrayName, component, allScalars);
}

inline int svtkMultiThreshold::AddBandpassIntervalSet(
  double xmin, double xmax, int assoc, const char* arrayName, int component, int allScalars)
{
  return this->AddIntervalSet(xmin, xmax, CLOSED, CLOSED, assoc, arrayName, component, allScalars);
}

inline int svtkMultiThreshold::AddNotchIntervalSet(
  double xlo, double xhi, int assoc, const char* arrayName, int component, int allScalars)
{
  int band =
...
inline svtkMultiThreshold::Interval* svtkMultiThreshold::Set::GetIntervalPointer()
{
  return nullptr;
}

inline svtkMultiThreshold::BooleanSet* svtkMultiThreshold::Set::GetBooleanSetPointer()
{
  return nullptr;
}

inline svtkMultiThreshold::Interval* svtkMultiThreshold::Interval::GetIntervalPointer()
{
  return this;
}

inline svtkMultiThreshold::BooleanSet* svtkMultiThreshold::BooleanSet::GetBooleanSetPointer()
{
  return this;
}

#endif // svtkMultiThreshold_h
