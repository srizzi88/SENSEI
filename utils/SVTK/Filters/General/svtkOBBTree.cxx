...
svtkStandardNewMacro(svtkOBBTree);

#define svtkCELLTRIANGLES(CELLPTIDS, TYPE, IDX, PTID0, PTID1, PTID2)                                \
  {                                                                                                \
    switch (TYPE)                                                                                  \
    {                                                                                              \
      case SVTK_TRIANGLE:                                                                           \
...
void svtkOBBTree::FreeSearchStructure()
{
  if (this->Tree)
  {
...
void svtkOBBTree::DeleteTree(svtkOBBNode* OBBptr)
{
  ...
  svtkPoints* pts, double corner[3], double max[3], double mid[3], double min[3], double size[3])
{
  ...
  //
  tMin[0] = tMin[1] = tMin[2] = SVTK_DOUBLE_MAX;
  tMax[0] = tMax[1] = tMax[2] = -SVTK_DOUBLE_MAX;
  ...
  for (pointId = 0; pointId < numPts; pointId++)
  {
    ...
    for (i = 0; i < 3; i++)
    {
      svtkLine::DistanceToLine(x, mean, a[i], t, closest);
      if (t < tMin[i])
      {
        tMin[i] = t;
      }
...
void svtkOBBTree::ComputeOBB(
  svtkDataSet* input, double corner[3], double max[3], double mid[3], double min[3], double size[3])
{
  svtkIdType numPts, numCells, i;
  svtkIdList* cellList;
  svtkDataSet* origDataSet;

  svtkDebugMacro(<< "Computing OBB");

  if (input == nullptr || (numPts = input->GetNumberOfPoints()) < 1 ||
    (input->GetNumberOfCells()) < 1)
  {
    svtkErrorMacro(<< "Can't compute OBB - no data available!");
    return;
  }
...
// a sorted list of relative "sizes" of axes for comparison purposes.
void svtkOBBTree::ComputeOBB(
  svtkIdList* cells, double corner[3], double max[3], double mid[3], double min[3], double size[3])
{
  svtkIdType numCells, i, j, cellId, ptId, pId, qId, rId;
  int k, type;
  svtkIdType numPts = 0;
  ...
  for (i = 0; i < numCells; i++)
  {
    ...
    {
      case SVTK_POLY_DATA:
        ((svtkPolyData*)this->DataSet)->GetCellPoints(cellId, numPts, ptIds);
        break;
      case SVTK_UNSTRUCTURED_GRID:
        ((svtkUnstructuredGrid*)this->DataSet)->GetCellPoints(cellId, numPts, ptIds);
        break;
      default:
        svtkErrorMacro(<< "DataSet " << this->DataSet->GetClassName() << " not supported.");
        break;
    }
    for (j = 0; j < numPts - 2; j++)
    {
      svtkCELLTRIANGLES(ptIds, type, j, pId, qId, rId);
      if (pId < 0)
      {
        continue;
      }
      ...
      svtkMath::Cross(dp0, dp1, xp);
      tri_mass = 0.5 * svtkMath::Norm(xp);
      tot_mass += tri_mass;
      for (k = 0; k < 3; k++)
      {
        mean[k] += tri_mass * c[k];
  ...
  for (ptId = 0; ptId < numPts; ptId++)
  {
    ...
    for (i = 0; i < 3; i++)
    {
      svtkLine::DistanceToLine(p, mean, a[i], t, closest);
      if (t < tMin[i])
      {
        tMin[i] = t;
      }
...
// because svtkTriangle doesn't have an equivalently efficient method.
...
static inline int svtkOBBTreeLineIntersectsTriangle(const double p1[3], const double p2[3],
  double pt1[3], double pt2[3], double pt3[3], double tolerance, double point[3], double& t,
  int& sense)
{
  double normal[3];
  svtkTriangle::ComputeNormal(pt1, pt2, pt3, normal);
  ...
  double numerator = svtkMath::Dot(normal, v1t);
  double denominator = svtkMath::Dot(normal, v12);
  if (denominator == 0)
  {
    return 0;
  }
...
// assuming that the data is a closed svtkPolyData surface.
int svtkOBBTree::InsideOrOutside(const double point[3])
{
  // no points!
  ...
  for (svtkIdType i = 0; i < numCells; i++)
  {
    ...
    ((svtkPolyData*)this->DataSet)->GetCellPoints(i, numPts, ptIds);

    // break the cell into triangles
    for (svtkIdType j = 0; j < numPts - 2; j++)
    {
      svtkIdType pt1Id, pt2Id, pt3Id;
      svtkCELLTRIANGLES(ptIds, cellType, j, pt1Id, pt2Id, pt3Id);
      if (pt1Id < 0)
      ...
      double normal[3];
      svtkTriangle::ComputeNormal(pt1, pt2, pt3, normal);
      double dotProd = svtkMath::Dot(normal, v12);
      if (dotProd < 0)
      {
        dotProd = -dotProd;
      }
...
int svtkOBBTree::IntersectWithLine(
  const double p1[3], const double p2[3], svtkPoints* points, svtkIdList* cellIds)
{
  ...
  if (!this->DataSet->IsA("svtkPolyData"))
  {
    svtkErrorMacro("IntersectWithLine: this method requires a svtkPolyData");
    return 0;
  }
  ...
  svtkIdType* cellList = new svtkIdType[listMaxSize];
  ...
  svtkOBBNode** OBBstack = new svtkOBBNode*[this->GetLevel() + 1];
  ...
  { // simulate recursion without the overhead or limitations
    svtkOBBNode* node = OBBstack[--depth];
    ...
    if (this->LineIntersectsNode(node, p1, p2))
    {
      ...
      { // then this is a leaf node...get Cells
        cells = node->Cells;
        svtkIdType numCells = cells->GetNumberOfIds();
        for (svtkIdType i = 0; i < numCells; i++)
        {
          ...
          for (svtkIdType j = 0; j < numPts - 2; j++)
          {
            ...
            if (svtkOBBTreeLineIntersectsTriangle(
            ...
            { // have to grow the distanceList
              ...
              svtkIdType* tmpCellList = new svtkIdType[listMaxSize];
            ...
            if (cellType != SVTK_TRIANGLE_STRIP)
            {
              break;
            }
          }
  ...
  if (listSize != 0)
  {
    ...
    double ptol = this->Tolerance / sqrt(svtkMath::Dot(v12, v12));
...
// finite line
int svtkOBBTree::IntersectWithLine(const double a0[3], const double a1[3], double tol, double& t,
  double x[3], double pcoords[3], int& subId, svtkIdType& cellId, svtkGenericCell* cell)
{
  double tBest = SVTK_DOUBLE_MAX, xBest[3] = {0., 0., 0,}, pcoordsBest[3] = {0., 0., 0.};
  int subIdBest = -1;
  svtkIdType cellIdBest = -1;

  std::vector<svtkOBBNode*> OBBstack(this->GetLevel() + 1);
  OBBstack[0] = this->Tree;
  int depth = 1;
  while (depth > 0)
  { // simulate recursion without the overhead or limitations
    ...
    if (this->LineIntersectsNode(node, a0, a1))
    {
      ...
      { // then this is a leaf node...get Cells
        ...
        {
          svtkIdType thisId = node->Cells->GetId(ii);
...
void svtkOBBNode::DebugPrintTree(int level, double* leaf_vol, int* minCells, int* maxCells)
{
  ...
  svtkMath::Cross(this->Axes[0], this->Axes[1], xp);
  volume = fabs(svtkMath::Dot(xp, this->Axes[2]));
  for (i = 0; i < 3; i++)
  {
    c[i] =
  ...
  cout << "    " << svtkMath::Norm(this->Axes[0]) << " X " << svtkMath::Norm(this->Axes[1]) << " X "
       << svtkMath::Norm(this->Axes[2]) << "\n";
  for (i = 0; i < level; i++)
  {
    cout << "  ";
  }
...
//
void svtkOBBTree::BuildLocator()
{
  svtkIdType numPts, numCells, i;
  svtkIdList* cellList;

  svtkDebugMacro(<< "Building OBB tree");
  ...
  {
    svtkErrorMacro(<< "Can't build OBB tree - no data available!");
    return;
  }
  ...
  this->Tree = new svtkOBBNode;
  this->Level = 0;
  this->BuildTree(cellList, this->Tree, 0);

  svtkDebugMacro(<< "# Cells: " << numCells << ", Deepest tree level: " << this->Level
...
void svtkOBBTree::BuildTree(svtkIdList* cells, svtkOBBNode* OBBptr, int level)
{
  svtkIdType i, j, numCells = cells->GetNumberOfIds();
  svtkIdType cellId;
  svtkIdType ptId;
...
void svtkOBBTree::GenerateRepresentation(int level, svtkPolyData* pd)
{
  svtkPoints* pts;
  svtkCellArray* polys;
  ...
  {
    svtkErrorMacro(<< "No tree to generate representation from");
    return;
  }
...
void svtkOBBTree::GeneratePolygons(
...
int svtkOBBTree::DisjointOBBNodes(svtkOBBNode* nodeA, svtkOBBNode* nodeB, svtkMatrix4x4* XformBtoA)
{
  ...
  svtkOBBNode nodeBxformed, *pB, *pA;
  ...
  rangeAmin = rangeAmax = svtkMath::Dot(pA->Corner, AtoB);
  rangeBmin = rangeBmax = svtkMath::Dot(pB->Corner, AtoB);
  for (ii = 0; ii < 3; ii++)
  {
    // compute A range
    dotA = svtkMath::Dot(pA->Axes[ii], AtoB);
    if (dotA > 0)
    {
      rangeAmax += dotA;
    }
    ...
    // compute B range
    dotB = svtkMath::Dot(pB->Axes[ii], AtoB);
    if (dotB > 0)
    {
      rangeBmax += dotB;
    }
  ...
  { // plane is normal to pB->Axes[ii]
    ...
    rangeBmin = rangeBmax = svtkMath::Dot(pB->Corner, pB->Axes[ii]);
    rangeBmax += svtkMath::Dot(pB->Axes[ii], pB->Axes[ii]);

    // compute A range...
    rangeAmin = rangeAmax = svtkMath::Dot(pA->Corner, pB->Axes[ii]);
    for (jj = 0; jj < 3; jj++)
    {
      // (note: we are saving all 9 dotproducts for future use)
      dotA = dotAB[ii][jj] = svtkMath::Dot(pB->Axes[ii], pA->Axes[jj]);
      if (dotA > 0)
      {
        rangeAmax += dotA;
      }
  ...
  { // plane is normal to pA->Axes[ii]
    ...
    rangeAmin = rangeAmax = svtkMath::Dot(pA->Corner, pA->Axes[ii]);
    rangeAmax += svtkMath::Dot(pA->Axes[ii], pA->Axes[ii]);

    // compute B range...
    rangeBmin = rangeBmax = svtkMath::Dot(pB->Corner, pA->Axes[ii]);
    for (jj = 0; jj < 3; jj++)
    {
  ...
  for (ii = 0; ii < 3; ii++)
  {
    for (jj = 0; jj < 3; jj++)
    {
      ...
      svtkMath::Cross(pA->Axes[ii], pB->Axes[jj], AtoB);
      rangeAmin = rangeAmax = svtkMath::Dot(pA->Corner, AtoB);
      rangeBmin = rangeBmax = svtkMath::Dot(pB->Corner, AtoB);
      for (kk = 0; kk < 3; kk++)
      {
        // compute A range
        dotA = svtkMath::Dot(pA->Axes[kk], AtoB);
        if (dotA > 0)
        {
          rangeAmax += dotA;
        }
        ...
        // compute B range
        dotB = svtkMath::Dot(pB->Axes[kk], AtoB);
        if (dotB > 0)
        {
          rangeBmax += dotB;
        }
...
int svtkOBBTree::TriangleIntersectsNode(
  svtkOBBNode* nodeA, double p0[3], double p1[3], double p2[3], svtkMatrix4x4* XformBtoA)
{
  svtkOBBNode* pA;
  ...
  svtkMath::Cross(v0, v1, xprod);
  // computing B range is easy...
  rangeBmin = rangeBmax = svtkMath::Dot(pB[0], xprod);
  // compute A range...
  rangeAmin = rangeAmax = svtkMath::Dot(pA->Corner, xprod);
  for (jj = 0; jj < 3; jj++)
  {
    dotA = svtkMath::Dot(xprod, pA->Axes[jj]);
    if (dotA > 0)
    {
      rangeAmax += dotA;
    }
  ...
  { // plane is normal to pA->Axes[ii]
    ...
    // compute B range...
    rangeBmin = rangeBmax = svtkMath::Dot(pB[0], pA->Axes[ii]);
    dotB = svtkMath::Dot(pB[1], pA->Axes[ii]);
    if (dotB > rangeBmax)
    {
      rangeBmax = dotB;
    }
    ...
    dotB = svtkMath::Dot(pB[2], pA->Axes[ii]);
    if (dotB > rangeBmax)
    {
      rangeBmax = dotB;
    }
  ...
  for (ii = 0; ii < 3; ii++)
  {
    for (jj = 0; jj < 3; jj++)
    {
      ...
      svtkMath::Cross(pA->Axes[ii], v0, AtoB);
      rangeAmin = rangeAmax = svtkMath::Dot(pA->Corner, AtoB);
      rangeBmin = rangeBmax = svtkMath::Dot(pB[jj], AtoB);
      for (kk = 0; kk < 3; kk++)
      {
        // compute A range
        dotA = svtkMath::Dot(pA->Axes[kk], AtoB);
      ...
      // compute B range
      dotB = svtkMath::Dot(pB[(jj + 2) % 3], AtoB);
      if (dotB > rangeBmax)
      {
        rangeBmax = dotB;
      }
...
int svtkOBBTree::LineIntersectsNode(svtkOBBNode* pA, const double b0[3], const double b1[3])
{
  ...
  {
    // computing A range is easy...
    rangeAmin = svtkMath::Dot(pA->Corner, pA->Axes[ii]);
    rangeAmax = rangeAmin + svtkMath::Dot(pA->Axes[ii], pA->Axes[ii]);

    // compute B range...
    rangeBmin = svtkMath::Dot(b0, pA->Axes[ii]);
    rangeBmax = rangeBmin;
    dotB = svtkMath::Dot(b1, pA->Axes[ii]);
    if (dotB < rangeBmin)
    {
      rangeBmin = dotB;
    }
...
int svtkOBBTree::IntersectWithOBBTree(svtkOBBTree* OBBTreeB, svtkMatrix4x4* XformBtoA,
  ...
  void* data_arg)
{
  ...
  svtkOBBNode **OBBstackA, **OBBstackB, *nodeA, *nodeB;
  ...
  OBBstackA = new svtkOBBNode*[maxStackDepth];
  OBBstackB = new svtkOBBNode*[maxStackDepth];
...
void svtkOBBTree::PrintSelf(ostream& os, svtkIndent indent)
{
