/*=========================================================================
  ...
  Module:    svtkRecursiveDividingCubes.cxx
...
=========================================================================*/
#include "svtkRecursiveDividingCubes.h"
...
svtkStandardNewMacro(svtkRecursiveDividingCubes);

svtkRecursiveDividingCubes::svtkRecursiveDividingCubes()
{
  this->Value = 0.0;
  this->Distance = 0.1;
  this->Increment = 1;
...
static svtkPoints* NewPts;          // points being generated
static svtkDoubleArray* NewNormals; // points being generated
static svtkCellArray* NewVerts;     // verts being generated

int svtkRecursiveDividingCubes::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  svtkDoubleArray* voxelScalars;

  svtkDebugMacro(<< "Executing recursive dividing cubes...");
  //
  ...
  {
    svtkErrorMacro("Bad input: only treats 3D structured point datasets");
    return 1;
  }
  ...
  voxelPts = svtkIdList::New();
  voxelPts->Allocate(8);
  voxelPts->SetNumberOfIds(8);

  voxelScalars = svtkDoubleArray::New();
...
#define SVTK_POINTS_PER_POLY_VERTEX 10000

void svtkRecursiveDividingCubes::SubDivide(double origin[3], double h[3], double values[8])
{
  int i;
  double hNew[3];
  ...
  if (h[0] < this->Distance && h[1] < this->Distance && h[2] < this->Distance)
  {
    ...
    if (!(this->Count++ % this->Increment)) // add a point
    {
      ...
      if (!(NewPts->GetNumberOfPoints() % SVTK_POINTS_PER_POLY_VERTEX))
      {
        svtkDebugMacro(<< "point# " << NewPts->GetNumberOfPoints());
      }
    }
...
int svtkRecursiveDividingCubes::FillInputPortInformation(int, svtkInformation* info)
{
  info->Set(svtkAlgorithm::INPUT_REQUIRED_DATA_TYPE(), "svtkImageData");
  return 1;
}

void svtkRecursiveDividingCubes::PrintSelf(ostream& os, svtkIndent indent)
{
