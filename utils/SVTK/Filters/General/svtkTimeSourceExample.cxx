/*=========================================================================
  ...
  Module:    svtkTimeSourceExample.cxx
...
svtkStandardNewMacro(svtkTimeSourceExample);

//----------------------------------------------------------------------------
double svtkTimeSourceExample::ValueFunction(double t)
{
  return sin(2 * svtkMath::Pi() * t);
}

//----------------------------------------------------------------------------
double svtkTimeSourceExample::XFunction(double t)
{
  return sin(2 * svtkMath::Pi() * t) * this->XAmplitude;
}

//----------------------------------------------------------------------------
double svtkTimeSourceExample::YFunction(double t)
{
  return sin(2 * svtkMath::Pi() * t) * this->YAmplitude;
}

//----------------------------------------------------------------------------
void svtkTimeSourceExample::LookupTimeAndValue(double& time, double& value)
{
  double t = time;
  if (this->Analytic)
  {
...
//----------------------------------------------------------------------------
int svtkTimeSourceExample::NumCellsFunction(double t)
{
  int numCells = 1;
  if (this->Growing)
  {
...
//----------------------------------------------------------------------------
svtkTimeSourceExample::svtkTimeSourceExample()
{
...
//----------------------------------------------------------------------------
int svtkTimeSourceExample::RequestInformation(
  svtkInformation* reqInfo, svtkInformationVector** inVector, svtkInformationVector* outVector)
{
  ...
  if (!this->Analytic)
  {
    info->Set(svtkStreamingDemandDrivenPipeline::TIME_STEPS(), this->Steps, this->NumSteps);
  }
  else
  {
...
//----------------------------------------------------------------------------
int svtkTimeSourceExample::RequestData(svtkInformation* svtkNotUsed(reqInfo),
  ...
  for (int i = 0; i < 1; i++)
  {
    for (int j = 0; j < numCells; j++)
    {
      for (int k = 0; k < 1; k++)
      {
        ...
        output->InsertNextCell(SVTK_VOXEL, 8, ptcells);
      }
    }
  }
  id->Delete();
...
//----------------------------------------------------------------------------
void svtkTimeSourceExample::PrintSelf(ostream& os, svtkIndent indent)
{
