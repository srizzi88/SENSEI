/*=========================================================================
  ...
  Module:    svtkUncertaintyTubeFilter.cxx
...
svtkStandardNewMacro(svtkUncertaintyTubeFilter);
...
public:
  svtkTubePoint();                                  // method sets up storage
  svtkTubePoint& operator=(const svtkTubePoint& hp); // for resizing
...
public:
  svtkTubeArray();
  ~svtkTubeArray() { delete[] this->Array; }
  svtkIdType GetNumberOfPoints() { return this->MaxId + 1; }
  svtkTubePoint* GetTubePoint(svtkIdType i) { return this->Array + i; }
  svtkTubePoint* InsertNextTubePoint()
  {
  ...
  svtkTubePoint* Resize(svtkIdType sz); // reallocates data
  void Reset() { this->MaxId = -1; }

  svtkTubePoint* Array; // pointer to data
...
//-----------------------------------------------------------------------------
svtkTubePoint::svtkTubePoint() = default;

//-----------------------------------------------------------------------------
svtkTubePoint& svtkTubePoint::operator=(const svtkTubePoint& hp)
{
  for (int i = 0; i < 3; i++)
  {
    this->X[i] = hp.X[i];
...
//-----------------------------------------------------------------------------
svtkTubeArray::svtkTubeArray()
{
  this->MaxId = -1;
  this->Array = new svtkTubePoint[1000];
  this->Size = 1000;
  this->Extend = 5000;
}

//-----------------------------------------------------------------------------
svtkTubePoint* svtkTubeArray::Resize(svtkIdType sz)
{
  svtkTubePoint* newArray;
  ...
  newArray = new svtkTubePoint[newSize];
...
svtkUncertaintyTubeFilter::svtkUncertaintyTubeFilter()
{
  this->Tubes = nullptr;
...
static double IntersectEllipse(double vector[3], double v[3])
{
  ...
  double den =
    v[0] * v[0] * b * b * c * c + v[1] * v[1] * a * a * c * c + v[2] * v[2] * a * a * b * b;
...
//-----------------------------------------------------------------------------
int svtkUncertaintyTubeFilter::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  svtkTubePoint *sPrev, *sPtr, *sNext;
  int k;
  double v0[3], v1[3];

  svtkDebugMacro(<< "Generating uncertainty tubes");
  ...
  svtkIdType numLines = inLines->GetNumberOfCells();
  ...
  this->Tubes = new svtkTubeArray[this->NumberOfTubes];

  svtkDoubleArray* newNormals = svtkDoubleArray::New();
  ...
  for (k = 0, inLines->InitTraversal(); inLines->GetNextCell(npts, pts); k++)
  {
    ...
    if (!svtkPolyLine::GenerateSlidingNormals(inPts, singlePolyline, newNormals))
    {
      svtkWarningMacro("Could not generate normals for line. "
                      "Skipping to next.");
    ...
    for (idx = 0; idx < npts; idx++)
    {
      ...
      svtkMath::Normalize(sPtr->V0);

      // Produce orthogonal axis
      svtkMath::Cross(sPtr->V0, sPtr->V1, sPtr->V2);
      svtkMath::Normalize(sPtr->V2);
      svtkMath::Cross(sPtr->V2, sPtr->V0, sPtr->V1);
      svtkMath::Normalize(sPtr->V1);
...
//-----------------------------------------------------------------------------
int svtkUncertaintyTubeFilter::BuildTubes(
  svtkPointData* pd, svtkPointData* outPD, svtkCellData* cd, svtkCellData* outCD, svtkPolyData* output)
{
  svtkTubePoint* sPtr;
  svtkPoints* newPts;
  ...
  // Initialize
  //
  svtkDebugMacro(<< "Creating uncertainty tubes");
  ...
  newStrips->AllocateEstimate(3 * this->NumberOfTubes, SVTK_CELL_SIZE);
  //
...
//-----------------------------------------------------------------------------
void svtkUncertaintyTubeFilter::PrintSelf(ostream& os, svtkIndent indent)
{
