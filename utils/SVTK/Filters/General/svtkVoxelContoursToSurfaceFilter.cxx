/*=========================================================================
  ...
  Module:    svtkVoxelContoursToSurfaceFilter.cxx
...
=========================================================================*/
#include "svtkVoxelContoursToSurfaceFilter.h"
...
svtkStandardNewMacro(svtkVoxelContoursToSurfaceFilter);

svtkVoxelContoursToSurfaceFilter::svtkVoxelContoursToSurfaceFilter()
{
...
void svtkVoxelContoursToSurfaceFilter::AddLineToLineList(double x1, double y1, double x2, double y2)
{
...
void svtkVoxelContoursToSurfaceFilter::SortLineList()
{
  int i, j;
  double tmp[4];
  double tmpval;
...
void svtkVoxelContoursToSurfaceFilter::CastLines(
...
void svtkVoxelContoursToSurfaceFilter::PushDistances(
...
int svtkVoxelContoursToSurfaceFilter::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  double currentZ;
  svtkStructuredPoints* volume;
  float *volumePtr, *slicePtr;
  svtkContourFilter* contourFilter;
  svtkPolyData* contourOutput;
  svtkAppendPolyData* appendFilter;

  svtkDebugMacro(<< "Creating surfaces from contours");
  ...
  volume = svtkStructuredPoints::New();
  volume->SetDimensions(gridSize[0], gridSize[1], chunkSize);
  volume->SetSpacing(this->Spacing);
  volume->AllocateScalars(SVTK_FLOAT, 1);
  svtkDataArray* volumeArrayDA = volume->GetPointData()->GetScalars();
  svtkFloatArray* volumeArray = svtkArrayDownCast<svtkFloatArray>(volumeArrayDA);
  assert(volumeArray);
  volumePtr = volumeArray->GetPointer(0);

  contourFilter = svtkContourFilter::New();
  contourFilter->SetInputData(volume);
  contourFilter->SetNumberOfContours(1);
  contourFilter->SetValue(0, 0.0);

  appendFilter = svtkAppendPolyData::New();
  ...
  while (currentSlice <= lastSlice)
  {
    ...
    contourOutput = svtkPolyData::New();
...
void svtkVoxelContoursToSurfaceFilter::PrintSelf(ostream& os, svtkIndent indent)
{
