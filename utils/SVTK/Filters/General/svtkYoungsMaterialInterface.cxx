...
class svtkYoungsMaterialInterfaceCellCut
{
public:
  enum
  {
...
public:
  ...
  public:
    ...
    const std::string& normal(const svtkYoungsMaterialInterfaceInternals& storage) const
    {
    ...
    const std::string& ordering(const svtkYoungsMaterialInterfaceInternals& storage) const
    {
...
svtkStandardNewMacro(svtkYoungsMaterialInterface);

/*!
  The default constructor
  \sa ~svtkYoungsMaterialInterface()
*/
svtkYoungsMaterialInterface::svtkYoungsMaterialInterface()
{
  this->FillMaterial = 0;
  ...
  this->Internals = new svtkYoungsMaterialInterfaceInternals;
  this->MaterialBlockMapping = svtkSmartPointer<svtkIntArray>::New();
  this->UseAllBlocks = true;

  svtkDebugMacro(<< "svtkYoungsMaterialInterface::svtkYoungsMaterialInterface() ok\n");
}
...
void svtkYoungsMaterialInterface::PrintSelf(ostream& os, svtkIndent indent)
{
...
void svtkYoungsMaterialInterface::SetNumberOfMaterials(int n)
{
  //   svtkDebugMacro(<<"Resize Materials to "<<n<<"\n");
  this->NumberOfDomains = -1;
  this->Internals->Materials.resize(n);
  this->Modified();
}

int svtkYoungsMaterialInterface::GetNumberOfMaterials()
{
  return static_cast<int>(this->Internals->Materials.size());
}

void svtkYoungsMaterialInterface::SetMaterialVolumeFractionArray(int M, const char* volume)
{
  svtkDebugMacro(<< "SetMaterialVolumeFractionArray " << M << " : " << volume << "\n");
  this->NumberOfDomains = -1;
  if (M < 0)
  {
    svtkErrorMacro(<< "Bad material index " << M << "\n");
    return;
  }
...
void svtkYoungsMaterialInterface::SetMaterialNormalArray(int M, const char* normal)
{
  svtkDebugMacro(<< "SetMaterialNormalArray " << M << " : " << normal << "\n");
  this->NumberOfDomains = -1;
  if (M < 0)
  {
    svtkErrorMacro(<< "Bad material index " << M << "\n");
...
void svtkYoungsMaterialInterface::SetMaterialOrderingArray(int M, const char* ordering)
{
  svtkDebugMacro(<< "SetMaterialOrderingArray " << M << " : " << ordering << "\n");
  this->NumberOfDomains = -1;
  if (M < 0)
  {
    svtkErrorMacro(<< "Bad material index " << M << "\n");
...
void svtkYoungsMaterialInterface::SetMaterialArrays(
  int M, const char* volume, const char* normal, const char* ordering)
{
  ...
  svtkDebugMacro(<< "Set Material " << M << " : " << volume << "," << normal << "," << ordering
                << "\n");
  svtkYoungsMaterialInterfaceInternals::MaterialDescription md;
  md.setVolume(volume);
  md.setNormal(normal);
  md.setNormalX("");
  md.setNormalY("");
...
void svtkYoungsMaterialInterface::SetMaterialArrays(int M, const char* volume, const char* normalX,
  const char* normalY, const char* normalZ, const char* ordering)
{
  ...
  svtkDebugMacro(<< "Set Material " << M << " : " << volume << "," << normalX << "," << normalY
                << "," << normalZ << "," << ordering << "\n");
  svtkYoungsMaterialInterfaceInternals::MaterialDescription md;
  md.setVolume(volume);
  md.setNormal("");
...
//-----------------------------------------------------------------------------
void svtkYoungsMaterialInterface::SetMaterialNormalArray(const char* volume, const char* normal)
{
...
//-----------------------------------------------------------------------------
void svtkYoungsMaterialInterface::SetMaterialOrderingArray(const char* volume, const char* ordering)
{
...
//-----------------------------------------------------------------------------
void svtkYoungsMaterialInterface::RemoveAllMaterials()
{
  this->NumberOfDomains = -1;
  svtkDebugMacro(<< "Remove All Materials\n");
  this->Internals->NormalArrayMap.clear();
  this->Internals->OrderingArrayMap.clear();
  this->SetNumberOfMaterials(0);
}

int svtkYoungsMaterialInterface::FillInputPortInformation(int, svtkInformation* info)
{
...
// internal classes
struct svtkYoungsMaterialInterface_IndexedValue
{
  double value;
  int index;
  inline bool operator<(const svtkYoungsMaterialInterface_IndexedValue& iv) const
  {
    return value < iv.value;
  }
};

struct svtkYoungsMaterialInterface_Mat
{
  // input
  svtkDataArray* fractionArray;
  svtkDataArray* normalArray;
  svtkDataArray* normalXArray;
  svtkDataArray* normalYArray;
  svtkDataArray* normalZArray;
  svtkDataArray* orderingArray;

  // temporary
  svtkIdType numberOfCells;
  svtkIdType numberOfPoints;
  svtkIdType cellCount;
  svtkIdType cellArrayCount;
  svtkIdType pointCount;
  svtkIdType* pointMap;

  // output
  std::vector<unsigned char> cellTypes;
  std::vector<svtkIdType> cells;
  svtkDataArray** outCellArrays;
  svtkDataArray** outPointArrays; // last point array is point coords
};

static inline void svtkYoungsMaterialInterface_GetPointData(int nPointData,
  svtkDataArray** inPointArrays, svtkDataSet* input,
  std::vector<std::pair<int, svtkIdType> >& prevPointsMap, int svtkNotUsed(nmat),
  svtkYoungsMaterialInterface_Mat* Mats, int a, svtkIdType i, double* t)
{
  if ((i) >= 0)
  {
    if (a < (nPointData - 1))
  ...
  else
  {
    ...
    DBG_ASSERT(prev_m >= 0);
    svtkIdType prev_i = (prevPointsMap[j].second);
...
#define GET_POINT_DATA(a, i, t)                                                                    \
  svtkYoungsMaterialInterface_GetPointData(                                                         \
    nPointData, inPointArrays, input, prevPointsMap, nmat, Mats, a, i, t)

struct CellInfo
{
  double points[svtkYoungsMaterialInterface::MAX_CELL_POINTS][3];
  svtkIdType pointIds[svtkYoungsMaterialInterface::MAX_CELL_POINTS];
  int triangulation[svtkYoungsMaterialInterface::MAX_CELL_POINTS * 4];
  int edges[svtkYoungsMaterialInterface::MAX_CELL_POINTS][2];
...
int svtkYoungsMaterialInterface::CellProduceInterface(
...
void svtkYoungsMaterialInterface::RemoveAllMaterialBlockMappings()
{
  svtkDebugMacro(<< "RemoveAllMaterialBlockMappings\n");
  this->MaterialBlockMapping->Reset();
}

void svtkYoungsMaterialInterface::AddMaterialBlockMapping(int b)
{
  svtkDebugMacro(<< "AddMaterialBlockMapping " << b << "\n");
  this->MaterialBlockMapping->InsertNextValue(b);
}

void svtkYoungsMaterialInterface::UpdateBlockMapping()
{
  ...
  for (int i = 0; i < n; ++i)
  {
    ...
    svtkDebugMacro(<< "MaterialBlockMapping " << b << "\n");
    if (b < 0)
      curmat = (-b) - 1;
    else
    {
      svtkDebugMacro(<< "Material " << curmat << ": Adding block " << b << "\n");
...
//-----------------------------------------------------------------------------
void svtkYoungsMaterialInterface::Aggregate(int nmat, int* inputsPerMaterial)
{
...
//-----------------------------------------------------------------------------
int svtkYoungsMaterialInterface::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  // debug statistics
  svtkIdType debugStats_PrimaryTriangulationfailed = 0;
  svtkIdType debugStats_Triangulationfailed = 0;
  svtkIdType debugStats_NullNormal = 0;
  svtkIdType debugStats_NoInterfaceFound = 0;

  // Initialize number of materials
  int nmat = static_cast<int>(this->Internals->Materials.size());
  if (nmat <= 0)
  {
    svtkErrorMacro(<< "Invalid materials size\n");
    return 0;
  }

  // allocate composite iterator
  svtkSmartPointer<svtkCompositeDataIterator> inputIterator;
  ...
  while (!inputIterator->IsDoneWithTraversal())
  {
    ...
    {
      int m = 0;
      for (std::vector<svtkYoungsMaterialInterfaceInternals::MaterialDescription>::iterator it =
             this->Internals->Materials.begin();
           it != this->Internals->Materials.end(); ++it, ++m)
      {
        svtkDataArray* fraction = input->GetCellData()->GetArray((*it).volume().c_str());
  ...
  std::map<int, svtkSmartPointer<svtkUnstructuredGrid> > outputBlocks;
  ...
  while (inputIterator->IsDoneWithTraversal() == 0)
  {
    ...
    svtkIdType nCells = input->GetNumberOfCells();
    svtkIdType nPoints = input->GetNumberOfPoints();

    // -------------- temporary data initialization -------------------
    svtkDataArray** inCellArrays = new svtkDataArray*[nCellData];
    for (int i = 0; i < nCellData; i++)
    {
      inCellArrays[i] = input->GetCellData()->GetArray(i);
    }

    svtkDataArray** inPointArrays =
      new svtkDataArray*[nPointData + 1]; // last point array is point coords
    ...
    svtkYoungsMaterialInterface_Mat* Mats = new svtkYoungsMaterialInterface_Mat[nmat];
    {
      int m = 0;
      for (std::vector<svtkYoungsMaterialInterfaceInternals::MaterialDescription>::iterator it =
             this->Internals->Materials.begin();
           it != this->Internals->Materials.end(); ++it, ++m)
      {
        ...
        {
          svtkDebugMacro(<< "Material " << m << ": volume fraction array '" << (*it).volume()
                        << "' not found\n");
        }
        if (!Mats[m].orderingArray)
        {
          svtkDebugMacro(<< "Material " << m << " material ordering array '"
                        << (*it).ordering(*this->Internals) << "' not found\n");
        }
        if (!Mats[m].normalArray && !Mats[m].normalXArray && !Mats[m].normalYArray &&
          !Mats[m].normalZArray)
        {
          svtkDebugMacro(<< "Material " << m << " normal  array '" << (*it).normal(*this->Internals)
                        << "' not found\n");
        }
        ...
        Mats[m].outCellArrays = new svtkDataArray*[nCellData];
        for (int i = 0; i < nCellData; ++i)
        {
          Mats[m].outCellArrays[i] = svtkDataArray::CreateDataArray(inCellArrays[i]->GetDataType());
        ...
        Mats[m].pointCount = 0;
        Mats[m].outPointArrays = new svtkDataArray*[nPointData];

        for (int i = 0; i < (nPointData - 1); i++)
        {
          Mats[m].outPointArrays[i] =
            svtkDataArray::CreateDataArray(inPointArrays[i]->GetDataType());
        ...
        Mats[m].outPointArrays[nPointData - 1] = svtkDoubleArray::New();
    ...
    {
      svtkCell* svtkcell = input->GetCell(c);
      int cellDim = svtkcell->GetCellDimension();
      int np = svtkcell->GetNumberOfPoints();
      int nf = svtkcell->GetNumberOfFaces();
    ...
    for (int m = 0; m < nmat; m++)
    {
      svtkDebugMacro(<< "Mat #" << m << " : cells=" << Mats[m].numberOfCells << ", points="
      ...
      Mats[m].pointMap = new svtkIdType[nPoints];
    ...
    svtkConvexPointSet* cpsCell = svtkConvexPointSet::New();

    double* interpolatedValues = new double[MAX_CELL_POINTS * pointDataComponents];
    svtkYoungsMaterialInterface_IndexedValue* matOrdering =
      new svtkYoungsMaterialInterface_IndexedValue[nmat];

    std::vector<std::pair<int, svtkIdType> > prevPointsMap;
    prevPointsMap.reserve(MAX_CELL_POINTS * nmat);

    for (svtkIdType ci = 0; ci < nCells; ci++)
    {
      ...
      svtkCell* svtkcell = input->GetCell(ci);
      CellInfo cell;
      cell.dim = svtkcell->GetCellDimension();
      cell.np = svtkcell->GetNumberOfPoints();
      cell.nf = svtkcell->GetNumberOfFaces();
      cell.type = svtkcell->GetCellType();
      ...
      {
        cell.pointIds[p] = svtkcell->GetPointId(p);
        DBG_ASSERT(cell.pointIds[p] >= 0 && cell.pointIds[p] < nPoints);
        svtkcell->GetPoints()->GetPoint(p, cell.points[p]);
      }
      ...
      cell.triangulationOk = (svtkcell->Triangulate(ci, ptIds, pts) != 0);
      cell.ntri = 0;
      ...
      else
      {
        ...
        svtkWarningMacro(<< "Triangulation failed on primary cell\n");
      }

      // get 3D cell edges.
      if (cell.dim == 3)
      {
        svtkCell3D* cell3D = svtkCell3D::SafeDownCast(svtkcell);
        cell.nEdges = svtkcell->GetNumberOfEdges();
        for (int i = 0; i < cell.nEdges; i++)
        {
          const svtkIdType* edgePoints;
      ...
      for (int mi = 0; mi < nmat; mi++)
      {
              ...
              this->VolumeFractionRange[1]))
        {
          ...
          int interfaceCellType = SVTK_EMPTY_CELL;
          ...
          if ((!mi) || (!this->OnionPeel))
          {
            ...
            if (svtkMath::Norm(normal) == 0.0) // should it be <EPSILON ?
            {
          ...
          {
            interfaceCellType = SVTK_EMPTY_CELL;
            // Mats[m].cellTypes.push_back( SVTK_EMPTY_CELL );
            ...
            if (!cell.triangulationOk)
            {
              ...
              svtkWarningMacro(<< "Cell triangulation failed\n");
            }
          }
          ...
          else if (cell.dim == 2)
          {
            ...
            bool interfaceFound = svtkYoungsMaterialInterfaceCellCut::cellInterfaceD(cell.points,
            ...
            {
              nInterfaceEdges = 2;
              interfaceCellType = this->FillMaterial ? SVTK_POLYGON : SVTK_LINE;
              // Mats[m].cellTypes.push_back( this->FillMaterial ? SVTK_POLYGON : SVTK_LINE );

              // remaining volume is a polygon
              nextCell.dim = 2;
              nextCell.np = nRemCellPoints;
              nextCell.nf = nRemCellPoints;
              nextCell.type = SVTK_POLYGON;
              ...
              {
                svtkIdType id = remCellPointIds[i];
                if (id < 0)
                {
            ...
            {
              svtkWarningMacro(<< "no interface found for cell " << ci << ", mi=" << mi
                              << ", m=" << m << ", frac=" << fraction << "\n");
              nInterfaceEdges = 0;
              nOutCellPoints = 0;
              nInsidePoints = 0;
          ...
          else
          {
            ...
            svtkYoungsMaterialInterfaceCellCut::cellInterface3D(cell.np, cell.points, cell.nEdges,
              ...
              nInterfaceEdges < 3) // degenerated case, considered as null interface
            {
              ...
              svtkDebugMacro(<< "no interface found for cell " << ci << ", mi=" << mi << ", m=" << m
                            << ", frac=" << fraction << "\n");
              nInterfaceEdges = 0;
              nOutCellPoints = 0;
              nInsidePoints = 0;
            ...
            else
            {
              ...
              if (this->FillMaterial)
              {
                interfaceCellType = SVTK_CONVEX_POINT_SET;
                // Mats[m].cellTypes.push_back( SVTK_CONVEX_POINT_SET );
              ...
              {
                interfaceCellType = SVTK_POLYGON;
                // Mats[m].cellTypes.push_back( SVTK_POLYGON );
              }
              ...
              {
                nextCell.type = SVTK_CONVEX_POINT_SET;
                nextCell.np = nInterfaceEdges + nOutsidePoints;
                svtkcell = cpsCell;
                svtkcell->Points->Reset();
                svtkcell->PointIds->Reset();
                svtkcell->Points->SetNumberOfPoints(nextCell.np);
                svtkcell->PointIds->SetNumberOfIds(nextCell.np);
                for (int i = 0; i < nextCell.np; i++)
                {
                  svtkcell->PointIds->SetId(i, i);
                }
              ...
              {
                svtkIdType id = -(int)(prevPointsMap.size() + 1);
                DBG_ASSERT((-id - 1) == prevPointsMap.size());
          ...
          //  create output cell
          if (interfaceCellType != SVTK_EMPTY_CELL)
          {
            ...
            if (this->FillMaterial)
            {
              for (int p = 0; p < nInsidePoints; p++)
              {
                svtkIdType ptId = cell.pointIds[insidePointIds[p]];
                if (ptId >= 0)
                {
                  if (Mats[m].pointMap[ptId] == -1)
                  {
                    svtkIdType nptId = Mats[m].pointCount + nInterfaceEdges + pointsCopied;
            ...
            for (int p = 0; p < nOutCellPoints; ++p)
            {
              ...
              if (pointIndex >= 0)
              {
                ...
                svtkIdType ptId = cell.pointIds[pointIndex];
                if (ptId >= 0)
                {
              ...
              processedEfectiveMat < nEffectiveMat)
            {
              ...
              svtkcell->Initialize();
              nextCell.nf = svtkcell->GetNumberOfFaces();
              if (nextCell.dim == 3)
              {
                svtkCell3D* cell3D = svtkCell3D::SafeDownCast(svtkcell);
                nextCell.nEdges = svtkcell->GetNumberOfEdges();
              ...
              nextCell.triangulationOk = (svtkcell->Triangulate(ci, ptIds, pts) != 0);
              nextCell.ntri = 0;
              if (nextCell.triangulationOk)
              {
                ...
                {
                  svtkIdType j = ptIds->GetId(i); // cell ids have been set with local ids
              ...
              {
                debugStats_Triangulationfailed++;
                svtkWarningMacro(<< "Triangulation failed. Info: cell " << ci << ", material " << mi
        ...
        //                      {
        //                              //svtkWarningMacro(<<"ignoring cell #"<<ci<<", m="<<m<<",
        //                              mi="<<mi<<", frac="<<fraction<<"\n");
        //                      }
    ...
    for (int m = 0; m < nmat; m++)
    {
      ...
      {
        svtkDebugMacro(<< "Mat #" << m << " : cellCount=" << Mats[m].cellCount << ", numberOfCells="
      ...
      svtkSmartPointer<svtkUnstructuredGrid> ugOutput = svtkSmartPointer<svtkUnstructuredGrid>::New();
      ...
      svtkIdTypeArray* cellArrayData = svtkIdTypeArray::New();
      cellArrayData->SetNumberOfValues(Mats[m].cellArrayCount);
      svtkIdType* cellArrayDataPtr = cellArrayData->WritePointer(0, Mats[m].cellArrayCount);
      for (svtkIdType i = 0; i < Mats[m].cellArrayCount; i++)
      ...
      for (svtkIdType i = 0; i < Mats[m].cellCount; i++)
      ...
      {
        svtkDataArray* attr = input->GetCellData()->GetAttribute(i);
        if (attr != nullptr)
        {
      ...
      for (int i = 0; i < svtkDataSetAttributes::NUM_ATTRIBUTES; ++i)
      {
        svtkDataArray* attr = input->GetPointData()->GetAttribute(i);
        if (attr != nullptr)
        {
  ...
  {
    svtkDebugMacro(<< "PrimaryTriangulationfailed " << debugStats_PrimaryTriangulationfailed
                  << "\n");
  }
  if (debugStats_Triangulationfailed)
  {
    svtkDebugMacro(<< "Triangulationfailed " << debugStats_Triangulationfailed << "\n");
  }
  if (debugStats_NullNormal)
  {
    svtkDebugMacro(<< "NullNormal " << debugStats_NullNormal << "\n");
  }
  if (debugStats_NoInterfaceFound)
  {
    svtkDebugMacro(<< "NoInterfaceFound " << debugStats_NoInterfaceFound << "\n");
  }

  // Build final composite output. also tagging blocks with their associated Id
  svtkDebugMacro(<< this->NumberOfDomains << " Domains, " << nmat << " Materials\n");
  ...
  for (int m = 0; m < nmat; ++m)
  {
    svtkMultiBlockDataSet* matBlock = svtkMultiBlockDataSet::New();
  ...
  int blockIndex = 0;
  for (std::map<int, svtkSmartPointer<svtkUnstructuredGrid> >::iterator it = outputBlocks.begin();
       it != outputBlocks.end(); ++it, ++blockIndex)
  {
    if (it->second->GetNumberOfCells() > 0)
    {
      ...
      svtkMultiBlockDataSet* matBlock = svtkMultiBlockDataSet::SafeDownCast(output->GetBlock(mat));
...
namespace svtkYoungsMaterialInterfaceCellCutInternals
{
 ...
 *********************/
/*
  Formula from SVTK in svtkTriangle.cxx, method TriangleArea
*/
FUNC_DECL
...
//         ####     ####
// ------------------------------------
void svtkYoungsMaterialInterfaceCellCut::cellInterface3D(int ncoords, double coords[][3], int nedge,
  ...
  int& nOutside, int outPoints[])
{
  ...
  for (int i = 0; i < nedge; i++)
  {
    ...
    if (dist[e0] * dist[e1] < 0)
    {
      ...
      {
        t = (0 - dist[e0]) / edist;
        t = svtkMath::ClampValue(t, 0.0, 1.0);
      }
      else
      {
        t = 0;
  ...
  if (np > 3)
  {
    ...
    svtkYoungsMaterialInterfaceCellCutInternals::CWVertex pts[MAX_CELL_POINTS];
    for (int i = 0; i < np; i++)
    {
      double vec[3];
      for (int comp = 0; comp < 3; comp++)
...
double svtkYoungsMaterialInterfaceCellCut::findTetraSetCuttingPlane(const double normal[3],
  const double fraction, const int vertexCount, const double vertices[][3], const int tetraCount,
  const int tetras[][4])
{
  svtkYoungsMaterialInterfaceCellCutInternals::Real3 N = { normal[0], normal[1], normal[2] };
  svtkYoungsMaterialInterfaceCellCutInternals::Real3 V[LOCAL_ARRAY_SIZE(vertexCount)];
  svtkYoungsMaterialInterfaceCellCutInternals::uchar4 tet[LOCAL_ARRAY_SIZE(tetraCount)];
  ...
  svtkYoungsMaterialInterfaceCellCutInternals::Real3 vmin, vmax;
  svtkYoungsMaterialInterfaceCellCutInternals::Real scale;
  vmin = vmax = V[0];
  ...
  double d = dist0 +
    svtkYoungsMaterialInterfaceCellCutInternals::findTetraSetCuttingPlane(
...
bool svtkYoungsMaterialInterfaceCellCut::cellInterfaceD(double points[][3], int nPoints,
...
double svtkYoungsMaterialInterfaceCellCut::findTriangleSetCuttingPlane(const double normal[3],
  ...
  const int triangles[][3], bool axisSymetric)
{
  ...
  svtkYoungsMaterialInterfaceCellCutInternals::uchar3 tri[LOCAL_ARRAY_SIZE(triangleCount)];
  ...
  if (axisSymetric)
  {
    svtkYoungsMaterialInterfaceCellCutInternals::Real2 N = { normal[0], normal[1] };
    svtkYoungsMaterialInterfaceCellCutInternals::Real2 V[LOCAL_ARRAY_SIZE(vertexCount)];
    for (int i = 0; i < vertexCount; i++)
    {
      V[i].x = vertices[i][0] - vertices[0][0];
      V[i].y = vertices[i][1] - vertices[0][1];
    }
    svtkYoungsMaterialInterfaceCellCutInternals::Real2 vmin, vmax;
    svtkYoungsMaterialInterfaceCellCutInternals::Real scale;
    vmin = vmax = V[0];
    for (int i = 1; i < vertexCount; i++)
    {
    ...
    d = dist0 +
      svtkYoungsMaterialInterfaceCellCutInternals::findTriangleSetCuttingCone(
  ...
  else
  {
    ...
    double dist0 =
      vertices[0][0] * normal[0] + vertices[0][1] * normal[1] + vertices[0][2] * normal[2];
    d = dist0 +
      svtkYoungsMaterialInterfaceCellCutInternals::findTriangleSetCuttingPlane(
        N, fraction, vertexCount, triangleCount, tri, V) *
        scale;
  }
