...
int TestAssertion(ostream& strm, svtkIndent indent, const char* label, int assertion);
...
int TestEmpty(ostream& strm)
{
  ...
  // actual test
  strm << "Test svtkBridgeDataSet Start" << endl;

  strm << "Create an empty svtkUnstructuredGrid" << endl;
  svtkUnstructuredGrid* g = svtkUnstructuredGrid::New();
  strm << "Empty unstructured grid created" << endl;

  strm << "Create a svtkBridgeDataSet" << endl;
  svtkBridgeDataSet* ds = svtkBridgeDataSet::New();
  strm << "svtkBridgeDataSet created" << endl;

  strm << "Init the svtkBridgeDataSet with the empty unstructured grid" << endl;
  ds->SetDataSet(g);
  strm << "svtkBridgeDataSet initialized with the empty unstructured grid" << endl;
  ...
  svtkCellTypes* types = svtkCellTypes::New();
  ...
  svtkGenericCellIterator* it = ds->NewCellIterator(-1);
  ...
  svtkGenericPointIterator* pit = ds->NewPointIterator();
  ...
  MacroTest(strm, indent, "default volatile bounds", !svtkMath::AreBoundsInitialized(b));

  ds->GetBounds(bounds);
  MacroTest(strm, indent, "default bounds", !svtkMath::AreBoundsInitialized(bounds));
...
#endif
  strm << "Delete the svtkBridgeDataSet" << endl;
  ds->Delete();
  strm << "svtkBridgeDataSet deleted" << endl;

  strm << "Delete the empty svtkUnstructuredGrid" << endl;
  g->Delete();
  strm << "Empty svtkUnstructuredGrid deleted" << endl;

  strm << "Test svtkBridgeDataSet creation End" << endl;
...
int TestWithPoints(ostream& strm)
{
  ...
  strm << "Add points to the svtkUnstructuredGrid" << endl;
  g->SetPoints(pts);
  strm << "Points added to the svtkUnstructuredGrid" << endl;

  strm << "Create a svtkBridgeDataSet" << endl;
  svtkBridgeDataSet* ds = svtkBridgeDataSet::New();
  strm << "svtkBridgeDataSet created" << endl;

  strm << "Init the svtkBridgeDataSet with the unstructured grid" << endl;
  ds->SetDataSet(g);
  strm << "svtkBridgeDataSet initialized with the unstructured grid" << endl;
  ...
  strm << "Delete the svtkUnstructuredGrid" << endl;
  g->Delete();
  strm << "svtkUnstructuredGrid deleted" << endl;
...
int TestWithPointsAndCells(ostream& strm)
{
  ...
  svtkGenericAdaptorCell* cab = nullptr;

  while (itNum < itCount)
  {
    svtkGenericCellIterator* it = ds->NewCellIterator(itNum);
  ...
  svtkGenericAttributeCollection* attributes = nullptr;
  attributes = ds->GetAttributes();
  MacroTest(strm, indent, "attributes exist", attributes != nullptr);
  MacroTest(strm, indent, "empty attributes", attributes->IsEmpty());
  MacroTest(strm, indent, "empty attributes", attributes->GetNumberOfAttributes() == 0);
  ...
  strm << "svtkBridgeCell::GetBoundaryIterator() test start" << endl;
  ...
  //  int count;
  //  std::string s;
  //  svtkOStrStreamWrapper *ost=0;
  //  svtkGenericAdaptorCell *cab=0;
  ...
  svtkGenericCellIterator* boundaries = ds->NewCellIterator(-1); // just for creation
  MacroTest(strm, indent, "boundaries exists", boundaries != nullptr);

  i = 0;
  count = ds->GetNumberOfCells(-1);

  svtkGenericAdaptorCell* cab2;
  ...
  strm << "svtkBridgeCell::GetBoundaryIterator() test end" << endl;
...
int TestWithPointsAndCellsAndPointData(ostream& strm)
{
  ...
  strm << "Add point data to the svtkUnstructuredGrid" << endl;
  int m = 0;
  svtkDoubleArray* attrib = svtkDoubleArray::New();
  while (m < 17)
  {
  ...
  attrib->Delete();
  attrib = nullptr;
  strm << "Point data added to the svtkUnstructuredGrid" << endl;
  ...
  svtkGenericAttribute* attribute = nullptr;
  ...
  MacroTest(strm, indent, "attribute centering", attribute->GetCentering() == svtkPointCentered);
  MacroTest(strm, indent, "attribute type", attribute->GetComponentType() == SVTK_DOUBLE);
...
int otherCreation(int svtkNotUsed(argc), char* svtkNotUsed(argv)[])
{
  if (TestEmpty(cout))
  {
    return 1;
