...
Module:    svtkGenericGlyph3DFilter.cxx
...
svtkStandardNewMacro(svtkGenericGlyph3DFilter);

// Construct object with scaling on, scaling mode is by scalar value,
// scale factor = 1.0, the range is (0,1), orient geometry is on, and
// orientation is by vector. Clamping and indexing are turned off. No
// initial sources are defined.
svtkGenericGlyph3DFilter::svtkGenericGlyph3DFilter()
{
  this->Scaling = 1;
...
//-----------------------------------------------------------------------------
int svtkGenericGlyph3DFilter::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  svtkDataArray* sourceNormals = nullptr;
  svtkGenericAttribute* inScalars = nullptr;
  svtkGenericAttribute* inVectors = nullptr;
  svtkGenericAttribute* inNormals = nullptr;
  //  svtkGenericAttribute *sourceNormals=0;
  ...
  svtkPolyData* defaultSource = nullptr;
  ...
  {
    svtkDebugMacro("No attributes, nothing to do.");
    return 1;
  }
  if (this->InputScalarsSelection != nullptr)
  {
    ...
    if (attrib != -1)
    {
      ...
      {
        inScalars = nullptr;
        svtkDebugMacro("The attribute is not a scalar.");
      }
    }
    else
    {
      svtkDebugMacro("No scalar attribute.");
    }
  }
  if (this->InputVectorsSelection != nullptr)
  {
    svtkDebugMacro("this->InputVectorsSelection!=0");
    attrib = attributes->FindAttribute(this->InputVectorsSelection);
    svtkDebugMacro("inVectors just set");
    if (attrib != -1)
    {
      ...
      {
        inVectors = nullptr;
        svtkDebugMacro("The attribute is not a vector.");
      }
      else
      {
        svtkDebugMacro("The attribute is a vector.");
      }
    }
    else
    {
      svtkDebugMacro("No vector attribute.");
    }
  }
  else
  {
    svtkDebugMacro("No input vector selection.");
  }
  ...
  if (this->InputNormalsSelection != nullptr)
  {
    ...
    if (attrib != -1)
    {
      ...
      {
        inNormals = nullptr;
        svtkDebugMacro("The attribute is not a normal vector.");
      }
    }
    else
    {
      svtkDebugMacro("No normal attribute.");
    }
  }
...
#if 0
  svtkDataArray* temp = 0;
  if (pd)
  {
    temp = pd->GetArray(svtkDataSetAttributes::GhostArrayName());
  }
  if ( (!temp) || (temp->GetDataType() != SVTK_UNSIGNED_CHAR)
  ...
  {
    inGhostLevels = ((svtkUnsignedCharArray*)temp)->GetPointer(0);
  }
#endif
  ...
  if (!this->GetSource(0))
  {
    defaultSource = svtkPolyData::New();
  ...
  //  svtkGenericAdaptorCell *acell=0;
  //  svtkCellIterator *it=input->NewVertexIterator();
  svtkGenericPointIterator* it = input->NewPointIterator();
  it->Begin();
  while (!it->IsAtEnd())
  {
...
int svtkGenericGlyph3DFilter::RequestInformation(svtkInformation* svtkNotUsed(request),
...
void svtkGenericGlyph3DFilter::SetSourceData(int id, svtkPolyData* pd)
{
...
svtkPolyData* svtkGenericGlyph3DFilter::GetSource(int id)
{
...
//-----------------------------------------------------------------------------
void svtkGenericGlyph3DFilter::PrintSelf(ostream& os, svtkIndent indent)
{
...
//-----------------------------------------------------------------------------
int svtkGenericGlyph3DFilter::RequestUpdateExtent(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  // get the info objects
  svtkInformation* inInfo = inputVector[0]->GetInformationObject(0);
...
//----------------------------------------------------------------------------
int svtkGenericGlyph3DFilter ::FillInputPortInformation(int port, svtkInformation* info)
{
