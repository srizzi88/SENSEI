...
#ifdef ENABLE_IO
//------------------------------------------------------------------------------
void WriteGrid(svtkUniformGrid* grid, const std::string& prefix)
{
  assert("pre: grid is nullptr" && (grid != nullptr));

  svtkXMLImageDataWriter* writer = svtkXMLImageDataWriter::New();
...
//------------------------------------------------------------------------------
void WriteAMR(svtkOverlappingAMR* amr, const std::string& prefix)
{
#ifdef ENABLE_IO
  std::ostringstream oss;
  oss.clear();
...
//------------------------------------------------------------------------------
svtkUniformGrid* GetGrid(double gridOrigin[3], double h[3], int ndim[3])
{
...
//------------------------------------------------------------------------------
svtkUniformGrid* RefinePatch(
  svtkUniformGrid* vgrid, int level, int dim, int patchExtent[6], int ratio)
{
...
//------------------------------------------------------------------------------
void Get2DAMRData(svtkOverlappingAMR* amrData, int ratio)
{
  ...
  svtkUniformGrid* vgrid = GetGrid(const_cast<double*>(origin), const_cast<double*>(h), vdim);
...
//------------------------------------------------------------------------------
void Get3DAMRData(svtkOverlappingAMR* amrData, int ratio)
{
  assert("pre: input AMR Data is nullptr" && (amrData != nullptr));
  assert("pre: input AMR Data is nullptr" && (ratio >= 2));
...
//------------------------------------------------------------------------------
void RegisterGrids(
  svtkOverlappingAMR* amr, int dim, int ratio, svtkStructuredAMRGridConnectivity* gridConnectivity)
{
...
//------------------------------------------------------------------------------
void GetGhostedAMRData(svtkOverlappingAMR* amr, svtkStructuredAMRGridConnectivity* amrConnectivity,
  svtkOverlappingAMR* ghostedAMR)
{
...
int Test2DAMR(const int ratio)
{
  ...
  svtkOverlappingAMR* amr = svtkOverlappingAMR::New();
  ...
  svtkStructuredAMRGridConnectivity* amrGridConnectivity = svtkStructuredAMRGridConnectivity::New();
...
//------------------------------------------------------------------------------
int TestSimpleAMRGridConnectivity(int svtkNotUsed(argc), char* argv[])
{
  int rc = 0;
  int dim = atoi(argv[1]);
