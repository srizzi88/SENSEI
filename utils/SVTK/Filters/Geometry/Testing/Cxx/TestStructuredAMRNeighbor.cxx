...
=========================================================================*/
// .NAME TestStructuredAMRNeighbor.cxx -- Test svtkStructuredAMRNeighbor
//
// .SECTION Description
...
int TestParentNeighbor()
{
  ...
  int relationship = svtkStructuredAMRNeighbor::PARENT;
  ...
  int gridExtent[6];
  int neiExtent[6];
  KMIN(wholeExtent) = KMAX(wholeExtent) = KMIN(gridExtent) = KMAX(gridExtent) = KMIN(neiExtent) =
    KMAX(neiExtent) = 0;

  int gridLevel = 1;
  int neiLevel = 0;
  int orientation[3];
  orientation[2] = svtkStructuredNeighbor::UNDEFINED;

  int gridOverlap[6];
  int neiOverlap[6];
  KMIN(gridOverlap) = KMAX(gridOverlap) = KMIN(neiOverlap) = KMAX(neiOverlap) = 0;

  int ExpectedSendExtent[6];
  int ExpectedRcvExtent[6];
  KMIN(ExpectedSendExtent) = KMAX(ExpectedSendExtent) = KMIN(ExpectedRcvExtent) =
    KMAX(ExpectedRcvExtent) = 0;
  ...
  orientation[0] = orientation[1] = svtkStructuredNeighbor::SUBSET_BOTH;
  ...
  amrnei.ComputeSendAndReceiveExtent(gridExtent, gridExtent, neiExtent, wholeExtent, NG);
...
int TestPartiallyOverlappingParent()
{
  ...
  int relationship = svtkStructuredAMRNeighbor::PARTIALLY_OVERLAPPING_PARENT;
  ...
  // STEP 3: Setup orientation tuple
  orientation[0] = svtkStructuredNeighbor::SUBSET_LO;
  orientation[1] = svtkStructuredNeighbor::SUBSET_BOTH;

  // STEP 4: Setup expected send/rcv extents
  IMIN(ExpectedSendExtent) = JMIN(ExpectedSendExtent) = 4;
  IMAX(ExpectedSendExtent) = 7;
  JMAX(ExpectedSendExtent) = 10;

  IMIN(ExpectedRcvExtent) = JMIN(ExpectedRcvExtent) = 1;
  IMAX(ExpectedRcvExtent) = 3;
  JMAX(ExpectedRcvExtent) = 6;
...
int TestChildNeighbor()
{
  ...
  int relationship = svtkStructuredAMRNeighbor::CHILD;
  ...
  // STEP 3: Setup orientation tuple
  orientation[0] = orientation[1] = svtkStructuredNeighbor::SUPERSET;
...
int TestPartiallyOverlappingChild()
{
  ...
  int relationship = svtkStructuredAMRNeighbor::PARTIALLY_OVERLAPPING_CHILD;
  ...
  // STEP 3: Setup orientation tuple
  orientation[0] = svtkStructuredNeighbor::SUBSET_HI;
  orientation[1] = svtkStructuredNeighbor::SUPERSET;

  // STEP 4: Setup expected send/rcv extents
  IMIN(ExpectedSendExtent) = JMIN(ExpectedSendExtent) = 1;
  IMAX(ExpectedSendExtent) = 3;
  JMAX(ExpectedSendExtent) = 6;

  IMIN(ExpectedRcvExtent) = JMIN(ExpectedRcvExtent) = 4;
  IMAX(ExpectedRcvExtent) = 7;
  JMAX(ExpectedRcvExtent) = 10;
...
int TestSameLevelSibling()
{
  ...
  int relationship = svtkStructuredAMRNeighbor::SAME_LEVEL_SIBLING;
  ...
  // STEP 3: Setup orientation tuple
  orientation[0] = svtkStructuredNeighbor::LO;
  orientation[1] = svtkStructuredNeighbor::SUBSET_LO;

  // STEP 4: Setup expected send/rcv extents
  IMIN(ExpectedSendExtent) = 10;
  JMIN(ExpectedSendExtent) = 8;
  IMAX(ExpectedSendExtent) = 11;
  JMAX(ExpectedSendExtent) = 11;

  IMIN(ExpectedRcvExtent) = 9;
  JMIN(ExpectedRcvExtent) = 7;
  IMAX(ExpectedRcvExtent) = JMAX(ExpectedRcvExtent) = 10;
...
int TestCoarseToFineNeighbor()
{
  ...
  int relationship = svtkStructuredAMRNeighbor::COARSE_TO_FINE_SIBLING;
  ...
  // STEP 3: Setup orientation tuple
  orientation[0] = svtkStructuredNeighbor::HI;
  orientation[1] = svtkStructuredNeighbor::ONE_TO_ONE;

  // STEP 4: Setup expected send/rcv extents
  IMIN(ExpectedSendExtent) = 1;
  JMIN(ExpectedSendExtent) = 0;
  IMAX(ExpectedSendExtent) = 2;
  JMAX(ExpectedSendExtent) = 2;

  IMIN(ExpectedRcvExtent) = 4;
  JMIN(ExpectedRcvExtent) = 0;
  IMAX(ExpectedRcvExtent) = 5;
  JMAX(ExpectedRcvExtent) = 4;
...
int TestFineToCoarseNeighbor()
{
  ...
  int relationship = svtkStructuredAMRNeighbor::FINE_TO_COARSE_SIBLING;
  ...
  // STEP 4: Setup expected send/rcv extents
  IMIN(ExpectedSendExtent) = 4;
  JMIN(ExpectedSendExtent) = 0;
  IMAX(ExpectedSendExtent) = 5;
  JMAX(ExpectedSendExtent) = 4;

  IMIN(ExpectedRcvExtent) = 1;
  JMIN(ExpectedRcvExtent) = 0;
  IMAX(ExpectedRcvExtent) = 2;
  JMAX(ExpectedRcvExtent) = 2;
...
int TestAssignmentOperator()
{
  ...
  svtkStructuredAMRNeighbor B;
  B.GridLevel = B.NeighborLevel = 100;
  B.RelationShip = svtkStructuredAMRNeighbor::SAME_LEVEL_SIBLING;
  ...
  if (B.RelationShip != svtkStructuredAMRNeighbor::UNDEFINED)
  {
    ++rc;
  }
  return (rc);
...
int TestGetRelationShipString()
{
  int rc = 0;
  svtkStructuredAMRNeighbor A;
  A.RelationShip = svtkStructuredAMRNeighbor::PARENT;
  ...
  A.RelationShip = svtkStructuredAMRNeighbor::PARTIALLY_OVERLAPPING_PARENT;
  ...
  A.RelationShip = svtkStructuredAMRNeighbor::CHILD;
  ...
  A.RelationShip = svtkStructuredAMRNeighbor::PARTIALLY_OVERLAPPING_CHILD;
  ...
  A.RelationShip = svtkStructuredAMRNeighbor::SAME_LEVEL_SIBLING;
  ...
  A.RelationShip = svtkStructuredAMRNeighbor::COARSE_TO_FINE_SIBLING;
  ...
  A.RelationShip = svtkStructuredAMRNeighbor::FINE_TO_COARSE_SIBLING;
