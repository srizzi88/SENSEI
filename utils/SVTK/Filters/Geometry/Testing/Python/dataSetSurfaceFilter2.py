...
# with svtkIdTypeArray and svtkFloatArray point data and svtkIdTypeArray cell data.
#
# When svtkDataSetSurfaceFilter has sub-division level < 2 there is no
# interpolation being done so the point-data idtype array is expected to be copied

class TestDataSetSurfaceFilter(Testing.svtkTest):
    def setUp(self):
        # corner-points (9)
        ...
        pts=svtk.svtkPoints()
        pts.Allocate(21)
        self.point_ids=svtk.svtkIdTypeArray()
        self.point_ids.SetName('point-ids')
        self.point_values=svtk.svtkFloatArray()
        ...
        midnodes=[(9+0,15+1,9+2,15+0),
                  ...
                  (9+2,15+4,9+4,15+3),
                  (9+3,15+5,9+5,15+4)]
        self.ug=svtk.svtkUnstructuredGrid()
        self.ug.SetPoints(pts)
        self.ug.Allocate(4)
        self.cell_ids=svtk.svtkIdTypeArray()
        self.cell_ids.SetName('cell-ids')
        i=1
        for (c,m) in zip(corners,midnodes):
            self.ug.InsertNextCell(svtk.SVTK_QUADRATIC_QUAD,8,(c+m))
