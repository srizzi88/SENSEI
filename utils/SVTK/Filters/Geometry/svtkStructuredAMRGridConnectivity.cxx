/*=========================================================================
 ...
 Module:    svtkStructuredAMRGridConnectivity.cxx
 ...
 =========================================================================*/
#include "svtkStructuredAMRGridConnectivity.h"
...
svtkStandardNewMacro(svtkStructuredAMRGridConnectivity);

//-----------------------------------------------------------------------------
svtkStructuredAMRGridConnectivity::svtkStructuredAMRGridConnectivity()
{
  ...
  IMIN(this->WholeExtent) = JMIN(this->WholeExtent) = KMIN(this->WholeExtent) = SVTK_INT_MAX;

  IMAX(this->WholeExtent) = JMAX(this->WholeExtent) = KMAX(this->WholeExtent) = SVTK_INT_MIN;
}
...
//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::PrintSelf(std::ostream& os, svtkIndent indent)
{
...
//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::Initialize(
...
//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::SetNumberOfGrids(const unsigned int N)
{
  if (N == 0)
  {
    svtkErrorMacro("Number of grids cannot be 0.");
    return;
  }
...
//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::SetBlockTopology(const int gridID)
{
...
//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::ComputeNeighbors()
{
...
//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::GetGhostedExtent(const int gridID, int ext[6])
{
...
//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::SetGhostedExtent(const int gridID, int ext[6])
{
  assert("pre: grid ID is out-of-bounds!" && (gridID >= 0) &&
    (gridID < static_cast<int>(this->NumberOfGrids)));
  assert("pre: ghosted-extents vector has not been allocated" &&
...
//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::CreateGhostLayers(const int N)
{
  if (N == 0)
  {
    svtkWarningMacro("N=0 ghost layers requested! No ghost layers will be created");
    return;
  }
...
//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::InitializeGhostData(const int gridID)
{
  assert("pre: gridID is out-of-bounds!" && (gridID >= 0) &&
    (gridID < static_cast<int>(this->NumberOfGrids)));
  ...
  int numNodes = svtkStructuredData::GetNumberOfPoints(ghostedExtent, this->DataDescription);
  int numCells = svtkStructuredData::GetNumberOfCells(ghostedExtent, this->DataDescription);
  ...
  if (this->GetNodeCentered())
  {
    ...
    this->GhostedGridPointData[gridID] = svtkPointData::New();
    svtkPointData* PD = this->GridPointData[gridID];
    for (int array = 0; array < PD->GetNumberOfArrays(); ++array)
    {
      int dataType = PD->GetArray(array)->GetDataType();
      svtkDataArray* dataArray = svtkDataArray::CreateDataArray(dataType);
  ...
  if (this->GetCellCentered())
  {
    ...
    this->GhostedGridCellData[gridID] = svtkCellData::New();
    svtkCellData* CD = this->GridCellData[gridID];
...
//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::TransferRegisteredDataToGhostedData(const int gridID)
{
  assert("pre: grid ID is out-of-bounds!" && (gridID >= 0) &&
    (gridID < static_cast<int>(this->NumberOfGrids)));
  ...
  svtkStructuredData::GetCellExtentFromPointExtent(
    ...
    for (int j = JMIN(registeredExtent); j <= JMAX(registeredExtent); ++j)
    {
      for (int k = KMIN(registeredExtent); k <= KMAX(registeredExtent); ++k)
      {
        ...
        if (this->GetNodeCentered())
        {
          ...
          svtkIdType sourcePntIdx = svtkStructuredData::ComputePointIdForExtent(
            registeredExtent, ijk, this->DataDescription);

          // Compute the target index to the ghosted data
          svtkIdType targetPntIdx =
            svtkStructuredData::ComputePointIdForExtent(ghostedExtent, ijk, this->DataDescription);
        ...
        if (this->IsNodeWithinExtent(i, j, k, registeredCellExtent))
        {
          ...
          // a point id.
          svtkIdType sourceCellIdx = svtkStructuredData::ComputePointIdForExtent(
            registeredCellExtent, ijk, this->DataDescription);

          // Compute the target cell idx. Note, since we are passing to
          // ComputePointIdForExtent a cell extent, this is a cell id, not
          // a point id.
          svtkIdType targetCellIdx = svtkStructuredData::ComputePointIdForExtent(
...
//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::TransferLocalNodeCenteredNeighborData(
  const int svtkNotUsed(gridID), svtkStructuredAMRNeighbor& svtkNotUsed(nei))
{
  svtkErrorMacro("Node-centered AMR datasets are currently not supported!");
}

//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::GetLocalCellCentersAtSameLevel(
  const int gridID, svtkStructuredAMRNeighbor& nei)
{
  ...
  int RcvCellExtent[6];
  svtkStructuredData::GetCellExtentFromPointExtent(const_cast<int*>(nei.RcvExtent), RcvCellExtent);
    ...
    for (int j = JMIN(RcvCellExtent); j <= JMAX(RcvCellExtent); ++j)
    {
      for (int k = KMIN(RcvCellExtent); k <= KMAX(RcvCellExtent); ++k)
      {
          ...
          !this->IsNodeWithinExtent(i, j, k, RegisteredGridCellExtent))
        {
          ...
          svtkIdType sourceIdx = svtkStructuredData::ComputePointIdForExtent(
            NeighborCellExtent, ijk, this->DataDescription);

          svtkIdType targetIdx = svtkStructuredData::ComputePointIdForExtent(
            GhostedCellExtent, ijk, this->DataDescription);
...
//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::GetLocalCellCentersFromCoarserLevel(
  const int gridID, svtkStructuredAMRNeighbor& nei)
{
  ...
  int rcvDataDescription =
    svtkStructuredData::GetDataDescriptionFromExtent(const_cast<int*>(nei.RcvExtent));
  int RcvCellExtent[6];
  svtkStructuredData::GetCellExtentFromPointExtent(const_cast<int*>(nei.RcvExtent), RcvCellExtent);
...
//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::GetLocalCellCentersFromFinerLevel(
  const int gridID, svtkStructuredAMRNeighbor& nei)
{
  ...
  // STEP 4: Get RcvCell extent
  //  int rcvDataDescription =
  //      svtkStructuredData::GetDataDescriptionFromExtent(
  ...
  int GridRcvDataDescription = svtkStructuredData::GetDataDescriptionFromExtent(GridRcvExtent);
  //  assert("pre: mismatching data description" &&
  //          (rcvDataDescription == GridRcvDataDescription) );
  int GridRcvCellExtent[6];
  svtkStructuredData::GetCellExtentFromPointExtent(GridRcvExtent, GridRcvCellExtent);
    ...
    for (int j = JMIN(GridRcvCellExtent); j <= JMAX(GridRcvCellExtent); ++j)
    {
      for (int k = KMIN(GridRcvCellExtent); k <= KMAX(GridRcvCellExtent); ++k)
      {
          ...
          this->IsNodeWithinExtent(i, j, k, GhostedCellExtent))
        {
          ...
          if (this->CellCenteredDonorLevel[gridID][targetIdx] < nei.NeighborLevel)
          {
            std::vector<svtkIdType> sourceIds;
            ...
            {
              svtkWarningMacro("Empty list of sources!");
            }
...
//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::TransferLocalCellCenteredNeighborData(
  const int gridID, svtkStructuredAMRNeighbor& nei)
{
  ...
  if (nei.RelationShip == svtkStructuredAMRNeighbor::CHILD)
  {
  ...
  int GhostedGridExtent[6];
  this->GetGhostedExtent(gridID, GhostedGridExtent);
  int numCells = svtkStructuredData::GetNumberOfCells(GhostedGridExtent, this->DataDescription);
...
//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::TransferLocalNeighborData(
  const int gridID, svtkStructuredAMRNeighbor& nei)
{
  if (this->GetNodeCentered())
  {
...
//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::TransferGhostDataFromNeighbors(const int gridID)
{
  // Sanity check
  assert("pre: gridID is out-of-bounds!" && (gridID >= 0) &&
    (gridID < static_cast<int>(this->NumberOfGrids)));
...
//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::AverageFieldData(svtkFieldData* source, svtkIdType* sourceIds,
  const int N, svtkFieldData* target, svtkIdType targetIdx)
{
  ...
  {
    // Get source array
    svtkDataArray* sourceArray = source->GetArray(arrayIdx);
    assert("ERROR: encountered nullptr source array" && (sourceArray != nullptr));

    // Get target array
    svtkDataArray* targetArray = target->GetArray(arrayIdx);
    ...
    for (int comp = 0; comp < numComponents; ++comp)
    {
      ...
      {
        svtkIdType sourceIdx = sourceIds[src];
...
//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::CopyFieldData(
  svtkFieldData* source, svtkIdType sourceIdx, svtkFieldData* target, svtkIdType targetIdx)
{
  assert("pre: source field data is nullptr!" && (source != nullptr));
  assert("pre: target field data is nullptr!" && (target != nullptr));
  assert("pre: source number of arrays does not match target!" &&
...
//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::ComputeNeighborSendAndRcvExtent(
...
//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::CreateGhostedMaskArrays(const int gridID)
{
  assert("pre: gridID is out-of-bounds!" && (gridID >= 0) &&
    (gridID < static_cast<int>(this->NumberOfGrids)));
  ...
  {
    this->GhostedPointGhostArray[gridID] = svtkUnsignedCharArray::New();
  }
  else
  {
  ...
  {
    this->GhostedCellGhostArray[gridID] = svtkUnsignedCharArray::New();
  }
  else
  {
  ...
  int numNodes = svtkStructuredData::GetNumberOfPoints(ghostExtent, this->DataDescription);
  int numCells = svtkStructuredData::GetNumberOfCells(ghostExtent, this->DataDescription);
  ...
  for (int i = IMIN(ghostExtent); i <= IMAX(ghostExtent); ++i)
  {
    for (int j = JMIN(ghostExtent); j <= JMAX(ghostExtent); ++j)
    {
      for (int k = KMIN(ghostExtent); k <= KMAX(ghostExtent); ++k)
      {
        ...
        ijk[2] = k;
        svtkIdType pntIdx =
          svtkStructuredData::ComputePointIdForExtent(ghostExtent, ijk, this->DataDescription);

        if (this->IsNodeWithinExtent(i, j, k, registeredGridExtent))
        {
          // Copy data from registered grid
          svtkIdType srcIdx = svtkStructuredData::ComputePointIdForExtent(
        ...
        else
        {
          ...
          unsigned char p = 0;
          p |= svtkDataSetAttributes::DUPLICATEPOINT;
  ...
  for (int i = IMIN(ghostCellExtent); i <= IMAX(ghostCellExtent); ++i)
  {
    for (int j = JMIN(ghostCellExtent); j <= JMAX(ghostCellExtent); ++j)
    {
      for (int k = KMIN(ghostCellExtent); k <= KMAX(ghostCellExtent); ++k)
      {
        ...
        ijk[2] = k;
        svtkIdType cellIdx =
          svtkStructuredData::ComputePointIdForExtent(ghostCellExtent, ijk, this->DataDescription);

        if (this->IsNodeWithinExtent(i, j, k, registeredCellExtent))
        {
          svtkIdType srcCellIdx = svtkStructuredData::ComputePointIdForExtent(
            registeredCellExtent, ijk, this->DataDescription);
          unsigned char p = 0;
        ...
        {
          unsigned char p = 0;
          p |= svtkDataSetAttributes::DUPLICATECELL;
          this->GhostedCellGhostArray[gridID]->SetValue(cellIdx, p);
        }
      } // END for all k
    }   // END for all j
...
//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::CreateGhostedExtent(const int gridId, const int N)
{
  assert("pre: gridId is out-of-bounds!" && (gridId >= 0) &&
    (gridId < static_cast<int>(this->NumberOfGrids)));
  assert("pre: ghosted extents vector has not been allocated!" &&
...
//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::FillCellsGhostArray(
  const int gridId, svtkUnsignedCharArray* cellsArray)
{
  ...
  int ext[6];
  this->GetGridExtent(gridId, ext);
  int dataDescription = svtkStructuredData::GetDataDescription(ext);
  int numCells = svtkStructuredData::GetNumberOfCells(ext, dataDescription);
  if (numCells != cellsArray->GetNumberOfTuples())
  {
    svtkErrorMacro("CellsArray may not be allocated properly!");
    return;
  }

  // STEP 1: Get the cell extent
  int cellext[6];
  svtkStructuredData::GetCellExtentFromPointExtent(ext, cellext, dataDescription);
  ...
  for (int i = IMIN(cellext); i <= IMAX(cellext); ++i)
  {
    for (int j = JMIN(cellext); j <= JMAX(cellext); ++j)
    {
      for (int k = KMIN(cellext); k <= KMAX(cellext); ++k)
      {
        ...
        svtkIdType idx = svtkStructuredData::ComputePointIdForExtent(cellext, ijk, dataDescription);
  ...
  for (int nei = 0; nei < numNeis; ++nei)
  {
    ...
    if ((rel == svtkStructuredAMRNeighbor::CHILD) ||
      (rel == svtkStructuredAMRNeighbor::PARTIALLY_OVERLAPPING_CHILD))
    {
      ...
      for (int i = IMIN(overlapCellExt); i <= IMAX(overlapCellExt); ++i)
      {
        for (int j = JMIN(overlapCellExt); j <= JMAX(overlapCellExt); ++j)
        {
          for (int k = KMIN(overlapCellExt); k <= KMAX(overlapCellExt); ++k)
          {
            ...
            ijk[2] = k;
            svtkIdType idx =
              svtkStructuredData::ComputePointIdForExtent(cellext, ijk, dataDescription);
            assert("pre: cell index is out-of-bounds!" && (idx < numCells));
            ghostArrayPtr[idx] |= svtkDataSetAttributes::REFINEDCELL;
          } // END for all k
        }   // END for all j
      }     // END for all i
...
//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::MarkNodeProperty(const int gridId, const int i, const int j,
...
//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::FillNodesGhostArray(
  const int gridId, svtkUnsignedCharArray* nodesArray)
{
  assert("pre: grid index is out-of-bounds" && (gridId >= 0) &&
    (gridId < static_cast<int>(this->NumberOfGrids)));
  ...
  int ext[6];
  this->GetGridExtent(gridId, ext);
  int gridDataDescription = svtkStructuredData::GetDataDescriptionFromExtent(ext);
  ...
  for (int i = IMIN(ext); i <= IMAX(ext); ++i)
  {
    for (int j = JMIN(ext); j <= JMAX(ext); ++j)
    {
      for (int k = KMIN(ext); k <= KMAX(ext); ++k)
      {
        ...
        ijk[1] = j;
        ijk[2] = k;
        svtkIdType idx = svtkStructuredData::ComputePointIdForExtent(ext, ijk, gridDataDescription);
...
//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::GetNodeOrientation(
...
//-----------------------------------------------------------------------------
bool svtkStructuredAMRGridConnectivity::IsNodeWithinExtent(
...
//-----------------------------------------------------------------------------
bool svtkStructuredAMRGridConnectivity::IsNodeOnSharedBoundary(
...
//-----------------------------------------------------------------------------
bool svtkStructuredAMRGridConnectivity::IsNodeOnBoundaryOfExtent(
...
//-----------------------------------------------------------------------------
bool svtkStructuredAMRGridConnectivity::IsNodeInterior(
  const int i, const int j, const int k, int GridExtent[6])
{
  bool status = false;
  switch (this->DataDescription)
...
//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::FillGhostArrays(
...
//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::RegisterGrid(const int gridIdx, const int level,
  int extents[6], svtkUnsignedCharArray* nodesGhostArray, svtkUnsignedCharArray* cellGhostArray,
  svtkPointData* pointData, svtkCellData* cellData, svtkPoints* gridNodes)
{
...
//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::RegisterGrid(const int gridIdx, const int level,
  const int refinementRatio, int extents[6], svtkUnsignedCharArray* nodesGhostArray,
  svtkUnsignedCharArray* cellGhostArray, svtkPointData* pointData, svtkCellData* cellData,
  svtkPoints* gridNodes)
{
...
//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::EstablishNeighbors(const int i, const int j)
{
  ...
  else
  {
    ...
    svtkErrorMacro("Code should not reach here!");
  }
  ...
  // STEP 4: Use svtkStructuredGridConnectivity to establish neighbors
  ...
  if (gridConnectivity->GetNumberOfNeighbors(0) != 0)
  {
    ...
    svtkStructuredNeighbor nei0 = gridConnectivity->GetGridNeighbor(0, 0);
    svtkStructuredNeighbor nei1 = gridConnectivity->GetGridNeighbor(1, 0);

    svtkStructuredAMRNeighbor amrNei0 = this->GetAMRNeighbor(
      i, this->GridLevels[i], ext1, j, this->GridLevels[j], ext2, normalizedLevel, levelDiff, nei0);
    this->Neighbors[i].push_back(amrNei0);

    svtkStructuredAMRNeighbor amrNei1 = this->GetAMRNeighbor(
...
//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::ComputeAMRNeighborOverlapExtents(const int iLevel,
  const int jLevel, const int normalizedLevel, const svtkStructuredNeighbor& nei, int orient[3],
...
//-----------------------------------------------------------------------------
svtkStructuredAMRNeighbor svtkStructuredAMRGridConnectivity::GetAMRNeighbor(const int svtkNotUsed(i),
  const int iLevel, int next1[6], const int j, const int jLevel, int next2[6],
  const int normalizedLevel, const int levelDiff, svtkStructuredNeighbor& nei)
{
  // STEP 0: Get the overlap extent data-description & dimension
  int overlapDim = svtkStructuredData::GetDataDimension(nei.OverlapExtent);
  ...
  int relationShip = svtkStructuredAMRNeighbor::UNDEFINED;
  if (iLevel == jLevel)
  {
    ...
    relationShip = svtkStructuredAMRNeighbor::SAME_LEVEL_SIBLING;
  } // END if
  else if (iLevel < jLevel)
  {
    if (overlapDim == this->DataDimension - 1)
    {
      ...
      relationShip = svtkStructuredAMRNeighbor::COARSE_TO_FINE_SIBLING;
    } // END if
    else
    {
      ...
      if (levelDiff <= 1)
      {
        ...
        {
          relationShip = svtkStructuredAMRNeighbor::CHILD;
        }
        else
        {
          relationShip = svtkStructuredAMRNeighbor::PARTIALLY_OVERLAPPING_CHILD;
        }
      } // END if levelDiff
    }   // END else
  }     // END else if
  else
  {
    if (overlapDim == this->DataDimension - 1)
    {
      ...
      relationShip = svtkStructuredAMRNeighbor::FINE_TO_COARSE_SIBLING;
    } // END if
    else
    {
      ...
      if (levelDiff <= 1)
      {
        ...
        {
          relationShip = svtkStructuredAMRNeighbor::PARENT;
        }
        else
        {
          relationShip = svtkStructuredAMRNeighbor::PARTIALLY_OVERLAPPING_PARENT;
        }
      } // END if levelDiff
    }   // END else
  }     // END else
  ...
  svtkStructuredAMRNeighbor amrNei(
...
//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::GetOrientationVector(
  const int dataDescription, int orient[3], int& ndim)
{
  switch (dataDescription)
  {
    ...
    case SVTK_XYZ_GRID:
      ...
      break;
    default:
      svtkErrorMacro("Undefined data-description!");
  }
}

//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::GetCoarsenedExtent(
...
//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::CoarsenExtent(
...
//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::GetRefinedExtent(
  const int gridIdx, int fromLevel, int toLevel, int ext[6])
{
  assert("pre: grid index is out-of-bounds!" && (gridIdx >= 0) &&
    (gridIdx < static_cast<int>(this->NumberOfGrids)));
...
//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::RefineExtent(
  int orient[3], int ndim, int fromLevel, int toLevel, int ext[6])
{
  assert("pre: ndim must be either 1, 2 or 3" && (ndim > 0 && ndim <= 3));
...
//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::GetCellRefinedExtent(int orient[3], int ndim, const int i,
...
//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::GetWholeExtentAtLevel(const int level, int ext[6])
{
...
//-----------------------------------------------------------------------------
void svtkStructuredAMRGridConnectivity::ComputeWholeExtent()
{
  if (!this->LevelExists(0))
  {
    svtkErrorMacro("AMR dataset has no level 0!\n");
    return;
  }
  ...
  this->DataDescription = svtkStructuredData::GetDataDescriptionFromExtent(this->WholeExtent);
  this->DataDimension = svtkStructuredData::GetDataDimension(this->DataDescription);
}
