/*=========================================================================
 ...
 Module:    svtkStructuredGridConnectivity.cxx
...
svtkStandardNewMacro(svtkStructuredGridConnectivity);
...
//------------------------------------------------------------------------------
svtkStructuredGridConnectivity::svtkStructuredGridConnectivity()
{
...
//------------------------------------------------------------------------------
void svtkStructuredGridConnectivity::PrintSelf(std::ostream& os, svtkIndent indent)
{
...
//------------------------------------------------------------------------------
void svtkStructuredGridConnectivity::RegisterGrid(const int gridID, int ext[6],
  svtkUnsignedCharArray* nodesGhostArray, svtkUnsignedCharArray* cellGhostArray,
...
//------------------------------------------------------------------------------
void svtkStructuredGridConnectivity::AcquireDataDescription()
{
  ...
  svtkStructuredExtent::GetDimensions(this->WholeExtent, dims);

  this->DataDescription = svtkStructuredData::GetDataDescription(dims);
  ...
  assert("pre: grid description cannot be empty" && (this->DataDescription != SVTK_EMPTY));
}

//------------------------------------------------------------------------------
svtkStructuredNeighbor svtkStructuredGridConnectivity::GetGridNeighbor(
...
//------------------------------------------------------------------------------
svtkIdList* svtkStructuredGridConnectivity::GetNeighbors(const int gridID, int* extents)
{
  ...
  svtkIdList* neiList = svtkIdList::New();
  neiList->SetNumberOfIds(N);

  unsigned int nei = 0;
  for (; nei < this->Neighbors[gridID].size(); ++nei)
  {
    svtkIdType neiId = this->Neighbors[gridID][nei].NeighborID;
...
//------------------------------------------------------------------------------
void svtkStructuredGridConnectivity::ComputeNeighbors()
{
  ...
  for (unsigned int i = 0; i < this->NumberOfGrids; ++i)
  {
    ...
    // svtkPStructuredGridConnectivity without any modification.
...
//------------------------------------------------------------------------------
void svtkStructuredGridConnectivity::SearchNeighbors(
  const int gridID, const int i, const int j, const int k, svtkIdList* neiList)
{
  ...
  for (unsigned int nei = 0; nei < this->Neighbors[gridID].size(); ++nei)
  {
    svtkStructuredNeighbor* myNei = &this->Neighbors[gridID][nei];
...
//------------------------------------------------------------------------------
void svtkStructuredGridConnectivity::MarkCellProperty(
  unsigned char& pfield, unsigned char* nodeGhostFields, const int numNodes)
{
  ...
  {
    if (nodeGhostFields[i] & svtkDataSetAttributes::DUPLICATEPOINT)
    {
      pfield |= svtkDataSetAttributes::DUPLICATECELL;
      return;
    }
  } // END for all nodes
}
...
//------------------------------------------------------------------------------
void svtkStructuredGridConnectivity::MarkNodeProperty(const int gridID, const int i, const int j,
  ...
  if (!this->IsNodeInterior(i, j, k, realExtent))
  {
    ...
    if (this->IsNodeOnSharedBoundary(gridID, realExtent, i, j, k))
    {
      ...
      if (neiList->GetNumberOfIds() > 0)
      {
        ...
        for (svtkIdType nei = 0; nei < neiList->GetNumberOfIds(); ++nei)
        {
          svtkIdType neiIdx = neiList->GetId(nei);
          ...
          {
            p |= svtkDataSetAttributes::HIDDENPOINT;
            break;
          }
        } // END for all neis
...
//------------------------------------------------------------------------------
void svtkStructuredGridConnectivity::FillNodesGhostArray(const int gridID, const int dataDescription,
  int GridExtent[6], int RealExtent[6], svtkUnsignedCharArray* nodesArray)
{
  int ijk[3];
  for (int i = GridExtent[0]; i <= GridExtent[1]; ++i)
  {
    for (int j = GridExtent[2]; j <= GridExtent[3]; ++j)
    {
      for (int k = GridExtent[4]; k <= GridExtent[5]; ++k)
      {
        ...
        ijk[2] = k;
        svtkIdType idx =
          svtkStructuredData::ComputePointIdForExtent(GridExtent, ijk, dataDescription);
...
//------------------------------------------------------------------------------
void svtkStructuredGridConnectivity::FillCellsGhostArray(const int dataDescription,
  const int numNodesPerCell, int dims[3], int CellExtent[6], svtkUnsignedCharArray* nodesArray,
  svtkUnsignedCharArray* cellsArray)
{
  ...
  svtkIdList* cellNodeIds = svtkIdList::New();
  ...
  for (int i = CellExtent[0]; i <= CellExtent[1]; ++i)
  {
    for (int j = CellExtent[2]; j <= CellExtent[3]; ++j)
    {
      for (int k = CellExtent[4]; k <= CellExtent[5]; ++k)
      {
        ...
        // cell extent
        svtkIdType idx =
          svtkStructuredData::ComputePointIdForExtent(CellExtent, ijk, dataDescription);

        cellNodeIds->Reset();
        svtkStructuredData::GetCellPoints(idx, cellNodeIds, dataDescription, dims);
        assert(cellNodeIds->GetNumberOfIds() == numNodesPerCell);
        assert(cellNodeGhostFields != nullptr);

        for (int ii = 0; ii < numNodesPerCell; ++ii)
        {
          svtkIdType nodeIdx = cellNodeIds->GetId(ii);
...
//------------------------------------------------------------------------------
void svtkStructuredGridConnectivity::FillGhostArrays(
  const int gridID, svtkUnsignedCharArray* nodesArray, svtkUnsignedCharArray* cellsArray)
{
  ...
  int dataDescription = svtkStructuredData::GetDataDescriptionFromExtent(GridExtent);

  // STEP 3: Get the cell extent
  int CellExtent[6];
  svtkStructuredData::GetCellExtentFromPointExtent(GridExtent, CellExtent, dataDescription);

  // STEP 4: Get the data dimension
  int dim = svtkStructuredData::GetDataDimension(dataDescription);
  assert("pre: data dimensions must be 1, 2 or 3" && (dim >= 1) && (dim <= 3));

  // STEP 5: Get the grid dimensions from the given extent.
  int dims[3];
  svtkStructuredData::GetDimensionsFromExtent(GridExtent, dims);
...
//------------------------------------------------------------------------------
void svtkStructuredGridConnectivity::GetRealExtent(
  const int gridID, int GridExtent[6], int RealExtent[6])
{
  ...
  svtkStructuredExtent::Clamp(RealExtent, this->WholeExtent);
}

//------------------------------------------------------------------------------
bool svtkStructuredGridConnectivity::IsNodeOnSharedBoundary(
...
//------------------------------------------------------------------------------
bool svtkStructuredGridConnectivity::IsGhostNode(
...
//------------------------------------------------------------------------------
bool svtkStructuredGridConnectivity::IsNodeOnBoundary(const int i, const int j, const int k)
{
  return (this->IsNodeOnBoundaryOfExtent(i, j, k, this->WholeExtent));
}

//------------------------------------------------------------------------------
void svtkStructuredGridConnectivity::EstablishNeighbors(const int i, const int j)
{
  assert("pre: i < j" && (i < j));
...
//------------------------------------------------------------------------------
void svtkStructuredGridConnectivity::DetectNeighbors(
  const int i, const int j, int ex1[6], int ex2[6], int orientation[3], int ndim)
{
  ...
  for (int ii = 0; ii < 3; ++ii)
  {
    ...
    iOrientation[ii] = svtkStructuredNeighbor::UNDEFINED;
    jOrientation[ii] = svtkStructuredNeighbor::UNDEFINED;
  }
...
//------------------------------------------------------------------------------
void svtkStructuredGridConnectivity::SetBlockTopology(const int gridID)
{
  int gridExtent[6];
  this->GetGridExtent(gridID, gridExtent);
...
//------------------------------------------------------------------------------
void svtkStructuredGridConnectivity::SetNeighbors(
  const int i, const int j, int i2jOrientation[3], int j2iOrientation[3], int overlapExtent[6])
{
  svtkStructuredNeighbor Ni2j(j, overlapExtent, i2jOrientation);
  svtkStructuredNeighbor Nj2i(i, overlapExtent, j2iOrientation);
...
//------------------------------------------------------------------------------
void svtkStructuredGridConnectivity::PrintExtent(int ex[6])
{
  for (int i = 0; i < 3; ++i)
  {
...
//------------------------------------------------------------------------------
int svtkStructuredGridConnectivity::DoPartialOverlap(int s[2], int S[2], int overlap[2])
{
  if (this->InBounds(s[0], S[0], S[1]) && this->InBounds(s[1], S[0], S[1]))
  {
    overlap[0] = s[0];
    overlap[1] = s[1];
    return SVTK_PARTIAL_OVERLAP;
  }
  ...
  return SVTK_NO_OVERLAP;
}

//------------------------------------------------------------------------------
int svtkStructuredGridConnectivity::PartialOverlap(
...
//------------------------------------------------------------------------------
int svtkStructuredGridConnectivity::IntervalOverlap(int A[2], int B[2], int overlap[2])
{
...
//------------------------------------------------------------------------------
void svtkStructuredGridConnectivity::GetIJKBlockOrientation(
...
//------------------------------------------------------------------------------
void svtkStructuredGridConnectivity::CreateGhostedExtent(const int gridID, const int N)
{
...
//------------------------------------------------------------------------------
void svtkStructuredGridConnectivity::CreateGhostedMaskArrays(const int gridID)
{
  // Sanity check
  assert("pre: gridID is out-of-bounds!" && (gridID >= 0) &&
    (gridID < static_cast<int>(this->NumberOfGrids)));
    ...
    for (int j = ghostedExtent[2]; j <= ghostedExtent[3]; ++j)
    {
      for (int k = ghostedExtent[4]; k <= ghostedExtent[5]; ++k)
      {
        ...
        if (this->IsNodeWithinExtent(i, j, k, gridExtent))
        {
          ...
          svtkIdType srcidx =
            svtkStructuredData::ComputePointIdForExtent(gridExtent, ijk, this->DataDescription);
  ...
  int dim = svtkStructuredData::GetDataDimension(this->DataDescription);
  assert("pre: data dimensions must be 1, 2 or 3" && (dim >= 1) && (dim <= 3));
  ...
  int CellExtent[6];
  svtkStructuredData::GetCellExtentFromPointExtent(ghostedExtent, CellExtent);
...
//------------------------------------------------------------------------------
void svtkStructuredGridConnectivity::AllocatePointData(
  svtkPointData* RPD, const int N, svtkPointData* PD)
{
...
//------------------------------------------------------------------------------
void svtkStructuredGridConnectivity::AllocateCellData(svtkCellData* RCD, const int N, svtkCellData* CD)
{
...
//------------------------------------------------------------------------------
void svtkStructuredGridConnectivity::InitializeGhostData(const int gridID)
{
  // Sanity check
  assert("pre: gridID is out-of-bounds!" && (gridID >= 0) &&
    (gridID < static_cast<int>(this->NumberOfGrids)));
  ...
  int numNodes = svtkStructuredData::GetNumberOfPoints(GhostedGridExtent, this->DataDescription);
  ...
  if (this->GridPoints[gridID] != nullptr)
  {
    ...
    this->GhostedGridPoints[gridID] = svtkPoints::New();
...
//------------------------------------------------------------------------------
void svtkStructuredGridConnectivity::CopyCoordinates(
  svtkPoints* source, svtkIdType sourceIdx, svtkPoints* target, svtkIdType targetIdx)
{
...
//------------------------------------------------------------------------------
void svtkStructuredGridConnectivity::CopyFieldData(
...
//------------------------------------------------------------------------------
void svtkStructuredGridConnectivity::TransferRegisteredDataToGhostedData(const int gridID)
{
  // Sanity check
  assert("pre: gridID is out-of-bounds!" && (gridID >= 0) &&
    (gridID < static_cast<int>(this->NumberOfGrids)));
  ...
  for (int i = GridExtent[0]; i <= GridExtent[1]; ++i)
  {
    for (int j = GridExtent[2]; j <= GridExtent[3]; ++j)
    {
      for (int k = GridExtent[4]; k <= GridExtent[5]; ++k)
      {
        ...
        svtkIdType targetIdx =
          svtkStructuredData::ComputePointIdForExtent(GhostedGridExtent, ijk, this->DataDescription);
        ...
        if (this->IsNodeWithinExtent(i, j, k, GridCellExtent))
        {
          ...
          // a point id.
          svtkIdType sourceCellIdx =
            svtkStructuredData::ComputePointIdForExtent(GridCellExtent, ijk, this->DataDescription);
...
//------------------------------------------------------------------------------
void svtkStructuredGridConnectivity::TransferGhostDataFromNeighbors(const int gridID)
{
  // Sanity check
  assert("pre: gridID is out-of-bounds!" && (gridID >= 0) &&
    (gridID < static_cast<int>(this->NumberOfGrids)));
...
//------------------------------------------------------------------------------
void svtkStructuredGridConnectivity::TransferLocalNeighborData(
  const int gridID, const svtkStructuredNeighbor& Neighbor)
{
  // Sanity check
  assert("pre: gridID is out-of-bounds!" && (gridID >= 0) &&
    (gridID < static_cast<int>(this->NumberOfGrids)));
  ...
  this->GetGhostedGridExtent(gridID, GhostedGridExtent);
  int GhostedGridCellExtent[6];
  svtkStructuredData::GetCellExtentFromPointExtent(GhostedGridExtent, GhostedGridCellExtent);
  ...
  svtkStructuredData::GetCellExtentFromPointExtent(NeighborExtent, NeighborCellExtent);
  ...
  for (int i = Neighbor.RcvExtent[0]; i <= Neighbor.RcvExtent[1]; ++i)
  {
    for (int j = Neighbor.RcvExtent[2]; j <= Neighbor.RcvExtent[3]; ++j)
    {
      for (int k = Neighbor.RcvExtent[4]; k <= Neighbor.RcvExtent[5]; ++k)
      {
        ...
        svtkIdType srcIdx =
          svtkStructuredData::ComputePointIdForExtent(NeighborExtent, ijk, this->DataDescription);
...
//------------------------------------------------------------------------------
void svtkStructuredGridConnectivity::ComputeNeighborSendAndRcvExtent(const int gridID, const int N)
{
  // Sanity check
  assert("pre: gridID is out-of-bounds!" && (gridID >= 0) &&
    (gridID < static_cast<int>(this->NumberOfGrids)));
...
//------------------------------------------------------------------------------
void svtkStructuredGridConnectivity::CreateGhostLayers(const int N)
{
  if (N == 0)
  {
