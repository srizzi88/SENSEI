/*=========================================================================
 ...
 Module:    svtkStructuredGridConnectivity.h
 ...
 *
 *
 *  svtkStructuredGridConnectivity is a concrete instance of svtkObject that
 ...
 *  see svtkPStructuredGridConnectivity.
 *
 * @sa
 *  svtkPStructuredGridConnectivity
 */

#ifndef svtkStructuredGridConnectivity_h
#define svtkStructuredGridConnectivity_h

#define SVTK_NO_OVERLAP 0
#define SVTK_NODE_OVERLAP 1
#define SVTK_EDGE_OVERLAP 2
#define SVTK_PARTIAL_OVERLAP 3

// SVTK include directives
#include "svtkAbstractGridConnectivity.h"
#include "svtkFiltersGeometryModule.h" // For export macro
#include "svtkStructuredData.h"        // For data description definitions
#include "svtkStructuredNeighbor.h"    // For Structured Neighbor object definition
...
class SVTKFILTERSGEOMETRY_EXPORT svtkStructuredGridConnectivity : public svtkAbstractGridConnectivity
{
public:
  static svtkStructuredGridConnectivity* New();
  svtkTypeMacro(svtkStructuredGridConnectivity, svtkAbstractGridConnectivity);
   ...
   */
  svtkGetMacro(DataDimension, int);
  //@}
   ...
   */
  virtual void RegisterGrid(const int gridID, int extents[6], svtkUnsignedCharArray* nodesGhostArray,
   ...
   */
  svtkStructuredNeighbor GetGridNeighbor(const int gridID, const int nei);
   ...
   */
  svtkIdList* GetNeighbors(const int gridID, int* extents);
...
protected:
  svtkStructuredGridConnectivity();
  ~svtkStructuredGridConnectivity() override;
  ...
  void FillNodesGhostArray(const int gridID, const int dataDescription, int GridExtent[6],
    int RealExtent[6], svtkUnsignedCharArray* nodeArray);
  ...
  void FillCellsGhostArray(const int dataDescription, const int numNodesPerCell, int dims[3],
    int CellExtent[6], svtkUnsignedCharArray* nodesArray, svtkUnsignedCharArray* cellsArray);
   ...
   * with.
   */
  void SearchNeighbors(const int gridID, const int i, const int j, const int k, svtkIdList* neiList);
   ...
   * axis, i, j, k. See svtkStructuredNeighbor::NeighborOrientation for a list
   ...
   * topology of the structured domain, e.g., SVTK_XYZ_GRID, SVTK_XY_PLANE, etc.
   */
   ...
   * is given by N.
   */
  void AllocatePointData(svtkPointData* RPD, const int N, svtkPointData* PD);
   ...
   * by N.
   */
  void AllocateCellData(svtkCellData* RCD, const int N, svtkCellData* CD);
   ...
   */
  void TransferLocalNeighborData(const int gridID, const svtkStructuredNeighbor& Neighor);
  ...
  std::vector<std::vector<svtkStructuredNeighbor> > Neighbors;
  std::map<std::pair<int, int>, int> NeighborPair2NeighborListIndex;

private:
  svtkStructuredGridConnectivity(const svtkStructuredGridConnectivity&) = delete;
  void operator=(const svtkStructuredGridConnectivity&) = delete;
};
...
//------------------------------------------------------------------------------
inline int svtkStructuredGridConnectivity::GetNeighborIndex(
...
//------------------------------------------------------------------------------
inline void svtkStructuredGridConnectivity::GetGhostedExtent(
  int* ghostedExtent, int GridExtent[6], const int minIdx, const int maxIdx, const int N)
{
...
//------------------------------------------------------------------------------
inline void svtkStructuredGridConnectivity::SetGhostedGridExtent(const int gridID, int ext[6])
{
...
//------------------------------------------------------------------------------
inline void svtkStructuredGridConnectivity::GetGridExtent(const int gridID, int ext[6])
{
...
//------------------------------------------------------------------------------
inline void svtkStructuredGridConnectivity::GetGhostedGridExtent(const int gridID, int ext[6])
{
  assert("pre: gridID out-of-bounds!" &&
    (gridID >= 0 && gridID < static_cast<int>(this->NumberOfGrids)));
  ...
  if (this->GhostedExtents.size() == 0)
  {
    ext[0] = ext[2] = ext[4] = -1;
    ext[1] = ext[3] = ext[5] = 0;
    svtkErrorMacro("No ghosted extents found for registered grid extends!!!");
    return;
  }
...
//------------------------------------------------------------------------------
inline bool svtkStructuredGridConnectivity::IsNodeOnBoundaryOfExtent(
...
//------------------------------------------------------------------------------
inline bool svtkStructuredGridConnectivity::IsNodeInterior(
...
//------------------------------------------------------------------------------
inline void svtkStructuredGridConnectivity::DetermineNeighborOrientation(
  const int idx, int A[2], int B[2], int overlap[2], int orient[3])
{
  ...
  if (overlap[0] == overlap[1])
  {
    if (A[1] == B[0])
    {
      orient[idx] = svtkStructuredNeighbor::HI;
    }
    else if (A[0] == B[1])
    {
      orient[idx] = svtkStructuredNeighbor::LO;
    }
    else
    {
      orient[idx] = svtkStructuredNeighbor::UNDEFINED;
      assert("ERROR: Code should not reach here!" && false);
    }
  ...
  else if (this->IsSubset(A, B))
  {
    if ((A[0] == B[0]) && (A[1] == B[1]))
    {
      orient[idx] = svtkStructuredNeighbor::ONE_TO_ONE;
    }
    else if (this->StrictlyInsideBounds(A[0], B[0], B[1]) &&
      this->StrictlyInsideBounds(A[1], B[0], B[1]))
    {
      orient[idx] = svtkStructuredNeighbor::SUBSET_BOTH;
    }
    else if (A[0] == B[0])
    {
      orient[idx] = svtkStructuredNeighbor::SUBSET_HI;
    }
    else if (A[1] == B[1])
    {
      orient[idx] = svtkStructuredNeighbor::SUBSET_LO;
    }
    else
    {
      orient[idx] = svtkStructuredNeighbor::UNDEFINED;
  ...
  {
    orient[idx] = svtkStructuredNeighbor::SUPERSET;
  }
...
//------------------------------------------------------------------------------
inline int svtkStructuredGridConnectivity::Get1DOrientation(const int idx, const int ExtentLo,
...
//------------------------------------------------------------------------------
inline bool svtkStructuredGridConnectivity::HasBlockConnection(
...
//------------------------------------------------------------------------------
inline void svtkStructuredGridConnectivity::RemoveBlockConnection(
  const int gridID, const int blockDirection)
{
  // Sanity check
  assert("pre: gridID is out-of-bounds" && (gridID >= 0) &&
...
//------------------------------------------------------------------------------
inline void svtkStructuredGridConnectivity::AddBlockConnection(
  const int gridID, const int blockDirection)
{
  // Sanity check
  assert("pre: gridID is out-of-bounds" && (gridID >= 0) &&
...
//------------------------------------------------------------------------------
inline void svtkStructuredGridConnectivity::ClearBlockConnections(const int gridID)
{
  // Sanity check
  assert("pre: gridID is out-of-bounds" && (gridID >= 0) &&
    (gridID < static_cast<int>(this->NumberOfGrids)));
...
//------------------------------------------------------------------------------
inline int svtkStructuredGridConnectivity::GetNumberOfConnectingBlockFaces(const int gridID)
{
  // Sanity check
  assert("pre: gridID is out-of-bounds" && (gridID >= 0) &&
    (gridID < static_cast<int>(this->NumberOfGrids)));
...
//------------------------------------------------------------------------------
inline void svtkStructuredGridConnectivity::SetNumberOfGrids(const unsigned int N)
{
  if (N == 0)
  {
...
#endif /* svtkStructuredGridConnectivity_h */
