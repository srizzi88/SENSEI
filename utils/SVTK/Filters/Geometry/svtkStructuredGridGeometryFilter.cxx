/*=========================================================================
  ...
  Module:    svtkStructuredGridGeometryFilter.cxx
...
svtkStandardNewMacro(svtkStructuredGridGeometryFilter);

// Construct with initial extent of all the data
svtkStructuredGridGeometryFilter::svtkStructuredGridGeometryFilter()
{
  this->Extent[0] = 0;
...
int svtkStructuredGridGeometryFilter::RequestData(svtkInformation* svtkNotUsed(request),
    ...
    case 3: // ------------------- grab points in volume  --------------
      ...
      for (k = 0; k < (diff[2] + 1); k++)
      {
        for (j = 0; j < (diff[1] + 1); j++)
        {
          ...
          svtkIdType cellK = (k == dims[2] - 1) ? k - 1 : k;
          svtkIdType cellJ = (j == dims[1] - 1) ? j - 1 : j;
          ...
          for (i = 0; i < (diff[0] + 1); i++)
          {
            ...
            {
              svtkIdType cellI = (i == dims[0] - 1) ? i - 1 : i;
...
void svtkStructuredGridGeometryFilter::SetExtent(
...
void svtkStructuredGridGeometryFilter::SetExtent(int extent[6])
{
  int i;
...
int svtkStructuredGridGeometryFilter::RequestUpdateExtent(svtkInformation* svtkNotUsed(request),
...
int svtkStructuredGridGeometryFilter::FillInputPortInformation(int, svtkInformation* info)
{
  info->Set(svtkAlgorithm::INPUT_REQUIRED_DATA_TYPE(), "svtkStructuredGrid");
  return 1;
}

void svtkStructuredGridGeometryFilter::PrintSelf(ostream& os, svtkIndent indent)
{
