/*=========================================================================
 ...
 Module:    svtkStructuredNeighbor.h
 ...
 =========================================================================*/
/**
 * @class   svtkStructuredNeighbor
 *
 *
 *  An internal, light-weight class used to store neighbor information.
 */

#ifndef svtkStructuredNeighbor_h
#define svtkStructuredNeighbor_h

#include "svtkFiltersGeometryModule.h" // For export macro
#include "svtkObject.h"

class SVTKFILTERSGEOMETRY_EXPORT svtkStructuredNeighbor
{
public:
  // An enum that defines the neighboring orientation which is stored in a
  // 3-tuple svtkStructuredNeighbor::Orientation. In each dimension, there
   ...
   * Default Constructor
   */
  svtkStructuredNeighbor();

  /**
   * Custom constructor. Constructs a neighbor with the prescribed neighbor
   * grid/block ID and overlap.
   */
  svtkStructuredNeighbor(const int NeiID, int overlap[6]);

  /**
   * Custom constructor. Constructs a neighbor with the prescribed neigbhor
   * grid/block ID, overlap extent, and orientation
   */
  svtkStructuredNeighbor(const int NeiID, int overlap[6], int orient[3]);

  /**
   * Copy constructor
   */
  svtkStructuredNeighbor(const svtkStructuredNeighbor& N) { *this = N; }

  /**
   * Default destructor
   */
  virtual ~svtkStructuredNeighbor();

  //@{
  /**
   * Overload assignment operator
   */
  svtkStructuredNeighbor& operator=(const svtkStructuredNeighbor& N)
  {
    if (this != &N)
    {
...
#endif /* svtkStructuredNeighbor_h */
// SVTK-HeaderTest-Exclude: svtkStructuredNeighbor.h
