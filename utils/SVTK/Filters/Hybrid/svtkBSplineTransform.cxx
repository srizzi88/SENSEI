/*=========================================================================
  ...
  Module:    svtkBSplineTransform.cxx
...
svtkStandardNewMacro(svtkBSplineTransform);

class svtkBSplineTransformConnectionHolder : public svtkAlgorithm
{
public:
  static svtkBSplineTransformConnectionHolder* New();
  svtkTypeMacro(svtkBSplineTransformConnectionHolder, svtkAlgorithm);

  svtkBSplineTransformConnectionHolder() { this->SetNumberOfInputPorts(1); }
};

svtkStandardNewMacro(svtkBSplineTransformConnectionHolder);
...
inline void svtkBSplineTransformWeights(double F[4], double G[4], double f)
{
...
static int svtkBSplineTransformBorder(int gridId0[3], int gridId1[3], int gridId2[3], int gridId3[3],
  double* ff[3], double* gg[3], int ext[3], int borderMode)
{
  ...
  switch (borderMode)
  {
    ...
    case SVTK_BSPLINE_ZERO_AT_BORDER:
...
//-------------------------------------------------------------
template <class T>
class svtkBSplineTransformFunction
{
public:
  static void Cubic(const double point[3], double displacement[3], double derivatives[3][3],
    void* gridPtrVoid, int gridExt[6], svtkIdType gridInc[3], int borderMode);
};

template <class T>
void svtkBSplineTransformFunction<T>::Cubic(const double point[3], double displacement[3],
  double derivatives[3][3], void* gridPtrVoid, int gridExt[6], svtkIdType gridInc[3], int borderMode)
{
  ...
  for (int i = 0; i < 3; i++)
  {
    ...
    if (ext[i] != 0)
    {
      ...
      int idx = svtkMath::Floor(point[i]);
      ...
      svtkBSplineTransformWeights(ff[i], gg[i], f);
    }
  }
  ...
  {
    pointIsInvalid =
      svtkBSplineTransformBorder(gridId0, gridId1, gridId2, gridId3, ff, gg, ext, borderMode);
  }
...
//----------------------------------------------------------------------------
svtkBSplineTransform::svtkBSplineTransform()
{
  this->ConnectionHolder = svtkBSplineTransformConnectionHolder::New();
  this->BorderMode = SVTK_BSPLINE_EDGE;
...
//----------------------------------------------------------------------------
void svtkBSplineTransform::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
const char* svtkBSplineTransform::GetBorderModeAsString()
{
...
svtkMTimeType svtkBSplineTransform::GetMTime()
{
...
//----------------------------------------------------------------------------
void svtkBSplineTransform::ForwardTransformPoint(const double inPoint[3], double outPoint[3])
{
...
void svtkBSplineTransform::ForwardTransformPoint(const float point[3], float output[3])
{
  double fpoint[3];
  fpoint[0] = point[0];
  fpoint[1] = point[1];
...
void svtkBSplineTransform::ForwardTransformDerivative(
...
void svtkBSplineTransform::InverseTransformDerivative(
  const double inPoint[3], double outPoint[3], double derivative[3][3])
{
  if (!this->GridPointer || !this->CalculateSpline)
  {
...
//----------------------------------------------------------------------------
void svtkBSplineTransform::InverseTransformPoint(const double point[3], double output[3])
{
...
//----------------------------------------------------------------------------
// convert double to double and back again
void svtkBSplineTransform::InverseTransformPoint(const float point[3], float output[3])
{
  double fpoint[3];
  double fderivative[3][3];
  fpoint[0] = point[0];
...
//----------------------------------------------------------------------------
void svtkBSplineTransform::InternalDeepCopy(svtkAbstractTransform* transform)
{
  svtkBSplineTransform* gridTransform = (svtkBSplineTransform*)transform;
...
//----------------------------------------------------------------------------
void svtkBSplineTransform::InternalUpdate()
{
  svtkImageData* grid = this->GetCoefficientData();
  this->GridPointer = nullptr;
  ...
  {
    case SVTK_FLOAT:
      this->CalculateSpline = &(svtkBSplineTransformFunction<float>::Cubic);
      break;
    case SVTK_DOUBLE:
      this->CalculateSpline = &(svtkBSplineTransformFunction<double>::Cubic);
      break;
    default:
      this->CalculateSpline = nullptr;
      svtkErrorMacro("InternalUpdate: grid type must be float or double");
      break;
  }
...
//----------------------------------------------------------------------------
svtkAbstractTransform* svtkBSplineTransform::MakeTransform()
{
  return svtkBSplineTransform::New();
}

//----------------------------------------------------------------------------
void svtkBSplineTransform::SetCoefficientConnection(svtkAlgorithmOutput* output)
{
  this->ConnectionHolder->SetInputConnection(output);
}

//----------------------------------------------------------------------------
void svtkBSplineTransform::SetCoefficientData(svtkImageData* grid)
{
...
//----------------------------------------------------------------------------
svtkImageData* svtkBSplineTransform::GetCoefficientData()
{
