/*=========================================================================
  ...
  Module:    svtkGreedyTerrainDecimation.cxx
...
=========================================================================*/
#include "svtkGreedyTerrainDecimation.h"
...
svtkStandardNewMacro(svtkGreedyTerrainDecimation);

// Define some constants describing vertices
//
#define SVTK_VERTEX_NO_TRIANGLE (-1)
#define SVTK_VERTEX_INSERTED (-2)

#define SVTK_IN_TRIANGLE 0
#define SVTK_INTERIOR_EDGE 1
#define SVTK_BOUNDARY_EDGE 2

//----------------------------------------------------------------------
// Supporting classes for points
class svtkTerrainInfo
{
public:
  svtkTerrainInfo()
    : TriangleId(SVTK_VERTEX_NO_TRIANGLE)
  {
  }
  svtkIdType TriangleId;
};
...
class svtkGreedyTerrainDecimationTerrainInfoType : public std::vector<svtkTerrainInfo>
{
public:
  typedef std::vector<svtkTerrainInfo> Superclass;
  typedef Superclass::size_type size_type;
  svtkGreedyTerrainDecimationTerrainInfoType(size_type n, const svtkTerrainInfo& value)
    : std::vector<svtkTerrainInfo>(n, value)
  {
  }
};
...
class svtkGreedyTerrainDecimationPointInfoType : public std::vector<svtkIdType>
{
};

// Begin svtkGreedyTerrainDecimation class implementation-----------------------------------------
//
//----------------------------------------------------------------------
svtkGreedyTerrainDecimation::svtkGreedyTerrainDecimation()
{
  this->ErrorMeasure = SVTK_ERROR_SPECIFIED_REDUCTION;
...
//----------------------------------------------------------------------
svtkGreedyTerrainDecimation::~svtkGreedyTerrainDecimation() = default;

//----------------------------------------------------------------------
inline void svtkGreedyTerrainDecimation::GetTerrainPoint(int i, int j, double x[3])
{
...
//----------------------------------------------------------------------
inline void svtkGreedyTerrainDecimation::ComputeImageCoordinates(svtkIdType inputPtId, int ij[2])
{
...
//----------------------------------------------------------------------
inline svtkIdType svtkGreedyTerrainDecimation::InsertNextPoint(svtkIdType inputPtId, double x[3])
{
  if ((this->CurrentPointId + 1) >= (svtkIdType)this->PointInfo->size())
  {
...
//----------------------------------------------------------------------
inline double* svtkGreedyTerrainDecimation::GetPoint(svtkIdType id)
{
  return this->Points->GetPointer(3 * id);
}

//----------------------------------------------------------------------
inline void svtkGreedyTerrainDecimation::GetPoint(svtkIdType id, double x[3])
{
...
//----------------------------------------------------------------------
void svtkGreedyTerrainDecimation::EstimateOutputSize(
  const svtkIdType numInputPts, svtkIdType& numPts, svtkIdType& numTris)
{
  switch (this->ErrorMeasure)
  {
    case SVTK_ERROR_NUMBER_OF_TRIANGLES:
      numTris = this->NumberOfTriangles;
      break;
    case SVTK_ERROR_SPECIFIED_REDUCTION:
...
//----------------------------------------------------------------------
int svtkGreedyTerrainDecimation::SatisfiesErrorMeasure(double error)
{
  switch (this->ErrorMeasure)
  {
    case SVTK_ERROR_NUMBER_OF_TRIANGLES:
    ...
    case SVTK_ERROR_ABSOLUTE:
      return error <= this->AbsoluteError ? 1 : 0;

    case SVTK_ERROR_RELATIVE:
...
void svtkGreedyTerrainDecimation::UpdateTriangles(svtkIdType ptId)
{
  svtkIdType ncells;
  svtkIdType* cells;
  svtkIdType npts;
...
void svtkGreedyTerrainDecimation::UpdateTriangle(
  svtkIdType triId, svtkIdType p1, svtkIdType p2, svtkIdType p3)
{
...
//----------------------------------------------------------------------
void svtkGreedyTerrainDecimation::InsertBoundaryVertices()
{
  int i, j;
  svtkIdType inputPtId, offset;
...
int svtkGreedyTerrainDecimation::InCircle(double x[3], double x1[3], double x2[3], double x3[3])
{
...
svtkIdType svtkGreedyTerrainDecimation::FindTriangle(double x[3], svtkIdType ptIds[3], svtkIdType tri,
  double tol, svtkIdType nei[3], svtkIdList* neighbors, int& status)
{
  ...
  for (inside = 1, minProj = SVTK_DEL2D_TOLERANCE, ic = 0; ic < 3; ic++)
  {
    ...
    {
      svtkErrorMacro("Duplicate point");
      return -1;
    }
  ...
  else if (!inside && (fabs(minProj) < SVTK_DEL2D_TOLERANCE)) // on edge
  {
    ...
    {
      nei[0] = (-1);
      status = SVTK_BOUNDARY_EDGE;
    }
    else
    {
      nei[0] = neighbors->GetId(0);
      status = SVTK_INTERIOR_EDGE;
    }
    return tri;
  }
...
// triangle id.
void svtkGreedyTerrainDecimation::CheckEdge(
  svtkIdType ptId, double x[3], svtkIdType p1, svtkIdType p2, svtkIdType tri, int depth)
{
  if (depth > 15)
  {
    return;
...
//----------------------------------------------------------------------
svtkIdType svtkGreedyTerrainDecimation::AddPointToTriangulation(svtkIdType inputPtId)
{
  svtkIdType ptId, nei[3], tri[4];
  svtkIdType nodes[4][3];
  svtkIdType pts[3];
  svtkIdType numNeiPts;
  const svtkIdType* neiPts;
  svtkIdType i, p1 = 0, p2 = 0;
  int ij[2];
  double x[3];
  int status;

  // Make sure the point has not been previously inserted
  if ((*this->TerrainInfo)[inputPtId].TriangleId == SVTK_VERTEX_INSERTED)
  {
    return -1;
  }
  ...
  if (tri[0] >= 0) // found a triangle
  {
    ...
    if (status == SVTK_IN_TRIANGLE) // in triangle
    {
    ...
    else if (status == SVTK_INTERIOR_EDGE) // on interior triangle edge; has a neighbor
    {
      // update cell list
    ...
    else // if ( status == SVTK_BOUNDARY_EDGE ) // on boundary triangle edge; no neighbor
    {
      // update cell list
      for (i = 0; i < 3; i++)
      {
    ...
    (*this->TerrainInfo)[inputPtId].TriangleId = SVTK_VERTEX_INSERTED;
...
//----------------------------------------------------------------------
void svtkGreedyTerrainDecimation::ComputePointNormal(int i, int j, float n[3])
{
  ...
  if (dx == 0.0 || dy == 0.0)
  {
    ...
    svtkErrorMacro("Could not compute normal.");
    return;
  }
  vx[0] = (float)(dx);
  vx[1] = 0.0;
  ...
  vy[1] = (float)(dy);
  vy[2] = (float)(y1 - y0);
  svtkMath::Cross(vx, vy, n);
  svtkMath::Normalize(n);
}

//----------------------------------------------------------------------
int svtkGreedyTerrainDecimation::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  svtkIdType numInputPts = input->GetNumberOfPoints(), numPts, numTris;
  svtkIdType inputPtId;
  ...
  //
  svtkDebugMacro(<< "Decimating terrain...");

  if (input->GetDataDimension() != 2)
  {
    svtkWarningMacro(<< "This class treats 2D height fields only");
    return 1;
  }
  if ((this->Heights = this->InputPD->GetScalars()) == nullptr)
  {
    svtkWarningMacro(<< "This class requires height scalars");
    return 1;
  }
  ...
  // Top element of SVTK's priority queue returns the minimum error value. Since we want the
  // maximum error, we use 1/error relationship to insert errors.
  this->TerrainError = svtkPriorityQueue::New();
  this->TerrainError->Allocate(numInputPts, (svtkIdType)((double)0.25 * numInputPts));
  ...
  this->Points = static_cast<svtkDoubleArray*>(newPts->GetData());

  // initialize the normals
  if (this->ComputeNormals)
  {
    this->Normals = svtkFloatArray::New();
  ...
  this->TerrainInfo = new svtkGreedyTerrainDecimationTerrainInfoType(numInputPts, svtkTerrainInfo());

  this->PointInfo = new svtkGreedyTerrainDecimationPointInfoType;
  ...
  //
  int abortExecute = 0;
  svtkIdType numInsertedPoints = 0;
  ...
  svtkDebugMacro(<< "Output TIN contains: " << this->Mesh->GetNumberOfPoints() << " points"
...
/*----------------------------------------------------------------------
  ...
  A triangle may be characterized in one of four ways:
    SVTK_TWO_TRIANGLES: We can create a two triangle representation
    SVTK_BOTTOM_TRIANGLE: We should only scan the lower triangle
    SVTK_TOP_TRIANGLE: We should only scan the upper triangle
    SVTK_DEGENERATE: The points are colinear (not scan converted)

  Configuration of the two triangles
  --------------------------------------------------------------------------*/
#define SVTK_TWO_TRIANGLES 0 // most often
#define SVTK_BOTTOM_TRIANGLE 1
#define SVTK_TOP_TRIANGLE 2
#define SVTK_DEGENERATE 3 // should never happen in this application
...
//
void svtkGreedyTerrainDecimation::UpdateTriangle(
  svtkIdType tri, int ij1[2], int ij2[2], int ij3[2], double h[3])
{
  int *min, *max, *midL, *midR, *mid, mid2[2];
  double t, tt;
  int i, j, xL, xR;
  double hMin, hMax, hMidL, hMidR, hL, hR;
  svtkIdType idx, inputPtId, maxInputPtId = 0;
  ...
  switch (type)
  {
    case SVTK_BOTTOM_TRIANGLE:
    case SVTK_TWO_TRIANGLES:
      ...
      if (type == SVTK_BOTTOM_TRIANGLE)
      {
        break;
      }
      SVTK_FALLTHROUGH;

    case SVTK_TOP_TRIANGLE:
...
//
int svtkGreedyTerrainDecimation::CharacterizeTriangle(int ij1[2], int ij2[2], int ij3[3], int*& min,
  ...
  double& hMax, double& hL, double& hR)
{
  ...
  if (ij1[1] == ij2[1])
  {
    ...
    if (ij3[1] < ij1[1])
    {
      min = ij3;
      hMin = h[2];
      return SVTK_BOTTOM_TRIANGLE;
    }
    else
    {
      max = ij3;
      hMax = h[2];
      return SVTK_TOP_TRIANGLE;
    }
  }
  ...
  return SVTK_TWO_TRIANGLES;
}

//----------------------------------------------------------------------
int svtkGreedyTerrainDecimation::FillInputPortInformation(int, svtkInformation* info)
{
...
//----------------------------------------------------------------------
void svtkGreedyTerrainDecimation::PrintSelf(ostream& os, svtkIndent indent)
{
  this->Superclass::PrintSelf(os, indent);

  os << indent << "Error Measure: ";
  if (this->ErrorMeasure == SVTK_ERROR_NUMBER_OF_TRIANGLES)
  {
    os << "Number of triangles\n";
    os << indent << "Number of triangles: " << this->NumberOfTriangles << "\n";
  }
  else if (this->ErrorMeasure == SVTK_ERROR_SPECIFIED_REDUCTION)
  {
    os << "Specified reduction\n";
    os << indent << "Reduction: " << this->Reduction << "\n";
  }
  else if (this->ErrorMeasure == SVTK_ERROR_ABSOLUTE)
  {
    os << "Absolute\n";
    os << indent << "Absolute Error: " << this->AbsoluteError << "\n";
  }
  else // this->ErrorMeasure == SVTK_ERROR_RELATIVE
  {
    os << "Relative\n";
