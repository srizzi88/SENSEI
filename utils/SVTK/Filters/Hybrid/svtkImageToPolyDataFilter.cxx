/*=========================================================================
  ...
  Module:    svtkImageToPolyDataFilter.cxx
...
=========================================================================*/
#include "svtkImageToPolyDataFilter.h"
...
svtkStandardNewMacro(svtkImageToPolyDataFilter);

svtkCxxSetObjectMacro(svtkImageToPolyDataFilter, LookupTable, svtkScalarsToColors);

svtkImageToPolyDataFilter::svtkImageToPolyDataFilter()
{
  this->OutputStyle = SVTK_STYLE_POLYGONALIZE;
  this->ColorMode = SVTK_COLOR_MODE_LINEAR_256;
  this->Smoothing = 1;
  ...
  this->Table = svtkUnsignedCharArray::New();
...
int svtkImageToPolyDataFilter::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  svtkPolyData* tmpOutput;
  svtkPolyData* tmpInput;
  svtkAppendPolyData* append;
  svtkPolyData* appendOutput;
  svtkDataArray* inScalars = input->GetPointData()->GetScalars();
  svtkIdType numPixels = input->GetNumberOfPoints();
  int dims[3], numComp;
  double origin[3], spacing[3];
  svtkUnsignedCharArray* pixels;
  int type;
  ...
  svtkDebugMacro(<< "Vectorizing image...");

  if (inScalars == nullptr || numPixels < 1)
  {
    svtkDebugMacro(<< "Not enough input to create output");
    return 1;
  }

  append = svtkAppendPolyData::New();
  tmpOutput = svtkPolyData::New();
  tmpInput = svtkPolyData::New();
  ...
  for (pieceNum = j = 0; j < numPieces[1] && !abortExecute; j++)
  {
    ...
    for (i = 0; i < numPieces[0] && !abortExecute; i++)
    {
      ...
      svtkDebugMacro(<< "Processing #" << pieceNum);
      ...
      svtkDebugMacro(<< "Quantizing color...image size (" << newDims[0] << ", " << newDims[1]
                    << ")");

      // Generate polygons according to mode setting
      //
      if (this->OutputStyle == SVTK_STYLE_PIXELIZE)
      {
        this->PixelizeImage(pixels, newDims, newOrigin, spacing, tmpOutput);
      }
      else if (this->OutputStyle == SVTK_STYLE_RUN_LENGTH)
      {
        this->RunLengthImage(pixels, newDims, newOrigin, spacing, tmpOutput);
      }
      else // SVTK_STYLE_POLYGONALIZE
      {
...
void svtkImageToPolyDataFilter::PixelizeImage(svtkUnsignedCharArray* pixels, int dims[3],
  double origin[3], double spacing[3], svtkPolyData* output)
{
  ...
  polyColors = svtkUnsignedCharArray::New();
...
void svtkImageToPolyDataFilter::RunLengthImage(svtkUnsignedCharArray* pixels, int dims[3],
  double origin[3], double spacing[3], svtkPolyData* output)
{
  int i, j;
  svtkIdType pts[4], id;
...
void svtkImageToPolyDataFilter::PolygonalizeImage(svtkUnsignedCharArray* pixels, int dims[3],
  double origin[3], double spacing[3], svtkPolyData* output)
{
  int numPolys;
  ...
  svtkUnsignedCharArray* pointDescr = svtkUnsignedCharArray::New();
  pointDescr->Allocate(numPixels / 2, numPixels / 2);

  svtkCellArray* edgeConn = svtkCellArray::New();
  ...
  delete[] this->Visited;
  svtkDebugMacro(<< "Constructed polygons...");
  ...
  svtkDebugMacro(<< "Output generated...");
...
//
svtkUnsignedCharArray* svtkImageToPolyDataFilter::QuantizeImage(
  svtkDataArray* inScalars, int numComp, int type, int dims[3], int extent[4])
{
  ...
  pixels = svtkUnsignedCharArray::New();
  pixels->SetNumberOfValues(3 * numPixels);
  outPixels = pixels->GetPointer(0);

  // Figure out how to quantize
  //
  if (this->ColorMode == SVTK_COLOR_MODE_LINEAR_256)
  {
    // Check scalar type
    if (type != SVTK_UNSIGNED_CHAR || numComp != 3)
    {
      svtkErrorMacro(<< "Wrong input scalar type");
      return nullptr;
    }
    else
    {
      inPixels = static_cast<svtkUnsignedCharArray*>(inScalars)->GetPointer(0);
    }
  ...
  else // using provided lookup table
  {
    ...
    {
      svtkErrorMacro(<< "LUT mode requires single component scalar and LUT");
      return nullptr;
    }
...
void svtkImageToPolyDataFilter::BuildTable(unsigned char* svtkNotUsed(inPixels))
{
...
int svtkImageToPolyDataFilter::FillInputPortInformation(int, svtkInformation* info)
{
  info->Set(svtkAlgorithm::INPUT_REQUIRED_DATA_TYPE(), "svtkImageData");
  return 1;
}

void svtkImageToPolyDataFilter::PrintSelf(ostream& os, svtkIndent indent)
{
  ...
  else // this->OutputStyle == SVTK_STYLE_POLYGONALIZE
  {
    os << indent << "Polygonalize\n";
  }

  os << indent << "Color Mode: ";
  if (this->ColorMode == SVTK_STYLE_PIXELIZE)
  {
    os << indent << "LUT\n";
  }
  else // this->ColorMode == SVTK_STYLE_POLYGONALIZE
  {
    os << indent << "Linear256\n";
  }
...
int svtkImageToPolyDataFilter::IsSameColor(unsigned char* p1, unsigned char* p2)
{
...
unsigned char* svtkImageToPolyDataFilter::GetColor(unsigned char* rgb)
{
...
void svtkImageToPolyDataFilter::GetIJ(int id, int& i, int& j, int dims[2])
{
  i = id % dims[0];
  j = id / dims[0];
}
...
int svtkImageToPolyDataFilter::GetNeighbors(
...
int svtkImageToPolyDataFilter::ProcessImage(svtkUnsignedCharArray* scalars, int dims[2])
{
  int numPixels = dims[0] * dims[1];
  svtkIdList *wave, *wave2, *tmpWave;
...
void svtkImageToPolyDataFilter::GeneratePolygons(svtkPolyData* edges, int svtkNotUsed(numPolys),
  svtkPolyData* output, svtkUnsignedCharArray* polyColors, svtkUnsignedCharArray* pointDescr)
{
  svtkCellArray *newPolys, *inPolys;
  int i, numPts;
...
int svtkImageToPolyDataFilter::BuildEdges(svtkUnsignedCharArray* svtkNotUsed(pixels), int dims[3],
  double origin[3], double spacing[3], svtkUnsignedCharArray* pointDescr, svtkPolyData* edges)
{
  double x[3];
  int i, j, edgeCount;
  svtkIdType ptId, p0, p1, p2, p3, startId, attrId, id[8], pts[4];
  svtkCellArray* edgeConn = edges->GetLines();
  svtkPoints* points = edges->GetPoints();
...
void svtkImageToPolyDataFilter::BuildPolygons(svtkUnsignedCharArray* svtkNotUsed(pointDescr),
  svtkPolyData* edges, int numPolys, svtkUnsignedCharArray* polyColors)
{
  svtkPoints* points = edges->GetPoints();
  svtkIdType numPts = points->GetNumberOfPoints(), ptId;
...
void svtkImageToPolyDataFilter::SmoothEdges(svtkUnsignedCharArray* pointDescr, svtkPolyData* edges)
{
    ...
    for (ptId = 0; ptId < numPts; ptId++)
    {
      if (pointDescr->GetValue(ptId) == 0) // can smooth
      {
        ...
        for (i = 0; i < ncells; i++)
        {
          ...
          {
            svtkErrorMacro("Bad cell in smoothing operation");
            connId = pts[0];
          }
...
//
void svtkImageToPolyDataFilter::DecimateEdges(
  svtkPolyData* edges, svtkUnsignedCharArray* pointDescr, double tol2)
{
  svtkPoints* points = edges->GetPoints();
  svtkIdType numPts = points->GetNumberOfPoints(), ptId, prevId, nextId;
  svtkIdType npts;
  ...
  for (ptId = 0; ptId < numPts; ptId++)
  {
    if (pointDescr->GetValue(ptId) == 0)
    {
      ...
      if (ncells == 2)
      {
        ...
        if (svtkLine::DistanceToLine(x, xPrev, xNext) <= tol2)
        {
