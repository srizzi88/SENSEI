/*=========================================================================
  ...
  Module:    svtkImplicitModeller.cxx
...
=========================================================================*/
#include "svtkImplicitModeller.h"
...
svtkStandardNewMacro(svtkImplicitModeller);

struct svtkImplicitModellerAppendInfo
{
  svtkImplicitModeller* Modeller;
  svtkDataSet** Input;
...
svtkImplicitModeller::svtkImplicitModeller()
{
  ...
  this->ProcessMode = SVTK_CELL_MODE;
...
void svtkImplicitModeller::SetOutputScalarType(int type)
{
  double scalarMax;

  svtkDebugMacro(<< this->GetClassName() << " (" << this << "): setting OutputScalarType to "
                << type);
...
void svtkImplicitModeller::SetCapValue(double value)
{
  svtkDebugMacro(<< this->GetClassName() << " (" << this << "): setting CapValue to " << value);
...
double svtkImplicitModeller::GetScalarTypeMax(int type)
{
  switch (type)
  {
    case SVTK_UNSIGNED_CHAR:
      return (double)SVTK_UNSIGNED_CHAR_MAX;
    case SVTK_CHAR:
      return (double)SVTK_CHAR_MAX;
    case SVTK_UNSIGNED_SHORT:
      return (double)SVTK_UNSIGNED_SHORT_MAX;
    case SVTK_SHORT:
      return (double)SVTK_SHORT_MAX;
    case SVTK_UNSIGNED_INT:
      return (double)SVTK_UNSIGNED_INT_MAX;
    case SVTK_INT:
      return (double)SVTK_INT_MAX;
    case SVTK_UNSIGNED_LONG:
      return (double)SVTK_UNSIGNED_LONG_MAX;
    case SVTK_LONG:
      return (double)SVTK_LONG_MAX;
    case SVTK_FLOAT:
      return (double)SVTK_FLOAT_MAX;
    case SVTK_DOUBLE:
      return (double)SVTK_DOUBLE_MAX;
    default:
      return 0;
  }
}
...
void svtkImplicitModeller::StartAppend()
{
  this->StartAppend(0);
}

void svtkImplicitModeller::StartAppend(int internal)
{
  svtkIdType numPts;
  svtkIdType i;
  double maxDistance;
  ...
  svtkDataArray* newScalars = this->GetOutput()->GetPointData()->GetScalars();
  for (i = 0; i < numPts; i++)
  {
...
//----------------------------------------------------------------------------
// Templated append for SVTK_VOXEL_MODE process mode and any type of output data
template <class OT>
void svtkImplicitModellerAppendExecute(svtkImplicitModeller* self, svtkDataSet* input,
  svtkImageData* outData, int outExt[6], double maxDistance, svtkCellLocator* locator, int id, OT*)
{
  int i, j, k;
  int subId;
  svtkIdType cellId;
  ...
  if (self->GetOutputScalarType() != SVTK_FLOAT && self->GetOutputScalarType() != SVTK_DOUBLE)
  {
...
// in svtkImplicitModellerAppendExecute; here we just setup for the per voxel
// processing.
static SVTK_THREAD_RETURN_TYPE svtkImplicitModeller_ThreadedAppend(void* arg)
{
  int threadCount;
  int threadId;
  svtkImplicitModellerAppendInfo* userData;
  svtkImageData* output;
  double maxDistance;
  int i;
  double adjBounds[6];
  double* spacing;
  ...
  threadCount = ((svtkMultiThreader::ThreadInfo*)(arg))->NumberOfThreads;
  userData = (svtkImplicitModellerAppendInfo*)(((svtkMultiThreader::ThreadInfo*)(arg))->UserData);
  ...
  {
    svtkGenericWarningMacro("Sanity check failed.");
    return SVTK_THREAD_RETURN_VALUE;
  }
  ...
  svtkCellLocator* locator = svtkCellLocator::New();
  ...
  {
    svtkTemplateMacro(svtkImplicitModellerAppendExecute(userData->Modeller, userData->Input[threadId],
      output, outExt, userData->MaximumDistance, locator, threadId, static_cast<SVTK_TT*>(nullptr)));
    default:
...
//----------------------------------------------------------------------------
// Templated append for SVTK_CELL_MODE process mode and any type of output data
template <class OT>
void svtkImplicitModellerAppendExecute(
  svtkImplicitModeller* self, svtkDataSet* input, svtkImageData* outData, double maxDistance, OT*)
{
...
void svtkImplicitModeller::Append(svtkDataSet* input)
{
  ...
  if (this->ProcessMode == SVTK_CELL_MODE)
  {
    if (!output->GetPointData()->GetScalars())
    {
    ...
    {
      svtkTemplateMacro(svtkImplicitModellerAppendExecute(
        this, input, output, this->InternalMaxDistance, static_cast<SVTK_TT*>(nullptr)));
    }
  }
  else
  {
    svtkImplicitModellerAppendInfo info;
    double minZ, maxZ;
    int slabMin, slabMax, slabSize, i;
    svtkClipPolyData **minClipper = nullptr, **maxClipper = nullptr;
    svtkPlane **minPlane = nullptr, **maxPlane = nullptr;
    double *spacing, *origin;
    ...
    info.Input = new svtkDataSet*[this->NumberOfThreads];
    ...
    else
    {
        ...
        for (i = 0; i < this->NumberOfThreads; i++)
        {
          switch (input->GetDataObjectType())
          {
            case SVTK_STRUCTURED_GRID:
              info.Input[i] = svtkStructuredGrid::New();
              break;
            case SVTK_IMAGE_DATA:
              info.Input[i] = svtkImageData::New();
              break;
            case SVTK_UNSTRUCTURED_GRID:
              info.Input[i] = svtkUnstructuredGrid::New();
              break;
            case SVTK_RECTILINEAR_GRID:
              info.Input[i] = svtkRectilinearGrid::New();
              break;
            default:
              svtkErrorMacro(<< "Unexpected DataSet type!");
              delete[] info.Input;
              return;
          }
      ...
      {
        minClipper = new svtkClipPolyData*[this->NumberOfThreads];
        maxClipper = new svtkClipPolyData*[this->NumberOfThreads];
        minPlane = new svtkPlane*[this->NumberOfThreads];
        maxPlane = new svtkPlane*[this->NumberOfThreads];
        ...
        for (i = 0; i < this->NumberOfThreads; i++)
        {
          ...
          minPlane[i] = svtkPlane::New();
          minPlane[i]->SetNormal(0.0, 0.0, -1.0);
          minPlane[i]->SetOrigin(0.0, 0.0, minZ);

          minClipper[i] = svtkClipPolyData::New();
          minClipper[i]->SetInputData((svtkPolyData*)input);
    ...
    this->Threader->SetSingleMethod(svtkImplicitModeller_ThreadedAppend, (void*)&info);
...
void svtkImplicitModeller::EndAppend()
{
...
//----------------------------------------------------------------------------
int svtkImplicitModeller::RequestInformation(svtkInformation* svtkNotUsed(request),
...
//----------------------------------------------------------------------------
int svtkImplicitModeller::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* svtkNotUsed(outputVector))
{
  ...
  svtkDebugMacro(<< "Executing implicit model");
...
double svtkImplicitModeller::ComputeModelBounds(svtkDataSet* input)
{
  const double* bounds;
  double maxDist;
  int i;
    ...
    this->ModelBounds[2] >= this->ModelBounds[3] || this->ModelBounds[4] >= this->ModelBounds[5])
  {
    ...
    {
      svtkDataSet* dsInput = svtkDataSet::SafeDownCast(this->GetInput());
      if (dsInput != nullptr)
      {
      ...
      {
        svtkErrorMacro(<< "An input must be specified to Compute the model bounds.");
  ...
  outInfo->Set(svtkDataObject::SPACING(), tempd, 3);
...
void svtkImplicitModeller::SetSampleDimensions(int i, int j, int k)
{
  int dim[3];
...
//----------------------------------------------------------------------------
void svtkImplicitModeller::SetSampleDimensions(int dim[3])
{
  int dataDim, i;
...
//----------------------------------------------------------------------------
void svtkImplicitModeller::Cap(svtkDataArray* s)
{
  int i, j, k;
  int idx;
...
//----------------------------------------------------------------------------
const char* svtkImplicitModeller::GetProcessModeAsString()
{
  if (this->ProcessMode == SVTK_CELL_MODE)
  {
    return "PerCell";
...
//----------------------------------------------------------------------------
int svtkImplicitModeller::FillInputPortInformation(int svtkNotUsed(port), svtkInformation* info)
{
...
//----------------------------------------------------------------------------
svtkTypeBool svtkImplicitModeller::ProcessRequest(
...
void svtkImplicitModeller::PrintSelf(ostream& os, svtkIndent indent)
{
