...
=========================================================================*/
/**
 * @class   svtkImplicitModeller
 * @brief   compute distance from input geometry on structured point dataset
 *
 * svtkImplicitModeller is a filter that computes the distance from the input
 ...
 * svtkPolyData, it is appropriately clipped for each thread; that is, each
 * thread only considers the input which could affect its slab of the output.
 * <P>
 * This filter can now produce output of any type supported by svtkImageData.
 ...
 * requirements as an svtkImageShiftScale and/or svtkImageCast could have
 ...
 *
 * @sa
 * svtkSampleFunction svtkContourFilter
 */

#ifndef svtkImplicitModeller_h
#define svtkImplicitModeller_h

#include "svtkFiltersHybridModule.h" // For export macro
#include "svtkImageAlgorithm.h"

#define SVTK_VOXEL_MODE 0
#define SVTK_CELL_MODE 1

class svtkDataArray;
class svtkExtractGeometry;
class svtkMultiThreader;

class SVTKFILTERSHYBRID_EXPORT svtkImplicitModeller : public svtkImageAlgorithm
{
public:
  svtkTypeMacro(svtkImplicitModeller, svtkImageAlgorithm);
   ...
   */
  svtkSetMacro(ScaleToMaximumDistance, svtkTypeBool);
  svtkGetMacro(ScaleToMaximumDistance, svtkTypeBool);
  svtkBooleanMacro(ScaleToMaximumDistance, svtkTypeBool);
  //@}
   ...
   */
  svtkSetClampMacro(ProcessMode, int, 0, 1);
  svtkGetMacro(ProcessMode, int);
  void SetProcessModeToPerVoxel() { this->SetProcessMode(SVTK_VOXEL_MODE); }
  void SetProcessModeToPerCell() { this->SetProcessMode(SVTK_CELL_MODE); }
   ...
   * process mode.
   */
  svtkSetMacro(LocatorMaxLevel, int);
  svtkGetMacro(LocatorMaxLevel, int);
  //@}
...
protected:
  ...
  svtkTypeBool ScaleToMaximumDistance;
...
private:
  svtkImplicitModeller(const svtkImplicitModeller&) = delete;
  void operator=(const svtkImplicitModeller&) = delete;
};
