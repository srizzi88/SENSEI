/*=========================================================================
  ...
  Module:    svtkPCAAnalysisFilter.cxx
...
=========================================================================*/
#include "svtkPCAAnalysisFilter.h"
...
svtkStandardNewMacro(svtkPCAAnalysisFilter);

//------------------------------------------------------------------------
// Matrix ops. Some taken from svtkThinPlateSplineTransform.cxx
...
//----------------------------------------------------------------------------
// protected
svtkPCAAnalysisFilter::svtkPCAAnalysisFilter()
{
  this->Evals = svtkFloatArray::New();
...
//----------------------------------------------------------------------------
// protected
int svtkPCAAnalysisFilter::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  for (i = 0; i < n; i++)
  {
    for (int j = 0; j < s; j++)
    {
      tmpInput = svtkPointSet::SafeDownCast(mbInput->GetBlock(j));
      if (!tmpInput)
      {
        continue;
      }
  ...
  svtkMath::JacobiN(T, s, ev, evecMat);
  ...
  for (int j = 0; j < s; j++)
  {
    ...
    svtkPointSet* block = svtkPointSet::SafeDownCast(output->GetBlock(j));
...
//----------------------------------------------------------------------------
// public
void svtkPCAAnalysisFilter::GetParameterisedShape(svtkFloatArray* b, svtkPointSet* shape)
{
  svtkPointSet* output = nullptr;

  svtkMultiBlockDataSet* mbOutput = this->GetOutput();
  ...
  {
    output = svtkPointSet::SafeDownCast(mbOutput->GetBlock(i));
    if (output)
    {
      break;
    }
  ...
  if (!output)
  {
    svtkErrorMacro(<< "No valid output block was found.");
    return;
  }
  ...
  {
    svtkErrorMacro(<< "Input shape does not have the correct number of points");
    return;
  }
...
//----------------------------------------------------------------------------
// public
void svtkPCAAnalysisFilter::GetShapeParameters(svtkPointSet* shape, svtkFloatArray* b, int bsize)
{
  svtkPointSet* output = nullptr;
...
//----------------------------------------------------------------------------
// public
void svtkPCAAnalysisFilter::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
// public
int svtkPCAAnalysisFilter::GetModesRequiredFor(double proportion)
{
  int i;
