/*=========================================================================
  ...
  Module:    svtkProjectedTerrainPath.cxx
...
struct svtkEdge
{
  svtkEdge(svtkIdType v1, svtkIdType v2)
    : V1(v1)
    , V2(v2)
    , tPos(-1.0)
    , tNeg(-1.0)
...
class svtkEdgeList : public std::vector<svtkEdge>
{
};
typedef svtkEdgeList EdgeListType;
typedef svtkEdgeList::iterator EdgeListIterator;

// Begin svtkProjectedTerrainPath class implementation--------------------------
//
svtkStandardNewMacro(svtkProjectedTerrainPath);

//-----------------------------------------------------------------------------
svtkProjectedTerrainPath::svtkProjectedTerrainPath()
{
  ...
  this->MaximumNumberOfLines = SVTK_ID_MAX;
  this->PositiveLineError = nullptr;
  this->NegativeLineError = nullptr;
}

//-----------------------------------------------------------------------------
svtkProjectedTerrainPath::~svtkProjectedTerrainPath() = default;

//----------------------------------------------------------------------------
void svtkProjectedTerrainPath::SetSourceConnection(svtkAlgorithmOutput* algOutput)
{
  this->SetInputConnection(1, algOutput);
}

//-----------------------------------------------------------------------------
void svtkProjectedTerrainPath::SetSourceData(svtkImageData* source)
{
  this->SetInputDataInternal(1, source);
}

//-----------------------------------------------------------------------------
svtkImageData* svtkProjectedTerrainPath::GetSource()
{
...
//-----------------------------------------------------------------------------
int svtkProjectedTerrainPath::FillInputPortInformation(int port, svtkInformation* info)
{
  if (port == 0)
  {
...
//
inline void svtkProjectedTerrainPath::GetImageIndex(double x[3], double loc[2], int ij[2])
{
...
//-----------------------------------------------------------------------------
int svtkProjectedTerrainPath::RequestData(
  svtkInformation*, svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  // get the input and output
  svtkInformation* linesInfo = inputVector[0]->GetInformationObject(0);
  svtkInformation* imageInfo = inputVector[1]->GetInformationObject(0);
  svtkInformation* outInfo = outputVector->GetInformationObject(0);

  svtkPolyData* lines = svtkPolyData::SafeDownCast(linesInfo->Get(svtkDataObject::DATA_OBJECT()));
  svtkImageData* image = svtkImageData::SafeDownCast(imageInfo->Get(svtkDataObject::DATA_OBJECT()));
  svtkPolyData* output = svtkPolyData::SafeDownCast(outInfo->Get(svtkDataObject::DATA_OBJECT()));

  svtkPoints* inPoints = lines->GetPoints();
  svtkIdType numPts = inPoints->GetNumberOfPoints();
  svtkCellArray* inLines = lines->GetLines();
  svtkIdType numLines;
  if (!inLines || (numLines = inLines->GetNumberOfCells()) <= 0)
  {
    svtkErrorMacro("This filter requires lines as input");
    return 1;
  }

  if (!image)
  {
    svtkErrorMacro("This filter requires an image as input");
    return 1;
  }
  ...
  svtkPointData* inPD = lines->GetPointData();
  ...
  this->PositiveLineError = svtkPriorityQueue::New();
  this->NegativeLineError = svtkPriorityQueue::New();
  this->NumLines = 0;
  for (inLines->InitTraversal(); inLines->GetNextCell(npts, pts);)
  {
    for (j = 0; j < (npts - 1); j++)
    {
      this->EdgeList->push_back(svtkEdge(pts[j], pts[j + 1]));
      this->ComputeError(static_cast<svtkIdType>(this->EdgeList->size() - 1)); // puts edges in
                                                                              // queues
      this->NumLines++;
    }
  }
  ...
  outLines->AllocateEstimate(static_cast<svtkIdType>(this->EdgeList->size()), 2);
  ...
  outLines->Delete();
  svtkDebugMacro("Produced " << outLines->GetNumberOfCells() << " lines from " << numLines
                            << " input polylines");
...
void svtkProjectedTerrainPath::RemoveOcclusions()
{
  double error;
  svtkIdType eId;
...
void svtkProjectedTerrainPath::HugTerrain()
{
...
void svtkProjectedTerrainPath::SplitEdge(svtkIdType eId, double t)
{
  this->NumLines++;

  // Get the points defining the edge
  svtkEdge& e = (*this->EdgeList)[eId];
  double p1[3], p2[3];
  ...
  svtkIdType pId = this->Points->InsertNextPoint(x);

  // We will create a new edge and update the old one.
  svtkIdType v2 = e.V2;
  e.V2 = pId;
  this->EdgeList->push_back(svtkEdge(pId, v2));
  svtkIdType eNew = static_cast<svtkIdType>(this->EdgeList->size() - 1);
...
double svtkProjectedTerrainPath::GetHeight(double loc[2], int ij[2])
{
...
void svtkProjectedTerrainPath::ComputeError(svtkIdType edgeId)
{
  svtkEdge& e = (*this->EdgeList)[edgeId];
  double p1[3], p2[3];
  this->Points->GetPoint(e.V1, p1);
  this->Points->GetPoint(e.V2, p2);
  ...
  double negError = SVTK_FLOAT_MAX;
  double posError = -SVTK_FLOAT_MAX;
...
//-----------------------------------------------------------------------------
void svtkProjectedTerrainPath::PrintSelf(ostream& os, svtkIndent indent)
{
