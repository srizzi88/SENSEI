/*=========================================================================
  ...
  Module:    svtkProjectedTerrainPath.h
 ...
 *
 * svtkProjectedTerrainPath projects an input polyline onto a terrain. (The
...
#ifndef svtkProjectedTerrainPath_h
#define svtkProjectedTerrainPath_h
...
class SVTKFILTERSHYBRID_EXPORT svtkProjectedTerrainPath : public svtkPolyDataAlgorithm
{
public:
  //@{
  /**
   ...
   */
  svtkTypeMacro(svtkProjectedTerrainPath, svtkPolyDataAlgorithm);
   ...
   */
  static svtkProjectedTerrainPath* New();
   ...
   */
  void SetSourceData(svtkImageData* source);
  svtkImageData* GetSource();
  //@}

  /**
   * Specify the second input (the terrain) onto which the polyline(s) should be projected.
   * Note: svtkImageData* is required
   */
   ...
   */
  svtkSetClampMacro(ProjectionMode, int, SIMPLE_PROJECTION, HUG_PROJECTION);
  svtkGetMacro(ProjectionMode, int);
   ...
   */
  svtkSetClampMacro(HeightTolerance, double, 0.0, SVTK_FLOAT_MAX);
  svtkGetMacro(HeightTolerance, double);
  //@}
   ...
   */
  svtkSetClampMacro(MaximumNumberOfLines, svtkIdType, 1, SVTK_ID_MAX);
  svtkGetMacro(MaximumNumberOfLines, svtkIdType);
  //@}

protected:
  svtkProjectedTerrainPath();
  ~svtkProjectedTerrainPath() override;
  ...
  void ComputeError(svtkIdType edgeId);
  void RemoveOcclusions();
  void HugTerrain();
  void SplitEdge(svtkIdType eId, double t);

  // ivars that the API addresses
  int ProjectionMode;
  double HeightOffset;
  double HeightTolerance;
  svtkIdType MaximumNumberOfLines;
  ...
  svtkPriorityQueue* PositiveLineError; // errors above terrain
  svtkPriorityQueue* NegativeLineError; // errors below terrain

  // This is a PIMPL'd vector representing edges
  svtkEdgeList* EdgeList;

private:
  svtkProjectedTerrainPath(const svtkProjectedTerrainPath&) = delete;
  void operator=(const svtkProjectedTerrainPath&) = delete;
};
