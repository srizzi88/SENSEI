/*=========================================================================
  ...
  Module:    svtkRenderLargeImage.cxx
...
=========================================================================*/
#include "svtkRenderLargeImage.h"
...
#include <vector>
//----------------------------------------------------------------------------
svtkStandardNewMacro(svtkRenderLargeImage);

svtkCxxSetObjectMacro(svtkRenderLargeImage, Input, svtkRenderer);
//----------------------------------------------------------------------------
...
public:
  ...
  //
  svtkRenderLargeImage2DHelperClass()
  {
...
//----------------------------------------------------------------------------
svtkRenderLargeImage::svtkRenderLargeImage()
{
  this->Input = nullptr;
  this->Magnification = 3;
  this->SetNumberOfInputPorts(0);
  this->SetNumberOfOutputPorts(1);
  this->StoredData = new svtkRenderLargeImage2DHelperClass();
}
//----------------------------------------------------------------------------
svtkRenderLargeImage::~svtkRenderLargeImage()
{
...
//----------------------------------------------------------------------------
void svtkRenderLargeImage::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
svtkImageData* svtkRenderLargeImage::GetOutput()
{
  return svtkImageData::SafeDownCast(this->GetOutputDataObject(0));
}

//----------------------------------------------------------------------------
svtkTypeBool svtkRenderLargeImage::ProcessRequest(
...
void svtkRenderLargeImage::RequestInformation(svtkInformation* svtkNotUsed(request),
...
void svtkRenderLargeImage::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** svtkNotUsed(inputVector), svtkInformationVector* outputVector)
{
  svtkInformation* outInfo = outputVector->GetInformationObject(0);
  svtkImageData* data = svtkImageData::SafeDownCast(outInfo->Get(svtkDataObject::DATA_OBJECT()));
  data->SetExtent(outInfo->Get(svtkStreamingDemandDrivenPipeline::UPDATE_EXTENT()));
  ...
  if (this->GetOutput()->GetScalarType() != SVTK_UNSIGNED_CHAR)
  {
  ...
  cam->SetViewAngle(2.0 * atan(tan(0.5 * viewAngle * svtkMath::Pi() / 360.0) / this->Magnification) *
    360.0 / svtkMath::Pi());
...
//----------------------------------------------------------------------------
int svtkRenderLargeImage::FillOutputPortInformation(int svtkNotUsed(port), svtkInformation* info)
{
  // now add our info
...
//----------------------------------------------------------------------------
void svtkRenderLargeImage::Rescale2DActors()
{
  svtkActor2D* actor;
  svtkProp* aProp;
  svtkRenderer* aren;
...
void svtkRenderLargeImage::Shift2DActors(int x, int y)
{
  svtkActor2D* actor;
  svtkCoordinate *c1, *c2;
  double d1[3], d2[3];
...
void svtkRenderLargeImage::Restore2DActors()
{
  svtkActor2D* actor;
  svtkCoordinate *c1, *c2;
  svtkCoordinate *n1, *n2;
