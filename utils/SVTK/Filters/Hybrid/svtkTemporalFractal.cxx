/*=========================================================================
  ...
  Program:   ParaView
  Module:    svtkTemporalFractal.cxx
...
//----------------------------------------------------------------------------
class TemporalFractalOutputUtil : public svtkObject
{
public:
  static TemporalFractalOutputUtil* New();
  svtkTypeMacro(TemporalFractalOutputUtil, svtkObject);

  void AddDataSet(svtkDataObject* newData, unsigned int level)
  {
    assert(newData);
  ...
  void CreateOutput(svtkMultiBlockDataSet* mbs)
  {
    for (size_t i = 0; i < this->DataSets.size(); i++)
    {
      svtkDataObject* dataSet = DataSets[i];
      unsigned int level = this->Levels[i];
      svtkMultiBlockDataSet* block = svtkMultiBlockDataSet::SafeDownCast(mbs->GetBlock(level));
      if (!block)
      {
  ...
  void CreateOutput(svtkHierarchicalBoxDataSet* hbds)
  {
    ...
    {
      unsigned int level = this->Levels[i];
      svtkUniformGrid* grid = svtkUniformGrid::SafeDownCast(this->DataSets[i]);
      assert(grid);
...
private:
  ...
  std::vector<svtkSmartPointer<svtkDataObject> > DataSets;
  std::vector<unsigned int> Levels;
};

svtkStandardNewMacro(TemporalFractalOutputUtil);
//----------------------------------------------------------------------------

svtkStandardNewMacro(svtkTemporalFractal);

svtkTemporalFractal::svtkTemporalFractal()
{
...
int svtkTemporalFractal::FillOutputPortInformation(int svtkNotUsed(port), svtkInformation* info)
{
...
void svtkTemporalFractal::SetBlockInfo(svtkUniformGrid* grid, int level, int* ext, int onFace[6])
{
  // onFace[0]:xmin
  // onFace[1]:xmax
  // onFace[2]:ymin
...
//----------------------------------------------------------------------------
// This handles any alterations necessary for ghost levels.
void svtkTemporalFractal::SetRBlockInfo(svtkRectilinearGrid* grid, int level, int* ext, int onFace[6])
{
  if (this->GhostLevels)
  {
    if (!onFace[0])
  ...
  svtkMath::RandomSeed(1234);
  int coord = 0;
  while (coord < 3)
  {
    coords[coord] = svtkDoubleArray::New();
    // grid->SetOrigin(origin);
    ...
    while (i < c)
    {
      ...
      double epsilon = (svtkMath::Random() - 0.5) * spacing[coord] * 0.4;
...
//----------------------------------------------------------------------------
int svtkTemporalFractal::TwoDTest(double bds[6], int level, int target)
{
...
int svtkTemporalFractal::MandelbrotTest(double x, double y)
{
...
//----------------------------------------------------------------------------
svtkTypeBool svtkTemporalFractal::ProcessRequest(
...
int svtkTemporalFractal::RequestDataObject(
  svtkInformation*, svtkInformationVector**, svtkInformationVector* outputVector)
{
  svtkInformation* info = outputVector->GetInformationObject(0);

  svtkCompositeDataSet* dset = this->GenerateRectilinearGrids
    ? static_cast<svtkCompositeDataSet*>(svtkMultiBlockDataSet::New())
    : static_cast<svtkCompositeDataSet*>(svtkHierarchicalBoxDataSet::New());

  info->Set(svtkDataObject::DATA_OBJECT(), dset);
  dset->Delete();
  return 1;
}
...
int svtkTemporalFractal::RequestInformation(
  svtkInformation*, svtkInformationVector**, svtkInformationVector* outputVector)
{
  svtkInformation* info = outputVector->GetInformationObject(0);
  ...
  if (this->DiscreteTimeSteps)
  {
    ...
    info->Set(svtkStreamingDemandDrivenPipeline::TIME_STEPS(), tsteps, 11);
  }

  double trange[2];
  trange[0] = 0.0;
  trange[1] = 10.0;
  info->Set(svtkStreamingDemandDrivenPipeline::TIME_RANGE(), trange, 2);

  return 1;
}

//----------------------------------------------------------------------------
int svtkTemporalFractal::RequestData(
  svtkInformation* request, svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  svtkInformation* info = outputVector->GetInformationObject(0);
  ...
  double timeStep = info->Get(svtkStreamingDemandDrivenPipeline::UPDATE_TIME_STEP());

  if (!info->Has(svtkStreamingDemandDrivenPipeline::UPDATE_PIECE_NUMBER()) ||
    !info->Has(svtkStreamingDemandDrivenPipeline::UPDATE_NUMBER_OF_PIECES()))
  {
    svtkErrorMacro("Expected information not found. "
  ...
  dset->GetInformation()->Set(svtkDataObject::DATA_TIME_STEP(), timeStep);

  outInfo->Set(svtkDataObject::DATA_OBJECT(), dset);
  dset->Delete();
...
//----------------------------------------------------------------------------
int svtkTemporalFractal ::RequestOneTimeStep(svtkCompositeDataSet* output,
  ...
  svtkInformationVector* outputVector)
{
  ...
  int piece = info->Get(svtkStreamingDemandDrivenPipeline::UPDATE_PIECE_NUMBER());
  int numPieces = info->Get(svtkStreamingDemandDrivenPipeline::UPDATE_NUMBER_OF_PIECES());

  this->OutputUtil = svtkSmartPointer<TemporalFractalOutputUtil>::New();
  ...
  if (svtkHierarchicalBoxDataSet::SafeDownCast(output))
  {
    this->OutputUtil->CreateOutput(svtkHierarchicalBoxDataSet::SafeDownCast(output));
  }
  else if (svtkMultiBlockDataSet::SafeDownCast(output))
  {
    this->OutputUtil->CreateOutput(svtkMultiBlockDataSet::SafeDownCast(output));
  }
  ...
  info->Set(svtkStreamingDemandDrivenPipeline::BOUNDS(), bounds, 6);

  if (!this->GenerateRectilinearGrids)
  {
    svtkHierarchicalBoxDataSet* hbds = svtkHierarchicalBoxDataSet::SafeDownCast(output);
...
//----------------------------------------------------------------------------
int svtkTemporalFractal::LineTest2(
...
//----------------------------------------------------------------------------
int svtkTemporalFractal::LineTest(
...
//----------------------------------------------------------------------------
void svtkTemporalFractal::Traverse(int& blockId, int level, svtkDataObject* output, int x0, int x3,
  int y0, int y3, int z0, int z3, int onFace[6])
{
  ...
  if (this->TwoDimensional)
  {
    ...
    else
    {
      ...
      {
        svtkDataObject* newData(nullptr);
        if (this->GenerateRectilinearGrids)
        {
...
//----------------------------------------------------------------------------
void svtkTemporalFractal::AddTestArray(svtkHierarchicalBoxDataSet* output)
{
  ...
  while (level < levels)
  {
    ...
    while (block < blocks)
    {
      ...
      grid = svtkUniformGrid::SafeDownCast(output->GetDataSet(level, block));
...
//----------------------------------------------------------------------------
void svtkTemporalFractal::AddVectorArray(svtkHierarchicalBoxDataSet* output)
{
  double* origin = this->GetTopLevelOrigin();
...
//----------------------------------------------------------------------------
void svtkTemporalFractal::AddFractalArray(svtkCompositeDataSet* output)
{
  svtkCompositeDataIterator* iter = output->NewIterator();

  svtkImageMandelbrotSource* fractalSource = svtkImageMandelbrotSource::New();
  int dims[3];
  ...
  for (iter->InitTraversal(); !iter->IsDoneWithTraversal(); iter->GoToNextItem())
  {
    ...
    {
      svtkRectilinearGrid* grid = svtkRectilinearGrid::SafeDownCast(iter->GetCurrentDataObject());
      assert("check: grid_exists" && grid != nullptr);
...
//----------------------------------------------------------------------------
void svtkTemporalFractal::AddBlockIdArray(svtkHierarchicalBoxDataSet* output)
{
  int levels = output->GetNumberOfLevels();
  int level = 0;
  int blockId = 0;
...
//----------------------------------------------------------------------------
void svtkTemporalFractal::AddDepthArray(svtkHierarchicalBoxDataSet* output)
{
  int levels = output->GetNumberOfLevels();
  int level = 0;
  int blockId = 0;
...
void svtkTemporalFractal::AddGhostLevelArray(svtkDataSet* grid, int dim[3], int onFace[6])
{
  ...
  for (k = 0; k < dims[2]; ++k)
  {
    ...
    for (j = 0; j < dims[1]; ++j)
    {
      ...
      for (i = 0; i < dims[0]; ++i)
      {
        ...
        {
          *ptr |= svtkDataSetAttributes::DUPLICATECELL;
        }
        ++ptr;
      }
    }
  }
  array->SetName(svtkDataSetAttributes::GhostArrayName());
  grid->GetCellData()->AddArray(array);
  array->Delete();
}

//----------------------------------------------------------------------------
void svtkTemporalFractal::CellExtentToBounds(int level, int ext[6], double bds[6])
{
  int spacingFactor = 1;
  double spacing[3];
...
//----------------------------------------------------------------------------
void svtkTemporalFractal::ExecuteRectilinearMandelbrot(svtkRectilinearGrid* grid, double* ptr)
{
  int a0 = 0;
  int a1 = 1;
  int a2 = 2;
...
//----------------------------------------------------------------------------
double svtkTemporalFractal::EvaluateSet(double p[4])
{
  unsigned short count = 0;
  double v0, v1;
  double cReal, cImag, zReal, zImag;
...
//----------------------------------------------------------------------------
void svtkTemporalFractal::GetContinuousIncrements(
...
//----------------------------------------------------------------------------
void svtkTemporalFractal::PrintSelf(ostream& os, svtkIndent indent)
{
