/*=========================================================================
  ...
  Module:    svtkTemporalInterpolator.cxx
...
svtkStandardNewMacro(svtkTemporalInterpolator);

//----------------------------------------------------------------------------
svtkTemporalInterpolator::svtkTemporalInterpolator()
{
...
//----------------------------------------------------------------------------
svtkTemporalInterpolator::~svtkTemporalInterpolator() = default;

//----------------------------------------------------------------------------
void svtkTemporalInterpolator::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
int svtkTemporalInterpolator::FillInputPortInformation(int port, svtkInformation* info)
{
  if (port == 0)
  {
...
//----------------------------------------------------------------------------
int svtkTemporalInterpolator::FillOutputPortInformation(int svtkNotUsed(port), svtkInformation* info)
{
...
//----------------------------------------------------------------------------
int svtkTemporalInterpolator::RequestDataObject(
...
int svtkTemporalInterpolator::RequestInformation(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  {
    inTimes = inInfo->Get(svtkStreamingDemandDrivenPipeline::TIME_STEPS());
    numTimes = inInfo->Length(svtkStreamingDemandDrivenPipeline::TIME_STEPS());
  ...
  if (numTimes < 2)
  {
    svtkErrorMacro(<< "Not enough input time steps for interpolation");
    return 0;
  }
  ...
  if (this->DiscreteTimeStepInterval > 0.0)
  {
    ...
    outInfo->Set(svtkStreamingDemandDrivenPipeline::TIME_STEPS(), &OutputTimeValues[0],
...
int svtkTemporalInterpolator::RequestData(
  svtkInformation*, svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  svtkInformation* inInfo = inputVector[0]->GetInformationObject(0);
  svtkInformation* outInfo = outputVector->GetInformationObject(0);
  ...
  {
    svtkDataObject* data0 = inData->GetBlock(0);
    svtkDataObject* data1 = inData->GetBlock(1);
    if (data0 == nullptr && data1 == nullptr)
    {
      svtkErrorMacro("Null data set");
      return 0;
    }
    // interpolate i-1 and i
    double t0 = data0->GetInformation()->Get(svtkDataObject::DATA_TIME_STEP());
    double t1 = data1->GetInformation()->Get(svtkDataObject::DATA_TIME_STEP());
  ...
  outData->GetInformation()->Set(svtkDataObject::DATA_TIME_STEP(), upTime);

  svtkSmartPointer<svtkDoubleArray> originalTimes = svtkSmartPointer<svtkDoubleArray>::New();
  originalTimes->SetName("OriginalTimeSteps");
  originalTimes->SetNumberOfTuples(numTimeSteps);
  for (int i = 0; i < numTimeSteps; i++)
  {
    originalTimes->SetValue(
      i, inData->GetBlock(i)->GetInformation()->Get(svtkDataObject::DATA_TIME_STEP()));
  }
...
int svtkTemporalInterpolator::RequestUpdateExtent(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  // get the info objects
  svtkInformation* outInfo = outputVector->GetInformationObject(0);
  ...
  if (outInfo->Has(svtkStreamingDemandDrivenPipeline::UPDATE_TIME_STEP()))
  {
    ...
    int numInTimes = inInfo->Length(svtkStreamingDemandDrivenPipeline::TIME_STEPS());
    ...
    if (inTimes)
    {
      ...
      inInfo->Set(svtkMultiTimeStepAlgorithm::UPDATE_TIME_STEPS(), inUpTimes, numInUpTimes);
    }
  }
  return 1;
}
...
//----------------------------------------------------------------------------
svtkTemporalInterpolator::ArrayMatch svtkTemporalInterpolator::VerifyArrays(
  svtkDataArray** arrays, int N)
{
  svtkIdType Nt = arrays[0]->GetNumberOfTuples();
  svtkIdType Nc = arrays[0]->GetNumberOfComponents();
  for (int i = 1; i < N; ++i)
  {
...
//----------------------------------------------------------------------------
svtkDataObject* svtkTemporalInterpolator ::InterpolateDataObject(
  svtkDataObject* in1, svtkDataObject* in2, double ratio)
{
  if (svtkDataSet::SafeDownCast(in1))
  {
    //
    // if we have reached the Leaf/DataSet level, we can interpolate directly
    //
    svtkDataSet* inds1 = svtkDataSet::SafeDownCast(in1);
    svtkDataSet* inds2 = svtkDataSet::SafeDownCast(in2);
    return this->InterpolateDataSet(inds1, inds2, ratio);
  }
  else if (svtkCompositeDataSet::SafeDownCast(in1))
  {
    svtkCompositeDataSet* mgds[2];
    mgds[0] = svtkCompositeDataSet::SafeDownCast(in1);
    mgds[1] = svtkCompositeDataSet::SafeDownCast(in2);

    // It is essential that mgds[0] an mgds[1] has the same structure.
    //
    // We need to loop over blocks etc and build up a new dataset
    //
    svtkCompositeDataSet* output = mgds[0]->NewInstance();
    ...
    {
      svtkDataObject* dataobj1 = iter->GetCurrentDataObject();
      svtkDataObject* dataobj2 = mgds[1]->GetDataSet(iter);
      if (!dataobj1 || !dataobj2)
      {
        svtkWarningMacro("The composite datasets were not identical in structure.");
        continue;
      }
      svtkDataObject* result = this->InterpolateDataObject(dataobj1, dataobj2, ratio);
      if (result)
      {
      ...
      {
        svtkErrorMacro(<< "Unexpected error during interpolation");
  ...
  {
    svtkErrorMacro("We cannot yet interpolate this type of dataset");
    return nullptr;
  }
}

//----------------------------------------------------------------------------
svtkDataSet* svtkTemporalInterpolator ::InterpolateDataSet(
  svtkDataSet* in1, svtkDataSet* in2, double ratio)
{
  ...
  //
  svtkDataSet* output = input[0]->NewInstance();
  ...
  for (int s = 0; s < input[0]->GetPointData()->GetNumberOfArrays(); ++s)
  {
    ...
    if (arrays[1])
    {
      ...
      {
        svtkWarningMacro(<< "Interpolation aborted for array "
...
struct svtkTemporalExecute
{
  ...
  {
    using T = svtk::GetAPIType<ArrayIn1>;
    const double oneMinusRatio = 1.0 - ratio;
    auto in1 = svtk::DataArrayValueRange(input1);
    auto in2 = svtk::DataArrayValueRange(input2);
    auto out = svtk::DataArrayValueRange(output);

    for (svtkIdType i = 0; i < in1.size(); i++)
    {
...
//----------------------------------------------------------------------------
svtkDataArray* svtkTemporalInterpolator ::InterpolateDataArray(
  double ratio, svtkDataArray** arrays, svtkIdType numTuples)
{
  //
  // Create the output
  //
  svtkAbstractArray* aa = arrays[0]->CreateArray(arrays[0]->GetDataType());
  svtkDataArray* output = svtkArrayDownCast<svtkDataArray>(aa);
  ...
  if (!svtkArrayDispatch::Dispatch3SameValueType::Execute(
        arrays[0], arrays[1], output, svtkTemporalExecute{}, ratio))
  {
    svtkTemporalExecute{}(arrays[0], arrays[1], output, ratio);
  }
