/*=========================================================================
  ...
  Module:    svtkHyperTreeGridContour.cxx
...
svtkStandardNewMacro(svtkHyperTreeGridContour);

//-----------------------------------------------------------------------------
svtkHyperTreeGridContour::svtkHyperTreeGridContour()
{
...
//----------------------------------------------------------------------------
void svtkHyperTreeGridContour::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
int svtkHyperTreeGridContour::FillOutputPortInformation(int, svtkInformation* info)
{
...
//-----------------------------------------------------------------------------
void svtkHyperTreeGridContour::SetLocator(svtkIncrementalPointLocator* locator)
{
...
//-----------------------------------------------------------------------------
void svtkHyperTreeGridContour::CreateDefaultLocator()
{
...
//-----------------------------------------------------------------------------
svtkMTimeType svtkHyperTreeGridContour::GetMTime()
{
...
//-----------------------------------------------------------------------------
int svtkHyperTreeGridContour::ProcessTrees(svtkHyperTreeGrid* input, svtkDataObject* outputDO)
{
  ...
  if (!this->InScalars)
  {
    svtkWarningMacro(<< "No scalar data to contour");
    return 1;
  }
  ...
  this->Helper = new svtkContourHelper(this->Locator, newVerts, newLines, newPolys,
  ...
  this->CellSigns = (svtkBitArray**)malloc(numContours * sizeof(svtkBitArray*));
  this->Signs.resize(numContours, true);
  for (int c = 0; c < numContours; ++c)
  {
    this->CellSigns[c] = svtkBitArray::New();
  ...
  for (svtkIdType c = 0; c < this->GetNumberOfContours(); ++c)
  {
    if (this->CellSigns[c])
    {
...
//-----------------------------------------------------------------------------
bool svtkHyperTreeGridContour::RecursivelyPreProcessTree(svtkHyperTreeGridNonOrientedCursor* cursor)
{
    ...
    for (int child = 0; child < numChildren; ++child)
    {
      ...
      if (!selected)
      {
        ...
        for (int c = 0; c < numContours; ++c)
        {
          ...
          svtkIdType childId = cursor->GetGlobalNodeIndex();
...
//-----------------------------------------------------------------------------
void svtkHyperTreeGridContour::RecursivelyProcessTree(
  svtkHyperTreeGridNonOrientedMooreSuperCursor* supercursor)
{
  // Retrieve global index of input cursor
  svtkIdType id = supercursor->GetGlobalNodeIndex();
  ...
  if (!supercursor->IsLeaf())
  {
    ...
    for (svtkIdType c = 0; c < this->ContourValues->GetNumberOfContours() && !selected; ++c)
    {
      ...
      for (unsigned int neighbor = 0; neighbor < nn && !selected; ++neighbor)
      {
        ...
        {
          svtkIdType idN = supercursor->GetGlobalNodeIndex(icursorN);
    ...
    for (unsigned int cornerIdx = 0; cornerIdx < numLeavesCorners; ++cornerIdx)
    {
      ...
      if (owner)
      {
        ...
        for (unsigned int _cornerIdx = 0; _cornerIdx < numLeavesCorners; ++_cornerIdx)
        {
          ...
          svtkIdType idN = supercursor->GetGlobalNodeIndex(cursorId);
