/*=========================================================================
  ...
  Module:    svtkHyperTreeGridGeometry.cxx
...
svtkStandardNewMacro(svtkHyperTreeGridGeometry);

//-----------------------------------------------------------------------------
svtkHyperTreeGridGeometry::svtkHyperTreeGridGeometry()
{
  // Create storage for corners of leaf cells
  this->Points = svtkPoints::New();

  // Create storage for untructured leaf cells
  this->Cells = svtkCellArray::New();
  ...
  this->FaceIDs = svtkIdList::New();
  this->FacePoints = svtkPoints::New();
  this->FacePoints->SetNumberOfPoints(4);
  this->FacesA = svtkIdTypeArray::New();
  this->FacesA->SetNumberOfComponents(2);
  this->FacesB = svtkIdTypeArray::New();
  this->FacesB->SetNumberOfComponents(2);
  this->FaceScalarsA = svtkDoubleArray::New();
  this->FaceScalarsA->SetNumberOfTuples(4);
  this->FaceScalarsB = svtkDoubleArray::New();
...
//----------------------------------------------------------------------------
void svtkHyperTreeGridGeometry::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
int svtkHyperTreeGridGeometry::FillOutputPortInformation(int, svtkInformation* info)
{
...
//-----------------------------------------------------------------------------
int svtkHyperTreeGridGeometry::ProcessTrees(svtkHyperTreeGrid* input, svtkDataObject* outputDO)
{
  // Downcast output data object to polygonal data set
  svtkPolyData* output = svtkPolyData::SafeDownCast(outputDO);
  if (!output)
  {
  ...
  {
    this->Normals =
      svtkDoubleArray::SafeDownCast(this->InData->GetArray(input->GetInterfaceNormalsName()));
    this->Intercepts =
      svtkDoubleArray::SafeDownCast(this->InData->GetArray(input->GetInterfaceInterceptsName()));
  } // this->HasInterface
  ...
  if (this->Dimension == 3)
  {
    ...
    this->EdgeFlags = svtkUnsignedCharArray::New();
    this->EdgeFlags->SetName("svtkEdgeFlags");
    this->EdgeFlags->SetNumberOfComponents(1);

    svtkPointData* outPointData = output->GetPointData();
    outPointData->AddArray(this->EdgeFlags);
    outPointData->SetActiveAttribute(this->EdgeFlags->GetName(), svtkDataSetAttributes::EDGEFLAG);

    svtkNew<svtkHyperTreeGridNonOrientedVonNeumannSuperCursor> cursor;
...
//----------------------------------------------------------------------------
void svtkHyperTreeGridGeometry::RecursivelyProcessTreeNot3D(
...
//----------------------------------------------------------------------------
// JB Meme code que svtkAdaptativeDataSetSurfaceFiltre ??
void svtkHyperTreeGridGeometry::ProcessLeaf1D(svtkHyperTreeGridNonOrientedGeometryCursor* cursor)
{
  // Cell at cursor center is a leaf, retrieve its global index
  svtkIdType inId = cursor->GetGlobalNodeIndex();
  if (inId < 0)
  {
    return;
  }
  ...
  svtkIdType outId = this->Cells->InsertNextCell(2, ids);
...
//----------------------------------------------------------------------------
// JB Meme code que svtkAdaptativeDataSetSurfaceFiltre ??
void svtkHyperTreeGridGeometry::ProcessLeaf2D(svtkHyperTreeGridNonOrientedGeometryCursor* cursor)

{
  ...
  if (this->HasInterface)
  {
    size_t int12sz = 12 * sizeof(svtkIdType);
...
//----------------------------------------------------------------------------
void svtkHyperTreeGridGeometry::RecursivelyProcessTree3D(
  svtkHyperTreeGridNonOrientedVonNeumannSuperCursor* cursor, unsigned char crtWorkFaces)
{
  ...
  {
    // Cursor is at leaf, process it depending on its dimension
    this->ProcessLeaf3D(cursor); // JBSVTK9 ProcessLeaf3D2 prends en compte les interfaces... :)?
    return;
  } // if ( cursor->IsLeaf() )
...
//----------------------------------------------------------------------------
// JB Meme code que svtkAdaptativeDataSetSurfaceFiltre ??
void svtkHyperTreeGridGeometry::ProcessLeaf3D(
  svtkHyperTreeGridNonOrientedVonNeumannSuperCursor* superCursor)
{
  // Cell at cursor center is a leaf, retrieve its global index, and mask
  svtkIdType inId = superCursor->GetGlobalNodeIndex();
  if (inId < 0)
  {
    return;
  }
  ...
  for (unsigned int c = 0; c < nc; ++c)
  {
    ...
    svtkIdType idN;
    unsigned int levelN;
    svtkHyperTree* treeN = superCursor->GetInformation(VonNeumannCursors3D[c], levelN, leafN, idN);
      ...
      (superCursor->IsMasked() && treeN && leafN && levelN <= level && !maskedN))
    {
      ...
      if (cursorN->GetTree() && superCursor->GetTree())
      {
        ...
        edgeFlag = (static_cast<unsigned char>(svtkMathUtilities::NearlyEqual(
                     boundsN[((VonNeumannOrientations3D[c] + 1) % 3) * 2],
                     bounds[((VonNeumannOrientations3D[c] + 1) % 3) * 2]))) |
          (static_cast<unsigned char>(
             svtkMathUtilities::NearlyEqual(boundsN[((VonNeumannOrientations3D[c] + 1) % 3) * 2 + 1],
               bounds[((VonNeumannOrientations3D[c] + 1) % 3) * 2 + 1]))
            << 1) |
          (static_cast<unsigned char>(
             svtkMathUtilities::NearlyEqual(boundsN[((VonNeumannOrientations3D[c] + 2) % 3) * 2],
               bounds[((VonNeumannOrientations3D[c] + 2) % 3) * 2]))
            << 2) |
          (static_cast<unsigned char>(
             svtkMathUtilities::NearlyEqual(boundsN[((VonNeumannOrientations3D[c] + 2) % 3) * 2 + 1],
  ...
  if (this->HasInterface)
  {
    ...
    svtkIdType nA = this->FacesA->GetNumberOfTuples();
    if (nA > 0)
    {
      this->FaceIDs->Reset();
      svtkIdType i0 = 0;
      ...
      // Create new face
      svtkIdType outId = this->Cells->InsertNextCell(this->FaceIDs);
    ...
    svtkIdType nB = this->FacesB->GetNumberOfTuples();
    if (nB > 0)
    {
      this->FaceIDs->Reset();
      int i0 = 0;
...
//----------------------------------------------------------------------------
void svtkHyperTreeGridGeometry::AddFace(svtkIdType useId, const double* origin, const double* size,
  unsigned int offset, unsigned int orientation, unsigned char hideEdge)
{
  ...
  // Insert next face
  svtkIdType outId = this->Cells->InsertNextCell(4, ids);

  // Copy face data from that of the cell from which it comes
  this->OutData->CopyData(this->InData, useId, outId);
}
//----------------------------------------------------------------------------
void svtkHyperTreeGridGeometry::AddFace2(svtkIdType inId, svtkIdType useId, const double* origin,
  const double* size, unsigned int offset, unsigned int orientation, bool create)
{
  ...
  if (this->HasInterface)
  {
    ...
    if (type < 2)
    {
      ...
      for (svtkIdType pId = 0; pId < 4; ++pId)
      {
  ...
  if (create)
  {
    ...
    svtkIdType outId = this->Cells->InsertNextCell(nPts, ids);
