/*=========================================================================
  ...
  Module:    svtkBandedPolyDataContourFilter.cxx
...
//------------------------------------------------------------------------------
// Construct object.
svtkBandedPolyDataContourFilter::svtkBandedPolyDataContourFilter()
{
  Internal = new svtkBandedPolyDataContourFilterInternals;

  this->ContourValues = svtkContourValues::New();
  this->Clipping = 0;
  this->ScalarMode = SVTK_SCALAR_MODE_INDEX;
  this->Component = 0;
...
int svtkBandedPolyDataContourFilter::ClipEdge(int v1, int v2, svtkPoints* newPts,
  svtkDataArray* inScalars, svtkDoubleArray* outScalars, svtkPointData* inPD, svtkPointData* outPD,
  svtkIdType edgePts[])
{
  ...
  for (auto iter = b; iter != e; ++iter)
  {
    ...
    svtkIdType ptId = newPts->InsertNextPoint(x);
...
//------------------------------------------------------------------------------
inline int svtkBandedPolyDataContourFilter::InsertCell(
...
//------------------------------------------------------------------------------
inline int svtkBandedPolyDataContourFilter::InsertLine(
...
//------------------------------------------------------------------------------
int svtkBandedPolyDataContourFilter::ComputeClippedIndex(double s)
{
...
//------------------------------------------------------------------------------
int svtkBandedPolyDataContourFilter::InsertNextScalar(svtkFloatArray* scalars, int cellId, int idx)
{
  if (idx < 0)
  {
    return cellId;
  ...
  if (this->ScalarMode == SVTK_SCALAR_MODE_INDEX)
  {
    double value = idx;
...
int svtkBandedPolyDataContourFilter::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  svtkDataArray* inScalars = pd->GetScalars();
  int abort = 0;
  svtkPoints* newPts;
  svtkIdType npts = 0;
  svtkIdType cellId = 0;
  ...
  svtkIdType v;
  svtkIdType vR;
  const svtkIdType* intPts;
  svtkIdType intCellId;
  svtkIdType numIntPts;
  svtkIdType numPts, numCells, estimatedSize;

  svtkDebugMacro(<< "Executing banded contour filter");
  ...
  {
    svtkErrorMacro(<< "Input scalars expected to have " << this->Component + 1 << " components");
    return 0;
  }

  if (this->ContourValues->GetNumberOfContours() < 1)
  {
    svtkWarningMacro(<< "No contour values");
    return 1;
  }
  ...
  //
  // Estimate allocation size, stolen from svtkContourGrid...
  //
  estimatedSize = static_cast<svtkIdType>(pow(static_cast<double>(numCells), .9));
  ...
  svtkDoubleArray* outScalars = svtkDoubleArray::New();
  ...
  svtkEdgeTable* edgeTable = svtkEdgeTable::New();
  svtkCellArray* intList = svtkCellArray::New(); // intersection point ids
  ...
  {
    svtkCellArray* verts = input->GetVerts();
  ...
  if (input->GetLines()->GetNumberOfCells() > 0)
  {
    ...
    svtkIdType* fullLine = new svtkIdType[maxCellSize];
  ...
  svtkIdType numStrips = input->GetStrips()->GetNumberOfCells();
  if (numPolys > 0 || numStrips > 0)
  {
    ...
    svtkCellArray* tmpPolys = nullptr;

    // If contour edges requested, set things up.
    svtkCellArray* contourEdges = nullptr;
    if (this->GenerateContourEdges)
    {
      contourEdges = svtkCellArray::New();
    ...
    if (numStrips > 0)
    {
      ...
      {
        svtkTriangleStrip::DecomposeStrip(npts, pts, tmpPolys);
      }
      polys = tmpPolys;
    }
    ...
    svtkIdType updateCount = numPolys / 20 + 1;
    svtkIdType count = 0;
         ...
         abort = this->GetAbortExecute())
    {
      ...
      svtkDebugMacro(<< "clip_scalar=" << clip_scalar << "\n"
                    << "\tpolygon=" << polygon << "\n"
                    << "\tindex=" << index);
      ...
      while (r1 < l1)
      {
        ...
        svtkDebugMacro(<< "band: clip_scalar=" << clip_scalar << " points=[" << *l2 << polygon[*l2]
  ...
  svtkDebugMacro(<< "Created " << cellId << " total cells\n");
  svtkDebugMacro(<< "Created " << output->GetVerts()->GetNumberOfCells() << " verts\n");
  svtkDebugMacro(<< "Created " << output->GetLines()->GetNumberOfCells() << " lines\n");
  svtkDebugMacro(<< "Created " << output->GetPolys()->GetNumberOfCells() << " polys\n");
  svtkDebugMacro(<< "Created " << output->GetStrips()->GetNumberOfCells() << " strips\n");
  ...
  outCD->SetActiveAttribute(arrayIdx, svtkDataSetAttributes::SCALARS);
...
//------------------------------------------------------------------------------
svtkPolyData* svtkBandedPolyDataContourFilter::GetContourEdgesOutput()
{
...
//------------------------------------------------------------------------------
svtkMTimeType svtkBandedPolyDataContourFilter::GetMTime()
{
...
//------------------------------------------------------------------------------
void svtkBandedPolyDataContourFilter::PrintSelf(ostream& os, svtkIndent indent)
{
