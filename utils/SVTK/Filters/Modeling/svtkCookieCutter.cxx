...
svtkStandardNewMacro(svtkCookieCutter);
svtkCxxSetObjectMacro(svtkCookieCutter, Locator, svtkIncrementalPointLocator);
...
#define SVTK_DEGENERATE_TOL 0.001
...
struct SortPoint
{
  ...
  svtkIdType Id;   // Id assigned to INTERSECTION or ON point, used later to build loops
  svtkIdType OnId; // Id of edge pair if classification of point is ON
  double X[3];    // position of point
  SortPoint(double t, int c, svtkIdType id, svtkIdType onId, double x[3])
    : T(t)
    , Class(c)
    , Id(id)
    , OnId(onId)
...
int ClassifySegment(SortedPointsType& sortedPoints, int i, int j, svtkIdType npts, double* p,
  double bds[6], double n[3])
{
  ...
  if (svtkPolygon::PointInPolygon(midSegment, npts, p, bds, n) == 1)
  {
...
int CleanSortedPolyline(SortedPointsType& sortedPoints)
{
  ...
  for (i = 0; i < (num - 1); ++i)
  {
    ...
    if (fabs(tp - t) <= SVTK_DEGENERATE_TOL)
    {
      needsAnalysis = true;
    }
  }
  ...
  for (i = 0; i < num;)
  {
    ...
    while (fabs(tp - t) <= SVTK_DEGENERATE_TOL && ip < imax)
    {
      ip++;
      tp = sortedPoints[ip].T;
    }
...
//
void CleanSortedPolygon(svtkIdType npts, SortedPointsType& sortedPoints)
{
  int num, i, im, ip, j;
  ...
  for (i = 0; i < imax;)
  {
    ...
    if (i == 0)
    {
      ...
      while (fabs(t - tm) <= SVTK_DEGENERATE_TOL)
      {
        im--;
        tm = npts - sortedPoints[im].T;
      }
...
int ClassifyPolyline(
  SortedPointsType& sortedPoints, svtkIdType npts, double* p, double bds[6], double n[3])
{
...
// A helper class used to crop svtkCookieCutter input polys and lines.
class svtkCookieCutterHelper
{
public:
  svtkCookieCutterHelper(svtkIncrementalPointLocator* locator, svtkPoints* inPts,
    svtkCellData* inCellData, svtkPoints* outPts, svtkCellArray* outLines, svtkCellArray* outPolys,
    svtkCellData* outCellData);

  virtual ~svtkCookieCutterHelper();

  // Process a polyline
  void CropLine(svtkIdType cellId, svtkIdType cellOffset, svtkIdType npts, const svtkIdType* pts,
    svtkPolygon* loop, double* l, double loopBds[6], double n[3]);

  // Process a polygon
  void CropPoly(svtkIdType cellId, svtkIdType cellOffset, svtkPolygon* poly, svtkIdType npts,
    const svtkIdType* pts, svtkPolygon* loop, double* l, double loopBds[6], double n[3]);

protected:
  // Convenience method
  inline void InsertPoint(double x[3], svtkCellArray* ca);
  ...
  // loops.
  int ResolveTopology(svtkPolyData* pd);

public:
  svtkIncrementalPointLocator* Locator;
  svtkPoints* InPoints;
  svtkCellData* InCellData;
  svtkPoints* OutPoints;
  svtkCellArray* OutLines;
  svtkCellArray* OutPolys;
  svtkCellData* OutCellData;
};

svtkCookieCutterHelper::svtkCookieCutterHelper(svtkIncrementalPointLocator* locator, svtkPoints* inPts,
  svtkCellData* inCellData, svtkPoints* outPts, svtkCellArray* outLines, svtkCellArray* outPolys,
  svtkCellData* outCellData)
  : Locator(locator)
  , InPoints(inPts)
...
svtkCookieCutterHelper::~svtkCookieCutterHelper() {}

void svtkCookieCutterHelper::InsertPoint(double x[3], svtkCellArray* ca)
{
  svtkIdType ptId;
...
void svtkCookieCutterHelper::CropLine(svtkIdType cellId, svtkIdType cellOffset, svtkIdType npts,
  const svtkIdType* pts, svtkPolygon* loop, double* l, double loopBds[6], double n[3])
{
  ...
  svtkIdType numInts = 0, numLoopPts = loop->Points->GetNumberOfPoints();
  for (numInts = 0, i = 0; i < (npts - 1); ++i)
  {
    ...
    for (j = 0; j < numLoopPts; ++j)
    {
      ...
      if ((result = svtkLine::Intersection(x0, x1, y0, y1, u, v)) == 2)
      {
      ...
      else if (result == 3) // parallel lines
      {
        ...
        svtkMath::Subtract(x1, x0, x10);
        double tol2 = 1.0e-08 * sqrt(svtkMath::Dot(x10, x10));
        if (svtkLine::DistanceBetweenLines(x0, x1, y0, y1, c, c2, u, v) > tol2)
        {
          continue; // no intersection just move ahead
        }
        else
        {
          svtkIdType onId = i * numLoopPts + j;
          svtkLine::DistanceToLine(x0, y0, y1, u, c);
          if (-SVTK_DEGENERATE_TOL <= u && u <= (1.0 + SVTK_DEGENERATE_TOL))
          {
            sortedPoints.push_back(
              SortPoint(static_cast<double>(i), SortPoint::ON, numInts, onId, c));
            numInts++;
          }
          svtkLine::DistanceToLine(x1, y0, y1, u, c);
          if (-SVTK_DEGENERATE_TOL <= u && u <= (1.0 + SVTK_DEGENERATE_TOL))
          {
            sortedPoints.push_back(
          ...
          svtkLine::DistanceToLine(y0, x0, x1, u, c);
          if (-SVTK_DEGENERATE_TOL <= u && u <= (1.0 + SVTK_DEGENERATE_TOL))
          {
            sortedPoints.push_back(
          ...
          svtkLine::DistanceToLine(y1, x0, x1, u, c);
          if (-SVTK_DEGENERATE_TOL <= u && u <= (1.0 + SVTK_DEGENERATE_TOL))
          {
            sortedPoints.push_back(
              SortPoint(static_cast<double>(i) + u, SortPoint::ON, numInts, onId, c));
  ...
  svtkIdType startIdx = 0, endIdx = 0, numInsertedPts;
...
int svtkCookieCutterHelper::ResolveTopology(svtkPolyData* pd)
{
  svtkIdType numLines = pd->GetNumberOfLines();
  ...
  svtkIdType pid, *cells, numPts = pts->GetNumberOfPoints();
...
void svtkCookieCutterHelper::CropPoly(svtkIdType cellId, svtkIdType cellOffset, svtkPolygon* poly,
  svtkIdType npts, const svtkIdType* pts, svtkPolygon* loop, double* l, double loopBds[6], double n[3])
{
  ...
  svtkIdType i, j, newCellId, numPts = 0, numInts = 0;
  double t, u, v, x[3], x0[3], x1[3], y0[3], y1[3];
  int result;
  SortedPointsType loopPoints, polyPoints;
  svtkIdType numLoopPts = loop->Points->GetNumberOfPoints();
  for (i = 0; i < npts; ++i)
  {
    t = static_cast<double>(i);
    this->InPoints->GetPoint(pts[i], x);
  ...
  // build adjacency information. We are using svtkPolyData because it does
  ...
  svtkNew<svtkPoints> pDataPts;
  pDataPts->SetNumberOfPoints(numPts);
  svtkNew<svtkCellArray> pDataLines;
  ...
  int loopIdx;
  size_t sze, ii, ip;
  std::set<svtkIdType> edgeExist;
  svtkIdType eId, id0, id1;
  ...
  svtkIdType numInsertedPts, *cells, thisCell, nextId, startId;
  svtkIdType ncells;
  svtkIdType thisNPts;
  const svtkIdType* thisPts;
...
svtkCookieCutter::svtkCookieCutter()
{
...
//----------------------------------------------------------------------------
void svtkCookieCutter::SetLoopsConnection(svtkAlgorithmOutput* algOutput)
{
  this->SetInputConnection(1, algOutput);
}

//----------------------------------------------------------------------------
svtkAlgorithmOutput* svtkCookieCutter::GetLoopsConnection()
{
  return this->GetInputConnection(1, 0);
}

//----------------------------------------------------------------------------
void svtkCookieCutter::SetLoopsData(svtkDataObject* input)
{
  this->SetInputData(1, input);
}

//----------------------------------------------------------------------------
svtkDataObject* svtkCookieCutter::GetLoops()
{
...
//----------------------------------------------------------------------------
int svtkCookieCutter::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  // get the info objects
  svtkInformation* inInfo = inputVector[0]->GetInformationObject(0);
  svtkInformation* loopInfo = inputVector[1]->GetInformationObject(0);
  svtkInformation* outInfo = outputVector->GetInformationObject(0);

  // get the input and output
  svtkPolyData* input = svtkPolyData::SafeDownCast(inInfo->Get(svtkDataObject::DATA_OBJECT()));
  svtkPolyData* loops = svtkPolyData::SafeDownCast(loopInfo->Get(svtkDataObject::DATA_OBJECT()));
  svtkPolyData* output = svtkPolyData::SafeDownCast(outInfo->Get(svtkDataObject::DATA_OBJECT()));

  // Initialize and check data
  svtkDebugMacro(<< "Cookie cutting...");
  ...
  for (loopPolys->InitTraversal(); loopPolys->GetNextCell(npts, pts);)
  {
    ...
    loop->GetBounds(bds);
    svtkPolygon::ComputeNormal(loop->Points, n);
    ...
    if (inVerts->GetNumberOfCells() > 0)
    {
      for (inVerts->InitTraversal(); inVerts->GetNextCell(npts, pts); ++cellId)
      {
        for (svtkIdType i = 0; i < npts; ++i)
        {
          ...
          if (svtkPolygon::PointInPolygon(x, numLoopPts, l, bds, n) == 1)
          {
            svtkIdType ptId;
            this->Locator->InsertUniquePoint(x, ptId);
            svtkIdType newCellId = outVerts->InsertNextCell(1, &ptId);
    ...
    {
      svtkIdType cellOffset = outVerts->GetNumberOfCells();
    ...
    {
      svtkIdType cellOffset = outVerts->GetNumberOfCells() + outLines->GetNumberOfCells();
    ...
    if (inStrips->GetNumberOfCells() > 0)
    {
      ...
      {
        svtkIdType numTriPts = 3, triPts[3];
...
//----------------------------------------------------------------------------
int svtkCookieCutter::RequestUpdateExtent(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  // get the info objects
  svtkInformation* inInfo = inputVector[0]->GetInformationObject(0);
  ...
  if (loopInfo)
  {
    loopInfo->Set(svtkStreamingDemandDrivenPipeline::UPDATE_PIECE_NUMBER(), 0);
    loopInfo->Set(svtkStreamingDemandDrivenPipeline::UPDATE_NUMBER_OF_PIECES(), 1);
    loopInfo->Set(svtkStreamingDemandDrivenPipeline::UPDATE_NUMBER_OF_GHOST_LEVELS(), 0);
  }
...
//----------------------------------------------------------------------------
int svtkCookieCutter::FillInputPortInformation(int port, svtkInformation* info)
{
  if (port == 0)
  {
...
//----------------------------------------------------------------------------
void svtkCookieCutter::PrintSelf(ostream& os, svtkIndent indent)
{
...
//-----------------------------------------------------------------------------
void svtkCookieCutter::CreateDefaultLocator()
{
  if (this->Locator == nullptr)
  {
