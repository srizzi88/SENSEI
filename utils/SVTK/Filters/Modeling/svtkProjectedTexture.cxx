/*=========================================================================
  ...
  Module:    svtkProjectedTexture.cxx
...
=========================================================================*/
#include "svtkProjectedTexture.h"
...
svtkStandardNewMacro(svtkProjectedTexture);
...
svtkProjectedTexture::svtkProjectedTexture()
{
  this->Position[0] = 0.0;
  this->Position[1] = 0.0;
  this->Position[2] = 1.0;
  ...
  this->CameraMode = SVTK_PROJECTED_TEXTURE_USE_PINHOLE;
  this->SRange[0] = 0.0;
  this->SRange[1] = 1.0;
  this->TRange[0] = 0.0;
  this->TRange[1] = 1.0;
...
void svtkProjectedTexture::SetFocalPoint(double fp[3])
{
  this->SetFocalPoint(fp[0], fp[1], fp[2]);
}

void svtkProjectedTexture::SetFocalPoint(double x, double y, double z)
{
  ...
  svtkMath::Normalize(orientation);
  if (this->Orientation[0] != orientation[0] || this->Orientation[1] != orientation[1] ||
...
int svtkProjectedTexture::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  svtkDebugMacro(<< "Generating texture coordinates!");
  ...
  svtkMath::Normalize(this->Orientation);

  svtkMath::Cross(this->Orientation, this->Up, rightv);
  svtkMath::Normalize(rightv);

  svtkMath::Cross(rightv, this->Orientation, upv);
  ...
  for (i = 0; i < numPts; i++)
  {
    ...
    proj = svtkMath::Dot(diff, this->Orientation);
    ...
    if (this->CameraMode == SVTK_PROJECTED_TEXTURE_USE_TWO_MIRRORS)
    {
      // First calculate elevation coordinate t.
      if (proj < 1.0e-10 && proj > -1.0e-10)
      {
        svtkWarningMacro(<< "Singularity:  point located at elevation frustum Position");
        tcoords[1] = tOffset;
      }
      else
      {
      ...
      if (proj < 1.0e-10 && proj > -1.0e-10)
      {
        svtkWarningMacro(<< "Singularity:  point located at azimuth frustum Position");
        tcoords[0] = sOffset;
      }
      else
      {
    ...
    else
    {
      if (proj < 1.0e-10 && proj > -1.0e-10)
      {
        svtkWarningMacro(<< "Singularity:  point located at frustum Position");
        tcoords[0] = sOffset;
        tcoords[1] = tOffset;
      }
      else
...
void svtkProjectedTexture::PrintSelf(ostream& os, svtkIndent indent)
{
  ...
  if (this->CameraMode == SVTK_PROJECTED_TEXTURE_USE_PINHOLE)
  {
    os << "Pinhole\n";
  }
  else if (this->CameraMode == SVTK_PROJECTED_TEXTURE_USE_TWO_MIRRORS)
  {
    os << "Two Mirror\n";
  }
  else
