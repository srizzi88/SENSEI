/*=========================================================================
  ...
  Module:    svtkRuledSurfaceFilter.h
...
=========================================================================*/
/**
 * @class   svtkRuledSurfaceFilter
 * @brief   generates a surface from a set of lines
 *
 * svtkRuledSurfaceFilter is a filter that generates a surface from a set of
 ...
 *
 * An important use of this filter is to combine it with svtkStreamTracer to
 ...
 * generated.  sides (i.e., a ribbon), use svtkRibbonFilter.
 *
 * @sa
 * svtkRibbonFilter svtkStreamTracer
 */

#ifndef svtkRuledSurfaceFilter_h
#define svtkRuledSurfaceFilter_h
...
#define SVTK_RULED_MODE_RESAMPLE 0
#define SVTK_RULED_MODE_POINT_WALK 1

class SVTKFILTERSMODELING_EXPORT svtkRuledSurfaceFilter : public svtkPolyDataAlgorithm
{
public:
  svtkTypeMacro(svtkRuledSurfaceFilter, svtkPolyDataAlgorithm);
   ...
   */
  static svtkRuledSurfaceFilter* New();

  //@{
  /**
   * Set/Get the factor that controls tearing of the surface.
   */
  svtkSetClampMacro(DistanceFactor, double, 1.0, SVTK_DOUBLE_MAX);
  svtkGetMacro(DistanceFactor, double);
  //@}
   ...
   */
  svtkSetMacro(CloseSurface, svtkTypeBool);
  svtkGetMacro(CloseSurface, svtkTypeBool);
  svtkBooleanMacro(CloseSurface, svtkTypeBool);
  //@}
   ...
   */
  svtkSetClampMacro(RuledMode, int, SVTK_RULED_MODE_RESAMPLE, SVTK_RULED_MODE_POINT_WALK);
  svtkGetMacro(RuledMode, int);
  void SetRuledModeToResample() { this->SetRuledMode(SVTK_RULED_MODE_RESAMPLE); }
  void SetRuledModeToPointWalk() { this->SetRuledMode(SVTK_RULED_MODE_POINT_WALK); }
   ...
   * Resolution[0]).
   */
  svtkSetVector2Macro(Resolution, int);
  svtkGetVectorMacro(Resolution, int, 2);
  //@}
   ...
   */
  svtkSetMacro(OrientLoops, svtkTypeBool);
  svtkGetMacro(OrientLoops, svtkTypeBool);
  svtkBooleanMacro(OrientLoops, svtkTypeBool);
  //@}

protected:
  svtkRuledSurfaceFilter();
  ~svtkRuledSurfaceFilter() override;
  ...
  int OnRatio;
  int Offset;
  svtkTypeBool CloseSurface;
  int RuledMode;
  int Resolution[2];
  svtkTypeBool PassLines;
  svtkTypeBool OrientLoops;

private:
  svtkIdList* Ids;
  double Weights[4];

  void Resample(svtkPolyData* output, svtkPolyData* input, svtkPoints* inPts, svtkPoints* newPts,
    int npts, const svtkIdType* pts, int npts2, const svtkIdType* pts2);
  void PointWalk(svtkPolyData* output, svtkPoints* inPts, int npts, const svtkIdType* pts, int npts2,
    const svtkIdType* pts2);

private:
  svtkRuledSurfaceFilter(const svtkRuledSurfaceFilter&) = delete;
  void operator=(const svtkRuledSurfaceFilter&) = delete;
};
