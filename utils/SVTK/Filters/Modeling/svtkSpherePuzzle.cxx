...
svtkStandardNewMacro(svtkSpherePuzzle);

//----------------------------------------------------------------------------
// Construct a new puzzle.
svtkSpherePuzzle::svtkSpherePuzzle()
{
...
//----------------------------------------------------------------------------
int svtkSpherePuzzle::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** svtkNotUsed(inputVector), svtkInformationVector* outputVector)
{
  ...
  svtkTransformFilter* tf = svtkTransformFilter::New();
...
//----------------------------------------------------------------------------
void svtkSpherePuzzle::MarkHorizontal(int section)
{
  int i;
...
//----------------------------------------------------------------------------
void svtkSpherePuzzle::MarkVertical(int section)
{
  int i, j, offset;
...
//----------------------------------------------------------------------------
void svtkSpherePuzzle::MoveHorizontal(int slab, int percentage, int rightFlag)
{
  int offset;
  int tmp;
  int i;
...
//----------------------------------------------------------------------------
void svtkSpherePuzzle::MoveVertical(int half, int percentage, int rightFlag)
{
  int tmp;
  ...
  theta = (double)(half)*svtkMath::Pi() / 4.0;
  this->Transform->RotateWXYZ(
    ((double)(percentage) / 100.0) * (360.0 / 2.0), sin(theta), -cos(theta), 0.0);
}

//----------------------------------------------------------------------------
int svtkSpherePuzzle::SetPoint(double x, double y, double z)
{
  double pt[3];
  double theta, phi;
  int xi, yi;
  ...
  pt[1] = y;
  pt[2] = z;
  svtkMath::Normalize(pt);

  // Convert this into phi and theta.
  theta = 180.0 - atan2(pt[0], pt[1]) * 180 / svtkMath::Pi();
  phi = 90.0 - asin(pt[2]) * 180 / svtkMath::Pi();
  ...
  svtkDebugMacro("point: " << x << ", " << y << ", " << z);
  svtkDebugMacro("theta: " << theta << ",  phi: " << phi);
  svtkDebugMacro("theta: " << xi << ", " << xn << ",  phi: " << yi << ", " << y);
...
//----------------------------------------------------------------------------
void svtkSpherePuzzle::MovePoint(int percentage)
{
  if (!this->Active)
  {
    return;
...
//----------------------------------------------------------------------------
void svtkSpherePuzzle::PrintSelf(ostream& os, svtkIndent indent)
{
  int idx;
