/*=========================================================================
  ...
  Module:    svtkSpherePuzzleArrows.cxx
...
=========================================================================*/
#include "svtkSpherePuzzleArrows.h"
...
svtkStandardNewMacro(svtkSpherePuzzleArrows);

//----------------------------------------------------------------------------
// Construct a new puzzle.
svtkSpherePuzzleArrows::svtkSpherePuzzleArrows()
{
  int idx;
...
//----------------------------------------------------------------------------
// Destruct the puzzle.
svtkSpherePuzzleArrows::~svtkSpherePuzzleArrows() = default;

//----------------------------------------------------------------------------
void svtkSpherePuzzleArrows::SetPermutationComponent(int comp, int val)
{
...
//----------------------------------------------------------------------------
int svtkSpherePuzzleArrows::RequestData(svtkInformation* svtkNotUsed(request),
...
void svtkSpherePuzzleArrows::AppendArrow(int id1, int id2, svtkPoints* pts, svtkCellArray* polys)
{
  ...
  double x, y, z;
  int num, idx;
  svtkIdType ptId1, ptId2, ptId3, ptId4, ptId5;

  // Convert the start and end piece ids into sphere coordinates.
  phi1 = svtkMath::Pi() * ((id1 / 8) + 0.5) / 4.0;
  theta1 = svtkMath::Pi() * ((id1 % 8) + 0.5) / 4.0;
  phi2 = svtkMath::Pi() * ((id2 / 8) + 0.5) / 4.0;
  theta2 = svtkMath::Pi() * ((id2 % 8) + 0.5) / 4.0;
  dPhi = phi2 - phi1;
  dTheta = theta2 - theta1;
  // Take the short way around.
  while (dPhi > svtkMath::Pi())
  {
    dPhi -= 2 * svtkMath::Pi();
  }
  while (dPhi < -svtkMath::Pi())
  ...
  while (dTheta > svtkMath::Pi())
  {
    dTheta -= 2 * svtkMath::Pi();
  }
  while (dTheta < -svtkMath::Pi())
  {
    dTheta += 2 * svtkMath::Pi();
...
//----------------------------------------------------------------------------
void svtkSpherePuzzleArrows::SetPermutation(svtkSpherePuzzle* puz)
{
  this->SetPermutation(puz->GetState());
}

//----------------------------------------------------------------------------
void svtkSpherePuzzleArrows::PrintSelf(ostream& os, svtkIndent indent)
{
  int i;
