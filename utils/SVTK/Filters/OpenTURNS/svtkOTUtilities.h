...
/**
 * @brief
 * Set of utilities for OpenTURNS<->SVTK conversions
 *
 */

#ifndef svtkOTUtilities_h
#define svtkOTUtilities_h
...
public:
   ...
   */
  static OT::Sample* SingleDimArraysToSample(svtkDataArrayCollection* arrays);
   ...
   * so it is caller's responsibility to delete it with the delete operator.
   */
  static OT::Sample* ArrayToSample(svtkDataArray* arr);

  /**
   * Methods to convert a Sample into a multi-component array.
   * The data array will have the same dimension as the sample.
   * This method allocates a new svtkDataArray and returns it,
   * so it is caller's responsibility to delete it with the delete operator.
   */
  static svtkDataArray* SampleToArray(OT::Sample* ns);
};

#endif
// SVTK-HeaderTest-Exclude: svtkOTUtilities.h
