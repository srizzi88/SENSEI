/*=========================================================================
  ...
  Module:    svtkDistributedDataFilter.h
...
/**
 * @class   svtkDistributedDataFilter
 * @brief   Serial stand-in for parallel filter that distributes data among processors
 *
 * This filter is a serial implementation of the svtkPDistributedDataFilter
 ...
 * input is a non-composite dataset, the output will be an svtkUnstructuredGrid
 ...
 * builds.
 *
 * @sa svtkPDistributedDataFilter
 */

#ifndef svtkDistributedDataFilter_h
#define svtkDistributedDataFilter_h
...
class SVTKFILTERSPARALLEL_EXPORT svtkDistributedDataFilter : public svtkDataObjectAlgorithm
{
public:
  svtkTypeMacro(svtkDistributedDataFilter, svtkDataObjectAlgorithm);

  void PrintSelf(ostream& os, svtkIndent indent) override;

  static svtkDistributedDataFilter* New();
   ...
   * or other tree building default parameters.  See svtkPKdTree and
   * svtkKdTree for more information about these options.
  ...
  svtkPKdTree* GetKdtree();

  /**
   * When this filter executes, it creates a svtkPKdTree (K-d tree)
   ...
   * the data according to region assignment.  See svtkPKdTree for
   * more information about region assignment.
   */

  svtkBooleanMacro(RetainKdtree, int);
  svtkGetMacro(RetainKdtree, int);
  svtkSetMacro(RetainKdtree, int);
   ...
   * When the new svtkUnstructuredGrid is created, by default it
  ...
  svtkBooleanMacro(IncludeAllIntersectingCells, int);
  svtkGetMacro(IncludeAllIntersectingCells, int);
  svtkSetMacro(IncludeAllIntersectingCells, int);

  /**
   * Set this variable if you want the cells of the output
   * svtkUnstructuredGrid to be clipped to the spatial region
   * boundaries.  By default this is off.
   */

  svtkBooleanMacro(ClipCells, int);
  svtkGetMacro(ClipCells, int);
  svtkSetMacro(ClipCells, int);
  ...
  {
    this->SetBoundaryMode(svtkDistributedDataFilter::ASSIGN_TO_ONE_REGION);
  }
  void SetBoundaryModeToAssignToAllIntersectingRegions()
  {
    this->SetBoundaryMode(svtkDistributedDataFilter::ASSIGN_TO_ALL_INTERSECTING_REGIONS);
  }
  void SetBoundaryModeToSplitBoundaryCells()
  {
    this->SetBoundaryMode(svtkDistributedDataFilter::SPLIT_BOUNDARY_CELLS);
  }
  int GetBoundaryMode();
  //@}
  ...
  svtkBooleanMacro(UseMinimalMemory, int);
  svtkGetMacro(UseMinimalMemory, int);
  svtkSetMacro(UseMinimalMemory, int);
   ...
   */
  svtkGetMacro(MinimumGhostLevel, int);
  svtkSetMacro(MinimumGhostLevel, int);

  /**
   * Turn on collection of timing data
   */

  svtkBooleanMacro(Timing, int);
  svtkSetMacro(Timing, int);
  svtkGetMacro(Timing, int);
   ...
   */
  svtkBSPCuts* GetCuts() { return this->UserCuts; }
  void SetCuts(svtkBSPCuts* cuts);

  /**
   * svtkBSPCuts doesn't have information about process assignments for the cuts.
   ...
   * assignments explicitly. Look at svtkPKdTree::AssignRegions for details about
...
protected:
  ...
  ~svtkDistributedDataFilter() override;

  /**
   * Build a svtkUnstructuredGrid to store the input.
   */
   ...
   * output is svtkUnstructuredGrid. If input is composite dataset, output is
  ...
  svtkBSPCuts* UserCuts;

  std::vector<int> UserRegionAssignments;

private:
  svtkDistributedDataFilter(const svtkDistributedDataFilter&) = delete;
  void operator=(const svtkDistributedDataFilter&) = delete;
};
#endif
