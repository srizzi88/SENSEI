...
svtkStandardNewMacro(svtkExtractCTHPart);
svtkCxxSetObjectMacro(svtkExtractCTHPart, ClipPlane, svtkPlane);
svtkCxxSetObjectMacro(svtkExtractCTHPart, Controller, svtkMultiProcessController);
...
public:
  ...
  svtkBoundingBox GlobalInputBounds;

  // Counter used to scale progress events.
  int TotalNumberOfDatasets;
};

class svtkExtractCTHPart::VectorOfFragments : public std::vector<svtkSmartPointer<svtkPolyData> >
{
};

class svtkExtractCTHPart::VectorOfSolids : public std::vector<svtkSmartPointer<svtkUnstructuredGrid> >
{
};

class svtkExtractCTHPart::ScaledProgress
{
  svtkExtractCTHPart* Self;
  double Shift;
  double Scale;

public:
  ScaledProgress(double shift, double scale, svtkExtractCTHPart* self)
  {
    assert(
...
//-----------------------------------------------------------------------------
svtkExtractCTHPart::svtkExtractCTHPart()
{
  this->Internals = new svtkExtractCTHPartInternal();
...
svtkMTimeType svtkExtractCTHPart::GetMTime()
{
  svtkMTimeType mTime = this->Superclass::GetMTime();
  if (this->ClipPlane)
  {
    svtkMTimeType time = this->ClipPlane->GetMTime();
...
//-----------------------------------------------------------------------------
void svtkExtractCTHPart::RemoveVolumeArrayNames()
{
...
//-----------------------------------------------------------------------------
void svtkExtractCTHPart::AddVolumeArrayName(const char* arrayName)
{
...
//-----------------------------------------------------------------------------
int svtkExtractCTHPart::GetNumberOfVolumeArrayNames()
{
  return static_cast<int>(this->Internals->VolumeArrayNames.size());
}

//-----------------------------------------------------------------------------
const char* svtkExtractCTHPart::GetVolumeArrayName(int idx)
{
...
//----------------------------------------------------------------------------
int svtkExtractCTHPart::FillInputPortInformation(int port, svtkInformation* info)
{
...
//-----------------------------------------------------------------------------
int svtkExtractCTHPart::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  svtkSmartPointer<svtkCompositeDataSet> inputCD = svtkCompositeDataSet::SafeDownCast(inputDO);
  svtkRectilinearGrid* inputRG = svtkRectilinearGrid::SafeDownCast(inputDO);
  ...
  {
    svtkErrorMacro("Failed to compute global bounds.");
    return 0;
  }
  ...
  for (svtkExtractCTHPartInternal::VolumeArrayNamesType::iterator iter =
       ...
       iter != this->Internals->VolumeArrayNames.end(); ++iter, ++array_index)
  {
    ...
    output->GetMetaData(array_index)->Set(svtkCompositeDataSet::NAME(), iter->c_str());

    svtkSmartPointer<svtkDataSet> part = nullptr;
    ...
    {
      // Add extra arrays.
      svtkNew<svtkIntArray> partArray;
...
//-----------------------------------------------------------------------------
bool svtkExtractCTHPart::ComputeGlobalBounds(svtkCompositeDataSet* input)
{
  ...
  if (!this->Controller->AllReduce(min_point, min_result, 3, svtkCommunicator::MIN_OP))
  {
    return false;
  }
  if (!this->Controller->AllReduce(max_point, max_result, 3, svtkCommunicator::MAX_OP))
  {
    return false;
  }
...
svtkSmartPointer<svtkDataSet> svtkExtractCTHPart::ExtractContour(
  svtkCompositeDataSet* input, const char* arrayName)
{
  ...
  int counter = 0;
  svtkExtractCTHPart::VectorOfFragments fragments;
  for (iter->InitTraversal(); !iter->IsDoneWithTraversal(); iter->GoToNextItem(), ++counter)
  {
    ...
    svtkDataObject* dataObj = iter->GetCurrentDataObject();
    svtkRectilinearGrid* rg = svtkRectilinearGrid::SafeDownCast(dataObj);
    svtkUniformGrid* ug = svtkUniformGrid::SafeDownCast(dataObj);

    if (ug)
    {
      if (!this->ExtractClippedContourOnBlock<svtkUniformGrid>(fragments, ug, arrayName))
      {
        return nullptr;
      }
    }
    ...
    {
      if (!this->ExtractClippedContourOnBlock<svtkRectilinearGrid>(fragments, rg, arrayName))
      {
        return nullptr;
      }
    }
    ...
    {
      warn_once = false;
      svtkWarningMacro(<< dataObj->GetClassName() << " will be ignored.");
    }
    if ((counter % 1000) == 0)
    {
  ...
  svtkSmartPointer<svtkDataSet> output = svtkSmartPointer<svtkPolyData>::New();
  if (fragments.empty())
  {
...
//-----------------------------------------------------------------------------
// return nullptr on error.
svtkSmartPointer<svtkDataSet> svtkExtractCTHPart::ExtractSolid(
  svtkCompositeDataSet* input, const char* arrayName)
{
  assert(input != nullptr && arrayName != nullptr && arrayName[0] != 0);
  ...
  int counter = 0;
  svtkExtractCTHPart::VectorOfSolids solids;
  for (iter->InitTraversal(); !iter->IsDoneWithTraversal(); iter->GoToNextItem(), ++counter)
  {
    // each iteration is 1/(total num of datasets)'th for the work.
    ScaledProgress sp(counter * 1.0 / this->Internals->TotalNumberOfDatasets,
    ...
    if (ug)
    {
      if (!this->ExtractClippedVolumeOnBlock<svtkUniformGrid>(solids, ug, arrayName))
      {
        return nullptr;
      }
    }
    ...
    {
      if (!this->ExtractClippedVolumeOnBlock<svtkRectilinearGrid>(solids, rg, arrayName))
      {
        return nullptr;
      }
    }
  ...
  svtkSmartPointer<svtkDataSet> output = svtkSmartPointer<svtkUnstructuredGrid>::New();
  if (solids.empty())
  {
...
void svtkExtractCTHPart::DetermineSurfaceValue(int dataType)
{
...
//-----------------------------------------------------------------------------
template <class T>
bool svtkExtractCTHPart::ExtractClippedContourOnBlock(
  svtkExtractCTHPart::VectorOfFragments& fragments, T* dataset, const char* arrayName)
{
  assert(arrayName != nullptr && arrayName[0] != 0 && dataset != nullptr);

  svtkDataArray* volumeFractionArray = dataset->GetCellData()->GetArray(arrayName);
  ...
  if (this->Capping)
  {
    ...
    svtkNew<svtkClipPolyData> scalarClipper;
...
//-----------------------------------------------------------------------------
template <class T>
bool svtkExtractCTHPart::ExtractContourOnBlock(
  svtkExtractCTHPart::VectorOfFragments& fragments, T* dataset, const char* arrayName)
{
  assert(arrayName != nullptr && arrayName[0] != 0 && dataset != nullptr);

  svtkDataArray* volumeFractionArray = dataset->GetPointData()->GetArray(arrayName);
  ...
  // Extract contour.
  svtkNew<svtkContourFilter> contourer;
  ...
  contourer->SetInputArrayToProcess(0, 0, 0, svtkDataObject::FIELD_ASSOCIATION_POINTS, arrayName);
  contourer->Update();

  svtkPolyData* output = svtkPolyData::SafeDownCast(contourer->GetOutputDataObject(0));
...
template <class T>
void svtkExtractCTHPart::ExtractExteriorSurface(
  svtkExtractCTHPart::VectorOfFragments& fragments, T* input)
{
  ...
  //  svtkUnsignedCharArray *ghostArray=static_cast<svtkUnsignedCharArray
...
template <class T>
bool svtkExtractCTHPart::ExtractClippedVolumeOnBlock(
  VectorOfSolids& solids, T* dataset, const char* arrayName)
{
  ...
  // Clip the volume.
  svtkNew<svtkClipDataSet> blockClipper;
  blockClipper->SetInputData(inputClone);
  blockClipper->SetValue(this->VolumeFractionSurfaceValueInternal);
  blockClipper->SetInputArrayToProcess(0, 0, 0, svtkDataObject::FIELD_ASSOCIATION_POINTS, arrayName);
  blockClipper->Update();
  svtkSmartPointer<svtkUnstructuredGrid> solidFragment = blockClipper->GetOutput();
  ...
  svtkNew<svtkClipDataSet> fragClipper;
...
int svtkExtractCTHPart::IsGhostFace(
  int axis0, int maxFlag, int dims[3], svtkUnsignedCharArray* ghostArray)
{
  ...
  int result = ghostArray->GetValue(svtkStructuredData::ComputeCellId(dims, ijk));
  ...
  if (dims[axis1] == 3)
  {
    ...
    ijk[axis1] = 1;
    result = result && ghostArray->GetValue(svtkStructuredData::ComputeCellId(dims, ijk));
  }
...
//----------------------------------------------------------------------------
// Description:
// Merely the same implementation than in svtkDataSetSurfaceFilter, without
// dealing with the whole extents.
void svtkExtractCTHPart::ExecuteFaceQuads(svtkDataSet* input, svtkPolyData* output, int maxFlag,
  int originalExtents[6], int ext[6], int aAxis, int bAxis, int cAxis)
{
  ...
  // However, svtkGeometryFilter created a 2d image as a max face, but the cells are copied
...
//-----------------------------------------------------------------------------
void svtkExtractCTHPart::ExecuteCellDataToPointData(
...
//-----------------------------------------------------------------------------
void svtkExtractCTHPart::TriggerProgressEvent(double val)
{
...
//-----------------------------------------------------------------------------
void svtkExtractCTHPart::PrintSelf(ostream& os, svtkIndent indent)
{
