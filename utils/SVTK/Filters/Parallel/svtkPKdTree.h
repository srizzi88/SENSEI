 ...
 *      svtkDataSets distributed across processors.  We assume each
...
#ifndef svtkPKdTree_h
#define svtkPKdTree_h
...
class SVTKFILTERSPARALLEL_EXPORT svtkPKdTree : public svtkKdTree
{
public:
  svtkTypeMacro(svtkPKdTree, svtkKdTree);

  void PrintSelf(ostream& os, svtkIndent indent) override;
  void PrintTiming(ostream& os, svtkIndent indent) override;
  void PrintTables(ostream& os, svtkIndent indent);

  static svtkPKdTree* New();
   ...
   */
  svtkIdType GetTotalNumberOfCells() { return this->TotalNumCells; }
   ...
   */
  svtkGetMacro(RegionAssignment, int);
  //@}
   ...
   */
  int GetRegionAssignmentList(int procId, svtkIntArray* list);
   ...
   * of a given process.
   */
  void GetAllProcessesBorderingOnPoint(float x, float y, float z, svtkIntArray* list);
   ...
   * processes added.
   */
  int GetProcessListForRegion(int regionId, svtkIntArray* processes);
   ...
   * the supplied svtkIntArray.  Returns the number of regions.
   */
  int GetRegionListForProcess(int processId, svtkIntArray* regions);
   ...
   * This method takes a process ID and two svtkIdLists.  It
   ...
   * uses the cell lists created in svtkKdTree::CreateCellLists().
   ...
   */
  svtkIdType GetCellListsForProcessRegions(
   ...
   * \pre orderedList_exists: orderedList!=0
   */
  int ViewOrderAllProcessesFromPosition(const double cameraPosition[3], svtkIntArray* orderedList);

  /**
   * An added feature of svtkPKdTree is that it will calculate the
...
private:
  ...
  std::vector<std::vector<svtkIdType> > CellCountList; // indexed by region ID
  ...
  int BuildGlobalIndexLists(svtkIdType ncells);

  std::vector<svtkIdType> StartVal;
  std::vector<svtkIdType> EndVal;
  std::vector<svtkIdType> NumCells;
  svtkIdType TotalNumCells;
  ...
  int DivideRegion(svtkKdNode* kd, int L, int level, int tag);
  int BreadthFirstDivide(double* bounds);
  void enQueueNode(svtkKdNode* kd, int L, int level, int tag);
  //@}
  ...
  void RetrieveData(svtkKdNode* kd, int* buf);
#else
  void ReduceData(svtkKdNode* kd, int* sources);
  void BroadcastData(svtkKdNode* kd);
#endif

  void GetDataBounds(int L, int K, int R, float dataBounds[12]);
  void GetLocalMinMax(int L, int R, int me, float* min, float* max);

  static int FillOutTree(svtkKdNode* kd, int level);
  static int ComputeDepth(svtkKdNode* kd);
  static void PackData(svtkKdNode* kd, double* data);
  static void UnpackData(svtkKdNode* kd, double* data);
  static void CheckFixRegionBoundaries(svtkKdNode* tree);
  ...
  void AddProcessRegions(int procId, svtkKdNode* kd);
  ...
  void AddEntry(svtkIdType* list, int len, svtkIdType id);
#endif
  static int BinarySearch(svtkIdType* list, int len, svtkIdType which);

  static int FindNextLocalArrayIndex(
    const char* n, const std::vector<std::string>& names, int len, int start = 0);

  svtkPKdTree(const svtkPKdTree&) = delete;
  void operator=(const svtkPKdTree&) = delete;
};
