...
#include "svtkPSphereSource.h"
#include "svtkPlaneSource.h"
#include "svtkPolyData.h"
#include "svtkStreamingDemandDrivenPipeline.h"
#include "svtksys/FStream.hxx"

svtkStandardNewMacro(svtkPipelineSize);

// Returns size in kibibytes (1024 bytes)
unsigned long svtkPipelineSize::GetEstimatedSize(svtkAlgorithm* input, int inputPort, int connection)
{
  unsigned long sizes[3];
  unsigned long memorySize = 0;

  if (svtkAlgorithmOutput* inInfo = input->GetInputConnection(inputPort, connection))
  {
    if (svtkAlgorithm* srcAlg = svtkAlgorithm::SafeDownCast(inInfo->GetProducer()))
    {
...
void svtkPipelineSize::ComputeSourcePipelineSize(
  svtkAlgorithm* src, int outputPort, unsigned long size[3])
{
  // watch for special sources
  // handle svtkDataReader subclasses
  if (src->IsA("svtkDataReader"))
  {
    svtksys::ifstream* ifs;
    svtkDataReader* rdr = svtkDataReader::SafeDownCast(src);
#ifdef _WIN32
    ifs = new svtksys::ifstream(rdr->GetFileName(), ios::in | ios::binary);
#else
    ifs = new svtksys::ifstream(rdr->GetFileName(), ios::in);
#endif
    if (!ifs->fail())
    {
      ifs->seekg(0, ios::end);
  ...
  svtkLargeInteger sz;
  if (src->IsA("svtkConeSource"))
  {
    svtkConeSource* s = svtkConeSource::SafeDownCast(src);
    sz = s->GetResolution();
    sz = sz * 32 / 1024;
  ...
  if (src->IsA("svtkPlaneSource"))
  {
    svtkPlaneSource* s = svtkPlaneSource::SafeDownCast(src);
    sz = s->GetXResolution();
  ...
  if (src->IsA("svtkPSphereSource"))
  {
    svtkPSphereSource* s = svtkPSphereSource::SafeDownCast(src);
...
void svtkPipelineSize::GenericComputeSourcePipelineSize(
  svtkAlgorithm* src, int outputPort, unsigned long size[3])
{
  unsigned long outputSize[2];
  unsigned long inputPipelineSize[3];
  svtkLargeInteger mySize = 0;
  unsigned long maxSize = 0;
  svtkLargeInteger goingDownstreamSize = 0;
  ...
  for (idx = 0; idx < numberOfInputs; ++idx)
  {
    ...
    inputSize[idx] = 0;
    if (svtkAlgorithmOutput* inInfo = src->GetInputConnection(port, conn))
    {
      if (svtkAlgorithm* srcAlg = svtkAlgorithm::SafeDownCast(inInfo->GetProducer()))
      {
        ...
        svtkDemandDrivenPipeline* ddp =
          svtkDemandDrivenPipeline::SafeDownCast(srcAlg->GetExecutive());
        if (ddp &&
          ddp->GetOutputInformation(inInfo->GetIndex())
            ->Get(svtkDemandDrivenPipeline::RELEASE_DATA()))
        {
...
void svtkPipelineSize::GenericComputeOutputMemorySize(
  svtkAlgorithm* src, int outputPort, unsigned long* svtkNotUsed(inputSize), unsigned long size[2])
{
  int idx;
  svtkLargeInteger tmp = 0;
  svtkLargeInteger sz = 0;

  svtkDemandDrivenPipeline* ddp = svtkDemandDrivenPipeline::SafeDownCast(src->GetExecutive());
  ...
  {
    svtkInformation* outInfo = ddp->GetOutputInformation(idx);
    if (outInfo)
    {
      tmp = 0;
      svtkInformation* dataInfo = outInfo->Get(svtkDataObject::DATA_OBJECT())->GetInformation();
      ...
      if (dataInfo->Get(svtkDataObject::DATA_EXTENT_TYPE()) == SVTK_3D_EXTENT)
      {
        ...
        if (scalarInfo)
        {
          tmp = svtkDataArray::GetDataTypeSize(scalarInfo->Get(svtkDataObject::FIELD_ARRAY_TYPE()));
...
unsigned long svtkPipelineSize::GetNumberOfSubPieces(
  unsigned long memoryLimit, svtkAlgorithm* mapper, int piece, int numPieces)
{
  ...
  {
    oldSize = size;
    svtkInformation* inInfo = mapper->GetInputInformation();
    inInfo->Set(svtkStreamingDemandDrivenPipeline::UPDATE_PIECE_NUMBER(), piece * subDivisions);
    inInfo->Set(
      svtkStreamingDemandDrivenPipeline::UPDATE_NUMBER_OF_PIECES(), numPieces * subDivisions);
...
void svtkPipelineSize::PrintSelf(ostream& os, svtkIndent indent)
{
