/*=========================================================================
  ...
  Module:    svtkPResampleToImage.cxx
...
svtkStandardNewMacro(svtkPResampleToImage);

svtkCxxSetObjectMacro(svtkPResampleToImage, Controller, svtkMultiProcessController);
...
inline void ExtractFieldMetaData(svtkDataSetAttributes* data, std::vector<FieldMetaData>* metadata)
{
  if (data == nullptr || data->GetNumberOfTuples() == 0)
  {
    // do not consider arrays from empty svtkDataSetAttributes.
    // see paraview/paraview#18590
    metadata->clear();
    return;
  }

  if (auto maskArray = data->GetArray("svtkValidPointMask"))
  {
    // data may not be valid if the only array is svtkValidPointMask and it's set
    // to 0.
  ...
  for (std::size_t i = 0; i < numFields; ++i)
  {
    ...
    svtkDataArray* da = data->GetArray(static_cast<int>(i));
...
inline void InitializeFieldData(
  const std::vector<FieldMetaData>& metadata, svtkIdType numTuples, svtkDataSetAttributes* data)
{
  std::size_t numFields = metadata.size();
  for (std::size_t i = 0; i < numFields; ++i)
  {
    const FieldMetaData& md = metadata[i];
    svtkDataArray* da = svtkDataArray::CreateDataArray(md.DataType);
...
struct SerializeWorklet
{
  ...
  void operator()(ArrayType* array, svtkIdType tupleIdx, diy::MemoryBuffer& buffer) const
  {
    using T = svtk::GetAPIType<ArrayType>; // Need value type for diy::save template resolution
    const auto tuple = svtk::DataArrayTupleRange(array)[tupleIdx];
...
inline void SerializeFieldData(svtkFieldData* field, svtkIdType tuple, diy::MemoryBuffer& bb)
{
  int numFields = field->GetNumberOfArrays();
  for (int i = 0; i < numFields; ++i)
  {
    svtkDataArray* da = field->GetArray(i);
    if (!svtkArrayDispatch::Dispatch::Execute(da, SerializeWorklet{}, tuple, bb))
    {
...
struct DeserializeWorklet
{
  ...
  void operator()(ArrayType* array, svtkIdType tupleIdx, diy::MemoryBuffer& buffer) const
  {
    auto tuple = svtk::DataArrayTupleRange(array)[tupleIdx];
...
inline void DeserializeFieldData(diy::MemoryBuffer& bb, svtkFieldData* field, svtkIdType tuple)
{
  int numFields = field->GetNumberOfArrays();
  for (int i = 0; i < numFields; ++i)
  {
    svtkDataArray* da = field->GetArray(i);
    if (!svtkArrayDispatch::Dispatch::Execute(da, DeserializeWorklet{}, tuple, bb))
    {
      svtkGenericWarningMacro(<< "Dispatch failed, fallback to svtkDataArray Get/Set");
...
struct PointList
{
  ...
  svtkIdType DataSize;             // size in bytes of serialized data of one point
};
...
inline svtkIdType ComputeSerializedFieldDataSize(const std::vector<FieldMetaData>& fieldMetaData)
{
  svtkNew<svtkDataSetAttributes> attribs;
  InitializeFieldData(fieldMetaData, 1, attribs);
  diy::MemoryBuffer bb;
  SerializeFieldData(attribs, 0, bb);
  return static_cast<svtkIdType>(bb.buffer.size());
}
...
inline void GetPointsFromImage(svtkImageData* img, const char* maskArrayName, PointList* points)
{
  ...
  svtkPointData* pd = img->GetPointData();
  svtkCharArray* maskArray = svtkArrayDownCast<svtkCharArray>(pd->GetArray(maskArrayName));
  ...
  for (int k = extent[4]; k <= extent[5]; ++k)
  {
    for (int j = extent[2]; j <= extent[3]; ++j)
    {
      ...
      {
        int ijk[3] = { i, j, k };
        svtkIdType id = img->ComputePointId(ijk);
        if (mask[id])
        {
...
void SetPointsToImage(
  const std::vector<FieldMetaData>& fieldMetaData, PointList& points, svtkImageData* img)
{
  svtkPointData* pd = img->GetPointData();
  ...
  {
    svtkIdType id = img->ComputePointId(points.Indices[i].data());
...
inline void GetGlobalFieldMetaData(
  diy::mpi::communicator& comm, svtkDataSetAttributes* data, std::vector<FieldMetaData>* metadata)
{
...
//---------------------------------------------------------------------------
svtkPResampleToImage::svtkPResampleToImage()
  : Controller(nullptr)
{
...
//----------------------------------------------------------------------------
void svtkPResampleToImage::PrintSelf(ostream& os, svtkIndent indent)
{
...
//---------------------------------------------------------------------------
int svtkPResampleToImage::RequestData(
