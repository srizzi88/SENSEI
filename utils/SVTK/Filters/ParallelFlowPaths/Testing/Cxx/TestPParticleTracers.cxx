...
#define EXPECT(expected, actual, msg, so)                                                          \
  if (!(expected == actual))                                                                       \
  {                                                                                                \
    svtkGenericWarningMacro(<< msg << " Expecting a value of " << expected                          \
...
protected:
    ...
    svtkInformation*, svtkInformationVector**, svtkInformationVector* outputInfoVector)
  {
    ...
    outInfo->Set(svtkStreamingDemandDrivenPipeline::TIME_STEPS(), &this->TimeSteps[0],
    ...
    svtkInformationVector* outputVector)
  {
    ...
    if (outImage)
    {
      ...
      // svtkGenericWarningMacro("time step in producer is " << timeStep);
      svtkFloatArray* outArray1 = svtkFloatArray::New();
    ...
    svtkFloatArray* outArray = svtkFloatArray::New();
...
int TestPParticleTracer(svtkMPIController* c, int staticOption)
{
  ...
  svtkNew<svtkPParticleTracer> filter;
...
int TestPParticlePathFilter(svtkMPIController* c, int staticOption)
{
  svtkNew<TestTimeSource> imageSource;
  int size(5);
  imageSource->SetExtent(0, size - 1, 0, 1, 0, size - 1);
  ...
  svtkPolyData* out = filter->GetOutput();
  svtkPointData* pd = out->GetPointData();
...
int TestPStreaklineFilter(svtkMPIController* c, int staticOption)
{
  svtkNew<TestTimeSource> imageSource;
  int size(5);
  imageSource->SetExtent(0, size - 1, 0, 1, 0, size - 1);
  ...
  svtkNew<svtkPStreaklineFilter> filter;
  filter->SetStaticMesh(staticOption);
  filter->SetStaticSeeds(staticOption);
  filter->SetInputConnection(0, imageSource->GetOutputPort());
  filter->SetInputData(1, ps);
...
{
  svtkSmartPointer<svtkMPIController> c = svtkSmartPointer<svtkMPIController>::New();
