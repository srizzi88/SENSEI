...
int TestPStreamGeometry(int argc, char* argv[])
{
  ...
  int size(5);
  svtkNew<TestVectorFieldSource> imageSource;
  ...
  double radius = 0.8;
  double scale = 1;
  double maximumPropagation = radius * scale * 2 * svtkMath::Pi();
  double angle = svtkMath::Pi() / 20;
  int numTraces = 1;
  ...
  tracer->SetInputArrayToProcess(0, 0, 0, svtkDataObject::FIELD_ASSOCIATION_POINTS, "Velocity");
  svtkNew<svtkPolyData> singleSeed;
  {
    svtkNew<svtkPoints> seedPoints;
  ...
  svtkDoubleArray* integrationTime =
    svtkArrayDownCast<svtkDoubleArray>(out->GetPointData()->GetArray("IntegrationTime"));
  for (svtkIdType i = 0; i < out->GetNumberOfPoints(); i++)
  {
    double coord[3];
