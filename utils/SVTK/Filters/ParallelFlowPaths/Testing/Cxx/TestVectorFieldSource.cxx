...
svtkStandardNewMacro(TestVectorFieldSource);

void TestVectorFieldSource::SetBoundingBox(
...
void TestVectorFieldSource::SetExtent(int xMin, int xMax, int yMin, int yMax, int zMin, int zMax)
{
  int modified = 0;
...
void TestVectorFieldSource::ExecuteDataWithInformation(
  svtkDataObject* outData, svtkInformation* outInfo)
{
  svtkImageData* outImage = this->AllocateOutputData(outData, outInfo);
  if (outImage->GetNumberOfPoints() <= 0)
  {
    return;
  }

  svtkDataArray* outArray = svtkArrayDownCast<svtkDataArray>(svtkAbstractArray::CreateArray(SVTK_FLOAT));
  ...
  for (svtkIdType i = 0; i < outImage->GetNumberOfPoints(); i++)
  {
