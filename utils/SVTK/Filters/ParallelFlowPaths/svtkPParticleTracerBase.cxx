...
svtkPParticleTracerBase::svtkPParticleTracerBase()
{
...
//---------------------------------------------------------------------------
svtkPolyData* svtkPParticleTracerBase::Execute(svtkInformationVector** inputVector)
{
  ...
  return svtkParticleTracerBase::Execute(inputVector);
}

//---------------------------------------------------------------------------
bool svtkPParticleTracerBase::SendParticleToAnotherProcess(
  ParticleInformation& info, ParticleInformation& previousInfo, svtkPointData* pd)
{
  if (info.PointId < 0 && info.TailPointId < 0)
  {
    svtkErrorMacro("Bad particle information.");
    assert(0);
    return true;
  }
  ...
  remoteInfo.PreviousPD = svtkSmartPointer<svtkPointData>::New();
  // the point data that we get values from.
  svtkPointData* fromPD = nullptr;
  svtkIdType fromTupleId = -1;
  ...
  for (int i = 0; i < this->ProtoPD->GetNumberOfArrays(); i++)
  {
    ...
    svtkDataArray* arrFrom = fromPD->GetArray(arrName);
    svtkDataArray* arrTo = remoteInfo.PreviousPD->GetArray(i);
  ...
  {
    svtkErrorMacro(<< "Unexpected time value in MPISendList - expected ("
...
//---------------------------------------------------------------------------
void svtkPParticleTracerBase::AssignSeedsToProcessors(double t, svtkDataSet* source, int sourceID,
  int ptId, ParticleVector& localSeedPoints, int& localAssignedCount)
{
  ...
  this->Controller->AllReduce(
    owningProcess.data(), realOwningProcess.data(), numSeeds, svtkCommunicator::MAX_OP);
...
//---------------------------------------------------------------------------
void svtkPParticleTracerBase::AssignUniqueIds(
  svtkParticleTracerBaseNamespace::ParticleVector& localSeedPoints)
{
  ...
  if (this->Controller->GetNumberOfProcesses() > 1)
  {
    ...
    std::vector<svtkIdType> recvNumParticles(this->Controller->GetNumberOfProcesses(), 0);
...
//---------------------------------------------------------------------------
bool svtkPParticleTracerBase::SendReceiveParticles(
  RemoteParticleVector& sParticles, RemoteParticleVector& rParticles)
{
  ...
  svtkIdType messageSize = numParticles * typeSize;
  ...
  for (int i = 0; i < numParticles; i++)
  {
    ...
    svtkPointData* pd = sParticles[i].PreviousPD;
    char* data = &sendMessage[i * typeSize] + 2 * size1;
    for (int j = 0; j < nArrays; j++)
    {
      svtkDataArray* arr = pd->GetArray(j);
  ...
  std::vector<svtkIdType> messageLength(this->Controller->GetNumberOfProcesses(), 0);
  std::vector<svtkIdType> messageOffset(this->Controller->GetNumberOfProcesses(), 0);
  ...
  std::vector<svtkIdType> owningProcess(numAllParticles, -1);
  ...
  std::vector<svtkIdType> realOwningProcess(numAllParticles);
  if (numAllParticles)
  {
    this->Controller->AllReduce(
      owningProcess.data(), realOwningProcess.data(), numAllParticles, svtkCommunicator::MAX_OP);
  }
  ...
  int counter = 0;
  for (std::vector<svtkIdType>::iterator it = realOwningProcess.begin();
  ...
  for (int i = 0; i < numAllParticles; i++)
  {
    if (realOwningProcess[i] == myRank)
    {
      ...
      rParticles[counter].PreviousPD = svtkSmartPointer<svtkPointData>::New();
      rParticles[counter].PreviousPD->CopyAllocate(this->ProtoPD);
      svtkPointData* pd = rParticles[counter].PreviousPD;
...
//---------------------------------------------------------------------------
int svtkPParticleTracerBase::RequestUpdateExtent(
  svtkInformation* request, svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  int numSources = inputVector[1]->GetNumberOfInformationObjects();
  for (int i = 0; i < numSources; i++)
  {
    svtkInformation* sourceInfo = inputVector[1]->GetInformationObject(i);
    if (sourceInfo)
    {
...
//---------------------------------------------------------------------------
void svtkPParticleTracerBase::PrintSelf(ostream& os, svtkIndent indent)
{
...
//---------------------------------------------------------------------------
bool svtkPParticleTracerBase::UpdateParticleListFromOtherProcesses()
{
  if (!this->Controller)
  {
    return false;
  ...
  for (size_t i = 0; i < received.size(); i++)
  {
    ...
    info.Current.TailPointId = info.Previous.TailPointId =
      static_cast<svtkIdType>(this->Tail.size());
...
//---------------------------------------------------------------------------
bool svtkPParticleTracerBase::IsPointDataValid(svtkDataObject* input)
{
  if (this->Controller->GetNumberOfProcesses() == 1)
  {
  ...
  if (this->Controller->GetLocalProcessId() == 0)
  {
    ...
    {
      this->GetPointDataArrayNames(svtkDataSet::SafeDownCast(input), arrayNames);
    }
    stream << retVal;
  ...
  if (this->Controller->GetLocalProcessId() != 0)
  {
    ...
    {
      this->GetPointDataArrayNames(svtkDataSet::SafeDownCast(input), tempNames);
  ...
  int tmp = retVal;
  this->Controller->AllReduce(&tmp, &retVal, 1, svtkCommunicator::MIN_OP);

  return (retVal != 0);
}

//---------------------------------------------------------------------------
svtkCxxSetObjectMacro(svtkPParticleTracerBase, Controller, svtkMultiProcessController);
//---------------------------------------------------------------------------
