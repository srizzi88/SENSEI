...
svtkUnstructuredGrid* global::Grid;

int CheckGrid(svtkUnstructuredGrid* ghostGrid, const int iteration)
{
  int rc = 0;
  ...
  svtkMPIController* cntrl =
    svtkMPIController::SafeDownCast(svtkMultiProcessController::GetGlobalController());

  // check node fields by the iteration number
  svtkDoubleArray* nodeXYZ =
    svtkArrayDownCast<svtkDoubleArray>(ghostGrid->GetPointData()->GetArray("NodeXYZ"));
  ...
  for (svtkIdType nodeIdx = 0; nodeIdx < ghostGrid->GetNumberOfPoints(); ++nodeIdx)
  {
    ...
    for (int dim = 0; dim < 3; ++dim)
    {
      ...
      if (!svtkMathUtilities::NearlyEqual<double>(actual, expected))
      {
        ++numOfErrors;
        ++rc;
  ...
  out << err.str();
  svtkMPIUtilities::SynchronizedPrintf(cntrl, "%s", out.str().c_str());

  // likewise, check cell-fields
  svtkDoubleArray* cellXYZ =
    svtkArrayDownCast<svtkDoubleArray>(ghostGrid->GetCellData()->GetArray("CentroidXYZ"));
  assert("pre: nodeXYZ numtuples mismatch!" &&
  ...
  for (svtkIdType cellIdx = 0; cellIdx < ghostGrid->GetNumberOfCells(); ++cellIdx)
  {
    ...
    centroid[1] = 0.0;
    centroid[2] = 0.0;
    for (svtkIdType n = 0; n < ptIds->GetNumberOfIds(); ++n)
    {
      svtkIdType idx = ptIds->GetId(n);
...
void UpdateGrid(const int iteration)
{
  ...
  svtkDoubleArray* nodeXYZ =
    svtkArrayDownCast<svtkDoubleArray>(global::Grid->GetPointData()->GetArray("NodeXYZ"));
  assert("pre: nodeXYZ != nullptr" && (nodeXYZ != nullptr));
  assert("pre: nodeXYZ numtuples mismatch!" &&
  ...
  double* ptr = static_cast<double*>(nodeXYZ->GetVoidPointer(0));
  for (svtkIdType nodeIdx = 0; nodeIdx < global::Grid->GetNumberOfPoints(); ++nodeIdx)
  {
  ...
  svtkDoubleArray* cellXYZ =
    svtkArrayDownCast<svtkDoubleArray>(global::Grid->GetCellData()->GetArray("CentroidXYZ"));
  assert("pre: nodeXYZ numtuples mismatch!" &&
    (global::Grid->GetNumberOfCells() == cellXYZ->GetNumberOfTuples()));
  assert("pre: nodeXYZ numcomponents mismatch!" && (cellXYZ->GetNumberOfComponents() == 3));

  double* cptr = static_cast<double*>(cellXYZ->GetVoidPointer(0));
  for (svtkIdType cellIdx = 0; cellIdx < global::Grid->GetNumberOfCells(); ++cellIdx)
  {
...
{
  svtkDoubleArray* centerXYZ = svtkDoubleArray::New();
  ...
  double centroid[3];
  svtkIdList* ptIds = svtkIdList::New();
  for (svtkIdType cell = 0; cell < global::Grid->GetNumberOfCells(); ++cell)
  {
...
//------------------------------------------------------------------------------
void WriteDataSet(svtkUnstructuredGrid* grid, const std::string& file)
{
  std::ostringstream oss;
  oss << file << "-" << global::Rank << ".svtk";
...
void GenerateDataSet()
{
  ...
  svtkExtentRCBPartitioner* partitioner = svtkExtentRCBPartitioner::New();
  ...
  int numNodes = svtkStructuredData::GetNumberOfPoints(ext, dataDescription);
  int numCells = svtkStructuredData::GetNumberOfCells(ext, dataDescription);

  int dims[3];
  svtkStructuredData::GetDimensionsFromExtent(ext, dims, dataDescription);

  svtkPoints* nodes = svtkPoints::New();
  ...
  svtkIdType* globalIdxPtr = static_cast<svtkIdType*>(globalIds->GetVoidPointer(0));
  ...
  svtkIdType globalNodeIdx;
  svtkIdType localNodeIdx;
  svtkIdType cell[8];
    ...
    for (int j = JMIN(ext); j < JMAX(ext); ++j)
    {
      for (int k = KMIN(ext); k < KMAX(ext); ++k)
      {
        ...
        for (int node = 0; node < 8; ++node)
        {
          ...
          localNodeIdx = svtkStructuredData::ComputePointId(dims, ijk, dataDescription);
          ...
          globalNodeIdx = svtkStructuredData::ComputePointId(global::Dims, IJK, dataDescription);

          globalIdxPtr[localNodeIdx] = globalNodeIdx;
          GetPoint(I(IJK), J(IJK), K(IJK), &nodesPtr[localNodeIdx * 3]);
        } // END for all nodes

        global::Grid->InsertNextCell(SVTK_HEXAHEDRON, 8, cell);
      } // END for all k
    }   // END for all j
  }     // END for all i
