/*=========================================================================
  ...
  Module:    svtkPConnectivityFilter.cxx
...
typedef svtkTypeList::Unique<
  svtkTypeList::Create<svtkAOSDataArrayTemplate<int>, svtkAOSDataArrayTemplate<unsigned long>,
    svtkAOSDataArrayTemplate<char>, svtkAOSDataArrayTemplate<unsigned char>,
    svtkAOSDataArrayTemplate<float>, svtkAOSDataArrayTemplate<double> > >::Result PointArrayTypes;

struct WorkerBase
{
  WorkerBase(svtkMPIController* subController)
   ...
   */
  svtkWeakPointer<svtkMPIController> SubController;
};
...
{
  ExchangeBoundsWorker(svtkMPIController* subController)
    : WorkerBase(subController)
  {
    memset(this->Bounds, 0, sizeof(this->Bounds));
  }

  bool Execute(const double bounds[6], const svtkSmartPointer<svtkDataArray>& allBoundsArray)
  {
    memcpy(this->Bounds, bounds, 6 * sizeof(double));
    this->AllBoundsArray = allBoundsArray;

    using Dispatcher = svtkArrayDispatch::DispatchByArray<PointArrayTypes>;
  ...
  void operator()(TArray* allBounds)
  {
    ...
    double bounds[6];
    svtkBoundingBox myBoundingBox(this->Bounds);
...
protected:
  ...
  svtkWeakPointer<svtkDataArray> AllBoundsArray;
};
...
{
  FindMyNeighborsWorker(svtkMPIController* subController)
    : WorkerBase(subController)
    , MyNeighbors(nullptr)
  {
    memset(this->Bounds, 0, sizeof(this->Bounds));
  ...
  void DoExecute()
  {
    ...
    for (int p = 0; p < this->SubController->GetNumberOfProcesses(); ++p)
    {
      ...
      svtkBoundingBox potentialNeighborBoundingBox(potentialNeighborBounds);
...
{
  AssemblePointsAndRegionIdsWorker(svtkMPIController* subController)
    : WorkerBase(subController)
  ...
  bool Execute(const std::vector<int>& regionStarts,
    const svtkSmartPointer<svtkDataArray>& allBoundsArray,
    const svtkSmartPointer<svtkPointSet>& localResult,
    std::map<int, svtkSmartPointer<svtkDataArray> >& pointsForMyNeighbors,
    std::map<int, svtkSmartPointer<svtkIdTypeArray> >& regionIdsForMyNeighbors)
  {
  ...
  void operator()(TArray* allBounds)
  {
    ...
    int myRank = this->SubController->GetLocalProcessId();
    svtkPoints* outputPoints = this->LocalResult->GetPoints();
    TArray* pointArray = TArray::SafeDownCast(outputPoints->GetData());
    svtkPointData* outputPD = this->LocalResult->GetPointData();
    svtkIdTypeArray* pointRegionIds = svtkIdTypeArray::SafeDownCast(outputPD->GetArray("RegionId"));
    for (int p = 0; p < this->SubController->GetNumberOfProcesses(); ++p)
    {
      if (myRank == p)
      {
      ...
      (*this->RegionIdsForMyNeighbors)[p] = svtkSmartPointer<svtkIdTypeArray>::New();

      typename TArray::ValueType bb[6];
      allBounds->GetTypedTuple(p, bb);

      svtkBoundingBox neighborBB(bb[0], bb[1], bb[2], bb[3], bb[4], bb[5]);
      for (svtkIdType id = 0; id < this->LocalResult->GetNumberOfPoints(); ++id)
      {
...
protected:
  ...
  svtkWeakPointer<svtkPointSet> LocalResult;
...
struct SendReceivePointsWorker : public WorkerBase
{
  ...
  SendReceivePointsWorker(svtkMPIController* subController)
    : WorkerBase(subController)
  ...
  bool Execute(const svtkSmartPointer<svtkDataArray>& allBoundsArray,
    const std::map<int, int>& sendLengths, const std::map<int, int>& recvLengths,
    const std::map<int, svtkSmartPointer<svtkDataArray> >& pointsForMyNeighbors,
    const std::map<int, svtkSmartPointer<svtkIdTypeArray> >& regionIdsForMyNeighbors,
    std::map<int, svtkSmartPointer<svtkDataArray> >& pointsFromMyNeighbors,
    std::map<int, svtkSmartPointer<svtkIdTypeArray> >& regionIdsFromMyNeighbors)
  {
  ...
  template <class TArray>
  void operator()(TArray* svtkNotUsed(array))
  {
    ...
    std::map<int, svtkMPICommunicator::Request> sendRequestsPoints;
    std::map<int, svtkMPICommunicator::Request> sendRequestsRegionIds;
    std::vector<svtkMPICommunicator::Request> recvRequestsPoints(this->RecvLengths.size());
    std::vector<svtkMPICommunicator::Request> recvRequestsRegionIds(this->RecvLengths.size());
    ...
    {
      svtkIdType toRank = nbrIter->first;
      svtkIdType numToRank = nbrIter->second;
      if (numToRank > 0)
      {
        ...
        svtkIdTypeArray* idArray = this->RegionIdsForMyNeighbors.at(toRank);
 ...
 */
void ExchangeNumberOfPointsToSend(svtkMPIController* subController,
  ...
  std::map<int, int>& sendLengths, std::map<int, int>& recvLengths)
{
  ...
  recvLengths.clear();
  std::vector<svtkMPICommunicator::Request> recvRequests(myNeighbors.size());
  int requestIdx = 0;
  ...
  std::map<int, svtkMPICommunicator::Request> sendRequests;
...
svtkStandardNewMacro(svtkPConnectivityFilter);

svtkPConnectivityFilter::svtkPConnectivityFilter() {}

svtkPConnectivityFilter::~svtkPConnectivityFilter() {}

int svtkPConnectivityFilter::RequestData(
  svtkInformation* request, svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  svtkMultiProcessController* globalController = svtkMultiProcessController::GetGlobalController();
  ...
  if (globalController)
  {
    globalController->AllReduce(&hasCells, &ranksWithCells, 1, svtkCommunicator::SUM_OP);
  ...
  if (numRanks > 1 && ranksWithCells > 1)
  {
    ...
    {
      svtkErrorMacro("ExtractionMode " << this->GetExtractionModeAsString()
  ...
  subController.TakeReference(
    svtkMPIController::SafeDownCast(globalController)->PartitionController(hasCells, 0));
  ...
  if (!output->GetPoints())
  {
    svtkErrorMacro("No points in data set");
    success = 0;
  }

  // Check that all ranks succeeded in local connectivity.
  int globalSuccess = 0;
  subController->AllReduce(&success, &globalSuccess, 1, svtkCommunicator::MIN_OP);

  if (!globalSuccess)
  {
    svtkErrorMacro("An error occurred on at least one process.");
    return 0;
  }
  ...
  svtkSmartPointer<svtkDataArray> allBoundsArray;
  ...
  {
    svtkErrorMacro("Unsupported points array type encountered when exchanging bounds.");
    return 0;
  }
  ...
  {
    svtkErrorMacro("Unsupported points array type encountered when finding neighbors.");
    return 0;
  }
  ...
  {
    svtkErrorMacro("Unsupported points array type encountered when sending and receiving points.");
    return 0;
  }
  ...
  if (output->GetNumberOfPoints() > 0)
  {
    ...
    for (int rank = 0; rank < numRanks; ++rank)
    {
      ...
      for (svtkIdType ptId = 0; ptId < pointsFromMyNeighbors[rank]->GetNumberOfTuples(); ++ptId)
      {
        double x[3];
        pointsFromMyNeighbors[rank]->GetTuple(ptId, x);

        svtkIdType localId = localPointLocator->FindClosestPoint(x);
        ...
        if (pointGhostArray &&
          pointGhostArray->GetTypedComponent(localId, 0) & svtkDataSetAttributes::DUPLICATEPOINT)
        {
          continue;
        }
        double y[3];
        output->GetPoints()->GetPoint(localId, y);
        double dist2 = svtkMath::Distance2BetweenPoints(x, y);
        if (dist2 > 1e-6)
        {
        ...
        svtkIdTypeArray* localRegionIds =
          svtkIdTypeArray::SafeDownCast(outputPD->GetArray("RegionId"));
        svtkIdType localRegionId =
          localRegionIds->GetTypedComponent(localId, 0) + regionStarts[myRank];

        svtkIdType remoteRegionId = regionIdsFromMyNeighbors[rank]->GetTypedComponent(ptId, 0);
  ...
  std::vector<svtkIdType> localLinks;
  for (size_t i = 0; i < links.size(); ++i)
  {
    for (std::set<svtkIdType>::iterator iter = std::begin(links[i]); iter != std::end(links[i]);
         ++iter)
    {
      localLinks.push_back(static_cast<svtkIdType>(i));
  ...
  svtkIdType localNumLinks = static_cast<svtkIdType>(localLinks.size());
  std::vector<svtkIdType> linkCounts(numRanks, -1);
  std::vector<svtkIdType> linkStarts(numRanks + 1, 0);
  ...
  std::vector<svtkIdType> allLinks(linkStarts[numRanks]);

  subController->AllGatherV(&localLinks[0], &allLinks[0], static_cast<svtkIdType>(localLinks.size()),
  ...
  typedef struct _RegionNode
  {
    ...
    svtkIdType OriginalRegionId;

    // Current local region id
    svtkIdType CurrentRegionId;

    std::vector<svtkIdType> Links;
  } RegionNode;

  size_t linkIdx = 0;
  std::vector<RegionNode> regionNodes(regionStarts[numRanks]);
  for (svtkIdType regionId = 0; regionId < regionStarts[numRanks]; ++regionId)
  {
  ...
  do
  {
    ...
    {
      for (std::vector<svtkIdType>::iterator linkIter = nodeIter->Links.begin();
           linkIter != nodeIter->Links.end(); ++linkIter)
      {
        svtkIdType linkedRegionId = *linkIter;
  ...
  // algorithm.
  std::set<svtkIdType> currentRegionIds;
  for (std::vector<RegionNode>::iterator nodeIter = regionNodes.begin();
       nodeIter != regionNodes.end(); ++nodeIter)
  {
  ...
  std::map<svtkIdType, svtkIdType> relabeledRegionMap;
  svtkIdType contiguousLabel = 0;
  for (std::set<svtkIdType>::iterator setIter = currentRegionIds.begin();
  ...
  std::vector<svtkIdType> regionIdMap(regionNodes.size(), -1);
  for (std::vector<RegionNode>::iterator nodeIter = regionNodes.begin();
       nodeIter != regionNodes.end(); ++nodeIter)
  {
  ...
  svtkIdTypeArray* cellRegionIds = svtkIdTypeArray::SafeDownCast(outputCD->GetArray("RegionId"));
  for (svtkIdType i = 0; i < output->GetNumberOfCells(); ++i)
  {
    // Offset the cellRegionId by the starting region id on this rank.
    svtkIdType cellRegionId = cellRegionIds->GetValue(i) + regionStarts[myRank];
  ...
  for (svtkIdType i = 0; i < output->GetNumberOfPoints(); ++i)
  {
    ...
    svtkIdType pointRegionId = pointRegionIds->GetValue(i) + regionStarts[myRank];
    pointRegionIds->SetValue(i, regionNodes[pointRegionId].CurrentRegionId);
  }

  // Sum up number of cells in each region.
  svtkIdType numContiguousLabels = contiguousLabel;
  std::vector<svtkIdType> localRegionSizes(numContiguousLabels, 0);
  if (cellRegionIds)
  {
    ...
    for (svtkIdType i = 0; i < cellRegionIds->GetNumberOfValues(); ++i)
    {
      if (cellGhostArray &&
        cellGhostArray->GetTypedComponent(i, 0) & svtkDataSetAttributes::DUPLICATECELL)
      {
        continue;
      }
  ...
  std::vector<svtkIdType> globalRegionSizes(numContiguousLabels, 0);
  subController->AllReduce(
    &localRegionSizes[0], &globalRegionSizes[0], numContiguousLabels, svtkCommunicator::SUM_OP);
  ...
  for (svtkIdType i = 0; i < numContiguousLabels; ++i)
  {
  ...
  if (this->ExtractionMode == SVTK_EXTRACT_LARGEST_REGION ||
    this->ExtractionMode == SVTK_EXTRACT_CLOSEST_POINT_REGION)
  {
    double threshold = 0.0;
    if (this->ExtractionMode == SVTK_EXTRACT_LARGEST_REGION)
    {
      svtkIdType largestRegionCount = 0;
      svtkIdType largestRegionId = 0;
      for (svtkIdType i = 0; i < this->RegionSizes->GetNumberOfTuples(); ++i)
      {
        svtkIdType candidateCount = this->RegionSizes->GetValue(i);
    ...
    else if (this->ExtractionMode == SVTK_EXTRACT_CLOSEST_POINT_REGION)
    {
      ...
      double minDist2 = SVTK_DOUBLE_MAX;
      svtkIdType minId = 0;
      for (svtkIdType i = 0; i < output->GetNumberOfPoints(); ++i)
      {
        double x[3];
        output->GetPoint(i, x);
        double dist2 = svtkMath::Distance2BetweenPoints(x, this->ClosestPoint);
        if (dist2 < minDist2)
        {
          minDist2 = dist2;
          minId = i;
      ...
      double globalMinDist2 = SVTK_DOUBLE_MAX;
      subController->AllReduce(&minDist2, &globalMinDist2, 1, svtkCommunicator::MIN_OP);

      int minDist2Rank = 0;
      svtkIdType minDist2Region = 0;
      ...
      subController->AllReduce(&minDist2Rank, &globalMinDist2Rank, 1, svtkCommunicator::MAX_OP);
    ...
    svtkNew<svtkThreshold> thresholder;
    thresholder->SetInputData(output);
    thresholder->ThresholdBetween(threshold, threshold);
    thresholder->SetInputArrayToProcess(
      0, 0, 0, svtkDataObject::FIELD_ASSOCIATION_CELLS, "RegionId");
    thresholder->Update();

    if (output->IsA("svtkPolyData"))
    {
      // It's too bad we have to do this, but svtkThreshold produces
...
void svtkPConnectivityFilter::PrintSelf(ostream& os, svtkIndent indent)
{
