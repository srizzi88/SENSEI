/*=========================================================================
 ...
 Module:    svtkPStructuredGridConnectivity.cxx
...
svtkStandardNewMacro(svtkPStructuredGridConnectivity);

//------------------------------------------------------------------------------
svtkPStructuredGridConnectivity::svtkPStructuredGridConnectivity()
{
...
//------------------------------------------------------------------------------
void svtkPStructuredGridConnectivity::PrintSelf(std::ostream& os, svtkIndent indent)
{
...
//------------------------------------------------------------------------------
void svtkPStructuredGridConnectivity::Initialize()
{
  if (!this->Initialized)
  {
...
//------------------------------------------------------------------------------
void svtkPStructuredGridConnectivity::SetNumberOfGrids(const unsigned int N)
{
  if (N == 0)
  {
...
//------------------------------------------------------------------------------
void svtkPStructuredGridConnectivity::RegisterGrid(const int gridID, int extents[6],
...
//------------------------------------------------------------------------------
void svtkPStructuredGridConnectivity::RegisterRemoteGrid(
...
//------------------------------------------------------------------------------
void svtkPStructuredGridConnectivity::ComputeNeighbors()
{
...
//------------------------------------------------------------------------------
void svtkPStructuredGridConnectivity::CreateGhostLayers(const int N)
{
  assert("pre: Instance has not been initialized!" && this->Initialized);
  if (N == 0)
  {
...
//------------------------------------------------------------------------------
void svtkPStructuredGridConnectivity::TransferRemoteNeighborData(
  const int gridIdx, const int nei, const svtkStructuredNeighbor& Neighbor)
{
  // Sanity check
    ...
    for (int j = Neighbor.RcvExtent[2]; j <= Neighbor.RcvExtent[3]; ++j)
    {
      for (int k = Neighbor.RcvExtent[4]; k <= Neighbor.RcvExtent[5]; ++k)
      {
        ...
        if (this->HasPoints(gridIdx))
        {
          ...
          svtkIdType srcIdx =
            svtkStructuredData::ComputePointIdForExtent(const_cast<int*>(Neighbor.RcvExtent), ijk);
        ...
        if (this->HasCellData(gridIdx) && this->IsNodeWithinExtent(i, j, k, RcvCellExtent))
        {
          ...
          // and not a point ID.
          svtkIdType sourceCellIdx = svtkStructuredData::ComputePointIdForExtent(RcvCellExtent, ijk);
...
//------------------------------------------------------------------------------
void svtkPStructuredGridConnectivity::TransferGhostDataFromNeighbors(const int gridID)
{
...
//------------------------------------------------------------------------------
void svtkPStructuredGridConnectivity::PackGhostData()
{
...
//------------------------------------------------------------------------------
void svtkPStructuredGridConnectivity::SerializeBufferSizes(int*& sizesbuf, svtkIdType& N)
{
...
//------------------------------------------------------------------------------
void svtkPStructuredGridConnectivity::DeserializeBufferSizesForProcess(
  int* buffersizes, svtkIdType N, const int svtkNotUsed(processId))
{
...
//------------------------------------------------------------------------------
void svtkPStructuredGridConnectivity::ExchangeBufferSizes()
{
  svtkIdType N = 0;
  ...
  svtkIdType numRanks = this->Controller->GetNumberOfProcesses();
  svtkIdType* rcvcounts = new svtkIdType[numRanks];
  this->Controller->AllGather(&N, rcvcounts, 1);

  // STEP 2: Calculate the receive buffer size & Allocate
  svtkIdType rcvBufferSize = rcvcounts[0];
  ...
  svtkIdType* offSet = new svtkIdType[numRanks];
  offSet[0] = 0;
  for (int i = 1; i < numRanks; ++i)
  {
...
//------------------------------------------------------------------------------
void svtkPStructuredGridConnectivity::UnpackGhostData()
{
  assert(
    "pre: RcvBuffers is not properly allocated!" && this->RcvBuffers.size() == this->NumberOfGrids);
...
//------------------------------------------------------------------------------
void svtkPStructuredGridConnectivity::ExchangeGhostDataInit()
{
...
//------------------------------------------------------------------------------
void svtkPStructuredGridConnectivity::PostReceives()
{
  // STEP 0: Acquire MPI controller from supplied Multi-Process controller
  svtkMPIController* myMPIController = svtkMPIController::SafeDownCast(this->Controller);
  ...
  for (unsigned int idx = 0; idx < this->GridIds.size(); ++idx)
  {
    ...
    for (size_t nei = 0; nei < NumNeis; ++nei)
    {
      ...
      if (this->IsGridLocal(neiGridIdx))
      {
        ...
        // directly using svtkStructuredGrid::TransferLocalNeighborData().
...
//------------------------------------------------------------------------------
void svtkPStructuredGridConnectivity::PostSends()
{
  // STEP 0: Acquire MPI controller from supplied Multi-Process controller
  svtkMPIController* myMPIController = svtkMPIController::SafeDownCast(this->Controller);
  assert("pre: Cannot acquire MPI controller" && (myMPIController != nullptr));
...
//------------------------------------------------------------------------------
void svtkPStructuredGridConnectivity::CommunicateGhostData()
{
  ...
  this->MPIRequests = new svtkMPICommunicator::Request[this->TotalNumberOfMsgs];
...
//------------------------------------------------------------------------------
void svtkPStructuredGridConnectivity::ExchangeGhostDataPost()
{
  svtkMPIController* myMPIController = svtkMPIController::SafeDownCast(this->Controller);
  assert("pre: Cannot acquire MPI controller" && (myMPIController != nullptr));
...
//------------------------------------------------------------------------------
void svtkPStructuredGridConnectivity::ExchangeGhostData()
{
  assert("pre: Instance has not been initialized!" && this->Initialized);
  ...
  // de-serialized into the SVTK data-structures.
...
//------------------------------------------------------------------------------
void svtkPStructuredGridConnectivity::SerializeGhostPoints(
  const int gridIdx, int ext[6], svtkMultiProcessStream& bytestream)
{
  ...
  int dataDescription = svtkStructuredData::GetDataDescriptionFromExtent(ext);
  int N = svtkStructuredData::GetNumberOfPoints(ext, dataDescription);
  //  bytestream << N;
  ...
  for (int i = ext[0]; i <= ext[1]; ++i)
  {
    for (int j = ext[2]; j <= ext[3]; ++j)
    {
      for (int k = ext[4]; k <= ext[5]; ++k)
      {
        ...
        svtkIdType sourceIdx = svtkStructuredData::ComputePointIdForExtent(GridExtent, ijk);
        ...
        svtkIdType targetIdx = svtkStructuredData::ComputePointIdForExtent(ext, ijk, dataDescription);
...
//------------------------------------------------------------------------------
void svtkPStructuredGridConnectivity::DeserializeGhostPoints(
  const int gridIdx, const int nei, int ext[6], svtkMultiProcessStream& bytestream)
{
  ...
  this->RemotePoints[gridIdx][nei] = svtkPoints::New();
...
//------------------------------------------------------------------------------
void svtkPStructuredGridConnectivity::SerializeDataArray(
  svtkDataArray* dataArray, svtkMultiProcessStream& bytestream)
{
  ...
  svtkIdType N = dataArray->GetNumberOfTuples();
  ...
  switch (dataArray->GetDataType())
  {
    ...
    case SVTK_INT:
      ...
      break;
    default:
      svtkErrorMacro("Cannot serialize data array of type" << dataArray->GetDataType());
  }
}

//------------------------------------------------------------------------------
void svtkPStructuredGridConnectivity::DeserializeDataArray(svtkDataArray*& dataArray,
  ...
  svtkMultiProcessStream& bytestream)
{
  ...
  switch (dataType)
  {
    case SVTK_FLOAT:
    {
      ...
      // STEP 1: Allocate svtkdata array
    ...
    break;
    default:
      svtkErrorMacro("Cannot de-serialize data array of this type");
      assert(false);
  }
}

//------------------------------------------------------------------------------
void svtkPStructuredGridConnectivity::SerializeFieldData(
  int GridExtent[6], int ext[6], svtkFieldData* fieldData, svtkMultiProcessStream& bytestream)
{
  ...
  {
    svtkDataArray* myArray = fieldData->GetArray(array);
    ...
    int numTuples = svtkStructuredData::GetNumberOfPoints(ext);
    ...
    svtkDataArray* ghostArray = svtkDataArray::CreateDataArray(myArray->GetDataType());
    ...
    for (int i = ext[0]; i <= ext[1]; ++i)
    {
      for (int j = ext[2]; j <= ext[3]; ++j)
      {
        for (int k = ext[4]; k <= ext[5]; ++k)
        {
          ...
          // be a cell index if the incoming GridExtent and ext are cell extents.
          svtkIdType targetIdx = svtkStructuredData::ComputePointIdForExtent(ext, ijk);
...
//------------------------------------------------------------------------------
void svtkPStructuredGridConnectivity::DeserializeFieldData(
  int* svtkNotUsed(ext), svtkFieldData* fieldData, svtkMultiProcessStream& bytestream)
{
  assert("pre: Cannot deserialize an empty bytestream" && !bytestream.Empty());
...
//------------------------------------------------------------------------------
void svtkPStructuredGridConnectivity::SerializeGhostPointData(
  const int gridIdx, int ext[6], svtkMultiProcessStream& bytestream)
{
...
//------------------------------------------------------------------------------
void svtkPStructuredGridConnectivity::DeserializeGhostPointData(
  const int gridIdx, const int nei, int ext[6], svtkMultiProcessStream& bytestream)
{
  assert("pre: Cannot deserialize an empty bytestream" && !bytestream.Empty());
  assert("pre:Grid index is out-of-bounds" && (gridIdx >= 0) &&
  ...
  this->RemotePointData[gridIdx][nei] = svtkPointData::New();
  this->DeserializeFieldData(ext, this->RemotePointData[gridIdx][nei], bytestream);
}

//------------------------------------------------------------------------------
void svtkPStructuredGridConnectivity::SerializeGhostCellData(
  const int gridIdx, int ext[6], svtkMultiProcessStream& bytestream)
{
  assert("pre: Grid to be serialized must be local" && this->IsGridLocal(gridIdx));
  assert("pre: gridIdx is out-of-bounds" && (gridIdx >= 0) &&
  ...
  svtkStructuredData::GetCellExtentFromPointExtent(GridExtent, GridCellExtent);

  // STEP 1: Get the cell extent of the sub-extent
  int cellExtent[6];
  svtkStructuredData::GetCellExtentFromPointExtent(ext, cellExtent);
...
//------------------------------------------------------------------------------
void svtkPStructuredGridConnectivity::DeserializeGhostCellData(
  const int gridIdx, const int nei, int ext[6], svtkMultiProcessStream& bytestream)
{
  assert("pre: Cannot deserialize an empty bytestream" && !bytestream.Empty());
  assert("pre:Grid index is out-of-bounds" && (gridIdx >= 0) &&
  ...
  this->RemoteCellData[gridIdx][nei] = svtkCellData::New();
  int cellext[6];
  svtkStructuredData::GetCellExtentFromPointExtent(ext, cellext);
  this->DeserializeFieldData(cellext, this->RemoteCellData[gridIdx][nei], bytestream);
}

//------------------------------------------------------------------------------
void svtkPStructuredGridConnectivity::SerializeGhostData(const int sendGridID, const int rcvGrid,
...
//------------------------------------------------------------------------------
void svtkPStructuredGridConnectivity::DeserializeGhostData(const int gridID, const int neiGridID,
  const int svtkNotUsed(neiGridIdx), int rcvext[6], unsigned char* buffer, unsigned int size)
{
...
//------------------------------------------------------------------------------
void svtkPStructuredGridConnectivity::ExchangeGridExtents()
{
  assert("pre: Instance has not been initialized!" && this->Initialized);
...
//------------------------------------------------------------------------------
void svtkPStructuredGridConnectivity::SerializeGridExtents(int*& sndbuffer, svtkIdType& N)
{
  assert("pre: Instance has not been initialized!" && this->Initialized);
...
//------------------------------------------------------------------------------
void svtkPStructuredGridConnectivity::DeserializeGridExtentForProcess(
  int* rcvbuffer, svtkIdType& N, const int processId)
{
  // Sanity checks
  assert("pre: Instance has not been initialized!" && this->Initialized);
