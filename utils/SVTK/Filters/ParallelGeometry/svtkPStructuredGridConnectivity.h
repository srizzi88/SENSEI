/*=========================================================================
 ...
 Module:    svtkPStructuredGridConnectivity.h
 ...
 *
 *
 *  svtkPStructuredGridConnectivity inherits from svtkStructuredGridConnectivity
...
#ifndef svtkPStructuredGridConnectivity_h
#define svtkPStructuredGridConnectivity_h

// SVTK include directives
#include "svtkFiltersParallelGeometryModule.h" // For export macro
#include "svtkMPICommunicator.h"               // Needed for svtkMPICommunicator::Request
...
// class svtkMPICommunicator::Request;

class SVTKFILTERSPARALLELGEOMETRY_EXPORT svtkPStructuredGridConnectivity
  : public svtkStructuredGridConnectivity
{
public:
  static svtkPStructuredGridConnectivity* New();
  svtkTypeMacro(svtkPStructuredGridConnectivity, svtkStructuredGridConnectivity);
  ...
  /**
   * See svtkStructuredGridConnectivity::RegisterGrid
   */
  void RegisterGrid(const int gridID, int extents[6], svtkUnsignedCharArray* nodesGhostArray,
    svtkUnsignedCharArray* cellGhostArray, svtkPointData* pointData, svtkCellData* cellData,
    svtkPoints* gridNodes) override;

  /**
   * Returns the number of local grids registers by the process that owns
   * the current svtkPStructuredGridConnectivity instance
   */
  ...
  /**
   * Initializes this instance of svtkPStructuredGridConnectivity, essentially,
   ...
   * data-set.
   * See svtkStructuredGridConnectivity::ComputeNeighbors
   */
...
protected:
  ...
  ~svtkPStructuredGridConnectivity() override;
  ...
  std::vector<std::vector<svtkPoints*> > RemotePoints;
  std::vector<std::vector<svtkPointData*> > RemotePointData;
  std::vector<std::vector<svtkCellData*> > RemoteCellData;
  ...
  svtkMPICommunicator::Request* MPIRequests;
  ...
  /**
   * Clears all internal SVTK data-structures that are used to store the remote
   * ghost data.
   */
   ...
   * to the SVTK remote point data-structures.
   */
   ...
   * process.
   */
  void DeserializeBufferSizesForProcess(int* buffersizes, svtkIdType N, const int processId);

  /**
   * Helper method to serialize the buffer sizes for the grids of this process
   * to neighboring grids.
   */
  void SerializeBufferSizes(int*& sizesbuf, svtkIdType& N);
   ...
   */
  void SerializeGhostPoints(const int gridIdx, int ext[6], svtkMultiProcessStream& bytestream);

  /**
   * Serializes the data array into a bytestream.
   */
  void SerializeDataArray(svtkDataArray* dataArray, svtkMultiProcessStream& bytestream);

  /**
   * Helper method to serialize field data. Called from
   * SerializeGhostPointData and SerializeGhostCellData.
   */
  void SerializeFieldData(int sourceExtent[6], int targetExtent[6], svtkFieldData* fieldData,
   ...
   * SerializeGhostData.
   */
  void SerializeGhostPointData(const int gridIdx, int ext[6], svtkMultiProcessStream& bytestream);

  /**
   * Helper method to serialize ghost cell data. Called from
   * SerializeGhostData.
   */
  void SerializeGhostCellData(const int gridIdx, int ext[6], svtkMultiProcessStream& bytestream);
   ...
   */
  void DeserializeDataArray(svtkDataArray*& dataArray, const int dataType, const int numberOfTuples,
    const int numberOfComponents, svtkMultiProcessStream& bytestream);

  /**
   * Helper method to de-serialize field data. Called from
   * DeserializeGhostPointData and DeserializeGhostCellData.
   */
  void DeserializeFieldData(int ext[6], svtkFieldData* fieldData, svtkMultiProcessStream& bytestream);
   ...
   * from svtkPStructuredGridConnectivity::PackGhostData().
   */
   ...
   */
  void SerializeGridExtents(int*& sndbuffer, svtkIdType& N);

  /**
   * Deserializes the received grid extent information to the GridExtents
   * internal data-structures.
   */
  void DeserializeGridExtentForProcess(int* rcvbuffer, svtkIdType& N, const int processId);

private:
  svtkPStructuredGridConnectivity(const svtkPStructuredGridConnectivity&) = delete;
  void operator=(const svtkPStructuredGridConnectivity&) = delete;
};
...
inline bool svtkPStructuredGridConnectivity::GridExtentsAreEqual(int rhs[6], int lhs[6])
{
  for (int i = 0; i < 6; ++i)
  {
    if (rhs[i] != lhs[i])
...
//------------------------------------------------------------------------------
inline bool svtkPStructuredGridConnectivity::HasPointData(const int gridIdx)
{
  // Sanity check
...
//------------------------------------------------------------------------------
inline bool svtkPStructuredGridConnectivity::HasCellData(const int gridIdx)
{
  // Sanity check
  assert("pre: grid index is out-of-bounds!" && (gridIdx >= 0) &&
    (gridIdx < static_cast<int>(this->NumberOfGrids)));
...
//------------------------------------------------------------------------------
inline bool svtkPStructuredGridConnectivity::HasPoints(const int gridIdx)
{
  // Sanity check
  assert("pre: grid index is out-of-bounds!" && (gridIdx >= 0) &&
    (gridIdx < static_cast<int>(this->NumberOfGrids)));
...
//------------------------------------------------------------------------------
inline void svtkPStructuredGridConnectivity::InitializeMessageCounters()
{
  this->TotalNumberOfMsgs = this->TotalNumberOfRcvs = this->TotalNumberOfSends = 0;
}

//------------------------------------------------------------------------------
inline void svtkPStructuredGridConnectivity::ClearRawBuffers()
{
...
//------------------------------------------------------------------------------
inline void svtkPStructuredGridConnectivity::ClearRemoteData()
{
...
//------------------------------------------------------------------------------
inline bool svtkPStructuredGridConnectivity::IsGridRemote(const int gridID)
{
  return (!this->IsGridLocal(gridID));
}

//------------------------------------------------------------------------------
inline bool svtkPStructuredGridConnectivity::IsGridLocal(const int gridID)
{
...
//------------------------------------------------------------------------------
inline int svtkPStructuredGridConnectivity::GetGridRank(const int gridID)
{
  assert("pre: Instance has not been initialized!" && this->Initialized);
...
#endif /* svtkPStructuredGridConnectivity_h */
