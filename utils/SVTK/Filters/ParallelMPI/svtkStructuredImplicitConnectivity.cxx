...
bool Interval::Intersects(const Interval& A, const Interval& B, Interval& Overlap, int& type)
{
  ...
  {
    svtkGenericWarningMacro(<< "Undefined interval intersection!"
...
struct DomainMetaData
{
  ...
  bool HasGrid(int ext[6]) { return (svtkStructuredExtent::Smaller(ext, this->WholeExtent)); }

  /// \brief Initializes the domain metadata.
  void Initialize(int wholeExt[6])
  {
    memcpy(this->WholeExtent, wholeExt, 6 * sizeof(int));
    this->DataDescription = svtkStructuredData::GetDataDescriptionFromExtent(wholeExt);

    if (this->DataDescription == SVTK_EMPTY)
    {
      return;
    }

    // Sanity checks!
    assert("pre: data description is SVTK_EMPTY!" && (this->DataDescription != SVTK_EMPTY));
    assert("pre: dataset must be 2-D or 3-D" && (this->DataDescription >= SVTK_XY_PLANE));
    ...
    switch (this->DataDescription)
    {
      ...
      case SVTK_XYZ_GRID:
        ...
        break;
      default:
        svtkGenericWarningMacro(<< "Cannot handle data description: " << this->DataDescription
                               << "\n");
    } // END switch
...
struct StructuredGrid
{
  ...
  void Initialize(StructuredGrid* grid)
  {
    ...
    svtkIdType nnodes = svtkStructuredData::GetNumberOfPoints(this->Extent, grid->DataDescription);

    // Allocate coordinates, if needed
    if (grid->Nodes != nullptr)
    {
      this->Nodes = svtkPoints::New();
    ...
    {
      int dims[3];
      svtkStructuredData::GetDimensionsFromExtent(this->Extent, dims, this->DataDescription);

      this->X_Coords = svtkDataArray::CreateDataArray(grid->X_Coords->GetDataType());
      this->X_Coords->SetNumberOfTuples(dims[0]);
      for (svtkIdType idx = 0; idx < grid->X_Coords->GetNumberOfTuples(); ++idx)
      {
        this->X_Coords->SetTuple(idx, idx, grid->X_Coords);
      }

      this->Y_Coords = svtkDataArray::CreateDataArray(grid->Y_Coords->GetDataType());
      this->Y_Coords->SetNumberOfTuples(dims[1]);
      for (svtkIdType idx = 0; idx < grid->Y_Coords->GetNumberOfTuples(); ++idx)
      {
        this->Y_Coords->SetTuple(idx, idx, grid->Y_Coords);
      }

      this->Z_Coords = svtkDataArray::CreateDataArray(grid->Z_Coords->GetDataType());
      this->Z_Coords->SetNumberOfTuples(dims[2]);
      for (svtkIdType idx = 0; idx < grid->Z_Coords->GetNumberOfTuples(); ++idx)
      {
    ...
    if (grid->PointData != nullptr)
    {
      ...
      {
        svtkDataArray* a = this->PointData->GetArray(array);
    ...
    for (I(ijk) = IMIN(grid->Extent); I(ijk) <= IMAX(grid->Extent); ++I(ijk))
    {
      for (J(ijk) = JMIN(grid->Extent); J(ijk) <= JMAX(grid->Extent); ++J(ijk))
      {
        for (K(ijk) = KMIN(grid->Extent); K(ijk) <= KMAX(grid->Extent); ++K(ijk))
        {
          ...
          svtkIdType srcIdx = svtkStructuredData::ComputePointIdForExtent(grid->Extent, ijk, desc);

          // Compute the target index
          svtkIdType targetIdx = svtkStructuredData::ComputePointIdForExtent(this->Extent, ijk, desc);
  ...
  //------------------------------------------------------------------------------
  void Initialize(int id, int ext[6], svtkDataArray* x_coords, svtkDataArray* y_coords,
    svtkDataArray* z_coords, svtkPointData* fields)
  {
    ...
    this->DataDescription = svtkStructuredData::GetDataDescriptionFromExtent(ext);
    ...
    this->X_Coords = svtkDataArray::CreateDataArray(x_coords->GetDataType());
    this->X_Coords->SetVoidArray(x_coords->GetVoidPointer(0), x_coords->GetNumberOfTuples(), 1);

    this->Y_Coords = svtkDataArray::CreateDataArray(y_coords->GetDataType());
    this->Y_Coords->SetVoidArray(y_coords->GetVoidPointer(0), y_coords->GetNumberOfTuples(), 1);

    this->Z_Coords = svtkDataArray::CreateDataArray(z_coords->GetDataType());
  ...
  //------------------------------------------------------------------------------
  void Initialize(int id, int ext[6], svtkPoints* nodes, svtkPointData* fields)
  {
    this->ID = id;
    memcpy(this->Extent, ext, 6 * sizeof(int));
    this->DataDescription = svtkStructuredData::GetDataDescriptionFromExtent(ext);
...
public:
  ...
  void Exchange(svtkMPIController* comm);
  int NumMsgs();
  void Clear();
...
private:
  ...
  std::vector<svtkMPICommunicator::Request> Requests;

  // exchanges buffer-sizes
  void AllocateRcvBuffers(svtkMPIController* comm);
};
...
//------------------------------------------------------------------------------
void CommunicationManager::AllocateRcvBuffers(svtkMPIController* comm)
{
...
//------------------------------------------------------------------------------
void CommunicationManager::Exchange(svtkMPIController* comm)
{
  std::map<int, unsigned char*>::iterator it;
...
svtkStandardNewMacro(svtkStructuredImplicitConnectivity);

//------------------------------------------------------------------------------
svtkStructuredImplicitConnectivity::svtkStructuredImplicitConnectivity()
{
...
//------------------------------------------------------------------------------
void svtkStructuredImplicitConnectivity::PrintSelf(ostream& os, svtkIndent indent)
{
...
//------------------------------------------------------------------------------
void svtkStructuredImplicitConnectivity::SetWholeExtent(int wholeExt[6])
{
  delete this->DomainInfo;

  this->DomainInfo = new svtk::detail::DomainMetaData();
...
//------------------------------------------------------------------------------
void svtkStructuredImplicitConnectivity::RegisterGrid(
  const int gridID, int extent[6], svtkPoints* gridNodes, svtkPointData* pointData)
{
  // Sanity Checks!
  ...
  {
    this->InputGrid = new svtk::detail::StructuredGrid();
    this->InputGrid->Initialize(gridID, extent, gridNodes, pointData);
  }
}

//------------------------------------------------------------------------------
void svtkStructuredImplicitConnectivity::RegisterRectilinearGrid(const int gridID, int extent[6],
  svtkDataArray* xcoords, svtkDataArray* ycoords, svtkDataArray* zcoords, svtkPointData* pointData)
{
  // Sanity Checks!
  assert("pre: nullptr Domain, whole extent is not set!" && (this->DomainInfo != nullptr));
  assert("pre: input not nullptr in this process!" && (this->InputGrid == nullptr));
...
//------------------------------------------------------------------------------
void svtkStructuredImplicitConnectivity::ExchangeExtents()
{
  // Sanity checks!
...
//------------------------------------------------------------------------------
void svtkStructuredImplicitConnectivity::ComputeNeighbors()
{
  if (!this->InputGrid)
  {
    return;
  ...
  int type;
  svtk::detail::Interval A;                // used to store the local interval at each dim
  svtk::detail::Interval B;                // used to store the remote interval at each dim
  svtk::detail::Interval Overlap;          // used to store the computed overlap
  svtk::detail::ImplicitNeighbor Neighbor; // used to store neighbor information
  ...
  for (int rank = 0; rank < nranks; ++rank)
  {
    ...
    std::fill(
      Neighbor.Orientation, Neighbor.Orientation + 3, svtk::detail::IntervalsConnect::UNDEFINED);
    ...
    for (int dim = 0; dim < this->DomainInfo->NDim; ++dim)
    {
      ...
      if (A.ImplicitNeighbor(B, type))
      {
        ...
        {
          svtkGenericWarningMacro(<< "Invalid implicit connectivity type! "
                                 << "Code should not reach here!\n");
        } // END else
      }   // END if implicit
...
//------------------------------------------------------------------------------
bool svtkStructuredImplicitConnectivity::GlobalDataDescriptionMatch()
{
  int sum = -1;
  this->Controller->AllReduce(&this->DomainInfo->DataDescription, &sum, 1, svtkCommunicator::SUM_OP);
...
//------------------------------------------------------------------------------
bool svtkStructuredImplicitConnectivity::HasImplicitConnectivity()
{
  if (this->DomainInfo == nullptr)
  {
    svtkGenericWarningMacro(<< "nullptr domain, WholeExtent not set!");
    return false;
  }
...
//------------------------------------------------------------------------------
void svtkStructuredImplicitConnectivity::GetGlobalImplicitConnectivityState()
{
  // Sanity checks!
  assert("pre: null controller!" && (this->Controller != nullptr));
  ...
  this->Controller->AllReduce(
    sndbuffer, this->DomainInfo->GlobalImplicit, 3, svtkCommunicator::SUM_OP);
}

//------------------------------------------------------------------------------
void svtkStructuredImplicitConnectivity::EstablishConnectivity()
{
  // Sanity checks!
  assert("pre: null controller!" && (this->Controller != nullptr));
...
//------------------------------------------------------------------------------
void svtkStructuredImplicitConnectivity::GetOutputStructuredGrid(
  const int gridID, svtkStructuredGrid* grid)
{
...
//------------------------------------------------------------------------------
void svtkStructuredImplicitConnectivity::GetOutputImageData(const int gridID, svtkImageData* grid)
{
  assert("pre: nullptr output grid!" && (grid != nullptr));
  assert("pre: output grid is nullptr!" && (this->OutputGrid != nullptr));
  assert("pre: mismatch gridID" && (this->OutputGrid->ID == gridID));
...
//------------------------------------------------------------------------------
void svtkStructuredImplicitConnectivity::GetOutputRectilinearGrid(
  const int gridID, svtkRectilinearGrid* grid)
{
  assert("pre: nullptr output grid!" && (grid != nullptr));
  assert("pre: output grid is nullptr!" && (this->OutputGrid != nullptr));
  assert("pre: mismatch gridID" && (this->OutputGrid->ID == gridID));
...
//------------------------------------------------------------------------------
void svtkStructuredImplicitConnectivity::ConstructOutput()
{
  if (this->OutputGrid != nullptr)
  {
    this->OutputGrid->Clear();
  ...
  this->OutputGrid = new svtk::detail::StructuredGrid();
  this->OutputGrid->Initialize(this->InputGrid);
}

//------------------------------------------------------------------------------
void svtkStructuredImplicitConnectivity::UpdateNeighborList(const int dim)
{
  ...
  svtk::detail::ImplicitNeighbor* neiPtr = nullptr;
  ...
  for (size_t nei = 0; nei < nNeis; ++nei)
  {
    ...
    if (orient == svtk::detail::IntervalsConnect::IMPLICIT_HI ||
      orient == svtk::detail::IntervalsConnect::IMPLICIT_LO ||
      orient == svtk::detail::IntervalsConnect::UNDEFINED)
    {
      continue;
    ...
    assert("post: overlap extent out-of-bounds of output grid extent!" &&
      svtkStructuredExtent::Smaller(neiPtr->Overlap, this->OutputGrid->Extent));
  } // END for all neighbors
}

//------------------------------------------------------------------------------
void svtkStructuredImplicitConnectivity::PackData(int ext[6], svtkMultiProcessStream& bytestream)
{
  // Sanity checks
  assert("pre: input grid is nullptr!" && (this->InputGrid != nullptr));
  assert("pre: output grid is nullptr!" && (this->OutputGrid != nullptr));
  assert("pre: extent is out-of-bounds the output grid!" &&
    svtkStructuredExtent::Smaller(ext, this->OutputGrid->Extent));

  bytestream.Push(ext, 6);

  if (this->OutputGrid->Nodes != nullptr)
  {
    bytestream << SVTK_STRUCTURED_GRID;
    svtkIdType nnodes = svtkStructuredData::GetNumberOfPoints(ext);
    bytestream << nnodes;
    ...
    for (I(ijk) = IMIN(ext); I(ijk) <= IMAX(ext); ++I(ijk))
    {
      for (J(ijk) = JMIN(ext); J(ijk) <= JMAX(ext); ++J(ijk))
      {
        ...
        {
          svtkIdType idx = svtkStructuredData::ComputePointIdForExtent(
  ...
  {
    bytestream << SVTK_RECTILINEAR_GRID;
  ...
  {
    bytestream << SVTK_UNIFORM_GRID;
  }

  // serialize the node-centered fields
  if (this->OutputGrid->PointData != nullptr)
  {
    svtkFieldDataSerializer::SerializeSubExtent(
...
//------------------------------------------------------------------------------
void svtkStructuredImplicitConnectivity::UnPackData(unsigned char* buffer, unsigned int size)
{
  assert("pre: output grid is nullptr!" && (this->OutputGrid != nullptr));
  ...
  if (datatype == SVTK_STRUCTURED_GRID)
  {
    int nnodes = 0;
    bytestream >> nnodes;
  ...
  else if (datatype == SVTK_RECTILINEAR_GRID)
  {
    ...
    for (int dim = 0; dim < 3; ++dim)
    {
      ...
      {
        double coordinate;
        svtkIdType lastIdx = coords[dim]->GetNumberOfTuples() - 1;
  ...
  if (this->OutputGrid->PointData != nullptr)
  {
    svtkFieldDataSerializer::DeSerializeToSubExtent(
      ext, this->OutputGrid->Extent, this->OutputGrid->PointData, bytestream);
  }
...
//------------------------------------------------------------------------------
void svtkStructuredImplicitConnectivity::AllocateBuffers(const int dim)
{
  assert("pre: dimension index out-of-bounds!" && (dim >= 0) && (dim <= 2));
  ...
  if (this->CommManager == nullptr)
  {
    this->CommManager = new svtk::detail::CommunicationManager();
  }
  ...
  for (size_t nei = 0; nei < nNeis; ++nei)
  {
    svtk::detail::ImplicitNeighbor* neiPtr = &(this->InputGrid->Neighbors)[nei];
    int orient = neiPtr->Orientation[dim];
    ...
    else if (orient == svtk::detail::IntervalsConnect::IMPLICIT_LO)
    {
...
//------------------------------------------------------------------------------
void svtkStructuredImplicitConnectivity::GrowGrid(const int dim)
{
  assert("pre: dimension index out-of-bounds!" && (dim >= 0) && (dim <= 2));
  assert("pre: input grid is nullptr!" && this->InputGrid != nullptr);
...
//------------------------------------------------------------------------------
void svtkStructuredImplicitConnectivity::ExchangeData()
{
  // Sanity checks!
  assert("pre: null controller!" && (this->Controller != nullptr));
