/*=========================================================================
  ...
  Module:    svtkEllipsoidalGaussianKernel.cxx
...
svtkStandardNewMacro(svtkEllipsoidalGaussianKernel);

//----------------------------------------------------------------------------
svtkEllipsoidalGaussianKernel::svtkEllipsoidalGaussianKernel()
{
...
//----------------------------------------------------------------------------
void svtkEllipsoidalGaussianKernel::FreeStructures()
{
...
//----------------------------------------------------------------------------
void svtkEllipsoidalGaussianKernel::Initialize(
  svtkAbstractPointLocator* loc, svtkDataSet* ds, svtkPointData* pd)
{
...
//----------------------------------------------------------------------------
svtkIdType svtkEllipsoidalGaussianKernel::ComputeWeights(
  ...
  for (svtkIdType i = 0; i < numPts; ++i)
  {
    ...
    else // continue computing weights
    {
      ...
      if (this->NormalsArray)
      {
        ...
        mag = svtkMath::Dot(n, n);
        mag = (mag == 0.0 ? 1.0 : sqrt(mag));
        z2 = svtkMath::Dot(v, n) / mag;
        z2 = z2 * z2;
      }
      else
      {
...
//----------------------------------------------------------------------------
void svtkEllipsoidalGaussianKernel::PrintSelf(ostream& os, svtkIndent indent)
{
