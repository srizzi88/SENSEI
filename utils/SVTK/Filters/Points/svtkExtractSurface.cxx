...
svtkStandardNewMacro(svtkExtractSurface);

//----------------------------------------------------------------------------

// This templated class implements the heart of the algorithm.
// svtkExtractSurface populates the information in this class and
// then invokes Contour() to actually initiate execution.
template <class T>
class svtkExtractSurfaceAlgorithm
{
public:
  ...
  {
    if (*ePtr[0] >= svtkExtractSurfaceAlgorithm::Invalid ||
      *ePtr[1] >= svtkExtractSurfaceAlgorithm::Invalid ||
      *ePtr[2] >= svtkExtractSurfaceAlgorithm::Invalid ||
      *ePtr[3] >= svtkExtractSurfaceAlgorithm::Invalid)
    {
      return false;
    }
    else
  ...
  void InterpolateAxesEdge(double t, unsigned char loc, float x0[3], T const* const s,
    const int incs[3], float x1[3], svtkIdType vId, svtkIdType ijk[3], float g0[3])
  {
  ...
  void InterpolateEdge(double value, svtkIdType ijk[3], T const* const s, const int incs[3],
    float x[3], unsigned char edgeNum, unsigned char const* const edgeUses, svtkIdType* eIds);
  ...
  void GeneratePoints(double value, unsigned char loc, svtkIdType ijk[3], T const* const sPtr,
    const int incs[3], float x[3], unsigned char const* const edgeUses, svtkIdType* eIds);
  ...
  {
  public:
    svtkExtractSurfaceAlgorithm<TT>* Algo;
    double Value;
    Pass1(svtkExtractSurfaceAlgorithm<TT>* algo, double value)
    {
      this->Algo = algo;
      this->Value = value;
    }
  ...
  {
  public:
    Pass2(svtkExtractSurfaceAlgorithm<TT>* algo) { this->Algo = algo; }
    svtkExtractSurfaceAlgorithm<TT>* Algo;
    void operator()(svtkIdType slice, svtkIdType end)
    {
      for (; slice < end; ++slice)
  ...
  {
  public:
    Pass4(svtkExtractSurfaceAlgorithm<TT>* algo, double value)
    {
      this->Algo = algo;
      this->Value = value;
    }
  ...
  static void Contour(svtkExtractSurface* self, svtkImageData* input, int extent[6], svtkIdType* incs,
    T* scalars, svtkPoints* newPts, svtkCellArray* newTris, svtkFloatArray* newNormals,
...
template <class T>
const unsigned char svtkExtractSurfaceAlgorithm<T>::EdgeMap[12] = { 0, 5, 1, 4, 2, 7, 3, 6, 8, 9, 10,
  11 };

//----------------------------------------------------------------------------
// Map MC edges numbering to use the saner FlyingEdges edge numbering scheme.
template <class T>
const unsigned char svtkExtractSurfaceAlgorithm<T>::VertMap[12][2] = { { 0, 1 }, { 2, 3 }, { 4, 5 },
  { 6, 7 }, { 0, 2 }, { 1, 3 }, { 4, 6 }, { 5, 7 }, { 0, 4 }, { 1, 5 }, { 2, 6 }, { 3, 7 } };

//----------------------------------------------------------------------------
// The offsets of each vertex (in index space) from the voxel axes origin.
template <class T>
const unsigned char svtkExtractSurfaceAlgorithm<T>::VertOffsets[8][3] = { { 0, 0, 0 }, { 1, 0, 0 },
...
template <class T>
svtkExtractSurfaceAlgorithm<T>::svtkExtractSurfaceAlgorithm()
  : XCases(nullptr)
...
template <class T>
void svtkExtractSurfaceAlgorithm<T>::CountBoundaryYZInts(
...
// volume.
template <class T>
void svtkExtractSurfaceAlgorithm<T>::ComputeBoundaryGradient(svtkIdType ijk[3],
...
template <class T>
void svtkExtractSurfaceAlgorithm<T>::InterpolateEdge(double value, svtkIdType ijk[3],
...
template <class T>
void svtkExtractSurfaceAlgorithm<T>::GeneratePoints(double value, unsigned char loc,
  svtkIdType ijk[3], T const* const sPtr, const int incs[3], float x[3],
  unsigned char const* const edgeUses, svtkIdType* eIds)
{
...
// trimming).
template <class T>
void svtkExtractSurfaceAlgorithm<T>::ProcessXEdge(
  double value, T const* const inPtr, svtkIdType row, svtkIdType slice)
{
  ...
  for (svtkIdType i = 0; i < nxcells; ++i, ++ePtr)
  {
    ...
    if (s0 >= value)
    {
      edgeCase = svtkExtractSurfaceAlgorithm::LeftAbove;
    }
    else
    {
      edgeCase = svtkExtractSurfaceAlgorithm::Below;
    }
    if (s1 >= value)
    {
      edgeCase |= svtkExtractSurfaceAlgorithm::RightAbove;
    }

    // if edge intersects contour
    if (edgeCase == svtkExtractSurfaceAlgorithm::LeftAbove ||
      edgeCase == svtkExtractSurfaceAlgorithm::RightAbove)
    {
    ...
    {
      edgeCase |= svtkExtractSurfaceAlgorithm::Invalid;
    }
...
// information.
template <class T>
void svtkExtractSurfaceAlgorithm<T>::ProcessYZEdges(svtkIdType row, svtkIdType slice)
{
...
// algorithm.
template <class T>
void svtkExtractSurfaceAlgorithm<T>::GenerateOutput(
...
// interfaces the svtkExtractSurface class with the templated algorithm
// class. It also invokes the three passes of the Flying Edges algorithm.
template <class T>
void svtkExtractSurfaceAlgorithm<T>::Contour(svtkExtractSurface* self, svtkImageData* input,
  int extent[6], svtkIdType* incs, T* scalars, svtkPoints* newPts, svtkCellArray* newTris,
  svtkFloatArray* newNormals, svtkFloatArray* newGradients)
{
  ...
  svtkExtractSurfaceAlgorithm<T> algo;
...
svtkExtractSurface::svtkExtractSurface()
{
  this->Radius = 0.1;
...
//----------------------------------------------------------------------------
svtkExtractSurface::~svtkExtractSurface() = default;

//----------------------------------------------------------------------------
int svtkExtractSurface::RequestUpdateExtent(svtkInformation* svtkNotUsed(request),
...
//----------------------------------------------------------------------------
int svtkExtractSurface::RequestData(
  svtkInformation* request, svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  svtkDebugMacro(<< "Executing 3D structured contour");
  ...
  {
    svtkTemplateMacro(svtkExtractSurfaceAlgorithm<SVTK_TT>::Contour(
      this, input, exExt, incs, (SVTK_TT*)ptr, newPts, newTris, newNormals, newGradients));
  }
...
//----------------------------------------------------------------------------
int svtkExtractSurface::FillInputPortInformation(int, svtkInformation* info)
{
...
//----------------------------------------------------------------------------
void svtkExtractSurface::PrintSelf(ostream& os, svtkIndent indent)
{
