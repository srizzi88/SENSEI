/*=========================================================================
  ...
  Module:    svtkHierarchicalBinningFilter.cxx
...
svtkStandardNewMacro(svtkHierarchicalBinningFilter);
...
struct UniformBinning
{
  ...
  svtkIdType GetBinIndex(const double* x) const
  {
    int ijk[3];
...
// SVTK class refers to it in the header file (PIMPLd).
struct svtkBinTree
{
  svtkPoints* InPts; // the points to be binned
  svtkIdType NumPts;

  int NumLevels;
  int Divs[3];
  double Bounds[6];
  UniformBinning* Tree[SVTK_MAX_LEVEL + 1]; // a uniform binning for each level
  int NumBins;                             // the total number of bins (from all levels) in the tree
  int BatchSize;                           // build the offsets in parallel

  int OffsetsType;
  svtkDataArray* OffsetsArray; // container for offset array

  // Construction
  svtkBinTree(
    svtkIdType npts, svtkPoints* pts, int numLevels, int divs[3], double bounds[6], int offsetsType)
    : InPts(pts)
    , NumPts(npts)
    , NumLevels(numLevels)
  ...
  virtual void Execute(svtkPointSet* input, svtkPolyData* output) = 0;
  int GetNumberOfGlobalBins() { return this->NumBins; }
  int GetNumberOfBins(int level) { return this->Tree[level]->NumBins; }
  virtual svtkIdType GetLevelOffset(int level, svtkIdType& npts) = 0;
  virtual svtkIdType GetBinOffset(int globalBin, svtkIdType& npts) = 0;
  virtual svtkIdType GetLocalBinOffset(int level, int localBin, svtkIdType& npts) = 0;
  ...
  void ExportMetaData(svtkPolyData* output)
  {
...
// to use svtkIdType. Otherwise for performance reasons it's best to use an int
// (or other integral type). Typically sort() is 25-30% faster on smaller
// integral types, plus it takes a heck less memory (when svtkIdType is 64-bit
// and int is 32-bit).
...
// implements the operator() functors which are supplied to svtkSMPTools for
// threaded processesing.
template <typename TIds>
struct BinTree : public svtkBinTree
{
  ...
  BinTree(
    svtkIdType npts, svtkPoints* pts, int numLevels, int divs[3], double bounds[6], int offsetsType)
    : svtkBinTree(npts, pts, numLevels, divs, bounds, offsetsType)
  {
    // one extra allocation to simplify traversal
    this->Map = new BinTuple<TIds>[this->NumPts + 1];
    this->Map[this->NumPts].Bin = this->NumBins;
    if (offsetsType == SVTK_INT)
    {
      this->OffsetsArray = svtkIntArray::New();
    }
    else
    {
      this->OffsetsArray = svtkIdTypeArray::New();
    }
  ...
  svtkIdType GetNumberOfIds(svtkIdType binNum)
  {
    return (this->Offsets[binNum + 1] - this->Offsets[binNum]);
  }

  // Given a bin number, return the point ids in that bin.
  const BinTuple<TIds>* GetIds(svtkIdType binNum) { return this->Map + this->Offsets[binNum]; }

  // Explicit point representation (e.g., svtkPointSet), faster path
  ...
  public:
    ...
    void operator()(svtkIdType batch, svtkIdType batchEnd)
    {
  ...
  struct ShufflePoints
  {
    ...
    ShufflePoints(BinTree<T>* tree, svtkIdType numPts, TPts* inPts, TPts* outPts)
      : Tree(tree)
      , NumPts(numPts)
      , InPoints(inPts)
      , OutPoints(outPts)
  ...
  struct ShuffleArray
  {
    ...
    ShuffleArray(BinTree<T>* tree, svtkIdType numPts, int numComp, TA* in, TA* out)
      : Tree(tree)
      , NumPts(numPts)
      , NumComp(numComp)
      , InArray(in)
  ...
  void Execute(svtkPointSet* input, svtkPolyData* output) override
  {
    ...
    {
      svtkGenericWarningMacro("Type not supported\n");
      return;
    }

    // Now gather the points into contiguous runs in bins
    //
    svtkSMPTools::Sort(this->Map, this->Map + this->NumPts);
    ...
    svtkSMPTools::For(0, numBatches, offMapper);
    ...
    if (dataType == SVTK_FLOAT)
    {
      ...
      svtkSMPTools::For(0, this->NumPts, shuffle);
    }
    else if (dataType == SVTK_DOUBLE)
    {
      ...
      if (iArray)
      {
        ...
        switch (iArray->GetDataType()) // template macro burps on multiple template parameters
        {
          ...
          case SVTK_UNSIGNED_SHORT:
            ...
            break;
          default:
            svtkGenericWarningMacro("Unsupported attribute type");
        } // over all SVTK types
      }   // have valid array
    }     // for each candidate array
  }

  svtkIdType GetLevelOffset(int level, svtkIdType& npts) override
  {
    svtkIdType offset = this->Offsets[this->Tree[level]->LevelOffset];
    svtkIdType offset2 = this->Offsets[this->Tree[level + 1]->LevelOffset];

    npts = offset2 - offset;
    return offset;
  }

  svtkIdType GetBinOffset(int globalBin, svtkIdType& npts) override
  {
    svtkIdType offset = this->Offsets[globalBin];
    svtkIdType offset2 = this->Offsets[globalBin + 1];

    npts = offset2 - offset;
    return offset;
  }

  svtkIdType GetLocalBinOffset(int level, int localBin, svtkIdType& npts) override
  {
    svtkIdType offset = this->Offsets[this->Tree[level]->LevelOffset] + localBin;
    svtkIdType offset2 = this->Offsets[this->Tree[level]->LevelOffset] + localBin + 1;
...
//----------------------------------------------------------------------------
svtkHierarchicalBinningFilter::svtkHierarchicalBinningFilter()
{
...
int svtkHierarchicalBinningFilter::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  svtkPoints* inPts = input->GetPoints();
  int dataType = inPts->GetDataType();
  svtkPoints* outPts = inPts->NewInstance();
  ...
  // Bin the points, produce output
  if (numPts >= SVTK_INT_MAX)
  {
    this->Tree = new BinTree<svtkIdType>(numPts, inPts, numLevels, divs, bounds, SVTK_ID_TYPE);
    this->Tree->Execute(input, output);
  }
  else
  {
    this->Tree = new BinTree<int>(numPts, inPts, numLevels, divs, bounds, SVTK_INT);
    this->Tree->Execute(input, output);
  }
...
//----------------------------------------------------------------------------
int svtkHierarchicalBinningFilter::GetNumberOfGlobalBins()
{
  if (this->Tree)
  {
...
//----------------------------------------------------------------------------
int svtkHierarchicalBinningFilter::GetNumberOfBins(int level)
{
  if (this->Tree)
  {
...
//----------------------------------------------------------------------------
svtkIdType svtkHierarchicalBinningFilter::GetLevelOffset(int level, svtkIdType& npts)
{
  if (this->Tree)
  {
...
//----------------------------------------------------------------------------
svtkIdType svtkHierarchicalBinningFilter::GetBinOffset(int globalBin, svtkIdType& npts)
{
  if (this->Tree)
  {
...
//----------------------------------------------------------------------------
svtkIdType svtkHierarchicalBinningFilter::GetLocalBinOffset(int level, int localBin, svtkIdType& npts)
{
  if (this->Tree)
  {
...
//----------------------------------------------------------------------------
void svtkHierarchicalBinningFilter::GetBinBounds(int globalBin, double bounds[6])
{
  if (this->Tree)
  {
...
//----------------------------------------------------------------------------
void svtkHierarchicalBinningFilter::GetLocalBinBounds(int level, int localBin, double bounds[6])
{
  if (this->Tree)
  {
...
//----------------------------------------------------------------------------
int svtkHierarchicalBinningFilter::FillInputPortInformation(int, svtkInformation* info)
{
...
//----------------------------------------------------------------------------
void svtkHierarchicalBinningFilter::PrintSelf(ostream& os, svtkIndent indent)
{
