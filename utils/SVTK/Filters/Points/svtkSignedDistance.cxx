...
svtkStandardNewMacro(svtkSignedDistance);
svtkCxxSetObjectMacro(svtkSignedDistance, Locator, svtkAbstractPointLocator);
...
struct SignedDistance
{
  ...
  SignedDistance(T* pts, float* normals, int dims[3], double origin[3], double spacing[3],
    double radius, svtkAbstractPointLocator* loc, float* scalars)
    : Pts(pts)
    , Normals(normals)
    , Radius(radius)
    , Locator(loc)
  ...
  static void Execute(svtkSignedDistance* self, T* pts, float* normals, int dims[3],
...
svtkSignedDistance::svtkSignedDistance()
{
...
void svtkSignedDistance::StartAppend()
{
  ...
  for (i = 0; i < 3; i++)
  {
    tempd[i] = (this->Bounds[2 * i + 1] - this->Bounds[2 * i]) / (this->Dimensions[i] - 1);
  }
  output->SetSpacing(tempd);

  outInfo->Set(svtkDataObject::ORIGIN(), this->Bounds[0], this->Bounds[2], this->Bounds[4]);
...
void svtkSignedDistance::Append(svtkPolyData* input)
{
  ...
  float* scalars =
    static_cast<float*>(this->GetOutput()->GetPointData()->GetScalars()->GetVoidPointer(0));
  svtkDataArray* normalArray = input->GetPointData()->GetNormals();
  if (!normalArray || normalArray->GetDataType() != SVTK_FLOAT)
  {
    svtkErrorMacro(<< "Float normals required!");
    return;
  }
  ...
  {
    svtkTemplateMacro(SignedDistance<SVTK_TT>::Execute(this, (SVTK_TT*)inPtr, normals,
...
void svtkSignedDistance::EndAppend()
{
...
//----------------------------------------------------------------------------
int svtkSignedDistance::RequestInformation(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** svtkNotUsed(inputVector), svtkInformationVector* outputVector)
{
  ...
  for (i = 0; i < 3; i++)
  {
    ...
    {
      ar[i] = (this->Bounds[2 * i + 1] - this->Bounds[2 * i]) / (this->Dimensions[i] - 1);
    }
  }
...
//----------------------------------------------------------------------------
int svtkSignedDistance::RequestData(svtkInformation* svtkNotUsed(request),
...
void svtkSignedDistance::SetDimensions(int i, int j, int k)
{
  int dim[3];
...
//----------------------------------------------------------------------------
void svtkSignedDistance::SetDimensions(const int dim[3])
{
  int dataDim, i;
...
//----------------------------------------------------------------------------
int svtkSignedDistance::FillInputPortInformation(int svtkNotUsed(port), svtkInformation* info)
{
...
//----------------------------------------------------------------------------
svtkTypeBool svtkSignedDistance::ProcessRequest(
...
//----------------------------------------------------------------------------
void svtkSignedDistance::PrintSelf(ostream& os, svtkIndent indent)
{
