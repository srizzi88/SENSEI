...
// Testings of the svtkReebGraph features.
...
int LoadVolumeMesh(svtkUnstructuredGrid* vMesh)
{
  vMesh->Allocate();
  ...
  points->Delete();
  svtkIdType* vertexIds = (svtkIdType*)malloc(sizeof(svtkIdType) * 4);

  vertexIds[0] = 0;
  vertexIds[1] = 1;
  vertexIds[2] = 32;
  vertexIds[3] = 64;
  vMesh->InsertNextCell(SVTK_TETRA, 4, vertexIds);
  vertexIds[0] = 4;
  vertexIds[1] = 1;
  vertexIds[2] = 36;
  vertexIds[3] = 65;
...
// loading code generated automatically.
// put whatever mesh you like here.
int LoadSurfaceMesh(svtkPolyData* sMesh)
{
  ...
  vertexIds[1] = 1;
  vertexIds[2] = 64;
  sMesh->InsertNextCell(SVTK_TRIANGLE, 3, vertexIds);
  vertexIds[0] = 0;
  vertexIds[1] = 2;
  vertexIds[2] = 65;
  sMesh->InsertNextCell(SVTK_TRIANGLE, 3, vertexIds);
...
class AreaSimplificationMetric : public svtkReebGraphSimplificationMetric
{
public:
  svtkTypeMacro(AreaSimplificationMetric, svtkReebGraphSimplificationMetric);
  static AreaSimplificationMetric* New();
  double ComputeMetric(svtkDataSet* mesh, svtkDataArray* scalarField, svtkIdType startCriticalPoint,
    svtkAbstractArray* vertexList, svtkIdType endCriticalPoint) override;
};

svtkStandardNewMacro(AreaSimplificationMetric);

double AreaSimplificationMetric::ComputeMetric(svtkDataSet* mesh, svtkDataArray* scalarField,
  svtkIdType startCriticalPoint, svtkAbstractArray* vertexList, svtkIdType endCriticalPoint)
{
  ...
  std::map<svtkIdType, bool> visitedTriangles;
  ...
  for (int i = 0; i < vertexList->GetNumberOfTuples(); i++)
  {
    ...
    for (int j = 0; j < starTriangleList->GetNumberOfIds(); j++)
    {
      ...
      svtkTriangle* t = svtkTriangle::SafeDownCast(mesh->GetCell(tId));
      std::map<svtkIdType, bool>::iterator tIt = visitedTriangles.find(tId);
...
int DisplayReebGraph(svtkReebGraph* g)
{
  svtkDataArray* vertexInfo =
    svtkArrayDownCast<svtkDataArray>(g->GetVertexData()->GetAbstractArray("Vertex Ids"));
  if (!vertexInfo)
    return 1;

  svtkVariantArray* edgeInfo =
    svtkArrayDownCast<svtkVariantArray>(g->GetEdgeData()->GetAbstractArray("Vertex Ids"));
  if (!edgeInfo)
    return 2;
...
int DisplaySurfaceSkeleton(svtkPolyData* surfaceMesh, svtkTable* skeleton)
{

  // Rendering setting
  svtkRenderer* renderer = svtkRenderer::New();

  svtkRenderWindow* renderWindow = svtkRenderWindow::New();
  renderWindow->AddRenderer(renderer);
  renderWindow->SetSize(400, 400);

  svtkRenderWindowInteractor* windowInteractor = svtkRenderWindowInteractor::New();
  windowInteractor->SetRenderWindow(renderWindow);

  svtkPolyDataMapper* surfaceMapper = svtkPolyDataMapper::New();
  surfaceMapper->SetInputData(surfaceMesh);

  svtkActor* surfaceActor = svtkActor::New();
  ...
  svtkSphereSource* nodeSphere = svtkSphereSource::New();
  ...
  svtkActor** nodeActors =
    (svtkActor**)malloc(sizeof(svtkActor*) * 2 * skeleton->GetNumberOfColumns());

  int sampleId = 0;
  double* point = (double*)malloc(sizeof(double) * 3);

  svtkPolyData* embeddedSkeleton = svtkPolyData::New();
  embeddedSkeleton->AllocateExact(1024, 1024);

  svtkPoints* skeletonSamples = svtkPoints::New();
  ...
  {
    svtkDoubleArray* arc = svtkArrayDownCast<svtkDoubleArray>(skeleton->GetColumn(i));

    // critical point at the origin of the arc
    arc->GetTypedTuple(0, point);
    nodeActors[2 * i] = svtkActor::New();
    ...
    nodeActors[2 * i + 1] = svtkActor::New();
    ...
    {
      svtkIdType samplePair[2];
      samplePair[0] = j - 1 + initialSampleId;
      samplePair[1] = j + initialSampleId;
      embeddedSkeleton->InsertNextCell(SVTK_LINE, 2, samplePair);
    }
  }
  ...
  svtkActor* skeletonActor = svtkActor::New();
...
int DisplayVolumeSkeleton(svtkUnstructuredGrid* svtkNotUsed(volumeMesh), svtkTable* skeleton)
{
...
int TestReebGraph(int svtkNotUsed(argc), char* svtkNotUsed(argv)[])
{
  int errorCode;
  ...
  // Loading the mesh
  svtkPolyData* surfaceMesh = svtkPolyData::New();
  LoadSurfaceMesh(surfaceMesh);

  // Attaching a height scalar field to it
  svtkDoubleArray* surfaceScalarField = svtkDoubleArray::New();
  surfaceScalarField->SetNumberOfTuples(surfaceMesh->GetNumberOfPoints());
  for (svtkIdType vId = 0; vId < surfaceMesh->GetNumberOfPoints(); vId++)
  {
  ...
  svtkPolyDataToReebGraphFilter* surfaceReebGraphFilter = svtkPolyDataToReebGraphFilter::New();
  surfaceReebGraphFilter->SetInputData(surfaceMesh);
  surfaceReebGraphFilter->Update();
  svtkReebGraph* surfaceReebGraph = surfaceReebGraphFilter->GetOutput();
  cout << "      Test 2D.1 ";
  ...
  svtkReebGraphSimplificationFilter* surfaceSimplification = svtkReebGraphSimplificationFilter::New();
  ...
  {
    svtkTriangle* t = svtkTriangle::SafeDownCast(surfaceMesh->GetCell(i));
  ...
  svtkReebGraph* simplifiedSurfaceReebGraph = surfaceSimplification->GetOutput();
  metric->Delete();
  ...
  svtkReebGraphSurfaceSkeletonFilter* surfaceSkeletonFilter =
    svtkReebGraphSurfaceSkeletonFilter::New();
  ...
  svtkTable* surfaceSkeleton = surfaceSkeletonFilter->GetOutput();
  ...
  svtkAreaContourSpectrumFilter* areaSpectrumFilter = svtkAreaContourSpectrumFilter::New();
  ...
  svtkTable* areaSpectrum = areaSpectrumFilter->GetOutput();
  cout << "      Test 2D.5 ";
  ...
  //   svtkReebGraphToJoinSplitTreeFilter *splitTreeFilter =
  //     svtkReebGraphToJoinSplitTreeFilter::New();
  ...
  //   svtkReebGraph *splitTree = splitTreeFilter->GetOutput();
  ...
  // Loading the mesh
  svtkUnstructuredGrid* volumeMesh = svtkUnstructuredGrid::New();
  LoadVolumeMesh(volumeMesh);

  // Attaching a height scalar field to it
  svtkDoubleArray* volumeScalarField = svtkDoubleArray::New();
  volumeScalarField->SetNumberOfTuples(volumeMesh->GetNumberOfPoints());
  for (svtkIdType vId = 0; vId < volumeMesh->GetNumberOfPoints(); vId++)
  {
    double* p = (double*)malloc(sizeof(double) * 3);
  ...
  svtkUnstructuredGridToReebGraphFilter* volumeReebGraphFilter =
    svtkUnstructuredGridToReebGraphFilter::New();
  volumeReebGraphFilter->SetInputData(volumeMesh);
  volumeReebGraphFilter->Update();
  svtkReebGraph* volumeReebGraph = volumeReebGraphFilter->GetOutput();
  cout << "      Test 3D.1 ";
  ...
  svtkReebGraphSimplificationFilter* volumeSimplification = svtkReebGraphSimplificationFilter::New();
  ...
  svtkReebGraph* simplifiedVolumeReebGraph = volumeSimplification->GetOutput();
  cout << "      Test 3D.2 ";
  ...
  svtkReebGraphVolumeSkeletonFilter* volumeSkeletonFilter = svtkReebGraphVolumeSkeletonFilter::New();
  ...
  svtkTable* volumeSkeleton = volumeSkeletonFilter->GetOutput();
  ...
  svtkVolumeContourSpectrumFilter* volumeSpectrumFilter = svtkVolumeContourSpectrumFilter::New();
  ...
  svtkTable* volumeSpectrum = volumeSpectrumFilter->GetOutput();
  cout << "      Test 3D.5 ";
  ...
  cout << "      Not currently tested..." << endl;
  //   svtkReebGraphToJoinSplitTreeFilter *joinTreeFilter =
  //     svtkReebGraphToJoinSplitTreeFilter::New();
  ...
  //   svtkReebGraph *joinTree = joinTreeFilter->GetOutput();
