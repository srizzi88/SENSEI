/*=========================================================================
  ...
  Module:    svtkReebGraphVolumeSkeletonFilter.cxx
...
=========================================================================*/
#include "svtkReebGraphVolumeSkeletonFilter.h"
...
svtkStandardNewMacro(svtkReebGraphVolumeSkeletonFilter);

//----------------------------------------------------------------------------
svtkReebGraphVolumeSkeletonFilter::svtkReebGraphVolumeSkeletonFilter()
{
...
//----------------------------------------------------------------------------
int svtkReebGraphVolumeSkeletonFilter::FillInputPortInformation(int portNumber, svtkInformation* info)
{
  switch (portNumber)
  {
    case 0:
...
//----------------------------------------------------------------------------
int svtkReebGraphVolumeSkeletonFilter::FillOutputPortInformation(
...
//----------------------------------------------------------------------------
void svtkReebGraphVolumeSkeletonFilter::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
svtkTable* svtkReebGraphVolumeSkeletonFilter::GetOutput()
{
  return svtkTable::SafeDownCast(this->GetOutputDataObject(0));
}

//----------------------------------------------------------------------------
int svtkReebGraphVolumeSkeletonFilter::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  if ((inputMesh) && (inputGraph))
  {
    ...
    if (output)
    {
      ...
      svtkIdList* starTetList = svtkIdList::New();
      svtkIdType vertexIds[4];
      double point[3];
      ...
      do
      {
        ...
        svtkUnstructuredGrid* subMesh = svtkUnstructuredGrid::New();
        ...
        for (int i = 0; i < vertexList->GetNumberOfTuples(); i++)
        {
          ...
          {
            svtkIdType tId = starTetList->GetId(j);
        ...
        for (int i = 0; i < vertexList->GetNumberOfTuples(); i++)
        {
          ...
          for (int j = 0; j < starTetList->GetNumberOfIds(); j++)
          {
            ...
            if (!visitedTets[tId])
            {
              ...
              subMesh->InsertNextCell(SVTK_TETRA, 4, vertexIds);
