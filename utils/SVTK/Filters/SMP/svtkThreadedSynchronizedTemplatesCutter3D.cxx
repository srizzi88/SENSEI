/*=========================================================================
  ...
  Module:    svtkThreadedSynchronizedTemplatesCutter3D.cxx
...
svtkStandardNewMacro(svtkThreadedSynchronizedTemplatesCutter3D);
svtkCxxSetObjectMacro(svtkThreadedSynchronizedTemplatesCutter3D, CutFunction, svtkImplicitFunction);
...
svtkThreadedSynchronizedTemplatesCutter3D::svtkThreadedSynchronizedTemplatesCutter3D()
{
  SVTK_LEGACY_BODY(
    svtkThreadedSynchronizedTemplatesCutter3D::svtkThreadedSynchronizedTemplatesCutter3D, "SVTK 8.1");
...
svtkMTimeType svtkThreadedSynchronizedTemplatesCutter3D::GetMTime()
{
  svtkMTimeType mTime = this->Superclass::GetMTime();

  if (this->CutFunction != nullptr)
  {
    svtkMTimeType time = this->CutFunction->GetMTime();
...
//----------------------------------------------------------------------------
int svtkThreadedSynchronizedTemplatesCutter3D::FillOutputPortInformation(
...
//----------------------------------------------------------------------------
static void svtkThreadedSynchronizedTemplatesCutter3DInitializeOutput(
  int* ext, int precision, int estimatedNumberOfPieces, svtkImageData* input, svtkPolyData* o)
{
  svtkPoints* newPts;
...
inline void PointIndexToPosition(
  svtkImageData* img, int xidx, int yidx, int zidx, double position[3])
{
...
inline int GetSideOfSurface(svtkImplicitFunction* func, double value, const double pos[3])
{
...
//
template <class T>
void ContourImage(svtkThreadedSynchronizedTemplatesCutter3D* self, int* exExt, svtkImageData* data,
  svtkPolyData* output, T* ptr, bool outputTriangles)
{
  ...
  for (vidx = 0; vidx < numContours; vidx++)
  {
    ...
    svtkPlane* plane = svtkPlane::SafeDownCast(func);
...
{
public:
  DoThreadedCut(svtkThreadedSynchronizedTemplatesCutter3D* filter, int* exExt, svtkImageData* input,
    int numPieces)
    : Filter(filter)
    , ExExt(exExt)
    , Input(input)
  ...
  ~DoThreadedCut() { delete[] this->Outputs; }
  ...
  void Initialize()
  {
    svtkThreadedSynchronizedTemplatesCutter3DInitializeOutput(ExExt,
      Filter->GetOutputPointsPrecision(), svtkSMPTools::GetEstimatedNumberOfThreads(), Input,
      this->tlsPoly.Local());
  }
  ...
  void Reduce()
  {
    ...
    int p = 0;
    for (ThreadLocalPolyObject::iterator i = this->tlsPoly.begin(); i != this->tlsPoly.end(); ++i)
    {
      svtkPolyData* thisPiece = svtkPolyData::New();
  ...
  void operator()(svtkIdType begin, svtkIdType end)
  {
    ...
    for (int i = begin; i < end; i++)
    {
      ...
      et->GetExtent(exExt2);
      ContourImage(
        this->Filter, exExt2, this->Input, this->tlsPoly.Local(), (double*)nullptr, true);
    }
  }

private:
  svtkThreadedSynchronizedTemplatesCutter3D* Filter;
  int* ExExt;
  svtkImageData* Input;
  ...
  typedef svtkSMPThreadLocalObject<svtkPolyData> ThreadLocalPolyObject;
...
//
void svtkThreadedSynchronizedTemplatesCutter3D::ThreadedExecute(
  svtkImageData* data, svtkInformation* outInfo, int)
{
  ...
  int ncells = (exExt[1] - exExt[0]) * (exExt[3] - exExt[2]) * (exExt[5] - exExt[4]);
  ...
  {
    svtkPolyData* contour = functor.GetOutputPiece(i);
...
//----------------------------------------------------------------------------
int svtkThreadedSynchronizedTemplatesCutter3D::RequestData(svtkInformation* svtkNotUsed(request),
...
//----------------------------------------------------------------------------
void svtkThreadedSynchronizedTemplatesCutter3D::PrintSelf(ostream& os, svtkIndent indent)
{
