...
{
  double diff[3];
  svtkMath::Subtract(v1, v2, diff);
  if (svtkMath::Dot(diff, diff) < 1e-6)
  {
    return true;
  }
  return false;
...
bool CompareCell(svtkCell* c1, svtkCell* c2)
{
...
bool ComparePolyData(svtkPolyData* p1, svtkPolyData* p2)
{
  int pointPositionError = 0;
  for (svtkIdType i = 0; i < p1->GetNumberOfPoints(); ++i)
  {
  ...
  for (svtkIdType i = 0; i < p1->GetNumberOfCells(); ++i)
  {
...
int TestCapsuleSource(int argc, char* argv[])
{
  ...
  svtkSmartPointer<svtkCapsuleSource> capsuleSource = svtkSmartPointer<svtkCapsuleSource>::New();
  ...
  svtkSmartPointer<svtkPolyDataReader> r = svtkSmartPointer<svtkPolyDataReader>::New();
