...
int TestPointSource(int svtkNotUsed(argc), char* svtkNotUsed(argv)[])
{
  ...
  svtkSmartPointer<svtkPointSource> pointSource = svtkSmartPointer<svtkPointSource>::New();
  pointSource->SetDistributionToUniform();
  pointSource->SetNumberOfPoints(16);

  pointSource->SetOutputPointsPrecision(svtkAlgorithm::SINGLE_PRECISION);
  ...
  svtkSmartPointer<svtkPolyData> polyData = pointSource->GetOutput();
  ...
  pointSource->SetOutputPointsPrecision(svtkAlgorithm::DOUBLE_PRECISION);
