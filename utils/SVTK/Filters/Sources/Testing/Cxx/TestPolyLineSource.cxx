...
int TestPolyLineSource(int svtkNotUsed(argc), char* svtkNotUsed(argv)[])
{
  svtkSmartPointer<svtkPolyLineSource> source = svtkSmartPointer<svtkPolyLineSource>::New();
  ...
  {
    std::cerr << "Expected Closed setting to be " << expectedClosed << ", got " << actualClosed
              << std::endl;
    return EXIT_FAILURE;
  }
  ...
  svtkPoints* testPoints = source->GetPoints();
  if (testPoints->GetNumberOfPoints() != 4)
  {
    std::cerr << "Expected 4 points in svtkPoints returned from GetPoints() method, but got "
              << testPoints->GetNumberOfPoints() << " instead." << std::endl;
    return EXIT_FAILURE;
  }

  // Test the point values
  svtkPoints* outputPoints = source->GetPoints();
  for (int i = 0; i < 4; ++i)
  {
    double pt[3];
  ...
  // Test setting the points from a svtkPoints object
