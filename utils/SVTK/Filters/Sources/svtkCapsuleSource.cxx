...
svtkStandardNewMacro(svtkCapsuleSource);

//----------------------------------------------------------------------------
svtkCapsuleSource::svtkCapsuleSource(int res)
{
  res = res < 8 ? 8 : res;
  this->Radius = 0.5;
  this->Center[0] = 0.0;
...
//----------------------------------------------------------------------------
void InsertPole(
  svtkPoints* points, svtkFloatArray* normals, double center[3], double radius, double halfHeight)
{
  double x[3];
  x[0] = center[0];
...
//----------------------------------------------------------------------------
void FillHalfSphere(svtkPoints* points, svtkFloatArray* normals, double thetaResolution,
  double phiResolution, double startAngle, double sign, double center[3], double radius,
  double halfHeight)
{
  double n[3], x[3], norm;
  const double deltaTheta = svtkMath::Pi() / (thetaResolution - 1);
  const double deltaPhi = svtkMath::Pi() / (phiResolution - 1);
...
//----------------------------------------------------------------------------
void ConnectCylinderSide(svtkCellArray* faces, int minusPoleId, int plusPoleId, int clockwise,
...
//----------------------------------------------------------------------------
int svtkCapsuleSource::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** svtkNotUsed(inputVector), svtkInformationVector* outputVector)
{
  ...
  FillHalfSphere(newPoints, newNormals, this->ThetaResolution, this->PhiResolution,
    svtkMath::Pi() * 2.0, -1, this->Center, this->Radius, -1.0 * halfHeight);
...
//----------------------------------------------------------------------------
void svtkCapsuleSource::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
int svtkCapsuleSource::RequestInformation(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** svtkNotUsed(inputVector), svtkInformationVector* outputVector)
{
  svtkInformation* outInfo = outputVector->GetInformationObject(0);
  outInfo->Set(svtkStreamingDemandDrivenPipeline::UPDATE_NUMBER_OF_PIECES(), -1);
  const double halfLength = this->CylinderLength * 0.5;
  outInfo->Set(svtkStreamingDemandDrivenPipeline::BOUNDS(),
