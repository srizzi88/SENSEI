...
svtkStandardNewMacro(svtkFrustumSource);
svtkCxxSetObjectMacro(svtkFrustumSource, Planes, svtkPlanes);

// ----------------------------------------------------------------------------
svtkFrustumSource::svtkFrustumSource()
{
  this->Planes = nullptr;
  this->ShowLines = true;
...
// ----------------------------------------------------------------------------
int svtkFrustumSource::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** svtkNotUsed(inputVector), svtkInformationVector* outputVector)
{
  if (this->Planes == nullptr || this->Planes->GetNumberOfPlanes() != 6)
  {
    svtkErrorMacro(<< " 6 planes required.");
    return 0;
  }
  if (this->ShowLines)
  {
    ...
    {
      svtkErrorMacro(<< " LinesLength<=0.0");
      return 0;
    }
  }
  ...
  if (this->ShowLines)
  {
    ...
    double leftRight[3];
    svtkMath::Cross(left, right, leftRight);
    leftRightNull = leftRight[0] == 0.0 && leftRight[1] == 0.0 && leftRight[2] == 0.0;

    double bottomTop[3];
    svtkMath::Cross(bottom, top, bottomTop);
...
void svtkFrustumSource::ComputePoint(int planes[3], double* pt)
{
  // Ref: Real-Time Rendering, 3rd edition, Thomas Akenine-Moller, Eric Haines,
  // Naty Hoffman, page 783, section 16.17,
  // "Intersection Between Three Planes"

  svtkPlane* plane = this->Planes->GetPlane(planes[0]);
  ...
  double d0 = svtkMath::Dot(p0, n0);
  double d1 = svtkMath::Dot(p1, n1);
  double d2 = svtkMath::Dot(p2, n2);

  double c12[3];
  svtkMath::Cross(n1, n2, c12);
  double c20[3];
  svtkMath::Cross(n2, n0, c20);
  double c01[3];
  svtkMath::Cross(n0, n1, c01);

  double d = svtkMath::Determinant3x3(n0, n1, n2);
...
svtkMTimeType svtkFrustumSource::GetMTime()
{
...
// ----------------------------------------------------------------------------
void svtkFrustumSource::PrintSelf(ostream& os, svtkIndent indent)
{
