...
svtkStandardNewMacro(svtkHyperTreeGridSource);
svtkCxxSetObjectMacro(svtkHyperTreeGridSource, DescriptorBits, svtkBitArray);
svtkCxxSetObjectMacro(svtkHyperTreeGridSource, MaskBits, svtkBitArray);
svtkCxxSetObjectMacro(svtkHyperTreeGridSource, Quadric, svtkQuadric);

//----------------------------------------------------------------------------
svtkHyperTreeGridSource::svtkHyperTreeGridSource()
{
...
//-----------------------------------------------------------------------------
void svtkHyperTreeGridSource::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
void svtkHyperTreeGridSource::SetDimensions(const unsigned int* dims)
{
  this->Dimension = 0;
  unsigned int axis[2];
...
//----------------------------------------------------------------------------
void svtkHyperTreeGridSource::SetDimensions(unsigned int dimx, unsigned int dimy, unsigned int dimz)
{
  unsigned int dims[3];
  dims[0] = dimx;
  dims[1] = dimy;
...
//----------------------------------------------------------------------------
void svtkHyperTreeGridSource::SetIndexingModeToKJI()
{
  this->SetTransposedRootIndexing(false);
}

//----------------------------------------------------------------------------
void svtkHyperTreeGridSource::SetIndexingModeToIJK()
{
  this->SetTransposedRootIndexing(true);
}

//----------------------------------------------------------------------------
void svtkHyperTreeGridSource::SetLevelZeroMaterialIndex(svtkIdTypeArray* indexArray)
{
  ...
  this->LevelZeroMaterialMap.clear();
  svtkIdType len = indexArray->GetNumberOfTuples();
...
//----------------------------------------------------------------------------
unsigned int svtkHyperTreeGridSource::GetMaximumLevel()
{
  SVTK_LEGACY_REPLACED_BODY(
    svtkHyperTreeGridSource::GetMaximumLevel, "SVTK 9", svtkHyperTreeGridSource::GetMaxDepth);
  return this->GetMaxDepth();
}

//----------------------------------------------------------------------------
void svtkHyperTreeGridSource::SetMaximumLevel(unsigned int levels)
{
  SVTK_LEGACY_REPLACED_BODY(
    svtkHyperTreeGridSource::SetMaximumLevel, "SVTK 9", svtkHyperTreeGridSource::SetMaxDepth);
  this->SetMaxDepth(levels);
}
#endif

//----------------------------------------------------------------------------
unsigned int svtkHyperTreeGridSource::GetMaxDepth()
{
...
//----------------------------------------------------------------------------
void svtkHyperTreeGridSource::SetMaxDepth(unsigned int levels)
{
  if (levels < 1)
  {
    levels = 1;
...
//----------------------------------------------------------------------------
int svtkHyperTreeGridSource::FillOutputPortInformation(int, svtkInformation* info)
{
...
//----------------------------------------------------------------------------
int svtkHyperTreeGridSource::RequestInformation(
  svtkInformation*, svtkInformationVector**, svtkInformationVector* outputVector)
{
  ...
  outInfo->Set(svtkHyperTreeGrid::LEVELS(), this->MaxDepth);
  outInfo->Set(svtkHyperTreeGrid::DIMENSION(), this->Dimension);
...
//----------------------------------------------------------------------------
int svtkHyperTreeGridSource::RequestData(
  svtkInformation*, svtkInformationVector**, svtkInformationVector* outputVector)
{
  // Retrieve the output
  ...
  if (!output)
  {
    svtkErrorMacro("pre: output_not_HyperTreeGrid: " << outputDO->GetClassName());
    return 0;
  }
  ...
  switch (this->Dimension)
  {
    ...
    case 2:
    {
      ...
      svtkNew<svtkDoubleArray> coords1;
      ...
      svtkNew<svtkDoubleArray> coords2;
    ...
    case 3:
    {
      ...
      svtkNew<svtkDoubleArray> coordsx;
      ...
      svtkNew<svtkDoubleArray> coordsy;
      ...
      svtkNew<svtkDoubleArray> coordsz;
...
//----------------------------------------------------------------------------
int svtkHyperTreeGridSource::ProcessTrees(svtkHyperTreeGrid*, svtkDataObject* outputDO)
{
  ...
  for (svtkIdType itree = 0; itree < nbTrees; ++itree)
  {
    svtkIdType index = itree;
...
//----------------------------------------------------------------------------
void svtkHyperTreeGridSource::InitTreeFromDescriptor(
...
//-----------------------------------------------------------------------------
int svtkHyperTreeGridSource::InitializeFromStringDescriptor()
{
  ...
  {
    svtkErrorMacro(<< "Material mask is used but has length " << strlen(this->Mask)
  ...
  for (size_t i = 0; i < descLen; ++i)
  {
    ...
    switch (c)
    {
      case ' ':
        ...
        if (this->UseMask && m != ' ')
        {
          svtkErrorMacro(<< "Space separators do not match between "
      ...
      case '|':
        ...
        if (this->UseMask && m != '|')
        {
          svtkErrorMacro(<< "Level separators do not match between "
                           "descriptor and material mask.");
          return 0;
        }
        ...
        if (rootLevel)
        {
          ...
          {
            svtkErrorMacro(<< "String " << this->Descriptor << " describes " << nRefined + nLeaves
                          << " root cells != " << nTotal);
            return 0;
          }
        } // if (rootLevel)
        else
        {
          ...
          {
            svtkErrorMacro(<< "String level descriptor " << descriptor.str().c_str()
      ...
      case 'R':
        ...
        if (this->UseMask && m == '0')
        {
          svtkErrorMacro(<< "A refined branch must contain material.");
          return 0;
        }
      ...
      default:
        svtkErrorMacro(<< "Unrecognized character: " << c << " at pos " << i << " in descriptor "
                      << this->Descriptor);
  ...
  if (descriptor.str().size() != nNextLevel)
  {
    svtkErrorMacro(<< "String level descriptor " << descriptor.str().c_str() << " has cardinality "
  ...
  for (unsigned int i = 1; i < nLevels; ++i)
  {
    this->LevelBitsIndex.push_back(
      this->LevelBitsIndex[i - 1] + static_cast<svtkIdType>(this->LevelDescriptors[i - 1].length()));
  }
...
//----------------------------------------------------------------------------
void svtkHyperTreeGridSource::SubdivideFromStringDescriptor(svtkHyperTreeGrid* output,
  ...
  int idx[3], int parentPos)
{
  ...
  svtkIdType id = this->LevelBitsIndexCnt[level];
    ...
    if (this->Dimension == 1)
    {
      switch (this->Orientation)
      {
        ...
        case 2:
          ...
          break;
        default:
          svtkErrorMacro(<< "Incorrect orientation in 1D: " << this->Orientation);
          return;
      } // switch (this->Orientation)
    }   // if (this->Dimension == 1)
    else if (this->Dimension == 2)
    {
      switch (this->Orientation)
      {
        ...
        case 2:
          ...
          break;
        default:
          svtkErrorMacro(<< "Incorrect orientation in 2D: " << this->Orientation);
          return;
      } // switch (this->Orientation)
    }   // else if (this->Dimension == 2)
...
//----------------------------------------------------------------------------
int svtkHyperTreeGridSource::InitializeFromBitsDescriptor()
{
  // Verify that grid and material specifications are consistent
  if (this->UseMask && !this->LevelZeroMaterialIndex &&
    this->MaskBits->GetSize() != this->DescriptorBits->GetSize())
  {
    svtkErrorMacro(<< "Material mask is used but has length " << this->MaskBits->GetSize()
  ...
  if (this->LevelZeroMaterialIndex)
  {
    nTotal = static_cast<svtkIdType>(this->LevelZeroMaterialMap.size());
  }
  else
  {
    for (unsigned int idim = 0; idim < 3; ++idim)
    {
      if (this->Dimensions[idim] != 1)
      {
        nTotal *= static_cast<svtkIdType>(this->Dimensions[idim] - 1);
      }
    }
  }
  ...
  svtkIdType nNextLevel = nTotal;
  svtkIdType nCurrentLevelCount = 0;
  svtkIdType descSize = this->DescriptorBits->GetNumberOfTuples();
  ...
  if (nCurrentLevelCount != nNextLevel)
  {
    svtkErrorMacro(<< "Level descriptor " << nCurrentLevel << " has cardinality "
...
//----------------------------------------------------------------------------
void svtkHyperTreeGridSource::SubdivideFromBitsDescriptor(svtkHyperTreeGrid* output,
  svtkHyperTreeGridNonOrientedCursor* cursor, unsigned int level, int treeIdx, int childIdx,
  int idx[3], int parentPos)
{
  // Get handle on point data
  ...
  svtkIdType startIdx = this->LevelBitsIndex[level];
...
//-----------------------------------------------------------------------------
void svtkHyperTreeGridSource::SubdivideFromQuadric(svtkHyperTreeGrid* output,
  svtkHyperTreeGridNonOrientedCursor* cursor, unsigned int level, int treeIdx, const int idx[3],
  double origin[3], double size[3])
{
  ...
  // Calculate the node global index
  svtkIdType id = cursor->GetTree()->GetGlobalIndexFromLocal(cursor->GetVertexId());
...
//-----------------------------------------------------------------------------
void svtkHyperTreeGridSource::SetQuadricCoefficients(double q[10])
{
  if (!this->Quadric)
  {
    this->Quadric = svtkQuadric::New();
...
//-----------------------------------------------------------------------------
void svtkHyperTreeGridSource::GetQuadricCoefficients(double q[10])
{
  this->Quadric->GetCoefficients(q);
}

//-----------------------------------------------------------------------------
double* svtkHyperTreeGridSource::GetQuadricCoefficients()
{
  return this->Quadric->GetCoefficients();
}

//----------------------------------------------------------------------------
svtkMTimeType svtkHyperTreeGridSource::GetMTime()
{
...
//----------------------------------------------------------------------------
svtkBitArray* svtkHyperTreeGridSource::ConvertDescriptorStringToBitArray(const std::string& str)
{
  svtkBitArray* desc = svtkBitArray::New();
  desc->Allocate(static_cast<svtkIdType>(str.length()));
  ...
  for (std::string::const_iterator dit = str.begin(); dit != str.end(); ++dit)
  {
    switch (*dit)
    {
      ...
      default:
        svtkErrorMacro(<< "Unrecognized character: " << *dit << " in string " << str);
        desc->Delete();
        return nullptr;
    } // switch(*dit)
  }
...
//----------------------------------------------------------------------------
svtkBitArray* svtkHyperTreeGridSource::ConvertMaskStringToBitArray(const std::string& str)
{
