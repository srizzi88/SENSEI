/*=========================================================================
  ...
  Module:    svtkParametricFunctionSource.cxx
...
svtkStandardNewMacro(svtkParametricFunctionSource);
svtkCxxSetObjectMacro(svtkParametricFunctionSource, ParametricFunction, svtkParametricFunction);

//----------------------------------------------------------------------------
svtkParametricFunctionSource::svtkParametricFunctionSource()
  : ParametricFunction(nullptr)
  , UResolution(50)
  , VResolution(50)
  , WResolution(50)
  , GenerateTextureCoordinates(0)
  , ScalarMode(svtkParametricFunctionSource::SCALAR_NONE)
  , OutputPointsPrecision(svtkAlgorithm::SINGLE_PRECISION)
{
 ...
 *
 */
void AddTriCells(svtkCellArray* cellArray, int id1, int id2, int id3, int id4, bool clockwise)
{
...
//----------------------------------------------------------------------------
void svtkParametricFunctionSource::MakeTriangles(svtkCellArray* cells, int PtsU, int PtsV)
{
  int id1 = 0;
  int id2 = 0;
  int id3 = 0;
  ...
  svtkDebugMacro(<< "Executing MakeTriangles()");

  bool clockwise = (this->ParametricFunction->GetClockwiseOrdering() != 0);

  svtkIdType numCells = (PtsU + this->ParametricFunction->GetJoinU() - 1) *
  ...
  cells->Modified();
  svtkDebugMacro(<< "MakeTriangles() finished.");
}

//----------------------------------------------------------------------------
int svtkParametricFunctionSource::RequestData(svtkInformation* svtkNotUsed(info),
  svtkInformationVector** svtkNotUsed(inputV), svtkInformationVector* output)
{
  svtkDebugMacro(<< "Executing");

  // Check that a parametric function has been defined
  if (!this->ParametricFunction)
  {
    svtkErrorMacro(<< "Parametric function not defined");
    return 1;
  }
  ...
  switch (this->ParametricFunction->GetDimension())
  {
    ...
    case 2:
      ...
      break;
    default:
      svtkErrorMacro("Functions of dimension " << this->ParametricFunction->GetDimension()
                                              << " are not supported.");
  }
...
//----------------------------------------------------------------------------
void svtkParametricFunctionSource::Produce1DOutput(svtkInformationVector* output)
{
  svtkIdType numPts = this->UResolution + 1;
  ...
  svtkPolyData* outData = static_cast<svtkPolyData*>(outInfo->Get(svtkDataObject::DATA_OBJECT()));
  outData->SetPoints(pts);
  outData->SetLines(lines);
}

//----------------------------------------------------------------------------
void svtkParametricFunctionSource::Produce2DOutput(svtkInformationVector* output)
{
  ...
  svtkSmartPointer<svtkFloatArray> sval = svtkSmartPointer<svtkFloatArray>::New();
  ...
  svtkSmartPointer<svtkFloatArray> nval = svtkSmartPointer<svtkFloatArray>::New();
  ...
  // Texture coordinates
  svtkSmartPointer<svtkFloatArray> newTCoords = svtkSmartPointer<svtkFloatArray>::New();
    ...
    for (int j = 0; j < PtsV; ++j)
    {
      ...
      if (this->ScalarMode != SCALAR_NONE)
      {
        switch (this->ScalarMode)
        {
          ...
          case SCALAR_PHASE:
            ...
            {
              scalar = svtkMath::DegreesFromRadians(atan2(rel_v, rel_u));
              if (scalar < 0)
              {
                scalar += 360;
              }
      ...
      if (this->ParametricFunction->GetDerivativesAvailable() && this->GenerateNormals)
      {
        ...
        if (this->ParametricFunction->GetClockwiseOrdering() == 0)
        {
          ...
          svtkMath::Cross(Dv, Du, n);
        }
        else
        {
          // Clockwise ordering
          svtkMath::Cross(Du, Dv, n);
        }
  ...
  svtkInformation* outInfo = output->GetInformationObject(0);
  svtkPolyData* outData = static_cast<svtkPolyData*>(outInfo->Get(svtkDataObject::DATA_OBJECT()));
  svtkCellArray* tris = svtkCellArray::New();
  ...
  if (this->GenerateNormals)
  {
    ...
    else
    {
      ...
      pd->SetPoints(points);
      pd->SetPolys(tris);
      svtkSmartPointer<svtkPolyDataNormals> norm = svtkSmartPointer<svtkPolyDataNormals>::New();
      // we prevent svtkPolyDataNormals from generating new points
...
//----------------------------------------------------------------------------
svtkMTimeType svtkParametricFunctionSource::GetMTime()
{
  svtkMTimeType mTime = this->Superclass::GetMTime();
  svtkMTimeType funcMTime;
...
//----------------------------------------------------------------------------
void svtkParametricFunctionSource::PrintSelf(ostream& os, svtkIndent indent)
{
