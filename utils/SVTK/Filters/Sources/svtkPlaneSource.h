 ...
 *
 * svtkPlaneSource creates an m x n array of quadrilaterals arranged as
...
#ifndef svtkPlaneSource_h
#define svtkPlaneSource_h

#include "svtkFiltersSourcesModule.h" // For export macro
#include "svtkPolyDataAlgorithm.h"

class SVTKFILTERSSOURCES_EXPORT svtkPlaneSource : public svtkPolyDataAlgorithm
{
public:
  void PrintSelf(ostream& os, svtkIndent indent) override;
  svtkTypeMacro(svtkPlaneSource, svtkPolyDataAlgorithm);

  /**
   * Construct plane perpendicular to z-axis, resolution 1x1, width
   * and height 1.0, and centered at the origin.
   */
  static svtkPlaneSource* New();

  //@{
  /**
   * Specify the resolution of the plane along the first axes.
   */
  svtkSetMacro(XResolution, int);
  svtkGetMacro(XResolution, int);
  //@}
   ...
   */
  svtkSetMacro(YResolution, int);
  svtkGetMacro(YResolution, int);
  //@}
...
private:
  svtkPlaneSource(const svtkPlaneSource&) = delete;
  void operator=(const svtkPlaneSource&) = delete;
};
