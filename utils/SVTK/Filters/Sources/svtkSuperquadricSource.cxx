/*=========================================================================
  ...
  Module:    svtkSuperquadricSource.cxx
...
svtkStandardNewMacro(svtkSuperquadricSource);
...
// Description:
svtkSuperquadricSource::svtkSuperquadricSource(int res)
{
  res = res < 4 ? 4 : res;
...
void svtkSuperquadricSource::SetPhiResolution(int i)
{
  if (i < 4)
  {
    i = 4;
  ...
  if (i > SVTK_MAX_SUPERQUADRIC_RESOLUTION)
  {
    i = SVTK_MAX_SUPERQUADRIC_RESOLUTION;
  }
...
void svtkSuperquadricSource::SetThetaResolution(int i)
{
  if (i < 8)
  {
    i = 8;
...
void svtkSuperquadricSource::SetThetaRoundness(double e)
{
...
void svtkSuperquadricSource::SetPhiRoundness(double e)
{
  if (e < SVTK_MIN_SUPERQUADRIC_ROUNDNESS)
  {
    e = SVTK_MIN_SUPERQUADRIC_ROUNDNESS;
...
int svtkSuperquadricSource::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** svtkNotUsed(inputVector), svtkInformationVector* outputVector)
{
  ...
  if (this->Toroidal)
  {
    phiLim[0] = -svtkMath::Pi();
    phiLim[1] = svtkMath::Pi();

    thetaLim[0] = -svtkMath::Pi();
    thetaLim[1] = svtkMath::Pi();
  ...
  {
    // Ellipsoidal
    phiLim[0] = -svtkMath::Pi() / 2.0;
    phiLim[1] = svtkMath::Pi() / 2.0;
  ...
  ptidx = new svtkIdType[ptsPerStrip];
...
void svtkSuperquadricSource::PrintSelf(ostream& os, svtkIndent indent)
{
...
static double cf(double w, double m, double a = 0)
{
  ...
  if (w == svtkMath::Pi() || w == -svtkMath::Pi())
  {
    c = -1.0;
  }
  else
