...
svtkStandardNewMacro(svtkTextSource);

#define svtkfont_width 9
#define svtkfont_row_width 864
#define svtkfont_height 15
// clang-format off
static unsigned char svtkfont_bits[] = {
...
int svtkTextSource::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** svtkNotUsed(inputVector), svtkInformationVector* outputVector)
{
  ...
  while (this->Text[pos])
  {
    if (this->Text[pos] != 32)
    {
      for (col = 0; col < svtkfont_width; col++)
      {
        acol = (this->Text[pos] - 32) * svtkfont_width + col - 1;
        for (row = 0; row < svtkfont_height; row++)
        {
          pixelPos = acol + row * svtkfont_row_width;
          if (svtkfont_bits[pixelPos / 8] & (0x01 << pixelPos % 8))
          {
            if (drawingBlack)
            {
              x[0] = pos * svtkfont_width + col + 1;
              x[1] = svtkfont_height - row;
    ...
    else
    {
      ...
      if (this->Backing)
      {
        ...
        x[0] = pos * svtkfont_width + svtkfont_width;
        x[1] = svtkfont_height;
        newPoints->InsertNextPoint(x);
        newScalars->InsertNextValue(black[0]);
        newScalars->InsertNextValue(black[1]);
...
void svtkTextSource::PrintSelf(ostream& os, svtkIndent indent)
{
