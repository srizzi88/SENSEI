...
int TestComputeQuartiles(int, char*[])
{
  ...
  svtkNew<svtkComputeQuartiles> quartiles;

  svtkNew<svtkTest::ErrorObserver> errorObserver1;
  // First verify that absence of input does not cause trouble
  quartiles->GetExecutive()->AddObserver(svtkCommand::ErrorEvent, errorObserver1);
  quartiles->Update();
  errorObserver1->CheckErrorMessage("Input port 0 of algorithm svtkComputeQuartiles");

  // Now set the real input table
  quartiles->SetInputData(svtkStatisticsAlgorithm::INPUT_DATA, table);
  quartiles->Update();
