...
int TestExtractFunctionalBagPlot(int, char*[])
{
  ...
  svtkNew<svtkDoubleArray> density;
  density->SetName("Density");
  density->SetNumberOfValues(numCols);

  svtkNew<svtkStringArray> varName;
  ...
  svtkNew<svtkTable> inTableDensity;
  ...
  svtkNew<svtkExtractFunctionalBagPlot> ebp;
  ...
  ebp->GetExecutive()->AddObserver(svtkCommand::ErrorEvent, errorObserver1);
  ebp->Update();
  int status =
    errorObserver1->CheckErrorMessage("Input port 0 of algorithm svtkExtractFunctionalBagPlot");

  ebp->SetInputData(0, table);
  ebp->SetInputData(1, inTableDensity);
  ebp->SetInputArrayToProcess(0, 1, 0, svtkDataObject::FIELD_ASSOCIATION_ROWS, "Density");
  ebp->SetInputArrayToProcess(1, 1, 0, svtkDataObject::FIELD_ASSOCIATION_ROWS, "ColName");
  ebp->Update();

  svtkTable* outBPTable = ebp->GetOutput();
  svtkDoubleArray* q3Points = nullptr;
  for (svtkIdType i = 0; i < outBPTable->GetNumberOfColumns(); i++)
  {
    const char* colName = outBPTable->GetColumnName(i);
    if (svtksys::SystemTools::StringStartsWith(colName, "Q3Points"))
    {
      q3Points = svtkArrayDownCast<svtkDoubleArray>(outBPTable->GetColumn(i));
      break;
    }
  }
  svtkDoubleArray* q2Points =
    svtkArrayDownCast<svtkDoubleArray>(outBPTable->GetColumnByName("QMedPoints"));
