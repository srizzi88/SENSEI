/*=========================================================================
  ...
  Module:    svtkCorrelativeStatistics.cxx
...
svtkObjectFactoryNewMacro(svtkCorrelativeStatistics);

// ----------------------------------------------------------------------
svtkCorrelativeStatistics::svtkCorrelativeStatistics()
{
...
// ----------------------------------------------------------------------
svtkCorrelativeStatistics::~svtkCorrelativeStatistics() = default;

// ----------------------------------------------------------------------
void svtkCorrelativeStatistics::PrintSelf(ostream& os, svtkIndent indent)
{
  this->Superclass::PrintSelf(os, indent);
}

// ----------------------------------------------------------------------
void svtkCorrelativeStatistics::Aggregate(
...
// ----------------------------------------------------------------------
void svtkCorrelativeStatistics::Learn(
...
// ----------------------------------------------------------------------
void svtkCorrelativeStatistics::Derive(svtkMultiBlockDataSet* inMeta)
{
  ...
  int numDoubles = 9;
  svtkStdString doubleNames[] = { "Variance X", "Variance Y", "Covariance", "Determinant",
  ...
  for (int i = 0; i < nRow; ++i)
  {
    ...
    if (varX < SVTK_DBL_MIN || varY < SVTK_DBL_MIN)
    {
...
// ----------------------------------------------------------------------
svtkDoubleArray* svtkCorrelativeStatistics::CalculatePValues(svtkDoubleArray* statCol)
{
...
// ----------------------------------------------------------------------
void svtkCorrelativeStatistics::Test(
  svtkTable* inData, svtkMultiBlockDataSet* inMeta, svtkTable* outMeta)
{
  ...
  svtkStringArray* nameColX = svtkStringArray::New();
  nameColX->SetName("Variable X");

  svtkStringArray* nameColY = svtkStringArray::New();
  ...
  svtkStringArray* varsX =
    svtkArrayDownCast<svtkStringArray>(primaryTab->GetColumnByName("Variable X"));
  svtkStringArray* varsY =
    svtkArrayDownCast<svtkStringArray>(primaryTab->GetColumnByName("Variable Y"));
       ...
       rit != this->Internals->Requests.end(); ++rit)
  {
    ...
    if (r >= nRowPrim)
    {
      svtkWarningMacro("Incomplete input: model does not have a row for pair"
    ...
    if (detS < SVTK_DBL_MIN || sX2 < 0. || sY2 < 0.)
    {
    ...
    else
    {
      ...
      if (sXY < sqrt(SVTK_DBL_MIN) || sXY < (.5 * sqrt(SVTK_DBL_EPSILON) * fabs(sX2 - sY2)))
      {
...
// ----------------------------------------------------------------------
class BivariateRegressionDeviationsFunctor : public svtkStatisticsAlgorithm::AssessFunctor
{
public:
  svtkDataArray* DataX;
  svtkDataArray* DataY;
  ...
  BivariateRegressionDeviationsFunctor(svtkDataArray* valsX, svtkDataArray* valsY, double meanX,
...
// ----------------------------------------------------------------------
void svtkCorrelativeStatistics::SelectAssessFunctor(
  svtkTable* outData, svtkDataObject* inMetaDO, svtkStringArray* rowNames, AssessFunctor*& dfunc)
{
  ...
  for (int r = 0; r < nRowPrim; ++r)
  {
    if (varX->GetValue(r) == varNameX && varY->GetValue(r) == varNameY)
    {
      ...
      svtkAbstractArray* arrX = outData->GetColumnByName(varNameX);
      svtkAbstractArray* arrY = outData->GetColumnByName(varNameY);
      if (!arrX || !arrY)
      {
        return;
      }
      ...
      // not fit here).
      svtkDataArray* valsX = svtkArrayDownCast<svtkDataArray>(arrX);
      svtkDataArray* valsY = svtkArrayDownCast<svtkDataArray>(arrY);
      if (!valsX || !valsY)
      {
        return;
      }
      ...
      double invDetXY;
      if (detXY < SVTK_DBL_MIN || varianceX < 0. || varianceY < 0.)
      {
        invDetXY = svtkMath::Nan();
      }
      else
      {
        invDetXY = 1. / detXY;
