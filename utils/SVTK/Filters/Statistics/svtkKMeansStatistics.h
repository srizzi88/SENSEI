...
/*-------------------------------------------------------------------------
  ...
  -------------------------------------------------------------------------*/
/**
 * @class   svtkKMeansStatistics
 * @brief   A class for KMeans clustering
 *
 *
 * This class takes as input an optional svtkTable on port LEARN_PARAMETERS
 ...
 * of type svtkIdType), while the remaining columns are a
 ...
 * of type svtkIdType, then the table will be ignored and a single run will be performed using
 ...
 *   one svtkTable with columns specifying the following for each run:
 ...
 * coordinates.
 *
 * * Derive:  An additional svtkTable is stored in the multiblock dataset output on port
 ...
 *
 * The code can handle a wide variety of data types as it operates on svtkAbstractArrays
 * and is not limited to svtkDataArrays.  A default distance functor that
 * computes the sum of the squares of the Euclidean distance between two objects is provided
 * (svtkKMeansDistanceFunctor). The default distance functor can be overridden to use alternative
 * distance metrics.
 *
 * @par Thanks:
...
#ifndef svtkKMeansStatistics_h
#define svtkKMeansStatistics_h
...
class SVTKFILTERSSTATISTICS_EXPORT svtkKMeansStatistics : public svtkStatisticsAlgorithm
{
public:
  svtkTypeMacro(svtkKMeansStatistics, svtkStatisticsAlgorithm);
  void PrintSelf(ostream& os, svtkIndent indent) override;
  static svtkKMeansStatistics* New();

  //@{
  /**
   * Set the DistanceFunctor.
   */
  virtual void SetDistanceFunctor(svtkKMeansDistanceFunctor*);
  svtkGetObjectMacro(DistanceFunctor, svtkKMeansDistanceFunctor);
  //@}
   ...
   */
  svtkSetMacro(DefaultNumberOfClusters, int);
  svtkGetMacro(DefaultNumberOfClusters, int);
  //@}
   ...
   */
  svtkSetStringMacro(KValuesArrayName);
  svtkGetStringMacro(KValuesArrayName);
  //@}
   ...
   */
  svtkSetMacro(MaxNumIterations, int);
  svtkGetMacro(MaxNumIterations, int);
  //@}
...
protected:
  svtkKMeansStatistics();
  ~svtkKMeansStatistics() override;
   ...
   */
  void SelectAssessFunctor(svtkTable* inData, svtkDataObject* inMeta, svtkStringArray* rowNames,
    AssessFunctor*& dfunc) override;

  /**
   * Subroutine to update new cluster centers from the old centers.
   * Called from within Learn (and will be overridden by svtkPKMeansStatistics
   * to handle distributed datasets).
   */
  virtual void UpdateClusterCenters(svtkTable* newClusterElements, svtkTable* curClusterElements,
   ...
   * to handle distributed datasets).
   */
  virtual svtkIdType GetTotalNumberOfObservations(svtkIdType numObservations);
   ...
   */
  int InitializeDataAndClusterCenters(svtkTable* inParameters, svtkTable* inData,
   ...
   * to handle distributed datasets).
   */
  virtual void CreateInitialClusterCenters(svtkIdType numToAllocate,
   ...
   * overridden.
   */
  svtkKMeansDistanceFunctor* DistanceFunctor;

private:
  svtkKMeansStatistics(const svtkKMeansStatistics&) = delete;
  void operator=(const svtkKMeansStatistics&) = delete;
};
