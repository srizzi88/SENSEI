...
#ifndef svtkPCAStatistics_h
#define svtkPCAStatistics_h
...
class SVTKFILTERSSTATISTICS_EXPORT svtkPCAStatistics : public svtkMultiCorrelativeStatistics
{
public:
  svtkTypeMacro(svtkPCAStatistics, svtkMultiCorrelativeStatistics);
  void PrintSelf(ostream& os, svtkIndent indent) override;
  static svtkPCAStatistics* New();
   ...
   */
  svtkSetMacro(NormalizationScheme, int);
  svtkGetMacro(NormalizationScheme, int);
   ...
   */
  virtual svtkTable* GetSpecifiedNormalization();
  virtual void SetSpecifiedNormalization(svtkTable*);
  //@}
   ...
   * This function:
   * void GetEigenvalues(int request, int i, svtkDoubleArray*);
   ...
   * option is turned on.
   */
  void GetEigenvalues(int request, svtkDoubleArray*);
  void GetEigenvalues(svtkDoubleArray*);
   ...
   * This function:
   * void GetEigenvectors(int request, svtkDoubleArray* eigenvectors)
   ...
   */
  void GetEigenvectors(int request, svtkDoubleArray* eigenvectors);
  void GetEigenvectors(svtkDoubleArray* eigenvectors);
  void GetEigenvector(int i, svtkDoubleArray* eigenvector);
  void GetEigenvector(int request, int i, svtkDoubleArray* eigenvector);
  //@}
   ...
   * When set to svtkPCAStatistics::FULL_BASIS, the entire set of basis vectors
   ...
   * When set to svtkPCAStatistics::FIXED_BASIS_SIZE, only the first N basis vectors
   ...
   * When set to svtkPCAStatistics::FIXED_BASIS_ENERGY, the number of basis vectors
   ...
   * By default BasisScheme is set to svtkPCAStatistics::FULL_BASIS.
   */
  svtkSetMacro(BasisScheme, int);
  svtkGetMacro(BasisScheme, int);
   ...
   * basis scheme.
   */
  svtkSetMacro(FixedBasisSize, int);
  svtkGetMacro(FixedBasisSize, int);
  //@}
   ...
   */
  svtkSetClampMacro(FixedBasisEnergy, double, 0., 1.);
  svtkGetMacro(FixedBasisEnergy, double);
  //@}
...
protected:
  svtkPCAStatistics();
  ~svtkPCAStatistics() override;

  /**
   * This algorithm accepts a svtkTable containing normalization values for
   ...
   */
  virtual svtkDoubleArray* CalculatePValues(svtkIdTypeArray*, svtkDoubleArray*);
...
private:
  svtkPCAStatistics(const svtkPCAStatistics&) = delete;
  void operator=(const svtkPCAStatistics&) = delete;
};

#endif // svtkPCAStatistics_h
