...
Module:    svtkStatisticsAlgorithmPrivate.h
 ...
 * through the APIs of the svtkStatistics classes APIs
 *
 * @par Thanks:
...
#ifndef svtkStatisticsAlgorithmPrivate_h
#define svtkStatisticsAlgorithmPrivate_h
...
{
public:
  svtkStatisticsAlgorithmPrivate() {}
  ~svtkStatisticsAlgorithmPrivate() {}
  // --------------------------------------------------------------------
  /**
   ...
   */
  svtkIdType GetNumberOfRequests() { return static_cast<svtkIdType>(this->Requests.size()); }
  // --------------------------------------------------------------------
  //@{
  /**
   * Return the number of columns associated with request \a r.
   */
  svtkIdType GetNumberOfColumnsForRequest(svtkIdType r)
  {
    if (r < 0 || r > static_cast<svtkIdType>(this->Requests.size()))
    {
      return 0;
    }
    std::set<std::set<svtkStdString> >::iterator it = this->Requests.begin();
    for (svtkIdType i = 0; i < r; ++i)
    {
      ++it;
    }
    return static_cast<svtkIdType>(it->size());
  }
  // --------------------------------------------------------------------
  //@}
  //@{
   ...
   */
  bool GetColumnForRequest(svtkIdType r, svtkIdType c, svtkStdString& columnName)
  {
    if (r < 0 || r > static_cast<svtkIdType>(this->Requests.size()) || c < 0)
    {
      return false;
    }
    std::set<std::set<svtkStdString> >::const_iterator it = this->Requests.begin();
    for (svtkIdType i = 0; i < r; ++i)
    {
      ++it;
    }
    if (c > static_cast<svtkIdType>(it->size()))
    {
      return false;
    }
    std::set<svtkStdString>::const_iterator cit = it->begin();
    for (svtkIdType j = 0; j < c; ++j)
    {
      ++cit;
    }
    columnName = *cit;
  ...
  std::set<std::set<svtkStdString> > Requests;
  std::set<svtkStdString> Buffer;
};

#endif // svtkStatisticsAlgorithmPrivate_h

// SVTK-HeaderTest-Exclude: svtkStatisticsAlgorithmPrivate.h
