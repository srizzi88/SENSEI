/*=========================================================================

  Program:   Visualization Toolkit
  Module:    svtkTextureMapToCylinder.cxx

  Copyright (c) Ken Martin, Will Schroeder, Bill Lorensen
  All rights reserved.
  See Copyright.txt or http://www.kitware.com/Copyright.htm for details.

     This software is distributed WITHOUT ANY WARRANTY; without even
     the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR
     PURPOSE.  See the above copyright notice for more information.

=========================================================================*/
#include "svtkTextureMapToCylinder.h"

#include "svtkCellData.h"
#include "svtkDataSet.h"
#include "svtkFloatArray.h"
#include "svtkInformation.h"
#include "svtkInformationVector.h"
#include "svtkLine.h"
#include "svtkMath.h"
#include "svtkOBBTree.h"
#include "svtkObjectFactory.h"
#include "svtkPointData.h"
#include "svtkPoints.h"

svtkStandardNewMacro(svtkTextureMapToCylinder);

// Create object with cylinder axis parallel to z-axis (points (0,0,-0.5)
// and (0,0,0.5)). The PreventSeam ivar is set to true. The cylinder is
// automatically generated.
svtkTextureMapToCylinder::svtkTextureMapToCylinder()
{
  this->Point1[0] = 0.0;
  this->Point1[1] = 0.0;
  this->Point1[2] = -0.5;

  this->Point2[0] = 0.0;
  this->Point2[1] = 0.0;
  this->Point2[2] = 0.5;

  this->AutomaticCylinderGeneration = 1;
  this->PreventSeam = 1;
}

int svtkTextureMapToCylinder::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  // get the info objects
  svtkInformation* inInfo = inputVector[0]->GetInformationObject(0);
  svtkInformation* outInfo = outputVector->GetInformationObject(0);

  // get the input and output
  svtkDataSet* input = svtkDataSet::SafeDownCast(inInfo->Get(svtkDataObject::DATA_OBJECT()));
  svtkDataSet* output = svtkDataSet::SafeDownCast(outInfo->Get(svtkDataObject::DATA_OBJECT()));

  svtkFloatArray* newTCoords;
  svtkIdType numPts = input->GetNumberOfPoints();
  svtkIdType ptId;
  int i;
  double x[3], tc[2], thetaX, thetaY, closest[3], v[3];
  double axis[3], vP[3], vec[3];

  svtkDebugMacro(<< "Generating Cylindrical Texture Coordinates");

  // First, copy the input to the output as a starting point
  output->CopyStructure(input);

  if (numPts < 1)
  {
    svtkErrorMacro(<< "Can't generate texture coordinates without points");
    return 1;
  }

  double point1[3] = { this->Point1[0], this->Point1[1], this->Point1[2] };
  double point2[3] = { this->Point2[0], this->Point2[1], this->Point2[2] };

  if (this->AutomaticCylinderGeneration)
  {
    svtkPoints* pts = svtkPoints::New();
    pts->SetNumberOfPoints(numPts);
    double corner[3], max[3], mid[3], min[3], size[3], l;
    svtkOBBTree* OBB = svtkOBBTree::New();

    for (ptId = 0; ptId < numPts; ptId++)
    {
      input->GetPoint(ptId, x);
      pts->SetPoint(ptId, x);
    }

    OBB->ComputeOBB(pts, corner, max, mid, min, size);
    pts->Delete();
    OBB->Delete();

    for (i = 0; i < 3; i++)
    {
      l = (mid[i] + min[i]) / 2.0;
      point1[i] = corner[i] + l;
      point2[i] = corner[i] + max[i] + l;
    }

    svtkDebugMacro(<< "Cylinder axis computed as \tPoint1: (" << point1[0] << ", " << point1[1]
                  << ", " << point1[2] << ")\n\t\t\t\tPoint2: (" << point2[0] << ", " << point2[1]
                  << ", " << point2[2] << ")");
  }

  // compute axis which is theta (angle measure) origin
  for (i = 0; i < 3; i++)
  {
    axis[i] = point2[i] - point1[i];
  }
  if (svtkMath::Norm(axis) == 0.0)
  {
    svtkErrorMacro(<< "Bad cylinder axis");
    return 1;
  }

  v[0] = 1.0;
  v[1] = v[2] = 0.0;
  svtkMath::Cross(axis, v, vP);
  if (svtkMath::Norm(vP) == 0.0)
  { // must be prependicular
    v[1] = 1.0;
    v[0] = v[2] = 0.0;
    svtkMath::Cross(axis, v, vP);
  }
  svtkMath::Cross(vP, axis, vec);
  if (svtkMath::Normalize(vec) == 0.0)
  {
    svtkErrorMacro(<< "Bad cylinder axis");
    return 1;
  }
  newTCoords = svtkFloatArray::New();
  newTCoords->SetName("Texture Coordinates");
  newTCoords->SetNumberOfComponents(2);
  newTCoords->Allocate(2 * numPts);

  // loop over all points computing spherical coordinates
  for (ptId = 0; ptId < numPts; ptId++)
  {
    input->GetPoint(ptId, x);
    svtkLine::DistanceToLine(x, point1, point2, tc[1], closest);

    for (i = 0; i < 3; i++)
    {
      v[i] = x[i] - closest[i];
    }
    svtkMath::Normalize(v);

    thetaX = acos((double)svtkMath::Dot(v, vec));
    svtkMath::Cross(vec, v, vP);
    thetaY = svtkMath::Dot(axis, vP); // not really interested in angle, just +/- sign

    if (this->PreventSeam)
    {
      tc[0] = thetaX / svtkMath::Pi();
    }
    else
    {
      tc[0] = thetaX / (2.0 * svtkMath::Pi());
      if (thetaY < 0.0)
      {
        tc[0] = 1.0 - tc[0];
      }
    }

    newTCoords->InsertTuple(ptId, tc);
  }

  output->GetPointData()->CopyTCoordsOff();
  output->GetPointData()->PassData(input->GetPointData());
  output->GetCellData()->PassData(input->GetCellData());

  output->GetPointData()->SetTCoords(newTCoords);
  newTCoords->Delete();

  return 1;
}

void svtkTextureMapToCylinder::PrintSelf(ostream& os, svtkIndent indent)
{
  this->Superclass::PrintSelf(os, indent);

  os << indent
     << "Automatic Cylinder Generation: " << (this->AutomaticCylinderGeneration ? "On\n" : "Off\n");
  os << indent << "Prevent Seam: " << (this->PreventSeam ? "On\n" : "Off\n");
  os << indent << "Point1: (" << this->Point1[0] << ", " << this->Point1[1] << ", "
     << this->Point1[2] << ")\n";
  os << indent << "Point2: (" << this->Point2[0] << ", " << this->Point2[1] << ", "
     << this->Point2[2] << ")\n";
}
