/*=========================================================================
  ...
  Module:    svtkTextureMapToPlane.cxx
...
svtkStandardNewMacro(svtkTextureMapToPlane);

// Construct with s,t range=(0,1) and automatic plane generation turned on.
svtkTextureMapToPlane::svtkTextureMapToPlane()
{
...
int svtkTextureMapToPlane::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  {
    svtkErrorMacro(<< "Not enough points for automatic plane mapping\n");
    return 1;
  }
      ...
      this->Point1[0] == 0.0 && this->Point1[1] == 0.0 && this->Point1[2] == 0.0))
  {
    ...
    for (minProj = 1.0, i = 0; i < 3; i++)
    {
      ...
      axis[i] = 1.0;
      if ((proj = fabs(svtkMath::Dot(this->Normal, axis))) < minProj)
      {
        minProj = proj;
        dir = i;
      }
    ...
    svtkMath::Cross(this->Normal, axis, tAxis);
    svtkMath::Normalize(tAxis);

    svtkMath::Cross(tAxis, this->Normal, sAxis);
    ...
    s = svtkMath::Dot(sAxis, axis);
    t = svtkMath::Dot(tAxis, axis);
    ...
    for (i = 0; i < numPts && !abort; i++)
    {
      ...
      tcoords[0] = this->SRange[0] + svtkMath::Dot(sAxis, axis) * sSf;
      tcoords[1] = this->TRange[0] + svtkMath::Dot(tAxis, axis) * tSf;
  ...
  else // use the axes specified
  {
    ...
    sDenom = svtkMath::Dot(sAxis, sAxis);
    tDenom = svtkMath::Dot(tAxis, tAxis);

    if (sDenom == 0.0 || tDenom == 0.0)
    {
      svtkErrorMacro("Bad plane definition");
      sDenom = tDenom = 1.0;
    }
...
#define SVTK_TOLERANCE 1.0e-03

void svtkTextureMapToPlane::ComputeNormal(svtkDataSet* output)
{
  svtkIdType numPts = output->GetNumberOfPoints();
  double m[9], v[3], x[3];
  svtkIdType ptId;
  int dir = 0, i;
  ...
  c2 = m + 3;
  c3 = m + 6;
  if ((det = svtkMath::Determinant3x3(c1, c2, c3)) <= SVTK_TOLERANCE)
  {
    return;
  }

  this->Normal[0] = svtkMath::Determinant3x3(v, c2, c3) / det;
  this->Normal[1] = svtkMath::Determinant3x3(c1, v, c3) / det;
  this->Normal[2] = -1.0; // because of the formulation
}

void svtkTextureMapToPlane::PrintSelf(ostream& os, svtkIndent indent)
{
