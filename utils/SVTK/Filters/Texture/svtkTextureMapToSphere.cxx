/*=========================================================================
  ...
  Module:    svtkTextureMapToSphere.cxx
...
int svtkTextureMapToSphere::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  if (!input || !output)
  {
    svtkErrorMacro(<< "Invalid input or output");
    return 1;
  }
  ...
  double diff, PiOverTwo = svtkMath::Pi() / 2.0;

  svtkDebugMacro(<< "Generating Spherical Texture Coordinates");
  ...
  for (ptId = 0; ptId < numPts; ptId++)
  {
    ...
    rho = sqrt((double)svtkMath::Distance2BetweenPoints(x, this->Center));
    if (rho != 0.0)
    {
...
void svtkTextureMapToSphere::ComputeCenter(svtkDataSet* dataSet)
{
  if (this->AutomaticSphereGeneration)
  {
    ...
    svtkDebugMacro(<< "Center computed as: (" << this->Center[0] << ", " << this->Center[1] << ", "
                  << this->Center[2] << ")");
  }
}

void svtkTextureMapToSphere::PrintSelf(ostream& os, svtkIndent indent)
{
