 ...
 * This grid will have to be an input of the SVTK filter. The two scalar fields, however,
 * are assumed to be stored in the SVTK unstructured grid, and will be specified using the
 * SetField1() and SetField2() accessors. The FSCP will be passed into the filter as a
 * second input port. The data type of FSCP is required to be a svtkPolyData, with each of
 ...
 *
 *     svtkSmartPointer<svtkFiberSurface> fiberSurface =
 *                            svtkSmartPointer<svtkFiberSurface>::New();
 ...
 * svtkPolyData containing the output fiber surfaces.
 */

#ifndef svtkFiberSurface_h
#define svtkFiberSurface_h

#include "svtkFiltersTopologyModule.h" // For export macro
#include "svtkPolyDataAlgorithm.h"

class SVTKFILTERSTOPOLOGY_EXPORT svtkFiberSurface : public svtkPolyDataAlgorithm
{
public:
  static svtkFiberSurface* New();
  svtkTypeMacro(svtkFiberSurface, svtkPolyDataAlgorithm);
...
private:
  svtkFiberSurface(const svtkFiberSurface&) = delete;
  void operator=(const svtkFiberSurface&) = delete;
};
#endif
