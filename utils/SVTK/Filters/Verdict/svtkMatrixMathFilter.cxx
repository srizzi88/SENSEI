...
=========================================================================*/
#include "svtkMatrixMathFilter.h"
...
svtkStandardNewMacro(svtkMatrixMathFilter);

svtkMatrixMathFilter::~svtkMatrixMathFilter() = default;

svtkMatrixMathFilter::svtkMatrixMathFilter()
{
  this->Operation = NONE;
  this->SetInputArrayToProcess(
    0, 0, 0, svtkDataObject::FIELD_ASSOCIATION_POINTS_THEN_CELLS, svtkDataSetAttributes::TENSORS);
}

void svtkMatrixMathFilter::PrintSelf(ostream& os, svtkIndent indent)
{
...
int svtkMatrixMathFilter::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  svtkDataArray* inTensors = this->GetInputArrayToProcess(0, inputVector, association);

  bool const pointQuality = svtkDataObject::FIELD_ASSOCIATION_POINTS == association;
  bool const cellQuality = svtkDataObject::FIELD_ASSOCIATION_CELLS == association;
  if (!pointQuality && !cellQuality)
  {
    svtkWarningMacro("Unknown association " << association);
    return 1;
  }

  svtkIdType const nCells = in->GetNumberOfCells();
  svtkIdType const nPoints = in->GetNumberOfPoints();
  if ((pointQuality && 0 == nPoints) || (cellQuality && 0 == nCells))
  {
    svtkWarningMacro("No data to work.");
    return 1;
  }
  ...
  switch (this->GetOperation())
  {
    ...
    case INVERSE:
      ...
      break;
    default:
      svtkWarningMacro("Bad Operation (" << this->GetOperation() << ")");
      return 1;
  }
  ...
  for (svtkIdType i = 0, n = (pointQuality ? nPoints : nCells); i < n; ++i)
  {
    ...
    switch (this->GetOperation())
    {
      case DETERMINANT:
      {
        ...
        double const q = svtkMath::Determinant3x3(reinterpret_cast<double(*)[3]>(tensor));
      ...
      case EIGENVECTOR:
      {
        ...
        if (inTensors->GetNumberOfComponents() == 6)
        {
          svtkMath::TensorFromSymmetricTensor(d);
        }
      ...
      case INVERSE:
      {
        ...
        // svtkMath::Invert3x3 should quite fit here, unfortunately, it does not
        ...
        // Compute the adjoint
        double d1 = svtkMath::Determinant2x2(b2, b3, c2, c3);
        double d2 = -svtkMath::Determinant2x2(a2, a3, c2, c3);
        double d3 = svtkMath::Determinant2x2(a2, a3, b2, b3);

        double e1 = -svtkMath::Determinant2x2(b1, b3, c1, c3);
        double e2 = svtkMath::Determinant2x2(a1, a3, c1, c3);
        double e3 = -svtkMath::Determinant2x2(a1, a3, b1, b3);

        double f1 = svtkMath::Determinant2x2(b1, b2, c1, c2);
        double f2 = -svtkMath::Determinant2x2(a1, a2, c1, c2);
        double f3 = svtkMath::Determinant2x2(a1, a2, b1, b2);

        // Divide by the determinant
        double det = a1 * d1 + b1 * d2 + c1 * d3;

        // Compute inverse only if the matrix is non-singular
        if (det < -SVTK_DBL_EPSILON || det > SVTK_DBL_EPSILON)
        {
          AI[0][0] = d1 / det;
          AI[1][0] = d2 / det;
          AI[2][0] = d3 / det;
