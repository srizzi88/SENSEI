/*=========================================================================

  Program:   Visualization Toolkit
  Module:    QSVTKOpenGLWindow.cxx

  Copyright (c) Ken Martin, Will Schroeder, Bill Lorensen
  All rights reserved.
  See Copyright.txt or http://www.kitware.com/Copyright.htm for details.

     This software is distributed WITHOUT ANY WARRANTY; without even
     the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR
     PURPOSE.  See the above copyright notice for more information.

=========================================================================*/
#include "QSVTKOpenGLWindow.h"

#include <QApplication>
#include <QDesktopWidget>
#include <QOpenGLContext>
#include <QOpenGLFramebufferObject>
#include <QOpenGLFunctions>
#include <QOpenGLFunctions_3_2_Core>
#include <QOpenGLTexture>
#include <QPointer>
#include <QScopedValueRollback>
#include <QtDebug>

#include "QSVTKInteractor.h"
#include "QSVTKInteractorAdapter.h"
#include "QSVTKRenderWindowAdapter.h"
#include "svtkCommand.h"
#include "svtkGenericOpenGLRenderWindow.h"
#include "svtkInteractorStyleTrackballCamera.h"
#include "svtkLogger.h"
#include "svtkNew.h"
#include "svtkObjectFactory.h"
#include "svtkOpenGLState.h"

QSVTKOpenGLWindow::QSVTKOpenGLWindow(QOpenGLWindow::UpdateBehavior ub, QWindow* p)
  : QSVTKOpenGLWindow(svtkSmartPointer<svtkGenericOpenGLRenderWindow>::New(), nullptr, ub, p)
{
}

QSVTKOpenGLWindow::QSVTKOpenGLWindow(
  QOpenGLContext* shareContext, QOpenGLWindow::UpdateBehavior ub, QWindow* p)
  : QSVTKOpenGLWindow(svtkSmartPointer<svtkGenericOpenGLRenderWindow>::New(), shareContext, ub, p)
{
}

QSVTKOpenGLWindow::QSVTKOpenGLWindow(
  svtkGenericOpenGLRenderWindow* rw, QOpenGLWindow::UpdateBehavior ub, QWindow* p)
  : QSVTKOpenGLWindow(rw, nullptr, ub, p)
{
}

QSVTKOpenGLWindow::QSVTKOpenGLWindow(svtkGenericOpenGLRenderWindow* renderWin,
  QOpenGLContext* shareContext, QOpenGLWindow::UpdateBehavior ub, QWindow* p)
  : Superclass(shareContext, ub, p)
  , RenderWindow(nullptr)
  , RenderWindowAdapter(nullptr)
  , EnableHiDPI(true)
  , UnscaledDPI(72)
  , DefaultCursor(QCursor(Qt::ArrowCursor))
{
  this->setRenderWindow(renderWin);
}

//-----------------------------------------------------------------------------
QSVTKOpenGLWindow::~QSVTKOpenGLWindow()
{
  this->makeCurrent();
  this->cleanupContext();
}

//-----------------------------------------------------------------------------
void QSVTKOpenGLWindow::setRenderWindow(svtkRenderWindow* win)
{
  svtkGenericOpenGLRenderWindow* gwin = svtkGenericOpenGLRenderWindow::SafeDownCast(win);
  if (gwin == nullptr && win != nullptr)
  {
    qDebug() << "QSVTKOpenGLWindow requires a `svtkGenericOpenGLRenderWindow`. `"
             << win->GetClassName() << "` is not supported.";
  }
  this->setRenderWindow(gwin);
}

//-----------------------------------------------------------------------------
void QSVTKOpenGLWindow::setRenderWindow(svtkGenericOpenGLRenderWindow* win)
{
  if (this->RenderWindow == win)
  {
    return;
  }

  // this will release all OpenGL resources associated with the old render
  // window, if any.
  if (this->RenderWindowAdapter)
  {
    this->makeCurrent();
    this->RenderWindowAdapter.reset(nullptr);
  }
  this->RenderWindow = win;
  if (this->RenderWindow)
  {
    this->RenderWindow->SetReadyForRendering(false);

    // if an interactor wasn't provided, we'll make one by default
    if (!this->RenderWindow->GetInteractor())
    {
      // create a default interactor
      svtkNew<QSVTKInteractor> iren;
      // iren->SetUseTDx(this->UseTDx);
      this->RenderWindow->SetInteractor(iren);
      iren->Initialize();

      // now set the default style
      svtkNew<svtkInteractorStyleTrackballCamera> style;
      iren->SetInteractorStyle(style);
    }

    if (this->isValid())
    {
      // this typically means that the render window is being changed after the
      // QSVTKOpenGLWindow has initialized itself in a previous update
      // pass, so we emulate the steps to ensure that the new svtkRenderWindow is
      // brought to the same state (minus the actual render).
      this->makeCurrent();
      this->initializeGL();
      this->updateSize();
    }
  }
}

//-----------------------------------------------------------------------------
svtkRenderWindow* QSVTKOpenGLWindow::renderWindow() const
{
  return this->RenderWindow;
}

//-----------------------------------------------------------------------------
QSVTKInteractor* QSVTKOpenGLWindow::interactor() const
{
  return this->RenderWindow ? QSVTKInteractor::SafeDownCast(this->RenderWindow->GetInteractor())
                            : nullptr;
}

//-----------------------------------------------------------------------------
QSurfaceFormat QSVTKOpenGLWindow::defaultFormat(bool stereo_capable)
{
  return QSVTKRenderWindowAdapter::defaultFormat(stereo_capable);
}

//-----------------------------------------------------------------------------
void QSVTKOpenGLWindow::setEnableHiDPI(bool enable)
{
  this->EnableHiDPI = enable;
  if (this->RenderWindowAdapter)
  {
    this->RenderWindowAdapter->setEnableHiDPI(enable);
  }
}

//-----------------------------------------------------------------------------
void QSVTKOpenGLWindow::setUnscaledDPI(int dpi)
{
  this->UnscaledDPI = dpi;
  if (this->RenderWindowAdapter)
  {
    this->RenderWindowAdapter->setUnscaledDPI(dpi);
  }
}

//-----------------------------------------------------------------------------
void QSVTKOpenGLWindow::setDefaultCursor(const QCursor& cursor)
{
  this->DefaultCursor = cursor;
  if (this->RenderWindowAdapter)
  {
    this->RenderWindowAdapter->setDefaultCursor(cursor);
  }
}

//-----------------------------------------------------------------------------
void QSVTKOpenGLWindow::initializeGL()
{
  this->Superclass::initializeGL();
  if (this->RenderWindow)
  {
    Q_ASSERT(this->RenderWindowAdapter.data() == nullptr);
    this->RenderWindowAdapter.reset(
      new QSVTKRenderWindowAdapter(this->context(), this->RenderWindow, this));
    this->RenderWindowAdapter->setDefaultCursor(this->defaultCursor());
    this->RenderWindowAdapter->setEnableHiDPI(this->EnableHiDPI);
    this->RenderWindowAdapter->setUnscaledDPI(this->UnscaledDPI);
  }
  this->connect(this->context(), SIGNAL(aboutToBeDestroyed()), SLOT(cleanupContext()),
    static_cast<Qt::ConnectionType>(Qt::UniqueConnection | Qt::DirectConnection));
}

//-----------------------------------------------------------------------------
void QSVTKOpenGLWindow::updateSize()
{
  if (this->RenderWindowAdapter)
  {
    this->RenderWindowAdapter->resize(this->width(), this->height());
  }
}

//-----------------------------------------------------------------------------
void QSVTKOpenGLWindow::resizeGL(int w, int h)
{
  svtkLogF(TRACE, "resizeGL(%d, %d)", w, h);
  this->Superclass::resizeGL(w, h);
  this->updateSize();
}

//-----------------------------------------------------------------------------
void QSVTKOpenGLWindow::paintGL()
{
  svtkLogF(TRACE, "paintGL");
  this->Superclass::paintGL();
  if (this->RenderWindow)
  {
    Q_ASSERT(this->RenderWindowAdapter);
    this->RenderWindowAdapter->paint();

    // If render was triggered by above calls, that may change the current context
    // due to things like progress events triggering updates on other widgets
    // (e.g. progress bar). Hence we need to make sure to call makeCurrent()
    // before proceeding with blit-ing.
    this->makeCurrent();

    QOpenGLFunctions_3_2_Core* f =
      QOpenGLContext::currentContext()->versionFunctions<QOpenGLFunctions_3_2_Core>();
    if (f)
    {
      const QSize deviceSize = this->size() * this->devicePixelRatioF();
      const auto fmt = this->context()->format();
      if (fmt.stereo() && this->RenderWindow->GetStereoRender() &&
        this->RenderWindow->GetStereoType() == SVTK_STEREO_CRYSTAL_EYES)
      {
        this->RenderWindowAdapter->blitLeftEye(
          this->defaultFramebufferObject(), GL_BACK_LEFT, QRect(QPoint(0, 0), deviceSize));
        this->RenderWindowAdapter->blitRightEye(
          this->defaultFramebufferObject(), GL_BACK_RIGHT, QRect(QPoint(0, 0), deviceSize));
      }
      else
      {
        this->RenderWindowAdapter->blit(
          this->defaultFramebufferObject(), GL_BACK, QRect(QPoint(0, 0), deviceSize));
      }
    }
  }
  else
  {
    // no render window set, just fill with white.
    QOpenGLFunctions* f = QOpenGLContext::currentContext()->functions();
    f->glClearColor(1.0f, 1.0f, 1.0f, 1.0f);
    f->glClear(GL_COLOR_BUFFER_BIT);
  }
}

//-----------------------------------------------------------------------------
void QSVTKOpenGLWindow::cleanupContext()
{
  this->RenderWindowAdapter.reset(nullptr);
}

//-----------------------------------------------------------------------------
bool QSVTKOpenGLWindow::event(QEvent* evt)
{
  // Forward event to the Widget containing this window. This is required
  // due to QTBUG-61836 that prevents the use of the flag
  // Qt::TransparentForMouseInput. This flag should indicate that this window
  // should not catch any event and let them pass through to the widget.
  // The containing widget should then forward back only the required events for
  // this window (such as mouse events and resize events).
  // Until this misbehavior is fixed, we have to handle forwarding of events.
  emit this->windowEvent(evt);

  if (this->RenderWindowAdapter)
  {
    this->RenderWindowAdapter->handleEvent(evt);
  }

  return this->Superclass::event(evt);
}

//-----------------------------------------------------------------------------
#if !defined(SVTK_LEGACY_REMOVE)
void QSVTKOpenGLWindow::SetRenderWindow(svtkRenderWindow* win)
{
  SVTK_LEGACY_REPLACED_BODY(
    QSVTKOpenGLWindow::SetRenderWindow, "SVTK 9.0", QSVTKOpenGLWindow::setRenderWindow);
  svtkGenericOpenGLRenderWindow* gwin = svtkGenericOpenGLRenderWindow::SafeDownCast(win);
  if (gwin == nullptr && win != nullptr)
  {
    qDebug() << "QSVTKOpenGLWindow requires a `svtkGenericOpenGLRenderWindow`. `"
             << win->GetClassName() << "` is not supported.";
  }
  this->setRenderWindow(gwin);
}
#endif

//-----------------------------------------------------------------------------
#if !defined(SVTK_LEGACY_REMOVE)
void QSVTKOpenGLWindow::SetRenderWindow(svtkGenericOpenGLRenderWindow* win)
{
  SVTK_LEGACY_REPLACED_BODY(
    QSVTKOpenGLWindow::SetRenderWindow, "SVTK 9.0", QSVTKOpenGLWindow::setRenderWindow);
  this->setRenderWindow(win);
}
#endif

//-----------------------------------------------------------------------------
#if !defined(SVTK_LEGACY_REMOVE)
svtkRenderWindow* QSVTKOpenGLWindow::GetRenderWindow()
{
  SVTK_LEGACY_REPLACED_BODY(
    QSVTKOpenGLWindow::GetRenderWindow, "SVTK 9.0", QSVTKOpenGLWindow::renderWindow);
  return this->renderWindow();
}
#endif

//-----------------------------------------------------------------------------
#if !defined(SVTK_LEGACY_REMOVE)
QSVTKInteractorAdapter* QSVTKOpenGLWindow::GetInteractorAdapter()
{
  SVTK_LEGACY_BODY(QSVTKOpenGLWindow::GetInteractorAdapter, "SVTK 9.0");
  return nullptr;
}
#endif

//-----------------------------------------------------------------------------
#if !defined(SVTK_LEGACY_REMOVE)
QSVTKInteractor* QSVTKOpenGLWindow::GetInteractor()
{
  SVTK_LEGACY_REPLACED_BODY(
    QSVTKOpenGLWindow::GetInteractor, "SVTK 9.0", QSVTKOpenGLWindow::interactor);
  return this->interactor();
}
#endif

//-----------------------------------------------------------------------------
#if !defined(SVTK_LEGACY_REMOVE)
void QSVTKOpenGLWindow::setQSVTKCursor(const QCursor& cursor)
{
  SVTK_LEGACY_REPLACED_BODY(QSVTKOpenGLWindow::setQSVTKCursor, "SVTK 9.0", QSVTKOpenGLWindow::setCursor);
  this->setCursor(cursor);
}
#endif

//-----------------------------------------------------------------------------
#if !defined(SVTK_LEGACY_REMOVE)
void QSVTKOpenGLWindow::setDefaultQSVTKCursor(const QCursor& cursor)
{
  SVTK_LEGACY_REPLACED_BODY(
    QSVTKOpenGLWindow::setDefaultQSVTKCursor, "SVTK 9.0", QSVTKOpenGLWindow::setDefaultCursor);
  this->setDefaultCursor(cursor);
}
#endif
