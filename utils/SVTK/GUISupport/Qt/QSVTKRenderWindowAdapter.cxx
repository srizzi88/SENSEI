/*=========================================================================
  ...
  Module:    QSVTKRenderWindowAdapter.cxx
...
=========================================================================*/
#include "QSVTKRenderWindowAdapter.h"
...
#define QSVTKInternalsDebugMacro(msg)                                                               \
  if (this->Logger)                                                                                \
  {                                                                                                \
    std::ostringstream str;                                                                        \
    str << "QSVTKRenderWindowAdapter(" << this << "): " << msg;                                     \
    cout << str.str() << endl;                                                                     \
    this->Logger->logMessage(                                                                      \
      QOpenGLDebugMessage::createApplicationMessage(QString(str.str().c_str())));                  \
  }

class QSVTKRenderWindowAdapter::QSVTKInternals
{
  bool needToRecreateFBO() const;
  void recreateFBO();
  void renderWindowEventHandler(svtkObject*, unsigned long eventid, void* callData);
...
public:
  QSVTKRenderWindowAdapter* Self;
  QSVTKInteractorAdapter InteractorAdapter;

  svtkSmartPointer<svtkGenericOpenGLRenderWindow> RenderWindow;
  std::vector<unsigned long> RenderWindowObserverIds;

  // This flag is used as an indicate that `QSVTKInteractorAdapter::render`
  // should request the svtkGenericOpenGLRenderWindow to render.
  // We need this to avoid re-rendering when the app directly triggers a render
  // by calling renderWindow->Render();
  bool DoSVTKRenderInPaintGL = true;

  bool InPaint = false;

  int UnscaledDPI = 72; // same default as svtkWindow::DPI
  ...
  QSVTKInternals(QOpenGLContext* cntxt, svtkGenericOpenGLRenderWindow* renWin,
    QObject* widgetOrWindow, QSVTKRenderWindowAdapter* self)
    ...
    , Surface(nullptr)
  {
    ...
    QSVTKInternalsDebugMacro("constructor");
    ...
    this->RenderWindowObserverIds.push_back(renWin->AddObserver(
      svtkCommand::WindowMakeCurrentEvent, this, &QSVTKInternals::renderWindowEventHandler));
    this->RenderWindowObserverIds.push_back(renWin->AddObserver(
      svtkCommand::WindowIsCurrentEvent, this, &QSVTKInternals::renderWindowEventHandler));
    this->RenderWindowObserverIds.push_back(renWin->AddObserver(
      svtkCommand::WindowFrameEvent, this, &QSVTKInternals::renderWindowEventHandler));
    this->RenderWindowObserverIds.push_back(
      renWin->AddObserver(svtkCommand::StartEvent, this, &QSVTKInternals::renderWindowEventHandler));
    this->RenderWindowObserverIds.push_back(
      renWin->AddObserver(svtkCommand::EndEvent, this, &QSVTKInternals::renderWindowEventHandler));
    this->RenderWindowObserverIds.push_back(renWin->AddObserver(
      svtkCommand::CursorChangedEvent, this, &QSVTKInternals::renderWindowEventHandler));

    // First and foremost, make sure svtkRenderWindow is not using offscreen
    ...
    if (this->RenderWindow->GetUseOffScreenBuffers())
    {
      svtkGenericWarningMacro(
        "disabling `UseOffScreenBuffers` since QSVTKRenderWindowAdapter already "
    ...
    // request SVTK to do a render.
    this->DoSVTKRenderInPaintGL = true;
  ...
  ~QSVTKInternals()
  {
    QSVTKInternalsDebugMacro("destructor");
  ...
  void activateBuffers()
  {
    ...
    QSVTKInternalsDebugMacro("activateBuffers");
    if (!this->FBO || this->needToRecreateFBO())
    {
      ...
      // that. The DoSVTKRenderInPaintGL in `frame` if the rendering result is
      // viewable.
      this->DoSVTKRenderInPaintGL = true;
    }
    else
  ...
  {
    QSVTKInternalsDebugMacro("resize (" << w << ", " << h << ")");
    svtkLogF(TRACE, "resize(%d, %d)", w, h);
    ...
    // since we've resize, we request a svtkRenderWindow::Render in `paintGL`
  ...
  void paint()
  {
    ...
    QSVTKInternalsDebugMacro("paint");
    QScopedValueRollback<bool> var(this->InPaint, true);
    if (this->DoSVTKRenderInPaintGL)
    {
      svtkLogScopeF(TRACE, "requesting render");
    ...
    this->DoSVTKRenderInPaintGL = false;
  }

  void frame()
  {
    QSVTKInternalsDebugMacro("frame");
    ...
    if (using_double_buffer && !swap_buffers)
    {
      ...
      svtkLogF(TRACE, "frame using_double_buffer=%d, swap_buffers=%d -- ignored",
        using_double_buffer, this->RenderWindow->GetSwapBuffers());
      return;
    }

    svtkLogF(TRACE, "frame using_double_buffer=%d, swap_buffers=%d", using_double_buffer,
  ...
  void setCursor(int svtk_cursor)
  {
    switch (svtk_cursor)
    {
...
//----------------------------------------------------------------------------
bool QSVTKRenderWindowAdapter::QSVTKInternals::needToRecreateFBO() const
{
  if (!this->FBO)
  {
    return true;
  ...
  {
    svtkLogF(TRACE, "%d != %d", sizes.size(), neededColorAttachments);
    return true;
  }
...
//----------------------------------------------------------------------------
void QSVTKRenderWindowAdapter::QSVTKInternals::recreateFBO()
{
  svtkLogF(TRACE, "recreateFBO");
  Q_ASSERT(this->Context && this->Surface);
  Q_ASSERT(this->isCurrent());
  this->FBO.reset(nullptr);
...
//-----------------------------------------------------------------------------
void QSVTKRenderWindowAdapter::QSVTKInternals::renderWindowEventHandler(
  svtkObject*, unsigned long eventid, void* callData)
{
  switch (eventid)
  {
    case svtkCommand::WindowMakeCurrentEvent:
      this->makeCurrent();
      break;

    case svtkCommand::WindowIsCurrentEvent:
    {
    ...
    case svtkCommand::WindowFrameEvent:
      this->frame();
      break;

    case svtkCommand::StartEvent:
      SVTK_FALLTHROUGH;
    case svtkCommand::StartPickEvent:
    ...
    case svtkCommand::CursorChangedEvent:
    {
...
//-----------------------------------------------------------------------------
void QSVTKRenderWindowAdapter::QSVTKInternals::updateDPI() const
{
...
//-----------------------------------------------------------------------------
QSVTKRenderWindowAdapter::QSVTKRenderWindowAdapter(
  QOpenGLContext* cntxt, svtkGenericOpenGLRenderWindow* renWin, QWidget* widget)
  : QSVTKRenderWindowAdapter(cntxt, renWin, static_cast<QObject*>(widget))
{
}

//-----------------------------------------------------------------------------
QSVTKRenderWindowAdapter::QSVTKRenderWindowAdapter(
  QOpenGLContext* cntxt, svtkGenericOpenGLRenderWindow* renWin, QWindow* window)
  : QSVTKRenderWindowAdapter(cntxt, renWin, static_cast<QObject*>(window))
{
}

//-----------------------------------------------------------------------------
QSVTKRenderWindowAdapter::QSVTKRenderWindowAdapter(
  QOpenGLContext* cntxt, svtkGenericOpenGLRenderWindow* renWin, QObject* widgetOrWindow)
  : Superclass(widgetOrWindow)
  , Internals(new QSVTKRenderWindowAdapter::QSVTKInternals(cntxt, renWin, widgetOrWindow, this))
...
//-----------------------------------------------------------------------------
void QSVTKRenderWindowAdapter::contextAboutToBeDestroyed()
{
  this->Internals.reset(nullptr);
}

//-----------------------------------------------------------------------------
void QSVTKRenderWindowAdapter::paint()
{
  if (this->Internals)
  {
...
//-----------------------------------------------------------------------------
void QSVTKRenderWindowAdapter::resize(int width, int height)
{
  if (this->Internals)
  {
...
//-----------------------------------------------------------------------------
bool QSVTKRenderWindowAdapter::blit(
...
//-----------------------------------------------------------------------------
bool QSVTKRenderWindowAdapter::handleEvent(QEvent* evt)
{
...
//-----------------------------------------------------------------------------
void QSVTKRenderWindowAdapter::setEnableHiDPI(bool value)
{
  if (this->Internals)
  {
...
//-----------------------------------------------------------------------------
void QSVTKRenderWindowAdapter::setUnscaledDPI(int unscaledDPI)
{
  if (this->Internals)
  {
...
//-----------------------------------------------------------------------------
QSurfaceFormat QSVTKRenderWindowAdapter::defaultFormat(bool stereo_capable)
{
  QSurfaceFormat fmt;
