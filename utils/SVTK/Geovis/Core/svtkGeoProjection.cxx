...
svtkStandardNewMacro(svtkGeoProjection);

static int svtkGeoProjectionNumProj = -1;

//-----------------------------------------------------------------------------
class svtkGeoProjection::svtkInternals
{
public:
...
//-----------------------------------------------------------------------------
int svtkGeoProjection::GetNumberOfProjections()
{
  if (svtkGeoProjectionNumProj < 0)
  {
    svtkGeoProjectionNumProj = 0;
    for (const PJ_LIST* pj = proj_list_operations(); pj && pj->id; ++pj)
      ++svtkGeoProjectionNumProj;
  }
  return svtkGeoProjectionNumProj;
}
//-----------------------------------------------------------------------------
const char* svtkGeoProjection::GetProjectionName(int projection)
{
  if (projection < 0 || projection >= svtkGeoProjection::GetNumberOfProjections())
    return nullptr;

  return proj_list_operations()[projection].id;
}
//-----------------------------------------------------------------------------
const char* svtkGeoProjection::GetProjectionDescription(int projection)
{
  if (projection < 0 || projection >= svtkGeoProjection::GetNumberOfProjections())
    return nullptr;
...
//-----------------------------------------------------------------------------
svtkGeoProjection::svtkGeoProjection()
{
  this->Name = nullptr;
...
//-----------------------------------------------------------------------------
void svtkGeoProjection::PrintSelf(ostream& os, svtkIndent indent)
{
...
//-----------------------------------------------------------------------------
int svtkGeoProjection::GetIndex()
{
  int i = 0;
...
//-----------------------------------------------------------------------------
const char* svtkGeoProjection::GetDescription()
{
...
//-----------------------------------------------------------------------------
projPJ svtkGeoProjection::GetProjection()
{
  this->UpdateProjection();
...
//-----------------------------------------------------------------------------
int svtkGeoProjection::UpdateProjection()
{
...
//-----------------------------------------------------------------------------
void svtkGeoProjection::SetOptionalParameter(const char* key, const char* value)
{
  ...
  {
    svtkErrorMacro("Invalid Optional Parameter Key/Value pair. None can be null");
  }
}

//-----------------------------------------------------------------------------
void svtkGeoProjection::RemoveOptionalParameter(const char* key)
{
  this->Internals->OptionalParameters.erase(key);
  this->Modified();
}
//-----------------------------------------------------------------------------
int svtkGeoProjection::GetNumberOfOptionalParameters()
{
  return static_cast<int>(this->Internals->OptionalParameters.size());
}
//-----------------------------------------------------------------------------
const char* svtkGeoProjection::GetOptionalParameterKey(int index)
{
  return this->Internals->GetKeyAt(index);
}
//-----------------------------------------------------------------------------
const char* svtkGeoProjection::GetOptionalParameterValue(int index)
{
  return this->Internals->GetValueAt(index);
}
//-----------------------------------------------------------------------------
void svtkGeoProjection::ClearOptionalParameters()
{
