/*=========================================================================
  ...
  Module:    svtkGDALRasterConverter.cxx
...
// Although svtkImageData and GDALDataset have different origin position,
...
svtkStandardNewMacro(svtkGDALRasterConverter);

//----------------------------------------------------------------------------
class svtkGDALRasterConverter::svtkGDALRasterConverterInternal
{
public:
  GDALDataType ToGDALDataType(int svtkDataType);

  template <typename SVTK_TYPE>
  void CopyToSVTK(GDALDataset* gdalData, svtkDataArray* svtkData, svtkUniformGrid* uniformGridData);
...
//----------------------------------------------------------------------------
// Translates svtk data type to GDAL data type
GDALDataType svtkGDALRasterConverter::svtkGDALRasterConverterInternal::ToGDALDataType(int svtkDataType)
{
  GDALDataType gdalType = GDT_Unknown;
  switch (svtkDataType)
  {
    case SVTK_TYPE_UINT8:
      gdalType = GDT_Byte;
      break;
    case SVTK_TYPE_INT16:
      gdalType = GDT_Int16;
      break;
    case SVTK_TYPE_UINT16:
      gdalType = GDT_UInt16;
      break;
    case SVTK_TYPE_INT32:
      gdalType = GDT_Int32;
      break;
    case SVTK_TYPE_UINT32:
      gdalType = GDT_UInt32;
      break;
    case SVTK_TYPE_FLOAT32:
      gdalType = GDT_Float32;
      break;
    case SVTK_TYPE_FLOAT64:
...
//----------------------------------------------------------------------------
// Copies contents of GDALDataset to svtkDataArray
template <typename SVTK_TYPE>
void svtkGDALRasterConverter::svtkGDALRasterConverterInternal::CopyToSVTK(
  GDALDataset* dataset, svtkDataArray* array, svtkUniformGrid* uniformGridData)
{
  ...
  // Copy from GDAL to svtkImageData, one band at a time
  SVTK_TYPE* buffer = new SVTK_TYPE[numElements];
  for (int i = 0; i < stride; i++)
  {
    ...
    if (band->GetColorInterpretation() == GCI_PaletteIndex)
    {
      ...
      svtkNew<svtkLookupTable> colorTable;
      ...
      for (int j = 0; j < numEntries; ++j)
      {
        ...
        if (categoryNames)
        {
          ...
          {
            colorTable->SetAnnotation(svtkVariant(j), categoryNames[j]);
          }
        }
        else
        {
          ...
          ss.clear();
          ss << "Category " << j;
          colorTable->SetAnnotation(svtkVariant(j), ss.str());
        }
      }
...
template <typename SVTK_TYPE>
void svtkGDALRasterConverter::svtkGDALRasterConverterInternal::FindDataRange(
  GDALRasterBand* band, double* minValue, double* maxValue)
{
  ...
  SVTK_TYPE* buffer = new SVTK_TYPE[xSize * ySize];
  GDALDataType gdalDataType = band->GetRasterDataType();
  CPLErr err =
    band->RasterIO(GF_Read, 0, 0, xSize, ySize, buffer, xSize, ySize, gdalDataType, 0, 0);
  if (err == CE_Failure)
  ...
  *minValue = SVTK_DOUBLE_MAX;
  *maxValue = SVTK_DOUBLE_MIN;
...
//----------------------------------------------------------------------------
// Copy svtkDataArray contents to GDAL raster bands
struct StaticCopyToGDAL
{
  ...
  void operator()(ArrayT* array, GDALDataset* gdalData)
  {
    ...
    svtkLookupTable* inputColorTable = array->GetLookupTable();
    if (inputColorTable)
    {
      gdalColorTable.reset(new GDALColorTable);
      svtkIdType tableSize = inputColorTable->GetNumberOfTableValues();
      double inputColor[4] = { 0.0, 0.0, 0.0, 1.0 };
      GDALColorEntry gdalColor;
      for (svtkIdType i = 0; i < tableSize; ++i)
      {
    ...
    for (int i = 0; i < stride; i++)
    {
      ...
      const auto tuples = svtk::DataArrayTupleRange(array);
      auto tupleIter = tuples.cbegin();

      // Copy data from svtk iterator to buffer
...
//----------------------------------------------------------------------------
svtkGDALRasterConverter::svtkGDALRasterConverter()
{
  this->Internal = new svtkGDALRasterConverterInternal();
...
//----------------------------------------------------------------------------
void svtkGDALRasterConverter::PrintSelf(ostream& os, svtkIndent indent)
{
  Superclass::PrintSelf(os, indent);
  os << "svtkGDALRasterConverter" << std::endl;
}

//----------------------------------------------------------------------------
// Copy image data contents, origin, & spacing to GDALDataset
bool svtkGDALRasterConverter::CopyToGDAL(svtkImageData* input, GDALDataset* output, int flipAxis[3])
{
  ...
  {
    svtkErrorMacro(<< "Image dimensions do not match.");
    return false;
  }
  ...
  int index = -1;
  svtkDataArray* array = input->GetFieldData()->GetArray("NO_DATA_VALUE", index);
  svtkDoubleArray* noDataArray = svtkDoubleArray::SafeDownCast(array);
  if (noDataArray)
  {
    for (int i = 0; i < array->GetNumberOfTuples(); i++)
    {
      double value = noDataArray->GetTuple1(i);
      if (!svtkMath::IsNan(value))
      {
...
//----------------------------------------------------------------------------
GDALDataset* svtkGDALRasterConverter::CreateGDALDataset(
  svtkImageData* imageData, const char* mapProjection, int flipAxis[3])
{
  int* dimensions = imageData->GetDimensions();
  svtkDataArray* array = imageData->GetCellData()->GetScalars();
  int svtkDataType = array->GetDataType();
  int rasterCount = array->GetNumberOfComponents();
  GDALDataset* dataset =
    this->CreateGDALDataset(dimensions[0] - 1, dimensions[1] - 1, svtkDataType, rasterCount);
...
//----------------------------------------------------------------------------
svtkUniformGrid* svtkGDALRasterConverter::CreateSVTKUniformGrid(GDALDataset* dataset)
{
  // Set svtk origin & spacing from GDALGeoTransform
  double geoTransform[6] = {};
  if (dataset->GetGeoTransform(geoTransform) != CE_None)
  {
    svtkErrorMacro(<< "Error calling GetGeoTransform()");
    return nullptr;
  }

  // Initialize image
  svtkUniformGrid* image = svtkUniformGrid::New();
  ...
  {
    case GDT_Byte:
      array = svtkDataArray::CreateDataArray(SVTK_TYPE_UINT8);
      this->Internal->CopyToSVTK<svtkTypeUInt8>(dataset, array, image);
      break;

    case GDT_UInt16:
      array = svtkDataArray::CreateDataArray(SVTK_TYPE_UINT16);
      this->Internal->CopyToSVTK<svtkTypeUInt16>(dataset, array, image);
      break;

    case GDT_Int16:
      array = svtkDataArray::CreateDataArray(SVTK_TYPE_INT16);
      this->Internal->CopyToSVTK<svtkTypeInt16>(dataset, array, image);
      break;

    case GDT_UInt32:
      array = svtkDataArray::CreateDataArray(SVTK_TYPE_UINT32);
      this->Internal->CopyToSVTK<svtkTypeUInt32>(dataset, array, image);
      break;

    case GDT_Int32:
      array = svtkDataArray::CreateDataArray(SVTK_TYPE_INT32);
      this->Internal->CopyToSVTK<svtkTypeInt32>(dataset, array, image);
      break;

    case GDT_Float32:
      array = svtkDataArray::CreateDataArray(SVTK_TYPE_FLOAT32);
      this->Internal->CopyToSVTK<svtkTypeFloat32>(dataset, array, image);
      break;

    case GDT_Float64:
      array = svtkDataArray::CreateDataArray(SVTK_TYPE_FLOAT64);
      this->Internal->CopyToSVTK<svtkTypeFloat64>(dataset, array, image);
      break;
    default:
      break;
  }
...
//----------------------------------------------------------------------------
GDALDataset* svtkGDALRasterConverter::CreateGDALDataset(
  int xDim, int yDim, int svtkDataType, int numberOfBands)
{
  GDALDriver* driver = GetGDALDriverManager()->GetDriverByName("MEM");
  GDALDataType gdalType = this->Internal->ToGDALDataType(svtkDataType);
  GDALDataset* dataset = driver->Create("", xDim, yDim, numberOfBands, gdalType, nullptr);
  return dataset;
}

//----------------------------------------------------------------------------
void svtkGDALRasterConverter::CopyBandInfo(GDALDataset* src, GDALDataset* dest)
{
...
//----------------------------------------------------------------------------
void svtkGDALRasterConverter::SetGDALProjection(GDALDataset* dataset, const char* projectionString)
{
...
//----------------------------------------------------------------------------
void svtkGDALRasterConverter::SetGDALGeoTransform(
...
//----------------------------------------------------------------------------
void svtkGDALRasterConverter::CopyNoDataValues(GDALDataset* src, GDALDataset* dst)
{
  ...
  {
    svtkWarningMacro("raster count different between src & dst datasets");
    return;
  }
  ...
  double noDataValue = svtkMath::Nan();
  int success = -1;
  for (int i = 0; i < numSrcBands; i++)
  {
    int index = i + 1;
...
//----------------------------------------------------------------------------
void svtkGDALRasterConverter::WriteTifFile(GDALDataset* dataset, const char* filename)
{
  const char* fmt = "GTiff";
  GDALDriver* driver = GetGDALDriverManager()->GetDriverByName(fmt);
  if (driver == nullptr)
  {
    svtkErrorMacro(<< "Cannot write GTiff file. GDALDriver is a nullptr");
...
//----------------------------------------------------------------------------
bool svtkGDALRasterConverter::FindDataRange(
  GDALDataset* dataset, int bandId, double* minValue, double* maxValue)
{
  ...
  {
    case GDT_Byte:
      this->Internal->FindDataRange<svtkTypeUInt8>(band, minValue, maxValue);
      break;

    case GDT_Int16:
      this->Internal->FindDataRange<svtkTypeInt16>(band, minValue, maxValue);
      break;

    case GDT_UInt16:
      this->Internal->FindDataRange<svtkTypeUInt16>(band, minValue, maxValue);
      break;

    case GDT_UInt32:
      this->Internal->FindDataRange<svtkTypeUInt32>(band, minValue, maxValue);
      break;

    case GDT_Int32:
      this->Internal->FindDataRange<svtkTypeInt32>(band, minValue, maxValue);
      break;

    case GDT_Float32:
      this->Internal->FindDataRange<svtkTypeFloat32>(band, minValue, maxValue);
      break;

    case GDT_Float64:
      this->Internal->FindDataRange<svtkTypeFloat64>(band, minValue, maxValue);
      break;
    default:
      break;
  }
