/*=========================================================================
  ...
  Module:    svtkGDALRasterReprojection.cxx
...
svtkStandardNewMacro(svtkGDALRasterReprojection);
//----------------------------------------------------------------------------
svtkGDALRasterReprojection::svtkGDALRasterReprojection()
{
  this->MaxError = 0.0;
...
//----------------------------------------------------------------------------
void svtkGDALRasterReprojection::PrintSelf(ostream& os, svtkIndent indent)
{
  Superclass::PrintSelf(os, indent);
  os << "svtkGDALRasterReprojection" << std::endl
...
//----------------------------------------------------------------------------
bool svtkGDALRasterReprojection::SuggestOutputDimensions(GDALDataset* dataset,
  const char* projection, double geoTransform[6], int* nPixels, int* nLines, double maxError)
{
  ...
  {
    svtkWarningMacro(<< "OGRSpatialReference::SetFromUserInput(" << projection << ") returned "
  ...
  if (errcode != OGRERR_NONE)
  {
    svtkWarningMacro(<< "OGRSpatialReference::exportToWKT("
  ...
  {
    svtkErrorMacro(<< "SuggestOutputDimensions: " << CPLGetLastErrorMsg());
  }
  ...
  if (err == CE_Failure)
  {
    svtkErrorMacro(<< "GDALSuggestedWarpOutput failed with message: " << CPLGetLastErrorMsg());
  }
...
//----------------------------------------------------------------------------
bool svtkGDALRasterReprojection::Reproject(GDALDataset* input, GDALDataset* output)
{
  ...
  if (err == CE_Failure)
  {
    svtkErrorMacro(<< "GDALReprojectImage failed with message: " << CPLGetLastErrorMsg());
    return false;
  }
