...
// Pipeline
class TesterVTU3D : public svtkAlgorithm
{
public:
  static TesterVTU3D* New();
  svtkTypeMacro(TesterVTU3D, svtkAlgorithm);
  TesterVTU3D()
  {
...
private:
  ...
  bool DoCheckData(svtkMultiBlockDataSet* multiBlock)
  {
    ...
    svtkUnstructuredGrid* unstructuredGrid =
      svtkUnstructuredGrid::SafeDownCast(multiPiece->GetPiece(0));
...
svtkStandardNewMacro(TesterVTU3D);
  ...
  const std::string& engineType)
{
    ...
    {
      const std::string svtkFileName = svtksys::SystemTools::FileIsDirectory(fileName)
        ? fileName + "/svtk.xml"
        : fileName + ".dir/svtk.xml";

      svtksys::ofstream fxml(svtkFileName, svtksys::ofstream::out);
...
int TestIOADIOS2VTX_VTU3D(int argc, char* argv[])
{
  auto lf_DoTest = [&](const std::string& fileName, const size_t steps) {
    ...
    svtkNew<TesterVTU3D> testerVTU3D;
  ...
  svtksys::SystemTools::MakeDirectory("bp3");
  svtksys::SystemTools::MakeDirectory("bp4");
