  ...
  const pugi::xml_node& dataSetNode, const std::set<std::string>& specialNames)
{
  ...
  for (const pugi::xml_node& dataArrayNode : dataSetNode)
  {
    const pugi::xml_attribute xmlName = XMLAttribute(
      "Name", dataArrayNode, true, "when parsing Name attribute in ADIOS2 SVTK XML schema", true);
    ...
    if (isSpecialName)
    {
      ...
      else if (specialName == "vertices")
      {
        ...
        const pugi::xml_attribute xmlOrder = XMLAttribute("Ordering", dataArrayNode, true,
          "when parsing vertices \"Order\" attribute in ADIOS2 SVTK XML schema", false);
    ...
    for (const pugi::xml_node& componentNode : dataArrayNode)
    {
      if (componentNode.type() != pugi::node_pcdata)
      {
        throw std::runtime_error("ERROR: NumberOfComponents attribute found, but "
          ...
          " is not of plain data type in ADIOS2 SVTK XML schema\n");
      }
      // TRIM
    ...
    if (xmlNumberOfComponents)
    {
      ...
      if (dataArray.VectorVariables.size() != components)
      {
        throw std::runtime_error("ERROR: NumberOfComponents " + std::to_string(components) +
          ...
          " inside DataArray node " + std::string(xmlName.name()) + " in ADIOS2 SVTK XML schema");
      }
    }
    ...
    if (dataArray.IsScalar() && (name == "TIME" || name == "CYCLE"))
    {
      throw std::invalid_argument("ERROR: data array " + name +
        " expected to have a least one component, in ADIOS2 SVTK XML "
        "schema\n");
    }
  }
...
{
  svtksys::ifstream file(fileName.c_str());
...
// allowed types
template svtkSmartPointer<svtkDataArray> NewDataArray<int>();
template svtkSmartPointer<svtkDataArray> NewDataArray<unsigned int>();
template svtkSmartPointer<svtkDataArray> NewDataArray<long int>();
template svtkSmartPointer<svtkDataArray> NewDataArray<unsigned long int>();
template svtkSmartPointer<svtkDataArray> NewDataArray<long long int>();
template svtkSmartPointer<svtkDataArray> NewDataArray<unsigned long long int>();
template svtkSmartPointer<svtkDataArray> NewDataArray<float>();
template svtkSmartPointer<svtkDataArray> NewDataArray<double>();
...
svtkSmartPointer<svtkIdTypeArray> NewDataArrayIdType()
{
  return svtkSmartPointer<svtkIdTypeArray>::New();
}
...
{
  const std::string engineType = svtksys::SystemTools::FileIsDirectory(fileName) ? "BP4" : "BP3";
  return engineType;
}
