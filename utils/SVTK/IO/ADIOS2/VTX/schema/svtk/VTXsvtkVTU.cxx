...
#include "VTXsvtkVTU.h"
#include "VTXsvtkVTU.txx"
...
VTXsvtkVTU::VTXsvtkVTU(const std::string& schema, adios2::IO& io, adios2::Engine& engine)
  : VTXsvtkBase("vtu", schema, io, engine)
{
  Init();
  InitTimes();
}

VTXsvtkVTU::~VTXsvtkVTU() {}

// PRIVATE
void VTXsvtkVTU::DoFill(svtkMultiBlockDataSet* multiBlock, const size_t step)
{
...
void VTXsvtkVTU::ReadPiece(const size_t step, const size_t pieceID)
{
  if (!ReadDataSets(types::DataSetType::Cells, step, pieceID))
  {
    throw std::invalid_argument("ERROR: VTU UnstructuredGrid data model requires Cells "
                                "information, in SVTK::IOADIOS2 VTX reader\n");
  }
  ...
  // Set Grid
  // Associate Points
  std::vector<svtkIdType> nodeSizes;
  {
  ...
  // Associate Cells
  {
    ...
    if (itConnectivity == dataSet.end())
    {
      throw std::invalid_argument("ERROR: VTU UnstructuredGrid data model requires the variable "
                                  "connectivity, in SVTK::IOADIOS2 VTX reader");
    }

    types::DataArray& connectivity = itConnectivity->second;
    if (connectivity.IsUpdated)
    {
      svtkIdTypeArray* iconnectivity = svtkIdTypeArray::SafeDownCast(connectivity.Data.GetPointer());
      svtkIdType* pconn = iconnectivity->GetPointer(0);

      // increase the connectivity offsets to match the local block point id
      svtkIdType blockOffset = 0;
      ...
      for (const auto& blockPair : connectivity.BlockCounts)
      {
        ...
        {
          const svtkIdType nPoints = pconn[linearOffset];
      ...
      svtkIdType size = connectivity.Data->GetSize();
      ...
      {
        throw std::invalid_argument("ERROR: VTU UnstructuredGrid data model requires the variable "
                                    "types, in SVTK::IOADIOS2 VTX reader");
      }
      ...
      if (types.Data->GetSize() == 1)
      {
        ...
        if (types.Data->GetDataType() == SVTK_UNSIGNED_INT)
        {
          svtkUnsignedIntArray* itypes = svtkUnsignedIntArray::SafeDownCast(types.Data.GetPointer());
          type = static_cast<int>(itypes->GetValue(0));
        }
        else if (types.Data->GetDataType() == SVTK_INT)
        {
          svtkIntArray* itypes = svtkIntArray::SafeDownCast(types.Data.GetPointer());
...
void VTXsvtkVTU::Init()
{
  ...
  const pugi::xml_node xmlSVTKFileNode = vtx::helper::XMLNode(
    "SVTKFile", xmlDocument, true, "when reading SVTKFile type=UnstructuredGrid node", true, true);

  const pugi::xml_node xmlUnstructuredGridNode = vtx::helper::XMLNode(
    "UnstructuredGrid", xmlSVTKFileNode, true, "when reading UnstructuredGrid node", true, true);
