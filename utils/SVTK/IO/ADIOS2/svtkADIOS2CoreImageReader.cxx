...
#include "Core/svtkADIOS2CoreTypeTraits.h"
...
#include "svtkMultiProcessController.h" // For the MPI controller member
#endif
...
//----------------------------------------------------------------------------
svtkStandardNewMacro(svtkADIOS2CoreImageReader);
namespace
{
...
//----------------------------------------------------------------------------
struct svtkADIOS2CoreImageReader::svtkADIOS2CoreImageReaderImpl
{

  svtkNew<svtkMultiPieceDataSet> Flatten(svtkMultiBlockDataSet* ibds);
  ...
  svtkADIOS2ArraySelection ArraySelection;
  // For ParaView GUI display usage.
  svtkNew<svtkStringArray> AvailableArray;
};

svtkNew<svtkMultiPieceDataSet> svtkADIOS2CoreImageReader::svtkADIOS2CoreImageReaderImpl::Flatten(
  svtkMultiBlockDataSet* ibds)
{
...
#ifdef IOADIOS2_HAVE_MPI
  auto ctrl = svtkMultiProcessController::GetGlobalController();
  if (ctrl)
  {
  ...
  svtkNew<svtkMultiPieceDataSet> mpds;
  mpds->SetNumberOfPieces(total);
  for (svtkDataObject* obj : svtk::Range(ibds, Opts::VisitOnlyLeaves))
  {
...
//----------------------------------------------------------------------------
void svtkADIOS2CoreImageReader::PrintSelf(ostream& os, svtkIndent indent)
{
  this->Superclass::PrintSelf(os, indent);
}

//----------------------------------------------------------------------------
int svtkADIOS2CoreImageReader::CanReadFile(const std::string& name)
{
  if (!svtksys::SystemTools::FileExists(name))
  {
    return 0;
  }
...
//----------------------------------------------------------------------------
int svtkADIOS2CoreImageReader::CanReadFile(const char* fileName)
{
  return this->CanReadFile(std::string(fileName));
}

//----------------------------------------------------------------------------
void svtkADIOS2CoreImageReader::SetFileName(const char* fileName)
{
  this->FileName = std::string(fileName);
}

//----------------------------------------------------------------------------
int svtkADIOS2CoreImageReader::GetNumberOfArrays()
{
  return this->Impl->ArraySelection.GetNumberOfArrays();
}

//----------------------------------------------------------------------------
const char* svtkADIOS2CoreImageReader::GetArrayName(int index)
{
  return this->Impl->ArraySelection.GetArrayName(index);
}

//----------------------------------------------------------------------------
void svtkADIOS2CoreImageReader::SetArrayStatus(const char* name, int status)
{
  this->Impl->ArraySelection.SetArrayStatus(name, status);
}

//----------------------------------------------------------------------------
int svtkADIOS2CoreImageReader::GetArrayStatus(const char* name)
{
  return this->Impl->ArraySelection.GetArrayStatus(name);
}

//----------------------------------------------------------------------------
svtkStringArray* svtkADIOS2CoreImageReader::GetAllDimensionArrays()
{
  return this->Impl->AvailableArray;
}

//----------------------------------------------------------------------------
svtkStringArray* svtkADIOS2CoreImageReader::GetAllTimeStepArrays()
{
  return this->Impl->AvailableArray;
}

//----------------------------------------------------------------------------
void svtkADIOS2CoreImageReader::SetActiveScalar(const std::pair<std::string, VarType>& as)
{
  this->Impl->ActiveScalar = as;
}

//----------------------------------------------------------------------------
std::pair<std::string, svtkADIOS2CoreImageReader::VarType>&
svtkADIOS2CoreImageReader::GetActiveScalar()
{
  return this->Impl->ActiveScalar;
}

//----------------------------------------------------------------------------
const std::pair<std::string, svtkADIOS2CoreImageReader::VarType>&
svtkADIOS2CoreImageReader::GetActiveScalar() const
{
  return this->Impl->ActiveScalar;
}

//----------------------------------------------------------------------------
svtkADIOS2CoreImageReader::StringToParams& svtkADIOS2CoreImageReader::GetAvilableVariables()
{
  return this->Impl->AvailVars;
}

//----------------------------------------------------------------------------
const svtkADIOS2CoreImageReader::StringToParams& svtkADIOS2CoreImageReader::GetAvilableVariables()
  const
{
  return this->Impl->AvailVars;
}
...
//----------------------------------------------------------------------------
svtkADIOS2CoreImageReader::StringToParams& svtkADIOS2CoreImageReader::GetAvailableAttributes()
{
  return this->Impl->AvailAtts;
}

//----------------------------------------------------------------------------
const svtkADIOS2CoreImageReader::StringToParams& svtkADIOS2CoreImageReader::GetAvailableAttributes()
  const
{
  return this->Impl->AvailAtts;
}
...
//----------------------------------------------------------------------------
void svtkADIOS2CoreImageReader::SetController(svtkMultiProcessController* controller)
{
#ifdef IOADIOS2_HAVE_MPI
  ...
  {
    svtkErrorMacro(
      "svtkADIOS2CoreImageReader is built with MPI but an invalid MPI controller is provided");
    return;
  }
#endif
...
//----------------------------------------------------------------------------
int svtkADIOS2CoreImageReader::ProcessRequest(
...
//----------------------------------------------------------------------------
int svtkADIOS2CoreImageReader::RequestDataObjectInternal(svtkInformationVector* outputVector)
{
  svtkSmartPointer<svtkDataObject> output = svtkDataObject::GetData(outputVector, 0);
  if (!output)
  {
    output = svtk::TakeSmartPointer(svtkDataObjectTypes::NewDataObject(SVTK_MULTIBLOCK_DATA_SET));
    outputVector->GetInformationObject(0)->Set(svtkDataObject::DATA_OBJECT(), output);
    this->GetOutputInformation(0)->Set(svtkDataObject::DATA_EXTENT_TYPE(), output->GetExtentType());
  }
  return 1;
}

//----------------------------------------------------------------------------
bool svtkADIOS2CoreImageReader::OpenAndReadMetaData()
{
  // Is name valid
  ...
  if (!this->Impl->Adios)
  {
    ...
    if (!this->Controller)
    {
      svtkErrorMacro("The reader is built with MPI support but the application is not launched in "
                    "parallel mode. Abort reading.");
      return false;
    }
    svtkMPICommunicator* comm =
      static_cast<svtkMPICommunicator*>(this->Controller->GetCommunicator());
  ...
  try
  {
    this->Impl->AdiosIO = this->Impl->Adios->DeclareIO("svtkADIOS2ImageRead");
    ...
    this->Impl->AvailableArray->Allocate(static_cast<svtkIdType>(this->Impl->AvailVars.size()));
  ...
  {
    svtkErrorMacro("failed to open and read meta data: " << ex.what());
    return false;
  }
...
//----------------------------------------------------------------------------
svtkADIOS2CoreImageReader::svtkADIOS2CoreImageReader()
  : DimensionArrayAsCell(true)
  , IsColumnMajor(false)
  , Impl(new svtkADIOS2CoreImageReaderImpl)
{
...
//----------------------------------------------------------------------------
int svtkADIOS2CoreImageReader::RequestInformation(
  svtkInformation* request, svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  if (!this->Impl->HasReadMetaData && !this->OpenAndReadMetaData())
  {
    ...
    svtkErrorMacro("unable to open file and data");
    return 0;
  }

  if (!this->Impl->AvailVars.size())
  {
    svtkErrorMacro("No variables can be inquired in the provided file. Abort reading");
    return 0;
  }
  ...
  if (!this->TimeStepArray.empty() && this->GatherTimeSteps())
  {
    ...
    outInfo->Set(svtkStreamingDemandDrivenPipeline::TIME_STEPS(), this->Impl->TimeSteps.data(),
...
//----------------------------------------------------------------------------
int svtkADIOS2CoreImageReader::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** svtkNotUsed(inputVector), svtkInformationVector* outputVector)
{
  ...
  {
    this->Impl->Adios.reset(nullptr);
    svtkErrorMacro("No inquire variable is specified. Abort reading now");
    return 0;
  }
  ...
  {
    this->Impl->Adios.reset(nullptr);
    svtkErrorMacro("An invalid time step array name is specified. Abort reading now");
    return 0;
  }
  ...
  if (!this->TimeStepArray.empty())
  {
    this->RequestTimeStep = outInfo->Get(svtkStreamingDemandDrivenPipeline::UPDATE_TIME_STEP());
    if (!this->Impl->TimeStepsReverseMap.count(this->RequestTimeStep))
    {
      svtkErrorMacro("The requested time step " << this->RequestTimeStep << " is not avaible!");
      return 0;
    }
  ...
  {
    this->Impl->Adios.reset(nullptr);
    svtkErrorMacro("unable to initialize work distribution");
    return 0;
  }
  ...
  svtkSmartPointer<svtkMultiBlockDataSet> rootMB = svtkMultiBlockDataSet::GetData(outInfo);
  svtkNew<svtkMultiPieceDataSet> mpds = this->Impl->Flatten(mbds);
  rootMB->SetBlock(0, mpds);
  if (!this->TimeStepArray.empty())
  {
    rootMB->GetInformation()->Set(svtkDataObject::DATA_TIME_STEP(), this->RequestTimeStep);
  }
...
//----------------------------------------------------------------------------
bool svtkADIOS2CoreImageReader::InitWorkDistribution()
{
  try
  {
    ...
    if (typeStr.empty())
    {
      svtkErrorMacro("Cannot find a type for " << varName << " invalid name is provided");
      return 1;
    }
  ...
  {
    svtkErrorMacro("failed to initialize work distribution: " << e.what());
    return false;
  }
  return true;
}
...
//----------------------------------------------------------------------------
std::string svtkADIOS2CoreImageReader::FetchTypeStringFromVarName(const std::string& name)
{
...
//----------------------------------------------------------------------------
void svtkADIOS2CoreImageReader::UpdateDimensionFromDimensionArray()
{
  ...
  {
    svtkErrorMacro("Can not use the dimension of array "
      << this->DimensionArray
...
//----------------------------------------------------------------------------
void svtkADIOS2CoreImageReader::ConvertArraySelectionToInqVar()
{
  ...
  for (auto& iter : this->Impl->ArraySelection)
  {
    if (iter.second) // Enabled by the user
    {
      ...
      {
        svtkWarningMacro("The dimension of array " << arrayName << " is not supported. Skipping");
      }
    }
  }
...
//----------------------------------------------------------------------------
void svtkADIOS2CoreImageReader::ReadImageBlocks(svtkMultiBlockDataSet* mbds)
{
  try
  {
    // One adios block is mapped to one svtk image data.
    size_t blockExtentI{ 0 };
    for (size_t blockI = this->Impl->BlockStart;
         blockI < this->Impl->BlockStart + this->Impl->BlockCount; blockI++)
    {
      svtkNew<svtkImageData> outputImage;
      ...
      for (const auto& iter : this->Impl->InquiredVars)
      {
        ...
        {
          svtkErrorMacro("Inquire variable " << varName << " cannot be found in the provided file");
          continue;
        }
        ...
        svtkSmartPointer<svtkAbstractArray> dataArray;
        // Use the adios_types_map in adios2 code base to generate types
        if (typeStr == "string")
        { // svtkStringArray uses svtkStdString instead of std::string. So we manually
          // do the work here
          ...
          svtkNew<svtkStringArray> array;
          dataArray = array;
          array->SetNumberOfComponents(1);
          array->SetName(varName.c_str());
          array->SetNumberOfTuples(static_cast<svtkIdType>(varADIOS2.SelectionSize()));
          this->Impl->BpReader.Get(varADIOS2, dynamic_cast<std::string*>(array->GetPointer(0)));
        }
        else if (typeStr == "char")
        {
          dataArray = this->PopulateDataArrayFromVar<char, NativeToSVTKType>(varName, blockI);
        }
        else if (typeStr == "int8_t")
        {
          dataArray = this->PopulateDataArrayFromVar<int8_t, NativeToSVTKType>(varName, blockI);
        }
        else if (typeStr == "uint8_t")
        {
          dataArray = this->PopulateDataArrayFromVar<uint8_t, NativeToSVTKType>(varName, blockI);
        }
        else if (typeStr == "int16_t")
        {
          dataArray = this->PopulateDataArrayFromVar<int16_t, NativeToSVTKType>(varName, blockI);
        }
        else if (typeStr == "uint16_t")
        {
          dataArray = this->PopulateDataArrayFromVar<uint16_t, NativeToSVTKType>(varName, blockI);
        }
        else if (typeStr == "int32_t")
        {
          dataArray = this->PopulateDataArrayFromVar<int32_t, NativeToSVTKType>(varName, blockI);
        }
        else if (typeStr == "uint32_t")
        {
          dataArray = this->PopulateDataArrayFromVar<uint32_t, NativeToSVTKType>(varName, blockI);
        }
        else if (typeStr == "int64_t")
        {
          dataArray = this->PopulateDataArrayFromVar<int64_t, NativeToSVTKType>(varName, blockI);
        }
        else if (typeStr == "uint64_t")
        {
          dataArray = this->PopulateDataArrayFromVar<uint64_t, NativeToSVTKType>(varName, blockI);
        }
        else if (typeStr == "float")
        {
          dataArray = this->PopulateDataArrayFromVar<float, NativeToSVTKType>(varName, blockI);
        }
        else if (typeStr == "double")
        {
          dataArray = this->PopulateDataArrayFromVar<double, NativeToSVTKType>(varName, blockI);
        }
        else if (typeStr == "long double")
        {
          svtkWarningMacro(<< "ADIOS2 type long double is not supported yet. Skipping array "
                          << varName);
          continue;
        }
        else if (typeStr == "float complex")
        {
          svtkWarningMacro(<< "ADIOS2 type float complex is not supported yet. Skipping array "
                          << varName);
          continue;
        }
        else if (typeStr == "double complex")
        {
          svtkWarningMacro(<< "ADIOS2 type double complex is not supported yet. Skipping array "
                          << varName);
          continue;
        }
        ...
        {
          if (this->Impl->ActiveScalar.second == svtkADIOS2CoreImageReader::VarType::CellData)
          {
...
//----------------------------------------------------------------------------
bool svtkADIOS2CoreImageReader::GatherTimeSteps()
{
  ...
  {
    svtkErrorMacro("Type " << typeStr << " is not supported yet as a time array type in SVTK");
    return false;
  }
  return true;
}
...
//----------------------------------------------------------------------------
template <typename T>
void svtkADIOS2CoreImageReader::CalculateWorkDistribution(const std::string& varName)
{
...
svtkSmartPointer<svtkAbstractArray> svtkADIOS2CoreImageReader::PopulateDataArrayFromVar(
  const std::string& varName, size_t blockIndex)
{
  svtkSmartPointer<svtkAbstractArray> array = svtkDataArray::CreateDataArray(U<T>::SVTKType);
  try
  {
  ...
  {
    svtkErrorMacro("Fail to populate data array from variable " << varName << ex.what());
  }
  return array;
}
...
//----------------------------------------------------------------------------
template <typename T>
void svtkADIOS2CoreImageReader::GatherTimeStepsFromADIOSTimeArray()
{
  try
  {
  ...
  {
    svtkErrorMacro("Fail to gather time steps from time array " << this->TimeStepArray << ex.what());
  }
}
