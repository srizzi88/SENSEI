...
int Validate(svtkMultiBlockDataSet* mb)
{
  ensure(mb != nullptr, "expecting svtkMultiBlockDataSet.");
  ensure(mb->GetNumberOfBlocks() == 1, "expecting num-blocks == num-levels == 1");

  auto mp = svtkMultiPieceDataSet::SafeDownCast(mb->GetBlock(0));
  ensure(mp != nullptr, "expecting level is maintained in a svtkMultiPieceDataSet.");
  ensure(mp->GetNumberOfPieces() == 8, "expecting 8 datasets in level 0");
  for (int cc = 0; cc < 8; ++cc)
  {
    auto pd = svtkPolyData::SafeDownCast(mp->GetPiece(cc));
...
int TestAMReXParticlesReader(int argc, char* argv[])
{
  // Test 3D
  {
    char* fname = svtkTestUtilities::ExpandDataFileName(argc, argv, "Data/AMReX/MFIX-Exa/plt00000");
    svtkNew<svtkAMReXParticlesReader> reader;
  ...
  // Test 2D
  {
    char* fname = svtkTestUtilities::ExpandDataFileName(argc, argv, "Data/AMReX/Sample2D/plt00100");
    svtkNew<svtkAMReXParticlesReader> reader;
    reader->SetPlotFileName(fname);
    delete[] fname;
