/*=========================================================================
  ...
  Module:    svtkAMREnzoReaderInternal.cxx
...
#include "svtk_hdf5.h" // for the HDF5 library
...
{
  return (svtksys::SystemTools::GetFilenameName(std::string(path)));
}
...
//------------------------------------------------------------------------------
void svtkEnzoReaderBlock::DeepCopy(const svtkEnzoReaderBlock* other)
{
...
void svtkEnzoReaderBlock::GetParentWiseIds(std::vector<svtkEnzoReaderBlock>& blocks)
{
  if (this->ParentId != 0)
  {
    ...
    // beginning position
    svtkEnzoReaderBlock& parent = blocks[this->ParentId];
  ...
  else
  {
    ...
    svtkEnzoReaderBlock& block0 = blocks[0];
...
//------------------------------------------------------------------------------
void svtkEnzoReaderBlock::GetLevelBasedIds(std::vector<svtkEnzoReaderBlock>& blocks)
{
...
svtkEnzoReaderInternal::svtkEnzoReaderInternal()
{
  this->Init();
}
...
//------------------------------------------------------------------------------
void svtkEnzoReaderInternal::Init()
{
  this->DataTime = 0.0;
  this->FileName = nullptr;
...
//------------------------------------------------------------------------------
void svtkEnzoReaderInternal::ReleaseDataArray()
{
  if (this->DataArray)
  {
...
//------------------------------------------------------------------------------
int svtkEnzoReaderInternal::GetBlockAttribute(
...
//------------------------------------------------------------------------------
int svtkEnzoReaderInternal::LoadAttribute(const char* attribute, int blockIdx)
{
  // TODO: implement this
  ...
  {
    this->DataArray = svtkFloatArray::New();
    this->DataArray->SetNumberOfTuples(numTupls);
    float* arrayPtr =
      static_cast<float*>(svtkArrayDownCast<svtkFloatArray>(this->DataArray)->GetPointer(0));
    H5Dread(attrIndx, dataType, H5S_ALL, H5S_ALL, H5P_DEFAULT, arrayPtr);
    arrayPtr = nullptr;
  }
  else if (H5Tequal(dataType, H5T_NATIVE_DOUBLE))
  {
    this->DataArray = svtkDoubleArray::New();
    this->DataArray->SetNumberOfTuples(numTupls);
    double* arrayPtr =
      static_cast<double*>(svtkArrayDownCast<svtkDoubleArray>(this->DataArray)->GetPointer(0));
    H5Dread(attrIndx, dataType, H5S_ALL, H5S_ALL, H5P_DEFAULT, arrayPtr);
    arrayPtr = nullptr;
  }
  else if (H5Tequal(dataType, H5T_NATIVE_INT))
  {
    this->DataArray = svtkIntArray::New();
    this->DataArray->SetNumberOfTuples(numTupls);
    int* arrayPtr =
      static_cast<int*>(svtkArrayDownCast<svtkIntArray>(this->DataArray)->GetPointer(0));
    H5Dread(attrIndx, dataType, H5S_ALL, H5S_ALL, H5P_DEFAULT, arrayPtr);
    arrayPtr = nullptr;
  }
  else if (H5Tequal(dataType, H5T_NATIVE_UINT))
  {
    this->DataArray = svtkUnsignedIntArray::New();
    this->DataArray->SetNumberOfTuples(numTupls);
    unsigned int* arrayPtr = static_cast<unsigned int*>(
      svtkArrayDownCast<svtkUnsignedIntArray>(this->DataArray)->GetPointer(0));
    H5Dread(attrIndx, dataType, H5S_ALL, H5S_ALL, H5P_DEFAULT, arrayPtr);
    arrayPtr = nullptr;
  }
  else if (H5Tequal(dataType, H5T_NATIVE_SHORT))
  {
    this->DataArray = svtkShortArray::New();
    this->DataArray->SetNumberOfTuples(numTupls);
    short* arrayPtr =
      static_cast<short*>(svtkArrayDownCast<svtkShortArray>(this->DataArray)->GetPointer(0));
    H5Dread(attrIndx, dataType, H5S_ALL, H5S_ALL, H5P_DEFAULT, arrayPtr);
    arrayPtr = nullptr;
  }
  else if (H5Tequal(dataType, H5T_NATIVE_USHORT))
  {
    this->DataArray = svtkUnsignedShortArray::New();
    this->DataArray->SetNumberOfTuples(numTupls);
    unsigned short* arrayPtr = static_cast<unsigned short*>(
      svtkArrayDownCast<svtkUnsignedShortArray>(this->DataArray)->GetPointer(0));
    H5Dread(attrIndx, dataType, H5S_ALL, H5S_ALL, H5P_DEFAULT, arrayPtr);
    arrayPtr = nullptr;
  }
  else if (H5Tequal(dataType, H5T_NATIVE_UCHAR))
  {
    ...
    this->DataArray = svtkUnsignedCharArray::New();
    this->DataArray->SetNumberOfTuples(numTupls);
    unsigned char* arrayPtr = static_cast<unsigned char*>(
      svtkArrayDownCast<svtkUnsignedCharArray>(this->DataArray)->GetPointer(0));
    H5Dread(attrIndx, dataType, H5S_ALL, H5S_ALL, H5P_DEFAULT, arrayPtr);
    arrayPtr = nullptr;
  }
  else if (H5Tequal(dataType, H5T_NATIVE_LONG))
  {
    this->DataArray = svtkLongArray::New();
    this->DataArray->SetNumberOfTuples(numTupls);
    long* arrayPtr =
      static_cast<long*>(svtkArrayDownCast<svtkLongArray>(this->DataArray)->GetPointer(0));
    H5Dread(attrIndx, dataType, H5S_ALL, H5S_ALL, H5P_DEFAULT, arrayPtr);
    arrayPtr = nullptr;
  }
  else if (H5Tequal(dataType, H5T_NATIVE_LLONG))
  {
    this->DataArray = svtkLongLongArray::New();
    this->DataArray->SetNumberOfTuples(numTupls);
    long long* arrayPtr =
      static_cast<long long*>(svtkArrayDownCast<svtkLongLongArray>(this->DataArray)->GetPointer(0));
    H5Dread(attrIndx, dataType, H5S_ALL, H5S_ALL, H5P_DEFAULT, arrayPtr);
    arrayPtr = nullptr;
  }
  else
  {
    //        svtkErrorMacro( "Unknown HDF5 data type --- it is not FLOAT, "       <<
...
void svtkEnzoReaderInternal::ReadBlockStructures()
{
  svtksys::ifstream stream(this->HierarchyFileName.c_str());
  if (!stream)
  {
    svtkGenericWarningMacro(
  ...
  while (stream)
  {
    ...
    if (theStr == "Grid")
    {
      ...
      {
        svtkGenericWarningMacro("The blocks in the hierarchy file "
...
void svtkEnzoReaderInternal::ReadGeneralParameters()
{
  svtksys::ifstream stream(this->MajorFileName.c_str());
  if (!stream)
  {
    svtkGenericWarningMacro("Invalid parameter file " << this->MajorFileName.c_str() << endl);
    return;
  }
...
void svtkEnzoReaderInternal::DetermineRootBoundingBox()
{
  svtkEnzoReaderBlock& block0 = this->Blocks[0];
...
void svtkEnzoReaderInternal::GetAttributeNames()
{
  ...
  {
    svtkEnzoReaderBlock& tmpBlock = this->Blocks[i];
  ...
  if (fileIndx < 0)
  {
    svtkGenericWarningMacro("Failed to open HDF5 grid file " << blckFile.c_str());
    return;
  }
...
void svtkEnzoReaderInternal::CheckAttributeNames()
{
  // number of cells of the reference block
  svtkEnzoReaderBlock& theBlock = this->Blocks[this->ReferenceBlock];
...
// ----------------------------------------------------------------------------
// get the meta data
void svtkEnzoReaderInternal::ReadMetaData()
{
