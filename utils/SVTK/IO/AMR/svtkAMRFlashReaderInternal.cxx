/*=========================================================================
  ...
  Module:    svtkFlashReaderInternal.cxx
...
//-----------------------------------------------------------------------------
void svtkFlashReaderInternal::GetBlockAttribute(
  const char* attribute, int blockIdx, svtkDataSet* pDataSet)
{
  ...
  {
    //   svtkDebugMacro( "Data attribute name or svtkDataSet nullptr, or " <<
  ...
  if (dataIndx < 0)
  {
    //   svtkErrorMacro( "Invalid attribute name." << endl );
    return;
  }
  ...
  if (numbDims != 4)
  {
    //   svtkErrorMacro( "Error with reading the data dimensions." << endl );
    return;
  }
  ...
  svtkDoubleArray* dataAray = svtkDoubleArray::New();
  ...
  {
    //   svtkErrorMacro( "Invalid data attribute type." << endl );
  }
...
//-----------------------------------------------------------------------------
void svtkFlashReaderInternal::Init()
{
  this->FileName = nullptr;
  this->FileIndex = -1;
  this->MinBounds[0] = this->MinBounds[1] = this->MinBounds[2] = SVTK_DOUBLE_MAX;
  this->MaxBounds[0] = this->MaxBounds[1] = this->MaxBounds[2] = -SVTK_DOUBLE_MAX;
...
// ----------------------------------------------------------------------------
int svtkFlashReaderInternal::GetCycle()
{
...
// ----------------------------------------------------------------------------
double svtkFlashReaderInternal::GetTime()
{
  const bool bTmCycle = true;
...
//-----------------------------------------------------------------------------
void svtkFlashReaderInternal::ReadMetaData()
{
  if (this->FileIndex >= 0)
  {
    return;
  ...
  if (this->FileIndex < 0)
  {
    svtkGenericWarningMacro("Failed to open file " << this->FileName << "." << endl);
    return;
  }
  ...
  {
    svtkGenericWarningMacro("Invalid Flash file, without any "
      << "block/particle." << endl);
    return;
  }
...
//-----------------------------------------------------------------------------
void svtkFlashReaderInternal::ReadProcessorIds()
{
  hid_t rootIndx = H5Gopen(this->FileIndex, "/");
  if (rootIndx < 0)
  {
    svtkGenericWarningMacro("Failed to open the root group" << endl);
    return;
  }
  ...
  if (errorIdx < 0)
  {
    svtkGenericWarningMacro("Failed to get the number of objects "
  ...
  if (this->HaveProcessorsInfo)
  {
    ...
    if (procnumId < 0)
    {
      svtkGenericWarningMacro("Processor Id information not found." << endl);
    }
    ...
    {
      svtkGenericWarningMacro("Error with getting the number of "
        << "processor Ids." << endl);
    }
...
//-----------------------------------------------------------------------------
void svtkFlashReaderInternal::ReadDoubleScalars(hid_t fileIndx)
{
  // Should only be used for FLASH3 files

  if (this->FileFormatVersion < FLASH_READER_FLASH3_FFV8)
  {
    svtkGenericWarningMacro("Error with the format version." << endl);
    return;
  }
  ...
  if (realScalarsId < 0)
  {
    svtkGenericWarningMacro("Real scalars not found in FLASH3." << endl);
    return;
  }
  ...
  if (spaceId < 0)
  {
    svtkGenericWarningMacro("Failed to get the real scalars space." << endl);
    return;
  }
...
//-----------------------------------------------------------------------------
void svtkFlashReaderInternal::ReadIntegerScalars(hid_t fileIndx)
{
  // Should only be used for FLASH3 files
  ...
  if (intScalarsId < 0)
  {
    svtkGenericWarningMacro("Integer scalars not found in FLASH3." << endl);
    return;
  }
  ...
  if (spaceId < 0)
  {
    svtkGenericWarningMacro("Failed to get the integer scalars space." << endl);
    return;
  }
...
//-----------------------------------------------------------------------------
void svtkFlashReaderInternal::ReadVersionInformation(hid_t fileIndx)
{
  ...
  if (h5_FFV < 0)
  {
    ...
    else
    {
      ...
      svtkByteSwap::SwapLE(&this->SimulationInformation.FileFormatVersion);
...
//-----------------------------------------------------------------------------
void svtkFlashReaderInternal::ReadSimulationParameters(hid_t fileIndx, bool bTmCycle)
{
  if (this->FileFormatVersion < FLASH_READER_FLASH3_FFV8)
  {
    ...
    if (simparamsId < 0)
    {
      svtkGenericWarningMacro("Simulation parameters unavailable." << endl);
    }
  ...
  {
    svtkGenericWarningMacro("Inconsistency in the number of blocks." << endl);
    return;
  }
...
//-----------------------------------------------------------------------------
void svtkFlashReaderInternal::GetBlockMinMaxGlobalDivisionIds()
{
...
// ----------------------------------------------------------------------------
void svtkFlashReaderInternal::ReadBlockBounds()
{
  // Read the bounding box description for the blocks
  hid_t bboxId = H5Dopen(this->FileIndex, "bounding box");
  if (bboxId < 0)
  {
    svtkGenericWarningMacro("Blocks bounding info not found." << endl);
    return;
  }
  ...
  if (this->FileFormatVersion <= FLASH_READER_FLASH3_FFV8)
  {
    ...
    {
      svtkGenericWarningMacro("Error with number of blocks "
    ...
    this->MinBounds[0] = SVTK_DOUBLE_MAX;
    this->MinBounds[1] = SVTK_DOUBLE_MAX;
    this->MinBounds[2] = SVTK_DOUBLE_MAX;
    this->MaxBounds[0] = -SVTK_DOUBLE_MAX;
    this->MaxBounds[1] = -SVTK_DOUBLE_MAX;
    this->MaxBounds[2] = -SVTK_DOUBLE_MAX;
  ...
  else if (this->FileFormatVersion == FLASH_READER_FLASH3_FFV9)
  {
    if (static_cast<int>(bbox_ndims) != 3 ||
      ...
      static_cast<int>(bbox_dims[2]) != 2)
    {
      svtkGenericWarningMacro("Error with number of blocks." << endl);
      return;
    }
...
void svtkFlashReaderInternal::ReadBlockCenters()
{
  ...
  if (this->FileFormatVersion <= FLASH_READER_FLASH3_FFV8)
  {
    ...
    {
      svtkGenericWarningMacro("Error with number of blocks or "
...
// ----------------------------------------------------------------------------
void svtkFlashReaderInternal::ReadBlockStructures()
{
  // temporarily disable error reporting
  H5E_auto_t old_errorfunc;
  void* old_clientdata = nullptr;
  ...
  if (gid_ndims != 2)
  {
    svtkGenericWarningMacro("Error with reading block connectivity." << endl);
    return;
  }
  ...
  switch (gid_dims[1])
  {
    ...
    default:
      svtkGenericWarningMacro("Invalid block connectivity." << endl);
      break;
  }
...
// ----------------------------------------------------------------------------
void svtkFlashReaderInternal::ReadRefinementLevels()
{
  // Read the bounding box description for the blocks
  hid_t refinementId = H5Dopen(this->FileIndex, "refine level");
  if (refinementId < 0)
  {
    svtkGenericWarningMacro("Refinement levels not found." << endl);
    return;
  }
...
//-----------------------------------------------------------------------------
void svtkFlashReaderInternal::ReadDataAttributeNames()
{
  hid_t unknownsId = H5Dopen(this->FileIndex, "unknown names");
  if (unknownsId < 0)
  {
    svtkGenericWarningMacro("Data attributes not found." << endl);
    return;
  }
  ...
  {
    svtkGenericWarningMacro("Error with reading data attributes." << endl);
    return;
  }
...
// ----------------------------------------------------------------------------
void svtkFlashReaderInternal::ReadParticlesComponent(
  hid_t dataIndx, const char* compName, double* dataBuff)
{
  if (!compName || this->FileFormatVersion < FLASH_READER_FLASH3_FFV8)
  {
    svtkGenericWarningMacro("Invalid component name of particles or "
...
// ----------------------------------------------------------------------------
void svtkFlashReaderInternal::ReadParticleAttributes()
{
  // temporarily disable error reporting
  H5E_auto_t old_errorfunc;
  void* old_clientdata = nullptr;
  ...
  if (p_ndims != 1)
  {
    svtkGenericWarningMacro("Error with number of data attributes." << endl);
  }
  ...
  for (int i = 0; i < numMembers; i++)
  {
      ...
      strcmp(member_name, "particle_z"))
    {
      ...
      {
        svtkGenericWarningMacro("Only DOUBLE and INT supported." << endl);
      }
    }
...
// ----------------------------------------------------------------------------
void svtkFlashReaderInternal::ReadParticleAttributesFLASH3()
{
  // Should only be used for FLASH3 files
  if (this->FileFormatVersion < FLASH_READER_FLASH3_FFV8)
  {
  ...
  if (ndims != 2 || dims[1] != 1)
  {
    if (ndims != 2)
    {
      svtkGenericWarningMacro("FLASH3 expecting particle names ndims of 2, got " << ndims << endl);
    }
    if (dims[1] != 1)
    {
  ...
  if (pointId < 0)
  {
    svtkGenericWarningMacro("FLASH3 no tracer particles" << endl);
    this->NumberOfParticles = 0;
    old_clientdata = nullptr;
    return;
  }
  ...
  if (p_ndims != 2)
  {
    svtkGenericWarningMacro("FLASH3, expecting particle tracer ndims of 2, got" << p_ndims << endl);
  }
  this->NumberOfParticles = p_dims[0];
