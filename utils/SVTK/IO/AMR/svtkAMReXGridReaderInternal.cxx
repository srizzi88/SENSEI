/*=========================================================================
  ...
  Module:    svtkAMReXGridReaderInternal.cxx
...
svtkAMReXGridHeader::svtkAMReXGridHeader()
  : versionName()
  , variableNamesSize(0)
  , variableNames()
  , dim(0)
...
void svtkAMReXGridHeader::PrintSelf(std::ostream& os, svtkIndent indent)
{
  this->PrintSelfGenericHeader(os, indent);
}

void svtkAMReXGridHeader::PrintSelfGenericHeader(ostream& os, svtkIndent indent)
{
...
bool svtkAMReXGridHeader::Parse(const std::string& headerData)
{
...
bool svtkAMReXGridHeader::ParseGenericHeader(const std::string& headerData)
{
  char c;
  ...
  {
    svtkGenericWarningMacro("Failed to read versionName string.");
    return false;
  }
  ...
  {
    svtkGenericWarningMacro("dim must be 1, 2, or 3.");
    return false;
  }
  hstream >> this->time;
  ...
  {
    svtkGenericWarningMacro("finestLevel must be >= 0");
    return false;
  }
...
svtkAMReXGridLevelHeader::svtkAMReXGridLevelHeader()
  : level(0)
  , dim(0)
  , levelVersion(0)
  , levelHow(0)
...
void svtkAMReXGridLevelHeader::PrintSelf(ostream& os, svtkIndent indent)
{
  this->PrintSelfLevelHeader(os, indent);
}

void svtkAMReXGridLevelHeader::PrintSelfLevelHeader(ostream& os, svtkIndent indent)
{
...
bool svtkAMReXGridLevelHeader::Parse(int _level, int _dim, const std::string& headerData)
{
...
bool svtkAMReXGridLevelHeader::ParseLevelHeader(int _level, int _dim, const std::string& headerData)
{
  std::istringstream hstream(headerData);
  this->level = _level;
  this->dim = _dim;
...
svtkAMReXGridReaderInternal::svtkAMReXGridReaderInternal()
  : FileName()
  , LevelHeader()
{
...
void svtkAMReXGridReaderInternal::DestroyHeader()
{
  delete this->Header;
  this->Header = nullptr;
}

void svtkAMReXGridReaderInternal::DestroyLevelHeader()
{
...
void svtkAMReXGridReaderInternal::PrintSelf(ostream& os, svtkIndent indent)
{
...
void svtkAMReXGridReaderInternal::SetFileName(char* fName)
{
...
//-----------------------------------------------------------------------------
void svtkAMReXGridReaderInternal::ReadMetaData()
{
...
//-----------------------------------------------------------------------------
bool svtkAMReXGridReaderInternal::ReadHeader()
{
  this->DestroyHeader();
  ...
  auto headerPtr = new svtkAMReXGridHeader();
...
//-----------------------------------------------------------------------------
bool svtkAMReXGridReaderInternal::ReadLevelHeader()
{
  this->DestroyLevelHeader();
  ...
  for (int level = 0; level <= this->Header->finestLevel; ++level)
  {
    ...
    auto headerPtr = new svtkAMReXGridLevelHeader();
...
int svtkAMReXGridReaderInternal::GetNumberOfLevels()
{
  return this->headersAreRead ? this->Header->finestLevel : -1;
}

int svtkAMReXGridReaderInternal::GetBlockLevel(const int blockIdx)
{
  if (this->headersAreRead)
  {
...
int svtkAMReXGridReaderInternal::GetNumberOfBlocks()
{
  if (this->headersAreRead)
  {
    int numberOfLevels = this->GetNumberOfLevels() + 1;
...
int svtkAMReXGridReaderInternal::GetBlockIndexWithinLevel(int blockIdx, int level)
{
  if (this->headersAreRead)
  {
...
void svtkAMReXGridReaderInternal::GetBlockAttribute(
  const char* attribute, int blockIdx, svtkDataSet* pDataSet)
{
  if (this->headersAreRead)
  {
    ...
    if (fb.open(FABFileName, std::ios::binary | std::ios::in))
    {
      ...
      {
        svtkNew<svtkDoubleArray> dataArray;
        dataArray->SetName(attribute);
        dataArray->SetNumberOfTuples(numberOfPoints);
...
int svtkAMReXGridReaderInternal::GetOffsetOfAttribute(const char* attribute)
{
  int i = 0, position = 0;
  bool found = false;
...
void svtkAMReXGridReaderInternal::ReadFAB(std::istream& is)
{
  char f, a, b;
  is >> f;
  is >> a;
...
int svtkAMReXGridReaderInternal::ReadVersion(std::istream& is)
{
  char colon;
  is >> colon;
  if (colon == ':')
...
void svtkAMReXGridReaderInternal::ReadOrder(std::istream& is, std::vector<int>& ar)
{
  char c;
  is >> c; // '('
  int size;
...
void svtkAMReXGridReaderInternal::PrintOrder(std::vector<int>& ar)
{
  size_t size = ar.size();
  std::cout << "(" << size << ", (";
...
void svtkAMReXGridReaderInternal::ReadFormat(std::istream& is, std::vector<long>& ar)
{
  char c;
  is >> c; // '('
  int size;
...
void svtkAMReXGridReaderInternal::PrintFormat(std::vector<long>& ar)
{
  size_t size = ar.size();
  std::cout << "(" << size << ", (";
  for (size_t i = 0; i < size; ++i)
...
RealDescriptor* svtkAMReXGridReaderInternal::ReadRealDescriptor(std::istream& is)
{
  std::vector<long> fmt;
  std::vector<int> ord;
  char c;
...
int svtkAMReXGridReaderInternal::ReadBoxArray(std::istream& is, int* boxArray, int* boxArrayDim)
{
  char c;
  is >> c; // read '('
  for (int dd = 0; dd < 3; ++dd)
...
void svtkAMReXGridReaderInternal::PrintBoxArray(int* boxArray)
{
  std::cout << "(";
  std::cout << "(";
...
int svtkAMReXGridReaderInternal::ReadNumberOfAttributes(std::istream& is)
{
  int numberOfAttributes;
...
void svtkAMReXGridReaderInternal::ReadBlockAttribute(
  std::istream& is, int numberOfPoints, int size, char* buffer)
{
  is.read(buffer, numberOfPoints * size);
}

void svtkAMReXGridReaderInternal::Convert(
...
void svtkAMReXGridReaderInternal::PermuteOrder(
