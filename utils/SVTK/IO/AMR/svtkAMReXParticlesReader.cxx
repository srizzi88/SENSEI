/*=========================================================================
  ...
  Module:    svtkAMReXParticlesReader.cxx
...
=========================================================================*/
#include "svtkAMReXParticlesReader.h"
...
std::string ReadAndBroadCastFile(
  const std::string& filename, svtkMultiProcessController* controller, svtkAMReXParticlesReader* self)
{
  std::string contents;
  if (controller == nullptr || controller->GetLocalProcessId() == 0)
  {
    svtksys::ifstream stream(filename.c_str(), std::ios::binary);
    if (stream)
    {
    ...
    {
      svtkErrorWithObjectMacro(self, "Failed to open file '" << filename << "'.");
    }
  }
...
class svtkAMReXParticlesReader::AMReXParticleHeader
{
  template <typename RealType, typename IntType>
  bool ReadParticles(
    svtkPolyData* pd, const int count, istream& ifp, const svtkAMReXParticlesReader* self) const
  {
    auto selection = self->GetPointDataArraySelection();

    // read integer data.
    svtkNew<svtkAOSDataArrayTemplate<IntType> > istuff;
    ...
    // read real data.
    svtkNew<svtkAOSDataArrayTemplate<RealType> > rstuff;
    ...
    if (this->num_int > 0)
    {
      ...
      std::vector<svtkIdType*> idtypeptrs(this->num_int, nullptr);
      ...
      for (int cc = 0; cc < this->num_int; ++cc)
      {
        ...
        {
          svtkNew<svtkAOSDataArrayTemplate<IntType> > iarray;
      ...
      for (int cc = 0; cc < count; ++cc)
      {
        for (int kk = 0; kk < this->num_int; ++kk, ++isource)
        {
          ...
          {
            idtypeptrs[kk][cc] = static_cast<svtkIdType>(*isource);
          }
        }
      }
    }
    ...
    if (this->num_real > 0)
    {
      ...
      svtkNew<svtkAOSDataArrayTemplate<RealType> > coords;
      ...
      for (int cc = this->num_real_base; cc < this->num_real; ++cc)
      {
        ...
        svtkNew<svtkAOSDataArrayTemplate<RealType> > rarray;
      ...
      else
      {
        ...
        svtkNew<svtkAOSDataArrayTemplate<RealType> > newcoords;
    ...
    // for (svtkIdType cc=0; cc < count; ++cc)
    //{
    ...
    int filenumber, int zerofill) const
  {
    ...
    return (svtksys::SystemTools::FileExists(str.str(), /*isFile*/ true)) ? str.str()
                                                                         : std::string();
  }
...
public:
  ...
  int num_real;
  bool is_checkpoint;
  svtkIdType num_particles;
  svtkIdType max_next_id;
  int finest_level;
  int num_levels;
  ...
  bool Parse(const std::string& headerData, svtkAMReXParticlesReader* self)
  {
    ...
    if (version.empty())
    {
      svtkErrorWithObjectMacro(self, "Failed to read version string.");
      return false;
    }
      ...
      version.find("Version_Two_Dot_Zero") != std::string::npos)
    {
      ...
      {
        svtkErrorWithObjectMacro(self, "Bad version string: " << version);
        return false;
      }
    }
    else
    ...
    {
      svtkErrorWithObjectMacro(self, "dim must be 1, 2, or 3.");
      return false;
    }
    ...
    {
      svtkErrorWithObjectMacro(self, "potentially incorrect num_int_extra=" << this->num_int_extra);
      return false;
    }
    ...
    {
      svtkErrorWithObjectMacro(self, "num_particles must be >=0");
      return false;
    }
    ...
    {
      svtkErrorWithObjectMacro(self, "max_next_id must be > 0");
      return false;
    }
    ...
    {
      svtkErrorWithObjectMacro(self, "finest_level must be >= 0");
      return false;
    }
  ...
  bool ReadGrid(int level, int idx, svtkPolyData* pd, const svtkAMReXParticlesReader* self) const
  {
    ...
    svtksys::ifstream ifp(fname.c_str(), std::ios::binary);
    if (!ifp.good())
    {
      return false;
    }
    ...
    {
      return this->ReadParticles<float, svtkTypeInt32>(pd, gridInfo.count, ifp, self);
    }
    else if (this->real_type == 64 && this->int_type == 32)
    {
      return this->ReadParticles<double, svtkTypeInt32>(pd, gridInfo.count, ifp, self);
    }
    else
    {
      return false;
  ...
  void PopulatePointArraySelection(svtkDataArraySelection* selection) const
  {
...
svtkStandardNewMacro(svtkAMReXParticlesReader);
svtkCxxSetObjectMacro(svtkAMReXParticlesReader, Controller, svtkMultiProcessController);
//----------------------------------------------------------------------------
svtkAMReXParticlesReader::svtkAMReXParticlesReader()
  : Controller(nullptr)
  , PlotFileName()
  , PlotFileNameMTime()
  , MetaDataMTime()
  ...
  , Header(nullptr)
{
  ...
  this->PointDataArraySelection->AddObserver(
    svtkCommand::ModifiedEvent, this, &svtkAMReXParticlesReader::Modified);
}
...
//----------------------------------------------------------------------------
void svtkAMReXParticlesReader::SetPlotFileName(const char* fname)
{
...
//----------------------------------------------------------------------------
void svtkAMReXParticlesReader::SetParticleType(const std::string& str)
{
...
//----------------------------------------------------------------------------
svtkDataArraySelection* svtkAMReXParticlesReader::GetPointDataArraySelection() const
{
  return this->PointDataArraySelection;
}

//----------------------------------------------------------------------------
int svtkAMReXParticlesReader::CanReadFile(const char* fname, const char* particleType)
{
  if (fname && svtksystools::FileIsDirectory(fname))
  {
    if (!svtksystools::FileExists(std::string(fname) + "/Header", true))
    {
      return 0;
    }
    ...
    if (svtksystools::FileIsDirectory(particles))
    {
      const std::string header(particles + "/Header");
      if (svtksystools::FileExists(header, /*isFile*/ true))
      {
        svtksys::ifstream ifp(header.c_str(), std::ios::binary);
        if (ifp)
        {
...
//----------------------------------------------------------------------------
const char* svtkAMReXParticlesReader::GetPlotFileName() const
{
  return (this->PlotFileName.empty() ? nullptr : this->PlotFileName.c_str());
}

//----------------------------------------------------------------------------
void svtkAMReXParticlesReader::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
int svtkAMReXParticlesReader::RequestInformation(
...
//----------------------------------------------------------------------------
int svtkAMReXParticlesReader::RequestData(
  svtkInformation*, svtkInformationVector**, svtkInformationVector* outputVector)
{
  ...
  {
    svtkNew<svtkMultiPieceDataSet> piece;
    output->SetBlock(cc, piece);
    output->GetMetaData(cc)->Set(
      svtkCompositeDataSet::NAME(), (std::string("Level ") + std::to_string(cc)).c_str());
...
//----------------------------------------------------------------------------
bool svtkAMReXParticlesReader::ReadMetaData()
{
  ...
  {
    svtkErrorMacro("PlotFileName must be specified.");
    return false;
  }

  if (this->ParticleType.empty())
  {
    svtkErrorMacro("ParticleType must be specified.");
    return false;
  }
...
//----------------------------------------------------------------------------
bool svtkAMReXParticlesReader::ReadLevel(
  const int level, svtkMultiPieceDataSet* levelDS, const int piece_idx, const int num_pieces) const
{
  ...
  for (int cc = start_grid_idx; cc < start_grid_idx + grids_count; ++cc)
  {
    ...
    {
      svtkGenericWarningMacro("Failed to read grid for level " << level << ", index " << cc);
      return false;
    }
