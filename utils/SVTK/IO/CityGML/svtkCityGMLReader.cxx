...
class svtkCityGMLReader::Implementation
{
public:
  enum PolygonType
  {
...
public:
  Implementation(svtkCityGMLReader* reader, int lod, int useTransparencyAsOpacity)
  {
    this->Initialize(reader, lod, useTransparencyAsOpacity);
  }

  void Initialize(svtkCityGMLReader* reader, int lod, int useTransparencyAsOpacity)
  {
    this->Reader = reader;
    this->LOD = lod;
  ...
  void InitializeImplicitGeometry()
  {
    ...
    {
      this->RelativeGeometryDataSets = svtkSmartPointer<svtkMultiBlockDataSet>::New();
    }
  ...
  void ReadImplicitGeometry(
    const pugi::xml_node& implicitGeometryNode, svtkMultiBlockDataSet* output, const char* element)
  {
    ...
    {
      svtkWarningWithObjectMacro(this->Reader, << "Cannot find cached multi surface for id=" << id);
      return;
    }
    ...
    svtkDataObject* obj = transformFilter->GetOutputDataObject(0);
    this->SetField(obj, "element", element);
    output->SetBlock(output->GetNumberOfBlocks(), obj);
  }

  void ReadImplicitGeometry(pugi::xml_document& doc, svtkMultiBlockDataSet* output,
    const char* gmlNamespace, const char* feature)
  {
    svtkNew<svtkMultiBlockDataSet> b;
  ...
  bool IsNewPolygonNeeded(PolygonType polygonType, size_t materialIndex,
    std::unordered_map<size_t, svtkPolyData*>& materialIndexToPolyData, const std::string& imageURI,
    std::unordered_map<std::string, svtkPolyData*>& imageURIToPolyData)
  {
    switch (polygonType)
    {
  ...
  void SavePolygon(PolygonType polygonType, size_t materialIndex,
    std::unordered_map<size_t, svtkPolyData*>& materialIndexToPolyData, const std::string& imageURI,
    std::unordered_map<std::string, svtkPolyData*>& imageURIToPolyData, svtkPolyData* polyData)
  {
    switch (polygonType)
    {
      case PolygonType::MATERIAL:
  ...
  svtkPolyData* GetPolygon(PolygonType polygonType, size_t materialIndex,
    std::unordered_map<size_t, svtkPolyData*>& materialIndexToPolyData, const std::string& imageURI,
    std::unordered_map<std::string, svtkPolyData*>& imageURIToPolyData)
  {
    switch (polygonType)
   ...
   * Return true if the texture is found, false otherwise
   */
  svtkIdType TCoordsFromString(const std::string& textureCoordinates, svtkDoubleArray* output)
  {
  ...
  void ReadLinearRingPolygon(pugi::xml_node nodeRing, svtkPoints* points, svtkCellArray* polys)
  {
    svtkIdType i = 0;
    svtkNew<svtkPolygon> poly;
    svtkIdList* polyPointIds = poly->GetPointIds();
        ...
        for (svtkIdType j = 0; j < 3; ++j)
        {
          ...
          if (iss.fail())
          {
            ...
            else
            {
              ...
              if (!svtkMathUtilities::FuzzyCompare(pFirst[0], pLast[0]) ||
                !svtkMathUtilities::FuzzyCompare(pFirst[1], pLast[1]) ||
                !svtkMathUtilities::FuzzyCompare(pFirst[2], pLast[2]))
              {
    ...
    {
      svtkIdType n = std::distance(nodeRing.begin(), nodeRing.end());
  ...
  void ReadLinearRingLines(pugi::xml_node nodeRing, svtkPoints* points, svtkCellArray* lines)
  {
    pugi::xml_node posList = nodeRing.child("gml:posList");
    if (posList)
    {
      svtkNew<svtkLine> line;
      svtkIdType i = 1;
      std::istringstream iss(posList.child_value());
      bool validPoint = true;
      double p[3] = { 0., 0., 0. };
      ...
      points->InsertNextPoint(p);
      svtkIdType firstPointIndex = points->GetNumberOfPoints() - 1;
      do
      {
        std::fill(p, p + 3, 0);
        for (svtkIdType j = 0; j < 3; ++j)
  ...
  static void SetField(svtkDataObject* obj, const char* name, const char* value)
  {
    svtkFieldData* fd = obj->GetFieldData();
    if (!fd)
    {
      svtkNew<svtkFieldData> newfd;
      obj->SetFieldData(newfd);
    }
    svtkNew<svtkStringArray> sa;
  ...
  static void SetField(svtkDataObject* obj, const char* name, float* value, svtkIdType size)
  {
    svtkFieldData* fd = obj->GetFieldData();
    if (!fd)
    {
   ...
   */
  void ReadMultiSurface(const pugi::xml_node& multiSurfaceNode, svtkMultiBlockDataSet* output)
  {
    ...
    std::unordered_map<size_t, svtkPolyData*> materialIndexToPolyData;
    std::unordered_map<std::string, svtkPolyData*> imageURIToPolyData;
    size_t polyDataCount = 0;
    // used to prevent polydata from being deleted until the end of the function
    svtkNew<svtkCollection> polyDataList;
    ...
    for (auto it = xpoly.begin(); it != xpoly.end(); ++it)
    {
      ...
      svtkPolyData* polyData = GetPolygon(
        polygonType, materialIndex, materialIndexToPolyData, imageURI, imageURIToPolyData);
      svtkIdType exteriorTcoordsCount = 0;

      svtkNew<svtkPolyData> exteriorContour;
      svtkNew<svtkPoints> exteriorPoints;
      exteriorPoints->SetDataType(SVTK_DOUBLE);
      svtkNew<svtkCellArray> exteriorCells;
      exteriorContour->SetPoints(exteriorPoints);
      bool hasTexture = (polygonType == PolygonType::TEXTURE);
      if (hasTexture)
      {
        svtkNew<svtkDoubleArray> exteriorTcoords;
      ...
      if (nodeInterior)
      {
        ...
        svtkNew<svtkPolyData> interiorContour;
        svtkNew<svtkPoints> interiorPoints;
        interiorPoints->SetDataType(SVTK_DOUBLE);
        svtkNew<svtkCellArray> interiorCells;
        svtkNew<svtkDoubleArray> interiorTCoords;
        ...
        svtkNew<svtkPolygon> exteriorPolygon;
        ...
        svtkMath::Cross(exteriorPolygonNormal, zAxis, rotationAxis);
        double angleRad = svtkMath::AngleBetweenVectors(exteriorPolygonNormal, zAxis);
        double angle = svtkMath::DegreesFromRadians(angleRad);
        svtkNew<svtkTransform> transform;
        ...
        transformFilter->Update();
        svtkPointSet* xyPoly = transformFilter->GetOutput();
        svtkPoints* xyPoints = xyPoly->GetPoints();
        double p[3];
        xyPoints->GetPoint(0, p);
        double referenceZ = p[2];
        for (svtkIdType pointId = 1; pointId < xyPoints->GetNumberOfPoints(); ++pointId)
        {
        ...
        svtkNew<svtkContourTriangulator> triangulator;
        triangulator->SetInputConnection(transformFilter->GetOutputPort());

        svtkNew<svtkTransformFilter> transformBackFilter;
        ...
        svtkPolyData* polyWithHoles = svtkPolyData::SafeDownCast(transformBackFilter->GetOutput());

        svtkNew<svtkAppendPolyData> appendPolyWithHoles;
        appendPolyWithHoles->AddInputData(polyData);
        appendPolyWithHoles->AddInputData(polyWithHoles);
        appendPolyWithHoles->Update();
        svtkPolyData* newPolyData = svtkPolyData::SafeDownCast(appendPolyWithHoles->GetOutput());
      ...
      else
      {
        ...
        if (exteriorTcoordsCount != exteriorPoints->GetNumberOfPoints())
        {
          if (hasTexture)
          {
            ...
            if (exteriorTcoordsCount < exteriorPoints->GetNumberOfPoints())
            {
              svtkDataArray* exteriorTcoords = exteriorContour->GetPointData()->GetTCoords();
        ...
        svtkNew<svtkTriangleFilter> triangulate;
        ...
        append->Update();
        svtkPolyData* newPolyData = svtkPolyData::SafeDownCast(append->GetOutput());
        SavePolygon(polygonType, materialIndex, materialIndexToPolyData, imageURI,
          imageURIToPolyData, newPolyData);
        polyDataList->AddItem(newPolyData);
      }
  ...
  void ReadMultiSurfaceGroup(pugi::xml_document& doc, svtkMultiBlockDataSet* output,
    ...
    int maximumNumberOfNodes = std::numeric_limits<int>::max())
  {
    ...
    {
      svtkNew<svtkMultiBlockDataSet> groupBlock;
      ostr.str("");
  ...
  void ReadReliefFeature(pugi::xml_document& doc, svtkMultiBlockDataSet* output)
  {
    svtkNew<svtkPoints> points;
    points->SetDataType(SVTK_DOUBLE);
  ...
  void ReadWaterBody(pugi::xml_document& doc, svtkMultiBlockDataSet* output)
  {
    svtkNew<svtkMultiBlockDataSet> b;
...
private:
  ...
  std::unordered_map<std::string, svtkDataObject*> RelativeGeometryIdToDataSet;
  // used to store the datasets
  svtkSmartPointer<svtkMultiBlockDataSet> RelativeGeometryDataSets;
};

svtkStandardNewMacro(svtkCityGMLReader);

//----------------------------------------------------------------------------
svtkCityGMLReader::svtkCityGMLReader()
{
...
//----------------------------------------------------------------------------
int svtkCityGMLReader::RequestData(
  svtkInformation*, svtkInformationVector**, svtkInformationVector* outputVector)
{
  ...
  {
    svtkErrorMacro(<< "XPath Error:  " << e.what());
    return 0;
  }
  catch (std::runtime_error& e)
  {
    svtkErrorMacro(<< "Error:  " << e.what());
    return 0;
  }
  return 1;
}
...
//----------------------------------------------------------------------------
void svtkCityGMLReader::PrintSelf(ostream& os, svtkIndent indent)
{
