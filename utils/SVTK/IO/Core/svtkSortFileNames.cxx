...
svtkStandardNewMacro(svtkSortFileNames);
...
{
public:
  typedef std::vector<svtkSmartPointer<svtkStringArray> > VectorType;

  static svtkStringArrayVector* New() { return new svtkStringArrayVector; }

  void Delete() { delete this; }

  void Reset() { this->Container.clear(); }

  void InsertNextStringArray(svtkStringArray* stringArray)
  {
    this->Container.push_back(stringArray);
  }

  svtkStringArray* GetStringArray(int i)
  {
...
private:
  svtkStringArrayVector()
    : Container()
  {
  }
  ~svtkStringArrayVector() { this->Container.clear(); }

  VectorType Container;
};

//----------------------------------------------------------------------------
svtkSortFileNames::svtkSortFileNames()
{
  ...
  this->Groups = svtkStringArrayVector::New();
}
...
void svtkSortFileNames::PrintSelf(ostream& os, svtkIndent indent)
{
...
void svtkSortFileNames::SetInputFileNames(svtkStringArray* input)
{
  svtkSetObjectBodyMacro(InputFileNames, svtkStringArray, input);
}

int svtkSortFileNames::GetNumberOfGroups()
{
  this->Update();
...
svtkStringArray* svtkSortFileNames::GetNthGroup(int i)
{
  this->Update();

  if (!this->GetGrouping())
  {
    svtkErrorMacro(<< "GetNthGroup(): Grouping not on.");
    return nullptr;
  }
  ...
  {
    svtkErrorMacro(<< "GetNthGroup(i): index " << i << " is out of range");
    return nullptr;
  }
}

svtkStringArray* svtkSortFileNames::GetFileNames()
{
  this->Update();
...
void svtkSortFileNames::GroupFileNames(svtkStringArray* input, svtkStringArrayVector* output)
{
  std::string baseName;
  ...
  svtkIdType numberOfStrings = input->GetNumberOfValues();
  for (svtkIdType i = 0; i < numberOfStrings; i++)
  {
    std::string& fileName = input->GetValue(i);
    extension = svtksys::SystemTools::GetFilenameLastExtension(fileName);
    fileNamePath = svtksys::SystemTools::GetFilenamePath(fileName);
    baseName = svtksys::SystemTools::GetFilenameWithoutLastExtension(fileName);
  ...
  while (!ungroupedFiles.empty())
  {
    ...
    svtkStringArray* newGroup = svtkStringArray::New();
...
static bool svtkCompareFileNamesIgnoreCase(const std::string& s1, const std::string& s2)
{
...
static bool svtkCompareFileNamesNumeric(const std::string& s1, const std::string& s2)
{
  unsigned int n1 = static_cast<unsigned int>(s1.length());
  unsigned int n2 = static_cast<unsigned int>(s2.length());
...
// Sort filenames numerically
static bool svtkCompareFileNamesNumericIgnoreCase(const std::string& s1, const std::string& s2)
{
  unsigned int n1 = static_cast<unsigned int>(s1.length());
  unsigned int n2 = static_cast<unsigned int>(s2.length());
  ...
  if ((i1 == n1) && (i2 == n2))
  {
    return svtkCompareFileNamesIgnoreCase(s1, s2);
  }
...
void svtkSortFileNames::SortFileNames(svtkStringArray* input, svtkStringArray* output)
{
  // convert svtkStringArray into an STL vector
  std::vector<std::string> fileNames;
  svtkIdType numberOfStrings = input->GetNumberOfValues();
  for (svtkIdType j = 0; j < numberOfStrings; j++)
  {
    std::string& fileName = input->GetValue(j);

    // skip anything that is a directory
    if (this->SkipDirectories && svtksys::SystemTools::FileIsDirectory(fileName))
    {
      continue;
    }
  ...
  if (this->IgnoreCase)
  {
    if (this->NumericSort)
    {
      ...
      std::sort(fileNames.begin(), fileNames.end(), svtkCompareFileNamesNumericIgnoreCase);
    }
    else
    {
      // lexicographic sort without case sensitivity
      std::sort(fileNames.begin(), fileNames.end(), svtkCompareFileNamesIgnoreCase);
    }
  }
  else
  {
    ...
    {
      // numeric sort
      std::sort(fileNames.begin(), fileNames.end(), svtkCompareFileNamesNumeric);
    }
    else
    {
...
void svtkSortFileNames::Execute()
{
