/*=========================================================================
  ...
  Module:    svtkEnSightGoldBinaryReader.cxx
...
#if defined(_WIN32)
#define SVTK_STAT_STRUCT struct _stat64
#define SVTK_STAT_FUNC _stat64
#elif defined _DARWIN_FEATURE_64_BIT_INODE || defined __FreeBSD__ || defined __NetBSD__ ||         \
  defined __OpenBSD__
// The BSDs use stat().
#define SVTK_STAT_STRUCT struct stat
#define SVTK_STAT_FUNC stat
#else
// here, we're relying on _FILE_OFFSET_BITS defined in svtkWin32Header.h to help
// us on POSIX without resorting to using stat64.
#define SVTK_STAT_STRUCT struct stat64
#define SVTK_STAT_FUNC stat64
#endif

svtkStandardNewMacro(svtkEnSightGoldBinaryReader);
class svtkEnSightGoldBinaryReader::FileOffsetMapInternal
{
  typedef std::string MapKey;
  typedef std::map<int, svtkTypeInt64> MapValue;
...
//----------------------------------------------------------------------------
svtkEnSightGoldBinaryReader::svtkEnSightGoldBinaryReader()
{
  this->FileOffsets = new svtkEnSightGoldBinaryReader::FileOffsetMapInternal;
...
//----------------------------------------------------------------------------
int svtkEnSightGoldBinaryReader::OpenFile(const char* filename)
{
  if (!filename)
  {
    svtkErrorMacro(<< "Missing filename.");
    return 0;
  }
  ...
  // Open the new file
  svtkDebugMacro(<< "Opening file " << filename);
  SVTK_STAT_STRUCT fs;
  if (!SVTK_STAT_FUNC(filename, &fs))
  {
    // Find out how big the file is.
    this->FileSize = static_cast<svtkTypeUInt64>(fs.st_size);

    std::ios_base::openmode mode = ios::in;
#ifdef _WIN32
    mode |= ios::binary;
#endif
    this->GoldIFile = new svtksys::ifstream(filename, mode);
  }
  else
  {
    svtkErrorMacro("stat failed.");
    return 0;
  }
  if (!this->GoldIFile || this->GoldIFile->fail())
  {
    svtkErrorMacro(<< "Could not open file " << filename);
    return 0;
  }
  ...
  {
    svtkErrorMacro(<< filename << " is missing header information");
    return 0;
  }
...
//----------------------------------------------------------------------------
int svtkEnSightGoldBinaryReader::InitializeFile(const char* fileName)
{
  ...
  {
    svtkErrorMacro("Error with line reading upon file initialization");
    return 0;
  }

  if (sscanf(line, " %*s %s", subLine) != 1)
  {
    svtkErrorMacro("Error with subline extraction upon file initialization");
    return 0;
  }

  if (strncmp(subLine, "Binary", 6) != 0 && strncmp(subLine, "binary", 6) != 0)
  {
    svtkErrorMacro("This is not a binary data set. Try "
...
//----------------------------------------------------------------------------
int svtkEnSightGoldBinaryReader::ReadGeometryFile(
  const char* fileName, int timeStep, svtkMultiBlockDataSet* output)
{
  ...
  while (lineRead > 0 && strncmp(line, "part", 4) == 0)
  {
    ...
    {
      svtkErrorMacro("Invalid part id; check that ByteOrder is set correctly.");
      return 0;
    }
    realId = this->InsertNewPartId(partId);

    // Increment the number of geometry parts such that the measured geometry,
    // if any, can be properly combined into a svtkMultiBlockDataSet object.
    // --- fix to bug #7453
...
//----------------------------------------------------------------------------
int svtkEnSightGoldBinaryReader::CountTimeSteps()
{
  int count = 0;
  while (1)
  {
...
//----------------------------------------------------------------------------
int svtkEnSightGoldBinaryReader::SkipTimeStep()
{
  char line[80], subLine[80];
  int lineRead;
...
//----------------------------------------------------------------------------
int svtkEnSightGoldBinaryReader::SkipStructuredGrid(char line[256])
{
  char subLine[80];
  int lineRead;
  int iblanked = 0;
  ...
  {
    svtkErrorMacro("Invalid dimensions read; check that ByteOrder is set correctly.");
    return -1;
  }
...
//----------------------------------------------------------------------------
int svtkEnSightGoldBinaryReader::SkipUnstructuredGrid(char line[256])
{
  int lineRead = 1;
  int i;
  int numElements;
  ...
  while (lineRead && strncmp(line, "part", 4) != 0)
  {
    if (strncmp(line, "coordinates", 11) == 0)
    {
      ...
      {
        svtkErrorMacro("Invalid number of points; check that ByteOrder is set correctly.");
        return -1;
      }
    ...
    else if (strncmp(line, "point", 5) == 0 || strncmp(line, "g_point", 7) == 0)
    {
      ...
      {
        svtkErrorMacro("Invalid number of point cells; check that ByteOrder is set correctly.");
        return -1;
      }
    ...
    else if (strncmp(line, "bar2", 4) == 0 || strncmp(line, "g_bar2", 6) == 0)
    {
      ...
      if (numElements < 0 ||
        static_cast<unsigned int>(numElements * this->SizeOfInt) > this->FileSize)
      {
        svtkErrorMacro("Invalid number of bar2 cells; check that ByteOrder is set correctly.");
        return -1;
      }
      if (this->ElementIdsListed)
      {
    ...
    else if (strncmp(line, "bar3", 4) == 0 || strncmp(line, "g_bar3", 6) == 0)
    {
      ...
      if (numElements < 0 ||
        static_cast<unsigned int>(numElements * this->SizeOfInt) > this->FileSize)
      {
        svtkErrorMacro("Invalid number of bar3 cells; check that ByteOrder is set correctly.");
        return -1;
      }
    ...
    {
      svtkDebugMacro("nsided");
      int* numNodesPerElement;
      int numNodes = 0;

      // cellType = svtkEnSightReader::NSIDED;
      this->ReadInt(&numElements);
      if (numElements < 0 ||
        static_cast<unsigned int>(numElements * this->SizeOfInt) > this->FileSize)
      {
        svtkErrorMacro("Invalid number of nsided cells; check that ByteOrder is set correctly.");
        return -1;
      }
      ...
      strncmp(line, "g_tria3", 7) == 0 || strncmp(line, "g_tria6", 7) == 0)
    {
      ...
      if (numElements < 0 ||
        static_cast<unsigned int>(numElements * this->SizeOfInt) > this->FileSize)
      {
        svtkErrorMacro("Invalid number of triangle cells; check that ByteOrder is set correctly.");
        return -1;
      }
      if (this->ElementIdsListed)
      {
      ...
      if (cellType == svtkEnSightReader::TRIA6)
      {
        // Skip nodeIdList.
      ...
      strncmp(line, "g_quad4", 7) == 0 || strncmp(line, "g_quad8", 7) == 0)
    {
      ...
      if (numElements < 0 ||
        static_cast<unsigned int>(numElements * this->SizeOfInt) > this->FileSize)
      {
        svtkErrorMacro("Invalid number of quad cells; check that ByteOrder is set correctly.");
        return -1;
      }
      if (this->ElementIdsListed)
      {
      ...
      if (cellType == svtkEnSightReader::QUAD8)
      {
        // Skip nodeIdList.
    ...
    else if (strncmp(line, "nfaced", 6) == 0)
    {
      ...
      if (numElements < 0 ||
        static_cast<unsigned int>(numElements * this->SizeOfInt) > this->FileSize)
      {
        svtkErrorMacro("Invalid number of nfaced cells; check that ByteOrder is set correctly.");
        return -1;
      }
      ...
      strncmp(line, "g_tetra4", 8) == 0 || strncmp(line, "g_tetra10", 9) == 0)
    {
      ...
      if (cellType == svtkEnSightReader::TETRA10)
      {
        // Skip nodeIdList.
      ...
      strncmp(line, "g_pyramid5", 10) == 0 || strncmp(line, "g_pyramid13", 11) == 0)
    {
      ...
      if (numElements < 0 ||
        static_cast<unsigned int>(numElements * this->SizeOfInt) > this->FileSize)
      {
        svtkErrorMacro("Invalid number of pyramid cells; check that ByteOrder is set correctly.");
        return -1;
      }
      if (this->ElementIdsListed)
      {
      ...
      if (cellType == svtkEnSightReader::PYRAMID13)
      {
        // Skip nodeIdList.
      ...
      strncmp(line, "g_hexa8", 7) == 0 || strncmp(line, "g_hexa20", 8) == 0)
    {
      ...
      if (numElements < 0 ||
        static_cast<unsigned int>(numElements * this->SizeOfInt) > this->FileSize)
      {
        svtkErrorMacro("Invalid number of hexahedral cells; check that ByteOrder is set correctly.");
        return -1;
      }
      if (this->ElementIdsListed)
      {
      ...
      if (cellType == svtkEnSightReader::HEXA20)
      {
        // Skip nodeIdList.
      ...
      strncmp(line, "g_penta6", 8) == 0 || strncmp(line, "g_penta15", 9) == 0)
    {
      ...
      if (numElements < 0 ||
        static_cast<unsigned int>(numElements * this->SizeOfInt) > this->FileSize)
      {
        svtkErrorMacro("Invalid number of pentagonal cells; check that ByteOrder is set correctly.");
        return -1;
      }
      if (this->ElementIdsListed)
      {
      ...
      if (cellType == svtkEnSightReader::PENTA15)
      {
        // Skip nodeIdList.
...
//----------------------------------------------------------------------------
int svtkEnSightGoldBinaryReader::SkipRectilinearGrid(char line[256])
{
  char subLine[80];
  int lineRead;
  int iblanked = 0;
  ...
  if (dimensions[0] < 0 ||
    ...
    static_cast<unsigned int>((dimensions[0] + dimensions[1] + dimensions[2]) * this->SizeOfInt) >
      this->FileSize)
  {
    svtkErrorMacro("Invalid dimensions read; check that BytetOrder is set correctly.");
    return -1;
  }
  ...
  if (iblanked)
  {
    svtkWarningMacro("SVTK does not handle blanking for rectilinear grids.");
    this->GoldIFile->seekg(sizeof(int) * numPts, ios::cur);
  }
...
//----------------------------------------------------------------------------
int svtkEnSightGoldBinaryReader::SkipImageData(char line[256])
{
  char subLine[80];
  int lineRead;
  int iblanked = 0;
  ...
  if (iblanked)
  {
    svtkWarningMacro("SVTK does not handle blanking for image data.");
    numPts = dimensions[0] * dimensions[1] * dimensions[2];
    if (dimensions[0] < 0 ||
      static_cast<unsigned int>(dimensions[0] * this->SizeOfInt) > this->FileSize ||
      dimensions[1] < 0 ||
...
//----------------------------------------------------------------------------
int svtkEnSightGoldBinaryReader::ReadMeasuredGeometryFile(
  const char* fileName, int timeStep, svtkMultiBlockDataSet* output)
{
  char line[80], subLine[80];
  svtkIdType i;
  ...
  {
    svtkByteSwap::Swap4LERange(xCoords, this->NumberOfMeasuredPoints);
    svtkByteSwap::Swap4LERange(yCoords, this->NumberOfMeasuredPoints);
    svtkByteSwap::Swap4LERange(zCoords, this->NumberOfMeasuredPoints);
  }
  else
  {
    svtkByteSwap::Swap4BERange(xCoords, this->NumberOfMeasuredPoints);
    svtkByteSwap::Swap4BERange(yCoords, this->NumberOfMeasuredPoints);
    svtkByteSwap::Swap4BERange(zCoords, this->NumberOfMeasuredPoints);
  }
...
//----------------------------------------------------------------------------
int svtkEnSightGoldBinaryReader::ReadScalarsPerNode(const char* fileName, const char* description,
...
//----------------------------------------------------------------------------
int svtkEnSightGoldBinaryReader::ReadVectorsPerNode(const char* fileName, const char* description,
...
//----------------------------------------------------------------------------
int svtkEnSightGoldBinaryReader::ReadTensorsPerNode(const char* fileName, const char* description,
...
//----------------------------------------------------------------------------
int svtkEnSightGoldBinaryReader::ReadScalarsPerElement(const char* fileName, const char* description,
...
//----------------------------------------------------------------------------
int svtkEnSightGoldBinaryReader::ReadVectorsPerElement(const char* fileName, const char* description,
  int timeStep, svtkMultiBlockDataSet* compositeOutput)
{
  char line[80];
  int partId, realId, numCells, numCellsPerElement, i, idx;
...
//----------------------------------------------------------------------------
int svtkEnSightGoldBinaryReader::ReadTensorsPerElement(const char* fileName, const char* description,
  int timeStep, svtkMultiBlockDataSet* compositeOutput)
{
  char line[80];
  int partId, realId, numCells, numCellsPerElement, i, idx;
...
//----------------------------------------------------------------------------
int svtkEnSightGoldBinaryReader::CreateUnstructuredGridOutput(
  int partId, char line[80], const char* name, svtkMultiBlockDataSet* compositeOutput)
{
  int lineRead = 1;
  int i, j;
  svtkIdType* nodeIds;
  ...
  while (lineRead && strncmp(line, "part", 4) != 0)
  {
    ...
    else if (strncmp(line, "g_point", 7) == 0)
    {
      ...
      if (numElements < 0 ||
        static_cast<unsigned int>(numElements * this->SizeOfInt) > this->FileSize)
      {
        svtkErrorMacro("Invalid number of g_point cells; check that ByteOrder is set correctly.");
        return -1;
      }
      if (this->ElementIdsListed)
      { // skip element ids.
    ...
    else if (strncmp(line, "g_bar2", 6) == 0)
    {
      ...
      if (numElements < 0 ||
        static_cast<unsigned int>(numElements * this->SizeOfInt) > this->FileSize)
      {
        svtkErrorMacro("Invalid number of g_bar2 cells; check that ByteOrder is set correctly.");
        return -1;
      }
      if (this->ElementIdsListed)
      {
    ...
    else if (strncmp(line, "g_bar3", 6) == 0)
    {
      ...
      if (numElements < 0 ||
        static_cast<unsigned int>(numElements * this->SizeOfInt) > this->FileSize)
      {
        svtkErrorMacro("Invalid number of g_bar3 cells; check that ByteOrder is set correctly.");
        return -1;
      }
    ...
    else if (strncmp(line, "nsided", 6) == 0)
    {
      ...
      for (i = 0; i < numElements; i++)
      {
        nodeIds = new svtkIdType[numNodesPerElement[i]];
        for (j = 0; j < numNodesPerElement[i]; j++)
        {
          nodeIds[j] = nodeIdList[nodeCount] - 1;
          nodeCount++;
        }
        cellId = output->InsertNextCell(SVTK_POLYGON, numNodesPerElement[i], nodeIds);
    ...
    else if (strncmp(line, "g_nsided", 8) == 0)
    {
      ...
      if (numElements < 0 ||
        static_cast<unsigned int>(numElements * this->SizeOfInt) > this->FileSize)
      {
        svtkErrorMacro("Invalid number of g_nsided cells; check that ByteOrder is set correctly.");
        return -1;
      }
    ...
    else if (strncmp(line, "nfaced", 6) == 0)
    {
      ...
      cellType = svtkEnSightReader::NFACED;
      this->ReadInt(&numElements);
      if (numElements < 0 ||
        static_cast<unsigned int>(numElements * this->SizeOfInt) > this->FileSize)
      {
      ...
      svtkIdType* faceAry = nullptr; // array of Ids describing a svtkPolyhedron
      // yyy end
      ...
      for (i = 0; i < numElements; i++)
      {
        ...
        // yyy begin
        arayIdx = 0;
        faceAry = new svtkIdType[numFacesPerElement[i] + numNodesPerElement[i]];
        ...
        cellId = output->InsertNextCell(
          SVTK_POLYHEDRON, elementNodeCount, nodeIds, numFacesPerElement[i], faceAry);
        delete[] faceAry;
        faceAry = nullptr;
        // yyy end
...
//----------------------------------------------------------------------------
int svtkEnSightGoldBinaryReader::CreateStructuredGridOutput(
  int partId, char line[80], const char* name, svtkMultiBlockDataSet* compositeOutput)
{
  char subLine[80];
  int lineRead;
...
//----------------------------------------------------------------------------
int svtkEnSightGoldBinaryReader::CreateRectilinearGridOutput(
  int partId, char line[80], const char* name, svtkMultiBlockDataSet* compositeOutput)
{
  char subLine[80];
  int lineRead;
  ...
  {
    svtkDebugMacro("creating new rectilinear grid output");
...
//----------------------------------------------------------------------------
int svtkEnSightGoldBinaryReader::CreateImageDataOutput(
  int partId, char line[80], const char* name, svtkMultiBlockDataSet* compositeOutput)
{
  char subLine[80];
  int lineRead;
...
int svtkEnSightGoldBinaryReader::ReadLine(char result[80])
{
  ...
  if (this->Fortran)
  {
    ...
    {
      svtkDebugMacro("Read (fortran) failed");
      return 0;
    }
  }
...
int svtkEnSightGoldBinaryReader::ReadPartId(int* result)
{
  // first swap like normal.
  if (this->ReadInt(result) == 0)
  {
    svtkErrorMacro("Read failed");
    return 0;
  }
  ...
  if (this->ByteOrder == FILE_UNKNOWN_ENDIAN)
  {
    int tmpLE = *result;
    int tmpBE = *result;
    svtkByteSwap::Swap4LE(&tmpLE);
    svtkByteSwap::Swap4BE(&tmpBE);
    ...
    svtkErrorMacro("Byte order could not be determined.");
    return 0;
  }
...
// Internal function to read a single integer.
// Returns zero if there was an error.
int svtkEnSightGoldBinaryReader::ReadInt(int* result)
{
  char dummy[4];
  if (this->Fortran)
  {
    ...
    {
      svtkErrorMacro("Read (fortran) failed.");
      return 0;
    }
  }
  ...
  if (this->ByteOrder == FILE_LITTLE_ENDIAN)
  {
    svtkByteSwap::Swap4LE(result);
  }
  else if (this->ByteOrder == FILE_BIG_ENDIAN)
  {
    svtkByteSwap::Swap4BE(result);
  }
...
// Returns zero if there was an error.
int svtkEnSightGoldBinaryReader::ReadIntArray(int* result, int numInts)
{
  if (numInts <= 0)
  {
    return 1;
  ...
  if (this->ByteOrder == FILE_LITTLE_ENDIAN)
  {
    svtkByteSwap::Swap4LERange(result, numInts);
  }
  else
  {
    svtkByteSwap::Swap4BERange(result, numInts);
  }
...
// Internal function to read a single svtkTypeInt64.
// Returns zero if there was an error.
int svtkEnSightGoldBinaryReader::ReadLong(svtkTypeInt64* result)
{
  char dummy[4];
  if (this->Fortran)
  {
  ...
  if (!this->GoldIFile->read((char*)result, sizeof(svtkTypeInt64)))
  {
    svtkErrorMacro("Read failed");
    return 0;
  }
  ...
  if (this->ByteOrder == FILE_LITTLE_ENDIAN)
  {
    svtkByteSwap::Swap8LE(result);
  }
  else if (this->ByteOrder == FILE_BIG_ENDIAN)
  {
    svtkByteSwap::Swap8BE(result);
  }
...
// Returns zero if there was an error.
int svtkEnSightGoldBinaryReader::ReadFloatArray(float* result, int numFloats)
{
  if (numFloats <= 0)
  {
    return 1;
  ...
  if (this->ByteOrder == FILE_LITTLE_ENDIAN)
  {
    svtkByteSwap::Swap4LERange(result, numFloats);
  }
  else
  {
    svtkByteSwap::Swap4BERange(result, numFloats);
  }
...
//----------------------------------------------------------------------------
void svtkEnSightGoldBinaryReader::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
int svtkEnSightGoldBinaryReader::SeekToCachedTimeStep(const char* fileName, int realTimeStep)
{
  typedef svtkEnSightGoldBinaryReader::FileOffsetMapInternal MapType;
...
void svtkEnSightGoldBinaryReader::AddTimeStepToCache(
  const char* fileName, int realTimeStep, svtkTypeInt64 address)
{
  if (this->FileOffsets->Map.find(fileName) == this->FileOffsets->Map.end())
  {
    std::map<int, svtkTypeInt64> tsMap;
...
//----------------------------------------------------------------------------
void svtkEnSightGoldBinaryReader::AddFileIndexToCache(const char* fileName)
{
  ...
  if (this->FileOffsets->Map.find(fileName) == this->FileOffsets->Map.end())
  {
    ...
    // Read the last 80 characters (+ a svtkTypeInt64) of the file and check for FILE_INDEX
    svtkIdType seekOffset = (svtkIdType(-80) * static_cast<svtkIdType>(sizeof(char))) -
      static_cast<svtkIdType>(sizeof(svtkTypeInt64));
    ...
    if (strncmp(line, "FILE_INDEX", 10) == 0)
    {
      ...
      {
        this->ReadLong(&addr);
        // The file index points at the description line, while SVTK points at BEGIN TIMESTEP
