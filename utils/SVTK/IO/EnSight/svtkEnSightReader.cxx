...
//----------------------------------------------------------------------------
typedef std::vector<svtkSmartPointer<svtkIdList> > svtkEnSightReaderCellIdsTypeBase;
class svtkEnSightReaderCellIdsType : public svtkEnSightReaderCellIdsTypeBase
{
};

//----------------------------------------------------------------------------
svtkEnSightReader::svtkEnSightReader()
{
  ...
  this->UnstructuredPartIds = svtkIdList::New();
  ...
  this->VariableTimeSetIds = svtkIdList::New();
  this->ComplexVariableTimeSetIds = svtkIdList::New();
  this->VariableFileSetIds = svtkIdList::New();
  this->ComplexVariableFileSetIds = svtkIdList::New();

  this->TimeSetFileNameNumbers = svtkIdListCollection::New();
  this->TimeSetsWithFilenameNumbers = svtkIdList::New();
  this->TimeSets = svtkDataArrayCollection::New();
  this->FileSetFileNameNumbers = svtkIdListCollection::New();
  this->FileSetsWithFilenameNumbers = svtkIdList::New();
  this->FileSetNumberOfSteps = svtkIdListCollection::New();

  this->TimeSetIds = svtkIdList::New();
  this->FileSets = svtkIdList::New();
...
//----------------------------------------------------------------------------
void svtkEnSightReader::ClearForNewCaseFileName()
{
  this->UnstructuredPartIds->Reset();
  svtkGenericEnSightReader::ClearForNewCaseFileName();
}

//----------------------------------------------------------------------------
int svtkEnSightReader::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** svtkNotUsed(inputVector), svtkInformationVector* outputVector)
{
  svtkDebugMacro("In execute ");
  ...
  if (outInfo->Has(svtkStreamingDemandDrivenPipeline::UPDATE_TIME_STEP()) && tsLength > 0)
  {
  ...
  svtkDebugMacro("Executing with: " << this->ActualTimeValue);

  int i, timeSet, fileSet, timeStep, timeStepInFile, fileNum;
  svtkDataArray* times;
  svtkIdList *numStepsList, *filenameNumbers;
  float newTime;
  int numSteps;
  char* fileName;
  int filenameNum;
  ...
  {
    svtkErrorMacro("error reading case file");
    return 0;
  }
    ...
    if (this->UseTimeSets)
    {
      ...
      if (timeSet >= 0)
      {
        ...
        if (this->UseFileSets)
        {
          ...
          numStepsList =
            static_cast<svtkIdList*>(this->FileSetNumberOfSteps->GetItemAsObject(fileSet));
  ...
  if (this->MeasuredFileName)
  {
    ...
    {
      svtkErrorMacro("error reading measured geometry file");
      delete[] fileName;
      return 0;
    }
    delete[] fileName;
  ...
  if ((this->NumberOfVariables + this->NumberOfComplexVariables) > 0)
  {
    ...
    {
      svtkErrorMacro("error reading variable files");
      return 0;
    }
  }
...
//----------------------------------------------------------------------------
int svtkEnSightReader::RequestInformation(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** svtkNotUsed(inputVector), svtkInformationVector* outputVector)
{
  svtkDebugMacro("In execute information");
  ...
  if (this->GetTimeSets())
  {
    ...
    {
      svtkDataArray* array = this->GetTimeSets()->GetItem(i);
      if (array)
      {
  ...
  if (!timeValues.empty())
  {
    ...
    if (numTimeValues > 0)
    {
      svtkInformation* outInfo = outputVector->GetInformationObject(0);
      outInfo->Set(
        svtkStreamingDemandDrivenPipeline::TIME_STEPS(), &uniqueTimeValues[0], numTimeValues);
      double timeRange[2];
...
//----------------------------------------------------------------------------
int svtkEnSightReader::ReadCaseFileGeometry(char* line)
{
  char subLine[256];
      ...
      strncmp(line, "rigid_body:", 11) == 0))
  {
    if (strncmp(line, "model:", 6) == 0)
    {
      if (sscanf(line, " %*s %d%*[ \t]%d%*[ \t]%s", &timeSet, &fileSet, subLine) == 3)
      {
        ...
        svtkDebugMacro(<< this->GetGeometryFileName());
      }
    ...
    else if (strncmp(line, "measured:", 9) == 0)
    {
      if (sscanf(line, " %*s %d%*[ \t]%d%*[ \t]%s", &timeSet, &fileSet, subLine) == 3)
      {
        ...
        svtkDebugMacro(<< this->GetMeasuredFileName());
      }
      else if (sscanf(line, " %*s %d%*[ \t]%s", &timeSet, subLine) == 2)
      {
        this->MeasuredTimeSet = timeSet;
    ...
    else if (strncmp(line, "match:", 6) == 0)
    {
      ...
      svtkDebugMacro(<< this->GetMatchFileName());
    }
    lineRead = this->ReadNextDataLine(line);
...
//----------------------------------------------------------------------------
int svtkEnSightReader::ReadCaseFileVariable(char* line)
{
    ...
    strncmp(line, "FILE", 4) != 0)
  {
    ...
    else if (strncmp(line, "scalar", 6) == 0)
    {
      ...
      {
        svtkDebugMacro("scalar per node");
        this->VariableMode = svtkEnSightReader::SCALAR_PER_NODE;
      ...
      {
        svtkDebugMacro("scalar per element");
        this->VariableMode = svtkEnSightReader::SCALAR_PER_ELEMENT;
        if (sscanf(line, " %*s %*s %*s %d %d %s", &timeSet, &fileSet, subLine) == 3)
        {
          this->VariableTimeSetIds->InsertNextId(timeSet);
          this->VariableFileSetIds->InsertNextId(fileSet);
      ...
      {
        svtkDebugMacro("scalar per measured node");
        this->VariableMode = svtkEnSightReader::SCALAR_PER_MEASURED_NODE;
    ...
    else if (strncmp(line, "vector", 6) == 0)
    {
      ...
      if (strcmp(subLine, "node:") == 0)
      {
        svtkDebugMacro("vector per node");
        this->VariableMode = svtkEnSightReader::VECTOR_PER_NODE;
        if (sscanf(line, " %*s %*s %*s %d %d %s", &timeSet, &fileSet, subLine) == 3)
        {
          this->VariableTimeSetIds->InsertNextId(timeSet);
          this->VariableFileSetIds->InsertNextId(fileSet);
      ...
      {
        svtkDebugMacro("vector per element");
        this->VariableMode = svtkEnSightReader::VECTOR_PER_ELEMENT;
        if (sscanf(line, " %*s %*s %*s %d %d %s", &timeSet, &fileSet, subLine) == 3)
        {
          this->VariableTimeSetIds->InsertNextId(timeSet);
          this->VariableFileSetIds->InsertNextId(fileSet);
      ...
      {
        svtkDebugMacro("vector per measured node");
        this->VariableMode = svtkEnSightReader::VECTOR_PER_MEASURED_NODE;
        if (sscanf(line, " %*s %*s %*s %*s %d %d %s", &timeSet, &fileSet, subLine) == 3)
        {
          this->VariableTimeSetIds->InsertNextId(timeSet);
          this->VariableFileSetIds->InsertNextId(fileSet);
    ...
    else if (strncmp(line, "tensor", 6) == 0)
    {
      ...
      {
        svtkErrorMacro("Error while reading: " << line);
      }
      if (!(strcmp(symm, "symm") == 0 && strcmp(per, "per") == 0))
      {
        ...
        if (strcmp(per, "per") == 0)
        {
          ...
          svtkWarningMacro("Looks almost like a valid case file, continuing");
        }
        else
        {
          svtkErrorMacro("Trouble reading: " << line);
        }
      }
      if (strcmp(subLine, "node:") == 0)
      {
        svtkDebugMacro("tensor symm per node");
        this->VariableMode = svtkEnSightReader::TENSOR_SYMM_PER_NODE;
        if (sscanf(line, " %*s %*s %*s %*s %d %d %s", &timeSet, &fileSet, subLine) == 3)
        {
          this->VariableTimeSetIds->InsertNextId(timeSet);
          this->VariableFileSetIds->InsertNextId(fileSet);
      ...
      {
        svtkDebugMacro("tensor symm per element");
        this->VariableMode = svtkEnSightReader::TENSOR_SYMM_PER_ELEMENT;
        if (sscanf(line, " %*s %*s %*s %*s %d %d %s", &timeSet, &fileSet, subLine) == 3)
        {
          this->VariableTimeSetIds->InsertNextId(timeSet);
          this->VariableFileSetIds->InsertNextId(fileSet);
      ...
      {
        svtkErrorMacro("Unknown type, faulty line was:" << line);
      }
      this->AddVariableFileName(subLine);
      this->NumberOfVariables++;
    }
    else if (strncmp(line, "complex", 6) == 0)
    {
      ...
      if (strcmp(subLine, "scalar") == 0)
      {
        ...
        if (strcmp(subLine, "node:") == 0)
        {
          svtkDebugMacro("complex scalar per node");
          this->VariableMode = svtkEnSightReader::COMPLEX_SCALAR_PER_NODE;
          if (sscanf(line, " %*s %*s %*s %*s %d %d %s", &timeSet, &fileSet, subLine) == 3)
          {
        ...
        {
          svtkDebugMacro("complex scalar per element");
          this->VariableMode = svtkEnSightReader::COMPLEX_SCALAR_PER_ELEMENT;
          if (sscanf(line, " %*s %*s %*s %*s %d %d %s", &timeSet, &fileSet, subLine) == 3)
          {
            this->ComplexVariableTimeSetIds->InsertNextId(timeSet);
            this->ComplexVariableFileSetIds->InsertNextId(fileSet);
      ...
      else if (strcmp(subLine, "vector") == 0)
      {
        ...
        if (strcmp(subLine, "node:") == 0)
        {
          svtkDebugMacro("complex vector per node");
          this->VariableMode = svtkEnSightReader::COMPLEX_VECTOR_PER_NODE;
          if (sscanf(line, " %*s %*s %*s %*s %d %d %s", &timeSet, &fileSet, subLine) == 3)
          {
            this->ComplexVariableTimeSetIds->InsertNextId(timeSet);
            this->ComplexVariableFileSetIds->InsertNextId(fileSet);
        ...
        {
          svtkDebugMacro("complex vector per element");
          this->VariableMode = svtkEnSightReader::COMPLEX_VECTOR_PER_ELEMENT;
          if (sscanf(line, " %*s %*s %*s %*s %d %d %s", &timeSet, &fileSet, subLine) == 3)
          {
            this->ComplexVariableTimeSetIds->InsertNextId(timeSet);
            this->ComplexVariableFileSetIds->InsertNextId(fileSet);
    ...
    {
      svtkErrorMacro("invalid VARIABLE line: " << line);
      delete this->IS;
      this->IS = nullptr;
      return 0;
    }
...
//----------------------------------------------------------------------------
int svtkEnSightReader::ReadCaseFileTime(char* line)
{
  char formatLine[256];
  char subLine[256];
    ...
    strncmp(line, "FILE", 4) != 0)
  {
    ...
    {
      svtkErrorMacro("Error with svtkEnSightReader: 'time set' not found!!!");
      return 0;
    }
    ...
    {
      svtkErrorMacro("Error with svtkEnSightReader: 'number of steps' not found!!!");
      return 0;
    }
    ...
    if (this->ReadNextDataLine(line) == 0)
    {
      svtkErrorMacro("Error with svtkEnSightReader: 'filename ......' not found!!!");
      return 0;
    }

    if (strncmp(line, "filename", 8) == 0)
    {
      svtkIdList* filenameNumbers = svtkIdList::New();
      ...
      if (strncmp(subLine, "numbers", 7) == 0)
      {
        ...
        if (sscanf(line, "%*s %*s %d", &filenameNum) != 1)
        {
          ...
          if (this->ReadNextDataLine(line) == 0)
          {
            svtkErrorMacro("Error with svtkEnSightReader: filename numbers not found!!!");
            return 0;
          }
        ...
        for (i = 0; i < numTimeSteps; i++)
        {
          ...
          while (sscanf(line, formatLine, &filenameNum) != 1)
          {
            if (this->ReadNextDataLine(line) == 0)
            {
              svtkErrorMacro("Error with svtkEnSightReader: insufficient filename numbers!!!");
              return 0;
            }
      ...
      // 'filename increment: <int>'
      {
        ...
        {
          svtkErrorMacro("Error with svtkEnSightReader: 'filename start number' not found!!!");
          return 0;
        }

        if (this->ReadNextDataLine(line) == 0)
        {
          svtkErrorMacro("Error with svtkEnSightReader: 'filename increment' not found!!!");
          return 0;
        }
      ...
      if (lineRead == 0)
      {
        svtkErrorMacro("Error with svtkEnSightReader: 'time values' not found!!!");
        return 0;
      }
    }

    // 'time values:' --- to obtain timeStep(s)
    svtkFloatArray* timeValues = svtkFloatArray::New();
    ...
    for (i = 0; i < numTimeSteps; i++)
    {
      ...
      while (sscanf(line, formatLine, &timeStep) != 1)
      {
        if (this->ReadNextDataLine(line) == 0)
        {
          svtkErrorMacro("Error with svtkEnSightReader: insufficient time values!!!");
          return 0;
        }
...
//----------------------------------------------------------------------------
int svtkEnSightReader::ReadCaseFileFile(char* line)
{
  ...
  while (lineRead && strncmp(line, "FORMAT", 6) != 0 && strncmp(line, "GEOMETRY", 8) != 0 &&
    ...
    strncmp(line, "FILE", 4) != 0)
  {
    svtkIdList* filenameNums = svtkIdList::New();
    svtkIdList* numSteps = svtkIdList::New();
...
//----------------------------------------------------------------------------
int svtkEnSightReader::ReadCaseFile()
{
  char line[256];
  char subLine[256];
  int stringRead;
  int i;
  int ret;
  svtkDebugMacro("In svtkEnSightReader::ReadCaseFile");

  // Initialize
  //
  if (!this->CaseFileName)
  {
    svtkErrorMacro("A CaseFileName must be specified.");
    return 0;
  }
  ...
  if (strncmp(line, "FORMAT", 6) == 0)
  {
    ...
    if (stringRead == 1)
    {
      if (strcmp(subLine, "gold") == 0 && strcmp(this->GetClassName(), "svtkEnSight6Reader") == 0)
      {
        // The class is svtkEnSight6Reader, but the case file says "gold".
        svtkErrorMacro("This is not an EnSight6 file.");
        delete this->IS;
        this->IS = nullptr;
        return 0;
      }
    ...
    {
      if (strcmp(this->GetClassName(), "svtkEnSightGoldReader") == 0)
      {
        // The class is svtkEnSightGoldReader, but the case file does
        // not say "gold".
        svtkErrorMacro("This is not an EnSight Gold file.");
        delete this->IS;
        this->IS = nullptr;
        return 0;
      }
  ...
  while (ret)
  {
    ...
    else if (strncmp(line, "VARIABLE", 8) == 0)
    {
      ...
      svtkDebugMacro(<< "*** VARIABLE section");
...
//----------------------------------------------------------------------------
int svtkEnSightReader::ReadVariableFiles(svtkMultiBlockDataSet* output)
{
  int i, j;
  ...
  for (i = 0; i < this->NumberOfVariables; i++)
  {
    ...
    if (this->UseTimeSets)
    {
      ...
      if (this->UseFileSets)
      {
        ...
        numStepsList = static_cast<svtkIdList*>(
...
//----------------------------------------------------------------------------
void svtkEnSightReader::AddVariableFileName(const char* fileName1, const char* fileName2)
{
  int size;
  int i;
  ...
  if (this->VariableMode < 8)
  {
    ...
    svtkDebugMacro(<< "file name: " << this->VariableFileNames[size]);
  }
  else
  {
    ...
    svtkDebugMacro("real file name: " << this->ComplexVariableFileNames[2 * size]);
    ...
    svtkDebugMacro("imag. file name: " << this->ComplexVariableFileNames[2 * size + 1]);
  }
}

//----------------------------------------------------------------------------
void svtkEnSightReader::AddVariableDescription(const char* description)
{
  int size;
  int i;
...
//----------------------------------------------------------------------------
void svtkEnSightReader::AddVariableType()
{
  int size;
  int i;
  int* types = nullptr;
...
//----------------------------------------------------------------------------
int svtkEnSightReader::GetSectionType(const char* line)
{
  if (strncmp(line, "coordinates", 5) == 0)
  {
    return svtkEnSightReader::COORDINATES;
  }
  else if (strncmp(line, "block", 4) == 0)
  {
    return svtkEnSightReader::BLOCK;
  }
  else if (this->GetElementType(line) != -1)
  {
    return svtkEnSightReader::ELEMENT;
  }
  else
  {
    return -1;
...
//----------------------------------------------------------------------------
int svtkEnSightReader::GetElementType(const char* line)
{
  if (strncmp(line, "point", 5) == 0)
  {
    return svtkEnSightReader::POINT;
  }
  else if (strncmp(line, "bar2", 4) == 0)
  {
    return svtkEnSightReader::BAR2;
  }
  else if (strncmp(line, "bar3", 4) == 0)
  {
    return svtkEnSightReader::BAR3;
  }
  else if (strncmp(line, "nsided", 6) == 0)
  {
    return svtkEnSightReader::NSIDED;
  }
  else if (strncmp(line, "tria3", 5) == 0)
  {
    return svtkEnSightReader::TRIA3;
  }
  else if (strncmp(line, "tria6", 5) == 0)
  {
    return svtkEnSightReader::TRIA6;
  }
  else if (strncmp(line, "quad4", 5) == 0)
  {
    return svtkEnSightReader::QUAD4;
  }
  else if (strncmp(line, "quad8", 5) == 0)
  {
    return svtkEnSightReader::QUAD8;
  }
  else if (strncmp(line, "nfaced", 6) == 0)
  {
    return svtkEnSightReader::NFACED;
  }
  else if (strncmp(line, "tetra4", 6) == 0)
  {
    return svtkEnSightReader::TETRA4;
  }
  else if (strncmp(line, "tetra10", 7) == 0)
  {
    return svtkEnSightReader::TETRA10;
  }
  else if (strncmp(line, "pyramid5", 8) == 0)
  {
    return svtkEnSightReader::PYRAMID5;
  }
  else if (strncmp(line, "pyramid13", 9) == 0)
  {
    return svtkEnSightReader::PYRAMID13;
  }
  else if (strncmp(line, "hexa8", 5) == 0)
  {
    return svtkEnSightReader::HEXA8;
  }
  else if (strncmp(line, "hexa20", 6) == 0)
  {
    return svtkEnSightReader::HEXA20;
  }
  else if (strncmp(line, "penta6", 6) == 0)
  {
    return svtkEnSightReader::PENTA6;
  }
  else if (strncmp(line, "penta15", 7) == 0)
  {
    return svtkEnSightReader::PENTA15;
  }
  else
  {
    return -1;
...
void svtkEnSightReader::ReplaceWildcards(char* filename, int num)
{
...
//----------------------------------------------------------------------------
void svtkEnSightReader::RemoveLeadingBlanks(char* line)
{
  int count = 0;
  while (line[count] == ' ')
  {
...
//----------------------------------------------------------------------------
svtkIdList* svtkEnSightReader::GetCellIds(int index, int cellType)
{
  // Check argument range.
  if (cellType < POINT || cellType >= NUMBER_OF_ELEMENT_TYPES)
  {
    svtkErrorMacro("Cell type " << cellType << " out of range.  Only " << NUMBER_OF_ELEMENT_TYPES - 1
                               << " types exist.");
    return nullptr;
  }
  if (index < 0 || index > this->UnstructuredPartIds->GetNumberOfIds())
  {
    svtkErrorMacro("Index " << index << " out of range.  Only "
  ...
  if (!this->CellIds)
  {
    this->CellIds = new svtkEnSightReaderCellIdsType;
  }

  // Get the index of the actual svtkIdList requested.
  ...
  // Make sure this svtkIdList exists.
  if (!(*this->CellIds)[cellIdsIndex])
  {
    svtkIdList* nl = svtkIdList::New();
    (*this->CellIds)[cellIdsIndex] = nl;
    nl->Delete();
  }

  // Return the requested svtkIdList.
  return (*this->CellIds)[cellIdsIndex];
}

//----------------------------------------------------------------------------
void svtkEnSightReader::AddToBlock(
  svtkMultiBlockDataSet* output, unsigned int blockNo, svtkDataSet* dataset)
{
  svtkDataObject* blockDO = output->GetBlock(blockNo);
  if (blockDO)
  {
    svtkErrorMacro("Block already has a svtkDataSet assigned to it.");
    return;
  }
...
//----------------------------------------------------------------------------
svtkDataSet* svtkEnSightReader::GetDataSetFromBlock(
  svtkMultiBlockDataSet* output, unsigned int blockno)
{
  return svtkDataSet::SafeDownCast(output->GetBlock(blockno));
}

//----------------------------------------------------------------------------
void svtkEnSightReader::SetBlockName(
  svtkMultiBlockDataSet* output, unsigned int blockNo, const char* name)
{
  output->GetMetaData(blockNo)->Set(svtkCompositeDataSet::NAME(), name);
}

//----------------------------------------------------------------------------
void svtkEnSightReader::PrintSelf(ostream& os, svtkIndent indent)
{
