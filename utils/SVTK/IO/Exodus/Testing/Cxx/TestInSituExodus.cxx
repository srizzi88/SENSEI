...
#include "svtkCPExodusIIInSituReader.h"
...
bool readExodusCopy(std::string fileName, svtkMultiBlockDataSet* mbds)
{
  ...
  int numNodeArrays = reader->GetNumberOfObjectArrays(svtkExodusIIReader::NODAL);
  for (int i = 0; i < numNodeArrays; ++i)
  {
    reader->SetObjectArrayStatus(svtkExodusIIReader::NODAL, i, 1);
  }

  // Enable all element result (cell data) arrays
  int numElementBlockArrays = reader->GetNumberOfObjectArrays(svtkExodusIIReader::ELEM_BLOCK);
  for (int i = 0; i < numElementBlockArrays; ++i)
  {
    reader->SetObjectArrayStatus(svtkExodusIIReader::ELEM_BLOCK, i, 1);
  }
...
svtkUnstructuredGridBase* getConnectivityBlock(svtkMultiBlockDataSet* mbds)
{
  svtkUnstructuredGridBase* result = nullptr;
  if (svtkDataObject* tmpDO = mbds->GetBlock(0))
  {
    if (svtkMultiBlockDataSet* tmpMBDS = svtkMultiBlockDataSet::SafeDownCast(tmpDO))
    {
      result = svtkUnstructuredGridBase::SafeDownCast(tmpMBDS->GetBlock(0));
    }
  }
  return result;
}
...
bool compareDataSets(svtkDataSet* ref, svtkDataSet* test)
{
  // Compare number of points
  svtkIdType refNumPoints = ref->GetNumberOfPoints();
  svtkIdType testNumPoints = test->GetNumberOfPoints();
  ...
  for (svtkIdType pointId = 0; pointId < testNumPoints; ++pointId)
  {
  ...
  svtkPointData* refPointData = ref->GetPointData();
  svtkPointData* testPointData = test->GetPointData();
  ...
  {
    svtkDataArray* testArray = testPointData->GetArray(arrayIndex);
    const char* arrayName = testArray->GetName();
    svtkDataArray* refArray = refPointData->GetArray(arrayName);
    if (refArray == nullptr)
    {
#ifdef GLOM_WORKAROUND
      cerr << "Warning: "
    ...
    svtkIdType refNumTuples = refArray->GetNumberOfTuples();
    svtkIdType testNumTuples = testArray->GetNumberOfTuples();
    ...
    for (svtkIdType i = 0; i < testNumTuples; ++i)
    {
  ...
  svtkIdType refNumCells = ref->GetNumberOfCells();
  svtkIdType testNumCells = test->GetNumberOfCells();
  ...
  svtkNew<svtkGenericCell> refCell;
  svtkNew<svtkGenericCell> testCell;

  // Test out the iterators, too:
  svtkSmartPointer<svtkCellIterator> refCellIter =
    svtkSmartPointer<svtkCellIterator>::Take(ref->NewCellIterator());
  svtkSmartPointer<svtkCellIterator> testCellIter =
    svtkSmartPointer<svtkCellIterator>::Take(test->NewCellIterator());

  for (svtkIdType cellId = 0; cellId < testNumCells && !refCellIter->IsDoneWithTraversal() &&
  ...
  // "gloms" multi-component arrays together, while the in-situ doesn't (yet?).
  svtkCellData* refCellData = ref->GetCellData();
  svtkCellData* testCellData = test->GetCellData();
  ...
  {
    svtkDataArray* testArray = testCellData->GetArray(arrayIndex);
    const char* arrayName = testArray->GetName();
    svtkDataArray* refArray = refCellData->GetArray(arrayName);
    if (refArray == nullptr)
    {
#ifdef GLOM_WORKAROUND
      cerr << "Warning: "
...
void populateAttributes(svtkDataSet* ref, svtkDataSet* test)
{
  svtkIdType numPoints = ref->GetNumberOfPoints();

  // Create/set scalars for the filters
  svtkNew<svtkDoubleArray> refScalars;
  ...
  svtkNew<svtkCPExodusIIResultsArrayTemplate<double> > testScalars;
  ...
  svtkNew<svtkFloatArray> refNormals;
  ...
  svtkNew<svtkCPExodusIIResultsArrayTemplate<double> > testNormals;
...
void testContourFilter(svtkUnstructuredGridBase* input, svtkDataSet*& output, double& time)
{
...
void testDataSetSurfaceFilter(svtkUnstructuredGridBase* input, svtkDataSet*& output, double& time)
{
  svtkNew<svtkTimerLog> timer;
  svtkNew<svtkDataSetSurfaceFilter> extractSurface;
...
void testCutterFilter(svtkUnstructuredGridBase* input, svtkDataSet*& output, double& time)
{
  svtkNew<svtkTimerLog> timer;

  // Create plane for testing slicing
  svtkNew<svtkPlane> slicePlane;
...
void testExtractGeometryFilter(svtkUnstructuredGridBase* input, svtkDataSet*& output, double& time)
{
  svtkNew<svtkTimerLog> timer;
  ...
  svtkNew<svtkExtractGeometry> extract;
...
void testGlyph3DFilter(svtkUnstructuredGridBase* input, svtkDataSet*& output, double& time)
{
  svtkNew<svtkTimerLog> timer;
...
void testWarpScalarFilter(svtkUnstructuredGridBase* input, svtkDataSet*& output, double& time)
{
  svtkNew<svtkTimerLog> timer;
  svtkNew<svtkWarpScalar> warpScalar;
...
void testWarpVectorFilter(svtkUnstructuredGridBase* input, svtkDataSet*& output, double& time)
{
  svtkNew<svtkTimerLog> timer;
  svtkNew<svtkWarpVector> warpVector;
...
void testPipeline(svtkUnstructuredGridBase* input, svtkDataSet*& output, double& time)
{
  svtkNew<svtkTimerLog> timer;
...
bool validateFilterOutput(const std::string& name, svtkDataSet*& refOutput, svtkDataSet*& testOutput)
{
...
bool testFilters(svtkUnstructuredGridBase* ref, svtkUnstructuredGridBase* test)
{
  ...
  svtkDataSet* refOutput(nullptr);
  svtkDataSet* testOutput(nullptr);
  ...
  if (!testOutput->IsA("svtkCPExodusIIElementBlock"))
  {
...
bool testCopies(svtkUnstructuredGridBase* test)
{
  svtkNew<svtkUnstructuredGrid> svtkTarget;
  svtkSmartPointer<svtkUnstructuredGridBase> mappedTarget =
    svtkSmartPointer<svtkUnstructuredGridBase>::Take(test->NewInstance());
  ...
  // Deep copy: test --> svtk
  svtkTarget->DeepCopy(test);
  if (!compareDataSets(test, svtkTarget))
  {
    FAILB("Deep copy insitu --> SVTK failed.")
  }
  svtkTarget->Reset();

  // Shallow copy: test --> svtk
  svtkTarget->ShallowCopy(test); // Should really deep copy.
  if (!compareDataSets(test, svtkTarget))
  {
    FAILB("Shallow copy insitu --> SVTK failed.")
  }
  svtkTarget->Reset();
...
int TestInSituExodus(int argc, char* argv[])
{
  ...
  char* fileNameC = svtkTestUtilities::ExpandDataFileName(argc, argv, "Data/box-noglom.ex2");
  std::string fileName(fileNameC);
  delete[] fileNameC;

  // Read reference copy
  svtkNew<svtkMultiBlockDataSet> refMBDS;
  readExodusCopy(fileName, refMBDS);
  svtkUnstructuredGridBase* refGrid(getConnectivityBlock(refMBDS));
  if (!refGrid)
  {
  ...
  // Read in-situ copy
  svtkNew<svtkCPExodusIIInSituReader> reader;
  reader->SetFileName(fileName.c_str());
  reader->Update();
  svtkMultiBlockDataSet* testMBDS = reader->GetOutput();
  svtkUnstructuredGridBase* grid(getConnectivityBlock(testMBDS));
  if (!grid)
  {
