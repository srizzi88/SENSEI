...
rdr.SetFileName(str(SVTK_DATA_ROOT) + "/Data/dodecahedron.exo")
rdr.Update()

tfm = svtkTransformFilter()
xfm = svtkTransform()
...
rd2 = svtkExodusIIReader()
rd2.SetFileName(str(SVTK_DATA_ROOT) + "/Data/cube-1.exo")
rd2.Update()

shr = svtkShrinkFilter()
...
rr = svtkRenderer()
ac = svtkActor()
dm = svtkDataSetMapper()
ac.SetMapper(dm)
dm.SetInputData(tfm.GetOutput())
rr.AddActor(ac)
a2 = svtkActor()
d2 = svtkDataSetMapper()
a2.SetMapper(d2)
