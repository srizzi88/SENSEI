/*=========================================================================
  ...
  Module:    svtkCPExodusInSituReader.cxx
...
svtkStandardNewMacro(svtkCPExodusIIInSituReader);

//------------------------------------------------------------------------------
void svtkCPExodusIIInSituReader::PrintSelf(ostream& os, svtkIndent indent)
{
...
//------------------------------------------------------------------------------
svtkCPExodusIIInSituReader::svtkCPExodusIIInSituReader()
  : FileName(nullptr)
  , FileId(-1)
...
//------------------------------------------------------------------------------
svtkTypeBool svtkCPExodusIIInSituReader::ProcessRequest(
...
//------------------------------------------------------------------------------
int svtkCPExodusIIInSituReader::RequestData(
  svtkInformation*, svtkInformationVector**, svtkInformationVector* outputVector)
{
  ...
  //
  // output                             svtkMultiBlockDataSet
  //   - Block 0: this->ElementBlocks   svtkMultiBlockDataSet
  //     - Block N: Element blocks      svtkCPExodusIIElementBlock
...
//------------------------------------------------------------------------------
int svtkCPExodusIIInSituReader::RequestInformation(
...
//------------------------------------------------------------------------------
bool svtkCPExodusIIInSituReader::ExOpen()
{
...
//------------------------------------------------------------------------------
bool svtkCPExodusIIInSituReader::ExGetMetaData()
{
  // Generic metadata:
  ...
  if (error < 0)
  {
    svtkErrorMacro("Error retrieving file metadata.");
    return false;
  }
  ...
  if (error < 0)
  {
    svtkErrorMacro("Error retrieving number of nodal variables.");
    return false;
  }
  ...
  for (int i = 0; i < numNodalVars; ++i)
  {
    ...
    if (error < 0)
    {
      svtkErrorMacro("Error retrieving nodal variable name at index" << i);
      return false;
    }
  ...
  if (error < 0)
  {
    svtkErrorMacro("Error retrieving number of element variables.");
    return false;
  }
  ...
  for (int i = 0; i < numElemVars; ++i)
  {
    ...
    if (error < 0)
    {
      svtkErrorMacro("Error retrieving element variable name at index" << i);
      return false;
    }
    // Trim excess null chars from the strings:
  ...
  if (error < 0)
  {
    svtkErrorMacro("Failed to get the element block ids.");
    return false;
  }
  ...
  if (error < 0)
  {
    svtkErrorMacro("Error retrieving the number of timesteps.");
    return false;
  }
  ...
  if (numTimeSteps > 0)
  {
    ...
    if (error < 0)
    {
      svtkErrorMacro("Error retrieving timestep array.");
      return false;
    }
  }
  return true;
...
//------------------------------------------------------------------------------
bool svtkCPExodusIIInSituReader::ExGetCoords()
{
  this->Points->Reset();
  svtkNew<svtkCPExodusIINodalCoordinatesTemplate<double> > nodeCoords;
  ...
  if (error < 0)
  {
    ...
    delete[] y;
    delete[] z;
    svtkErrorMacro("Error retrieving coordinates.");
    return false;
  }
...
//------------------------------------------------------------------------------
bool svtkCPExodusIIInSituReader::ExGetNodalVars()
{
  ...
  for (int nodalVarIndex = 0; nodalVarIndex < numNodalVars; ++nodalVarIndex)
  {
    ...
    svtkNew<svtkCPExodusIIResultsArrayTemplate<double> > nodalVarArray;
...
//------------------------------------------------------------------------------
bool svtkCPExodusIIInSituReader::ExGetElemBlocks()
{
  ...
  for (int blockInd = 0; blockInd < numElemBlk; ++blockInd)
  {
    ...
    if (error < 0)
    {
      svtkErrorMacro("Failed to get the element block metadata for block " << blockInd);
      return false;
    }

    // Get element block connectivity
    svtkNew<svtkCPExodusIIElementBlock> block;
    ...
    if (error < 0)
    {
      svtkErrorMacro("Failed to get the connectivity for block " << blockInd);
      return false;
    }
    ...
    for (int elemVarIndex = 0; elemVarIndex < numElemVars; ++elemVarIndex)
    {
      ...
      svtkNew<svtkCPExodusIIResultsArrayTemplate<double> > elemVarArray;
      elemVarArray->SetExodusScalarArrays(varsVector, numElem);
      elemVarArray->SetName(this->ElementVariableNames[elemVarIndex].c_str());

      if (error < 0)
      {
        svtkErrorMacro("Failed to read element block variable array '"
...
//------------------------------------------------------------------------------
void svtkCPExodusIIInSituReader::ExClose()
{
