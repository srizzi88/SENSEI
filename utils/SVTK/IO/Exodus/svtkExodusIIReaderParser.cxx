/*=========================================================================
  ...
  Program:   ParaView
  Module:    svtkExodusIIReaderParser.cxx
...
svtkStandardNewMacro(svtkExodusIIReaderParser);
//-----------------------------------------------------------------------------
svtkExodusIIReaderParser::svtkExodusIIReaderParser()
{
...
//-----------------------------------------------------------------------------
void svtkExodusIIReaderParser::StartElement(const char* tagName, const char** attrs)
{
  ...
  else if (tName == "assembly")
  {
    ...
    svtkIdType vertexID = this->AddVertexToSIL(node_name.c_str());
  ...
  else if (tName == "part")
  {
    ...
    svtkIdType partVertex = this->GetPartVertex(partNumberString.c_str());
  ...
  else if (tName == "material-specification")
  {
    ...
    svtkIdType partVertex = this->CurrentVertex.back();
  ...
  else if (tName == "block")
  {
    ...
    if (id >= 0)
    {
      if (this->InBlocks && !this->BlockPartNumberString.empty())
      {
        ...
        svtkIdType blockVertex = this->AddVertexToSIL(blockString);
  ...
  else if (tName == "material")
  {
    ...
    if (material)
    {
      ...
      svtkIdType vertex = this->AddVertexToSIL(node_name.c_str());
...
//-----------------------------------------------------------------------------
void svtkExodusIIReaderParser::EndElement(const char* tagName)
{
  const char* name = strrchr(tagName, ':');
  // If tag name has xml namespace separator, get rid of namespace:
  name = name ? name + 1 : tagName;
...
//-----------------------------------------------------------------------------
void svtkExodusIIReaderParser::FinishedParsing()
{
  std::map<int, svtkIdType> blockID_to_partVertexID;
  ...
  if (!this->Part_To_VertexID.empty())
  {
    ...
    for (iterIS = this->BlockID_To_Part.begin(); iterIS != this->BlockID_To_Part.end(); ++iterIS)
    {
      ...
      svtkIdType partVertex = this->Part_To_VertexID[iterIS->second];
      svtkIdType blockVertex = this->BlockID_To_VertexID[iterIS->first];
      this->AddCrossEdgeToSIL(partVertex, blockVertex);
  ...
  for (iter = this->BlockID_To_VertexID.begin(); iter != this->BlockID_To_VertexID.end(); ++iter)
  {
    ...
    {
      svtkIdType partVertex = blockID_to_partVertexID[iter->first];
  ...
  for (iter2 = this->BlockID_To_MaterialName.begin(); iter2 != this->BlockID_To_MaterialName.end();
       ++iter2)
  {
    svtkIdType blockVertex = this->BlockID_To_VertexID[iter2->first];
    if (this->MaterialName_To_VertexID.find(iter2->second) != this->MaterialName_To_VertexID.end())
    {
      svtkIdType materialVertex = this->MaterialName_To_VertexID[iter2->second];
...
//-----------------------------------------------------------------------------
svtkIdType svtkExodusIIReaderParser::AddVertexToSIL(const char* name)
{
...
//-----------------------------------------------------------------------------
svtkIdType svtkExodusIIReaderParser::AddChildEdgeToSIL(svtkIdType src, svtkIdType dst)
{
...
//-----------------------------------------------------------------------------
svtkIdType svtkExodusIIReaderParser::AddCrossEdgeToSIL(svtkIdType src, svtkIdType dst)
{
  svtkIdType id = this->SIL->AddEdge(src, dst).Id;
...
//-----------------------------------------------------------------------------
svtkIdType svtkExodusIIReaderParser::GetPartVertex(const char* part_number_instance_string)
{
  std::map<std::string, svtkIdType>::iterator iter =
  ...
  svtkIdType vertex = this->AddVertexToSIL(part_number_instance_string);
...
//-----------------------------------------------------------------------------
void svtkExodusIIReaderParser::Go(const char* filename)
{
  this->SIL->Initialize();
  ...
  this->NamesArray = svtkSmartPointer<svtkStringArray>::New();
  this->NamesArray->SetName("Names");
  this->CrossEdgesArray = svtkSmartPointer<svtkUnsignedCharArray>::New();
...
//-----------------------------------------------------------------------------
std::string svtkExodusIIReaderParser::GetBlockName(int id)
{
  if (this->BlockID_To_VertexID.find(id) != this->BlockID_To_VertexID.end())
  {
    svtkIdType vertex = this->BlockID_To_VertexID[id];
...
//-----------------------------------------------------------------------------
void svtkExodusIIReaderParser::PrintSelf(ostream& os, svtkIndent indent)
{
