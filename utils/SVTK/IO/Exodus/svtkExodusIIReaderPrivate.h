#ifndef svtkExodusIIReaderPrivate_h
#define svtkExodusIIReaderPrivate_h
...
#include "svtkToolkits.h" // make sure SVTK_USE_PARALLEL is properly set
 ...
 *
 */
class SVTKIOEXODUS_EXPORT svtkExodusIIReaderPrivate : public svtkObject
{
public:
  static svtkExodusIIReaderPrivate* New();
  void PrintData(ostream& os, svtkIndent indent);
  svtkTypeMacro(svtkExodusIIReaderPrivate, svtkObject);
  ...
  svtkMutableDirectedGraph* GetSIL() { return this->SIL; }

  /// Read requested data and store in unstructured grid.
  int RequestData(svtkIdType timeStep, svtkMultiBlockDataSet* output);
   ...
   */
  int SetUpEmptyGrid(svtkMultiBlockDataSet* output);
  ...
  svtkGetMacro(SqueezePoints, int);
  ...
  svtkBooleanMacro(SqueezePoints, int);
  ...
  svtkGetMacro(GenerateObjectIdArray, svtkTypeBool);
  svtkSetMacro(GenerateObjectIdArray, svtkTypeBool);
  static const char* GetObjectIdArrayName() { return "ObjectId"; }

  svtkSetMacro(GenerateGlobalElementIdArray, svtkTypeBool);
  svtkGetMacro(GenerateGlobalElementIdArray, svtkTypeBool);
  static const char* GetGlobalElementIdArrayName() { return "GlobalElementId"; }

  svtkSetMacro(GenerateGlobalNodeIdArray, svtkTypeBool);
  svtkGetMacro(GenerateGlobalNodeIdArray, svtkTypeBool);
  static const char* GetGlobalNodeIdArrayName() { return "GlobalNodeId"; }

  svtkSetMacro(GenerateImplicitElementIdArray, svtkTypeBool);
  svtkGetMacro(GenerateImplicitElementIdArray, svtkTypeBool);
  static const char* GetImplicitElementIdArrayName() { return "ImplicitElementId"; }

  svtkSetMacro(GenerateImplicitNodeIdArray, svtkTypeBool);
  svtkGetMacro(GenerateImplicitNodeIdArray, svtkTypeBool);
   ...
   */
  svtkSetMacro(GenerateFileIdArray, svtkTypeBool);
  svtkGetMacro(GenerateFileIdArray, svtkTypeBool);
  ...
  svtkSetMacro(FileId, int);
  svtkGetMacro(FileId, int);
  ...
  virtual void SetApplyDisplacements(svtkTypeBool d);
  svtkGetMacro(ApplyDisplacements, svtkTypeBool);

  virtual void SetDisplacementMagnitude(double s);
  svtkGetMacro(DisplacementMagnitude, double);

  svtkSetMacro(HasModeShapes, int);
  svtkGetMacro(HasModeShapes, int);

  svtkSetMacro(ModeShapeTime, double);
  svtkGetMacro(ModeShapeTime, double);

  svtkSetMacro(AnimateModeShapes, int);
  svtkGetMacro(AnimateModeShapes, int);

  svtkSetMacro(IgnoreFileTime, bool);
  svtkGetMacro(IgnoreFileTime, bool);

  svtkDataArray* FindDisplacementVectors(int timeStep);
  ...
  struct SVTKIOEXODUS_EXPORT ArrayInfoType
  {
    ...
    std::vector<svtkStdString> OriginalNames;
  ...
  struct SVTKIOEXODUS_EXPORT ObjectInfoType
  {
  ...
  struct SVTKIOEXODUS_EXPORT MapInfoType : public ObjectInfoType
  {
  };

  /// A struct to hold information about Exodus blocks or sets
  /// (they have some members in common)
  struct SVTKIOEXODUS_EXPORT BlockSetInfoType : public ObjectInfoType
  {
    /// Id (1-based) of first entry in file-local list across all blocks in file
    svtkIdType FileOffset;
    /** A map from nodal IDs in an Exodus file to nodal IDs in the output mesh.
     * Should only be used when SqueezePoints is true.
     * Otherwise, just subtract 1 from any Exodus node ID to get the SVTK node ID.
     */
    std::map<svtkIdType, svtkIdType> PointMap;
    /** A map from nodal ids in the output mesh to those in an Exodus file.
     * Should only be used when SqueezePoints is true.
     * Otherwise, just add 1 to any SVTK node ID to get the Exodus node ID.
     */
    std::map<svtkIdType, svtkIdType> ReversePointMap;
    /** The next svtk ID to use for a connectivity entry when point squeezing is on
     * and no point ID exists.
     */
    svtkIdType NextSqueezePoint;
    /// Cached cell connectivity arrays for mesh
    svtkUnstructuredGrid* CachedConnectivity;
  ...
  struct SVTKIOEXODUS_EXPORT BlockInfoType : public BlockSetInfoType
  {
    svtkStdString OriginalName; // useful to reset the name if XML metadata is invalid.
    svtkStdString TypeName;
    // number of boundaries per entry
    // The index is the dimensionality of the entry. 0=node, 1=edge, 2=face
    int BdsPerEntry[3];
    svtkIdType AttributesPerEntry;
    std::vector<svtkStdString> AttributeNames;
    std::vector<int> AttributeStatus;
    // SVTK cell type (a function of TypeName and BdsPerEntry...)
    int CellType;
    // Number of points per cell as used by SVTK
  ...
  /// (aggregated) into multi-component SVTK arrays.
  enum GlomTypes
  {
  ...
  svtkTimeStamp InformationTimeStamp;

  friend class svtkExodusIIReader;
  friend class svtkPExodusIIReader;

  virtual void SetParser(svtkExodusIIReaderParser*);
  svtkGetObjectMacro(Parser, svtkExodusIIReaderParser);

  // BUG #15632: This method allows svtkPExodusIIReader to pass time information
  ...
  int GetPartStatus(const svtkStdString& name);
  void SetPartStatus(int idx, int on);
  void SetPartStatus(const svtkStdString& name, int flag);

  int GetNumberOfMaterials();
  const char* GetMaterialName(int idx);
  int GetMaterialStatus(int idx);
  int GetMaterialStatus(const svtkStdString& name);
  void SetMaterialStatus(int idx, int on);
  void SetMaterialStatus(const svtkStdString& name, int flag);

  int GetNumberOfAssemblies();
  const char* GetAssemblyName(int idx);
  int GetAssemblyStatus(int idx);
  int GetAssemblyStatus(const svtkStdString& name);
  void SetAssemblyStatus(int idx, int on);
  void SetAssemblyStatus(const svtkStdString& name, int flag);

  void SetFastPathObjectType(svtkExodusIIReader::ObjectType type)
  {
    this->FastPathObjectType = type;
  }
  void SetFastPathObjectId(svtkIdType id) { this->FastPathObjectId = id; }
  svtkSetStringMacro(FastPathIdType);
...
protected:
  svtkExodusIIReaderPrivate();
  ~svtkExodusIIReaderPrivate() override;
  ...
  int VerifyIntegrationPointGlom(
    int nn, char** np, svtksys::RegularExpression& re, svtkStdString& field, svtkStdString& ele);

  /// Aggregate Exodus array names into SVTK arrays with multiple components
   ...
   */
  int AssembleOutputConnectivity(svtkIdType timeStep, int otyp, int oidx, int conntypidx,
    BlockSetInfoType* bsinfop, svtkUnstructuredGrid* output);
   ...
   * This adds time-varying results arrays to the grid's svtkPointData object.
   */
  int AssembleOutputPointArrays(
    svtkIdType timeStep, BlockSetInfoType* bsinfop, svtkUnstructuredGrid* output);
  /** Add the requested arrays to the output grid's cell data.
   * This adds time-varying results arrays to the grid's svtkCellData object.
   */
  int AssembleOutputCellArrays(
    svtkIdType timeStep, int otyp, int oidx, BlockSetInfoType* bsinfop, svtkUnstructuredGrid* output);
  ...
  int AssembleOutputProceduralArrays(
    svtkIdType timeStep, int otyp, int oidx, svtkUnstructuredGrid* output);
   ...
   */
  int AssembleArraysOverTime(svtkMultiBlockDataSet* output);
   ...
   * face, but each block will have a different svtkPoints object.
   */
  svtkIdType GetPolyhedronFaceConnectivity(svtkIdType fileLocalFaceId, svtkIdType*& facePtIds);
  ...
  void AddPointArray(svtkDataArray* src, BlockSetInfoType* bsinfop, svtkUnstructuredGrid* output);

  /// Insert cells referenced by a node set.
  void InsertSetNodeCopies(svtkIdTypeArray* refs, int otyp, int obj, SetInfoType* sinfo);

  /// Insert cells referenced by an edge, face, or element set.
  void InsertSetCellCopies(svtkIdTypeArray* refs, int otyp, int obj, SetInfoType* sinfo);

  /// Insert cells referenced by a side set.
  void InsertSetSides(svtkIdTypeArray* refs, int otyp, int obj, SetInfoType* sinfo);
   ...
   */
  svtkDataArray* GetCacheOrRead(svtkExodusIICacheKey);
   ...
   * (see obj_types in svtkExodusIIReader.cxx).
   */
   ...
   * in svtkExodusIIReader.cxx). The integer objectIndex is not the ID of the
   ...
   */
  svtkIdType GetSqueezePointId(BlockSetInfoType* bsinfop, int i);

  /// Determine the SVTK cell type for a given edge/face/element block
  ...
  svtkTypeBool GenerateObjectIdArray;
  svtkTypeBool GenerateGlobalIdArray;
  svtkTypeBool GenerateFileIdArray;
  svtkTypeBool GenerateGlobalElementIdArray;
  svtkTypeBool GenerateGlobalNodeIdArray;
  svtkTypeBool GenerateImplicitElementIdArray;
  svtkTypeBool GenerateImplicitNodeIdArray;

  /** Defaults to 0. Set by svtkPExodusIIReader on each entry of ReaderList.
   * Used to generate the file ID array over all output cells.
   */
  int FileId;

  /// A least-recently-used cache to hold raw arrays.
  svtkExodusIICache* Cache;
  //
  /// The size of the cache in MiB.
  double CacheSize;

  svtkTypeBool ApplyDisplacements;
  float DisplacementMagnitude;
  svtkTypeBool HasModeShapes;
  svtkTypeBool AnimateModeShapes;
   ...
   * circular references.
   */
  svtkExodusIIReader* Parent;

  svtkExodusIIReaderParser* Parser;
   ...
   * to the same face.
   */
  std::map<int, std::vector<std::vector<svtkIdType> > > PolyhedralFaceConnArrays;

  svtkExodusIIReader::ObjectType FastPathObjectType;
  svtkIdType FastPathObjectId;
  char* FastPathIdType;

  svtkMutableDirectedGraph* SIL;

private:
  svtkExodusIIReaderPrivate(const svtkExodusIIReaderPrivate&) = delete;
  void operator=(const svtkExodusIIReaderPrivate&) = delete;
};

#endif
#endif
#endif // svtkExodusIIReaderPrivate_h
// SVTK-HeaderTest-Exclude: svtkExodusIIReaderPrivate.h
