...
bool svtkExodusIIReaderVariableCheck::Start(std::string name, const int* truth, int numTruth)
{
  this->SeqTruth.clear();
...
std::vector<std::string>::size_type svtkExodusIIReaderVariableCheck::Length()
{
  return this->OriginalNames.size();
}

int svtkExodusIIReaderVariableCheck::Accept(
  ...
  svtkExodusIIReaderPrivate* priv, int objtyp)
{
  ...
  svtkExodusIIReaderPrivate::ArrayInfoType ainfo;
  ainfo.Name = this->Prefix;
  ainfo.Source = svtkExodusIIReaderPrivate::Result;
...
svtkExodusIIReaderVariableCheck::svtkExodusIIReaderVariableCheck()
{
  this->GlomType = -1;
}

svtkExodusIIReaderVariableCheck::~svtkExodusIIReaderVariableCheck() = default;

bool svtkExodusIIReaderVariableCheck::CheckTruth(const int* truth)
{
  if (!truth)
    return false;
...
bool svtkExodusIIReaderVariableCheck::UniquifyName(svtkExodusIIReaderPrivate::ArrayInfoType& ainfo,
  std::vector<svtkExodusIIReaderPrivate::ArrayInfoType>& arrays)
{
  bool nameChanged = false;
  std::vector<svtkExodusIIReaderPrivate::ArrayInfoType>::iterator it = arrays.begin();
  while (it != arrays.end())
  {
...
svtkExodusIIReaderScalarCheck::svtkExodusIIReaderScalarCheck()
{
  this->GlomType = svtkExodusIIReaderPrivate::Scalar;
}

bool svtkExodusIIReaderScalarCheck::StartInternal(std::string name, const int*, int)
{
  this->Prefix = name;
...
bool svtkExodusIIReaderScalarCheck::Add(std::string, const int*)
{ // Scalars never have more than 1 name
  return false;
}

svtkExodusIIReaderVectorCheck::svtkExodusIIReaderVectorCheck(const char* seq, int n)
{
  this->Endings.clear();
  this->Endings.insert(this->Endings.begin(), seq, seq + n);
  this->Endings = svtksys::SystemTools::LowerCase(this->Endings);
  switch (n)
  {
    case 2:
      this->GlomType = svtkExodusIIReaderPrivate::Vector2;
      break;
    case 3:
      this->GlomType = svtkExodusIIReaderPrivate::Vector3;
      break;
    default:
...
bool svtkExodusIIReaderVectorCheck::StartInternal(std::string name, const int*, int)
{
...
bool svtkExodusIIReaderVectorCheck::Add(std::string name, const int* truth)
{
...
std::vector<std::string>::size_type svtkExodusIIReaderVectorCheck::Length()
{
...
svtkExodusIIReaderTensorCheck::svtkExodusIIReaderTensorCheck(
  const char* seq, int n, int rank, int dim)
{
  this->NumEndings = svtkMath::Binomial(rank + dim - 1, rank);
  if (n == (int)this->NumEndings && rank > 0 && dim > 0)
  {
    ...
    {
      this->GlomType = svtkExodusIIReaderPrivate::SymmetricTensor;
    }
  }
  else
  {
    svtkGenericWarningMacro("Invalid number of tensor endings " << n
                                                               << " for "
                                                                  "rank "
...
bool svtkExodusIIReaderTensorCheck::StartInternal(std::string name, const int*, int)
{
  std::string::size_type len = name.size();
  if ((len > (unsigned)this->Rank) &&
    svtksys::SystemTools::LowerCase(name.substr(len - this->Rank)) ==
...
bool svtkExodusIIReaderTensorCheck::Add(std::string name, const int* truth)
{
  ...
  if (svtksys::SystemTools::LowerCase(name.substr(len - this->Rank)) !=
...
std::vector<std::string>::size_type svtkExodusIIReaderTensorCheck::Length()
{
  std::vector<std::string>::size_type len = this->OriginalNames.size();
...
svtkExodusIIReaderIntPointCheck::svtkExodusIIReaderIntPointCheck()
  : RegExp("(.*)_([^_]*)_GP([0-9,]+)$")
{
  this->GlomType = svtkExodusIIReaderPrivate::IntegrationPoint;
}

bool svtkExodusIIReaderIntPointCheck::StartInternal(std::string name, const int*, int)
{
...
bool svtkExodusIIReaderIntPointCheck::Add(std::string name, const int*)
{
...
std::vector<std::string>::size_type svtkExodusIIReaderIntPointCheck::Length()
{
...
/*
int svtkExodusIIReaderIntPointCheck::Accept(
std::vector<svtkExodusIIReaderPrivate::ArrayInfoType>& arr, int startIndex, svtkExodusIIReaderPrivate*
priv, int objtyp )
{
}
*/

bool svtkExodusIIReaderIntPointCheck::StartIntegrationPoints(
  std::string cellType, std::string iptName)
{
  ...
  svtksys::RegularExpression ctrexp;
...
bool svtkExodusIIReaderIntPointCheck::AddIntegrationPoint(std::string iptName)
{
  std::string::size_type rank = iptName.size();
  if (rank != this->Rank)
  {
