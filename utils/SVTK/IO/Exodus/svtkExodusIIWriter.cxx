...
svtkObjectFactoryNewMacro(svtkExodusIIWriter);
svtkCxxSetObjectMacro(svtkExodusIIWriter, ModelMetadata, svtkModelMetadata);
...
svtkExodusIIWriter::svtkExodusIIWriter()
{
  this->fid = -1;
...
void svtkExodusIIWriter::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
svtkTypeBool svtkExodusIIWriter::ProcessRequest(
...
//----------------------------------------------------------------------------
int svtkExodusIIWriter::RequestInformation(svtkInformation* svtkNotUsed(request),
...
//----------------------------------------------------------------------------
int svtkExodusIIWriter::RequestUpdateExtent(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* svtkNotUsed(outputVector))
{
  svtkInformation* inInfo = inputVector[0]->GetInformationObject(0);
  if (this->WriteAllTimeSteps && inInfo->Has(svtkStreamingDemandDrivenPipeline::TIME_STEPS()))
  {
    double* timeSteps = inInfo->Get(svtkStreamingDemandDrivenPipeline::TIME_STEPS());
...
//----------------------------------------------------------------------------
int svtkExodusIIWriter::FillInputPortInformation(int svtkNotUsed(port), svtkInformation* info)
{
...
//----------------------------------------------------------------------------
int svtkExodusIIWriter::RequestData(svtkInformation* request, svtkInformationVector** inputVector,
...
//----------------------------------------------------------------------------
int svtkExodusIIWriter::GlobalContinueExecuting(int localContinueExecution)
{
  return localContinueExecution;
}

//----------------------------------------------------------------------------
void svtkExodusIIWriter::WriteData()
{
  ...
  {
    svtkErrorMacro("svtkExodusIIWriter::WriteData Unable to flatten hierarchy");
    return;
  }
  ...
  if (newHierarchy && !this->FlattenedInput.empty())
  {
    ...
    svtkErrorMacro("Temporal data with changing topology is not yet supported.");
    return;
  }
  ...
  if (this->WriteAllTimeSteps && this->CurrentTimeIndex != 0 && !newHierarchy)
  {
    ...
    {
      svtkErrorMacro("svtkExodusIIWriter::WriteData results");
    }
    return;
  }
  else
  {
    ...
    {
      svtkErrorMacro("svtkExodusIIWriter: WriteData can't create exodus file");
      return;
    }

    if (!this->WriteInitializationParameters())
    {
      svtkErrorMacro(<< "svtkExodusIIWriter::WriteData init params");
      return;
    }

    if (!this->WriteInformationRecords())
    {
      svtkErrorMacro("svtkExodusIIWriter::WriteData information records");
      return;
    }

    if (!this->WritePoints())
    {
      svtkErrorMacro("svtkExodusIIWriter::WriteData points");
      return;
    }

    if (!this->WriteCoordinateNames())
    {
      svtkErrorMacro("svtkExodusIIWriter::WriteData coordinate names");
      return;
    }

    if (!this->WriteGlobalPointIds())
    {
      svtkErrorMacro("svtkExodusIIWriter::WriteData global point IDs");
      return;
    }

    if (!this->WriteBlockInformation())
    {
      svtkErrorMacro("svtkExodusIIWriter::WriteData block information");
      return;
    }

    if (!this->WriteGlobalElementIds())
    {
      svtkErrorMacro("svtkExodusIIWriter::WriteData global element IDs");
      return;
    }

    if (!this->WriteVariableArrayNames())
    {
      svtkErrorMacro("svtkExodusIIWriter::WriteData variable array names");
      return;
    }

    if (!this->WriteNodeSetInformation())
    {
      svtkErrorMacro("svtkExodusIIWriter::WriteData can't node sets");
      return;
    }

    if (!this->WriteSideSetInformation())
    {
      svtkErrorMacro("svtkExodusIIWriter::WriteData can't side sets");
      return;
    }

    if (!this->WriteProperties())
    {
      svtkErrorMacro("svtkExodusIIWriter::WriteData can't properties");
      return;
    }
...
//----------------------------------------------------------------------------
char* svtkExodusIIWriter::StrDupWithNew(const char* s)
{
  char* newstr = nullptr;
...
//----------------------------------------------------------------------------
void svtkExodusIIWriter::StringUppercase(std::string& str)
{
...
//----------------------------------------------------------------------------
int svtkExodusIIWriter::FlattenHierarchy(svtkDataObject* input, const char* name, bool& changed)
{
  if (input->IsA("svtkMultiBlockDataSet"))
  {
    svtkMultiBlockDataSet* castObj = svtkMultiBlockDataSet::SafeDownCast(input);
  ...
  {
    svtkCompositeDataSet* castObj = svtkCompositeDataSet::SafeDownCast(input);
    svtkSmartPointer<svtkCompositeDataIterator> iter;
    iter.TakeReference(castObj->NewIterator());
    svtkDataObjectTreeIterator* treeIter = svtkDataObjectTreeIterator::SafeDownCast(castObj);
    if (treeIter)
    {
  ...
  {
    svtkSmartPointer<svtkUnstructuredGrid> output = svtkSmartPointer<svtkUnstructuredGrid>::New();
    if (input->IsA("svtkUnstructuredGrid"))
    {
      output->ShallowCopy(input);
    }
    else
    {
      svtkDataSet* castObj = svtkDataSet::SafeDownCast(input);
      ...
      svtkIdType numPoints = castObj->GetNumberOfPoints();
  ...
  {
    svtkErrorMacro(<< "Incorrect class type " << input->GetClassName() << " on input");
    return 0;
  }
  return 1;
}
...
//----------------------------------------------------------------------------
int svtkExodusIIWriter::CreateNewExodusFile()
{
  ...
  if (this->NumberOfProcesses == 1)
  {
    if (this->WriteAllTimeSteps == false || this->CurrentTimeIndex == 0)
    {
      ...
      if (fid <= 0)
      {
        svtkErrorMacro(<< "svtkExodusIIWriter: CreateNewExodusFile can't create " << this->FileName);
      }
    }
    else
    {
      char* myFileName = new char[SVTK_MAXPATH];
      snprintf(myFileName, SVTK_MAXPATH, "%s-s.%06d", this->FileName, this->CurrentTimeIndex);
      this->fid = ex_create(myFileName, EX_CLOBBER, &compWordSize, &IOWordSize);
      if (fid <= 0)
      {
        svtkErrorMacro(<< "svtkExodusIIWriter: CreateNewExodusFile can't create " << myFileName);
      }
      delete[] myFileName;
    }
  }
  else
  {
    ...
    if (this->fid <= 0)
    {
      svtkErrorMacro(<< "svtkExodusIIWriter: CreateNewExodusFile can't create " << myFileName.str());
    }
  }
...
//
//------------------------------------------------------------------
void svtkExodusIIWriter::CloseExodusFile()
{
  if (this->fid >= 0)
  {
    ex_close(this->fid);
...
//----------------------------------------------------------------------------
int svtkExodusIIWriter::IsDouble()
{
  ...
  for (size_t i = 0; i < this->FlattenedInput.size(); i++)
  {
    ...
    svtkCellData* cd = this->FlattenedInput[i]->GetCellData();
    if (cd)
    {
      int numCellArrays = cd->GetNumberOfArrays();
      for (int j = 0; j < numCellArrays; j++)
      {
        svtkDataArray* a = cd->GetArray(j);
    ...
    svtkPointData* pd = this->FlattenedInput[i]->GetPointData();
    if (pd)
    {
      int numPtArrays = pd->GetNumberOfArrays();
      for (int j = 0; j < numPtArrays; j++)
      {
        svtkDataArray* a = pd->GetArray(j);
        int type = a->GetDataType();
...
//----------------------------------------------------------------------------
void svtkExodusIIWriter::RemoveGhostCells()
{
  for (size_t i = 0; i < this->FlattenedInput.size(); i++)
  {
    svtkUnsignedCharArray* da = this->FlattenedInput[i]->GetCellGhostArray();

    if (da)
    {
      svtkThreshold* t = svtkThreshold::New();
      t->SetInputData(this->FlattenedInput[i]);
      t->ThresholdByLower(0);
      t->SetInputArrayToProcess(
        0, 0, 0, svtkDataObject::FIELD_ASSOCIATION_CELLS, svtkDataSetAttributes::GhostArrayName());

      t->Update();

      this->FlattenedInput[i] = svtkSmartPointer<svtkUnstructuredGrid>(t->GetOutput());
      t->Delete();

      this->FlattenedInput[i]->GetCellData()->RemoveArray(svtkDataSetAttributes::GhostArrayName());
      this->FlattenedInput[i]->GetPointData()->RemoveArray(svtkDataSetAttributes::GhostArrayName());
...
//----------------------------------------------------------------------------
int svtkExodusIIWriter::CheckParametersInternal(int _NumberOfProcesses, int _MyRank)
{
  if (!this->FileName)
  {
...
//----------------------------------------------------------------------------
int svtkExodusIIWriter::CheckParameters()
{
  return this->CheckParametersInternal(1, 0);
}

int svtkExodusIIWriter::CheckInputArrays()
{
  ...
  for (i = 0; i < this->FlattenedInput.size(); i++)
  {
    ...
    svtkIntArray* bia = this->GetBlockIdArray(this->BlockIdArrayName, this->FlattenedInput[i]);
    if (bia)
    {
    ...
    svtkDataArray* da = cd->GetGlobalIds();
    if (!da)
    ...
    if (da)
    {
      svtkIdTypeArray* ia = svtkArrayDownCast<svtkIdTypeArray>(da);
      if (!ia)
      {
        svtkWarningMacro(<< "svtkExodusIIWriter, element ID array is not an Id array, ignoring it");
    ...
    if (da)
    {
      ...
      if (!ia)
      {
        svtkWarningMacro(<< "svtkExodusIIWriter, node ID array is not an Id array, ignoring it");
...
int svtkExodusIIWriter::ConstructBlockInfoMap()
{
  ...
  for (size_t i = 0; i < this->FlattenedInput.size(); i++)
  {
    ...
    svtkIdType ncells = this->FlattenedInput[i]->GetNumberOfCells();
    if (!this->BlockIdList[i])
    {
      ...
      this->MaxId += SVTK_NUMBER_OF_CELL_TYPES;
    }
    ...
    for (int j = 0; j < ncells; j++)
    {
      ...
      else
      {
        ...
        {
          svtkWarningMacro("Block ids are not unique across the hierarchy ");
        }
...
int svtkExodusIIWriter::ConstructVariableInfoMaps()
{
  ...
  for (size_t i = 0; i < this->FlattenedInput.size(); i++)
  {
    svtkFieldData* fd = this->FlattenedInput[i]->GetFieldData();
    for (int j = 0; j < fd->GetNumberOfArrays(); j++)
    {
      ...
      if (name == nullptr)
      {
        svtkWarningMacro("Array in input field data has Null name, cannot output it");
        continue;
      }
      ...
      {
        svtkErrorMacro("Disagreement in the hierarchy for the number of components in " << name);
        return 0;
      }
    }
    ...
    for (int j = 0; j < cd->GetNumberOfArrays(); j++)
    {
      ...
      if (name == nullptr)
      {
        svtkWarningMacro("Array in input cell data has Null name, cannot output it");
        continue;
      }
      std::string upper(name);
      this->StringUppercase(upper);
      ...
      if (!this->WriteOutGlobalElementIdArray &&
        cd->IsArrayAnAttribute(j) == svtkDataSetAttributes::GLOBALIDS)
      {
        continue;
      }
    ...
    for (int j = 0; j < pd->GetNumberOfArrays(); j++)
    {
      ...
      if (name == nullptr)
      {
        svtkWarningMacro("Array in input point data has Null name, cannot output it");
        continue;
      }
      std::string upper(name);
      this->StringUppercase(upper);
      ...
      if (!this->WriteOutGlobalNodeIdArray &&
        pd->IsArrayAnAttribute(j) == svtkDataSetAttributes::GLOBALIDS)
      {
        continue;
      }
      if (strncmp(upper.c_str(), "PEDIGREE", 8) == 0)
  ...
  if (ttsize > 0)
  {
    ...
    for (blockIter = this->BlockInfoMap.begin(); blockIter != this->BlockInfoMap.end(); ++blockIter)
    {
      ...
      for (varIter = this->BlockVariableMap.begin(); varIter != this->BlockVariableMap.end();
           ++varIter)
      {
        svtkCellData* cd = this->FlattenedInput[blockIter->second.GridIndex]->GetCellData();
        int truth = 0;
        if (cd)
        {
          truth = 1;
...
int svtkExodusIIWriter::CreateDefaultMetadata()
{
  ...
  dimNames[0] = svtkExodusIIWriter::StrDupWithNew("X");
  dimNames[1] = svtkExodusIIWriter::StrDupWithNew("Y");
  dimNames[2] = svtkExodusIIWriter::StrDupWithNew("Z");
...
//----------------------------------------------------------------------------
char* svtkExodusIIWriter::GetCellTypeName(int t)
{
...
//----------------------------------------------------------------------------
int svtkExodusIIWriter::CreateBlockIdMetadata(svtkModelMetadata* em)
{
  // svtkModelMetadata frees the memory when its done so we need to create a copy
  ...
  for (iter = this->BlockInfoMap.begin(); iter != this->BlockInfoMap.end(); ++iter)
  {
    ...
    blockNames[index] = svtkExodusIIWriter::GetCellTypeName(iter->second.Type);
...
//----------------------------------------------------------------------------
int svtkExodusIIWriter::CreateBlockVariableMetadata(svtkModelMetadata* em)
{
  size_t narrays = this->GlobalVariableMap.size();
  char** flattenedNames = nullptr;
  if (narrays > 0)
  {
    flattenedNames = svtkExodusIIWriter::FlattenOutVariableNames(
  ...
  if (narrays > 0)
  {
    ...
    for (var_iter = this->BlockVariableMap.begin(); var_iter != this->BlockVariableMap.end();
         ++var_iter)
    {
      nms[index] = svtkExodusIIWriter::StrDupWithNew(var_iter->first.c_str());
  ...
  if (narrays > 0)
  {
    ...
    {
      nms[index] = svtkExodusIIWriter::StrDupWithNew(iter->first.c_str());
...
int svtkExodusIIWriter::CreateSetsMetadata(svtkModelMetadata* em)
{
  bool isASideSet = false;
  bool isANodeSet = false;

  if (this->OriginalInput->IsA("svtkMultiBlockDataSet"))
  {
    int numNodeSets = 0;
    int sumNodes = 0;
    svtkNew<svtkStringArray> nodeSetNames;
    svtkSmartPointer<svtkIntArray> nodeSetIds = svtkSmartPointer<svtkIntArray>::New();
    svtkSmartPointer<svtkIntArray> nodeSetSizes = svtkSmartPointer<svtkIntArray>::New();
    svtkSmartPointer<svtkIntArray> nodeSetNumDF = svtkSmartPointer<svtkIntArray>::New();
    svtkSmartPointer<svtkIntArray> nodeIds = svtkSmartPointer<svtkIntArray>::New();
    int numSideSets = 0;
    int sumSides = 0;
    svtkNew<svtkStringArray> sideSetNames;
    svtkSmartPointer<svtkIntArray> sideSetIds = svtkSmartPointer<svtkIntArray>::New();
    svtkSmartPointer<svtkIntArray> sideSetSizes = svtkSmartPointer<svtkIntArray>::New();
    svtkSmartPointer<svtkIntArray> sideSetNumDF = svtkSmartPointer<svtkIntArray>::New();
    svtkSmartPointer<svtkIntArray> sideSetElementList = svtkSmartPointer<svtkIntArray>::New();
    svtkSmartPointer<svtkIntArray> sideSetSideList = svtkSmartPointer<svtkIntArray>::New();

    svtkMultiBlockDataSet* castObj = svtkMultiBlockDataSet::SafeDownCast(this->OriginalInput);
    svtkSmartPointer<svtkDataObjectTreeIterator> iter;
    iter.TakeReference(castObj->NewTreeIterator());
    iter->VisitOnlyLeavesOff();
    ...
    for (iter->InitTraversal(); !iter->IsDoneWithTraversal(); iter->GoToNextItem())
    {
      const char* name = iter->GetCurrentMetaData()->Get(svtkCompositeDataSet::NAME());
      if (iter->GetCurrentDataObject()->IsA("svtkMultiBlockDataSet"))
      {
      ...
      else if (isANodeSet)
      {
        ...
        svtkUnstructuredGrid* grid = svtkUnstructuredGrid::SafeDownCast(iter->GetCurrentDataObject());
        svtkFieldData* field = grid->GetPointData();
        svtkIdTypeArray* globalIds =
          svtkArrayDownCast<svtkIdTypeArray>(field ? field->GetArray("GlobalNodeId") : nullptr);
        if (globalIds)
        {
          int size = 0;
          for (int c = 0; c < grid->GetNumberOfCells(); c++)
          {
            svtkCell* cell = grid->GetCell(c);
            for (int p = 0; p < cell->GetNumberOfPoints(); p++)
            {
              size++;
              svtkIdType pointId = cell->GetPointId(p);
              svtkIdType gid = globalIds->GetValue(pointId);
        ...
        // else
        // {
        // svtkErrorMacro ("We have a node set, but it doesn't have GlobalNodeIDs");
        // }
      }
      else if (isASideSet)
      {
        ...
        svtkUnstructuredGrid* grid = svtkUnstructuredGrid::SafeDownCast(iter->GetCurrentDataObject());
        svtkFieldData* field = grid->GetCellData();
        int cells = grid->GetNumberOfCells();
        svtkIdTypeArray* sourceElement =
          svtkArrayDownCast<svtkIdTypeArray>(field ? field->GetArray("SourceElementId") : nullptr);
        svtkIntArray* sourceSide =
          svtkArrayDownCast<svtkIntArray>(field ? field->GetArray("SourceElementSide") : nullptr);
        ...
        // else
        // {
        // svtkErrorMacro ("We have a side set, but it doesn't have SourceElementId or
        // SourceElementSide");
        // }
      }
    }
...
//----------------------------------------------------------------------------
int svtkExodusIIWriter::ParseMetadata()
{
  svtkModelMetadata* em = this->GetModelMetadata();
  ...
  for (int n = 0; n < nblocks; n++)
  {
    ...
    if (iter == this->BlockInfoMap.end())
    {
      svtkErrorMacro(<< "Unknown id " << ids[n] << " found in meta data");
      return 0;
    }
...
//----------------------------------------------------------------------------
int svtkExodusIIWriter::WriteInitializationParameters()
{
  svtkModelMetadata* em = this->GetModelMetadata();
...
//----------------------------------------------------------------------------
int svtkExodusIIWriter::WriteInformationRecords()
{
...
template <typename T>
int svtkExodusIIWriterWritePoints(
  std::vector<svtkSmartPointer<svtkUnstructuredGrid> > input, int numPoints, int fid)
{
  ...
  {
    svtkPoints* pts = input[i]->GetPoints();
    if (pts)
    {
...
//----------------------------------------------------------------------------
int svtkExodusIIWriter::WritePoints()
{
  if (this->PassDoubles)
  {
    return svtkExodusIIWriterWritePoints<double>(this->FlattenedInput, this->NumPoints, this->fid);
  }
  else
  {
    return svtkExodusIIWriterWritePoints<float>(this->FlattenedInput, this->NumPoints, this->fid);
  }
}
...
//---------------------------------------------------------
int svtkExodusIIWriter::WriteCoordinateNames()
{
  svtkModelMetadata* em = this->GetModelMetadata();
...
//----------------------------------------------------------------------------
int svtkExodusIIWriter::WriteGlobalPointIds()
{
  ...
  for (size_t i = 0; i < this->FlattenedInput.size(); i++)
  {
    svtkIdType npoints = this->FlattenedInput[i]->GetNumberOfPoints();

    svtkIdType* ids = this->GlobalNodeIdList[i];
    if (ids)
    {
...
//----------------------------------------------------------------------------
int svtkExodusIIWriter::WriteBlockInformation()
{
  int rc;
  ...
  for (size_t i = 0; i < this->FlattenedInput.size(); i++)
  {
    svtkCellArray* ca = this->FlattenedInput[i]->GetCells();
    ...
    for (int j = 0; j < ncells; j++)
    {
      ...
      svtkIdType elementOffset = this->CellToElementOffset[i][j];
      int offset;
      ...
      {
        case SVTK_VOXEL: // reorder to exodus HEX type
  ...
  for (blockIter = this->BlockInfoMap.begin(); blockIter != this->BlockInfoMap.end(); ++blockIter)
  {
    char* type_name = svtkExodusIIWriter::GetCellTypeName(blockIter->second.Type);
    ...
    if (rc < 0)
    {
      svtkErrorMacro(<< "Problem adding block with id " << blockIter->first);
      continue;
    }
    ...
    if (blockIter->second.NumElements > 0)
    {
      ...
      if (rc < 0)
      {
        svtkErrorMacro(<< "Problem writing connectivity " << blockIter->first);
        continue;
      }
...
//----------------------------------------------------------------------------
int svtkExodusIIWriter::WriteGlobalElementIds()
{
  int rc = 0;

  // if (sizeof(svtkIdType) != sizeof(int))
  //  {
  //  svtkErrorMacro(<<"svtkExodusIIWriter::WriteGlobalElementIds cannot convert svtkIdType to int.");
  //  return -1;
  //  }
  ...
  if (this->AtLeastOneGlobalElementIdList)
  {
    ...
    for (size_t i = 0; i < this->FlattenedInput.size(); i++)
    {
      svtkIdType* ids = this->GlobalElementIdList[i];
      if (ids)
      {
        int ncells = this->FlattenedInput[i]->GetNumberOfCells();
        for (int j = 0; j < ncells; j++)
        {
          ...
          svtkIdType offset = this->CellToElementOffset[i][j];
...
//----------------------------------------------------------------------------
int svtkExodusIIWriter::WriteVariableArrayNames()
{
  int rc = 0;
  ...
  if (this->NumberOfScalarGlobalArrays > 0)
  {
    ...
    if (rc < 0)
    {
      svtkErrorMacro(<< "svtkExodusIIWriter::WriteVariableArrayNames cell variables");
  ...
  if (this->NumberOfScalarNodeArrays > 0 && this->NumPoints > 0)
  {
    ...
    if (rc < 0)
    {
      svtkErrorMacro(<< "svtkExodusIIWriter::WriteVariableArrayNames "
  ...
  /*
    ...
    if (ngvars > 0)
      ...
      if (rc < 0)
        {
        svtkErrorMacro(<<
          "svtkExodusIIWriter::WriteVariableArrayNames global variables");
        return 0;
        }
      }
  */
...
//----------------------------------------------------------------------------
void svtkExodusIIWriter::ConvertVariableNames(std::map<std::string, VariableInfo>& variableMap)
{
...
char** svtkExodusIIWriter::FlattenOutVariableNames(
...
//----------------------------------------------------------------------------
std::string svtkExodusIIWriter::CreateNameForScalarArray(
  const char* root, int component, int numComponents)
{
  ...
  {
    svtkErrorMacro("CreateNameForScalarArray: Component out of range");
    return std::string();
  }
  if (numComponents == 1)
  {
...
//----------------------------------------------------------------------------
svtkIdType svtkExodusIIWriter::GetNodeLocalId(svtkIdType id)
{
  if (!this->LocalNodeIdMap)
  {
    this->LocalNodeIdMap = new std::map<svtkIdType, svtkIdType>;
    svtkIdType index = 0;
    for (size_t i = 0; i < this->FlattenedInput.size(); i++)
    {
      svtkIdType npoints = this->FlattenedInput[i]->GetNumberOfPoints();
      ...
      if (ids)
      {
        for (int j = 0; j < npoints; j++)
        {
          this->LocalNodeIdMap->insert(std::map<svtkIdType, svtkIdType>::value_type(ids[j], index));
          index++;
        }
      }
      else
  ...
  std::map<svtkIdType, svtkIdType>::iterator mapit = this->LocalNodeIdMap->find(id);
...
//----------------------------------------------------------------------------
int svtkExodusIIWriter::WriteNodeSetInformation()
{
  int rc = 0;
  int i, j;
  ...
  for (i = 0; i < nnsets; i++)
  {
    svtkStdString name = em->GetNodeSetNames()->GetValue(node_ids[i]);
...
//----------------------------------------------------------------------------
svtkIdType svtkExodusIIWriter::GetElementLocalId(svtkIdType id)
{
  if (!this->LocalElementIdMap)
  {
    this->LocalElementIdMap = new std::map<svtkIdType, svtkIdType>;
    for (size_t i = 0; i < this->FlattenedInput.size(); i++)
    {
      if (this->GlobalElementIdList[i])
      {
        svtkIdType ncells = this->FlattenedInput[i]->GetNumberOfCells();
        for (svtkIdType j = 0; j < ncells; j++)
        {
          svtkIdType gid = this->GlobalElementIdList[i][j];
          std::map<svtkIdType, svtkIdType>::iterator mapit = this->LocalElementIdMap->find(gid);
          if (mapit != this->LocalElementIdMap->end())
          {
            svtkErrorMacro("Overlapping gids in the dataset");
            continue;
          }
          ...
          {
            svtkWarningMacro("svtkExodusIIWriter: The block id map has come out of sync");
            continue;
          }

          int index = blockIter->second.ElementStartIndex + CellToElementOffset[i][j];
          this->LocalElementIdMap->insert(std::map<svtkIdType, svtkIdType>::value_type(gid, index));
        }
      }
    }
  }

  std::map<svtkIdType, svtkIdType>::iterator mapit = this->LocalElementIdMap->find(id);
  if (mapit == this->LocalElementIdMap->end())
  {
    return -1;
  }
...
int svtkExodusIIWriter::GetElementType(svtkIdType id)
{
  for (size_t i = 0; i < this->FlattenedInput.size(); i++)
  {
    if (this->GlobalElementIdList[i])
...
//-----------------------------------------------------------------------
int svtkExodusIIWriter::WriteSideSetInformation()
{
  int i, j, k;
  int rc = 0;
  ...
  for (i = 0; i < nssets; i++)
  {
    svtkStdString name = em->GetSideSetNames()->GetValue(sids[i]);
...
//----------------------------------------------------------------------------
int svtkExodusIIWriter::BlockVariableTruthValue(int blockIdx, int varIdx)
{
  int tt = 0;
  ...
  {
    svtkWarningMacro(<< "svtkExodusIIWriter::BlockVariableTruthValue invalid index");
  }
...
// Properties
//-----------------------------------------------------------------------
int svtkExodusIIWriter::WriteProperties()
{
  int rc = 0;
  int i;
...
double svtkExodusIIWriterGetComponent(iterT* it, svtkIdType ind)
{
  svtkVariant v(it->GetValue(ind));
  return v.ToDouble();
}

//----------------------------------------------------------------------------
double svtkExodusIIWriter::ExtractGlobalData(const char* name, int comp, int ts)
{
  double ret = 0.0;
  for (size_t i = 0; i < this->FlattenedInput.size(); i++)
  {
    // find the first block that matches this global data.  Assumes it's global.
    svtkDataArray* da = this->FlattenedInput[i]->GetFieldData()->GetArray(name);
    if (da)
    {
...
//----------------------------------------------------------------------------
void svtkExodusIIWriter::ExtractCellData(const char* name, int comp, svtkDataArray* buffer)
{
  buffer->SetNumberOfTuples(this->NumCells);
  for (size_t i = 0; i < this->FlattenedInput.size(); i++)
  {
    svtkDataArray* da = this->FlattenedInput[i]->GetCellData()->GetArray(name);
    int ncells = this->FlattenedInput[i]->GetNumberOfCells();
    if (da)
    {
      svtkArrayIterator* arrayIter = da->NewIterator();
      svtkIdType ncomp = da->GetNumberOfComponents();
      for (svtkIdType j = 0; j < ncells; j++)
      {
        std::map<int, Block>::const_iterator blockIter =
          this->BlockInfoMap.find(this->BlockIdList[i]->GetValue(j));
        ...
        {
          svtkArrayIteratorTemplateMacro(buffer->SetTuple1(index,
            svtkExodusIIWriterGetComponent(static_cast<SVTK_TT*>(arrayIter), j * ncomp + comp)));
        }
      }
      arrayIter->Delete();
    }
...
//----------------------------------------------------------------------------
void svtkExodusIIWriter::ExtractPointData(const char* name, int comp, svtkDataArray* buffer)
{
  buffer->SetNumberOfTuples(this->NumPoints);
  int index = 0;
  for (size_t i = 0; i < this->FlattenedInput.size(); i++)
  {
    svtkDataArray* da = this->FlattenedInput[i]->GetPointData()->GetArray(name);
    if (da)
    {
      svtkArrayIterator* iter = da->NewIterator();
      svtkIdType ncomp = da->GetNumberOfComponents();
      svtkIdType nvals = ncomp * da->GetNumberOfTuples();
      for (svtkIdType j = comp; j < nvals; j += ncomp)
      {
        switch (da->GetDataType())
        {
          svtkArrayIteratorTemplateMacro(buffer->SetTuple1(
            index++, svtkExodusIIWriterGetComponent(static_cast<SVTK_TT*>(iter), j)));
        }
      }
      iter->Delete();
    }
    ...
    {
      svtkIdType nvals = this->FlattenedInput[i]->GetNumberOfPoints();
      for (svtkIdType j = 0; j < nvals; j++)
      {
        buffer->SetTuple1(index++, 0);
      }
    }
...
//----------------------------------------------------------------------------
int svtkExodusIIWriter::WriteGlobalData(int timestep, svtkDataArray* buffer)
{
  std::map<std::string, VariableInfo>::const_iterator varIter;
  buffer->Initialize();
  ...
  int rc;
  if (buffer->IsA("svtkDoubleArray"))
  {
    svtkDoubleArray* da = svtkArrayDownCast<svtkDoubleArray>(buffer);
    rc = ex_put_glob_vars(
      this->fid, timestep + 1, this->NumberOfScalarGlobalArrays, da->GetPointer(0));
  }
  else /* (buffer->IsA ("svtkFloatArray")) */
  {
    svtkFloatArray* fa = svtkArrayDownCast<svtkFloatArray>(buffer);
    rc = ex_put_glob_vars(
      this->fid, timestep + 1, this->NumberOfScalarGlobalArrays, fa->GetPointer(0));
  }
  if (rc < 0)
  {
    svtkErrorMacro(<< "svtkExodusIIWriter::WriteNextTimeStep glob vars");
    return 0;
  }
  return 1;
}
...
//----------------------------------------------------------------------------
int svtkExodusIIWriter::WriteCellData(int timestep, svtkDataArray* buffer)
{
  std::map<std::string, VariableInfo>::const_iterator varIter;
  for (varIter = this->BlockVariableMap.begin(); varIter != this->BlockVariableMap.end(); ++varIter)
  {
    ...
    for (int component = 0; component < numComp; component++)
    {
           ...
           ++blockIter)
      {
        ...
        if (rc < 0)
        {
          svtkErrorMacro(<< "svtkExodusIIWriter::WriteNextTimeStep ex_put_elem_var");
          return 0;
        }
      }
    }
...
//----------------------------------------------------------------------------
int svtkExodusIIWriter::WritePointData(int timestep, svtkDataArray* buffer)
{
  if (this->NumPoints == 0)
  {
    return 1;
  ...
  for (varIter = this->NodeVariableMap.begin(); varIter != this->NodeVariableMap.end(); ++varIter)
  {
    ...
    for (int component = 0; component < numComp; component++)
    {
      ...
      if (rc < 0)
      {
        svtkErrorMacro(<< "svtkExodusIIWriter::WriteNextTimeStep ex_put_nodal_var");
        return 0;
      }
    }
  }
...
namespace
{
unsigned int GetLongestFieldDataName(svtkFieldData* fd)
{
...
unsigned int GetLongestDataSetName(svtkDataSet* ds)
{
...
//----------------------------------------------------------------------------
unsigned int svtkExodusIIWriter::GetMaxNameLength()
{
  unsigned int maxName = 32;
  if (svtkMultiBlockDataSet* mb = svtkMultiBlockDataSet::SafeDownCast(this->OriginalInput))
  {
    svtkCompositeDataIterator* iter = mb->NewIterator();
    iter->SkipEmptyNodesOn();
    for (iter->InitTraversal(); !iter->IsDoneWithTraversal(); iter->GoToNextItem())
    {
      if (svtkDataSet* dataSet = svtkDataSet::SafeDownCast(iter->GetCurrentDataObject()))
      {
        ...
        if (svtkInformation* info = iter->GetCurrentMetaData())
        {
          if (const char* objectName = info->Get(svtkCompositeDataSet::NAME()))
          {
  ...
  else if (svtkDataSet* dataSet = svtkDataSet::SafeDownCast(this->OriginalInput))
  {
...
//----------------------------------------------------------------------------
int svtkExodusIIWriter::WriteNextTimeStep()
{
  int rc = 0;
  int ts = this->CurrentTimeIndex - this->FileTimeOffset;
  float tsv = 0.;
  if (this->GetInput()->GetInformation()->Has(svtkDataObject::DATA_TIME_STEP()))
  {
    tsv = this->GetInput()->GetInformation()->Get(svtkDataObject::DATA_TIME_STEP());
  }

  svtkSmartPointer<svtkDataArray> buffer;
  if (this->PassDoubles)
  {
    ...
    if (rc < 0)
    {
      svtkErrorMacro(<< "svtkExodusIIWriter::WriteNextTimeStep time step values"
                    << " fid " << this->fid << " ts " << ts + 1 << " tsv " << tsv);
      return 0;
    }
    buffer = svtkSmartPointer<svtkDoubleArray>::New();
  }
  else
  {
    ...
    buffer = svtkSmartPointer<svtkFloatArray>::New();
  }
...
void svtkExodusIIWriter::CheckBlockInfoMap()
{
  // no op for serial version
}

bool svtkExodusIIWriter::SameTypeOfCells(svtkIntArray* cellToBlockId, svtkUnstructuredGrid* input)
{
  ...
  for (svtkIdType cellId = 0; cellId < cellToBlockId->GetNumberOfTuples(); ++cellId)
  {
    svtkIdType blockId = cellToBlockId->GetValue(cellId);
...
svtkIntArray* svtkExodusIIWriter::GetBlockIdArray(const char* name, svtkUnstructuredGrid* input)
{
  ...
  if (da)
  {
    svtkIntArray* ia = svtkArrayDownCast<svtkIntArray>(da);
    if (ia != nullptr && svtkExodusIIWriter::SameTypeOfCells(ia, input))
    {
    ...
    this->IgnoreMetaDataWarning == 0)
  {
    ...
    svtkWarningMacro(<< "Attempting to proceed without metadata");
  }
  return nullptr;
}
