 ...
 *   svtkDistributedDataFilter looks for metadata attached to
 ...
 *   many files.
 *
 *   One way to initialize this object is by using svtkExodusModel
 *   (a Sandia class used by the Sandia Exodus reader).
 *   That class will take an open Exodus II file and a
 *   svtkUnstructuredGrid drawn from it and will set the required fields.
 *
 ...
 *   SetBlockIds and SetBlockIdArrayName.  Your svtkUnstructuredGrid must
 ...
 *
 * @sa
 *   svtkDistributedDataFilter svtkExtractCells
 */

#ifndef svtkModelMetadata_h
#define svtkModelMetadata_h

#include "svtkIOExodusModule.h" // For export macro
#include "svtkObject.h"
#include "svtkSmartPointer.h" // for svtkSmartPointer
#include "svtkStringArray.h"  // for svtkStringArray
class svtkDataSet;
class svtkCharArray;
...
class SVTKIOEXODUS_EXPORT svtkModelMetadata : public svtkObject
{
public:
  svtkTypeMacro(svtkModelMetadata, svtkObject);
  void PrintSelf(ostream& os, svtkIndent indent) override;
  static svtkModelMetadata* New();
   ...
   * an Exodus file.
   */
  svtkSetMacro(TimeStepIndex, int);
   ...
   */
  svtkSetMacro(NumberOfNodeSets, int);
  int GetNumberOfNodeSets() const { return this->NumberOfNodeSets; }
  //@}

  void SetNodeSetNames(svtkStringArray* names) { this->NodeSetNames = names; }
  svtkStringArray* GetNodeSetNames() const { return this->NodeSetNames; }
   ...
   */
  svtkSetMacro(SumNodesPerNodeSet, int);
   ...
   */
  svtkSetMacro(NumberOfSideSets, int);
  int GetNumberOfSideSets() const { return this->NumberOfSideSets; }
  //@}

  void SetSideSetNames(svtkStringArray* names) { this->SideSetNames = names; }
  svtkStringArray* GetSideSetNames() const { return this->SideSetNames; }
   ...
   */
  svtkSetMacro(SumSidesPerSideSet, int);
   ...
   */
  svtkSetMacro(AllVariablesDefinedInAllBlocks, svtkTypeBool);
  svtkBooleanMacro(AllVariablesDefinedInAllBlocks, svtkTypeBool);
  svtkTypeBool GetAllVariablesDefinedInAllBlocks() const
  {
   ...
   * in the svtkModelMetadata object.  Resetting a particular field,
...
private:
  ...
  svtkModelMetadataSTLCloak* BlockIdIndex; // computed map

  // Node Sets - arrays that are input to the class with Set*

  int NumberOfNodeSets; // (G)

  svtkSmartPointer<svtkStringArray> NodeSetNames;
  ...
  svtkSmartPointer<svtkStringArray> SideSetNames;
  ...
  // Those with similar names were combined into vectors in SVTK.  Here
  // are all the original names from the Exodus file, the names given
  // the variables in the SVTK ugrid, and a mapping from the SVTK names
  // to the Exodus names.
  ...
  svtkTypeBool AllVariablesDefinedInAllBlocks;

private:
  svtkModelMetadata(const svtkModelMetadata&) = delete;
  void operator=(const svtkModelMetadata&) = delete;
};
#endif
