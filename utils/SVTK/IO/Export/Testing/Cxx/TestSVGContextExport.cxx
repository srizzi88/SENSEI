...
{
  void SetSpritePoint(int x, int y, svtkImageData* sprite);
...
void drawPolyLinePolyData(svtkContext2D *painter)
{
  // Setup points
  svtkSmartPointer<svtkPoints> points =
    svtkSmartPointer<svtkPoints>::New();
  ...
  svtkSmartPointer<svtkUnsignedCharArray> colors =
    svtkSmartPointer<svtkUnsignedCharArray>::New();
  ...
  svtkNew<svtkCellArray> polylines;
  ...
  svtkSmartPointer<svtkPolyData> polydata =
    svtkSmartPointer<svtkPolyData>::New();
  polydata->SetPoints(points);
  polydata->SetLines(polylines);
  painter->GetPen()->SetWidth(2.0);
  painter->DrawPolyData(475,  200, polydata, colors, SVTK_SCALAR_MODE_USE_POINT_DATA);
}
...
int TestSVGContextExport(int, char*[])
{
  ...
  std::string filename =
    svtkTestingInteractor::TempDirectory + std::string("/TestSVGContextExport.svg");
...
bool ContextSVGTest::Paint(svtkContext2D* painter)
{
  ...
  for (int i = 0; i < 10; ++i)
  {
    painter->GetPen()->SetLineType(i % (svtkPen::DENSE_DOT_LINE + 1));
  ...
  // Texturing:
  svtkNew<svtkImageData> pattern;
  pattern->SetDimensions(6, 6, 1);
  svtkNew<svtkUnsignedCharArray> patternScalars;
  patternScalars->SetNumberOfComponents(3);
  patternScalars->SetNumberOfTuples(36);
  patternScalars->FillValue(0);
  for (svtkIdType row = 0; row < 6; ++row)
  {
    for (svtkIdType col = 0; col < 6; ++col)
    {
      // Alternate red/blue:
      svtkIdType i = row * 6 + col;
      svtkIdType test = row + col;
  ...
  // Stretching:
  painter->GetBrush()->SetTextureProperties(svtkBrush::Nearest | svtkBrush::Stretch);
  painter->DrawQuad(200, 485, 300, 400, 190, 420, 125, 390);

  // Tiling:
  painter->GetBrush()->SetTextureProperties(svtkBrush::Linear | svtkBrush::Repeat);
  ...
  svtkNew<svtkUnsignedCharArray> spriteScalars;
  ...
  svtkContextDevice2D *dev = painter->GetDevice();
...
//------------------------------------------------------------------------------
void ContextSVGTest::SetSpritePoint(int x, int y, svtkImageData* sprite)
{
