...
svtkStandardNewMacro(svtkIVExporter);

svtkIVExporter::svtkIVExporter()
{
...
#define SVTK_INDENT_MORE                                                                            \
  {                                                                                                \
    indent[indent_now] = ' ';                                                                      \
    indent_now += 4;                                                                               \
    indent[indent_now] = 0;                                                                        \
  }
#define SVTK_INDENT_LESS                                                                            \
  {                                                                                                \
    indent[indent_now] = ' ';                                                                      \
    indent_now -= 4;                                                                               \
    indent[indent_now] = 0;                                                                        \
...
void svtkIVExporter::WriteData()
{
  ...
  {
    svtkErrorMacro(<< "no actors found for writing OpenInventor file.");
    return;
  }
  ...
  fprintf(fp, "%sorientation %g %g %g %g\n%s}\n", indent, tempd[1], tempd[2], tempd[3],
    tempd[0] * svtkMath::Pi() / 180.0, indent);
  SVTK_INDENT_LESS;
...
void svtkIVExporter::WriteALight(svtkLight* aLight, FILE* fp)
{
...
void svtkIVExporter::WriteAnActor(svtkActor* anActor, FILE* fp)
{
  svtkDataSet* ds;
  svtkPolyData* pd;
  svtkGeometryFilter* gf = nullptr;
  ...
  svtkAlgorithmOutput* pdProducer = nullptr;
  // we really want polydata
  if (ds->GetDataObjectType() != SVTK_POLY_DATA)
  {
    gf = svtkGeometryFilter::New();
  ...
  else
  {
    ...
    pd = static_cast<svtkPolyData*>(ds);
  ...
  // the following is based on a guess about how SVTK's GetAmbient
  ...
  if (anActor->GetTexture())
  {
    ...
    {
      mappedScalars = static_cast<svtkUnsignedCharArray*>(scalars);
    }
...
void svtkIVExporter::WritePointData(svtkPoints* points, svtkDataArray* normals, svtkDataArray* tcoords,
...
void svtkIVExporter::PrintSelf(ostream& os, svtkIndent ind)
{
