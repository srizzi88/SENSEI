/*=========================================================================

  Program:   Visualization Toolkit
  Module:    svtkVtkJSViewNodeFactory.cxx

  Copyright (c) Ken Martin, Will Schroeder, Bill Lorensen
  All rights reserved.
  See Copyright.txt or http://www.kitware.com/Copyright.htm for details.

     This software is distributed WITHOUT ANY WARRANTY; without even
     the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR
     PURPOSE.  See the above copyright notice for more information.

=========================================================================*/
#include "svtkJSONRenderWindowExporter.h"

#include <svtkDataObject.h>
#include <svtkDataSet.h>
#include <svtkObjectFactory.h>
#include <svtkRenderWindow.h>
#include <svtkViewNode.h>

#include "svtkArchiver.h"
#include "svtkJSONDataSetWriter.h"
#include "svtkVtkJSSceneGraphSerializer.h"
#include "svtkVtkJSViewNodeFactory.h"

#include <memory>
#include <sstream>

namespace
{
// When exporting a SVTK render window, we must also write the datasets
// associated with the render window into the same archive. To do this, we
// construct an intermediate archiver that neither opens nore closes the
// archive and simply pipes its contents into a subdirectory of a parent
// archive.
class svtkJSONDataSetArchiver : public svtkArchiver
{
public:
  static svtkJSONDataSetArchiver* New();
  svtkTypeMacro(svtkJSONDataSetArchiver, svtkArchiver);

  virtual void SetRenderWindowArchiver(svtkArchiver*);
  svtkGetObjectMacro(RenderWindowArchiver, svtkArchiver);

  virtual void OpenArchive() override {}
  virtual void CloseArchive() override {}
  virtual void InsertIntoArchive(
    const std::string& relativePath, const char* data, std::size_t size) override
  {
    this->RenderWindowArchiver->InsertIntoArchive(this->SubArchiveName(relativePath), data, size);
  }

  virtual bool Contains(const std::string& relativePath) override
  {
    return this->RenderWindowArchiver->Contains(this->SubArchiveName(relativePath));
  }

private:
  svtkJSONDataSetArchiver() { this->RenderWindowArchiver = svtkArchiver::New(); }
  virtual ~svtkJSONDataSetArchiver() override { this->SetRenderWindowArchiver(nullptr); }

  std::string SubArchiveName(const std::string& relativePath)
  {
    return std::string(this->GetArchiveName()) + "/" + relativePath;
  }

  svtkArchiver* RenderWindowArchiver;
};
svtkStandardNewMacro(svtkJSONDataSetArchiver);
svtkCxxSetObjectMacro(svtkJSONDataSetArchiver, RenderWindowArchiver, svtkArchiver);
}

//----------------------------------------------------------------------------
svtkStandardNewMacro(svtkJSONRenderWindowExporter);
svtkCxxSetObjectMacro(svtkJSONRenderWindowExporter, Archiver, svtkArchiver);

//----------------------------------------------------------------------------
svtkJSONRenderWindowExporter::svtkJSONRenderWindowExporter()
{
  this->Serializer = svtkVtkJSSceneGraphSerializer::New();
  this->Archiver = svtkArchiver::New();
  this->Factory = svtkVtkJSViewNodeFactory::New();
  this->Factory->SetSerializer(this->Serializer);
  this->CompactOutput = true;
}

//----------------------------------------------------------------------------
svtkJSONRenderWindowExporter::~svtkJSONRenderWindowExporter()
{
  this->SetSerializer(nullptr);
  this->SetArchiver(nullptr);
  this->Factory->Delete();
}

//----------------------------------------------------------------------------
void svtkJSONRenderWindowExporter::SetSerializer(svtkVtkJSSceneGraphSerializer* args)
{
  svtkDebugMacro(<< this->GetClassName() << " (" << this << "): setting Serializer to " << args);
  if (this->Serializer != args)
  {
    svtkVtkJSSceneGraphSerializer* tempSGMacroVar = this->Serializer;
    this->Serializer = args;
    if (this->Serializer != nullptr)
    {
      this->Serializer->Register(this);
    }
    if (tempSGMacroVar != nullptr)
    {
      tempSGMacroVar->UnRegister(this);
    }
    this->Factory->SetSerializer(this->Serializer);
    this->Modified();
  }
}

//----------------------------------------------------------------------------
void svtkJSONRenderWindowExporter::WriteData()
{
  if (this->GetSerializer() == nullptr)
  {
    svtkErrorMacro(<< "No scene!");
    return;
  }
  this->GetSerializer()->Reset();

  if (this->GetArchiver() == nullptr)
  {
    svtkErrorMacro(<< "No archiver!");
    return;
  }

  if (this->GetArchiver()->GetArchiveName() == nullptr)
  {
    svtkErrorMacro(<< "Please specify Archive Name to use");
    return;
  }

  // Populate the scene instance
  {
    // Construct a top-level node for the render window
    svtkViewNode* vn = this->Factory->CreateNode(this->RenderWindow);

    // Build the scene graph
    vn->Traverse(svtkViewNode::build);

    // Construct the svtk-js representation of the scene graph
    vn->Traverse(svtkViewNode::synchronize);

    // Update the datasets associated with the scene graph
    vn->Traverse(svtkViewNode::render);

    // Delete the top level node
    vn->Delete();
  }

  // Open the archive for writing
  this->GetArchiver()->OpenArchive();

  // Write the top-level index file describing the scene elements and their
  // topology.
  {
    std::stringstream stream;
    Json::StreamWriterBuilder builder;
    builder["commentStyle"] = "None";
    builder["indentation"] = this->CompactOutput ? "" : "  ";
    std::unique_ptr<Json::StreamWriter> writer(builder.newStreamWriter());
    writer->write(this->GetSerializer()->GetRoot(), &stream);

    std::string index = stream.str();
    this->GetArchiver()->InsertIntoArchive("index.json", index.c_str(), index.size());
  }

  // Write the associated data arrays into the archive
  {
    svtkNew<svtkJSONDataSetWriter> dsWriter;
    svtkNew<svtkJSONDataSetArchiver> dsArchiver;
    dsArchiver->SetRenderWindowArchiver(this->GetArchiver());
    dsWriter->SetArchiver(dsArchiver);
    dsWriter->GetArchiver()->SetArchiveName("data");

    for (svtkIdType i = 0; i < this->GetSerializer()->GetNumberOfDataArrays(); ++i)
    {
      std::string daArchiveName = this->GetSerializer()->GetDataArrayId(i);

      // Only write the array if its id (which is its hash) has not yet been
      // added to the archive.
      if (!dsArchiver->Contains(daArchiveName))
      {
        dsWriter->WriteArrayContents(this->GetSerializer()->GetDataArray(i), daArchiveName.c_str());
      }
    }
  }

  // Close the archive
  this->GetArchiver()->CloseArchive();
}

//----------------------------------------------------------------------------
void svtkJSONRenderWindowExporter::PrintSelf(ostream& os, svtkIndent indent)
{
  this->Superclass::PrintSelf(os, indent);
}
