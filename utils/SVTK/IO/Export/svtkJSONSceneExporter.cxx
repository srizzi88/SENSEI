/*=========================================================================
  ...
  Module:    svtkJSONSceneExporter.cxx
...
=========================================================================*/
#include "svtkJSONSceneExporter.h"
...
svtkStandardNewMacro(svtkJSONSceneExporter);

// ----------------------------------------------------------------------------

svtkJSONSceneExporter::svtkJSONSceneExporter()
{
...
void svtkJSONSceneExporter::WriteDataObject(ostream& os, svtkDataObject* dataObject, svtkActor* actor)
{
  ...
  if (dataObject->IsA("svtkDataSet"))
  {
    ...
    std::string dsMeta =
      this->WriteDataSet(svtkDataSet::SafeDownCast(dataObject), addOnMeta.c_str());
    if (!dsMeta.empty())
    {
      os << dsMeta;
    }
  ...
  // Handle composite
  if (dataObject->IsA("svtkCompositeDataSet"))
  {
...
std::string svtkJSONSceneExporter::ExtractRenderingSetup(svtkActor* actor)
{
...
std::string svtkJSONSceneExporter::CurrentDataSetPath() const
{
  std::stringstream path;
  path << this->FileName << "/" << this->DatasetCount + 1;
  return svtksys::SystemTools::ConvertToOutputPath(path.str());
}

// ----------------------------------------------------------------------------

std::string svtkJSONSceneExporter::WriteDataSet(svtkDataSet* dataset, const char* addOnMeta = nullptr)
{
  if (!dataset)
  {
    return "";
  ...
  auto* polyData = svtkPolyData::SafeDownCast(dataset);
  svtkSmartPointer<svtkPolyData> newDataset;
  std::string polyLODsConfig;
  if (this->WritePolyLODs && polyData)
  {
    newDataset = this->WritePolyLODSeries(polyData, polyLODsConfig);
    // Write the smallest poly LOD to the svtkjs file
    dataset = newDataset.Get();
  }

  svtkNew<svtkJSONDataSetWriter> dsWriter;
...
void svtkJSONSceneExporter::WriteLookupTable(const char* name, svtkScalarsToColors* lookupTable)
{
  ...
  svtkDiscretizableColorTransferFunction* dctfn =
    svtkDiscretizableColorTransferFunction::SafeDownCast(lookupTable);
  if (dctfn != nullptr)
  {
    const char* INDENT = "    ";
    ...
    // Fill nodes
    svtkIdType nbNodes = dctfn->GetSize();
    double node[6];
    for (svtkIdType i = 0; i < nbNodes; i++)
    {
...
void svtkJSONSceneExporter::WriteData()
{
  this->DatasetCount = 0;
  ...
  if (!svtksys::SystemTools::MakeDirectory(this->FileName))
  {
    svtkErrorMacro(<< "Can not create directory " << this->FileName);
    return;
  }

  svtkRenderer* renderer = this->GetActiveRenderer();
  if (!renderer)
  {
  ...
  svtkPropCollection* renProps = renderer->GetViewProps();
  svtkIdType nbProps = renProps->GetNumberOfItems();
  for (svtkIdType rpIdx = 0; rpIdx < nbProps; rpIdx++)
  {
    svtkProp* renProp = svtkProp::SafeDownCast(renProps->GetItemAsObject(rpIdx));
    ...
    svtkActor* actor = svtkActor::SafeDownCast(renProp);
    if (!actor)
    {
      continue;
    }
    ...
    svtkMapper* mapper = actor->GetMapper();
    svtkDataObject* dataObject = mapper->GetInputDataObject(0, 0);
...
size_t getFileSize(const std::string& path)
{
  ...
  svtksys::SystemTools::Stat_t stat_buf;
  int res = svtksys::SystemTools::Stat(path, &stat_buf);
  if (res < 0)
  {
...
std::string svtkJSONSceneExporter::WriteTexture(svtkTexture* texture)
{
  ...
  // Make sure it exists
  if (!svtksys::SystemTools::MakeDirectory(path))
  {
    svtkErrorMacro(<< "Cannot create directory " << path);
    return "";
  }

  path += "/texture.jpg";
  path = svtksys::SystemTools::ConvertToOutputPath(path);

  svtkSmartPointer<svtkImageData> image = texture->GetInput();
...
std::string svtkJSONSceneExporter::WriteTextureLODSeries(svtkTexture* texture)
{
  // If this texture has already been written, just re-use the one
  // we have.
  ...
  std::string path = svtksys::SystemTools::GetParentDirectory(this->FileName);
  path += "/";
  path = svtksys::SystemTools::ConvertToOutputPath(path);
  ...
  while (true)
  {
    ...
    svtkNew<svtkImageResize> shrink;
...
svtkSmartPointer<svtkPolyData> svtkJSONSceneExporter::WritePolyLODSeries(
  svtkPolyData* dataset, std::string& polyLODsConfig)
{
  svtkSmartPointer<svtkPolyData> polyData = dataset;
  std::vector<std::string> files;
  ...
  while (true)
  {
    ...
    if (static_cast<size_t>(dataSize) * 1000 <= baseSize || tooSimilar)
    {
      ...
      // The latest "polyData" will be written into the .svtkjs directory
      break;
    }
    ...
    svtkNew<svtkQuadricClustering> cc;
    cc->UseInputPointsOn();
    cc->CopyCellDataOn();
    ...
    {
      svtkNew<svtkQuadricClustering> defaultCC;
...
void svtkJSONSceneExporter::PrintSelf(ostream& os, svtkIndent indent)
{
