...
svtkStandardNewMacro(svtkOOGLExporter);

svtkOOGLExporter::svtkOOGLExporter()
{
...
void svtkOOGLExporter::WriteData()
{
  FILE* fp;
  int i, j;
  ...
  {
    svtkErrorMacro(<< "no actors found for writing Geomview OOGL file.");
    return;
  }
  ...
  if (!fp)
  {
    svtkErrorMacro(<< "unable to open Geomview OOGL file " << this->FileName);
    return;
  }
  ...
  //  Write header
  //
  svtkDebugMacro("Writing Geomview OOGL file");
...
void svtkOOGLExporter::WriteALight(svtkLight* aLight, FILE* fp)
{
...
void svtkOOGLExporter::WriteAnActor(svtkActor* anActor, FILE* fp, int count)
{
  svtkDataSet* ds;
  svtkPolyData* pd;
  ...
  // usage of GetColors() has been deprecated in SVTK 4.0
...
void svtkOOGLExporter::PrintSelf(ostream& os, svtkIndent ind)
{
  svtkExporter::PrintSelf(os, ind);
