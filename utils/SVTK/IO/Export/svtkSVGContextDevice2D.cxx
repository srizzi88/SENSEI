/*=========================================================================
  ...
  Module:    svtkSVGContextDevice2D.cxx
...
#include "svtkSVGContextDevice2D.h"
...
struct EllipseHelper
{
  ...
  void UpdateDegrees(float degrees) { this->UpdateRadians(svtkMath::RadiansFromDegrees(degrees)); }
...
struct FontKey
{
  ...
  explicit FontKey(svtkTextProperty* tprop)
    : TextProperty(svtkSmartPointer<svtkTextProperty>::New())
  {
  ...
  bool operator<(const FontKey& other) const
  {
    ...
    if (thisFontFamily == SVTK_FONT_FILE)
    {
...
struct FontInfo
{
  using CharType = svtkUnicodeString::value_type;
  ...
  void ProcessString(const svtkUnicodeString& str)
  {
    svtkUnicodeString::const_iterator it = str.begin();
    ...
    svtkUnicodeString::const_iterator next = it;
    std::advance(next, 1);
    while (next != end)
    {
...
struct ImageInfo
{
  explicit ImageInfo(svtkImageData* img)
  {
    ...
    svtkUnsignedCharArray* png = pngWriter->GetResult();
    ...
    svtkNew<svtkBase64OutputStream> base64Encoder;
...
struct PatternInfo
{
  explicit PatternInfo(const ImageInfo& img, int textureProperty)
    ...
    : TextureProperty(textureProperty & (svtkBrush::Repeat | svtkBrush::Stretch))
    , ImageSize(img.Size)
    , ImageId(img.Id)
  {
  }
...
// SVG's y axis is inverted compared to SVTK's:
struct YConverter
{
  float Height;
...
// Need to be able to use svtkColor4f in a std::map. Must be outside of the anon
// namespace to work.
static bool operator<(const svtkColor4f& a, const svtkColor4f& b)
{
  for (int i = 0; i < 4; ++i)
  {
    if (a[i] < b[i])
...
struct svtkSVGContextDevice2D::Details
{
  ...
  const ImageInfo& GetImageInfo(svtkImageData* img)
  {
    ...
    if (insertResult.second)
    {
      ...
      id << "svtkEmbeddedImage" << this->ImageSet.size();
  ...
  const PatternInfo& GetPatternInfo(svtkImageData* texture, int textureProperty)
  {
    ...
    {
      std::ostringstream id;
      id << "svtkPattern" << this->PatternSet.size();
      // This is safe; setting the id won't change the sort order in the set.
  ...
  const ClipRectInfo& GetClipRectInfo(const std::array<int, 4>& rect)
  {
    ...
    {
      std::ostringstream id;
      id << "svtkClipRect" << this->ClipRectSet.size();
      // This is safe; setting the id won't change the sort order in the set.
  ...
  FontInfo& GetFontInfo(svtkTextProperty* tprop)
  {
    FontKey key(tprop);
    ...
    if (it == this->FontMap.end())
    {
      ...
      tmp << "svtkExportedFont-" << std::hex << this << "_" << std::dec << this->FontMap.size()
...
//------------------------------------------------------------------------------
svtkStandardNewMacro(svtkSVGContextDevice2D);
svtkCxxSetObjectMacro(svtkSVGContextDevice2D, Viewport, svtkViewport);

//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::PrintSelf(std::ostream& os, svtkIndent indent)
{
  this->Superclass::PrintSelf(os, indent);
}

//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::SetSVGContext(svtkXMLDataElement* context, svtkXMLDataElement* defs)
{
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::GenerateDefinitions()
{
  if (this->EmbedFonts)
  {
    this->WriteFonts();
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::Begin(svtkViewport* vp)
{
  ...
  this->SetViewport(vp);
  this->CanvasHeight = static_cast<float>(vp->GetSVTKWindow()->GetSize()[1]);
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::End()
{
  this->SetViewport(nullptr);
}

//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::DrawPoly(float* points, int n, unsigned char* colors, int nc_comps)
{
  if (!colors)
  {
    svtkNew<svtkXMLDataElement> polyLine;
  ...
  else
  {
    ...
    for (int i = 0; i < n - 1; ++i)
    {
      const svtkVector2f p1(points + i * 2);
      const svtkColor4ub c1(colors + i * nc_comps);
      const svtkVector2f p2(points + (i + 1) * 2);
      const svtkColor4ub c2(colors + (i + 1) * nc_comps);
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::DrawLines(float* points, int n, unsigned char* colors, int nc_comps)
{
  if (!colors)
  {
    // Use path instead of lines for a more efficient/compact representation:
    svtkNew<svtkXMLDataElement> path;
    path->SetName("path");
  ...
  else
  {
    ...
    for (int i = 0; i < numLines; ++i)
    {
      const svtkVector2f p1(points + i * 4);
      const svtkVector2f p2(points + i * 4 + 2);
      const svtkColor4ub c1(colors + i * 2 * nc_comps);
      const svtkColor4ub c2(colors + (i * 2 + 1) * nc_comps);
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::DrawPoints(float* points, int n, unsigned char* colors, int nc_comps)
{
  if (!colors)
  {
  ...
  else
  {
    ...
    for (int i = 0; i < n; ++i)
    {
      ...
      svtkNew<svtkXMLDataElement> point;
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::DrawPointSprites(
  svtkImageData* spriteIn, float* points, int n, unsigned char* colors, int nc_comps)
{
  if (nc_comps != 3 && nc_comps != 4)
  {
    svtkErrorMacro("Unsupported number of components: " << nc_comps);
    return;
  }

  svtkImageData* rgba = this->PreparePointSprite(spriteIn);
  if (!rgba)
  {
    svtkErrorMacro("Unsupported point sprite format.");
    return;
  }

  assert(rgba->GetScalarType() == SVTK_UNSIGNED_CHAR);
  assert(rgba->GetNumberOfScalarComponents() == 4);

  int dims[3];
  rgba->GetDimensions(dims);
  svtkIdType numPoints = rgba->GetNumberOfPoints();
  svtkUnsignedCharArray* colorArray =
    svtkArrayDownCast<svtkUnsignedCharArray>(rgba->GetPointData()->GetScalars());
  ...
  typedef std::map<svtkColor4f, std::string> SpriteMap;
  SpriteMap spriteMap;
  ...
  for (int i = 0; i < n; ++i)
  {
    ...
    if (colors)
    {
      ...
      switch (nc_comps)
      {
        ...
        default:
          svtkErrorMacro("Unsupported number of color components: " << nc_comps);
          continue;
      }
    }
    else
    {
      svtkColor4ub penColor = this->Pen->GetColorObject();
      color = svtkColor4f(
    ...
    else
    {
      ...
      svtkNew<svtkImageData> spriteImage;
    ...
    svtkNew<svtkXMLDataElement> use;
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::DrawMarkers(
  int shape, bool highlight, float* points, int n, unsigned char* colors, int nc_comps)
{
  ...
  if (!colors)
  {
    ...
    for (int i = 0; i < n; ++i)
    {
      ...
      svtkNew<svtkXMLDataElement> node;
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::DrawQuad(float* points, int n)
{
  this->DrawPolygon(points, n);
}

//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::DrawQuadStrip(float* points, int n)
{
  if (n < 4 || n % 2 != 0)
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::DrawPolygon(float* points, int n)
{
  svtkNew<svtkXMLDataElement> path;
  path->SetName("path");
  this->ActiveNode->AddNestedElement(path);
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::DrawColoredPolygon(
  float* points, int numPoints, unsigned char* colors, int nc_comps)
{
  ...
  if (sameColor)
  {
    const svtkColor4ub oldBrush = this->Brush->GetColorObject();
    switch (nc_comps)
    {
      case 4:
      ...
      default:
        svtkWarningMacro("Unsupported number of color components: " << nc_comps);
        return;
    }
  ...
  const bool useAlpha = nc_comps == 4;
  const svtkVector2f p0(points);
  const svtkColor4ub c0(colors);

  // We may have 3 or 4 components, so initialize these with a sane alpha value:
  svtkColor4ub c1{ 0, 0, 0, 255 };
  svtkColor4ub c2{ 0, 0, 0, 255 };

  for (int i = 1; i < numPoints - 1; ++i)
  {
    const svtkVector2f p1(points + 2 * i);
    const svtkVector2f p2(points + 2 * (i + 1));
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::DrawEllipseWedge(float cx, float cy, float outRx, float outRy,
  float inRx, float inRy, float startAngle, float stopAngle)
{
  ...
  if (!isArc)
  {
    if (isFilled)
    {
      ...
      if (isCircle)
      {
        svtkNew<svtkXMLDataElement> circle;
      ...
      {
        svtkNew<svtkXMLDataElement> ellipse;
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::DrawEllipticArc(
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::DrawString(float* point, const svtkStdString& string)
{
  this->DrawString(point, svtkUnicodeString::from_utf8(string));
}

//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::ComputeStringBounds(const svtkStdString& string, float bounds[4])
{
  this->ComputeStringBounds(svtkUnicodeString::from_utf8(string), bounds);
}

//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::DrawString(float* point, const svtkUnicodeString& string)
{
  svtkTextRenderer* tren = svtkTextRenderer::GetInstance();
  if (!tren)
  {
    svtkErrorMacro("svtkTextRenderer unavailable. Link to svtkRenderingFreeType "
                  "to get the default implementation.");
    return;
  }

  int backend = this->TextAsPath ? svtkTextRenderer::Default : tren->DetectBackend(string);

  if (backend == svtkTextRenderer::FreeType)
  {
    // Embed freetype text and fonts in the SVG:
    FontInfo& info = this->Impl->GetFontInfo(this->TextProp);
    info.ProcessString(string);

    svtkNew<svtkXMLDataElement> text;
  ...
  else
  {
    ...
    svtkNew<svtkPath> tPath;
    int dpi = this->Viewport->GetSVTKWindow()->GetDPI();
    if (!tren->StringToPath(this->TextProp, string, tPath, dpi, backend))
    {
      svtkErrorMacro("Error generating path for MathText string '" << string << "'.");
      return;
    }
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::ComputeStringBounds(const svtkUnicodeString& string, float bounds[4])
{
  svtkTextRenderer* tren = svtkTextRenderer::GetInstance();
  if (!tren)
  {
  ...
  assert(this->Viewport && this->Viewport->GetSVTKWindow());
  int dpi = this->Viewport->GetSVTKWindow()->GetDPI();

  svtkTextRenderer::Metrics m;
  if (!tren->GetMetrics(this->TextProp, string, m, dpi))
  {
    svtkErrorMacro("Error computing bbox for string '" << string << "'.");
    std::fill(bounds, bounds + 4, 0.f);
    return;
  }
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::ComputeJustifiedStringBounds(const char* string, float bounds[4])
{
  this->ComputeStringBounds(svtkUnicodeString::from_utf8(string), bounds);
}

//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::DrawMathTextString(float* point, const svtkStdString& str)
{
  this->DrawString(point, str);
}

//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::DrawImage(float p[2], float scale, svtkImageData* image)
{
  int dims[3];
  image->GetDimensions(dims);
  dims[0] *= scale;
  dims[1] *= scale;
  this->DrawImage(svtkRectf(p[0], p[1], dims[0], dims[1]), image);
}

//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::DrawImage(const svtkRectf& pos, svtkImageData* image)
{
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::SetColor4(unsigned char[])
{
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::SetTexture(svtkImageData* image, int properties)
{
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::SetPointSize(float size)
{
  this->Pen->SetWidth(size);
}

//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::SetLineWidth(float width)
{
  this->Pen->SetWidth(width);
}

//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::SetLineType(int type)
{
  this->Pen->SetLineType(type);
}

//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::SetMatrix(svtkMatrix3x3* m)
{
  // Adjust the transform to account for the fact that SVG's y-axis is reversed:
  std::array<double, 9> mat3;
  svtkSVGContextDevice2D::AdjustMatrixForSVG(m->GetData(), mat3.data());

  std::array<double, 16> mat4;
  svtkSVGContextDevice2D::Matrix3ToMatrix4(mat3.data(), mat4.data());
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::GetMatrix(svtkMatrix3x3* mat3)
{
  svtkSVGContextDevice2D::Matrix4ToMatrix3(this->Matrix->GetMatrix()->GetData(), mat3->GetData());
  svtkSVGContextDevice2D::AdjustMatrixForSVG(mat3->GetData(), mat3->GetData());
}

//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::MultiplyMatrix(svtkMatrix3x3* m)
{
  // Adjust the transform to account for the fact that SVG's y-axis is reversed:
  std::array<double, 9> mat3;
  svtkSVGContextDevice2D::AdjustMatrixForSVG(m->GetData(), mat3.data());
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::PushMatrix()
{
  this->Matrix->Push();
}

//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::PopMatrix()
{
  this->Matrix->Pop();
  this->ApplyTransform();
}
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::SetClipping(int* x)
{
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::EnableClipping(bool enable)
{
...
//------------------------------------------------------------------------------
svtkSVGContextDevice2D::svtkSVGContextDevice2D()
  : Impl(new Details)
  , Viewport(nullptr)
  , ContextNode(nullptr)
  , ActiveNode(nullptr)
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::PushGraphicsState()
{
  svtkNew<svtkXMLDataElement> newGState;
  newGState->SetName("g");
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::PopGraphicsState()
{
  if (this->ActiveNode == this->ContextNode)
  {
    svtkErrorMacro("Internal error: Attempting to pop graphics state past "
                  "context node. This likely means there's a pop with no "
                  "corresponding push.");
    return;
  }

  svtkXMLDataElement* oldActive = this->ActiveNode;
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::SetupClippingAndTransform()
{
  ...
  {
    svtkErrorMacro("This method must only be called when there is, at most, one "
  ...
  const bool transformChanged =
    svtkSVGContextDevice2D::Transform2DEqual(this->ActiveNodeTransform.data(), mat4);
  ...
  svtkSVGContextDevice2D::Matrix4ToMatrix3(mat4, this->ActiveNodeTransform.data());
  ...
  const bool isIdentity = svtkSVGContextDevice2D::Transform2DEqual(ident.data(), mat4);
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::ApplyPenStateToNode(svtkXMLDataElement* node)
{
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::ApplyPenColorToNode(svtkXMLDataElement* node)
{
  node->SetAttribute("stroke", ColorToString(this->Pen->GetColor()).c_str());
}

//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::ApplyPenOpacityToNode(svtkXMLDataElement* node)
{
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::ApplyPenWidthToNode(svtkXMLDataElement* node)
{
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::ApplyPenStippleToNode(svtkXMLDataElement* node)
{
  // These match the OpenGL2 implementation:
  switch (this->Pen->GetLineType())
  {
    default:
      svtkErrorMacro("Unknown line type: " << this->Pen->GetLineType());
      SVTK_FALLTHROUGH;
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::ApplyPenAsFillColorToNode(svtkXMLDataElement* node)
{
  node->SetAttribute("fill", ColorToString(this->Pen->GetColor()).c_str());
}

//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::ApplyPenAsFillOpacityToNode(svtkXMLDataElement* node)
{
  if (this->Pen->GetOpacity() != 255)
  {
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::ApplyBrushStateToNode(svtkXMLDataElement* node)
{
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::ApplyBrushColorToNode(svtkXMLDataElement* node)
{
  node->SetAttribute("fill", ColorToString(this->Brush->GetColor()).c_str());
}

//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::ApplyBrushOpacityToNode(svtkXMLDataElement* node)
{
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::ApplyBrushTextureToNode(svtkXMLDataElement* node)
{
  svtkImageData* img = this->Brush->GetTexture();
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::ApplyTextPropertyStateToNode(svtkXMLDataElement* node, float x, float y)
{
  svtkFreeTypeTools* ftt = svtkFreeTypeTools::GetInstance();
  if (!ftt)
  {
    svtkErrorMacro("Error embedding fonts: No svtkFreeTypeTools instance "
                  "available.");
    return;
  }
  ...
  using FaceMetrics = svtkFreeTypeTools::FaceMetrics;
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::ApplyTextPropertyStateToNodeForPath(
  svtkXMLDataElement* node, float x, float y)
{
  svtkVector3d colord;
  this->TextProp->GetColor(colord.GetData());
  svtkColor3ub color = {
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::ApplyTransform()
{
  this->SetupClippingAndTransform();
}

//------------------------------------------------------------------------------
std::string svtkSVGContextDevice2D::AddCrossSymbol(bool)
{
  ...
  {
    svtkNew<svtkXMLDataElement> symbol;
...
//------------------------------------------------------------------------------
std::string svtkSVGContextDevice2D::AddPlusSymbol(bool)
{
  std::ostringstream idStream;
  idStream << "Plus";
...
//------------------------------------------------------------------------------
std::string svtkSVGContextDevice2D::AddSquareSymbol(bool)
{
  std::ostringstream idStream;
  idStream << "Square";
  ...
  if (!this->DefinitionNode->FindNestedElementWithNameAndId("symbol", id.c_str()))
  {
    ...
    svtkNew<svtkXMLDataElement> rect;
...
//------------------------------------------------------------------------------
std::string svtkSVGContextDevice2D::AddCircleSymbol(bool)
{
  std::ostringstream idStream;
  idStream << "Circle";
...
//------------------------------------------------------------------------------
std::string svtkSVGContextDevice2D::AddDiamondSymbol(bool)
{
  std::ostringstream idStream;
  idStream << "Diamond";
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::DrawPath(svtkPath* path, std::ostream& out)
{
  // The text renderer always uses floats to generate paths, so we'll optimize
  // a bit here:
  svtkFloatArray* points = svtkArrayDownCast<svtkFloatArray>(path->GetPoints()->GetData());
  svtkIntArray* codes = path->GetCodes();

  if (!points)
  {
    svtkErrorMacro("This method expects the path point precision to be floats.");
    return;
  }

  svtkIdType numTuples = points->GetNumberOfTuples();
  if (numTuples != codes->GetNumberOfTuples() || codes->GetNumberOfComponents() != 1 ||
    points->GetNumberOfComponents() != 3)
  {
    svtkErrorMacro("Invalid path data.");
    return;
  }
  ...
  typedef svtkPath::ControlPointType CodeEnum;
  typedef svtkIntArray::ValueType CodeType;
  CodeType* code = codes->GetPointer(0);
  CodeType* codeEnd = code + numTuples;

  typedef svtkFloatArray::ValueType PointType;
  ...
  while (code < codeEnd)
  {
    ...
    switch (static_cast<CodeEnum>(*code))
    {
      ...
      case svtkPath::CONIC_CURVE:
        assert(CodeEnum(code[1]) == svtkPath::CONIC_CURVE);
        if (lastCode != *code)
        {
          lastCode = *code;
          out << "Q";
      ...
      case svtkPath::CUBIC_CURVE:
        assert(CodeEnum(code[1]) == svtkPath::CUBIC_CURVE);
        assert(CodeEnum(code[2]) == svtkPath::CUBIC_CURVE);
        if (lastCode != *code)
        {
          lastCode = *code;
          out << "C";
      ...
      default:
        svtkErrorMacro("Unknown control code.");
    }
  }
}

//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::DrawLineGradient(const svtkVector2f& p1, const svtkColor4ub& c1,
  const svtkVector2f& p2, const svtkColor4ub& c2, bool useAlpha)
{
  const svtkColor4ub aveColor = { static_cast<unsigned char>(static_cast<int>(c1[0] + c2[0]) / 2),
  ...
  {
    YConverter y(this->CanvasHeight);
    svtkNew<svtkXMLDataElement> line;
  ...
  const svtkVector2f avePos = (p1 + p2) * 0.5;
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::DrawTriangleGradient(const svtkVector2f& p1, const svtkColor4ub& c1,
  const svtkVector2f& p2, const svtkColor4ub& c2, const svtkVector2f& p3, const svtkColor4ub& c3,
  bool useAlpha)
{
  ...
  {
    YConverter y(this->CanvasHeight);
    const svtkColor4ub aveColor = { static_cast<unsigned char>(
    ...
    svtkNew<svtkXMLDataElement> polygon;
  ...
  //  +----+----+
  // v2   v23   v3
  const svtkVector2f p12 = (p1 + p2) * 0.5;
  const svtkVector2f p23 = (p2 + p3) * 0.5;
  const svtkVector2f p13 = (p1 + p3) * 0.5;
  const svtkColor4ub c12 = { static_cast<unsigned char>(static_cast<int>(c1[0] + c2[0]) / 2),
    static_cast<unsigned char>(static_cast<int>(c1[1] + c2[1]) / 2),
    static_cast<unsigned char>(static_cast<int>(c1[2] + c2[2]) / 2),
    static_cast<unsigned char>(static_cast<int>(c1[3] + c2[3]) / 2) };
  const svtkColor4ub c23 = { static_cast<unsigned char>(static_cast<int>(c2[0] + c3[0]) / 2),
  ...
  const svtkColor4ub c13 = { static_cast<unsigned char>(static_cast<int>(c1[0] + c3[0]) / 2),
...
//------------------------------------------------------------------------------
bool svtkSVGContextDevice2D::AreaLessThanTolerance(
  const svtkVector2f& p1, const svtkVector2f& p2, const svtkVector2f& p3)
{
...
//------------------------------------------------------------------------------
bool svtkSVGContextDevice2D::LengthLessThanTolerance(const svtkVector2f& p1, const svtkVector2f& p2)
{
  return (p2 - p1).SquaredNorm() < this->SubdivisionThreshold;
}

//------------------------------------------------------------------------------
bool svtkSVGContextDevice2D::ColorsAreClose(
  const svtkColor4ub& c1, const svtkColor4ub& c2, bool useAlpha)
{
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::WriteFonts()
{
  svtkFreeTypeTools* ftt = svtkFreeTypeTools::GetInstance();
  if (!ftt)
  {
  ...
  using FaceMetrics = svtkFreeTypeTools::FaceMetrics;
  using GlyphOutline = svtkFreeTypeTools::GlyphOutline;
  ...
  for (const auto& fontEntry : this->Impl->FontMap)
  {
    ...
    {
      svtkWarningMacro("Cannot embed non-scalable fonts (referring to font file: "
        << key.TextProperty->GetFontFile() << ")");
      continue;
    }

    svtkNew<svtkXMLDataElement> font;
    ...
    svtkNew<svtkXMLDataElement> face;
    ...
    for (auto charId : info->Chars)
    {
      ...
      svtkUnicodeString unicode(1, charId);

      svtkNew<svtkXMLDataElement> glyph;
      face->AddNestedElement(glyph);
      glyph->SetName("glyph");
      glyph->SetAttributeEncoding(SVTK_ENCODING_UTF_8);
    ...
    {
      const svtkUnicodeString unicode1(1, charPair.first);
      const svtkUnicodeString unicode2(1, charPair.second);
      ...
      svtkNew<svtkXMLDataElement> hkern;
      font->AddNestedElement(hkern);
      hkern->SetName("hkern");
      hkern->SetAttributeEncoding(SVTK_ENCODING_UTF_8);
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::WriteImages()
{
  for (const ImageInfo& info : this->Impl->ImageSet)
  {
    svtkNew<svtkXMLDataElement> image;
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::WritePatterns()
{
  for (const PatternInfo& info : this->Impl->PatternSet)
  {
    svtkNew<svtkXMLDataElement> pattern;
    ...
    // We only care about Repeat and Stretch, since SVG doesn't allow control
    // over Nearest/Linear interpolation.
    const bool isTiled = ((info.TextureProperty & svtkBrush::Repeat) != 0);
    if (isTiled)
    {
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::WriteClipRects()
{
  for (const auto& info : this->Impl->ClipRectSet)
  {
    svtkNew<svtkXMLDataElement> clipPath;
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::AdjustMatrixForSVG(const double in[9], double out[9])
{
  // Adjust the transform to account for the fact that SVG's y-axis is reversed:
  //
  // [S] = [T]^-1 [V] [T]
  ...
  // [V] is the transform in SVTK space (inputs from Context2D API).
  //       | 1  0  0 |
  ...
  std::array<double, 9> SVTKToSVG;
  svtkSVGContextDevice2D::GetSVGMatrix(SVTKToSVG.data());
  svtkMatrix3x3::Multiply3x3(SVTKToSVG.data(), in, tmpMat3.data());
  svtkMatrix3x3::Multiply3x3(tmpMat3.data(), SVTKToSVG.data(), out);
}

//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::GetSVGMatrix(double svg[9])
{
  svg[0] = 1.;
  svg[1] = 0.;
  svg[2] = 0.;
...
//------------------------------------------------------------------------------
bool svtkSVGContextDevice2D::Transform2DEqual(const double mat3[9], const double mat4[16])
{
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::Matrix3ToMatrix4(const double mat3[9], double mat4[16])
{
  mat4[0] = mat3[0];
  mat4[1] = mat3[1];
  mat4[2] = 0.;
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::Matrix4ToMatrix3(const double mat4[16], double mat3[9])
{
  mat3[0] = mat4[0];
  mat3[1] = mat4[1];
  mat3[2] = mat4[3];
...
//------------------------------------------------------------------------------
float svtkSVGContextDevice2D::GetScaledPenWidth()
{
  float x, y;
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::GetScaledPenWidth(float& x, float& y)
{
...
//------------------------------------------------------------------------------
void svtkSVGContextDevice2D::TransformSize(float& x, float& y)
{
  // Get current 3x3 SVG transform:
  std::array<double, 9> m;
  svtkSVGContextDevice2D::Matrix4ToMatrix3(this->Matrix->GetMatrix()->GetData(), m.data());

  // Invert it (we want to go from local space --> global space)
  svtkMatrix3x3::Invert(m.data(), m.data());
...
//------------------------------------------------------------------------------
svtkImageData* svtkSVGContextDevice2D::PreparePointSprite(svtkImageData* in)
{
  ...
  {
    svtkWarningMacro("Images with " << numComps << " components not supported.");
    return nullptr;
  }

  // Need to convert scalar type?
  if (in->GetScalarType() != SVTK_UNSIGNED_CHAR)
  {
    svtkNew<svtkImageCast> cast;
  ...
  { // If RGB, append a constant alpha.
    ...
    if (!data)
    {
      svtkErrorMacro("Internal error: svtkImageCast failed.");
      in->UnRegister(this);
      return nullptr;
    }

    svtkIdType numTuples = data->GetNumberOfTuples();
    svtkNew<svtkUnsignedCharArray> newData;
    ...
    SVTK_ASSUME(data->GetNumberOfComponents() == 3);
    SVTK_ASSUME(newData->GetNumberOfComponents() == 4);
