...
//------------------------------------------------------------------------------
svtkStandardNewMacro(svtkSVGExporter);

//------------------------------------------------------------------------------
void svtkSVGExporter::PrintSelf(std::ostream& os, svtkIndent indent)
{
  this->Superclass::PrintSelf(os, indent);
}

//------------------------------------------------------------------------------
svtkSVGExporter::svtkSVGExporter()
  : Title(nullptr)
  , Description(nullptr)
  , FileName(nullptr)
  , Device(nullptr)
  ...
  , TextAsPath(true)
{
  ...
  this->SetDescription("SVTK Exported Scene");
}
...
//------------------------------------------------------------------------------
void svtkSVGExporter::WriteData()
{
  ...
  {
    svtkErrorMacro("No RenderWindow set -- nothing to export.");
    return;
  }
...
//------------------------------------------------------------------------------
void svtkSVGExporter::PrepareDocument()
{
  const int* size = this->RenderWindow->GetSize();

  this->RootNode = svtkXMLDataElement::New();
  ...
  {
    svtkNew<svtkXMLDataElement> title;
    title->SetName("title");
  ...
  this->Device = svtkSVGContextDevice2D::New();
...
//------------------------------------------------------------------------------
void svtkSVGExporter::RenderContextActors()
{
...
//------------------------------------------------------------------------------
void svtkSVGExporter::RenderBackground(svtkRenderer* ren)
{
  if (ren->Transparent())
  {
    return;
  ...
  svtkRectf renRect(renOrigin[0], renOrigin[1], renSize[0], renSize[1]);

  svtkNew<svtkContext2D> ctx;
  ctx->Begin(this->Device);
  this->Device->Begin(ren);

  if (ren->GetTexturedBackground())
  {
    svtkTexture* tex = ren->GetBackgroundTexture();
    svtkImageData* image = tex->GetInput();
  ...
  else if (ren->GetGradientBackground())
  {
    ...
    const float canvasHeight = ren->GetSVTKWindow()->GetSize()[1];

    svtkNew<svtkXMLDataElement> gradient;
...
//------------------------------------------------------------------------------
void svtkSVGExporter::RenderContextActor(svtkContextActor* actor, svtkRenderer* ren)
{
