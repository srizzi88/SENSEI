/*=========================================================================
  ...
  Module:    svtkSingleVTPExporter.cxx
...
svtkStandardNewMacro(svtkSingleVTPExporter);

svtkSingleVTPExporter::svtkSingleVTPExporter()
{
...
void svtkSingleVTPExporter::SetFileName(const char* fn)
{
...
void svtkSingleVTPExporter::WriteData()
{
  svtkRenderer* ren;
...
void svtkSingleVTPExporter::ProcessTriangle(const svtkIdType* pts, svtkPolyData* opd)
{
  svtkCellArray* newPolys = opd->GetPolys();
  svtkPoints* opts = opd->GetPoints();
  svtkPointData* optd = opd->GetPointData();
  svtkDataArray* otc = opd->GetPointData()->GetTCoords();
...
svtkPolyData* svtkSingleVTPExporter::FixTextureCoordinates(svtkPolyData* ipd)
{
  // do we have tcoords and are they out of range, if not just return
  svtkPolyData* opd = svtkPolyData::New();
  svtkNew<svtkPoints> opts;
  opts->SetDataTypeToDouble();
  opts->DeepCopy(ipd->GetPoints());
  opd->SetPoints(opts);
  svtkPointData* iptd = ipd->GetPointData();
  svtkPointData* optd = opd->GetPointData();
  optd->CopyAllOn();
  ...
  {
    svtkCellArray* cells = ipd->GetPolys();
...
void svtkSingleVTPExporter::WriteVTP(std::vector<actorData>& actors)
{
  ...
  svtkNew<svtkPolyData> opd;
  svtkNew<svtkPoints> opts;
  opts->SetDataTypeToDouble();
  opd->SetPoints(opts);
  svtkNew<svtkCellArray> overts;
  svtkNew<svtkCellArray> olines;
  svtkNew<svtkCellArray> opolys;
  svtkNew<svtkCellArray> ostrips;
  opd->SetVerts(overts);
  opd->SetLines(olines);
  opd->SetPolys(opolys);
  opd->SetStrips(ostrips);
  svtkNew<svtkFloatArray> otcoords;
  otcoords->SetNumberOfComponents(2);
  opd->GetPointData()->SetTCoords(otcoords);
  svtkNew<svtkUnsignedCharArray> oscalars;
  ...
  for (size_t i = 0; i < actors.size(); ++i)
  {
    ...
    svtkPolyData* mypd = findPolyData(ad->Actor->GetMapper()->GetInputDataObject(0, 0));
    svtkDataArray* norms = mypd->GetPointData()->GetNormals();
    if (!norms)
    {
      haveNormals = false;
      break;
  ...
  if (haveNormals)
  {
    svtkNew<svtkFloatArray> otnormals;
  ...
  {
    actorData* ad = &(actors[i]);
    svtkProperty* prop = ad->Actor->GetProperty();
  ...
  for (size_t i = 0; i < actors.size(); ++i)
  {
    ...
    triFilter->Update();
    svtkPolyData* ipd = triFilter->GetOutput();
    svtkPoints* ipts = ipd->GetPoints();
    if (!ipts)
    {
      continue;
    }
    ...
    svtkIdType inpts = ipts->GetNumberOfPoints();
    ...
    svtkDataArray* itc = iptd->GetTCoords();
    if (!itc || !ad->Texture)
    {
      for (int j = 0; j < inpts; ++j)
      {
    ...
    if (haveNormals)
    {
      svtkFloatArray* otnormals = static_cast<svtkFloatArray*>(opd->GetPointData()->GetNormals());
      svtkDataArray* inorm = iptd->GetNormals();
      for (int j = 0; j < inpts; ++j)
      {
    ...
    svtkDataArray* is = iptd->GetScalars();
    svtkProperty* prop = ad->Actor->GetProperty();
    double* dcolor = prop->GetDiffuseColor();
    double diffuse = prop->GetDiffuse();
    double* acolor = prop->GetAmbientColor();
    ...
    float col[4] = { static_cast<float>(
                       svtkMath::Min(maxColor * (dcolor[0] * diffuse + acolor[0] * ambient), 255.0)),
      static_cast<float>(
        svtkMath::Min(maxColor * (dcolor[1] * diffuse + acolor[1] * ambient), 255.0)),
      static_cast<float>(
        svtkMath::Min(maxColor * (dcolor[2] * diffuse + acolor[2] * ambient), 255.0)),
...
void svtkSingleVTPExporter::WriteTexture(std::vector<actorData>& actors)
{
  // used to keep track of textures that are used by multiple actors
  std::map<svtkTexture*, actorData> knownTextures;
  ...
  for (size_t i = 0; i < actors.size(); ++i)
  {
    actorData* ad = &(actors[i]);
    svtkPolyData* mypd = findPolyData(ad->Actor->GetMapper()->GetInputDataObject(0, 0));
    svtkDataArray* tcoords = mypd->GetPointData()->GetTCoords();
  ...
  // allocate the image
  svtkNew<svtkImageData> oimage;
  oimage->SetDimensions(this->TextureSize[0], this->TextureSize[1], 1);
  oimage->AllocateScalars(SVTK_UNSIGNED_CHAR, 4);
  ...
  for (auto ad : actors)
  {
    if (ad.Texture)
    {
      ...
      if (kti == knownTextures.end())
      {
        svtkImageData* iimage = ad.Texture->GetInput();
        int dims[3];
...
void svtkSingleVTPExporter::PrintSelf(ostream& os, svtkIndent indent)
{
