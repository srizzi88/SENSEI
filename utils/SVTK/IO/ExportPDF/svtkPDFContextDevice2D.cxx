/*=========================================================================
  ...
  Module:    svtkPDFContextDevice2D.cxx
...
std::pair<double, double> GetScaleFactor(svtkMatrix3x3* mat)
{
...
// Need to be able to use svtkColor3f in a std::map. Must be outside of the anon
// namespace to work.
static bool operator<(const svtkColor3f& a, const svtkColor3f& b)
{
  for (int i = 0; i < 3; ++i)
  {
    if (a[i] < b[i])
...
struct TextHelper
{
  ...
  HPDF_Doc Document;
  HPDF_Page Page;
  svtkTextProperty* TextProp;
  HPDF_Font Font;
  const std::string& String;
  svtkMatrix3x3* Transform;
  double ScaleX;
  double ScaleY;
  ...
  TextHelper(
    HPDF_Doc doc, HPDF_Page page, svtkTextProperty* tprop, const std::string& str, svtkMatrix3x3* mat)
    : Document{ doc }
    , Page{ page }
    , TextProp{ tprop }
    , Font{ nullptr }
    ...
    , BBoxWidth{ 0.f }
    , BBoxHeight{ 0.f }
    , Theta{ svtkMath::RadiansFromDegrees(static_cast<float>(tprop->GetOrientation())) }
...
private:
  bool LoadFont()
  {
    ...
    else
    {
      ...
      switch (family)
      {
        ...
        default:
          ...
          svtkGenericWarningMacro("Unknown font code (" << family << ")");
          return false;
      }
    ...
    if (!this->Font)
    {
      svtkGenericWarningMacro("Error preparing libharu font object.");
      return false;
    }
    ...
    // In SVTK, the default leading is:
...
//------------------------------------------------------------------------------
// svtkPDFContextDevice2D::Details
//------------------------------------------------------------------------------

struct svtkPDFContextDevice2D::Details
{
  Details()
    : Page(nullptr)
  {
  ...
  float computeWorldRadius(const float x1, const float y1, const float x2, const float y2,
    svtkMatrix4x4* matrix, const float penWidth)
  {
    ...
    svtkPDFContextDevice2D::Matrix4ToMatrix3(mat, transMat);
    ...
    svtkMatrix3x3::MultiplyPoint(transMat, boundPt1, boundPt1Px);
    svtkMatrix3x3::MultiplyPoint(transMat, boundPt2, boundPt2Px);
...
svtkStandardNewMacro(svtkPDFContextDevice2D);
svtkCxxSetObjectMacro(svtkPDFContextDevice2D, Renderer, svtkRenderer);

//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::PrintSelf(std::ostream& os, svtkIndent indent)
{
  this->Superclass::PrintSelf(os, indent);
}

//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::SetHaruObjects(void* doc, void* page)
{
  if (page && doc)
  {
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::DrawPoly(float* points, int n, unsigned char* colors, int nc_comps)
{
  ...
  {
    svtkVector2f width = this->GetUnscaledPenWidth() * 0.5f;
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::DrawLines(float* f, int n, unsigned char* colors, int nc_comps)
{
  assert(nc_comps == 0 || colors != nullptr);
  assert(n > 0);
  assert(f != nullptr);
  ...
  {
    svtkVector2f width = this->GetUnscaledPenWidth();
    const float radius = std::max(width[0], width[1]) * 0.5f;
    HPDF_REAL bbox[4];
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::DrawPoints(float* points, int n, unsigned char* colors, int nc_comps)
{
  assert(nc_comps == 0 || colors != nullptr);
  assert(n > 0);
  assert(points != nullptr);
  ...
  const svtkVector2f width = this->GetUnscaledPenWidth();
  const svtkVector2f halfWidth = width * 0.5f;
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::DrawPointSprites(
  svtkImageData* spriteIn, float* points, int n, unsigned char* colors, int nc_comps)
{
  ...
  svtkImageData* rgb = this->PrepareImageData(spriteIn);
  if (!rgb)
  {
  ...
  assert(rgb->GetScalarType() == SVTK_UNSIGNED_CHAR);
  assert(rgb->GetNumberOfScalarComponents() == 3);

  int dims[3];
  rgb->GetDimensions(dims);
  svtkIdType numPoints = rgb->GetNumberOfPoints();
  ...
  typedef std::map<svtkColor3f, HPDF_Image> SpriteMap;
  SpriteMap spriteMap;
  ...
  for (int i = 0; i < n; ++i)
  {
    ...
    else
    {
      ...
      // Using int since we're iterating to j < 0 (and svtkIdType is unsigned).
      ...
      // svtkIdType is a 32-bit integer, so we disable it in that case.
#ifdef SVTK_USE_64BIT_IDS
      if (numPoints > static_cast<svtkIdType>(SVTK_INT_MAX))
      {
        svtkErrorMacro("FIXME: Image data too large for indexing with int.");
        this->PopGraphicsState();
        rgb->UnRegister(this);
        return;
      }
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::DrawMarkers(
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::DrawQuad(float* p, int n)
{
  assert(n > 0);
  assert(p != nullptr);
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::DrawQuadStrip(float* p, int n)
{
  assert(n > 0);
  assert(p != nullptr);
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::DrawPolygon(float* f, int n)
{
  assert(n > 0);
  assert(f != nullptr);
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::DrawColoredPolygon(
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::DrawEllipseWedge(float x, float y, float outRx, float outRy, float inRx,
  float inRy, float startAngle, float stopAngle)
{
  ...
  {
    // SVTK  uses 0 degrees = East with CCW rotation, but
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::DrawEllipticArc(
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::DrawString(float* point, const svtkStdString& string)
{
  svtkTextRenderer* tren = svtkTextRenderer::GetInstance();
  if (!tren)
  {
    svtkGenericWarningMacro("svtkTextRenderer unavailable. Link to "
                           "svtkRenderingFreeType to get the default "
                           "implementation.");
    return;
  }
  ...
  if (backend != svtkTextRenderer::MathText)
  {
    ...
    if (!helper.Valid)
    {
      svtkErrorMacro("Error preparing to draw string: " << string);
      this->PopGraphicsState();
      return;
    }
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::ComputeStringBounds(const svtkStdString& string, float bounds[4])
{
  svtkNew<svtkMatrix3x3> mat;
  this->GetMatrix(mat);
  TextHelper helper(this->Impl->Document, this->Impl->Page, this->TextProp, string, mat);
  if (!helper.Valid)
  {
    svtkErrorMacro("Error determining bounding box for string '" << string << "'.");
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::DrawString(float* point, const svtkUnicodeString& string)
{
  this->DrawString(point, std::string(string.utf8_str()));
}

//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::ComputeStringBounds(const svtkUnicodeString& string, float bounds[4])
{
  this->ComputeStringBounds(string.utf8_str(), bounds);
}

//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::ComputeJustifiedStringBounds(const char* string, float bounds[4])
{
  this->ComputeStringBounds(string, bounds);
}

//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::DrawMathTextString(float* point, const svtkStdString& str)
{
  this->DrawString(point, str);
}

//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::DrawImage(float p[2], float scale, svtkImageData* image)
{
  assert(p);
  assert(image);
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::DrawImage(const svtkRectf& pos, svtkImageData* image)
{
  assert(image);

  svtkImageData* rgb = this->PrepareImageData(image);
  if (!rgb)
  {
    return;
  }
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::SetColor4(unsigned char[4])
{
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::SetTexture(svtkImageData* image, int properties)
{
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::SetPointSize(float size)
{
  this->Pen->SetWidth(size);
}

//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::SetLineWidth(float width)
{
  this->Pen->SetWidth(width);
}

//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::DrawPolyData(
  float p[2], float scale, svtkPolyData* polyData, svtkUnsignedCharArray* colors, int scalarMode)
{
  ...
  svtkNew<svtkCellTypes> types;
  polyData->GetCellTypes(types);
  if (!types->IsType(SVTK_LINE) && !types->IsType(SVTK_POLY_LINE) && !types->IsType(SVTK_TRIANGLE) &&
    !types->IsType(SVTK_QUAD) && !types->IsType(SVTK_POLYGON))
  {
    return;
  }
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::SetLineType(int type)
{
  this->Pen->SetLineType(type);
}

//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::SetMatrix(svtkMatrix3x3* mat3)
{
  double mat4[16];
  svtkPDFContextDevice2D::Matrix3ToMatrix4(mat3, mat4);
  this->Matrix->SetMatrix(mat4);
  this->ApplyTransform();
}

//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::GetMatrix(svtkMatrix3x3* mat3)
{
  svtkPDFContextDevice2D::Matrix4ToMatrix3(this->Matrix->GetMatrix()->GetData(), mat3);
}

//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::MultiplyMatrix(svtkMatrix3x3* mat3)
{
  double mat4[16];
  svtkPDFContextDevice2D::Matrix3ToMatrix4(mat3, mat4);
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::PushMatrix()
{
  this->Matrix->Push();
}

//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::PopMatrix()
{
  this->Matrix->Pop();
  this->ApplyTransform();
}
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::SetClipping(int* x)
{
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::EnableClipping(bool enable)
{
  if (enable)
  {
    this->PushGraphicsState();
...
//------------------------------------------------------------------------------
svtkPDFContextDevice2D::svtkPDFContextDevice2D()
  : Impl(new Details)
  , Renderer(nullptr)
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::PushGraphicsState()
{
  HPDF_Page_GSave(this->Impl->Page);
}

//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::PopGraphicsState()
{
  HPDF_Page_GRestore(this->Impl->Page);
}

//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::ApplyPenState()
{
  const svtkVector2f width = this->GetUnscaledPenWidth();
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::ApplyStrokeColor(unsigned char* color, int numComps)
{
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::ApplyLineWidth(float width)
{
  HPDF_Page_SetLineWidth(this->Impl->Page, width);
}

//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::ApplyLineType(int type)
{
  ...
  {
    default:
      svtkErrorMacro("Unknown line type: " << type);
      SVTK_FALLTHROUGH;
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::Stroke()
{
  HPDF_Page_Stroke(this->Impl->Page);
}

//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::ApplyPenStateAsFill()
{
  this->ApplyFillColor(this->Pen->GetColorObject().GetData(), 4);
}

//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::ApplyBrushState()
{
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::ApplyTextPropertyState()
{
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::ApplyFillColor(unsigned char* color, int numComps)
{
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::ApplyFillAlpha(unsigned char alpha)
{
  auto gstateIter = this->Impl->AlphaGStateMap.find(alpha);
  if (gstateIter == this->Impl->AlphaGStateMap.end())
  {
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::Fill(bool stroke)
{
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::FillEvenOdd(bool stroke)
{
  if (this->IsInTexturedFill)
  {
    this->FillTexture();
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::BeginClipPathForTexture()
{
  assert(!this->IsInTexturedFill);
  this->IsInTexturedFill = true;
  this->TextureBounds[0] = this->TextureBounds[2] = SVTK_INT_MAX;
  this->TextureBounds[1] = this->TextureBounds[3] = SVTK_INT_MIN;
  this->PushGraphicsState(); // so we can pop the clip path
  this->ApplyFillAlpha(255); // Match the OpenGL implementation
}

//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::RegisterTexturePoints(float* data, int numPoints)
{
  if (this->IsInTexturedFill)
  {
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::FillTexture()
{
  assert(this->IsInTexturedFill);

  this->IsInTexturedFill = false;

  if (this->TextureBounds[0] == SVTK_INT_MAX || this->TextureBounds[1] == SVTK_INT_MIN ||
    this->TextureBounds[2] == SVTK_INT_MAX || this->TextureBounds[3] == SVTK_INT_MIN)
  ...
  svtkImageData* image = this->Brush->GetTexture();
  assert(image);
  ...
  const bool isTiled = ((this->Brush->GetTextureProperties() & svtkBrush::Repeat) != 0);
...
//------------------------------------------------------------------------------
svtkImageData* svtkPDFContextDevice2D::PrepareImageData(svtkImageData* in)
{
  ...
  { // If RGBA, blend into brush color -- Haru doesn't support RGBA.
    ...
    { // Fill the background image with brush color, saturate alpha
      ...
      background->AllocateScalars(SVTK_UNSIGNED_CHAR, 4);
      auto* scalars = svtkUnsignedCharArray::SafeDownCast(background->GetPointData()->GetScalars());
    ...
    svtkNew<svtkImageBlend> blender;
    ...
    svtkNew<svtkImageExtractComponents> extract;
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::DrawEllipticArcSegments(
  float x, float y, float rX, float rY, float startAngle, float stopAngle, bool startPath)
{
  ...
  // step in radians:
  float step =
    svtkMath::RadiansFromDegrees(stopAngle - startAngle) / static_cast<float>(numSegments);

  float rstart = svtkMath::RadiansFromDegrees(startAngle);
...
//------------------------------------------------------------------------------
int svtkPDFContextDevice2D::GetNumberOfArcIterations(
  float rX, float rY, float startAngle, float stopAngle)
{
  ...
  return static_cast<int>(
    ceil(svtkMath::RadiansFromDegrees(std::fabs(stopAngle - startAngle)) / maxStep));
}

//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::DrawCrossMarkers(
  bool highlight, float* points, int n, unsigned char* colors, int nc_comps)
{
  ...
  this->ApplyLineType(svtkPen::SOLID_LINE);
  if (!colors)
  {
    this->ApplyStrokeColor(this->Pen->GetColorObject().GetData(), 4);
  }
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::DrawPlusMarkers(
  bool highlight, float* points, int n, unsigned char* colors, int nc_comps)
{
  const float markerSize = this->Pen->GetWidth();
  const float delta = markerSize * 0.5f;
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::DrawSquareMarkers(
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::DrawCircleMarkers(
  bool, float* points, int n, unsigned char* colors, int nc_comps)
{
  const float markerSize = this->Pen->GetWidth();
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::DrawDiamondMarkers(
  bool, float* points, int n, unsigned char* colors, int nc_comps)
{
  const float markerSize = this->Pen->GetWidth();
  const float radius = markerSize * 0.5f;
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::DrawPath(svtkPath* path, float originX, float originY)
{
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::ApplyTransform()
{
  ...
  svtkPDFContextDevice2D::HPDFTransformToMatrix3(
    oldTrans.a, oldTrans.b, oldTrans.c, oldTrans.d, oldTrans.x, oldTrans.y, oldInvTransMat3);
  svtkMatrix3x3::Invert(oldInvTransMat3, oldInvTransMat3);

  // Multiply the inverse current transform with the new one:
  double newTransMat3[9];
  double* mat = this->Matrix->GetMatrix()->GetData();
  svtkPDFContextDevice2D::Matrix4ToMatrix3(mat, newTransMat3);

  svtkMatrix3x3::Multiply3x3(oldInvTransMat3, newTransMat3, newTransMat3);
  ...
  float hpdfMat[6];
  svtkPDFContextDevice2D::Matrix3ToHPDFTransform(newTransMat3, hpdfMat);
  HPDF_Page_Concat(
    this->Impl->Page, hpdfMat[0], hpdfMat[1], hpdfMat[2], hpdfMat[3], hpdfMat[4], hpdfMat[5]);
}

//------------------------------------------------------------------------------
svtkVector2f svtkPDFContextDevice2D::GetUnscaledPenWidth()
{
  ...
  return svtkVector2f{ static_cast<float>(width / sx), static_cast<float>(width / sy) };
}

//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::Matrix3ToMatrix4(svtkMatrix3x3* mat3, double mat4[16])
{
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::Matrix4ToMatrix3(double mat4[16], svtkMatrix3x3* mat3)
{
  double* mat3Data = mat3->GetData();
  svtkPDFContextDevice2D::Matrix4ToMatrix3(mat4, mat3Data);
}

//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::Matrix4ToMatrix3(double mat4[16], double mat3[9])
{
  mat3[0] = mat4[0];
  mat3[1] = mat4[1];
  mat3[2] = mat4[3];
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::Matrix4ToHPDFTransform(const double mat4[16], float hpdfMat[6])
{
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::Matrix3ToHPDFTransform(const double mat3[9], float hpdfMat[6])
{
...
//------------------------------------------------------------------------------
void svtkPDFContextDevice2D::HPDFTransformToMatrix3(
