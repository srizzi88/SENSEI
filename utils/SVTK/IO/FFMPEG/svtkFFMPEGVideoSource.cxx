/*=========================================================================
  ...
  Module:    svtkFFMPEGVideoSource.cxx
...
//
// add libs to svtk build, I just add them to the FFMPEG_LIBAVCODEC_LIBRARIES variable
//
// Ws2_32.lib
// Bcrypt.lib
// Secur32.dll
...
svtkStandardNewMacro(svtkFFMPEGVideoSource);

//----------------------------------------------------------------------------
svtkFFMPEGVideoSource::svtkFFMPEGVideoSource()
  ...
  , FileName(nullptr)
{
  ...
  this->Internal = new svtkFFMPEGVideoSourceInternal;
...
svtkFFMPEGVideoSource::~svtkFFMPEGVideoSource()
{
  this->svtkFFMPEGVideoSource::ReleaseSystemResources();
  delete[] this->FileName;
  delete this->Internal;
}

//----------------------------------------------------------------------------
void svtkFFMPEGVideoSource::Initialize()
{
  if (this->Initialized)
  {
    return;
  ...
  {
    svtkErrorMacro("Could not open source file " << this->FileName);
    return;
  }
  ...
  {
    svtkErrorMacro("Could not find stream information");
    return;
  }
  ...
  {
    svtkErrorMacro("Could not find video stream in input file ");
    return;
  }
  ...
  if (!dec)
  {
    svtkErrorMacro("Failed to find codec for video");
    return;
  }
  ...
  {
    svtkErrorMacro("Failed to open codec for video");
    return;
  }
  ...
  {
    svtkErrorMacro("Failed to create RGB context");
  }
  ...
  if (this->Internal->AudioStreamIndex >= 0)
  {
    ...
    if (!adec)
    {
      svtkErrorMacro("Failed to find codec for audio");
      return;
    }
    ...
    {
      svtkErrorMacro("Failed to open codec for audio");
      return;
    }
    ...
    {
      svtkErrorMacro("Could not allocate audio frame");
      return;
    }
  }
  ...
  {
    svtkErrorMacro("Could not allocate frame");
    return;
  }
...
void* svtkFFMPEGVideoSource::FeedThread(svtkMultiThreader::ThreadInfo* data)
{
  svtkFFMPEGVideoSource* self = static_cast<svtkFFMPEGVideoSource*>(data->UserData);
...
//
void* svtkFFMPEGVideoSource::Feed(svtkMultiThreader::ThreadInfo* data)
{
  bool done = false;
...
static void svtkThreadSleep(double time)
{
...
void* svtkFFMPEGVideoSource::DrainThread(svtkMultiThreader::ThreadInfo* data)
{
  svtkFFMPEGVideoSource* self = static_cast<svtkFFMPEGVideoSource*>(data->UserData);
  return self->Drain(data);
}

void* svtkFFMPEGVideoSource::Drain(svtkMultiThreader::ThreadInfo* data)
{
  bool done = false;
  unsigned short count = 0;
  ...
  while (!done)
  {
    ...
    if (ret == 0)
    {
      svtkThreadSleep(startTime + frame / rate);
      if (this->VideoCallback)
      {
        svtkFFMPEGVideoSourceVideoCallbackData cbd;
...
void* svtkFFMPEGVideoSource::DrainAudioThread(svtkMultiThreader::ThreadInfo* data)
{
  svtkFFMPEGVideoSource* self = static_cast<svtkFFMPEGVideoSource*>(data->UserData);
  return self->DrainAudio(data);
}

void* svtkFFMPEGVideoSource::DrainAudio(svtkMultiThreader::ThreadInfo* data)
{
  bool done = false;
  unsigned short count = 0;
  ...
  while (!done)
  {
    ...
    if (ret == 0)
    {
      ...
      int sampleFormat = SVTK_UNSIGNED_CHAR;
      bool packed = true;
      switch (this->Internal->AudioDecodeContext->sample_fmt)
      {
        default:
        case AV_SAMPLE_FMT_U8:
          sampleFormat = SVTK_UNSIGNED_CHAR;
          break;
      ...
      {
        svtkFFMPEGVideoSourceAudioCallbackData cbd;
...
void svtkFFMPEGVideoSource::ReadFrame()
{
  ...
  while (!gotFrame && (!this->EndOfFile || this->Internal->Packet.size > 0))
  {
    ...
    if (this->Internal->Packet.size > 0)
    {
      ...
      {
        svtkErrorMacro("codec did not receive video frame");
        return;
      }
      if (ret == AVERROR_EOF)
      {
    ...
    if (ret == AVERROR(EAGAIN) && !this->EndOfFile)
    {
      ...
      if (fret >= 0 && this->Internal->Packet.stream_index == this->Internal->VideoStreamIndex)
      {
        ...
        {
          svtkErrorMacro("codec did not send packet");
          return;
        }
      }
      if (fret == AVERROR_EOF)
...
void svtkFFMPEGVideoSource::InternalGrab()
{
...
//----------------------------------------------------------------------------
void svtkFFMPEGVideoSource::ReleaseSystemResources()
{
  if (this->Initialized)
  {
...
//----------------------------------------------------------------------------
void svtkFFMPEGVideoSource::Grab()
{
  if (this->Recording)
  {
    return;
...
//----------------------------------------------------------------------------
void svtkFFMPEGVideoSource::Play()
{
  this->svtkVideoSource::Play();
}

//----------------------------------------------------------------------------
void svtkFFMPEGVideoSource::Record()
{
  if (this->Playing)
  {
    this->Stop();
  ...
  if (!this->Recording)
  {
    ...
    this->FeedThreadId = this->PlayerThreader->SpawnThread(
      (svtkThreadFunctionType)&svtkFFMPEGVideoSource::FeedThread, this);
    this->DrainThreadId = this->PlayerThreader->SpawnThread(
      (svtkThreadFunctionType)&svtkFFMPEGVideoSource::DrainThread, this);
    if (this->Internal->AudioDecodeContext)
    {
      this->DrainAudioThreadId = this->PlayerThreader->SpawnThread(
        (svtkThreadFunctionType)&svtkFFMPEGVideoSource::DrainAudioThread, this);
    }
    else
    {
...
//----------------------------------------------------------------------------
void svtkFFMPEGVideoSource::Stop()
{
...
void svtkFFMPEGVideoSource::SetFrameSize(int x, int y, int z)
{
...
//----------------------------------------------------------------------------
void svtkFFMPEGVideoSource::SetFrameRate(float rate)
{
...
//----------------------------------------------------------------------------
void svtkFFMPEGVideoSource::SetOutputFormat(int format)
{
