...
int TestGDALRasterPalette(int argc, char** argv)
{
  ...
  reader->Update();
  svtkUniformGrid* image = svtkUniformGrid::SafeDownCast(reader->GetOutput());
  ...
  svtkLookupTable* colorTable = image->GetCellData()->GetScalars()->GetLookupTable();
  if (!colorTable)
  {
  ...
  int retVal = svtkRegressionTestImageThreshold(renderWindow, 3.0);
