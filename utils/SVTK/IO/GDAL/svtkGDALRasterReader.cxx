...
svtkStandardNewMacro(svtkGDALRasterReader);

//-----------------------------------------------------------------------------
class svtkGDALRasterReader::svtkGDALRasterReaderInternal
{
public:
  svtkGDALRasterReaderInternal(svtkGDALRasterReader* reader);
  ~svtkGDALRasterReaderInternal();

  void ReadMetaData(const std::string& fileName);
  void ReadData(const std::string& fileName);

  template <typename SVTK_TYPE, typename RAW_TYPE>
  ...
  void ReadColorTable(GDALRasterBand* rasterBand, svtkLookupTable* colorTable) const;

  static void SelectionCallback(
    svtkObject* obj, unsigned long eventid, void* clientdata, void* calldata);
  ...
  svtkDataArraySelection* CellArraySelection;
  svtkCallbackCommand* SelectionObserver;

  svtkSmartPointer<svtkUniformGrid> UniformGridData;
  svtkGDALRasterReader* Reader;
};

//-----------------------------------------------------------------------------
svtkGDALRasterReader::svtkGDALRasterReaderInternal::svtkGDALRasterReaderInternal(
  svtkGDALRasterReader* reader)
  : NumberOfBands(0)
  ...
  , Reader(reader)
{
  ...
  this->CellArraySelection = svtkDataArraySelection::New();
  this->SelectionObserver = svtkCallbackCommand::New();
  this->SelectionObserver->SetCallback(
    &svtkGDALRasterReader::svtkGDALRasterReaderInternal::SelectionCallback);
  this->SelectionObserver->SetClientData(this);
  this->CellArraySelection->AddObserver(svtkCommand::ModifiedEvent, this->SelectionObserver);
...
//-----------------------------------------------------------------------------
void svtkGDALRasterReader::svtkGDALRasterReaderInternal::ReadMetaData(const std::string& fileName)
{
...
//-----------------------------------------------------------------------------
void svtkGDALRasterReader::svtkGDALRasterReaderInternal::ReadData(const std::string& fileName)
{
  ...
  // Initialize
  this->UniformGridData = svtkSmartPointer<svtkUniformGrid>::New();
  ...
  switch (this->TargetDataType)
  {
    case (GDT_UInt16):
    {
      ...
      this->GenericReadData<svtkUnsignedShortArray, unsigned short>();
      break;
    }
    case (GDT_Int16):
    {
      this->Reader->SetDataScalarTypeToShort();
      this->GenericReadData<svtkShortArray, short>();
      break;
    }
    case (GDT_UInt32):
    {
      this->Reader->SetDataScalarTypeToUnsignedInt();
      this->GenericReadData<svtkUnsignedIntArray, unsigned int>();
      break;
    }
    case (GDT_Int32):
    {
      this->Reader->SetDataScalarTypeToInt();
      this->GenericReadData<svtkIntArray, int>();
      break;
    }
    case (GDT_Float32):
    {
      this->Reader->SetDataScalarTypeToFloat();
      this->GenericReadData<svtkFloatArray, float>();
      break;
    }
    case (GDT_Float64):
    {
      this->Reader->SetDataScalarTypeToDouble();
      this->GenericReadData<svtkDoubleArray, double>();
      break;
    }
    case (GDT_Byte):
    default:
    {
      ...
      this->GenericReadData<svtkUnsignedCharArray, unsigned char>();
      break;
    }
  }
}
...
//-----------------------------------------------------------------------------
template <typename SVTK_TYPE, typename RAW_TYPE>
void svtkGDALRasterReader::svtkGDALRasterReaderInternal::GenericReadData()
{
  // Pixel data.
  std::vector<RAW_TYPE> rawUniformGridData;

  // Color table
  svtkSmartPointer<svtkLookupTable> colorTable = svtkSmartPointer<svtkLookupTable>::New();
  ...
  if (this->Reader->CollateBands)
  {
    ...
    (void)err; // unused
    this->Convert<SVTK_TYPE, RAW_TYPE>(
  ...
  for (size_t i = 0; i < allBands.size(); ++i)
  {
    ...
    if (allBands[i])
    {
      ...
      err = allBands[i]->RasterIO(GF_Read, windowX, windowY, windowWidth, windowHeight,
        ...
        destWidth, destHeight, this->TargetDataType, pixelSpace, lineSpace);
      assert(err == CE_None);
      this->Convert<SVTK_TYPE, RAW_TYPE>(rawUniformGridData, destWidth, destHeight, groupIndex,
...
//----------------------------------------------------------------------------
void svtkGDALRasterReader::svtkGDALRasterReaderInternal::ReleaseData()
{
  if (this->GDALData)
  {
...
//-----------------------------------------------------------------------------
template <typename SVTK_TYPE, typename RAW_TYPE>
void svtkGDALRasterReader::svtkGDALRasterReaderInternal::Convert(
  ...
  const std::vector<int>& groupIndex, const char* name, bool flipX, bool flipY)
{
  ...
  svtkSmartPointer<SVTK_TYPE> scArr(svtkSmartPointer<SVTK_TYPE>::New());
  scArr->SetName(name);
...
//-----------------------------------------------------------------------------
bool svtkGDALRasterReader::svtkGDALRasterReaderInternal::GetGeoCornerPoint(
...
//-----------------------------------------------------------------------------
const double* svtkGDALRasterReader::svtkGDALRasterReaderInternal::GetGeoCornerPoints()
{
...
void svtkGDALRasterReader::svtkGDALRasterReaderInternal::GetOriginSpacing(
...
//-----------------------------------------------------------------------------
void svtkGDALRasterReader::svtkGDALRasterReaderInternal::ReadColorTable(
  GDALRasterBand* rasterBand, svtkLookupTable* colorTable) const
{
  ...
  for (int i = 0; i < numEntries; ++i)
  {
    ...
    if (categoryNames)
    {
      ...
      {
        colorTable->SetAnnotation(svtkVariant(i), categoryNames[i]);
      }
    }
    else
    {
      ...
      ss.clear();
      ss << "Category " << i;
      colorTable->SetAnnotation(svtkVariant(i), ss.str());
    }
  }
}

//----------------------------------------------------------------------------
void svtkGDALRasterReader::svtkGDALRasterReaderInternal::SelectionCallback(svtkObject* svtkNotUsed(obj),
  unsigned long svtkNotUsed(eventid), void* clientdata, void* svtkNotUsed(calldata))
{
  static_cast<svtkGDALRasterReader::svtkGDALRasterReaderInternal*>(clientdata)->Reader->Modified();
}

//-----------------------------------------------------------------------------
void svtkGDALRasterReader::PrintSelf(std::ostream& os, svtkIndent indent)
{
...
//-----------------------------------------------------------------------------
svtkGDALRasterReader::svtkGDALRasterReader()
  : svtkImageReader2()
{
  this->Impl = new svtkGDALRasterReaderInternal(this);
...
//-----------------------------------------------------------------------------
int svtkGDALRasterReader::CanReadFile(const char* fname)
{
...
//-----------------------------------------------------------------------------
const char* svtkGDALRasterReader::GetProjectionString() const
{
  return this->Projection.c_str();
}

//-----------------------------------------------------------------------------
const double* svtkGDALRasterReader::GetGeoCornerPoints()
{
  return this->Impl->GetGeoCornerPoints();
}

//-----------------------------------------------------------------------------
const std::vector<std::string>& svtkGDALRasterReader::GetMetaData()
{
  return this->MetaData;
}

//-----------------------------------------------------------------------------
std::vector<std::string> svtkGDALRasterReader::GetDomainMetaData(const std::string& domain)
{
...
//-----------------------------------------------------------------------------
const std::string& svtkGDALRasterReader::GetDriverShortName()
{
  return this->DriverShortName;
}

//-----------------------------------------------------------------------------
const std::string& svtkGDALRasterReader::GetDriverLongName()
{
  return this->DriverLongName;
}

svtkIdType svtkGDALRasterReader::GetNumberOfCells()
{
...
//-----------------------------------------------------------------------------
int svtkGDALRasterReader::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** svtkNotUsed(inputVector), svtkInformationVector* outputVector)
{
  if (this->TargetDimensions[0] <= 0 || this->TargetDimensions[1] <= 0)
  {
    svtkWarningMacro(<< "Invalid target dimensions");
  }

  this->Impl->ReadData(this->FileName);
  if (!this->Impl->GDALData)
  {
    svtkErrorMacro("Failed to read " << this->FileName);
    return 0;
  }
  ...
  svtkSmartPointer<svtkStringArray> projectionData = svtkSmartPointer<svtkStringArray>::New();
  ...
  svtkSmartPointer<svtkDoubleArray> noDataArray = svtkSmartPointer<svtkDoubleArray>::New();
  ...
  svtkDataObject* dataObj = outInfo->Get(svtkDataObject::DATA_OBJECT());
  if (!dataObj)
  {
    return 0;
  }

  svtkUniformGrid::SafeDownCast(dataObj)->ShallowCopy(this->Impl->UniformGridData);
  return 1;
}

//-----------------------------------------------------------------------------
int svtkGDALRasterReader::RequestInformation(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** svtkNotUsed(inputVector), svtkInformationVector* outputVector)
{
  ...
  {
    svtkErrorMacro("Invalid image dimensions");
    return 0;
  }
  ...
  outInfo->Set(svtkGDAL::FLIP_AXIS(), flip, 3);
  outInfo->Set(svtkGDAL::MAP_PROJECTION(), this->Impl->GDALData->GetProjectionRef());

  return 1;
}

//-----------------------------------------------------------------------------
int* svtkGDALRasterReader::GetRasterDimensions()
{
  return this->Impl->RasterDimensions;
}

//-----------------------------------------------------------------------------
int svtkGDALRasterReader::FillOutputPortInformation(int port, svtkInformation* info)
{
  if (port == 0)
  {
  ...
  {
    svtkErrorMacro("Port: " << port << " is not a valid port");
    return 0;
  }
}

//-----------------------------------------------------------------------------
double svtkGDALRasterReader::GetInvalidValue(size_t bandIndex, int* hasNoData)
{
  if (bandIndex >= this->Impl->NoDataValue.size())
  {
    svtkErrorMacro("bandIndex >= " << this->Impl->NoDataValue.size() << ": " << bandIndex);
    return 0;
  }
  if (hasNoData)
  {
...
//-----------------------------------------------------------------------------
int svtkGDALRasterReader::GetNumberOfCellArrays()
{
  return this->Impl->CellArraySelection->GetNumberOfArrays();
}

//-----------------------------------------------------------------------------
const char* svtkGDALRasterReader::GetCellArrayName(int index)
{
  return this->Impl->CellArraySelection->GetArrayName(index);
}

//-----------------------------------------------------------------------------
int svtkGDALRasterReader::GetCellArrayStatus(const char* name)
{
  return this->Impl->CellArraySelection->ArrayIsEnabled(name);
}

//-----------------------------------------------------------------------------
void svtkGDALRasterReader::SetCellArrayStatus(const char* name, int status)
{
  if (status)
  {
...
//-----------------------------------------------------------------------------
void svtkGDALRasterReader::DisableAllCellArrays()
{
  this->Impl->CellArraySelection->DisableAllArrays();
}

//-----------------------------------------------------------------------------
void svtkGDALRasterReader::EnableAllCellArrays()
{
