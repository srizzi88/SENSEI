...
svtkStandardNewMacro(svtkGDALVectorReader);

int svtkGDALVectorReader::OGRRegistered = 0;

// -----------------------------------------------------------------------------
class svtkGDALVectorReader::Internal
{
public:
  ...
  void SetupPolyData(svtkPolyData** pd, svtkCellArray** lines, svtkCellArray** verts,
    std::vector<svtkAbstractArray*>* fields, int numFields, OGRFeatureDefn* fdef)
  {
    *pd = svtkPolyData::New();
    ...
    {
      svtkNew<svtkIdTypeArray> featIds;
      featIds->SetName("_svtkPedigreeIds");
  ...
  bool ReadLayer(OGRLayer* layer, svtkMultiBlockDataSet* mbds)
  {
    OGRFeature* feat;
    svtkPolyData* pd = 0;
    svtkIdType nTotPoly = 0;
    svtkCellArray *lines = 0, *verts = 0;

    OGRFeatureDefn* fdef = layer->GetLayerDefn();
    int numFields = fdef->GetFieldCount();
    std::vector<svtkAbstractArray*> fields;
    ...
    while ((feat = layer->GetNextFeature()))
    {
      ...
      svtkIdType nPoly = this->insertGeometryRecursive(geom, pd, pts, lines, verts);
      if (!nPoly)
      {
        continue;
      }
      ...
      const char* sval;
      svtkIntArray* iarr;
      svtkDoubleArray* rarr;
      svtkStringArray* sarr;
      for (int f = 0; f < numFields; ++f)
      {
        ...
        {
          case OFTInteger:
            iarr = svtkArrayDownCast<svtkIntArray>(fields[f]);
            ...
            break;
          case OFTReal:
            rarr = svtkArrayDownCast<svtkDoubleArray>(fields[f]);
          ...
          case OFTString:
          default:
            sarr = svtkArrayDownCast<svtkStringArray>(fields[f]);
      ...
      if (this->AddFeatureIds)
      {
        svtkIdTypeArray* idarr = svtkArrayDownCast<svtkIdTypeArray>(fields[numFields]);
        for (i = 0; i < nPoly; ++i)
        {
  ...
  svtkIdType insertGeometryRecursive(
    OGRGeometry* geom, svtkPolyData* pd, svtkPoints* pts, svtkCellArray* lines, svtkCellArray* verts)
  {
    if (!geom)
    {
      return 0;
...
// -----------------------------------------------------------------------------
svtkGDALVectorReader::svtkGDALVectorReader()
{
  this->FileName = 0;
  ...
  if (!svtkGDALVectorReader::OGRRegistered)
  {
    svtkGDALVectorReader::OGRRegistered = 1;
    OGRRegisterAll();
  }
...
// -----------------------------------------------------------------------------
void svtkGDALVectorReader::PrintSelf(ostream& os, svtkIndent indent)
{
...
// -----------------------------------------------------------------------------
int svtkGDALVectorReader::GetNumberOfLayers()
{
  if (this->InitializeInternal() == SVTK_ERROR)
  {
    return -1;
  }
...
// -----------------------------------------------------------------------------
int svtkGDALVectorReader::GetLayerType(int layerIndex)
{
  if (this->InitializeInternal() == SVTK_ERROR)
  {
    return -1;
...
// -----------------------------------------------------------------------------
int svtkGDALVectorReader::GetFeatureCount(int layerIndex)
{
  if (this->InitializeInternal() == SVTK_ERROR)
  {
    return -1;
...
// -----------------------------------------------------------------------------
int svtkGDALVectorReader::GetActiveLayerType()
{
...
// -----------------------------------------------------------------------------
int svtkGDALVectorReader::GetActiveLayerFeatureCount()
{
  return this->ActiveLayer < 0 || this->ActiveLayer >= this->GetNumberOfLayers()
    ? 0
...
// -----------------------------------------------------------------------------
const char* svtkGDALVectorReader::GetLayerProjection(int layerIndex)
{
  if (layerIndex < 0)
  {
    svtkErrorMacro(<< "Layer index cannot be negative");
  }
...
// -----------------------------------------------------------------------------
const char* svtkGDALVectorReader::GetLayerProjectionAsProj4(int layerIndex)
{
  if (layerIndex < 0)
  {
    svtkErrorMacro(<< "Layer index cannot be negative");
  ...
  svtkGDALVectorReader::Internal* p = this->Implementation;
  if (!p->Source)
  {
    svtkErrorMacro(<< "Source dataset not provided");
    return nullptr;
  }
  int layerCount = p->Source->GetLayerCount();
  if (layerIndex >= layerCount)
  {
    svtkErrorMacro(<< "Layer index " << layerIndex
  ...
  if (!layer)
  {
    svtkErrorMacro(<< "Cannot access the GDAL dataset layer at index " << layerIndex);
    return nullptr;
  }
  if (!layer->GetSpatialRef())
  {
    svtkErrorMacro(<< "Cannot access the spatial ref object.");
    return nullptr;
  }
  char* projStr;
...
// -----------------------------------------------------------------------------
std::map<int, std::string> svtkGDALVectorReader::GetLayersProjection()
{
  return this->LayersProjection;
}

// -----------------------------------------------------------------------------
int svtkGDALVectorReader::RequestInformation(
...
// -----------------------------------------------------------------------------
int svtkGDALVectorReader::RequestData(
  svtkInformation* request, svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  (void)request;
  (void)inputVector;
  ...
  if (!this->FileName)
  {
    // svtkWarningMacro( "No filename specified" );
    return 0;
  }

  svtkMultiBlockDataSet* mbds = 0;
  svtkInformation* oi = outputVector->GetInformationObject(0);
  if (!oi)
  {
    return 0;
  }
...
// -----------------------------------------------------------------------------
int svtkGDALVectorReader::InitializeInternal()
{
  if (!this->FileName)
  {
    svtkErrorMacro(<< "FileName not set or empty:");
    return SVTK_ERROR;
  }
  ...
  {
    this->Implementation =
      new svtkGDALVectorReader::Internal(this->FileName, this->AppendFeatures, this->AddFeatureIds);
    if (!this->Implementation || this->Implementation->LastError)
    {
      if (this->Implementation)
      {
        svtkErrorMacro(<< this->Implementation->LastError);
        delete this->Implementation;
        this->Implementation = 0;
      }
      return SVTK_ERROR;
