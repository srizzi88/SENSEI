...
def load_geojson(input_string, feature_properties={}):
  '''Parses input_string with svtkGeoJSONReader, returns svtkPolyData

  feature_properties is a dictionary of name-default_values
  to attach as cell data in the returned svtkPolyData.
  '''
  reader = svtk.svtkGeoJSONReader()
  #reader.DebugOn()
...
"""
  prop0_default = svtk.svtkVariant('default')
  feature_properties = {'prop0': prop0_default}
  polydata = load_geojson(input_string, feature_properties)
  if polydata is None:
    print('Failed to read input string and return svtkPolyData')
    sys.exit(1)
