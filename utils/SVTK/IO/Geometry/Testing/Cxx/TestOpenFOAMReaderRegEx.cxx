...
int TestOpenFOAMReaderRegEx(int argc, char* argv[])
{
  ...
  char* filename =
    svtkTestUtilities::ExpandDataFileName(argc, argv, "Data/OpenFOAM/regex/test.foam");
  ...
  // Print summary
  svtkMultiBlockDataSet* result = reader->GetOutput();
  if (result->GetNumberOfBlocks() > 1)
  {
    if (svtkMultiBlockDataSet* patches = svtkMultiBlockDataSet::SafeDownCast(result->GetBlock(1)))
    {
      int num_blocks = patches->GetNumberOfBlocks();
      for (int i = 0; i < num_blocks; i++)
      {
        // Get name of this boundary patch and fail if this patch name is not expected
        const char* patch_name = patches->GetMetaData(i)->Get(svtkCompositeDataSet::NAME());
        if (!patch_name || expected_data.find(patch_name) == expected_data.end())
          return EXIT_FAILURE;

        if (svtkPolyData* patch = svtkPolyData::SafeDownCast(patches->GetBlock(i)))
        {
          if (svtkCellData* fields = patch->GetCellData())
          {
            for (int j = 0; j < fields->GetNumberOfArrays(); j++)
            {
              if (svtkFloatArray* array = svtkFloatArray::SafeDownCast(fields->GetAbstractArray(j)))
              {
