...
int UnitTestSTLWriter(int argc, char* argv[])
{
  ...
  svtkNew<svtkUnsignedCharArray> shortBinaryHeader;
  for (unsigned char c = 100; c < 135; c++)
  {
    shortBinaryHeader->InsertNextValue(c);
  }
  svtkNew<svtkUnsignedCharArray> longBinaryHeader;
  for (unsigned char c = 100; c < 195; c++)
  {
    longBinaryHeader->InsertNextValue(c);
  }

  svtkSmartPointer<svtkSTLWriter> writer1 = svtkSmartPointer<svtkSTLWriter>::New();
  ...
  reader->SetFileName(fileName.c_str());
  reader->Update();
  svtkUnsignedCharArray* readBinaryHeader = reader->GetBinaryHeader();
  ...
  svtkSmartPointer<svtkSTLWriter> writer2 = svtkSmartPointer<svtkSTLWriter>::New();
  writer2->AddObserver(svtkCommand::ErrorEvent, errorObserver);

  writer2->SetFileName("foo");
  svtkSmartPointer<svtkPolyData> emptyPolyData = svtkSmartPointer<svtkPolyData>::New();
  ...
  if (svtksys::SystemTools::FileExists("/dev/full"))
  {
