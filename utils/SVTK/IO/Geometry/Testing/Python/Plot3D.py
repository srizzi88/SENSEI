...
class TestPlot3D(Testing.svtkTest):
    def testReader3D(self):
        ...
        for name in names:
            ...
            r.SetFileName(str(SVTK_DATA_ROOT) + "/Data/" + name[0])
            r.SetQFileName(str(SVTK_DATA_ROOT) + "/Data/" + name[1])
            r.Update()
    ...
    def testMetaReader(self):
        ...
        r.SetFileName(str(SVTK_DATA_ROOT) + "/Data/multi.p3d")
        r.Update()
