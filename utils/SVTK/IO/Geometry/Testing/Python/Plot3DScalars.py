...
for scalarFunction in scalarFunctions:
    exec("pl3d" + scalarFunction + " = svtk.svtkMultiBlockPLOT3DReader()")
    ...
    exec("plane" + scalarFunction + " = svtk.svtkStructuredGridGeometryFilter()")
    eval("plane" + scalarFunction).SetInputData(output)
    eval("plane" + scalarFunction).SetExtent(25, 25, 0, 100, 0, 100)

    exec("mapper" + scalarFunction + " = svtk.svtkPolyDataMapper()")
    ...
    exec("actor" + scalarFunction + " = svtk.svtkActor()")
    eval("actor" + scalarFunction).SetMapper(eval("mapper" + scalarFunction))

    exec("ren" + scalarFunction + " = svtk.svtkRenderer()")
