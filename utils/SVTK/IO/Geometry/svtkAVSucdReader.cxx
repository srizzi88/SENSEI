...
svtkStandardNewMacro(svtkAVSucdReader);

// Internal Classes/Structures
struct svtkAVSucdReader::idMapping : public std::map<svtkIdType, svtkIdType>
{
};

//----------------------------------------------------------------------------
svtkAVSucdReader::svtkAVSucdReader()
{
...
//----------------------------------------------------------------------------
void svtkAVSucdReader::SetByteOrderToBigEndian()
{
  this->ByteOrder = FILE_BIG_ENDIAN;
}

//----------------------------------------------------------------------------
void svtkAVSucdReader::SetByteOrderToLittleEndian()
{
  this->ByteOrder = FILE_LITTLE_ENDIAN;
}

//----------------------------------------------------------------------------
const char* svtkAVSucdReader::GetByteOrderAsString()
{
...
//----------------------------------------------------------------------------
int svtkAVSucdReader::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** svtkNotUsed(inputVector), svtkInformationVector* outputVector)
{
  ...
  svtkDebugMacro(<< "Reading AVS UCD file");
...
//----------------------------------------------------------------------------
void svtkAVSucdReader::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
void svtkAVSucdReader::ReadFile(svtkUnstructuredGrid* output)
{
...
//----------------------------------------------------------------------------
int svtkAVSucdReader::RequestInformation(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** svtkNotUsed(inputVector), svtkInformationVector* svtkNotUsed(outputVector))
{
  ...
  { // most likely an ASCII file
    ...
    this->FileStream = new svtksys::ifstream(this->FileName);
    char c = '\0';
  ...
  else
  {
    ...
    svtkTypeUInt64 trueFileLength = this->FileStream->tellg();
    svtkTypeUInt64 calculatedFileLength = 0; // not known yet
    ...
    while (attempts < 2)
    {
      ...
      svtkDebugMacro(<< this->NumberOfNodes << " " << this->NumberOfCells << " "
      ...
      svtkTypeUInt64 numNodes = this->NumberOfNodes;
      svtkTypeUInt64 numCells = this->NumberOfCells;
      svtkTypeUInt64 numNodeFields = this->NumberOfNodeFields;
      svtkTypeUInt64 numCellFields = this->NumberOfCellFields;
      svtkTypeUInt64 numFields = this->NumberOfFields;
      svtkTypeUInt64 numListNodes = this->NlistNodes;
      ...
      svtkDebugMacro(<< "TFL = " << trueFileLength << "\tCFL = " << calculatedFileLength << endl);
    ...
    if (trueFileLength != calculatedFileLength)
    {
      svtkErrorMacro("Calculated file length inconsistent with actual length; file corrupt?");
      return 0;
    }
    ...
    if (this->NumberOfNodeFields)
    {
      ...
      for (i = 0; i < this->NumberOfNodeComponents; i++)
      {
        ...
        svtkDebugMacro(<< i + 1 << " :found ND label = " << label << " [" << ncomp_list[i] << "]"
                      << endl);
    ...
    if (this->NumberOfCellFields)
    {
      ...
      {
        this->GetLabel(buffer1, i, label);
        svtkDebugMacro(<< i + 1 << " :found CD label = " << label << " [" << ncomp_list[i] << "]"
                      << endl);
    ...
    if (this->NumberOfFields)
    {
      ...
      {
        this->GetLabel(buffer1, i, label);
        svtkDebugMacro(<< "found MD label = " << label << endl);
      }
    }
  } // end of Binary part
  for (i = 0; i < this->NumberOfNodeComponents; i++)
  ...
  svtkDebugMacro(<< "end of ExecuteInformation\n");

  return 1;
}

//----------------------------------------------------------------------------
void svtkAVSucdReader::GetCellDataRange(int cellComp, int index, float* min, float* max)
{
...
//----------------------------------------------------------------------------
void svtkAVSucdReader::GetNodeDataRange(int nodeComp, int index, float* min, float* max)
{
...
//----------------------------------------------------------------------------
void svtkAVSucdReader::ReadGeometry(
  svtkUnstructuredGrid* output, idMapping& nodeMap, idMapping& cellMap)
{
  // add a material array
  svtkIntArray* materials = svtkIntArray::New();
  materials->SetNumberOfTuples(this->NumberOfCells);
  materials->SetName("Material Id");

  svtkFloatArray* coords = svtkFloatArray::New();
  ...
  if (this->BinaryFile)
  {
    ...
    if (types == nullptr)
    {
      svtkErrorMacro(<< "Error allocating types memory\n");
    }

    svtkIdTypeArray* listcells = svtkIdTypeArray::New();
...
//----------------------------------------------------------------------------
void svtkAVSucdReader::ReadBinaryCellTopology(
  svtkIntArray* materials, int* types, svtkIdTypeArray* listcells)
{
  int i, j, k2 = 0;
  int* mat = materials->GetPointer(0);
  svtkIdType* list = listcells->GetPointer(0);
  int* ctype = new int[4 * this->NumberOfCells];
  if (ctype == nullptr)
  {
    svtkErrorMacro(<< "Error allocating ctype memory");
    return;
  }
  ...
  {
    svtkErrorMacro(<< "Error allocating topology_list memory");
    return;
  }
  ...
  {
    *list++ = ctype[4 * i + 2];
    if (ctype[4 * i + 3] == svtkAVSucdReader::PYR)
    { // UCD ordering is 0,1,2,3,4 => SVTK ordering is 1,2,3,4,0
  ...
  for (i = 0; i < this->NumberOfCells; i++)
  {
    *mat++ = ctype[4 * i + 1];
    switch (ctype[4 * i + 3])
    {
      case svtkAVSucdReader::PT:
        types[i] = SVTK_VERTEX;
        break;
      case svtkAVSucdReader::LINE:
        types[i] = SVTK_LINE;
        break;
      case svtkAVSucdReader::TRI:
        types[i] = SVTK_TRIANGLE;
        break;
      case svtkAVSucdReader::QUAD:
        types[i] = SVTK_QUAD;
        break;
      case svtkAVSucdReader::TET:
        types[i] = SVTK_TETRA;
        break;
      case svtkAVSucdReader::PYR:
        types[i] = SVTK_PYRAMID;
        break;
      case svtkAVSucdReader::PRISM:
        types[i] = SVTK_WEDGE;
        break;
      case svtkAVSucdReader::HEX:
        types[i] = SVTK_HEXAHEDRON;
        break;
      default:
        svtkErrorMacro(<< "cell type: " << ctype[4 * i + 3] << "not supported\n");
        delete[] ctype;
        return;
    }
  }
...
//----------------------------------------------------------------------------
void svtkAVSucdReader::ReadASCIICellTopology(
  svtkIntArray* materials, svtkUnstructuredGrid* output, const idMapping& nodeMap, idMapping& cellMap)
{
  int i, k;
  svtkIdType list[8];
  int* mat = materials->GetPointer(0);
  ...
  for (i = 0; i < this->NumberOfCells; i++)
  {
    svtkIdType id;
    *(this->FileStream) >> id;
    cellMap.insert(std::make_pair(id, static_cast<svtkIdType>(i)));
    *(this->FileStream) >> mat[i];
    *(this->FileStream) >> ctype;
    svtkDebugMacro(<< mat[i] << ", " << ctype);
    if (ctype == "pt")
    {
      for (k = 0; k < 1; k++)
      {
      ...
      output->InsertNextCell(SVTK_VERTEX, 1, list);
    }
    else if (ctype == "line")
    {
      for (k = 0; k < 2; k++)
      ...
      output->InsertNextCell(SVTK_LINE, 2, list);
    }
    else if (ctype == "tri")
    {
      for (k = 0; k < 3; k++)
      ...
      output->InsertNextCell(SVTK_TRIANGLE, 3, list);
    }
    else if (ctype == "quad")
    {
      for (k = 0; k < 4; k++)
      ...
      output->InsertNextCell(SVTK_QUAD, 4, list);
    }
    else if (ctype == "tet")
    {
      for (k = 0; k < 4; k++)
      ...
      output->InsertNextCell(SVTK_TETRA, 4, list);
    }
    else if (ctype == "pyr")
    {
      for (k = 0; k < 5; k++)
      ...
      list[3] = list[4];
      list[4] = tmp;
      output->InsertNextCell(SVTK_PYRAMID, 5, list);
    }
    else if (ctype == "prism")
    {
      for (k = 0; k < 6; k++)
      ...
      output->InsertNextCell(SVTK_WEDGE, 6, list);
    }
    else if (ctype == "hex")
    {
      for (k = 0; k < 8; k++)
      ...
      output->InsertNextCell(SVTK_HEXAHEDRON, 8, list);
    }
    else
    {
      svtkErrorMacro(<< "cell type: " << ctype << " is not supported\n");
      return;
    }
...
//----------------------------------------------------------------------------
void svtkAVSucdReader::ReadXYZCoords(svtkFloatArray* coords, idMapping& nodeMap)
{
  int i;
  ...
  else
  {
    ...
    for (i = 0; i < this->NumberOfNodes; i++)
    {
      ...
      *(this->FileStream) >> ptr[3 * i] >> ptr[3 * i + 1] >> ptr[3 * i + 2];
      nodeMap.insert(std::make_pair(id, static_cast<svtkIdType>(i)));
    }
  } // end of ASCII read
}

//----------------------------------------------------------------------------
void svtkAVSucdReader::ReadNodeData(svtkUnstructuredGrid* output, const idMapping& nodeMap)
{
  int i, j, n;
  float* ptr;
  svtkDebugMacro(<< "Begin of ReadNodeData()\n");
  if (this->BinaryFile)
  {
    for (i = 0; i < this->NumberOfNodeComponents; i++)
    {
  ...
  else
  {
    ...
    *(this->FileStream) >> this->NumberOfNodeComponents;
    ...
    svtkFloatArray** scalars = new svtkFloatArray*[this->NumberOfNodeComponents];
    for (i = 0; i < this->NumberOfNodeComponents; i++)
    {
      j = 0;
  ...
  svtkDebugMacro(<< "End of ReadNodeData()\n");
}

//----------------------------------------------------------------------------
void svtkAVSucdReader::ReadCellData(svtkUnstructuredGrid* output, const idMapping& cellMap)
{
  int i, j, n;
  float* ptr;
  svtkDebugMacro(<< "Begin of ReadCellData()\n");
  if (this->BinaryFile)
  {
    for (i = 0; i < this->NumberOfCellComponents; i++)
    {
  ...
  else
  {
    ...
    *(this->FileStream) >> this->NumberOfCellComponents;
    ...
    svtkFloatArray** scalars = new svtkFloatArray*[this->NumberOfCellComponents];
    for (i = 0; i < this->NumberOfCellComponents; i++)
    {
      j = 0;
      while (this->FileStream->get(c) && c != ',')
  ...
  svtkDebugMacro(<< "End of ReadCellData()\n");
}

//----------------------------------------------------------------------------
const char* svtkAVSucdReader::GetPointArrayName(int index)
{
  return this->PointDataArraySelection->GetArrayName(index);
}

//----------------------------------------------------------------------------
int svtkAVSucdReader::GetPointArrayStatus(const char* name)
{
  return this->PointDataArraySelection->ArrayIsEnabled(name);
}

//----------------------------------------------------------------------------
void svtkAVSucdReader::SetPointArrayStatus(const char* name, int status)
{
  if (status)
  {
...
//----------------------------------------------------------------------------
const char* svtkAVSucdReader::GetCellArrayName(int index)
{
  return this->CellDataArraySelection->GetArrayName(index);
}

//----------------------------------------------------------------------------
int svtkAVSucdReader::GetCellArrayStatus(const char* name)
{
  return this->CellDataArraySelection->ArrayIsEnabled(name);
}

//----------------------------------------------------------------------------
void svtkAVSucdReader::SetCellArrayStatus(const char* name, int status)
{
  if (status)
  {
...
//----------------------------------------------------------------------------
int svtkAVSucdReader::GetNumberOfCellArrays()
{
  return this->CellDataArraySelection->GetNumberOfArrays();
}

//----------------------------------------------------------------------------
int svtkAVSucdReader::GetNumberOfPointArrays()
{
  return this->PointDataArraySelection->GetNumberOfArrays();
}

//----------------------------------------------------------------------------
void svtkAVSucdReader::EnableAllPointArrays()
{
  this->PointDataArraySelection->EnableAllArrays();
}

//----------------------------------------------------------------------------
void svtkAVSucdReader::DisableAllPointArrays()
{
  this->PointDataArraySelection->DisableAllArrays();
}

//----------------------------------------------------------------------------
void svtkAVSucdReader::EnableAllCellArrays()
{
  this->CellDataArraySelection->EnableAllArrays();
}

//----------------------------------------------------------------------------
void svtkAVSucdReader::DisableAllCellArrays()
{
  this->CellDataArraySelection->DisableAllArrays();
}

//----------------------------------------------------------------------------
int svtkAVSucdReader::GetLabel(char* string, int number, char* label)
{
  int i, j, k, len;
  char current;
...
int svtkAVSucdReader::ReadIntBlock(int n, int* block)
{
  if (this->BinaryFile)
  {
...
//----------------------------------------------------------------------------
int svtkAVSucdReader::ReadFloatBlock(int n, float* block)
{
  if (this->BinaryFile)
  {
