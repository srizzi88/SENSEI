...
svtkStandardNewMacro(svtkCGMWriter);

svtkCxxSetObjectMacro(svtkCGMWriter, Viewport, svtkViewport);

svtkCGMWriter::svtkCGMWriter()
{
...
//-------------------methods svtk uses to write data---------------------------
//
...
#define SVTK_HASH_INDEX 737
svtkColorHash::svtkColorHash()
{
  int i;
  this->Table = new svtkIdList*[SVTK_HASH_INDEX];
  for (i = 0; i < SVTK_HASH_INDEX; i++)
  {
...
int svtkColorHash::InsertUniqueColor(cgmImagePtr im, int r, int g, int b)
{
  int index = (65536 * r + 256 * g * b) % SVTK_HASH_INDEX;
  ...
  {
    svtkIdType numIds = this->Table[index]->GetNumberOfIds();
    int red, green, blue;
...
int svtkColorHash::GetColorIndex(cgmImagePtr im, int r, int g, int b)
{
  int index = (65536 * r + 256 * g * b) % SVTK_HASH_INDEX;
  svtkIdType cgmIndex;
  svtkIdType numIds = this->Table[index]->GetNumberOfIds();
...
#undef SVTK_HASH_INDEX

// ------------------------------end svtkColorHash stuff---------------
...
//
static svtkColorHash* DefineLUTColors(cgmImagePtr im, unsigned char* colors, int numColors, int bpp)
{
  svtkColorHash* colorHash = new svtkColorHash;
  unsigned char* ptr;
...
#ifdef SVTK_NOT_DEFINED
static int GetLUTColor(int svtkNotUsed(red), int svtkNotUsed(green), int svtkNotUsed(blue))
{
  return 0;
}
#endif

typedef struct _svtkSortValues
{
  float z;
  int cellId;
} svtkSortValues;
...
extern "C"
{
  int svtkCGMqsortCompare(const void* val1, const void* val2)
  {
    if (((svtkSortValues*)val1)->z > ((svtkSortValues*)val2)->z)
    {
      return (-1);
    }
    else if (((svtkSortValues*)val1)->z < ((svtkSortValues*)val2)->z)
    {
      return (1);
    }
    else
...
void svtkCGMWriter::WriteData()
{
  FILE* outf;
  svtkPolyData* input = this->GetInput();

  svtkIdType numCells = input->GetNumberOfCells(), cellId;
  ...
  if ((outf = svtksys::SystemTools::Fopen(this->FileName, "wb")) == nullptr)
  {
    svtkErrorMacro(<< "Cannot open CGM file");
    return;
  }

  cgmImagePtr im;
  svtkPoints *inPts = input->GetPoints(), *pts;
  svtkGenericCell* cell = svtkGenericCell::New();
  svtkDataArray* inScalars = input->GetCellData()->GetScalars();
  ...
  cgmPoint* points;
  svtkSortValues* depth = nullptr; // warnings
  ...
  {
    svtkPoints* displayPts = svtkPoints::New();
  ...
  //
  svtkPoints* scaledPts = svtkPoints::New();
  ...
  svtkColorHash* colorHash = nullptr;

  if (this->ColorMode == SVTK_COLOR_MODE_DEFAULT)
  {
    if (inScalars && inScalars->GetDataType() == SVTK_UNSIGNED_CHAR)
    {
      colorMode = SVTK_COLOR_MODE_DEFAULT;
      bpp = inScalars->GetNumberOfComponents();
      colors = static_cast<svtkUnsignedCharArray*>(inScalars)->GetPointer(0);
    }
    else
    {
      colorMode = SVTK_COLOR_MODE_SPECIFIED_COLOR;
    }
  }
  else
  {
  ...
  if (this->Sort)
  {
    depth = new svtkSortValues[numCells];
    ...
    qsort(depth, numCells, sizeof(svtkSortValues), svtkCGMqsortCompare);
  }
  ...
  for (cellId = 0; cellId < numCells; cellId++)
  {
    ...
    if (colorMode == SVTK_COLOR_MODE_DEFAULT)
    {
      ...
      switch (bpp)
      {
        ...
        case 4:
          ...
          break;
        default:
          svtkErrorMacro(<< "Unsupported bpp in svtkCGMWriter::WriteData");
          rgbColor[0] = 0;
          rgbColor[1] = 0;
          rgbColor[2] = 0;
          break;
    ...
    else if (colorMode == SVTK_COLOR_MODE_SPECIFIED_COLOR)
    {
    ...
    else // if ( colorMode == SVTK_COLOR_MODE_RANDOM_COLORS )
    {
      color = GetColor(static_cast<int>(svtkMath::Random(0, 255)),
        static_cast<int>(svtkMath::Random(0, 255)), static_cast<int>(svtkMath::Random(0, 255)),
        CGMColors);
    }
    ...
    switch (type)
    {
      ...
      case SVTK_TRIANGLE_STRIP:
        ...
        break;
      default:
        svtkErrorMacro(<< "Unsupported CGM type");
    }
  }
  if (colorMode == SVTK_COLOR_MODE_DEFAULT)
  {
...
void svtkCGMWriter::PrintSelf(ostream& os, svtkIndent indent)
{
  ...
  else if (this->ColorMode == SVTK_COLOR_MODE_SPECIFIED_COLOR)
  {
...
static int cgmCgmHeader(cgmImagePtr im)
{
  ...
  buf = reinterpret_cast<const unsigned char*>("svtk: Visualization Toolkit");
...
static int cgmImageColorDeallocate(cgmImagePtr svtkNotUsed(im), int svtkNotUsed(color))
/* wogl: the parameter names are commented to avoid compiler warnings */
/* From gd library, see README file for copyright information */
/* gej: should work unchanged */
{
