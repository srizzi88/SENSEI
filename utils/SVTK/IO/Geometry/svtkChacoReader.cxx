...
svtkStandardNewMacro(svtkChacoReader);

//----------------------------------------------------------------------------
// Description:
// Instantiate object with nullptr filename.
svtkChacoReader::svtkChacoReader()
{
  ...
  this->DataCache = svtkUnstructuredGrid::New();
...
//----------------------------------------------------------------------------
void svtkChacoReader::ClearWeightArrayNames()
{
  int i = 0;
  if (this->VarrayName)
  {
...
//----------------------------------------------------------------------------
void svtkChacoReader::MakeWeightArrayNames(int nv, int ne)
{
  int i = 0;
  if (nv > 0)
  {
...
//----------------------------------------------------------------------------
const char* svtkChacoReader::GetVertexWeightArrayName(int weight)
{
...
//----------------------------------------------------------------------------
const char* svtkChacoReader::GetEdgeWeightArrayName(int weight)
{
...
//----------------------------------------------------------------------------
int svtkChacoReader::RequestInformation(svtkInformation* svtkNotUsed(request),
...
//----------------------------------------------------------------------------
int svtkChacoReader::RequestData(svtkInformation* svtkNotUsed(request),
...
//----------------------------------------------------------------------------
int svtkChacoReader::BuildOutputGrid(svtkUnstructuredGrid* output)
{
  int i = 0;
  if (this->OpenCurrentFile() != 1)
  {
    svtkWarningMacro(<< "Can't open file");
    return 0;
  }
  ...
  {
    svtkDoubleArray* da = svtkArrayDownCast<svtkDoubleArray>(
  ...
  if (this->RemakeDataCacheFlag)
  {
    ...
    // and creating a svtkUnstructuredGrid from it.
  ...
  else
  {
    ...
    svtkIntArray* ia = svtkArrayDownCast<svtkIntArray>(
    ...
    ia = svtkArrayDownCast<svtkIntArray>(
...
//----------------------------------------------------------------------------
int svtkChacoReader::ReadFile(svtkUnstructuredGrid* output)
{
  int i = 0;
  svtkIdType id = 0;
  ...
  svtkPoints* ptarray = svtkPoints::New();
  ...
  if (!memoryOK)
  {
    svtkErrorMacro(<< "ReadFile memory allocation failure");
    delete[] x;
    delete[] y;
    return 0;
  }
  ...
  // by svtkPChacoReader, and all processes must create output
  ...
  svtkIdType* idx = nullptr;
  svtkIdType* nbors = nullptr;
  ...
  svtkDoubleArray** varrays = nullptr;
  svtkDoubleArray** earrays = nullptr;
  double* vwgt = nullptr;
  double* ewgt = nullptr;

  if (vw)
  {
    varrays = new svtkDoubleArray*[this->NumberOfVertexWeights];
    for (i = 0; i < this->NumberOfVertexWeights; i++)
    {
      varrays[i] = svtkDoubleArray::New();
  ...
  if (ew)
  {
    earrays = new svtkDoubleArray*[this->NumberOfEdgeWeights];
    for (i = 0; i < this->NumberOfEdgeWeights; i++)
    {
      earrays[i] = svtkDoubleArray::New();
  ...
  svtkIdTypeArray* ca = svtkIdTypeArray::New();
  ...
  if (idx == nullptr)
  {
    ...
    svtkIdType* captr = ca->GetPointer(0);
    ...
    output->SetCells(SVTK_VERTEX, cells);
    cells->Delete();
  }
  else
  {
    ...
    for (id = 0; id < this->NumberOfVertices; id++)
    {
      ...
      {
        svtkIdType nbor = nbors[n] - 1; // internal id
        ...
        if (nbor > id)
        {
          ...
          {
            svtkErrorMacro(<< "Too many edges in Chaco file");
            retVal = 0;
            break;
          }
    ...
    {
      svtkErrorMacro(<< "Too few edges in Chaco file");
      retVal = 0;
    }
    ...
    if (retVal)
    {
      ...
      cells->ImportLegacyFormat(ca);
      output->SetCells(SVTK_LINE, cells);
      cells->Delete();
    }
    else
    {
...
//----------------------------------------------------------------------------
void svtkChacoReader::AddElementIds(svtkUnstructuredGrid* output)
{
  // We arbitrarily assign the element ids, since Chaco files do
  // not have the notion of Element IDs.

  svtkIdType len = output->GetNumberOfCells();
...
//----------------------------------------------------------------------------
void svtkChacoReader::PrintSelf(ostream& os, svtkIndent indent)
{
  int i = 0;
...
//----------------------------------------------------------------------------
void svtkChacoReader::CloseCurrentFile()
{
...
//----------------------------------------------------------------------------
int svtkChacoReader::OpenCurrentFile()
{
  int result = 0;
  ...
  if (this->CurrentGeometryFP == nullptr)
  {
    ...
    this->CurrentGeometryFP = svtksys::SystemTools::Fopen(buf, "r");

    if (this->CurrentGeometryFP == nullptr)
    {
      svtkErrorMacro(<< "Problem opening " << buf);
      this->SetCurrentBaseName(nullptr);
    }
    else
    {
      ...
      this->CurrentGraphFP = svtksys::SystemTools::Fopen(buf, "r");
...
void svtkChacoReader::ResetInputBuffers()
{
  this->Line_length = 200;
  this->Offset = 0;
  this->Break_pnt = 200;
...
//----------------------------------------------------------------------------
int svtkChacoReader::InputGeom(svtkIdType nvtxs, // Number of vertices to read in
  ...
  double* x, double* y, double* z)
{
  ...
  if (end_flag == -1)
  {
    svtkErrorMacro(<< "No values found in geometry file " << this->BaseName << ".coords");
    return 0;
  }
  ...
  if (igeom == 0)
  {
    ...
    if (end_flag == 0)
    {
      ...
      if (end_flag == 0)
      {
        ...
        if (!end_flag)
        {
          svtkErrorMacro(<< "Invalid geometry file " << this->BaseName << ".coords");
          return 0;
        }
      }
    }
  ...
  for (int nread = 1; nread < nvtxs; nread++)
  {
    ...
    if (i == EOF)
    {
      svtkErrorMacro(<< "Too few lines in " << this->BaseName << ".coords");
      return 0;
    }
    else if (i != ndims)
    {
      svtkErrorMacro(<< "Wrong dimension in " << this->BaseName << ".coords");
      return 0;
    }
  }
...
//----------------------------------------------------------------------------
int svtkChacoReader::InputGraph1()
{
  ...
  if (numVertices <= 0)
  {
    svtkErrorMacro(<< "Invalid file " << this->BaseName << ".graph");
    return 0;
  }
  ...
  if (!end_flag)
  {
    svtkIdType option = this->ReadInt(fin, &end_flag);
  ...
  {
    svtkIdType j = this->ReadInt(fin, &end_flag);
    if (!end_flag)
...
//----------------------------------------------------------------------------
int svtkChacoReader::InputGraph2(
  svtkIdType** start,     // start[i]: location of vertex i in adjacency array
  svtkIdType** adjacency, // by vertex by vertex neighbor
  ...
  double** eweights)     // edge weights in order in file (or nullptr)
{
  svtkIdType* adjptr;
  double* ewptr;
  svtkIdType vtx, sum_edges, vertex, new_vertex;
  double weight, eweight;
  svtkIdType neighbor, j;
  int retVal = 1;

  svtkIdType nvtxs = this->NumberOfVertices;
  svtkIdType narcs = this->NumberOfEdges;
  ...
  if (nvtxs < 1)
  {
    svtkErrorMacro(<< "svtkChacoReader::InputGraph2, NumberOfVertices not set");
    return 0;
  }

  if (start == nullptr)
  {
    svtkErrorMacro(<< "svtkChacoReader::InputGraph2, parameter list");
    return 0;
  }
  ...
  *start = new svtkIdType[nvtxs + 1];
  if (adjacency && (narcs > 0))
  {
    *adjacency = new svtkIdType[2 * narcs + 1]; // why +1 ?
  }
  ...
  while (((vwgt_dim > 0) || vtxnums || narcs) && end_flag != -1)
  {
    ...
    if (vtxnums)
    {
      ...
      if (end_flag)
      {
        ...
        svtkErrorMacro(<< "Missing vertex number " << this->BaseName << ".graph, line " << line_num);
        retVal = 0;
        goto done;
      }
      if (j != vertex && j != vertex + 1)
      {
        svtkErrorMacro(<< "Out of order vertex " << this->BaseName << ".graph, line " << line_num);
        retVal = 0;
        goto done;
      }
      if (j != vertex)
    ...
    if ((vwgt_dim > 0) && new_vertex)
    {
      for (j = 0; j < (vwgt_dim); j++)
      {
        ...
        if (end_flag)
        {
          svtkErrorMacro(<< "Vertex weights " << this->BaseName << ".graph, line " << line_num);
          retVal = 0;
          goto done;
        }
    ...
    while (!end_flag)
    {
      if (ewgt_dim > 0)
      {
        for (j = 0; j < ewgt_dim; j++)
        {
          ...
          if (end_flag)
          {
            svtkErrorMacro(<< "Edge weights " << this->BaseName << ".graph, line " << line_num);
            retVal = 0;
            goto done;
          }
      ...
      {
        svtkErrorMacro(<< "Too many adjacencies " << this->BaseName << ".graph, line " << line_num);
        retVal = 0;
        goto done;
      }
...
//----------------------------------------------------------------------------
double svtkChacoReader::ReadVal(FILE* infile, int* end_flag)
{
  double val;
  char* ptr;
  char* ptr2;
...
//----------------------------------------------------------------------------
svtkIdType svtkChacoReader::ReadInt(FILE* infile, int* end_flag)
{
  svtkIdType val;
  char* ptr;
  char* ptr2;
...
//----------------------------------------------------------------------------
void svtkChacoReader::FlushLine(FILE* infile)
{
  char c;
