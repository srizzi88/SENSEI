...
/**
 * @class   svtkChacoReader
 * @brief   Read a Chaco file and create a svtkUnstructuredGrid.
 *
 * svtkChacoReader is an unstructured grid source object that reads Chaco
 ...
 *
 * Note that the Chaco "edges" become SVTK "cells", and the Chaco
 * "vertices" become SVTK "points".
 */

#ifndef svtkChacoReader_h
#define svtkChacoReader_h

#include "svtkIOGeometryModule.h" // For export macro
#include "svtkUnstructuredGridAlgorithm.h"

class SVTKIOGEOMETRY_EXPORT svtkChacoReader : public svtkUnstructuredGridAlgorithm
{
public:
  static svtkChacoReader* New();
  svtkTypeMacro(svtkChacoReader, svtkUnstructuredGridAlgorithm);
   ...
   * global IDs for the cells in the output svtkUnstructuredGrid.  These cells
   * represent the edges that were in the Chaco file.  Each edge is a svtkLine.
   * Default is ON.
   */
   ...
   * the points in the output svtkUnstructuredGrid.  These points are the vertices
  ...
  svtkSetMacro(GenerateVertexWeightArrays, svtkTypeBool);
  svtkGetMacro(GenerateVertexWeightArrays, svtkTypeBool);
  svtkBooleanMacro(GenerateVertexWeightArrays, svtkTypeBool);
  ...
  svtkGetMacro(NumberOfVertexWeights, int);
  ...
  svtkSetMacro(GenerateEdgeWeightArrays, svtkTypeBool);
  svtkGetMacro(GenerateEdgeWeightArrays, svtkTypeBool);
  svtkBooleanMacro(GenerateEdgeWeightArrays, svtkTypeBool);
  ...
  svtkGetMacro(NumberOfEdgeWeights, int);
  ...
  svtkGetMacro(NumberOfVertices, svtkIdType);
  //@}
  ...
  svtkGetMacro(NumberOfCellWeightArrays, int);
  ...
  svtkGetMacro(NumberOfPointWeightArrays, int);

protected:
  svtkChacoReader();
  ~svtkChacoReader() override;

  int BuildOutputGrid(svtkUnstructuredGrid* gr);
  ...
  int InputGeom(svtkIdType nvtxs, int igeom, double* x, double* y, double* z);
  int InputGraph1();
  int InputGraph2(svtkIdType** start, svtkIdType** adjacency, double** vweights, double** eweights);
  int GetCoordsMetadata();
  void GetGraphMetadata();

  // methods for creating svtkUnstructuredGrid from Chaco file data

  int ReadFile(svtkUnstructuredGrid* output);
  void AddElementIds(svtkUnstructuredGrid* output);
  void AddNodeIds(svtkUnstructuredGrid* output);
  ...
  svtkTypeBool GenerateVertexWeightArrays;
  svtkTypeBool GenerateEdgeWeightArrays;

  FILE* CurrentGeometryFP;
  FILE* CurrentGraphFP;
  char* CurrentBaseName;
  svtkSetStringMacro(CurrentBaseName);
  ...
  // add new metadata, you must modify svtkPChacoReader::RequestInformation
  ...
  // options change.
  svtkUnstructuredGrid* DataCache;
...
private:
  svtkChacoReader(const svtkChacoReader&) = delete;
  void operator=(const svtkChacoReader&) = delete;

  double ReadVal(FILE* infile, int* end_flag);
  svtkIdType ReadInt(FILE* infile, int* end_flag);
