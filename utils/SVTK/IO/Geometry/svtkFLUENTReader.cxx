...
struct svtkFLUENTReader::ScalarDataChunk
{
  int subsectionId;
  svtkIdType zoneId;
...
struct svtkFLUENTReader::VectorDataChunk
{
  int subsectionId;
  svtkIdType zoneId;
...
struct svtkFLUENTReader::stdString
{
  std::string value;
};
struct svtkFLUENTReader::intVector
{
  std::vector<int> value;
};
struct svtkFLUENTReader::doubleVector
{
  std::vector<double> value;
};
struct svtkFLUENTReader::stringVector
{
  std::vector<std::string> value;
};
struct svtkFLUENTReader::cellVector
{
  std::vector<Cell> value;
};
struct svtkFLUENTReader::faceVector
{
  std::vector<Face> value;
};
struct svtkFLUENTReader::stdMap
{
  std::map<int, std::string> value;
};
struct svtkFLUENTReader::scalarDataVector
{
  std::vector<ScalarDataChunk> value;
};
struct svtkFLUENTReader::vectorDataVector
{
  std::vector<VectorDataChunk> value;
};
struct svtkFLUENTReader::intVectorVector
{
  std::vector<std::vector<int> > value;
};

//----------------------------------------------------------------------------
svtkFLUENTReader::svtkFLUENTReader()
{
...
//----------------------------------------------------------------------------
int svtkFLUENTReader::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** svtkNotUsed(inputVector), svtkInformationVector* outputVector)
{
  ...
  svtkMultiBlockDataSet* output =
    svtkMultiBlockDataSet::SafeDownCast(outInfo->Get(svtkMultiBlockDataSet::DATA_OBJECT()));

  output->SetNumberOfBlocks(static_cast<unsigned int>(this->CellZones->value.size()));
  // svtkUnstructuredGrid *Grid[CellZones.size()];

  std::vector<svtkUnstructuredGrid*> grid;
  grid.resize(this->CellZones->value.size());

  for (size_t test = 0; test < this->CellZones->value.size(); test++)
  {
    grid[test] = svtkUnstructuredGrid::New();
  }
  ...
  for (size_t i = 0; i < this->Cells->value.size(); i++)
  {
    ...
    else if (this->Cells->value[i].type == 7)
    {
      this->ConvexPointSet->GetPointIds()->SetNumberOfIds(
        static_cast<svtkIdType>(this->Cells->value[i].nodes.size()));
      for (size_t j = 0; j < this->Cells->value[i].nodes.size(); j++)
      {
        this->ConvexPointSet->GetPointIds()->SetId(
          static_cast<svtkIdType>(j), this->Cells->value[i].nodes[j]);
      }
      grid[location]->InsertNextCell(
  ...
  for (size_t l = 0; l < this->ScalarDataChunks->value.size(); l++)
  {
    ...
    svtkDoubleArray* v = svtkDoubleArray::New();
    for (size_t m = 0; m < this->ScalarDataChunks->value[l].scalarData.size(); m++)
    {
      v->InsertValue(static_cast<svtkIdType>(m), this->ScalarDataChunks->value[l].scalarData[m]);
    }
  ...
  for (size_t l = 0; l < this->VectorDataChunks->value.size(); l++)
  {
    ...
    {
      v->InsertComponent(
        static_cast<svtkIdType>(m), 0, this->VectorDataChunks->value[l].iComponentData[m]);
      v->InsertComponent(
        static_cast<svtkIdType>(m), 1, this->VectorDataChunks->value[l].jComponentData[m]);
      v->InsertComponent(
        static_cast<svtkIdType>(m), 2, this->VectorDataChunks->value[l].kComponentData[m]);
    }
...
//----------------------------------------------------------------------------
void svtkFLUENTReader::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
int svtkFLUENTReader::RequestInformation(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** svtkNotUsed(inputVector), svtkInformationVector* svtkNotUsed(outputVector))
{
  ...
  {
    svtkErrorMacro("Unable to open cas file.");
    return 0;
  }
  ...
  if (!dat_file_opened)
  {
    svtkWarningMacro("Unable to open dat file.");
  }
  ...
  this->NumberOfCells = static_cast<svtkIdType>(this->Cells->value.size());
  return 1;
}

//----------------------------------------------------------------------------
bool svtkFLUENTReader::OpenCaseFile(const char* filename)
{
  std::ios_base::openmode mode = ios::in;
#ifdef _WIN32
  mode |= ios::binary;
#endif
  this->FluentCaseFile = new svtksys::ifstream(filename, mode);
...
//----------------------------------------------------------------------------
int svtkFLUENTReader::GetNumberOfCellArrays()
{
  return this->CellDataArraySelection->GetNumberOfArrays();
}

//----------------------------------------------------------------------------
const char* svtkFLUENTReader::GetCellArrayName(int index)
{
  return this->CellDataArraySelection->GetArrayName(index);
}

//----------------------------------------------------------------------------
int svtkFLUENTReader::GetCellArrayStatus(const char* name)
{
  return this->CellDataArraySelection->ArrayIsEnabled(name);
}

//----------------------------------------------------------------------------
void svtkFLUENTReader::SetCellArrayStatus(const char* name, int status)
{
  if (status)
  {
...
//----------------------------------------------------------------------------
void svtkFLUENTReader::EnableAllCellArrays()
{
  this->CellDataArraySelection->EnableAllArrays();
}

//----------------------------------------------------------------------------
void svtkFLUENTReader::DisableAllCellArrays()
{
...
//----------------------------------------------------------------------------
bool svtkFLUENTReader::OpenDataFile(const char* filename)
{
  ...
  mode |= ios::binary;
#endif
  this->FluentDataFile = new svtksys::ifstream(dfilename.c_str(), mode);
  if (this->FluentDataFile->fail())
  {
    svtkErrorMacro("Could not open data file "
...
//----------------------------------------------------------------------------
int svtkFLUENTReader::GetCaseChunk()
{
...
//----------------------------------------------------------------------------
int svtkFLUENTReader::GetCaseIndex()
{
  std::string sindex;
...
//----------------------------------------------------------------------------
void svtkFLUENTReader::GetNumberOfCellZones()
{
  int match;
...
//----------------------------------------------------------------------------
int svtkFLUENTReader::GetDataIndex()
{
  std::string sindex;
...
//----------------------------------------------------------------------------
int svtkFLUENTReader::GetDataChunk()
{
...
void svtkFLUENTReader::LoadVariableNames()
{
...
//----------------------------------------------------------------------------
void svtkFLUENTReader::ParseCaseFile()
{
...
//----------------------------------------------------------------------------
int svtkFLUENTReader::GetDimension()
{
...
//----------------------------------------------------------------------------
void svtkFLUENTReader::GetLittleEndianFlag()
{
  size_t start = this->CaseBuffer->value.find('(', 1);
...
//----------------------------------------------------------------------------
void svtkFLUENTReader::GetNodesAscii()
{
  size_t start = this->CaseBuffer->value.find('(', 1);
  size_t end = this->CaseBuffer->value.find(')', 1);
  std::string info = this->CaseBuffer->value.substr(start + 1, end - start - 1);
...
//----------------------------------------------------------------------------
void svtkFLUENTReader::GetNodesSinglePrecision()
{
  size_t start = this->CaseBuffer->value.find('(', 1);
  size_t end = this->CaseBuffer->value.find(')', 1);
  std::string info = this->CaseBuffer->value.substr(start + 1, end - start - 1);
...
//----------------------------------------------------------------------------
void svtkFLUENTReader::GetNodesDoublePrecision()
{
  size_t start = this->CaseBuffer->value.find('(', 1);
  size_t end = this->CaseBuffer->value.find(')', 1);
  std::string info = this->CaseBuffer->value.substr(start + 1, end - start - 1);
...
//----------------------------------------------------------------------------
void svtkFLUENTReader::GetCellsAscii()
{
  if (this->CaseBuffer->value.at(5) == '0')
  { // Cell Info
    size_t start = this->CaseBuffer->value.find('(', 1);
...
//----------------------------------------------------------------------------
void svtkFLUENTReader::GetCellsBinary()
{
  size_t start = this->CaseBuffer->value.find('(', 1);
  size_t end = this->CaseBuffer->value.find(')', 1);
  std::string info = this->CaseBuffer->value.substr(start + 1, end - start - 1);
...
//----------------------------------------------------------------------------
void svtkFLUENTReader::GetFacesAscii()
{
...
//----------------------------------------------------------------------------
void svtkFLUENTReader::GetFacesBinary()
{
  size_t start = this->CaseBuffer->value.find('(', 1);
  size_t end = this->CaseBuffer->value.find(')', 1);
  std::string info = this->CaseBuffer->value.substr(start + 1, end - start - 1);
...
//----------------------------------------------------------------------------
void svtkFLUENTReader::GetPeriodicShadowFacesAscii()
{
  size_t start = this->CaseBuffer->value.find('(', 1);
  size_t end = this->CaseBuffer->value.find(')', 1);
  std::string info = this->CaseBuffer->value.substr(start + 1, end - start - 1);
...
//----------------------------------------------------------------------------
void svtkFLUENTReader::GetPeriodicShadowFacesBinary()
{
  size_t start = this->CaseBuffer->value.find('(', 1);
  size_t end = this->CaseBuffer->value.find(')', 1);
  std::string info = this->CaseBuffer->value.substr(start + 1, end - start - 1);
...
//----------------------------------------------------------------------------
void svtkFLUENTReader::GetCellTreeAscii()
{
  size_t start = this->CaseBuffer->value.find('(', 1);
  size_t end = this->CaseBuffer->value.find(')', 1);
  std::string info = this->CaseBuffer->value.substr(start + 1, end - start - 1);
...
//----------------------------------------------------------------------------
void svtkFLUENTReader::GetCellTreeBinary()
{
...
//----------------------------------------------------------------------------
void svtkFLUENTReader::GetFaceTreeAscii()
{
  size_t start = this->CaseBuffer->value.find('(', 1);
  size_t end = this->CaseBuffer->value.find(')', 1);
  std::string info = this->CaseBuffer->value.substr(start + 1, end - start - 1);
...
//----------------------------------------------------------------------------
void svtkFLUENTReader::GetFaceTreeBinary()
{
  size_t start = this->CaseBuffer->value.find('(', 1);
  size_t end = this->CaseBuffer->value.find(')', 1);
  std::string info = this->CaseBuffer->value.substr(start + 1, end - start - 1);
...
//----------------------------------------------------------------------------
void svtkFLUENTReader::GetInterfaceFaceParentsAscii()
{
  size_t start = this->CaseBuffer->value.find('(', 1);
  size_t end = this->CaseBuffer->value.find(')', 1);
  std::string info = this->CaseBuffer->value.substr(start + 1, end - start - 1);
...
//----------------------------------------------------------------------------
void svtkFLUENTReader::GetInterfaceFaceParentsBinary()
{
  size_t start = this->CaseBuffer->value.find('(', 1);
  size_t end = this->CaseBuffer->value.find(')', 1);
  std::string info = this->CaseBuffer->value.substr(start + 1, end - start - 1);
...
//----------------------------------------------------------------------------
void svtkFLUENTReader::GetNonconformalGridInterfaceFaceInformationAscii()
{
  size_t start = this->CaseBuffer->value.find('(', 1);
  size_t end = this->CaseBuffer->value.find(')', 1);
  std::string info = this->CaseBuffer->value.substr(start + 1, end - start - 1);
...
//----------------------------------------------------------------------------
void svtkFLUENTReader::GetNonconformalGridInterfaceFaceInformationBinary()
{
  size_t start = this->CaseBuffer->value.find('(', 1);
  size_t end = this->CaseBuffer->value.find(')', 1);
...
//----------------------------------------------------------------------------
void svtkFLUENTReader::CleanCells()
{
...
//----------------------------------------------------------------------------
void svtkFLUENTReader::PopulateCellNodes()
{
  for (size_t i = 0; i < this->Cells->value.size(); i++)
  {
    const svtkIdType id = static_cast<svtkIdType>(i);
...
//----------------------------------------------------------------------------
int svtkFLUENTReader::GetCaseBufferInt(int ptr)
{
  union mix_i {
    int i;
    char c[4];
...
//----------------------------------------------------------------------------
float svtkFLUENTReader::GetCaseBufferFloat(int ptr)
{
  union mix_f {
    float f;
    char c[4];
...
//----------------------------------------------------------------------------
double svtkFLUENTReader::GetCaseBufferDouble(int ptr)
{
  union mix_i {
    double d;
    char c[8];
...
//----------------------------------------------------------------------------
void svtkFLUENTReader::PopulateTriangleCell(int i)
{
...
//----------------------------------------------------------------------------
void svtkFLUENTReader::PopulateTetraCell(int i)
{
...
//----------------------------------------------------------------------------
void svtkFLUENTReader::PopulateQuadCell(int i)
{
  this->Cells->value[i].nodes.resize(4);
...
//----------------------------------------------------------------------------
void svtkFLUENTReader::PopulateHexahedronCell(int i)
{
...
//----------------------------------------------------------------------------
void svtkFLUENTReader::PopulatePyramidCell(int i)
{
...
//----------------------------------------------------------------------------
void svtkFLUENTReader::PopulateWedgeCell(int i)
{
...
//----------------------------------------------------------------------------
void svtkFLUENTReader::PopulatePolyhedronCell(int i)
{
...
//----------------------------------------------------------------------------
void svtkFLUENTReader::ParseDataFile()
{
...
//----------------------------------------------------------------------------
int svtkFLUENTReader::GetDataBufferInt(int ptr)
{
  union mix_i {
    int i;
    char c[4];
...
//----------------------------------------------------------------------------
float svtkFLUENTReader::GetDataBufferFloat(int ptr)
{
  union mix_f {
    float f;
    char c[4];
...
//----------------------------------------------------------------------------
double svtkFLUENTReader::GetDataBufferDouble(int ptr)
{
  union mix_i {
    double d;
    char c[8];
...
//------------------------------------------------------------------------------
void svtkFLUENTReader::GetData(int dataType)
{
...
//----------------------------------------------------------------------------
void svtkFLUENTReader::SetDataByteOrderToBigEndian()
{
...
//----------------------------------------------------------------------------
void svtkFLUENTReader::SetDataByteOrderToLittleEndian()
{
...
//----------------------------------------------------------------------------
void svtkFLUENTReader::SetDataByteOrder(int byteOrder)
{
...
//----------------------------------------------------------------------------
int svtkFLUENTReader::GetDataByteOrder()
{
#ifdef SVTK_WORDS_BIGENDIAN
  if (this->SwapBytes)
  {
...
//----------------------------------------------------------------------------
const char* svtkFLUENTReader::GetDataByteOrderAsString()
{
#ifdef SVTK_WORDS_BIGENDIAN
  if (this->SwapBytes)
  {
...
//------------------------------------------------------------------------------
void svtkFLUENTReader::GetSpeciesVariableNames()
{
