...
svtkStandardNewMacro(svtkGAMBITReader);

//----------------------------------------------------------------------------
svtkGAMBITReader::svtkGAMBITReader()
{
...
//----------------------------------------------------------------------------
int svtkGAMBITReader::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** svtkNotUsed(inputVector), svtkInformationVector* outputVector)
{
  ...
  svtkDebugMacro(<< "Reading GAMBIT Neutral file");
...
//----------------------------------------------------------------------------
void svtkGAMBITReader::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
void svtkGAMBITReader::ReadFile(svtkUnstructuredGrid* output)
{
...
//----------------------------------------------------------------------------
void svtkGAMBITReader::ReadNodeData(svtkUnstructuredGrid* svtkNotUsed(output))
{
  svtkWarningMacro("Not implemented due to lack of examples");
}

//----------------------------------------------------------------------------
void svtkGAMBITReader::ReadCellData(svtkUnstructuredGrid* svtkNotUsed(output))
{
  svtkWarningMacro("Not implemented due to lack of examples");
}

//----------------------------------------------------------------------------
int svtkGAMBITReader::RequestInformation(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** svtkNotUsed(inputVector), svtkInformationVector* svtkNotUsed(outputVector))
{
  ...
  *(this->FileStream) >> this->NumberOfVelocityComponents;
  ...
  {
    svtkErrorMacro(<< "Error reading file");
  }
...
//----------------------------------------------------------------------------
void svtkGAMBITReader::ReadGeometry(svtkUnstructuredGrid* output)
{
...
//----------------------------------------------------------------------------
void svtkGAMBITReader::ReadBoundaryConditionSets(svtkUnstructuredGrid* output)
{
  ...
  svtkIntArray* bcscalar = svtkIntArray::New();
    ...
    if (itype == 0) // nodes
    {
      ...
      for (f = 0; f < nentry; f++)
      {
        *(this->FileStream) >> node;
        ...
        {
          svtkErrorMacro(<< "Node value is outside of range");
        }
      }
      this->FileStream->get(c);
      // read here the end of section
      ...
      if (strncmp(buf, "ENDOFSECTION", 12))
      {
        svtkErrorMacro(<< "Error reading ENDOFSECTION tag at end of group");
      }
    }
  ...
  svtkDebugMacro(<< "All BCS read successfully");
  if (isUsable)
  {
...
//----------------------------------------------------------------------------
void svtkGAMBITReader::ReadMaterialTypes(svtkUnstructuredGrid* output)
{
  ...
  svtkDebugMacro(<< "All groups read successfully");
...
//----------------------------------------------------------------------------
void svtkGAMBITReader::ReadCellConnectivity(svtkUnstructuredGrid* output)
{
  int i, k;
  svtkIdType list[27];
  char c, buf[128];
  ...
  for (i = 1; i <= this->NumberOfCells; i++)
  {
    ...
    *(this->FileStream) >> id >> ntype >> ndp;
    ...
    switch (ntype)
    {
      ...
      default:
      {
        svtkErrorMacro(<< "cell type: " << ntype << " is not supported\n");
        return;
      }
  ...
  if (strncmp(buf, "ENDOFSECTION", 12))
  {
    svtkErrorMacro(<< "Error reading ENDOFSECTION tag at end of connectivity");
  }
}

//----------------------------------------------------------------------------
void svtkGAMBITReader::ReadXYZCoords(svtkDoubleArray* coords)
{
  int i;
  ...
  if (strncmp(buf, "ENDOFSECTION", 12))
  {
    svtkErrorMacro("Error reading ENDOFSECTION tag at end of coordinates section");
  }
}
