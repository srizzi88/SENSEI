/*=========================================================================

  Program:   Visualization Toolkit
  Module:    svtkGAMBITReader.h

  Copyright (c) Ken Martin, Will Schroeder, Bill Lorensen
  All rights reserved.
  See Copyright.txt or http://www.kitware.com/Copyright.htm for details.

     This software is distributed WITHOUT ANY WARRANTY; without even
     the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR
     PURPOSE.  See the above copyright notice for more information.

=========================================================================*/
/**
 * @class   svtkGAMBITReader
 * @brief   reads a dataset in Fluent GAMBIT neutral file format
 *
 * svtkGAMBITReader creates an unstructured grid dataset. It reads ASCII files
 * stored in GAMBIT neutral format, with optional data stored at the nodes or
 * at the cells of the model. A cell-based fielddata stores the material id.
 *
 * @par Thanks:
 * Thanks to Jean M. Favre (CSCS, Swiss Center for Scientific Computing) who
 * developed this class.
 * Please address all comments to Jean Favre (jfavre at cscs.ch)
 *
 * @sa
 * svtkAVSucdReader
 */

#ifndef svtkGAMBITReader_h
#define svtkGAMBITReader_h

#include "svtkIOGeometryModule.h" // For export macro
#include "svtkUnstructuredGridAlgorithm.h"

class svtkDoubleArray;
class SVTKIOGEOMETRY_EXPORT svtkGAMBITReader : public svtkUnstructuredGridAlgorithm
{
public:
  static svtkGAMBITReader* New();
  svtkTypeMacro(svtkGAMBITReader, svtkUnstructuredGridAlgorithm);
  void PrintSelf(ostream& os, svtkIndent indent) override;

  //@{
  /**
   * Specify the file name of the GAMBIT data file to read.
   */
  svtkSetStringMacro(FileName);
  svtkGetStringMacro(FileName);
  //@}

  //@{
  /**
   * Get the total number of cells. The number of cells is only valid after a
   * successful read of the data file is performed.
   */
  svtkGetMacro(NumberOfCells, int);
  //@}

  //@{
  /**
   * Get the total number of nodes. The number of nodes is only valid after a
   * successful read of the data file is performed.
   */
  svtkGetMacro(NumberOfNodes, int);
  //@}

  //@{
  /**
   * Get the number of data components at the nodes and cells.
   */
  svtkGetMacro(NumberOfNodeFields, int);
  svtkGetMacro(NumberOfCellFields, int);
  //@}

protected:
  svtkGAMBITReader();
  ~svtkGAMBITReader() override;
  int RequestInformation(svtkInformation*, svtkInformationVector**, svtkInformationVector*) override;
  int RequestData(svtkInformation*, svtkInformationVector**, svtkInformationVector*) override;

  char* FileName;

  int NumberOfNodes;
  int NumberOfCells;
  int NumberOfNodeFields;
  int NumberOfCellFields;
  int NumberOfElementGroups;
  int NumberOfBoundaryConditionSets;
  int NumberOfCoordinateDirections;
  int NumberOfVelocityComponents;
  istream* FileStream;

  enum GAMBITCellType
  {
    EDGE = 1,
    QUAD = 2,
    TRI = 3,
    BRICK = 4,
    PRISM = 5,
    TETRA = 6,
    PYRAMID = 7
  };

private:
  void ReadFile(svtkUnstructuredGrid* output);
  void ReadGeometry(svtkUnstructuredGrid* output);
  void ReadNodeData(svtkUnstructuredGrid* output);
  void ReadCellData(svtkUnstructuredGrid* output);

  void ReadXYZCoords(svtkDoubleArray* coords);

  void ReadCellConnectivity(svtkUnstructuredGrid* output);
  void ReadMaterialTypes(svtkUnstructuredGrid* output);
  void ReadBoundaryConditionSets(svtkUnstructuredGrid* output);

  svtkGAMBITReader(const svtkGAMBITReader&) = delete;
  void operator=(const svtkGAMBITReader&) = delete;
};

#endif
