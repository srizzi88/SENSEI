/*=========================================================================
  ...
  Module:    svtkGLTFDocumentLoader.cxx
...
// they match the definitions from gl.h but are redefined below to avoid including svtkOpenGL.h
...
//----------------------------------------------------------------------------
svtkIdType GetNumberOfCellsForPrimitive(int mode, int cellSize, int numberOfIndices)
{
  if (cellSize <= 0)
  {
    svtkWarningWithObjectMacro(nullptr, "Invalid cell size. Ignoring connectivity.");
    return 0;
  }
  switch (mode)
  {
    case svtkGLTFDocumentLoaderInternals::GL_TRIANGLES:
    case svtkGLTFDocumentLoaderInternals::GL_LINES:
    case svtkGLTFDocumentLoaderInternals::GL_POINTS:
      return numberOfIndices / cellSize;
    case svtkGLTFDocumentLoaderInternals::GL_TRIANGLE_FAN:
      return numberOfIndices - 2;
    case svtkGLTFDocumentLoaderInternals::GL_LINE_LOOP:
      return numberOfIndices;
    case svtkGLTFDocumentLoaderInternals::GL_LINE_STRIP:
    case svtkGLTFDocumentLoaderInternals::GL_TRIANGLE_STRIP:
      return 1; // Number of strips
    default:
      svtkWarningWithObjectMacro(nullptr, "Invalid primitive draw mode. Ignoring connectivity.");
      return 0;
  }
}

//----------------------------------------------------------------------------
void GenerateIndicesForPrimitive(svtkGLTFDocumentLoader::Primitive& primitive)
{
  primitive.Indices = svtkSmartPointer<svtkCellArray>::New();

  svtkIdType nVert = primitive.Geometry->GetPoints()->GetNumberOfPoints();

  // Handles cases where we need a single cell
  if (primitive.Mode == svtkGLTFDocumentLoaderInternals::GL_LINE_STRIP ||
    primitive.Mode == svtkGLTFDocumentLoaderInternals::GL_TRIANGLE_STRIP ||
    primitive.Mode == svtkGLTFDocumentLoaderInternals::GL_LINE_LOOP)
  {
    primitive.Indices->AllocateEstimate(1, 1);
    std::vector<svtkIdType> cell(nVert);
    // Append all indices
    std::iota(cell.begin(), cell.end(), 0);
    if (primitive.Mode == svtkGLTFDocumentLoaderInternals::GL_LINE_LOOP)
    {
      cell.push_back(0);
    }
    primitive.Indices->InsertNextCell(static_cast<svtkIdType>(cell.size()), cell.data());
  }
  else
  {
    svtkIdType nCells = GetNumberOfCellsForPrimitive(primitive.Mode, primitive.CellSize, nVert);
    primitive.Indices->AllocateEstimate(nCells, 1);
    std::vector<svtkIdType> cell(primitive.CellSize, 0);
    for (int cellId = 0; cellId < nCells; cellId++)
    {
      // Triangle fan (for each vertex N, create primitive {0, n-1, n})
      if (primitive.Mode == svtkGLTFDocumentLoaderInternals::GL_TRIANGLE_FAN)
      {
        cell[0] = 0;
        cell[1] = cellId + 1;
        cell[2] = cellId + 2;
...
//----------------------------------------------------------------------------
const std::vector<std::string> svtkGLTFDocumentLoader::SupportedExtensions = {
  "KHR_lights_punctual"
};

//----------------------------------------------------------------------------
svtkStandardNewMacro(svtkGLTFDocumentLoader);

/** Metadata loading **/
//----------------------------------------------------------------------------
bool svtkGLTFDocumentLoader::LoadModelMetaDataFromFile(std::string fileName)
{
  svtkGLTFDocumentLoaderInternals impl;
  ...
  {
    svtkErrorMacro("Could not allocate InternalModel");
    return false;
  }

  fileName = svtksys::SystemTools::CollapseFullPath(fileName);
...
struct svtkGLTFDocumentLoader::BufferDataExtractionWorker
{
  int ByteOffset;
  int ByteStride;
  int Count;
  ...
  /**
   * Extracts data from a binary buffer into a typed svtk array.
  ...
  void operator()(ArrayType* output)
  {
    ...
    SVTK_ASSUME(output->GetNumberOfComponents() == this->NumberOfComponents);
         ...
         it != this->Inbuf->begin() + this->ByteOffset + this->Count * step; it += step)
    {
      ...
      for (auto elemIt = it; elemIt != it + this->NumberOfComponents * size; elemIt += size)
      {
        ...
        if (this->Normalized)
        {
          ...
          {
            realVal = svtkMath::Max(val / max, -1.0f);
          }
          else
          {
            realVal = val / max;
...
//----------------------------------------------------------------------------
struct svtkGLTFDocumentLoader::AccessorLoadingWorker
{
   ...
   */
  template <typename ArrayType, typename svtkArrayDispatchType>
    ...
    ArrayType* output, const Accessor& accessor, const BufferView& bufferView)
  {
    switch (accessor.ComponentTypeValue)
    {
      case ComponentType::BYTE:
        this->ExecuteBufferDataExtractionWorker<char, ArrayType, svtkArrayDispatchType>(
          output, accessor, bufferView);
        break;
      case ComponentType::UNSIGNED_BYTE:
        this->ExecuteBufferDataExtractionWorker<unsigned char, ArrayType, svtkArrayDispatchType>(
          output, accessor, bufferView);
        break;
      case ComponentType::SHORT:
        this->ExecuteBufferDataExtractionWorker<short, ArrayType, svtkArrayDispatchType>(
          output, accessor, bufferView);
        break;
      case ComponentType::UNSIGNED_SHORT:
        this->ExecuteBufferDataExtractionWorker<uint16_t, ArrayType, svtkArrayDispatchType>(
          output, accessor, bufferView);
        break;
      case ComponentType::UNSIGNED_INT:
        this->ExecuteBufferDataExtractionWorker<uint32_t, ArrayType, svtkArrayDispatchType>(
          output, accessor, bufferView);
        break;
      case ComponentType::FLOAT:
        this->ExecuteBufferDataExtractionWorker<float, ArrayType, svtkArrayDispatchType>(
          output, accessor, bufferView);
        break;
      default:
        return;
  ...
  /**
   * Determines svtkArrayDispatch type, then calls DispatchWorkerExecutionByComponentType,
    ...
    ArrayType* output, const Accessor& accessor, const BufferView& bufferView)
  {
    ...
    {
      this->DispatchWorkerExecutionByComponentType<ArrayType, svtkArrayDispatch::Reals>(
        output, accessor, bufferView);
    }
    else
    {
      this->DispatchWorkerExecutionByComponentType<ArrayType, svtkArrayDispatch::Integrals>(
        output, accessor, bufferView);
    }
  }
   ...
   */
  template <typename ComponentType, typename ArrayType, typename svtkArrayDispatchType>
    ...
    ArrayType* output, const Accessor& accessor, const BufferView& bufferView)
  {
    ...
    svtkArrayDispatch::DispatchByValueType<svtkArrayDispatchType>::Execute(output, worker);
  }

  void Setup(int accessorId, svtkGLTFDocumentLoader::AccessorType expectedType)
  {
  ...
  void operator()(ArrayType* output)
  {
    ...
    if (accessor.IsSparse)
    {
      ...
      // Load indices
      svtkNew<svtkIntArray> sparseIndices;
      ...
      DispatchWorkerExecution<svtkIntArray>(sparseIndices, mockIndicesAccessor, indicesBufferView);

      // Load values
      svtkNew<ArrayType> sparseValues;
...
void ExtractAndCastCellBufferData(const std::vector<char>& inbuf,
  svtkSmartPointer<svtkCellArray> output, int byteOffset, int byteStride, int count,
  int numberOfComponents, int mode = svtkGLTFDocumentLoaderInternals::GL_TRIANGLES)
{
  if (output == nullptr)
  {
    return;
  ...
  // Compute cell size
  svtkIdType cellSize = numberOfComponents;
  if (mode == svtkGLTFDocumentLoaderInternals::GL_LINE_STRIP ||
    mode == svtkGLTFDocumentLoaderInternals::GL_TRIANGLE_STRIP)
  {
    cellSize = count;
  }
  else if (mode == svtkGLTFDocumentLoaderInternals::GL_LINE_LOOP)
  {
    cellSize = count + 1;
  }

  // Preallocate cells
  svtkIdType nCells = GetNumberOfCellsForPrimitive(mode, numberOfComponents, count);
  output->AllocateEstimate(nCells, 1);

  std::vector<svtkIdType> currentCell(cellSize);
  ...
  if (mode == svtkGLTFDocumentLoaderInternals::GL_TRIANGLE_FAN)
  {
    ...
    for (auto it = accessorBegin; it != accessorEnd; it += step)
    {
      ...
      currentCell[i] = static_cast<svtkIdType>(val);
      ...
      {
        output->InsertNextCell(static_cast<svtkIdType>(currentCell.size()), currentCell.data());
  ...
  else
  {
    ...
    for (auto it = accessorBegin; it != accessorEnd; it += step)
    {
      ...
      *cellPosition = static_cast<svtkIdType>(val);
    ...
    if (mode == svtkGLTFDocumentLoaderInternals::GL_LINE_LOOP)
    {
...
//----------------------------------------------------------------------------
bool svtkGLTFDocumentLoader::ExtractPrimitiveAccessorData(Primitive& primitive)
{
  // Load connectivity
  if (primitive.IndicesId >= 0)
  {
    ...
    switch (accessor.ComponentTypeValue)
    {
      ...
      case ComponentType::UNSIGNED_INT:
        ...
        break;
      default:
        svtkErrorMacro("Invalid accessor.componentType for primitive connectivity. Expected either "
  ...
  {
    svtkErrorMacro("Error loading mesh.primitive.attributes");
    return false;
  }
  return true;
}
...
//----------------------------------------------------------------------------
bool svtkGLTFDocumentLoader::ExtractPrimitiveAttributes(Primitive& primitive)
{
  ...
  using AttributeArrayTypes =
    svtkTypeList::Create<svtkFloatArray, svtkIntArray, svtkUnsignedShortArray>;
  ...
  for (auto& attributePair : primitive.AttributeIndices)
  {
    ...
    if (attributePair.first == "JOINTS_0")
    {
      primitive.AttributeValues[attributePair.first] =
        svtkSmartPointer<svtkUnsignedShortArray>::New();
    }
    else
    {
      primitive.AttributeValues[attributePair.first] = svtkSmartPointer<svtkFloatArray>::New();
    }
    ...
    svtkArrayDispatch::DispatchByArray<AttributeArrayTypes>::Execute(
      primitive.AttributeValues[attributePair.first], worker);

    if (!worker.Result)
    {
      svtkErrorMacro("Error loading mesh.primitive attribute '" << attributePair.first << "'");
      return false;
    }
  }
  ...
  for (auto& target : primitive.Targets)
  {
    for (auto& attributePair : target.AttributeIndices)
    {
      ...
      Accessor accessor = this->InternalModel->Accessors[attributePair.second];
      target.AttributeValues[attributePair.first] = svtkSmartPointer<svtkFloatArray>::New();
      worker.Setup(attributePair.second, accessor.Type);
      svtkArrayDispatch::DispatchByArray<AttributeArrayTypes>::Execute(
...
//----------------------------------------------------------------------------
bool svtkGLTFDocumentLoader::LoadAnimationData()
{
  AccessorLoadingWorker worker;
  worker.Accessors = &(this->InternalModel->Accessors);
  worker.BufferViews = &(this->InternalModel->BufferViews);
  ...
  using AttributeArrayTypes = svtkTypeList::Create<svtkFloatArray>;
  ...
  for (Animation& animation : this->InternalModel->Animations)
  {
    ...
    {
      // Create arrays
      sampler.InputData = svtkSmartPointer<svtkFloatArray>::New();
      sampler.OutputData = svtkSmartPointer<svtkFloatArray>::New();

      // Load inputs (time stamps)
      worker.Setup(sampler.Input, AccessorType::SCALAR);
      svtkArrayDispatch::DispatchByArray<AttributeArrayTypes>::Execute(sampler.InputData, worker);
      if (!worker.Result)
      {
        svtkErrorMacro(
      ...
      maxDuration = svtkMath::Max(maxDuration, duration);

      // Load outputs (frame data)
      worker.Setup(sampler.Output, this->InternalModel->Accessors[sampler.Output].Type);
      svtkArrayDispatch::DispatchByArray<AttributeArrayTypes>::Execute(sampler.OutputData, worker);
      if (!worker.Result)
      {
        svtkErrorMacro(
      ...
      if (numberOfComponents == this->GetNumberOfComponentsForType(AccessorType::SCALAR))
      {
        ...
        if (nInput == 0 || nOutput % nInput != 0)
        {
          ...
          svtkErrorMacro("Invalid animation.sampler data. The number of outputs should be a "
...
//----------------------------------------------------------------------------
bool svtkGLTFDocumentLoader::LoadImageData()
{
  svtkNew<svtkImageReader2Factory> factory;

  for (Image& image : this->InternalModel->Images)
  {
    svtkSmartPointer<svtkImageReader2> reader = nullptr;
    image.ImageData = svtkSmartPointer<svtkImageData>::New();
      ...
      image.BufferView < static_cast<int>(this->InternalModel->BufferViews.size()))
    {
      ...
      {
        svtkErrorMacro("Invalid bufferView.buffer value for bufferView " << bufferView.Name);
        return false;
      }
      reader->SetMemoryBufferLength(
        static_cast<svtkIdType>(this->InternalModel->Buffers[bufferId].size()));
    ...
    {
      // Check for data-uri
      if (svtksys::SystemTools::StringStartsWith(image.Uri, "data:"))
      {
        svtkGLTFUtils::GetBinaryBufferFromUri(
          image.Uri, this->InternalModel->FileName, buffer, image.Uri.size());
        // If mime-type is defined, get appropriate reader here (only two possible values)
        std::string type = svtkGLTFUtils::GetDataUriMimeType(image.Uri);
        if (type == "image/jpeg")
        {
          reader = svtkSmartPointer<svtkJPEGReader>::New();
        }
        ...
        {
          svtkErrorMacro("Invalid MIME-Type for image");
          return false;
        }
        reader->SetMemoryBufferLength(static_cast<svtkIdType>(image.Uri.size()));
      ...
      else
      {
        std::string imageFilePath(
          svtkGLTFUtils::GetResourceFullPath(image.Uri, this->InternalModel->FileName));
        reader.TakeReference(factory->CreateImageReader2(imageFilePath.c_str()));
        if (reader == nullptr)
        {
          svtkErrorMacro("Invalid format for image " << image.Uri);
          return false;
        }
    ...
    if (reader == nullptr)
    {
      svtkErrorMacro("Invalid image object");
      return false;
    }
    reader->Update();
...
//----------------------------------------------------------------------------
bool svtkGLTFDocumentLoader::LoadSkinMatrixData()
{
  AccessorLoadingWorker worker;
  worker.Accessors = &(this->InternalModel->Accessors);
  worker.BufferViews = &(this->InternalModel->BufferViews);
  ...
  using AttributeArrayTypes = svtkTypeList::Create<svtkFloatArray, svtkIntArray>;
  ...
  for (Skin& skin : this->InternalModel->Skins)
  {
    if (skin.InverseBindMatricesAccessorId < 0)
    {
      ...
      svtkNew<svtkMatrix4x4> id;
      id->Identity();
      skin.InverseBindMatrices.push_back(id);
      continue;
    }
    svtkNew<svtkFloatArray> matrixValues;
    worker.Setup(skin.InverseBindMatricesAccessorId, AccessorType::MAT4);
    svtkArrayDispatch::DispatchByArray<AttributeArrayTypes>::Execute(matrixValues, worker);
    ...
    {
      svtkErrorMacro("Error loading skin.invertBindMatrices data");
      return false;
    }
...
//----------------------------------------------------------------------------
bool svtkGLTFDocumentLoader::LoadModelData(const std::vector<char>& glbBuffer)
{
  svtkGLTFDocumentLoaderInternals impl;
  impl.Self = this;
  ...
  if (this->InternalModel == nullptr)
  {
    svtkErrorMacro("Error loading model data: metadata was not loaded");
    return false;
  }
...
/** svtk object building and animation operations **/
//----------------------------------------------------------------------------
bool svtkGLTFDocumentLoader::ApplyAnimation(float t, int animationId, bool forceStep)
{
    ...
    switch (channel.TargetPath)
    {
      ...
      case Animation::Channel::PathType::WEIGHTS:
        ...
        if (numberOfComponents == 0)
        {
          ...
          {
            svtkErrorMacro("Invalid node.mesh value.");
            return false;
          }
...
//----------------------------------------------------------------------------
void svtkGLTFDocumentLoader::ResetAnimation(int animationId)
{
  const Animation& animation = this->InternalModel->Animations[animationId];
  for (const Animation::Channel& channel : animation.Channels)
  {
...
//----------------------------------------------------------------------------
bool svtkGLTFDocumentLoader::BuildPolyDataFromPrimitive(Primitive& primitive)
{
  // Positions
  primitive.Geometry = svtkSmartPointer<svtkPolyData>::New();
  if (primitive.AttributeValues.count("POSITION"))
  {
    primitive.Geometry->SetPoints(svtkSmartPointer<svtkPoints>::New());
  ...
  switch (primitive.Mode)
  {
    ...
    case svtkGLTFDocumentLoaderInternals::GL_TRIANGLE_STRIP:
      ...
      break;
    default:
      svtkWarningMacro("Invalid primitive draw mode. Ignoring connectivity.");
  }
...
//-----------------------------------------------------------------------------
void svtkGLTFDocumentLoader::Node::UpdateTransform()
{
...
//----------------------------------------------------------------------------
void svtkGLTFDocumentLoader::Animation::Sampler::GetInterpolatedData(float t,
  size_t numberOfComponents, std::vector<float>* output, bool forceStep, bool isRotation) const
{
  ...
  if (this->Interpolation != Animation::Sampler::InterpolationMode::STEP && !forceStep)
  {
    ...
    if (this->Interpolation == Animation::Sampler::InterpolationMode::LINEAR)
    {
      ...
      if (isRotation)
      {
        ...
        svtkQuaternion<float> prevQuaternion(prevTuple.data());
        svtkQuaternion<float> nextQuaternion(nextTuple.data());
    ...
    else
    {
      ...
      if (isRotation)
      {
        ...
        svtkQuaternion<float> quaternion(output->data());
  ...
  else
  {
    ...
    {
      output->push_back(this->OutputData->GetValue(static_cast<svtkIdType>(i)));
    }
  }
}
...
/** File operations **/
//----------------------------------------------------------------------------
bool svtkGLTFDocumentLoader::LoadFileBuffer(
  const std::string& fileName, std::vector<char>& glbBuffer)
{
  ...
  {
    svtkErrorMacro("Invalid .glb file " << fileName);
    return false;
  }
  ...
  if (!fin.is_open())
  {
    svtkErrorMacro("Error opening file " << fileName);
    return false;
  }
  ...
  for (auto& chunk : chunkInfo)
  {
    ...
    // Jump to next chunk
    fin.seekg(chunk.second + svtkGLTFUtils::GLBChunkHeaderSize, std::ios::cur);
  }
  svtkErrorMacro("Could not find any valid BIN chunks in file " << fileName);
  return false;
}

//----------------------------------------------------------------------------
bool svtkGLTFDocumentLoader::BuildModelSVTKGeometry()
{
  if (this->InternalModel == nullptr)
  {
    svtkErrorMacro("Error building model data: metadata was not loaded");
    return false;
  }
...
//----------------------------------------------------------------------------
void svtkGLTFDocumentLoader::BuildGlobalTransforms(
  unsigned int nodeIndex, svtkSmartPointer<svtkTransform> parentTransform)
{
  ...
  node.GlobalTransform = svtkSmartPointer<svtkTransform>::New();
...
//----------------------------------------------------------------------------
void svtkGLTFDocumentLoader::PrintSelf(ostream& os, svtkIndent indent)
{
  this->Superclass::PrintSelf(os, indent);
}

//----------------------------------------------------------------------------
std::shared_ptr<svtkGLTFDocumentLoader::Model> svtkGLTFDocumentLoader::GetInternalModel()
{
  return std::shared_ptr<Model>(this->InternalModel);
}

//----------------------------------------------------------------------------
const std::vector<std::string>& svtkGLTFDocumentLoader::GetSupportedExtensions()
{
  return svtkGLTFDocumentLoader::SupportedExtensions;
}

//----------------------------------------------------------------------------
const std::vector<std::string>& svtkGLTFDocumentLoader::GetUsedExtensions()
{
...
/** types and enums **/
//----------------------------------------------------------------------------
unsigned int svtkGLTFDocumentLoader::GetNumberOfComponentsForType(
  svtkGLTFDocumentLoader::AccessorType type)
{
  switch (type)
  {
