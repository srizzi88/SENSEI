 ...
 *
 *
 * svtkGLTFDocument loader is an internal utility class which defines data structures and functions
 * with the purpose of deserializing a glTF model from a glTF file, loading its data from binary
 * buffers and creating svtk objects with the extracted geometry.
...
#ifndef svtkGLTFDocumentLoader_h
#define svtkGLTFDocumentLoader_h
...
class SVTKIOGEOMETRY_EXPORT svtkGLTFDocumentLoader : public svtkObject
{
public:
  static svtkGLTFDocumentLoader* New();
  svtkTypeMacro(svtkGLTFDocumentLoader, svtkObject);
  ...
  struct MorphTarget
  {
    ...
    // attribute values
    std::map<std::string, svtkSmartPointer<svtkFloatArray> > AttributeValues;
  };
   ...
   * svtk data structures and finally used to build a svtkPolyData object.
   */
  struct Primitive
  {
    // accessor indices from the .glTF file, the map's keys correspond to attribute names
    std::map<std::string, int> AttributeIndices;
    int IndicesId;
    svtkSmartPointer<svtkCellArray> Indices;

    // attribute values from buffer data
    std::map<std::string, svtkSmartPointer<svtkDataArray> > AttributeValues;

    svtkSmartPointer<svtkPolyData> Geometry;
   ...
   * Nodes can contain transform properties (stored as svtkTransform objects) as well as indices to
  ...
  struct Node
  {
    ...
    svtkSmartPointer<svtkMatrix4x4> Matrix;
  ...
  struct Skin
  {
    std::vector<svtkSmartPointer<svtkMatrix4x4> > InverseBindMatrices;
  ...
  struct Animation
  {
    struct Sampler
    {
      ...
      svtkSmartPointer<svtkFloatArray> InputData;
      svtkSmartPointer<svtkFloatArray> OutputData;
  ...
  /**
   * Converts the internal Model's loaded data into more convenient svtk objects.
   */
  bool BuildModelSVTKGeometry();
   ...
   */
  static unsigned int GetNumberOfComponentsForType(svtkGLTFDocumentLoader::AccessorType type);
...
protected:
  svtkGLTFDocumentLoader() = default;
  ~svtkGLTFDocumentLoader() override = default;
...
private:
  ...
  svtkGLTFDocumentLoader(const svtkGLTFDocumentLoader&) = delete;
  void operator=(const svtkGLTFDocumentLoader&) = delete;
   ...
   * into the Primitive's corresponding svtk arrays.
   */
   ...
   * binary buffers. These arrays are then stored in the primitive's different svtk arrays.
  ...
  /**
   * Creates and populates the Primitive's geometry svtkPolyData member with all the vertex attribute
  ...
  /**
   * Load Model's Images into svtkImageData objects, from filesystem and bufferView when specified.
   */
  bool LoadImageData();
   ...
   * Recursive.
   */
  void BuildGlobalTransforms(unsigned int nodeIndex, svtkSmartPointer<svtkTransform> parentTransform);
