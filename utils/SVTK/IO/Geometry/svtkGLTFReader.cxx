...
void AddIntegerToFieldData(
  std::string arrayName, int value, svtkSmartPointer<svtkFieldData> fieldData)
{
...
void AddFloatToFieldData(
  std::string arrayName, float value, svtkSmartPointer<svtkFieldData> fieldData)
{
...
void AddVecNfToFieldData(const std::string& arrayName, const std::vector<float>& multiplier,
  svtkSmartPointer<svtkFieldData> fieldData)
{
  svtkNew<svtkFloatArray> array;
  array->SetName(arrayName.c_str());
...
void AddTextureInfoToFieldData(const std::string& prefix, int textureIndex, int textureCoordIndex,
  svtkSmartPointer<svtkFieldData> fieldData, std::vector<float> multiplier = std::vector<float>())
{
...
//----------------------------------------------------------------------------
void AddMaterialToFieldData(int materialId, svtkSmartPointer<svtkFieldData> fieldData,
  const svtkGLTFDocumentLoader::Model& model)
{
  ...
  if (materialId >= 0 && materialId < static_cast<int>(model.Materials.size()))
  {
    ...
    if (material.AlphaMode == svtkGLTFDocumentLoader::Material::AlphaModeType::MASK)
    {
      AddFloatToFieldData("AlphaCutoff", material.AlphaCutoff, fieldData);
    }
    else if (material.AlphaMode == svtkGLTFDocumentLoader::Material::AlphaModeType::OPAQUE)
    {
...
//----------------------------------------------------------------------------
svtkSmartPointer<svtkDataArray> ApplyMorphingToDataArray(svtkSmartPointer<svtkDataArray> origin,
  const std::vector<float>& weights, const std::vector<svtkSmartPointer<svtkFloatArray> >& targets)
{
  if (origin == nullptr)
  {
    return nullptr;
  ...
  svtkSmartPointer<svtkDataArray> result = svtkSmartPointer<svtkDataArray>::Take(origin->NewInstance());
...
//----------------------------------------------------------------------------
void SetupWeightedTransformFilterForGLTFSkinning(svtkSmartPointer<svtkWeightedTransformFilter> filter,
  const std::vector<svtkSmartPointer<svtkTransform> >& jointMats,
  const svtkSmartPointer<svtkPolyData> poly)
{
  ...
  size_t nbTransforms = svtkMath::Max<size_t>(jointMats.size(), 4);
...
//----------------------------------------------------------------------------
void AddTransformToFieldData(const svtkSmartPointer<svtkTransform> transform,
  svtkSmartPointer<svtkFieldData> fieldData, const std::string& name)
{
  svtkSmartPointer<svtkDoubleArray> matrixArray;
  if (fieldData->HasArray(name.c_str()))
  {
    matrixArray = svtkDoubleArray::SafeDownCast(fieldData->GetArray(name.c_str()));
    matrixArray->Resize(0);
  }
  else
  {
    matrixArray = svtkSmartPointer<svtkDoubleArray>::New();
...
//----------------------------------------------------------------------------
void AddJointMatricesToFieldData(const std::vector<svtkSmartPointer<svtkTransform> >& jointMats,
  svtkSmartPointer<svtkFieldData> fieldData)
{
...
void AddGlobalTransformToFieldData(
  const svtkSmartPointer<svtkTransform> globalTransform, svtkSmartPointer<svtkFieldData> fieldData)
{
  // Create array to store the matrix's values
...
void AddMorphingWeightsToFieldData(
  const std::vector<float>& weights, svtkSmartPointer<svtkFieldData> fieldData)
{
  svtkNew<svtkFloatArray> weightsArray;
  weightsArray->SetName("morphingWeights");
  weightsArray->SetNumberOfValues(static_cast<svtkIdType>(weights.size()));
...
void AddInfoToFieldData(const std::vector<float>* morphingWeights,
  const std::vector<svtkSmartPointer<svtkTransform> >& jointMats,
  svtkSmartPointer<svtkTransform> globalTransform, svtkSmartPointer<svtkFieldData> fieldData)
{
...
//----------------------------------------------------------------------------
void PrepareMorphingTargetArrays(std::vector<svtkSmartPointer<svtkFloatArray> >& positionArrays,
  std::vector<svtkSmartPointer<svtkFloatArray> >& normalArrays,
  std::vector<svtkSmartPointer<svtkFloatArray> >& tangentArrays,
  std::vector<svtkGLTFDocumentLoader::MorphTarget>& targets)
{
...
//----------------------------------------------------------------------------
void ApplyMorphingToPolyData(std::vector<svtkGLTFDocumentLoader::MorphTarget>& targets,
  std::vector<float>* morphingWeights, svtkSmartPointer<svtkPolyData> inputPolyData,
  svtkSmartPointer<svtkPolyData> outputPolyData)
{
  // Prepare target arrays
  std::vector<svtkSmartPointer<svtkFloatArray> > positionArrays;
  std::vector<svtkSmartPointer<svtkFloatArray> > normalArrays;
  std::vector<svtkSmartPointer<svtkFloatArray> > tangentArrays;
  ...
  if (points != nullptr)
  {
    outputPolyData->SetPoints(svtkSmartPointer<svtkPoints>::New());
...
//----------------------------------------------------------------------------
bool BuildMultiBlockDatasetFromMesh(svtkGLTFDocumentLoader::Model& m, unsigned int meshId,
  svtkSmartPointer<svtkMultiBlockDataSet> parentDataSet,
  svtkSmartPointer<svtkMultiBlockDataSet> meshDataSet, std::string& dataSetName,
  svtkSmartPointer<svtkTransform> globalTransform,
  const std::vector<svtkSmartPointer<svtkTransform> >& jointMats, bool applyDeformations,
  std::vector<float>* morphingWeights)
{
  if (meshId >= m.Meshes.size())
  {
    svtkErrorWithObjectMacro(nullptr, "Invalid mesh index " << meshId);
    return false;
  }
  ...
  if (!meshDataSet && !createNewPolyData)
  {
    ...
    meshDataSet = svtkSmartPointer<svtkMultiBlockDataSet>::New();
    ...
    parentDataSet->GetMetaData(parentDataSet->GetNumberOfBlocks() - 1)
      ->Set(svtkCompositeDataSet::NAME(), dataSetName);
  }

  svtkGLTFDocumentLoader::Mesh mesh = m.Meshes[meshId];

  int blockId = 0;
  for (auto& primitive : mesh.Primitives)
  {
    svtkSmartPointer<svtkPolyData> meshPolyData;
    ...
    {
      meshPolyData = svtkSmartPointer<svtkPolyData>::New();
      ...
      svtkNew<svtkTransformPolyDataFilter> filter;
      ...
      if (morphingWeights != nullptr && !morphingWeights->empty())
      {
        ...
        {
          svtkErrorWithObjectMacro(nullptr, "Invalid number of morphing weights");
          return false;
        }
      ...
      {
        // Setup filter
        svtkNew<svtkWeightedTransformFilter> skinningFilter;
      ...
      {
        filter->SetOutput(svtkPolyData::SafeDownCast(meshDataSet->GetBlock(blockId)));
      }
      filter->Update();
    }
    else
    {
      ...
      {
        meshPolyData = svtkPolyData::SafeDownCast(meshDataSet->GetBlock(blockId));
      }
    }
    AddInfoToFieldData(morphingWeights, jointMats, globalTransform,
      svtkPolyData::SafeDownCast(meshDataSet->GetBlock(blockId))->GetFieldData());
  }
  return true;
}

//----------------------------------------------------------------------------
void ComputeJointMatrices(const svtkGLTFDocumentLoader::Model& m,
  const svtkGLTFDocumentLoader::Skin& skin, svtkGLTFDocumentLoader::Node& node,
  std::vector<svtkSmartPointer<svtkTransform> >& jointMats)
{
  jointMats.clear();
  ...
  {
    const svtkGLTFDocumentLoader::Node& jointNode = m.Nodes[skin.Joints[jointId]];

    svtkNew<svtkTransform> jointGlobalTransform;
    svtkNew<svtkTransform> inverseMeshGlobalTransform;
    svtkNew<svtkTransform> inverseBindTransform;
    svtkNew<svtkTransform> jointTransform;
     ...
     * The mesh will be transformed (using svtkWeightedTransformFilter) using this matrix:
     * mat4 skinMat =
...
//----------------------------------------------------------------------------
bool BuildMultiBlockDataSetFromNode(svtkGLTFDocumentLoader::Model& m, unsigned int nodeId,
  svtkSmartPointer<svtkMultiBlockDataSet> parentDataSet,
  svtkSmartPointer<svtkMultiBlockDataSet> nodeDataset, std::string nodeName, bool applyDeformations)
{
  if (nodeId >= m.Nodes.size())
  {
    svtkErrorWithObjectMacro(nullptr, "Invalid node index " << nodeId);
    return false;
  }
  ...
  if (!nodeDataset)
  {
    ...
    nodeDataset = svtkSmartPointer<svtkMultiBlockDataSet>::New();
    parentDataSet->SetBlock(parentDataSet->GetNumberOfBlocks(), nodeDataset);
    parentDataSet->GetMetaData(parentDataSet->GetNumberOfBlocks() - 1)
      ->Set(svtkCompositeDataSet::NAME(), nodeName);
  }

  svtkGLTFDocumentLoader::Node node = m.Nodes[nodeId];
  int blockId = 0;
  if (node.Mesh >= 0)
  {
    std::vector<svtkSmartPointer<svtkTransform> > jointMats;
    ...
    if (node.Skin >= 0)
    {
      ...
      const svtkGLTFDocumentLoader::Skin& skin = m.Skins[node.Skin];
    ...
    svtkSmartPointer<svtkMultiBlockDataSet> meshDataSet = nullptr;
    if (!createNewBlocks)
    {
      meshDataSet = svtkMultiBlockDataSet::SafeDownCast(nodeDataset->GetBlock(blockId));
    }
          ...
          node.GlobalTransform, jointMats, applyDeformations, morphingWeights))
    {
      svtkErrorWithObjectMacro(
        nullptr, "Could not build svtkMultiBlockDataSet from mesh " << node.Mesh);
      return false;
    }
    blockId++;
  }
  for (int child : node.Children)
  {
    ...
    svtkSmartPointer<svtkMultiBlockDataSet> childDataset;
    std::string childDatasetName = "Node_" + value_to_string(child);
    if (!createNewBlocks)
    {
      // find existing child dataset for this node
      childDataset = svtkMultiBlockDataSet::SafeDownCast(nodeDataset->GetBlock(blockId));
    }
    if (!BuildMultiBlockDataSetFromNode(
          m, child, nodeDataset, childDataset, childDatasetName, applyDeformations))
    {
      svtkErrorWithObjectMacro(nullptr, "Could not build svtkMultiBlockDataSet from node " << child);
...
//----------------------------------------------------------------------------
bool BuildMultiBlockDataSetFromScene(svtkGLTFDocumentLoader::Model& m, svtkIdType sceneId,
  svtkSmartPointer<svtkMultiBlockDataSet> dataSet, bool applyDeformations)
{
  if (sceneId < 0 || sceneId >= static_cast<svtkIdType>(m.Scenes.size()))
  {
    svtkErrorWithObjectMacro(nullptr, "Invalid scene index " << sceneId);
    return false;
  }

  svtkGLTFDocumentLoader::Scene scene = m.Scenes[sceneId];
  ...
  for (int node : scene.Nodes)
  {
    ...
    svtkSmartPointer<svtkMultiBlockDataSet> nodeDataset = nullptr;
    if (!createNewBlocks)
    {
      // find existing child dataset for this node
      nodeDataset = svtkMultiBlockDataSet::SafeDownCast(dataSet->GetBlock(blockId));
    }
    if (!BuildMultiBlockDataSetFromNode(
          m, node, dataSet, nodeDataset, nodeDatasetName, applyDeformations))
    {
      svtkErrorWithObjectMacro(nullptr, "Could not build svtkMultiBlockDataSet from node " << node);
      return false;
    }
    blockId++;
  }
...
//----------------------------------------------------------------------------
svtkStandardNewMacro(svtkGLTFReader);

//----------------------------------------------------------------------------
svtkGLTFReader::svtkGLTFReader()
{
...
//----------------------------------------------------------------------------
void svtkGLTFReader::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
void svtkGLTFReader::StoreTextureData()
{
  ...
  {
    svtkGLTFReader::GLTFTexture readerTexture;
    ...
    {
      svtkWarningMacro("Image index is out of range");
      continue;
    }
...
//----------------------------------------------------------------------------
int svtkGLTFReader::RequestInformation(
  svtkInformation* request, svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  {
    svtksys::SystemTools::ConvertToUnixSlashes(fileNameAsString);
  }

  if (!svtksys::SystemTools::FileIsFullPath(fileNameAsString))
  {
    fileNameAsString = svtksys::SystemTools::CollapseFullPath(fileNameAsString);
  }
  ...
  if (!this->IsMetaDataLoaded)
  {
    ...
    {
      svtkErrorMacro("Error loading model metadata from file " << this->FileName);
      return 0;
    }
  ...
  // Add this info to the output svtkInformation
  auto model = this->Loader->GetInternalModel();
  ...
  {
    for (svtkIdType i = 0; i < this->AnimationSelection->GetNumberOfArrays(); i++)
    {
  ...
  {
    int maxFrameIndex = svtkMath::Floor(this->GetFrameRate() * maxDuration);
    if (info->Has(svtkStreamingDemandDrivenPipeline::TIME_STEPS()))
    {
      info->Remove(svtkStreamingDemandDrivenPipeline::TIME_STEPS());
    }
    double period = 1.0 / this->GetFrameRate();
    // Append sampled time steps
    for (int i = 0; i <= maxFrameIndex; i++)
    {
      info->Append(svtkStreamingDemandDrivenPipeline::TIME_STEPS(), i * period);
    }
    // Append the last step of the animation, if it doesn't match with the last sampled step
    if (maxDuration != maxFrameIndex * period)
    {
      info->Append(svtkStreamingDemandDrivenPipeline::TIME_STEPS(), maxDuration);
    }

    // Add TIME_RANGE()
    std::array<double, 2> timeRange = { { 0.0, maxDuration } };
    info->Set(svtkStreamingDemandDrivenPipeline::TIME_RANGE(), timeRange.data(), 2);
  }
  else if (info->Has(svtkStreamingDemandDrivenPipeline::TIME_STEPS()))
  {
    info->Remove(svtkStreamingDemandDrivenPipeline::TIME_STEPS());
  }

  this->NumberOfAnimations = static_cast<svtkIdType>(model->Animations.size());
  this->NumberOfScenes = static_cast<svtkIdType>(model->Scenes.size());
  return 1;
}

//----------------------------------------------------------------------------
int svtkGLTFReader::RequestData(
  svtkInformation*, svtkInformationVector**, svtkInformationVector* outputVector)
{
  ...
  {
    this->OutputDataSet = svtkSmartPointer<svtkMultiBlockDataSet>::New();
  }
  ...
  if (this->FrameRate > 0)
  {
    double time = info->Get(svtkStreamingDemandDrivenPipeline::UPDATE_TIME_STEP());
    for (svtkIdType i = 0; i < this->NumberOfAnimations; i++)
    {
  ...
  svtkIdType selectedScene = this->CurrentScene;
  if (selectedScene < 0 || selectedScene >= static_cast<svtkIdType>(model->Scenes.size()))
  {
  ...
  {
    svtkErrorMacro("Error building MultiBlockDataSet object");
    return 0;
  }
...
//----------------------------------------------------------------------------
void svtkGLTFReader::EnableAnimation(svtkIdType animationIndex)
{
  if (this->AnimationSelection == nullptr)
  {
    svtkErrorMacro("Error accessing animations: model is not loaded yet");
    return;
  }
  if (animationIndex < 0 || animationIndex >= this->AnimationSelection->GetNumberOfArrays())
  {
    svtkErrorMacro("Out of range animation index");
    return;
  }
...
//----------------------------------------------------------------------------
void svtkGLTFReader::DisableAnimation(svtkIdType animationIndex)
{
  if (this->AnimationSelection == nullptr)
  {
    svtkErrorMacro("Error accessing animations: model is not loaded yet");
...
//----------------------------------------------------------------------------
bool svtkGLTFReader::IsAnimationEnabled(svtkIdType animationIndex)
{
  if (this->AnimationSelection == nullptr)
  {
    svtkErrorMacro("Error accessing animations: model is not loaded yet");
...
//----------------------------------------------------------------------------
std::string svtkGLTFReader::GetAnimationName(svtkIdType animationIndex)
{
  if (this->Loader == nullptr || this->Loader->GetInternalModel() == nullptr)
  {
    svtkErrorMacro("Error while accessing animations: model is not loaded");
    return "";
  }
  if (animationIndex < 0 ||
    animationIndex >= static_cast<svtkIdType>(this->Loader->GetInternalModel()->Animations.size()))
  {
    svtkErrorMacro("Out of range animation index");
    return "";
  }
...
//----------------------------------------------------------------------------
float svtkGLTFReader::GetAnimationDuration(svtkIdType animationIndex)
{
  if (this->Loader == nullptr || this->Loader->GetInternalModel() == nullptr)
  {
    svtkErrorMacro("Error while accessing animations: model is not loaded");
...
//----------------------------------------------------------------------------
std::string svtkGLTFReader::GetSceneName(svtkIdType sceneIndex)
{
  if (this->Loader == nullptr || this->Loader->GetInternalModel() == nullptr)
  {
    svtkErrorMacro("Error while accessing scenes: model is not loaded");
    return "";
  }
  if (sceneIndex < 0 ||
    sceneIndex >= static_cast<svtkIdType>(this->Loader->GetInternalModel()->Scenes.size()))
  {
    svtkErrorMacro("Out of range scene index");
    return "";
  }
...
//----------------------------------------------------------------------------
svtkIdType svtkGLTFReader::GetNumberOfTextures()
{
  return static_cast<svtkIdType>(this->Textures.size());
}

//----------------------------------------------------------------------------
svtkGLTFReader::GLTFTexture svtkGLTFReader::GetGLTFTexture(svtkIdType textureIndex)
{
  if (textureIndex < 0 || textureIndex >= static_cast<svtkIdType>(this->Textures.size()))
  {
    svtkErrorMacro("Out of range texture index");
    return svtkGLTFReader::GLTFTexture{ nullptr, 0, 0, 0, 0 };
  }
  return this->Textures[textureIndex];
}

//----------------------------------------------------------------------------
void svtkGLTFReader::SetScene(const std::string& scene)
{
  ...
  for (svtkIdType i = 0; i < this->SceneNames->GetNumberOfValues(); i++)
  {
  ...
  svtkWarningMacro("Scene '" << scene << "' does not exist.");
}

//----------------------------------------------------------------------------
void svtkGLTFReader::CreateSceneNamesArray()
{
  if (this->Loader == nullptr || this->Loader->GetInternalModel() == nullptr)
  {
    svtkErrorMacro("Error while accessing scenes: model is not loaded");
  ...
  this->SceneNames = svtkSmartPointer<svtkStringArray>::New();
...
//----------------------------------------------------------------------------
svtkStringArray* svtkGLTFReader::GetAllSceneNames()
{
  if (this->Loader == nullptr || this->Loader->GetInternalModel() == nullptr)
  {
    svtkErrorMacro("Error while accessing scenes: model is not loaded");
...
//----------------------------------------------------------------------------
svtkDataArraySelection* svtkGLTFReader::GetAnimationSelection()
{
  return this->AnimationSelection;
}

//----------------------------------------------------------------------------
void svtkGLTFReader::CreateAnimationSelection()
{
  if (this->Loader == nullptr || this->Loader->GetInternalModel() == nullptr)
  {
    svtkErrorMacro("Error while accessing animations: model is not loaded");
  ...
  this->AnimationSelection = svtkSmartPointer<svtkDataArraySelection>::New();
  std::map<std::string, unsigned int> duplicateNameCounters;
  ...
  this->PreviousAnimationSelection = svtkSmartPointer<svtkDataArraySelection>::New();
  this->PreviousAnimationSelection->CopySelections(this->AnimationSelection);
  this->AnimationSelection->AddObserver(svtkCommand::ModifiedEvent, this, &svtkGLTFReader::Modified);
}

//---------------------------------------------------------------------------
void svtkGLTFReader::SetApplyDeformationsToGeometry(bool flag)
{
