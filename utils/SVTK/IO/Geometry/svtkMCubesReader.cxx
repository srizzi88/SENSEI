...
svtkStandardNewMacro(svtkMCubesReader);

// Construct object with FlipNormals turned off and Normals set to true.
svtkMCubesReader::svtkMCubesReader()
{
...
int svtkMCubesReader::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** svtkNotUsed(inputVector), svtkInformationVector* outputVector)
{
  ...
  svtkDebugMacro(<< "Reading marching cubes file");
  ...
  {
    svtkErrorMacro(<< "Please specify input FileName");
    return 0;
  }
  ...
  if (this->LimitsFileName != nullptr &&
    (limitp = svtksys::SystemTools::Fopen(this->LimitsFileName, "rb")) != nullptr &&
    stat(this->FileName, &buf) == 0)
  {
    ...
    if (errorOccurred)
    {
      svtkErrorMacro("MCubesReader error reading file: " << this->LimitsFileName
    ...
    {
      svtkByteSwap::Swap4BERange(fbounds, 6);
    }
    else
    {
      svtkByteSwap::Swap4LERange(fbounds, 6);
    }
    fclose(limitp);
    bounds[0] = fbounds[0];
    bounds[1] = fbounds[1];
  ...
  else // read data to get bounds
  {
    ...
    // cannot use svtkMath uninitialize bounds for this computation
    ...
    for (i = 0; fread(&point, sizeof(pointType), 1, fp); i++)
    {
      ...
      if (byteOrder == SVTK_FILE_BYTE_ORDER_BIG_ENDIAN)
      {
        svtkByteSwap::Swap4BERange((float*)(&point), 6);
      }
      else
      {
        svtkByteSwap::Swap4LERange((float*)(&point), 6);
      }
      for (j = 0; j < 3; j++)
      {
      ...
      if (i && ((i % 10000) == 0))
      {
        svtkDebugMacro(<< "Triangle vertices #" << i);
      }
    }
    numTris = i / 3;
    numPts = i;
  ...
  for (i = 0; i < numTris; i++)
  {
    for (j = 0; j < 3; j++)
    {
      ...
      if (val != 1)
      {
        svtkErrorMacro(<< "Error reading triangle " << i << " (" << numTris << "), point/normal "
                      << j);
      }
...
void svtkMCubesReader::SetLocator(svtkIncrementalPointLocator* locator)
{
...
void svtkMCubesReader::SetDataByteOrderToBigEndian()
{
#ifndef SVTK_WORDS_BIGENDIAN
  this->SwapBytesOn();
#else
...
void svtkMCubesReader::SetDataByteOrderToLittleEndian()
{
...
void svtkMCubesReader::SetDataByteOrder(int byteOrder)
{
  if (byteOrder == SVTK_FILE_BYTE_ORDER_BIG_ENDIAN)
  {
...
int svtkMCubesReader::GetDataByteOrder()
{
#ifdef SVTK_WORDS_BIGENDIAN
  if (this->SwapBytes)
  {
...
const char* svtkMCubesReader::GetDataByteOrderAsString()
{
#ifdef SVTK_WORDS_BIGENDIAN
  if (this->SwapBytes)
  {
...
void svtkMCubesReader::CreateDefaultLocator()
{
  if (this->Locator == nullptr)
  {
...
void svtkMCubesReader::PrintSelf(ostream& os, svtkIndent indent)
{
...
svtkMTimeType svtkMCubesReader::GetMTime()
{
