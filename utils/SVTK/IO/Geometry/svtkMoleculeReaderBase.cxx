/*=========================================================================
  ...
  Module:    svtkMoleculeReaderBase.cxx
...
static double svtkMoleculeReaderBaseCovRadius[103] = { 0.32, 1.6, 0.68, 0.352, 0.832, 0.72, 0.68,
...
static double svtkMoleculeReaderBaseAtomColors[][3] = { { 255, 255, 255 }, { 127, 0, 127 },
...
static double svtkMoleculeReaderBaseRadius[] = {
...
svtkMoleculeReaderBase::svtkMoleculeReaderBase()
{
...
int svtkMoleculeReaderBase::FillOutputPortInformation(int port, svtkInformation* info)
{
  if (port == 1)
  {
...
int svtkMoleculeReaderBase::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** svtkNotUsed(inputVector), svtkInformationVector* outputVector)
{
  ...
  svtkInformation* outMolInfo = outputVector->GetInformationObject(1);
  if (outMolInfo)
  {
    this->Molecule = svtkMolecule::SafeDownCast(outMolInfo->Get(svtkDataObject::DATA_OBJECT()));
  }
  ...
  svtkDebugMacro(<< "opening base file " << this->FileName);
...
int svtkMoleculeReaderBase::ReadMolecule(FILE* fp, svtkPolyData* output)
{
  int i;
  svtkCellArray* newBonds;

  svtkDebugMacro(<< "Scanning the Molecule file");

  if (!this->AtomType)
  {
    this->AtomType = svtkIdTypeArray::New();
  }
  else
  {
    this->AtomType->Reset();
  ...
  if (!this->AtomTypeStrings)
  {
    this->AtomTypeStrings = svtkStringArray::New();
  }
  else
  {
  ...
  if (!this->Residue)
  {
    this->Residue = svtkIdTypeArray::New();
  }
  else
  {
    this->Residue->Reset();
  ...
  if (!this->Chain)
  {
    this->Chain = svtkUnsignedCharArray::New();
  }
  else
  {
    this->Chain->Reset();
  ...
  if (!this->SecondaryStructures)
  {
    this->SecondaryStructures = svtkUnsignedCharArray::New();
  }
  else
  {
  ...
  if (!this->SecondaryStructuresBegin)
  {
    this->SecondaryStructuresBegin = svtkUnsignedCharArray::New();
  }
  else
  {
  ...
  if (!this->SecondaryStructuresEnd)
  {
    this->SecondaryStructuresEnd = svtkUnsignedCharArray::New();
  }
  else
  {
  ...
  if (!this->IsHetatm)
  {
    this->IsHetatm = svtkUnsignedCharArray::New();
  }
  else
  {
    this->IsHetatm->Reset();
  ...
  svtkDebugMacro(<< "End of scanning");
  output->SetPoints(this->Points);

  newBonds = svtkCellArray::New();
  ...
  svtkDebugMacro(<< "read " << this->NumberOfAtoms << " atoms and found "
  ...
  {
    this->RGB = svtkUnsignedCharArray::New();
  }
  ...
  {
    this->RGB->InsertNextTuple(&svtkMoleculeReaderBaseAtomColors[AtomType->GetValue(i)][0]);
  }
  ...
  for (i = 0; i < this->NumberOfAtoms; i++)
  {
    this->Radii->InsertNextTuple3(svtkMoleculeReaderBaseRadius[AtomType->GetValue(i)],
      svtkMoleculeReaderBaseRadius[AtomType->GetValue(i)],
      svtkMoleculeReaderBaseRadius[AtomType->GetValue(i)]);
  }
...
int svtkMoleculeReaderBase::MakeBonds(
  svtkPoints* newPts, svtkIdTypeArray* atype, svtkCellArray* newBonds)
{
  int i, j, k;
  int nbonds;
  double dx, dy, dz;
  ...
  for (i = this->NumberOfAtoms - 1; i > 0; i--)
  {
     ...
     */
    radius =
      (svtkMoleculeReaderBaseCovRadius[atype->GetValue(i)] + 2.0 + 0.56) * std::max(BScale, HBScale);
    ...
    for (k = result->GetNumberOfIds() - 1; k >= 0; k--)
    {
      ...
      dist = svtkMoleculeReaderBaseCovRadius[atype->GetValue(i)] +
        svtkMoleculeReaderBaseCovRadius[atype->GetValue(j)] + 0.56;
      max = dist * dist;
...
int svtkMoleculeReaderBase::MakeAtomType(const char* atype)
{
  int anum = 0;
...
void svtkMoleculeReaderBase::PrintSelf(ostream& os, svtkIndent indent)
{
