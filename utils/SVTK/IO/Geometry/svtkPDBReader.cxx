...
svtkStandardNewMacro(svtkPDBReader);

svtkPDBReader::svtkPDBReader() = default;

svtkPDBReader::~svtkPDBReader() = default;

void svtkPDBReader::ReadSpecificMolecule(FILE* fp)
{
  ...
  svtkIntArray* Sheets = svtkIntArray::New();
  Sheets->SetNumberOfComponents(4);
  Sheets->Allocate(500);

  svtkIntArray* Helix = svtkIntArray::New();
  Helix->SetNumberOfComponents(4);
  Helix->Allocate(50);

  svtkDebugMacro(<< "PDB File (" << this->HBScale << ", " << this->BScale << ")");
...
void svtkPDBReader::PrintSelf(ostream& os, svtkIndent indent)
{
