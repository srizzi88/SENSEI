...
svtkStandardNewMacro(svtkTecplotReader);
...
bool FileStreamReader::open(const char* fileName)
{
  if (!this->Open)
  {
    ...
    FILE* ff = svtksys::SystemTools::Fopen(fileName, "rb");
...
{
public:
  svtkTecplotReaderInternal() { this->Init(); }
  ~svtkTecplotReaderInternal() { this->Init(); }
...
private:
  svtkTecplotReaderInternal(const svtkTecplotReaderInternal&) = delete;
  void operator=(const svtkTecplotReaderInternal&) = delete;
};
// ==========================================================================//
...
// ----------------------------------------------------------------------------
svtkTecplotReader::svtkTecplotReader()
{
  this->SelectionObserver = svtkCallbackCommand::New();
  this->SelectionObserver->SetClientData(this);
  this->SelectionObserver->SetCallback(&svtkTecplotReader::SelectionModifiedCallback);

  this->DataArraySelection = svtkDataArraySelection::New();
  this->DataArraySelection->AddObserver(svtkCommand::ModifiedEvent, this->SelectionObserver);

  this->FileName = nullptr;
  this->Internal = new svtkTecplotReaderInternal;
...
// ----------------------------------------------------------------------------
void svtkTecplotReader::SetFileName(const char* fileName)
{
...
//----------------------------------------------------------------------------
void svtkTecplotReader::SelectionModifiedCallback(svtkObject*, unsigned long, void* tpReader, void*)
{
  static_cast<svtkTecplotReader*>(tpReader)->Modified();
}

// ----------------------------------------------------------------------------
int svtkTecplotReader::FillOutputPortInformation(int svtkNotUsed(port), svtkInformation* info)
{
...
// ----------------------------------------------------------------------------
int svtkTecplotReader::RequestInformation(
...
// ----------------------------------------------------------------------------
int svtkTecplotReader::RequestData(svtkInformation* svtkNotUsed(request),
...
// ----------------------------------------------------------------------------
const char* svtkTecplotReader::GetDataTitle()
{
  return this->DataTitle.c_str();
}

// ----------------------------------------------------------------------------
int svtkTecplotReader::GetNumberOfBlocks()
{
  return int(this->ZoneNames.size());
}

// ----------------------------------------------------------------------------
const char* svtkTecplotReader::GetBlockName(int blockIdx)
{
...
// ----------------------------------------------------------------------------
int svtkTecplotReader::GetNumberOfDataAttributes()
{
...
// ----------------------------------------------------------------------------
const char* svtkTecplotReader::GetDataAttributeName(int attrIndx)
{
...
// ----------------------------------------------------------------------------
int svtkTecplotReader::IsDataAttributeCellBased(int attrIndx)
{
  int cellBasd = -1;
  if (attrIndx >= 0 && attrIndx < this->GetNumberOfDataAttributes())
  {
...
// ----------------------------------------------------------------------------
int svtkTecplotReader::IsDataAttributeCellBased(const char* attrName)
{
  int cellBased = -1;
  int varIndex = -1;
...
//-----------------------------------------------------------------------------
int svtkTecplotReader::GetNumberOfDataArrays()
{
  return this->DataArraySelection->GetNumberOfArrays();
}

//-----------------------------------------------------------------------------
const char* svtkTecplotReader::GetDataArrayName(int arrayIdx)
{
  return this->DataArraySelection->GetArrayName(arrayIdx);
}

//-----------------------------------------------------------------------------
int svtkTecplotReader::GetDataArrayStatus(const char* arayName)
{
  return this->DataArraySelection->ArrayIsEnabled(arayName);
}

//-----------------------------------------------------------------------------
void svtkTecplotReader::SetDataArrayStatus(const char* arayName, int bChecked)
{
  svtkDebugMacro("Set cell array \"" << arayName << "\" status to: " << bChecked);
...
// ----------------------------------------------------------------------------
void svtkTecplotReader::PrintSelf(ostream& os, svtkIndent indent)
{
...
// ----------------------------------------------------------------------------
void svtkTecplotReader::GetArraysFromPointPackingZone(
  int numNodes, svtkPoints* theNodes, svtkPointData* nodeData)
{
  ...
  {
    svtkErrorMacro(<< "File not open, errors with reading, or nullptr svtkPoints /"
                  << "svtkPointData.");
    return;
  }
  ...
  std::vector<svtkFloatArray*> zoneData;
  ...
  for (v = 0; v < this->NumberOfVariables; v++)
  {
    ...
    {
      svtkFloatArray* theArray = svtkFloatArray::New();
  ...
  {
    svtkFloatArray* fa = zoneData.at(i);
    if (fa)
    {
      fa->FastDelete();
    }
...
// ----------------------------------------------------------------------------
void svtkTecplotReader::GetArraysFromBlockPackingZone(
  int numNodes, int numCells, svtkPoints* theNodes, svtkPointData* nodeData, svtkCellData* cellData)
{
  ...
  int isZcoord;
  std::vector<svtkFloatArray*> zoneData;
  svtkDataSetAttributes* attribute[2] = { nodeData, cellData };
...
// ----------------------------------------------------------------------------
void svtkTecplotReader::GetStructuredGridFromBlockPackingZone(int iDimSize, int jDimSize,
  int kDimSize, int zoneIndx, const char* zoneName, svtkMultiBlockDataSet* multZone)
{
  if (!zoneName || !multZone)
  {
    svtkErrorMacro("Zone name un-specified or nullptr svtkMultiBlockDataSet.");
    return;
  }
  ...
  {
    this->Internal->TopologyDim = svtkMath::Max(this->Internal->TopologyDim, 1);
  }
  else if (kDimSize == 1)
  {
    this->Internal->TopologyDim = svtkMath::Max(this->Internal->TopologyDim, 2);
  }
  else
  {
    this->Internal->TopologyDim = svtkMath::Max(this->Internal->TopologyDim, 3);
  }
  ...
  // Create svtkPoints and svtkStructuredGrid and associate them
  svtkPoints* pntCords = svtkPoints::New();
  svtkStructuredGrid* strcGrid = svtkStructuredGrid::New();
    ...
    (this->Internal->TopologyDim == 0 && this->Internal->GeometryDim > 1))
  {
    ...
    multZone->GetMetaData(zoneIndx)->Set(svtkCompositeDataSet::NAME(), zoneName);
  }
  strcGrid->Delete();
  strcGrid = nullptr;
}
...
// ----------------------------------------------------------------------------
void svtkTecplotReader::GetStructuredGridFromPointPackingZone(int iDimSize, int jDimSize,
  int kDimSize, int zoneIndx, const char* zoneName, svtkMultiBlockDataSet* multZone)
{
  if (!zoneName || !multZone)
  {
...
void svtkTecplotReader::GetPolygonalGridFromBlockPackingZone(int numNodes, int numCells,
  int numFaces, int zoneIndx, const char* zoneName, svtkMultiBlockDataSet* multZone)
{
  svtkPoints* gridPnts = svtkPoints::New();
  svtkUnstructuredGrid* unstruct = svtkUnstructuredGrid::New();
  this->GetArraysFromBlockPackingZone(
...
void svtkTecplotReader::GetPolyhedralGridFromBlockPackingZone(int numNodes, int numCells,
  int numFaces, int zoneIndx, const char* zoneName, svtkMultiBlockDataSet* multZone)
{
  svtkPoints* gridPnts = svtkPoints::New();
  svtkUnstructuredGrid* unstruct = svtkUnstructuredGrid::New();
...
// ----------------------------------------------------------------------------
void svtkTecplotReader::GetUnstructuredGridFromBlockPackingZone(int numNodes, int numCells,
  const char* cellType, int zoneIndx, const char* zoneName, svtkMultiBlockDataSet* multZone)
{
  if (!cellType || !zoneName || !multZone)
  {
    svtkErrorMacro(<< "Zone name / cell type un-specified, or nullptr "
...
// ----------------------------------------------------------------------------
void svtkTecplotReader::GetUnstructuredGridFromPointPackingZone(int numNodes, int numCells,
  const char* cellType, int zoneIndx, const char* zoneName, svtkMultiBlockDataSet* multZone)
{
  if (!cellType || !zoneName || !multZone)
  {
...
void svtkTecplotReader::GetPolyhedralGridCells(
  int numCells, int numFaces, svtkUnstructuredGrid* unstruct) const
{
  ...
  for (svtkIdType i = 1; i < numFaces; ++i)
  {
    tok = this->Internal->GetNextToken();
    while (tok.empty())
    {
  ...
  std::vector<std::vector<svtkIdType> > faces;
  for (svtkIdType i = 0; i < numFaces; ++i)
  {
    const size_t nodeCount = nodeCountPerFace[i];
    std::vector<svtkIdType> face;
    ...
    for (size_t j = 0; j < nodeCount; ++j)
    {
      ...
      auto aVertexIndex = static_cast<svtkIdType>(atoi(tok.c_str()));
  ...
  std::map<svtkIdType, std::vector<svtkIdType> > polyhedra;
  ...
  for (svtkIdType i = 0; i < numFaces; ++i)
  {
    ...
    const auto rightCell = static_cast<svtkIdType>(atoi(tok.c_str()));
    if (rightCell > 0)
    {
  ...
  for (svtkIdType i = 0; i < numFaces; ++i)
  {
    ...
    const auto leftCell = static_cast<svtkIdType>(atoi(tok.c_str()));
    if (leftCell > 0)
    {
  ...
  for (auto& entry : polyhedra)
  {
    ...
    std::vector<svtkIdType> polyhedron;

    for (auto& aFaceIndex : facesOfPolyhedron)
    {
      const auto& aFace = faces[aFaceIndex];
      const auto faceSize = static_cast<svtkIdType>(aFace.size());
    ...
    unstruct->InsertNextCell(
      SVTK_POLYHEDRON, static_cast<svtkIdType>(facesOfPolyhedron.size()), polyhedron.data());
  }

  if (unstruct->GetNumberOfCells() != numCells)
  {
    svtkWarningMacro(<< "Number of polyhedral cells does not match.");
  }
}

void OrderEdges(const std::vector<svtkIdType>& faceEdges,
  const std::vector<std::pair<svtkIdType, svtkIdType> >& allEdges, svtkIdList* face)
{
  face->Reset();
  if (faceEdges.empty())
    return;
  ...
  const svtkIdType from = firstEdge.first;
  face->InsertNextId(from);
  svtkIdType to = firstEdge.second;
  ...
  while (to != from)
  {
    ...
    if (face->GetNumberOfIds() > static_cast<svtkIdType>(faceEdges.size() + 1))
    {
...
void svtkTecplotReader::GetPolygonalGridCells(
  int numFaces, int numEdges, svtkUnstructuredGrid* unstruct) const
{
  ...
  for (int i = 0; i < numEdges; ++i)
  {
    ...
    const auto e1 = static_cast<svtkIdType>(atoi(tok1.c_str()));
    const auto e2 = static_cast<svtkIdType>(atoi(tok2.c_str()));
    edges.emplace_back(e1 - 1, e2 - 1); // convert from FORTRAN to C-indexing
  }

  std::map<svtkIdType, std::vector<svtkIdType> > faceEdges;
  ...
  for (int i = 0; i < numEdges; ++i)
  {
    ...
    const auto leftElement = static_cast<svtkIdType>(atoi(tok.c_str()));
    if (leftElement > 0)
    {
  ...
  for (int i = 0; i < numEdges; ++i)
  {
    ...
    const auto rightElement = static_cast<svtkIdType>(atoi(tok.c_str()));
    if (rightElement > 0)
    {
  ...
  {
    svtkWarningMacro(<< " number of faces does not match.");
  }

  svtkNew<svtkIdList> face;
  for (auto& entry : faceEdges)
  {
    OrderEdges(entry.second, edges, face);
    unstruct->InsertNextCell(SVTK_POLYGON, face->GetNumberOfIds(), face->GetPointer(0));
  }
}

// ----------------------------------------------------------------------------
void svtkTecplotReader::GetUnstructuredGridCells(
  int numberCells, const char* cellTypeStr, svtkUnstructuredGrid* unstrctGrid)
{
  if (!cellTypeStr || !unstrctGrid)
  {
    svtkErrorMacro(<< "Cell type (connectivity type) unspecified or nullptr "
                  << "svtkUnstructuredGrid object.");
    return;
  }
  ...
  {
    numCellPnts = 8;
    theCellType = SVTK_HEXAHEDRON;
    this->Internal->TopologyDim = svtkMath::Max(this->Internal->TopologyDim, 3);
  }
  else if (strcmp(cellTypeStr, "TRIANGLE") == 0)
  {
    numCellPnts = 3;
    theCellType = SVTK_TRIANGLE;
    this->Internal->TopologyDim = svtkMath::Max(this->Internal->TopologyDim, 2);
  }
  else if (strcmp(cellTypeStr, "QUADRILATERAL") == 0)
  {
    numCellPnts = 4;
    theCellType = SVTK_QUAD;
    this->Internal->TopologyDim = svtkMath::Max(this->Internal->TopologyDim, 2);
  }
  else if (strcmp(cellTypeStr, "TETRAHEDRON") == 0)
  {
    numCellPnts = 4;
    theCellType = SVTK_TETRA;
    this->Internal->TopologyDim = svtkMath::Max(this->Internal->TopologyDim, 3);
  }
  else if (strcmp(cellTypeStr, "POINT") == 0 || strcmp(cellTypeStr, "") == 0)
  {
    numCellPnts = 1;
    theCellType = SVTK_VERTEX;
    this->Internal->TopologyDim = svtkMath::Max(this->Internal->TopologyDim, 0);
  }
  else
  {
    svtkErrorMacro(<< this->FileName << ": Unknown cell type for a zone.");
    return;
  }

  // the storage of each cell begins with the number of points per cell,
  // followed by a list of point ids representing the cell
  svtkIdTypeArray* cellInfoList = svtkIdTypeArray::New();
  cellInfoList->SetNumberOfValues((numCellPnts + 1) * numberCells);
  svtkIdType* cellInforPtr = cellInfoList->GetPointer(0);

  // type of each cell
  svtkUnsignedCharArray* cellTypeList = svtkUnsignedCharArray::New();
  ...
  for (int c = 0; c < numberCells; c++)
  {
    ...
    *cellInforPtr++ = numCellPnts;
    ...
    {
      *cellInforPtr++ =
        (theCellType == SVTK_VERTEX ? c : atoi(this->Internal->GetNextToken().c_str()) - 1);
    }
  }
  ...
  svtkCellArray* theCellArray = svtkCellArray::New();
  theCellArray->ImportLegacyFormat(cellInfoList);
  cellInfoList->Delete();
  cellInfoList = nullptr;

  // create a svtkUnstructuredGrid object and attach the 3 arrays (types, locations,
...
// ----------------------------------------------------------------------------
void svtkTecplotReader::GetDataArraysList()
{
...
// ----------------------------------------------------------------------------
void svtkTecplotReader::ReadFile(svtkMultiBlockDataSet* multZone)
{
  ...
  if (multZone == nullptr)
  {
    svtkErrorMacro("svtkMultiBlockDataSet multZone nullptr!");
    return;
  }
...
#define READ_UNTIL_LINE_END                                                                        \
  ...
  while (!this->Internal->NextCharEOF)
  {
    ...
    else if (tok == "ZONE")
    {
      ...
      while (!(tok.front() == '-') && !(tok.front() == '.') && !isdigit(tok.front()))
      {
        if (tok == "T")
        {
          ...
          {
            svtkErrorMacro(<< this->FileName << ": Zone titles MUST be "
                          << "quoted.");
            return;
          }
        }
          ...
          else
          {
            do
            {
              ...
              if (var_format_type.find('-') != std::string::npos)
              {
                ...
                svtksys::SystemTools::Split(var_format_type, var_range, '-');
        ...
        {
          svtkWarningMacro(<< this->FileName << "; Tecplot zone record parameter "
        ...
        else if (tok == "NUMCONNECTEDBOUNDARYFACES")
        {
          ...
          {
            svtkWarningMacro(<< "Non-zero number of connected boundary faces is not supported.");
          }
        }
        else if (tok == "TOTALNUMBOUNDARYCONNECTIONS")
        {
          ...
          {
            svtkWarningMacro(<< "Non-zero number of total #boundary faces is not supported.");
          }
        }
        else
        {
          svtkDebugMacro(<< this->FileName << "; encountered an unknown token: '" << tok
      ...
      if (zoneType.empty())
      {
        ...
        {
          // UNKNOWN FORMAT
          svtkErrorMacro(<< this->FileName << ": The format " << format.c_str()
      ...
      else
      {
        ...
        {
          svtkWarningMacro(<< " ZONETYPE '" << zoneType << "' is currently supported.");
        }
      }
    ...
    {
      // UNKNOWN RECORD TYPE
      svtkErrorMacro(<< this->FileName << ": The record type " << tok.c_str()
                    << " found in the file is unknown.");
      return;
    }
