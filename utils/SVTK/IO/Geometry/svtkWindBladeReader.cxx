...
Module:    svtkWindBladeReader.cxx
...
// svtkStandardNewMacro(svtkWindBladeReader);
svtkObjectFactoryNewMacro(svtkWindBladeReader);

float svtkWindBladeReader::DRY_AIR_CONSTANT = 287.04;
int svtkWindBladeReader::NUM_PART_SIDES = 4;       // Blade parts rhombus
const int svtkWindBladeReader::NUM_BASE_SIDES = 5; // Base pyramid
const int svtkWindBladeReader::LINE_SIZE = 256;
int svtkWindBladeReader::DIMENSION = 3;
int svtkWindBladeReader::BYTES_PER_DATA = 4;
int svtkWindBladeReader::SCALAR = 1;
int svtkWindBladeReader::VECTOR = 2;
int svtkWindBladeReader::FLOAT = 1;
int svtkWindBladeReader::INTEGER = 2;
...
//----------------------------------------------------------------------------
svtkWindBladeReader::svtkWindBladeReader()
{
  ...
  this->GPoints = svtkPoints::New();
  this->XSpacing = svtkFloatArray::New();
  this->YSpacing = svtkFloatArray::New();
  this->ZSpacing = svtkFloatArray::New();
  this->ZTopographicValues = nullptr;

  // Blade geometry
  this->BPoints = svtkPoints::New();
  this->NumberOfBladePoints = 0;
  this->NumberOfBladeCells = 0;

  // Static tower information
  this->NumberOfBladeTowers = 0;
  this->XPosition = svtkFloatArray::New();
  this->YPosition = svtkFloatArray::New();
  this->HubHeight = svtkFloatArray::New();
  this->AngularVeloc = svtkFloatArray::New();
  this->BladeLength = svtkFloatArray::New();
  this->BladeCount = svtkIntArray::New();
  ...
  this->SelectionObserver->SetCallback(&svtkWindBladeReader::SelectionCallback);
  ...
  this->DivideVariables = svtkStringArray::New();
...
//----------------------------------------------------------------------------
svtkTypeBool svtkWindBladeReader::ProcessRequest(
  svtkInformation* reqInfo, svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
#ifdef SVTK_WORDS_BIGENDIAN
  svtkWarningMacro("Warning WindBlade reader does not yet work on big endian processors");
#endif
  if (reqInfo->Has(svtkDemandDrivenPipeline::REQUEST_DATA_NOT_GENERATED()))
  {
    int port = reqInfo->Get(svtkDemandDrivenPipeline::FROM_OUTPUT_PORT());
    if (port != 0)
    {
      svtkInformation* fieldInfo = outputVector->GetInformationObject(0);
      fieldInfo->Set(svtkDemandDrivenPipeline::DATA_NOT_GENERATED(), 1);
    }
    if (port != 1)
    {
      svtkInformation* bladeInfo = outputVector->GetInformationObject(1);
      bladeInfo->Set(svtkDemandDrivenPipeline::DATA_NOT_GENERATED(), 1);
    }
    if (port != 2)
    {
      svtkInformation* groundInfo = outputVector->GetInformationObject(2);
      groundInfo->Set(svtkDemandDrivenPipeline::DATA_NOT_GENERATED(), 1);
    }
    return 1;
  }
...
//----------------------------------------------------------------------------
int svtkWindBladeReader::RequestInformation(svtkInformation* reqInfo,
  svtkInformationVector** svtkNotUsed(inputVector), svtkInformationVector* outputVector)
{
  ...
  {
    svtkInformation* bladeInfo = outputVector->GetInformationObject(1);
    bladeInfo->Set(svtkDemandDrivenPipeline::REQUEST_DATA_NOT_GENERATED());
    svtkInformation* groundInfo = outputVector->GetInformationObject(2);
    groundInfo->Set(svtkDemandDrivenPipeline::REQUEST_DATA_NOT_GENERATED());
  }
  else if (port == 1)
  {
    svtkInformation* fieldInfo = outputVector->GetInformationObject(0);
    fieldInfo->Set(svtkDemandDrivenPipeline::REQUEST_DATA_NOT_GENERATED());
    svtkInformation* groundInfo = outputVector->GetInformationObject(2);
    groundInfo->Set(svtkDemandDrivenPipeline::REQUEST_DATA_NOT_GENERATED());
  }
  else if (port == 2)
  ...
  svtkInformation* fieldInfo = outputVector->GetInformationObject(0);
  svtkStructuredGrid* field = this->GetFieldOutput();

  svtkInformation* bladeInfo = outputVector->GetInformationObject(1);
  this->GetBladeOutput();

  svtkInformation* groundInfo = outputVector->GetInformationObject(2);
  svtkStructuredGrid* ground = this->GetGroundOutput();
  ...
  if (this->NumberOfVariables == 0)
  {
    ...
    this->Data = new svtkFloatArray*[this->NumberOfVariables];
    for (int var = 0; var < this->NumberOfVariables; var++)
    {
      this->Data[var] = svtkFloatArray::New();
    ...
    fieldInfo->Set(svtkStreamingDemandDrivenPipeline::WHOLE_EXTENT(), this->WholeExtent, 6);
    fieldInfo->Set(CAN_PRODUCE_SUB_EXTENT(), 1);

    ground->SetDimensions(this->GDimension);
    groundInfo->Set(svtkStreamingDemandDrivenPipeline::WHOLE_EXTENT(), this->GExtent, 6);
    groundInfo->Set(CAN_PRODUCE_SUB_EXTENT(), 1);

    bladeInfo->Set(svtkStreamingDemandDrivenPipeline::WHOLE_EXTENT(), this->WholeExtent, 6);
    ...
    if (this->NumberOfTimeSteps > 0)
    {
      ...
      fieldInfo->Set(svtkStreamingDemandDrivenPipeline::TIME_RANGE(), tRange, 2);
      bladeInfo->Set(svtkStreamingDemandDrivenPipeline::TIME_RANGE(), tRange, 2);
    }
    else
    {
      fieldInfo->Remove(svtkStreamingDemandDrivenPipeline::TIME_STEPS());
      fieldInfo->Set(
        svtkStreamingDemandDrivenPipeline::TIME_STEPS(), this->TimeSteps, this->NumberOfTimeSteps);
      bladeInfo->Remove(svtkStreamingDemandDrivenPipeline::TIME_STEPS());
      bladeInfo->Set(
        svtkStreamingDemandDrivenPipeline::TIME_STEPS(), this->TimeSteps, this->NumberOfTimeSteps);
    }
  }
...
//----------------------------------------------------------------------------
void svtkWindBladeReader::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
int svtkWindBladeReader::RequestData(svtkInformation* reqInfo,
  svtkInformationVector** svtkNotUsed(inVector), svtkInformationVector* outVector)
{
  ...
  if (port == 0)
  {
    ...
    this->Internal->FilePtr = svtksys::SystemTools::Fopen(fileName.str(), "rb");
    if (this->Internal->FilePtr == nullptr)
    {
      svtkWarningMacro(<< "Could not open file " << fileName.str());
      return 0;
    }
...
//----------------------------------------------------------------------------
void svtkWindBladeReader::DivideByDensity(const char* varName)
{
...
//----------------------------------------------------------------------------
void svtkWindBladeReader::CalculatePressure(int pressure, int prespre, int tempg, int density)
{
  ...
  if (fread(tempgData, sizeof(float), this->BlockSize, this->Internal->FilePtr) != this->BlockSize)
  {
    ...
    // warning
    svtkWarningMacro("WindBladeReader error reading file: "
...
//----------------------------------------------------------------------------
void svtkWindBladeReader::CalculateVorticity(int vort, int uvw, int density)
{
...
//----------------------------------------------------------------------------
void svtkWindBladeReader::LoadVariableData(int var)
{
  this->Data[var]->Delete();
  this->Data[var] = svtkFloatArray::New();
...
// Load one variable data array of BLOCK structure into ParaView
//----------------------------------------------------------------------------
bool svtkWindBladeReader::ReadGlobalData()
{
  // kwsys_stl::string fileName = this->Filename;
  std::string fileName = this->Filename;
  svtksys::SystemTools::ConvertToUnixSlashes(fileName);

  svtksys::ifstream inStrStream(fileName.c_str());
...
//
//----------------------------------------------------------------------------
void svtkWindBladeReader::ReadDataVariables(istream& inStr)
{
  ...
  delete[] this->VariableName;
  this->VariableName = new svtkStdString[totalVariables];
  delete[] this->VariableStruct;
  delete[] this->VariableCompSize;
  delete[] this->VariableBasicType;
  delete[] this->VariableByteCount;
  ...
  for (int i = 0; i < this->NumberOfFileVariables; i++)
  {
    ...
    {
      svtkWarningMacro("Error in structure type " << structType);
    }
    ...
    {
      svtkWarningMacro("Error in basic type " << basicType);
    }
  }
...
//
//----------------------------------------------------------------------------
bool svtkWindBladeReader::FindVariableOffsets()
{
  ...
  if (this->Internal->FilePtr == nullptr)
  {
    svtkErrorMacro("Could not open file " << fileName.str());
    return false;
  }
...
//----------------------------------------------------------------------------
void svtkWindBladeReader::FillCoordinates()
{
  this->Points->Delete();
  this->Points = svtkPoints::New();
  ...
  {
    // Save svtkPoints instead of spacing coordinates because topography file
    // requires this to be svtkStructuredGrid and not svtkRectilinearGrid
    for (int k = this->SubExtent[4]; k <= this->SubExtent[5]; k++)
    {
...
// Fill in the rectilinear points for the requested subextents
//----------------------------------------------------------------------------
void svtkWindBladeReader::FillGroundCoordinates()
{
  this->GPoints->Delete();
  this->GPoints = svtkPoints::New();
...
//----------------------------------------------------------------------------
void svtkWindBladeReader::CreateCoordinates()
{
  // If dataset is flat, x and y are constant spacing, z is stretched
  if (this->UseTopographyFile == 0)
  {
...
//----------------------------------------------------------------------------
void svtkWindBladeReader::CreateZTopography(float* zValues)
{
  ...
  FILE* filePtr = svtksys::SystemTools::Fopen(fileName.str(), "rb");
...
//
//----------------------------------------------------------------------------
float svtkWindBladeReader::GDeform(float sigma, float sigmaMax, int flag)
{
...
//----------------------------------------------------------------------------
void svtkWindBladeReader::Spline(float* x, float* y, // arrays
...
//----------------------------------------------------------------------------
void svtkWindBladeReader::Splint(float* xa, float* ya, // arrays sent to Spline()
...
//----------------------------------------------------------------------------
void svtkWindBladeReader::SetupBladeData()
{
  ...
  svtksys::ifstream inStr(fileName.str().c_str());

  if (!inStr)
  {
    svtkWarningMacro("Could not open " << fileName.str() << endl);
  }
  ...
  svtksys::ifstream inStr2(fileName2.str().c_str());
  ...
  if (!inStr2)
  {
         ...
         j += this->TimeStepDelta)
    {
      ...
      if (inStr2.good())
      {
        svtkWarningMacro("Success with " << fileName3.str());
        break;
      }
      else
      {
        svtkWarningMacro("Failure with " << fileName3.str());
      }
    }
  }
...
//----------------------------------------------------------------------------
void svtkWindBladeReader::LoadBladeData(int timeStep)
{
  this->BPoints->Delete();
  this->BPoints = svtkPoints::New();
...
//----------------------------------------------------------------------------
void svtkWindBladeReader::InitFieldData(
  svtkInformationVector* outVector, std::ostringstream& fileName, svtkStructuredGrid* field)
{
  ...
  svtkInformation* fieldInfo = outVector->GetInformationObject(0);

  // Set the extent info for this processor
  fieldInfo->Get(svtkStreamingDemandDrivenPipeline::UPDATE_EXTENT(), this->SubExtent);
  ...
  field->GetInformation()->Set(svtkDataObject::DATA_TIME_STEP(), dTime);
...
//----------------------------------------------------------------------------
void svtkWindBladeReader::SetUpFieldVars(svtkStructuredGrid* field)
{
...
//----------------------------------------------------------------------------
void svtkWindBladeReader::InitBladeData(svtkInformationVector* outVector)
{
  svtkInformation* bladeInfo = outVector->GetInformationObject(1);
  svtkUnstructuredGrid* blade = this->GetBladeOutput();
  ...
  // Actual time for the time step
  blade->GetInformation()->Set(svtkDataObject::DATA_TIME_STEP(), dTime);
...
//----------------------------------------------------------------------------
void svtkWindBladeReader::SetUpGroundData(svtkInformationVector* outVector)
{
  svtkInformation* groundInfo = outVector->GetInformationObject(2);
  svtkStructuredGrid* ground = this->GetGroundOutput();

  // Set the extent info for this processor
  groundInfo->Get(svtkStreamingDemandDrivenPipeline::UPDATE_EXTENT(), this->GSubExtent);
...
//----------------------------------------------------------------------------
void svtkWindBladeReader::InitPressureData(
...
//----------------------------------------------------------------------------
void svtkWindBladeReader::SetUpPressureData(
...
//----------------------------------------------------------------------------
void svtkWindBladeReader::SetUpVorticityData(
...
//----------------------------------------------------------------------------
void svtkWindBladeReader::InitVariableData(
...
//----------------------------------------------------------------------------
bool svtkWindBladeReader::SetUpGlobalData(const std::string& fileName, std::stringstream& inStr)
{
  char inBuf[LINE_SIZE];
  if (!inStr)
  {
    svtkWarningMacro("Could not open the global .wind file " << fileName);
  }

  std::string::size_type dirPos = std::string(fileName).rfind('/');
  if (dirPos == std::string::npos)
  {
    svtkWarningMacro("Bad input file name " << fileName);
  }
...
//----------------------------------------------------------------------------
void svtkWindBladeReader::ProcessZCoords(float* topoData, float* zValues)
{
...
//----------------------------------------------------------------------------
void svtkWindBladeReader::ReadBladeHeader(
...
void svtkWindBladeReader::ReadBladeData(std::stringstream& inStr)
{
  ...
  svtkFloatArray* force2 = svtkFloatArray::New();
  ...
  svtkFloatArray* bladeComp = svtkFloatArray::New();
  ...
  svtkFloatArray* bladeVeloc = svtkFloatArray::New();
  ...
  svtkFloatArray* bladeAzimUVW = svtkFloatArray::New();
  ...
  svtkFloatArray* bladeAxialUVW = svtkFloatArray::New();
  ...
  svtkFloatArray* bladeDragUVW = svtkFloatArray::New();
  ...
  svtkFloatArray* bladeLiftUVW = svtkFloatArray::New();
  ...
  float x, y, z;
  svtkIdType cell[NUM_BASE_SIDES];
  ...
  while (inStr.getline(inBuf, LINE_SIZE))
  {
    ...
    for (int side = 0; side < NUM_PART_SIDES; side++)
    {
      ...
      float dist = svtkMath::Distance2BetweenPoints(hubPnt, bladePnt);
    ...
    if (sectionNum == 0)
    {
      svtkIdType numBPnts = this->BPoints->GetNumberOfPoints();
      ...
      svtkMath::Cross(vec2, vec1, bladeAzimUVWVec);
      svtkMath::Normalize(bladeAzimUVWVec);
      ...
      svtkMath::Normalize(bladeDragUVWVec);
      svtkMath::Cross(bladeDragUVWVec, vec1, bladeLiftUVWVec);
      svtkMath::Normalize(bladeLiftUVWVec);
    }
    ...
    blade->InsertNextCell(SVTK_POLYGON, NUM_PART_SIDES, cell);
  ...
  for (int j = 0; j < this->NumberOfBladeTowers; j++)
  {
    ...
    blade->InsertNextCell(SVTK_PYRAMID, NUM_BASE_SIDES, cell);
...
//----------------------------------------------------------------------------
void svtkWindBladeReader::SelectionCallback(
  svtkObject*, unsigned long svtkNotUsed(eventid), void* clientdata, void* svtkNotUsed(calldata))
{
  static_cast<svtkWindBladeReader*>(clientdata)->Modified();
}

//----------------------------------------------------------------------------
int svtkWindBladeReader::GetNumberOfPointArrays()
{
  return this->PointDataArraySelection->GetNumberOfArrays();
}

//----------------------------------------------------------------------------
void svtkWindBladeReader::EnableAllPointArrays()
{
  this->PointDataArraySelection->EnableAllArrays();
}

//----------------------------------------------------------------------------
void svtkWindBladeReader::DisableAllPointArrays()
{
  this->PointDataArraySelection->DisableAllArrays();
}

//----------------------------------------------------------------------------
const char* svtkWindBladeReader::GetPointArrayName(int index)
{
  return this->VariableName[index].c_str();
}

//----------------------------------------------------------------------------
int svtkWindBladeReader::GetPointArrayStatus(const char* name)
{
  return this->PointDataArraySelection->ArrayIsEnabled(name);
}

//----------------------------------------------------------------------------
void svtkWindBladeReader::SetPointArrayStatus(const char* name, int status)
{
  if (status)
  {
...
//----------------------------------------------------------------------------
svtkStructuredGrid* svtkWindBladeReader::GetFieldOutput()
{
  return svtkStructuredGrid::SafeDownCast(this->GetExecutive()->GetOutputData(0));
}

//----------------------------------------------------------------------------
svtkUnstructuredGrid* svtkWindBladeReader::GetBladeOutput()
{
...
//----------------------------------------------------------------------------
svtkStructuredGrid* svtkWindBladeReader::GetGroundOutput()
{
...
//----------------------------------------------------------------------------
int svtkWindBladeReader::FillOutputPortInformation(int port, svtkInformation* info)
{
  // Field data
  if (port == 0)
  {
