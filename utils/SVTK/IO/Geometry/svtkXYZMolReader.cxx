...
//----------------------------------------------------------------------------
svtkStandardNewMacro(svtkXYZMolReader);

//----------------------------------------------------------------------------
svtkXYZMolReader::svtkXYZMolReader()
{
  this->TimeStep = 0;
  this->MaxTimeStep = 0;
}
...
//----------------------------------------------------------------------------
svtkXYZMolReader::~svtkXYZMolReader() = default;

//----------------------------------------------------------------------------
char* svtkXYZMolReader::GetNextLine(FILE* fp, char* line, int maxlen)
{
  int cc;
  int len;
  int comment;
...
//----------------------------------------------------------------------------
int svtkXYZMolReader::GetLine1(const char* line, int* cnt)
{
  char dummy[1024] = "";
...
//----------------------------------------------------------------------------
int svtkXYZMolReader::GetLine2(const char* line, char* name)
{
  char dummy[1024] = "";
...
//----------------------------------------------------------------------------
int svtkXYZMolReader::GetAtom(const char* line, char* atom, float* x)
{
...
//----------------------------------------------------------------------------
void svtkXYZMolReader::InsertAtom(const char* atom, float* pos)
{
...
//----------------------------------------------------------------------------
int svtkXYZMolReader::CanReadFile(const char* name)
{
  if (!name)
  {
    return 0;
  ...
  FILE* fp = svtksys::SystemTools::Fopen(name, "r");
  if (!fp)
  {
    return 0;
  }
...
//----------------------------------------------------------------------------
void svtkXYZMolReader::ReadSpecificMolecule(FILE* fp)
{
  const int maxlen = 1024;
  char buffer[maxlen];
  char comment[maxlen];
  ...
  while ((lptr = this->GetNextLine(fp, buffer, maxlen)))
  {
    ...
    {
      have_header = 1;
      svtkDebugMacro("Have header. Number of atoms is: " << num);
      ccnt = 0;
      if (cnt > 0)
      {
        timestep++;
    ...
    else if (have_header)
    {
      if (this->GetAtom(lptr, atom, pos))
      {
        ...
        if (ccnt >= num)
        {
          svtkErrorMacro("Expecting " << num << " atoms, found: " << ccnt);
          return;
        }
        else
        {
      ...
      {
        svtkDebugMacro("Have comment");
      }
      else
      {
        svtkErrorMacro("Expecting atom, got: " << lptr);
        return;
      }
    }
    else
  ...
  if (selectstep >= timestep)
  {
    ...
    svtkErrorMacro("Only have " << timestep << " time step(s)");
    return;
  }

  svtkDebugMacro("Number of atoms: " << num << " (" << rcnt << ")");
  if (num != rcnt)
  {
    this->NumberOfAtoms = 0;
    svtkErrorMacro("Expecting " << num << " atoms, got " << rcnt);
    return;
  }
...
//----------------------------------------------------------------------------
void svtkXYZMolReader::PrintSelf(ostream& os, svtkIndent indent)
{
