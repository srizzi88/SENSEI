...
#include "svtk_h5part.h"
// clang-format off
#include SVTK_H5PART(H5Part.h)
// clang-format on
...
static void svtkPickArray(char*& arrayPtr, const std::initializer_list<const char*>& values,
  svtkDataArraySelection* selection)
{
  ...
  for (int cc = 0, max = selection->GetNumberOfArrays(); cc < max; ++cc)
  {
    ...
    {
      if (svtksys::SystemTools::Strucmp(aname, value) == 0)
      {
        arrayPtr = svtksys::SystemTools::DuplicateString(aname);
        return;
      }
    }
  }
...
//----------------------------------------------------------------------------
svtkStandardNewMacro(svtkH5PartReader);
//----------------------------------------------------------------------------
svtkH5PartReader::svtkH5PartReader()
{
...
//----------------------------------------------------------------------------
void svtkH5PartReader::SetFileName(char* filename)
{
  ...
  if (filename)
  {
    this->FileName = svtksys::SystemTools::DuplicateString(filename);
    this->FileModifiedTime.Modified();
  }
  this->Modified();
}
//----------------------------------------------------------------------------
void svtkH5PartReader::CloseFile()
{
...
//----------------------------------------------------------------------------
int svtkH5PartReader::IndexOfVectorComponent(const char* name)
{
  ...
  //
  svtksys::RegularExpression re1(".*_([0-9]+)");
  if (re1.find(name))
  {
...
//----------------------------------------------------------------------------
std::string svtkH5PartReader::NameOfVectorComponent(const char* name)
{
  if (!this->CombineVectorComponents)
  {
    return name;
...
//----------------------------------------------------------------------------
int svtkH5PartReader::RequestInformation(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** svtkNotUsed(inputVector), svtkInformationVector* outputVector)
{
  ...
  // if (1 || NeedToReadInformation)
  {
    ...
    {
      svtkErrorMacro(<< "No time steps in data");
      return 0;
    }
  ...
  svtkPickArray(this->Xarray, { "x", "coords_0", "coords0" }, this->PointDataArraySelection);
  svtkPickArray(this->Yarray, { "y", "coords_1", "coords1" }, this->PointDataArraySelection);
  svtkPickArray(this->Zarray, { "z", "coords_2", "coords2" }, this->PointDataArraySelection);
  return 1;
}

int GetSVTKDataType(hid_t datatype)
{
...
//----------------------------------------------------------------------------
int svtkH5PartReader::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** svtkNotUsed(inputVector), svtkInformationVector* outputVector)
{
  ...
  for (int i = 0; i < N; i++)
  {
    ...
    if (!svtksys::SystemTools::Strucmp(name, this->Xarray))
    {
      processarray = true;
      coordarrays[0] = name;
    }
    if (!svtksys::SystemTools::Strucmp(name, this->Yarray))
    {
      processarray = true;
      coordarrays[1] = name;
    }
    if (!svtksys::SystemTools::Strucmp(name, this->Zarray))
    {
      processarray = true;
      coordarrays[2] = name;
    }
  ...
  if (outInfo->Has(svtkStreamingDemandDrivenPipeline::UPDATE_TIME_STEP()))
  {
    ...
    output->GetInformation()->Set(svtkDataObject::DATA_TIME_STEP(), requestedTimeValue);
  }
  else
  {
    double timevalue[1];
    ...
    output->GetInformation()->Set(svtkDataObject::DATA_TIME_STEP(), timevalue[0]);
  }
  ...
  svtkIdType Nt = H5PartGetNumParticles(this->H5FileId);
  if (piece < Nt)
  {
    if (numPieces > 1)
    {
      ...
      svtkIdType myNt = piece < rem ? div + 1 : div;
      svtkIdType myOffset = piece < rem ? (div + 1) * piece : (div + 1) * rem + div * (piece - rem);
  ...
  svtkSmartPointer<svtkDataArray> coords = nullptr;
  for (FieldMap::iterator it = scalarFields.begin(); it != scalarFields.end(); it++)
  {
    ...
    //
    svtkSmartPointer<svtkDataArray> dataarray = nullptr;
    hid_t datatype = H5PartGetNativeDatasetType(H5FileId, array_name);
    int svtk_datatype = GetSVTKDataType(datatype);

    if (svtk_datatype != SVTK_VOID)
    {
      dataarray.TakeReference(svtkDataArray::CreateDataArray(svtk_datatype));
      dataarray->SetNumberOfComponents(Nc);
      dataarray->SetNumberOfTuples(Nt);
      dataarray->SetName(rootname.c_str());
      ...
      for (int c = 0; c < Nc; c++)
      {
        ...
        else
        {
          ...
          svtkDataArray* temparray =
            svtkDataArray::CreateDataArray(GetSVTKDataType(component_datatype));
    ...
    {
      H5Tclose(datatype);
      svtkErrorMacro("An unexpected data type was encountered");
      return 0;
    }
    H5Tclose(datatype);
    //
...
//----------------------------------------------------------------------------
int svtkH5PartReader::GetCoordinateArrayStatus(const char* name)
{
  return this->PointDataArraySelection->ArrayIsEnabled(name);
}
//----------------------------------------------------------------------------
void svtkH5PartReader::SetCoordinateArrayStatus(const char* name, int status)
{
  if (status)
  {
...
//----------------------------------------------------------------------------
const char* svtkH5PartReader::GetPointArrayName(int index)
{
  return this->PointDataArraySelection->GetArrayName(index);
}
//----------------------------------------------------------------------------
int svtkH5PartReader::GetPointArrayStatus(const char* name)
{
  return this->PointDataArraySelection->ArrayIsEnabled(name);
}
//----------------------------------------------------------------------------
void svtkH5PartReader::SetPointArrayStatus(const char* name, int status)
{
...
//----------------------------------------------------------------------------
void svtkH5PartReader::Enable(const char* name)
{
  this->SetPointArrayStatus(name, 1);
}
//----------------------------------------------------------------------------
void svtkH5PartReader::Disable(const char* name)
{
  this->SetPointArrayStatus(name, 0);
}
//----------------------------------------------------------------------------
void svtkH5PartReader::EnableAll()
{
  this->PointDataArraySelection->EnableAllArrays();
}
//----------------------------------------------------------------------------
void svtkH5PartReader::DisableAll()
{
  this->PointDataArraySelection->DisableAllArrays();
}
//----------------------------------------------------------------------------
int svtkH5PartReader::GetNumberOfPointArrays()
{
  return this->PointDataArraySelection->GetNumberOfArrays();
}
//----------------------------------------------------------------------------
void svtkH5PartReader::PrintSelf(ostream& os, svtkIndent indent)
{
