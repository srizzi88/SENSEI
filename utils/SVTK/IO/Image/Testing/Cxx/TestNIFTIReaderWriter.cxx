...
=========================================================================*/
/*
Test NIFTI support in SVTK by reading a file, writing it, and
  ...
  const char* infile, const char* infile2, const char* outfile, bool planarRGB)
{
  ...
  {
    svtkNew<svtkStringArray> filenames;
  ...
  svtkNew<svtkNIFTIImageWriter> writer;
  writer->SetInputConnection(reader->GetOutputPort());
  writer->SetFileName(outfile);
  // copy most information directly from the header
  svtkNIFTIImageHeader* header = writer->GetNIFTIHeader();
  header->DeepCopy(reader->GetNIFTIHeader());
  header->SetDescrip("SVTK Test Data");
  ...
  svtkOStrStreamWrapper s;
  reader->Print(s);
  header->Print(s);
  writer->Print(s);

  svtkNew<svtkNIFTIImageReader> reader2;
  ...
  svtkNew<svtkImageMathematics> diff;
  ...
  {
    svtkNew<svtkMatrix4x4> m;
    m->DeepCopy(writer->GetQFormMatrix());
    m->Invert();
    svtkMatrix4x4::Multiply4x4(m, reader2->GetQFormMatrix(), m);
    double sqdiff = 0.0;
    for (int i = 0; i < 4; i++)
    {
      for (int j = 0; j < 4; j++)
  ...
  if (writer->GetSFormMatrix())
  {
    ...
    m->Invert();
    svtkMatrix4x4::Multiply4x4(m, reader2->GetSFormMatrix(), m);
    double sqdiff = 0.0;
    for (int i = 0; i < 4; i++)
    {
      for (int j = 0; j < 4; j++)
...
{
  svtkNew<svtkNIFTIImageHeader> header1;
  svtkNew<svtkNIFTIImageHeader> header2;

  header1->SetIntentCode(svtkNIFTIImageHeader::IntentZScore);
  ...
  header1->SetXYZTUnits(svtkNIFTIImageHeader::UnitsMM | svtkNIFTIImageHeader::UnitsSec);
  header1->SetDimInfo(0);
  header1->SetDescrip("Test header");
  header1->SetAuxFile("none");
  header1->SetQFormCode(svtkNIFTIImageHeader::XFormScannerAnat);
  ...
  header1->SetSFormCode(svtkNIFTIImageHeader::XFormAlignedAnat);
  double matrix[16];
  ...
  bool success = true;
  success &= (header2->GetIntentCode() == svtkNIFTIImageHeader::IntentZScore);
  ...
  success &=
    (header2->GetXYZTUnits() == (svtkNIFTIImageHeader::UnitsMM | svtkNIFTIImageHeader::UnitsSec));
  success &= (header2->GetDimInfo() == 0);
  success &= (strcmp(header2->GetDescrip(), "Test header") == 0);
  success &= (strcmp(header2->GetAuxFile(), "none") == 0);
  success &= (header2->GetQFormCode() == svtkNIFTIImageHeader::XFormScannerAnat);
...
int TestNIFTIReaderWriter(int argc, char* argv[])
{
  ...
  for (int i = 0; i < 5; i++)
  {
    ...
    char* infile = svtkTestUtilities::ExpandDataFileName(argc, argv, testfiles[i][0]);
    bool planarRGB = (i == 2);
    if (i == 5)
    {
      infile2 = svtkTestUtilities::ExpandDataFileName(argc, argv, testfiles[6][0]);
    }
    if (!infile)
    {
    ...
    svtkNew<svtkNIFTIImageReader> testReader;
