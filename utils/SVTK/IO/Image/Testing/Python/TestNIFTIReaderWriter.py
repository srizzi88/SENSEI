...
def TestDisplay(file1):
    ...
    inpath = os.path.join(str(SVTK_DATA_ROOT), "Data", file1)

    reader = svtk.svtkNIFTIImageReader()
...
def TestReadWriteRead(infile, outfile):
    ...
    inpath = os.path.join(str(SVTK_DATA_ROOT), "Data", infile)
    outpath = os.path.join(str(SVTK_TEMP_DIR), outfile)
    ...
    writer = svtk.svtkNIFTIImageWriter()
    ...
    reader2 = svtk.svtkNIFTIImageReader()
    reader2.SetFileName(outpath)
    reader2.TimeAsVectorOn()
    reader2.Update()

    diff = svtk.svtkImageMathematics()
