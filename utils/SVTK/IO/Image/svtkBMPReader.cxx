...
svtkStandardNewMacro(svtkBMPReader);
...
//----------------------------------------------------------------------------
void svtkBMPReader::ExecuteInformation()
{
  int xsize, ysize;
  FILE* fp;
  svtkTypeInt32 tmp;
  svtkTypeInt32 offset;
  svtkTypeInt32 infoSize;
  svtkTypeInt16 stmp1, stmp2;
  ...
  if (errorOccurred)
  {
    svtkErrorMacro("Error reading file: " << this->InternalFileName << " Premature end of file.");
    fclose(fp);
    return;
  }

  svtkByteSwap::Swap4LE(&infoSize);

  // error checking
  if ((infoSize != 40) && (infoSize != 12))
  {
    svtkErrorMacro("Unknown file type! " << this->InternalFileName << " is not a Windows BMP file!");
    fclose(fp);
    return;
  }
  ...
  if (infoSize == 40)
  {
    ...
    svtkByteSwap::Swap4LE(&xsize);
    svtkByteSwap::Swap4LE(&ysize);
  }
  else
  {
    ...
    svtkByteSwap::Swap2LE(&stmp1);
    svtkByteSwap::Swap2LE(&stmp2);
    xsize = stmp1;
    ysize = stmp2;
  }
  ...
  if (errorOccurred)
  {
    svtkErrorMacro("BMPReader error reading file: " << this->InternalFileName
  ...
  {
    svtkErrorMacro("Only BMP depths of (8,24) are supported. Not " << this->Depth);
    fclose(fp);
    return;
  }
  ...
  if (infoSize == 40)
  {
    ...
    {
      svtkErrorMacro("BMPReader error reading file: "
  ...
  if (fclose(fp))
  {
    svtkWarningMacro("File close failed on " << this->InternalFileName);
  }

  // Offset is the true header size. See bug 14397
  svtkByteSwap::Swap4LE(&offset);
...
void svtkBMPReader::ComputeDataIncrements()
{
  int idx;
  svtkIdType fileDataLength;
...
template <class OT>
void svtkBMPReaderUpdate2(svtkBMPReader* self, svtkImageData* data, OT* outPtr)
{
  svtkIdType inIncr[3], outIncr[3];
  OT *outPtr0, *outPtr1, *outPtr2;
  svtkIdType streamSkip0, streamSkip1;
  svtkIdType streamRead;
  ...
  streamRead = (svtkIdType)(pixelRead * self->GetDataIncrements()[0]);
  streamSkip0 = (svtkIdType)(self->GetDataIncrements()[1] - streamRead);
  streamSkip1 = (svtkIdType)(self->GetDataIncrements()[2] -
  ...
  {
    streamSkip0 = (svtkIdType)(-streamRead - self->GetDataIncrements()[1]);
  }
  ...
  for (idx2 = dataExtent[4]; idx2 <= dataExtent[5]; ++idx2)
  {
    ...
    for (idx1 = dataExtent[2]; !self->AbortExecute && idx1 <= dataExtent[3]; ++idx1)
    {
      ...
      {
        svtkErrorWithObjectMacro(self,
          "File operation failed. row = "
            << idx1 << ", Read = " << streamRead << ", Skip0 = " << streamSkip0 << ", Skip1 = "
            << streamSkip1 << ", FilePos = " << static_cast<svtkIdType>(self->GetFile()->tellg())
...
void svtkBMPReader::ExecuteDataWithInformation(svtkDataObject* output, svtkInformation* outInfo)
{
  ...
  {
    svtkTemplateMacro(svtkBMPReaderUpdate2(this, data, static_cast<SVTK_TT*>(outPtr)));
    default:
...
//----------------------------------------------------------------------------
void svtkBMPReader::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
int svtkBMPReader::CanReadFile(const char* fname)
{
  // get the magic number by reading in a file
