 ...
 *
 * svtkBMPReader is a source object that reads Windows BMP files.
...
#ifndef svtkBMPReader_h
#define svtkBMPReader_h

#include "svtkIOImageModule.h" // For export macro
#include "svtkImageReader.h"
class svtkLookupTable;

class SVTKIOIMAGE_EXPORT svtkBMPReader : public svtkImageReader
{
public:
  static svtkBMPReader* New();
  svtkTypeMacro(svtkBMPReader, svtkImageReader);
   ...
   * exported
   */
  svtkSetMacro(Allow8BitBMP, svtkTypeBool);
  svtkGetMacro(Allow8BitBMP, svtkTypeBool);
  svtkBooleanMacro(Allow8BitBMP, svtkTypeBool);
  //@}
   ...
   */
  svtkGetMacro(Colors, unsigned char*);
  //@}

protected:
  svtkBMPReader();
  ~svtkBMPReader() override;

  unsigned char* Colors;
  short Depth;
  svtkTypeBool Allow8BitBMP;
...
private:
  svtkBMPReader(const svtkBMPReader&) = delete;
  void operator=(const svtkBMPReader&) = delete;
};
#endif
