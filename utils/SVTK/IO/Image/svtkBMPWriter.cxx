...
svtkStandardNewMacro(svtkBMPWriter);

svtkCxxSetObjectMacro(svtkBMPWriter, Result, svtkUnsignedCharArray);

svtkBMPWriter::svtkBMPWriter()
{
...
void svtkBMPWriter::WriteFileHeader(ostream* file, svtkImageData*, int wExt[6])
{
  long temp;
...
void svtkBMPWriter::WriteFile(ostream* file, svtkImageData* data, int extent[6], int wExtent[6])
{
  int idx1, idx2;
  ...
  {
    svtkErrorMacro("BMPWriter only accepts unsigned char scalars!");
    return;
  }
...
void svtkBMPWriter::MemoryWrite(int dim, svtkImageData* input, int wExt[6], svtkInformation* inInfo)
{
  ...
  svtkUnsignedCharArray* r = svtkUnsignedCharArray::New();
  r->SetNumberOfComponents(1);
  size_t alen = oss->str().length();
  r->SetNumberOfTuples(static_cast<svtkIdType>(alen));
...
//----------------------------------------------------------------------------
void svtkBMPWriter::PrintSelf(ostream& os, svtkIndent indent)
{
