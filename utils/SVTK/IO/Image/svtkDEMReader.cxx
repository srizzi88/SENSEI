...
svtkStandardNewMacro(svtkDEMReader);

#define SVTK_SW 0
#define SVTK_NW 1
#define SVTK_NE 2
#define SVTK_SE 3
#define SVTK_METERS_PER_FEET .305
#define SVTK_METERS_PER_ARC_SECOND 23.111

void ConvertDNotationToENotation(char* line);

svtkDEMReader::svtkDEMReader()
{
  int i, j;
...
//----------------------------------------------------------------------------
int svtkDEMReader::RequestInformation(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** svtkNotUsed(inputVector), svtkInformationVector* outputVector)
{
  ...
  svtkImageData::SetNumberOfScalarComponents(1, outInfo);
  svtkImageData::SetScalarType(SVTK_FLOAT, outInfo);
...
int svtkDEMReader::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** svtkNotUsed(inputVector), svtkInformationVector* outputVector)
{
  // get the data object
  svtkInformation* outInfo = outputVector->GetInformationObject(0);
  ...
  if (output->GetScalarType() != SVTK_FLOAT)
  {
    svtkErrorMacro("Execute: This source only outputs floats.");
    return 1;
  }
...
int svtkDEMReader::ReadTypeARecord()
{
  char record[1025];
  ...
  svtkDebugMacro(<< "reading DEM header: type A record");
  ...
  {
    elevationConversion = SVTK_METERS_PER_FEET;
  }
  else if (this->ElevationUnitOfMeasure == 3) // arc-seconds
  {
    elevationConversion = SVTK_METERS_PER_ARC_SECOND;
  }
...
void svtkDEMReader::ComputeExtentOriginAndSpacing(int extent[6], double origin[3], double spacing[3])
{
  ...
  //
  eastMost = this->GroundCoords[SVTK_NE][0];
  if (eastMost < this->GroundCoords[SVTK_SE][0])
  {
    eastMost = this->GroundCoords[SVTK_SE][0];
  }
  westMost = this->GroundCoords[SVTK_NW][0];
  if (westMost > this->GroundCoords[SVTK_SW][0])
  {
    westMost = this->GroundCoords[SVTK_SW][0];
  }
  northMost = this->GroundCoords[SVTK_NE][1];
  if (northMost < this->GroundCoords[SVTK_NW][1])
  {
    northMost = this->GroundCoords[SVTK_NW][1];
  }
  southMost = this->GroundCoords[SVTK_SW][1];
  if (southMost > this->GroundCoords[SVTK_SE][1])
  {
    southMost = this->GroundCoords[SVTK_SE][1];
  }
  ...
  {
    planeConversion = SVTK_METERS_PER_FEET;
  }
  else if (this->PlaneUnitOfMeasure == 3) // arc-seconds
  {
    planeConversion = SVTK_METERS_PER_ARC_SECOND;
  }

  //
  // get the origin from the ground coordinates
  //
  origin[0] = this->GroundCoords[SVTK_SW][0];
  origin[1] = this->GroundCoords[SVTK_SW][1];
...
int svtkDEMReader::ReadProfiles(svtkImageData* data)
{
  char record[145];
  float *outPtr, *ptr;
  ...
  svtkDebugMacro(<< "reading profiles");
...
const char* svtkDEMReader::GetElevationReferenceAsString()
{
...
void svtkDEMReader::PrintSelf(ostream& os, svtkIndent indent)
{
