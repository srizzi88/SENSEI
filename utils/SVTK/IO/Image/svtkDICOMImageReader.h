...
#ifndef svtkDICOMImageReader_h
#define svtkDICOMImageReader_h

#include "svtkIOImageModule.h" // For export macro
#include "svtkImageReader2.h"

class svtkDICOMImageReaderVector;
class DICOMParser;
class DICOMAppHelper;

class SVTKIOIMAGE_EXPORT svtkDICOMImageReader : public svtkImageReader2
{
public:
  //@{
  /**
   ...
   */
  static svtkDICOMImageReader* New();
  svtkTypeMacro(svtkDICOMImageReader, svtkImageReader2);
  //@}
  ...
  void SetFileName(const char* fn) override
  {
    ...
    this->svtkImageReader2::SetFileName(fn);
  }
  //@}
   ...
   */
  svtkGetStringMacro(DirectoryName);
  //@}
   ...
   */
  double* GetPixelSpacing() SVTK_SIZEHINT(3);
   ...
   * DICOMParser
   */
  float* GetImagePositionPatient() SVTK_SIZEHINT(3);
   ...
   */
  float* GetImageOrientationPatient() SVTK_SIZEHINT(6);
...
protected:
  ...
  // Constructor
  //
  svtkDICOMImageReader();

  //
  // Destructor
  //
  ~svtkDICOMImageReader() override;
  ...
  //
  // svtkDICOMImageReaderVector wants to be a PIMPL and it will be, but not quite yet.
  //
  svtkDICOMImageReaderVector* DICOMFileNames;
  char* DirectoryName;
...
private:
  svtkDICOMImageReader(const svtkDICOMImageReader&) = delete;
  void operator=(const svtkDICOMImageReader&) = delete;
};
