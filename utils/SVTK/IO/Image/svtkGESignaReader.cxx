...
svtkStandardNewMacro(svtkGESignaReader);

int svtkGESignaReader::CanReadFile(const char* fname)
{
...
void svtkGESignaReader::ExecuteInformation()
{
  ...
  FILE* fp = svtksys::SystemTools::Fopen(this->InternalFileName, "rb");
  if (!fp)
  {
  ...
  if (fread(&magic, 4, 1, fp) != 1)
  {
    svtkErrorMacro("GESignaReader error reading file: " << this->FileName
  ...
  if (magic != 0x494d4746)
  {
    svtkErrorMacro(<< "Unknown file type! Not a GE ximg file!");
    fclose(fp);
    return;
  }
  ...
  {
    svtkErrorMacro("GESignaReader error reading file: "
      << this->FileName << " Premature EOF while reading pixel offset.");
    fclose(fp);
    return;
  }
  svtkByteSwap::Swap4BE(&offset);
  ...
  svtkByteSwap::Swap4BE(&compression);
  ...
  svtkByteSwap::Swap4BE(&examHdrOffset);
  ...
  svtkByteSwap::Swap4BE(&seriesHdrOffset);
  ...
  svtkByteSwap::Swap4BE(&imgHdrOffset);
  ...
  svtkByteSwap::Swap2BE(&examnumber);
  ...
  svtkByteSwap::Swap2BE(&patientage);
  ...
  svtkByteSwap::Swap2BE(&patientsex);
...
static void svtkcopygenesisimage(FILE* infp, int width, int height, int compress, short* map_left,
...
static void svtkGESignaReaderUpdate2(
  svtkGESignaReader* self, unsigned short* outPtr, int* outExt, svtkIdType*)
{
  FILE* fp = svtksys::SystemTools::Fopen(self->GetInternalFileName(), "rb");
  if (!fp)
  {
    return;
  }
  ...
  if (fread(&magic, 4, 1, fp) != 1)
  {
    svtkGenericWarningMacro("GESignaReader error reading file: "
  ...
  if (magic != 0x494d4746)
  {
    svtkGenericWarningMacro(<< "Unknown file type! Not a GE ximg file!");
    fclose(fp);
    return;
  }
  ...
  { // packed/compacked
    ...
    svtkByteSwap::Swap4BE(&packHdrOffset);
    ...
    for (i = 0; i < height; i++)
    {
      ...
      svtkByteSwap::Swap2BE(leftMap.data() + i);
      ...
      svtkByteSwap::Swap2BE(widthMap.data() + i);
    }
  }
...
static void svtkGESignaReaderUpdate(
  svtkGESignaReader* self, svtkImageData* data, unsigned short* outPtr)
{
  ...
  for (idx2 = outExtent[4]; idx2 <= outExtent[5]; ++idx2)
  {
    ...
    // read in a PNG file
    svtkGESignaReaderUpdate2(self, outPtr2, outExtent, outIncr);
...
void svtkGESignaReader::ExecuteDataWithInformation(svtkDataObject* output, svtkInformation* outInfo)
{
  ...
  svtkGESignaReaderUpdate(this, data, (unsigned short*)(outPtr));
}

//----------------------------------------------------------------------------
void svtkGESignaReader::PrintSelf(ostream& os, svtkIndent indent)
{
