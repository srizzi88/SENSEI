...
svtkStandardNewMacro(svtkHDRReader);
...
//----------------------------------------------------------------------------
void svtkHDRReader::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
void svtkHDRReader::ExecuteInformation()
{
  ...
  // SVTK errors are handled in ReadHeaderData.
...
//----------------------------------------------------------------------------
int svtkHDRReader::CanReadFile(const char* fname)
{
  ...
  if (ifs.fail())
  {
    svtkErrorMacro(<< "Could not open file " << fname);
    return 0;
  }
...
void svtkHDRReader::ExecuteDataWithInformation(svtkDataObject* output, svtkInformation* outInfo)
{
...
//----------------------------------------------------------------------------
void svtkHDRReader::HDRReaderUpdate(svtkImageData* data, float* outPtr)
{
  ...
  if (this->FlippedX)
  {
    svtkNew<svtkImageFlip> flipXFilter;
  ...
  {
    // Permute X and Y
    svtkNew<svtkImagePermute> permuteAxisFilter;
...
void svtkHDRReader::ConvertAllDataFromRGBToXYZ(float* outPtr, int size)
{
...
bool svtkHDRReader::HDRReaderUpdateSlice(float* outPtr, int* outExt)
{
  this->OpenFile();
  ...
  while (nbLinesLeft > 0)
  {
    ...
    if ((rgbe[0] != 2) || (rgbe[1] != 2) || (rgbe[2] & 0x80))
    {
      ...
      {
        svtkErrorMacro(<< "HDRReader: First 4 bytes of the line " << totalNbLines - nbLinesLeft
                      << " are wrong");
        this->CloseFile();
        return false;
      }
      ...
      {
        svtkErrorMacro(<< "HDRReader : Bad line data");
        this->CloseFile();
        return false;
      }
    ...
    {
      svtkErrorMacro(<< "HDRReader: Wrong scanline width");
      this->CloseFile();
      return false;
    }
...
//----------------------------------------------------------------------------
bool svtkHDRReader::HasError(std::istream* is)
{
  if (!*is)
  {
    svtkErrorMacro(<< "HDRReader : Read error");
    this->CloseFile();
    return true;
  }
  return false;
...
int svtkHDRReader::GetWidth() const
{
  return this->DataExtent[1] - this->DataExtent[0] + 1;
}

int svtkHDRReader::GetHeight() const
{
  return this->DataExtent[3] - this->DataExtent[2] + 1;
}

//----------------------------------------------------------------------------
bool svtkHDRReader::ReadHeaderData()
{
  ...
  while (true)
  {
    ...
    if (strcmp(buffer, "FORMAT") == 0)
    {
      ...
      {
        this->Format = svtkHDRReader::FORMAT_32BIT_RLE_RGBE;
      }
      else if (format == "32-bit_rle_xyze")
      {
        this->Format = svtkHDRReader::FORMAT_32BIT_RLE_XYZE;
      }
    }
...
//----------------------------------------------------------------------------
void svtkHDRReader::FillOutPtrRLE(
...
//----------------------------------------------------------------------------
void svtkHDRReader::FillOutPtrNoRLE(
  int* outExt, float*& outPtr, std::vector<unsigned char>& lineBuffer)
{
  // Convert lineBuffer to RGB floats
  for (int i = outExt[0]; i <= outExt[1]; ++i)
...
//----------------------------------------------------------------------------
bool svtkHDRReader::ReadAllFileNoRLE(std::istream* is, float* outPtr, int decrPtr, int* outExt)
{
...
//----------------------------------------------------------------------------
bool svtkHDRReader::ReadLineRLE(std::istream* is, unsigned char* lineBufferPtr)
{
...
//----------------------------------------------------------------------------
void svtkHDRReader::RGBE2Float(unsigned char* rgbe, float& r, float& g, float& b)
{
...
//----------------------------------------------------------------------------
void svtkHDRReader::XYZ2RGB(const float convertMatrix[3][3], float& r, float& g, float& b)
{
