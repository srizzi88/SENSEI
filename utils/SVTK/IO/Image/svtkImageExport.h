...
=========================================================================*/
/**
 * @class   svtkImageExport
 * @brief   Export SVTK images to third-party systems.
 *
 * svtkImageExport provides a way of exporting image data at the end
 ...
 * of the SVTK pipeline to a third-party pipeline.  This interface
 * conforms to the interface of svtkImageImport.
...
#ifndef svtkImageExport_h
#define svtkImageExport_h

#include "svtkIOImageModule.h" // For export macro
#include "svtkImageAlgorithm.h"

class SVTKIOIMAGE_EXPORT svtkImageExport : public svtkImageAlgorithm
{
public:
  static svtkImageExport* New();
  svtkTypeMacro(svtkImageExport, svtkImageAlgorithm);
  void PrintSelf(ostream& os, svtkIndent indent) override;

  /**
   * Get the number of bytes required for the output C array.
   */
  svtkIdType GetDataMemorySize();
  ...
  int* GetDataDimensions() SVTK_SIZEHINT(3)
  {
  ...
  {
    return svtkImageScalarTypeNameMacro(this->GetDataScalarType());
  }
   ...
   */
  int* GetDataExtent() SVTK_SIZEHINT(6);
  void GetDataExtent(int* ptr);
  double* GetDataSpacing() SVTK_SIZEHINT(3);
  void GetDataSpacing(double* ptr);
  double* GetDataOrigin() SVTK_SIZEHINT(3);
  void GetDataOrigin(double* ptr);
  double* GetDataDirection() SVTK_SIZEHINT(9);
   ...
   */
  svtkBooleanMacro(ImageLowerLeft, svtkTypeBool);
  svtkGetMacro(ImageLowerLeft, svtkTypeBool);
  svtkSetMacro(ImageLowerLeft, svtkTypeBool);
  //@}
   ...
   * callbacks.  See further documentation in svtkImageImport.h.
   */
...
protected:
  ...
  svtkTypeBool ImageLowerLeft;
  int DataDimensions[3];
  void* ExportVoidPointer;

  svtkMTimeType LastPipelineMTime;

private:
  svtkImageExport(const svtkImageExport&) = delete;
  void operator=(const svtkImageExport&) = delete;
