...
svtkStandardNewMacro(svtkJPEGReader);
...
struct svtk_jpeg_error_mgr
{
  struct jpeg_error_mgr pub; /* "public" fields */
  jmp_buf setjmp_buffer;     /* for return to caller */
  svtkJPEGReader* JPEGReader;
  FILE* fp;
};

// this is called on jpeg error conditions
extern "C" void svtk_jpeg_error_exit(j_common_ptr cinfo)
{
  /* cinfo->err really points to a my_error_mgr struct, so coerce pointer */
  svtk_jpeg_error_mgr* err = reinterpret_cast<svtk_jpeg_error_mgr*>(cinfo->err);

  /* Return control to the setjmp point */
  longjmp(err->setjmp_buffer, 1);
}

extern "C" void svtk_jpeg_output_message(j_common_ptr cinfo)
{
  ...
  svtk_jpeg_error_mgr* err = reinterpret_cast<svtk_jpeg_error_mgr*>(cinfo->err);
  svtkWarningWithObjectMacro(err->JPEGReader, "libjpeg error: " << buffer);
}

extern "C" void jpg_null(j_decompress_ptr svtkNotUsed(cinfo)) {}

extern "C" boolean fill_input_buffer(j_decompress_ptr svtkNotUsed(cinfo))
{
  svtkGenericWarningMacro(<< "libjpeg error: unexpected end of JPEG data!");
  return TRUE;
}
...
void svtkJPEGReader::ExecuteInformation()
{
  // certain variables must be stored here for longjmp
  struct svtk_jpeg_error_mgr jerr;
  ...
  {
    jerr.fp = svtksys::SystemTools::Fopen(this->InternalFileName, "rb");
    if (!jerr.fp)
    {
      svtkErrorWithObjectMacro(this, "Unable to open file " << this->InternalFileName);
      return;
    }
  }
  else
  ...
  // for any jpeg error call svtk_jpeg_error_exit
  jerr.pub.error_exit = svtk_jpeg_error_exit;
  // for any output message call svtk_jpeg_output_message
  jerr.pub.output_message = svtk_jpeg_output_message;
  if (setjmp(jerr.setjmp_buffer))
  {
    ...
    if (jerr.fp)
    {
      ...
      svtkErrorWithObjectMacro(this, "libjpeg could not read file: " << this->InternalFileName);
    }
    else
    {
      svtkErrorWithObjectMacro(this,
...
template <class OT>
int svtkJPEGReaderUpdate2(svtkJPEGReader* self, OT* outPtr, int* outExt, svtkIdType* outInc, long)
{
  // certain variables must be stored here for longjmp
  struct svtk_jpeg_error_mgr jerr;
  ...
  {
    jerr.fp = svtksys::SystemTools::Fopen(self->GetInternalFileName(), "rb");
    if (!jerr.fp)
    {
      return 1;
    }
  ...
  svtkIdType rowbytes = cinfo.output_components * cinfo.output_width;
...
template <class OT>
void svtkJPEGReaderUpdate(svtkJPEGReader* self, svtkImageData* data, OT* outPtr)
{
  ...
  for (idx2 = outExtent[4]; idx2 <= outExtent[5]; ++idx2)
  {
    ...
    // read in a JPEG file
    if (svtkJPEGReaderUpdate2(self, outPtr2, outExtent, outIncr, pixSize) == 2)
    {
      const char* fn = self->GetInternalFileName();
      svtkErrorWithObjectMacro(self, "libjpeg could not read file: " << fn);
    }
...
void svtkJPEGReader::ExecuteDataWithInformation(svtkDataObject* output, svtkInformation* outInfo)
{
  ...
  {
    svtkTemplateMacro(svtkJPEGReaderUpdate(this, data, (SVTK_TT*)(outPtr)));
    default:
      svtkErrorMacro(<< "UpdateFromFile: Unknown data type");
  }
}

int svtkJPEGReader::CanReadFile(const char* fname)
{
  // certain variables must be stored here for longjmp
  struct svtk_jpeg_error_mgr jerr;
  jerr.JPEGReader = this;
  ...
  // open the file
  jerr.fp = svtksys::SystemTools::Fopen(fname, "rb");
  if (!jerr.fp)
  {
    return 0;
  }
  ...
  // for any jpeg error call svtk_jpeg_error_exit
  jerr.pub.error_exit = svtk_jpeg_error_exit;
  // for any output message call svtk_jpeg_error_exit
  jerr.pub.output_message = svtk_jpeg_error_exit;
...
//----------------------------------------------------------------------------
void svtkJPEGReader::PrintSelf(ostream& os, svtkIndent indent)
{
