...
svtkStandardNewMacro(svtkJPEGWriter);

svtkCxxSetObjectMacro(svtkJPEGWriter, Result, svtkUnsignedCharArray);

svtkJPEGWriter::svtkJPEGWriter()
{
...
void svtkJPEGWriter::Write()
{
  ...
  int* wExtent;
  wExtent = this->GetInputInformation(0, 0)->Get(svtkStreamingDemandDrivenPipeline::WHOLE_EXTENT());
...
extern "C"
{
  static void svtkJPEGWriteToMemoryInit(j_compress_ptr cinfo)
  {
    svtkJPEGWriter* self = svtkJPEGWriter::SafeDownCast(static_cast<svtkObject*>(cinfo->client_data));
    if (self)
    {
...
extern "C"
{
  static boolean svtkJPEGWriteToMemoryEmpty(j_compress_ptr cinfo)
  {
    ...
    if (self)
    {
      ...
      svtkIdType oldSize = uc->GetSize();
      uc->Resize(static_cast<svtkIdType>(oldSize + oldSize / 2));
      // Resize do grow the array but it is not the size we expect
      svtkIdType newSize = uc->GetSize();
...
extern "C"
{
  static void svtkJPEGWriteToMemoryTerm(j_compress_ptr cinfo)
  {
    svtkJPEGWriter* self = svtkJPEGWriter::SafeDownCast(static_cast<svtkObject*>(cinfo->client_data));
    if (self)
    {
      ...
      svtkIdType realSize = uc->GetSize() - static_cast<svtkIdType>(cinfo->dest->free_in_buffer);
...
struct SVTK_JPEG_ERROR_MANAGER
{
  struct jpeg_error_mgr pub;
  jmp_buf setjmp_buffer;
};

typedef struct SVTK_JPEG_ERROR_MANAGER* SVTK_JPEG_ERROR_PTR;

extern "C"
{
  /* The JPEG library does not expect the error function to return.
     Therefore we must use this ugly longjmp call.  */
  void SVTK_JPEG_ERROR_EXIT(j_common_ptr cinfo)
  {
    SVTK_JPEG_ERROR_PTR jpegErr = reinterpret_cast<SVTK_JPEG_ERROR_PTR>(cinfo->err);
...
#endif
void svtkJPEGWriter::WriteSlice(svtkImageData* data, int* uExtent)
{
  // Call the correct templated function for the output
  unsigned int ui;

  // Call the correct templated function for the input
  if (data->GetScalarType() != SVTK_UNSIGNED_CHAR)
  {
    svtkWarningMacro("JPEGWriter only supports unsigned char input");
    return;
  }

  if (data->GetNumberOfScalarComponents() > MAX_COMPONENTS)
  {
    svtkErrorMacro("Exceed JPEG limits for number of components ("
  ...
  struct SVTK_JPEG_ERROR_MANAGER jerr;
  this->TempFP = nullptr;
  ...
  if (this->WriteToMemory)
  {
    ...
    compressionDestination.init_destination = svtkJPEGWriteToMemoryInit;
    compressionDestination.empty_output_buffer = svtkJPEGWriteToMemoryEmpty;
    compressionDestination.term_destination = svtkJPEGWriteToMemoryTerm;
  ...
  if (!this->WriteToMemory)
  {
    ...
    {
      this->ErrorCode = svtkErrorCode::OutOfDiskSpaceError;
      fclose(this->TempFP);
      return;
    }
  }
...
void svtkJPEGWriter::PrintSelf(ostream& os, svtkIndent indent)
{
