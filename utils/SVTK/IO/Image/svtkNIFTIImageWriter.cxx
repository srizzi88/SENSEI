/*=========================================================================
  ...
  Module:    svtkNIFTIImageWriter.cxx
...
svtkStandardNewMacro(svtkNIFTIImageWriter);
svtkCxxSetObjectMacro(svtkNIFTIImageWriter, QFormMatrix, svtkMatrix4x4);
svtkCxxSetObjectMacro(svtkNIFTIImageWriter, SFormMatrix, svtkMatrix4x4);
svtkCxxSetObjectMacro(svtkNIFTIImageWriter, NIFTIHeader, svtkNIFTIImageHeader);

//----------------------------------------------------------------------------
svtkNIFTIImageWriter::svtkNIFTIImageWriter()
{
  ...
  this->NIFTIVersion = 0;
  // Default description is "SVTKX.Y.Z"
  const char* version = svtkVersion::GetSVTKVersion();
  size_t l = strlen(version);
  this->Description = new char[l + 4];
  memcpy(this->Description, "SVTK", 3);
...
//----------------------------------------------------------------------------
svtkNIFTIImageHeader* svtkNIFTIImageWriter::GetNIFTIHeader()
{
  if (!this->NIFTIHeader)
  {
...
//----------------------------------------------------------------------------
void svtkNIFTIImageWriter::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
char* svtkNIFTIImageWriter::ReplaceExtension(
...
// - NIFTI data type is set from SVTK data type
// - NIFTI pixdim set from SVTK spacing
// - dimensionality is:
...
void svtkNIFTIImageWriterSetInformation(nifti_2_header* hdr, svtkInformation* info)
{
  ...
  double spacing[3];
  info->Get(svtkDataObject::SPACING(), spacing);

  int scalarType = scalarInfo->Get(svtkDataObject::FIELD_ARRAY_TYPE());
  int numComponents = scalarInfo->Get(svtkDataObject::FIELD_NUMBER_OF_COMPONENTS());

  // map SVTK type to NIFTI type and bits
  static const int typeMap[][3] = {
#if SVTK_TYPE_CHAR_IS_SIGNED
    { SVTK_CHAR, NIFTI_TYPE_INT8, 8 },
#else
    { SVTK_CHAR, NIFTI_TYPE_UINT8, 8 },
#endif
    { SVTK_SIGNED_CHAR, NIFTI_TYPE_INT8, 8 },
    { SVTK_UNSIGNED_CHAR, NIFTI_TYPE_UINT8, 8 },
    { SVTK_SHORT, NIFTI_TYPE_INT16, 16 },
    { SVTK_UNSIGNED_SHORT, NIFTI_TYPE_UINT16, 16 },
    { SVTK_INT, NIFTI_TYPE_INT32, 32 },
    { SVTK_UNSIGNED_INT, NIFTI_TYPE_UINT32, 32 },
#if SVTK_SIZEOF_LONG == 4
    { SVTK_LONG, NIFTI_TYPE_INT32, 32 },
    { SVTK_UNSIGNED_LONG, NIFTI_TYPE_UINT32, 32 },
#else
    { SVTK_LONG, NIFTI_TYPE_INT64, 64 },
    { SVTK_UNSIGNED_LONG, NIFTI_TYPE_UINT64, 64 },
#endif
    { SVTK_LONG_LONG, NIFTI_TYPE_INT64, 64 },
    { SVTK_UNSIGNED_LONG_LONG, NIFTI_TYPE_UINT64, 64 },
#if !defined(SVTK_LEGACY_REMOVE)
    { SVTK___INT64, NIFTI_TYPE_INT64, 64 },
    { SVTK_UNSIGNED___INT64, NIFTI_TYPE_UINT64, 64 },
#endif
    { SVTK_FLOAT, NIFTI_TYPE_FLOAT32, 32 },
    { SVTK_DOUBLE, NIFTI_TYPE_FLOAT64, 64 },
    { 0, 0, 0 }
  };
...
void svtkNIFTIImageWriterSetQForm(nifti_2_header* hdr, double mmat[16], double qfac)
{
  double rmat[3][3];
  rmat[0][0] = mmat[0];
  rmat[0][1] = mmat[1];
  ...
  double quat[4];
  svtkMath::Matrix3x3ToQuaternion(rmat, quat);
  if (quat[0] < 0)
  {
    quat[0] = -quat[0];
    quat[1] = -quat[1];
...
void svtkNIFTIImageWriterSetSForm(nifti_2_header* hdr, double mmat[16], double qfac)
{
  if (qfac < 0)
  {
...
void svtkNIFTIImageWriterMatrix(double mmat[16], svtkMatrix4x4* matrix, const double origin[3])
{
  ...
  if (matrix)
  {
    ...
    svtkMatrix4x4::DeepCopy(mmat, matrix);
  }
  else
  {
    svtkMatrix4x4::Identity(mmat);
  }
...
//----------------------------------------------------------------------------
int svtkNIFTIImageWriter::GenerateHeader(svtkInformation* info, bool singleFile)
{
  // create the header
  nifti_2_header hdr;
  int version = 0;
  ...
  {
    this->OwnHeader = svtkNIFTIImageHeader::New();
  }
  else
  {
  ...
  svtkNIFTIImageWriterSetInformation(&hdr, info);
  if (hdr.datatype == 0)
  {
    svtkErrorMacro("Illegal data type for NIFTI file.");
    return 0;
  }
  ...
  // set the header size
  hdr.sizeof_hdr =
    (version == 2 ? svtkNIFTIImageHeader::NIFTI2HeaderSize : svtkNIFTIImageHeader::NIFTI1HeaderSize);
  ...
  double origin[3];
  info->Get(svtkDataObject::ORIGIN(), origin);

  if (this->QFormMatrix || (origin[0] != 0 || origin[1] != 0 || origin[2] != 0))
  {
    hdr.qform_code = 1; // SCANNER_ANAT
    double mat16[16];
    svtkNIFTIImageWriterMatrix(mat16, this->QFormMatrix, origin);
    svtkNIFTIImageWriterSetQForm(&hdr, mat16, qfac);
  }
  ...
  if (this->SFormMatrix)
  {
    ...
    double mat16[16];
    svtkNIFTIImageWriterMatrix(mat16, this->SFormMatrix, origin);
    svtkNIFTIImageWriterSetSForm(&hdr, mat16, qfac);
  }
  ...
  if (this->TimeDimension)
  {
    ...
    if (hdr.dim[5] % tdim != 0)
    {
      svtkErrorMacro("Number of components in the image data must be "
...
//----------------------------------------------------------------------------
int svtkNIFTIImageWriter::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* svtkNotUsed(outputVector))
{
  ...
  svtkImageData* data = svtkImageData::SafeDownCast(info->Get(svtkDataObject::DATA_OBJECT()));
  ...
  char* hdrname = svtkNIFTIImageWriter::ReplaceExtension(filename, ".img", ".hdr");
  char* imgname = svtkNIFTIImageWriter::ReplaceExtension(filename, ".hdr", ".img");

  svtkDebugMacro(<< "Writing NIFTI file " << hdrname);
  ...
  else
  {
    ...
    if (extent[1] - extent[0] + 1 > SVTK_SHORT_MAX || extent[3] - extent[2] + 1 > SVTK_SHORT_MAX ||
      extent[5] - extent[4] + 1 > SVTK_SHORT_MAX)
    {
      svtkErrorMacro("Image too large to store in NIFTI-1 format");
      delete[] hdrname;
      delete[] imgname;
      return 0;
    }
  ...
  {
    ufile = svtksys::SystemTools::Fopen(hdrname, "wb");
  }
  ...
  else if (!this->ErrorCode)
  {
    ...
    {
      fclose(ufile);
      ufile = svtksys::SystemTools::Fopen(imgname, "wb");
    }
  }

  if (!file && !ufile)
  {
    svtkErrorMacro("Cannot open file " << imgname);
    this->SetErrorCode(svtkErrorCode::CannotOpenFileError);
    return 0;
  }
  ...
  if (this->ErrorCode == svtkErrorCode::OutOfDiskSpaceError)
  {
    ...
    svtkErrorMacro("Out of disk space, removing incomplete file " << imgname);
    svtksys::SystemTools::RemoveFile(imgname);
    if (!singleFile)
    {
      svtksys::SystemTools::RemoveFile(hdrname);
    }
  }
