...
#ifndef svtkNIFTIImageWriter_h
#define svtkNIFTIImageWriter_h
...
class SVTKIOIMAGE_EXPORT svtkNIFTIImageWriter : public svtkImageWriter
{
public:
  //@{
  /**
   ...
   */
  static svtkNIFTIImageWriter* New();
  svtkTypeMacro(svtkNIFTIImageWriter, svtkImageWriter);
  //@}
   ...
   */
  svtkSetMacro(NIFTIVersion, int);
  svtkGetMacro(NIFTIVersion, int);
  //@}

  //@{
  /**
   * Set a short description (max 80 chars) of how the file was produced.
   * The default description is "SVTKX.Y" where X.Y is the SVTK version.
   */
  svtkSetStringMacro(Description);
  svtkGetStringMacro(Description);
  //@}
   ...
   */
  svtkGetMacro(TimeDimension, int);
  svtkSetMacro(TimeDimension, int);
  svtkGetMacro(TimeSpacing, double);
  svtkSetMacro(TimeSpacing, double);
  //@}
   ...
   * as compared to SVTK. Use with caution.
   */
  svtkSetMacro(QFac, double);
  svtkGetMacro(QFac, double);
  //@}
   ...
   */
  void SetQFormMatrix(svtkMatrix4x4*);
   ...
   */
  void SetSFormMatrix(svtkMatrix4x4*);
   ...
   */
  void SetNIFTIHeader(svtkNIFTIImageHeader* hdr);
  svtkNIFTIImageHeader* GetNIFTIHeader();
  //@}

protected:
  svtkNIFTIImageWriter();
  ~svtkNIFTIImageWriter() override;

  /**
   * Generate the header information for the file.
   */
  int GenerateHeader(svtkInformation* info, bool singleFile);
  ...
  svtkNIFTIImageHeader* OwnHeader;
  int NIFTIVersion;
  //@}
...
private:
  svtkNIFTIImageWriter(const svtkNIFTIImageWriter&) = delete;
  void operator=(const svtkNIFTIImageWriter&) = delete;
};

#endif // svtkNIFTIImageWriter_h
