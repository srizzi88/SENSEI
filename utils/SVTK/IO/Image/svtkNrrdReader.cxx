...
//-----------------------------------------------------------------------------
static int NrrdType2SVTKType(std::string nrrdType)
{
  ...
  {
    svtkGenericWarningMacro(<< "Reading blocks not supported.");
    return SVTK_VOID;
  }
  else
  {
    svtkGenericWarningMacro(<< "Unknown type: '" << nrrdType << "'");
    return SVTK_VOID;
  }
}
...
svtkObjectFactoryNewMacro(svtkNrrdReader);

//-----------------------------------------------------------------------------
svtkNrrdReader::svtkNrrdReader()
{
  this->DataFiles = svtkStringArray::New();
...
void svtkNrrdReader::PrintSelf(ostream& os, svtkIndent indent)
{
...
//-----------------------------------------------------------------------------
int svtkNrrdReader::CanReadFile(const char* filename)
{
  svtksys::ifstream file(filename, ios::in | ios::binary);
...
//-----------------------------------------------------------------------------
int svtkNrrdReader::RequestInformation(
...
//-----------------------------------------------------------------------------
int svtkNrrdReader::ReadHeaderInternal(svtkCharArray* headerBuffer)
{
  if (!this->FileName)
  {
  ...
  svtkIdType buffersize = file.gcount();
  ...
  while ((s = strchr(s + 1, '\n')) != nullptr)
  {
    ...
    {
      s[2] = '\0';
      headerBuffer->SetNumberOfTuples(static_cast<svtkIdType>(s + 3 - bufferStart));
      break;
    }
    if ((s[1] == '\r') && (s[2] == '\n'))
    {
      s[3] = '\0';
      headerBuffer->SetNumberOfTuples(static_cast<svtkIdType>(s + 4 - bufferStart));
      break;
    }
  }
...
//-----------------------------------------------------------------------------
int svtkNrrdReader::ReadHeader(svtkCharArray* headerBuffer)
{
  ...
  {
    svtkErrorMacro(<< this->FileName << " is not a nrrd file.");
    return 0;
  }
  ...
  while (1)
  {
    ...
    if (delm != line.npos)
    {
      ...
      {
        this->DataScalarType = NrrdType2SVTKType(description);
        if (this->DataScalarType == SVTK_VOID)
          return 0;
      }
      else if (field == "encoding")
      {
        ...
        {
          svtkErrorMacro(<< "Unsupported encoding: " << description);
          return 0;
        }
      }
      ...
      else if (field == "space")
      {
        ...
        {
          svtkErrorMacro(<< "Time in NRRD array not supported (yet).");
          return 0;
        }
      ...
      else if (field == "endian")
      {
        ...
        {
          svtkErrorMacro(<< "Unknown endian: '" << description << "'");
          return 0;
        }
      }
      else if ((field == "line skip") || (field == "lineskip"))
      {
        ...
        {
          svtkErrorMacro(<< "line skip not supported");
          return 0;
        }
      }
      else if ((field == "byte skip") || (field == "byteskip"))
      {
        if (atoi(description.c_str()) != 0)
        {
          svtkErrorMacro(<< "byte skip not supported");
          return 0;
        }
      }
      ...
      {
        svtkWarningMacro(<< "Unknown field: '" << field << "'");
      }
      continue;
    }
  ...
  // SVTK really needs to know the difference.  Here we are going to guess.
  ...
  if (this->DataFiles->GetNumberOfValues() > 0)
  {
    if (this->DataFiles->GetNumberOfValues() > 1)
    {
      ...
      {
        svtkErrorMacro(<< "Data split into multiple files is only supported"
    ...
    std::string parentDir = svtksys::SystemTools::GetParentDirectory(this->FileName);
    for (svtkIdType i = 0; i < this->DataFiles->GetNumberOfValues(); i++)
    {
      std::string relativePath = this->DataFiles->GetValue(i);
      std::string fullPath = svtksys::SystemTools::CollapseFullPath(relativePath, parentDir.c_str());
...
//=============================================================================
int svtkNrrdReader::RequestData(
  svtkInformation* request, svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  {
    svtkImageData* outputData = svtkImageData::GetData(outputVector);
    this->AllocateOutputData(outputData, outputVector->GetInformationObject(0));
    if (outputData == nullptr)
    {
      svtkErrorMacro(<< "Data not created correctly?");
      return 0;
    }
  ...
  {
    svtkErrorMacro(<< "Bad encoding set");
    result = 0;
  }
...
//=============================================================================
template <typename T>
int svtkNrrdReaderReadDataAsciiTemplate(svtkNrrdReader* self, svtkImageData* output, T* outBuffer)
{
  ...
  svtkIdType fileDataIncrements[3];
  ...
  svtkStringArray* filenames = self->GetFileNames();
  svtkStdString filename = self->GetFileName();
  ...
  if (self->GetFileDimensionality() == 3)
  {
    ...
    if (file.fail())
    {
      svtkErrorWithObjectMacro(self, "Could not open file " << filename);
      return 0;
    }
    // Skip to start of first slab.
    for (svtkIdType skip = 0; skip < fileDataIncrements[2] * outExtent[4]; skip++)
    {
      T dummy;
      file >> dummy;
    }
  ...
  for (fileDataIndex[2] = outExtent[4]; fileDataIndex[2] <= outExtent[5]; fileDataIndex[2]++)
  {
    ...
    // Skip unused rows.
    for (svtkIdType skip = 0; skip < fileDataIncrements[1] * outExtent[2]; skip++)
    {
      T dummy;
      file >> dummy;
    }
    ...
    {
      // Skip unused columns.
      for (svtkIdType skip = 0; skip < fileDataIncrements[0] * outExtent[0]; skip++)
      {
        T dummy;
        file >> dummy;
      }
      ...
      // Skip unused columns.
      for (svtkIdType skip = 0; skip < fileDataIncrements[0] * (fileDataExtent[1] - outExtent[1]);
           skip++)
      {
        T dummy;
        file >> dummy;
    ...
    // Skip unused rows.
    for (svtkIdType skip = 0; skip < fileDataIncrements[1] * (fileDataExtent[2] - outExtent[2]);
         skip++)
    {
      T dummy;
      file >> dummy;
...
//=============================================================================
int svtkNrrdReader::ReadDataAscii(svtkImageData* output)
{
  void* outBuffer = output->GetScalarPointer();
  switch (output->GetScalarType())
  {
    svtkTemplateMacro(svtkNrrdReaderReadDataAsciiTemplate(this, output, (SVTK_TT*)(outBuffer)));
    default:
