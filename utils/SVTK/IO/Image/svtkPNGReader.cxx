...
svtkStandardNewMacro(svtkPNGReader);
...
class svtkPNGReader::svtkInternals
{
public:
  ...
  bool IsHeaderValid(unsigned char header[])
  {
    ...
    if (!is_png)
    {
      svtkErrorWithObjectMacro(nullptr, << "Unknown file type! Not a PNG file!");
    }
    return is_png;
  }
  ...
  bool CheckFileHeader(FILE* fp)
  {
    ...
    {
      svtkErrorWithObjectMacro(nullptr,
  ...
  bool CheckBufferHeader(const unsigned char* buffer, svtkIdType length)
  {
    unsigned char header[8];
    if (length < 8)
    {
      svtkErrorWithObjectMacro(nullptr, "MemoryBuffer is too short, could not read the header");
      return false;
    }
  ...
  bool CreateLibPngStructs(png_structp& pngPtr, png_infop& infoPtr, png_infop& endInfo)
  {
    ...
    if (!pngPtr)
    {
      svtkErrorWithObjectMacro(nullptr, "Out of memory.");
      return false;
    }
    ...
    if (!endInfo)
    {
      ...
      svtkErrorWithObjectMacro(nullptr, "Unable to read PNG file!");
      return false;
    }
    return true;
  }

  void InitLibPngInput(svtkPNGReader* self, png_structp pngPtr, MemoryBufferStream* stream, FILE* fp)
  {
...
//----------------------------------------------------------------------------
void svtkPNGReader::ExecuteInformation()
{
  ...
  if (this->GetMemoryBuffer())
  {
    ...
    {
      svtkErrorMacro("Invalid MemoryBuffer header: not a PNG file");
      return;
    }
  }
  else
  {
    ...
    {
      svtkErrorMacro("Invalid file header: not a PNG file");
      fclose(fp);
      return;
    }
  }
...
//----------------------------------------------------------------------------
template <class OT>
void svtkPNGReader::svtkPNGReaderUpdate2(OT* outPtr, int* outExt, svtkIdType* outInc, long pixSize)
{
  svtkPNGReader::svtkInternals* impl = this->Internals;
  unsigned int ui;
  int i;
  FILE* fp = nullptr;
  MemoryBufferStream stream;
...
template <class OT>
void svtkPNGReader::svtkPNGReaderUpdate(svtkImageData* data, OT* outPtr)
{
  ...
  for (idx2 = outExtent[4]; idx2 <= outExtent[5]; ++idx2)
  {
    ...
    // read in a PNG file
    this->svtkPNGReaderUpdate2(outPtr2, outExtent, outIncr, pixSize);
...
void svtkPNGReader::ExecuteDataWithInformation(svtkDataObject* output, svtkInformation* outInfo)
{
  svtkImageData* data = this->AllocateOutputData(output, outInfo);

  if (!this->GetMemoryBuffer() && this->InternalFileName == nullptr)
  {
    svtkErrorMacro(<< "Either a FileName, FilePrefix or MemoryBuffer must be specified.");
    return;
  }
  ...
  {
    svtkTemplateMacro(this->svtkPNGReaderUpdate(data, (SVTK_TT*)(outPtr)));
    default:
...
//----------------------------------------------------------------------------
int svtkPNGReader::CanReadFile(const char* fname)
{
...
//----------------------------------------------------------------------------
void svtkPNGReader::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
void svtkPNGReader::GetTextChunks(const char* key, int beginEndIndex[2])
{
  this->Internals->GetTextChunks(key, beginEndIndex);
}

//----------------------------------------------------------------------------
const char* svtkPNGReader::GetTextKey(int index)
{
  return this->Internals->TextKeyValue[index].first.c_str();
}

//----------------------------------------------------------------------------
const char* svtkPNGReader::GetTextValue(int index)
{
  return this->Internals->TextKeyValue[index].second.c_str();
}

size_t svtkPNGReader::GetNumberOfTextChunks()
{
