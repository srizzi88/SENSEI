/*=========================================================================
  ...
  Module:    svtkPostScriptWriter.cxx
...
=========================================================================*/
#include "svtkPostScriptWriter.h"

#include "svtkImageData.h"
#include "svtkObjectFactory.h"
#include "svtkPointData.h"

svtkStandardNewMacro(svtkPostScriptWriter);

#define SVTK_MARGIN 0.95

void svtkPostScriptWriter::WriteFileTrailer(ostream* file, svtkImageData* svtkNotUsed(cache))
{
  *file << "\ngrestore\nshowpage\n%%%%Trailer\n";
}

void svtkPostScriptWriter::WriteFileHeader(ostream* file, svtkImageData* cache, int wExt[6])
{
  ...
  if (scols > pagewid * SVTK_MARGIN || srows > pagehgt * SVTK_MARGIN)
  {
    if (scols > pagewid * SVTK_MARGIN)
    {
      scale = scale * (pagewid * SVTK_MARGIN / scols);
      scols = (int)(scale * cols * pixfac);
      srows = (int)(scale * rows * pixfac);
    }
    if (srows > pagehgt * SVTK_MARGIN)
    {
      scale = scale * (pagehgt * SVTK_MARGIN / srows);
      scols = (int)(scale * cols * pixfac);
      srows = (int)(scale * rows * pixfac);
    }
  }
  ...
  *file << "} bind def\n";
  ...
  {
    svtkWarningMacro(" svtkPostScriptWriter only supports 1 and 3 component images");
  }
...
void svtkPostScriptWriter::WriteFile(
  ostream* file, svtkImageData* data, int extent[6], int wExtent[6])
{
  ...
  switch (data->GetScalarType())
  {
    case SVTK_UNSIGNED_CHAR:
      break;
    default:
      svtkErrorMacro("PostScriptWriter only accepts unsigned char scalars!");
      return;
  }
...
//----------------------------------------------------------------------------
void svtkPostScriptWriter::PrintSelf(ostream& os, svtkIndent indent)
{
