...
int TestImportExportOBJ(int argc, char* argv[])
{
  ...
  auto renderWindow = svtkSmartPointer<svtkRenderWindow>::New();
  ...
  if (extension == "3ds")
  {
    auto importer = svtkSmartPointer<svtk3DSImporter>::New();
  ...
  {
    auto importer = svtkSmartPointer<svtkGLTFImporter>::New();
    importer->SetFileName(argv[1]);
    importer->SetRenderWindow(renderWindow);
    renderWindow = importer->GetRenderWindow();
    renderer = importer->GetRenderer();
  ...
  auto exporter = svtkSmartPointer<svtkOBJExporter>::New();
