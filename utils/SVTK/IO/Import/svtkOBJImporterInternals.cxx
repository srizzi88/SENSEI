/*=========================================================================
  ...
  Module:    svtkOBJImporterInternals.cxx
...
void obj_set_material_defaults(svtkOBJImportedMaterial* mtl)
{
  ...
  if (localVerbosity > 0)
  {
    svtkGenericWarningMacro("Created a default svtkOBJImportedMaterial, texture filename is "
...
void checkTextureMapFile(svtkOBJImportedMaterial* current_mtl, std::string& texturePath)
{
  // try texture as specified
  bool bFileExistsNoPath = svtksys::SystemTools::FileExists(current_mtl->texture_filename);
  std::vector<std::string> path_and_file(2);
  path_and_file[0] = texturePath;
  path_and_file[1] = std::string(current_mtl->texture_filename);
  std::string joined = svtksys::SystemTools::JoinPath(path_and_file);
  bool bFileExistsInPath = svtksys::SystemTools::FileExists(joined);
  // if the file does not exist and it has a png extension try for jpg instead
  if (!(bFileExistsNoPath || bFileExistsInPath))
  {
    if (svtksys::SystemTools::GetFilenameLastExtension(current_mtl->texture_filename) == ".png")
    {
      // try jpg
      std::string jpgName =
        svtksys::SystemTools::GetFilenameWithoutLastExtension(current_mtl->texture_filename) +
        ".jpg";
      bFileExistsNoPath = svtksys::SystemTools::FileExists(jpgName);
      path_and_file[0] = texturePath;
      path_and_file[1] = jpgName;
      joined = svtksys::SystemTools::JoinPath(path_and_file);
      bFileExistsInPath = svtksys::SystemTools::FileExists(joined);
    ...
    if (!(bFileExistsNoPath || bFileExistsInPath))
    {
      svtkGenericWarningMacro(<< "mtl file " << current_mtl->name
...
bool tokenGetString(size_t& t, std::vector<Token>& tokens, std::string& result)
{
  ...
  {
    svtkGenericWarningMacro("bad syntax");
    return false;
  }
...
bool tokenGetTexture(size_t& t, std::vector<Token>& tokens, svtkOBJImportedMaterial* current_mtl,
...
std::vector<svtkOBJImportedMaterial*> svtkOBJPolyDataProcessor::ParseOBJandMTL(
  std::string Filename, int& result_code)
{
  std::vector<svtkOBJImportedMaterial*> listOfMaterials;
  result_code = 0;
  ...
  svtksys::ifstream in(Filename.c_str(), std::ios::in | std::ios::binary);
  if (!in)
  {
    return listOfMaterials;
  }
  ...
  svtkOBJImportedMaterial* current_mtl = nullptr;
  for (size_t t = 0; t < tokens.size(); ++t)
  {
    if (tokens[t].Type == Token::Number)
    {
      svtkErrorMacro("Number found outside of a command or option on token# "
    ...
    {
      current_mtl = (new svtkOBJImportedMaterial);
    ...
    // svtkErrorMacro("Unknown command in mtl file at token# " <<
  ...
  svtkRenderWindow* renderWindow, svtkRenderer* renderer, svtkOBJPolyDataProcessor* reader)
{
  ...
  {
    svtkErrorWithObjectMacro(reader, "RenderWindow is null, failure!");
    return;
  }
  if (nullptr == (renderer))
  {
    svtkErrorWithObjectMacro(reader, "Renderer is null, failure!");
    return;
  }
  if (nullptr == (reader))
  {
    svtkErrorWithObjectMacro(reader, "svtkOBJPolyDataProcessor is null, failure!");
    return;
  }
  ...
  std::map<std::string, svtkSmartPointer<svtkTexture> > knownTextures;

  for (int port_idx = 0; port_idx < reader->GetNumberOfOutputPorts(); port_idx++)
  {
    svtkPolyData* objPoly = reader->GetOutput(port_idx);
    ...
    svtkDebugWithObjectMacro(reader,
    ...
    {
      svtkSmartPointer<svtkTIFFReader> tex_tiff_Loader = svtkSmartPointer<svtkTIFFReader>::New();
      svtkSmartPointer<svtkBMPReader> tex_bmp_Loader = svtkSmartPointer<svtkBMPReader>::New();
      svtkSmartPointer<svtkJPEGReader> tex_jpg_Loader = svtkSmartPointer<svtkJPEGReader>::New();
      svtkSmartPointer<svtkPNGReader> tex_png_Loader = svtkSmartPointer<svtkPNGReader>::New();
      ...
      if (!textureFilename.empty())
      {
        if (bIsReadableJPEG)
        {
          ...
          svtkSmartPointer<svtkTexture> svtk_texture = svtkSmartPointer<svtkTexture>::New();
          svtk_texture->AddInputConnection(tex_jpg_Loader->GetOutputPort());
          actor->SetTexture(svtk_texture);
          knownTextures[textureFilename] = svtk_texture;
        }
        else if (bIsReadablePNG)
        {
          ...
          svtkSmartPointer<svtkTexture> svtk_texture = svtkSmartPointer<svtkTexture>::New();
          svtk_texture->AddInputConnection(tex_png_Loader->GetOutputPort());
          actor->SetTexture(svtk_texture);
          knownTextures[textureFilename] = svtk_texture;
        }
        else if (bIsReadableBMP)
        {
          ...
          svtkSmartPointer<svtkTexture> svtk_texture = svtkSmartPointer<svtkTexture>::New();
          svtk_texture->AddInputConnection(tex_bmp_Loader->GetOutputPort());
          actor->SetTexture(svtk_texture);
          knownTextures[textureFilename] = svtk_texture;
        }
        else if (bIsReadableTIFF)
        {
          ...
          svtkSmartPointer<svtkTexture> svtk_texture = svtkSmartPointer<svtkTexture>::New();
          svtk_texture->AddInputConnection(tex_tiff_Loader->GetOutputPort());
          actor->SetTexture(svtk_texture);
          knownTextures[textureFilename] = svtk_texture;
        }
        else
    ...
    svtkSmartPointer<svtkProperty> properties = svtkSmartPointer<svtkProperty>::New();

    svtkOBJImportedMaterial* raw_mtl_data = reader->GetMaterial(port_idx);
    if (raw_mtl_data)
    {
    ...
    // properties->ShadingOn(); // use ShadingOn() if loading svtkMaterial from xml
...
svtkOBJImportedMaterial::svtkOBJImportedMaterial()
{
  this->name = "x";
