...
struct svtkVRMLAllocator
{
  ...
  static svtkHeap* Heap;
};

template <class T>
class svtkVRMLVectorType
{
public:
  svtkVRMLVectorType(int usenew = 0)
    : UseNew(usenew)
  {
    this->Init();
  }

  ~svtkVRMLVectorType(void)
  {
    if (this->UseNew)
    {
      delete[] this->Data;
  ...
  void Init()
  {
    ...
    if (!this->UseNew)
    {
      ...
      void* mem = svtkVRMLAllocator::AllocateMemory(this->Allocated * sizeof(T));
  ...
  svtkVRMLVectorType<T>& operator+=(T datum)
  {
  ...
  void* operator new(size_t n) { return svtkVRMLAllocator::AllocateMemory(n); }
...
#endif
// SVTK-HeaderTest-Exclude: svtkVRML.h
