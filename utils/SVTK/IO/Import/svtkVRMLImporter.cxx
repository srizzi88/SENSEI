...
/* ======================================================================
  ...
  Ported to SVTK By:     Thomas D. Citriniti
...
#include "svtkVRMLImporter_Yacc.h"

//----------------------------------------------------------------------------
class svtkVRMLImporterInternal
{
public:
  svtkVRMLImporterInternal()
    : Heap(1)
  {
  }
  svtkVRMLVectorType<svtkObject*> Heap;
};

//----------------------------------------------------------------------------
// Heap to manage memory leaks
svtkHeap* svtkVRMLAllocator::Heap = nullptr;

//----------------------------------------------------------------------------
svtkStandardNewMacro(svtkVRMLImporter);

//----------------------------------------------------------------------------
svtkVRMLImporter::svtkVRMLImporter()
{
  this->Internal = new svtkVRMLImporterInternal;
  ...
  this->FileFD = nullptr;
  this->Parser = new svtkVRMLYaccData;
...
svtkVRMLImporter::~svtkVRMLImporter()
{
  ...
  {
    svtkObject* obj = this->Internal->Heap.Pop();
    if (obj)
    {
      obj->Delete();
    }
...
//----------------------------------------------------------------------------
void svtkVRMLImporter::PrintSelf(ostream& os, svtkIndent indent)
{
...
int svtkVRMLImporter::OpenImportFile()
{
  svtkDebugMacro(<< "Opening import file");
  ...
  this->FileFD = svtksys::SystemTools::Fopen(this->FileName, "r");
...
//----------------------------------------------------------------------------
svtkPoints* svtkVRMLImporter::PointsNew()
{
...
//----------------------------------------------------------------------------
svtkFloatArray* svtkVRMLImporter::FloatArrayNew()
{
...
//----------------------------------------------------------------------------
svtkIdTypeArray* svtkVRMLImporter::IdTypeArrayNew()
{
...
//----------------------------------------------------------------------------
void svtkVRMLImporter::DeleteObject(svtkObject* obj)
{
...
//----------------------------------------------------------------------------
int svtkVRMLImporter::ImportBegin()
{
  int ret = 1;
  try
  {
    ...
    svtkVRMLAllocator::Initialize();
    this->Parser->typeList = new svtkVRMLVectorType<VrmlNodeType*>;
    this->Parser->typeList->Init();

    this->Parser->useList = new svtkVRMLVectorType<svtkVRMLUseStruct*>;
    this->Parser->useList->Init();

    this->Parser->currentField = new svtkVRMLVectorType<VrmlNodeType::FieldRec*>;
    ...
    this->Parser->CurrentProtoStack = new svtkVRMLVectorType<VrmlNodeType*>;

    // Lets redefine the YY_INPUT macro on Flex and get chars from memory
    this->Parser->theyyInput = svtkVRMLYaccData::memyyInput;
    ...
    this->Parser->yyin = svtksys::SystemTools::Fopen(this->FileName, "r");
    if (!this->Parser->yyin)
    {
      throw std::exception();
    }
    ...
    this->Parser->theyyInput = svtkVRMLYaccData::defyyInput;
  ...
  {
    svtkErrorMacro(<< "Unable to read VRML file! Error at line " << this->Parser->currentLineNumber);
    ret = 0;
  }
  catch (std::string& s)
  {
...
//----------------------------------------------------------------------------
void svtkVRMLImporter::ImportEnd()
{
...
void svtkVRMLImporter::enterNode(const char* nodeType)
{
  ...
  *this->Parser->currentField += fr;
  ...
  if (nodeTypeName == "Appearance")
  {
    ...
    this->CurrentProperty = svtkProperty::New();
    if (this->Parser->creatingDEF)
    {
      *this->Parser->useList +=
        new svtkVRMLUseStruct(this->Parser->curDEFName, this->CurrentProperty);
    ...
    nodeTypeName == "Sphere")
  {
    ...
    {
      svtkCylinderSource* cyl = svtkCylinderSource::New();
    ...
    if (this->Parser->creatingDEF)
    {
      *this->Parser->useList += new svtkVRMLUseStruct(this->Parser->curDEFName, pmap);
      this->Parser->creatingDEF = 0;
    }
  }
  else if (nodeTypeName == "DirectionalLight")
  {
    ...
    this->CurrentLight = svtkLight::New();
    this->Renderer->AddLight(this->CurrentLight);
    if (this->Parser->creatingDEF)
    {
      *this->Parser->useList += new svtkVRMLUseStruct(this->Parser->curDEFName, this->CurrentLight);
      this->Parser->creatingDEF = 0;
    }
  }
    ...
    nodeTypeName == "PointSet")
  {
    ...
    this->CurrentMapper = svtkPolyDataMapper::New();
    ...
    this->CurrentScalars = svtkFloatArray::New();
    if (this->Parser->creatingDEF)
    {
      *this->Parser->useList += new svtkVRMLUseStruct(this->Parser->curDEFName, this->CurrentMapper);
      this->Parser->creatingDEF = 0;
    }
  }
  else if (nodeTypeName == "Shape")
  {
    ...
    this->CurrentActor = svtkActor::New();
    if (this->CurrentProperty)
    {
      this->CurrentActor->SetProperty(this->CurrentProperty);
    }
    ...
    if (this->Parser->creatingDEF)
    {
      *this->Parser->useList += new svtkVRMLUseStruct(this->Parser->curDEFName, this->CurrentActor);
      this->Parser->creatingDEF = 0;
    }
  }
...
//----------------------------------------------------------------------------
void svtkVRMLImporter::exitNode()
{
    ...
    nodeTypeName == "PointSet")
  {
    ...
    // a similar scheme is implemented in svtkOBJReader
    ...
    else
    {
      ...
      {
        svtkErrorMacro(<< "Number of faces does not match texture faces, output may not be correct");
        ...
        this->CurrentNormalCells->GetNumberOfCells() !=
          this->CurrentMapper->GetInput()->GetPolys()->GetNumberOfCells())
      {
        svtkErrorMacro(<< "Number of faces does not match normal faces, output may not be correct");
        tcoordsCorrespond = true; // don't rejig
      }
      else
      {
        ...
        if (this->CurrentTCoords && this->CurrentTCoordCells)
        {
          svtkCellArray* polys = this->CurrentMapper->GetInput()->GetPolys();
          polys->InitTraversal();
          ...
          {
            svtkIdType nTCoordPts;
            const svtkIdType* tcoordPts;
            this->CurrentTCoordCells->GetNextCell(nTCoordPts, tcoordPts);
            if (npts != nTCoordPts)
            {
              svtkErrorMacro(<< "Face size differs to texture face size, output may not be correct");
              break;
            }
        ...
        if (this->CurrentNormals && this->CurrentNormalCells)
        {
          ...
          while (polys->GetNextCell(npts, pts))
          {
            svtkIdType nNormalPts;
            const svtkIdType* normalPts;
            this->CurrentNormalCells->GetNextCell(nNormalPts, normalPts);
            if (npts != nNormalPts)
            {
              svtkErrorMacro(<< "Face size differs to normal face size, output may not be correct");
              break;
            }
            for (svtkIdType j = 0; j < npts; j++)
            {
    ...
    {
      svtkPolyData* pd = this->CurrentMapper->GetInput();
      if (pd == nullptr)
      {
        pd = svtkPolyData::New();
    ...
    else // must rejig
    {
      ...
      svtkNew<svtkFloatArray> newScalars;
      if (this->CurrentScalars)
      {
      ...
      svtkIdType npts;
      const svtkIdType* pts;
      for (svtkIdType i = 0; polys->GetNextCell(npts, pts); i++)
      {
        svtkIdType n_tcoord_pts = 0;
        const svtkIdType* tcoord_pts = nullptr;
        if (this->CurrentTCoordCells)
        {
          this->CurrentTCoordCells->GetNextCell(n_tcoord_pts, tcoord_pts);
        }
        svtkIdType n_normal_pts = 0;
        const svtkIdType* normal_pts = nullptr;
        if (this->CurrentNormalCells)
        {
        ...
        // then we must do something else SVTK will complain.
...
//----------------------------------------------------------------------------
void svtkVRMLImporter::enterField(const char* fieldName)
{
  VrmlNodeType::FieldRec* fr = this->Parser->currentField->Top();
  assert(fr != nullptr);
  ...
  if (fr->nodeType != nullptr)
  {
    ...
    {
      svtkErrorMacro(<< "Error: Node's of type " << fr->nodeType->getName()
...
//----------------------------------------------------------------------------
void svtkVRMLImporter::exitField()
{
  VrmlNodeType::FieldRec* fr = this->Parser->currentField->Top();
  assert(fr != nullptr);
  ...
  if (fieldName == "radius")
  {
    ...
    {
      static_cast<svtkSphereSource*>(this->CurrentSource)->SetRadius(this->Parser->yylval.sffloat);
    }
    // Set the Cylinder radius
    else if (nodeTypeName == "Cylinder")
    {
      static_cast<svtkCylinderSource*>(this->CurrentSource)->SetRadius(this->Parser->yylval.sffloat);
    }
  }
  ...
  {
    svtkCubeSource* cube = static_cast<svtkCubeSource*>(this->CurrentSource);
  ...
  else if (fieldName == "height")
  {
    ...
    {
      static_cast<svtkConeSource*>(this->CurrentSource)->SetHeight(this->Parser->yylval.sffloat);
    }
    // or set the current Cylinder height
    else if (nodeTypeName == "Cylinder")
    {
      static_cast<svtkCylinderSource*>(this->CurrentSource)->SetHeight(this->Parser->yylval.sffloat);
    }
  }
  // For the bottomRadius field (only for Cone shapes)
  else if (fieldName == "bottomRadius" && nodeTypeName == "Cone")
  {
    static_cast<svtkConeSource*>(this->CurrentSource)->SetRadius(this->Parser->yylval.sffloat);
  }
  ...
  else if (fieldName == "coordIndex")
  {
    ...
    svtkIdType index = 0, cnt = 0;
    svtkIdType nbPoints = this->Parser->yylval.mfint32->GetMaxId();
  ...
  else if (fieldName == "point")
  {
    ...
    if (nodeTypeName == "Coordinate")
    {
      ...
      svtkIdType nbPoints = this->CurrentPoints->GetNumberOfPoints();
      for (svtkIdType i = 0; i < nbPoints; i++)
      {
      ...
      {
        *this->Parser->useList +=
          new svtkVRMLUseStruct(this->Parser->curDEFName, this->CurrentPoints);
        this->Parser->creatingDEF = 0;
      }
    }
  ...
  else if (fieldName == "coord")
  {
    ...
    if (this->Parser->creatingDEF)
    {
      *this->Parser->useList += new svtkVRMLUseStruct(this->Parser->curDEFName, this->CurrentPoints);
      this->Parser->creatingDEF = 0;
    }
    ...
    if (nodeTypeName == "PointSet")
    {
      svtkNew<svtkPolyData> pd;
      svtkNew<svtkCellArray> cells;
      svtkIdType nbPoints = this->Parser->yylval.vec3f->GetNumberOfPoints();
      for (svtkIdType i = 0; i < nbPoints; i++)
      {
  ...
  else if (fieldName == "color")
  {
    ...
    if (nodeTypeName == "Color")
    {
      ...
      this->CurrentLut = svtkLookupTable::New();
      ...
      if (this->Parser->creatingDEF)
      {
        *this->Parser->useList += new svtkVRMLUseStruct(this->Parser->curDEFName, this->CurrentLut);
        this->Parser->creatingDEF = 0;
      }
    }
  }
  ...
  else if (fieldName == "colorIndex")
  {
    ...
    svtkCellArray* cells = (pd->GetNumberOfPolys() > 0) ? pd->GetPolys() : pd->GetLines();
    cells->InitTraversal();
    const svtkIdType* pts;
    svtkIdType npts;
    ...
    {
      for (svtkIdType i = 0; i <= this->Parser->yylval.mfint32->GetMaxId(); i++)
      {
    ...
    {
      cells->GetNextCell(npts, pts);
      svtkIdType len = this->Parser->yylval.mfint32->GetMaxId();
      for (svtkIdType i = 0, j = 0, index = 0; i <= len; i++)
      {
  ...
  else if (fieldName == "vector")
  {
    ...
    this->CurrentNormals = svtkFloatArray::New();
  ...
  else if (fieldName == "texCoordIndex")
  {
    ...
    this->CurrentTCoordCells = svtkCellArray::New();
  ...
  else if (fieldName == "normalIndex")
  {
    ...
    this->CurrentNormalCells = svtkCellArray::New();
...
//----------------------------------------------------------------------------
void svtkVRMLImporter::useNode(const char* name)
{
  svtkObject* useO = this->GetVRMLDEFObject(name);
  if (!useO)
  {
    return;
  }
  ...
  if (className.find("Actor") != std::string::npos)
  {
    ...
    actor->ShallowCopy(static_cast<svtkActor*>(useO));
    if (this->CurrentProperty)
    {
  ...
  {
    svtkActor* actor = svtkActor::New();
    actor->SetMapper(static_cast<svtkPolyDataMapper*>(useO));
    if (this->CurrentProperty)
    {
      actor->SetProperty(this->CurrentProperty);
    }
  ...
  else if (className == "svtkPoints")
  {
    svtkPoints* points = static_cast<svtkPoints*>(useO);
  ...
  else if (className == "svtkLookupTable")
  {
    svtkLookupTable* lut = static_cast<svtkLookupTable*>(useO);
    lut->Register(this);
    if (this->CurrentLut)
    {
      this->CurrentLut->Delete();
    ...
    // Seed the scalars with default values.
    this->CurrentScalars->Reset();
    svtkIdType nbPts = this->CurrentPoints->GetNumberOfPoints();
...
svtkObject* svtkVRMLImporter::GetVRMLDEFObject(const char* name)
{
  ...
  {
    const svtkVRMLUseStruct* nt = (*this->Parser->useList)[i];
...
//----------------------------------------------------------------------------
std::string svtkVRMLImporter::GetOutputsDescription()
{
  std::stringstream ss;
  for (int i = 0; i < this->Internal->Heap.Count(); i++)
  {
    svtkObject* obj = this->Internal->Heap.Get(i);
    svtkPoints* points = svtkPoints::SafeDownCast(obj);
    svtkAbstractArray* array = svtkAbstractArray::SafeDownCast(obj);
    if (points)
    {
    ...
    else if (array)
    {
      ...
      ss << svtkImporter::GetArrayDescription(array, svtkIndent(1));
    }
  }
  return ss.str();
}
