...
//----------------------------------------------------------------------------
bool VerifyArrayValue(svtkTree* tree, svtkIdType index, const char* arrayName, const char* baseline)
{
  svtkAbstractArray* array = tree->GetVertexData()->GetAbstractArray(arrayName);
  if (!array)
  {
...
bool VerifyArrayAttribute(
  svtkTree* tree, const char* arrayName, const char* attributeName, const char* baseline)
{
  svtkAbstractArray* array = tree->GetVertexData()->GetAbstractArray(arrayName);
  if (!array)
  {
...
//----------------------------------------------------------------------------
bool VerifyColor(svtkTree* tree, svtkIdType vertex, unsigned char r, unsigned char g, unsigned char b)
{
  svtkUnsignedCharArray* array =
...
int TestPhyloXMLTreeReadWrite(int argc, char* argv[])
{
  ...
  char* inputFile =
    svtkTestUtilities::ExpandDataFileName(argc, argv, "Data/Infovis/XML/example_phylo.xml");
  cout << "reading from a file: " << inputFile << endl;

  // read the input file into a svtkTree
  svtkNew<svtkPhyloXMLTreeReader> reader;
  ...
  for (svtkIdType vertex = 0; vertex < 6; ++vertex)
  {
  ...
  svtkAbstractArray* weightArray = tree->GetEdgeData()->GetAbstractArray("weight");
  if (!weightArray)
  {
    cout << "could not find weight array" << endl;
    return EXIT_FAILURE;
  }
  for (svtkIdType edge = 0; edge < tree->GetNumberOfEdges(); ++edge)
  {
  ...
  // next step:
  // write this svtkTree out to to a string in PhyloXML format
  svtkNew<svtkPhyloXMLTreeWriter> writer;
  ...
  // recreate a svtkTree from this PhyloXML string.
  svtkNew<svtkPhyloXMLTreeReader> reader2;
  ...
  svtkNew<svtkPhyloXMLTreeWriter> writer2;
