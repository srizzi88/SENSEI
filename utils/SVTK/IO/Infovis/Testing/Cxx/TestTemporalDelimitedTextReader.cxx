...
int TestTemporalDelimitedTextReader(int argc, char* argv[])
{
  ...
  const char* fname = svtkTestUtilities::ExpandDataFileName(argc, argv, testFNames);
  std::string filename = fname;
  delete[] fname;

  { // TEST 1: No TimeStepColumn given, output the whole CSV
    svtkNew<svtkTemporalDelimitedTextReader> reader;

    reader->SetFileName(filename.c_str());
    reader->UpdateInformation();
    // Nothing should be done when no FieldDelimiterCharacters have been set

    svtkTable* emptyTable = reader->GetOutput();
  ...
  { // TEST 2: A specific Timestep
    ...
    svtkTable* table2 = reader->GetOutput();
