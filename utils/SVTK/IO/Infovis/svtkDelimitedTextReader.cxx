/*=========================================================================
  ...
  Module:    svtkDelimitedTextReader.cxx
...
/// fields, inserting them into a svtkTable.
...
public:
  ...
  DelimitedTextIterator(const svtkIdType max_records, const svtkUnicodeString& record_delimiters,
    const svtkUnicodeString& field_delimiters, const svtkUnicodeString& string_delimiters,
    const svtkUnicodeString& whitespace, const svtkUnicodeString& escape, bool have_headers,
    bool unicode_array_output, bool merg_cons_delimiters, bool use_string_delimeter,
    svtkTable* const output_table)
...
private:
  void InsertField()
  {
      ...
      0 == this->CurrentRecordIndex)
    {
      ...
      else
      {
        ...
        if (this->UnicodeArrayOutput)
        {
          ...
          svtkArrayDownCast<svtkUnicodeStringArray>(array)->SetValue(
        ...
        else
        {
          ...
          svtkArrayDownCast<svtkStringArray>(array)->InsertValue(this->CurrentRecordIndex, s);
        }
      }
    ...
    else if (this->CurrentFieldIndex < this->OutputTable->GetNumberOfColumns())
    {
      ...
      if (this->UnicodeArrayOutput)
      {
        svtkUnicodeStringArray* uarray = svtkArrayDownCast<svtkUnicodeStringArray>(
      ...
      {
        svtkStringArray* sarray =
          svtkArrayDownCast<svtkStringArray>(this->OutputTable->GetColumn(this->CurrentFieldIndex));
        std::string s;
        this->CurrentField.utf8_str(s);
...
svtkStandardNewMacro(svtkDelimitedTextReader);

svtkDelimitedTextReader::svtkDelimitedTextReader()
  : FileName(nullptr)
  , UnicodeCharacterSet(nullptr)
  , MaxRecords(0)
  , UnicodeRecordDelimiters(svtkUnicodeString::from_utf8("\r\n"))
  , UnicodeFieldDelimiters(svtkUnicodeString::from_utf8(","))
  , UnicodeStringDelimiters(svtkUnicodeString::from_utf8("\""))
  , UnicodeWhitespace(svtkUnicodeString::from_utf8(" \t\r\n\v\f"))
  , UnicodeEscapeCharacter(svtkUnicodeString::from_utf8("\\"))
  , HaveHeaders(false)
...
void svtkDelimitedTextReader::PrintSelf(ostream& os, svtkIndent indent)
{
...
void svtkDelimitedTextReader::SetInputString(const char* in)
{
  int len = 0;
  if (in != nullptr)
  {
...
void svtkDelimitedTextReader::SetInputString(const char* in, int len)
{
...
void svtkDelimitedTextReader::SetUnicodeRecordDelimiters(const svtkUnicodeString& delimiters)
{
  this->UnicodeRecordDelimiters = delimiters;
  this->Modified();
}

svtkUnicodeString svtkDelimitedTextReader::GetUnicodeRecordDelimiters()
{
  return this->UnicodeRecordDelimiters;
}

void svtkDelimitedTextReader::SetUTF8RecordDelimiters(const char* delimiters)
{
  this->UnicodeRecordDelimiters = svtkUnicodeString::from_utf8(delimiters);
  this->Modified();
}

const char* svtkDelimitedTextReader::GetUTF8RecordDelimiters()
{
  return this->UnicodeRecordDelimiters.utf8_str();
}

void svtkDelimitedTextReader::SetUnicodeFieldDelimiters(const svtkUnicodeString& delimiters)
{
  this->UnicodeFieldDelimiters = delimiters;
  this->Modified();
}

svtkUnicodeString svtkDelimitedTextReader::GetUnicodeFieldDelimiters()
{
  return this->UnicodeFieldDelimiters;
}

void svtkDelimitedTextReader::SetUTF8FieldDelimiters(const char* delimiters)
{
  this->UnicodeFieldDelimiters = svtkUnicodeString::from_utf8(delimiters);
  this->Modified();
}

const char* svtkDelimitedTextReader::GetUTF8FieldDelimiters()
{
  return this->UnicodeFieldDelimiters.utf8_str();
}

void svtkDelimitedTextReader::SetUnicodeStringDelimiters(const svtkUnicodeString& delimiters)
{
  this->UnicodeStringDelimiters = delimiters;
  this->Modified();
}

svtkUnicodeString svtkDelimitedTextReader::GetUnicodeStringDelimiters()
{
  return this->UnicodeStringDelimiters;
}

void svtkDelimitedTextReader::SetUTF8StringDelimiters(const char* delimiters)
{
  this->UnicodeStringDelimiters = svtkUnicodeString::from_utf8(delimiters);
  this->Modified();
}

const char* svtkDelimitedTextReader::GetUTF8StringDelimiters()
{
  return this->UnicodeStringDelimiters.utf8_str();
}

svtkStdString svtkDelimitedTextReader::GetLastError()
{
  return this->LastError;
}

int svtkDelimitedTextReader::RequestData(
...
int svtkDelimitedTextReader::ReadData(svtkTable* const output_table)
{
  this->LastError = "";
  ...
  try
  {
    ...
    if (!this->ReadFromInputString)
    {
      ...
      // const svtkIdType total_bytes = file_stream.tellg();
    ...
    svtkStdString character_set;
    svtkTextCodec* transCodec = nullptr;

    if (this->UnicodeCharacterSet)
    {
      this->UnicodeOutputArrays = true;
      character_set = this->UnicodeCharacterSet;
      transCodec = svtkTextCodecFactory::CodecForName(this->UnicodeCharacterSet);
    }
    else
    {
      char tstring[2];
      ...
      this->UnicodeFieldDelimiters = svtkUnicodeString::from_utf8(fieldDelimiterCharacters);
      this->UnicodeStringDelimiters = svtkUnicodeString::from_utf8(tstring);
      this->UnicodeOutputArrays = false;
      transCodec = svtkTextCodecFactory::CodecToHandle(*input_stream_pt);
    }
      ...
      else
      {
        ...
        else
        {
          throw std::runtime_error(
            "Could not find pedigree id array: " + svtkStdString(this->PedigreeIdArrayName));
        }
      }
    }
    ...
    {
      svtkStringToNumeric* converter = svtkStringToNumeric::New();
      ...
      svtkTable* clone = output_table->NewInstance();
