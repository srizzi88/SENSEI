...
svtkStandardNewMacro(svtkISIReader);
...
void svtkISIReader::PrintSelf(ostream& os, svtkIndent indent)
{
  ...
  svtkInformation*, svtkInformationVector**, svtkInformationVector* outputVector)
{
  ...
  if (!file)
  {
    svtkErrorMacro(<< "svtkISIReader could not open file " << this->FileName);
    return 0;
  }
  ...
  {
    svtkErrorMacro(<< "File " << this->FileName << " is not an ISI file");
    return 0;
  }
  ...
  {
    svtkErrorMacro(<< "File " << this->FileName << " is not an ISI version 1.0 file");
    return 0;
  }
  ...
  for (my_getline(file, line_buffer); file; my_getline(file, line_buffer))
  {
    ...
    for (; file;)
    {
      ...
      {
        svtkStringArray* const new_column = svtkStringArray::New();
