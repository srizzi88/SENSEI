/*=========================================================================
  ...
  Module:    svtkNewickTreeReader.cxx
...
svtkStandardNewMacro(svtkNewickTreeReader);
...
//----------------------------------------------------------------------------
svtkNewickTreeReader::svtkNewickTreeReader()
{
...
//----------------------------------------------------------------------------
svtkNewickTreeReader::~svtkNewickTreeReader() = default;

//----------------------------------------------------------------------------
svtkTree* svtkNewickTreeReader::GetOutput()
{
  return this->GetOutput(0);
}

//----------------------------------------------------------------------------
svtkTree* svtkNewickTreeReader::GetOutput(int idx)
{
  return svtkTree::SafeDownCast(this->GetOutputDataObject(idx));
}

//----------------------------------------------------------------------------
void svtkNewickTreeReader::SetOutput(svtkTree* output)
{
  this->GetExecutive()->SetOutputData(0, output);
}

//----------------------------------------------------------------------------
int svtkNewickTreeReader::ReadNewickTree(const char* buffer, svtkTree& tree)
{
  // Read through the input file to count the number of nodes in the tree.
  svtkIdType numNodes = 0;
...
//----------------------------------------------------------------------------
int svtkNewickTreeReader::ReadMeshSimple(const std::string& fname, svtkDataObject* doOutput)
{
  svtkDebugMacro(<< "Reading Newick tree ...");
  ...
  else
  {
    ...
    {
      svtkErrorMacro(<< "Input string is empty!");
      return 1;
    }
  }
  ...
  {
    svtkErrorMacro(<< "Error reading a svtkTree from the input.");
    return 1;
  }
...
//----------------------------------------------------------------------------
void svtkNewickTreeReader::CountNodes(const char* buffer, svtkIdType* numNodes)
{
  char* current;
  char* start;
  char temp;
...
//----------------------------------------------------------------------------
svtkIdType svtkNewickTreeReader::BuildTree(char* buffer, svtkMutableDirectedGraph* g,
  svtkDoubleArray* weights, svtkStringArray* names, svtkIdType parent)
{
  char* current;
  char* start;
  char* colon = nullptr;
...
//----------------------------------------------------------------------------
int svtkNewickTreeReader::FillOutputPortInformation(int, svtkInformation* info)
{
...
//----------------------------------------------------------------------------
void svtkNewickTreeReader::PrintSelf(ostream& os, svtkIndent indent)
{
