...
svtkStandardNewMacro(svtkRISReader);
...
void svtkRISReader::PrintSelf(ostream& os, svtkIndent indent)
{
  ...
  svtkInformation*, svtkInformationVector**, svtkInformationVector* outputVector)
{
  ...
  if (!file)
  {
    svtkErrorMacro(<< "svtkRISReader could not open file " << this->FileName);
    return 0;
  }
  ...
  /*
    ...
    == '-' && line_buffer[5] == ' ')
      ...
      // Set the table value ...
      svtkStdString old_value = table->GetValue(record_count, columns[tag_type]).ToString();
      if(old_value.empty())
        {
