...
#if (SVTK_SIZEOF_ID_TYPE == 8) && !defined(_DARWIN_FEATURE_64_BIT_INODE) &&                         \
...
svtkLSDynaFile_t SVTK_LSDYNA_OPENFILE(const char* fname)
{
#ifndef _WIN32
  svtkLSDynaFile_t f = open(fname, O_RDONLY);
  return f;
#else
  svtkLSDynaFile_t f = svtksys::SystemTools::Fopen(fname, "rb");
  setvbuf(f, nullptr, _IONBF, 0); // disable buffer
  return f;
#endif
}

std::string svtkLSGetFamilyFileName(
...
struct LSDynaFamily::BufferingInfo
{
  ...
  svtkIdType numWordsToRead;
  svtkIdType loopTimes;
  svtkIdType leftOver;
  const svtkIdType size; // default buffer size
private:
...
{
  this->FD = SVTK_LSDYNA_BADFILE; // No file open
...
LSDynaFamily::~LSDynaFamily()
{
  if (!SVTK_LSDYNA_ISBADFILE(this->FD))
  {
    SVTK_LSDYNA_CLOSEFILE(this->FD);
  }
...
int LSDynaFamily::ScanDatabaseDirectory()
{
  ...
  while (tryAdapt >= 0)
  {
    tmpFile = svtkLSGetFamilyFileName(
...
//-----------------------------------------------------------------------------
int LSDynaFamily::SkipToWord(SectionType sType, svtkIdType sId, svtkIdType wordNumber)
{
  ...
  else
  {
    ...
    mark.Offset += wordNumber;
    if (sId >= (svtkIdType)this->TimeStepMarks.size())
    {
      return 1;
    }
  ...
  while ((mark.FileNumber < (svtkIdType)this->Files.size()) &&
  ...
  if (mark.FileNumber > (svtkIdType)this->Files.size())
  {
  ...
  if (this->FNum < 0 || (this->FNum != mark.FileNumber))
  {
    ...
    this->FD = SVTK_LSDYNA_OPENFILE(this->Files[mark.FileNumber].c_str());
    if (SVTK_LSDYNA_ISBADFILE(this->FD))
    {
      return errno;
    }
  ...
  svtkLSDynaOff_t offset = static_cast<svtkLSDynaOff_t>(mark.Offset * this->WordSize);
  // FIXME: Handle case where wordNumber + mark.Offset > (7=factor)*512*512
  if (SVTK_LSDYNA_SEEKTELL(this->FD, offset, SEEK_SET) != offset)
  {
    return errno;
  }
...
int LSDynaFamily::MarkTimeStep()
{
  ...
  mark.Offset = SVTK_LSDYNA_TELL(this->FD) / this->GetWordSize() - 1;
...
//-----------------------------------------------------------------------------
int LSDynaFamily::SkipWords(svtkIdType numWords)
{
  if (numWords <= 0)
  {
    return 0;
  ...
  if (this->FNum < 0 || SVTK_LSDYNA_ISBADFILE(this->FD))
  {
    return -1;
  }
  svtkIdType offset = numWords * this->WordSize;

  // determine where we are currently in the file
  svtkIdType currentPos = SVTK_LSDYNA_TELL(this->FD);

  SVTK_LSDYNA_SEEK(this->FD, static_cast<svtkLSDynaOff_t>(offset), SEEK_CUR);
  svtkIdType amountMoved = SVTK_LSDYNA_TELL(this->FD) - currentPos;
  ...
  if (offset > 0)
  {
    ...
    // if the skip is too big for one file, advance to the correct file
    ++this->FNum;
    while ((this->FNum < (svtkIdType)this->Files.size()) && (offset > this->FileSizes[this->FNum]))
    {
      offset -= this->FileSizes[this->FNum];
      ++this->FNum;
    }

    this->FD = SVTK_LSDYNA_OPENFILE(this->Files[this->FNum].c_str());
    this->FWord = 0;
    if (SVTK_LSDYNA_ISBADFILE(this->FD))
    ...
    SVTK_LSDYNA_SEEK(this->FD, static_cast<svtkLSDynaOff_t>(offset), SEEK_CUR);
    amountMoved = SVTK_LSDYNA_TELL(this->FD);
    offset -= amountMoved;
  }

  this->FWord = SVTK_LSDYNA_TELL(this->FD);
  return 0;
}

//-----------------------------------------------------------------------------
int LSDynaFamily::BufferChunk(WordType wType, svtkIdType chunkSizeInWords)
{
  ...
  // file.
  svtkIdType bytesLeft = chunkSizeInWords * this->WordSize;
  svtkIdType bytesRead;
  unsigned char* buf = this->Chunk;
  this->ChunkValid = 0;
  this->ChunkWord = 0;
  while (bytesLeft)
  {
    bytesRead = static_cast<svtkIdType>(SVTK_LSDYNA_READ(this->FD, (void*)buf, bytesLeft));
    ...
    if (bytesRead < bytesLeft)
    {
      ...
      { // try advancing to next file
        SVTK_LSDYNA_CLOSEFILE(this->FD);
        if (++this->FNum == (svtkIdType)this->Files.size())
...
//-----------------------------------------------------------------------------
svtkIdType LSDynaFamily::InitPartialChunkBuffering(
  const svtkIdType& numTuples, const svtkIdType& numComps)
{
  if (numTuples == 0)
  {
  ...
  {
    const svtkIdType size(this->BufferInfo->size);
  ...
  svtkIdType numChunks = this->BufferInfo->loopTimes;
...
//-----------------------------------------------------------------------------
svtkIdType LSDynaFamily::GetNextChunk(const WordType& wType)
{
  svtkIdType size;
...
int LSDynaFamily::AdvanceFile()
{
  ...
  {
    SVTK_LSDYNA_CLOSEFILE(this->FD);
    // this->FD = SVTK_LSDYNA_BADFILE;
  }
  this->FWord = 0;
  this->ChunkValid = 0;
  if (this->FNum + 1 < (svtkIdType)this->Files.size())
  {
    this->FNum++;
    this->FAdapt = this->FileAdaptLevels[this->FNum];
  }
...
void LSDynaFamily::MarkSectionStart(int adaptLevel, SectionType m)
{
  ...
  if (!SVTK_LSDYNA_ISBADFILE(this->FD))
  {
    myWord = SVTK_LSDYNA_TELL(this->FD) / this->WordSize;
  }
  else
  {
    myWord = 0;
...
//-----------------------------------------------------------------------------
void LSDynaFamily::SetStateSize(svtkIdType sz)
{
  this->StateSize = sz;
}

//-----------------------------------------------------------------------------
svtkIdType LSDynaFamily::GetStateSize() const
{
  return this->StateSize;
}

//-----------------------------------------------------------------------------
svtkIdType LSDynaFamily::GetNumberOfFiles()
{
  return static_cast<svtkIdType>(this->Files.size());
}
...
//-----------------------------------------------------------------------------
svtkIdType LSDynaFamily::GetFileSize(int i)
{
...
{
  if (!SVTK_LSDYNA_ISBADFILE(this->FD) && !this->FileHandlesClosed)
  {
    SVTK_LSDYNA_CLOSEFILE(this->FD);
    this->FD = SVTK_LSDYNA_BADFILE;
    this->ClearBuffer();
...
void LSDynaFamily::OpenFileHandles()
{
  ...
  {
    this->FD = SVTK_LSDYNA_OPENFILE(this->Files[this->FNum].c_str());
    SVTK_LSDYNA_SEEK(this->FD, static_cast<svtkLSDynaOff_t>(this->FWord), SEEK_SET);
    this->FileHandlesClosed = false;
  }
}
