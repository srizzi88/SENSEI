...
#ifndef _WIN32
#include <unistd.h>
typedef off_t svtkLSDynaOff_t; // sanity
typedef int svtkLSDynaFile_t;
#define SVTK_LSDYNA_BADFILE -1
#define SVTK_LSDYNA_TELL(fid) lseek(fid, 0, SEEK_CUR)
#define SVTK_LSDYNA_SEEK(fid, off, whence) lseek(fid, off, whence)
#define SVTK_LSDYNA_SEEKTELL(fid, off, whence) lseek(fid, off, whence)
#define SVTK_LSDYNA_READ(fid, ptr, cnt) read(fid, ptr, cnt)
#define SVTK_LSDYNA_ISBADFILE(fid) (fid < 0)
#define SVTK_LSDYNA_CLOSEFILE(fid) close(fid)
#else // _WIN32
typedef long svtkLSDynaOff_t; // insanity
typedef FILE* svtkLSDynaFile_t;
#define SVTK_LSDYNA_BADFILE 0
#define SVTK_LSDYNA_TELL(fid) ftell(fid)
#define SVTK_LSDYNA_SEEK(fid, off, whence) fseek(fid, off, whence)
#define SVTK_LSDYNA_SEEKTELL(fid, off, whence) fseek(fid, off, whence), ftell(fid)
#define SVTK_LSDYNA_READ(fid, ptr, cnt) fread(ptr, 1, cnt, fid)
#define SVTK_LSDYNA_ISBADFILE(fid) (fid == 0)
#define SVTK_LSDYNA_CLOSEFILE(fid) fclose(fid)
#endif
#ifdef SVTKSNL_HAVE_ERRNO_H
#include <errno.h>
#endif
...
public:
  ...
  int SkipToWord(SectionType sType, svtkIdType sId, svtkIdType wordNumber);
  int MarkTimeStep();
  int SkipWords(svtkIdType numWords);
  int BufferChunk(WordType wType, svtkIdType chunkSizeInWords);
  int ClearBuffer();
  ...
  svtkIdType InitPartialChunkBuffering(const svtkIdType& numTuples, const svtkIdType& numComps);
  svtkIdType GetNextChunk(const WordType& wType);

  inline char* GetNextWordAsChars();
  inline double GetNextWordAsFloat();
  inline svtkIdType GetNextWordAsInt();
  ...
  // Not needed (yet):
  // void GetCurrentWord( SectionType& stype, svtkIdType& sId, svtkIdType& wN );
  int AdvanceFile();
  ...
  void SetStateSize(svtkIdType sz);
  svtkIdType GetStateSize() const;

  svtkIdType GetNumberOfFiles();
  std::string GetFileName(int i);
  svtkIdType GetFileSize(int i);

  int GetCurrentAdaptLevel() const { return this->FAdapt; }
  int TimeAdaptLevel(int i) const { return this->TimeAdaptLevels[i]; }

  svtkIdType GetCurrentFWord() const { return this->FWord; }
...
protected:
  ...
  std::vector<svtkIdType> FileSizes;
...
//-----------------------------------------------------------------------------
inline svtkIdType LSDynaFamily::GetNextWordAsInt()
{
  if (this->ChunkWord >= this->ChunkValid)
  {
