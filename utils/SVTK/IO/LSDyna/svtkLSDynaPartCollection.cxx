...
//-----------------------------------------------------------------------------
class svtkLSDynaPartCollection::LSDynaPartStorage
{
protected:
  //---------------------------------------------------------------------------
  ...
  struct PartInfo
  {
    PartInfo(svtkLSDynaPart* p, const int& type, const svtkIdType& pId, const svtkIdType& start,
      const svtkIdType& npts)
      : numCells(1)
    ...
    svtkIdType numCells;          // number of cells in this continuous block
    svtkIdType startId;           // the global index to start of this block
    svtkIdType cellStructureSize; // stores the size of the cell array for this section
    svtkIdType partId; // id of the part this block represents, because the part can be nullptr
    svtkLSDynaPart* part;
  };
  ...
  struct PartInsertion
  {
    ...
    svtkIdType numCellsInserted;
  };
  //---------------------------------------------------------------------------

public:
  LSDynaPartStorage(const svtkIdType& numMaterials)
    ...
    , PartIteratorLoc(0)
  {
    ...
    this->Parts = new svtkLSDynaPart*[numMaterials];
    for (svtkIdType i = 0; i < numMaterials; ++i)
    {
  ...
  ~LSDynaPartStorage()
  {
    for (svtkIdType i = 0; i < this->NumParts; ++i)
    {
      if (this->Parts[i])
      {
        this->Parts[i]->Delete();
  ...
  //---------------------------------------------------------------------------
  svtkIdType GetNumParts() const { return NumParts; }

  //---------------------------------------------------------------------------
  void RegisterCell(const int& partType, const svtkIdType& matId, const svtkIdType& npts)
  {
  ...
  //---------------------------------------------------------------------------
  void ConstructPart(const svtkIdType& index, const std::string& name, const int& materialId,
    const int& numGlobalNodes, const int& wordSize)
  {
    svtkLSDynaPart* p = svtkLSDynaPart::New();
  ...
  //---------------------------------------------------------------------------
  void InsertCell(
    const int& partType, const int& cellType, const svtkIdType& npts, svtkIdType conn[8])
  {
  ...
  //---------------------------------------------------------------------------
  bool PartExists(const svtkIdType& index) const
  {
  ...
  //---------------------------------------------------------------------------
  svtkLSDynaPart* GetPart(const svtkIdType& index) { return this->Parts[index]; }

  //---------------------------------------------------------------------------
  svtkUnstructuredGrid* GetPartGrid(const svtkIdType& index)
  {
  ...
  //---------------------------------------------------------------------------
  bool GetNextPart(svtkLSDynaPart*& part)
  {
    ...
    svtkIdType pos = this->PartIteratorLoc + 1;
    this->PartIterator = nullptr;
    this->PartIteratorLoc = -1;

    // find the next part
    for (svtkIdType i = pos; i < this->NumParts; i++)
    {
  ...
  void AllocateParts()
  {
    svtkIdType numCells = 0, cellLength = 0;
    for (svtkIdType i = 0; i < this->NumParts; ++i)
    {
      svtkLSDynaPart* part = this->Parts[i];
  ...
  //---------------------------------------------------------------------------
  bool GetInfoForPart(svtkLSDynaPart* part, svtkIdType& numCells, svtkIdType& cellArrayLength) const
    ...
    for (int i = 0; i < LSDynaMetaData::NUM_CELL_TYPES; ++i)
    {
      ...
      for (it = this->Info[i].begin(); it != this->Info[i].end(); ++it)
      {
        ...
        if (info->part != nullptr)
        {
          ...
          info->part->PrintSelf(cout, svtkIndent().GetNextIndent());
          std::cout << std::endl;
          std::cout << std::endl;
        }
      }
  ...
  //---------------------------------------------------------------------------
  bool GetNextCellPart(svtkIdType& startId, svtkIdType& numCells, svtkLSDynaPart*& part)
  {
...
protected:
  ...
  svtkLSDynaPart* PartIterator;
  svtkIdType PartIteratorLoc;
};

svtkStandardNewMacro(svtkLSDynaPartCollection);
//-----------------------------------------------------------------------------
svtkLSDynaPartCollection::svtkLSDynaPartCollection()
{
...
//-----------------------------------------------------------------------------
void svtkLSDynaPartCollection::PrintSelf(ostream& os, svtkIndent indent)
{
...
//-----------------------------------------------------------------------------
void svtkLSDynaPartCollection::InitCollection(
  LSDynaMetaData* metaData, svtkIdType* mins, svtkIdType* maxs)
{
  delete this->Storage;
  delete[] this->MinIds;
  delete[] this->MaxIds;
  ...
  this->Storage = new LSDynaPartStorage(static_cast<svtkIdType>(metaData->PartIds.size()));

  this->MinIds = new svtkIdType[LSDynaMetaData::NUM_CELL_TYPES];
  this->MaxIds = new svtkIdType[LSDynaMetaData::NUM_CELL_TYPES];
...
//-----------------------------------------------------------------------------
void svtkLSDynaPartCollection::BuildPartInfo()
{
...
//-----------------------------------------------------------------------------
void svtkLSDynaPartCollection::RegisterCellIndexToPart(
  const int& partType, const svtkIdType& matId, const svtkIdType&, const svtkIdType& npts)
...
//-----------------------------------------------------------------------------
void svtkLSDynaPartCollection::AllocateParts()
{
  this->Storage->AllocateParts();
}

//-----------------------------------------------------------------------------
void svtkLSDynaPartCollection::InitCellInsertion()
{
  this->Storage->InitCellInsertion();
}

//-----------------------------------------------------------------------------
void svtkLSDynaPartCollection::InsertCell(const int& partType, const svtkIdType&, const int& cellType,
...
//-----------------------------------------------------------------------------
void svtkLSDynaPartCollection::SetCellDeadFlags(
...
//-----------------------------------------------------------------------------
void svtkLSDynaPartCollection::AddProperty(const LSDynaMetaData::LSDYNA_TYPES& type,
  const char* name, const int& offset, const int& numComps)
{

  svtkLSDynaPart* part = nullptr;
...
//-----------------------------------------------------------------------------
void svtkLSDynaPartCollection::FillCellProperties(float* buffer,
  const LSDynaMetaData::LSDYNA_TYPES& type, const svtkIdType& startId, const svtkIdType& numCells,
...
//-----------------------------------------------------------------------------
void svtkLSDynaPartCollection::FillCellProperties(double* buffer,
  const LSDynaMetaData::LSDYNA_TYPES& type, const svtkIdType& startId, const svtkIdType& numCells,
  const int& numPropertiesInCell)
{
  this->FillCellArray(buffer, type, startId, numCells, numPropertiesInCell);
...
//-----------------------------------------------------------------------------
template <typename T>
void svtkLSDynaPartCollection::FillCellArray(T* buffer, const LSDynaMetaData::LSDYNA_TYPES& type,
  const svtkIdType& startId, svtkIdType numCells, const int& numPropertiesInCell)
{
  // we only need to iterate the array for the subsection we need
  T* loc = buffer;
  svtkIdType size, globalStartId;
  svtkLSDynaPart* part;
  this->Storage->InitCellIteration(type, startId);
  while (this->Storage->GetNextCellPart(globalStartId, size, part))
  {
    svtkIdType start = std::max(globalStartId, startId);
    svtkIdType end = std::min(globalStartId + size, startId + numCells);
    if (end < start)
    {
      break;
    }
    svtkIdType is = end - start;
    if (part)
    {
...
//-----------------------------------------------------------------------------
void svtkLSDynaPartCollection::ReadCellUserIds(
  const LSDynaMetaData::LSDYNA_TYPES& type, const int& status)
{
  ...
  svtkIdType numChunks = this->MetaData->Fam.InitPartialChunkBuffering(numCells, 1);
  svtkIdType startId = 0;
  if (this->MetaData->Fam.GetWordSize() == 8 && numCells > 0)
  {
    for (svtkIdType i = 0; i < numChunks; ++i)
    {
      svtkIdType chunkSize = this->MetaData->Fam.GetNextChunk(LSDynaFamily::Float);
      svtkIdType numCellsInChunk = chunkSize;
      svtkIdType* buf = this->MetaData->Fam.GetBufferAs<svtkIdType>();
...
//-----------------------------------------------------------------------------
template <typename T>
void svtkLSDynaPartCollection::FillCellUserIdArray(
  T* buffer, const LSDynaMetaData::LSDYNA_TYPES& type, const svtkIdType& startId, svtkIdType numCells)
{
  ...
  while (this->Storage->GetNextCellPart(globalStartId, size, part))
  {
    ...
    svtkIdType is = (end - start) * numWordsPerIdType;
    if (part)
    {
      part->EnableCellUserIds();
      for (svtkIdType i = 0; i < is; i += numWordsPerIdType)
      {
        part->SetNextCellUserIds((svtkIdType)loc[i]);
      }
    }
    // perfectly valid to have a nullptr part being returned
    // just skip it as the user doesn't want it loaded.
...
//-----------------------------------------------------------------------------
bool svtkLSDynaPartCollection::IsActivePart(const int& id) const
{
  return this->Storage->PartExists(id);
}

//-----------------------------------------------------------------------------
svtkUnstructuredGrid* svtkLSDynaPartCollection::GetGridForPart(const int& index) const
{
  return this->Storage->GetPartGrid(index);
}

//-----------------------------------------------------------------------------
int svtkLSDynaPartCollection::GetNumberOfParts() const
{
  return static_cast<int>(this->Storage->GetNumParts());
}
//-----------------------------------------------------------------------------
void svtkLSDynaPartCollection::DisbleDeadCells()
{
  this->Storage->DisableDeadCells();
}

//-----------------------------------------------------------------------------
void svtkLSDynaPartCollection::GetPartReadInfo(const int& partType, svtkIdType& numberOfCells,
  svtkIdType& numCellsToSkipStart, svtkIdType& numCellsToSkipEnd) const
{
  svtkIdType size = this->MaxIds[partType] - this->MinIds[partType];
  if (size <= 0)
  {
    numberOfCells = 0;
    // skip everything
...
//-----------------------------------------------------------------------------
void svtkLSDynaPartCollection::FinalizeTopology()
{
  this->Storage->FinalizeTopology();
}

//-----------------------------------------------------------------------------
void svtkLSDynaPartCollection::ReadPointUserIds(const svtkIdType& numTuples, const char* name)
{
  this->SetupPointPropertyForReading(numTuples, 1, name, true, true, false, false);
}

//-----------------------------------------------------------------------------
void svtkLSDynaPartCollection::ReadPointProperty(const svtkIdType& numTuples,
  const svtkIdType& numComps, const char* name, const bool& isProperty, const bool& isGeometryPoints,
...
//-----------------------------------------------------------------------------
void svtkLSDynaPartCollection::SetupPointPropertyForReading(const svtkIdType& numTuples,
  const svtkIdType& numComps, const char* name, const bool& isIdType, const bool& isProperty,
  const bool& isGeometryPoints, const bool& isRoadPoints)
{
  ...
  svtkLSDynaPart* part = nullptr;
  svtkLSDynaPart** validParts = new svtkLSDynaPart*[this->Storage->GetNumParts()];
  svtkIdType idx = 0;
  if (!isRoadPoints)
  {
...
bool sortPartsOnGlobalIds(const svtkLSDynaPart* p1, const svtkLSDynaPart* p2)
{
...
//-----------------------------------------------------------------------------
template <typename T>
void svtkLSDynaPartCollection::FillPointProperty(const svtkIdType& numTuples,
  const svtkIdType& numComps, svtkLSDynaPart** parts, const svtkIdType numParts)
{
  ...
  // are reading in.
  std::list<svtkLSDynaPart*> sortedParts(parts, parts + numParts);
  std::list<svtkLSDynaPart*>::iterator partIt;

  sortedParts.sort(sortPartsOnGlobalIds);

  // find the max as the subset of points
  const svtkIdType maxGlobalPoint(sortedParts.back()->GetMaxGlobalPointId());
  svtkIdType minGlobalPoint = maxGlobalPoint;
  ...
  const svtkIdType realNumberOfTuples(maxGlobalPoint - minGlobalPoint);
  const svtkIdType numPointsToSkipStart(minGlobalPoint);
  const svtkIdType numPointsToSkipEnd(numTuples - (realNumberOfTuples + minGlobalPoint));

  svtkIdType offset = numPointsToSkipStart;
  const svtkIdType numPointsToRead(1048576);
  const svtkIdType loopTimes(realNumberOfTuples / numPointsToRead);
  const svtkIdType leftOver(realNumberOfTuples % numPointsToRead);
  const svtkIdType bufferChunkSize(numPointsToRead * numComps);

  T* buf = nullptr;
  p->Fam.SkipWords(numPointsToSkipStart * numComps);
  for (svtkIdType j = 0; j < loopTimes; ++j, offset += numPointsToRead)
  {
