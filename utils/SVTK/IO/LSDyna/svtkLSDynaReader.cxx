...
#include "svtkLSDynaSummaryParser.h"
...
svtkStandardNewMacro(svtkLSDynaReader);

// Names of svtkDataArrays provided with grid:
...
#ifdef SVTK_LSDYNA_DBG_MULTIBLOCK
static void svtkDebugMultiBlockStructure(svtkIndent indent, svtkMultiGroupDataSet* mbds);
#endif // SVTK_LSDYNA_DBG_MULTIBLOCK

namespace
{
static const char* svtkLSDynaCellTypes[] = { "Point", "Beam", "Shell", "Thick Shell", "Solid",
...
static int svtkLSNextSignificantLine(istream& deck, std::string& line)
{
  while (deck.good())
  {
...
static void svtkLSTrimWhitespace(std::string& line)
{
...
static void svtkLSDowncaseFirstWord(std::string& downcased, const std::string& line)
{
...
void svtkLSSplitString(std::string& input, std::vector<std::string>& splits, const char* separators)
{
...
struct Converter
{
  ...
  svtkIdType* convert(svtkIdType* buff, const svtkIdType&) { return buff; }
};
...
struct Converter<8, 4, cellLength>
{
  ...
  svtkIdType* convert(int* buff, const svtkIdType& size)
  {
    for (svtkIdType i = 0; i < size; ++i)
    {
      this->Conn[i] = static_cast<svtkIdType>(buff[i]);
    }
    return Conn;
  }
  svtkIdType Conn[cellLength];
};
...
struct Converter<4, 8, cellLength>
{
  ...
  {
    svtkIdType idx = 0;
    for (svtkIdType i = 0; i < size; i += 2, ++idx)
    {
      this->Conn[idx] = static_cast<svtkIdType>(buff[i]);
    }
    return Conn;
  }
  svtkIdType Conn[cellLength];
...
struct FillBlock
{
  Converter<sizeof(svtkIdType), wordSize, cellLength> BC;

  template <typename T>
  FillBlock(T* buff, svtkLSDynaPartCollection* parts, LSDynaMetaData* p,
    const svtkIdType& numWordsPerCell, const int& cellType)
  {
    ...
    const svtkIdType numFileWordsPerCell(numWordsPerCell * numWordsPerIdType);
    const svtkIdType offsetToMatId(numWordsPerIdType * (numWordsPerCell - 1));
    svtkIdType* conn;

    svtkIdType nc = 0, j = 0, matlId = 0;
    svtkIdType numCellsToSkip = 0, numCellsToSkipEnd = 0, chunkSize = 0;
    ...
    svtkIdType numChunks = p->Fam.InitPartialChunkBuffering(nc, numWordsPerCell);
    for (svtkIdType i = 0; i < numChunks; ++i)
    {
      ...
      for (j = 0; j < chunkSize; j += numWordsPerCell)
      {
        ...
        buff += offsetToMatId;
        matlId = static_cast<svtkIdType>(*buff);
...
struct FillBlock<LSDynaMetaData::SOLID, wordSize, cellLength>
{
  ...
  template <typename T>
  FillBlock(T* buff, svtkLSDynaPartCollection* parts, LSDynaMetaData* p,
    const svtkIdType& numWordsPerCell, const int& svtkNotUsed(cellType))
  {
    // determine the relationship between the file bit size and
    // the host machine bit size. This allows us to read 64 bit files on a
    // 32 bit machine. The Converter allows us to easily convert 32bit
    ...
    const int numWordsPerIdType(p->Fam.GetWordSize() / sizeof(T));
    const svtkIdType numFileWordsPerCell(numWordsPerCell * numWordsPerIdType);
    const svtkIdType offsetToMatId(numWordsPerIdType * cellLength);
    svtkIdType* conn;
...
struct FillBlock<LSDynaMetaData::ROAD_SURFACE, wordSize, cellLength>
{
  template <typename T>
  FillBlock(
    T*, svtkLSDynaPartCollection* parts, LSDynaMetaData* p, const svtkIdType&, const int& cellType)
  {
    // This is a ROAD_SURFACE specialization
    // has a weird weaving of cell types
    svtkIdType nc = 0, segId = 0, segSz = 0;
    svtkIdType numCellsToSkip = 0, numCellsToSkipEnd = 0;
    ...
    svtkIdType currentCell = 0;
    svtkIdType conn[4];
    for (svtkIdType i = 0; i < p->Dict["NSURF"]; ++i)
    {
      ...
      for (svtkIdType t = 0; t < segSz; ++t, ++currentCell)
      {
...
svtkLSDynaReader::svtkLSDynaReader()
{
...
void svtkLSDynaReader::PrintSelf(ostream& os, svtkIndent indent)
{
  ...
  {
    os << svtkLSDynaCellTypes[ct] << ":" << endl;
...
void svtkLSDynaReader::Dump(ostream& os)
{
  svtkIndent indent;
  os << indent << "Title: \"" << this->GetTitle() << "\"" << endl
...
void svtkLSDynaReader::DebugDump()
{
  this->Dump(cout);
}

int svtkLSDynaReader::CanReadFile(const char* fname)
{
  if (!fname)
    return 0;

  std::string dbDir = svtksys::SystemTools::GetFilenamePath(fname);
  std::string dbName = svtksys::SystemTools::GetFilenameName(fname);
  std::string dbExt;
  ...
  {
    svtksys::SystemTools::Stat_t st;
    if (svtksys::SystemTools::Stat(fname, &st) == 0)
    {
      dbName.insert(0, "/");
...
void svtkLSDynaReader::SetDatabaseDirectory(const std::string& f)
{
  this->SetDatabaseDirectory(f.c_str());
}

void svtkLSDynaReader::SetDatabaseDirectory(const char* f)
{
  svtkDebugMacro(<< this->GetClassName() << " (" << this << "): setting DatabaseDirectory to " << f);
  if (!f)
  {
...
std::string svtkLSDynaReader::GetDatabaseDirectory()
{
  return this->P->Fam.GetDatabaseDirectory();
}
#else
const char* svtkLSDynaReader::GetDatabaseDirectory()
{
...
int svtkLSDynaReader::IsDatabaseValid()
{
  return this->P->FileIsValid;
}

void svtkLSDynaReader::SetFileName(const std::string& f)
{
  this->SetFileName(f.c_str());
}

void svtkLSDynaReader::SetFileName(const char* f)
{
  std::string dbDir = svtksys::SystemTools::GetFilenamePath(f);
  std::string dbName = svtksys::SystemTools::GetFilenameName(f);
  std::string dbExt;
  std::string::size_type dot;
  ...
  {
    svtksys::SystemTools::Stat_t st;
    if (svtksys::SystemTools::Stat(f, &st) == 0)
    {
      dbName.insert(0, "/");
...
#if defined(SVTK_LEGACY_REMOVE)
std::string svtkLSDynaReader::GetFileName()
{
  std::string filename = this->P->Fam.GetDatabaseDirectory() + "/d3plot";
  return filename;
}
#else
const char* svtkLSDynaReader::GetFileName()
{
  static thread_local std::string surrogate;
...
char* svtkLSDynaReader::GetTitle()
{
  return this->P->Title;
}

int svtkLSDynaReader::GetDimensionality()
{
  return this->P->Dimensionality;
}

void svtkLSDynaReader::SetTimeStep(svtkIdType t)
{
  ...
  if (!p->FileIsValid)
  {
    if (p->Fam.GetDatabaseDirectory().empty())
    {
      svtkErrorMacro("You haven't set the LS-Dyna database directory!");
      return;
    }
...
svtkIdType svtkLSDynaReader::GetTimeStep()
{
  return this->P->CurrentState;
}

svtkIdType svtkLSDynaReader::GetNumberOfTimeSteps()
{
  return (svtkIdType)this->P->TimeValues.size();
}

double svtkLSDynaReader::GetTimeValue(svtkIdType s)
{
  if (s < 0 || s >= (svtkIdType)this->P->TimeValues.size())
  {
    return -1.0;
  }
...
svtkIdType svtkLSDynaReader::GetNumberOfNodes()
{
  return this->P->NumberOfNodes;
}

svtkIdType svtkLSDynaReader::GetNumberOfCells()
{
  svtkIdType tmp = 0;
...
svtkIdType svtkLSDynaReader::GetNumberOfSolidCells()
{
  return this->P->NumberOfCells[LSDynaMetaData::SOLID];
}

svtkIdType svtkLSDynaReader::GetNumberOfThickShellCells()
{
  return this->P->NumberOfCells[LSDynaMetaData::THICK_SHELL];
}

svtkIdType svtkLSDynaReader::GetNumberOfShellCells()
{
  return this->P->NumberOfCells[LSDynaMetaData::SHELL];
}

svtkIdType svtkLSDynaReader::GetNumberOfRigidBodyCells()
{
  return this->P->NumberOfCells[LSDynaMetaData::RIGID_BODY];
}

svtkIdType svtkLSDynaReader::GetNumberOfRoadSurfaceCells()
{
  return this->P->NumberOfCells[LSDynaMetaData::ROAD_SURFACE];
}

svtkIdType svtkLSDynaReader::GetNumberOfBeamCells()
{
  return this->P->NumberOfCells[LSDynaMetaData::BEAM];
}

svtkIdType svtkLSDynaReader::GetNumberOfParticleCells()
{
  return this->P->NumberOfCells[LSDynaMetaData::PARTICLE];
}

svtkIdType svtkLSDynaReader::GetNumberOfContinuumCells()
{
  svtkIdType tmp = 0;
...
// =================================== Point array queries
int svtkLSDynaReader::GetNumberOfPointArrays()
{
  return (int)this->P->PointArrayNames.size();
}

const char* svtkLSDynaReader::GetPointArrayName(int a)
{
...
int svtkLSDynaReader::GetPointArrayStatus(int a)
{
...
void svtkLSDynaReader::SetPointArrayStatus(int a, int stat)
{
  if (a < 0 || a >= (int)this->P->PointArrayStatus.size())
  {
    svtkWarningMacro("Cannot set status of non-existent point array " << a);
    return;
  }
...
int svtkLSDynaReader::GetNumberOfComponentsInPointArray(int a)
{
  if (a < 0 || a >= (int)this->P->PointArrayStatus.size())
    return 0;
...
// =================================== Cell array queries
int svtkLSDynaReader::GetNumberOfCellArrays(int ct)
{
  return (int)this->P->CellArrayNames[ct].size();
}

const char* svtkLSDynaReader::GetCellArrayName(int ct, int a)
{
...
int svtkLSDynaReader::GetCellArrayStatus(int ct, int a)
{
...
int svtkLSDynaReader::GetNumberOfComponentsInCellArray(int ct, int a)
{
  if (a < 0 || a >= (int)this->P->CellArrayStatus[ct].size())
    return 0;
...
void svtkLSDynaReader::SetCellArrayStatus(int ct, int a, int stat)
{
  if (a < 0 || a >= (int)this->P->CellArrayStatus[ct].size())
  {
    svtkWarningMacro("Cannot set status of non-existent point array " << a);
...
// =================================== Cell array queries
int svtkLSDynaReader::GetNumberOfSolidArrays()
{
  return (int)this->P->CellArrayNames[LSDynaMetaData::SOLID].size();
}

const char* svtkLSDynaReader::GetSolidArrayName(int a)
{
...
int svtkLSDynaReader::GetSolidArrayStatus(int a)
{
...
int svtkLSDynaReader::GetNumberOfComponentsInSolidArray(int a)
{
  if (a < 0 || a >= (int)this->P->CellArrayStatus[LSDynaMetaData::SOLID].size())
    return 0;
...
void svtkLSDynaReader::SetSolidArrayStatus(int a, int stat)
{
  if (a < 0 || a >= (int)this->P->CellArrayStatus[LSDynaMetaData::SOLID].size())
  {
    svtkWarningMacro("Cannot set status of non-existent point array " << a);
...
// =================================== Cell array queries
int svtkLSDynaReader::GetNumberOfThickShellArrays()
{
  return (int)this->P->CellArrayNames[LSDynaMetaData::THICK_SHELL].size();
}

const char* svtkLSDynaReader::GetThickShellArrayName(int a)
{
...
int svtkLSDynaReader::GetThickShellArrayStatus(int a)
{
...
int svtkLSDynaReader::GetNumberOfComponentsInThickShellArray(int a)
{
  if (a < 0 || a >= (int)this->P->CellArrayStatus[LSDynaMetaData::THICK_SHELL].size())
    return 0;
...
void svtkLSDynaReader::SetThickShellArrayStatus(int a, int stat)
{
  if (a < 0 || a >= (int)this->P->CellArrayStatus[LSDynaMetaData::THICK_SHELL].size())
  {
    svtkWarningMacro("Cannot set status of non-existent point array " << a);
...
// =================================== Cell array queries
int svtkLSDynaReader::GetNumberOfShellArrays()
{
  return (int)this->P->CellArrayNames[LSDynaMetaData::SHELL].size();
}

const char* svtkLSDynaReader::GetShellArrayName(int a)
{
...
int svtkLSDynaReader::GetShellArrayStatus(int a)
{
...
int svtkLSDynaReader::GetNumberOfComponentsInShellArray(int a)
{
  if (a < 0 || a >= (int)this->P->CellArrayStatus[LSDynaMetaData::SHELL].size())
    return 0;
...
void svtkLSDynaReader::SetShellArrayStatus(int a, int stat)
{
  if (a < 0 || a >= (int)this->P->CellArrayStatus[LSDynaMetaData::SHELL].size())
  {
    svtkWarningMacro("Cannot set status of non-existent point array " << a);
...
// =================================== Cell array queries
int svtkLSDynaReader::GetNumberOfRigidBodyArrays()
{
  return (int)this->P->CellArrayNames[LSDynaMetaData::RIGID_BODY].size();
}

const char* svtkLSDynaReader::GetRigidBodyArrayName(int a)
{
...
int svtkLSDynaReader::GetRigidBodyArrayStatus(int a)
{
...
int svtkLSDynaReader::GetNumberOfComponentsInRigidBodyArray(int a)
{
  if (a < 0 || a >= (int)this->P->CellArrayStatus[LSDynaMetaData::RIGID_BODY].size())
    return 0;
...
void svtkLSDynaReader::SetRigidBodyArrayStatus(int a, int stat)
{
  if (a < 0 || a >= (int)this->P->CellArrayStatus[LSDynaMetaData::RIGID_BODY].size())
  {
    svtkWarningMacro("Cannot set status of non-existent point array " << a);
...
// =================================== Cell array queries
int svtkLSDynaReader::GetNumberOfRoadSurfaceArrays()
{
  return (int)this->P->CellArrayNames[LSDynaMetaData::ROAD_SURFACE].size();
}

const char* svtkLSDynaReader::GetRoadSurfaceArrayName(int a)
{
...
int svtkLSDynaReader::GetRoadSurfaceArrayStatus(int a)
{
...
int svtkLSDynaReader::GetNumberOfComponentsInRoadSurfaceArray(int a)
{
  if (a < 0 || a >= (int)this->P->CellArrayStatus[LSDynaMetaData::ROAD_SURFACE].size())
    return 0;
...
void svtkLSDynaReader::SetRoadSurfaceArrayStatus(int a, int stat)
{
  if (a < 0 || a >= (int)this->P->CellArrayStatus[LSDynaMetaData::ROAD_SURFACE].size())
  {
    svtkWarningMacro("Cannot set status of non-existent point array " << a);
...
// =================================== Cell array queries
int svtkLSDynaReader::GetNumberOfBeamArrays()
{
  return (int)this->P->CellArrayNames[LSDynaMetaData::BEAM].size();
}

const char* svtkLSDynaReader::GetBeamArrayName(int a)
{
...
int svtkLSDynaReader::GetBeamArrayStatus(int a)
{
...
int svtkLSDynaReader::GetNumberOfComponentsInBeamArray(int a)
{
  if (a < 0 || a >= (int)this->P->CellArrayStatus[LSDynaMetaData::BEAM].size())
    return 0;
...
void svtkLSDynaReader::SetBeamArrayStatus(int a, int stat)
{
  if (a < 0 || a >= (int)this->P->CellArrayStatus[LSDynaMetaData::BEAM].size())
  {
    svtkWarningMacro("Cannot set status of non-existent point array " << a);
...
// =================================== Cell array queries
int svtkLSDynaReader::GetNumberOfParticleArrays()
{
  return (int)this->P->CellArrayNames[LSDynaMetaData::PARTICLE].size();
}

const char* svtkLSDynaReader::GetParticleArrayName(int a)
{
...
int svtkLSDynaReader::GetParticleArrayStatus(int a)
{
...
int svtkLSDynaReader::GetNumberOfComponentsInParticleArray(int a)
{
  if (a < 0 || a >= (int)this->P->CellArrayStatus[LSDynaMetaData::PARTICLE].size())
    return 0;
...
void svtkLSDynaReader::SetParticleArrayStatus(int a, int stat)
{
  if (a < 0 || a >= (int)this->P->CellArrayStatus[LSDynaMetaData::PARTICLE].size())
  {
    svtkWarningMacro("Cannot set status of non-existent point array " << a);
...
// =================================== Part queries
int svtkLSDynaReader::GetNumberOfPartArrays()
{
  return (int)this->P->PartNames.size();
}

const char* svtkLSDynaReader::GetPartArrayName(int a)
{
...
int svtkLSDynaReader::GetPartArrayStatus(int a)
{
...
void svtkLSDynaReader::SetPartArrayStatus(int a, int stat)
{
  if (a < 0 || a >= (int)this->P->PartStatus.size())
  {
    svtkWarningMacro("Cannot set status of non-existent point array " << a);
...
// ===================================
void svtkLSDynaReader::ResetPartsCache()
{
  if (this->Parts)
  {
    this->Parts->Delete();
...
int svtkLSDynaReader::ReadHeaderInformation(int curAdapt)
{
  LSDynaMetaData* p = this->P;
  ...
  switch (p->Dimensionality)
  {
    ...
    case 4:
      ...
      break;
    default:
      svtkErrorMacro("Unknown Dimensionality " << p->Dimensionality << " encountered");
      p->FileIsValid = 0;
      return 0;
  }
  ...
  {
    if (iddtmp & (static_cast<svtkIdType>(1) << itmp))
    {
  ...
  if (p->NumberOfCells[LSDynaMetaData::PARTICLE] > 0)
  {
    ...
    svtkIdType sphAttributes = p->Fam.GetNextWordAsInt();
...
int svtkLSDynaReader::ScanDatabaseTimeSteps()
{
  LSDynaMetaData* p = this->P;
  ...
  {
    svtkErrorMacro("Database has bad state size (" << p->Fam.GetStateSize() << ").");
    return 1;
  }
...
int svtkLSDynaReader::RequestInformation(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** svtkNotUsed(iinfo), svtkInformationVector* oinfo)
{
  LSDynaMetaData* p = this->P;
  // If the time step is set before RequestInformation is called, we must
  // read the header information immediately in order to determine whether
  ...
  {
    svtkErrorMacro("No valid time steps in the LS-Dyna database");
    return 0;
  }
  ...
  else if (p->CurrentState >= static_cast<svtkIdType>(p->TimeValues.size()))
  {
    p->CurrentState = static_cast<svtkIdType>(p->TimeValues.size() - 1);
  }
  ...
  svtkInformation* outInfo = oinfo->GetInformationObject(0);
  outInfo->Set(
    svtkStreamingDemandDrivenPipeline::TIME_STEPS(), &p->TimeValues[0], (int)p->TimeValues.size());
  double timeRange[2];
...
//----------------------------------------------------------------------------
int svtkLSDynaReader::ReadTopology()
{
...
// ============================================================= Section parsing
int svtkLSDynaReader::ReadNodes()
{
  LSDynaMetaData* p = this->P;
...
//-----------------------------------------------------------------------------
int svtkLSDynaReader::ReadDeletion()
{
...
//-----------------------------------------------------------------------------
void svtkLSDynaReader::ReadDeletionArray(svtkUnsignedCharArray* arr, const int& pos, const int& size)
{
  // setup to do a block read, way faster than converting each
  // float/double individually
  LSDynaMetaData* p = this->P;
  svtkIdType startId = 0;
  svtkIdType numChunks = p->Fam.InitPartialChunkBuffering(arr->GetNumberOfTuples(), size);
  if (p->Fam.GetWordSize() == 8)
  {
    for (svtkIdType i = 0; i < numChunks; ++i)
    {
      svtkIdType chunkSize = p->Fam.GetNextChunk(LSDynaFamily::Float);
      svtkIdType numCellsInChunk = chunkSize / size;
...
//-----------------------------------------------------------------------------
int svtkLSDynaReader::ReadState(svtkIdType step)
{
  // remember C style return so zero is pass
  if (this->ReadNodeStateInfo(step))
  {
    svtkErrorMacro("Problem reading state point information.");
    return 1;
  }
  if (this->ReadCellStateInfo(step))
  {
    svtkErrorMacro("Problem reading state cell information.");
    return 1;
  }
  if (this->ReadDeletion())
  {
    svtkErrorMacro("Problem reading state deletion information.");
    return 1;
  }
  return 0;
}
...
//-----------------------------------------------------------------------------
int svtkLSDynaReader::ReadNodeStateInfo(svtkIdType step)
{
  LSDynaMetaData* p = this->P;
...
//-----------------------------------------------------------------------------
int svtkLSDynaReader::ReadCellStateInfo(svtkIdType svtkNotUsed(step))
{
  ...
  svtkIdType cells[] = { p->Dict["NEL8"], p->Dict["NELT"], p->Dict["NEL2"], p->Dict["NEL4"] };
  svtkIdType cellVals[] = { p->Dict["NV3D"], p->Dict["NV3DT"], p->Dict["NV1D"], p->Dict["NV2D"] };
...
#undef SVTK_LS_CELLARRAY
  return 0;
}

//-----------------------------------------------------------------------------
void svtkLSDynaReader::ReadCellProperties(const int& type, const int& numTuples)
{
  ...
  svtkIdType numChunks = this->P->Fam.InitPartialChunkBuffering(numCells, numTuples);
  svtkIdType startId = 0;
  if (this->P->Fam.GetWordSize() == 8 && numCells > 0)
  {
    ...
    {
      // we need offsets!
      svtkIdType chunkSize = this->P->Fam.GetNextChunk(LSDynaFamily::Float);
      svtkIdType numCellsInChunk = chunkSize / numTuples;
...
int svtkLSDynaReader::ReadSPHState(svtkIdType svtkNotUsed(step))
{
  LSDynaMetaData* p = this->P;
...
#define SVTK_LS_SPHARRAY(cond, celltype, arrayname, numComps)                                       \
  ...
  int startPos = 0; // used to keep track of the startpos between calls to SVTK_LS_CELLARRAY
  SVTK_LS_SPHARRAY(
  ...
  SVTK_LS_SPHARRAY(p->Dict["isphfg(3)"], LSDynaMetaData::PARTICLE, LS_ARRAYNAME_PRESSURE, 1);
  SVTK_LS_SPHARRAY(p->Dict["isphfg(4)"], LSDynaMetaData::PARTICLE, LS_ARRAYNAME_STRESS, 6);
  SVTK_LS_SPHARRAY(p->Dict["isphfg(5)"], LSDynaMetaData::PARTICLE, LS_ARRAYNAME_EPSTRAIN, 1);
  SVTK_LS_SPHARRAY(p->Dict["isphfg(6)"], LSDynaMetaData::PARTICLE, LS_ARRAYNAME_DENSITY, 1);
  SVTK_LS_SPHARRAY(p->Dict["isphfg(7)"], LSDynaMetaData::PARTICLE, LS_ARRAYNAME_INTERNALENERGY, 1);
  SVTK_LS_SPHARRAY(p->Dict["isphfg(8)"], LSDynaMetaData::PARTICLE, LS_ARRAYNAME_NUMNEIGHBORS, 1);
  SVTK_LS_SPHARRAY(p->Dict["isphfg(9)"], LSDynaMetaData::PARTICLE, LS_ARRAYNAME_STRAIN, 6);
  SVTK_LS_SPHARRAY(p->Dict["isphfg(10)"], LSDynaMetaData::PARTICLE, LS_ARRAYNAME_MASS, 1);
...
#undef SVTK_LS_SPHARRAY
  return 0;
}

int svtkLSDynaReader::ReadUserMaterialIds()
{
  LSDynaMetaData* p = this->P;
  svtkIdType m, numMats;
...
int svtkLSDynaReader::ReadPartTitlesFromRootFile()
{
  /*
  ...
  */
  ...
  if (p->PreStateSize <= 0)
  {
    svtkErrorMacro("Database has bad pre state size(" << p->PreStateSize << ").");
    return 1;
  }

  // when called this method is at the right spot to read the part names
  svtkIdType currentFileLoc = p->Fam.GetCurrentFWord();
  svtkIdType currentAdaptLevel = p->Fam.GetCurrentAdaptLevel();
  ...
  svtkIdType numParts = static_cast<svtkIdType>(p->PartIds.size());
  svtkIdType partTitlesByteSize =
  ...
  svtkIdType fileSize = p->Fam.GetFileSize(0);
  ...
  svtkIdType nameWordSize = 72 / p->Fam.GetWordSize();
  for (svtkIdType i = 0; i < numParts; ++i)
  {
    p->Fam.BufferChunk(LSDynaFamily::Int, 1);
    p->Fam.GetNextWordAsInt(); // svtkIdType partId
...
void svtkLSDynaReader::ResetPartInfo()
{
  LSDynaMetaData* p = this->P;
  p->PartNames.clear();
  p->PartIds.clear();
...
#define SVTK_LSDYNA_PARTLABEL(dict, fmt)                                                            \
  N = p->Dict[dict];                                                                               \
  for (i = 0; i < N; ++i, ++mat)                                                                   \
  {                                                                                                \
    if (arbitraryMaterials)                                                                        \
  ...
  SVTK_LSDYNA_PARTLABEL("NUMMAT8", "Part%d"); // was "PartSolid%d
  SVTK_LSDYNA_PARTLABEL("NUMMATT", "Part%d"); // was "PartThickShell%d
  SVTK_LSDYNA_PARTLABEL("NUMMAT4", "Part%d"); // was "PartShell%d
  SVTK_LSDYNA_PARTLABEL("NUMMAT2", "Part%d"); // was "PartBeam%d
  SVTK_LSDYNA_PARTLABEL("NGPSPH", "Part%d");  // was "PartParticle%d
  SVTK_LSDYNA_PARTLABEL("NSURF", "Part%d");   // was "PartRoadSurface%d
  SVTK_LSDYNA_PARTLABEL("NUMMAT", "Part%d");  // was "PartRigidBody%d

#undef SVTK_LSDYNA_PARTLABEL
}

int svtkLSDynaReader::ReadInputDeck()
{
  if (!this->InputDeck)
  {
  ...
  svtksys::ifstream deck(this->InputDeck, ios::in);
  if (!deck.good())
  {
    return 0;
  }
  ...
  int retval;
  if (svtksys::SystemTools::StringStartsWith(header.c_str(), "<?xml"))
  {
...
int svtkLSDynaReader::ReadInputDeckXML(istream& deck)
{
  svtkLSDynaSummaryParser* parser = svtkLSDynaSummaryParser::New();
  parser->MetaData = this->P;
  parser->SetStream(&deck);
  // We must be able to parse the file and end up with 1 part per material ID
  if (!parser->Parse() || this->P->GetTotalMaterialCount() != (svtkIdType)this->P->PartNames.size())
  {
...
int svtkLSDynaReader::ReadInputDeckKeywords(istream& deck)
{
  int success = 1;
  ...
  while (
    deck.good() && svtkLSNextSignificantLine(deck, line) && curPart < (int)this->P->PartNames.size())
  {
    if (line[0] == '*')
    {
      svtkLSDowncaseFirstWord(lineLowercase, line.substr(1));
      if (svtksys::SystemTools::StringStartsWith(lineLowercase.c_str(), "part"))
      {
        // found a part
        // ... read the next non-comment line as the part name
        if (svtkLSNextSignificantLine(deck, line))
        {
          // Get rid of leading and trailing newlines, whitespace, etc.
          svtkLSTrimWhitespace(line);
          partName = line;
        }
        else
        {
      ...
      else if (svtksys::SystemTools::StringStartsWith(lineLowercase.c_str(), "parameter"))
      {
        // found a reference
  ...
  if (success)
  {
    ...
    std::string deckDir = svtksys::SystemTools::GetFilenamePath(this->InputDeck);
    std::string deckName = svtksys::SystemTools::GetFilenameName(this->InputDeck);
    std::string deckExt;
    std::string::size_type dot;
...
int svtkLSDynaReader::WriteInputDeckSummary(const char* fname)
{
  svtksys::ofstream xmlSummary(fname, ios::out | ios::trunc);
  if (!xmlSummary.good())
  {
    return 1;
  }
...
int svtkLSDynaReader::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** svtkNotUsed(iinfo), svtkInformationVector* oinfo)
{
  LSDynaMetaData* p = this->P;
  ...
  svtkMultiBlockDataSet* mbds = nullptr;
  svtkInformation* oi = oinfo->GetInformationObject(0);
  if (!oi)
  {
    return 0;
  }

  if (oi->Has(svtkStreamingDemandDrivenPipeline::UPDATE_TIME_STEP()))
  {
    // Only return single time steps for now.
    double requestedTimeStep = oi->Get(svtkStreamingDemandDrivenPipeline::UPDATE_TIME_STEP());
    int timeStepLen = oi->Length(svtkStreamingDemandDrivenPipeline::TIME_STEPS());
    double* timeSteps = oi->Get(svtkStreamingDemandDrivenPipeline::TIME_STEPS());
    ...
    oi->Set(svtkDataObject::DATA_TIME_STEP(), p->TimeValues[p->CurrentState]);
  }

  mbds = svtkMultiBlockDataSet::SafeDownCast(oi->Get(svtkDataObject::DATA_OBJECT()));
  if (!mbds)
  {
    return 0;
  }
  ...
  {
    svtkErrorMacro("Problem reading state data for time step " << p->CurrentState);
    return 1;
  }
  ...
  if (this->GetNumberOfParticleCells())
  {
    ...
    {
      svtkErrorMacro("Problem reading smooth particle hydrodynamics state.");
      return 1;
    }
  }
  ...
  for (int i = 0; i < size; ++i)
  {
    ...
    {
      svtkUnstructuredGrid* ug = this->Parts->GetGridForPart(i);
      this->ComputeDeflectionAndUpdateGeometry(ug);

      mbds->SetBlock(i, ug);
      mbds->GetMetaData(i)->Set(svtkCompositeDataSet::NAME(), this->P->PartNames[i].c_str());
    }
    else
    {
...
//-----------------------------------------------------------------------------
template <typename T>
void svtkLSDynaReader::FillDeletionArray(T* buffer, svtkUnsignedCharArray* arr,
  const svtkIdType& start, const svtkIdType& numCells, const int& deathPos, const int& cellSize)
{
  unsigned char val;
...
int svtkLSDynaReader::FillTopology(T* buff)
{
  ...
  FillBlock<LSDynaMetaData::PARTICLE, wordSize, 1>(buff, this->Parts, this->P, 2, SVTK_VERTEX);
  ...
  FillBlock<LSDynaMetaData::SOLID, wordSize, 8>(buff, this->Parts, this->P, 9, SVTK_HEXAHEDRON);

  // READ THICK_SHELL
  FillBlock<LSDynaMetaData::THICK_SHELL, wordSize, 8>(
    buff, this->Parts, this->P, 9, SVTK_QUADRATIC_QUAD);

  // READ BEAM
  FillBlock<LSDynaMetaData::BEAM, wordSize, 2>(buff, this->Parts, this->P, 6, SVTK_LINE);

  // READ SHELL and RIGID_BODY
  // uses a specialization to weave SHELL and RIGID BODY cells together
  FillBlock<LSDynaMetaData::SHELL, wordSize, 4>(buff, this->Parts, this->P, 5, SVTK_QUAD);
  ...
  if (this->P->ReadRigidRoadMvmt)
  {
    ...
    FillBlock<LSDynaMetaData::ROAD_SURFACE, wordSize, 4>(buff, this->Parts, this->P, 5, SVTK_QUAD);
  }
  return 0;
}

//-----------------------------------------------------------------------------
int svtkLSDynaReader::ReadConnectivityAndMaterial()
{
  LSDynaMetaData* p = this->P;
  if (p->ConnectivityUnpacked == 0)
  {
    // FIXME
    svtkErrorMacro("Packed connectivity isn't supported yet.");
    return 1;
  }
...
//-----------------------------------------------------------------------------
template <typename T, int blockType, svtkIdType numWordsPerCell, svtkIdType cellLength>
void svtkLSDynaReader::ReadBlockCellSizes()
{
  // determine the relationship between the file bit size and
  // the host machine bit size. This allows us to read 64 bit files on a
  // 32 bit machine
  ...
  svtkIdType nc = 0, t = 0, j = 0, matlId = 0;
  svtkIdType numCellsToSkip = 0, numCellsToSkipEnd = 0, chunkSize = 0;
  ...
  // buffer the amount in small chunks so we don't create a massive buffer
  svtkIdType numChunks = this->P->Fam.InitPartialChunkBuffering(nc, numWordsPerCell);
  for (svtkIdType i = 0; i < numChunks; ++i)
  {
...
//-----------------------------------------------------------------------------
template <typename T>
int svtkLSDynaReader::FillPartSizes()
{
  // READ PARTICLES
  this->P->Fam.SkipToWord(LSDynaFamily::SPHNodeData, this->P->Fam.GetCurrentAdaptLevel(), 0);
...
//-----------------------------------------------------------------------------
int svtkLSDynaReader::ReadPartSizes()
{
  LSDynaMetaData* p = this->P;
  if (p->ConnectivityUnpacked == 0)
  {
  ...
  if (p->Fam.GetWordSize() == 8)
  {
    return this->FillPartSizes<svtkIdType>();
  }
  else
  {
...
//-----------------------------------------------------------------------------
void svtkLSDynaReader::SetDeformedMesh(svtkTypeBool deformed)
{
...
svtkSmartPointer<T> svtkComputeDifference(T* aArray, T* bArray)
{
  ...
  const svtkIdType numTuples = aArray->GetNumberOfTuples();
  ...
  svtkVector<typename T::ValueType, NumberOfComponents> tupleA, tupleB;
  svtkSmartPointer<T> result = svtkSmartPointer<T>::New();
  result->SetNumberOfComponents(NumberOfComponents);
  result->SetNumberOfTuples(numTuples);
  for (svtkIdType cc = 0, max = numTuples; cc < max; ++cc)
  {
...
//-----------------------------------------------------------------------------
int svtkLSDynaReader::ComputeDeflectionAndUpdateGeometry(svtkUnstructuredGrid* ug)
{
  ...
  LSDynaMetaData* p = this->P;
  svtkDataArray* deflectedCoords =
    ug ? ug->GetPointData()->GetArray(LS_ARRAYNAME_DEFLECTION) : nullptr;
  if (deflectedCoords)
  {
    svtkSmartPointer<svtkDataArray> deflection;
    if (p->Fam.GetWordSize() == 8)
    {
      deflection =
        svtkComputeDifference<svtkDoubleArray, 3>(svtkDoubleArray::SafeDownCast(deflectedCoords),
          svtkDoubleArray::SafeDownCast(ug->GetPoints()->GetData()));
    }
    else
    {
      deflection =
        svtkComputeDifference<svtkFloatArray, 3>(svtkFloatArray::SafeDownCast(deflectedCoords),
          svtkFloatArray::SafeDownCast(ug->GetPoints()->GetData()));
    }
