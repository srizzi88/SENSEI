...
class TestGraphWriterReader(Testing.svtkTest):

    def test(self):

        mdgTree = dm.svtkMutableDirectedGraph()
        mug = dm.svtkMutableUndirectedGraph()
        ...
        w = il.svtkGraphWriter()

        tmpdir = svtkGetTempDir()
        fname = tmpdir+"/testgrapwriread.svtk"

        w.SetFileName(fname)
        w.SetInputData(mdgTree)
        w.Write()

        r = il.svtkGraphReader()
        r.SetFileName(fname)
        r.Update()

        g = r.GetOutput()

        self.assertTrue(g.IsA("svtkDirectedGraph"))
        ...
        self.assertTrue(g.IsA("svtkUndirectedGraph"))
        self.assertTrue(g.GetNumberOfVertices() == 10)
        self.assertTrue(g.GetNumberOfEdges() == 9)
