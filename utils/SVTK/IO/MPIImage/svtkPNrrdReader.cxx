...
#ifdef MPI_VERSION
#if (MPI_VERSION >= 2)
#define SVTK_USE_MPI_IO 1
#endif
#endif
#if !defined(SVTK_USE_MPI_IO) && defined(ROMIO_VERSION)
#define SVTK_USE_MPI_IO 1
#endif
#if !defined(SVTK_USE_MPI_IO) && defined(MPI_SEEK_SET)
#define SVTK_USE_MPI_IO 1
#endif

// If SVTK_USE_MPI_IO is set, that means we will read the data ourself using
// MPIIO.  Otherwise, just delegate everything to the superclass.

#ifdef SVTK_USE_MPI_IO
...
#define MPICall(funcall)                                                                           \
  {                                                                                                \
    ...
    {                                                                                              \
      ...
      svtkErrorMacro(<< "Received error when calling" << endl                                       \
                    << #funcall << endl                                                            \
                    << endl                                                                        \
                    << errormsg);                                                                  \
    }                                                                                              \
  }
#endif // SVTK_USE_MPI_IO

//-----------------------------------------------------------------------------

svtkStandardNewMacro(svtkPNrrdReader);
svtkCxxSetObjectMacro(svtkPNrrdReader, Controller, svtkMultiProcessController);
svtkCxxSetObjectMacro(svtkPNrrdReader, GroupedController, svtkMultiProcessController);

//-----------------------------------------------------------------------------
#ifdef SVTK_USE_MPI_IO
template <class T>
inline void svtkPNrrdReaderMaskBits(T* data, svtkIdType length, svtkTypeUInt64 _mask)
{
  T mask = (T)_mask;

  // If the mask is the identity, just return.
  if ((_mask == (svtkTypeUInt64)~0UL) || (mask == (T)~0) || (_mask == 0))
    return;
...
template <>
void svtkPNrrdReaderMaskBits(float*, svtkIdType, svtkTypeUInt64)
{
  return;
}
template <>
void svtkPNrrdReaderMaskBits(double*, svtkIdType, svtkTypeUInt64)
{
  return;
}
#endif // SVTK_USE_MPI_IO
...
//-----------------------------------------------------------------------------
svtkPNrrdReader::svtkPNrrdReader()
{
...
void svtkPNrrdReader::PrintSelf(ostream& os, svtkIndent indent)
{
...
//-----------------------------------------------------------------------------
int svtkPNrrdReader::GetDataScalarTypeSize()
{
  switch (this->GetDataScalarType())
  {
    svtkTemplateMacro(return sizeof(SVTK_TT));
    default:
...
//-----------------------------------------------------------------------------
#ifdef SVTK_USE_MPI_IO
void svtkPNrrdReader::PartitionController(const int extent[6])
{
  ...
  if (numZ >= 32768)
  {
    svtkErrorMacro("I do not support more than 32768 files.");
    return;
  }
  ...
  svtkMultiProcessController* subController = this->Controller->PartitionController(extentHash, 0);
  this->SetGroupedController(subController);
  subController->Delete();
}
#else  // SVTK_USE_MPI_IO
void svtkPNrrdReader::PartitionController(const int*)
{
  svtkErrorMacro(<< "svtkPNrrdReader::PartitionController() called when MPIIO "
                << "not available.");
}
#endif // SVTK_USE_MPI_IO
...
#ifdef SVTK_USE_MPI_IO
unsigned long svtkPNrrdReader::GetHeaderSize(svtkMPIOpaqueFileHandle& file)
{
...
#else  // SVTK_USE_MPI_IO
unsigned long svtkPNrrdReader::GetHeaderSize(svtkMPIOpaqueFileHandle&)
{
  svtkErrorMacro(<< "svtkPNrrdReader::GetHeaderSize() called when MPIIO "
                << "not available.");
  return 0;
}
#endif // SVTK_USE_MPI_IO
...
//-----------------------------------------------------------------------------
#ifdef SVTK_USE_MPI_IO
void svtkPNrrdReader::SetupFileView(svtkMPIOpaqueFileHandle& file, const int extent[6])
{
  int arrayOfSizes[3];
...
#else  // SVTK_USE_MPI_IO
void svtkPNrrdReader::SetupFileView(svtkMPIOpaqueFileHandle&, const int[6])
{
  svtkErrorMacro(<< "svtkPNrrdReader::SetupFileView() called when MPIIO "
                << "not available.");
}
#endif // SVTK_USE_MPI_IO
...
//-----------------------------------------------------------------------------
#ifdef SVTK_USE_MPI_IO
void svtkPNrrdReader::ReadSlice(int slice, const int extent[6], void* buffer)
{
  this->ComputeInternalFileName(slice);

  svtkMPICommunicator* mpiComm =
    svtkMPICommunicator::SafeDownCast(this->GroupedController->GetCommunicator());

  // Open the file for this slice.
  svtkMPIOpaqueFileHandle file;
  int result;
  result = MPI_File_open(*mpiComm->GetMPIComm()->GetHandle(), this->InternalFileName,
    MPI_MODE_RDONLY, MPI_INFO_NULL, &file.Handle);
  if (!(result == MPI_SUCCESS))
  {
    svtkErrorMacro("Could not open file: " << this->InternalFileName);
    return;
  }
  ...
  svtkIdType length = this->GetDataScalarTypeSize();
  ...
  {
    MPI_Status stat;
    // we know this will fit in an int because it can't exceed SVTK_INT_MAX.
    const int remaining =
      static_cast<int>(std::min(length - pos, static_cast<svtkIdType>(SVTK_INT_MAX)));
    MPICall(
    ...
    if (MPI_UNDEFINED == rd)
    {
      svtkErrorMacro("Error obtaining number of values read in " << remaining << "-byte read.");
    }
    pos += static_cast<svtkIdType>(rd);
  }

  MPICall(MPI_File_close(&file.Handle));
}
#else  // SVTK_USE_MPI_IO
void svtkPNrrdReader::ReadSlice(int, const int[6], void*)
{
  svtkErrorMacro(<< "svtkPNrrdReader::ReadSlice() called with MPIIO "
                << "not available.");
}
#endif // SVTK_USE_MPI_IO
...
void svtkPNrrdReader::TransformData(svtkImageData* data)
{
  if (!this->Transform)
    return;

  svtkDataArray* fileData = data->GetPointData()->GetScalars();
  svtkDataArray* dataData = fileData->NewInstance();
  ...
  svtkIdType dataMinExtent[3];
  svtkIdType fileMinExtent[3];
  svtkIdType dataExtentSize[3];
  svtkIdType fileExtentSize[3];
  for (int i = 0; i < 3; i++)
  {
  ...
  for (svtkIdType file_k = 0; file_k < fileExtentSize[2]; file_k++)
  {
    for (svtkIdType file_j = 0; file_j < fileExtentSize[1]; file_j++)
    {
      for (svtkIdType file_i = 0; file_i < fileExtentSize[0]; file_i++)
      {
        double fileXYZ[3];
        ...
        svtkIdType data_i = static_cast<svtkIdType>(dataXYZ[0]) - dataMinExtent[0];
        svtkIdType data_j = static_cast<svtkIdType>(dataXYZ[1]) - dataMinExtent[1];
        svtkIdType data_k = static_cast<svtkIdType>(dataXYZ[2]) - dataMinExtent[2];

        svtkIdType fileTuple = ((file_k * fileExtentSize[1] + file_j) * fileExtentSize[0]) + file_i;
        svtkIdType dataTuple = ((data_k * dataExtentSize[1] + data_j) * dataExtentSize[0]) + data_i;
...
#else  // SVTK_USE_MPI_IO
void svtkPNrrdReader::TransformData(svtkImageData*)
{
  svtkErrorMacro(<< "svtkPNrrdReader::TransformData() called with MPIIO "
                << "not available.");
}
#endif // SVTK_USE_MPI_IO

//-----------------------------------------------------------------------------
void svtkPNrrdReader::ExecuteDataWithInformation(svtkDataObject* output, svtkInformation* outInfo)
{
#ifdef SVTK_USE_MPI_IO
  svtkMPIController* MPIController = svtkMPIController::SafeDownCast(this->Controller);
  if (!MPIController)
  {
  ...
  {
    svtkErrorMacro("Either a valid FileName, FileNames, or FilePattern"
                  " must be specified.");
    return;
  }

  // SVTK stores images in traditional "right handed" coordinates.  That is, the
  ...
  // to flip the y axis.
  svtkTransform* saveTransform = this->Transform;
  ...
  svtkDataArray* outputDataArray = data->GetPointData()->GetScalars();
  ...
  svtkDebugMacro("Reading extent: " << inExtent[0] << ", " << inExtent[1] << ", " << inExtent[2]
                                   << ", " << inExtent[3] << ", " << inExtent[4] << ", "
                                   << inExtent[5]);

  // Respect the Transform.
  int outExtent[6];
  svtkIdType outIncrements[3];
  ...
  {
    svtkByteSwap::SwapVoidRange(dataBuffer, numValues, typeSize);
  }

  // Mask bits as necessary.
  switch (this->GetDataScalarType())
  {
    svtkTemplateMacro(svtkPNrrdReaderMaskBits((SVTK_TT*)dataBuffer, numValues, this->DataMask));
  }
...
//-----------------------------------------------------------------------------
int svtkPNrrdReader::ReadHeader()
{
  if (!this->Controller)
  {
    svtkErrorMacro(<< "No MPI Controller available.");
    return 0;
  }

  SVTK_CREATE(svtkCharArray, headerBuffer);
...
//-----------------------------------------------------------------------------
int svtkPNrrdReader::ReadHeader(svtkCharArray* headerBuffer)
{
  return this->Superclass::ReadHeader(headerBuffer);
}
