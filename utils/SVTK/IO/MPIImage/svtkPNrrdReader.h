...
/**
 * @class   svtkPNrrdReader
 * @brief   Read nrrd files efficiently from parallel file systems (and reasonably well elsewhere).
 *
 *
 *
 * svtkPNrrdReader is a subclass of svtkNrrdReader that will read Nrrd format
...
#ifndef svtkPNrrdReader_h
#define svtkPNrrdReader_h
...
class SVTKIOMPIIMAGE_EXPORT svtkPNrrdReader : public svtkNrrdReader
{
public:
  svtkTypeMacro(svtkPNrrdReader, svtkNrrdReader);
  static svtkPNrrdReader* New();
...
protected:
  svtkPNrrdReader();
  ~svtkPNrrdReader() override;

  virtual int ReadHeader() override;
  virtual int ReadHeader(svtkCharArray* headerBuffer) override;
...
private:
  svtkPNrrdReader(const svtkPNrrdReader&) = delete;
  void operator=(const svtkPNrrdReader&) = delete;
};

#endif // svtkPNrrdReader_h
