/*=========================================================================
  ...
  Module:    svtkPWindBladeReader.cxx
...
#include "svtkPWindBladeReader.h"
...
svtkStandardNewMacro(svtkPWindBladeReader);
...
//-----------------------------------------------------------------------------
svtkPWindBladeReader::svtkPWindBladeReader()
{
...
//-----------------------------------------------------------------------------
void svtkPWindBladeReader::PrintSelf(ostream& os, svtkIndent indent)
{
  this->Superclass::PrintSelf(os, indent);
}

//-----------------------------------------------------------------------------
int svtkPWindBladeReader::RequestData(
  svtkInformation* reqInfo, svtkInformationVector** inVector, svtkInformationVector* outVector)
{
  if (!svtkMPIController::GetGlobalController()->IsA("svtkMPIController"))
  {
    // serial case
  ...
  else if (port == 1)
  {
    if (this->UseTurbineFile == 1 &&
      svtkMultiProcessController::GetGlobalController()->GetLocalProcessId() == 0)
    {
...
//----------------------------------------------------------------------------
void svtkPWindBladeReader::CalculatePressure(int pressure, int prespre, int tempg, int density)
{
  if (!svtkMPIController::GetGlobalController()->IsA("svtkMPIController"))
  {
...
//----------------------------------------------------------------------------
void svtkPWindBladeReader::CalculateVorticity(int vort, int uvw, int density)
{
  if (!svtkMPIController::GetGlobalController()->IsA("svtkMPIController"))
...
//----------------------------------------------------------------------------
void svtkPWindBladeReader::LoadVariableData(int var)
{
  if (!svtkMPIController::GetGlobalController()->IsA("svtkMPIController"))
...
// Load one variable data array of BLOCK structure into ParaView
//----------------------------------------------------------------------------
bool svtkPWindBladeReader::ReadGlobalData()
{
  if (!svtkMPIController::GetGlobalController()->IsA("svtkMPIController"))
  {
  ...
  std::vector<char> inBuf(svtkWindBladeReader::LINE_SIZE);
  MPI_File tempFile;
  char native[7] = "native";
  ...
  for (i = 0; i < tempSize; i = i + svtkWindBladeReader::LINE_SIZE)
  {
    if (i + svtkWindBladeReader::LINE_SIZE > tempSize)
    {
    ...
    {
      MPICall(
        MPI_File_read_all(tempFile, &(inBuf[0]), svtkWindBladeReader::LINE_SIZE, MPI_BYTE, &status));
      inStr.write(&(inBuf[0]), svtkWindBladeReader::LINE_SIZE);
    }
  }
...
//
//----------------------------------------------------------------------------
bool svtkPWindBladeReader::FindVariableOffsets()
{
  if (!svtkMPIController::GetGlobalController()->IsA("svtkMPIController"))
...
//----------------------------------------------------------------------------
void svtkPWindBladeReader::CreateZTopography(float* zValues)
{
  if (!svtkMPIController::GetGlobalController()->IsA("svtkMPIController"))
...
//----------------------------------------------------------------------------
void svtkPWindBladeReader::SetupBladeData()
{
  if (!svtkMPIController::GetGlobalController()->IsA("svtkMPIController"))
  ...
  for (i = 0; i < tempSize; i = i + svtkWindBladeReader::LINE_SIZE)
  {
    ...
    else
    {
      MPICall(
        MPI_File_read_all(tempFile, &(inBuf[0]), svtkWindBladeReader::LINE_SIZE, MPI_BYTE, &status));
      inStr2.write(&(inBuf[0]), svtkWindBladeReader::LINE_SIZE);
    }
  }
      ...
      for (i = 0; i < tempSize; i = i + svtkWindBladeReader::LINE_SIZE)
      {
        ...
        else
        {
          MPICall(MPI_File_read_all(
            tempFile, &(inBuf[0]), svtkWindBladeReader::LINE_SIZE, MPI_BYTE, &status));
          inStr2.write(&(inBuf[0]), svtkWindBladeReader::LINE_SIZE);
        }
      }
      MPICall(MPI_File_close(&tempFile));
  ...
  if (numColumns >= 13 && inStr2)
  {
    ...
    while (inStr2.getline(&(inBuf[0]), svtkWindBladeReader::LINE_SIZE) &&
...
//----------------------------------------------------------------------------
void svtkPWindBladeReader::LoadBladeData(int timeStep)
{
  if (!svtkMPIController::GetGlobalController()->IsA("svtkMPIController"))
  ...
  for (i = 0; i < tempSize; i = i + svtkWindBladeReader::LINE_SIZE)
  {
    ...
    {
      MPICall(
        MPI_File_read(tempFile, &(inBuf[0]), svtkWindBladeReader::LINE_SIZE, MPI_BYTE, &status));
      inStr.write(&(inBuf[0]), svtkWindBladeReader::LINE_SIZE);
    }
  }
  MPICall(MPI_File_close(&tempFile));
