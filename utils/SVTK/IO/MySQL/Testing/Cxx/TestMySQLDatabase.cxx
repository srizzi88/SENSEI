...
//
// You should set SVTK_MYSQL_TEST_URL to the address of a MySQL
// database that you can use for testing.  For example, if you have
// MySQL running on foo.mycompany.com, your test user is called
// 'svtktest' and the database is called 'elephant', the URL looks like
// 'mysql://svtktest@foo.mycompany.com/elephant'.
//
// We recommend creating a user and a 'svtktest' database, then giving
// the user in question rights *only* to that database.  The password
// 'svtktest' is hard-coded into this file for the moment until we
...
#include "svtkTable.h"
#include "svtkTimePointUtility.h"
...
int TestMySQLDatabase(int, char** const)
{
  ...
  svtkMySQLDatabase* tmp = svtkMySQLDatabase::New();

  bool status = db->Open("svtktest");
  ...
  db = svtkMySQLDatabase::SafeDownCast(svtkSQLDatabase::CreateFromURL(SVTK_MYSQL_TEST_URL));
  status = db->Open("svtktest");
  ...
  std::vector<svtkStdString> tables;
  while (query->NextRow())
  {
  ...
  {
    svtkStdString tblName(schema->GetTableNameFromHandle(tblHandle));
    cerr << "   Table: " << tblName << "\n";
    ...
    while (query->NextRow())
    {
      ...
      svtkStdString colName(schema->GetIndexColumnNameFromHandle(tblHandle, idxHandle, cnmHandle));
      for (int field = 0; field < numFields; ++field)
      {
        if (field)
        {
  ...
  queryStr = "INSERT INTO atable (somename,somenmbr) VALUES ( " +
    query->EscapeString(svtkStdString("Str\"ang'eS\ntring"), true) + ", 2 )";
  query->SetQuery(queryStr);
  if (!query->Execute())
  {
    cerr << "Query failed" << endl;
  ...
  for (std::vector<svtkStdString>::iterator it = tables.begin(); it != tables.end(); ++it)
  {
    queryStr = "DROP TABLE ";
    queryStr += *it;
    query->SetQuery(queryStr);
  ...
  query->NextRow();
  svtkTypeUInt64 date = svtkTimePointUtility::ISO8601ToTimePoint(query->DataValue(0).ToString());
  svtkTypeUInt64 time = svtkTimePointUtility::ISO8601ToTimePoint(query->DataValue(1).ToString());
  svtkTypeUInt64 timestamp = svtkTimePointUtility::ISO8601ToTimePoint(query->DataValue(2).ToString());
  svtkTypeUInt64 datetime = svtkTimePointUtility::ISO8601ToTimePoint(query->DataValue(3).ToString());
  int month, day, year, hour, minute, second, msec;
  svtkTimePointUtility::GetDate(date, year, month, day);
  if (year != 2008 || month != 1 || day != 1)
  {
    cerr << "Date read incorrectly" << endl;
    return 1;
  }
  svtkTimePointUtility::GetTime(time, hour, minute, second, msec);
  if (hour != 1 || minute != 23 || second != 45)
  {
    cerr << "Time read incorrectly" << endl;
    return 1;
  }
  svtkTimePointUtility::GetDateTime(timestamp, year, month, day, hour, minute, second, msec);
  if (year != 2008 || month != 1 || day != 1 || hour != 1 || minute != 23 || second != 45)
  {
    cerr << "Timestamp read incorrectly" << endl;
    return 1;
  }
  svtkTimePointUtility::GetDateTime(datetime, year, month, day, hour, minute, second, msec);
  if (year != 2008 || month != 1 || day != 1 || hour != 1 || minute != 23 || second != 45)
  {
