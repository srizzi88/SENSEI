...
#ifndef svtkMPASReader_h
#define svtkMPASReader_h
...
class SVTKIONETCDF_EXPORT svtkMPASReader : public svtkUnstructuredGridAlgorithm
{
public:
  static svtkMPASReader* New();
  svtkTypeMacro(svtkMPASReader, svtkUnstructuredGridAlgorithm);
   ...
   */
  svtkGetMacro(MaximumPoints, int);
  //@}
  ...
  svtkUnstructuredGrid* GetOutput(int index);
  //@}
   ...
   */
  svtkSetMacro(UseDimensionedArrayNames, bool);
  svtkGetMacro(UseDimensionedArrayNames, bool);
  svtkBooleanMacro(UseDimensionedArrayNames, bool);
  //@}
   ...
   */
  svtkIdType GetNumberOfDimensions();
  std::string GetDimensionName(int idx);
  svtkStringArray* GetAllDimensions();
   ...
   */
  svtkSetMacro(VerticalDimension, std::string);
  svtkGetMacro(VerticalDimension, std::string);
  //@}
  ...
  svtkGetVector2Macro(VerticalLevelRange, int);

  svtkSetMacro(LayerThickness, int);
  svtkGetMacro(LayerThickness, int);
  svtkGetVector2Macro(LayerThicknessRange, int);

  void SetCenterLon(int val);
  svtkGetVector2Macro(CenterLonRange, int);

  svtkSetMacro(ProjectLatLon, bool);
  svtkGetMacro(ProjectLatLon, bool);

  svtkSetMacro(IsAtmosphere, bool);
  svtkGetMacro(IsAtmosphere, bool);

  svtkSetMacro(IsZeroCentered, bool);
  svtkGetMacro(IsZeroCentered, bool);

  svtkSetMacro(ShowMultilayerView, bool);
  svtkGetMacro(ShowMultilayerView, bool);
...
protected:
  ...
  svtkDataArray* LoadPointVarData(int variable);
  svtkDataArray* LoadCellVarData(int variable);
  svtkDataArray* LookupPointDataArray(int varIdx);
  svtkDataArray* LookupCellDataArray(int varIdx);

  /**
   * Update the "Time" svtkStringArray in dataset's FieldData to contain the
   ...
   */
  void LoadTimeFieldData(svtkUnstructuredGrid* dataset);

private:
  svtkMPASReader(const svtkMPASReader&) = delete;
  void operator=(const svtkMPASReader&) = delete;
