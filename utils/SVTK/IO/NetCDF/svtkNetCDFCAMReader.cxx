/*=========================================================================
  ...
  Module:    svtkNetCDFCAMReader.cxx
...
bool IsCellInverted(double points[4][3])
{
  ...
  // See BUG #0014897.
  double normal[3];
  svtkPolygon::ComputeNormal(3, points[0], normal);
  if (normal[2] > 0)
  {
    return true;
  }
  svtkPolygon::ComputeNormal(3, points[1], normal);
  if (normal[2] > 0)
  {
    return true;
  }
...
class svtkNetCDFCAMReader::Internal
{
public:
  Internal(svtkNetCDFCAMReader* r)
    : reader(r)
    , nc_points(-1)
    , nc_connectivity(-1)
  {
  ...
  svtkNetCDFCAMReader* reader;
  int nc_points;
  int nc_connectivity;
};

bool svtkNetCDFCAMReader::Internal::nc_err(int nc_ret, bool msg_on_err) const
{
  if (nc_ret == NC_NOERR)
  {
    return false;
...
std::string svtkNetCDFCAMReader::Internal::GetNameDimension(int nc_file, int nc_var) const
{
  int ndims;
...
//----------------------------------------------------------------------------
svtkStandardNewMacro(svtkNetCDFCAMReader);

//----------------------------------------------------------------------------
svtkNetCDFCAMReader::svtkNetCDFCAMReader()
{
  ...
  this->SelectionObserver->SetCallback(&svtkNetCDFCAMReader::SelectionCallback);
...
//----------------------------------------------------------------------------
int svtkNetCDFCAMReader::CanReadFile(const char* fileName)
{
...
//----------------------------------------------------------------------------
void svtkNetCDFCAMReader::SetFileName(const char* fileName)
{
  svtkDebugMacro(<< " setting FileName to " << (fileName ? fileName : "(null)"));
...
//----------------------------------------------------------------------------
void svtkNetCDFCAMReader::SetConnectivityFileName(const char* fileName)
{
  svtkDebugMacro(<< " setting ConnectivityFileName to " << (fileName ? fileName : "(null)"));
...
//----------------------------------------------------------------------------
int svtkNetCDFCAMReader::RequestInformation(svtkInformation* svtkNotUsed(reqInfo),
  svtkInformationVector** svtkNotUsed(inputVector), svtkInformationVector* outputVector)
{
  if (this->FileName == nullptr)
  {
    svtkWarningMacro("Missing a file name.");
    return 0;
  }
  ...
  if (this->Internals->nc_points == -1)
  {
    ...
    {
      svtkErrorMacro(<< "Can't read file " << this->FileName);
      return 0;
    }
  ...
  {
    svtkErrorMacro("Cannot find the number of time steps (time dimension).");
    return 0;
  }
  size_t size;
  if (this->Internals->nc_err(nc_inq_dimlen(this->Internals->nc_points, dimid, &size)))
  ...
  if (this->NumberOfTimeSteps > 0)
  {
    ...
    outInfo->Set(svtkStreamingDemandDrivenPipeline::TIME_STEPS(), this->TimeSteps,
...
//----------------------------------------------------------------------------
void svtkNetCDFCAMReader::BuildVarArray()
{
...
//----------------------------------------------------------------------------
int svtkNetCDFCAMReader::RequestUpdateExtent(
...
//----------------------------------------------------------------------------
int svtkNetCDFCAMReader::GetNumberOfPointArrays()
{
  return this->PointDataArraySelection->GetNumberOfArrays();
}

//----------------------------------------------------------------------------
const char* svtkNetCDFCAMReader::GetPointArrayName(int index)
{
  return this->PointDataArraySelection->GetArrayName(index);
}

//----------------------------------------------------------------------------
int svtkNetCDFCAMReader::GetPointArrayStatus(const char* name)
{
  return this->PointDataArraySelection->ArrayIsEnabled(name);
}

//----------------------------------------------------------------------------
void svtkNetCDFCAMReader::SetPointArrayStatus(const char* name, int status)
{
  if (status)
  {
...
//----------------------------------------------------------------------------
void svtkNetCDFCAMReader::DisableAllPointArrays()
{
  this->PointDataArraySelection->DisableAllArrays();
}

//----------------------------------------------------------------------------
void svtkNetCDFCAMReader::EnableAllPointArrays()
{
  this->PointDataArraySelection->EnableAllArrays();
}

//----------------------------------------------------------------------------
int svtkNetCDFCAMReader::RequestData(
  svtkInformation*, svtkInformationVector**, svtkInformationVector* outputVector)
{
  if (this->FileName == nullptr || this->ConnectivityFileName == nullptr)
  {
  ...
  svtkDebugMacro(<< "Reading NetCDF CAM file.");
  ...
  if (this->Internals->nc_connectivity == -1)
  {
    ...
    {
      svtkErrorMacro(<< "Can't read file " << this->ConnectivityFileName);
      return 0;
    }
    ...
    this->VerticalDimension == VERTICAL_DIMENSION_INTERFACE_LAYERS)
  {
    ...
    {
      svtkErrorMacro("Cannot find the number of levels (lev dimension).");
      return 0;
    }
    ...
    {
      svtkErrorMacro("Cannot find the number of levels (lev variable).");
      return 0;
    }
    int ndims;
    ...
    {
      svtkErrorMacro("The lev variable is not consistent.");
      return 0;
    }
  }
  int dimid;
  ...
  {
    svtkErrorMacro("Cannot find the number of points (ncol dimension).");
    return 0;
  }
  int lonid;
  if (this->Internals->nc_err(nc_inq_varid(this->Internals->nc_points, "lon", &lonid)))
  {
    svtkErrorMacro("Cannot find coordinates (lon variable).");
    return 0;
  }
  int latid;
  if (this->Internals->nc_err(nc_inq_varid(this->Internals->nc_points, "lat", &latid)))
  {
    svtkErrorMacro("Cannot find coordinates (lat variable).");
    return 0;
  }
  ...
  if (var_type == NC_DOUBLE)
  {
    ...
    {
      points->SetPoint(static_cast<svtkIdType>(i), array[i], array[i + numFilePoints], numLevels);
    }
  }
  else
  {
    ...
    {
      points->SetPoint(
        static_cast<svtkIdType>(i), array[i], array[i + numFilePoints], numLevels - 1);
    }
  }
  ...
  std::vector<svtkIdType> boundaryPoints;

  // To avoid creating multiple duplicates, we create a
  // svtkIncrementalOctreePointLocator.
  svtkSmartPointer<svtkIncrementalOctreePointLocator> locator =
    svtkSmartPointer<svtkIncrementalOctreePointLocator>::New();
  ...
  {
    svtkErrorMacro("Cannot find the number of cells (ncells dimension).");
    return 0;
  }
  int connid;
  if (this->Internals->nc_err(
  ...
  {
    svtkErrorMacro("Cannot find cell connectivity (element_corners dimension).");
    return 0;
  }
  ...
  size_t piece = outInfo->Get(svtkStreamingDemandDrivenPipeline::UPDATE_PIECE_NUMBER());
  size_t numPieces = outInfo->Get(svtkStreamingDemandDrivenPipeline::UPDATE_NUMBER_OF_PIECES());
    ...
    if (IsCellInverted(coords) == true)
    {
      ...
      for (int j = 0; j < 4; ++j)
      {
        ...
        if (locator->InsertUniquePoint(coords[j], newPtId) == 1)
        {
          ...
          assert(newPtId >= static_cast<svtkIdType>(numFilePoints) && pointIds[j] < newPtId);
  ...
  svtkIdType numPointsPerLevel = points->GetNumberOfPoints();
    ...
    originalNumLevels == numLevels)
  {
    ...
    points->InsertPoint(static_cast<svtkIdType>(numPointsPerLevel * numLocalLevels - 1), 0, 0, 0);
    for (svtkIdType pt = 0; pt < numPointsPerLevel; pt++)
    {
      double point[3];
      ...
      for (size_t lev = 0; lev < numLocalLevels; lev++)
      {
        ...
        points->SetPoint(static_cast<svtkIdType>(pt + lev * numPointsPerLevel), point);
      }
    }
  }
  ...
  for (int i = 0; i < nvars; i++)
  {
    ...
    svtkDoubleArray* doubleArray = nullptr;
    svtkFloatArray* floatArray = nullptr;
    ...
    if (this->VerticalDimension != VERTICAL_DIMENSION_SINGLE_LAYER)
    {
      for (size_t lev = 0; lev < numLocalLevels; lev++)
      {
        ...
        if (doubleArray)
        {
          ...
          {
            svtkErrorMacro("Problem getting NetCDF variable " << varname);
            return 0;
          }
        }
        else
  ...
  svtkIdType newPtId = 0;
  for (std::vector<svtkIdType>::const_iterator it = boundaryPoints.begin();
       it != boundaryPoints.end(); ++it, ++newPtId)
  {
    for (size_t lev = 0; lev < numLocalLevels; lev++)
    {
      svtkIdType srcId = static_cast<svtkIdType>((*it) + lev * numPointsPerLevel);
      svtkIdType destId =
        static_cast<svtkIdType>((newPtId + numFilePoints) + lev * numPointsPerLevel);
  ...
  if (this->VerticalDimension != VERTICAL_DIMENSION_SINGLE_LAYER)
  {
    ...
    svtkNew<svtkFloatArray> levelPointData;
    levelPointData->SetName(levName);
    levelPointData->SetNumberOfTuples(points->GetNumberOfPoints());
    for (size_t j = 0; j < numLocalLevels; j++)
    {
      for (svtkIdType i = 0; i < numPointsPerLevel; i++)
      {
        levelPointData->SetValue(static_cast<svtkIdType>(j * numPointsPerLevel + i), levelData[j]);
      }
    }
  ...
  {
    output->Allocate(static_cast<svtkIdType>(numLocalCells));
  }
  else
  {
    // we have numLocalLevels points so we have (numLocalLevels-1) cells.
    output->Allocate(static_cast<svtkIdType>(numLocalCells * (numLocalLevels - 1)));
  }
  for (size_t i = 0; i < numLocalCells; i++)
  {
    svtkIdType pointIds[4];
      ...
      numLevels == originalNumLevels)
    {
      ...
      for (size_t lev = 0; lev < (numLocalLevels - 1); lev++)
      {
        ...
        for (int j = 0; j < 4; j++)
        {
          hexIds[j] = static_cast<svtkIdType>(pointIds[j] + lev * numPointsPerLevel);
          hexIds[j + 4] = static_cast<svtkIdType>(pointIds[j] + (1 + lev) * numPointsPerLevel);
        }
    ...
    else
    {
      ...
      output->InsertNextCell(SVTK_QUAD, 4, pointIds);
    }
  }
  ...
  if (numLocalCells != numCellsPerLevel)
  {
    ...
    // svtkNew<svtkCleanUnstructuredGrid> cleanGrid;
...
//----------------------------------------------------------------------------
bool svtkNetCDFCAMReader::GetPartitioning(size_t piece, size_t numPieces, size_t numLevels,
  size_t numCellsPerLevel, size_t& beginLevel, size_t& endLevel, size_t& beginCell, size_t& endCell)
{
  ...
  {
    svtkErrorMacro("Bad piece information for partitioning.");
    return false;
  }
  ...
  int levelsPerPiece = svtkMath::Ceil(numLevels / static_cast<double>(numPieces));
  int piecesPerLevel = svtkMath::Ceil(numPieces / static_cast<double>(numLevels));
...
//----------------------------------------------------------------------------
void svtkNetCDFCAMReader::SelectionCallback(
  svtkObject*, unsigned long svtkNotUsed(eventid), void* clientdata, void* svtkNotUsed(calldata))
{
  static_cast<svtkNetCDFCAMReader*>(clientdata)->Modified();
}

//----------------------------------------------------------------------------
void svtkNetCDFCAMReader::PrintSelf(ostream& os, svtkIndent indent)
{
