...
#define CALL_NETCDF(call)                                                                          \
  CALL_NETCDF_GENERIC(call, svtkErrorMacro(<< "netCDF Error: " << errorstring); return 0;)

#define CALL_NETCDF_GW(call)                                                                       \
  CALL_NETCDF_GENERIC(call, svtkGenericWarningMacro(<< "netCDF Error: " << errorstring); return 0;)
...
static bool ReadTextAttribute(int ncFD, int varId, const char* name, svtkStdString& result)
{
  size_t length;
  ...
  // the svtkStdString will do for us).  However, sometimes a null terminating
  // character is written in the attribute anyway.  In a C string this is no big
  // deal.  But it means that the svtkStdString has a null character in it and it
...
// a svtkDoubleArray.
static void GetRangeOfAllComponents(svtkDoubleArray* array, double range[2])
{
...
//=============================================================================
svtkNetCDFCFReader::svtkDimensionInfo::svtkDimensionInfo(int ncFD, int id)
{
  this->DimId = id;
...
int svtkNetCDFCFReader::svtkDimensionInfo::LoadMetaData(int ncFD)
{
  this->Units = UNDEFINED_UNITS;
  ...
  this->Coordinates = svtkSmartPointer<svtkDoubleArray>::New();
  this->Coordinates->SetName((this->Name + "_Coordinates").c_str());
  this->Coordinates->SetNumberOfComponents(1);
  this->Coordinates->SetNumberOfTuples(static_cast<svtkIdType>(dimLen));

  this->Bounds = svtkSmartPointer<svtkDoubleArray>::New();
  this->Bounds->SetName((this->Name + "_Bounds").c_str());
  this->Bounds->SetNumberOfComponents(1);
  this->Bounds->SetNumberOfTuples(static_cast<svtkIdType>(dimLen + 1));

  this->SpecialVariables = svtkSmartPointer<svtkStringArray>::New();
    ...
    (nc_inq_vardimid(ncFD, varId, &varDim) == NC_NOERR) && (varDim == this->DimId))
  {
    ...
    this->Spacing =
      (this->Coordinates->GetValue(static_cast<svtkIdType>(dimLen - 1)) - this->Origin) /
      (dimLen - 1);
    ...
    for (size_t i = 1; i < dimLen; i++)
    {
      ...
      double actualValue = this->Coordinates->GetValue(static_cast<svtkIdType>(i));
    ...
    {
      units = svtksys::SystemTools::LowerCase(units);
      ...
      if ((units.find(" since ") != svtkStdString::npos) ||
        (units.find(" after ") != svtkStdString::npos) || (units == "second") ||
      ...
      else if (svtksys::RegularExpression("degrees?_?n").find(units))
      {
        this->Units = LATITUDE_UNITS;
      }
      else if (svtksys::RegularExpression("degrees?_?e").find(units))
      {
    ...
    // Check positive.
    svtkStdString positive;
    if (ReadTextAttribute(ncFD, varId, "positive", positive))
    {
      positive = svtksys::SystemTools::LowerCase(positive);
      if (positive.find("down") != svtkStdString::npos)
      {
        // Flip the values of the coordinates.
        for (svtkIdType i = 0; i < this->Coordinates->GetNumberOfTuples(); i++)
        {
    ...
    if (ReadTextAttribute(ncFD, varId, "bounds", boundsName))
    {
      ...
      CALL_NETCDF_GW(nc_get_vars_double(ncFD, boundsVarId, start, count, nullptr,
        this->Bounds->GetPointer(static_cast<svtkIdType>(dimLen))));
    }
    else
    {
      // Bounds not given.  Set them based on the coordinates.
      this->Bounds->SetValue(0, this->Coordinates->GetValue(0) - 0.5 * this->Spacing);
      for (svtkIdType i = 1; i < static_cast<svtkIdType>(dimLen); i++)
      {
        double v0 = this->Coordinates->GetValue(i - 1);
        double v1 = this->Coordinates->GetValue(i);
        this->Bounds->SetValue(i, 0.5 * (v0 + v1));
      }
      this->Bounds->SetValue(static_cast<svtkIdType>(dimLen),
        this->Coordinates->GetValue(static_cast<svtkIdType>(dimLen - 1)) + 0.5 * this->Spacing);
    }
  }
  else
  {
    ...
    {
      this->Coordinates->SetValue(static_cast<svtkIdType>(i), static_cast<double>(i));
      this->Bounds->SetValue(static_cast<svtkIdType>(i), static_cast<double>(i) - 0.5);
    }
    this->Bounds->SetValue(static_cast<svtkIdType>(dimLen), static_cast<double>(dimLen) - 0.5);
    this->HasRegularSpacing = true;
    this->Origin = 0.0;
    this->Spacing = 1.0;
  }
...
//-----------------------------------------------------------------------------
class svtkNetCDFCFReader::svtkDimensionInfoVector
{
public:
  std::vector<svtkDimensionInfo> v;
};

//=============================================================================
svtkNetCDFCFReader::svtkDependentDimensionInfo::svtkDependentDimensionInfo(
  int ncFD, int varId, svtkNetCDFCFReader* parent)
{
...
//-----------------------------------------------------------------------------
int svtkNetCDFCFReader::svtkDependentDimensionInfo::LoadMetaData(
  int ncFD, int varId, svtkNetCDFCFReader* parent)
{
  ...
  this->GridDimensions = svtkSmartPointer<svtkIntArray>::New();
  this->SpecialVariables = svtkSmartPointer<svtkStringArray>::New();
  ...
  for (std::vector<std::string>::iterator iter = coordName.begin(); iter != coordName.end(); ++iter)
  {
    ...
    if (!ReadTextAttribute(ncFD, auxCoordVarId, "units", units))
      continue;
    units = svtksys::SystemTools::LowerCase(units);
    if (svtksys::RegularExpression("degrees?_?n").find(units))
    {
  ...
  // supported by svtkDataArray.  It also makes it exceedingly difficult to
  ...
  this->LongitudeCoordinates = svtkSmartPointer<svtkDoubleArray>::New();
  this->LatitudeCoordinates = svtkSmartPointer<svtkDoubleArray>::New();
...
//-----------------------------------------------------------------------------
int svtkNetCDFCFReader::svtkDependentDimensionInfo::LoadCoordinateVariable(
  int ncFD, int varId, svtkDoubleArray* coords)
{
  int dimIds[2];
  ...
  coords->SetNumberOfTuples(static_cast<svtkIdType>(dimSizes[0]));
...
//-----------------------------------------------------------------------------
int svtkNetCDFCFReader::svtkDependentDimensionInfo::LoadBoundsVariable(
  int ncFD, int varId, svtkDoubleArray* coords)
{
  int dimIds[3];
  CALL_NETCDF_GW(nc_inq_vardimid(ncFD, varId, dimIds));
  ...
  if (dimSizes[2] != 4)
  {
    svtkGenericWarningMacro(<< "Expected 2D dependent coordinate bounds to have"
  ...
  svtkIdType numTuples = static_cast<svtkIdType>(dimSizes[0]);
...
//-----------------------------------------------------------------------------
int svtkNetCDFCFReader::svtkDependentDimensionInfo::LoadUnstructuredBoundsVariable(
  int ncFD, int varId, svtkDoubleArray* coords)
{
  int dimIds[2];
  CALL_NETCDF_GW(nc_inq_vardimid(ncFD, varId, dimIds));
  ...
  svtkIdType numCells = static_cast<svtkIdType>(dimSizes[0]);
...
//-----------------------------------------------------------------------------
class svtkNetCDFCFReader::svtkDependentDimensionInfoVector
{
public:
  std::vector<svtkNetCDFCFReader::svtkDependentDimensionInfo> v;
};

//=============================================================================
svtkStandardNewMacro(svtkNetCDFCFReader);

//-----------------------------------------------------------------------------
svtkNetCDFCFReader::svtkNetCDFCFReader()
{
  ...
  this->DimensionInfo = new svtkDimensionInfoVector;
  this->DependentDimensionInfo = new svtkDependentDimensionInfoVector;
}
...
void svtkNetCDFCFReader::PrintSelf(ostream& os, svtkIndent indent)
{
...
//-----------------------------------------------------------------------------
int svtkNetCDFCFReader::CanReadFile(const char* filename)
{
...
//-----------------------------------------------------------------------------
void svtkNetCDFCFReader::SetOutputType(int type)
{
  ...
  {
    bool typeValid = ((type == -1) || (type == SVTK_IMAGE_DATA) || (type == SVTK_RECTILINEAR_GRID) ||
      (type == SVTK_STRUCTURED_GRID) || (type == SVTK_UNSTRUCTURED_GRID));
    if (typeValid)
    {
    ...
    {
      svtkErrorMacro(<< "Invalid OutputType: " << type);
    }
  }
}

//-----------------------------------------------------------------------------
int svtkNetCDFCFReader::RequestDataObject(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** svtkNotUsed(inputVector), svtkInformationVector* outputVector)
{
  ...
  for (int arrayIndex = 0; arrayIndex < numArrays; arrayIndex++)
  {
    ...
    switch (coordType)
    {
      case COORDS_UNIFORM_RECTILINEAR:
        preferredDataType = SVTK_IMAGE_DATA;
        break;
      case COORDS_NONUNIFORM_RECTILINEAR:
        preferredDataType = SVTK_RECTILINEAR_GRID;
        break;
      ...
      case COORDS_SPHERICAL_4SIDED_CELLS:
        preferredDataType = SVTK_STRUCTURED_GRID;
        break;
      case COORDS_EUCLIDEAN_PSIDED_CELLS:
      case COORDS_SPHERICAL_PSIDED_CELLS:
        preferredDataType = SVTK_UNSTRUCTURED_GRID;
        break;
      default:
        svtkErrorMacro(<< "Internal error: unknown coordinate type.");
        return 0;
    }
    ...
    else
    {
      switch (dataType)
      {
        case SVTK_IMAGE_DATA:
          if (preferredDataType != SVTK_IMAGE_DATA)
          {
            svtkWarningMacro("You have set the OutputType to a data type that"
                            " cannot fully represent the topology of the data."
                            " Some of the topology will be ignored.");
          }
          break;
        case SVTK_RECTILINEAR_GRID:
          if ((preferredDataType != SVTK_IMAGE_DATA) && (preferredDataType != SVTK_RECTILINEAR_GRID))
          {
            svtkWarningMacro("You have set the OutputType to a data type that"
                            " cannot fully represent the topology of the data."
                            " Some of the topology will be ignored.");
        ...
        case SVTK_UNSTRUCTURED_GRID:
          ...
          break;
        default:
          svtkErrorMacro(<< "Sanity check failed: bad internal type.");
          return 0;
      }
    }
  ...
  if (dataType == -1)
  {
    ...
    dataType = SVTK_IMAGE_DATA;
  }

  if (dataType == SVTK_IMAGE_DATA)
  {
  ...
  else if (dataType == SVTK_RECTILINEAR_GRID)
  {
    if (!output || !output->IsA("svtkRectilinearGrid"))
    {
  ...
  else if (dataType == SVTK_STRUCTURED_GRID)
  {
    if (!output || !output->IsA("svtkStructuredGrid"))
    {
  ...
  else if (dataType == SVTK_UNSTRUCTURED_GRID)
  {
...
//-----------------------------------------------------------------------------
int svtkNetCDFCFReader::RequestInformation(
  svtkInformation* request, svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  if (output)
  {
    if (output->GetExtentType() != SVTK_3D_EXTENT)
    {
...
//-----------------------------------------------------------------------------
int svtkNetCDFCFReader::RequestData(
  svtkInformation* request, svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  if (output)
  {
    ...
    else if (output->GetExtentType() == SVTK_PIECES_EXTENT)
    {
      int pieceNumber, numberOfPieces, ghostLevels;
      svtkInformation* outInfo = outputVector->GetInformationObject(0);
      pieceNumber = outInfo->Get(svtkStreamingDemandDrivenPipeline::UPDATE_PIECE_NUMBER());
      ...
      output->GetInformation()->Set(svtkStreamingDemandDrivenPipeline::UPDATE_EXTENT(), extent, 6);
    }
    else
    {
      svtkWarningMacro(<< "Invalid extent type encountered.  Data arrays may"
  ...
  svtkImageData* imageOutput = svtkImageData::GetData(outputVector);
  if (imageOutput)
  {
    this->AddRectilinearCoordinates(imageOutput);
  }

  svtkRectilinearGrid* rectilinearOutput = svtkRectilinearGrid::GetData(outputVector);
  ...
  svtkStructuredGrid* structuredOutput = svtkStructuredGrid::GetData(outputVector);
  if (structuredOutput)
  {
    switch (this->CoordinateType(this->LoadingDimensions))
    {
  ...
  svtkUnstructuredGrid* unstructuredOutput = svtkUnstructuredGrid::GetData(outputVector);
...
//-----------------------------------------------------------------------------
void svtkNetCDFCFReader::ExtentForDimensionsAndPiece(
  int pieceNumber, int numberOfPieces, int ghostLevels, int extent[6])
{
  SVTK_CREATE(svtkExtentTranslator, extentTranslator);
...
//-----------------------------------------------------------------------------
void svtkNetCDFCFReader::GetUpdateExtentForOutput(svtkDataSet* output, int extent[6])
{
  svtkInformation* info = output->GetInformation();
  if (info->Has(svtkStreamingDemandDrivenPipeline::UPDATE_EXTENT()))
  {
    info->Get(svtkStreamingDemandDrivenPipeline::UPDATE_EXTENT(), extent);
  }
  else
  {
...
//-----------------------------------------------------------------------------
void svtkNetCDFCFReader::AddRectilinearCoordinates(svtkImageData* imageOutput)
{
  double origin[3];
  ...
  {
    // Remember that netCDF dimension ordering is backward from SVTK.
    int dim = this->LoadingDimensions->GetValue(numDim - i - 1);
    svtkDimensionInfo* dimInfo = this->GetDimensionInfo(dim);
...
//-----------------------------------------------------------------------------
void svtkNetCDFCFReader::AddRectilinearCoordinates(svtkRectilinearGrid* rectilinearOutput)
{
  int extent[6];
  ...
  for (int i = 0; i < 3; i++)
  {
    svtkSmartPointer<svtkDoubleArray> coords;
    if (i < numDim)
    {
      // Remember that netCDF dimension ordering is backward from SVTK.
      int dim = this->LoadingDimensions->GetValue(numDim - i - 1);
      ...
      if ((extLow != 0) || (extHi != coords->GetNumberOfTuples() - 1))
      {
        ...
        SVTK_CREATE(svtkDoubleArray, newcoords);
    ...
    {
      coords = svtkSmartPointer<svtkDoubleArray>::New();
...
//-----------------------------------------------------------------------------
void svtkNetCDFCFReader::FakeRectilinearCoordinates(svtkRectilinearGrid* rectilinearOutput)
{
  ...
  {
    svtkNew<svtkDoubleArray> coordinate;
...
//-----------------------------------------------------------------------------
void svtkNetCDFCFReader::Add1DRectilinearCoordinates(svtkPoints* points, const int extent[6])
{
  points->SetDataTypeToDouble();
  points->SetNumberOfPoints(
    (extent[1] - extent[0] + 1) * (extent[3] - extent[2] + 1) * (extent[5] - extent[4] + 1));
  svtkDataArray* pointData = points->GetData();

  int numDimNetCDF = this->LoadingDimensions->GetNumberOfTuples();
  for (int dimSVTK = 0; dimSVTK < 3; dimSVTK++)
  {
    svtkSmartPointer<svtkDoubleArray> coords;
    if (dimSVTK < numDimNetCDF)
    {
      // Remember that netCDF dimension ordering is backward from SVTK.
      int dimNetCDF = this->LoadingDimensions->GetValue(numDimNetCDF - dimSVTK - 1);
      coords = this->GetDimensionInfo(dimNetCDF)->GetCoordinates();

      int ijk[3];
      svtkIdType pointIdx = 0;
      for (ijk[2] = extent[2 * 2]; ijk[2] <= extent[2 * 2 + 1]; ijk[2]++)
      {
        for (ijk[1] = extent[1 * 2]; ijk[1] <= extent[1 * 2 + 1]; ijk[1]++)
        {
          ...
          {
            pointData->SetComponent(pointIdx, dimSVTK, coords->GetValue(ijk[dimSVTK]));
            pointIdx++;
          }
        }
      }
    ...
    else
    {
      ...
      for (ijk[2] = extent[2 * 2]; ijk[2] <= extent[2 * 2 + 1]; ijk[2]++)
      {
        for (ijk[1] = extent[1 * 2]; ijk[1] <= extent[1 * 2 + 1]; ijk[1]++)
        {
          for (ijk[0] = extent[0 * 2]; ijk[0] <= extent[0 * 2 + 1]; ijk[0]++)
          {
            pointData->SetComponent(pointIdx, dimSVTK, 0.0);
            pointIdx++;
          }
        }
      }
...
//-----------------------------------------------------------------------------
void svtkNetCDFCFReader::Add2DRectilinearCoordinates(svtkPoints* points, const int extent[6])
{
  points->SetDataTypeToDouble();
  points->Allocate(
    (extent[1] - extent[0] + 1) * (extent[3] - extent[2] + 1) * (extent[5] - extent[4] + 1));

  svtkDependentDimensionInfo* info = this->FindDependentDimensionInfo(this->LoadingDimensions);

  svtkDoubleArray* longitudeCoordinates = info->GetLongitudeCoordinates();
  svtkDoubleArray* latitudeCoordinates = info->GetLatitudeCoordinates();

  svtkDoubleArray* verticalCoordinates = nullptr;
...
//-----------------------------------------------------------------------------
void svtkNetCDFCFReader::Add1DRectilinearCoordinates(svtkStructuredGrid* structuredOutput)
{
  int extent[6];
...
//-----------------------------------------------------------------------------
void svtkNetCDFCFReader::Add2DRectilinearCoordinates(svtkStructuredGrid* structuredOutput)
{
  int extent[6];
  structuredOutput->GetExtent(extent);
...
//-----------------------------------------------------------------------------
void svtkNetCDFCFReader::FakeStructuredCoordinates(svtkStructuredGrid* structuredOutput)
{
  int extent[6];
  structuredOutput->GetExtent(extent);
...
//-----------------------------------------------------------------------------
void svtkNetCDFCFReader::Add1DRectilinearCoordinates(
...
//-----------------------------------------------------------------------------
void svtkNetCDFCFReader::Add2DRectilinearCoordinates(
  svtkUnstructuredGrid* unstructuredOutput, const int extent[6])
{
  SVTK_CREATE(svtkPoints, points);
  this->Add2DRectilinearCoordinates(points, extent);
...
//-----------------------------------------------------------------------------
void svtkNetCDFCFReader::Add1DSphericalCoordinates(svtkPoints* points, const int extent[6])
{
  points->SetDataTypeToDouble();
  points->Allocate(
    (extent[1] - extent[0] + 1) * (extent[3] - extent[2] + 1) * (extent[5] - extent[4] + 1));

  svtkDoubleArray* coordArrays[3];
  for (svtkIdType i = 0; i < this->LoadingDimensions->GetNumberOfTuples(); i++)
  {
  ...
  {
    svtkErrorMacro(<< "Internal error: treating non spherical coordinates as if"
                  << " they were spherical.");
    return;
  }
  ...
  for (ijk[0] = extent[4]; ijk[0] <= extent[5]; ijk[0]++)
  {
    for (ijk[1] = extent[2]; ijk[1] <= extent[3]; ijk[1]++)
    {
      for (ijk[2] = extent[0]; ijk[2] <= extent[1]; ijk[2]++)
      {
        ...
        lon = svtkMath::RadiansFromDegrees(lon);
        lat = svtkMath::RadiansFromDegrees(lat);
...
//-----------------------------------------------------------------------------
void svtkNetCDFCFReader::Add2DSphericalCoordinates(svtkPoints* points, const int extent[6])
{
  points->SetDataTypeToDouble();
  points->Allocate(
    (extent[1] - extent[0] + 1) * (extent[3] - extent[2] + 1) * (extent[5] - extent[4] + 1));
...
//-----------------------------------------------------------------------------
void svtkNetCDFCFReader::Add1DSphericalCoordinates(svtkStructuredGrid* structuredOutput)
{
  int extent[6];
  structuredOutput->GetExtent(extent);
...
//-----------------------------------------------------------------------------
void svtkNetCDFCFReader::Add2DSphericalCoordinates(svtkStructuredGrid* structuredOutput)
{
  int extent[6];
  structuredOutput->GetExtent(extent);
...
//-----------------------------------------------------------------------------
void svtkNetCDFCFReader::Add1DSphericalCoordinates(
  svtkUnstructuredGrid* unstructuredOutput, const int extent[6])
{
  SVTK_CREATE(svtkPoints, points);
  this->Add1DSphericalCoordinates(points, extent);
...
//-----------------------------------------------------------------------------
void svtkNetCDFCFReader::Add2DSphericalCoordinates(
  svtkUnstructuredGrid* unstructuredOutput, const int extent[6])
{
  SVTK_CREATE(svtkPoints, points);
  this->Add2DSphericalCoordinates(points, extent);
...
//-----------------------------------------------------------------------------
void svtkNetCDFCFReader::AddStructuredCells(
  svtkUnstructuredGrid* unstructuredOutput, const int extent[6])
{
  svtkIdType numPoints[3];
  numPoints[0] = extent[1] - extent[0] + 1;
  numPoints[1] = extent[3] - extent[2] + 1;
  numPoints[2] = extent[5] - extent[4] + 1;

  svtkIdType numCells[3];
  numCells[0] = numPoints[0] - 1;
  numCells[1] = numPoints[1] - 1;
  numCells[2] = numPoints[2] - 1;

  svtkIdType nextPointRow = numPoints[0];
  svtkIdType nextPointSlab = nextPointRow * numPoints[1];

  bool extentIs2D = (numCells[2] < 1);

  if (extentIs2D)
  {
    svtkIdType totalNumCells = numCells[0] * numCells[1];
    unstructuredOutput->Allocate(totalNumCells);
    svtkCellArray* cells = unstructuredOutput->GetCells();
    cells->AllocateEstimate(totalNumCells, 4);

    for (int j = 0; j < numCells[1]; j++)
    {
      svtkIdType rowStart = j * nextPointRow;
      for (int i = 0; i < numCells[0]; i++)
      {
        svtkIdType lowCellPoint = rowStart + i;

        svtkIdType pointIds[4];
        ...
        unstructuredOutput->InsertNextCell(SVTK_QUAD, 4, pointIds);
      }
    }
  }
  else // !extentIs2D
  {
    svtkIdType totalNumCells = numCells[0] * numCells[1] * numCells[2];
    unstructuredOutput->Allocate(totalNumCells);
    svtkCellArray* cells = unstructuredOutput->GetCells();
    ...
    {
      svtkIdType slabStart = k * nextPointSlab;
      for (int j = 0; j < numCells[1]; j++)
      {
        svtkIdType rowStart = slabStart + j * nextPointRow;
        for (int i = 0; i < numCells[0]; i++)
        {
          svtkIdType lowCellPoint = rowStart + i;
          ...
          unstructuredOutput->InsertNextCell(SVTK_HEXAHEDRON, 8, pointIds);
        }
      }
    }
  }
...
//-----------------------------------------------------------------------------
void svtkNetCDFCFReader::AddUnstructuredRectilinearCoordinates(
  svtkUnstructuredGrid* unstructuredOutput, const int extent[6])
{
  svtkDependentDimensionInfo* info = this->FindDependentDimensionInfo(this->LoadingDimensions);
  ...
  svtkIdType totalNumCells = longitudeCoordinates->GetNumberOfTuples();
  ...
  SVTK_CREATE(svtkMergePoints, locator);
  ...
  std::vector<svtkIdType> cellPoints(numPointsPerCell);
  ...
  for (int cellId = extent[0]; cellId < extent[1]; cellId++)
  {
    ...
    unstructuredOutput->InsertNextCell(SVTK_POLYGON, numPointsPerCell, &cellPoints.at(0));
  }
...
//-----------------------------------------------------------------------------
void svtkNetCDFCFReader::AddUnstructuredSphericalCoordinates(
  svtkUnstructuredGrid* unstructuredOutput, const int extent[6])
{
  ...
  svtkPoints* points = unstructuredOutput->GetPoints();
  ...
  for (svtkIdType pointId = 0; pointId < numPoints; pointId++)
  {
    ...
    double lon = svtkMath::RadiansFromDegrees(lonLat[0]);
    double lat = svtkMath::RadiansFromDegrees(lonLat[1]);
...
//-----------------------------------------------------------------------------
int svtkNetCDFCFReader::ReadMetaData(int ncFD)
{
  ...
  std::set<svtkStdString> specialVariables;

  for (int i = 0; i < numDimensions; i++)
  {
    this->DimensionInfo->v[i] = svtkDimensionInfo(ncFD, i);

    // Record any special variables for this dimension.
    svtkStringArray* dimensionVariables = this->DimensionInfo->v[i].GetSpecialVariables();
    for (svtkIdType j = 0; j < dimensionVariables->GetNumberOfValues(); j++)
    {
  ...
  {
    svtkDependentDimensionInfo info(ncFD, i, this);
    if (!info.GetValid())
      continue;
    ...
    svtkStringArray* dimensionVariables = info.GetSpecialVariables();
    for (svtkIdType j = 0; j < dimensionVariables->GetNumberOfValues(); j++)
    {
      specialVariables.insert(dimensionVariables->GetValue(j));
    }
...
//-----------------------------------------------------------------------------
int svtkNetCDFCFReader::IsTimeDimension(int svtkNotUsed(ncFD), int dimId)
{
  return (this->GetDimensionInfo(dimId)->GetUnits() == svtkDimensionInfo::TIME_UNITS);
}

//-----------------------------------------------------------------------------
svtkSmartPointer<svtkDoubleArray> svtkNetCDFCFReader::GetTimeValues(int svtkNotUsed(ncFD), int dimId)
{
  return this->GetDimensionInfo(dimId)->GetCoordinates();
}

//-----------------------------------------------------------------------------
inline svtkNetCDFCFReader::svtkDimensionInfo* svtkNetCDFCFReader::GetDimensionInfo(int dimension)
{
  return &(this->DimensionInfo->v.at(dimension));
}

//-----------------------------------------------------------------------------
svtkNetCDFCFReader::svtkDependentDimensionInfo* svtkNetCDFCFReader::FindDependentDimensionInfo(
  svtkIntArray* dims)
{
  for (size_t i = 0; i < this->DependentDimensionInfo->v.size(); i++)
  {
    svtkIntArray* dependentDims = this->DependentDimensionInfo->v[i].GetGridDimensions();
    if (dims->GetNumberOfTuples() == dependentDims->GetNumberOfTuples())
    {
      bool same = true;
      for (svtkIdType j = 0; j < dims->GetNumberOfTuples(); j++)
      {
...
//-----------------------------------------------------------------------------
void svtkNetCDFCFReader::IdentifySphericalCoordinates(
  svtkIntArray* dimensions, int& longitudeDim, int& latitudeDim, int& verticalDim)
{
  ...
  for (int i = 0; i < dimensions->GetNumberOfTuples(); i++)
  {
    ...
    {
      case svtkDimensionInfo::LONGITUDE_UNITS:
        longitudeDim = i;
        break;
      case svtkDimensionInfo::LATITUDE_UNITS:
        latitudeDim = i;
        break;
      case svtkDimensionInfo::UNDEFINED_UNITS:
      case svtkDimensionInfo::TIME_UNITS:
      case svtkDimensionInfo::VERTICAL_UNITS:
      default:
        verticalDim = i;
        break;
    }
...
//-----------------------------------------------------------------------------
svtkNetCDFCFReader::CoordinateTypesEnum svtkNetCDFCFReader::CoordinateType(svtkIntArray* dimensions)
{
  svtkDependentDimensionInfo* dependentDimInfo = this->FindDependentDimensionInfo(dimensions);
...
//-----------------------------------------------------------------------------
bool svtkNetCDFCFReader::DimensionsAreForPointData(svtkIntArray* dimensions)
{
