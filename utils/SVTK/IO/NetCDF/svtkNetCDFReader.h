 ...
 * svtkImageData at each block.  A block is created for each variable except that
 * variables with matching dimensions will be placed in the same block.
 */

#ifndef svtkNetCDFReader_h
#define svtkNetCDFReader_h
...
class SVTKIONETCDF_EXPORT svtkNetCDFReader : public svtkDataObjectAlgorithm
{
public:
  svtkTypeMacro(svtkNetCDFReader, svtkDataObjectAlgorithm);
  static svtkNetCDFReader* New();
  ...
  //   // load.
  //   svtkGetObjectMacro(VariableArraySelection, svtkDataArraySelection);
   ...
   */
  virtual svtkStringArray* GetAllVariableArrayNames();
   ...
   */
  svtkGetObjectMacro(VariableDimensions, svtkStringArray);
  //@}
   ...
   * meaningless.
   */
  svtkGetObjectMacro(AllDimensions, svtkStringArray);
  //@}
   ...
   * this option should be used with caution as SVTK does not fully support NaN
   * values and therefore odd calculations may occur.  By default this is off.
   */
  svtkGetMacro(ReplaceFillValueWithNan, svtkTypeBool);
  svtkSetMacro(ReplaceFillValueWithNan, svtkTypeBool);
  svtkBooleanMacro(ReplaceFillValueWithNan, svtkTypeBool);
  //@}
   ...
   */
  svtkGetStringMacro(TimeUnits);
  svtkGetStringMacro(Calendar);
  //@}
...
protected:
   ...
   */
  svtkSmartPointer<svtkIntArray> LoadingDimensions;

  svtkSmartPointer<svtkDataArraySelection> VariableArraySelection;

  svtkSmartPointer<svtkStringArray> AllVariableArrayNames;

  /**
   * Placeholder for structure returned from GetVariableDimensions().
   */
  svtkStringArray* VariableDimensions;
   ...
   */
  svtkStringArray* AllDimensions;

  svtkTypeBool ReplaceFillValueWithNan;
   ...
   * dimensions.
   */
  svtkStdString DescribeDimensions(int ncFD, const int* dimIds, int numDims);
   ...
   */
  virtual svtkSmartPointer<svtkDoubleArray> GetTimeValues(int ncFD, int dimId);
   ...
   */
  virtual bool DimensionsAreForPointData(svtkIntArray* svtkNotUsed(dimensions)) { return true; }
   ...
   */
  virtual void GetUpdateExtentForOutput(svtkDataSet* output, int extent[6]);

  /**
   * Load the variable at the given time into the given data set.  Return 1
   * on success and 0 on failure.
   */
  virtual int LoadVariable(int ncFD, const char* varName, double time, svtkDataSet* output);

private:
  svtkNetCDFReader(const svtkNetCDFReader&) = delete;
  void operator=(const svtkNetCDFReader&) = delete;

  int UpdateExtent[6];
  char* TimeUnits;
  char* Calendar;
  svtkNetCDFReaderPrivate* Private;
};

#endif // svtkNetCDFReader_h
