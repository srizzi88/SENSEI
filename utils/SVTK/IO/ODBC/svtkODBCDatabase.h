 ...
 * use.
 *
 * Unlike the other SVTK database drivers, ODBC can read its parameters
 ...
 *
 * @warning
 * Also, svtkSQLDatabase::CreateFromURL() will only handle URLs of the
...
#ifndef svtkODBCDatabase_h
#define svtkODBCDatabase_h
...
class SVTKIOODBC_EXPORT svtkODBCDatabase : public svtkSQLDatabase
{

  friend class svtkODBCQuery;

public:
  svtkTypeMacro(svtkODBCDatabase, svtkSQLDatabase);
  void PrintSelf(ostream& os, svtkIndent indent) override;
  static svtkODBCDatabase* New();
   ...
   */
  svtkSetStringMacro(DataSourceName);
  svtkGetStringMacro(DataSourceName);
  //@}

  svtkSetMacro(ServerPort, int);
...
private:
  ...
  svtkODBCDatabase(const svtkODBCDatabase&) = delete;
  void operator=(const svtkODBCDatabase&) = delete;
};

#endif // svtkODBCDatabase_h
