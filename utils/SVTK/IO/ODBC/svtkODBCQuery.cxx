...
svtkStandardNewMacro(svtkODBCQuery);
 ...
 * (svtkODBCBoundParameter) to hold all the information the user passes
 ...
 * attach them all to the statement.  The svtkODBCBoundParameter
 ...
 * svtkODBCBoundParameter and use that for the data storage by
 * default.  That will still require special-case handling for blobs
 * and strings.
 *
 * The svtkODBCQueryInternals class will handle the bookkeeping for
 * which parameters are and aren't bound at any given time.
 */

// ----------------------------------------------------------------------

class svtkODBCBoundParameter
{
public:
  svtkODBCBoundParameter()
    : Data(nullptr)
    , DataLength(0)
    , BufferSize(0)
    , DataTypeC(0)
  ...
  ~svtkODBCBoundParameter() { delete[] this->Data; }
...
class svtkODBCQueryInternals
{
public:
  svtkODBCQueryInternals()
  {
    this->Statement = nullptr;
    this->CurrentRow = svtkVariantArray::New();
    this->ColumnNames = svtkStringArray::New();
    this->ColumnIsSigned = svtkBitArray::New();
    this->ColumnTypes = nullptr;
    this->NullPermitted = svtkBitArray::New();
  }
  ...
  bool PrepareQuery(const char* queryString, SQLHANDLE connection, svtkStdString& error_message);
  bool SetBoundParameter(int index, svtkODBCBoundParameter* param);
...
public:
  ...
  svtkVariantArray* CurrentRow;
  svtkStringArray* ColumnNames;
  svtkBitArray* ColumnIsSigned;
  svtkBitArray* NullPermitted;
  SQLSMALLINT* ColumnTypes;

  typedef std::vector<svtkODBCBoundParameter*> ParameterList;
  ParameterList UserParameterList;
};

// ----------------------------------------------------------------------

void svtkODBCQueryInternals::FreeStatement()
{
  if (this->Statement)
  {
    SQLRETURN status;
    ...
    if (status != SQL_SUCCESS && status != SQL_SUCCESS_WITH_INFO)
    {
      ...
      errorBuf << "svtkODBCQuery: Unable to close SQL cursor.  Error: "
...
#define SVTK_ODBC_C_TYPENAME_MACRO(type, return_type)                                               \
  SQLSMALLINT svtkODBCTypeNameC(type) { return return_type; }

SVTK_ODBC_C_TYPENAME_MACRO(signed char, SQL_C_STINYINT);
SVTK_ODBC_C_TYPENAME_MACRO(unsigned char, SQL_C_UTINYINT);
SVTK_ODBC_C_TYPENAME_MACRO(signed short, SQL_C_SSHORT);
SVTK_ODBC_C_TYPENAME_MACRO(unsigned short, SQL_C_USHORT);
SVTK_ODBC_C_TYPENAME_MACRO(signed int, SQL_C_SLONG);
SVTK_ODBC_C_TYPENAME_MACRO(unsigned int, SQL_C_ULONG);
SVTK_ODBC_C_TYPENAME_MACRO(signed long, SQL_C_SLONG);
SVTK_ODBC_C_TYPENAME_MACRO(unsigned long, SQL_C_ULONG);
SVTK_ODBC_C_TYPENAME_MACRO(float, SQL_C_FLOAT);
SVTK_ODBC_C_TYPENAME_MACRO(double, SQL_C_DOUBLE);
SVTK_ODBC_C_TYPENAME_MACRO(long long, SQL_C_SBIGINT);
SVTK_ODBC_C_TYPENAME_MACRO(unsigned long long, SQL_C_UBIGINT);
SVTK_ODBC_C_TYPENAME_MACRO(const char*, SQL_C_CHAR);
SVTK_ODBC_C_TYPENAME_MACRO(char*, SQL_C_CHAR);
SVTK_ODBC_C_TYPENAME_MACRO(unsigned char*, SQL_C_CHAR);
SVTK_ODBC_C_TYPENAME_MACRO(void*, SQL_C_BINARY);

#define SVTK_ODBC_SQL_TYPENAME_MACRO(type, return_type)                                             \
  SQLSMALLINT svtkODBCTypeNameSQL(type) { return return_type; }

SVTK_ODBC_SQL_TYPENAME_MACRO(signed char, SQL_TINYINT);
SVTK_ODBC_SQL_TYPENAME_MACRO(unsigned char, SQL_TINYINT);
SVTK_ODBC_SQL_TYPENAME_MACRO(signed short, SQL_SMALLINT);
SVTK_ODBC_SQL_TYPENAME_MACRO(unsigned short, SQL_SMALLINT);
SVTK_ODBC_SQL_TYPENAME_MACRO(signed int, SQL_INTEGER);
SVTK_ODBC_SQL_TYPENAME_MACRO(unsigned int, SQL_INTEGER);
SVTK_ODBC_SQL_TYPENAME_MACRO(signed long, SQL_INTEGER);
SVTK_ODBC_SQL_TYPENAME_MACRO(unsigned long, SQL_INTEGER);
SVTK_ODBC_SQL_TYPENAME_MACRO(float, SQL_REAL);
SVTK_ODBC_SQL_TYPENAME_MACRO(double, SQL_DOUBLE);
SVTK_ODBC_SQL_TYPENAME_MACRO(long long, SQL_BIGINT);
SVTK_ODBC_SQL_TYPENAME_MACRO(unsigned long long, SQL_BIGINT);
SVTK_ODBC_SQL_TYPENAME_MACRO(const char*, SQL_VARCHAR);
SVTK_ODBC_SQL_TYPENAME_MACRO(char*, SQL_VARCHAR);
SVTK_ODBC_SQL_TYPENAME_MACRO(unsigned char*, SQL_VARCHAR);
SVTK_ODBC_SQL_TYPENAME_MACRO(void*, SQL_VARBINARY);

// ----------------------------------------------------------------------

// Description:
// This function will build and populate a svtkODBCBoundParameter
// struct.  The default implementation works for POD data types (char,
// int, long, etc).  I'll need to special-case strings and blobs.

template <typename T>
svtkODBCBoundParameter* svtkBuildODBCBoundParameter(T data_value)
{
  svtkODBCBoundParameter* param = new svtkODBCBoundParameter;

  param->DataTypeC = svtkODBCTypeNameC(data_value);
  param->DataTypeSQL = svtkODBCTypeNameSQL(data_value);
...
template <>
svtkODBCBoundParameter* svtkBuildODBCBoundParameter<const char*>(const char* data_value)
{
  svtkODBCBoundParameter* param = new svtkODBCBoundParameter;
...
svtkODBCBoundParameter* svtkBuildODBCBoundParameter(
...
bool svtkODBCQueryInternals::PrepareQuery(
  const char* queryString, SQLHANDLE dbConnection, svtkStdString& error_message)
{
  this->FreeStatement();
  this->FreeUserParameterList();
  ...
  // Queries in SVTK currently only support scrolling forward through
  ...
  if (status != SQL_SUCCESS && status != SQL_SUCCESS_WITH_INFO)
  {
    error_message = svtkStdString(GetErrorMessage(SQL_HANDLE_STMT, this->Statement));
    return false;
  }
...
void svtkODBCQueryInternals::FreeUserParameterList()
{
...
bool svtkODBCQueryInternals::SetBoundParameter(int index, svtkODBCBoundParameter* param)
{
...
void svtkODBCQueryInternals::ClearBoundParameters()
{
  if (this->Statement)
  {
...
bool svtkODBCQueryInternals::BindParametersToStatement()
{
  if (this->Statement == nullptr)
  {
  ...
  for (SQLUSMALLINT i = 0; i < numParams; ++i)
  {
    if (this->UserParameterList[i])
    {
      ...
      if (status != SQL_SUCCESS)
      {
        svtkGenericWarningMacro(<< "Unable to bind parameter " << i
...
static svtkStdString GetErrorMessage(SQLSMALLINT handleType, SQLHANDLE handle, int* code)
{
...
svtkODBCQuery::svtkODBCQuery()
{
  this->Internals = new svtkODBCQueryInternals;
...
void svtkODBCQuery::PrintSelf(ostream& os, svtkIndent indent)
{
...
bool svtkODBCQuery::SetQuery(const char* newQuery)
{
  this->Active = false;
  this->SetQueryText(newQuery);

  svtkODBCDatabase* db = svtkODBCDatabase::SafeDownCast(this->Database);
  if (db == nullptr)
  {
    svtkErrorMacro(<< "SHOULDN'T HAPPEN: SetQuery called with null database.  This can only happen "
                     "when you instantiate svtkODBCQuery directly.  You should always call "
                     "svtkODBCDatabase::GetQueryInstance to make a query object.");
    return false;
  }
...
const char* svtkODBCQuery::GetQuery()
{
...
bool svtkODBCQuery::Execute()
{
  ...
  {
    svtkDebugMacro(<< "SQLExecute succeeded.");
    ...
    if (numColumns)
    {
      ...
      for (int i = 0; i < numColumns; ++i)
      {
        ...
        if (status != SQL_SUCCESS && status != SQL_SUCCESS_WITH_INFO)
        {
          ...
          errbuf << "During svtkODBCQuery::Execute while looking up column " << i << ": "
                 << GetErrorMessage(SQL_HANDLE_STMT, this->Internals->Statement);
          this->SetLastErrorText(errbuf.str().c_str());
          svtkErrorMacro(<< errbuf.str().c_str());
        }
        ...
        {
          std::ostringstream errbuf;
          errbuf << "svtkODBCQuery::Execute: Unable to get unsigned flag for column " << i << ": "
                 << GetErrorMessage(SQL_HANDLE_STMT, this->Internals->Statement);
          this->SetLastErrorText(errbuf.str().c_str());
          svtkErrorMacro(<< errbuf.str().c_str());
        }
...
int svtkODBCQuery::GetNumberOfFields()
{
  if (!this->Active)
  {
    return 0;
  ...
  {
    std::ostringstream errbuf;
    errbuf << "During svtkODBCQuery::GetNumberOfFields: "
           << GetErrorMessage(SQL_HANDLE_STMT, this->Internals->Statement);
    this->SetLastErrorText(errbuf.str().c_str());
    return 0;
  }
...
const char* svtkODBCQuery::GetFieldName(int column)
{
  if (!this->Active)
  {
...
int svtkODBCQuery::GetFieldType(int column)
{
  if (!this->Active)
  {
  ...
  else
  {
    ...
    switch (this->Internals->ColumnTypes[column])
    {
      ...
      default:
      {
        svtkWarningMacro(<< "Unknown type " << this->Internals->ColumnTypes[column]
...
svtkVariant svtkODBCQuery::DataValue(svtkIdType column)
{
...
void svtkODBCQuery::ClearCurrentRow()
{
  for (svtkIdType i = 0; i < this->Internals->CurrentRow->GetNumberOfTuples(); ++i)
  {
    this->Internals->CurrentRow->SetValue(i, svtkVariant());
  }
}

// ----------------------------------------------------------------------

bool svtkODBCQuery::CacheCurrentRow()
{
  int column;
  bool status = true;
  ...
  for (column = 0; column < this->GetNumberOfFields(); ++column)
  {
    switch (this->Internals->ColumnTypes[column])
    {
      ...
      default:
      {
        svtkWarningMacro(<< "DataValue: Unsupported SQL data type "
                        << this->Internals->ColumnTypes[column] << " on column " << column);
        status = false;
        this->Internals->CurrentRow->SetValue(column, svtkVariant());
      };
      break;
    }
  }
...
const char* svtkODBCQuery::GetLastErrorText()
{
...
bool svtkODBCQuery::BeginTransaction()
{
...
bool svtkODBCQuery::CommitTransaction()
{
  if (!this->Database->IsOpen())
  {
...
bool svtkODBCQuery::RollbackTransaction()
{
  if (!this->Database->IsOpen())
  {
...
bool svtkODBCQuery::CacheWideStringColumn(int column)
{
...
bool svtkODBCQuery::CacheIntColumn(int column)
{
  SQLRETURN status;
  SQLINTEGER buffer;
  SQLLEN actualLength;
  ...
  if (status == SQL_SUCCESS)
  {
    ...
    if (this->Internals->ColumnIsSigned->GetValue(column))
    {
      result = svtkVariant(buffer);
    }
    else
    {
      unsigned int foo(buffer);
      result = svtkVariant(foo);
    }
...
bool svtkODBCQuery::CacheLongLongColumn(int column)
{
  SQLRETURN status;
  SQLLEN actualLength;
  int dataType;
...
bool svtkODBCQuery::CacheCharColumn(int column)
{
  SQLRETURN status;
...
bool svtkODBCQuery::CacheBooleanColumn(int column)
{
  SQLRETURN status;
  unsigned char buffer;
  SQLLEN actualLength;
...
bool svtkODBCQuery::CacheFloatColumn(int column)
{
  SQLRETURN status;
  SQLFLOAT buffer;
  SQLLEN actualLength;
...
bool svtkODBCQuery::CacheDoubleColumn(int column)
{
  SQLRETURN status;
  SQLDOUBLE buffer;
  SQLLEN actualLength;
...
bool svtkODBCQuery::CacheStringColumn(int column)
{
  SQLRETURN status;
  SQLLEN bufferLength;
  SQLLEN indicator;
  ...
  this->Internals->CurrentRow->SetValue(column, svtkVariant(outbuf.str()));
  return true;
}

// ----------------------------------------------------------------------

bool svtkODBCQuery::CacheBinaryColumn(int column)
{
  SQLRETURN status;
  svtkStdString result;
...
bool svtkODBCQuery::CacheDecimalColumn(int column)
{
  this->Internals->CurrentRow->SetValue(column, svtkVariant());
  this->SetLastErrorText(nullptr);
  return true;
...
bool svtkODBCQuery::CacheNumericColumn(int column)
{
  this->Internals->CurrentRow->SetValue(column, svtkVariant());
  this->SetLastErrorText(nullptr);
  return true;
...
bool svtkODBCQuery::CacheTimeColumn(int column)
{
  this->Internals->CurrentRow->SetValue(column, svtkVariant());
  this->SetLastErrorText(nullptr);
  return true;
...
bool svtkODBCQuery::CacheIntervalColumn(int column)
{
  this->Internals->CurrentRow->SetValue(column, svtkVariant());
  this->SetLastErrorText(nullptr);
  return true;
...
bool svtkODBCQuery::BindParameter(int index, unsigned char value)
{
  this->Internals->SetBoundParameter(index, svtkBuildODBCBoundParameter(value));
  return true;
}

// ----------------------------------------------------------------------

bool svtkODBCQuery::BindParameter(int index, signed char value)
{
  this->Internals->SetBoundParameter(index, svtkBuildODBCBoundParameter(value));
  return true;
}
...
bool svtkODBCQuery::BindParameter(int index, unsigned short value)
{
  this->Internals->SetBoundParameter(index, svtkBuildODBCBoundParameter(value));
  return true;
}
...
bool svtkODBCQuery::BindParameter(int index, signed short value)
{
  this->Internals->SetBoundParameter(index, svtkBuildODBCBoundParameter(value));
  return true;
}
...
bool svtkODBCQuery::BindParameter(int index, unsigned int value)
{
  this->Internals->SetBoundParameter(index, svtkBuildODBCBoundParameter(value));
  return true;
}
...
bool svtkODBCQuery::BindParameter(int index, signed int value)
{
  this->Internals->SetBoundParameter(index, svtkBuildODBCBoundParameter(value));
  return true;
}
...
bool svtkODBCQuery::BindParameter(int index, unsigned long value)
{
  this->Internals->SetBoundParameter(index, svtkBuildODBCBoundParameter(value));
  return true;
}
...
bool svtkODBCQuery::BindParameter(int index, signed long value)
{
  this->Internals->SetBoundParameter(index, svtkBuildODBCBoundParameter(value));
  return true;
}
...
bool svtkODBCQuery::BindParameter(int index, unsigned long long value)
{
  this->Internals->SetBoundParameter(index, svtkBuildODBCBoundParameter(value));
  return true;
}
...
bool svtkODBCQuery::BindParameter(int index, long long value)
{
  this->Internals->SetBoundParameter(index, svtkBuildODBCBoundParameter(value));
  return true;
}
...
bool svtkODBCQuery::BindParameter(int index, float value)
{
  this->Internals->SetBoundParameter(index, svtkBuildODBCBoundParameter(value));
  return true;
}
...
bool svtkODBCQuery::BindParameter(int index, double value)
{
  this->Internals->SetBoundParameter(index, svtkBuildODBCBoundParameter(value));
  return true;
}
...
bool svtkODBCQuery::BindParameter(int index, const char* value)
{
  this->Internals->SetBoundParameter(index, svtkBuildODBCBoundParameter(value));
  return true;
}
...
bool svtkODBCQuery::BindParameter(int index, const svtkStdString& value)
{
  this->Internals->SetBoundParameter(index, svtkBuildODBCBoundParameter(value));
  return true;
}
...
bool svtkODBCQuery::BindParameter(int index, const char* data, size_t length)
{
  this->Internals->SetBoundParameter(
    index, svtkBuildODBCBoundParameter(data, static_cast<unsigned long>(length), false));
  return true;
}

// ----------------------------------------------------------------------

bool svtkODBCQuery::BindParameter(int index, const void* data, size_t length)
{
  this->Internals->SetBoundParameter(index,
    svtkBuildODBCBoundParameter(
...
bool svtkODBCQuery::ClearParameterBindings()
{
