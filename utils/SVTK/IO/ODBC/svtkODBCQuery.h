...
/**
 * @class   svtkODBCQuery
 * @brief   svtkSQLQuery implementation for ODBC connections to databases
 *
 *
 *
 * This is an implementation of svtkSQLQuery for ODBC databases.  See
 * the documentation for svtkSQLQuery and svtkRowQuery for information
 * about what the methods do.
 *
 * @sa
 * svtkSQLDatabase svtkSQLQuery svtkODBCDatabase
 */

#ifndef svtkODBCQuery_h
#define svtkODBCQuery_h

#include "svtkIOODBCModule.h" // For export macro
#include "svtkSQLQuery.h"

class svtkODBCDatabase;
class svtkVariant;
class svtkVariantArray;
class svtkODBCInternals;
class svtkODBCQueryInternals;

class SVTKIOODBC_EXPORT svtkODBCQuery : public svtkSQLQuery
{

  friend class svtkODBCDatabase;

public:
  svtkTypeMacro(svtkODBCQuery, svtkSQLQuery);
  void PrintSelf(ostream& os, svtkIndent indent) override;
  static svtkODBCQuery* New();
...
protected:
  svtkODBCQuery();
  ~svtkODBCQuery() override;

  svtkSetStringMacro(LastErrorText);
  svtkSetStringMacro(QueryText);
  svtkGetStringMacro(QueryText);

  char* QueryText;

private:
  svtkODBCQuery(const svtkODBCQuery&) = delete;
  void operator=(const svtkODBCQuery&) = delete;
  ...
  svtkODBCQueryInternals* Internals;
  bool InitialFetch;
  char* LastErrorText;
};

#endif // svtkODBCQuery_h
