/*=========================================================================
  ...
  Module:    svtkOggTheoraWriter.cxx
...
{
public:
  svtkOggTheoraWriterInternal(svtkOggTheoraWriter* creator);
  ~svtkOggTheoraWriterInternal();
...
//---------------------------------------------------------------------------
svtkOggTheoraWriterInternal::svtkOggTheoraWriterInternal(svtkOggTheoraWriter* creator)
{
  this->Writer = creator;
  this->Dim[0] = 0;
  this->Dim[1] = 0;
...
//---------------------------------------------------------------------------
svtkOggTheoraWriterInternal::~svtkOggTheoraWriterInternal()
{
  if (!this->closedFile)
  {
    this->End();
...
//---------------------------------------------------------------------------
int svtkOggTheoraWriterInternal::Start()
{
  this->closedFile = 0;
  ...
  {
    svtkGenericWarningMacro("Could not initialize ogg stream state.");
    return 0;
  }
  ...
  {
    svtkGenericWarningMacro(<< "Could not allocate the theora context.");
    return 0;
  }
  ...
  this->outFile = svtksys::SystemTools::Fopen(this->Writer->GetFileName(), "wb");
  if (!this->outFile)
  {
...
int svtkOggTheoraWriterInternal::WriteHeader()
{
  ...
  {
    svtkGenericWarningMacro("Internal Theora library error.");
    return 0;
  }
...
//---------------------------------------------------------------------------
int svtkOggTheoraWriterInternal::Write(svtkImageData* id)
{
...
//---------------------------------------------------------------------------
// ripped from libtheora-1.0/examples/encoder_example.c
int svtkOggTheoraWriterInternal::EncodeFrame(th_ycbcr_buffer, int lastFrame)
{
  if (th_encode_ycbcr_in(this->thEncContext, this->thImage) < 0)
  {
    svtkGenericWarningMacro("Error encoding frame.");
    return 0;
  }
  ...
  while ((ret = th_encode_packetout(this->thEncContext, lastFrame, &oggPacket)))
  {
    if (ret < 0)
    {
      svtkGenericWarningMacro("Error retrieving packet from codec.");
      return 0;
    }
    if (ogg_stream_packetin(&this->oggState, &oggPacket) < 0)
    {
      svtkGenericWarningMacro("Error inserting packet into stream.");
      return 0;
    }
...
//---------------------------------------------------------------------------
void svtkOggTheoraWriterInternal::End()
{
  // flush remaining frame
  if (this->haveImageData)
    if (!this->EncodeFrame(this->thImage, 1))
      svtkGenericWarningMacro("Failed to finish writing movie");
  this->haveImageData = false;
...
//---------------------------------------------------------------------------
void svtkOggTheoraWriterInternal::RGB2YCbCr(svtkImageData* id, th_ycbcr_buffer ycbcr)
{
  // convenience
...
//---------------------------------------------------------------------------
svtkStandardNewMacro(svtkOggTheoraWriter);

//---------------------------------------------------------------------------
svtkOggTheoraWriter::svtkOggTheoraWriter()
{
...
//---------------------------------------------------------------------------
void svtkOggTheoraWriter::Start()
{
  this->Error = 1;
  ...
  this->Internals = new svtkOggTheoraWriterInternal(this);
...
//---------------------------------------------------------------------------
void svtkOggTheoraWriter::Write()
{
  if (this->Error)
  {
    return;
...
//---------------------------------------------------------------------------
void svtkOggTheoraWriter::PrintSelf(ostream& os, svtkIndent indent)
{
