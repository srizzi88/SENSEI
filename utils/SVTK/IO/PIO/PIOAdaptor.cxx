...
int PIOAdaptor::initializeGlobal(const char* PIOFileName)
{
  ...
  if (!inStr)
  {
    svtkGenericWarningMacro("Could not open the global description .pio file: " << PIOFileName);
    return 0;
  }
  ...
  {
    svtkGenericWarningMacro("Bad input file name: " << PIOFileName);
    return 0;
  }
  else
  {
  ...
  //
  auto dir = svtkSmartPointer<svtkDirectory>::New();
  uint64_t numFiles = 0;
  ...
  {
    svtkGenericWarningMacro("Dump directory does not exist: " << this->dumpDirectory);
    return 0;
  }
  if (this->dumpFileName.size() == 0)
  {
    svtkGenericWarningMacro("No files exist with the base name :" << this->dumpBaseName);
    return 0;
  }
...
int PIOAdaptor::initializeDump(int timeStep)
{
  ...
  if (this->pioData == 0)
  {
    ...
    {
      svtkGenericWarningMacro("PIOFile " << this->dumpFileName[timeStep] << " can't be read ");
      return 0;
    }
  }
...
void PIOAdaptor::create_geometry(svtkMultiBlockDataSet* grid)
{
  ...
  else
  {
    ...
    svtkNew<svtkHyperTreeGrid> htgrid;
    htgrid->Initialize();
  ...
  if (this->hasTracers == true && this->useTracer == true)
  {
    ...
    svtkNew<svtkUnstructuredGrid> tgrid;
    tgrid->Initialize();
  ...
  // Create the SVTK structures within multiblock
  if (this->useHTG == true)
  {
  ...
  if (this->useTracer == true)
  {
    ...
    {
      svtkGenericWarningMacro("Tracers don't exist in .pio file: " << this->descFileName);
    }
  }
  ...
  svtkStdString eap_version(cdata);
  ...
  svtkStdString user_name(cdata);

  this->pioData->GetPIOData("hist_prbnm", cdata);
  svtkStdString problem_name(cdata);

  // Add FieldData array for cycle number
  svtkNew<svtkIntArray> cycleArray;
  ...
  svtkNew<svtkFloatArray> simTimeArray;
  ...
  svtkNew<svtkStringArray> versionArray;
  ...
  svtkNew<svtkStringArray> userNameArray;
  ...
  svtkNew<svtkStringArray> probNameArray;
...
void PIOAdaptor::create_tracer_UG(svtkMultiBlockDataSet* grid)
{
  svtkUnstructuredGrid* tgrid = svtkUnstructuredGrid::SafeDownCast(grid->GetBlock(1));
  tgrid->Initialize();
  ...
  for (int i = 0; i < numberOfTracers; i++)
  {
    ...
    cell[0] = i;
    tgrid->InsertNextCell(SVTK_VERTEX, 1, cell);
  }
...
void PIOAdaptor::create_amr_UG(svtkMultiBlockDataSet* grid,
...
void PIOAdaptor::create_amr_UG_1D(svtkMultiBlockDataSet* grid, int startCellIndx,
  int endCellIndx,        // Number of cells all levels
  int* cell_level,        // Level of the cell in the AMR
  int64_t* cell_daughter, // Daughter ID, 0 indicates no daughter
  double* cell_center[1]) // Cell center
{
  svtkUnstructuredGrid* ugrid = svtkUnstructuredGrid::SafeDownCast(grid->GetBlock(0));
  ugrid->Initialize();
  ...
  // Geometry
  svtkIdType* cell = new svtkIdType[numberOfDaughters];
  svtkNew<svtkPoints> points;
  ...
  for (int i = startCellIndx; i < endCellIndx; i++)
  {
    if (cell_daughter[i] == 0)
    {
      ...
      ugrid->InsertNextCell(SVTK_LINE, numberOfDaughters, cell);
    }
  }
...
void PIOAdaptor::create_amr_UG_2D(svtkMultiBlockDataSet* grid, int startCellIndx,
  int endCellIndx,        // Number of cells all levels
  int* cell_level,        // Level of the cell in the AMR
  int64_t* cell_daughter, // Daughter ID, 0 indicates no daughter
  double* cell_center[2]) // Cell center
{
  ...
  for (int i = startCellIndx; i < endCellIndx; i++)
  {
    if (cell_daughter[i] == 0)
    {
      ...
      ugrid->InsertNextCell(SVTK_QUAD, numberOfDaughters, cell);
    }
  }
  delete bhTree;
  delete[] cell;
...
void PIOAdaptor::create_amr_UG_3D(svtkMultiBlockDataSet* grid, int startCellIndx,
  int endCellIndx,        // Number of cells all levels
  int* cell_level,        // Level of the cell in the AMR
  int64_t* cell_daughter, // Daughter ID, 0 indicates no daughter
  double* cell_center[3]) // Cell center
{
  ...
  for (int i = startCellIndx; i < endCellIndx; i++)
  {
    if (cell_daughter[i] == 0)
    {
      ...
      ugrid->InsertNextCell(SVTK_HEXAHEDRON, numberOfDaughters, cell);
    }
  }
  delete bhTree;
  delete[] cell;
...
void PIOAdaptor::build_hypertree(
  svtkHyperTreeGridNonOrientedCursor* treeCursor, int64_t curIndex, int64_t* _daughter)
{
  int64_t curDaughter = _daughter[curIndex];
...
void PIOAdaptor::create_amr_HTG(svtkMultiBlockDataSet* grid,
  int numberOfCells,      // Number of cells all levels
  int* cell_level,        // Level of the cell in the AMR
  int64_t* cell_daughter, // Daughter
  double* cell_center[3]) // Cell center
{
  svtkHyperTreeGrid* htgrid = svtkHyperTreeGrid::SafeDownCast(grid->GetBlock(0));
  htgrid->Initialize();
  ...
  for (size_t i = 0; i < _myHyperTree.size(); i++)
  {
    ...
    svtkHyperTree* htree = htgrid->GetTree(tree);
...
void PIOAdaptor::load_variable_data(
  svtkMultiBlockDataSet* grid, svtkDataArraySelection* PointDataArraySelection)
{
...
void PIOAdaptor::add_amr_HTG_scalar(svtkMultiBlockDataSet* grid, svtkStdString varName,
...
void PIOAdaptor::add_amr_UG_scalar(svtkMultiBlockDataSet* grid, svtkStdString varName,
