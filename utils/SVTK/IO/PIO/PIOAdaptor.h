...
public:
  ...
  void create_geometry(svtkMultiBlockDataSet* grid);
  void load_variable_data(svtkMultiBlockDataSet* grid, svtkDataArraySelection* cellSelection);
...
protected:
  ...
  void create_tracer_UG(svtkMultiBlockDataSet* grid);

  // Create the unstructured grid for AMR
  void create_amr_UG(svtkMultiBlockDataSet* grid, int numProc, int* global_numcell,
    int* cell_level,        // Level within AMR
    int64_t* cell_daughter, // Daughter ID, 0 indicates no daughter
    double** cell_center);  // Cell center

  void create_amr_UG_1D(svtkMultiBlockDataSet* grid,
  ...
  void create_amr_UG_2D(svtkMultiBlockDataSet* grid,
    int startCellIndx,       // Number of cells all levels
    int endCellIndx,         // Number of cells all levels
    int* cell_level,         // Level within AMR
    int64_t* cell_daughter,  // Daughter ID, 0 indicates no daughter
  ...
  void create_amr_UG_3D(svtkMultiBlockDataSet* grid,
    int startCellIndx,       // Number of cells all levels
    int endCellIndx,         // Number of cells all levels
    int* cell_level,         // Level within AMR
    int64_t* cell_daughter,  // Daughter ID, 0 indicates no daughter
  ...
  void create_amr_HTG(svtkMultiBlockDataSet* grid,
  ...
  void build_hypertree(
    svtkHyperTreeGridNonOrientedCursor* treeCursor, int64_t curIndex, int64_t* daughter);

  // Add variable data to the unstructured grid
  void add_amr_UG_scalar(svtkMultiBlockDataSet* grid, svtkStdString varName,
  ...
  void add_amr_HTG_scalar(svtkMultiBlockDataSet* grid, svtkStdString varName,
    double* data[],          // Data for all cells
    int numberOfComponents); // Number of components in data
