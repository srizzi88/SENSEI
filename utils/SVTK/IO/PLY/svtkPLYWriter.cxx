...
svtkStandardNewMacro(svtkPLYWriter);

svtkCxxSetObjectMacro(svtkPLYWriter, LookupTable, svtkScalarsToColors);

svtkPLYWriter::svtkPLYWriter()
{
  this->FileName = nullptr;
  this->FileType = SVTK_BINARY;
  this->DataByteOrder = SVTK_LITTLE_ENDIAN;
  ...
  this->Alpha = 255;
  this->TextureCoordinatesName = SVTK_TEXTURECOORDS_UV;

  this->HeaderComments = svtkSmartPointer<svtkStringArray>::New();
  this->HeaderComments->InsertNextValue("SVTK generated PLY File");
...
void svtkPLYWriter::WriteData()
{
  ...
  svtkSmartPointer<svtkUnsignedCharArray> cellColors, pointColors;
  PlyFile* ply;
  ...
  {
    ply = svtkPLY::ply_open_for_writing_to_string(this->OutputString, 2, elemNames,
      this->FileType == SVTK_BINARY
        ? (this->DataByteOrder == SVTK_LITTLE_ENDIAN ? PLY_BINARY_LE : PLY_BINARY_BE)
        : PLY_ASCII);
  }
  else
  {
    ply = svtkPLY::ply_open_for_writing(this->FileName, 2, elemNames,
      this->FileType == SVTK_BINARY
        ? (this->DataByteOrder == SVTK_LITTLE_ENDIAN ? PLY_BINARY_LE : PLY_BINARY_BE)
        : PLY_ASCII);
  }
  ...
  if (ply == nullptr)
  {
    svtkErrorMacro(<< "Error opening PLY file");
    return;
  }
  ...
  svtkPLY::ply_element_count(ply, "vertex", numPts);
  svtkPLY::ply_describe_property(ply, "vertex", &vertProps[0]);
  svtkPLY::ply_describe_property(ply, "vertex", &vertProps[1]);
  svtkPLY::ply_describe_property(ply, "vertex", &vertProps[2]);
  if (pointColors)
  {
    svtkPLY::ply_describe_property(ply, "vertex", &vertProps[3]);
    svtkPLY::ply_describe_property(ply, "vertex", &vertProps[4]);
    svtkPLY::ply_describe_property(ply, "vertex", &vertProps[5]);
    if (pointAlpha)
    {
      svtkPLY::ply_describe_property(ply, "vertex", &vertProps[6]);
    }
  }
  if (textureCoords)
  {
    svtkPLY::ply_describe_property(ply, "vertex", &vertProps[7]);
    svtkPLY::ply_describe_property(ply, "vertex", &vertProps[8]);
  }

  svtkPLY::ply_element_count(ply, "face", numPolys);
  svtkPLY::ply_describe_property(ply, "face", &faceProps[0]);
  if (cellColors)
  {
    svtkPLY::ply_describe_property(ply, "face", &faceProps[1]);
    svtkPLY::ply_describe_property(ply, "face", &faceProps[2]);
    svtkPLY::ply_describe_property(ply, "face", &faceProps[3]);
    if (cellAlpha)
    {
      svtkPLY::ply_describe_property(ply, "face", &faceProps[4]);
    }
  }
  ...
  {
    svtkPLY::ply_put_comment(ply, this->HeaderComments->GetValue(idx));
  }
  svtkPLY::ply_put_obj_info(ply, "svtkPolyData points and polygons: svtk4.0");

  // complete the header
  svtkPLY::ply_header_complete(ply);

  // set up and write the vertex elements
  plyVertex vert;
  svtkPLY::ply_put_element_setup(ply, "vertex");
  double dpoint[3];
  for (i = 0; i < numPts; i++)
  {
    ...
    svtkPLY::ply_put_element(ply, (void*)&vert);
  }
  ...
  int verts[256];
  face.verts = verts;
  svtkPLY::ply_put_element_setup(ply, "face");
  svtkIdType npts = 0;
  ...
  for (polys->InitTraversal(), i = 0; i < numPolys; i++)
  {
    ...
    if (npts > 256)
    {
      svtkErrorMacro(<< "Ply file only supports polygons with <256 points");
    }
    else
    {
      for (j = 0; j < npts; j++)
      ...
      svtkPLY::ply_put_element(ply, (void*)&face);
    }
  } // for all polygons
...
svtkSmartPointer<svtkUnsignedCharArray> svtkPLYWriter::GetColors(
  svtkIdType num, svtkDataSetAttributes* dsa)
{
  unsigned char* c;
  svtkIdType i;
  int numComp;

  if (this->ColorMode == SVTK_COLOR_MODE_OFF ||
    (this->ColorMode == SVTK_COLOR_MODE_UNIFORM_CELL_COLOR &&
      svtkPointData::SafeDownCast(dsa) != nullptr) ||
    (this->ColorMode == SVTK_COLOR_MODE_UNIFORM_POINT_COLOR &&
      svtkCellData::SafeDownCast(dsa) != nullptr))
  {
    return nullptr;
  }
  else if (this->ColorMode == SVTK_COLOR_MODE_UNIFORM_COLOR ||
    this->ColorMode == SVTK_COLOR_MODE_UNIFORM_POINT_COLOR ||
    this->ColorMode == SVTK_COLOR_MODE_UNIFORM_CELL_COLOR)
  {
  ...
  {
    svtkDataArray* da;
    svtkUnsignedCharArray* rgbArray;
    ...
    else if ((rgbArray = svtkArrayDownCast<svtkUnsignedCharArray>(da)) != nullptr && numComp == 3)
    { // have unsigned char array of three components, copy it
      return rgbArray;
    }
    else if ((rgbArray = svtkArrayDownCast<svtkUnsignedCharArray>(da)) != nullptr && numComp == 4)
    {
      if (this->EnableAlpha)
      {
        return rgbArray;
...
const float* svtkPLYWriter::GetTextureCoordinates(svtkIdType num, svtkDataSetAttributes* dsa)
{
  svtkDataArray* tCoords = dsa->GetTCoords();
  if (!tCoords || (tCoords->GetNumberOfTuples() != num) || (tCoords->GetNumberOfComponents() != 2))
    return nullptr;

  svtkFloatArray* textureArray;
  if ((textureArray = svtkArrayDownCast<svtkFloatArray>(tCoords)) == nullptr)
    svtkErrorMacro(<< "PLY writer only supports float texture coordinates");

  return textureArray->GetPointer(0);
}

void svtkPLYWriter::PrintSelf(ostream& os, svtkIndent indent)
{
  this->Superclass::PrintSelf(os, indent);

  os << indent << "Data Byte Order: ";
  if (this->DataByteOrder == SVTK_LITTLE_ENDIAN)
  {
    os << "Little Endian\n";
  }
  else
  ...
  else if (this->ColorMode == SVTK_COLOR_MODE_UNIFORM_CELL_COLOR)
  {
    os << "Uniform Cell Color\n";
  }
  else if (this->ColorMode == SVTK_COLOR_MODE_UNIFORM_POINT_COLOR)
  {
...
void svtkPLYWriter::AddComment(const std::string& comment)
{
  this->HeaderComments->InsertNextValue(comment.c_str());
}

svtkPolyData* svtkPLYWriter::GetInput()
{
  return svtkPolyData::SafeDownCast(this->Superclass::GetInput());
}

svtkPolyData* svtkPLYWriter::GetInput(int port)
{
  return svtkPolyData::SafeDownCast(this->Superclass::GetInput(port));
}

int svtkPLYWriter::FillInputPortInformation(int, svtkInformation* info)
{
