/*=========================================================================
  ...
  Module:    svtkPExodusIIReader.cxx
...
#undef DBG_PEXOIIRDR
#define svtkPExodusIIReaderMAXPATHLEN 2048

static const int objTypes[] = { svtkExodusIIReader::EDGE_BLOCK, svtkExodusIIReader::FACE_BLOCK,
  svtkExodusIIReader::ELEM_BLOCK, svtkExodusIIReader::NODE_SET, svtkExodusIIReader::EDGE_SET,
  svtkExodusIIReader::FACE_SET, svtkExodusIIReader::SIDE_SET, svtkExodusIIReader::ELEM_SET,
  svtkExodusIIReader::NODE_MAP, svtkExodusIIReader::EDGE_MAP, svtkExodusIIReader::FACE_MAP,
  svtkExodusIIReader::ELEM_MAP };
static const int numObjTypes = sizeof(objTypes) / sizeof(objTypes[0]);

static const int objResultTypes[] = { svtkExodusIIReader::NODAL, svtkExodusIIReader::EDGE_BLOCK,
  svtkExodusIIReader::FACE_BLOCK, svtkExodusIIReader::ELEM_BLOCK, svtkExodusIIReader::NODE_SET,
  svtkExodusIIReader::EDGE_SET, svtkExodusIIReader::FACE_SET, svtkExodusIIReader::SIDE_SET,
  svtkExodusIIReader::ELEM_SET, svtkExodusIIReader::GLOBAL };
static const int numObjResultTypes = sizeof(objResultTypes) / sizeof(objResultTypes[0]);

static const int objAttribTypes[] = { svtkExodusIIReader::EDGE_BLOCK, svtkExodusIIReader::FACE_BLOCK,
  svtkExodusIIReader::ELEM_BLOCK };
static const int numObjAttribTypes = sizeof(objAttribTypes) / sizeof(objAttribTypes[0]);

svtkStandardNewMacro(svtkPExodusIIReader);

class svtkPExodusIIReaderUpdateProgress : public svtkCommand
{
public:
  svtkTypeMacro(
    svtkPExodusIIReaderUpdateProgress, svtkCommand) static svtkPExodusIIReaderUpdateProgress* New()
  {
    return new svtkPExodusIIReaderUpdateProgress;
  }
  void SetReader(svtkPExodusIIReader* r) { Reader = r; }
  void SetIndex(int i) { Index = i; }

protected:
  svtkPExodusIIReaderUpdateProgress()
  {
    Reader = nullptr;
    Index = 0;
  }
  ~svtkPExodusIIReaderUpdateProgress() override {}

  void Execute(svtkObject*, unsigned long event, void* callData) override
  {
    if (event == svtkCommand::ProgressEvent)
    {
...
svtkPExodusIIReader::svtkPExodusIIReader()
{
  this->ProcRank = 0;
  this->ProcSize = 1;
  ...
  this->MultiFileName = new char[svtkPExodusIIReaderMAXPATHLEN];
...
svtkPExodusIIReader::~svtkPExodusIIReader()
{
  ...
  std::vector<svtkExodusIIReader*>::iterator it;
...
//----------------------------------------------------------------------------
void svtkPExodusIIReader::SetController(svtkMultiProcessController* c)
{
...
//----------------------------------------------------------------------------
int svtkPExodusIIReader::RequestInformation(
  svtkInformation* request, svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  if (this->ProcRank == 0)
  {
    ...
    bool newPattern =
      ((this->FilePattern &&
         (!this->CurrentFilePattern ||
           !svtksys::SystemTools::ComparePath(this->FilePattern, this->CurrentFilePattern) ||
           ((this->FileRange[0] != this->CurrentFileRange[0]) ||
             (this->FileRange[1] != this->CurrentFileRange[1])))) ||
        (this->FilePrefix &&
          !svtksys::SystemTools::ComparePath(this->FilePrefix, this->CurrentFilePrefix)));
    ...
    if (!sanity)
    {
      svtkErrorMacro(<< "Must SetFilePattern AND SetFilePrefix, or SetFileName(s)");
    ...
    for (int fileIndex = 0; fileIndex < numFiles; ++fileIndex, ++reader_idx)
    {
      if (this->NumberOfFileNames > 1)
      {
        ...
        {
          svtkPExodusIIReader::DetermineFileId(this->FileNames[fileIndex]);
        }
      }
      else if (this->FilePattern)
      {
        snprintf(this->MultiFileName, svtkPExodusIIReaderMAXPATHLEN, this->FilePattern,
  ...
  if ((this->LastCommonTimeStep >= 0) && !this->GetHasModeShapes())
  {
    ...
    int numTimes = outInfo->Length(svtkStreamingDemandDrivenPipeline::TIME_STEPS());
    ...
    outInfo->Set(svtkStreamingDemandDrivenPipeline::TIME_STEPS(), &commonTimes[0], numTimes);
  }
  ...
  if (this->FilePrefix)
  {
    this->CurrentFilePrefix = svtksys::SystemTools::DuplicateString(this->FilePrefix);
    this->CurrentFilePattern = svtksys::SystemTools::DuplicateString(this->FilePattern);
...
//----------------------------------------------------------------------------
int svtkPExodusIIReader::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** svtkNotUsed(inputVector), svtkInformationVector* outputVector)
{
  ...
  processNumber = outInfo->Get(svtkStreamingDemandDrivenPipeline::UPDATE_PIECE_NUMBER());
  numProcessors = outInfo->Get(svtkStreamingDemandDrivenPipeline::UPDATE_NUMBER_OF_PIECES());
  ...
  {
#ifdef DBG_PEXOIIRDR
    svtkWarningMacro("Creating empty grid for processor: " << processNumber);
#endif
...
#ifdef DBG_PEXOIIRDR
  svtkWarningMacro("Processor: " << processNumber << " reading files: " << min << " " << max);
#endif

#ifdef DBG_PEXOIIRDR
  svtkWarningMacro("Parallel read for processor: " << processNumber);
#endif
  ...
  svtkSmartPointer<svtkAppendCompositeDataLeaves> append =
    svtkSmartPointer<svtkAppendCompositeDataLeaves>::New();
  ...
  */
  ...
  if (this->ReaderList.size() < numMyFiles)
  {
    ...
    {
      svtkExodusIIReader* er = svtkExodusIIReader::New();
      svtkPExodusIIReaderUpdateProgress* progress = svtkPExodusIIReaderUpdateProgress::New();
      progress->SetReader(this);
      progress->SetIndex(reader_idx);
      er->AddObserver(svtkCommand::ProgressEvent, progress);
      progress->Delete();
  ...
  for (fileIndex = min, reader_idx = 0; fileIndex <= max; ++fileIndex, ++reader_idx)
  {
    ...
    if (this->NumberOfFileNames > 1)
    {
      ...
      if (this->GetGenerateFileIdArray())
      {
        fileId = svtkPExodusIIReader::DetermineFileId(this->FileNames[fileIndex]);
      }
    }
    else if (this->FilePattern)
    {
    ...
    {
      svtkErrorMacro("Some weird problem with filename/filepattern");
      return 0;
    }
    ...
    { // Get the requested time step. We only support requests of a single time step in this reader
      ...
      else
      {
        ...
        output->GetInformation()->Set(svtkDataObject::DATA_TIME_STEP(), requestedTimeStep);
...
#if 0
    svtkCompositeDataSet* subgrid = this->ReaderList[reader_idx]->GetOutput();
    ...
    if ( ncells != 0 )
    {
      if ( this->ExodusModelMetadata )
      {
        svtkExodusModel* em = this->ReaderList[reader_idx]->GetExodusModel();
        if ( em )
        {
  ...
  if ( this->PackExodusModelOntoOutput )
  {
    ...
    if ( this->ExodusModel )
    {
      svtkModelMetadata::RemoveMetadata( output );
...
//----------------------------------------------------------------------------
void svtkPExodusIIReader::SetFileRange(int min, int max)
{
...
//----------------------------------------------------------------------------
void svtkPExodusIIReader::SetFileName(const char* name)
{
  this->SetFileNames(1, &name);
}

void svtkPExodusIIReader::SetFileNames(int nfiles, const char** names)
{
  ...
  {
    this->FileNames[i] = svtksys::SystemTools::DuplicateString(names[i]);
  }
...
//----------------------------------------------------------------------------
int svtkPExodusIIReader::DetermineFileId(const char* file)
{
...
int svtkPExodusIIReader::DeterminePattern(const char* file)
{
  char pattern[20] = "%s";
  int scount = 0;
  int cc = 0;
  ...
  svtksys::RegularExpression ex2RegEx("\\.ex2$");
  svtksys::RegularExpression ex2v2RegEx("\\.ex2v2$");
  svtksys::RegularExpression restartRegEx("\\.e-s\\.?[0-9]+(\\.ex2v[0-9]+)?$");
  ...
  svtksys::RegularExpression numberRegEx("^(.*[^0-9])([0-9]+)(\\.ex2v[0-9]+)?$");
    ...
    !numberRegEx.find(file))
  {
    ...
    this->FilePattern = svtksys::SystemTools::DuplicateString(pattern);
    this->FilePrefix = svtksys::SystemTools::DuplicateString(file);
    this->FileRange[0] = min;
    this->FileRange[1] = max;
    this->NumberOfFiles = max - min + 1;
    return SVTK_OK;
  ...
  for (cc = min + 100; true; cc += 100)
  {
    ...
    if (svtksys::SystemTools::Stat(buffer, &fs) == -1)
      break;
  }
  ...
  delete[] this->FilePrefix;
  this->FilePattern = svtksys::SystemTools::DuplicateString(pattern);
  this->FilePrefix = svtksys::SystemTools::DuplicateString(prefix.c_str());

  return SVTK_OK;
}

//----------------------------------------------------------------------------
void svtkPExodusIIReader::PrintSelf(ostream& os, svtkIndent indent)
{
  svtkExodusIIReader::PrintSelf(os, indent);
...
svtkIdType svtkPExodusIIReader::GetTotalNumberOfElements()
{
  svtkIdType total = 0;
  std::vector<svtkExodusIIReader*>::iterator it;
  for (it = this->ReaderList.begin(); it != this->ReaderList.end(); ++it)
...
svtkIdType svtkPExodusIIReader::GetTotalNumberOfNodes()
{
  svtkIdType total = 0;
  std::vector<svtkExodusIIReader*>::iterator it;
  for (it = this->ReaderList.begin(); it != this->ReaderList.end(); ++it)
...
static void BroadcastXmitString(svtkMultiProcessController* ctrl, char* str)
{
  int len;
  if (str)
  {
...
static bool BroadcastRecvString(svtkMultiProcessController* ctrl, std::vector<char>& str)
{
  int len;
  ctrl->Broadcast(&len, 1, 0);
  if (len)
...
static void BroadcastDoubleVector(
  svtkMultiProcessController* controller, std::vector<double>& dvec, int rank)
{
...
static void BroadcastIntVector(
  svtkMultiProcessController* controller, std::vector<int>& ivec, int rank)
{
...
static void BroadcastString(svtkMultiProcessController* controller, svtkStdString& str, int rank)
{
...
static void BroadcastStringVector(
  svtkMultiProcessController* controller, std::vector<svtkStdString>& svec, int rank)
{
  unsigned long len = static_cast<unsigned long>(svec.size());
  controller->Broadcast(&len, 1, 0);
  if (rank)
    svec.resize(len);
  std::vector<svtkStdString>::iterator it;
...
static void BroadcastObjectInfo(
  svtkMultiProcessController* controller, svtkExodusIIReaderPrivate::ObjectInfoType* oinfo, int rank)
{
...
static void BroadcastBlockSetInfo(svtkMultiProcessController* controller,
  svtkExodusIIReaderPrivate::BlockSetInfoType* bsinfo, int rank)
{
...
static void BroadcastBlockInfo(
  svtkMultiProcessController* controller, svtkExodusIIReaderPrivate::BlockInfoType* binfo, int rank)
{
...
static void BroadcastPartInfo(
  svtkMultiProcessController* controller, svtkExodusIIReaderPrivate::PartInfoType* pinfo, int rank)
{
  BroadcastObjectInfo(controller, pinfo, rank);
  BroadcastIntVector(controller, pinfo->BlockIndices, rank);
}

static void BroadcastAssemblyInfo(svtkMultiProcessController* controller,
  svtkExodusIIReaderPrivate::AssemblyInfoType* ainfo, int rank)
{
  BroadcastObjectInfo(controller, ainfo, rank);
  BroadcastIntVector(controller, ainfo->BlockIndices, rank);
}

static void BroadcastMaterialInfo(svtkMultiProcessController* controller,
  svtkExodusIIReaderPrivate::MaterialInfoType* minfo, int rank)
{
...
static void BroadcastSetInfo(
  svtkMultiProcessController* controller, svtkExodusIIReaderPrivate::SetInfoType* sinfo, int rank)
{
...
static void BroadcastArrayInfo(
  svtkMultiProcessController* controller, svtkExodusIIReaderPrivate::ArrayInfoType* ainfo, int rank)
{
  if (rank)
    ainfo->Reset();
...
static void BroadcastArrayInfoVector(svtkMultiProcessController* controller,
  std::vector<svtkExodusIIReaderPrivate::ArrayInfoType>& ainfo, int rank)
{
...
static void BroadcastSortedObjectIndices(
  svtkMultiProcessController* controller, std::map<int, std::vector<int> >& oidx, int rank)
{
...
static void BroadcastArrayInfoMap(svtkMultiProcessController* controller,
  std::map<int, std::vector<svtkExodusIIReaderPrivate::ArrayInfoType> >& oidx, int rank)
{
  unsigned long len = static_cast<unsigned long>(oidx.size());
  controller->Broadcast(&len, 1, 0);
  if (rank == 0)
  {
    int tmp;
    std::map<int, std::vector<svtkExodusIIReaderPrivate::ArrayInfoType> >::iterator it;
    for (it = oidx.begin(); it != oidx.end(); ++it)
    {
      tmp = it->first;
      controller->Broadcast(&tmp, 1, 0);
  ...
  else
  {
    ...
    for (i = 0; i < len; ++i)
    {
      std::vector<svtkExodusIIReaderPrivate::ArrayInfoType> blank;
      int key;
      controller->Broadcast(&key, 1, 0);
      oidx[key] = blank;
...
static void BroadcastModelParameters(
  svtkMultiProcessController* controller, ex_init_params& params, int svtkNotUsed(rank))
{
...
static void BroadcastBlockInfoVector(svtkMultiProcessController* controller,
  std::vector<svtkExodusIIReaderPrivate::BlockInfoType>& binfo, int rank)
{
  unsigned long len = static_cast<unsigned long>(binfo.size());
  controller->Broadcast(&len, 1, 0);
  if (rank)
    binfo.resize(len);
  std::vector<svtkExodusIIReaderPrivate::BlockInfoType>::iterator it;
...
static void BroadcastBlockInfoMap(svtkMultiProcessController* controller,
  std::map<int, std::vector<svtkExodusIIReaderPrivate::BlockInfoType> >& binfo, int rank)
{
  unsigned long len = static_cast<unsigned long>(binfo.size());
  controller->Broadcast(&len, 1, 0);
  int tmp;
  if (rank == 0)
  {
    std::map<int, std::vector<svtkExodusIIReaderPrivate::BlockInfoType> >::iterator it;
    for (it = binfo.begin(); it != binfo.end(); ++it)
    {
      tmp = it->first;
      controller->Broadcast(&tmp, 1, 0);
  ...
  {
    unsigned long i;
    std::vector<svtkExodusIIReaderPrivate::BlockInfoType> blank;
    for (i = 0; i < len; ++i)
    {
      controller->Broadcast(&tmp, 1, 0);
      binfo[tmp] = blank;
...
static void BroadcastSetInfoVector(svtkMultiProcessController* controller,
  std::vector<svtkExodusIIReaderPrivate::SetInfoType>& sinfo, int rank)
{
  unsigned long len = static_cast<unsigned long>(sinfo.size());
  controller->Broadcast(&len, 1, 0);
  if (rank)
    sinfo.resize(len);
  std::vector<svtkExodusIIReaderPrivate::SetInfoType>::iterator it;
...
static void BroadcastSetInfoMap(svtkMultiProcessController* controller,
  std::map<int, std::vector<svtkExodusIIReaderPrivate::SetInfoType> >& sinfo, int rank)
{
  unsigned long len = static_cast<unsigned long>(sinfo.size());
  controller->Broadcast(&len, 1, 0);
  int tmp;
  if (rank == 0)
  {
    std::map<int, std::vector<svtkExodusIIReaderPrivate::SetInfoType> >::iterator it;
    for (it = sinfo.begin(); it != sinfo.end(); ++it)
    {
      tmp = it->first;
      controller->Broadcast(&tmp, 1, 0);
  ...
  {
    unsigned long i;
    std::vector<svtkExodusIIReaderPrivate::SetInfoType> blank;
    for (i = 0; i < len; ++i)
    {
      controller->Broadcast(&tmp, 1, 0);
      sinfo[tmp] = blank;
...
static void BroadcastMapInfoVector(svtkMultiProcessController* controller,
  std::vector<svtkExodusIIReaderPrivate::MapInfoType>& minfo, int rank)
{
  unsigned long len = static_cast<unsigned long>(minfo.size());
  controller->Broadcast(&len, 1, 0);
  if (rank)
    minfo.resize(len);
  std::vector<svtkExodusIIReaderPrivate::MapInfoType>::iterator it;
...
static void BroadcastMapInfoMap(svtkMultiProcessController* controller,
  std::map<int, std::vector<svtkExodusIIReaderPrivate::MapInfoType> >& minfo, int rank)
{
  unsigned long len = static_cast<unsigned long>(minfo.size());
  controller->Broadcast(&len, 1, 0);
  int tmp;
  if (rank == 0)
  {
    std::map<int, std::vector<svtkExodusIIReaderPrivate::MapInfoType> >::iterator it;
    for (it = minfo.begin(); it != minfo.end(); ++it)
    {
      tmp = it->first;
      controller->Broadcast(&tmp, 1, 0);
  ...
  {
    unsigned long i;
    std::vector<svtkExodusIIReaderPrivate::MapInfoType> blank;
    for (i = 0; i < len; ++i)
    {
      controller->Broadcast(&tmp, 1, 0);
      minfo[tmp] = blank;
...
static void BroadcastPartInfoVector(svtkMultiProcessController* controller,
  std::vector<svtkExodusIIReaderPrivate::PartInfoType>& pinfo, int rank)
{
  unsigned long len = static_cast<unsigned long>(pinfo.size());
  controller->Broadcast(&len, 1, 0);
  if (rank)
    pinfo.resize(len);
  std::vector<svtkExodusIIReaderPrivate::PartInfoType>::iterator it;
...
static void BroadcastMaterialInfoVector(svtkMultiProcessController* controller,
  std::vector<svtkExodusIIReaderPrivate::MaterialInfoType>& minfo, int rank)
{
  unsigned long len = static_cast<unsigned long>(minfo.size());
  controller->Broadcast(&len, 1, 0);
  if (rank)
    minfo.resize(len);
  std::vector<svtkExodusIIReaderPrivate::MaterialInfoType>::iterator it;
  for (it = minfo.begin(); it != minfo.end(); ++it)
  {
...
static void BroadcastAssemblyInfoVector(svtkMultiProcessController* controller,
  std::vector<svtkExodusIIReaderPrivate::AssemblyInfoType>& ainfo, int rank)
{
  unsigned long len = static_cast<unsigned long>(ainfo.size());
  controller->Broadcast(&len, 1, 0);
  if (rank)
    ainfo.resize(len);
  std::vector<svtkExodusIIReaderPrivate::AssemblyInfoType>::iterator it;
...
void svtkPExodusIIReader::Broadcast(svtkMultiProcessController* ctrl)
{
  if (ctrl)
  {
    ...
    else
    {
      ...
      this->FilePattern =
        BroadcastRecvString(ctrl, tmp) ? svtksys::SystemTools::DuplicateString(&tmp[0]) : nullptr;
      this->FilePrefix =
        BroadcastRecvString(ctrl, tmp) ? svtksys::SystemTools::DuplicateString(&tmp[0]) : nullptr;
    }
