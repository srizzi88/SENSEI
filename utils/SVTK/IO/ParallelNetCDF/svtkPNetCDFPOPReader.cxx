...
Module:    svtkPNetCDFPOPReader.cxx
...
svtkStandardNewMacro(svtkPNetCDFPOPReader);
...
//----------------------------------------------------------------------------
// set default values
svtkPNetCDFPOPReader::svtkPNetCDFPOPReader()
{
  ...
  this->SelectionObserver->SetCallback(&svtkPNetCDFPOPReader::SelectionModifiedCallback);
  this->SelectionObserver->SetClientData(this);
  this->Internals = new svtkPNetCDFPOPReaderInternal;
  this->Internals->VariableArraySelection->AddObserver(
    svtkCommand::ModifiedEvent, this->SelectionObserver);
...
//----------------------------------------------------------------------------
void svtkPNetCDFPOPReader::PrintSelf(ostream& os, svtkIndent indent)
{
...
int svtkPNetCDFPOPReader::RequestInformation(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** svtkNotUsed(inputVector), svtkInformationVector* outputVector)
{
  ...
  if (this->IsReaderRank())
  {
    ...
    {
      svtkErrorMacro(<< "Can't read file " << nc_strerror(retval));
      this->SetOpenedFileName(0);
      return 0;
    }
...
int svtkPNetCDFPOPReader::RequestData(svtkInformation* request,
  svtkInformationVector** svtkNotUsed(inputVector), svtkInformationVector* outputVector)
{
  ...
  int subext[6];
  // svtkInformation * outInfo = output->GetInformationObject(0);
  outInfo->Get(svtkStreamingDemandDrivenPipeline::UPDATE_EXTENT(), subext);

  svtkRectilinearGrid* rgrid = svtkRectilinearGrid::SafeDownCast(output);
        ...
        0)
    {
      ...
      if (firstPass == true)
      {
        ...
        this->Controller->Broadcast(&buffer[0],
          static_cast<svtkIdType>(wholeCount[0] + wholeCount[1] + wholeCount[2]),
        ...
        xCoords->SetArray(z, static_cast<svtkIdType>(count[2]), 0, 1);
        svtkFloatArray* yCoords = svtkFloatArray::New();
        yCoords->SetArray(y, static_cast<svtkIdType>(count[1]), 0, 1);
        for (unsigned int q = 0; q < count[0]; q++)
        {
          x[q] = -x[q];
        }
        svtkFloatArray* zCoords = svtkFloatArray::New();
        zCoords->SetArray(x, static_cast<svtkIdType>(count[0]), 0, 1);
      ...
      // create svtkFloatArray and get the scalars into it
      svtkFloatArray* scalars = svtkFloatArray::New();
      svtkIdType numberOfTuples = static_cast<svtkIdType>(count[0] * count[1] * count[2]);
      ...
      for (int curDepth = subext[4]; curDepth <= subext[5]; curDepth++)
      {
        ...
        //      sourceRank << endl;
        // We could probably use svtkMPIController::NoBlockReceive(), but but I'm not sure
        // how it will behave since we're calling MPI_Isend() directly.  So, I'm going
        // to call MPI_Irecv directly, too...

        MPI_Comm* comm =
          ((svtkMPICommunicator*)this->Controller->GetCommunicator())->GetMPIComm()->GetHandle();
        MPI_Request recvReq;
...
void svtkPNetCDFPOPReader::SelectionModifiedCallback(
  svtkObject*, unsigned long, void* clientdata, void*)
{
  static_cast<svtkPNetCDFPOPReader*>(clientdata)->Modified();
}

//-----------------------------------------------------------------------------
int svtkPNetCDFPOPReader::GetNumberOfVariableArrays()
{
  return this->Internals->VariableArraySelection->GetNumberOfArrays();
}

//-----------------------------------------------------------------------------
const char* svtkPNetCDFPOPReader::GetVariableArrayName(int index)
{
...
//-----------------------------------------------------------------------------
int svtkPNetCDFPOPReader::GetVariableArrayStatus(const char* name)
{
  return this->Internals->VariableArraySelection->ArrayIsEnabled(name);
}

//-----------------------------------------------------------------------------
void svtkPNetCDFPOPReader::SetVariableArrayStatus(const char* name, int status)
{
  svtkDebugMacro("Set cell array \"" << name << "\" status to: " << status);
  if (this->Internals->VariableArraySelection->ArrayExists(name) == 0)
  {
    svtkErrorMacro(<< name << " is not available in the file.");
    return;
  }
...
int svtkPNetCDFPOPReader::ReadAndSend(svtkInformation* outInfo, int varID)
{
  int wholeExtent[6];
  outInfo->Get(svtkStreamingDemandDrivenPipeline::WHOLE_EXTENT(), wholeExtent);
    ...
    if (ReaderForDepth(curDepth) == rank)
    {
      ...
      for (int destRank = 0; destRank < this->Controller->GetNumberOfProcesses(); destRank++)
      {
        ...
        if (curDepth >= destExtent[0] && curDepth <= destExtent[1])
        {
          ...
          // svtkMPICommunicator can't handle arbitrary types, so we'll have to do it ourselves
          MPI_Comm* comm =
            ((svtkMPICommunicator*)this->Controller->GetCommunicator())->GetMPIComm()->GetHandle();
...
void svtkPNetCDFPOPReader::SetReaderRanks(svtkIdList* ranks)
{
  ...
  if (ranks)
  {
    for (svtkIdType i = 0; i < ranks->GetNumberOfIds(); i++)
    {
      svtkIdType rank = ranks->GetId(i);
...
int svtkPNetCDFPOPReader::ReaderForDepth(unsigned depth)
{
...
bool svtkPNetCDFPOPReader::IsReaderRank()
{
  int rank = this->Controller->GetLocalProcessId();
...
bool svtkPNetCDFPOPReader::IsFirstReaderRank()
{
...
//----------------------------------------------------------------------------
//
void svtkPNetCDFPOPReader::SetController(svtkMPIController* controller)
{
