...
template <class T>
void svtkPSLACReaderMapValues1(
  const T* inArray, T* outArray, int numComponents, svtkIdTypeArray* indexMap, svtkIdType offset = 0)
{
  svtkIdType numVals = indexMap->GetNumberOfTuples();
  for (svtkIdType i = 0; i < numVals; i++)
  {
    svtkIdType j = indexMap->GetValue(i) - offset;
...
// template<class T>
// void svtkPSLACReaderMapValues2(const T *inArray, T *outArray, int numComponents,
//                               svtkIdTypeArray *indexMap)
// {
//   svtkIdType numVals = indexMap->GetNumberOfTuples();
//   for (svtkIdType i = 0; i < numVals; i++)
//     {
//     svtkIdType j = indexMap->GetValue(i);
//     for (int c = 0; c < numComponents; c++)
//       {
...
static void SynchronizeBlocks(svtkMultiBlockDataSet* blocks, svtkMultiProcessController* controller,
  svtkInformationIntegerKey* typeKey)
{
  unsigned long localNumBlocks = blocks->GetNumberOfBlocks();
  unsigned long numBlocks;
  controller->AllReduce(&localNumBlocks, &numBlocks, 1, svtkCommunicator::MAX_OP);
  ...
  {
    svtkDataObject* object = blocks->GetBlock(blockId);
    if (object && !object->IsA("svtkUnstructuredGrid"))
    {
      svtkGenericWarningMacro(<< "Sanity error: found a block that is not an unstructured grid.");
    }
    int localBlockExists = (object != nullptr);
    int globalBlockExists = 0;
    controller->AllReduce(&localBlockExists, &globalBlockExists, 1, svtkCommunicator::LOGICAL_OR_OP);
    if (!localBlockExists && globalBlockExists)
    {
      SVTK_CREATE(svtkUnstructuredGrid, grid);
...
// information.
namespace svtkPSLACReaderTypes
{
...
const svtkIdType midpointPositionSize = sizeof(midpointPositionType) / sizeof(double);

typedef struct
{
  svtkIdType minEdgePoint;
  svtkIdType maxEdgePoint;
  svtkIdType globalId;
} midpointTopologyType;
const svtkIdType midpointTopologySize = sizeof(midpointTopologyType) / sizeof(svtkIdType);
...
typedef std::unordered_map<svtkSLACReader::EdgeEndpoints, midpointPointersType, EdgeEndpointsHash>
  MidpointsAvailableType;

//-----------------------------------------------------------------------------
// Convenience function for gathering midpoint information to a process.
static void GatherMidpoints(svtkMultiProcessController* controller,
  const midpointListsType& sendMidpoints, midpointListsType& recvMidpoints, int process)
{
  svtkIdType sendLength = static_cast<svtkIdType>(sendMidpoints.position.size());
  if (sendLength != static_cast<svtkIdType>(sendMidpoints.topology.size()))
  {
    svtkGenericWarningMacro(<< "Bad midpoint array structure.");
    return;
  }

  svtkIdType numProcesses = controller->GetNumberOfProcesses();

  // Gather the amount of data each process is going to send.
  std::vector<svtkIdType> receiveCounts(numProcesses);
  ...
  std::vector<svtkIdType> positionLengths(numProcesses);
  std::vector<svtkIdType> positionOffsets(numProcesses);
  std::vector<svtkIdType> topologyLengths(numProcesses);
  std::vector<svtkIdType> topologyOffsets(numProcesses);

  const double* sendPositionBuffer =
    ((sendLength > 0) ? reinterpret_cast<const double*>(&sendMidpoints.position.at(0)) : nullptr);
  const svtkIdType* sendTopologyBuffer =
    ((sendLength > 0) ? reinterpret_cast<const svtkIdType*>(&sendMidpoints.topology.at(0))
                      : nullptr);
  double* recvPositionBuffer;
  svtkIdType* recvTopologyBuffer;
  ...
  if (process == controller->GetLocalProcessId())
  {
    ...
    recvTopologyBuffer =
      ((numEntries > 0) ? reinterpret_cast<svtkIdType*>(&recvMidpoints.topology.at(0)) : nullptr);
  }
  else
  {
...
using namespace svtkPSLACReaderTypes;

//-----------------------------------------------------------------------------
// Simple hash function for svtkIdType.
struct svtkPSLACReaderIdTypeHash
{
  size_t operator()(svtkIdType val) const { return static_cast<size_t>(val); }
};

//=============================================================================
svtkObjectFactoryNewMacro(svtkPSLACReader);

svtkCxxSetObjectMacro(svtkPSLACReader, Controller, svtkMultiProcessController);

//-----------------------------------------------------------------------------
class svtkPSLACReader::svtkInternal
{
public:
  typedef std::unordered_map<svtkIdType, svtkIdType, svtkPSLACReaderIdTypeHash> GlobalToLocalIdType;
  GlobalToLocalIdType GlobalToLocalIds;

  // Description:
  // A map from local point ids to global ids.  Can also be used as the
  // global point ids.
  svtkSmartPointer<svtkIdTypeArray> LocalToGlobalIds;

  // Description:
  // The point data we expect to receive from each process.
  svtkSmartPointer<svtkIdTypeArray> PointsExpectedFromProcessesLengths;
  svtkSmartPointer<svtkIdTypeArray> PointsExpectedFromProcessesOffsets;

  // Description:
  // The point data we have to send to each process.  Stored as global ids.
  svtkSmartPointer<svtkIdTypeArray> PointsToSendToProcesses;
  svtkSmartPointer<svtkIdTypeArray> PointsToSendToProcessesLengths;
  svtkSmartPointer<svtkIdTypeArray> PointsToSendToProcessesOffsets;

  // Description:
  // The edge data we expect to receive from each process.
  svtkSmartPointer<svtkIdTypeArray> EdgesExpectedFromProcessesCounts;

  // Description:
  // The edge data we have to send to each process.  Stored as global ids.
  svtkSmartPointer<svtkIdTypeArray> EdgesToSendToProcesses;
  svtkSmartPointer<svtkIdTypeArray> EdgesToSendToProcessesLengths;
  svtkSmartPointer<svtkIdTypeArray> EdgesToSendToProcessesOffsets;
};

//-----------------------------------------------------------------------------
svtkPSLACReader::svtkPSLACReader()
{
  this->Controller = nullptr;
  this->SetController(svtkMultiProcessController::GetGlobalController());
  if (!this->Controller)
  {
    this->SetController(svtkSmartPointer<svtkDummyController>::New());
  }
  this->NumberOfPiecesCache = 0;
  this->RequestedPieceCache = -1;

  this->PInternal = new svtkPSLACReader::svtkInternal;
}
...
void svtkPSLACReader::PrintSelf(ostream& os, svtkIndent indent)
{
...
//-----------------------------------------------------------------------------
int svtkPSLACReader::RequestInformation(
  svtkInformation* request, svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  if (!this->Controller)
  {
    svtkErrorMacro(<< "I need a Controller to read the data.");
    return 0;
  }

  for (int i = 0; i < svtkPSLACReader::NUM_OUTPUTS; i++)
  {
  ...
  svtkInformation* request, svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  for (int i = 0; i < svtkSLACReader::NUM_OUTPUTS; i++)
  {
    ...
    {
      this->RequestedPiece = outInfo->Get(svtkStreamingDemandDrivenPipeline::UPDATE_PIECE_NUMBER());
      this->NumberOfPieces =
  ...
  if ((this->RequestedPiece != this->Controller->GetLocalProcessId()) ||
    (this->NumberOfPieces != this->Controller->GetNumberOfProcesses()))
  {
    svtkErrorMacro(<< "Process numbers do not match piece numbers.");
    return 0;
  }
...
//-----------------------------------------------------------------------------
int svtkPSLACReader::ReadTetrahedronInteriorArray(int meshFD, svtkIdTypeArray* connectivity)
{
  int tetInteriorVarId;
  CALL_NETCDF(nc_inq_varid(meshFD, "tetrahedron_interior", &tetInteriorVarId));
  svtkIdType numTets = this->GetNumTuplesInVariable(meshFD, tetInteriorVarId, NumPerTetInt);

  svtkIdType numTetsPerPiece = numTets / this->NumberOfPieces + 1;
  svtkIdType startTet = this->RequestedPiece * numTetsPerPiece;
  svtkIdType endTet = startTet + numTetsPerPiece;
  if (endTet > numTets)
    endTet = numTets;
  ...
  connectivity->SetNumberOfTuples(static_cast<svtkIdType>(count[0]));
  CALL_NETCDF(nc_get_vars_svtkIdType(
...
//-----------------------------------------------------------------------------
int svtkPSLACReader::ReadTetrahedronExteriorArray(int meshFD, svtkIdTypeArray* connectivity)
{
  int tetExteriorVarId;
  CALL_NETCDF(nc_inq_varid(meshFD, "tetrahedron_exterior", &tetExteriorVarId));
  svtkIdType numTets = this->GetNumTuplesInVariable(meshFD, tetExteriorVarId, NumPerTetExt);
...
//-----------------------------------------------------------------------------
int svtkPSLACReader::CheckTetrahedraWinding(int meshFD)
{
...
//-----------------------------------------------------------------------------
int svtkPSLACReader::ReadConnectivity(
  int meshFD, svtkMultiBlockDataSet* surfaceOutput, svtkMultiBlockDataSet* volumeOutput)
{
  ...
  this->PInternal->LocalToGlobalIds = svtkSmartPointer<svtkIdTypeArray>::New();
       ...
       outputIter->GoToNextItem())
  {
    ...
    svtkCellArray* cells = ugrid->GetCells();
  ...
  std::vector<svtkSLACReader::EdgeEndpoints> edgesNeeded;
  ...
  if (this->ReadMidpoints)
  {
    ...
    {
      svtkUnstructuredGrid* ugrid =
        svtkUnstructuredGrid::SafeDownCast(surfaceOutput->GetDataSet(outputIter));
      svtkCellArray* cells = ugrid->GetCells();
      ...
      for (cells->InitTraversal(); cells->GetNextCell(npts, pts);)
      {
        for (svtkIdType i = 0; i < npts; i++)
        {
          edgesNeeded.push_back(svtkSLACReader::EdgeEndpoints(pts[i], pts[(i + 1) % npts]));
        }
      }
    }
  }
  ...
  this->PInternal->LocalToGlobalIds->Allocate(
    static_cast<svtkIdType>(this->PInternal->GlobalToLocalIds.size()));
  svtkInternal::GlobalToLocalIdType::iterator itr;
  ...
  svtkSortDataArray::Sort(this->PInternal->LocalToGlobalIds);
  ...
  this->PInternal->PointsExpectedFromProcessesLengths = svtkSmartPointer<svtkIdTypeArray>::New();
  this->PInternal->PointsExpectedFromProcessesLengths->SetNumberOfTuples(this->NumberOfPieces);
  this->PInternal->PointsExpectedFromProcessesOffsets = svtkSmartPointer<svtkIdTypeArray>::New();
  this->PInternal->PointsExpectedFromProcessesOffsets->SetNumberOfTuples(this->NumberOfPieces);
  this->PInternal->PointsToSendToProcesses = svtkSmartPointer<svtkIdTypeArray>::New();
  this->PInternal->PointsToSendToProcessesLengths = svtkSmartPointer<svtkIdTypeArray>::New();
  this->PInternal->PointsToSendToProcessesLengths->SetNumberOfTuples(this->NumberOfPieces);
  this->PInternal->PointsToSendToProcessesOffsets = svtkSmartPointer<svtkIdTypeArray>::New();
  ...
  svtkIdType localId = 0;
  svtkIdType numLocalIds = this->PInternal->LocalToGlobalIds->GetNumberOfTuples();
  for (int process = 0; process < this->NumberOfPieces; process++)
  {
    SVTK_CREATE(svtkIdTypeArray, pointList);
    pointList->Allocate(this->NumberOfGlobalPoints / this->NumberOfPieces,
      this->NumberOfGlobalPoints / this->NumberOfPieces);
    svtkIdType lastId = this->EndPointRead(process);
    for (; (localId < numLocalIds); localId++)
    {
      svtkIdType globalId = this->PInternal->LocalToGlobalIds->GetValue(localId);
      if (globalId >= lastId)
        break;
    ...
    svtkIdType numPoints = pointList->GetNumberOfTuples();
       ...
       outputIter->GoToNextItem())
  {
    ...
    for (cellIter->GoToFirstCell(); !cellIter->IsDoneWithTraversal(); cellIter->GoToNextCell())
    {
      ...
      {
        const svtkIdType id = cell->GetId(i);
  ...
  if (this->ReadMidpoints)
  {
    ...
    this->PInternal->EdgesExpectedFromProcessesCounts = svtkSmartPointer<svtkIdTypeArray>::New();
    this->PInternal->EdgesExpectedFromProcessesCounts->SetNumberOfTuples(this->NumberOfPieces);
    this->PInternal->EdgesToSendToProcesses = svtkSmartPointer<svtkIdTypeArray>::New();
    this->PInternal->EdgesToSendToProcessesLengths = svtkSmartPointer<svtkIdTypeArray>::New();
    this->PInternal->EdgesToSendToProcessesLengths->SetNumberOfTuples(this->NumberOfPieces);
    this->PInternal->EdgesToSendToProcessesOffsets = svtkSmartPointer<svtkIdTypeArray>::New();
    this->PInternal->EdgesToSendToProcessesOffsets->SetNumberOfTuples(this->NumberOfPieces);

    std::vector<svtkSmartPointer<svtkIdTypeArray> > edgeLists(this->NumberOfPieces);
    for (int process = 0; process < this->NumberOfPieces; process++)
    {
      edgeLists[process] = svtkSmartPointer<svtkIdTypeArray>::New();
    ...
    for (size_t i = 0; i < edgesNeeded.size(); i++)
    {
      ...
      edgeLists[process]->InsertNextTypedTuple(static_cast<svtkIdType*>(ids));
    }
    for (int process = 0; process < this->NumberOfPieces; process++)
    {
      svtkIdType numEdges = edgeLists[process]->GetNumberOfTuples();
...
//-----------------------------------------------------------------------------
int svtkPSLACReader::RestoreMeshCache(svtkMultiBlockDataSet* surfaceOutput,
...
//-----------------------------------------------------------------------------
svtkSmartPointer<svtkDataArray> svtkPSLACReader::ReadPointDataArray(int ncFD, int varId)
{
  ...
  {
    svtkErrorMacro(<< "Encountered inconsistent number of coordinates.");
    return nullptr;
  }
  ...
  dataArray->SetNumberOfTuples(static_cast<svtkIdType>(count[0]));
  ...
  svtkSmartPointer<svtkDataArray> finalDataArray;
  finalDataArray.TakeReference(svtkDataArray::CreateDataArray(svtkType));
  ...
  svtkSmartPointer<svtkDataArray> sendBuffer;
  sendBuffer.TakeReference(svtkDataArray::CreateDataArray(svtkType));
  ...
  switch (svtkType)
  {
    svtkTemplateMacro(svtkPSLACReaderMapValues1((SVTK_TT*)dataArray->GetVoidPointer(0),
      (SVTK_TT*)sendBuffer->GetVoidPointer(0), static_cast<int>(numComponents),
  ...
  SVTK_CREATE(svtkIdTypeArray, sendLengths);
  sendLengths->SetNumberOfTuples(this->NumberOfPieces);
  SVTK_CREATE(svtkIdTypeArray, sendOffsets);
  ...
  for (int proc = 0; proc < this->NumberOfPieces; proc++)
  {
    ...
    svtkIdType destLength = static_cast<svtkIdType>(
      numComponents * this->PInternal->PointsExpectedFromProcessesLengths->GetValue(proc));
    svtkIdType destOffset = static_cast<svtkIdType>(
    ...
    this->Controller->GetCommunicator()->ScatterVVoidArray(sendBuffer->GetVoidPointer(0),
      ...
      sendOffsets->GetPointer(0), destLength, svtkType, proc);
  }
...
//-----------------------------------------------------------------------------
int svtkPSLACReader::ReadCoordinates(int meshFD, svtkMultiBlockDataSet* output)
{
...
//-----------------------------------------------------------------------------
int svtkPSLACReader::ReadFieldData(
...
//-----------------------------------------------------------------------------
int svtkPSLACReader::ReadMidpointCoordinates(
  int meshFD, svtkMultiBlockDataSet* svtkNotUsed(output), svtkSLACReader::MidpointCoordinateMap& map)
{
  ...
  svtkIdType numMidpointsPerPiece = this->NumberOfGlobalMidpoints / this->NumberOfPieces + 1;
  svtkIdType startMidpoint = this->RequestedPiece * numMidpointsPerPiece;
  svtkIdType endMidpoint = startMidpoint + numMidpointsPerPiece;
  ...
  midpointData->SetNumberOfTuples(static_cast<svtkIdType>(counts[0]));
  CALL_NETCDF(
  ...
  int pointsPerProcess = this->NumberOfGlobalPoints / this->NumberOfPieces + 1;
  for (svtkIdType i = 0; i < midpointData->GetNumberOfTuples(); i++)
  {
    ...
    midpointTopologyType topology;
    topology.minEdgePoint = static_cast<svtkIdType>(svtkMath::Min(mp[0], mp[1]));
    topology.maxEdgePoint = static_cast<svtkIdType>(svtkMath::Max(mp[0], mp[1]));
    ...
    svtkIdType process = topology.minEdgePoint / pointsPerProcess;
  ...
  for (int process = 0; process < this->NumberOfPieces; process++)
  {
    svtkIdType start = this->PInternal->EdgesToSendToProcessesOffsets->GetValue(process);
    svtkIdType end = start + this->PInternal->EdgesToSendToProcessesLengths->GetValue(process);
    ...
    {
      svtkWarningMacro("Inconsistent reader state detected. Skipping midpoint "
                      "sync.");
      end = start = 0;
    }
  ...
  typedef std::unordered_map<svtkIdType, svtkIdType, svtkPSLACReaderIdTypeHash> localMapType;
       ...
       posIter != midpointsToReceive.position.end(); ++posIter, ++topIter)
  {
    ...
    svtkIdType local0 = this->PInternal->GlobalToLocalIds[topIter->minEdgePoint];
    svtkIdType local1 = this->PInternal->GlobalToLocalIds[topIter->maxEdgePoint];
    ...
    map.AddMidpoint(svtkSLACReader::EdgeEndpoints(local0, local1),
      svtkSLACReader::MidpointCoordinates(posIter->coord, index));
  }
  return 1;
}

//-----------------------------------------------------------------------------
int svtkPSLACReader::ReadMidpointData(
  int meshFD, svtkMultiBlockDataSet* output, svtkSLACReader::MidpointIdMap& map)
{
  ...
  svtkIdType maxPointsAdded;
  this->Controller->AllReduce(&pointsAdded, &maxPointsAdded, 1, svtkCommunicator::MAX_OP);

  svtkIdType start = this->NumberOfGlobalPoints + this->NumberOfGlobalMidpoints +
    this->RequestedPiece * maxPointsAdded;
  svtkIdType end = start + pointsAdded;
  for (svtkIdType i = start; i < end; i++)
  {
...
//-----------------------------------------------------------------------------
int svtkPSLACReader::MeshUpToDate()
{
  ...
  int globalflag;
  this->Controller->AllReduce(&localflag, &globalflag, 1, svtkCommunicator::LOGICAL_AND_OP);
  return globalflag;
}
