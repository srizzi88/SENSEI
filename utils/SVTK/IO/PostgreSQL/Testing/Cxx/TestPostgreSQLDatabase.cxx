...
{
  // This test requires the user in SVTK_PSQL_TEST_URL to have
  ...
  svtkStringArray* dbNames = db->GetDatabases();
  cout << "Database list:\n";
  ...
  svtkStdString fauxDatabase = realDatabase + "blarney";
  ...
  svtkStdString dropQuery("DROP TABLE people");
  cout << dropQuery << endl;
  ...
  svtkStdString createQuery("CREATE TABLE people (name TEXT, age INTEGER, weight FLOAT)");
  ...
  db = svtkPostgreSQLDatabase::SafeDownCast(svtkSQLDatabase::CreateFromURL(SVTK_PSQL_TEST_URL));
  status = db->Open();
