/*=========================================================================
  ...
  Module:    svtkPostgreSQLDatabase.cxx
...
#define SVTK_CREATE(classname, varname)                                                             \
  svtkSmartPointer<classname> varname = svtkSmartPointer<classname>::New()

#include <libpq-fe.h>

#include <cassert>

svtkStandardNewMacro(svtkPostgreSQLDatabase);

// ----------------------------------------------------------------------
svtkPostgreSQLDatabase::svtkPostgreSQLDatabase()
{
  this->Connection = 0;
...
// ----------------------------------------------------------------------
void svtkPostgreSQLDatabase::PrintSelf(ostream& os, svtkIndent indent)
{
...
// ----------------------------------------------------------------------
svtkStdString svtkPostgreSQLDatabase::GetColumnSpecification(
...
// ----------------------------------------------------------------------
bool svtkPostgreSQLDatabase::Open(const char* password)
{
...
// ----------------------------------------------------------------------
void svtkPostgreSQLDatabase::Close()
{
  if (this->Connection)
  {
...
// ----------------------------------------------------------------------
bool svtkPostgreSQLDatabase::IsOpen()
{
...
// ----------------------------------------------------------------------
svtkSQLQuery* svtkPostgreSQLDatabase::GetQueryInstance()
{
  svtkPostgreSQLQuery* query = svtkPostgreSQLQuery::New();
  query->SetDatabase(this);
  return query;
}

// ----------------------------------------------------------------------
bool svtkPostgreSQLDatabase::HasError()
{
...
// ----------------------------------------------------------------------
const char* svtkPostgreSQLDatabase::GetLastErrorText()
{
  return this->LastErrorText;
}

// ----------------------------------------------------------------------
svtkStdString svtkPostgreSQLDatabase::GetURL()
{
  svtkStdString url = this->GetDatabaseType();
  url += "://";
  if (this->HostName && this->DatabaseName)
  {
    if (this->User && strlen(this->User) > 0)
...
// ----------------------------------------------------------------------
bool svtkPostgreSQLDatabase::ParseURL(const char* URL)
{
...
// ----------------------------------------------------------------------
svtkStringArray* svtkPostgreSQLDatabase::GetTables()
{
  ...
  if (!status)
  {
    svtkErrorMacro(<< "Database returned error: " << query->GetLastErrorText());
...
// ----------------------------------------------------------------------
svtkStringArray* svtkPostgreSQLDatabase::GetRecord(const char* table)
{
  ...
  if (!status)
  {
    svtkErrorMacro(<< "GetRecord(" << table
...
// ----------------------------------------------------------------------
bool svtkPostgreSQLDatabase::IsSupported(int feature)
{
  switch (feature)
  {
...
// ----------------------------------------------------------------------
svtkStringArray* svtkPostgreSQLDatabase::GetDatabases()
{
  if (!this->Connection)
  {
    svtkErrorMacro("Must be connected to a server to get a list of databases.");
    return 0;
  }
  ...
  if (!query)
  {
    svtkErrorMacro("Could not create a query.");
    return 0;
  }
  ...
  svtkStringArray* dbNames = svtkStringArray::New();
  while (query->NextRow())
  {
...
// ----------------------------------------------------------------------
bool svtkPostgreSQLDatabase::CreateDatabase(const char* dbName, bool dropExisting)
{
  if (!dbName)
  {
    svtkErrorMacro("Databases must have a non-nullptr name");
    return false;
  }
  ...
  if (!this->Connection)
  {
    ...
    if (err)
    {
      svtkErrorMacro("Must be connected to a server to create a database.");
      return false;
    }
  }
...
// ----------------------------------------------------------------------
bool svtkPostgreSQLDatabase::DropDatabase(const char* dbName)
{
  if (!dbName || strlen(dbName) == 0)
  {
    svtkErrorMacro("DropDatabase called with an empty database name");
    return false;
  }
  ...
  {
    this->SetLastErrorText(query->GetLastErrorText());
    svtkErrorMacro(<< "Could not drop database \"" << dbName << "\".  "
...
void svtkPostgreSQLDatabase::NullTrailingWhitespace(char* msg)
{
  ...
  // does not work well with SVTK's already lengthy error output.
...
bool svtkPostgreSQLDatabase::OpenInternal(const char* connectionOptions)
{
  assert(this->Connection == nullptr);
  this->Connection = new svtkPostgreSQLDatabasePrivate;
  ...
  else
  {
    ...
    svtkErrorMacro(<< "Unable to open database connection. " << this->GetLastErrorText());
    delete this->Connection;
    this->Connection = 0;
    return false;
  }
...
void svtkPostgreSQLDatabase::UpdateDataTypeMap()
{
  if (!this->IsOpen())
  {
    return;
  ...
  svtkSQLQuery* typeQuery = this->GetQueryInstance();
  typeQuery->SetQuery("SELECT oid, typname, typlen FROM pg_type");
  bool status = typeQuery->Execute();
  if (!status)
  {
    svtkErrorMacro(<< "I was totally surprised to see the data type query fail.  Error message: "
  ...
  else
  {
    while (typeQuery->NextRow())
    {
      ...
      {
        this->Connection->DataTypeMap[oid] = SVTK_TYPE_INT64;
      }
      else if (name == "int4" || (name == "oid" && len == 4))
      {
        this->Connection->DataTypeMap[oid] = SVTK_TYPE_INT32;
      }
      else if (name == "int2")
      {
        this->Connection->DataTypeMap[oid] = SVTK_TYPE_INT16;
      }
      else if (name == "char")
      {
        this->Connection->DataTypeMap[oid] = SVTK_TYPE_INT8;
      }
      ...
      {
        this->Connection->DataTypeMap[oid] = SVTK_FLOAT;
      }
      else if (name == "float8")
      {
        this->Connection->DataTypeMap[oid] = SVTK_DOUBLE;
      }
      else if (name == "abstime" || name == "reltime")
      {
        this->Connection->DataTypeMap[oid] = (len == 4 ? SVTK_TYPE_INT32 : SVTK_TYPE_INT64);
      }
      else if (name == "text")
      {
        this->Connection->DataTypeMap[oid] = SVTK_STRING;
      }
