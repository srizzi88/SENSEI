/*=========================================================================
  ...
  Module:    svtkPostgreSQLQuery.cxx
...
#include "svtkPostgreSQLDatabasePrivate.h"
...
#define DECLARE_CONVERTER(TargetType) svtkVariant ConvertStringTo##TargetType(bool, const char*);
...
svtkStandardNewMacro(svtkPostgreSQLQuery);
...
svtkVariant svtkPostgreSQLQuery::DataValue(svtkIdType column)
{
...
// ----------------------------------------------------------------------
svtkPostgreSQLQuery::svtkPostgreSQLQuery()
{
...
// ----------------------------------------------------------------------
void svtkPostgreSQLQuery::PrintSelf(ostream& os, svtkIndent indent)
{
...
// ----------------------------------------------------------------------
bool svtkPostgreSQLQuery::Execute()
{
  if (this->Query == 0)
  {
  ...
  svtkPostgreSQLDatabase* db = svtkPostgreSQLDatabase::SafeDownCast(this->Database);
  assert(db);
  ...
  if (!db->IsOpen())
  {
    ...
    svtkErrorMacro(<< "Cannot execute query.  Database connection is closed.");
    this->Active = false;
    return false;
  }

  this->QueryInternals = new svtkPostgreSQLQueryPrivate;
  ...
  switch (PQresultStatus(this->QueryInternals->QueryResults))
  {
    case PGRES_EMPTY_QUERY:
    {
      ...
      this->Active = false;
      this->DeleteQueryResults();
      svtkWarningMacro(<< "Query string was set but empty.");
    ...
    case PGRES_FATAL_ERROR:
    {
      ...
      svtkErrorMacro(<< "Fatal error during query: " << this->GetLastErrorText());
      this->DeleteQueryResults();
    };
    break;
    ...
    default:
    {
      ...
      svtkErrorMacro(<< "Unhandled server response: " << this->GetLastErrorText());
      this->DeleteQueryResults();
    };
    break;
  }
...
// ----------------------------------------------------------------------
int svtkPostgreSQLQuery::GetNumberOfFields()
{
  if (!this->Active || !this->QueryInternals)
  {
    svtkErrorMacro("Query is not active!");
    return 0;
  }
...
// ----------------------------------------------------------------------
const char* svtkPostgreSQLQuery::GetFieldName(int column)
{
  ...
  {
    svtkErrorMacro("Illegal field index " << column);
    return 0;
  }
...
// ----------------------------------------------------------------------
int svtkPostgreSQLQuery::GetFieldType(int column)
{
  if (!this->Active || !this->QueryInternals)
  {
    svtkErrorMacro("Query is not active!");
  ...
  if (!db)
  {
    svtkErrorMacro(<< "No database!  How did this happen?");
    return -1;
  }
  return db->Connection->GetSVTKTypeFromOID(PQftype(this->QueryInternals->QueryResults, column));
}

// ----------------------------------------------------------------------
bool svtkPostgreSQLQuery::NextRow()
{
...
// ----------------------------------------------------------------------
const char* svtkPostgreSQLQuery::GetLastErrorText()
{
  if (!this->Database)
  {
    return "No database";
...
svtkStdString svtkPostgreSQLQuery::EscapeString(svtkStdString s, bool addSurroundingQuotes)
{
  svtkStdString retval;
  if (addSurroundingQuotes)
  {
    retval = "'";
  }

  svtkPostgreSQLDatabase* db = static_cast<svtkPostgreSQLDatabase*>(this->Database);
  ...
  if (db && db->Connection)
  {
    ...
    if (error)
    {
      svtkErrorMacro(<< "Error while escaping string.  Expect the result to be unusable.");
    }
  }
  else
  {
...
// ----------------------------------------------------------------------
bool svtkPostgreSQLQuery::HasError()
{
  if (!this->Database)
  {
    return false;
...
// ----------------------------------------------------------------------
bool svtkPostgreSQLQuery::BeginTransaction()
{
  ...
  switch (PQresultStatus(result))
  {
    ...
    case PGRES_FATAL_ERROR:
    {
      ...
      svtkErrorMacro(<< "Error in BeginTransaction: " << this->GetLastErrorText());
      status = false;
    };
    break;
    default:
    {
      this->SetLastErrorText(PQresultErrorMessage(result));
      svtkWarningMacro(<< "Unexpected return code " << PQresultStatus(result) << " ("
...
// ----------------------------------------------------------------------
bool svtkPostgreSQLQuery::CommitTransaction()
{
  if (!this->TransactionInProgress)
  {
    svtkErrorMacro(<< "Cannot commit: no transaction is in progress.");
    return false;
  }
  ...
  switch (PQresultStatus(result))
  {
    ...
    {
      this->SetLastErrorText(PQresultErrorMessage(result));
      svtkErrorMacro(<< "Error in CommitTransaction: " << this->GetLastErrorText());
      this->TransactionInProgress = false;
      status = false;
    };
    break;
...
// ----------------------------------------------------------------------
bool svtkPostgreSQLQuery::RollbackTransaction()
{
  if (!this->TransactionInProgress)
  {
    svtkErrorMacro(<< "Cannot rollback: no transaction is in progress.");
    return false;
  }
  ...
  switch (PQresultStatus(result))
  {
    ...
    {
      this->SetLastErrorText(PQresultErrorMessage(result));
      svtkErrorMacro(<< "Error in RollbackTransaction: " << this->GetLastErrorText());
      this->TransactionInProgress = false;
      status = false;
    };
    break;
...
void svtkPostgreSQLQuery::DeleteQueryResults()
{
  this->Active = false;
  delete this->QueryInternals;
  this->QueryInternals = nullptr;
...
svtkVariant ConvertStringToBoolean(bool, const char* rawData)
{
  ...
  switch (rawData[0])
  {
    ...
    case 0:
    {
      return svtkVariant(false);
    }

    default:
    {
      svtkGenericWarningMacro(<< "Unable to convert raw data to boolean.  Data length is "
...
svtkVariant ConvertStringToSignedChar(bool isBinary, const char* rawData)
{
  if (isBinary)
  {
    return svtkVariant(rawData[0]);
  }
  else
  {
    svtkVariant converter(rawData);
    return svtkVariant(converter.ToChar());
  }
}

// ----------------------------------------------------------------------

svtkVariant ConvertStringToUnsignedChar(bool isBinary, const char* rawData)
{
  if (isBinary)
  {
    return svtkVariant(static_cast<unsigned char>(rawData[0]));
  }
  else
  {
    svtkVariant converter(rawData);
    return svtkVariant(converter.ToUnsignedChar());
  }
}

// ----------------------------------------------------------------------

svtkVariant ConvertStringToSignedShort(bool isBinary, const char* rawData)
{
  if (isBinary)
  {
    short result = 0;
    ConvertFromNetworkOrder(result, rawData);
    return svtkVariant(result);
  }
  else
  {
    svtkVariant converter(rawData);
    return svtkVariant(converter.ToShort());
  }
}

// ----------------------------------------------------------------------

svtkVariant ConvertStringToUnsignedShort(bool isBinary, const char* rawData)
{
  if (isBinary)
  {
  ...
  {
    svtkVariant converter(rawData);
    return svtkVariant(converter.ToUnsignedShort());
  }
}

// ----------------------------------------------------------------------

svtkVariant ConvertStringToSignedInt(bool isBinary, const char* rawData)
{
  if (isBinary)
  {
    int result = 0;
  ...
  {
    svtkVariant converter(rawData);
    return svtkVariant(converter.ToInt());
  }
}

// ----------------------------------------------------------------------

svtkVariant ConvertStringToUnsignedInt(bool isBinary, const char* rawData)
{
  if (isBinary)
  {
  ...
  {
    svtkVariant converter(rawData);
    return svtkVariant(converter.ToUnsignedInt());
  }
}

// ----------------------------------------------------------------------

svtkVariant ConvertStringToSignedLong(bool isBinary, const char* rawData)
{
  if (isBinary)
  {
  ...
  {
    svtkVariant converter(rawData);
    return svtkVariant(converter.ToLong());
  }
}

// ----------------------------------------------------------------------

svtkVariant ConvertStringToUnsignedLong(bool isBinary, const char* rawData)
{
  if (isBinary)
  {
...
svtkVariant ConvertStringToSignedLongLong(bool isBinary, const char* rawData)
{
  if (isBinary)
  {
    long long result = 0;
  ...
  {
    svtkVariant converter(rawData);
    return svtkVariant(converter.ToLongLong());
  }
}

// ----------------------------------------------------------------------

svtkVariant ConvertStringToUnsignedLongLong(bool isBinary, const char* rawData)
{
  if (isBinary)
  {
  ...
  {
    svtkVariant converter(rawData);
    return svtkVariant(converter.ToUnsignedLongLong());
  }
}

// ----------------------------------------------------------------------

svtkVariant ConvertStringToFloat(bool isBinary, const char* rawData)
{
  if (isBinary)
  {
    ...
    return svtkVariant(floatResult);
  }
  else
  {
    svtkStdString rawString(rawData);
    float finalResult;
    ...
    return svtkVariant(finalResult);
  } // end of handling string representation
}

// ----------------------------------------------------------------------

svtkVariant ConvertStringToVtkIdType(bool isBinary, const char* rawData)
{
  if (isBinary)
  {
    svtkIdType result = 0;
...
svtkVariant ConvertStringToDouble(bool isBinary, const char* rawData)
{
  if (isBinary)
  {
    // As of PostgreSQL version 8.3.0, libpq transmits a float in network
...
bool svtkPostgreSQLQuery::IsColumnBinary(int whichColumn)
{
  if ((!this->Active) || (!this->Database) || (!this->QueryInternals->QueryResults))
  {
    svtkWarningMacro(<< "No active query!");
    return false;
  }
  else if (whichColumn < 0 || whichColumn >= this->GetNumberOfFields())
  {
    svtkWarningMacro(<< "Illegal column index " << whichColumn);
    return false;
  }
  else
  {
...
const char* svtkPostgreSQLQuery::GetColumnRawData(int whichColumn)
{
  if ((!this->Active) || (!this->Database) || (!this->QueryInternals->QueryResults))
  {
    svtkWarningMacro(<< "No active query!");
...
int svtkPostgreSQLQuery::GetNumberOfRows()
{
  if (!this->Database || !this->Database->IsOpen() || !this->QueryInternals || !this->Active)
  {
    svtkWarningMacro(<< "No active query.  Cannot retrieve number of rows.");
    return 0;
  }
  else
  {
