...
int TestSQLiteDatabase(int /*argc*/, char* /*argv*/[])
{
  ...
  svtkSQLDatabase* db0 = svtkSQLDatabase::CreateFromURL(nullptr);
  if (db0)
  {
  ...
  svtkSmartPointer<svtkTest::ErrorObserver> queryObserver =
    svtkSmartPointer<svtkTest::ErrorObserver>::New();

  svtkSQLiteDatabase* db1 =
    svtkSQLiteDatabase::SafeDownCast(svtkSQLDatabase::CreateFromURL("sqlite://local.db"));

  status = db1->Open("", svtkSQLiteDatabase::CREATE_OR_CLEAR);
  svtkSQLQuery* query1 = db1->GetQueryInstance();
  ...
  svtkSQLiteDatabase* db2 =
    svtkSQLiteDatabase::SafeDownCast(svtkSQLDatabase::CreateFromURL("sqlite://local.db"));
  db2->AddObserver(svtkCommand::ErrorEvent, errorObserver);
  status = db2->Open("", svtkSQLiteDatabase::CREATE);
  if (status)
  {
  ...
  svtkSQLiteDatabase* db3 =
    svtkSQLiteDatabase::SafeDownCast(svtkSQLDatabase::CreateFromURL("sqlite://local.db"));
  status = db3->Open("", svtkSQLiteDatabase::USE_EXISTING_OR_CREATE);
  if (!status)
  {
    cerr << "Using USE_EXISTING_OR_CREATE did not work.\n";
    return 1;
  }
  svtkSQLQuery* query3 = db3->GetQueryInstance();
  ...
  svtkSQLiteDatabase* db4 =
    svtkSQLiteDatabase::SafeDownCast(svtkSQLDatabase::CreateFromURL("sqlite://local.db"));
  status = db4->Open("", svtkSQLiteDatabase::CREATE_OR_CLEAR);
  if (!status)
  {
    cerr << "Using CREATE_OR_CLEAR did not work.\n";
    return 1;
  }
  svtkSQLQuery* query4 = db4->GetQueryInstance();
  query4->AddObserver(svtkCommand::ErrorEvent, queryObserver);
  ...
  svtkSQLiteDatabase* db =
    svtkSQLiteDatabase::SafeDownCast(svtkSQLDatabase::CreateFromURL("sqlite://:memory:"));
  status = db->Open("");
  ...
  {
    svtkStdString tblNameSch(schema->GetTableNameFromHandle(tblHandle));
    svtkStdString tblNameDB(query->DataValue(0).ToString());
    cerr << "     " << tblNameDB << "\n";
  ...
  svtkStdString queryStr = "INSERT INTO atable (somename,somenmbr) VALUES ( " +
