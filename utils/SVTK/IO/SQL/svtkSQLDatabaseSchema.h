...
Module:    svtkSQLDatabaseSchema.h
...
#ifndef svtkSQLDatabaseSchema_h
#define svtkSQLDatabaseSchema_h
...
// This is a list of known supported SVTK SQL backend classes.
...
#define SVTK_SQL_ALLBACKENDS "*" // works for all backends
#define SVTK_SQL_MYSQL "svtkMySQLDatabase"
#define SVTK_SQL_POSTGRESQL "svtkPostgreSQLDatabase"
#define SVTK_SQL_SQLITE "svtkSQLiteDatabase"

class svtkSQLDatabaseSchemaInternals;

class SVTKIOSQL_EXPORT svtkSQLDatabaseSchema : public svtkObject
{
public:
  svtkTypeMacro(svtkSQLDatabaseSchema, svtkObject);
  void PrintSelf(ostream& os, svtkIndent indent) override;
  static svtkSQLDatabaseSchema* New();
   ...
   * Example usage:
   * svtkSQLDatabaseSchema* schema = svtkSQLDatabaseSchema::New();
   * schema->SetName( "Example" );
   * schema->AddPreamble( "dropPLPGSQL", "DROP LANGUAGE IF EXISTS PLPGSQL CASCADE",
   * SVTK_SQL_POSTGRESQL ); schema->AddPreamble( "loadPLPGSQL", "CREATE LANGUAGE PLPGSQL",
   * SVTK_SQL_POSTGRESQL ); schema->AddPreamble( "createsomefunction", "CREATE OR REPLACE FUNCTION
   * somefunction() RETURNS TRIGGER AS $btable$ " "BEGIN " "INSERT INTO btable (somevalue) VALUES
   * (NEW.somenmbr); " "RETURN NEW; " "END; $btable$ LANGUAGE PLPGSQL", SVTK_SQL_POSTGRESQL );
   */
  virtual int AddPreamble(
    const char* preName, const char* preAction, const char* preBackend = SVTK_SQL_ALLBACKENDS);
  ...
  virtual int AddTriggerToTable(int tblHandle, int trgType, const char* trgName,
    const char* trgAction, const char* trgBackend = SVTK_SQL_ALLBACKENDS);
  ...
  virtual int AddOptionToTable(
    int tblHandle, const char* optStr, const char* optBackend = SVTK_SQL_ALLBACKENDS);
  virtual int AddOptionToTable(
    const char* tblName, const char* optStr, const char* optBackend = SVTK_SQL_ALLBACKENDS)
  {
   ...
   * svtkSQLDatabaseSchema::COLUMN_TOKEN, svtkSQLDatabaseSchema::INTEGER, "tablekey",  0, "",
   * svtkSQLDatabaseSchema::COLUMN_TOKEN, svtkSQLDatabaseSchema::VARCHAR, "somename", 11, "NOT
   * nullptr", svtkSQLDatabaseSchema::COLUMN_TOKEN, svtkSQLDatabaseSchema::BIGINT,  "somenmbr", 17,
   * "DEFAULT 0", svtkSQLDatabaseSchema::INDEX_TOKEN, svtkSQLDatabaseSchema::PRIMARY_KEY, "bigkey",
   * svtkSQLDatabaseSchema::INDEX_COLUMN_TOKEN, "tablekey",
   * svtkSQLDatabaseSchema::END_INDEX_TOKEN,
   * svtkSQLDatabaseSchema::INDEX_TOKEN,  svtkSQLDatabaseSchema::UNIQUE, "reverselookup",
   * svtkSQLDatabaseSchema::INDEX_COLUMN_TOKEN, "somename",
   * svtkSQLDatabaseSchema::INDEX_COLUMN_TOKEN, "somenmbr",
   * svtkSQLDatabaseSchema::END_INDEX_TOKEN,
   * svtkSQLDatabaseSchema::TRIGGER_TOKEN,  svtkSQLDatabaseSchema::AFTER_INSERT,
   * "InsertTrigger", "DO NOTHING", SVTK_SQL_SQLITE,
   * svtkSQLDatabaseSchema::TRIGGER_TOKEN,  svtkSQLDatabaseSchema::AFTER_INSERT,
   * "InsertTrigger", "FOR EACH ROW EXECUTE PROCEDURE somefunction ()", SVTK_SQL_POSTGRESQL,
   * svtkSQLDatabaseSchema::TRIGGER_TOKEN,  svtkSQLDatabaseSchema::AFTER_INSERT,
   * "InsertTrigger", "FOR EACH ROW INSERT INTO btable SET SomeValue = NEW.SomeNmbr", SVTK_SQL_MYSQL,
...
protected:
  ...
  ~svtkSQLDatabaseSchema() override;

  char* Name;

  class svtkSQLDatabaseSchemaInternals* Internals;

private:
  svtkSQLDatabaseSchema(const svtkSQLDatabaseSchema&) = delete;
  void operator=(const svtkSQLDatabaseSchema&) = delete;
};

#endif // svtkSQLDatabaseSchema_h
