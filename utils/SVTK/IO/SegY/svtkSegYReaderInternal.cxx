/*=========================================================================
  ...
  Module:    svtkSegYReaderInternal.cxx
...
#include "svtkSegYBinaryHeaderBytesPositions.h"
...
//-----------------------------------------------------------------------------
svtkSegYReaderInternal::svtkSegYReaderInternal()
  : SampleInterval(0)
  , FormatCode(0)
  , SampleCountPerTrace(0)
{
  this->BinaryHeaderBytesPos = new svtkSegYBinaryHeaderBytesPositions();
  this->VerticalCRS = 0;
  this->TraceReader = new svtkSegYTraceReader();
}
...
//-----------------------------------------------------------------------------
void svtkSegYReaderInternal::SetXYCoordBytePositions(int x, int y)
{
  this->TraceReader->SetXYCoordBytePositions(x, y);
}

//-----------------------------------------------------------------------------
void svtkSegYReaderInternal::SetVerticalCRS(int v)
{
  this->VerticalCRS = v > 0 ? 1 : 0;
}

//-----------------------------------------------------------------------------
void svtkSegYReaderInternal::LoadTraces(int* extent)
{
  std::streamoff traceStartPos = FIRST_TRACE_START_POS;
  std::streamoff fileSize = svtkSegYIOUtils::Instance()->getFileSize(this->In);
  ...
  {
    svtkSegYTrace* pTrace = new svtkSegYTrace();
...
//-----------------------------------------------------------------------------
bool svtkSegYReaderInternal::ReadHeader()
{
  this->SampleInterval = svtkSegYIOUtils::Instance()->readShortInteger(
    this->BinaryHeaderBytesPos->SampleInterval, this->In);
  this->FormatCode =
    svtkSegYIOUtils::Instance()->readShortInteger(this->BinaryHeaderBytesPos->FormatCode, this->In);
  this->SampleCountPerTrace = svtkSegYIOUtils::Instance()->readShortInteger(
    this->BinaryHeaderBytesPos->NumSamplesPerTrace, this->In);
  return true;
}

//-----------------------------------------------------------------------------
bool svtkSegYReaderInternal::Is3DComputeParameters(
  int* extent, double origin[3], double spacing[3][3], int* spacingSign, bool force2D)
{
  ...
  while (traceStartPos + 240 < fileSize)
  {
    ...
    if (basisPointCount == 2)
    {
      ...
      iBasis[1][2] = 0.0;
      basisLength[1] = svtkMath::Normalize(iBasis[1]);
      if (fabs(svtkMath::Dot(iBasis[0], iBasis[1])) < 0.99)
      {
    ...
    if (basisPointCount == 1)
    {
      ...
      iBasis[0][2] = 0.0;
      basisLength[0] = svtkMath::Normalize(iBasis[0]);
      basisPointCount++;
    }
  ...
  if (basisPointCount == 3)
  {
    ...
    double bDot = svtkMath::Dot(iBasis[0], iBasis[1]);
    iBasis[1][0] -= bDot * iBasis[0][0];
    iBasis[1][1] -= bDot * iBasis[0][1];
    svtkMath::Normalize(iBasis[1]);
...
//-----------------------------------------------------------------------------
void svtkSegYReaderInternal::ExportData(
  svtkImageData* imageData, int* extent, double origin[3], double spacing[3][3], int* spacingSign)
{
  imageData->SetExtent(extent);
  imageData->SetOrigin(origin);
  imageData->SetSpacing(
    svtkMath::Norm(spacing[0]), svtkMath::Norm(spacing[1]), svtkMath::Norm(spacing[2]));
  ...
  for (int k = 0; k < dims[2]; ++k)
  {
    ...
    for (int j = 0; j < dims[1]; ++j)
    {
      ...
      for (int i = 0; i < dims[0]; ++i)
      {
        ...
        svtkSegYTrace* trace = this->Traces[destJ * dims[0] + destI];
...
//-----------------------------------------------------------------------------
void svtkSegYReaderInternal::ExportData(
  svtkStructuredGrid* grid, int* extent, double origin[3], double spacing[3][3])
{
  if (!grid)
  {
    return;
