...
int TestTRUCHASReader(int argc, char* argv[])
{
  ...
  char* fileName =
    svtkTestUtilities::ExpandDataFileName(argc, argv, "Data/TRUCHAS/viscoplastic-ring.h5");

  svtkNew<svtkTRUCHASReader> reader;
  ...
  svtkUnstructuredGrid* grid = svtkUnstructuredGrid::SafeDownCast(reader->GetOutput()->GetBlock(1));
  if (!grid)
  {
  ...
  {
    svtkDataArray* da = grid->GetCellData()->GetArray(a);
  ...
  {
    svtkDataArray* da = grid->GetPointData()->GetArray(a);
    cerr << da->GetName() << endl;
  }
  ...
  svtkDoubleArray* da = svtkDoubleArray::SafeDownCast(grid->GetCellData()->GetArray("Grad_T"));
  if (!da)
  {
    cerr << "Couldn't get "
         << "Grad_T"
  ...
  svtkInformation* inf = reader->GetExecutive()->GetOutputInformation(0);
  int numTimes = inf->Length(svtkStreamingDemandDrivenPipeline::TIME_STEPS());
  cerr << "FOUND " << numTimes << " timesteps " << endl;
  double tAlpha = inf->Get(svtkStreamingDemandDrivenPipeline::TIME_STEPS(), 0);
  double tOmega = inf->Get(svtkStreamingDemandDrivenPipeline::TIME_STEPS(), numTimes - 1);
  ...
  for (int i = 0; i < divs; i++)
  {
    ...
    grid = svtkUnstructuredGrid::SafeDownCast(reader->GetOutput()->GetBlock(1));
    da = svtkDoubleArray::SafeDownCast(grid->GetCellData()->GetArray("dTdt"));
