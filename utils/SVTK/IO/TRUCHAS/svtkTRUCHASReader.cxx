...
//----------------------------------------------------------------------------
class svtkTRUCHASReader::Internal
{
public:
  //----------------------------------------------------------------------------
  Internal()
  {
    ...
    this->SVTKBlockFileIndx = this->FileIndx;
...
#define MAX_NAME 1024
  ...
  //----------------------------------------------------------------------------
  bool ReadAvailableBlocks(svtkTRUCHASReader* self)
  {
  ...
  //----------------------------------------------------------------------------
  bool MakeSVTKBlocks(svtkTRUCHASReader* self)
  {
    if (this->SVTKBlockFileIndx == this->FileIndx &&
    ...
    for (unsigned int b = 0; b < totalNumBlocks; b++)
    {
      ...
      {
        this->grid[b] = svtkUnstructuredGrid::New();
    ...
    for (unsigned int c = 0; c < this->totalNumCells; c++)
    {
      ...
      if (self->BlockChoices->GetArraySetting(blockidx) != 0)
      {
        ...
        int i;
        svtkIdType list[8] = { *(cptr + 0), *(cptr + 1), *(cptr + 2), *(cptr + 3), *(cptr + 4),
        ...
        { /* tet element */
          ...
          this->grid[blockidx]->InsertNextCell(SVTK_TETRA, 4, &list[0]);
        }
        ...
        { /* pyramid element */
          ...
          this->grid[blockidx]->InsertNextCell(SVTK_PYRAMID, 5, &list[0]);
        }
        ...
        { /* wedge element */
          ...
          /* Convert from Exodus ordering to SVTK / XDMF ordering */
          i = list[1];
          list[1] = list[2];
          ...
          this->grid[blockidx]->InsertNextCell(SVTK_WEDGE, 6, &list[0]);
        }
        else
        { /* hex element */
          // cerr << "H" << endl;
          ...
          this->grid[blockidx]->InsertNextCell(SVTK_HEXAHEDRON, 8, &list[0]);
        }
      }
      blockptr++;
      cptr += 8;
  ...
  //----------------------------------------------------------------------------
  bool MoveSVTKBlocks(svtkTRUCHASReader* self, hid_t now_gid)
  {
  ...
  //----------------------------------------------------------------------------
  bool ReadArrayNames(svtkTRUCHASReader* self)
  {
...
private:
  ...
  hid_t SVTKBlockFileIndx;
  svtkMTimeType BlockChoiceTime;

  hid_t ArrayNameFileIndx;
};

//----------------------------------------------------------------------------
svtkStandardNewMacro(svtkTRUCHASReader);

//----------------------------------------------------------------------------
svtkTRUCHASReader::svtkTRUCHASReader()
  : svtkMultiBlockDataSetAlgorithm()
{
  this->Internals = new svtkTRUCHASReader::Internal;
  this->FileName = nullptr;
  this->BlockChoices = svtkDataArraySelection::New();
  this->PointArrayChoices = svtkDataArraySelection::New();
  this->CellArrayChoices = svtkDataArraySelection::New();
...
//----------------------------------------------------------------------------
int svtkTRUCHASReader::RequestInformation(
  svtkInformation* reqInfo, svtkInformationVector** inVector, svtkInformationVector* outVector)
{
  ...
  svtkInformation* info = outVector->GetInformationObject(0);
  info->Set(svtkStreamingDemandDrivenPipeline::TIME_RANGE(), tRange, 2);
  info->Set(svtkStreamingDemandDrivenPipeline::TIME_STEPS(), steps, numSteps);
...
//----------------------------------------------------------------------------
int svtkTRUCHASReader::RequestData(
  svtkInformation*, svtkInformationVector**, svtkInformationVector* outputVector)
{
  ...
  svtkDebugMacro(<< "Reading truchas unstructured grid...");
  ...
  // coordinates
  svtkPoints* pts = this->Internals->ReadGeometry();
  if (!pts)
  {
    return 0;
  }
  ...
  ret = this->Internals->MakeSVTKBlocks(this);
  if (!ret)
  {
    return 0;
  }

  std::vector<svtkUnstructuredGrid*>& grid = this->Internals->grid;
  ...
  {
    reqTS = outInfo->Get(svtkStreamingDemandDrivenPipeline::UPDATE_TIME_STEP());
    reqTime = reqTS;
  }
  ...
  ret = this->Internals->MoveSVTKBlocks(this, now_gid);
  if (!ret)
  {
    H5Gclose(now_gid);
    return 0;
...
#define MAX_NAME 1024
  ...
  for (nit = this->Internals->array_names.begin(); nit != this->Internals->array_names.end(); ++nit)
  {
    ...
    std::map<int, svtkDataArray*> gblockToArray;
    for (unsigned b = 0; b < totalNumBlocks; b++)
    {
    ...
    std::map<std::string, svtkDataArray*> nameToPointArray;
    for (int a = 0; a < this->Internals->PointData->GetNumberOfArrays(); a++)
    {
      // populate the map when filled in a different timestep
      svtkDataArray* da = svtkDataArray::SafeDownCast(this->Internals->PointData->GetArray(a));
    ...
    for (unsigned int b = 0; b < totalNumBlocks; b++)
    {
      ...
      svtkDataSetAttributes* arrayGroup = grid[b]->GetCellData();
      if (!forcells)
      {
        arrayGroup = grid[b]->GetPointData();
      }
      vArray = svtkDoubleArray::SafeDownCast(arrayGroup->GetArray(array_name));
      if (!vArray)
      {
        if (forcells)
        {
          if (isFloat)
          {
            vArray = svtkDoubleArray::New();
          }
          else
          {
            vArray = svtkIntArray::New();
          }
        ...
        {
          svtkDataArray* mArray = nameToPointArray[std::string(array_name)];
          if (!mArray)
          {
            if (isFloat)
            {
              mArray =
                svtkDoubleArray::SafeDownCast(this->Internals->PointData->GetArray(array_name));
            }
            else
            {
              mArray = svtkIntArray::SafeDownCast(this->Internals->PointData->GetArray(array_name));
            }
            if (!mArray)
            {
              if (isFloat)
              {
                mArray = svtkDoubleArray::New();
              }
              else
              {
                mArray = svtkIntArray::New();
              }
    ...
    for (unsigned int elem = 0; elem < dims[0]; elem++)
    {
      if (forcells)
      {
        ...
        svtkDataArray* vArray = gblockToArray[gblockid];
      ...
      {
        svtkDataArray* vArray = nameToPointArray[std::string(array_name)];
        for (unsigned int comp = 0; comp < dims[1]; comp++)
        {
          if (isFloat)
          {
  ...
  for (unsigned int b = 0; b < totalNumBlocks; b++)
  {
    ...
    output->GetMetaData(b)->Set(svtkCompositeDataSet::NAME(), bname);
  }
...
//----------------------------------------------------------------------------
int svtkTRUCHASReader::CanReadFile(const char* filename)
{
...
//----------------------------------------------------------------------------
void svtkTRUCHASReader::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
int svtkTRUCHASReader::GetNumberOfBlockArrays()
{
  return this->BlockChoices->GetNumberOfArrays();
}

//----------------------------------------------------------------------------
void svtkTRUCHASReader::SetBlockArrayStatus(const char* gridname, int status)
{
  if (status != 0)
  {
...
//----------------------------------------------------------------------------
int svtkTRUCHASReader::GetBlockArrayStatus(const char* arrayname)
{
  return this->BlockChoices->ArrayIsEnabled(arrayname);
}

//----------------------------------------------------------------------------
const char* svtkTRUCHASReader::GetBlockArrayName(int index)
{
  return this->BlockChoices->GetArrayName(index);
}

//----------------------------------------------------------------------------
int svtkTRUCHASReader::GetNumberOfPointArrays()
{
  return this->PointArrayChoices->GetNumberOfArrays();
}

//----------------------------------------------------------------------------
void svtkTRUCHASReader::SetPointArrayStatus(const char* gridname, int status)
{
  if (status != 0)
  {
...
//----------------------------------------------------------------------------
int svtkTRUCHASReader::GetPointArrayStatus(const char* arrayname)
{
  return this->PointArrayChoices->ArrayIsEnabled(arrayname);
}

//----------------------------------------------------------------------------
const char* svtkTRUCHASReader::GetPointArrayName(int index)
{
  return this->PointArrayChoices->GetArrayName(index);
}

//----------------------------------------------------------------------------
int svtkTRUCHASReader::GetNumberOfCellArrays()
{
  return this->CellArrayChoices->GetNumberOfArrays();
}

//----------------------------------------------------------------------------
void svtkTRUCHASReader::SetCellArrayStatus(const char* gridname, int status)
{
  if (status != 0)
  {
...
//----------------------------------------------------------------------------
int svtkTRUCHASReader::GetCellArrayStatus(const char* arrayname)
{
  return this->CellArrayChoices->ArrayIsEnabled(arrayname);
}

//----------------------------------------------------------------------------
const char* svtkTRUCHASReader::GetCellArrayName(int index)
{
