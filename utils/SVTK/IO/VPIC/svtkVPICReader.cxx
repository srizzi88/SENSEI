...
#include "svtkvpic/GridExchange.h"
#include "svtkvpic/VPICDataSet.h"
#include "svtkvpic/VPICView.h"

svtkStandardNewMacro(svtkVPICReader);

//----------------------------------------------------------------------------
// Constructor for VPIC Reader
//----------------------------------------------------------------------------
svtkVPICReader::svtkVPICReader()
{
  ...
  this->SelectionObserver->SetCallback(&svtkVPICReader::SelectionCallback);
...
//----------------------------------------------------------------------------
int svtkVPICReader::RequestInformation(svtkInformation* svtkNotUsed(reqInfo),
  svtkInformationVector** svtkNotUsed(inVector), svtkInformationVector* outVector)
{
  ...
  if (this->vpicData == 0)
  {
    ...
    this->VariableName = new svtkStdString[this->NumberOfVariables];
  ...
  if (this->vpicData->needsGridCalculation() == true)
  {
    ...
    if (this->MPIController)
    {
      this->MPIController->AllReduce(&processorUsed, &this->UsedRank, 1, svtkCommunicator::SUM_OP);
    }
...
//----------------------------------------------------------------------------
// Data is read into a svtkImageData
// BLOCK structured means data is organized by variable and then by cell
//----------------------------------------------------------------------------
int svtkVPICReader::RequestData(svtkInformation* svtkNotUsed(reqInfo),
  svtkInformationVector** svtkNotUsed(inVector), svtkInformationVector* outVector)
{
  svtkInformation* outInfo = outVector->GetInformationObject(0);
  svtkImageData* output = svtkImageData::SafeDownCast(outInfo->Get(svtkDataObject::DATA_OBJECT()));
...
//----------------------------------------------------------------------------
void svtkVPICReader::LoadVariableData(int var, int timeStep)
{
  this->data[var]->Delete();
  this->data[var] = svtkFloatArray::New();
  this->data[var]->SetName(VariableName[var].c_str());
...
// ParaView svtkFloatArray taking into account whether the processor is
...
//----------------------------------------------------------------------------
void svtkVPICReader::LoadComponent(float* varData, float* block, int comp, int numberOfComponents)
{
...
//----------------------------------------------------------------------------
void svtkVPICReader::SelectionCallback(
  svtkObject*, unsigned long svtkNotUsed(eventid), void* clientdata, void* svtkNotUsed(calldata))
{
  static_cast<svtkVPICReader*>(clientdata)->Modified();
}

//----------------------------------------------------------------------------
svtkImageData* svtkVPICReader::GetOutput()
{
  return this->GetOutput(0);
}

//----------------------------------------------------------------------------
svtkImageData* svtkVPICReader::GetOutput(int idx)
{
  if (idx)
  {
    return nullptr;
...
//----------------------------------------------------------------------------
int svtkVPICReader::GetNumberOfPointArrays()
{
  return this->PointDataArraySelection->GetNumberOfArrays();
}

//----------------------------------------------------------------------------
void svtkVPICReader::EnableAllPointArrays()
{
  this->PointDataArraySelection->EnableAllArrays();
}

//----------------------------------------------------------------------------
void svtkVPICReader::DisableAllPointArrays()
{
  this->PointDataArraySelection->DisableAllArrays();
}

//----------------------------------------------------------------------------
const char* svtkVPICReader::GetPointArrayName(int index)
{
  return this->VariableName[index].c_str();
}

//----------------------------------------------------------------------------
int svtkVPICReader::GetPointArrayStatus(const char* name)
{
  return this->PointDataArraySelection->ArrayIsEnabled(name);
}

//----------------------------------------------------------------------------
void svtkVPICReader::SetPointArrayStatus(const char* name, int status)
{
  if (status)
...
void svtkVPICReader::PrintSelf(ostream& os, svtkIndent indent)
{
