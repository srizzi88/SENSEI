...
//*****************************************************************************
class svtkVeraOutReader::Internals
{
public:
  Internals(svtkObject* owner)
  {
    this->Owner = owner;
  ...
  bool OpenFile()
  {
    ...
    {
      svtkErrorWithObjectMacro(this->Owner, "Couldn't H5Pcreate");
      return false;
    }
    herr_t err = H5Pset_fclose_degree(fileAccessPropListID, H5F_CLOSE_SEMI);
    if (err < 0)
    {
      svtkErrorWithObjectMacro(this->Owner, "Couldn't set file close access");
      return false;
    }
  ...
  void LoadMetaData()
  {
    ...
    if (this->GetNumberOfTimeSteps())
    {
      ...
      {
        svtkErrorWithObjectMacro(this->Owner, "Can't open Group /STATE_0001");
        return;
      }
  ...
  bool ReadDataSetDimensions(const char* groupName, const char* datasetName)
  {
    ...
    {
      svtkErrorWithObjectMacro(this->Owner, "Can't open group " << groupName);
      return false;
    }
    ...
    {
      H5Gclose(groupId);
      svtkErrorWithObjectMacro(this->Owner,
  ...
  svtkDataArray* ReadDataSet(const char* groupName, const char* datasetName)
  {
    ...
    svtkDataArray* arrayToReturn = nullptr;

    // Compute total size
    svtkIdType nbTuples = 1;
    ...
    {
      arrayToReturn = svtkFloatArray::New();
      arrayToReturn->SetNumberOfTuples(nbTuples);
      float* arrayPtr =
        static_cast<float*>(svtkArrayDownCast<svtkFloatArray>(arrayToReturn)->GetPointer(0));
      H5Dread(datasetId, dataType, H5S_ALL, H5S_ALL, H5P_DEFAULT, arrayPtr);
      arrayPtr = nullptr;
    }
    else if (H5Tequal(dataType, H5T_NATIVE_DOUBLE))
    {
      arrayToReturn = svtkDoubleArray::New();
      arrayToReturn->SetNumberOfTuples(nbTuples);
      double* arrayPtr =
        static_cast<double*>(svtkArrayDownCast<svtkDoubleArray>(arrayToReturn)->GetPointer(0));
      H5Dread(datasetId, dataType, H5S_ALL, H5S_ALL, H5P_DEFAULT, arrayPtr);
      arrayPtr = nullptr;
    }
    else if (H5Tequal(dataType, H5T_NATIVE_INT))
    {
      arrayToReturn = svtkIntArray::New();
      arrayToReturn->SetNumberOfTuples(nbTuples);
      int* arrayPtr =
        static_cast<int*>(svtkArrayDownCast<svtkIntArray>(arrayToReturn)->GetPointer(0));
      H5Dread(datasetId, dataType, H5S_ALL, H5S_ALL, H5P_DEFAULT, arrayPtr);
      arrayPtr = nullptr;
    }
    else if (H5Tequal(dataType, H5T_NATIVE_UINT))
    {
      arrayToReturn = svtkUnsignedIntArray::New();
      arrayToReturn->SetNumberOfTuples(nbTuples);
      unsigned int* arrayPtr = static_cast<unsigned int*>(
        svtkArrayDownCast<svtkUnsignedIntArray>(arrayToReturn)->GetPointer(0));
      H5Dread(datasetId, dataType, H5S_ALL, H5S_ALL, H5P_DEFAULT, arrayPtr);
      arrayPtr = nullptr;
    }
    else if (H5Tequal(dataType, H5T_NATIVE_SHORT))
    {
      arrayToReturn = svtkShortArray::New();
      arrayToReturn->SetNumberOfTuples(nbTuples);
      short* arrayPtr =
        static_cast<short*>(svtkArrayDownCast<svtkShortArray>(arrayToReturn)->GetPointer(0));
      H5Dread(datasetId, dataType, H5S_ALL, H5S_ALL, H5P_DEFAULT, arrayPtr);
      arrayPtr = nullptr;
    }
    else if (H5Tequal(dataType, H5T_NATIVE_USHORT))
    {
      arrayToReturn = svtkUnsignedShortArray::New();
      arrayToReturn->SetNumberOfTuples(nbTuples);
      unsigned short* arrayPtr = static_cast<unsigned short*>(
        svtkArrayDownCast<svtkUnsignedShortArray>(arrayToReturn)->GetPointer(0));
      H5Dread(datasetId, dataType, H5S_ALL, H5S_ALL, H5P_DEFAULT, arrayPtr);
      arrayPtr = nullptr;
    }
    else if (H5Tequal(dataType, H5T_NATIVE_UCHAR))
    {
      ...
      arrayToReturn = svtkUnsignedCharArray::New();
      arrayToReturn->SetNumberOfTuples(nbTuples);
      unsigned char* arrayPtr = static_cast<unsigned char*>(
        svtkArrayDownCast<svtkUnsignedCharArray>(arrayToReturn)->GetPointer(0));
      H5Dread(datasetId, dataType, H5S_ALL, H5S_ALL, H5P_DEFAULT, arrayPtr);
      arrayPtr = nullptr;
    }
    else if (H5Tequal(dataType, H5T_NATIVE_LONG))
    {
      arrayToReturn = svtkLongArray::New();
      arrayToReturn->SetNumberOfTuples(nbTuples);
      long* arrayPtr =
        static_cast<long*>(svtkArrayDownCast<svtkLongArray>(arrayToReturn)->GetPointer(0));
      H5Dread(datasetId, dataType, H5S_ALL, H5S_ALL, H5P_DEFAULT, arrayPtr);
      arrayPtr = nullptr;
    }
    else if (H5Tequal(dataType, H5T_NATIVE_LLONG))
    {
      arrayToReturn = svtkLongLongArray::New();
      arrayToReturn->SetNumberOfTuples(nbTuples);
      long long* arrayPtr =
        static_cast<long long*>(svtkArrayDownCast<svtkLongLongArray>(arrayToReturn)->GetPointer(0));
      H5Dread(datasetId, dataType, H5S_ALL, H5S_ALL, H5P_DEFAULT, arrayPtr);
      arrayPtr = nullptr;
    }
    else
  ...
  svtkDataArray* CreatePinFieldArray(svtkDataArray* dataSource)
  {
    ...
    svtkDataArray* outputField = dataSource->NewInstance();
    outputField->SetNumberOfTuples(this->NASSX * this->NPIN * this->NASSY * this->NPIN * this->NAX);
    svtkIdType srcIndex = 0;

    for (hsize_t sj = 0; sj < this->NASSY; sj++)
    {
      for (hsize_t si = 0; si < this->NASSX; si++)
      {
        svtkIdType assembyId = this->CoreMap->GetTuple1(si * this->NASSX + sj) - 1;
        svtkIdType dstOffset = si * this->NPIN + sj * this->NASSX * this->NPIN * this->NPIN;
    ...
    const char* groupName, hsize_t dimension, std::vector<std::string>& names)
  {
    ...
    if (status < 0)
    {
      svtkErrorWithObjectMacro(this->Owner, "Can't get group info for " << groupName);
      H5Gclose(groupId);
      return;
    }
  ...
  void ReadCore()
  {
    ...
    // Local vars
    svtkDataArray* dataSource;
    svtkDataArray* outputCellArray;
    ...
    for (svtkIdType idx = 0; idx < this->XCoordinates->GetNumberOfTuples(); idx++)
    {
      this->XCoordinates->SetTuple1(idx, ((float)idx) * axialStep);
    }

    this->YCoordinates->SetNumberOfTuples(NASSY * NPIN + 1);
    for (svtkIdType idx = 0; idx < this->YCoordinates->GetNumberOfTuples(); idx++)
    {
    ...
    for (hsize_t sj = 0; sj < this->NASSY; sj++)
    {
      for (hsize_t si = 0; si < this->NASSX; si++)
      {
        svtkIdType srcIdx = si * this->NASSX + sj; // Fortran ordering
        svtkIdType dstOffset = si * this->NPIN + sj * this->NASSX * this->NPIN * this->NPIN;
        for (hsize_t dk = 0; dk < this->NAX; dk++)
        {
          for (hsize_t dj = 0; dj < this->NPIN; dj++)
  ...
  void InitializeWithCoreData(svtkRectilinearGrid* output)
  {
    ...
    for (const svtkSmartPointer<svtkDataArray>& cellArray : this->CoreCellData)
    {
  ...
  void AddStateData(svtkRectilinearGrid* output, svtkIdType timestep)
  {
    if (this->FileId == -1)
    {
      return;
    ...
    {
      svtkErrorWithObjectMacro(this->Owner, "Can't open Group " << stateGroupName.str().c_str());
      return;
    }
    ...
    for (const std::string& dsName : datasetNames)
    {
      ...
      if (dataSource)
      {
        ...
        else
        {
          ...
          svtkDebugWithObjectMacro(this->Owner, << message.str());
        }
        dataSource->Delete();
      }
    }
  ...
  svtkNew<svtkDataArraySelection> CellDataArraySelection;
  svtkNew<svtkDataArraySelection> FieldDataArraySelection;
...
private:
  ...
  hsize_t NASS;
  svtkIdType SYMMETRY;
  svtkIdType NUMBER_OF_STATES;

  svtkNew<svtkFloatArray> XCoordinates;
  svtkNew<svtkFloatArray> YCoordinates;

  svtkObject* Owner;
  svtkSmartPointer<svtkDataArray> ZCoordinates;
  svtkSmartPointer<svtkDataArray> CoreMap;

  std::vector<svtkSmartPointer<svtkDataArray> > CoreCellData;
};
//*****************************************************************************

svtkStandardNewMacro(svtkVeraOutReader);

//----------------------------------------------------------------------------
// Constructor for svtkVeraOutReader
//----------------------------------------------------------------------------
svtkVeraOutReader::svtkVeraOutReader()
{
...
//----------------------------------------------------------------------------
// Destructor for svtkVeraOutReader
//----------------------------------------------------------------------------
svtkVeraOutReader::~svtkVeraOutReader()
{
...
//----------------------------------------------------------------------------
int svtkVeraOutReader::RequestInformation(
  svtkInformation* reqInfo, svtkInformationVector** inVector, svtkInformationVector* outVector)
{
  svtkDebugMacro(<< "In svtkVeraOutReader::RequestInformation" << endl);
  ...
  svtkDebugMacro(<< "In svtkVeraOutReader::RequestInformation read filename okay" << endl);
...
//----------------------------------------------------------------------------
// Default method: Data is read into a svtkUnstructuredGrid
//----------------------------------------------------------------------------
int svtkVeraOutReader::RequestData(svtkInformation* svtkNotUsed(reqInfo),
  svtkInformationVector** svtkNotUsed(inVector), svtkInformationVector* outVector)
{
  ...
  svtkDebugMacro(<< "In svtkVeraOutReader::RequestData" << endl);
  svtkInformation* outInfo = outVector->GetInformationObject(0);
  ...
  svtkIdType requestedTimeStep = 0;
  auto timeKey = svtkStreamingDemandDrivenPipeline::UPDATE_TIME_STEP();
  ...
  svtkDebugMacro(<< "Out svtkVeraOutReader::RequestData" << endl);
...
//  Print self.
//----------------------------------------------------------------------------
void svtkVeraOutReader::PrintSelf(ostream& os, svtkIndent indent)
{
...
svtkDataArraySelection* svtkVeraOutReader::GetCellDataArraySelection() const
{
...
svtkDataArraySelection* svtkVeraOutReader::GetFieldDataArraySelection() const
{
...
svtkMTimeType svtkVeraOutReader::GetMTime()
{
  svtkMTimeType mTime = this->Superclass::GetMTime();
  svtkMTimeType cellMTime = this->Internal->CellDataArraySelection->GetMTime();
  svtkMTimeType fieldMTime = this->Internal->FieldDataArraySelection->GetMTime();
