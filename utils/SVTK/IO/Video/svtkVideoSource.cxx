...
svtkStandardNewMacro(svtkVideoSource);
...
//----------------------------------------------------------------------------
svtkVideoSource::svtkVideoSource()
{
  int i;
  ...
  for (i = 0; i < 3; i++)
  {
    ...
    this->ClipRegion[i * 2 + 1] = SVTK_INT_MAX;
  ...
  this->PlayerThreader = svtkMultiThreader::New();
  this->PlayerThreadId = -1;

  this->FrameBufferMutex = svtkCriticalSection::New();
...
svtkVideoSource::~svtkVideoSource()
{
  ...
  this->svtkVideoSource::ReleaseSystemResources();
...
//----------------------------------------------------------------------------
void svtkVideoSource::PrintSelf(ostream& os, svtkIndent indent)
{
  int idx;
...
void svtkVideoSource::UpdateFrameBuffer()
{
  int i, oldExt;
  int ext[3];
  svtkDataArray* buffer;
  ...
  while (--i >= 0)
  {
    buffer = reinterpret_cast<svtkDataArray*>(this->FrameBuffer[i]);
    if (buffer->GetDataType() != SVTK_UNSIGNED_CHAR || buffer->GetNumberOfComponents() != 1 ||
      buffer->GetNumberOfTuples() != totalSize)
    {
      buffer->Delete();
      buffer = svtkUnsignedCharArray::New();
...
void svtkVideoSource::Initialize()
{
  if (this->Initialized)
  {
    return;
...
void svtkVideoSource::ReleaseSystemResources()
{
...
//----------------------------------------------------------------------------
void svtkVideoSource::SetFrameSize(int x, int y, int z)
{
...
//----------------------------------------------------------------------------
void svtkVideoSource::SetFrameRate(float rate)
{
...
//----------------------------------------------------------------------------
void svtkVideoSource::SetClipRegion(int x0, int x1, int y0, int y1, int z0, int z1)
{
...
// asynchronously.
void svtkVideoSource::InternalGrab()
{
  int i, index;
  ...
  ptr = reinterpret_cast<svtkUnsignedCharArray*>(this->FrameBuffer[index])->GetPointer(0);
  ...
  this->FrameBufferTimeStamps[index] = svtkTimerLog::GetUniversalTime();
...
static int svtkThreadSleep(svtkMultiThreader::ThreadInfo* data, double time)
{
  // loop either until the time has arrived or until the thread is ended
  for (int i = 0;; i++)
  {
    double remaining = time - svtkTimerLog::GetUniversalTime();
    ...
    if (remaining <= 0.0)
    {
      if (i == 0)
      {
        svtkGenericWarningMacro("Dropped a video frame.");
      }
      return 1;
    }
    ...
    svtksys::SystemTools::Delay(static_cast<unsigned int>(remaining * 1000.0));
  }
...
static void* svtkVideoSourceRecordThread(svtkMultiThreader::ThreadInfo* data)
{
  svtkVideoSource* self = (svtkVideoSource*)(data->UserData);
  ...
  do
  {
    self->InternalGrab();
    frame++;
  } while (svtkThreadSleep(data, startTime + frame / rate));
...
void svtkVideoSource::Record()
{
  ...
  if (!this->Recording)
  {
    ...
    this->Modified();
    this->PlayerThreadId =
      this->PlayerThreader->SpawnThread((svtkThreadFunctionType)&svtkVideoSourceRecordThread, this);
  }
}
...
static void* svtkVideoSourcePlayThread(svtkMultiThreader::ThreadInfo* data)
{
  svtkVideoSource* self = (svtkVideoSource*)(data->UserData);
...
void svtkVideoSource::Play()
{
  ...
  if (!this->Playing)
  {
    ...
    this->Modified();
    this->PlayerThreadId =
      this->PlayerThreader->SpawnThread((svtkThreadFunctionType)&svtkVideoSourcePlayThread, this);
  }
}
...
void svtkVideoSource::Rewind()
{
  ...
  {
    svtkWarningMacro("Rewind: bogus time stamp!");
  }
  else
  {
...
void svtkVideoSource::FastForward()
{
  this->FrameBufferMutex->Lock();
  ...
  if (stamp[j] != 0.0 && stamp[j] < 980000000.0)
  {
    svtkWarningMacro("FastForward: bogus time stamp!");
  }
  else
  {
    this->AdvanceFrameBuffer(i);
...
//----------------------------------------------------------------------------
// Rotate the buffers
void svtkVideoSource::Seek(int n)
{
  this->FrameBufferMutex->Lock();
...
// or 4 for RGBA)
void svtkVideoSource::SetOutputFormat(int format)
{
...
// to be device-specific (i.e. something other than svtkDataArray)
void svtkVideoSource::SetFrameBufferSize(int bufsize)
{
  int i;
  void** framebuffer;
  double* timestamps;
  ...
  if (bufsize < 0)
  {
    svtkErrorMacro(<< "SetFrameBufferSize: There must be at least one framebuffer");
  }
  ...
  if (this->FrameBuffer == nullptr)
  {
    if (bufsize > 0)
    {
      ...
      for (i = 0; i < bufsize; i++)
      {
        this->FrameBuffer[i] = svtkUnsignedCharArray::New();
  ...
  else
  {
    ...
    {
      framebuffer[i] = svtkUnsignedCharArray::New();
      timestamps[i] = 0.0;
    }
    ...
    {
      reinterpret_cast<svtkDataArray*>(this->FrameBuffer[i])->Delete();
    }
...
void svtkVideoSource::AdvanceFrameBuffer(int n)
{
...
//----------------------------------------------------------------------------
double svtkVideoSource::GetFrameTimeStamp(int frame)
{
  double timeStamp;
...
int svtkVideoSource::RequestInformation(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** svtkNotUsed(inputVector), svtkInformationVector* outputVector)
{
  ...
  svtkDataObject::SetPointDataActiveScalarInfo(
    outInfo, SVTK_UNSIGNED_CHAR, this->NumberOfScalarComponents);
...
void svtkVideoSource::UnpackRasterLine(char* outPtr, char* rowPtr, int start, int count)
{
...
// than 8-bit svtkUnsignedCharArray for the frame buffer.
int svtkVideoSource::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** svtkNotUsed(inputVector), svtkInformationVector* outputVector)
{
  svtkInformation* outInfo = outputVector->GetInformationObject(0);
  svtkImageData* data = this->AllocateOutputData(svtkImageData::GetData(outInfo), outInfo);
  int i, j;
  ...
  for (frame = firstFrame; frame <= finalFrame; frame++)
  {
    ...
    svtkDataArray* frameBuffer =
      reinterpret_cast<svtkDataArray*>(this->FrameBuffer[(index + frame) % this->FrameBufferSize]);
