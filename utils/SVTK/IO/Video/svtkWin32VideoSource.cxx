/*=========================================================================
  ...
  Module:    svtkWin32VideoSource.cxx
...
=========================================================================*/
#include "svtkWin32VideoSource.h"
...
class svtkWin32VideoSourceInternal
{
public:
  svtkWin32VideoSourceInternal() {}
  HWND CapWnd;
  HWND ParentWnd;
  CAPSTATUS CapStatus;
...
#define SVTK_BI_UYVY 0x59565955

svtkStandardNewMacro(svtkWin32VideoSource);

//----------------------------------------------------------------------------
svtkWin32VideoSource::svtkWin32VideoSource()
{
  this->Internal = new svtkWin32VideoSourceInternal;
  this->Initialized = 0;
...
svtkWin32VideoSource::~svtkWin32VideoSource()
{
  this->svtkWin32VideoSource::ReleaseSystemResources();
...
//----------------------------------------------------------------------------
void svtkWin32VideoSource::PrintSelf(ostream& os, svtkIndent indent)
{
...
LONG FAR PASCAL svtkWin32VideoSourceWinProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
  svtkWin32VideoSource* self = (svtkWin32VideoSource*)(svtkGetWindowLong(hwnd, svtkGWL_USERDATA));
...
//----------------------------------------------------------------------------
LRESULT PASCAL svtkWin32VideoSourceCapControlProc(HWND hwndC, int nState)
{
  svtkWin32VideoSource* self = (svtkWin32VideoSource*)(capGetUserData(hwndC));

  if (nState == CONTROLCALLBACK_PREROLL)
  {
    // cerr << "controlcallback preroll\n";
    self->SetStartTimeStamp(svtkTimerLog::GetUniversalTime());
  }
...
//----------------------------------------------------------------------------
LRESULT PASCAL svtkWin32VideoSourceCallbackProc(HWND hwndC, LPVIDEOHDR lpVHdr)
{
  svtkWin32VideoSource* self = (svtkWin32VideoSource*)(capGetUserData(hwndC));
...
LRESULT PASCAL svtkWin32VideoSourceStatusCallbackProc(
  HWND svtkNotUsed(hwndC), int nID, LPCSTR svtkNotUsed(lpsz))
{
  // svtkWin32VideoSource *self = (svtkWin32VideoSource *)(capGetUserData(hwndC));
...
//----------------------------------------------------------------------------
LRESULT PASCAL svtkWin32VideoSourceErrorCallbackProc(HWND hwndC, int ErrID, LPSTR lpErrorText)
{
  if (ErrID)
  {
    char buff[84];
    ...
    // svtkGenericWarningMacro(<< buff << ' ' << lpErrorText);
  }
  return 1;
}

//----------------------------------------------------------------------------
void svtkWin32VideoSource::Initialize()
{
  int i;
  ...
  strcpy(this->WndClassName, "SVTKVideo");

  // set up a class for the main window
  WNDCLASS wc;
  wc.lpszClassName = this->WndClassName;
  wc.hInstance = hinstance;
  wc.lpfnWndProc = reinterpret_cast<WNDPROC>(&svtkWin32VideoSourceWinProc);
  ...
  for (i = 1; i <= 10; i++)
  {
    ...
    snprintf(this->WndClassName, 16, "SVTKVideo %d", i);
  }

  if (i > 10)
  {
    svtkErrorMacro(<< "Initialize: failed to register SVTKVideo class"
                  << " (" << GetLastError() << ")");
    return;
  }
  ...
  if (!result)
  {
    svtkWarningMacro("Initialize: AdjustWindowRect failed, error: " << GetLastError());
  }

  this->Internal->ParentWnd = CreateWindow(this->WndClassName, "SVTK Video Window", style, 0, 0,
    r.right - r.left, r.bottom - r.top, nullptr, nullptr, hinstance, nullptr);

  if (!this->Internal->ParentWnd)
  {
    svtkErrorMacro(<< "Initialize: failed to create window"
                  << " (" << GetLastError() << ")");
    return;
  }

  // set the user data to 'this'
  svtkSetWindowLong(this->Internal->ParentWnd, svtkGWL_USERDATA, (svtkLONG)this);

  // Create the capture window
  this->Internal->CapWnd = capCreateCaptureWindow("Capture", WS_CHILD | WS_VISIBLE, 0, 0,
    this->FrameSize[0], this->FrameSize[1], this->Internal->ParentWnd, 1);

  if (!this->Internal->CapWnd)
  {
    svtkErrorMacro(<< "Initialize: failed to create capture window"
                  << " (" << GetLastError() << ")");
  ...
  {
    svtkErrorMacro(<< "Initialize: couldn't connect to driver"
                  << " (" << GetLastError() << ")");
    this->ReleaseSystemResources();
    return;
  }
  ...
  if (!capCaptureSetSetup(
        this->Internal->CapWnd, &this->Internal->CaptureParms, sizeof(CAPTUREPARMS)))
  {
    svtkErrorMacro(<< "Initialize: setup of capture parameters failed"
                  << " (" << GetLastError() << ")");
    this->ReleaseSystemResources();
    return;
  }
  ...
  {
    svtkErrorMacro(<< "Initialize: couldn't set user data for callback"
                  << " (" << GetLastError() << ")");
    this->ReleaseSystemResources();
    return;
  }
  ...
  if (!capSetCallbackOnCapControl(this->Internal->CapWnd, &svtkWin32VideoSourceCapControlProc))
  {
    svtkErrorMacro(<< "Initialize: couldn't set control callback"
                  << " (" << GetLastError() << ")");
    this->ReleaseSystemResources();
    return;
  }
  ...
  if (!capSetCallbackOnFrame(this->Internal->CapWnd, &svtkWin32VideoSourceCallbackProc))
  {
    svtkErrorMacro(<< "Initialize: couldn't set frame callback"
                  << " (" << GetLastError() << ")");
    this->ReleaseSystemResources();
    return;
  }
  // install the callback to copy frames into the buffer on stream grabs
  if (!capSetCallbackOnVideoStream(this->Internal->CapWnd, &svtkWin32VideoSourceCallbackProc))
  {
    svtkErrorMacro(<< "Initialize: couldn't set stream callback"
                  << " (" << GetLastError() << ")");
    this->ReleaseSystemResources();
    return;
  }
  // install the callback to get info on start/end of streaming
  if (!capSetCallbackOnStatus(this->Internal->CapWnd, &svtkWin32VideoSourceStatusCallbackProc))
  {
    svtkErrorMacro(<< "Initialize: couldn't set status callback"
                  << " (" << GetLastError() << ")");
    this->ReleaseSystemResources();
    return;
  }
  // install the callback to send messages to user
  if (!capSetCallbackOnError(this->Internal->CapWnd, &svtkWin32VideoSourceErrorCallbackProc))
  {
    svtkErrorMacro(<< "Initialize: couldn't set error callback"
                  << " (" << GetLastError() << ")");
    this->ReleaseSystemResources();
    return;
  }
...
//----------------------------------------------------------------------------
void svtkWin32VideoSource::SetPreview(int p)
{
  if (this->Preview == p)
  {
    return;
...
//----------------------------------------------------------------------------
void svtkWin32VideoSource::ReleaseSystemResources()
{
...
//----------------------------------------------------------------------------
void svtkWin32VideoSource::OnParentWndDestroy()
{
  ...
  if (this->Internal->CapWnd)
  {
    // MessageBox(this->Internal->ParentWnd, "capDriverDisconnect(this->Internal->CapWnd)", "",
    // MB_OK | MB_ICONEXCLAMATION);
    capDriverDisconnect(this->Internal->CapWnd);
    // MessageBox(this->Internal->ParentWnd, "DestroyWindow(this->Internal->CapWnd)", "", MB_OK |
    // MB_ICONEXCLAMATION);
...
// svtkVideoSource framebuffer (don't do the unpacking yet)
void svtkWin32VideoSource::LocalInternalGrab(void* lpptr)
{
  ...
  this->FrameCount++;
  this->FrameBufferTimeStamps[index] = this->StartTimeStamp + 0.001 * lpVHdr->dwTimeCaptured;

  unsigned char* ptr =
    (unsigned char*)((reinterpret_cast<svtkUnsignedCharArray*>(this->FrameBuffer[index]))
                       ->GetPointer(0));
  ...
  int inBytesPerRow = this->FrameSize[0] * (this->Internal->BitMapPtr->bmiHeader.biBitCount / 8);
  outBytesPerRow += outBytesPerRow % 4;
  int rows = this->FrameBufferExtent[3] - this->FrameBufferExtent[2] + 1;

  cptrDIB += this->FrameBufferExtent[0] * (this->Internal->BitMapPtr->bmiHeader.biBitCount / 8);
...
//----------------------------------------------------------------------------
void svtkWin32VideoSource::Grab()
{
  if (this->Recording)
  {
    return;
  ...
  this->SetStartTimeStamp(svtkTimerLog::GetUniversalTime());
  capGrabFrameNoStop(this->Internal->CapWnd);
}

//----------------------------------------------------------------------------
void svtkWin32VideoSource::Record()
{
  this->Initialize();
  if (!this->Initialized)
  {
...
//----------------------------------------------------------------------------
void svtkWin32VideoSource::Play()
{
  this->svtkVideoSource::Play();
}

//----------------------------------------------------------------------------
void svtkWin32VideoSource::Stop()
{
  if (this->Recording)
  {
    this->Recording = 0;
...
static inline void svtkYUVToRGB(unsigned char* yuv, unsigned char* rgb)
{
  /* // floating point
  int Y = yuv[0] - 16;
  int U = yuv[1] - 128;
...
//----------------------------------------------------------------------------
void svtkWin32VideoSource::UnpackRasterLine(char* outptr, char* inptr, int start, int count)
{
        ...
        switch (this->OutputFormat)
        {
          ...
          { // unpack UYVY megapixel to two RGB or RGBA pixels
            ...
            while (--count >= 0)
            {
              ...
              YUV[0] = *inptr++;
              odd = !odd;
              svtkYUVToRGB(YUV, (unsigned char*)outptr);
              outptr += 3;
              if (this->OutputFormat == SVTK_RGB)
              {
                continue;
              }
              *outptr++ = alpha;
...
//----------------------------------------------------------------------------
void svtkWin32VideoSource::VideoFormatDialog()
{
  this->Initialize();
  if (!this->Initialized)
  {
...
//----------------------------------------------------------------------------
void svtkWin32VideoSource::VideoSourceDialog()
{
  this->Initialize();
  if (!this->Initialized)
  {
...
void svtkWin32VideoSource::SetFrameSize(int x, int y, int z)
{
...
//----------------------------------------------------------------------------
void svtkWin32VideoSource::SetFrameRate(float rate)
{
...
//----------------------------------------------------------------------------
void svtkWin32VideoSource::SetOutputFormat(int format)
{
...
//----------------------------------------------------------------------------
// check the current video format and set up the SVTK video framebuffer to match
void svtkWin32VideoSource::DoVFWFormatCheck()
{
  ...
  else
  {
    ...
    fourcc[4] = '\0';
    svtkWarningMacro(<< "DoVFWFormatCheck: video compression mode " << fourcchex << " \"" << fourcc
                    << "\": can't grab");
  }
  ...
  if (bpp != this->FrameBufferBitsPerPixel)
  {
    switch (bpp)
    {
      ...
      case 4:
      case 8:
        this->OutputFormat = SVTK_LUMINANCE;
      ...
      case 24:
      case 32:
        if (this->OutputFormat != SVTK_RGBA)
        {
          this->OutputFormat = SVTK_RGB;
          this->NumberOfScalarComponents = 3;
        }
  ...
  if (bpp != this->FrameBufferBitsPerPixel || this->FrameSize[0] != width ||
...
//----------------------------------------------------------------------------
void svtkWin32VideoSource::DoVFWFormatSetup()
{
  ...
  for (i = 0; i < 3; i++)
  { // try for a
    if (this->OutputFormat == SVTK_RGBA || this->OutputFormat == SVTK_RGB)
    {
    ...
    if (capSetVideoFormat(
          this->Internal->CapWnd, this->Internal->BitMapPtr, sizeof(BITMAPINFOHEADER)))
    {
      break;
    }
  }
  if (i > 4)
  {
    svtkWarningMacro(<< "DoVFWFormatSetup: invalid video format for device"
                    << " (" << GetLastError() << ")");
  }
  this->DoVFWFormatCheck();
}
