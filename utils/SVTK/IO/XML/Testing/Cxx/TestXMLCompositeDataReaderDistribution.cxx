...
std::multiset<svtkIdType> PointCounts(svtkCompositeDataSet* cds)
{
  std::multiset<svtkIdType> result;

  using SmartIterator = svtkSmartPointer<svtkCompositeDataIterator>;
  auto it = SmartIterator::Take(cds->NewIterator());
  it->SkipEmptyNodesOn();
  for (it->InitTraversal(); !it->IsDoneWithTraversal(); it->GoToNextItem())
  {
    svtkPolyData* pd = svtkPolyData::SafeDownCast(it->GetCurrentDataObject());
    if (pd)
    {
...
bool VerifyCounts(svtkCompositeDataSet* cds, const std::multiset<svtkIdType>& expected)
{
  return PointCounts(cds) == expected;
}

std::string DumpCounts(svtkCompositeDataSet* cds)
{
  const std::multiset<svtkIdType> ids = PointCounts(cds);
...
int TestXMLCompositeDataReaderDistribution(int argc, char* argv[])
{
  ...
  svtkNew<svtkXMLMultiBlockDataReader> in;
  ...
  svtkNew<svtkInformation> req;
  req->Set(svtkCompositeDataPipeline::UPDATE_NUMBER_OF_PIECES(), 3);

  // Verify that block loading works as expected.
  {
    in->SetPieceDistribution(svtkXMLMultiBlockDataReader::Block);

    // First processor:
    req->Set(svtkCompositeDataPipeline::UPDATE_PIECE_NUMBER(), 0);
    in->Update(req);
    TEST_ASSERT(VerifyCounts(in->GetOutput(), { 1, 2, 3, 4 }),
      "Incorrect partitioning: " << DumpCounts(in->GetOutput()));

    // Second processor:
    req->Set(svtkCompositeDataPipeline::UPDATE_PIECE_NUMBER(), 1);
    in->Update(req);
    TEST_ASSERT(VerifyCounts(in->GetOutput(), { 5, 6, 7 }),
      "Incorrect partitioning: " << DumpCounts(in->GetOutput()));

    // Third processor:
    req->Set(svtkCompositeDataPipeline::UPDATE_PIECE_NUMBER(), 2);
    in->Update(req);
  ...
  {
    in->SetPieceDistribution(svtkXMLMultiBlockDataReader::Interleave);
  ...
  req->Append(svtkCompositeDataPipeline::UPDATE_COMPOSITE_INDICES(), 0);
  req->Append(svtkCompositeDataPipeline::UPDATE_COMPOSITE_INDICES(), 2);
  req->Append(svtkCompositeDataPipeline::UPDATE_COMPOSITE_INDICES(), 3);
  req->Append(svtkCompositeDataPipeline::UPDATE_COMPOSITE_INDICES(), 4);
  req->Append(svtkCompositeDataPipeline::UPDATE_COMPOSITE_INDICES(), 5);
  req->Append(svtkCompositeDataPipeline::UPDATE_COMPOSITE_INDICES(), 7);
  req->Append(svtkCompositeDataPipeline::UPDATE_COMPOSITE_INDICES(), 8);
  req->Append(svtkCompositeDataPipeline::UPDATE_COMPOSITE_INDICES(), 9);
