...
int TestXMLHyperTreeGridIO(int argc, char* argv[])
{
  ...
  svtkNew<svtkXMLHyperTreeGridReader> reader;
  reader->SetFileName(ifname.c_str());
  reader->Update();

  svtkHyperTreeGrid* read_in = reader->GetOutput();
  int size[3];
  svtkDataArray* coords;
  int ntp;
  ...
  svtksys::SystemTools::MakeDirectory(output_dir);

  std::string ofname = output_dir + "/" + fname;

  cout << "- WRITE BINARY --------------------------------" << endl;

  svtkNew<svtkXMLHyperTreeGridWriter> writer;
  writer->SetFileName(ofname.c_str());
  writer->SetInputData(read_in);
  writer->SetDataModeToBinary();
  writer->Write();

  svtkNew<svtkXMLHyperTreeGridReader> reader2;
  reader2->SetFileName(ofname.c_str());
  reader2->Update();
  svtkHyperTreeGrid* wrote_out = reader2->GetOutput();
