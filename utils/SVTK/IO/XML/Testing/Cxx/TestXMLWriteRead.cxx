...
int TestConvertType(const std::string& type, const std::string& fileName)
{
  ...
  if (type == "float")
  {
    source->SetOutputPointsPrecision(svtkAlgorithm::SINGLE_PRECISION);
  }
  else
  {
  ...
  svtkPoints* originalPoints = source->GetOutput()->GetPoints();
  svtkPoints* readPoints = reader->GetOutput()->GetPoints();
