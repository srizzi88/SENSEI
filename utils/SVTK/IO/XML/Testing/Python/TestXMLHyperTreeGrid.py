...
#Scalar Level
levelArray = svtk.svtkUnsignedCharArray()
...
xValues = svtk.svtkDoubleArray() #x
...
yValues = svtk.svtkDoubleArray() #y
...
zValues = svtk.svtkDoubleArray() #z
zValues.SetNumberOfValues(1)
zValues.SetValue(0, 0)
htg.SetZCoordinates(zValues)

#Create a cursor
geoCursor = svtk.svtkHyperTreeGridNonOrientedGeometryCursor()
...
#(inheriting the vta.SVTKAlgorithm). Indeed, the latter does not
#propose by the method SetInputData !
#print('With Depth Limiter Filter (HTG)')
depth = svtk.svtkHyperTreeGridDepthLimiter()
...
class MyAlgorithm(vta.SVTKAlgorithm):
  ...
  def FillInputPortInformation(self, svtkself, port, info):
    info.Set(svtk.svtkAlgorithm.INPUT_REQUIRED_DATA_TYPE(), "svtkHyperTreeGrid")
    return 1

  def FillOutputPortInformation(self, svtkself, port, info):
    info.Set(svtk.svtkDataObject.DATA_TYPE_NAME(), "svtkHyperTreeGrid")
    return 1
...
writer = svtk.svtkXMLHyperTreeGridWriter()
...
#
#print('Read')
ex = svtk.svtkXMLHyperTreeGridReader()
...
gc = svtk.svtkHyperTreeGridGhostCells()
...
if False:
  ...
  # not exist w2if.SetMagnification(3) # set the resolution of the output image (3 times the current resolution of svtk render window)
