...
class TestXMLPartitionedDataSetCollection(Testing.svtkTest):
    ...
    def test(self):
        ...
        w = ixml.svtkXMLPartitionedDataSetCollectionWriter()
        w.SetInputData(c)
        w.SetFileName(fname)
        w.Write()

        r = ixml.svtkXMLPartitionedDataSetCollectionReader()
        r.SetFileName(fname)
        r.Update()
