/*=========================================================================
  ...
  Module:    svtkXMLOffsetsManager.h
 ...
 *
 * This class is deisgned to work within svtkXMLWriter. It store a position
...
#ifndef svtkXMLOffsetsManager_DoNotInclude
#error "do not include unless you know what you are doing"
#endif

#ifndef svtkXMLOffsetsManager_h
#define svtkXMLOffsetsManager_h
...
{
public:
  // Construct with default (svtkMTimeType)-1  MTime
  OffsetsManager()
  {
    this->LastMTime = static_cast<svtkMTimeType>(-1); // almost invalid state
  }
  ...
  svtkTypeInt64& GetPosition(unsigned int t)
  {
    assert(t < this->Positions.size());
    return this->Positions[t];
  }
  svtkTypeInt64& GetRangeMinPosition(unsigned int t)
  {
    assert(t < this->RangeMinPositions.size());
    return this->RangeMinPositions[t];
  }
  svtkTypeInt64& GetRangeMaxPosition(unsigned int t)
  {
    assert(t < this->RangeMaxPositions.size());
    return this->RangeMaxPositions[t];
  }
  svtkTypeInt64& GetOffsetValue(unsigned int t)
  {
    assert(t < this->OffsetValues.size());
    return this->OffsetValues[t];
  }
  svtkMTimeType& GetLastMTime() { return this->LastMTime; }

private:
  svtkMTimeType LastMTime; // Previously written dataarray mtime
  ...
  std::vector<svtkTypeInt64> Positions;
  std::vector<svtkTypeInt64> RangeMinPositions; // Where is this
  std::vector<svtkTypeInt64> RangeMaxPositions; // Whee is this

  std::vector<svtkTypeInt64> OffsetValues; // Value of offset
};
...
#endif
// SVTK-HeaderTest-Exclude: svtkXMLOffsetsManager.h
