/*=========================================================================
  ...
  Module:    svtkXMLPHyperTreeGridReader.cxx
...
=========================================================================*/
#include "svtkXMLPHyperTreeGridReader.h"

#include "svtkCallbackCommand.h"
#include "svtkHyperTree.h"
#include "svtkHyperTreeCursor.h"
...
svtkStandardNewMacro(svtkXMLPHyperTreeGridReader);

//----------------------------------------------------------------------------
svtkXMLPHyperTreeGridReader::svtkXMLPHyperTreeGridReader()
{
...
//----------------------------------------------------------------------------
void svtkXMLPHyperTreeGridReader::CopyOutputInformation(svtkInformation* outInfo, int port)
{
...
//----------------------------------------------------------------------------
void svtkXMLPHyperTreeGridReader::PrintSelf(ostream& os, svtkIndent indent)
{
  this->Superclass::PrintSelf(os, indent);
}

//----------------------------------------------------------------------------
svtkHyperTreeGrid* svtkXMLPHyperTreeGridReader::GetOutput()
{
  return this->GetOutput(0);
}

//----------------------------------------------------------------------------
svtkHyperTreeGrid* svtkXMLPHyperTreeGridReader::GetOutput(int idx)
{
  return svtkHyperTreeGrid::SafeDownCast(this->GetOutputDataObject(idx));
}

//----------------------------------------------------------------------------
const char* svtkXMLPHyperTreeGridReader::GetDataSetName()
{
  return "PHyperTreeGrid";
}

//----------------------------------------------------------------------------
void svtkXMLPHyperTreeGridReader::GetOutputUpdateExtent(int& piece, int& numberOfPieces)
{
...
//----------------------------------------------------------------------------
void svtkXMLPHyperTreeGridReader::SetupOutputTotals()
{
  this->TotalNumberOfPoints = 0;
...
//----------------------------------------------------------------------------
void svtkXMLPHyperTreeGridReader::SetupOutputData()
{
...
//----------------------------------------------------------------------------
int svtkXMLPHyperTreeGridReader::ReadPieceData(int index)
{
  this->Piece = index;
...
//----------------------------------------------------------------------------
int svtkXMLPHyperTreeGridReader::CanReadPiece(int index)
{
  // If necessary, test whether the piece can be read.
  svtkXMLHyperTreeGridReader* reader = this->PieceReaders[index];
...
//----------------------------------------------------------------------------
void svtkXMLPHyperTreeGridReader::PieceProgressCallback()
{
...
//----------------------------------------------------------------------------
void svtkXMLPHyperTreeGridReader::SetupNextPiece() {}
...
template <class T>
void svtkXMLPHyperTreeGridAppendExecute(T* inPtr, T* outPtr, svtkIdType numTuple, svtkIdType numComp)
{
  for (svtkIdType nt = 0; nt < numTuple; nt++)
  {
    for (svtkIdType nc = 0; nc < numComp; nc++)
    {
...
//----------------------------------------------------------------------------
int svtkXMLPHyperTreeGridReader::ReadPieceData()
{
  ...
  svtkHyperTreeGrid* input = this->GetPieceInputAsHyperTreeGrid(this->Piece);
  svtkHyperTreeGrid* output = svtkHyperTreeGrid::SafeDownCast(this->GetCurrentOutput());

  if (!output)
  {
    svtkErrorMacro("Incorrect type of output: " << output->GetClassName());
    return 0;
  }
  ...
  while (it.GetNextTree(inIndex))
  {
    ...
    svtkIdType numberOfVertices = inCursor->GetTree()->GetNumberOfVertices();
  ...
  {
    svtkAbstractArray* inArray = input->GetPointData()->GetAbstractArray(i);
    svtkAbstractArray* outArray = output->GetPointData()->GetAbstractArray(i);
    if (outArray == nullptr)
    {
    ...
    svtkIdType numComp = inArray->GetNumberOfComponents();
    svtkIdType numTuple = inArray->GetNumberOfTuples();
    ...
    {
      svtkErrorMacro(<< "Execute: input" << this->Piece << " ScalarType (" << inArray->GetDataType()
    ...
    {
      svtkErrorMacro(<< "Execute: input" << this->Piece << " Name (" << inArray->GetName()
    ...
    {
      svtkTemplateMacro(svtkXMLPHyperTreeGridAppendExecute(
        static_cast<SVTK_TT*>(inPtr), static_cast<SVTK_TT*>(outPtr), numTuple, numComp));
      default:
...
//----------------------------------------------------------------------------
svtkXMLHyperTreeGridReader* svtkXMLPHyperTreeGridReader::CreatePieceReader()
{
  return svtkXMLHyperTreeGridReader::New();
}

//----------------------------------------------------------------------------
int svtkXMLPHyperTreeGridReader::FillOutputPortInformation(int, svtkInformation* info)
{
...
//----------------------------------------------------------------------------
int svtkXMLPHyperTreeGridReader::RequestInformation(
...
//----------------------------------------------------------------------------
svtkHyperTreeGrid* svtkXMLPHyperTreeGridReader::GetOutputAsHyperTreeGrid()
{
  return svtkHyperTreeGrid::SafeDownCast(this->GetOutputDataObject(0));
}

//----------------------------------------------------------------------------
svtkHyperTreeGrid* svtkXMLPHyperTreeGridReader::GetPieceInputAsHyperTreeGrid(int piece)
{
  svtkXMLHyperTreeGridReader* reader = this->PieceReaders[piece];
  if (!reader || reader->GetNumberOfOutputPorts() < 1)
  {
    return nullptr;
  }
  return static_cast<svtkHyperTreeGrid*>(reader->GetExecutive()->GetOutputData(0));
}

//----------------------------------------------------------------------------
svtkIdType svtkXMLPHyperTreeGridReader::GetNumberOfPoints()
{
  return this->TotalNumberOfPoints;
}

//----------------------------------------------------------------------------
void svtkXMLPHyperTreeGridReader::SetupEmptyOutput()
{
  this->GetCurrentOutput()->Initialize();
}

//----------------------------------------------------------------------------
void svtkXMLPHyperTreeGridReader::SetupOutputInformation(svtkInformation* svtkNotUsed(outInfo))
{
...
//----------------------------------------------------------------------------
void svtkXMLPHyperTreeGridReader::ReadXMLData()
{
...
//----------------------------------------------------------------------------
int svtkXMLPHyperTreeGridReader::ReadPrimaryElement(svtkXMLDataElement* ePrimary)
{
...
//----------------------------------------------------------------------------
void svtkXMLPHyperTreeGridReader::SetupUpdateExtent(int piece, int numberOfPieces)
{
  ...
  for (int i = this->StartPiece; i < this->EndPiece; ++i)
  {
    if (this->CanReadPiece(i))
    {
      ...
      svtkXMLHyperTreeGridReader* pReader = this->PieceReaders[i];
...
//----------------------------------------------------------------------------
svtkIdType svtkXMLPHyperTreeGridReader::GetNumberOfPointsInPiece(int piece)
{
  return this->PieceReaders[piece] ? this->PieceReaders[piece]->GetNumberOfPoints() : 0;
}

//----------------------------------------------------------------------------
void svtkXMLPHyperTreeGridReader::SetupPieces(int numPieces)
{
  this->Superclass::SetupPieces(numPieces);

  this->PieceReaders = new svtkXMLHyperTreeGridReader*[this->NumberOfPieces];
...
//----------------------------------------------------------------------------
void svtkXMLPHyperTreeGridReader::DestroyPieces()
{
  for (int i = 0; i < this->NumberOfPieces; ++i)
  {
    if (this->PieceReaders[i])
...
//----------------------------------------------------------------------------
int svtkXMLPHyperTreeGridReader::ReadPiece(svtkXMLDataElement* ePiece)
{
  ...
  svtkXMLHyperTreeGridReader* reader = this->CreatePieceReader();
...
//-----------------------------------------------------------------------------
void svtkXMLPHyperTreeGridReader::RecursivelyProcessTree(
  svtkHyperTreeGridNonOrientedCursor* inCursor, svtkHyperTreeGridNonOrientedCursor* outCursor)
{
  ...
  if (!inCursor->IsLeaf())
  {
    ...
    for (int child = 0; child < numChildren; ++child)
    {
      ...
      // PKF svtkHyperTreeGridCursor* childCursor = inCursor->Clone();
