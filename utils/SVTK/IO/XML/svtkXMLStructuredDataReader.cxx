/*=========================================================================
  ...
  Module:    svtkXMLStructuredDataReader.cxx
...
//----------------------------------------------------------------------------
svtkXMLStructuredDataReader::svtkXMLStructuredDataReader()
{
...
//----------------------------------------------------------------------------
void svtkXMLStructuredDataReader::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
int svtkXMLStructuredDataReader::ReadPrimaryElement(svtkXMLDataElement* ePrimary)
{
...
//----------------------------------------------------------------------------
void svtkXMLStructuredDataReader::SetupOutputInformation(svtkInformation* outInfo)
{
...
//----------------------------------------------------------------------------
void svtkXMLStructuredDataReader::CopyOutputInformation(svtkInformation* outInfo, int port)
{
...
//----------------------------------------------------------------------------
void svtkXMLStructuredDataReader::SetupEmptyOutput()
{
  this->GetCurrentOutput()->Initialize();
}

//----------------------------------------------------------------------------
void svtkXMLStructuredDataReader::SetupPieces(int numPieces)
{
  this->Superclass::SetupPieces(numPieces);
  this->PieceExtents = new int[numPieces * 6];
  this->PiecePointDimensions = new int[numPieces * 3];
  this->PiecePointIncrements = new svtkIdType[numPieces * 3];
  this->PieceCellDimensions = new int[numPieces * 3];
  this->PieceCellIncrements = new svtkIdType[numPieces * 3];
...
//----------------------------------------------------------------------------
void svtkXMLStructuredDataReader::DestroyPieces()
{
...
//----------------------------------------------------------------------------
svtkIdType svtkXMLStructuredDataReader::GetNumberOfPoints()
{
...
//----------------------------------------------------------------------------
svtkIdType svtkXMLStructuredDataReader::GetNumberOfCells()
{
...
//----------------------------------------------------------------------------
int svtkXMLStructuredDataReader::ReadPiece(svtkXMLDataElement* ePiece)
{
  ...
  if (strcmp(ePiece->GetName(), "Piece") == 0)
  {
    ...
    {
      svtkErrorMacro("Piece has no extent.");
    }
    if (ePiece->GetVectorAttribute("Extent", 6, pieceExtent) < 6)
    {
      svtkErrorMacro("Extent attribute is not 6 integers.");
      return 0;
    }
  }
  else if (ePiece->GetVectorAttribute("WholeExtent", 6, pieceExtent) < 6)
  {
    svtkErrorMacro("WholeExtent attribute is not 6 integers.");
    return 0;
  }
  ...
  svtkIdType* piecePointIncrements = this->PiecePointIncrements + this->Piece * 3;
  int* pieceCellDimensions = this->PieceCellDimensions + this->Piece * 3;
  svtkIdType* pieceCellIncrements = this->PieceCellIncrements + this->Piece * 3;
...
//----------------------------------------------------------------------------
void svtkXMLStructuredDataReader::ReadXMLData()
{
  ...
  int numPieces = outInfo->Get(
    svtkStreamingDemandDrivenPipeline::UPDATE_NUMBER_OF_PIECES());
  int piece = outInfo->Get(
    svtkStreamingDemandDrivenPipeline::UPDATE_PIECE_NUMBER());
...
//----------------------------------------------------------------------------
int svtkXMLStructuredDataReader::ReadArrayForPoints(
...
//----------------------------------------------------------------------------
int svtkXMLStructuredDataReader::ReadArrayForCells(svtkXMLDataElement* da, svtkAbstractArray* outArray)
{
  int* pieceExtent = this->PieceExtents + this->Piece * 6;
  int* pieceCellDimensions = this->PieceCellDimensions + this->Piece * 3;
  svtkIdType* pieceCellIncrements = this->PieceCellIncrements + this->Piece * 3;
...
void svtkXMLStructuredDataReaderSubExtentCopyValues(
  iterT* destIter, svtkIdType destIndex, iterT* srcIter, svtkIdType srcIndex, svtkIdType numValues)
{
  // for all contiguous fixed-size arrays ie. svtkDataArray.
  memcpy(destIter->GetArray()->GetVoidPointer(destIndex),
    srcIter->GetArray()->GetVoidPointer(srcIndex), numValues);
}
//----------------------------------------------------------------------------
template <>
void svtkXMLStructuredDataReaderSubExtentCopyValues(svtkArrayIteratorTemplate<svtkStdString>* destIter,
  svtkIdType destIndex, svtkArrayIteratorTemplate<svtkStdString>* srcIter, svtkIdType srcIndex,
  svtkIdType numValues)
{
  svtkIdType maxIndex = destIndex + numValues;
  for (svtkIdType cc = destIndex; cc < maxIndex; ++cc)
  {
...
//----------------------------------------------------------------------------
int svtkXMLStructuredDataReader::ReadSubExtent(int* inExtent, int* inDimensions,
  svtkIdType* inIncrements, int* outExtent, int* outDimensions, svtkIdType* outIncrements,
  int* subExtent, int* subDimensions, svtkXMLDataElement* da, svtkAbstractArray* array,
  FieldType fieldType)
{
  ...
  else
  {
    ...
    else
    {
      ...
      this->GetProgressRange(progressRange);
      svtkIdType rowTuples = subDimensions[0];
      svtkIdType partialSliceTuples =
        static_cast<svtkIdType>(inDimensions[0]) * static_cast<svtkIdType>(subDimensions[1]);
      int tupleSize = components * array->GetDataTypeSize();
      svtkAbstractArray* temp = array->NewInstance();
      temp->SetNumberOfComponents(array->GetNumberOfComponents());
      temp->SetNumberOfTuples(partialSliceTuples);
      svtkArrayIterator* srcIter = temp->NewIterator();
      svtkArrayIterator* destIter = array->NewIterator();
      ...
      for (int k = 0; k < subDimensions[2] && !this->AbortExecute; ++k)
      {
        ...
        for (int j = 0; j < subDimensions[1]; ++j)
        {
          ...
          {
            svtkArrayIteratorTemplateMacro(svtkXMLStructuredDataReaderSubExtentCopyValues(
              static_cast<SVTK_TT*>(destIter), destTuple * components, static_cast<SVTK_TT*>(srcIter),
              sourceTuple * components, tupleSize * rowTuples));
            default:
              svtkErrorMacro("Array not supported : " << array->GetDataTypeAsString());
              break;
          }
        }
      }
