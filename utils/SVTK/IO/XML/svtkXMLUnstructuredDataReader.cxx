/*=========================================================================
  ...
  Module:    svtkXMLUnstructuredDataReader.cxx
...
//----------------------------------------------------------------------------
svtkXMLUnstructuredDataReader::svtkXMLUnstructuredDataReader()
{
...
//----------------------------------------------------------------------------
void svtkXMLUnstructuredDataReader::PrintSelf(ostream& os, svtkIndent indent)
{
  this->Superclass::PrintSelf(os, indent);
}

//----------------------------------------------------------------------------
svtkPointSet* svtkXMLUnstructuredDataReader::GetOutputAsPointSet()
{
  return svtkPointSet::SafeDownCast(this->GetOutputDataObject(0));
}

//----------------------------------------------------------------------------
svtkXMLDataElement* svtkXMLUnstructuredDataReader ::FindDataArrayWithName(
  svtkXMLDataElement* eParent, const char* name)
{
  ...
  {
    svtkXMLDataElement* eNested = eParent->GetNestedElement(i);
...
//----------------------------------------------------------------------------
svtkIdTypeArray* svtkXMLUnstructuredDataReader::ConvertToIdTypeArray(svtkDataArray* a)
{
  // If it is already a svtkIdTypeArray, just return it.
  svtkIdTypeArray* ida = svtkArrayDownCast<svtkIdTypeArray>(a);
  if (ida)
  {
    return ida;
  }
...
//----------------------------------------------------------------------------
svtkUnsignedCharArray* svtkXMLUnstructuredDataReader::ConvertToUnsignedCharArray(svtkDataArray* a)
{
  // If it is already a svtkUnsignedCharArray, just return it.
  svtkUnsignedCharArray* uca = svtkArrayDownCast<svtkUnsignedCharArray>(a);
  if (uca)
  {
    return uca;
  }
...
//----------------------------------------------------------------------------
void svtkXMLUnstructuredDataReader::SetupEmptyOutput()
{
  this->GetCurrentOutput()->Initialize();
}

//----------------------------------------------------------------------------
void svtkXMLUnstructuredDataReader::SetupOutputTotals()
{
  this->TotalNumberOfPoints = 0;
...
//----------------------------------------------------------------------------
void svtkXMLUnstructuredDataReader::SetupNextPiece()
{
  this->StartPoint += this->NumberOfPoints[this->Piece];
}

//----------------------------------------------------------------------------
void svtkXMLUnstructuredDataReader::SetupUpdateExtent(int piece, int numberOfPieces, int ghostLevel)
{
...
//----------------------------------------------------------------------------
void svtkXMLUnstructuredDataReader::ReadXMLData()
{
...
//----------------------------------------------------------------------------
void svtkXMLUnstructuredDataReader::SetupPieces(int numPieces)
{
  this->Superclass::SetupPieces(numPieces);
  this->NumberOfPoints = new svtkIdType[numPieces];
...
//----------------------------------------------------------------------------
void svtkXMLUnstructuredDataReader::DestroyPieces()
{
...
//----------------------------------------------------------------------------
svtkIdType svtkXMLUnstructuredDataReader::GetNumberOfPoints()
{
  return this->TotalNumberOfPoints;
}

//----------------------------------------------------------------------------
svtkIdType svtkXMLUnstructuredDataReader::GetNumberOfCells()
{
  return this->TotalNumberOfCells;
}

//----------------------------------------------------------------------------
svtkIdType svtkXMLUnstructuredDataReader::GetNumberOfPieces()
{
  return this->NumberOfPieces;
}

//----------------------------------------------------------------------------
svtkIdType svtkXMLUnstructuredDataReader::GetNumberOfPointsInPiece(int piece)
{
...
void svtkXMLUnstructuredDataReader::SetupOutputInformation(svtkInformation* outInfo)
{
...
//----------------------------------------------------------------------------
void svtkXMLUnstructuredDataReader::CopyOutputInformation(svtkInformation* outInfo, int port)
{
  this->Superclass::CopyOutputInformation(outInfo, port);
}

//----------------------------------------------------------------------------
void svtkXMLUnstructuredDataReader::SetupOutputData()
{
...
//----------------------------------------------------------------------------
int svtkXMLUnstructuredDataReader::ReadPiece(svtkXMLDataElement* ePiece)
{
  ...
  {
    svtkErrorMacro("Piece " << this->Piece << " is missing its NumberOfPoints attribute.");
...
//----------------------------------------------------------------------------
int svtkXMLUnstructuredDataReader::ReadPieceData()
{
  ...
  svtkIdType totalPieceSize = superclassPieceSize + 1 * this->GetNumberOfPointsInPiece(this->Piece);
  if (totalPieceSize == 0)
  {
    totalPieceSize = 1;
  }
  ...
  if (ePoints)
  {
    ...
    {
      svtkXMLDataElement* eNested = ePoints->GetNestedElement(i);
      ...
      if (needToRead)
      {
        ...
        {
          svtkErrorMacro("Cannot read points array from "
...
struct ValidateOffsets
{
  ...
  {
    using ValueType = svtk::GetAPIType<ArrayT>;
    ...
    { // The last entry in the offsets is the size of the connectivity
      this->ConnSize = static_cast<svtkIdType>(*(range.cend() - 1));
    }
  }
};
...
struct ConstructCellArray
{
  ...
  svtkDataArray* Connectivity;
  bool ConnectivityIsValid;

  ConstructCellArray(svtkCellArray* cellArray, svtkDataArray* connectivity)
    : CellArray{ cellArray }
  ...
  void operator()(ArrayT* offsets)
  {
    ...
    ArrayT* conn = svtkArrayDownCast<ArrayT>(this->Connectivity);
    if (!conn)
    {
...
//----------------------------------------------------------------------------
int svtkXMLUnstructuredDataReader::ReadCellArray(svtkIdType numberOfCells,
  svtkIdType svtkNotUsed(totalNumberOfCells), svtkXMLDataElement* eCells, svtkCellArray* outCells)
{
  if (numberOfCells <= 0)
  {
    return 1;
  ...
  {
    svtkXMLDataElement* eOffsets = this->FindDataArrayWithName(eCells, "offsets");
    if (!eOffsets && !this->AbortExecute)
    {
      svtkErrorMacro("Cannot read cell offsets from "
    ...
    auto aOffsets = svtk::TakeSmartPointer(this->CreateArray(eOffsets));
    if (!aOffsets)
    {
      svtkErrorMacro("Cell offsets array missing from " << eCells->GetName());
      return 0;
    }

    if (!aOffsets->IsA("svtkDataArray"))
    {
      svtkErrorMacro("Cannot cast cell offsets from " << eCells->GetName() << " to svtkDataArray.");
      return 0;
    }

    cellOffsets = svtkArrayDownCast<svtkDataArray>(aOffsets.Get());
  ...
  {
    // Use a svtkCellArray::ArrayType64 to ensure we can represent the incoming offset array type.
    svtkSmartPointer<svtkCellArray::ArrayType64> newArray =
      svtkSmartPointer<svtkCellArray::ArrayType64>::New();
  ...
  {
    svtkErrorMacro("Cannot read cell connectivity from "
  ...
  const svtkIdType connLength = offsetValidator.ConnSize;

  // ------------------------ Read connectivity---------------------------------
  svtkSmartPointer<svtkDataArray> conn;
  {
    svtkXMLDataElement* eConn = this->FindDataArrayWithName(eCells, "connectivity");
    if (!eConn)
    {
      svtkErrorMacro("Cannot read cell connectivity from "
        << eCells->GetName() << " in piece " << this->Piece
    ...
    auto aConn = svtk::TakeSmartPointer(this->CreateArray(eConn));
    if (!aConn)
    {
      svtkErrorMacro("Cell connectivity array missing from " << eCells->GetName());
      return 0;
    }

    if (!aConn->IsA("svtkDataArray"))
    {
      svtkErrorMacro(
        "Cannot cast cell connectivity from " << eCells->GetName() << " to svtkDataArray.");
      return 0;
    }
    conn = svtkArrayDownCast<svtkDataArray>(aConn.Get());
  ...
  { // First execution: Directly construct output cell array:
    ...
    {
      svtkErrorMacro("Cannot read cell data from "
        << eCells->GetName()
  ...
  { // Construct a temporary svtkCellArray that holds the arrays, and then
    // append to the input outCells.
    svtkNew<svtkCellArray> tmpCells;
...
//----------------------------------------------------------------------------
int svtkXMLUnstructuredDataReader::ReadFaceArray(svtkIdType numberOfCells, svtkXMLDataElement* eCells,
  svtkIdTypeArray* outFaces, svtkIdTypeArray* outFaceOffsets)
{
  if (numberOfCells <= 0)
  {
    return 1;
  ...
  // Read the cell offsets.
  svtkXMLDataElement* efaceOffsets = this->FindDataArrayWithName(eCells, "faceoffsets");
  if (!efaceOffsets)
  {
    svtkErrorMacro("Cannot read face offsets from "
      << eCells->GetName() << " in piece " << this->Piece
      << " because the \"faceoffsets\" array could not be found.");
    return 0;
  }
  svtkAbstractArray* ac1 = this->CreateArray(efaceOffsets);
  svtkDataArray* c1 = svtkArrayDownCast<svtkDataArray>(ac1);
  ...
  svtkIdTypeArray* faceOffsets = this->ConvertToIdTypeArray(c1);
  if (!faceOffsets)
  {
    svtkErrorMacro("Cannot read cell offsets from " << eCells->GetName() << " in piece "
                                                   << this->Piece
                                                   << " because the \"offsets\" array could not be"
                                                   << " converted to a svtkIdTypeArray.");
    return 0;
  }
  ...
  svtkIdType* faceoffsetPtr = faceOffsets->GetPointer(0);
  svtkIdType maxOffset = -1;
  for (svtkIdType i = numberOfCells - 1; i >= 0; i--)
  {
  ...
  svtkIdType facesArrayLength = maxOffset;
  ...
  svtkXMLDataElement* efaces = this->FindDataArrayWithName(eCells, "faces");
  if (!efaces)
  {
    svtkErrorMacro("Cannot read faces from " << eCells->GetName() << " in piece " << this->Piece
                                            << " because the \"faces\" array could not be found.");
    faceOffsets->Delete();
    return 0;
  }
  svtkAbstractArray* ac0 = this->CreateArray(efaces);
  svtkDataArray* c0 = svtkArrayDownCast<svtkDataArray>(ac0);
  ...
  svtkIdTypeArray* faces = this->ConvertToIdTypeArray(c0);
  if (!faces)
  {
    svtkErrorMacro("Cannot read faces from " << eCells->GetName() << " in piece " << this->Piece
  ...
  // svtkUnstructuredGrid::FaceLocations[i] points to the beginning of the
  // i-th cell. Need to convert. Also note that for both arrays, a
  // non-polyhedron cell has a offset of -1.
  svtkIdType* facesPtr = faces->GetPointer(0);
  svtkIdType startLoc = outFaceOffsets->GetNumberOfTuples();
  svtkIdType* outFaceOffsetsPtr = outFaceOffsets->WritePointer(startLoc, numberOfCells);
  svtkIdType currLoc = startLoc;

  // (Paraview-BUG-13892)
  // outFaceOffsets should point to the offset location in the outFaces array
  svtkIdType currFaceLoc = outFaces->GetNumberOfTuples();
  for (svtkIdType i = 0; i < numberOfCells; ++i, ++outFaceOffsetsPtr)
  {
    ...
    else
    {
      *outFaceOffsetsPtr = currFaceLoc;
      ...
      svtkIdType numberOfCellFaces = facesPtr[currLoc - startLoc];
      currLoc += 1;
      currFaceLoc++;
      for (svtkIdType j = 0; j < numberOfCellFaces; j++)
      {
        // read numberOfPoints in a face
        svtkIdType tmpLoc = currLoc - startLoc;
        svtkIdType numberOfFacePoints = facesPtr[tmpLoc];
        // update the point ids with StartPoint (Paraview-BUG-13892)
        if (this->StartPoint > 0)
        {
          for (svtkIdType pidx = tmpLoc + 1; pidx < tmpLoc + 1 + numberOfFacePoints; pidx++)
          {
  ...
  svtkIdType length = faces->GetNumberOfTuples();
  svtkIdType* outFacesPtr = outFaces->WritePointer(startLoc, length);
  for (svtkIdType i = 0; i < length; ++i, ++outFacesPtr)
  {
...
//----------------------------------------------------------------------------
int svtkXMLUnstructuredDataReader::ReadArrayForPoints(
  svtkXMLDataElement* da, svtkAbstractArray* outArray)
{
  svtkIdType startPoint = this->StartPoint;
  svtkIdType numPoints = this->NumberOfPoints[this->Piece];
...
//----------------------------------------------------------------------------
int svtkXMLUnstructuredDataReader::PointsNeedToReadTimeStep(svtkXMLDataElement* eNested)
{
  ...
  else
  {
    ...
    int isLastTimeInArray =
      svtkXMLReader::IsTimeStepInArray(this->PointsTimeStep, this->TimeSteps, numTimeSteps);
    // If no time is specified or if time is specified and match then read
...
int svtkXMLUnstructuredDataReader::CellsNeedToReadTimeStep(
  svtkXMLDataElement* eNested, int& cellstimestep, unsigned long& cellsoffset)
{
  ...
  else
  {
    ...
    int isLastTimeInArray =
      svtkXMLReader::IsTimeStepInArray(cellstimestep, this->TimeSteps, numTimeSteps);
    // If no time is specified or if time is specified and match then read
    if (isCurrentTimeInArray && !isLastTimeInArray)
    {
      // CurrentTimeStep is in TimeSteps but Last is not := need to read
