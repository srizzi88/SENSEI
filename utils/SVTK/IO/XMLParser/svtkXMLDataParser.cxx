...
#include "svtkBase64InputStream.h"
...
svtkStandardNewMacro(svtkXMLDataParser);
svtkCxxSetObjectMacro(svtkXMLDataParser, Compressor, svtkDataCompressor);

//----------------------------------------------------------------------------
svtkXMLDataParser::svtkXMLDataParser()
{
  this->NumberOfOpenElements = 0;
  this->OpenElementsSize = 10;
  this->OpenElements = new svtkXMLDataElement*[this->OpenElementsSize];
  this->RootElement = nullptr;
  this->AppendedDataPosition = 0;
  this->AppendedDataMatched = 0;
  this->DataStream = nullptr;
  this->InlineDataStream = svtkBase64InputStream::New();
  this->AppendedDataStream = svtkBase64InputStream::New();
  ...
  this->ByteOrder = svtkXMLDataParser::BigEndian;
#else
  this->ByteOrder = svtkXMLDataParser::LittleEndian;
#endif
  this->HeaderType = 32;

  this->AttributesEncoding = SVTK_ENCODING_NONE;
...
//----------------------------------------------------------------------------
void svtkXMLDataParser::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
int svtkXMLDataParser::Parse(const char*)
{
  svtkErrorMacro("Parsing from a string is not supported.");
  return 0;
}

//----------------------------------------------------------------------------
int svtkXMLDataParser::Parse(const char*, unsigned int)
{
  svtkErrorMacro("Parsing from a string is not supported.");
  return 0;
}
...
//----------------------------------------------------------------------------
void svtkXMLDataParser::StartElement(const char* name, const char** atts)
{
  svtkXMLDataElement* element = svtkXMLDataElement::New();
  element->SetName(name);
  element->SetXMLByteIndex(this->GetXMLByteIndex());
  svtkXMLUtilities::ReadElementFromAttributeArray(element, atts, this->AttributesEncoding);
  ...
  if (strcmp(name, "AppendedData") == 0)
  {
    ...
    {
      this->AppendedDataStream->Delete();
      this->AppendedDataStream = svtkInputStream::New();
    }
  }
}

//----------------------------------------------------------------------------
void svtkXMLDataParser::SeekInlineDataPosition(svtkXMLDataElement* element)
{
...
//----------------------------------------------------------------------------
void svtkXMLDataParser::EndElement(const char*)
{
  svtkXMLDataElement* finished = this->PopOpenElement();
...
//----------------------------------------------------------------------------
int svtkXMLDataParser::ParsingComplete()
{
...
//----------------------------------------------------------------------------
int svtkXMLDataParser::CheckPrimaryAttributes()
{
  ...
  if (byte_order)
  {
    ...
    {
      svtkErrorMacro("Unsupported byte_order=\"" << byte_order << "\"");
      return 0;
    }
  }
  if (const char* header_type = this->RootElement->GetAttribute("header_type"))
  {
    ...
    {
      svtkErrorMacro("Unsupported header_type=\"" << header_type << "\"");
      return 0;
    }
  }
  return 1;
...
//----------------------------------------------------------------------------
void svtkXMLDataParser::FindAppendedDataPosition()
{
  ...
  char c = 0;
  svtkTypeInt64 returnPosition = this->TellG();
  ...
  if (c != '_')
  {
    svtkWarningMacro("First character in AppendedData is ASCII value "
...
//----------------------------------------------------------------------------
void svtkXMLDataParser::PushOpenElement(svtkXMLDataElement* element)
{
  if (this->NumberOfOpenElements == this->OpenElementsSize)
  {
    unsigned int newSize = this->OpenElementsSize * 2;
    svtkXMLDataElement** newOpenElements = new svtkXMLDataElement*[newSize];
    unsigned int i;
...
//----------------------------------------------------------------------------
svtkXMLDataElement* svtkXMLDataParser::PopOpenElement()
{
...
//----------------------------------------------------------------------------
void svtkXMLDataParser::FreeAllElements()
{
...
//----------------------------------------------------------------------------
int svtkXMLDataParser::ParseBuffer(const char* buffer, unsigned int count)
{
  ...
  if (matched == length)
  {
    ...
    // Artificially end the SVTKFile element.
    const char finish[] = "\n</SVTKFile>\n";
...
//----------------------------------------------------------------------------
template <class T>
size_t svtkXMLDataParserGetWordTypeSize(T*)
{
  return sizeof(T);
}

//----------------------------------------------------------------------------
size_t svtkXMLDataParser::GetWordTypeSize(int wordType)
{
  size_t size = 1;
  switch (wordType)
  {
    svtkTemplateMacro(size = svtkXMLDataParserGetWordTypeSize(static_cast<SVTK_TT*>(nullptr)));
...
//----------------------------------------------------------------------------
void svtkXMLDataParser::PerformByteSwap(void* data, size_t numWords, size_t wordSize)
{
  char* ptr = static_cast<char*>(data);
  if (this->ByteOrder == svtkXMLDataParser::BigEndian)
  {
    switch (wordSize)
    {
      case 1:
...
//----------------------------------------------------------------------------
int svtkXMLDataParser::ReadCompressionHeader()
{
  std::unique_ptr<svtkXMLDataHeader> ch(svtkXMLDataHeader::New(this->HeaderType, 3));
  ...
  if (r < headerSize)
  {
    svtkErrorMacro("Error reading beginning of compression header.  Read "
  ...
  if (this->NumberOfBlocks > 0)
  {
    ...
    this->BlockStartOffsets = new svtkTypeInt64[this->NumberOfBlocks];

    // Read the compressed block sizes.
    size_t len = ch->DataSize();
    if (this->DataStream->Read(ch->Data(), len) < len)
    {
      svtkErrorMacro("Error reading compression header.");
      return 0;
    }
...
//----------------------------------------------------------------------------
size_t svtkXMLDataParser::FindBlockSize(svtkTypeUInt64 block)
{
...
//----------------------------------------------------------------------------
int svtkXMLDataParser::ReadBlock(svtkTypeUInt64 block, unsigned char* buffer)
{
...
//----------------------------------------------------------------------------
unsigned char* svtkXMLDataParser::ReadBlock(svtkTypeUInt64 block)
{
...
//----------------------------------------------------------------------------
size_t svtkXMLDataParser::ReadUncompressedData(
  unsigned char* data, svtkTypeUInt64 startWord, size_t numWords, size_t wordSize)
{
  ...
  if (r < headerSize)
  {
    svtkErrorMacro("Error reading uncompressed binary data header.  "
                  "Read "
      << r << " of " << headerSize << " bytes.");
    return 0;
  }
  this->PerformByteSwap(uh->Data(), uh->WordCount(), uh->WordSize());
  svtkTypeUInt64 rsize = uh->Get(0);
  ...
  svtkTypeUInt64 size = (rsize / wordSize) * wordSize;

  // Convert the start/length into bytes.
  svtkTypeUInt64 offset = startWord * wordSize;
  ...
  svtkTypeUInt64 end = offset + length;
  if (end > size)
  {
    end = size;
  }
...
//----------------------------------------------------------------------------
size_t svtkXMLDataParser::ReadCompressedData(
  unsigned char* data, svtkTypeUInt64 startWord, size_t numWords, size_t wordSize)
{
  ...
  svtkTypeUInt64 beginOffset = startWord * wordSize;
  svtkTypeUInt64 endOffset = beginOffset + numWords * wordSize;

  // Find the total size of the data.
  svtkTypeUInt64 totalSize = this->NumberOfBlocks * this->BlockUncompressedSize;
  ...
  svtkTypeUInt64 firstBlock = beginOffset / this->BlockUncompressedSize;
  svtkTypeUInt64 lastBlock = endOffset / this->BlockUncompressedSize;
...
//----------------------------------------------------------------------------
svtkXMLDataElement* svtkXMLDataParser::GetRootElement()
{
  return this->RootElement;
}

//----------------------------------------------------------------------------
size_t svtkXMLDataParser::ReadBinaryData(
  void* in_buffer, svtkTypeUInt64 startWord, size_t numWords, int wordType)
{
  ...
  if (this->Compressor)
  {
    ...
    {
      svtkErrorMacro("ReadCompressionHeader failed. Aborting read.");
      return 0;
    }
    this->DataStream->StartReading();
...
//----------------------------------------------------------------------------
size_t svtkXMLDataParser::ReadAsciiData(
  void* buffer, svtkTypeUInt64 startWord, size_t numWords, int wordType)
{
  // Skip real read if aborting.
  if (this->Abort)
  {
  ...
  svtkTypeUInt64 endWord = startWord + numWords;
...
//----------------------------------------------------------------------------
size_t svtkXMLDataParser::ReadInlineData(svtkXMLDataElement* element, int isAscii, void* buffer,
...
//----------------------------------------------------------------------------
size_t svtkXMLDataParser::ReadAppendedData(
...
// the template.
template <class T>
T* svtkXMLParseAsciiData(istream& is, int* length, T*, long)
{
  int dataLength = 0;
...
//----------------------------------------------------------------------------
static float* svtkXMLParseAsciiData(istream& is, int* length, float*, int)
{
  int dataLength = 0;
  int dataBufferSize = 64;
...
//----------------------------------------------------------------------------
static double* svtkXMLParseAsciiData(istream& is, int* length, double*, int)
{
  int dataLength = 0;
  int dataBufferSize = 64;
...
//----------------------------------------------------------------------------
static char* svtkXMLParseAsciiData(istream& is, int* length, char*, int)
{
  int dataLength = 0;
  int dataBufferSize = 64;
...
//----------------------------------------------------------------------------
static unsigned char* svtkXMLParseAsciiData(istream& is, int* length, unsigned char*, int)
{
  int dataLength = 0;
  int dataBufferSize = 64;
...
//----------------------------------------------------------------------------
static signed char* svtkXMLParseAsciiData(istream& is, int* length, signed char*, int)
{
  int dataLength = 0;
  int dataBufferSize = 64;
...
//----------------------------------------------------------------------------
static unsigned char* svtkXMLParseAsciiBitData(istream& is, int* length)
{
  ...
  while (is >> value)
  {
    ...
    if (value != 0)
    { // Mimic the storage mechanism used by svtkBitArray
...
//----------------------------------------------------------------------------
int svtkXMLDataParser::ParseAsciiData(int wordType)
{
  ...
  switch (wordType)
  {
    svtkTemplateMacro(buffer = svtkXMLParseAsciiData(is, &length, static_cast<SVTK_TT*>(nullptr), 1));

    case SVTK_BIT:
      buffer = svtkXMLParseAsciiBitData(is, &length);
      break;
  }
...
//----------------------------------------------------------------------------
template <class T>
void svtkXMLDataParserFreeAsciiBuffer(T* buffer)
{
  delete[] buffer;
}

//----------------------------------------------------------------------------
void svtkXMLDataParser::FreeAsciiBuffer()
{
  void* buffer = this->AsciiDataBuffer;
  switch (this->AsciiDataWordType)
  {
    svtkTemplateMacro(svtkXMLDataParserFreeAsciiBuffer(static_cast<SVTK_TT*>(buffer)));

    case SVTK_BIT:
      svtkXMLDataParserFreeAsciiBuffer(static_cast<unsigned char*>(buffer));
      break;
  }
  this->AsciiDataBuffer = nullptr;
}
...
//----------------------------------------------------------------------------
void svtkXMLDataParser::UpdateProgress(float progress)
{
  this->Progress = progress;
  double dProgress = progress;
  this->InvokeEvent(svtkCommand::ProgressEvent, &dProgress);
}
