/*=========================================================================
  ...
  Module:    svtkXdmfReaderInternal.h
...
=========================================================================*/
/**
 * @class   svtkXdmfReaderInternal
 *
 */

#ifndef svtkXdmfReaderInternal_h
#define svtkXdmfReaderInternal_h
...
//?
#include SVTKXDMF2_HEADER(XdmfSet.h)
...
class SVTKIOXDMF2_EXPORT svtkXdmfDocument
{
public:
  //---------------------------------------------------------------------------
  //@{
   ...
   */
  svtkXdmfDomain* GetActiveDomain() { return this->ActiveDomain; }
...
// I don't use svtkDataArraySelection since it's very slow when it comes to large
// number of arrays.
class svtkXdmfArraySelection : public std::map<std::string, bool>
{
public:
  void Merge(const svtkXdmfArraySelection& other)
  {
    svtkXdmfArraySelection::const_iterator iter = other.begin();
  ...
  {
    svtkXdmfArraySelection::iterator iter = this->find(name);
    if (iter != this->end())
    {
      return iter->second;
    }
  ...
  {
    int cc = 0;
    for (svtkXdmfArraySelection::iterator iter = this->begin(); iter != this->end(); ++iter)
    {
...
//***************************************************************************
class SVTKIOXDMF2_EXPORT svtkXdmfDomain
{
private:
  ...
  svtkIdType SILBlocksRoot;
  std::map<std::string, svtkIdType> GridCenteredAttrbuteRoots;
  std::map<svtkIdType, std::map<XdmfInt64, svtkIdType> > GridCenteredAttrbuteValues;

  svtkSILBuilder* SILBuilder;
  svtkMutableDirectedGraph* SIL;
  svtkXdmfArraySelection* PointArrays;
  svtkXdmfArraySelection* CellArrays;
  svtkXdmfArraySelection* Grids;
  svtkXdmfArraySelection* Sets;
...
public:
  ...
  svtkXdmfDomain(xdmf2::XdmfDOM* xmlDom, int domain_index);
  ...
  //---------------------------------------------------------------------------
  svtkGraph* GetSIL() { return this->SIL; }
  ...
  //---------------------------------------------------------------------------
  /**
   * Returns the SVTK data type need for this domain. If the domain has only 1
   * grid, then a svtkDataSet-type is returned, otherwise a svtkMultiBlockDataSet
   * is required.
   * Returns -1 on error.
   */
  int GetSVTKDataType();
  ...
  // Returns SVTK data type based on grid type and topology.
  // Returns -1 on error.
  int GetSVTKDataType(xdmf2::XdmfGrid* xmfGrid);
  ...
  svtkXdmfArraySelection* GetPointArraySelection() { return this->PointArrays; }
  svtkXdmfArraySelection* GetCellArraySelection() { return this->CellArrays; }
  svtkXdmfArraySelection* GetGridSelection() { return this->Grids; }
  svtkXdmfArraySelection* GetSetsSelection() { return this->Sets; }
...
private:
  ...
  void CollectMetaData(xdmf2::XdmfGrid* xmfGrid, svtkIdType silParent);

  // Used by CollectMetaData().
  void CollectNonLeafMetaData(xdmf2::XdmfGrid* xmfGrid, svtkIdType silParent);

  // Used by CollectMetaData().
  void CollectLeafMetaData(xdmf2::XdmfGrid* xmfGrid, svtkIdType silParent);
   ...
   */
  bool UpdateGridAttributeInSIL(xdmf2::XdmfAttribute* xmfAttribute, svtkIdType gridSILId);
  //@}
};
...
#endif
#endif
// SVTK-HeaderTest-Exclude: svtkXdmfReaderInternal.h
