"""
This test verifies that svtk's Xdmf reader and writer work in general.
...
def DoFilesExist(xdmfFile, hdf5File, svtkFile, deleteIfSo):
  global CleanUpGood
  ...
  if svtkFile:
    vexists = os.path.exists(svtkFile)
    vlenOK = os.path.getsize(svtkFile) > 0
...
def DoDataObjectsDiffer(dobj1, dobj2):
  ...
  if class1 == class2:
    ...
    if (((class1 == "svtkImageData") or
         (class1 == "svtkUniformGrid") or
         (class1 == "svtkStructuredPoints")) and
        ((class2 == "svtkImageData") or
         (class2 == "svtkUniformGrid") or
         (class2 == "svtkStructuredPoints"))):
      pass
    else:
      if (((class1 == "svtkPolyData") or
           (class1 == "svtkUnstructuredGrid")) and
          ((class2 == "svtkPolyData") or
           (class2 == "svtkUnstructuredGrid"))):
        pass
      else:
        if (((class1 == "svtkDirectedGraph") or
             (class1 == "svtkMutableDirectedGraph")) and
            ((class2 == "svtkDirectedGraph") or
             (class2 == "svtkMutableDirectedGraph"))):
          pass
        else:
  ...
  if not dobj1.IsA("svtkPolyData") and\
     not dobj2.IsA("svtkMultiBlockDataSet") and\
  ...
  ds1 = svtk.svtkDataSet.SafeDownCast(dobj1)
  ds2 = svtk.svtkDataSet.SafeDownCast(dobj2)
  if ds1 and ds2:
    ...
    g1 = svtk.svtkGraph.SafeDownCast(dobj1)
    g2 = svtk.svtkGraph.SafeDownCast(dobj2)
    if g1 and g2:
...
def TestXdmfConversion(dataInput, fileName):
  ...
  svtkFile = fileName + ".svtk"

  xWriter = svtk.svtkXdmf3Writer()
  ...
  xWriter.Write()
  timer.StopTimer()
  print("svtkXdmf3Writer took %f seconds to write %s" % (timer.GetElapsedTime(), xdmfFile))

  ds = svtk.svtkDataSet.SafeDownCast(dataInput)
  if ds:
  ...
  xReader = svtk.svtkXdmf3Reader()
  xReader.SetFileName(xdmfFile)
  timer.StartTimer()
  xReader.Update()
  timer.StopTimer()
  print("svtkXdmf3Reader took %f seconds to read %s" % (timer.GetElapsedTime(), xdmfFile))
  ...
  if fail:
    ...
    if ds:
      DoFilesExist(xdmfFile, hdf5File, svtkFile, CleanUpGood)
    else:
...
def RunTest():
  ...
  dog = svtk.svtkDataObjectGenerator()
  i = 0
  for testObject in testObjects:
    fileName = "xdmfIOtest_" + str(i)
    print("Test svtk object {}".format(testObject))
  ...
  gsrc = svtk.svtkRandomGraphSource()
  gsrc.DirectedOn()
  gsrc.Update()
  ...
  xWriter.Write()
  timer.StopTimer()
  print("svtkXdmf3Writer took %f seconds to write %s" % (timer.GetElapsedTime(), gFileName))
  xReader = svtk.svtkXdmf3Reader()
  ...
  tsrc = svtk.svtkTimeSourceExample()
  tsrc.GrowingOn()
  ...
  xWriter.Write()
  timer.StopTimer()
  print("svtkXdmf3Writer took %f seconds to write %s" % (timer.GetElapsedTime(), tFileName))
  xReader = svtk.svtkXdmf3Reader()
  xReader.SetFileName(tFileName)
  xReader.UpdateInformation()
  oi = xReader.GetOutputInformation(0)
  timerange = oi.Get(svtk.svtkCompositeDataPipeline.TIME_STEPS())
  ii = tsrc.GetOutputInformation(0)
  correcttimes = ii.Get(svtk.svtkCompositeDataPipeline.TIME_STEPS())
