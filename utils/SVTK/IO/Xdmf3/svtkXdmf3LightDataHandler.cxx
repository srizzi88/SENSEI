/*=========================================================================
  ...
  Module:    svtkXdmf3LightDataHandler.cxx
  Language:  C++
...
#include SVTKXDMF3_HEADER(XdmfGridController.hpp)
...
#include SVTKXDMF3_HEADER(core/XdmfVisitor.hpp)
// clang-format on
...
//------------------------------------------------------------------------------
shared_ptr<svtkXdmf3LightDataHandler> svtkXdmf3LightDataHandler::New(svtkXdmf3SILBuilder* sb,
  svtkXdmf3ArraySelection* f, svtkXdmf3ArraySelection* ce, svtkXdmf3ArraySelection* pn,
  svtkXdmf3ArraySelection* gc, svtkXdmf3ArraySelection* sc, unsigned int processor,
  unsigned int nprocessors)
{
  shared_ptr<svtkXdmf3LightDataHandler> p(new svtkXdmf3LightDataHandler());
  p->SILBuilder = sb;
  p->FieldArrays = f;
  p->CellArrays = ce;
  p->PointArrays = pn;
...
//------------------------------------------------------------------------------
svtkXdmf3LightDataHandler::svtkXdmf3LightDataHandler()
{
  SILBuilder = nullptr;
  FieldArrays = nullptr;
  CellArrays = nullptr;
...
//------------------------------------------------------------------------------
void svtkXdmf3LightDataHandler::InspectXDMF(
  shared_ptr<XdmfItem> item, svtkIdType parentVertex, unsigned int depth)
{
  ...
  if (!coll)
  {
    ...
    if (grid)
    {
      ...
      // atomic dataset
      svtkIdType parent = parentVertex;
      if (parentVertex == -1)
      {
  ...
  else
  {
    ...
    svtkIdType silVertex = parentVertex;
...
//------------------------------------------------------------------------------
void svtkXdmf3LightDataHandler::ClearGridsIfNeeded(shared_ptr<XdmfItem> domain)
{
  if (this->SILBuilder->IsMaxedOut())
  {
...
//------------------------------------------------------------------------------
std::set<double> svtkXdmf3LightDataHandler::getTimes()
{
  return times;
}

//------------------------------------------------------------------------------
void svtkXdmf3LightDataHandler::InspectArrays(shared_ptr<XdmfItem> item)
{
  shared_ptr<XdmfGrid> grid = shared_dynamic_cast<XdmfGrid>(item);
  if (grid)
  {
...
//------------------------------------------------------------------------------
bool svtkXdmf3LightDataHandler::TooDeep(unsigned int depth)
{
...
//------------------------------------------------------------------------------
std::string svtkXdmf3LightDataHandler::UniqueName(const std::string& name, bool ForGrid)
{
  std::string gridName = name;
  unsigned int count = 1;

  svtkXdmf3ArraySelection* cache = (ForGrid ? this->GridsCache : this->SetsCache);
...
//------------------------------------------------------------------------------
void svtkXdmf3LightDataHandler::AddNamedBlock(
  svtkIdType parentVertex, std::string originalName, std::string uniqueName)
{
  this->GridsCache->AddArray(uniqueName.c_str());

  svtkIdType silVertex = this->SILBuilder->AddVertex(uniqueName.c_str());
  this->SILBuilder->AddChildEdge(this->SILBuilder->GetBlocksRoot(), silVertex);

  svtkIdType hierarchyVertex = this->SILBuilder->AddVertex(originalName.c_str());
  this->SILBuilder->AddChildEdge(parentVertex, hierarchyVertex);
  this->SILBuilder->AddCrossEdge(hierarchyVertex, silVertex);
}

//------------------------------------------------------------------------------
void svtkXdmf3LightDataHandler::AddNamedSet(std::string uniqueName)
{
...
void svtkXdmf3LightDataHandler::InspectTime(shared_ptr<XdmfItem> item)
{
...
//------------------------------------------------------------------------------
void svtkXdmf3LightDataHandler::GetSetTime(shared_ptr<XdmfGrid> child, unsigned int& cnt)
{
  if (!child->getTime())
  {
...
//------------------------------------------------------------------------------
void svtkXdmf3LightDataHandler::GetSetTime(shared_ptr<XdmfGraph> child, unsigned int& cnt)
{
  if (!child->getTime())
  {
    // grid collections without explicit times are implied to go 0...N
...
//------------------------------------------------------------------------------
bool svtkXdmf3LightDataHandler::ShouldRead(unsigned int piece, unsigned int npieces)
{
  if (this->NumProcs < 1)
  {
