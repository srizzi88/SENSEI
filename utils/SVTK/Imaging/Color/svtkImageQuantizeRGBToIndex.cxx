/*=========================================================================
  ...
  Module:    svtkImageQuantizeRGBToIndex.cxx
...
svtkStandardNewMacro(svtkImageQuantizeRGBToIndex);

class svtkColorQuantizeNode
{
public:
  svtkColorQuantizeNode()
  {
    this->Axis = -1;
    this->SplitPoint = -1;
    this->Index = -1;
  ...
  void SetImageIncrement(svtkIdType v[3]) { memcpy(this->ImageIncrement, v, 3 * sizeof(svtkIdType)); }
  ...
  void SetChild1(svtkColorQuantizeNode* n) { this->Child1 = n; }
  svtkColorQuantizeNode* GetChild1() { return this->Child1; }

  void SetChild2(svtkColorQuantizeNode* n) { this->Child2 = n; }
  svtkColorQuantizeNode* GetChild2() { return this->Child2; }
...
protected:
  ...
  svtkIdType ImageIncrement[3];
  int ImageExtent[6];
  int ImageType;
  void* Image;
  int* Histogram[3];
  svtkColorQuantizeNode *Child1, *Child2;
};

template <class T>
void svtkImageQuantizeRGBToIndexHistogram(
  T* inPtr, int extent[6], svtkIdType inIncrement[3], int type, int bounds[6], int* histogram[3])
{
  T *rgbPtr, v[3];
  int x, y, z, c;
  int value[3];
  ...
  for (z = extent[4]; z <= extent[5]; z++)
  {
    for (y = extent[2]; y <= extent[3]; y++)
    {
      for (x = extent[0]; x <= extent[1]; x++)
      {
        ...
        else if (type == SVTK_UNSIGNED_SHORT)
        {
...
template <class T>
void svtkImageQuantizeRGBToIndexExecute(
  svtkImageQuantizeRGBToIndex* self, svtkImageData* inData, svtkImageData* outData)
{
  int extent[6];
  svtkIdType inIncrement[3], outIncrement[3];
  T* rgbPtr;
  unsigned short* indexPtr;
  int x, y, z, c;
  int type;
  svtkColorQuantizeNode *root, *tmp;
  svtkColorQuantizeNode* leafNodes[65536];
  int numLeafNodes;
  ...
  int rgb[3];
  svtkTimerLog* timer;
  svtkIdType totalCount;
  double weight;

  timer = svtkTimerLog::New();
  timer->StartTimer();
  ...
  svtkImageData* image_sample = inData;
  auto sampler =
    svtkSmartPointer<svtkExtractVOI>::New(); // outside of scope to keep samples image alive
  ...
  root = new svtkColorQuantizeNode;
  root->SetIndex(0);
...
void svtkColorQuantizeNode::ComputeStdDev()
{
  int i, j;
  double mean;
  int count = 0;
  ...
  {
    svtkTemplateMacro(svtkImageQuantizeRGBToIndexHistogram(static_cast<SVTK_TT*>(this->Image),
...
void svtkColorQuantizeNode::Divide(int axis, int nextIndex)
{
  int newBounds[6];

  this->Child1 = new svtkColorQuantizeNode;
  this->Child2 = new svtkColorQuantizeNode;
...
svtkImageQuantizeRGBToIndex::svtkImageQuantizeRGBToIndex()
{
  this->LookupTable = svtkLookupTable::New();
  this->NumberOfColors = 256;
  this->InputType = SVTK_UNSIGNED_SHORT;
...
int svtkImageQuantizeRGBToIndex::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  {
    svtkErrorMacro("This filter can handles only 3 components");
    return 1;
  }

  // this filter expects that output is type unsigned short.
  if (outData->GetScalarType() != SVTK_UNSIGNED_SHORT)
  {
  ...
  {
    svtkTemplateMacro(svtkImageQuantizeRGBToIndexExecute<SVTK_TT>(this, inData, outData));
    default:
      svtkErrorMacro(<< "Execute: This ScalarType is not handled");
      return 1;
  }
...
int svtkImageQuantizeRGBToIndex::RequestInformation(svtkInformation* svtkNotUsed(request),
...
int svtkImageQuantizeRGBToIndex::RequestUpdateExtent(svtkInformation* svtkNotUsed(request),
...
void svtkImageQuantizeRGBToIndex::PrintSelf(ostream& os, svtkIndent indent)
{
