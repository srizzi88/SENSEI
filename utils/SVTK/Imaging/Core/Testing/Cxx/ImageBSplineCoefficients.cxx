...
int ImageBSplineCoefficients(int argc, char* argv[])
{
  ...
  svtkSmartPointer<svtkImageBSplineInterpolator> interp =
  ...
  int modes[3] = { SVTK_IMAGE_BORDER_CLAMP, SVTK_IMAGE_BORDER_REPEAT, SVTK_IMAGE_BORDER_MIRROR };

  int m = SVTK_IMAGE_BSPLINE_DEGREE_MAX;
  ...
  for (int j = 0; j <= m; j++)
  {
    for (int jj = 0; jj < 3; jj++)
    {
      ...
      for (int k = 0; k < 20; k++)
      {
        ...
        if (mode == SVTK_IMAGE_BORDER_REPEAT)
        {
          x += 256 * spacing[0];
          y -= 256 * spacing[1];
        }
        else if (mode == SVTK_IMAGE_BORDER_MIRROR)
        {
          x = origin[0] - x;
