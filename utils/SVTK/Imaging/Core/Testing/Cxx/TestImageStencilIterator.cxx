...
// Test the svtkImageStencilIterator under various conditions
...
static void GenerateImage(svtkImageData* image, int extent[6])
{
  image->Initialize();
...
static void GenerateStencil(svtkImageStencilData* stencil, int extent[6])
{
  stencil->Initialize();
...
static bool CheckStencilExtents(int imageExt[6], int stencilExt[6], int extent[6])
{
  ...
  svtkNew<svtkImageStencilData> stencil;
  GenerateStencil(stencil, stencilExt);

  svtkImageStencilIterator<unsigned char> iter;
