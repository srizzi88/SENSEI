...
svtkStandardNewMacro(svtkExtractVOI);

// Construct object to extract all of the input data.
svtkExtractVOI::svtkExtractVOI()
{
...
//------------------------------------------------------------------------------
int svtkExtractVOI::RequestUpdateExtent(svtkInformation* svtkNotUsed(request),
...
//------------------------------------------------------------------------------
int svtkExtractVOI::RequestInformation(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  double in_origin[3];
  double in_spacing[3];
  ...
  inInfo->Get(svtkDataObject::SPACING(), in_spacing);
  inInfo->Get(svtkDataObject::ORIGIN(), in_origin);
  ...
  {
    hasDirection = true;
    inInfo->Get(svtkDataObject::DIRECTION(), direction);
    outInfo->Set(svtkDataObject::DIRECTION(), direction, 9);
  }
  ...
  outInfo->Set(svtkDataObject::SPACING(), out_spacing, 3);
  outInfo->Set(svtkDataObject::ORIGIN(), out_origin, 3);

  return 1;
}

//------------------------------------------------------------------------------
int svtkExtractVOI::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  // Reset internal helper to the actual extents of the piece we're working on:
  svtkInformation* inInfo = inputVector[0]->GetInformationObject(0);
  svtkImageData* inGrid = svtkImageData::GetData(inInfo);
  this->Internal->Initialize(
...
//------------------------------------------------------------------------------
bool svtkExtractVOI::RequestDataImpl(
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  if ((this->SampleRate[0] < 1) || (this->SampleRate[1] < 1) || (this->SampleRate[2] < 1))
  {
...
//------------------------------------------------------------------------------
void svtkExtractVOI::PrintSelf(ostream& os, svtkIndent indent)
{
