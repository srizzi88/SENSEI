/*=========================================================================
  ...
  Module:    svtkImageBSplineCoefficients.cxx
...
svtkStandardNewMacro(svtkImageBSplineCoefficients);

//----------------------------------------------------------------------------
svtkImageBSplineCoefficients::svtkImageBSplineCoefficients()
{
  this->SplineDegree = 3;
...
//----------------------------------------------------------------------------
svtkImageBSplineCoefficients::~svtkImageBSplineCoefficients() = default;

//----------------------------------------------------------------------------
void svtkImageBSplineCoefficients::AllocateOutputData(
  svtkImageData* svtkNotUsed(output), svtkInformation* svtkNotUsed(outInfo), int* svtkNotUsed(uExtent))
{
  // turn into a no-op, we allocate our output manually
}

//----------------------------------------------------------------------------
svtkImageData* svtkImageBSplineCoefficients::AllocateOutputData(
  svtkDataObject* output, svtkInformation* svtkNotUsed(outInfo))
{
  // turn into a no-op, we allocate our output manually
...
//----------------------------------------------------------------------------
int svtkImageBSplineCoefficients::RequestData(
  svtkInformation* request, svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  if (outData->GetScalarType() != SVTK_FLOAT && outData->GetScalarType() != SVTK_DOUBLE)
  {
    svtkErrorMacro(<< "Execute: output data must be type float or double.");
    return 0;
  }
  ...
  {
    svtkErrorMacro(<< "Execute: input and output extents do not match: "
  ...
  int extentcache[6];
  memcpy(
    extentcache, outInfo->Get(svtkStreamingDemandDrivenPipeline::UPDATE_EXTENT()), 6 * sizeof(int));
  outInfo->Set(svtkStreamingDemandDrivenPipeline::UPDATE_EXTENT(),
    outInfo->Get(svtkStreamingDemandDrivenPipeline::WHOLE_EXTENT()), 6);
  ...
  for (int i = 0; i < 3; i++)
  {
    ...
    if (ie[2 * i + 1] > ie[2 * i])
    {
      if (!this->svtkThreadedImageAlgorithm::RequestData(request, &outputVector, outputVector))
      {
        return 0;
      }
    }
  ...
  outInfo->Set(svtkStreamingDemandDrivenPipeline::UPDATE_EXTENT(), extentcache, 6);
  return 1;
}

//----------------------------------------------------------------------------
int svtkImageBSplineCoefficients::RequestInformation(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  if (inScalarInfo)
  {
    ...
    {
      numComponents = inScalarInfo->Get(svtkDataObject::FIELD_NUMBER_OF_COMPONENTS());
    }
  ...
  {
    svtkDataObject::SetPointDataActiveScalarInfo(outInfo, SVTK_DOUBLE, numComponents);
  }
  else
  {
    svtkDataObject::SetPointDataActiveScalarInfo(outInfo, SVTK_FLOAT, numComponents);
  }
...
//----------------------------------------------------------------------------
int svtkImageBSplineCoefficients::RequestUpdateExtent(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  svtkInformation* inInfo = inputVector[0]->GetInformationObject(0);
  svtkInformation* outInfo = outputVector->GetInformationObject(0);
...
//----------------------------------------------------------------------------
template <class T>
void svtkImageBSplineCoefficientsExecute(svtkImageBSplineCoefficients* self, svtkImageData* inData,
  svtkImageData* outData, T* inPtr, T* outPtr, int extent[6], int axis, int threadId)
{
  ...
  svtkIdType inInc0 = inInc[permute[axis][0]];
  svtkIdType inInc1 = inInc[permute[axis][1]];
  svtkIdType inInc2 = inInc[permute[axis][2]];

  svtkIdType outInc[6];
  outData->GetIncrements(outInc);
  svtkIdType outInc0 = outInc[permute[axis][0]];
  svtkIdType outInc1 = outInc[permute[axis][1]];
  svtkIdType outInc2 = outInc[permute[axis][2]];
  ...
  double poles[4];
  long numPoles;
  svtkImageBSplineInternals::GetPoleValues(poles, numPoles, self->GetSplineDegree());
  ...
  for (int idx2 = outMin2; idx2 <= outMax2; idx2++)
  {
    ...
    for (int idx1 = outMin1; !self->AbortExecute && idx1 <= outMax1; idx1++)
    {
      ...
      for (int idxC = 0; idxC < numscalars; idxC++)
      {
        ...
        svtkImageBSplineInternals::ConvertToInterpolationCoefficients(
          image, inMax0 - inMin0 + 1, borderMode, poles, numPoles, SVTK_DBL_EPSILON);
...
void svtkImageBSplineCoefficients::ThreadedExecute(
  svtkImageData* inData, svtkImageData* outData, int outExt[6], int threadId)
{
  ...
  {
    svtkImageBSplineCoefficientsExecute(this, inData, outData, static_cast<float*>(inPtr),
      static_cast<float*>(outPtr), outExt, this->Iteration, threadId);
  }
  else if (outData->GetScalarType() == SVTK_DOUBLE)
  {
    svtkImageBSplineCoefficientsExecute(this, inData, outData, static_cast<double*>(inPtr),
      static_cast<double*>(outPtr), outExt, this->Iteration, threadId);
  }
}

//----------------------------------------------------------------------------
void svtkImageBSplineCoefficients::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
const char* svtkImageBSplineCoefficients::GetBorderModeAsString()
{
...
//----------------------------------------------------------------------------
const char* svtkImageBSplineCoefficients::GetOutputScalarTypeAsString()
{
  return svtkImageScalarTypeNameMacro(this->OutputScalarType);
}

//----------------------------------------------------------------------------
int svtkImageBSplineCoefficients::CheckBounds(const double point[3])
{
...
//----------------------------------------------------------------------------
void svtkImageBSplineCoefficients::Evaluate(const double p[3], double* val)
{
  ...
  {
    svtkErrorMacro("Called Evaluate() on empty extent");
    return;
  }
  ...
  if (scalarType == SVTK_FLOAT)
  {
    ...
    svtkImageBSplineInternals::InterpolatedValue(coeffs, value, width, height, slices, numscalars, x,
  ...
  else if (scalarType == SVTK_DOUBLE)
  {
    ...
    svtkImageBSplineInternals::InterpolatedValue(coeffs, val, width, height, slices, numscalars, x,
      y, z, this->SplineDegree, this->BorderMode);
  }
  else
  {
    svtkErrorMacro("Called Evaluate(), but data is not float or double.");
  }
}

//----------------------------------------------------------------------------
double svtkImageBSplineCoefficients::Evaluate(double x, double y, double z)
{
  svtkImageData* output = this->GetOutput();
  int extent[6];
  double spacing[3], origin[3];
