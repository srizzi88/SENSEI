/*=========================================================================
  ...
  Module:    svtkImageBSplineInternals.cxx
...
/*--------------------------------------------------------------------------*/
void svtkImageBSplineInternals::ConvertToInterpolationCoefficients(
...
/*--------------------------------------------------------------------------*/
double svtkImageBSplineInternals::InitialCausalCoefficient(double c[], /* coefficients */
...
/*--------------------------------------------------------------------------*/
double svtkImageBSplineInternals::InitialAntiCausalCoefficient(double c[], /* coefficients */
  long DataLength, /* number of samples or coefficients */
  long Border,     /* border mode */
  double z,        /* actual pole */
  double Tolerance /* admissible relative error */
...
/*--------------------------------------------------------------------------*/
int svtkImageBSplineInternals::GetPoleValues(double Pole[4], /* poles for b-spline */
...
template <class T>
int svtkImageBSplineGetInterpolationWeights(T xWeight[10], /* weights, size is SplineDegree + 1 */
...
/*--------------------------------------------------------------------------*/
template <class T>
int svtkImageBSplineInterpolatedValue(const T* Bcoeff, /* input B-spline array of coefficients */
...
{ /* begin InterpolatedValue */
  ...
  if (Width > 1)
  {
    svtkImageBSplineInternals::GetInterpolationWeights(xWeight, s, SplineDegree);
  }
  if (Height > 1)
  {
    svtkImageBSplineInternals::GetInterpolationWeights(yWeight, t, SplineDegree);
  }
  if (Slices > 1)
  {
    svtkImageBSplineInternals::GetInterpolationWeights(zWeight, r, SplineDegree);
  }
...
/*--------------------------------------------------------------------------*/
int svtkImageBSplineInternals::GetInterpolationWeights(float weights[10], double w, long degree)
{
  return svtkImageBSplineGetInterpolationWeights(weights, w, degree);
}

int svtkImageBSplineInternals::GetInterpolationWeights(double weights[10], double w, long degree)
{
  return svtkImageBSplineGetInterpolationWeights(weights, w, degree);
}

/*--------------------------------------------------------------------------*/
int svtkImageBSplineInternals::InterpolatedValue(const double* coeffs, double* value, long width,
  long height, long slices, long depth, double x, double y, double z, long degree, long border)
{
  return svtkImageBSplineInterpolatedValue(
    coeffs, value, width, height, slices, depth, x, y, z, degree, border);
}

int svtkImageBSplineInternals::InterpolatedValue(const float* coeffs, float* value, long width,
  long height, long slices, long depth, double x, double y, double z, long degree, long border)
{
  return svtkImageBSplineInterpolatedValue(
    coeffs, value, width, height, slices, depth, x, y, z, degree, border);
