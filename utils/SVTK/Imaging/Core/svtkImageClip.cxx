...
svtkStandardNewMacro(svtkImageClip);

//----------------------------------------------------------------------------
svtkImageClip::svtkImageClip()
{
  this->ClipData = 0;
  this->Initialized = 0;
...
//----------------------------------------------------------------------------
void svtkImageClip::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
void svtkImageClip::SetOutputWholeExtent(int extent[6], svtkInformation* outInfo)
{
  int idx;
  int modified = 0;
...
//----------------------------------------------------------------------------
void svtkImageClip::SetOutputWholeExtent(int minX, int maxX, int minY, int maxY, int minZ, int maxZ)
{
  int extent[6];
...
//----------------------------------------------------------------------------
void svtkImageClip::GetOutputWholeExtent(int extent[6])
{
  int idx;
...
int svtkImageClip::RequestInformation(svtkInformation* svtkNotUsed(request),
...
void svtkImageClip::ResetOutputWholeExtent()
{
  if (!this->GetInput())
  {
...
int svtkImageClip::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  int* inExt;
  svtkInformation* inInfo = inputVector[0]->GetInformationObject(0);
