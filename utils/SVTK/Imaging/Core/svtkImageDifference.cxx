/*=========================================================================
  ...
  Module:    svtkImageDifference.cxx
...
class svtkImageDifferenceSMPThreadLocal : public svtkSMPThreadLocal<svtkImageDifferenceThreadData>
{
public:
  typedef svtkSMPThreadLocal<svtkImageDifferenceThreadData>::iterator iterator;
};

// Construct object to extract all of the input data.
svtkImageDifference::svtkImageDifference()
{
  this->Threshold = 105;
  this->AllowShift = 1;
  this->Averaging = 1;
...
#define svtkImageDifferenceComputeError(c1, c2)                                                     \
...
class svtkImageDifferenceSMPFunctor
{
public:
  ...
  svtkImageDifferenceSMPFunctor(svtkImageDifference* algo, svtkImageData*** inputs,
    svtkImageData** outputs, int* extent, svtkIdType pieces)
    : Algorithm(algo)
    , Inputs(inputs)
    , Outputs(outputs)
    , Extent(extent)
...
private:
  svtkImageDifferenceSMPFunctor() = delete;

  svtkImageDifference* Algorithm;
...
//----------------------------------------------------------------------------
void svtkImageDifferenceSMPFunctor::operator()(svtkIdType begin, svtkIdType end)
{
...
//----------------------------------------------------------------------------
// Used with svtkSMPTools to compute the error
void svtkImageDifferenceSMPFunctor::Reduce()
{
  const char* errorMessage = nullptr;
  double error = 0.0;
  double thresholdedError = 0.0;

  for (svtkImageDifferenceSMPThreadLocal::iterator iter = this->Algorithm->SMPThreadData->begin();
...
int svtkImageDifference::RequestUpdateExtent(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  int* wholeExtent = inInfo->Get(svtkStreamingDemandDrivenPipeline::WHOLE_EXTENT());
  ...
  wholeExtent = inInfo->Get(svtkStreamingDemandDrivenPipeline::WHOLE_EXTENT());
  outInfo->Get(svtkStreamingDemandDrivenPipeline::UPDATE_EXTENT(), uExt);
...
//----------------------------------------------------------------------------
void svtkImageDifference::ThreadedRequestData(svtkInformation* svtkNotUsed(request),
  ...
  svtkImageData*** inData, svtkImageData** outData, int outExt[6], int id)
{
  ...
  int idx0, idx1, idx2;
  svtkIdType in1Inc0, in1Inc1, in1Inc2;
  svtkIdType in2Inc0, in2Inc1, in2Inc2;
  ...
  double error = 0.0;
  double thresholdedError = 0.0;
  svtkImageDifferenceThreadData* threadData = nullptr;
  ...
  if (inData[0][0]->GetScalarType() != SVTK_UNSIGNED_CHAR ||
    inData[1][0]->GetScalarType() != SVTK_UNSIGNED_CHAR ||
    outData[0]->GetScalarType() != SVTK_UNSIGNED_CHAR)
  {
          ...
          for (int direction = 0; direction <= 1; ++direction)
          {
            ...
            for (int averaging = 0; averaging <= (this->Averaging ? 1 : 0); ++averaging)
            {
                   ...
                   yneigh <= (this->AllowShift ? 2 : 0) && !done; ++yneigh)
              {
                ...
                {
                  svtkImageDifferenceComputeError(
...
int svtkImageDifference::RequestData(
  svtkInformation* request, svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  int r = 1;

  if (this->EnableSMP) // For svtkSMPTools implementation.
  {
    // Get the input and output data objects
    svtkImageData* inDataPointer[2];
    svtkImageData** inData[2] = { &inDataPointer[0], &inDataPointer[1] };
    svtkImageData* outData[1];
    ...
    svtkIdType pieces = this->SplitExtent(nullptr, extent, 0, this->NumberOfThreads);

    // Use svtkSMPTools to multithread the functor
    svtkImageDifferenceSMPThreadLocal threadData;
    svtkImageDifferenceSMPFunctor functor(this, inData, outData, extent, pieces);
  ...
  {
    // For svtkMultiThreader implementation.
    int n = this->NumberOfThreads;
    this->ThreadData = new svtkImageDifferenceThreadData[n];
  ...
  if (this->ErrorMessage)
  {
    ...
    svtkErrorMacro("RequestData: " << this->ErrorMessage);
    this->ErrorMessage = nullptr;
    this->Error = 1000.0;
...
// be the same size
int svtkImageDifference::RequestInformation(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  // get the info objects
  svtkInformation* outInfo = outputVector->GetInformationObject(0);
  svtkInformation* inInfo1 = inputVector[0]->GetInformationObject(0);
  svtkInformation* inInfo2 = inputVector[1]->GetInformationObject(0);

  int* in1Ext = inInfo1->Get(svtkStreamingDemandDrivenPipeline::WHOLE_EXTENT());
  int* in2Ext = inInfo2->Get(svtkStreamingDemandDrivenPipeline::WHOLE_EXTENT());
    ...
    in1Ext[3] != in2Ext[3] || in1Ext[4] != in2Ext[4] || in1Ext[5] != in2Ext[5])
  {
    ...
    svtkErrorMacro("ExecuteInformation: Input are not the same size.\n"
...
//----------------------------------------------------------------------------
svtkImageData* svtkImageDifference::GetImage()
{
...
//----------------------------------------------------------------------------
void svtkImageDifference::PrintSelf(ostream& os, svtkIndent indent)
{
