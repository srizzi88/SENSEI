...
svtkStandardNewMacro(svtkImageMagnify);

//----------------------------------------------------------------------------
// Constructor: Sets default filter to be identity.
svtkImageMagnify::svtkImageMagnify()
{
  this->Interpolate = 0;
...
int svtkImageMagnify::RequestInformation(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  for (idx = 0; idx < 3; idx++)
  {
    ...
    outSpacing[idx] = spacing[idx] / static_cast<double>(this->MagnificationFactors[idx]);
  }
...
int svtkImageMagnify::RequestUpdateExtent(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  // get the info objects
  svtkInformation* outInfo = outputVector->GetInformationObject(0);
...
void svtkImageMagnify::InternalRequestUpdateExtent(int* inExt, int* outExt)
{
  int idx;
  ...
  for (idx = 0; idx < 3; idx++)
  {
    ...
    inExt[idx * 2] = static_cast<int>(floor(
      static_cast<double>(outExt[idx * 2]) / static_cast<double>(this->MagnificationFactors[idx])));
    inExt[idx * 2 + 1] = static_cast<int>(floor(static_cast<double>(outExt[idx * 2 + 1]) /
...
template <class T>
void svtkImageMagnifyExecute(svtkImageMagnify* self, svtkImageData* inData, T* inPtr, int inExt[6],
      ...
      for (idxY = 0; !self->AbortExecute && idxY <= maxY; idxY++, magYIdx--)
      {
        ...
        for (idxX = 0; idxX <= maxX; idxX++, magXIdx--)
        {
          ...
          else
          {
            ...
            *outPtrC = static_cast<T>(static_cast<double>(dataP) * (magXIdx + 1) * iMagP +
...
void svtkImageMagnify::ThreadedRequestData(svtkInformation* svtkNotUsed(request),
  ...
  svtkImageData*** inData, svtkImageData** outData, int outExt[6], int id)
{
  ...
  {
    svtkTemplateMacro(svtkImageMagnifyExecute(this, inData[0][0], static_cast<SVTK_TT*>(inPtr), inExt,
...
void svtkImageMagnify::PrintSelf(ostream& os, svtkIndent indent)
{
