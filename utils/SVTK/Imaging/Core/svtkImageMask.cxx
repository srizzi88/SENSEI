...
svtkStandardNewMacro(svtkImageMask);

//----------------------------------------------------------------------------
svtkImageMask::svtkImageMask()
{
  this->NotMask = 0;
...
//----------------------------------------------------------------------------
void svtkImageMask::SetImageInputData(svtkImageData* in)
{
  this->SetInput1Data(in);
}

//----------------------------------------------------------------------------
void svtkImageMask::SetMaskInputData(svtkImageData* in)
{
  this->SetInput2Data(in);
}

//----------------------------------------------------------------------------
void svtkImageMask::SetMaskedOutputValue(int num, double* v)
{
  int idx;

  if (num < 1)
  {
    svtkErrorMacro("Output value must have length greater than 0");
    return;
  }
...
template <class T>
void svtkImageMaskExecute(svtkImageMask* self, int ext[6], svtkImageData* in1Data, T* in1Ptr,
  svtkImageData* in2Data, unsigned char* in2Ptr, svtkImageData* outData, T* outPtr, int id)
{
...
void svtkImageMask::ThreadedRequestData(svtkInformation* svtkNotUsed(request),
  ...
  svtkImageData*** inData, svtkImageData** outData, int outExt[6], int id)
{
  ...
  {
    svtkErrorMacro("Mask extent not large enough");
    return;
  }

  if (inData[1][0]->GetNumberOfScalarComponents() != 1)
  {
    svtkErrorMacro("Masks can have one component");
  }

  if (inData[0][0]->GetScalarType() != outData[0]->GetScalarType() ||
    inData[1][0]->GetScalarType() != SVTK_UNSIGNED_CHAR)
  {
    svtkErrorMacro(<< "Execute: image ScalarType (" << inData[0][0]->GetScalarType()
  ...
  {
    svtkTemplateMacro(
      svtkImageMaskExecute(this, outExt, inData[0][0], static_cast<SVTK_TT*>(inPtr1), inData[1][0],
        static_cast<unsigned char*>(inPtr2), outData[0], static_cast<SVTK_TT*>(outPtr), id));
    default:
...
int svtkImageMask::RequestInformation(svtkInformation* svtkNotUsed(request),
...
void svtkImageMask::PrintSelf(ostream& os, svtkIndent indent)
{
