...
svtkStandardNewMacro(svtkImageResize);
svtkCxxSetObjectMacro(svtkImageResize, Interpolator, svtkAbstractImageInterpolator);

//----------------------------------------------------------------------------
svtkImageResize::svtkImageResize()
{
  this->ResizeMethod = svtkImageResize::OUTPUT_DIMENSIONS;
...
//----------------------------------------------------------------------------
const char* svtkImageResize::GetResizeMethodAsString()
{
  switch (this->ResizeMethod)
  {
    case svtkImageResize::OUTPUT_DIMENSIONS:
      return "OutputDimensions";
    case svtkImageResize::OUTPUT_SPACING:
      return "OutputSpacing";
    case svtkImageResize::MAGNIFICATION_FACTORS:
...
//----------------------------------------------------------------------------
int svtkImageResize::RequestInformation(
  svtkInformation*, svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  switch (this->ResizeMethod)
  {
    ...
    case svtkImageResize::OUTPUT_SPACING:
    {
      for (int i = 0; i < 3; i++)
      {
        ...
        outDims[i] = static_cast<int>(d + SVTK_INTERPOLATE_FLOOR_TOL) + 1;
      }
    }
    break;
    case svtkImageResize::MAGNIFICATION_FACTORS:
  ...
  outInfo->Set(svtkStreamingDemandDrivenPipeline::WHOLE_EXTENT(), outExt, 6);
...
//----------------------------------------------------------------------------
int svtkImageResize::RequestUpdateExtent(
  svtkInformation*, svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  svtkInformation* outInfo = outputVector->GetInformationObject(0);
  svtkInformation* inInfo = inputVector[0]->GetInformationObject(0);
  ...
  svtkAbstractImageInterpolator* interpolator = this->GetInternalInterpolator();
  ...
  for (int j = 0; j < 3; j++)
  {
    ...
    extent[2 * j] = SVTK_INT_MAX;
    extent[2 * j + 1] = SVTK_INT_MIN;
    ...
    for (int ii = 0; ii < 2; ii++)
    {
      ...
      {
        double f;
        int k = svtkInterpolationMath::Floor(range[ii], f);
      ...
      {
        int k = svtkInterpolationMath::Round(range[ii]);
        if (k < extent[2 * j])
        {
          extent[2 * j] = k - extra;
        }
...
#define SVTK_RESIZE_CONVERT_INT_CLAMP(T, minval, maxval)                                            \
  void svtkImageResizeConvert(double v, T& u)                                                       \
  {                                                                                                \
    v = svtkMath::ClampValue(v, minval, maxval);                                                    \
    u = svtkInterpolationMath::Round(v);                                                            \
  }

#define SVTK_RESIZE_CONVERT_FLOAT(T)                                                                \
  void svtkImageResizeConvert(double v, T& u) { u = static_cast<T>(v); }

SVTK_RESIZE_CONVERT_INT_CLAMP(svtkTypeUInt8, 0.0, 255.0);
SVTK_RESIZE_CONVERT_INT_CLAMP(svtkTypeUInt16, 0.0, 65535.0);
SVTK_RESIZE_CONVERT_INT_CLAMP(svtkTypeUInt32, 0.0, 4294967295.0);
SVTK_RESIZE_CONVERT_INT_CLAMP(svtkTypeInt8, -128.0, 127.0);
SVTK_RESIZE_CONVERT_INT_CLAMP(svtkTypeInt16, -32768.0, 32767.0);
SVTK_RESIZE_CONVERT_INT_CLAMP(svtkTypeInt32, -2147483648.0, 2147483647.0);
SVTK_RESIZE_CONVERT_FLOAT(svtkTypeFloat32);
SVTK_RESIZE_CONVERT_FLOAT(svtkTypeFloat64);

//----------------------------------------------------------------------------
// Apply a 1D filter in the X direction.
// The inPtr parameter must be positioned at the correct slice.
template <class T>
void svtkImageResizeFilterX(const T* inPtr, double* outPtr, int ncomp, const int extent[6],
  const svtkIdType* a, const double* f, int kernelSize)
{
  ...
  else
  {
    do
    {
      ...
      do
      {
        ...
        tmpPtr++;
        svtkImageResizeConvert(val, *outPtr);
        outPtr++;
      } while (--i);
      a += kernelSize;
      f += kernelSize;
...
template <class T>
void svtkImageResizeFilterYOrZ(
  double** rowPtr, T* outPtr, int ncomp, const int extent[6], const double* f, int kernelSize)
{
  // number of data values in one row
  svtkIdType rowCounter = (extent[1] - extent[0] + 1) * ncomp;
  ...
  if (kernelSize == 1)
  {
    ...
    do
    {
      svtkImageResizeConvert(*tmpPtr, *outPtr);
      outPtr++;
      tmpPtr++;
    } while (--rowCounter);
  }
...
void svtkImageResizeFilter2D(const T* inPtr, U* outPtr, const svtkIdType outInc[3],
  const int extent[6], const svtkIdType* aX, const double* fX, int kernelSizeX, const svtkIdType* a,
  const double* f, int kernelSize, double** workPtr, int direction, svtkAlgorithm* progress)
{
  ...
  if (kernelSize == 1)
  {
    ...
    for (int idY = idYMin; idY <= idYMax; idY++)
    {
      ...
      svtkImageResizeFilterX(&inPtr[*a], *workPtr, ncomp, extent, aX, fX, kernelSizeX);

      svtkImageResizeFilterYOrZ(workPtr, outPtr, ncomp, extent, f, kernelSize);
  ...
  else
  {
    ...
    for (int idY = idYMin; idY <= idYMax; idY++)
    {
      ...
      if (j)
        do
        {
          svtkImageResizeFilterX(
            &inPtr[*a], workPtr[kernelSize - j], ncomp, extent, aX, fX, kernelSizeX);
      ...
      if (idY < idYMax)
      {
        ...
        {
          int i = kernelSize - j;
          const svtkIdType* b = a - i;
          const svtkIdType* c = a;
          do
          {
            if (*c++ != *b++)
            {
...
// directions.
template <class T>
void svtkImageResizeFilter3D(const T* inPtr, T* outPtr, const svtkIdType outInc[3],
  const int extent[6], const svtkIdType* aX, const double* fX, int kernelSizeX, const svtkIdType* aY,
  const double* fY, int kernelSizeY, const svtkIdType* aZ, const double* fZ, int kernelSizeZ,
  svtkAlgorithm* progress)
{
  svtkIdType rowSize = outInc[0] * (extent[1] - extent[0] + 1);
  int ncomp = static_cast<int>(outInc[0]);
  ...
  if (kernelSizeX == 1 && kernelSizeY == 1 && kernelSizeZ == 1)
  {
    ...
    svtkIdType progressGoal = (idYMax - idYMin + 1);
    progressGoal *= (idZMax - idZMin + 1);
    svtkIdType progressCount = 0;
    svtkIdType progressStep = (progressGoal + 49) / 50;
  ...
  else if (kernelSizeZ == 1 || kernelSizeY == 1)
  {
    ...
    const svtkIdType* aSlice = aZ;
    const svtkIdType* a = aY;
    const double* f = fY;
    if (kernelSizeY == 1)
    {
      sliceDirection = 1;
    ...
    svtkIdType workSize = rowSize * kernelSize;
    ...
    int progressStep = (progressGoal + 49) / 50;
    int progressCount = 0;
    svtkAlgorithm* rowProgress = nullptr;
    if (progressGoal == 1)
    {
    ...
    for (int slice = sliceMin; slice <= sliceMax; slice++)
    {
      ...
      svtkImageResizeFilter2D(&inPtr[*aSlice], outPtr, outInc, extent, aX, fX, kernelSizeX, a, f,
        kernelSize, workPtr, direction, rowProgress);
  ...
  else
  {
    ...
    svtkIdType sliceSize = rowSize * (extent[5] - extent[4] + 1);
    svtkIdType workSize = rowSize * kernelSizeZ;
    ...
    for (int idY = extent[2]; idY <= extent[3]; idY++)
    {
      ...
      if (j)
        do
        {
          svtkImageResizeFilter2D(&inPtr[*aY], slicePtr[kernelSizeY - j], sliceInc, extent, aX, fX,
            kernelSizeX, aZ, fZ, kernelSizeZ, workPtr, 2, nullptr);
      ...
      {
        svtkImageResizeFilterYOrZ(slicePtr, outPtr0, ncomp, extent, fY, kernelSizeY);
...
int svtkImageResize::RequestData(
...
//----------------------------------------------------------------------------
void svtkImageResize::ThreadedRequestData(svtkInformation*, svtkInformationVector**,
  svtkInformationVector*, svtkImageData***, svtkImageData** outData, int extent[6], int threadId)
{
  svtkDebugMacro("Execute: outData = " << outData);
  ...
  svtkAlgorithm* progress = ((threadId == 0) ? this : nullptr);
  ...
  if (outScalarType == inScalarType)
  {
    switch (inScalarType)
    {
      svtkTemplateAliasMacro(
        svtkImageResizeFilter3D(static_cast<const SVTK_TT*>(inPtr), static_cast<SVTK_TT*>(outPtr),
          outInc, extent, aX, fX, kernelSizeX, aY, fY, kernelSizeY, aZ, fZ, kernelSizeZ, progress));
      default:
  ...
  {
    svtkErrorMacro("ThreadedRequestData: output scalar type does not match "
                  "input scalar type");
  }
...
//----------------------------------------------------------------------------
void svtkImageResize::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
svtkAbstractImageInterpolator* svtkImageResize::GetInterpolator()
{
  if (this->Interpolator == nullptr)
  {
    svtkImageSincInterpolator* i = svtkImageSincInterpolator::New();
...
//----------------------------------------------------------------------------
svtkAbstractImageInterpolator* svtkImageResize::GetInternalInterpolator()
{
  if (this->Interpolate)
  {
  ...
  if (!this->NNInterpolator)
  {
    svtkImageInterpolator* nn = svtkImageInterpolator::New();
...
//----------------------------------------------------------------------------
svtkMTimeType svtkImageResize::GetMTime()
{
