/*=========================================================================
  ...
  Module:    svtkImageSincInterpolator.cxx
...
#define SVTK_INTERPOLATION_WINDOW_MASK 0x0000007f
#define SVTK_INTERPOLATION_WINDOW_XBLUR_MASK 0x00008000
#define SVTK_INTERPOLATION_WINDOW_XSIZE_MASK 0x00007f00
#define SVTK_INTERPOLATION_WINDOW_XSIZE_SHIFT 8
#define SVTK_INTERPOLATION_WINDOW_YBLUR_MASK 0x00800000
#define SVTK_INTERPOLATION_WINDOW_YSIZE_MASK 0x007f0000
#define SVTK_INTERPOLATION_WINDOW_YSIZE_SHIFT 16
#define SVTK_INTERPOLATION_WINDOW_ZBLUR_MASK 0x80000000
#define SVTK_INTERPOLATION_WINDOW_ZSIZE_MASK 0x7f000000
#define SVTK_INTERPOLATION_WINDOW_ZSIZE_SHIFT 24
...
#define SVTK_SINC_KERNEL_TABLE_DIVISIONS 256

svtkStandardNewMacro(svtkImageSincInterpolator);

//----------------------------------------------------------------------------
svtkImageSincInterpolator::svtkImageSincInterpolator()
{
  this->WindowFunction = SVTK_LANCZOS_WINDOW;
...
//----------------------------------------------------------------------------
void svtkImageSincInterpolator::PrintSelf(ostream& os, svtkIndent indent)
{
  this->Superclass::PrintSelf(os, indent);

  os << indent << "WindowFunction: " << this->GetWindowFunctionAsString() << "\n";
  os << indent << "WindowHalfWidth: " << this->WindowHalfWidth << "\n";
  os << indent << "UseWindowParameter: " << (this->UseWindowParameter ? "On\n" : "Off\n");
  os << indent << "WindowParameter: " << this->WindowParameter << "\n";
  os << indent << "BlurFactors: " << this->BlurFactors[0] << " " << this->BlurFactors[1] << " "
     << this->BlurFactors[2] << "\n";
  os << indent << "Antialiasing: " << (this->Antialiasing ? "On\n" : "Off\n");
  os << indent << "Renormalization: " << (this->Renormalization ? "On\n" : "Off\n");
}

//----------------------------------------------------------------------------
void svtkImageSincInterpolator::ComputeSupportSize(const double matrix[16], int size[3])
{
  // compute the default support size for when matrix is null
  if (this->Antialiasing)
  {
    size[0] = SVTK_SINC_KERNEL_SIZE_MAX;
    size[1] = SVTK_SINC_KERNEL_SIZE_MAX;
    size[2] = SVTK_SINC_KERNEL_SIZE_MAX;
  }
  else
  {
    for (int i = 0; i < 3; i++)
    {
      ...
      if (rowscale > (1.0 + SVTK_INTERPOLATE_FLOOR_TOL))
      {
        size[i] =
          2 * static_cast<int>(rowscale * this->WindowHalfWidth + 1.0 - SVTK_INTERPOLATE_FLOOR_TOL);
      }
    }
  }
...
//----------------------------------------------------------------------------
bool svtkImageSincInterpolator::IsSeparable()
{
  return true;
}

//----------------------------------------------------------------------------
void svtkImageSincInterpolator::SetWindowHalfWidth(int size)
{
  size = svtkMath::ClampValue(size, 1, SVTK_SINC_KERNEL_SIZE_MAX / 2);
...
//----------------------------------------------------------------------------
void svtkImageSincInterpolator::SetWindowFunction(int mode)
{
  mode = svtkMath::ClampValue(mode, SVTK_LANCZOS_WINDOW, SVTK_BLACKMAN_NUTTALL4);
...
//----------------------------------------------------------------------------
const char* svtkImageSincInterpolator::GetWindowFunctionAsString()
{
  const char* result = "";

  switch (this->WindowFunction)
  {
    case SVTK_LANCZOS_WINDOW:
      result = "Lanczos";
      break;
    case SVTK_KAISER_WINDOW:
      result = "Kaiser";
      break;
    case SVTK_COSINE_WINDOW:
      result = "Cosine";
      break;
    case SVTK_HANN_WINDOW:
      result = "Hann";
      break;
    case SVTK_HAMMING_WINDOW:
      result = "Hamming";
      break;
    case SVTK_BLACKMAN_WINDOW:
      result = "Blackman";
      break;
    case SVTK_BLACKMAN_HARRIS3:
      result = "BlackmanHarris3";
      break;
    case SVTK_BLACKMAN_HARRIS4:
      result = "BlackmanHarris4";
      break;
    case SVTK_NUTTALL_WINDOW:
      result = "Nuttall";
      break;
    case SVTK_BLACKMAN_NUTTALL3:
      result = "BlackmanNuttall3";
      break;
    case SVTK_BLACKMAN_NUTTALL4:
...
//----------------------------------------------------------------------------
void svtkImageSincInterpolator::SetUseWindowParameter(int val)
{
  val = (val != 0);
...
//----------------------------------------------------------------------------
void svtkImageSincInterpolator::SetWindowParameter(double val)
{
...
//----------------------------------------------------------------------------
void svtkImageSincInterpolator::SetBlurFactors(double x, double y, double z)
{
...
//----------------------------------------------------------------------------
void svtkImageSincInterpolator::SetAntialiasing(int val)
{
  val = (val != 0);
...
//----------------------------------------------------------------------------
void svtkImageSincInterpolator::SetRenormalization(int val)
{
  val = (val != 0);
...
//----------------------------------------------------------------------------
void svtkImageSincInterpolator::InternalDeepCopy(svtkAbstractImageInterpolator* a)
{
  svtkImageSincInterpolator* obj = svtkImageSincInterpolator::SafeDownCast(a);
  if (obj)
  {
...
//----------------------------------------------------------------------------
void svtkImageSincInterpolator::InternalUpdate()
{
  ...
  for (int i = 0; i < 3; i++)
  {
    ...
    hsize[i] = size;
    blurchange |=
      (fabs(this->BlurFactors[i] - this->LastBlurFactors[i]) >= SVTK_INTERPOLATE_FLOOR_TOL);
  }

  if (this->BlurFactors[0] > 1.0 + SVTK_INTERPOLATE_FLOOR_TOL)
  {
    mode |= SVTK_INTERPOLATION_WINDOW_XBLUR_MASK;
  }
  if (this->BlurFactors[1] > 1.0 + SVTK_INTERPOLATE_FLOOR_TOL)
  {
    mode |= SVTK_INTERPOLATION_WINDOW_YBLUR_MASK;
  }
  if (this->BlurFactors[2] > 1.0 + SVTK_INTERPOLATE_FLOOR_TOL)
  {
    mode |= SVTK_INTERPOLATION_WINDOW_ZBLUR_MASK;
  }

  mode |= (hsize[0] << SVTK_INTERPOLATION_WINDOW_XSIZE_SHIFT);
  mode |= (hsize[1] << SVTK_INTERPOLATION_WINDOW_YSIZE_SHIFT);
  mode |= (hsize[2] << SVTK_INTERPOLATION_WINDOW_ZSIZE_SHIFT);

  if (this->InterpolationInfo->InterpolationMode != mode || blurchange ||
    this->KernelLookupTable[0] == nullptr)
  {
...
inline double svtkSincPi(double x)
{
  x *= svtkMath::Pi();
...
inline double svtkBesselI0(double x)
{
  int m = 0;
  double x2 = 0.25 * x * x;
  double p = 1;
...
double svtkSincWindow::Lanczos(double x)
{
  return svtkSincPi(x);
}

double svtkSincWindow::Kaiser(double x, double a)
{
  double api = a * svtkMath::Pi();
  double y = 1 - x * x;
  y *= (y > 0); // if less than zero, set to zero

  return svtkBesselI0(api * sqrt(y)) / svtkBesselI0(api);
}

double svtkSincWindow::Cosine(double x)
{
  double halfpi = 0.5 * svtkMath::Pi();

  return cos(x * halfpi);
}

template <int N>
double svtkSincWindow::Hamming(double x, const double* a)
{
  double q = 0;
  double y = a[0];
  x *= svtkMath::Pi();
...
template <class F>
void svtkSincKernel::Lanczos(F* kernel, int size, int n, double p)
{
  double q = n * p;
  double x = p;
  double y = q;
  *kernel++ = 1.0;
  ...
  {
    int inbounds = (x < 1.0);
    *kernel++ = svtkSincWindow::Lanczos(x) * svtkSincPi(y) * inbounds;
    x += p;
    y += q;
  } while (--size);
}
...
template <class F>
void svtkSincKernel::Kaiser(F* kernel, int size, int n, double p, double a)
{
  ...
  *kernel++ = 1.0;
  ...
  {
    int inbounds = (x < 1.0);
    *kernel++ = svtkSincWindow::Kaiser(x, a) * svtkSincPi(y) * inbounds;
    x += p;
    y += q;
  } while (--size);
}
...
template <class F>
void svtkSincKernel::Cosine(F* kernel, int size, int n, double p)
{
  double q = n * p;
  double x = p;
  double y = q;
  *kernel++ = 1;
  ...
  {
    int inbounds = (x < 1.0);
    *kernel++ = svtkSincWindow::Cosine(x) * svtkSincPi(y) * inbounds;
    x += p;
    y += q;
  } while (--size);
}
...
template <class F, int N>
void svtkSincKernel::Hamming(F* kernel, int size, int n, double p, const double* a)
{
  double q = n * p;
  double x = p;
  double y = q;
  *kernel++ = 1.0;
  ...
  {
    int inbounds = (x < 1.0);
    *kernel++ = svtkSincWindow::Hamming<N>(x, a) * svtkSincPi(y) * inbounds;
    x += p;
    y += q;
  } while (--size);
}
...
void svtkSincInterpWeights(T* kernel, F* fX, F fx, int m)
{
  // table bins per unit
  int p = SVTK_SINC_KERNEL_TABLE_DIVISIONS;
...
template <class T>
void svtkRenormalizeKernel(T* kernel, int m, int n)
{
...
struct svtkImageSincInterpolate
{
  static void General(svtkInterpolationInfo* info, const F point[3], F* outPtr);
};

//----------------------------------------------------------------------------
template <class F, class T>
void svtkImageSincInterpolate<F, T>::General(svtkInterpolationInfo* info, const F point[3], F* outPtr)
{
  ...
  int xm =
    2 * ((mode & SVTK_INTERPOLATION_WINDOW_XSIZE_MASK) >> SVTK_INTERPOLATION_WINDOW_XSIZE_SHIFT);
  int ym =
    2 * ((mode & SVTK_INTERPOLATION_WINDOW_YSIZE_MASK) >> SVTK_INTERPOLATION_WINDOW_YSIZE_SHIFT);
  int zm =
    2 * ((mode & SVTK_INTERPOLATION_WINDOW_ZSIZE_MASK) >> SVTK_INTERPOLATION_WINDOW_ZSIZE_SHIFT);
  ...
  // the memory offsets
  svtkIdType factX[SVTK_SINC_KERNEL_SIZE_MAX];
  svtkIdType factY[SVTK_SINC_KERNEL_SIZE_MAX];
  svtkIdType factZ[SVTK_SINC_KERNEL_SIZE_MAX];
  ...
  F fX[SVTK_SINC_KERNEL_SIZE_MAX];
  F fY[SVTK_SINC_KERNEL_SIZE_MAX];
  F fZ[SVTK_SINC_KERNEL_SIZE_MAX];

  svtkSincInterpWeights(kernel[0], fX, fx, xm);
  svtkSincInterpWeights(kernel[1], fY, fy, ym);
  svtkSincInterpWeights(kernel[2], fZ, fz, zm);
...
template <class F>
void svtkImageSincInterpolatorGetInterpolationFunc(
  void (**interpolate)(svtkInterpolationInfo*, const F[3], F*), int dataType,
  int svtkNotUsed(interpolationMode))
{
  switch (dataType)
  {
    svtkTemplateAliasMacro(*interpolate = &(svtkImageSincInterpolate<F, SVTK_TT>::General));
    default:
      *interpolate = nullptr;
  }
}
...
template <class F, class T>
struct svtkImageSincRowInterpolate
{
  static void General(
...
template <class F, class T>
void svtkImageSincRowInterpolate<F, T>::General(
  svtkInterpolationWeights* weights, int idX, int idY, int idZ, F* outPtr, int n)
{
...
template <class F>
void svtkImageSincInterpolatorGetRowInterpolationFunc(
  ...
  int scalarType, int svtkNotUsed(interpolationMode))
{
  switch (scalarType)
  {
    svtkTemplateAliasMacro(*summation = &(svtkImageSincRowInterpolate<F, SVTK_TT>::General));
    default:
      *summation = nullptr;
  }
}
...
//----------------------------------------------------------------------------
template <class F>
void svtkImageSincInterpolatorPrecomputeWeights(const F newmat[16], const int outExt[6],
  int clipExt[6], const F bounds[6], svtkInterpolationWeights* weights)
{
  ...
  sizes[2] =
    2 * ((mode & SVTK_INTERPOLATION_WINDOW_ZSIZE_MASK) >> SVTK_INTERPOLATION_WINDOW_ZSIZE_SHIFT);
  blur[0] = ((mode & SVTK_INTERPOLATION_WINDOW_XBLUR_MASK) != 0);
  blur[1] = ((mode & SVTK_INTERPOLATION_WINDOW_YBLUR_MASK) != 0);
  blur[2] = ((mode & SVTK_INTERPOLATION_WINDOW_ZBLUR_MASK) != 0);
  ...
  for (int j = 0; j < 3; j++)
  {
    ...
    for (int i = outExt[2 * j]; i <= outExt[2 * j + 1]; i++)
    {
      ...
      F f = 0;
      int idx = svtkInterpolationMath::Floor(point, f);
      int lmax = 1;
      if (step > 1)
      {
        idx -= m2;
      ...
      int inId[SVTK_SINC_KERNEL_SIZE_MAX];
      ...
      {
        F g[SVTK_SINC_KERNEL_SIZE_MAX];
        svtkSincInterpWeights(kernel[j], g, f, m);
        ...
        else
        {
          ...
          F gg[SVTK_SINC_KERNEL_SIZE_MAX];
          int ll = 0;
          do
          {
            gg[ll] = 0;
...
//----------------------------------------------------------------------------
void svtkImageSincInterpolator::GetInterpolationFunc(
  void (**func)(svtkInterpolationInfo*, const double[3], double*))
{
  svtkImageSincInterpolatorGetInterpolationFunc(
...
//----------------------------------------------------------------------------
void svtkImageSincInterpolator::GetRowInterpolationFunc(
  void (**func)(svtkInterpolationWeights*, int, int, int, double*, int))
{
  svtkImageSincInterpolatorGetRowInterpolationFunc(
    func, this->InterpolationInfo->ScalarType, this->WindowFunction);
}
...
//----------------------------------------------------------------------------
void svtkImageSincInterpolator::PrecomputeWeightsForExtent(
  const double matrix[16], const int extent[6], int newExtent[6], svtkInterpolationWeights*& weights)
{
  ...
  svtkImageSincInterpolatorPrecomputeWeights(
...
//----------------------------------------------------------------------------
void svtkImageSincInterpolator::FreePrecomputedWeights(svtkInterpolationWeights*& weights)
{
...
void svtkImageSincInterpolator::BuildKernelLookupTable()
{
  if (this->KernelLookupTable[0])
  {
    this->FreeKernelLookupTable();
  ...
  for (int i = 0; i < 3; i++)
  {
    ...
    if (i > 0 && this->KernelSize[i] == this->KernelSize[0] &&
      fabs(this->BlurFactors[i] - this->BlurFactors[0]) < SVTK_INTERPOLATE_FLOOR_TOL)
    {
      kernel[i] = kernel[0];
      continue;
    }
    ...
    while (n > 1 && 2 * n * b > static_cast<double>(SVTK_SINC_KERNEL_SIZE_MAX))
    {
      --n;
      m = SVTK_SINC_KERNEL_SIZE_MAX;
    }

    // blur factor must be restricted to half the max kernel size
    if (b > 0.5 * SVTK_SINC_KERNEL_SIZE_MAX)
    {
      b = 0.5 * SVTK_SINC_KERNEL_SIZE_MAX;
    }

    // compute lookup table size and step size
    int size = m / 2 * SVTK_SINC_KERNEL_TABLE_DIVISIONS;
    double p = 1.0 / (b * n * SVTK_SINC_KERNEL_TABLE_DIVISIONS);
    ...
    {
      case SVTK_LANCZOS_WINDOW:
        svtkSincKernel::Lanczos(kernel[i], size, n, p);
        break;
      case SVTK_KAISER_WINDOW:
        svtkSincKernel::Kaiser(kernel[i], size, n, p, a);
        break;
      case SVTK_COSINE_WINDOW:
        svtkSincKernel::Cosine(kernel[i], size, n, p);
        break;
      case SVTK_HANN_WINDOW:
        svtkSincKernel::Hamming<float, 2>(kernel[i], size, n, p, hann);
        break;
      case SVTK_HAMMING_WINDOW:
        svtkSincKernel::Hamming<float, 2>(kernel[i], size, n, p, hamming);
        break;
      case SVTK_BLACKMAN_WINDOW:
        svtkSincKernel::Hamming<float, 3>(kernel[i], size, n, p, blackman);
        break;
      case SVTK_BLACKMAN_HARRIS3:
        svtkSincKernel::Hamming<float, 3>(kernel[i], size, n, p, harris3);
        break;
      case SVTK_BLACKMAN_HARRIS4:
        svtkSincKernel::Hamming<float, 4>(kernel[i], size, n, p, harris4);
        break;
      case SVTK_NUTTALL_WINDOW:
        svtkSincKernel::Hamming<float, 4>(kernel[i], size, n, p, nuttall);
        break;
      case SVTK_BLACKMAN_NUTTALL3:
        svtkSincKernel::Hamming<float, 3>(kernel[i], size, n, p, nuttall3);
        break;
      case SVTK_BLACKMAN_NUTTALL4:
        svtkSincKernel::Hamming<float, 4>(kernel[i], size, n, p, nuttall4);
        break;
    }
    ...
    {
      svtkRenormalizeKernel(kernel[i], SVTK_SINC_KERNEL_TABLE_DIVISIONS, m);
    }
    else if (b > 1.0)
    {
...
//----------------------------------------------------------------------------
void svtkImageSincInterpolator::FreeKernelLookupTable()
{
