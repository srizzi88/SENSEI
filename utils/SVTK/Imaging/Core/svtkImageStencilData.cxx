/*=========================================================================
  ...
  Module:    svtkImageStencilData.cxx
...
svtkStandardNewMacro(svtkImageStencilData);
...
int svtkImageStencilDataIndex(const int extent[6], int yIdx, int zIdx)
{
  int yMin = extent[2];
  int yMax = extent[3];
  int zMin = extent[4];
...
void svtkImageStencilDataInsertNextExtent(
...
struct svtkImageStencilDataOrFunctor
{
  svtkImageStencilDataOrFunctor(bool notA, bool notB)
    : not1(notA)
    , not2(notB)
  {
  }
...
// hints that a "not" operation should be applied to the operand before
// the functor is called.
struct svtkImageStencilDataAndFunctor
{
  svtkImageStencilDataAndFunctor(bool notA, bool notB)
    : not1(notA)
    , not2(notB)
  {
  }
...
// the range [ext1, ext2].
template <typename F>
void svtkImageStencilDataBoolean(int* clist1, int clistlen1, int* clist2, int clistlen2, int*& clist,
  int& clistlen, int* clistsmall, F operation, int ext1, int ext2)
{
  ...
  while (rnext != rlast)
  {
    ...
    if (value)
    {
      svtkImageStencilDataInsertNextExtent(r, rnext - 1, clist, clistlen, clistsmall);
    }
  }
}

// Clip the sub-extents in "clist" to the range [ext1,ext2]
void svtkImageStencilDataClipExtent(int ext1, int ext2, int* clist, int& clistlen)
{
...
//----------------------------------------------------------------------------
svtkImageStencilData::svtkImageStencilData()
{
  this->Spacing[0] = 1;
  this->Spacing[1] = 1;
  this->Spacing[2] = 1;
...
//----------------------------------------------------------------------------
void svtkImageStencilData::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
void svtkImageStencilData::Initialize()
{
  if (this->ExtentLists)
  {
...
//----------------------------------------------------------------------------
void svtkImageStencilData::CopyInformationFromPipeline(svtkInformation* info)
{
...
//----------------------------------------------------------------------------
void svtkImageStencilData::CopyInformationToPipeline(svtkInformation* info)
{
...
//----------------------------------------------------------------------------
void svtkImageStencilData::CopyOriginAndSpacingFromPipeline(svtkInformation* info)
{
...
//----------------------------------------------------------------------------
void svtkImageStencilData::SetExtent(const int extent[6])
{
  for (int i = 0; i < 6; i++)
  {
...
//----------------------------------------------------------------------------
void svtkImageStencilData::SetExtent(int x1, int x2, int y1, int y2, int z1, int z2)
{
...
//----------------------------------------------------------------------------
void svtkImageStencilData::ShallowCopy(svtkDataObject* o)
{
  svtkImageStencilData* s = svtkImageStencilData::SafeDownCast(o);
  ...
  svtkDataObject::ShallowCopy(o);
}

//----------------------------------------------------------------------------
void svtkImageStencilData::DeepCopy(svtkDataObject* o)
{
  svtkImageStencilData* s = svtkImageStencilData::SafeDownCast(o);
  ...
  svtkDataObject::DeepCopy(o);
}

//----------------------------------------------------------------------------
void svtkImageStencilData::InternalImageStencilDataCopy(svtkImageStencilData* s)
{
...
void svtkImageStencilData::ChangeExtent(const int extent[6])
{
  int oldExtent[6];
    ...
    for (int idz = oldExtent[4]; idz <= oldExtent[5]; idz++)
    {
      for (int idy = oldExtent[2]; idy <= oldExtent[3]; idy++)
      {
        if (idy >= extent[2] && idy <= extent[3] && idz >= extent[4] && idz <= extent[5])
        {
          ...
          {
            svtkImageStencilDataClipExtent(extent[0], extent[1], lists[k], listLengths[k]);
          }
...
//----------------------------------------------------------------------------
void svtkImageStencilData::AllocateExtents()
{
  int extent[6];
  this->GetExtent(extent);
...
int svtkImageStencilData::GetNextExtent(
...
int svtkImageStencilData::IsInside(int xIdx, int yIdx, int zIdx)
{
...
void svtkImageStencilData::Fill()
{
...
void svtkImageStencilData::InsertNextExtent(int r1, int r2, int yIdx, int zIdx)
{
  // calculate the index into the extent array
  int incr = svtkImageStencilDataIndex(this->Extent, yIdx, zIdx);

  svtkImageStencilDataInsertNextExtent(r1, r2, this->ExtentLists[incr],
    this->ExtentListLengths[incr],
    &this->ExtentListLengths[this->NumberOfExtentEntries + 2 * incr]);
}

//----------------------------------------------------------------------------
void svtkImageStencilData::LogicalOperationExtent(
  int r1, int r2, int yIdx, int zIdx, Operation operation)
{
  ...
  if (operation == Merge)
  {
    svtkImageStencilDataBoolean(clist1, clistlen1, clist2, clistlen2, clist, clistlen, clistsmall,
      svtkImageStencilDataOrFunctor(false, false), this->Extent[0], this->Extent[1]);
  }
  else if (operation == Erase)
  {
    svtkImageStencilDataBoolean(clist1, clistlen1, clist2, clistlen2, clist, clistlen, clistsmall,
      svtkImageStencilDataAndFunctor(false, true), this->Extent[0], this->Extent[1]);
  }
...
//----------------------------------------------------------------------------
void svtkImageStencilData::InsertAndMergeExtent(int r1, int r2, int yIdx, int zIdx)
{
  this->LogicalOperationExtent(r1, r2, yIdx, zIdx, Merge);
}

//----------------------------------------------------------------------------
void svtkImageStencilData::RemoveExtent(int r1, int r2, int yIdx, int zIdx)
{
  this->LogicalOperationExtent(r1, r2, yIdx, zIdx, Erase);
}

//----------------------------------------------------------------------------
svtkImageStencilData* svtkImageStencilData::GetData(svtkInformation* info)
{
  return info ? svtkImageStencilData::SafeDownCast(info->Get(DATA_OBJECT())) : nullptr;
}

//----------------------------------------------------------------------------
svtkImageStencilData* svtkImageStencilData::GetData(svtkInformationVector* v, int i)
{
  return svtkImageStencilData::GetData(v->GetInformationObject(i));
}

//----------------------------------------------------------------------------
void svtkImageStencilData::LogicalOperationInPlace(svtkImageStencilData* stencil, Operation operation)
{
  ...
  for (int idz = extent[4]; idz <= extent[5]; idz++)
  {
    for (int idy = extent[2]; idy <= extent[3]; idy++)
    {
      int incr = svtkImageStencilDataIndex(stencil->Extent, idy, idz);
      int clistlen2 = stencil->ExtentListLengths[incr];
      int* clist2 = stencil->ExtentLists[incr];

      incr = svtkImageStencilDataIndex(this->Extent, idy, idz);
      int& clistlen = this->ExtentListLengths[incr];
      int*& clist = this->ExtentLists[incr];
      int* clistsmall = &this->ExtentListLengths[this->NumberOfExtentEntries + 2 * incr];
      ...
      if (operation == Merge)
      {
        svtkImageStencilDataBoolean(clist1, clistlen1, clist2, clistlen2, clist, clistlen,
          clistsmall, svtkImageStencilDataOrFunctor(false, false), this->Extent[0], this->Extent[1]);
      }
      else if (operation == Erase)
      {
        svtkImageStencilDataBoolean(clist1, clistlen1, clist2, clistlen2, clist, clistlen,
          clistsmall, svtkImageStencilDataAndFunctor(false, true), this->Extent[0], this->Extent[1]);
      }
...
//----------------------------------------------------------------------------
void svtkImageStencilData::Add(svtkImageStencilData* stencil1)
{
...
//----------------------------------------------------------------------------
void svtkImageStencilData::Subtract(svtkImageStencilData* stencil1)
{
...
//----------------------------------------------------------------------------
void svtkImageStencilData::Replace(svtkImageStencilData* stencil1)
{
...
//----------------------------------------------------------------------------
int svtkImageStencilData::Clip(int extent[6])
{
  int currentExtent[6];
  this->GetExtent(currentExtent);

  if (svtkMath::ExtentIsWithinOtherExtent(currentExtent, extent))
  {
    ...
    for (int idy = currentExtent[2]; idy <= currentExtent[3]; idy++)
    {
      if (idy >= extent[2] && idy <= extent[3] && idz >= extent[4] && idz <= extent[5])
      {
        if (extent[0] > currentExtent[0] || extent[1] < currentExtent[1])
        {
          ...
          {
            svtkImageStencilDataClipExtent(extent[0], extent[1], lists[k], l);
            listLengths[k] = l;
            modified = true;
          }
        }
...
//----------------------------------------------------------------------------
svtkImageStencilRaster::svtkImageStencilRaster(const int extent[2])
{
  ...
  this->Tolerance = SVTK_STENCIL_TOL;
...
//----------------------------------------------------------------------------
svtkImageStencilRaster::~svtkImageStencilRaster()
{
...
//----------------------------------------------------------------------------
void svtkImageStencilRaster::PrepareForNewData(const int allocateExtent[2])
{
  if (this->UsedExtent[1] >= this->UsedExtent[0])
  {
...
//----------------------------------------------------------------------------
void svtkImageStencilRaster::PrepareExtent(int ymin, int ymax)
{
...
//----------------------------------------------------------------------------
void svtkImageStencilRaster::InsertPoint(int y, double x, int i)
{
...
//----------------------------------------------------------------------------
void svtkImageStencilRaster::InsertLine(const double pt1[2], const double pt2[2])
{
  double x1 = pt1[0];
  double x2 = pt2[0];
  double y1 = pt1[1];
  ...
  for (int i = 0; i < patternCount; i++)
  {
    ...
    if (ymin[i] >= iy1)
    {
      iy1 = svtkMath::Floor(ymin[i]) + 1;
    }
    if (ymax[i] < iy2)
    {
      iy2 = svtkMath::Floor(ymax[i]);
    }
...
//----------------------------------------------------------------------------
void svtkImageStencilRaster::InsertLine(const double pt1[2], const double pt2[2], bool, bool)
{
  SVTK_LEGACY_REPLACED_BODY(svtkImageStencilRaster::InsertLine(double[2], double[2], bool, bool),
    "SVTK 6.2", svtkImageStencilRaster::InsertLine(double[2], double[2]));
...
//----------------------------------------------------------------------------
void svtkImageStencilRaster::FillStencilData(
  svtkImageStencilData* data, const int extent[6], int xj, int yj)
{
  if (xj != 0)
  {
    ...
    for (int idY = ymin; idY <= ymax; idY++)
    {
      ...
      int lastr = SVTK_INT_MIN;
      for (;;)
      {
        // find the span with the lowest lower bound
        double x1 = SVTK_DOUBLE_MAX;
        int j = -1;
        for (int i = 0; i < 2; i++)
        {
        ...
        if (x1 >= xmin)
        {
          r1 = svtkMath::Floor(x1) + 1;
        }
        if (x2 < xmax)
        {
          r2 = svtkMath::Floor(x2);
        }
