...
svtkStandardNewMacro(svtkImageWrapPad);

//----------------------------------------------------------------------------
// Just clip the request.
void svtkImageWrapPad::ComputeInputUpdateExtent(int inExt[6], int outExt[6], int wholeExtent[6])
{
  int idx;
...
template <class T>
void svtkImageWrapPadExecute(svtkImageWrapPad* self, svtkImageData* inData, T* svtkNotUsed(inPtr),
  svtkImageData* outData, T* outPtr, int outExt[6], int id, int wholeExt[6])
{
  int min0, max0;
  ...
  for (outIdx2 = outExt[4]; outIdx2 <= outExt[5]; ++outIdx2, ++inIdx2)
  {
    ...
    inPtr1 = inPtr2;
    inIdx1 = start1;
    for (outIdx1 = outExt[2]; !self->AbortExecute && outIdx1 <= outExt[3]; ++outIdx1, ++inIdx1)
    {
      if (!id)
      {
        if (!(count % target))
...
void svtkImageWrapPad::ThreadedRequestData(svtkInformation* svtkNotUsed(request),
  ...
  svtkImageData*** inData, svtkImageData** outData, int outExt[6], int id)
{
  ...
  {
    svtkTemplateMacro(svtkImageWrapPadExecute(this, inData[0][0], static_cast<SVTK_TT*>(inPtr),
