/*=========================================================================
  ...
  Module:    svtkImageFourierCenter.cxx
...
=========================================================================*/
#include "svtkImageFourierCenter.h"
...
svtkStandardNewMacro(svtkImageFourierCenter);

//----------------------------------------------------------------------------
// Construct an instance of svtkImageFourierCenter filter.
svtkImageFourierCenter::svtkImageFourierCenter() = default;

//----------------------------------------------------------------------------
// This method tells the superclass which input extent is needed.
// This gets the whole input (even though it may not be needed).
int svtkImageFourierCenter::IterativeRequestUpdateExtent(
...
void svtkImageFourierCenter::ThreadedRequestData(svtkInformation* svtkNotUsed(request),
  ...
  svtkImageData*** inDataVec, svtkImageData** outDataVec, int outExt[6], int threadId)
{
  ...
  if (inData->GetScalarType() != SVTK_DOUBLE)
  {
    svtkErrorMacro(<< "Execute: Input must be type double.");
    return;
  }
  ...
  wholeExtent =
    outputVector->GetInformationObject(0)->Get(svtkStreamingDemandDrivenPipeline::WHOLE_EXTENT());
