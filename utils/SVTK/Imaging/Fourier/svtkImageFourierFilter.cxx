/*=========================================================================
  ...
  Module:    svtkImageFourierFilter.cxx
...
void svtkImageFourierFilter::ExecuteFftStep2(
  svtkImageComplex* p_in, svtkImageComplex* p_out, int N, int bsize, int fb)
{
  int i1, i2;
  svtkImageComplex *p1, *p2, *p3;
  svtkImageComplex q, fact1, fact, temp;
  ...
  fact1.Imag = 0.0;
  q.Real = 0.0;
  q.Imag = -(2.0 * svtkMath::Pi()) * fb / (bsize * 2.0);
  svtkImageComplexExponential(q, q);
  p3 = p_out;
  for (i1 = 0; i1 < N / (bsize * 2); ++i1)
  {
    ...
    for (i2 = 0; i2 < bsize; ++i2)
    {
      svtkImageComplexMultiply(fact, *p2, temp);
      svtkImageComplexAdd(temp, *p3, *p3);
      svtkImageComplexMultiply(q, fact, fact);
  ...
  svtkImageComplex* p_in, svtkImageComplex* p_out, int N, int bsize, int n, int fb)
{
  ...
  {
    q.Real = 0.0;
    q.Imag = -(2.0 * svtkMath::Pi()) * i0 * fb / (bsize * 1.0 * n);
    svtkImageComplexExponential(q, q);
    p3 = p_out;
...
// (fb = 1) => fft, (fb = -1) => rfft;
void svtkImageFourierFilter::ExecuteFftForwardBackward(
  svtkImageComplex* in, svtkImageComplex* out, int N, int fb)
{
  svtkImageComplex *p1, *p2, *p3;
  int block_size = 1;
  int rest_size = N;
...
// The contents of the input array are changed.
// (It is engineered for no decimation)
void svtkImageFourierFilter::ExecuteFft(svtkImageComplex* in, svtkImageComplex* out, int N)
{
...
// The contents of the input array are changed.
// (It is engineered for no decimation)
void svtkImageFourierFilter::ExecuteRfft(svtkImageComplex* in, svtkImageComplex* out, int N)
{
  this->ExecuteFftForwardBackward(in, out, N, -1);
}
...
int svtkImageFourierFilter::RequestData(
