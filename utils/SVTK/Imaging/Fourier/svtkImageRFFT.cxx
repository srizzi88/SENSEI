...
svtkStandardNewMacro(svtkImageRFFT);

//----------------------------------------------------------------------------
// This extent of the components changes to real and imaginary values.
int svtkImageRFFT::IterativeRequestInformation(
...
static void svtkImageRFFTInternalRequestUpdateExtent(
...
int svtkImageRFFT::IterativeRequestUpdateExtent(svtkInformation* input, svtkInformation* output)
{
  ...
  int inExt[6];
  svtkImageRFFTInternalRequestUpdateExtent(inExt, outExt, wExt, this->Iteration);
...
// is always doubles.
template <class T>
void svtkImageRFFTExecute(svtkImageRFFT* self, svtkImageData* inData, int inExt[6], T* inPtr,
...
// Not threaded yet.
void svtkImageRFFT::ThreadedRequestData(svtkInformation* svtkNotUsed(request),
  ...
  svtkImageData*** inDataVec, svtkImageData** outDataVec, int outExt[6], int threadId)
{
  ...
  {
    svtkTemplateMacro(svtkImageRFFTExecute(this, inData, inExt, static_cast<SVTK_TT*>(inPtr), outData,
