/*=========================================================================
  ...
  Module:    svtkImageAnisotropicDiffusion3D.cxx
...
=========================================================================*/
#include "svtkImageAnisotropicDiffusion3D.h"
...
svtkStandardNewMacro(svtkImageAnisotropicDiffusion3D);

//----------------------------------------------------------------------------
// Construct an instance of svtkImageAnisotropicDiffusion3D filter.
svtkImageAnisotropicDiffusion3D::svtkImageAnisotropicDiffusion3D()
{
...
//----------------------------------------------------------------------------
void svtkImageAnisotropicDiffusion3D::PrintSelf(ostream& os, svtkIndent indent)
{
...
void svtkImageAnisotropicDiffusion3D::SetNumberOfIterations(int num)
{
  int temp;

  svtkDebugMacro(<< "SetNumberOfIterations: " << num);
...
void svtkImageAnisotropicDiffusion3D::ThreadedRequestData(svtkInformation* svtkNotUsed(request),
  ...
  svtkImageData*** inData, svtkImageData** outData, int outExt[6], int id)
{
  ...
  svtkImageData* in = svtkImageData::New();
  in->SetExtent(inExt);
  in->AllocateScalars(SVTK_DOUBLE, inData[0][0]->GetNumberOfScalarComponents());
  in->CopyAndCastFrom(inData[0][0], inExt);

  svtkImageData* out = svtkImageData::New();
  out->SetExtent(inExt);
  out->AllocateScalars(SVTK_DOUBLE, inData[0][0]->GetNumberOfScalarComponents());
...
void svtkImageAnisotropicDiffusion3D::Iterate(svtkImageData* inData, svtkImageData* outData,
  double ar0, double ar1, double ar2, int* coreExtent, int count)
{
  ...
  // Avoid the warnings.
  th0 = th1 = th2 = th01 = th02 = th12 = th012 = df0 = df1 = df2 = df01 = df02 = df12 = df012 = 0.0;
  ...
  {
    svtkWarningMacro(<< "Iterate: NO NEIGHBORS");
    return;
  }
  ...
  svtkDebugMacro(<< "Iteration count: " << count << " (" << min0 << ", " << max0 << ", " << min1
                << ", " << max1 << ", " << min2 << ", " << max2 << ")");
        ...
        for (idx0 = min0; idx0 <= max0; ++idx0, inPtr0 += inInc0, outPtr0 += outInc0)
        {
          ...
          *outPtr0 = *inPtr0;
          ...
          if (this->GradientMagnitudeThreshold)
          {
            ...
            {
              // hack to diffuse
              th0 = th1 = th2 = th01 = th02 = th12 = th012 = SVTK_DOUBLE_MAX;
            }
          }
