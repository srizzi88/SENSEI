/*=========================================================================
  ...
  Module:    svtkImageCorrelation.cxx
...
=========================================================================*/
#include "svtkImageCorrelation.h"
...
svtkStandardNewMacro(svtkImageCorrelation);

//----------------------------------------------------------------------------
svtkImageCorrelation::svtkImageCorrelation()
{
...
int svtkImageCorrelation::RequestInformation(svtkInformation* svtkNotUsed(request),
...
//----------------------------------------------------------------------------
// Grow
int svtkImageCorrelation::RequestUpdateExtent(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  int inWExt2[6];
  inInfo2->Get(svtkStreamingDemandDrivenPipeline::WHOLE_EXTENT(), inWExt2);
  inInfo2->Set(svtkStreamingDemandDrivenPipeline::UPDATE_EXTENT(), inWExt2, 6);

  int inWExt1[6];
  inInfo1->Get(svtkStreamingDemandDrivenPipeline::WHOLE_EXTENT(), inWExt1);

  // try to get all the data required to handle the boundaries
  // but limit to the whole extent
  int idx;
  int inUExt1[6];
  outInfo->Get(svtkStreamingDemandDrivenPipeline::UPDATE_EXTENT(), inUExt1);
  ...
  inInfo1->Set(svtkStreamingDemandDrivenPipeline::UPDATE_EXTENT(), inUExt1, 6);
...
template <class T>
void svtkImageCorrelationExecute(svtkImageCorrelation* self, svtkImageData* in1Data, T* in1Ptr,
  svtkImageData* in2Data, T* in2Ptr, svtkImageData* outData, float* outPtr, int outExt[6], int id,
  int in2Extent[6])
{
  int idxC, idxX, idxY, idxZ;
  int maxC, maxX, maxY, maxZ;
  svtkIdType in1IncX, in1IncY, in1IncZ;
  svtkIdType in1CIncX, in1CIncY, in1CIncZ;
        ...
        for (kIdxZ = 0; kIdxZ <= zKernMax; kIdxZ++)
        {
          for (kIdxY = 0; kIdxY <= yKernMax; kIdxY++)
          {
            ...
            for (kIdxX = 0; kIdxX <= xKernMax; kIdxX++)
            {
              ...
              {
                *outPtr = *outPtr + static_cast<float>((*in1Ptr2) * (*in2Ptr2));
                in1Ptr2++;
                in2Ptr2++;
              }
            }
...
void svtkImageCorrelation::ThreadedRequestData(svtkInformation* svtkNotUsed(request),
  ...
  svtkImageData*** inData, svtkImageData** outData, int outExt[6], int id)
{
  ...
  in2Extent =
    inputVector[1]->GetInformationObject(0)->Get(svtkStreamingDemandDrivenPipeline::WHOLE_EXTENT());
  ...
  if (inData[0][0]->GetScalarType() != inData[1][0]->GetScalarType())
  {
    svtkErrorMacro(<< "Execute: input ScalarType, " << inData[0][0]->GetScalarType()
                  << " and input2 ScalarType " << inData[1][0]->GetScalarType()
                  << ", should match");
    return;
  }
  ...
  {
    svtkErrorMacro(<< "Execute: input depths must match");
    return;
  }

  switch (inData[0][0]->GetScalarType())
  {
    svtkTemplateMacro(svtkImageCorrelationExecute(this, inData[0][0], static_cast<SVTK_TT*>(in1Ptr),
      inData[1][0], static_cast<SVTK_TT*>(in2Ptr), outData[0], outPtr, outExt, id, in2Extent));
    default:
...
void svtkImageCorrelation::PrintSelf(ostream& os, svtkIndent indent)
{
