/*=========================================================================
  ...
  Module:    svtkImageEuclideanDistance.cxx
...
=========================================================================*/
#include "svtkImageEuclideanDistance.h"
...
svtkStandardNewMacro(svtkImageEuclideanDistance);

//----------------------------------------------------------------------------
// This defines the default values for the EDT parameters
svtkImageEuclideanDistance::svtkImageEuclideanDistance()
{
  this->MaximumDistance = SVTK_INT_MAX;
  this->Initialize = 1;
  this->ConsiderAnisotropy = 1;
  this->Algorithm = SVTK_EDT_SAITO;
}

//----------------------------------------------------------------------------
// This extent of the components changes to real and imaginary values.
int svtkImageEuclideanDistance::IterativeRequestInformation(
  svtkInformation* svtkNotUsed(input), svtkInformation* output)
{
  svtkDataObject::SetPointDataActiveScalarInfo(output, SVTK_DOUBLE, 1);
  return 1;
}
...
int svtkImageEuclideanDistance::IterativeRequestUpdateExtent(
  svtkInformation* input, svtkInformation* svtkNotUsed(output))
{
  int* wExt = input->Get(svtkStreamingDemandDrivenPipeline::WHOLE_EXTENT());
  input->Set(svtkStreamingDemandDrivenPipeline::UPDATE_EXTENT(), wExt, 6);
...
// is always doubles.
template <class TT>
void svtkImageEuclideanDistanceCopyData(svtkImageEuclideanDistance* self, svtkImageData* inData,
  TT* inPtr, svtkImageData* outData, int outExt[6], double* outPtr)
{
...
// is always doubles.
template <class T>
void svtkImageEuclideanDistanceInitialize(svtkImageEuclideanDistance* self, svtkImageData* inData,
  T* inPtr, svtkImageData* outData, int outExt[6], double* outPtr)
{
  svtkIdType inInc0, inInc1, inInc2;
  ...
  {
    svtkImageEuclideanDistanceCopyData(
...
//
static void svtkImageEuclideanDistanceExecuteSaito(
  svtkImageEuclideanDistance* self, svtkImageData* outData, int outExt[6], double* outPtr)
{
...
//
static void svtkImageEuclideanDistanceExecuteSaitoCached(
  svtkImageEuclideanDistance* self, svtkImageData* outData, int outExt[6], double* outPtr)
{
...
//----------------------------------------------------------------------------
void svtkImageEuclideanDistance::AllocateOutputScalars(
  svtkImageData* outData, int outExt[6], svtkInformation* outInfo)
{
...
int svtkImageEuclideanDistance::IterativeRequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  int outExt[6];
  outInfo->Get(svtkStreamingDemandDrivenPipeline::WHOLE_EXTENT(), outExt);
  this->AllocateOutputScalars(outData, outExt, outInfo);

  void* inPtr;
  void* outPtr;

  svtkDebugMacro(<< "Executing image euclidean distance");

  inPtr = inData->GetScalarPointerForExtent(
    inInfo->Get(svtkStreamingDemandDrivenPipeline::UPDATE_EXTENT()));
  outPtr = outData->GetScalarPointer();

  if (!inPtr)
  {
    svtkErrorMacro(<< "Execute: No scalars for update extent.");
    return 1;
  }
  ...
  {
    svtkErrorMacro(<< "Execute: Cannot handle more than 1 components");
    return 1;
  }
  ...
  if (this->GetIteration() == 0)
  {
    ...
    {
      svtkTemplateMacro(svtkImageEuclideanDistanceInitialize(
        this, inData, static_cast<SVTK_TT*>(inPtr), outData, outExt, static_cast<double*>(outPtr)));
      default:
  ...
  else
  {
    if (inData != outData)
      switch (inData->GetScalarType())
      {
        svtkTemplateMacro(svtkImageEuclideanDistanceCopyData(this, inData,
          static_cast<SVTK_TT*>(inPtr), outData, outExt, static_cast<double*>(outPtr)));
      }
  }
  ...
  {
    case SVTK_EDT_SAITO:
      svtkImageEuclideanDistanceExecuteSaito(this, outData, outExt, static_cast<double*>(outPtr));
      break;
    case SVTK_EDT_SAITO_CACHED:
      svtkImageEuclideanDistanceExecuteSaitoCached(
        this, outData, outExt, static_cast<double*>(outPtr));
      break;
    default:
      svtkErrorMacro(<< "Execute: Unknown Algorithm");
  }
...
//----------------------------------------------------------------------------
void svtkImageEuclideanDistance::PrintSelf(ostream& os, svtkIndent indent)
{
  ...
  os << indent << "Algorithm: ";
  if (this->Algorithm == SVTK_EDT_SAITO)
  {
    os << "Saito\n";
  }
  else
