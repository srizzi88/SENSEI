/*=========================================================================
  ...
  Module:    svtkImageGaussianSmooth.cxx
...
svtkStandardNewMacro(svtkImageGaussianSmooth);

//----------------------------------------------------------------------------
svtkImageGaussianSmooth::svtkImageGaussianSmooth()
{
...
//----------------------------------------------------------------------------
svtkImageGaussianSmooth::~svtkImageGaussianSmooth() = default;

//----------------------------------------------------------------------------
void svtkImageGaussianSmooth::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
void svtkImageGaussianSmooth::ComputeKernel(double* kernel, int min, int max, double std)
{
  int x;
  double sum;
...
//----------------------------------------------------------------------------
int svtkImageGaussianSmooth::RequestUpdateExtent(svtkInformation* svtkNotUsed(request),
...
//----------------------------------------------------------------------------
void svtkImageGaussianSmooth::InternalRequestUpdateExtent(int* inExt, int* wholeExtent)
{
  int idx, radius;
...
// previously.
template <class T>
void svtkImageGaussianSmoothExecute(svtkImageGaussianSmooth* self, int axis, double* kernel,
  int kernelSize, svtkImageData* inData, T* inPtrC, svtkImageData* outData, int outExt[6], T* outPtrC,
  int* pcycle, int target, int* pcount, int total)
{
  int maxC, max0 = 0, max1 = 0;
  int idxC, idx0, idx1, idxK;
  svtkIdType inIncs[3], outIncs[3];
  svtkIdType inInc0 = 0, inInc1 = 0, inIncK, outInc0 = 0, outInc1 = 0;
  T *outPtr1, *outPtr0;
...
//----------------------------------------------------------------------------
template <class T>
size_t svtkImageGaussianSmoothGetTypeSize(T*)
{
  return sizeof(T);
}
...
void svtkImageGaussianSmooth::ExecuteAxis(int axis, svtkImageData* inData, int inExt[6],
  svtkImageData* outData, int outExt[6], int* pcycle, int target, int* pcount, int total,
  svtkInformation* inInfo)
{
  ...
  {
    svtkTemplateMacro(outIncA *=
      static_cast<svtkIdType>(svtkImageGaussianSmoothGetTypeSize(static_cast<SVTK_TT*>(nullptr))));
    default:
  ...
  for (idxA = outExt[axis * 2]; idxA <= max; ++idxA)
  {
    ...
    {
      svtkTemplateMacro(svtkImageGaussianSmoothExecute(this, axis, kernel, kernelSize, inData,
        static_cast<SVTK_TT*>(inPtr), outData, outExt, static_cast<SVTK_TT*>(outPtr), pcycle, target,
        pcount, total));
      default:
        svtkErrorMacro("Unknown scalar type");
        return;
...
void svtkImageGaussianSmooth::ThreadedRequestData(svtkInformation* svtkNotUsed(request),
  ...
  svtkImageData** outData, int outExt[6], int id)
{
  ...
  switch (this->Dimensionality)
  {
    ...
    case 3:
      ...
      svtkImageData *temp0Data, *temp1Data;
      ...
      // create a temp data for intermediate results
      temp0Data = svtkImageData::New();
      temp0Data->SetExtent(temp0Ext);
      temp0Data->AllocateScalars(
        inData[0][0]->GetScalarType(), inData[0][0]->GetNumberOfScalarComponents());

      temp1Data = svtkImageData::New();
