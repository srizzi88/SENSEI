...
svtkStandardNewMacro(svtkImageGradient);

//----------------------------------------------------------------------------
// Construct an instance of svtkImageGradient filter.
svtkImageGradient::svtkImageGradient()
{
...
//----------------------------------------------------------------------------
void svtkImageGradient::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
int svtkImageGradient::RequestInformation(
  svtkInformation*, svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  svtkDataObject::SetPointDataActiveScalarInfo(outInfo, SVTK_DOUBLE, this->Dimensionality);
...
int svtkImageGradient::RequestUpdateExtent(
  svtkInformation*, svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  // Get input and output pipeline information.
  svtkInformation* outInfo = outputVector->GetInformationObject(0);
  ...
  int ugl = outInfo->Get(svtkStreamingDemandDrivenPipeline::UPDATE_NUMBER_OF_GHOST_LEVELS());
  inInfo->Set(svtkStreamingDemandDrivenPipeline::UPDATE_NUMBER_OF_GHOST_LEVELS(), ugl + 1);
...
// out of extent.
template <class T>
void svtkImageGradientExecute(svtkImageGradient* self, svtkImageData* inData, T* inPtr,
  svtkImageData* outData, double* outPtr, int outExt[6], int id)
{
  int idxX, idxY, idxZ;
  int maxX, maxY, maxZ;
...
int svtkImageGradient::RequestData(
  svtkInformation* request, svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  svtkImageData* input = svtkImageData::GetData(inputVector[0]);
  svtkInformation* outInfo = outputVector->GetInformationObject(0);
  int ue[6];
  outInfo->Get(svtkStreamingDemandDrivenPipeline::UPDATE_EXTENT(), ue);
  int ue2[6];
  outInfo->Get(svtkStreamingDemandDrivenPipeline::UPDATE_EXTENT(), ue2);
  ...
  outInfo->Set(svtkStreamingDemandDrivenPipeline::UPDATE_EXTENT(), ue2, 6);

  if (!this->Superclass::RequestData(request, inputVector, outputVector))
  {
    return 0;
  }
  svtkImageData* output = svtkImageData::GetData(outputVector);
  svtkDataArray* outArray = output->GetPointData()->GetScalars();
  ...
  outInfo->Set(svtkStreamingDemandDrivenPipeline::UPDATE_EXTENT(), ue, 6);
  return 1;
}
...
// boundary conditions.
void svtkImageGradient::ThreadedRequestData(svtkInformation*, svtkInformationVector** inputVector,
  ...
  int threadId)
{
  ...
  if (output->GetScalarType() != SVTK_DOUBLE)
  {
    svtkErrorMacro(
  ...
  if (!inputArray)
  {
    svtkErrorMacro("No input array was found. Cannot execute");
    return;
  }
  ...
  {
    svtkTemplateMacro(svtkImageGradientExecute(
      this, input, static_cast<SVTK_TT*>(inPtr), output, outPtr, outExt, threadId));
    default:
      svtkErrorMacro("Execute: Unknown ScalarType " << input->GetScalarType());
      return;
  }
}
