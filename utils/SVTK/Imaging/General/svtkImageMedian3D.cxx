...
svtkStandardNewMacro(svtkImageMedian3D);

//-----------------------------------------------------------------------------
// Construct an instance of svtkImageMedian3D filter.
svtkImageMedian3D::svtkImageMedian3D()
{
...
//-----------------------------------------------------------------------------
svtkImageMedian3D::~svtkImageMedian3D() = default;

//-----------------------------------------------------------------------------
void svtkImageMedian3D::PrintSelf(ostream& os, svtkIndent indent)
{
...
void svtkImageMedian3D::SetKernelSize(int size0, int size1, int size2)
{
  int volume;
  int modified = 1;
...
template <class T>
T svtkComputeMedianOfArray(T* aBegin, T* aEnd)
{
...
template <class T>
void svtkImageMedian3DExecute(svtkImageMedian3D* self, svtkImageData* inData, T* inPtr,
  svtkImageData* outData, T* outPtr, int outExt[6], int id, svtkDataArray* inArray)
{
    ...
    for (outIdx1 = outExt[2]; !self->AbortExecute && outIdx1 <= outExt[3]; ++outIdx1)
    {
      ...
      for (outIdx0 = outExt[0]; outIdx0 <= outExt[1]; ++outIdx0)
      {
        for (outIdxC = 0; outIdxC < numComp; outIdxC++)
        {
          ...
          *outPtr++ = svtkComputeMedianOfArray(workArray, workEnd);
        }
...
void svtkImageMedian3D::ThreadedRequestData(svtkInformation* svtkNotUsed(request),
  ...
  svtkImageData*** inData, svtkImageData** outData, int outExt[6], int id)
{
  ...
  {
    svtkErrorMacro(<< "Execute: input data type, " << inArray->GetDataType()
  ...
  {
    svtkTemplateMacro(svtkImageMedian3DExecute(this, inData[0][0], static_cast<SVTK_TT*>(inPtr),
      outData[0], static_cast<SVTK_TT*>(outPtr), outExt, id, inArray));
    default:
