...
svtkStandardNewMacro(svtkImageSobel3D);

//----------------------------------------------------------------------------
// Construct an instance of svtkImageSobel3D filter.
svtkImageSobel3D::svtkImageSobel3D()
{
  this->KernelSize[0] = 3;
  this->KernelSize[1] = 3;
  this->KernelSize[2] = 3;
...
//----------------------------------------------------------------------------
void svtkImageSobel3D::PrintSelf(ostream& os, svtkIndent indent)
{
  this->Superclass::PrintSelf(os, indent);
}

//----------------------------------------------------------------------------
int svtkImageSobel3D::RequestInformation(
  svtkInformation* request, svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  int retval = this->Superclass::RequestInformation(request, inputVector, outputVector);
  svtkInformation* outInfo = outputVector->GetInformationObject(0);
  svtkDataObject::SetPointDataActiveScalarInfo(outInfo, SVTK_DOUBLE, 3);
  return retval;
}
...
// out of extent.
template <class T>
void svtkImageSobel3DExecute(svtkImageSobel3D* self, svtkImageData* inData, T* inPtr,
  svtkImageData* outData, int* outExt, double* outPtr, int id, svtkInformation* inInfo)
{
  double r0, r1, r2, *r;
  ...
  svtkIdType inInc0L, inInc0R, inInc1L, inInc1R, inInc2L, inInc2R;
  T *inPtrL, *inPtrR;
  double sum;
    ...
    for (outIdx1 = min1; !self->AbortExecute && outIdx1 <= max1; ++outIdx1)
    {
      ...
      for (outIdx0 = min0; outIdx0 <= max0; ++outIdx0)
      {
        ...
        sum += static_cast<double>(
          inPtrR[inInc1L] + inPtrR[inInc1R] + inPtrR[inInc2L] + inPtrR[inInc2R]);
        sum += static_cast<double>(0.586 *
          (inPtrR[inInc1L + inInc2L] + inPtrR[inInc1L + inInc2R] + inPtrR[inInc1R + inInc2L] +
            inPtrR[inInc1R + inInc2R]));
        sum -= static_cast<double>(
          inPtrL[inInc1L] + inPtrL[inInc1R] + inPtrL[inInc2L] + inPtrL[inInc2R]);
        sum -= static_cast<double>(0.586 *
          (inPtrL[inInc1L + inInc2L] + inPtrL[inInc1L + inInc2R] + inPtrL[inInc1R + inInc2L] +
            inPtrL[inInc1R + inInc2R]));
        *outPtrV = sum * r0;
        ++outPtrV;
        // 02 Plane
        ...
        sum = 2.0 * (*inPtrR - *inPtrL);
        sum += static_cast<double>(
          inPtrR[inInc0L] + inPtrR[inInc0R] + inPtrR[inInc2L] + inPtrR[inInc2R]);
        sum += static_cast<double>(0.586 *
          (inPtrR[inInc0L + inInc2L] + inPtrR[inInc0L + inInc2R] + inPtrR[inInc0R + inInc2L] +
            inPtrR[inInc0R + inInc2R]));
        sum -= static_cast<double>(
          inPtrL[inInc0L] + inPtrL[inInc0R] + inPtrL[inInc2L] + inPtrL[inInc2R]);
        sum -= static_cast<double>(0.586 *
          (inPtrL[inInc0L + inInc2L] + inPtrL[inInc0L + inInc2R] + inPtrL[inInc0R + inInc2L] +
            inPtrL[inInc0R + inInc2R]));
        *outPtrV = sum * r1;
        ++outPtrV;
        // 01 Plane
        ...
        sum = 2.0 * (*inPtrR - *inPtrL);
        sum += static_cast<double>(
          inPtrR[inInc0L] + inPtrR[inInc0R] + inPtrR[inInc1L] + inPtrR[inInc1R]);
        sum += static_cast<double>(0.586 *
          (inPtrR[inInc0L + inInc1L] + inPtrR[inInc0L + inInc1R] + inPtrR[inInc0R + inInc1L] +
            inPtrR[inInc0R + inInc1R]));
        sum -= static_cast<double>(
          inPtrL[inInc0L] + inPtrL[inInc0R] + inPtrL[inInc1L] + inPtrL[inInc1R]);
        sum -= static_cast<double>(0.586 *
...
void svtkImageSobel3D::ThreadedRequestData(svtkInformation* svtkNotUsed(request),
  ...
  svtkImageData*** inData, svtkImageData** outData, int outExt[6], int id)
{
  ...
  {
    svtkWarningMacro("Expecting input with only one component.\n");
  }
  ...
  {
    svtkTemplateMacro(svtkImageSobel3DExecute(this, inData[0][0], static_cast<SVTK_TT*>(inPtr),
