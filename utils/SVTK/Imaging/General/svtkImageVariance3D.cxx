/*=========================================================================
  ...
  Module:    svtkImageVariance3D.cxx
...
svtkStandardNewMacro(svtkImageVariance3D);

//----------------------------------------------------------------------------
svtkImageVariance3D::svtkImageVariance3D()
{
...
//----------------------------------------------------------------------------
void svtkImageVariance3D::PrintSelf(ostream& os, svtkIndent indent)
{
...
void svtkImageVariance3D::SetKernelSize(int size0, int size1, int size2)
{
  int modified = 0;
...
int svtkImageVariance3D::RequestInformation(
...
template <class T>
void svtkImageVariance3DExecute(svtkImageVariance3D* self, svtkImageData* mask, svtkImageData* inData,
  T* inPtr, svtkImageData* outData, int* outExt, float* outPtr, int id, svtkInformation* inInfo)
{
...
void svtkImageVariance3D::ThreadedRequestData(svtkInformation* svtkNotUsed(request),
  ...
  svtkImageData*** inData, svtkImageData** outData, int outExt[6], int id)
{
  ...
  if (outData[0]->GetScalarType() != SVTK_FLOAT)
  {
    svtkErrorMacro(<< "Execute: output ScalarType, "
                  << svtkImageScalarTypeNameMacro(outData[0]->GetScalarType()) << " must be float");
    return;
  }

  switch (inData[0][0]->GetScalarType())
  {
    svtkTemplateMacro(svtkImageVariance3DExecute(this, mask, inData[0][0],
      static_cast<SVTK_TT*>(inPtr), outData[0], outExt, static_cast<float*>(outPtr), id, inInfo));
    default:
...
//----------------------------------------------------------------------------
int svtkImageVariance3D::RequestData(
  svtkInformation* request, svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  this->Ellipse->Update();
