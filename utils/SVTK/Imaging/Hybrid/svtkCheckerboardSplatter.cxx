/*=========================================================================
  ...
  Module:    svtkCheckerboardSplatter.cxx
...
=========================================================================*/
#include "svtkCheckerboardSplatter.h"
...
svtkStandardNewMacro(svtkCheckerboardSplatter);

//----------------------------------------------------------------------------
// Algorithm and integration with svtkSMPTools
template <typename TPoints, typename TScalars>
class svtkCheckerboardSplatterAlgorithm
{
public:
  // Pointers to functions which are selected based on user input
  double (svtkCheckerboardSplatterAlgorithm::*Sample)(svtkIdType ptId, double x[3], double p[3]);
  double (svtkCheckerboardSplatterAlgorithm::*SampleFactor)(svtkIdType ptId);

  // Information from the SVTK class
  svtkCheckerboardSplatter* Splatter;
  svtkIdType NPts;
  TPoints* Pts;
  TScalars* Scalars;
  svtkDataArray *InScalars, *InNormals;
  ...
  struct SortedPoints
  {
    ...
    bool operator<(const SortedPoints& spts) const { return Addr < spts.Addr; }
  };
  ...
  struct Squares
  {
    svtkIdType NPts; // the number of points in this square
    svtkIdType Pts;  // the list of points in this square
    Squares()
      : NPts(0)
      , Pts(0)
    {
  ...
  // each direction.
  svtkIdType CBWidth, CBDims[3]; // checkerboard information
  ...
  svtkIdType BDims[3], BSliceSize; // 8-way checkerboard blocks/groups
  ...
  svtkIdType NSquares; // number of squares of a particular color, 8 colors total
  svtkIdType Offsets[9];

  // Construct the algorithm; initialize key data members.
  svtkCheckerboardSplatterAlgorithm() = default;

  // Integration between SVTK and templated algorithm
  static void SplatPoints(svtkCheckerboardSplatter* self, svtkIdType npts, TPoints* points,
    svtkDataArray* inScalars, svtkDataArray* inNormals, svtkImageData* output, int extent[6],
    TScalars* scalars);
  ...
  double Gaussian(svtkIdType, double x[3], double p[3])
  {
    return ((x[0] - p[0]) * (x[0] - p[0]) + (x[1] - p[1]) * (x[1] - p[1]) +
      (x[2] - p[2]) * (x[2] - p[2]));
  }
  double EccentricGaussian(svtkIdType ptId, double x[3], double p[3])
  {
  ...
  double ScalarSampling(svtkIdType ptId)
  {
    return this->ScaleFactor * this->InScalars->GetComponent(ptId, 0);
  }
  double PositionSampling(svtkIdType) { return this->ScaleFactor; }
  ...
  {
  public:
    svtkCheckerboardSplatterAlgorithm* Algo;
    AssignSquares(svtkCheckerboardSplatterAlgorithm* algo) { this->Algo = algo; }
    void operator()(svtkIdType ptId, svtkIdType end)
    {
      svtkIdType addr;
      ...
      for (; ptId < end; ++ptId)
      {
        ...
        i = static_cast<unsigned char>((x[0] - this->Algo->CBOrigin[0]) / this->Algo->CBSpacing[0]);
        j = static_cast<unsigned char>((x[1] - this->Algo->CBOrigin[1]) / this->Algo->CBSpacing[1]);
        k = static_cast<unsigned char>((x[2] - this->Algo->CBOrigin[2]) / this->Algo->CBSpacing[2]);
        oct = (i % 2) | ((j % 2) << 1) | ((k % 2) << 2);

        // Compute the address based on the particular color / block
        addr = this->Algo->Offsets[oct] + (i / 2) + (j / 2) * this->Algo->BDims[0] +
          (k / 2) * this->Algo->BSliceSize;
  ...
  public:
    svtkCheckerboardSplatterAlgorithm* Algo;
    SplatSquares(svtkCheckerboardSplatterAlgorithm* algo) { this->Algo = algo; }
    void operator()(svtkIdType sqNum, svtkIdType end)
    {
      svtkIdType npts, pts;
      for (; sqNum < end; ++sqNum)
      {
  ...
  void SplatPoint(svtkIdType ptId);
  template <typename TTPoints>
  class Splat
  {
  public:
    svtkCheckerboardSplatterAlgorithm* Algo;
    svtkIdType XMin, XMax, YMin, YMax, PtId;
    double PD[3];
    Splat(svtkCheckerboardSplatterAlgorithm* algo) { this->Algo = algo; }
    void SetSliceBounds(svtkIdType min[3], svtkIdType max[3])
    {
      this->XMin = min[0];
      this->XMax = max[0];
      this->YMin = min[1];
    ...
    void SetSplatPoint(svtkIdType ptId, TTPoints p[3])
    {
      this->PtId = ptId;
  ...
  void SetScalar(svtkIdType ptId, double pd[3], double x[3], TScalars* sPtr)
  {
    ...
    {
      case SVTK_ACCUMULATION_MODE_MIN:
        if (Tv < *sPtr)
        {
          *sPtr = Tv;
        }
        break;
      case SVTK_ACCUMULATION_MODE_MAX:
        if (Tv > *sPtr)
        {
          *sPtr = Tv;
        }
        break;
      case SVTK_ACCUMULATION_MODE_SUM:
        *sPtr += Tv;
        break;
    }
  }
...
template <typename TPoints, typename TScalars>
void svtkCheckerboardSplatterAlgorithm<TPoints, TScalars>::SplatPoint(svtkIdType ptId)
{
  ...
  for (i = 0; i < 3; i++)
  {
    min[i] = static_cast<svtkIdType>(floor(static_cast<double>(loc[i]) - this->Footprint));
    max[i] = static_cast<svtkIdType>(ceil(static_cast<double>(loc[i]) + this->Footprint));
    if (min[i] < 0)
    {
      min[i] = 0;
    }
...
template <typename TPoints, typename TScalars>
void svtkCheckerboardSplatterAlgorithm<TPoints, TScalars>::Cap(TScalars* s, TScalars capValue)
{
  svtkIdType i, j, k, jOffset, kOffset;
...
template <typename TPoints, typename TScalars>
void svtkCheckerboardSplatterAlgorithm<TPoints, TScalars>::SplatPoints(svtkCheckerboardSplatter* self,
  svtkIdType npts, TPoints* pts, svtkDataArray* inScalars, svtkDataArray* inNormals,
  svtkImageData* output, int extent[6], TScalars* scalars)
{
  int i;

  // Populate the algorithm with relevant information from the SVTK class
  svtkCheckerboardSplatterAlgorithm<TPoints, TScalars> algo;
  algo.Splatter = self;
  algo.NPts = npts;
  algo.Pts = pts;
  ...
  {
    algo.R2 = algo.Spacing[0] * algo.Spacing[0] + algo.Spacing[1] * algo.Spacing[1] +
  ...
  {
    algo.Sample = &svtkCheckerboardSplatterAlgorithm::EccentricGaussian;
  }
  else
  {
    algo.Sample = &svtkCheckerboardSplatterAlgorithm::Gaussian;
  }

  if (self->GetScalarWarping() && algo.InScalars != nullptr)
  {
    algo.SampleFactor = &svtkCheckerboardSplatterAlgorithm::ScalarSampling;
  }
  else
  {
    algo.SampleFactor = &svtkCheckerboardSplatterAlgorithm::PositionSampling;
  }
  ...
  for (i = 0; i < 3; ++i)
  {
    algo.CBDims[i] = static_cast<svtkIdType>(
      ceil(static_cast<double>(algo.Dims[i] - 1) / static_cast<double>(algo.CBWidth)));
    algo.CBDims[i] = (algo.CBDims[i] > algo.MaxDim ? algo.MaxDim : algo.CBDims[i]);
  ...
  svtkIdType currentAddr, pStart, pEnd = 0;
  while (pEnd < npts)
  {
  ...
  {
    svtkSMPTools::For(algo.Offsets[i], algo.Offsets[i + 1], splatSquares);
  }
...
svtkCheckerboardSplatter::svtkCheckerboardSplatter()
{
...
//----------------------------------------------------------------------------
int svtkCheckerboardSplatter::FillInputPortInformation(int svtkNotUsed(port), svtkInformation* info)
{
...
//----------------------------------------------------------------------------
int svtkCheckerboardSplatter::RequestInformation(svtkInformation* svtkNotUsed(request),
...
//----------------------------------------------------------------------------
int svtkCheckerboardSplatter::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  svtkDataArray* outScalars = output->GetPointData()->GetScalars();
  ...
  if (npts == 0)
  {
    ...
    svtkWarningMacro(<< "No POINTS to splat!!");
    return 1;
  }
  else
  {
    svtkDebugMacro(<< "Splatting data, total of: " << npts << " points.");
  }
  ...
  if (this->OutputScalarType == SVTK_FLOAT)
  {
    switch (points->GetDataType())
    {
      case SVTK_DOUBLE:
        svtkCheckerboardSplatterAlgorithm<double, float>::SplatPoints(this, npts,
          static_cast<double*>(ptsPtr), inScalars, inNormals, output, extent,
          static_cast<float*>(scalarPtr));
        break;
      case SVTK_FLOAT:
        svtkCheckerboardSplatterAlgorithm<float, float>::SplatPoints(this, npts,
          static_cast<float*>(ptsPtr), inScalars, inNormals, output, extent,
          static_cast<float*>(scalarPtr));
        break;
      default:
        svtkWarningMacro(<< "Undefined input point type");
    }
  }
  else if (this->OutputScalarType == SVTK_DOUBLE)
  {
    switch (points->GetDataType())
    {
      case SVTK_DOUBLE:
        svtkCheckerboardSplatterAlgorithm<double, double>::SplatPoints(this, npts,
          static_cast<double*>(ptsPtr), inScalars, inNormals, output, extent,
          static_cast<double*>(scalarPtr));
        break;
      case SVTK_FLOAT:
        svtkCheckerboardSplatterAlgorithm<float, double>::SplatPoints(this, npts,
          static_cast<float*>(ptsPtr), inScalars, inNormals, output, extent,
          static_cast<double*>(scalarPtr));
        break;
      default:
  ...
  {
    svtkWarningMacro(<< "Only FLOAT or DOUBLE output scalar type is supported");
  }
...
// input data.
void svtkCheckerboardSplatter::ComputeModelBounds(
...
void svtkCheckerboardSplatter::SetSampleDimensions(int i, int j, int k)
{
  int dim[3];
...
//----------------------------------------------------------------------------
void svtkCheckerboardSplatter::SetSampleDimensions(int dim[3])
{
  int dataDim, i;
...
//----------------------------------------------------------------------------
const char* svtkCheckerboardSplatter::GetAccumulationModeAsString()
{
...
//----------------------------------------------------------------------------
void svtkCheckerboardSplatter::PrintSelf(ostream& os, svtkIndent indent)
{
