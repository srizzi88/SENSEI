/*=========================================================================
  ...
  Module:    svtkCheckerboardSplatter.h
...
=========================================================================*/
/**
 * @class   svtkCheckerboardSplatter
 * @brief   splat points into a volume with an elliptical, Gaussian distribution
 *
 * svtkCheckerboardSplatter is a filter that injects input points into a
 ...
 *
 * This class makes use of svtkSMPTools to implement a parallel, shared-memory
 * implementation. Hence performance will be significantly improved if SVTK is
 * built with SVTK_SMP_IMPLEMENTATION_TYPE set to something other than
 ...
 * version of svtkGaussianSplatter.
 *
 ...
 *
 * @warning
 * The input to this filter is of type svtkPointSet. Currently only real types
 ...
 *
 * @warning
 * While this class is very similar to svtkGaussianSplatter, it does produce
 * slightly different output in most cases (due to the way the footprint is
 * computed).
 *
 * @sa
 * svtkShepardMethod svtkGaussianSplatter
 */

#ifndef svtkCheckerboardSplatter_h
#define svtkCheckerboardSplatter_h

#include "svtkImageAlgorithm.h"
#include "svtkImagingHybridModule.h" // For export macro

#define SVTK_ACCUMULATION_MODE_MIN 0
#define SVTK_ACCUMULATION_MODE_MAX 1
#define SVTK_ACCUMULATION_MODE_SUM 2

class svtkDoubleArray;
class svtkCompositeDataSet;

class SVTKIMAGINGHYBRID_EXPORT svtkCheckerboardSplatter : public svtkImageAlgorithm
{
public:
  svtkTypeMacro(svtkCheckerboardSplatter, svtkImageAlgorithm);
   ...
   */
  static svtkCheckerboardSplatter* New();
   ...
   */
  svtkSetClampMacro(Footprint, int, 0, SVTK_INT_MAX);
  svtkGetMacro(Footprint, int);
  //@}
   ...
   * negative value.
   */
  svtkSetMacro(ExponentFactor, double);
  svtkGetMacro(ExponentFactor, double);
  //@}
   ...
   */
  svtkSetMacro(ScalarWarping, svtkTypeBool);
  svtkGetMacro(ScalarWarping, svtkTypeBool);
  svtkBooleanMacro(ScalarWarping, svtkTypeBool);
  //@}
   ...
   */
  svtkSetMacro(NormalWarping, svtkTypeBool);
  svtkGetMacro(NormalWarping, svtkTypeBool);
  svtkBooleanMacro(NormalWarping, svtkTypeBool);
  //@}
   ...
   */
  svtkSetClampMacro(Eccentricity, double, 0.001, SVTK_DOUBLE_MAX);
   ...
   */
  svtkSetClampMacro(AccumulationMode, int, SVTK_ACCUMULATION_MODE_MIN, SVTK_ACCUMULATION_MODE_SUM);
  svtkGetMacro(AccumulationMode, int);
  void SetAccumulationModeToMin() { this->SetAccumulationMode(SVTK_ACCUMULATION_MODE_MIN); }
  void SetAccumulationModeToMax() { this->SetAccumulationMode(SVTK_ACCUMULATION_MODE_MAX); }
  void SetAccumulationModeToSum() { this->SetAccumulationMode(SVTK_ACCUMULATION_MODE_SUM); }
   ...
   */
  svtkSetClampMacro(MaximumDimension, int, 0, 255);
  svtkGetMacro(MaximumDimension, int);
  //@}

  //@{
  /**
   * Set/get the crossover point expressed in footprint size where the
   * splatting operation is parallelized (through svtkSMPTools). By default
   ...
   */
  svtkSetClampMacro(ParallelSplatCrossover, int, 0, 255);
  svtkGetMacro(ParallelSplatCrossover, int);
  //@}
   ...
   */
  void ComputeModelBounds(svtkDataSet* input, svtkImageData* output, svtkInformation* outInfo);

protected:
  svtkCheckerboardSplatter();
  ~svtkCheckerboardSplatter() override {}
  ...
  svtkTypeBool NormalWarping;      // on/off warping of splat via normal
  double Eccentricity;            // elliptic distortion due to normals
  svtkTypeBool ScalarWarping;      // on/off warping of splat via scalar
  double ScaleFactor;             // splat size influenced by scale factor
  svtkTypeBool Capping;            // Cap side of volume to close surfaces
...
private:
  svtkCheckerboardSplatter(const svtkCheckerboardSplatter&) = delete;
  void operator=(const svtkCheckerboardSplatter&) = delete;
};
