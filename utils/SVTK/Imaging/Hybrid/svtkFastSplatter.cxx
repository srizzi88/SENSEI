...
svtkStandardNewMacro(svtkFastSplatter);

//-----------------------------------------------------------------------------

svtkFastSplatter::svtkFastSplatter()
{
  ...
  this->Buckets = svtkImageData::New();
...
void svtkFastSplatter::PrintSelf(ostream& os, svtkIndent indent)
{
...
int svtkFastSplatter::FillInputPortInformation(int port, svtkInformation* info)
{
  switch (port)
  {
    case 0:
...
int svtkFastSplatter::RequestInformation(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  outInfo->Set(svtkStreamingDemandDrivenPipeline::WHOLE_EXTENT(), 0, this->OutputDimensions[0] - 1,
    0, this->OutputDimensions[1] - 1, 0, this->OutputDimensions[2] - 1);
  svtkInformation* splatInfo = inputVector[1]->GetInformationObject(0);
  svtkImageData::SetScalarType(svtkImageData::GetScalarType(splatInfo), outInfo);
  // if (splatInfo->Has(svtkDataObject::SCALAR_NUMBER_OF_COMPONENTS()))
  //   {
  //   outInfo->Set(svtkDataObject::SCALAR_NUMBER_OF_COMPONENTS(),
  //                splatInfo->Get(svtkDataObject::SCALAR_NUMBER_OF_COMPONENTS()));
  //   }
...
//----------------------------------------------------------------------------
int svtkFastSplatter::RequestUpdateExtent(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  // get the info objects
  svtkInformation* outInfo = outputVector->GetInformationObject(0);
  ...
  splatInfo->Set(svtkStreamingDemandDrivenPipeline::UPDATE_EXTENT(),
    splatInfo->Get(svtkStreamingDemandDrivenPipeline::WHOLE_EXTENT()), 6);
  ...
  svtkDataObject* data = inInfo->Get(svtkDataObject::DATA_OBJECT());
  if (data->GetExtentType() == SVTK_3D_EXTENT)
  {
    int* inWholeExtent = inInfo->Get(svtkStreamingDemandDrivenPipeline::WHOLE_EXTENT());
    inInfo->Set(svtkStreamingDemandDrivenPipeline::UPDATE_EXTENT(), inWholeExtent, 6);
  }
...
template <class T>
void svtkFastSplatterBucketPoints(const T* points, svtkIdType numPoints, unsigned int* buckets,
  const int dimensions[3], const double origin[3], const double spacing[3])
{
  ...
  for (svtkIdType i = 0; i < numPoints; i++)
  {
    ...
    // Find the bucket.
    svtkIdType loc[3];
    loc[0] = static_cast<svtkIdType>(((p[0] - origin[0]) / spacing[0]) + 0.5);
    loc[1] = static_cast<svtkIdType>(((p[1] - origin[1]) / spacing[1]) + 0.5);
    loc[2] = static_cast<svtkIdType>(((p[2] - origin[2]) / spacing[2]) + 0.5);
    ...
    svtkIdType bucketId = (loc[2] * dimensions[0] * dimensions[1] + loc[1] * dimensions[0] + loc[0]);
...
template <class T>
void svtkFastSplatterConvolve(T* splat, const int splatDims[3], unsigned int* buckets, T* output,
...
// Execute method.
int svtkFastSplatter::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  this->NumberOfPointsSplatted = 0;
  ...
  if (svtkPointSet* const input =
        svtkPointSet::SafeDownCast(inInfo->Get(svtkDataObject::DATA_OBJECT())))
  {
    points = input->GetPoints();
  }
  else if (svtkGraph* const graph =
             svtkGraph::SafeDownCast(inInfo->Get(svtkDataObject::DATA_OBJECT())))
  {
    points = graph->GetPoints();
  }

  svtkInformation* splatInfo = inputVector[1]->GetInformationObject(0);
  svtkImageData* splatImage =
    svtkImageData::SafeDownCast(splatInfo->Get(svtkDataObject::DATA_OBJECT()));
  ...
  this->Buckets->SetExtent(0, this->OutputDimensions[0] - 1, 0, this->OutputDimensions[1] - 1, 0,
    this->OutputDimensions[2] - 1);
  this->Buckets->AllocateScalars(SVTK_UNSIGNED_INT, 1);
  ...
  {
    svtkTemplateMacro(svtkFastSplatterBucketPoints(static_cast<SVTK_TT*>(p),
  ...
  {
    svtkTemplateMacro(
      svtkFastSplatterConvolve(static_cast<SVTK_TT*>(splat), splatImage->GetDimensions(), buckets,
        static_cast<SVTK_TT*>(o), &(this->NumberOfPointsSplatted), this->OutputDimensions));
  }
  ...
  switch (this->LimitMode)
  {
    ...
    case ClampLimit:
      switch (output->GetScalarType())
      {
        svtkTemplateMacro(svtkFastSplatterClamp(static_cast<SVTK_TT*>(o),
          output->GetNumberOfPoints() * output->GetNumberOfScalarComponents(),
          static_cast<SVTK_TT>(this->MinValue), static_cast<SVTK_TT>(this->MaxValue)));
      }
      break;
    case FreezeScaleLimit:
      ...
      {
        svtkTemplateMacro(
          svtkFastSplatterFrozenScale(static_cast<SVTK_TT*>(o), output->GetNumberOfScalarComponents(),
            output->GetNumberOfPoints(), static_cast<SVTK_TT>(this->MinValue),
            static_cast<SVTK_TT>(this->MaxValue), this->LastDataMinValue, this->LastDataMaxValue));
      }
      break;
    ...
    case ScaleLimit:
      ...
      {
        svtkTemplateMacro(
          svtkFastSplatterScale(static_cast<SVTK_TT*>(o), output->GetNumberOfScalarComponents(),
            output->GetNumberOfPoints(), static_cast<SVTK_TT>(this->MinValue),
            static_cast<SVTK_TT>(this->MaxValue), &this->LastDataMinValue, &this->LastDataMaxValue));
      }
      break;
...
void svtkFastSplatter::SetSplatConnection(svtkAlgorithmOutput* input)
{
