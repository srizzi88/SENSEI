 ...
 *
 *
 * svtkFastSplatter takes any svtkPointSet as input (of which svtkPolyData and
 * svtkUnstructuredGrid inherit).  Each point in the data set is considered to be
 ...
 * of unstructured points consider svtkGaussianSplatter or svtkShepardMethod.
 *
 * Use input port 0 for the impulse data (svtkPointSet), and input port 1 for
 * the splat image (svtkImageData)
 *
 *
 * @bug
...
#ifndef svtkFastSplatter_h
#define svtkFastSplatter_h

#include "svtkImageAlgorithm.h"
#include "svtkImagingHybridModule.h" // For export macro

class SVTKIMAGINGHYBRID_EXPORT svtkFastSplatter : public svtkImageAlgorithm
{
public:
  svtkTypeMacro(svtkFastSplatter, svtkImageAlgorithm);
  static svtkFastSplatter* New();
   ...
   */
  svtkSetMacro(LimitMode, int);
  svtkGetMacro(LimitMode, int);
   ...
   */
  svtkGetMacro(NumberOfPointsSplatted, int);
  //@}
   ...
   * ParaView, SVTK users should prefer SetInputConnection(1, splat) instead.
   */
  void SetSplatConnection(svtkAlgorithmOutput*);

protected:
  svtkFastSplatter();
  ~svtkFastSplatter() override;
...
private:
  svtkFastSplatter(const svtkFastSplatter&) = delete;
  void operator=(const svtkFastSplatter&) = delete;
};

//-----------------------------------------------------------------------------

template <class T>
void svtkFastSplatterClamp(T* array, svtkIdType arraySize, T minValue, T maxValue)
{
...
template <class T>
void svtkFastSplatterScale(T* array, int numComponents, svtkIdType numTuples, T minValue, T maxValue,
...
template <class T>
void svtkFastSplatterFrozenScale(
  T* array, int numComponents, svtkIdType numTuples, T minValue, T maxValue, double min, double max)
{
  T* a;
