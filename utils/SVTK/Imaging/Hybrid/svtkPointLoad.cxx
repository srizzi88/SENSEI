...
svtkStandardNewMacro(svtkPointLoad);

// Construct with ModelBounds=(-1,1,-1,1,-1,1), SampleDimensions=(50,50,50),
// and LoadValue = 1.
svtkPointLoad::svtkPointLoad()
{
  this->LoadValue = 1.0;
...
void svtkPointLoad::SetSampleDimensions(int i, int j, int k)
{
  int dim[3];
...
// Specify the dimensions of the volume. A stress tensor will be computed for
// each point in the volume.
void svtkPointLoad::SetSampleDimensions(int dim[3])
{
...
int svtkPointLoad::RequestInformation(svtkInformation* svtkNotUsed(request),
...
//
void svtkPointLoad::ExecuteDataWithInformation(svtkDataObject* outp, svtkInformation* outInfo)
{
  int i, j, k;
  svtkFloatArray* newTensors;
  double tensor[9];
  svtkIdType numPts;
  ...
  svtkDebugMacro(<< "Computing point load stress tensors");
  ...
  for (k = 0; k < this->SampleDimensions[2]; k++)
  {
    ...
    for (j = 0; j < this->SampleDimensions[1]; j++)
    {
      ...
      for (i = 0; i < this->SampleDimensions[0]; i++)
      {
        ...
        if (rho < 1.0e-10)
        {
          svtkWarningMacro(<< "Attempting to set singularity, resetting");
          tensor[0] = SVTK_FLOAT_MAX; // Component(0,0)
          tensor[4] = SVTK_FLOAT_MAX; // Component(1,1);
          tensor[8] = SVTK_FLOAT_MAX; // Component(2,2);
        ...
        // than volume is in.
        txy = -(P / (twoPi * rho2) *
          (3.0 * x * y * z / rho3 - nu * x * y * (zPlus2rho) / (rho * rhoPlusz2)));
        ...
        seff = 0.333333 *
          sqrt((sx - sy) * (sx - sy) + (sy - sz) * (sy - sz) + (sz - sx) * (sz - sx) +
            6.0 * txy * txy + 6.0 * tyz * tyz + 6.0 * txz * txz);
...
void svtkPointLoad::PrintSelf(ostream& os, svtkIndent indent)
{
