...
svtkStandardNewMacro(svtkSliceCubes);

svtkCxxSetObjectMacro(svtkSliceCubes, Reader, svtkVolumeReader);

// Description:
// Construct with nullptr reader, output FileName specification, and limits
// FileName.
svtkSliceCubes::svtkSliceCubes()
{
  this->Reader = nullptr;
  ...
  svtkVolumeReader* reader, bool debug)
{
  ...
  if (slice == nullptr) // have to do conversion to double slice-by-slice
  {
    ...
    doubleScalars = svtkDoubleArray::New();
  ...
  if (debug)
    svtkGenericWarningMacro(<< "  Slice# " << imageRange[0]);
  ...
  if (debug)
    svtkGenericWarningMacro(<< "  Slice# " << imageRange[0] + 1);
  ...
  if (!slice2 || !slice3)
  {
    svtkGenericWarningMacro(<< "Cannot allocate data!");
    return 0;
  }
  ...
  for (k = 0; k < (dims[2] - 1); k++)
  {
    ...
    {
      if (debug)
        svtkGenericWarningMacro(<< "  Slice# " << imageRange[0] + k + 2);
      sp = reader->GetImage(imageRange[0] + k + 2);
      slice3scalars = (S*)sp->GetPointData()->GetScalars();
      if (slice3scalars == nullptr)
      {
        svtkGenericWarningMacro(<< "Can't read all the requested slices");
      ...
      for (i = 0; i < (dims[0] - 1); i++)
      {
        ...
        for (; edge[0] > -1; edge += 3)
        {
          for (ii = 0; ii < 3; ii++) // insert triangle
          {
            ...
            svtkMath::Normalize(point + 3);
            // swap bytes if necessary
            bool status = svtkByteSwap::SwapWrite4BERange(point, 6, outFP);
            if (!status)
            {
              svtkGenericWarningMacro(<< "SwapWrite failed!");
            }
          }
          numTriangles++;
        } // for each triangle
...
void svtkSliceCubes::Execute()
{
  FILE* outFP;
  svtkImageData* tempStructPts;
  ...
  svtkDebugMacro(<< "Executing slice cubes");
  if (this->Reader == nullptr)
  {
    svtkErrorMacro(<< "No reader specified...can't generate isosurface");
    return;
  }

  if (this->FileName == nullptr)
  {
    svtkErrorMacro(<< "No FileName specified...can't output isosurface");
    return;
  }

  if ((outFP = svtksys::SystemTools::Fopen(this->FileName, "wb")) == nullptr)
  {
    svtkErrorMacro(<< "Cannot open specified output file...");
    return;
  }
  ...
  {
    svtkErrorMacro(<< "Bad dimensions...slice must be 3D volume");
    fclose(outFP);
    return;
  }

  xmin[0] = xmin[1] = xmin[2] = SVTK_DOUBLE_MAX;
  xmax[0] = xmax[1] = xmax[2] = -SVTK_DOUBLE_MAX;

  inScalars = tempStructPts->GetPointData()->GetScalars();
  if (inScalars == nullptr)
  {
    svtkErrorMacro(<< "Must have scalars to generate isosurface");
  ...
  if (inScalars->GetNumberOfComponents() == 1)
  {
    switch (inScalars->GetDataType())
    {
      case SVTK_CHAR:
      {
        svtkCharArray* scalars = (svtkCharArray*)inScalars;
        char* s = scalars->GetPointer(0);
        svtkSliceCubesContour(s, scalars, imageRange, dims, origin, Spacing, this->Value, xmin, xmax,
          outFP, this->Reader, this->Debug);
      }
      break;
      case SVTK_UNSIGNED_CHAR:
      {
        svtkUnsignedCharArray* scalars = (svtkUnsignedCharArray*)inScalars;
      ...
      case SVTK_SHORT:
      {
        svtkShortArray* scalars = (svtkShortArray*)inScalars;
      ...
      {
        svtkUnsignedShortArray* scalars = (svtkUnsignedShortArray*)inScalars;
      ...
      case SVTK_INT:
      {
        svtkIntArray* scalars = (svtkIntArray*)inScalars;
      ...
      {
        svtkUnsignedIntArray* scalars = (svtkUnsignedIntArray*)inScalars;
      ...
      case SVTK_LONG:
      {
        svtkLongArray* scalars = (svtkLongArray*)inScalars;
      ...
      {
        svtkUnsignedLongArray* scalars = (svtkUnsignedLongArray*)inScalars;
      ...
      case SVTK_FLOAT:
      {
        svtkFloatArray* scalars = (svtkFloatArray*)inScalars;
      ...
      case SVTK_DOUBLE:
      {
        svtkDoubleArray* scalars = (svtkDoubleArray*)inScalars;
  ...
  if (this->LimitsFileName)
  {
    ...
    if ((outFP = svtksys::SystemTools::Fopen(this->LimitsFileName, "wb")) == nullptr)
    {
      svtkWarningMacro(<< "Sorry, couldn't write limits file...");
    }
    else
    {
      bool status = true;
      ...
      for (i = 0; i < 3 && status; i++)
      {
        ...
        status = svtkByteSwap::SwapWrite4BERange(forigin + i, 1, outFP);
        if (!status)
        {
          svtkWarningMacro(<< "SwapWrite failed.");
        }
        // swap if necessary
        if (status)
        {
          status = svtkByteSwap::SwapWrite4BERange(&t, 1, outFP);
          if (!status)
          {
            svtkWarningMacro(<< "SwapWrite failed.");
          }
      ...
      {
        ftmp = (float)xmin[i];
        status = svtkByteSwap::SwapWrite4BERange(&ftmp, 1, outFP);
        if (!status)
        {
          svtkWarningMacro(<< "SwapWrite failed.");
        }
...
void svtkSliceCubes::PrintSelf(ostream& os, svtkIndent indent)
{
