...
svtkStandardNewMacro(svtkVoxelModeller);
...
svtkVoxelModeller::svtkVoxelModeller()
{
  ...
  this->ScalarType = SVTK_BIT;
  this->ForegroundValue = 1.0;
  this->BackgroundValue = 0.0;
}

// Specify the position in space to perform the voxelization.
void svtkVoxelModeller::SetModelBounds(const double bounds[6])
{
  svtkVoxelModeller::SetModelBounds(
    bounds[0], bounds[1], bounds[2], bounds[3], bounds[4], bounds[5]);
}

void svtkVoxelModeller::SetModelBounds(
  double xmin, double xmax, double ymin, double ymax, double zmin, double zmax)
{
  if (this->ModelBounds[0] != xmin || this->ModelBounds[1] != xmax ||
    this->ModelBounds[2] != ymin || this->ModelBounds[3] != ymax || this->ModelBounds[4] != zmin ||
...
int svtkVoxelModeller::RequestInformation(svtkInformation* svtkNotUsed(request),
...
int svtkVoxelModeller::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  svtkIdType cellNum, i;
  int j, k;
  const double* bounds;
  double adjBounds[6];
  svtkCell* cell;
  double maxDistance, pcoords[3];
  svtkIdType numPts, idx, numCells;
  int subId;
  int min[3], max[3];
  ...
  //
  svtkDebugMacro(<< "Executing Voxel model");
...
double svtkVoxelModeller::ComputeModelBounds(double origin[3], double spacing[3])
{
  const double* bounds;
  double maxDist;
...
void svtkVoxelModeller::SetSampleDimensions(int i, int j, int k)
{
  int dim[3];
...
void svtkVoxelModeller::SetSampleDimensions(int dim[3])
{
  int dataDim, i;
...
int svtkVoxelModeller::FillInputPortInformation(int svtkNotUsed(port), svtkInformation* info)
{
  info->Set(svtkAlgorithm::INPUT_REQUIRED_DATA_TYPE(), "svtkDataSet");
  return 1;
}

void svtkVoxelModeller::PrintSelf(ostream& os, svtkIndent indent)
{
