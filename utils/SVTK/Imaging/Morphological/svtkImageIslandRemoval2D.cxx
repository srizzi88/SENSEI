/*=========================================================================
  ...
  Module:    svtkImageIslandRemoval2D.cxx
...
=========================================================================*/
#include "svtkImageIslandRemoval2D.h"
...
svtkStandardNewMacro(svtkImageIslandRemoval2D);

//----------------------------------------------------------------------------
// Constructor: Sets default filter to be identity.
svtkImageIslandRemoval2D::svtkImageIslandRemoval2D()
{
...
//----------------------------------------------------------------------------
void svtkImageIslandRemoval2D::PrintSelf(ostream& os, svtkIndent indent)
{
...
template <class T>
void svtkImageIslandRemoval2DExecute(svtkImageIslandRemoval2D* self, svtkImageData* inData, T* inPtr,
  svtkImageData* outData, T* outPtr, int outExt[6])
{
  ...
  svtkImage2DIslandPixel* pixels;    // All the pixels visited so far.
  int numPixels;                    // The number of pixels visited so far.
  svtkImage2DIslandPixel* newPixel;  // The last pixel in the list.
  int nextPixelIdx;                 // The index of the next pixel to grow.
  svtkImage2DIslandPixel* nextPixel; // The next pixel to grow.
  int keepValue;
  int area;
  ...
  pixels = new svtkImage2DIslandPixel[area + 8];
...
int svtkImageIslandRemoval2D::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  {
    svtkErrorMacro(<< "Execute: input ScalarType, "
                  << svtkImageScalarTypeNameMacro(inData->GetScalarType())
                  << ", must match out ScalarType "
                  << svtkImageScalarTypeNameMacro(outData->GetScalarType()));
    return 1;
  }
  ...
  {
    svtkTemplateMacro(svtkImageIslandRemoval2DExecute(
      this, inData, static_cast<SVTK_TT*>(inPtr), outData, static_cast<SVTK_TT*>(outPtr), outExt));
    default:
