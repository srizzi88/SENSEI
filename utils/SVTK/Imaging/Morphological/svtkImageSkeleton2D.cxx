/*=========================================================================
  ...
  Module:    svtkImageSkeleton2D.cxx
...
svtkStandardNewMacro(svtkImageSkeleton2D);

//----------------------------------------------------------------------------
// Construct an instance of svtkImageSkeleton2D filter.
svtkImageSkeleton2D::svtkImageSkeleton2D()
{
  this->Prune = 0;
}

//----------------------------------------------------------------------------
void svtkImageSkeleton2D::SetNumberOfIterations(int num)
{
  this->svtkImageIterateFilter::SetNumberOfIterations(num);
}
...
int svtkImageSkeleton2D::IterativeRequestUpdateExtent(svtkInformation* in, svtkInformation* out)
{
  int wholeExtent[6];
  int outExt[6];
  in->Get(svtkStreamingDemandDrivenPipeline::WHOLE_EXTENT(), wholeExtent);
  out->Get(svtkStreamingDemandDrivenPipeline::UPDATE_EXTENT(), outExt);
  ...
  in->Set(svtkStreamingDemandDrivenPipeline::UPDATE_EXTENT(), inExt, 6);
...
template <class T>
void svtkImageSkeleton2DExecute(svtkImageSkeleton2D* self, svtkImageData* inData, T* inPtr,
  svtkImageData* outData, int* outExt, T* outPtr, int id, int wholeExt[6])
{
...
void svtkImageSkeleton2D::ThreadedRequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* svtkNotUsed(outputVector),
  svtkImageData*** inDataV, svtkImageData** outDataV, int outExt[6], int id)
{
  svtkImageData* inData = inDataV[0][0];
  svtkImageData* outData = outDataV[0];
  void* inPtr;
  ...
  if (!inScalarInfo)
  {
    svtkErrorMacro("Missing ActiveScalar info in input information!");
    return;
  }
  ...
  tempData->AllocateScalars(inScalarInfo->Get(svtkImageData::FIELD_ARRAY_TYPE()),
    inScalarInfo->Get(svtkImageData::FIELD_NUMBER_OF_COMPONENTS()));
  tempData->CopyAndCastFrom(inData, inExt);

  inPtr = tempData->GetScalarPointerForExtent(outExt);
  switch (tempData->GetScalarType())
  {
    svtkTemplateMacro(svtkImageSkeleton2DExecute(this, tempData, static_cast<SVTK_TT*>(inPtr), outData,
      outExt, static_cast<SVTK_TT*>(outPtr), id, wholeExt));
    default:
...
void svtkImageSkeleton2D::PrintSelf(ostream& os, svtkIndent indent)
{
