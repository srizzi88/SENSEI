/*=========================================================================
  ...
  Module:    svtkImageCanvasSource2D.cxx
...
class svtkImageCanvasSource2DPixel
{ //;prevent man page generation
public:
  static svtkImageCanvasSource2DPixel* New() { return new svtkImageCanvasSource2DPixel; }
  int X;
  int Y;
  void* Pointer;
  svtkImageCanvasSource2DPixel* Next;
};

svtkStandardNewMacro(svtkImageCanvasSource2D);

//----------------------------------------------------------------------------
// Construct an instance of svtkImageCanvasSource2D with no data.
svtkImageCanvasSource2D::svtkImageCanvasSource2D()
{
...
//----------------------------------------------------------------------------
// Destructor: Deleting a svtkImageCanvasSource2D automatically
// deletes the associated
// svtkImageData.  However, since the data is reference counted, it may not
// actually be deleted.
svtkImageCanvasSource2D::~svtkImageCanvasSource2D()
{
...
//----------------------------------------------------------------------------
void svtkImageCanvasSource2D::PrintSelf(ostream& os, svtkIndent indent)
{
...
template <class T>
void svtkImageCanvasSource2DDrawImage(svtkImageData* image, svtkImageData* simage, T* ptr, T* sptr,
...
//----------------------------------------------------------------------------
void svtkImageCanvasSource2D::DrawImage(
  int x0, int y0, svtkImageData* image, int sx, int sy, int width, int height)
{
  if (!image)
  {
    return;
  ...
  {
    width = svtkMath::Min(width, ext[1] - ext[0] + 1);
  }
  if (height < 0)
  {
  ...
  {
    height = svtkMath::Min(height, ext[3] - ext[2] + 1);
  }
  ext[0] = svtkMath::Max(sx, ext[0]);
  ext[1] = svtkMath::Max(sx + width - 1, ext[1]);
  ext[2] = svtkMath::Max(sy, ext[2]);
  ext[3] = svtkMath::Max(sy + height - 1, ext[3]);
  clip->SetOutputWholeExtent(ext);

  svtkImageCast* ic = svtkImageCast::New();
  ...
  {
    svtkTemplateMacro(svtkImageCanvasSource2DDrawImage(this->ImageData, ic->GetOutput(),
      static_cast<SVTK_TT*>(ptr), static_cast<SVTK_TT*>(sptr), min0, max0, min1, max1));
    default:
      svtkErrorMacro(<< "FillBox: Cannot handle ScalarType.");
  }
  ic->Delete();
  clip->Delete();
  this->Modified();
...
// Draw a data.  Only implentented for 2D extents.
template <class T>
void svtkImageCanvasSource2DFillBox(
  svtkImageData* image, double* drawColor, T* ptr, int min0, int max0, int min1, int max1)
{
  T *ptr0, *ptr1, *ptrV;
  int idx0, idx1, idxV;
  svtkIdType inc0, inc1, inc2;
...
//----------------------------------------------------------------------------
// Draw a data.  Only implentented for 2D extents.
void svtkImageCanvasSource2D::FillBox(int min0, int max0, int min1, int max1)
{
  int* extent;
  void* ptr;
  int z = this->DefaultZ;
  ...
  switch (this->ImageData->GetScalarType())
  {
    svtkTemplateMacro(svtkImageCanvasSource2DFillBox(
      this->ImageData, this->DrawColor, static_cast<SVTK_TT*>(ptr), min0, max0, min1, max1));
    default:
      svtkErrorMacro(<< "FillBox: Cannot handle ScalarType.");
  }
  this->Modified();
...
template <class T>
void svtkImageCanvasSource2DFillTube(
  svtkImageData* image, double* drawColor, T* ptr, int a0, int a1, int b0, int b1, double radius)
{
  T *ptr0, *ptr1, *ptrV;
  int idx0, idx1, idxV;
  svtkIdType inc0, inc1, inc2;
...
//----------------------------------------------------------------------------
// Fill a tube (thick line for initial 2D implementation).
void svtkImageCanvasSource2D::FillTube(int a0, int a1, int b0, int b1, double radius)
{
  void* ptr;
  int z = this->DefaultZ;
  ...
  switch (this->ImageData->GetScalarType())
  {
    svtkTemplateMacro(svtkImageCanvasSource2DFillTube(
      this->ImageData, this->DrawColor, static_cast<SVTK_TT*>(ptr), a0, a1, b0, b1, radius));
    default:
      svtkErrorMacro(<< "FillTube: Cannot handle ScalarType.");
  }
  this->Modified();
}
...
template <class T>
void svtkImageCanvasSource2DFillTriangle(svtkImageData* image, double* drawColor, T* ptr, int a0,
...
//----------------------------------------------------------------------------
// Fill a tube (thick line for initial 2D implementation).
void svtkImageCanvasSource2D::FillTriangle(int a0, int a1, int b0, int b1, int c0, int c1)
{
  void* ptr;
  int z = this->DefaultZ;
  ...
  switch (this->ImageData->GetScalarType())
  {
    svtkTemplateMacro(svtkImageCanvasSource2DFillTriangle(
      this->ImageData, this->DrawColor, static_cast<SVTK_TT*>(ptr), a0, a1, b0, b1, c0, c1, z));
    default:
      svtkErrorMacro(<< "FillTriangle: Cannot handle ScalarType.");
  }
  this->Modified();
}
...
template <class T>
void svtkImageCanvasSource2DDrawPoint(
  svtkImageData* image, double* drawColor, T* ptr, int p0, int p1, int z)
{
  int min0, max0, min1, max1, min2, max2, maxV;
  int idxV;
  double* pf;
...
//----------------------------------------------------------------------------
// Draw a circle
void svtkImageCanvasSource2D::DrawPoint(int p0, int p1)
{
  void* ptr = nullptr;
  int z = this->DefaultZ;

  svtkDebugMacro(<< "Drawing a point: (" << p0 << ", " << p1 << ")");
  ...
  switch (this->ImageData->GetScalarType())
  {
    svtkTemplateMacro(svtkImageCanvasSource2DDrawPoint(
      this->ImageData, this->DrawColor, static_cast<SVTK_TT*>(ptr), p0, p1, z));
    default:
      svtkErrorMacro(<< "DrawPoint: Cannot handle ScalarType.");
  }
  this->Modified();
}
...
template <class T>
void svtkImageCanvasSource2DDrawCircle(
  svtkImageData* image, double* drawColor, T* ptr, int c0, int c1, double radius, int z)
{
  int min0, max0, min1, max1, min2, max2, maxV;
  int idxV;
  double* pf;
  ...
  numberOfSteps = static_cast<int>(ceil(2.0 * svtkMath::Pi() * radius));
...
//----------------------------------------------------------------------------
// Draw a circle
void svtkImageCanvasSource2D::DrawCircle(int c0, int c1, double radius)
{
  void* ptr = nullptr;
  int z = this->DefaultZ;

  svtkDebugMacro(<< "Drawing a circle: center = (" << c0 << ", " << c1 << "), radius = " << radius);
  ...
  switch (this->ImageData->GetScalarType())
  {
    svtkTemplateMacro(svtkImageCanvasSource2DDrawCircle(
      this->ImageData, this->DrawColor, static_cast<SVTK_TT*>(ptr), c0, c1, radius, z));
    default:
      svtkErrorMacro(<< "DrawCircle: Cannot handle ScalarType.");
  }
  this->Modified();
}
...
template <class T>
void svtkImageCanvasSource2DDrawSegment(
  svtkImageData* image, double* drawColor, T* ptr, int p0, int p1)
{
  double f0, f1;
  double s0, s1;
  int numberOfSteps;
...
void svtkImageCanvasSource2D::DrawSegment(int a0, int a1, int b0, int b1)
{
  int* extent;
  void* ptr;
  int z = this->DefaultZ;

  svtkDebugMacro(<< "Drawing a segment: " << a0 << ", " << a1 << " to " << b0 << ", " << b1);
  ...
  switch (this->ImageData->GetScalarType())
  {
    svtkTemplateMacro(svtkImageCanvasSource2DDrawSegment(
      this->ImageData, this->DrawColor, static_cast<SVTK_TT*>(ptr), a0, a1));
    default:
      svtkErrorMacro(<< "DrawSegment: Cannot handle ScalarType.");
  }
  this->Modified();
}
...
int svtkImageCanvasSource2D::ClipSegment(int& a0, int& a1, int& b0, int& b1)
{
  int min0, max0, min1, max1, min2, max2;
  double fract;
...
// First point is already shifted to origin.
template <class T>
void svtkImageCanvasSource2DDrawSegment3D(
  svtkImageData* image, double* drawColor, T* ptr, int p0, int p1, int p2)
{
  double f0, f1, f2;
  double s0, s1, s2;
  int numberOfSteps;
...
void svtkImageCanvasSource2D::DrawSegment3D(double* a, double* b)
{
  void* ptr;
  int a0, a1, a2;
  ...
  switch (this->ImageData->GetScalarType())
  {
    svtkTemplateMacro(svtkImageCanvasSource2DDrawSegment3D(
      this->ImageData, this->DrawColor, static_cast<SVTK_TT*>(ptr), a0, a1, a2));
    default:
      svtkErrorMacro(<< "DrawSegment3D: Cannot handle ScalarType.");
  }
  this->Modified();
}
...
//----------------------------------------------------------------------------
template <class T>
void svtkImageCanvasSource2DFill(svtkImageData* image, double* color, T* ptr, int x, int y)
{
  svtkImageCanvasSource2DPixel* pixel;
  svtkImageCanvasSource2DPixel *first, *last;
  svtkImageCanvasSource2DPixel* heap = nullptr;
  int min0, max0, min1, max1, min2, max2, maxV;
  int idxV;
  svtkIdType inc0, inc1, inc2;
  T fillColor[10];
  ...
  { // fill the same as draw
    svtkGenericWarningMacro("Fill: Cannot handle draw color same as fill color");
    return;
  }

  // Create the seed
  pixel = svtkImageCanvasSource2DPixel::New();
  pixel->X = x;
  pixel->Y = y;
...
void svtkImageCanvasSource2D::FillPixel(int x, int y)
{
  void* ptr;
  ...
  switch (this->ImageData->GetScalarType())
  {
    svtkTemplateMacro(svtkImageCanvasSource2DFill(
      this->ImageData, this->DrawColor, static_cast<SVTK_TT*>(ptr), x, y));
    default:
      svtkErrorMacro(<< "Fill: Cannot handle ScalarType.");
  }
  this->Modified();
}

//----------------------------------------------------------------------------
void svtkImageCanvasSource2D::SetExtent(int* extent)
{
  this->SetExtent(extent[0], extent[1], extent[2], extent[3], extent[4], extent[5]);
}

//----------------------------------------------------------------------------
void svtkImageCanvasSource2D::SetExtent(int xMin, int xMax, int yMin, int yMax, int zMin, int zMax)
{
  int modified = 0;
...
//----------------------------------------------------------------------------
void svtkImageCanvasSource2D::SetScalarType(int t)
{
...
//----------------------------------------------------------------------------
int svtkImageCanvasSource2D::GetScalarType() const
{
  return this->ImageData->GetScalarType();
}

//----------------------------------------------------------------------------
void svtkImageCanvasSource2D::SetNumberOfScalarComponents(int t)
{
...
//----------------------------------------------------------------------------
int svtkImageCanvasSource2D::GetNumberOfScalarComponents() const
{
  return this->ImageData->GetNumberOfScalarComponents();
}

//----------------------------------------------------------------------------
int svtkImageCanvasSource2D::RequestInformation(svtkInformation* svtkNotUsed(request),
...
//----------------------------------------------------------------------------
int svtkImageCanvasSource2D::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** svtkNotUsed(inputVector), svtkInformationVector* outputVector)
{
  // get the data object
  svtkInformation* outInfo = outputVector->GetInformationObject(0);
...
// ---------------------------------------------------------------------------
void svtkImageCanvasSource2D::InitializeCanvasVolume(svtkImageData* volume)
{
  if (!volume)
  {
    return;
