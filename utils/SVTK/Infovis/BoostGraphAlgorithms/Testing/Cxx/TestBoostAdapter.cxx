...
void TestTraversal(Graph g, int repeat, int& svtkNotUsed(errors))
{
...
template <typename Graph>
void TestGraph(Graph g, svtkIdType numVertices, svtkIdType numEdges, int repeat, int& errors)
{
  typedef typename graph_traits<Graph>::edge_descriptor Edge;
  typedef typename graph_traits<Graph>::vertex_descriptor Vertex;
  ...
  {
    int u = static_cast<int>(svtkMath::Random(0, numVertices));
    int v = static_cast<int>(svtkMath::Random(0, numVertices));
...
int TestBoostAdapter(int svtkNotUsed(argc), char* svtkNotUsed(argv)[])
{
  int errors = 0;
  int repeat = 100;
  svtkIdType numVertices = 1000;
  svtkIdType numEdges = 2000;
  ...
  svtkMutableUndirectedGraph* ug = svtkMutableUndirectedGraph::New();
  TestGraph(ug, numVertices, numEdges, repeat, errors);
  ug->Delete();
  cerr << "...done." << endl << endl;

  cerr << "Testing directed graph adapter..." << endl;
  svtkMutableDirectedGraph* dg = svtkMutableDirectedGraph::New();
  TestGraph(dg, numVertices, numEdges, repeat, errors);
  dg->Delete();
  cerr << "...done." << endl << endl;

  cerr << "Testing tree adapter..." << endl;
  svtkMutableDirectedGraph* builder = svtkMutableDirectedGraph::New();
  builder->AddVertex();
  for (svtkIdType i = 1; i < numVertices; i++)
  {
    builder->AddChild(static_cast<svtkIdType>(svtkMath::Random(0, i)));
  }
  svtkTree* t = svtkTree::New();
