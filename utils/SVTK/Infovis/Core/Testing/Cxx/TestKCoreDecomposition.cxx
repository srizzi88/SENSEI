...
int TestKCoreDecomposition(int svtkNotUsed(argc), char* svtkNotUsed(argv)[])
{
  SVTK_CREATE(svtkMutableDirectedGraph, dg);
  SVTK_CREATE(svtkMutableUndirectedGraph, ug);
  SVTK_CREATE(svtkMutableDirectedGraph, sdg);
  SVTK_CREATE(svtkMutableUndirectedGraph, sug);
  SVTK_CREATE(svtkKCoreDecomposition, kcp);
  ...
  // used for the SVTK filter implementation.  The test runs it once as a directed graph,
  ...
  svtkDataArray* da = kcp->GetOutput()->GetVertexData()->GetArray("KCoreDecompositionNumbers");
