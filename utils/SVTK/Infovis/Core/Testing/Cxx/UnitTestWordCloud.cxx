...
namespace
{
int TestOneByOne(svtkSmartPointer<svtkWordCloud>& wc, std::string name, size_t keptExpected,
...
int UnitTestWordCloud(int argc, char* argv[])
{
  ...
  auto wordCloud = svtkSmartPointer<svtkWordCloud>::New();
...
#define CHECK_CONTAINER_MTIMES(name)                                                               \
  ...
  svtkWordCloud::ColorDistributionContainer colorDistribution = { { .6, 1.0 } };
  wordCloud->SetColorDistribution(colorDistribution);

  svtkWordCloud::OffsetDistributionContainer offsetDistribution = { { -10, 20 } };
  wordCloud->SetOffsetDistribution(offsetDistribution);

  svtkWordCloud::OrientationsContainer orientations = { -90 };
  ...
  svtkWordCloud::ReplacementPairsContainer replacementPairs;
  svtkWordCloud::PairType pt1("old", "new");
  svtkWordCloud::PairType pt2("bill", "will");
  replacementPairs.push_back(pt1);
  wordCloud->SetReplacementPairs(replacementPairs);
  wordCloud->AddReplacementPair(pt2);

  svtkWordCloud::SizesContainer sizes;
  sizes[0] = 100;
  sizes[1] = 10;
  wordCloud->SetSizes(sizes);

  svtkWordCloud::StopWordsContainer words;
  words.insert("albany");
  ...
  int status4 = 0;
  {
    auto wc = svtkSmartPointer<svtkWordCloud>::New();
    ...
    svtkWordCloud::OrientationDistributionContainer oDist;
    oDist[0] = -90.0;
    oDist[1] = 90.0;
    ...
    svtkWordCloud::ColorDistributionContainer colorDist = { { 0.0, 1.0 } };
  ...
  auto errorObserver = svtkSmartPointer<svtkTest::ErrorObserver>::New();
  auto errorObserver1 = svtkSmartPointer<svtkTest::ErrorObserver>::New();
  int status5 = 0;
  {
    auto wc = svtkSmartPointer<svtkWordCloud>::New();
    wc->AddObserver(svtkCommand::ErrorEvent, errorObserver);
    wc->GetExecutive()->AddObserver(svtkCommand::ErrorEvent, errorObserver1);
    wc->SetFileName(argv[1]);
    wc->SetWordColorName("");
