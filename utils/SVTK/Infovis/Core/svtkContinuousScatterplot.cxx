...
=========================================================================*/
#include "svtkContinuousScatterplot.h"
...
svtkStandardNewMacro(svtkContinuousScatterplot);

// Data structure to store the fragment faces.
// Each face of the fragment can be represented using a svtkIdList.
typedef std::vector<svtkSmartPointer<svtkIdList> >* Polytope;

//----------------------------------------------------------------------------
svtkContinuousScatterplot::svtkContinuousScatterplot()
{
  ...
  // Create the svtkImageData object and pass to the output port.
  svtkImageData* output = svtkImageData::New();
  this->GetExecutive()->SetOutputData(0, output);
  output->Delete();
  this->Fields[0] = this->Fields[1] = nullptr;
}
//----------------------------------------------------------------------------
void svtkContinuousScatterplot::SetField1(const char* nm, svtkIdType xRes)
{
  this->Fields[0] = nm;
  this->ResX = xRes;
}
//----------------------------------------------------------------------------
void svtkContinuousScatterplot::SetField2(const char* nm, svtkIdType yRes)
{
  this->Fields[1] = nm;
  this->ResY = yRes;
}
//----------------------------------------------------------------------------
void svtkContinuousScatterplot::PrintSelf(ostream& os, svtkIndent indent)
{
  this->Superclass::PrintSelf(os, indent);
}
//----------------------------------------------------------------------------
int svtkContinuousScatterplot::FillInputPortInformation(int, svtkInformation* info)
{
  info->Set(svtkAlgorithm::INPUT_REQUIRED_DATA_TYPE(), "svtkUnstructuredGrid");
  return 1;
}
//----------------------------------------------------------------------------
int svtkContinuousScatterplot::FillOutputPortInformation(int, svtkInformation* info)
{
  info->Set(svtkDataObject::DATA_TYPE_NAME(), "svtkImageData");
  return 1;
}
//----------------------------------------------------------------------------
int svtkContinuousScatterplot::RequestData(
  svtkInformation*, svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  // get the input data set, which is required to be a svtkUnstructuredGrid.
  svtkUnstructuredGrid* input =
    svtkUnstructuredGrid::SafeDownCast(inInfo->Get(svtkDataObject::DATA_OBJECT()));
  // get the output data set, which is required to be a svtkImageData.
  ...
  {
    svtkErrorMacro("The resolution of the output image has to be a positive number.");
    return 1;
  }
  ...
  {
    svtkErrorMacro("At least two fields need to be specified.");
    return 1;
  }

  // Input point data, which should include the arrays defining the range space.
  svtkDataSetAttributes* inPD = input->GetPointData();
  ...
  {
    svtkErrorMacro("Array not found in input point data: " << this->Fields[0]);
    return 1;
  }
  ...
  {
    svtkErrorMacro("Array not found in input point data: " << this->Fields[1]);
    return 1;
  }
  ...
  svtkSmartPointer<svtkIdList> cut = svtkSmartPointer<svtkIdList>::New();
  ...
  svtkIdType newPointId, thisPointId, prevPointId;
  ...
  // The SVTK filter for computing the fragment geometric volume. This filter only
  // accepts triangular mesh as input, while our fragments are stored as a polygonal
  // mesh. Therefore, we need a mesh conversion.
  svtkSmartPointer<svtkMassProperties> volume = svtkSmartPointer<svtkMassProperties>::New();
  // This SVTK filter convert a polygonal mesh to a triangular mesh.
  ...
  svtkIdType binIndexFirst = 0, binIndexSecond = 0;
  ...
  svtkIdType ignored = 0;
  // point coordinates in the input grid
  svtkPoints* inputGridPoints = input->GetPoints();
  ...
  svtkSmartPointer<svtkDataSetAttributes> newPointsPD = svtkSmartPointer<svtkDataSetAttributes>::New();
  ...
  svtkSmartPointer<svtkMergePoints> pointLocator = svtkSmartPointer<svtkMergePoints>::New();
  ...
  svtkSmartPointer<svtkFloatArray> fragScalar = svtkSmartPointer<svtkFloatArray>::New();
  ...
  // the input grid.
  svtkSmartPointer<svtkDataSetAttributes> tetraPD = svtkSmartPointer<svtkDataSetAttributes>::New();
  ...
  svtkSmartPointer<svtkFloatArray> tetraF1 = svtkSmartPointer<svtkFloatArray>::New(),
                                 tetraF2 = svtkSmartPointer<svtkFloatArray>::New();
  ...
  svtkSmartPointer<svtkPolyData> polyhedra = svtkSmartPointer<svtkPolyData>::New();
  ...
  for (svtkIdType tetraIndex = 0; tetraIndex < input->GetNumberOfCells(); tetraIndex++)
  {
    // current tetrahedron vertex list.
    cell = svtkSmartPointer<svtkIdList>::New();
    input->GetCellPoints(tetraIndex, cell);

    // Test if the current cell is a tetrahedron or not. If not, ignore this cell.
    if (input->GetCellType(tetraIndex) != SVTK_TETRA || cell->GetNumberOfIds() != 4)
    {
      svtkWarningMacro("Current cell " << tetraIndex << " is not of a tetrahedron type.");
      continue;
    }
    ...
    for (svtkIdType cellIndex = 0; cellIndex < cell->GetNumberOfIds(); cellIndex++)
    {
      svtkIdType pointId = cell->GetId(cellIndex);
    ...
    for (svtkIdType cellPDIndex = 0; cellPDIndex < tetraPD->GetNumberOfTuples(); cellPDIndex++)
    {
    ...
    */
    ...
    Polytope ptp = new std::vector<svtkSmartPointer<svtkIdList> >();
    ...
    for (int fnr = 0; fnr < 4; fnr++)
    {
      ...
      face = svtkSmartPointer<svtkIdList>::New();
    ...
    for (size_t fieldNr = 0; fieldNr < 2; fieldNr++)
    {
      ...
      for (size_t cp = 0; cp < inputQ.size(); cp++)
      {
        ...
        for (double threshold = initThreshold; threshold < maxCell; threshold += fragWidth[fieldNr])
        {
          ...
          delete fragment;
          delete residual;
          fragment = new std::vector<svtkSmartPointer<svtkIdList> >();
          residual = new std::vector<svtkSmartPointer<svtkIdList> >();

          // Create the new cutting plane.
          cut = svtkSmartPointer<svtkIdList>::New();

          // Effectively, we start processing a new cell at this point.
          for (std::vector<svtkSmartPointer<svtkIdList> >::iterator faceIt = working->begin();
               faceIt != working->end(); ++faceIt)
          {
            fragmentFace = svtkSmartPointer<svtkIdList>::New();
            residualFace = svtkSmartPointer<svtkIdList>::New();
            ...
            for (int i = 0; i < nrFaceIds; i++)
            {
              ...
              switch (mask)
              {
                ...
                case 6:
                  ...
                  break;
                default:
                  svtkErrorMacro("Incomparable scalars " << prevScalar << ", " << thisScalar << ", "
                                                        << threshold);
              } // switch mask
          ...
          if (cut->GetNumberOfIds() > 2)
          {
            ...
            for (int i = 2; i < nrPoints; i++)
            {
              ...
              theta[i - 1] = acos(svtkMath::Dot(base, vec) / (baseNorm * svtkMath::Norm(vec)));
          ...
          if (fragment->size() > 3)
          {
            ...
            if (fieldNr)
            {
              ...
              float firstFieldRange = fragScalar->GetComponent((svtkIdType)cp, 0);
    ...
    for (size_t co = 0; co < outputQ.size(); co++)
    {
      ...
      polyhedra->AllocateEstimate(static_cast<svtkIdType>(outputQ[co]->size()), 1);

      // for each face of the fragment
      svtkSmartPointer<svtkIdList> poly = svtkSmartPointer<svtkIdList>::New();
      for (std::vector<svtkSmartPointer<svtkIdList> >::iterator fc = outputQ[co]->begin();
           fc != outputQ[co]->end(); ++fc)
      {
        ...
        polyhedra->InsertNextCell(SVTK_POLYGON, poly);
      }
      ...
      fragRangeValue[0] = fragScalar->GetComponent((svtkIdType)(firstFragNr + co), 0);
      fragRangeValue[1] = fragScalar->GetComponent((svtkIdType)(firstFragNr + co), 1);
  ...
  output->AllocateScalars(SVTK_DOUBLE, 1);

  // A scalar array is attached onto the output svtkImageData.
  // This array records the total volume of the fragments in each bin.
  svtkSmartPointer<svtkFloatArray> volumeArray = svtkSmartPointer<svtkFloatArray>::New();
