...
=========================================================================*/
/**
* @class svtkContinuousScatterplot
...
*        data structure.
*
* @section SVTK Filter Design
* The input and output ports of the filter:
*      Input port : the input data set should be a svtkUnstructuredGrid, with each of its
...
*      Output port: the output data set is a 2D image stored as a svtkImageData.
...
* Suppose we have a tetrahedral mesh stored in a svtkUnstructuredGrid, we call this
...
*
*     svtkSmartPointer<svtkContinuousScatterplot> csp =
*                            svtkSmartPointer<svtkContinuousScatterplot>::New();
*     csp->SetInputData(inputData);
*     csp->SetField1("f1",resX);
*     csp->SetField2("f2",resY);
*     csp->Update();
*
* Then the output, "csp->GetOutput()", will be a svtkImageData containing a scalar
...
#ifndef svtkContinuousScatterplot_h
#define svtkContinuousScatterplot_h

#include "svtkImageAlgorithm.h"
#include "svtkInfovisCoreModule.h" // For export macro

class SVTKINFOVISCORE_EXPORT svtkContinuousScatterplot : public svtkImageAlgorithm
{
public:
  static svtkContinuousScatterplot* New();
  svtkTypeMacro(svtkContinuousScatterplot, svtkImageAlgorithm);
  void PrintSelf(ostream& os, svtkIndent indent) override;

  /**
   * Get the tolerance used when comparing floating point numbers for equality.
   */
  svtkGetMacro(Epsilon, double);

  /**
   * Set the tolerance used when comparing floating point numbers for equality.
   */
  svtkSetMacro(Epsilon, double);
   ...
   */
  void SetField1(const char* fieldName, svtkIdType ResX);
   ...
   */
  void SetField2(const char* fieldName, svtkIdType ResY);

protected:
  svtkContinuousScatterplot();

  // Configure input port to accept only svtkUnstructuredGrid.
  int FillInputPortInformation(int port, svtkInformation* info) override;

  // Configure out port to be a svtkImageData data set.
  ...
  svtkIdType ResX, ResY;

private:
  svtkContinuousScatterplot(const svtkContinuousScatterplot&) = delete;
  void operator=(const svtkContinuousScatterplot&) = delete;
};
#endif
