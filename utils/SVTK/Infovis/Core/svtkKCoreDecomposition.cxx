/*=========================================================================
  ...
  Module:    svtkKCoreDecomposition.cxx
...
-------------------------------------------------------------------------*/
#include "svtkKCoreDecomposition.h"
...
svtkStandardNewMacro(svtkKCoreDecomposition);
...
public:
  ...
  {
    this->iti = svtkInEdgeIterator::New();
    this->ito = svtkOutEdgeIterator::New();
  ...
  void Initialize(svtkGraph* g, int v)
  {
    ...
    this->iti->Initialize(g, svtkIdType(v - 1));

    if (!this->Undirected)
    {
      this->ito->Initialize(g, svtkIdType(v - 1));
    }
  }
...
private:
  svtkInEdgeIterator* iti;
  svtkOutEdgeIterator* ito;
...
public:
  tableVert(int n)
  {
    this->_array = svtkIntArray::New();
    this->_array->SetNumberOfTuples(n);
  }

  tableVert(svtkIntArray* a)
  {
    this->_array = a;
...
svtkKCoreDecomposition::svtkKCoreDecomposition()
{
...
// of Networks."
void svtkKCoreDecomposition::Cores(svtkGraph* g, svtkIntArray* KCoreNumbers)
{
  int n, md, start, num;
  int w, pu, pw;
  int v, d, i, u, du;
  ...
  if (svtkDirectedGraph::SafeDownCast(g) && this->UseInDegreeNeighbors &&
...
int svtkKCoreDecomposition::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  if (this->CheckInputGraph)
  {
    ...
    while (it->HasNext())
    {
      ...
      {
        svtkErrorMacro("Found parallel edge between at vertex ID: " << e.Source << " and vertex ID: "
                                                                   << e.Target);
        foundParallelEdges = true;
      }

      if (svtkUndirectedGraph::SafeDownCast(input))
      {
        id =
        ...
        {
          svtkErrorMacro("Found parallel edge between at vertex ID: "
      ...
      {
        foundLoops = true;
        svtkErrorMacro("Found loop at vertex ID: " << e.Source);
      }
    }
    ...
    if (foundLoops || foundParallelEdges)
    {
      if (foundLoops)
      {
        svtkErrorMacro("Found loops in input graph.  Unable to compute K core.");
      }

      if (foundParallelEdges)
      {
        svtkErrorMacro("Found parallel edges in input graph.  Unable to compute K core.");
      }
      return 0;
    }
  }
  ...
  svtkIntArray* KCoreNumbers = svtkIntArray::New();
...
void svtkKCoreDecomposition::PrintSelf(ostream& os, svtkIndent indent)
{
