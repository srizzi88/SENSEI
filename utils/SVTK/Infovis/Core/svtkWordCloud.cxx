...
svtkStandardNewMacro(svtkWordCloud);
...
void AddReplacementPairsToStopList(svtkWordCloud*, svtkWordCloud::StopWordsContainer&);
bool AddWordToFinal(svtkWordCloud*, const std::string, const int, std::mt19937_64&,
  double orientation, std::vector<ExtentOffset>&, svtkImageBlend*, std::string&);

void ArchimedesSpiral(std::vector<ExtentOffset>&, svtkWordCloud::SizesContainer&);
void ReplaceMaskColorWithBackgroundColor(svtkImageData*, svtkWordCloud*);
void CreateBuiltInStopList(svtkWordCloud::StopWordsContainer& StopList);
void CreateStopListFromFile(std::string, svtkWordCloud::StopWordsContainer&);
void ExtractWordsFromString(std::string& str, std::vector<std::string>& words);
void ShowColorSeriesNames(ostream& os);
}

//----------------------------------------------------------------------------
svtkWordCloud::svtkWordCloud()
...
//--------------------------------------------------------------------------
int svtkWordCloud::RequestInformation(svtkInformation* svtkNotUsed(request),
...
//--------------------------------------------------------------------------
int svtkWordCloud::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** svtkNotUsed(inputVector), svtkInformationVector* outputVector)
{
  // Get the data object
  svtkInformation* outInfo = outputVector->GetInformationObject(0);
  ...
  {
    svtkErrorMacro(<< "No FileName is set. Use SetFileName to set a file.");
    return 0;
  }
  if (!svtksys::SystemTools::FileExists(this->FileName, true))
  {
    svtkErrorMacro(<< "FileName " << this->FileName << " does not exist");
    return 0;
  }
  if (this->FontFileName.size() > 0 && !svtksys::SystemTools::FileExists(this->FontFileName, true))
  {
    svtkErrorMacro(<< "FontFileName " << this->FontFileName << " does not exist");
    return 0;
  }
  if (this->MaskFileName.size() > 0 && !svtksys::SystemTools::FileExists(this->MaskFileName, true))
  {
    svtkErrorMacro(<< "MaskFileName " << this->MaskFileName << " does not exist");
    return 0;
  }
  if (this->StopListFileName.size() > 0 &&
    !svtksys::SystemTools::FileExists(this->StopListFileName, true))
  {
    svtkErrorMacro(<< "StopListFileName " << this->StopListFileName << " does not exist");
    return 0;
  }
  // Open the text file
  svtksys::ifstream t(this->FileName.c_str());
  ...
  svtkColor3ub maskColor = colors->GetColor3ub(this->MaskColorName.c_str());
  auto maskImage = svtkSmartPointer<svtkImageData>::New();

  // If a mask file is not defined, create a rectangular
  if (this->MaskFileName == "")
  {
    auto defaultMask = svtkSmartPointer<svtkImageCanvasSource2D>::New();
  ...
  {
    // Read the mask file
    auto readerFactory = svtkSmartPointer<svtkImageReader2Factory>::New();
    ...
    auto resize = svtkSmartPointer<svtkImageResize>::New();
    ...
    if (this->BWMask)
    {
      auto appendFilter = svtkSmartPointer<svtkImageAppendComponents>::New();
    ...
    {
      auto rgbImage = svtkSmartPointer<svtkImageExtractComponents>::New();
  ...
  auto final = svtkSmartPointer<svtkImageBlend>::New();
...
//--------------------------------------------------------------------------
void svtkWordCloud::PrintSelf(ostream& os, svtkIndent indent)
{
...
{
std::multiset<std::pair<std::string, int>, Comparator> FindWordsSortedByFrequency(
  std::string& s, svtkWordCloud* wordCloud)
{
  // Create a stop list
  svtkWordCloud::StopWordsContainer stopList;
...
void AddReplacementPairsToStopList(
  svtkWordCloud* wordCloud, svtkWordCloud::StopWordsContainer& stopList)
{
  for (auto p : wordCloud->GetReplacementPairs())
  {
    std::string from = std::get<0>(p);
...
bool AddWordToFinal(svtkWordCloud* wordCloud, const std::string word, const int frequency,
  std::mt19937_64& mt, double orientation, std::vector<ExtentOffset>& offset, svtkImageBlend* final,
  std::string& errorMessage)
{
  ...
  auto colors = svtkSmartPointer<svtkNamedColors>::New();
  auto textProperty = svtkSmartPointer<svtkTextProperty>::New();
  ...
  {
    auto colorScheme = svtkSmartPointer<svtkColorSeries>::New();
  ...
  if (wordCloud->GetFontFileName().length() > 0)
  {
    ...
    textProperty->SetFontFamily(SVTK_FONT_FILE);
  }
  else
  {
  ...
  svtkFreeTypeTools* freeType = svtkFreeTypeTools::GetInstance();
  freeType->ScaleToPowerTwoOff();

  auto textImage = svtkSmartPointer<svtkImageData>::New();
  ...
  svtkColor3ub maskColor = colors->GetColor3ub(wordCloud->GetMaskColorName().c_str());
  ...
  for (auto of : offset)
  {
      ...
      offsetY >= 0)
    {
      ...
      auto image = svtkSmartPointer<svtkImageData>::New();
      final->Update();

      // Does the text image overlap with images on the final image
      svtkImageIterator<unsigned char> finalIt(final->GetOutput(), textImage->GetExtent());
      bool good = true;
...
void ArchimedesSpiral(std::vector<ExtentOffset>& offset, svtkWordCloud::SizesContainer& sizes)
{
...
void ReplaceMaskColorWithBackgroundColor(svtkImageData* finalImage, svtkWordCloud* wordCloud)
{
  auto colors = svtkSmartPointer<svtkNamedColors>::New();

  svtkColor3ub backgroundColor = colors->GetColor3ub(wordCloud->GetBackgroundColorName().c_str());
  ...
  svtkImageIterator<unsigned char> finalIt(finalImage, finalImage->GetExtent());
  while (!finalIt.IsAtEnd())
  {
    auto finalSpan = finalIt.BeginSpan();
    while (finalSpan != finalIt.EndSpan())
...
void ShowColorSeriesNames(ostream& os)
{
  auto colorSeries = svtkSmartPointer<svtkColorSeries>::New();
...
void CreateStopListFromFile(std::string fileName, svtkWordCloud::StopWordsContainer& stopList)
{
  svtksys::ifstream t(fileName.c_str());
  std::stringstream buffer;
  buffer << t.rdbuf();
  std::string s = buffer.str();
  t.close();
...
void ExtractWordsFromString(std::string& str, std::vector<std::string>& words)
{
  ...
  return;
#else
  svtksys::RegularExpression re("([0-9A-Za-z]+)");
  size_t next = 0;
