...
=========================================================================*/
#ifndef svtkWordCloud_h
#define svtkWordCloud_h

#include "svtkImageAlgorithm.h"
#include "svtkImageData.h"         // For ImageData
 ...
 * depend on the frequency of a word in a document. svtkWordCloud
 ...
 * NOTE: svtkWordCloud has a built-in list of stop word. Stop words are
 * words that are filtered out before processing of the text, such as
 * the, is, at, which, and so on.
 *
 * NOTE: Color names are defined in svtkNamedColors. A visual
 * representation of color names is here: https://tinyurl.com/y3yxcxj6
 *
 * NOTE: svtkWordCloud offers Several methods to customize the resulting
 * visualization. The class provides defaults that provide a reasonable
 * result.
 *
 * BackgroundColorName - The svtkNamedColors name for the background
 ...
 *
 * ColorSchemeName - Name of a color scheme from svtkColorSeries to be
 ...
 * name of the svtknamedColors that defines the foreground of the
 ...
 * image using svtkImageAppendComponents.
 *
 ...
 * "bill" with "Bill Lorensen" or, "svtk" with "SVTK . Remember that
 ...
 * at, which, and so on.  svtkWordClass has built-in stop words. The
 ...
 * selected from svtkNamedColors. If the name is empty, the
...
class SVTKINFOVISCORE_EXPORT svtkWordCloud : public svtkImageAlgorithm
{
public:
  svtkTypeMacro(svtkWordCloud, svtkImageAlgorithm);
  void PrintSelf(ostream& os, svtkIndent indent) override;

  /**
   * Construct object with vertex cell generation turned off.
   */
  static svtkWordCloud* New();
...
#define SetStdContainerMacro(name, container)                                                      \
  ...
  //@{
  /**
   * Set/Get the svtkNamedColors name for the background(MidNightBlue).
   */
  //@}
   ...
   * svtkColorScheme to be used to select colors for the words (), if
   ...
   */
  //@}
  svtkSetMacro(Gap, int);
  svtkGetMacro(Gap, int);

  //@{
  /**
   * Set/Get MaskColorName, the name of the color for the mask
   * (black). This is the name of the svtkNamedColors that defines
   ...
   */
  //@}
  svtkSetMacro(MaxFontSize, int);
  svtkGetMacro(MaxFontSize, int);
   ...
   */
  //@}
  svtkSetMacro(MinFontSize, int);
  svtkGetMacro(MinFontSize, int);
   ...
   */
  //@}
  svtkSetMacro(MinFrequency, int);
  svtkGetMacro(MinFrequency, int);
   ...
   */
  //@}
  svtkSetMacro(FontMultiplier, int);
  svtkGetMacro(FontMultiplier, int);
   ...
   * words(). svtkWordClass has built-in stop words. The user-provided
   ...
   * words(). The name is selected from svtkNamedColors. If the name
...
private:
  svtkWordCloud(const svtkWordCloud&) = delete;
  void operator=(const svtkWordCloud&) = delete;
...
//  LocalWords:  svtkNamedColors SetMaskColorName
