/*=========================================================================
  ...
  Module:    svtkAttributeClustering2DLayoutStrategy.cxx
...
#include "svtkAttributeClustering2DLayoutStrategy.h"
...
class svtkAttributeClustering2DLayoutStrategy::Internals
{
public:
  ...
  std::vector<svtkLayoutEdge> Edges;
};

svtkStandardNewMacro(svtkAttributeClustering2DLayoutStrategy);
...
svtkAttributeClustering2DLayoutStrategy::svtkAttributeClustering2DLayoutStrategy()
  : Implementation(new Internals())
{

  // Create internal svtk classes
  this->DensityGrid = svtkSmartPointer<svtkFastSplatter>::New();
  this->SplatImage = svtkSmartPointer<svtkImageData>::New();
  this->RepulsionArray = svtkSmartPointer<svtkFloatArray>::New();
  this->AttractionArray = svtkSmartPointer<svtkFloatArray>::New();
  this->EdgeCountArray = svtkSmartPointer<svtkIntArray>::New();
...
void svtkAttributeClustering2DLayoutStrategy::SetVertexAttribute(const char* att)
{
...
// Helper functions
void svtkAttributeClustering2DLayoutStrategy::GenerateCircularSplat(
  svtkImageData* splat, int x, int y)
{
  splat->SetDimensions(x, y, 1);
  splat->AllocateScalars(SVTK_FLOAT, 1);
...
void svtkAttributeClustering2DLayoutStrategy::GenerateGaussianSplat(
  svtkImageData* splat, int x, int y)
{
  splat->SetDimensions(x, y, 1);
  splat->AllocateScalars(SVTK_FLOAT, 1);
...
void svtkAttributeClustering2DLayoutStrategy::Initialize()
{
  if (!this->VertexAttribute)
  {
    svtkErrorMacro("Layout strategy requires VertexAttribute to be set");
  ...
  svtkPoints* pts = this->Graph->GetPoints();
  svtkIdType numVertices = this->Graph->GetNumberOfVertices();

  // Make sure output point type is float
  if (pts->GetData()->GetDataType() != SVTK_FLOAT)
  {
    svtkErrorMacro("Layout strategy expects to have points of type float");
    this->LayoutComplete = 1;
    return;
  }

  // Get a quick pointer to the point data
  svtkFloatArray* array = svtkArrayDownCast<svtkFloatArray>(pts->GetData());
  ...
  for (svtkIdType i = 0; i < numVertices * 3; ++i)
  {
  ...
  for (svtkIdType i = 0; i < numVertices * 3; i += 3)
  {
    rawPointData[i] += this->RestDistance * (svtkMath::Random() - .5);
    rawPointData[i + 1] += this->RestDistance * (svtkMath::Random() - .5);
  }
  ...
  {
    svtkVariant v_source = vertexArr->GetVariantValue(i);
    for (int k = i; k < vertexArr->GetNumberOfTuples(); ++k)
    {
      svtkVariant v_target = vertexArr->GetVariantValue(k);
      if (v_source == v_target)
      {
        Internals::svtkLayoutEdge e;
        e.from = i;
        e.to = k;
        e.dead_edge = 0;
...
void svtkAttributeClustering2DLayoutStrategy::Layout()
{
  // Do I have a graph to layout?
  if (this->Graph == nullptr)
  {
    svtkErrorMacro("Graph Layout called with Graph==nullptr, call SetGraph(g) first");
    this->LayoutComplete = 1;
    return;
  }
  ...
  {
    svtkErrorMacro("Graph Layout already considered complete");
    return;
  }
  ...
  float attractValue;
  float epsilon = 1e-5;
  svtkIdType rawSourceIndex = 0;
  svtkIdType rawTargetIndex = 0;
  for (int i = 0; i < this->IterationsPerLayout; ++i)
  {

    // Initialize the repulsion and attraction arrays
    for (svtkIdType j = 0; j < numVertices * 3; ++j)
    {
    ...
    if (this->DensityGrid->GetOutput()->GetScalarType() != SVTK_FLOAT)
    {
      svtkErrorMacro("DensityGrid expected to be of type float");
      return;
    }
    ...
    for (svtkIdType j = 0; j < numVertices; ++j)
    {
      rawSourceIndex = j * 3;
    ...
    std::vector<Internals::svtkLayoutEdge>::iterator iter;
    ...
    for (svtkIdType j = 0; j < numVertices; ++j)
    {
      ...
      float pNormalize = svtkMath::Min(1.0f, 1.0f / forceDiv);
...
void svtkAttributeClustering2DLayoutStrategy::ResolveCoincidentVertices()
{
  ...
  svtkBitArray* giantGrid = svtkBitArray::New();
  svtkIdType xDim = static_cast<int>(sqrt(static_cast<double>(numVertices)) * 10);
  svtkIdType yDim = static_cast<int>(sqrt(static_cast<double>(numVertices)) * 10);
  svtkIdType gridSize = xDim * yDim;
  giantGrid->SetNumberOfValues(gridSize);

  // Initialize array to zeros
  for (svtkIdType i = 0; i < gridSize; ++i)
  {
  ...
  for (svtkIdType i = 0; i < numVertices; ++i)
  {
    ...
    if (giantGrid->GetValue(indexX + indexY * xDim))
    {
      ...
      while (collision && (collisionOps < 10))
      {
        ...
        // Move
        rawPointData[rawIndex] += jumpDistanceX * (svtkMath::Random() - .5);
        rawPointData[rawIndex + 1] += jumpDistanceY * (svtkMath::Random() - .5);
  ...
  // svtkWarningMacro("Collision Ops: " << totalCollisionOps);
}

void svtkAttributeClustering2DLayoutStrategy::PrintSelf(ostream& os, svtkIndent indent)
{
