...
Module:    svtkConeLayoutStrategy.cxx
...
svtkStandardNewMacro(svtkConeLayoutStrategy);

svtkConeLayoutStrategy::svtkConeLayoutStrategy()
{
  this->Compression = 0;
...
svtkConeLayoutStrategy::~svtkConeLayoutStrategy() = default;

double svtkConeLayoutStrategy::LocalPlacement(svtkIdType node, svtkPoints* points)
{
  svtkIdType child, prevChild, nrChildren;
  SVTK_CREATE(svtkOutEdgeIterator, children);
  ...
  if (nrChildren == 1)
  {
    ...
    // child.
    radius = this->LocalPlacement(children->NextGraphEdge()->GetTarget(), points);
    return radius;
  }
  ...
  for (svtkIdType j = 0; j < nrChildren; j++)
  {
    child = children->NextGraphEdge()->GetTarget();
...
void svtkConeLayoutStrategy::GlobalPlacement(svtkIdType root, svtkPoints* points,
...
void svtkConeLayoutStrategy::Layout()
{
  SVTK_CREATE(svtkMutableDirectedGraph, superGraph);
  SVTK_CREATE(svtkPoints, points);
  svtkIdType numVerts = this->Graph->GetNumberOfVertices();

  svtkGraph* temp;
  svtkIdType node, root, nrRoots;

  SVTK_CREATE(svtkPoints, tmpPoints);
  ...
  if (nrRoots == 0)
  {
    svtkErrorMacro(<< "No roots found in input dataset - output may be ill-defined.");
  }
...
void svtkConeLayoutStrategy::PrintSelf(ostream& os, svtkIndent indent)
{
