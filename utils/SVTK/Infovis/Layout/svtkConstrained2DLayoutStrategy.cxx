/*=========================================================================
  ...
  Module:    svtkConstrained2DLayoutStrategy.cxx
...
#include "svtkConstrained2DLayoutStrategy.h"
...
svtkStandardNewMacro(svtkConstrained2DLayoutStrategy);
...
svtkConstrained2DLayoutStrategy::svtkConstrained2DLayoutStrategy()
{
...
// Helper functions
void svtkConstrained2DLayoutStrategy::GenerateCircularSplat(svtkImageData* splat, int x, int y)
{
...
void svtkConstrained2DLayoutStrategy::GenerateGaussianSplat(svtkImageData* splat, int x, int y)
{
  splat->SetDimensions(x, y, 1);
  splat->AllocateScalars(SVTK_FLOAT, 1);
...
void svtkConstrained2DLayoutStrategy::Initialize()
{
  ...
  this->EdgeArray = new svtkLayoutEdge[numEdges];
...
void svtkConstrained2DLayoutStrategy::Layout()
{
  ...
  svtkDoubleArray* constraint = svtkArrayDownCast<svtkDoubleArray>(
    this->Graph->GetVertexData()->GetArray(this->GetInputArrayName()));
  if (constraint == nullptr)
  {
    svtkErrorMacro("svtkConstrained2DLayoutStrategy did not find a \"constraint\" array."
  ...
  for (int i = 0; i < this->IterationsPerLayout; ++i)
  {
    ...
    for (svtkIdType j = 0; j < numEdges; ++j)
    {
...
void svtkConstrained2DLayoutStrategy::ResolveCoincidentVertices()
{
  ...
  for (svtkIdType i = 0; i < numVertices; ++i)
  {
    ...
    if (giantGrid->GetValue(indexX + indexY * xDim))
    {
      ...
      while (collision && (collisionOps < 10))
      {
        ...
        // Move
        rawPointData[rawIndex] += jumpDistance * (svtkMath::Random() - .5);
        rawPointData[rawIndex + 1] += jumpDistance * (svtkMath::Random() - .5);
...
void svtkConstrained2DLayoutStrategy::PrintSelf(ostream& os, svtkIndent indent)
{
