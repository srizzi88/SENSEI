...
#undef SVTK_COSMIC_DBG

svtkStandardNewMacro(svtkCosmicTreeLayoutStrategy);

/// Represent a circle to be placed
class svtkCosmicTreeEntry
{
public:
  svtkCosmicTreeEntry(svtkIdType id, svtkIdType index, double radius)
  {
  ...
  double PlaceCounterClockwise(const svtkCosmicTreeEntry& neighbor, double Re)
  {
    ...
    if (dRe < 0)
    {
      ...
      this->Alpha = neighbor.Alpha + svtkMath::Pi();
  ...
  double Defect(const svtkCosmicTreeEntry& other) const
  {
  ...
  double Defect(const svtkCosmicTreeEntry& neighbor, double Re)
  {
    double ri = neighbor.Radius;
    double rj = this->Radius;
    double rij = ri + rj;
  ...
  bool operator<(const svtkCosmicTreeEntry& other) const
  {
 ...
 * need it as a svtkIdType).
 * @param[in,out] circles A vector of (x,y,z,r,child,idx) tuples for each node.
 * @param[out] Re The radius of the enclosing circle.
 */
static int svtkCosmicTreeLayoutStrategyComputeCentersQuick(
  svtkIdType N, std::vector<svtkCosmicTreeEntry>& circles, double& Re)
{
  int i;
  ...
  else
  {
    ...
    double Rtot = 0.;
    const double twopi = 2. * svtkMath::Pi();
    ...
    const double limit = 0.75 * svtkMath::Pi();
    for (i = 0; i < N; ++i)
    {
    ...
    do
    {
      ...
      for (i = 0; i < N; ++i)
      {
        svtkCosmicTreeEntry* circ = &circles[i];
...
svtkCosmicTreeLayoutStrategy::svtkCosmicTreeLayoutStrategy()
{
...
void svtkCosmicTreeLayoutStrategy::PrintSelf(ostream& os, svtkIndent indent)
{
...
void svtkCosmicTreeLayoutStrategy::Layout()
{
  ...
  if (numVertices == 0)
  {
    svtkWarningMacro("Tree has no vertices.");
    return;
  }
  ...
  svtkDoubleArray* radii; // radius of each node. May be read-only, read-write, or write-only.
  svtkDoubleArray*
    scale; // scale factor associated with each non-leaf node when SizeLeafNodesOnly is false.
  svtkDataArray* inputRadii = nullptr;
  ...
  else
  {
    ...
    scale->Delete();
    radii = svtkArrayDownCast<svtkDoubleArray>(inputRadii);
  ...
  svtkIdType currentRoot = this->LayoutRoot < 0 ? tree->GetRoot() : this->LayoutRoot;
  ...
  this->OffsetChildren(tree, newPoints, radii, scale, metaRoot, currentRoot,
    this->LayoutDepth < 0 ? 0 : this->LayoutDepth, mode);
#ifdef SVTK_COSMIC_DBG
  cout << "octr = [ ";
  for (svtkIdType k = 0; k < newPoints->GetNumberOfPoints(); ++k)
  {
...
void svtkCosmicTreeLayoutStrategy::LayoutChildren(svtkTree* tree, svtkPoints* pts,
  svtkDoubleArray* radii, svtkDoubleArray* scale, svtkIdType root, int depth, RadiusMode mode)
{
  svtkIdType child;
  svtkIdType childIdx;
  svtkIdType numberOfChildren = tree->GetNumberOfChildren(root);

  // State for the layout:
  double
    Rext; // The size of a circle that encloses the children (or the scaling factor when mode==ALL).
  std::vector<svtkCosmicTreeEntry> circles;
  ...
  switch (mode)
  {
    case ALL:
      ...
      for (childIdx = 0; childIdx < numberOfChildren; ++childIdx)
      {
        ...
        circles.push_back(svtkCosmicTreeEntry(child, childIdx, radii->GetValue(child)));
      }
      break;
    case NONE:
  ...
  {
    svtkCosmicTreeLayoutStrategyComputeCentersQuick(numberOfChildren, circles, Rext);
    std::vector<svtkCosmicTreeEntry>::iterator cit;
...
void svtkCosmicTreeLayoutStrategy::OffsetChildren(svtkTree* tree, svtkPoints* pts,
  svtkDoubleArray* radii, svtkDoubleArray* scale, double parent[4], svtkIdType root, int depth,
  RadiusMode mode)
{
...
svtkDoubleArray* svtkCosmicTreeLayoutStrategy::CreateRadii(
  svtkIdType numVertices, double initialValue, svtkDataArray* inputRadii)
{
...
svtkDoubleArray* svtkCosmicTreeLayoutStrategy::CreateScaleFactors(svtkIdType numVertices)
{
  svtkDoubleArray* scale = svtkDoubleArray::New();
