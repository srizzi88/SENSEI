...
public:
  ...
  void Insert(svtkVector2f& p, svtkIdType vert, float x1, float y1, float x2, float y2);
  void InsertChild(svtkVector2f& p, svtkIdType vert, float x1, float y1, float x2, float y2);
  void ForceAccumulate(float alpha, float charge);
  bool Repulse(svtkVector2f& prev, svtkVector2f& p, svtkIdType vert, float x1, float x2, float theta);
  void Visit(svtkVector2f& prev, svtkVector2f& p, svtkIdType vert, float x1, float y1, float x2,
...
Quad::Quad()
{
  ...
  this->Point = svtkVector2f(0.0f, 0.0f);
  this->Vertex = 0;
  this->Charge = 0.0f;
...
Quad::Quad(float* points, int n, float x1, float y1, float x2, float y2)
{
  ...
  for (int i = 0; i < n; ++i)
  {
    svtkVector2f p = *reinterpret_cast<svtkVector2f*>(points + 3 * i);
...
void Quad::Insert(svtkVector2f& p, svtkIdType vert, float x1, float y1, float x2, float y2)
{
  if (svtkMath::IsNan(p.GetX()) || svtkMath::IsNan(p.GetY()))
  {
    return;
  }
  if (this->Leaf)
...
void Quad::InsertChild(svtkVector2f& p, svtkIdType vert, float x1, float y1, float x2, float y2)
{
...
void Quad::ForceAccumulate(float alpha, float charge)
{
  ...
  if (this->ValidPoint)
  {
    ...
    if (!this->Leaf)
    {
      this->Point.SetX(this->Point.GetX() + static_cast<float>(svtkMath::Random()) - 0.5f);
      this->Point.SetY(this->Point.GetY() + static_cast<float>(svtkMath::Random()) - 0.5f);
    }
  ...
  this->Center = svtkVector2f(cx / this->Charge, cy / this->Charge);
}

bool Quad::Repulse(
  svtkVector2f& prev, svtkVector2f& p, svtkIdType vert, float x1, float x2, float theta)
{
  if (this->Vertex != vert)
  {
    ...
    else if (this->ValidPoint && !svtkMath::IsNan(dn) && !svtkMath::IsInf(dn))
    {
...
void Quad::Visit(svtkVector2f& prev, svtkVector2f& p, svtkIdType vert, float x1, float y1, float x2,
  float y2, float theta)
{
...
class svtkIncrementalForceLayout::Implementation
{
public:
  Implementation() = default;

  svtkVector2f& GetPosition(svtkIdType i)
  {
    return *(reinterpret_cast<svtkVector2f*>(this->Position + 3 * i));
  }

  float* Position;
  std::vector<svtkVector2f> LastPosition;
};

svtkStandardNewMacro(svtkIncrementalForceLayout);
svtkCxxSetObjectMacro(svtkIncrementalForceLayout, Graph, svtkGraph);

svtkIncrementalForceLayout::svtkIncrementalForceLayout()
{
  this->Impl = new Implementation();
  this->Graph = nullptr;
  this->Fixed = -1;
  this->GravityPoint = svtkVector2f(200.0f, 200.0f);
  this->Alpha = 0.1f;
  this->Theta = 0.8f;
  this->Charge = -50.0f;
  this->Strength = 1.0f;
...
void svtkIncrementalForceLayout::UpdatePositions()
{
  if (!this->Graph)
  {
    return;
  ...
  this->Impl->Position =
    svtkArrayDownCast<svtkFloatArray>(this->Graph->GetPoints()->GetData())->GetPointer(0);
  while (numVerts >= static_cast<svtkIdType>(this->Impl->LastPosition.size()))
  {
    this->Impl->LastPosition.push_back(svtkVector2f(0.0f, 0.0f));
  }

  // Swap pos and lastpos for fixed node
  if (this->Fixed >= 0 && this->Fixed < numVerts)
  {
    svtkVector2f& pos = this->Impl->GetPosition(this->Fixed);
    svtkVector2f temp = pos;
  ...
  {
    svtkIdType s = this->Graph->GetSourceVertex(e);
    svtkIdType t = this->Graph->GetTargetVertex(e);
    svtkVector2f& sPos = this->Impl->GetPosition(s);
    svtkVector2f& tPos = this->Impl->GetPosition(t);
  ...
  if (k)
  {
    ...
    {
      svtkVector2f& vPos = this->Impl->GetPosition(v);
      if (v != this->Fixed)
      {
  ...
  // Bounds
  float x1 = SVTK_FLOAT_MAX;
  float x2 = SVTK_FLOAT_MIN;
  float y1 = SVTK_FLOAT_MAX;
  float y2 = SVTK_FLOAT_MIN;
  for (svtkIdType i = 0; i < numVerts; ++i)
  {
    svtkVector2f p = this->Impl->GetPosition(i);
  ...
  {
    svtkVector2f& vPos = this->Impl->GetPosition(v);
    svtkVector2f& vLastPos = this->Impl->LastPosition[v];
    if (v != this->Fixed)
    {
...
void svtkIncrementalForceLayout::SetFixed(svtkIdType v)
{
  if (this->Fixed >= 0 && this->Fixed < static_cast<svtkIdType>(this->Impl->LastPosition.size()))
  {
...
void svtkIncrementalForceLayout::PrintSelf(ostream& os, svtkIndent indent)
{
