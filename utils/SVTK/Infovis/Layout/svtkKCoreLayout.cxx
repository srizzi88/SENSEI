...
///////////////////////////////////////////////////////////////////////////////////
// svtkKCoreLayout
svtkStandardNewMacro(svtkKCoreLayout);

// Default Constructor
svtkKCoreLayout::svtkKCoreLayout()
{
...
// -------------------------------------------------------------------------------------------------
void svtkKCoreLayout::PrintSelf(ostream& os, svtkIndent indent)
{
...
// -------------------------------------------------------------------------------------------------
void svtkKCoreLayout::SetGraphConnection(svtkAlgorithmOutput* input)
{
  this->SetInputConnection(0, input);
}

// -------------------------------------------------------------------------------------------------
int svtkKCoreLayout::FillInputPortInformation(int port, svtkInformation* info)
{
  if (port == 0)
  {
...
// -------------------------------------------------------------------------------------------------
int svtkKCoreLayout::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  // cout << ">\tsvtkKCoreLayout.RequestData()" << endl;
  ...
  // graph size
  svtkIdType num_verts = output->GetNumberOfVertices();
  ...
  svtkIntArray* kcore_array =
    svtkArrayDownCast<svtkIntArray>(output->GetVertexData()->GetArray(this->KCoreLabelArrayName));

  if (!kcore_array)
  {
    svtkErrorMacro(<< "Vertex attribute array " << this->KCoreLabelArrayName
                  << " is not a svtkIntArray.");
    return 0;
  }

  SVTK_CREATE(svtkDoubleArray, arrayX);
  SVTK_CREATE(svtkDoubleArray, arrayY);
  SVTK_CREATE(svtkDoubleArray, arrayRadius);
  SVTK_CREATE(svtkDoubleArray, arrayAngle);
  ...
  {
    svtkWarningMacro(<< "Neither Polar nor Cartesian output coordinate type specified.  Operating "
                       "as a Pass-Through filter");
    return 1;
  }

  // find the maximum core level
  for (svtkIdType i = 0; i < kcore_array->GetNumberOfTuples(); i++)
  {
    max_core_level =
  ...
  for (svtkIdType vidx = 0; vidx < num_verts; vidx++)
  {
    int current_level = kcore_array->GetValue(vidx);
    if (current_level == max_core_level)
    {
      radius = unit_radius;
      angle = float(rand() % 100000) / 100000 * 2.0 * svtkMath::Pi();
    ...
    else
    {
      ...
      {
        for (svtkIdType i = 0; i < neighbors_same_or_higher->GetNumberOfTuples(); i++)
        {
          svtkIdType neighbor_idx = neighbors_same_or_higher->GetValue(i);
