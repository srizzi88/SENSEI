...
Module:    svtkSpanTreeLayoutStrategy.cxx
...
// File: svtkSpanTreeLayoutStrategy.cxx
// Graph visualization library for SVTK
// (c) 2003 D.J. Duke
...
svtkStandardNewMacro(svtkSpanTreeLayoutStrategy);

svtkSpanTreeLayoutStrategy::svtkSpanTreeLayoutStrategy()
{
  this->TreeLayout = svtkConeLayoutStrategy::New();
...
void svtkSpanTreeLayoutStrategy::Layout()
{
  svtkSmartPointer<svtkPoints> points = svtkSmartPointer<svtkPoints>::New();
  svtkSmartPointer<svtkMutableDirectedGraph> spanningDAG =
  ...
  svtkSmartPointer<svtkGraphLayout> layoutWorker = svtkSmartPointer<svtkGraphLayout>::New();
  svtkSmartPointer<svtkOutEdgeIterator> outEdges = svtkSmartPointer<svtkOutEdgeIterator>::New();
  svtkSmartPointer<svtkInEdgeIterator> inEdges = svtkSmartPointer<svtkInEdgeIterator>::New();

  // Auxiliary structures used for building a spanning tree
  int* level;
  int* marks;
  svtkIdType* queue;
  svtkIdType front, back;

  // Handle for the layout computed for the spanning tree
  svtkPoints* layout;

  // Auxiliary structures for placing bends into edges.
  _svtkBridge_s* editlist;
  _svtkBridge_s link;
  link.delta = 0;
  link.anchor[1] = 0;
  svtkIdType editsize;
  svtkEdgeType edge;
  svtkIdType i, nrNodes, nrEdges;
  double pointS[3], pointT[3], pointA[3];
  double edgePoints[6];

  // ----------------------------------------------------------

  svtkDebugMacro(<< "svtkSpanTreeLayoutStrategy executing.");
  ...
  if (nrNodes == 0 || nrEdges == 0 || !this->TreeLayout)
  {
    if (nrNodes == 0)
    {
      svtkErrorMacro(<< "Cannot execute - no nodes in input.");
    }
    if (nrEdges == 0)
    {
      svtkErrorMacro(<< "Cannot execute - no edges in input.");
    }
    if (!this->TreeLayout)
    {
      svtkErrorMacro(<< "Cannot execute - no tree layout strategy.");
    }
    return;
  }
  ...
  queue = new svtkIdType[nrNodes];

  // Initialize spanning tree with all vertices of the graph.
  for (svtkIdType v = 0; v < nrNodes; v++)
  {
  ...
  for (svtkIdType v = 0; v < nrNodes; v++)
  {
    if (!marks[v]) // not visited
    {
      ...
      while (back != front)
      {
        ...
        {
          svtkIdType dst = outEdges->Next().Target;
        ...
        {
          svtkIdType origin = inEdges->Next().Source;
  ...
  editsize = 0;
  editlist = new _svtkBridge_s[nrEdges];
  ...
  svtkDebugMacro(<< "SpanTreeLayoutStrategy complete.");
}

void svtkSpanTreeLayoutStrategy::PrintSelf(ostream& os, svtkIndent indent)
{
  svtkGraphLayoutStrategy::PrintSelf(os, indent);
