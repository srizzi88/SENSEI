/*=========================================================================
  ...
  Module:    svtkSpanTreeLayoutStrategy.h
...
/**
 * @class   svtkSpanTreeLayoutStrategy
 *
 * svtkSpanTreeLayout is a strategy for drawing directed graphs that
 ...
 * or balloon layout.
 *
 * svtkSpanTreeLayout is based on an approach to 3D graph layout first
 ...
 * (a) it is reconstructed to use Titan/SVTK data structures;
...
#ifndef svtkSpanTreeLayoutStrategy_h
#define svtkSpanTreeLayoutStrategy_h

#include "svtkGraphLayoutStrategy.h"
#include "svtkInfovisLayoutModule.h" // For export macro

class SVTKINFOVISLAYOUT_EXPORT svtkSpanTreeLayoutStrategy : public svtkGraphLayoutStrategy
{
public:
  static svtkSpanTreeLayoutStrategy* New();

  svtkTypeMacro(svtkSpanTreeLayoutStrategy, svtkGraphLayoutStrategy);
   ...
   */
  svtkSetMacro(DepthFirstSpanningTree, bool);
  svtkGetMacro(DepthFirstSpanningTree, bool);
  svtkBooleanMacro(DepthFirstSpanningTree, bool);
  //@}
...
protected:
  svtkSpanTreeLayoutStrategy();
  ~svtkSpanTreeLayoutStrategy() override;

  svtkGraphLayoutStrategy* TreeLayout;
  bool DepthFirstSpanningTree;

private:
  svtkSpanTreeLayoutStrategy(const svtkSpanTreeLayoutStrategy&) = delete;
  void operator=(const svtkSpanTreeLayoutStrategy&) = delete;
};
