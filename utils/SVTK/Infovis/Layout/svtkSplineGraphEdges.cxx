/*=========================================================================
  ...
  Module:    svtkSplineGraphEdges.cxx
...
svtkStandardNewMacro(svtkSplineGraphEdges);
svtkCxxSetObjectMacro(svtkSplineGraphEdges, Spline, svtkSpline);
...
// optimized for j = 3.
double CubicSpline(svtkIdType i, double* k, double t)
{
  if (t >= k[i] && t < k[i + 1])
  {
...
// http://mathworld.wolfram.com/B-Spline.html
#if 0
double N(svtkIdType i, svtkIdType j, double* k, double t)
{
  if (j < 0)
  {
    return 0.0;
...
#if 0
double BCubic(svtkIdType i, double* k, double t)
{
  if (t < k[i-2] || t >= k[i+2])
  {
    return 0.0;
...
svtkSplineGraphEdges::svtkSplineGraphEdges()
{
...
svtkMTimeType svtkSplineGraphEdges::GetMTime()
{
...
int svtkSplineGraphEdges::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  if (!this->Spline)
  {
    svtkErrorMacro("Must have a valid spline.");
    return 0;
  }
  ...
  for (svtkIdType i = 0; i < output->GetNumberOfEdges(); ++i)
  {
    ...
    if (i % 1000 == 0)
    {
      double progress = static_cast<double>(i) / static_cast<double>(output->GetNumberOfEdges());
...
void svtkSplineGraphEdges::GeneratePoints(svtkGraph* g, svtkIdType e)
{
  ...
  {
    double len = sqrt(svtkMath::Distance2BetweenPoints(x, xPrev));
    length += len;
  }
  ...
  for (xPrev = points, x = points + 3; x != xEnd; xPrev += 3, x += 3)
  {
    double dist = sqrt(svtkMath::Distance2BetweenPoints(x, xPrev));
    if (dist == 0)
    {
      continue;
    }
  ...
  svtkIdType numNewPoints = this->NumberOfSubdivisions - 1;
  double* newPoints = new double[3 * numNewPoints];
  svtkIdType i;
  for (i = 0, x = newPoints; i < numNewPoints; i++, x += 3)
  {
    double t = static_cast<double>(i + 1) / static_cast<double>(this->NumberOfSubdivisions);
...
void svtkSplineGraphEdges::GenerateBSpline(svtkGraph* g, svtkIdType e)
{
  svtkIdType numInternalPoints;
  double* internalPoints;
  g->GetEdgePoints(e, numInternalPoints, internalPoints);
  ...
  svtkIdType numKnots = numPoints + 4;
  ...
  for (i = 0, xNew = newPoints; i < numNewPoints; i++, xNew += 3)
  {
    ...
    double* x = points;
    // double bsum = 0.0;
    for (svtkIdType j = 0; j < numPoints; ++j, x += 3)
    {
...
void svtkSplineGraphEdges::PrintSelf(ostream& os, svtkIndent indent)
{
