/*=========================================================================
  ...
  Module:    svtkStackedTreeLayoutStrategy.cxx
...
svtkStandardNewMacro(svtkStackedTreeLayoutStrategy);

svtkStackedTreeLayoutStrategy::svtkStackedTreeLayoutStrategy()
{
...
svtkStackedTreeLayoutStrategy::~svtkStackedTreeLayoutStrategy() = default;

void svtkStackedTreeLayoutStrategy::PrintSelf(ostream& os, svtkIndent indent)
{
...
void svtkStackedTreeLayoutStrategy::Layout(
  svtkTree* inputTree, svtkDataArray* coordsArray, svtkDataArray* sizeArray)
{
  ...
  svtkDataSetAttributes* data = inputTree->GetVertexData();

  SVTK_CREATE(svtkDoubleArray, textRotationArray);
  ...
  SVTK_CREATE(svtkDoubleArray, textBoundedSizeArray);
  ...
  if (this->Reverse)
  {
    SVTK_CREATE(svtkTreeLevelsFilter, levelFilter);
    SVTK_CREATE(svtkTree, newTree);
    newTree->ShallowCopy(inputTree);
    levelFilter->SetInputData(newTree);
    levelFilter->Update();
    svtkTree* levelTree = levelFilter->GetOutput();

    svtkIntArray* levelArray =
      svtkArrayDownCast<svtkIntArray>(levelTree->GetVertexData()->GetAbstractArray("level"));
    int max_level = 0;
  ...
  svtkIdType numVerts = inputTree->GetNumberOfVertices();
  ...
  for (svtkIdType i = 0; i < numVerts; i++)
  {
    ...
    else
    {
      ...
      else
      {
        ...
        x = r * cos(svtkMath::RadiansFromDegrees(theta));
        y = r * sin(svtkMath::RadiansFromDegrees(theta));
        z = 0.;

        double sector_arc_length =
          r * svtkMath::RadiansFromDegrees(sector_coords[1] - sector_coords[0]);
...
void svtkStackedTreeLayoutStrategy::LayoutEdgePoints(svtkTree* inputTree, svtkDataArray* sectorsArray,
  svtkDataArray* svtkNotUsed(sizeArray), svtkTree* outputTree)
{
  SVTK_CREATE(svtkTreeLevelsFilter, levelFilter);
  SVTK_CREATE(svtkTree, newTree);
  newTree->ShallowCopy(inputTree);
  ...
  double exteriorRadius = SVTK_DOUBLE_MAX;
  double sector_coords[4];
  int max_level = 0;
  ...
  svtkPoints* points = svtkPoints::New();
  svtkIdType rootId = outputTree->GetRoot();
  svtkIdType numVerts = outputTree->GetNumberOfVertices();
  points->SetNumberOfPoints(numVerts);
  for (svtkIdType i = 0; i < numVerts; i++)
  {
...
void svtkStackedTreeLayoutStrategy::LayoutChildren(svtkTree* tree, svtkDataArray* coordsArray,
  svtkDataArray* sizeArray, svtkIdType nchildren, svtkIdType parent, svtkIdType begin,
  float parentInnerRad, float parentOuterRad, float parentStartAng, float parentEndAng)
{
  ...
  svtkIdType num_spaces = nchildren - 1;
  ...
  double conversion = svtkMath::Pi() / 180.0;
  double spacing = 0.0;
  if (nchildren > 1)
  {
...
svtkIdType svtkStackedTreeLayoutStrategy::FindVertex(
  svtkTree* otree, svtkDataArray* array, float pnt[2])
{
  if (this->UseRectangularCoordinates)
  {
    float blimits[4];
    ...
    svtkFloatArray* boundsInfo = svtkArrayDownCast<svtkFloatArray>(array);
    ...
    svtkIdType child;
    SVTK_CREATE(svtkTreeDFSIterator, it);
    it->SetTree(otree);
  ...
  else
  {
    ...
    polar_location[1] = svtkMath::DegreesFromRadians(atan2(pnt[1], pnt[0]));
