/*=========================================================================
  ...
  Module:    svtkInteractorStyleFlight.cxx
...
=========================================================================*/
#include "svtkInteractorStyleFlight.h"
...
svtkStandardNewMacro(svtkInteractorStyleFlight);
...
//---------------------------------------------------------------------------
svtkInteractorStyleFlight::svtkInteractorStyleFlight()
{
  this->KeysDown = 0;
  this->UseTimers = 1;
  ...
  Transform = svtkPerspectiveTransform::New();
}
...
//---------------------------------------------------------------------------
void svtkInteractorStyleFlight::ForwardFly()
{
...
//---------------------------------------------------------------------------
void svtkInteractorStyleFlight::ReverseFly()
{
...
//----------------------------------------------------------------------------
void svtkInteractorStyleFlight::StartForwardFly()
{
  if (this->State != SVTKIS_NONE)
  {
    return;
  }
  this->StartState(SVTKIS_FORWARDFLY);
}

//----------------------------------------------------------------------------
void svtkInteractorStyleFlight::EndForwardFly()
{
  if (this->State != SVTKIS_FORWARDFLY)
  {
    return;
  }
  this->StopState();
...
//----------------------------------------------------------------------------
void svtkInteractorStyleFlight::StartReverseFly()
{
  if (this->State != SVTKIS_NONE)
  {
    return;
  }
  this->StartState(SVTKIS_REVERSEFLY);
}

//----------------------------------------------------------------------------
void svtkInteractorStyleFlight::EndReverseFly()
{
  if (this->State != SVTKIS_REVERSEFLY)
  {
    return;
  }
  this->StopState();
...
//---------------------------------------------------------------------------
void svtkInteractorStyleFlight::OnTimer()
{
  switch (this->State)
  {
    case SVTKIS_FORWARDFLY:
      this->ForwardFly();
      break;

    case SVTKIS_REVERSEFLY:
      this->ReverseFly();
      break;
...
//---------------------------------------------------------------------------
void svtkInteractorStyleFlight::OnMouseMove()
{
...
//---------------------------------------------------------------------------
void svtkInteractorStyleFlight::OnLeftButtonDown()
{
  int x = this->Interactor->GetEventPosition()[0];
  int y = this->Interactor->GetEventPosition()[1];
  this->FindPokedRenderer(x, y);
...
//---------------------------------------------------------------------------
void svtkInteractorStyleFlight::OnLeftButtonUp()
{
  switch (this->State)
  {
    case SVTKIS_FORWARDFLY:
...
//---------------------------------------------------------------------------
void svtkInteractorStyleFlight::OnMiddleButtonDown() {}

//---------------------------------------------------------------------------
void svtkInteractorStyleFlight::OnMiddleButtonUp() {}

//---------------------------------------------------------------------------
void svtkInteractorStyleFlight::OnRightButtonDown()
{
  int x = this->Interactor->GetEventPosition()[0];
  int y = this->Interactor->GetEventPosition()[1];
  this->FindPokedRenderer(x, y);
...
//---------------------------------------------------------------------------
void svtkInteractorStyleFlight::OnRightButtonUp()
{
  switch (this->State)
  {
    case SVTKIS_REVERSEFLY:
...
//---------------------------------------------------------------------------
void svtkInteractorStyleFlight::OnKeyDown()
{
  ...
  {
    svtkWarningMacro(<< "Dummy test to prevent compiler warning");
  }
#endif
}

//---------------------------------------------------------------------------
void svtkInteractorStyleFlight::OnKeyUp()
{
#ifdef _WIN32
  switch (this->Interactor->GetKeyCode())
  {
...
//---------------------------------------------------------------------------
void svtkInteractorStyleFlight::OnChar()
{
  switch (this->Interactor->GetKeyCode())
  {
    case '+':
...
//---------------------------------------------------------------------------
void svtkInteractorStyleFlight::JumpTo(double campos[3], double focpos[3])
{
  if (this->CurrentRenderer == nullptr)
  {
    return;
...
void svtkInteractorStyleFlight::FinishCamera(svtkCamera* cam)
{
  ...
  if (this->RestoreUpVector)
  {
    double delta[3];
    cam->GetViewUp(delta);
    double weight = svtkMath::Dot(this->DefaultUpVector, delta);
...
//---------------------------------------------------------------------------
void svtkInteractorStyleFlight::UpdateMouseSteering(svtkCamera* cam)
{
...
void svtkInteractorStyleFlight::UpdateSteering(svtkCamera* svtkNotUsed(cam))
{
  /*
    ...
    double *ViewUp = cam->GetViewUp();
    svtkMath::Cross(vec1, ViewUp, norm1);
    svtkMath::Cross(vec2, ViewUp, norm2);
    svtkMath::Normalize(norm1);
    svtkMath::Normalize(norm2);
    //
    double dot = svtkMath::Dot(norm1, norm2);
    if (dot>1.0) {
      dot = 1.0;
    }
    else if (dot<-1.0) {
    ...
    double d1,d2;
    d1 = svtkMath::Dot(norm1,this->IdealFocalPoint);
    d2 = svtkMath::Dot(norm1,FocalPoint);
    if (d1-d2>0) yaw = -yaw;
    //
    ...
    double LRaxis[3];
    this->GetLRVector(LRaxis, cam);
    svtkMath::Cross(vec1, LRaxis, norm1);
    svtkMath::Cross(vec2, LRaxis, norm2);
    svtkMath::Normalize(norm1);
    svtkMath::Normalize(norm2);
    //
    dot = svtkMath::Dot(norm1, norm2);
    if (dot>1.0) {
      dot = 1.0;
    } else if (dot<-1.0) {
      dot =-1.0;
...
//---------------------------------------------------------------------------
void svtkInteractorStyleFlight::SetupMotionVars(svtkCamera* cam)
{
  lPitch = 0;
  lYaw = 0;
...
//---------------------------------------------------------------------------
void svtkInteractorStyleFlight::MotionAlongVector(double vector[3], double amount, svtkCamera* cam)
{
...
//---------------------------------------------------------------------------
void svtkInteractorStyleFlight::GetLRVector(double vector[3], svtkCamera* cam)
{
  svtkMatrix4x4* vtm;
...
// Perform the motion
//---------------------------------------------------------------------------
void svtkInteractorStyleFlight::FlyByMouse(svtkCamera* cam)
{
  double a_vector[3];
...
//---------------------------------------------------------------------------
void svtkInteractorStyleFlight::FlyByKey(svtkCamera* cam)
{
  double speed = this->DiagonalLength * this->MotionStepSize * this->MotionUserScale;
  speed = speed * (this->Interactor->GetShiftKey() ? this->MotionAccelerationFactor : 1.0);
  if (this->DisableMotion)
...
//---------------------------------------------------------------------------
void svtkInteractorStyleFlight::PrintSelf(ostream& os, svtkIndent indent)
{
