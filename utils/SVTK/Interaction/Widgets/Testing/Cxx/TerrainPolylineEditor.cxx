...
const char TerrainPolylineEditorLog[] = "# StreamVersion 1\n"
...
int TerrainPolylineEditor(int argc, char* argv[])
{
  if (argc < 2)
  {
    std::cerr << "Demonstrates editing capabilities of a contour widget on terrain \n"
              ...
              << "(one cell and two or more points on that cell)." << std::endl;
    std::cerr << "\n\nUsage: " << argv[0] << "\n"
              << "  [-ProjectionMode (0,1 or 2)]\n"
              << "  [-HeightOffset heightOffset]\n"
              << "  [-InitialPath SomeSVTKXmlfileContainingPath.svtk]" << std::endl;
    return EXIT_FAILURE;
  }
  ...
  svtkSmartPointer<svtkTerrainDataPointPlacer> pointPlacer =
    svtkSmartPointer<svtkTerrainDataPointPlacer>::New();
  ...
  svtkSmartPointer<svtkTerrainContourLineInterpolator> interpolator =
    svtkSmartPointer<svtkTerrainContourLineInterpolator>::New();
  ...
  for (int i = 0; i < argc - 1; i++)
  {
    ...
    if (strcmp("-InitialPath", argv[i]) == 0)
    {
      ...
      //
      svtkSmartPointer<svtkPolyDataReader> terrainPathReader =
