...
=========================================================================*/
//
// This example tests the svtkAngleWidget.
...
class svtkAngleCallback : public svtkCommand
{
public:
  static svtkAngleCallback* New() { return new svtkAngleCallback; }
  void Execute(svtkObject*, unsigned long eid, void*) override
  {
    if (eid == svtkCommand::PlacePointEvent)
    {
      std::cout << "point placed\n";
    }
    else if (eid == svtkCommand::InteractionEvent)
    {
      ...
      std::cout << "Angle between "
                ...
                << point2[1] << "," << point2[2] << ") is " << this->Rep->GetAngle() << std::endl;
    }
  }
  svtkAngleRepresentation2D* Rep;
  svtkAngleCallback()
    : Rep(nullptr)
  {
  }
...
int TestAngleWidget2D(int argc, char* argv[])
{
  ...
  svtkSmartPointer<svtkAngleRepresentation2D> rep = svtkSmartPointer<svtkAngleRepresentation2D>::New();
  rep->SetHandleRepresentation(handle);

  svtkSmartPointer<svtkAngleWidget> widget = svtkSmartPointer<svtkAngleWidget>::New();
  widget->SetInteractor(iren);
  widget->CreateDefaultRepresentation();
  widget->SetRepresentation(rep);

  svtkSmartPointer<svtkAngleCallback> mcbk = svtkSmartPointer<svtkAngleCallback>::New();
  mcbk->Rep = rep;
  widget->AddObserver(svtkCommand::PlacePointEvent, mcbk);
  widget->AddObserver(svtkCommand::InteractionEvent, mcbk);
  ...
  return svtkTesting::InteractorEventLoop(argc, argv, iren, TestAngleWidget2DEventLog);
}
