...
static char ButtonWidgetEventLog[] = "# StreamVersion 1\n"
...
class svtkButtonCallback : public svtkCommand
{
public:
  static svtkButtonCallback* New() { return new svtkButtonCallback; }
  void Execute(svtkObject* caller, unsigned long, void*) override
  {
    svtkButtonWidget* buttonWidget = reinterpret_cast<svtkButtonWidget*>(caller);
    svtkTexturedButtonRepresentation* rep =
      reinterpret_cast<svtkTexturedButtonRepresentation*>(buttonWidget->GetRepresentation());
...
int TestButtonWidget(int argc, char* argv[])
{
  ...
  SVTK_CREATE(svtkTIFFReader, image1);
  ...
  // Create an image for the button
  char* fname2 = svtkTestUtilities::ExpandDataFileName(argc, argv, "Data/fran_cut.png");
  SVTK_CREATE(svtkPNGReader, image2);
  ...
  SVTK_CREATE(svtkAppendPolyData, apd);
  apd->AddInputConnection(glyph->GetOutputPort());
  apd->AddInputConnection(sphere->GetOutputPort());

  SVTK_CREATE(svtkPolyDataMapper, maceMapper);
  maceMapper->SetInputConnection(apd->GetOutputPort());

  SVTK_CREATE(svtkActor, maceActor);
  ...
  SVTK_CREATE(svtkButtonCallback, myCallback);
  myCallback->Glyph = glyph;

  SVTK_CREATE(svtkEllipticalButtonSource, button);
  button->TwoSidedOn();
  button->SetCircumferentialResolution(24);
  button->SetShoulderResolution(24);
  button->SetTextureResolution(24);

  SVTK_CREATE(svtkTexturedButtonRepresentation, rep);
  ...
  SVTK_CREATE(svtkButtonWidget, buttonWidget);
  buttonWidget->SetInteractor(iren);
  buttonWidget->SetRepresentation(rep);
  buttonWidget->AddObserver(svtkCommand::StateChangedEvent, myCallback);

  // Another 3D button widget, this time use alternative PlaceWidget() method
  SVTK_CREATE(svtkEllipticalButtonSource, button2);
  button2->TwoSidedOn();
  ...
  SVTK_CREATE(svtkTexturedButtonRepresentation, rep2);
  ...
  SVTK_CREATE(svtkButtonWidget, buttonWidget2);
  buttonWidget2->SetInteractor(iren);
  buttonWidget2->SetRepresentation(rep2);
  buttonWidget2->AddObserver(svtkCommand::StateChangedEvent, myCallback);

  // Okay now for the 2D version of the widget (in display space)
  SVTK_CREATE(svtkTexturedButtonRepresentation2D, rep3);
  ...
  SVTK_CREATE(svtkButtonWidget, buttonWidget3);
  buttonWidget3->SetInteractor(iren);
  buttonWidget3->SetRepresentation(rep3);
  buttonWidget3->AddObserver(svtkCommand::StateChangedEvent, myCallback);

  // Okay now for the 2D version of the widget (in world space)
  SVTK_CREATE(svtkTexturedButtonRepresentation2D, rep4);
  ...
  SVTK_CREATE(svtkButtonWidget, buttonWidget4);
  buttonWidget4->SetInteractor(iren);
  buttonWidget4->SetRepresentation(rep4);
  buttonWidget4->AddObserver(svtkCommand::StateChangedEvent, myCallback);

  // Finally now a set of svtkProp3D's to define a svtkProp3DButtonRepresentation
  ...
  SVTK_CREATE(svtkButtonWidget, buttonWidget5);
  buttonWidget5->SetInteractor(iren);
  buttonWidget5->SetRepresentation(rep5);
  buttonWidget5->AddObserver(svtkCommand::StateChangedEvent, myCallback);

  // Outline is for debugging
  SVTK_CREATE(svtkOutlineSource, outline);
