...
int TestCoincidentHandleWidgets(int argc, char* argv[])
{
  ...
  svtkNew<svtkSeedRepresentation> rep;
  seedWidget->SetRepresentation(rep);
  seedWidget->CompleteInteraction();
  seedWidget->RestartInteraction();
  svtkNew<svtkPointHandleRepresentation3D> handle;
  ...
  svtkHandleWidget* handleWidget = seedWidget->CreateNewHandle();
  handleWidget->EnabledOn();
  svtkHandleRepresentation* handleRep = rep->GetHandleRepresentation(0);
  double coords[3] = { 150, 150, 0 };
  rep->SetSeedDisplayPosition(0, coords);
  handleRep->VisibilityOn();
  seedWidget->GetSeed(0)->EnabledOn();

  svtkHandleWidget* handleWidget2 = seedWidget->CreateNewHandle();
  handleWidget2->EnabledOff();
  svtkHandleRepresentation* handleRep2 = rep->GetHandleRepresentation(1);
  ...
  // Exercise the test
  return svtkTesting::InteractorEventLoop(argc, argv, iren, TestCoincidentHandleWidgetsLog);
}
