...
=========================================================================*/
//
// This example tests the svtkDistanceWidget
...
// minimum value.
class svtkDistanceCallback : public svtkCommand
{
public:
  static svtkDistanceCallback* New() { return new svtkDistanceCallback; }
  void Execute(svtkObject* caller, unsigned long, void*) override;
  svtkDistanceCallback()
    : Renderer(nullptr)
  ...
  svtkDistanceRepresentation2D* Distance;
};

// Method re-positions the points using random perturbation
void svtkDistanceCallback::Execute(svtkObject*, unsigned long eid, void* callData)
{
  if (eid == svtkCommand::InteractionEvent || eid == svtkCommand::EndInteractionEvent)
  {
    double pos1[3], pos2[3];
    // Modify the measure axis
    this->Distance->GetPoint1WorldPosition(pos1);
    this->Distance->GetPoint2WorldPosition(pos2);
    double dist = sqrt(svtkMath::Distance2BetweenPoints(pos1, pos2));
  ...
  else
  {
    ...
    double p[3];
    p[0] = pos[0] + static_cast<int>(svtkMath::Random(-5.5, 5.5));
    p[1] = pos[1] + static_cast<int>(svtkMath::Random(-5.5, 5.5));
    p[2] = 0.0;
...
int TestDistanceWidget(int svtkNotUsed(argc), char* svtkNotUsed(argv)[])
{
  ...
  SVTK_CREATE(svtkDistanceRepresentation2D, rep);
  rep->SetHandleRepresentation(handle);
  svtkAxisActor2D* axis = rep->GetAxis();
  axis->UseFontSizeFromPropertyOn();
  svtkTextProperty* titleProp = axis->GetTitleTextProperty();
  ...
  svtkProperty2D* prop2D = rep->GetAxisProperty();
  if (!prop2D)
  {
  ...
  SVTK_CREATE(svtkDistanceWidget, widget);
  widget->SetInteractor(iren);
  widget->CreateDefaultRepresentation();
  widget->SetRepresentation(rep);

  SVTK_CREATE(svtkDistanceCallback, mcbk);
  mcbk->Renderer = ren1;
  ...
  // record events
  SVTK_CREATE(svtkInteractorEventRecorder, recorder);
  ...
  double pointDistance = svtkMath::Distance2BetweenPoints(p1w, p2w);
  ...
  pointDistance = svtkMath::Distance2BetweenPoints(p1wSet, p2wSet);
  pointDistance = sqrt(pointDistance);
  if (fabs(pointDistance - distance) > 0.01)
  {
