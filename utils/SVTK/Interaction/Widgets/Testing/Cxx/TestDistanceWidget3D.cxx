...
// minimum value.
class svtkDistanceWidget3DCallback : public svtkCommand
{
public:
  static svtkDistanceWidget3DCallback* New() { return new svtkDistanceWidget3DCallback; }
  void Execute(svtkObject* caller, unsigned long, void*) override;
  svtkDistanceWidget3DCallback()
    : Renderer(nullptr)
  ...
  svtkDistanceRepresentation3D* Distance;
};

// Method re-positions the points using random perturbation
void svtkDistanceWidget3DCallback::Execute(svtkObject*, unsigned long eid, void* callData)
{
...
int TestDistanceWidget3D(int argc, char* argv[])
{
  ...
  SVTK_CREATE(svtkPointHandleRepresentation3D, handle);
  handle->GetProperty()->SetColor(1, 0, 0);
  SVTK_CREATE(svtkDistanceRepresentation3D, rep);
  ...
  svtkActor* glyphActor = rep->GetGlyphActor();
  if (!glyphActor)
  {
  ...
  svtkFollower* labelActor = rep->GetLabelActor();
  if (!labelActor)
  {
  ...
  SVTK_CREATE(svtkDistanceWidget3DCallback, mcbk);
  mcbk->Renderer = ren1;
