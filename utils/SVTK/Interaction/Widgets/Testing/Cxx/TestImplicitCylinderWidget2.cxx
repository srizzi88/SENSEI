...
// This does the actual work: updates the svtkCylinder implicit function.
// This in turn causes the pipeline to update and clip the object.
// Callback for the interaction
class svtkTICWCallback : public svtkCommand
{
public:
  static svtkTICWCallback* New() { return new svtkTICWCallback; }

  void Execute(svtkObject* caller, unsigned long, void*) override
  {
    svtkImplicitCylinderWidget* cylWidget = reinterpret_cast<svtkImplicitCylinderWidget*>(caller);
    svtkImplicitCylinderRepresentation* rep =
      reinterpret_cast<svtkImplicitCylinderRepresentation*>(cylWidget->GetRepresentation());
...
int TestImplicitCylinderWidget2(int svtkNotUsed(argc), char* svtkNotUsed(argv)[])
{
  ...
  // This portion of the code clips the mace with the svtkCylinders
  // implicit function. The clipped region is colored green.
  svtkSmartPointer<svtkCylinder> cylinder = svtkSmartPointer<svtkCylinder>::New();
  ...
  svtkSmartPointer<svtkTICWCallback> myCallback = svtkSmartPointer<svtkTICWCallback>::New();
  myCallback->Cylinder = cylinder;
  myCallback->Actor = selectActor;

  svtkSmartPointer<svtkImplicitCylinderRepresentation> rep =
    svtkSmartPointer<svtkImplicitCylinderRepresentation>::New();
  rep->SetPlaceFactor(1.25);
  rep->PlaceWidget(glyph->GetOutput()->GetBounds());

  svtkSmartPointer<svtkImplicitCylinderWidget> cylWidget =
    svtkSmartPointer<svtkImplicitCylinderWidget>::New();
  cylWidget->SetInteractor(iren);
  cylWidget->SetRepresentation(rep);
  cylWidget->AddObserver(svtkCommand::InteractionEvent, myCallback);
  ...
  {
    std::cerr << "center (" << center1[0] << "," << center1[1] << "," << center1[2]
  ...
  {
    std::cerr << "center not set correctly. expected (" << center1[0] << "," << center1[1] << ","
              << center1[2] << "), got: (" << center2[0] << "," << center2[1] << "," << center2[2]
              << ")" << std::endl;
    return EXIT_FAILURE;
  }
