...
// This permit to change the svtkImplicitPlaneWidget2 mode pressing Ctrl keypad
// It swaps the widget mode between Manual and LockNormalToCamera
// Callback for the interaction
class svtkEnableSlaveCallback : public svtkCommand
{
public:
  static svtkEnableSlaveCallback* New() { return new svtkEnableSlaveCallback; }
  void Execute(svtkObject* caller, unsigned long, void*) override
  {
  ...
  // Attributes
  bool lockMode;
  svtkImplicitPlaneWidget2* pWidget;
};

int TestImplicitPlaneWidget2LockNormalToCamera(int svtkNotUsed(argc), char* svtkNotUsed(argv)[])
{
  ...
  svtkNew<svtkLODActor> maceActor;
  ...
  svtkNew<svtkLODActor> selectActor;
  ...
  svtkNew<svtkTIPW2Callback> myCallback;
  myCallback->Plane = plane;
  myCallback->Actor = selectActor;

  svtkNew<svtkImplicitPlaneRepresentation> rep;
  ...
  svtkNew<svtkEnableSlaveCallback> lockMode;
  lockMode->pWidget = planeWidget;

  // Link the Swapper to the RenderWindowInteractor
  iren->AddObserver(svtkCommand::KeyPressEvent, lockMode);

  planeWidget->AddObserver(svtkCommand::InteractionEvent, myCallback);
  planeWidget->AddObserver(svtkCommand::UpdateEvent, myCallback);
