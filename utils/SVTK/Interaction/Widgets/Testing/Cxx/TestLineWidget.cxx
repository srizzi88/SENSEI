...
class svtkLWCallback : public svtkCommand
{
public:
  static svtkLWCallback* New() { return new svtkLWCallback; }
  void Execute(svtkObject* caller, unsigned long, void*) override
  {
    svtkLineWidget* lineWidget = reinterpret_cast<svtkLineWidget*>(caller);
...
int TestLineWidget(int argc, char* argv[])
{
  ...
  svtkSmartPointer<svtkLWCallback> myCallback = svtkSmartPointer<svtkLWCallback>::New();
  myCallback->PolyData = seeds;
  myCallback->Actor = streamline;

  // The plane widget is used probe the dataset.
  //
  svtkSmartPointer<svtkLineWidget> lineWidget = svtkSmartPointer<svtkLineWidget>::New();
