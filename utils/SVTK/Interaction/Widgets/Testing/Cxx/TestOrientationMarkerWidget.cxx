...
{
  // create an actor out of parts of svtkAxesActor and svtkAnnotatedCubeActor
  // to have the widget follow.
  // part 1 is a helical spring to test svtkAxesActor SetUserDefinedShaft
  //
  double dt = svtkMath::Pi() / 20.0;
  double t = 0.0;
  double x = 0.0;
  int nPoints = 120;
  ...
  svtkSmartPointer<svtkPolyData> wiggle = svtkSmartPointer<svtkPolyData>::New();
  ...
  // part 2 is generated from svtkAnnotatedCubeActor to test
  // svtkAxesActor SetUserDefinedTip
  //
  svtkSmartPointer<svtkAnnotatedCubeActor> cube = svtkSmartPointer<svtkAnnotatedCubeActor>::New();
  ...
  svtkSmartPointer<svtkPropCollection> props = svtkSmartPointer<svtkPropCollection>::New();
  ...
  for (i = 0; i < nprops; i++)
  {
    svtkActor* node = svtkActor::SafeDownCast(props->GetNextProp(sit));
    ...
    {
      svtkPolyData* poly = svtkPolyData::SafeDownCast(node->GetMapper()->GetInput());
      if (poly)
      {
        ...
        svtkSmartPointer<svtkPolyData> newpoly = svtkSmartPointer<svtkPolyData>::New();
  ...
  //
  svtkSmartPointer<svtkAxesActor> axes = svtkSmartPointer<svtkAxesActor>::New();
  ...
  svtkProperty* property = axes->GetXAxisTipProperty();
  ...
  // actors: svtkAxesActor and a svtkAnnotatedCubeActor
  //
  ...
  svtkSmartPointer<svtkAxesActor> axes2 = svtkSmartPointer<svtkAxesActor>::New();
  ...
  svtkTextProperty* tprop = axes2->GetXAxisCaptionActor2D()->GetCaptionTextProperty();
  tprop->ItalicOn();
  tprop->ShadowOn();
  ...
  //
  svtkSmartPointer<svtkPropAssembly> assembly = svtkSmartPointer<svtkPropAssembly>::New();
  assembly->AddPart(axes2);
  assembly->AddPart(cube);

  // set up the widget
  //
  svtkSmartPointer<svtkOrientationMarkerWidget> widget =
    svtkSmartPointer<svtkOrientationMarkerWidget>::New();
