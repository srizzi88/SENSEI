...
// events.
class svtkPickingManagerCallback : public svtkCommand
{
public:
  static svtkPickingManagerCallback* New() { return new svtkPickingManagerCallback; }
  ...
  svtkPickingManagerCallback() = default;
};

//------------------------------------------------------------------------------
// Press 'Space' to reorganize the cube of seeds
class svtkPMSCubeCallback : public svtkCommand
{
public:
  static svtkPMSCubeCallback* New() { return new svtkPMSCubeCallback; }
  ...
  void Execute(svtkObject* caller, unsigned long, void*) override
  {
    ...
    // Reorganize the cube
    if (svtkStdString(iren->GetKeySym()) == "space")
    {
      const int baseCube = static_cast<int>(pow(this->Seeds.size(), 1. / 3.) / 2 + 0.5);
      std::list<svtkSmartPointer<svtkHandleWidget> >::iterator it = this->Seeds.begin();
      ...
      for (int i = -baseCube; i < baseCube; ++i)
      {
        for (int j = -baseCube; j < baseCube; ++j)
        {
          ...
          {
            svtkSphereHandleRepresentation* newHandleRep =
              svtkSphereHandleRepresentation::SafeDownCast((*it)->GetRepresentation());
  ...
  std::list<svtkSmartPointer<svtkHandleWidget> > Seeds;
};

//------------------------------------------------------------------------------
// Write timerlog in file
// Each time a render event occurs, the corresponding elapsed time is written.
class svtkPMSRecordPerfCallback : public svtkCommand
{
public:
  static svtkPMSRecordPerfCallback* New() { return new svtkPMSRecordPerfCallback; }

  svtkPMSRecordPerfCallback()
  {
    this->performanceReport.open("pickingManagerPerfs.txt");
    this->logTime = svtkTimerLog::New();
  }

  ~svtkPMSRecordPerfCallback() override
  {
  ...
  void Execute(svtkObject* svtkNotUsed(caller), unsigned long, void*) override
  {
    ...
    //   static_cast<svtkRenderWindowInteractor*>(caller);
  ...
  svtksys::ofstream performanceReport;
  svtkTimerLog* logTime;

private:
  svtkPMSRecordPerfCallback(const svtkPMSRecordPerfCallback&) = delete;
  void operator=(const svtkPMSRecordPerfCallback&) = delete;
};

//------------------------------------------------------------------------------
// Test Picking Manager with a lot of seeds
//------------------------------------------------------------------------------
int TestPickingManagerSeedWidget(int svtkNotUsed(argc), char* svtkNotUsed(argv)[])
{
  ...
  // Picking Manager
  svtkNew<svtkPickingManagerCallback> callMode;
  ...
  for (int i = -baseCube; i < baseCube; ++i)
  {
    for (int j = -baseCube; j < baseCube; ++j)
    {
      for (int k = -baseCube; k < baseCube; ++k)
      {
        svtkHandleWidget* newHandle = seedWidget->CreateNewHandle();
        newHandle->SetEnabled(1);
        svtkSphereHandleRepresentation* newHandleRep =
          svtkSphereHandleRepresentation::SafeDownCast(newHandle->GetRepresentation());
  ...
  svtkNew<svtkPMSCubeCallback> reorganizeCallback;
  reorganizeCallback->Seeds = seeds;
  iren->AddObserver(svtkCommand::KeyPressEvent, reorganizeCallback);
  ...
  // svtkNew<svtkPMSRecordPerfCallback> writePerfsCallback;
  // iren->AddObserver(svtkCommand::RenderEvent, writePerfsCallback);
