...
class svtkPickingManagerSeedWidgetTest2Callback : public svtkCommand
{
public:
  static svtkPickingManagerSeedWidgetTest2Callback* New()
  {
    return new svtkPickingManagerSeedWidgetTest2Callback;
  }
  ...
  void Execute(svtkObject* caller, unsigned long, void*) override
  {
    ...
    if (svtkStdString(iren->GetKeySym()) == "Alt_L" || svtkStdString(iren->GetKeySym()) == "Alt_R")
    {
      const int baseCube = static_cast<int>(pow(this->Seeds.size(), 1. / 3.) / 2 + 0.5);
      int n = 0;
      for (int i = -baseCube; i < baseCube; ++i)
      {
        for (int j = -baseCube; j < baseCube; ++j)
        {
          for (int k = -baseCube; k < baseCube; ++k)
          {
            if (n % 2 == 0)
            {
              svtkHandleWidget* w = this->Widget->GetSeed(n);
  ...
  svtkSeedWidget* Widget;
};

//------------------------------------------------------------------------------
// Test Picking Manager with a lot of seeds
//------------------------------------------------------------------------------
int TestPickingManagerSeedWidget2(int svtkNotUsed(argc), char* svtkNotUsed(argv)[])
{
  ...
  svtkNew<svtkInteractorStyleTrackballCamera> interactorStyle;
  ...
  svtkNew<svtkPickingManagerSeedWidgetTest2Callback> callback;
  callback->Seeds = seeds;
  callback->Widget = seedWidget;
  interactor->AddObserver(svtkCommand::KeyPressEvent, callback);
