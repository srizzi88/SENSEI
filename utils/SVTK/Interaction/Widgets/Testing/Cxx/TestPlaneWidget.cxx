...
int TestPlaneWidget(int argc, char* argv[])
{
  ...
  svtkSmartPointer<svtkPolyDataMapper> probeMapper = svtkSmartPointer<svtkPolyDataMapper>::New();
  probeMapper->SetInputConnection(probe->GetOutputPort());
  double tmp[2];
  pl3d_block0->GetScalarRange(tmp);
  probeMapper->SetScalarRange(tmp[0], tmp[1]);

  svtkSmartPointer<svtkActor> probeActor = svtkSmartPointer<svtkActor>::New();
  ...
  svtkSmartPointer<svtkTPWCallback> myCallback = svtkSmartPointer<svtkTPWCallback>::New();
  myCallback->PolyData = plane;
  myCallback->Actor = probeActor;

  // The plane widget is used probe the dataset.
  //
  svtkSmartPointer<svtkPlaneWidget> planeWidget = svtkSmartPointer<svtkPlaneWidget>::New();
