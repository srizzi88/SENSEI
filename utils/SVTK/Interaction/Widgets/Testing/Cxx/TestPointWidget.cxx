...
const char PointWidgetEventLog[] = "# StreamVersion 1\n"
...
class svtkmyPWCallback : public svtkCommand
{
public:
  static svtkmyPWCallback* New() { return new svtkmyPWCallback; }
  void Execute(svtkObject* caller, unsigned long, void*) override
  {
    svtkPointWidget* pointWidget = reinterpret_cast<svtkPointWidget*>(caller);
...
int TestPointWidget(int argc, char* argv[])
{
  ...
  svtkSmartPointer<svtkPolyDataMapper> glyphMapper = svtkSmartPointer<svtkPolyDataMapper>::New();
  glyphMapper->SetInputConnection(glyph->GetOutputPort());

  svtkSmartPointer<svtkActor> glyphActor = svtkSmartPointer<svtkActor>::New();
  ...
  svtkSmartPointer<svtkmyPWCallback> myCallback = svtkSmartPointer<svtkmyPWCallback>::New();
  myCallback->PolyData = point;
  myCallback->Actor = glyphActor;

  // The plane widget is used probe the dataset.
  //
  svtkSmartPointer<svtkPointWidget> pointWidget = svtkSmartPointer<svtkPointWidget>::New();
  pointWidget->SetInteractor(iren);
  pointWidget->SetInputData(pl3d_block0);
  pointWidget->AllOff();
  pointWidget->PlaceWidget();
  pointWidget->AddObserver(svtkCommand::InteractionEvent, myCallback);
