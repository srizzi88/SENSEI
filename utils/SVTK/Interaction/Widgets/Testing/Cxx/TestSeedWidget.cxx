...
=========================================================================*/
//
// This example tests the svtkSeedWidget
...
class svtkSeedCallback : public svtkCommand
{
public:
  static svtkSeedCallback* New() { return new svtkSeedCallback; }
  void Execute(svtkObject*, unsigned long event, void* calldata) override
  {
    if (event == svtkCommand::PlacePointEvent)
    {
    ...
    if (event == svtkCommand::StartInteractionEvent)
    {
      if (calldata)
      {
        std::cout << "Start interacting with seed : " << *(static_cast<int*>(calldata))
                  << std::endl;
      }
    }
    if (event == svtkCommand::InteractionEvent)
    {
      if (calldata)
      {
...
int TestSeedWidget(int argc, char* argv[])
{
  ...
  svtkSmartPointer<svtkSeedRepresentation> rep = svtkSmartPointer<svtkSeedRepresentation>::New();
  rep->SetHandleRepresentation(handle);

  svtkSmartPointer<svtkSeedWidget> widget = svtkSmartPointer<svtkSeedWidget>::New();
  widget->SetInteractor(iren);
  widget->SetRepresentation(rep);

  svtkSmartPointer<svtkSeedCallback> scbk = svtkSmartPointer<svtkSeedCallback>::New();
  scbk->SeedRepresentation = rep;
  widget->AddObserver(svtkCommand::PlacePointEvent, scbk);
  widget->AddObserver(svtkCommand::StartInteractionEvent, scbk);
  widget->AddObserver(svtkCommand::InteractionEvent, scbk);
  ...
  int retVal = svtkTesting::InteractorEventLoop(argc, argv, iren, TestSeedWidgetEventLog);
  ...
  if (endNumSeeds != 0)
  {
    ...
    {
      svtkSeedRepresentation* seedRep =
        svtkSeedRepresentation::SafeDownCast(widget->GetRepresentation());
