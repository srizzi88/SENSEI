...
// Test for svtkTensorProbeWidget class
//
#include "svtkActor.h"
...
{
  svtkSmartPointer<svtkXMLPolyDataReader> r = svtkSmartPointer<svtkXMLPolyDataReader>::New();
  const char* cfname =
    svtkTestUtilities::ExpandDataFileName(argc, argv, "Data/SyntheticPolyline.vtp");
  r->SetFileName(cfname);
  r->Update();
  delete[] cfname;

  svtkPolyData* pd = r->GetOutput();
  ...
  svtkSmartPointer<svtkTensorProbeWidget> w = svtkSmartPointer<svtkTensorProbeWidget>::New();
  w->SetInteractor(iren);
  svtkTensorProbeRepresentation* rep =
    svtkTensorProbeRepresentation::SafeDownCast(w->GetRepresentation());
  ...
  return svtkTesting::InteractorEventLoop(argc, argv, iren, TestTensorProbeWidgetEventLog);
}
