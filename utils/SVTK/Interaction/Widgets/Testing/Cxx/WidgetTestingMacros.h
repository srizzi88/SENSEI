#ifndef svtkWidgetTestingMacros_h
#define svtkWidgetTestingMacros_h
...
/// tests basic svtkObject methods
...
/// test svtkInteractorObserver methods
#define EXERCISE_BASIC_INTERACTOR_OBSERVER_METHODS(object)                                         \
  {                                                                                                \
    ...
    svtkSmartPointer<svtkCamera> cam1 = svtkSmartPointer<svtkCamera>::New();                           \
...
/// test svtkAbstractWidget methods
...
/// test svtkHoverWidget methods, timer duration is clamped so range macro will fail
...
/// test svtkProp methods
#define EXERCISE_BASIC_PROP_METHODS(className, object)                                             \
  {                                                                                                \
    EXERCISE_BASIC_OBJECT_METHODS(object);                                                         \
    svtkSmartPointer<svtkPropCollection> propCollection = svtkSmartPointer<svtkPropCollection>::New(); \
    ...
    object->Pick();                                                                                \
                                                                                                   \
    svtkMTimeType redrawMTime = object->GetRedrawMTime();                                           \
    std::cout << "Redraw Modified Time = " << redrawMTime << std::endl;                            \
                                                                                                   \
    svtkSmartPointer<className> copyProp = svtkSmartPointer<className>::New();                       \
    object->ShallowCopy(copyProp);                                                                 \
                                                                                                   \
    object->InitPathTraversal();                                                                   \
                                                                                                   \
    svtkSmartPointer<svtkAssemblyPath> assemblyPath = svtkSmartPointer<svtkAssemblyPath>::New();       \
    assemblyPath = object->GetNextPath();                                                          \
    std::cout << "Number of paths = " << object->GetNumberOfPaths() << std::endl;                  \
                                                                                                   \
    svtkSmartPointer<svtkMatrix4x4> mat = svtkSmartPointer<svtkMatrix4x4>::New();                      \
    ...
    {                                                                                              \
      std::cout << "No matrix." << std::endl;                                                      \
                                                                                                   ...
                                                                                                   \
    svtkSmartPointer<svtkInformation> info = svtkSmartPointer<svtkInformation>::New();                 \
    ...
    std::cout << "Has null required keys? " << object->HasKeys(nullptr) << std::endl;              \
                                                                                                   \
    std::cout << "Skipping the internal render calls, requires svtkViewPort. Testing get macros."   \
              << std::endl;                                                                        \
...
#define NOT_DEFINED_CONSUMERS_FAIL()                                                               \
  {                                                                                                \
    ...
    svtkSmartPointer<svtkActor> actor2 = object->GetConsumer(0);                                     \
    if (actor2 != actor)                                                                           \
    {                                                                                              \
...
/// test svtkWidgetRepresentation methods
#define EXERCISE_BASIC_REPRESENTATION_METHODS(className, object)                                   \
  ...
  svtkSmartPointer<svtkRenderer> ren2 = object->GetRenderer();                                       \
  if (ren2 != ren1)                                                                                \
  {                                                                                                \
...
/// test svtkAngleRepresentation methods
#define EXERCISE_BASIC_ANGLE_REPRESENTATION_METHODS(className, object)                             \
  {                                                                                                \
    svtkSmartPointer<svtkPointHandleRepresentation2D> phandle0 =                                     \
    ...
    {                                                                                              \
      ...
      return EXIT_FAILURE;                                                                         \
                                                                                                   ...
                                                                                                   \
    svtkSmartPointer<svtkPointHandleRepresentation2D> phandle =                                      \
      svtkSmartPointer<svtkPointHandleRepresentation2D>::New();                                      \
    object->SetHandleRepresentation(phandle);                                                      \
    object->InstantiateHandleRepresentation();                                                     \
                                                                                                   \
    svtkSmartPointer<svtkHandleRepresentation> handleRep = nullptr;                                  \
...
/// test svtkBorderRepresentation methods
#define EXERCISE_BASIC_BORDER_REPRESENTATION_METHODS(className, object)                            \
  {                                                                                                \
    ...
    svtkSmartPointer<svtkCoordinate> coord = object->GetPositionCoordinate();                        \
    pos2 = coord->GetValue();                                                                      \
    if (pos2 == nullptr || pos2[0] != pos[0] || pos2[1] != pos[1])                                 \
    {                                                                                              \
    ...
    object->SetShowBorderToActive();                                                               \
                                                                                                   \
    svtkSmartPointer<svtkProperty2D> borderProperty = object->GetBorderProperty();                   \
                                                                                                   \
...
/// test objects that have Property and SelectedProperty set/get, with svtkProperty
#define TEST_SET_GET_PROPERTY(object, variable)                                                    \
  {                                                                                                \
    ...
    svtkSmartPointer<svtkProperty> prop = node1->Get##variable();                                    \
    if (!prop)                                                                                     \
    {                                                                                              \
...
/// test svtkHandleRepresentation methods
...
#define EXERCISE_BASIC_HANDLE_REPRESENTATION_METHODS(className, object)                            \
  {                                                                                                \
    ...
    TEST_SET_GET_BOOLEAN(object, Constrained);                                                     \
                                                                                                   \
    svtkSmartPointer<svtkRenderer> ren3 = object->GetRenderer();                                     \
    ...
    std::cout << "MTime = " << object->GetMTime() << std::endl;                                    \
                                                                                                   \
    svtkSmartPointer<svtkPointPlacer> pplacer = svtkSmartPointer<svtkPointPlacer>::New();              \
    object->SetPointPlacer(pplacer);                                                               \
    svtkSmartPointer<svtkPointPlacer> pplacer2 = object->GetPointPlacer();                           \
    if (pplacer2 != pplacer)                                                                       \
    {                                                                                              \
...
/// test svtkAbstractPolygonalHandleRepresentation3D methods
#define EXERCISE_BASIC_ABSTRACT_POLYGONAL_HANDLE_REPRESENTATION3D_METHODS(className, object)       \
  {                                                                                                \
    ...
    object->SetHandle(pd);                                                                         \
    svtkSmartPointer<svtkPolyData> pd2 = object->GetHandle();                                        \
    if (pd2 == nullptr)                                                                            \
    {                                                                                              \
    ...
    TEST_SET_GET_PROPERTY(object, SelectedProperty);                                               \
                                                                                                   \
    svtkSmartPointer<svtkAbstractTransform> at = object->GetTransform();                             \
                                                                                                   \
    ...
    TEST_SET_GET_VECTOR3_DOUBLE_RANGE(object, LabelTextScale, 0.0, 10.0);                          \
                                                                                                   \
    svtkSmartPointer<svtkFollower> follower = object->GetLabelTextActor();                           \
    if (follower == nullptr)                                                                       \
    {                                                                                              \
