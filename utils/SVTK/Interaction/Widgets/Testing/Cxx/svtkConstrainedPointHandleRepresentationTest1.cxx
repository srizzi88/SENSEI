...
int svtkConstrainedPointHandleRepresentationTest1(int, char*[])
{
  svtkSmartPointer<svtkConstrainedPointHandleRepresentation> node1 =
    svtkSmartPointer<svtkConstrainedPointHandleRepresentation>::New();

  EXERCISE_BASIC_HANDLE_REPRESENTATION_METHODS(svtkConstrainedPointHandleRepresentation, node1);
  ...
  svtkSmartPointer<svtkPolyData> pd3 = svtkSmartPointer<svtkPolyData>::New();
  node1->SetActiveCursorShape(pd3);
  svtkSmartPointer<svtkPolyData> pd4 = node1->GetActiveCursorShape();
  if (pd4 != pd3)
  {
  ...
  svtkSmartPointer<svtkPlane> plane2 = node1->GetObliquePlane();
  ...
  svtkSmartPointer<svtkPlane> bplane = svtkSmartPointer<svtkPlane>::New();
  svtkSmartPointer<svtkPlane> bplane2 = svtkSmartPointer<svtkPlane>::New();
  node1->AddBoundingPlane(bplane);
  node1->AddBoundingPlane(bplane2);
  node1->RemoveBoundingPlane(bplane);
  node1->RemoveAllBoundingPlanes();

  svtkSmartPointer<svtkPlaneCollection> planeCol = svtkSmartPointer<svtkPlaneCollection>::New();
  node1->SetBoundingPlanes(planeCol);
  svtkSmartPointer<svtkPlaneCollection> planeCol2 = node1->GetBoundingPlanes();
  ...
  svtkSmartPointer<svtkPlanes> bplanes = svtkSmartPointer<svtkPlanes>::New();
  ...
  svtkSmartPointer<svtkProperty> prop = node1->GetProperty();
  if (!prop)
  {
  ...
  svtkSmartPointer<svtkProperty> prop2 = svtkSmartPointer<svtkProperty>::New();
  colour[0] += 0.1;
  colour[2] += 0.1;
  colour[2] += 0.1;
