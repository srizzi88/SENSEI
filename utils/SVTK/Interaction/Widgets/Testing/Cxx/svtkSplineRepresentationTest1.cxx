...
int svtkSplineRepresentationTest1(int, char*[])
{
  svtkSmartPointer<svtkSplineRepresentation> node1 = svtkSmartPointer<svtkSplineRepresentation>::New();

  EXERCISE_BASIC_REPRESENTATION_METHODS(svtkSplineRepresentation, node1);
  ...
  svtkSmartPointer<svtkProperty> prop = node1->GetHandleProperty();
  if (prop == nullptr)
  {
  ...
  svtkSmartPointer<svtkParametricSpline> pspline2 = node1->GetParametricSpline();
  if (pspline2 != pspline)
  {
  ...
  svtkSmartPointer<svtkDoubleArray> da = node1->GetHandlePositions();
  if (da == nullptr)
  {
  ...
  if (da == nullptr)
  {
    std::cerr << "HandlePositions array is null after initing with svtkPoints!" << std::endl;
    return EXIT_FAILURE;
  }
  else
  {
