/*=========================================================================
  ...
  Module:    svtkBiDimensionalRepresentation2D.cxx
...
svtkStandardNewMacro(svtkBiDimensionalRepresentation2D);

//----------------------------------------------------------------------
svtkBiDimensionalRepresentation2D::svtkBiDimensionalRepresentation2D()
  : svtkBiDimensionalRepresentation()
{
  ...
  this->LineMapper = svtkPolyDataMapper2D::New();
  this->LineMapper->SetInputData(this->LinePolyData);
  this->LineProperty = svtkProperty2D::New();
  this->LineActor = svtkActor2D::New();
  this->LineActor->SetProperty(this->LineProperty);
  this->LineActor->SetMapper(this->LineMapper);
  this->SelectedLineProperty = svtkProperty2D::New();
...
//----------------------------------------------------------------------
int svtkBiDimensionalRepresentation2D::ComputeInteractionState(int X, int Y, int modify)
{
  ...
  {
    this->InteractionState = svtkBiDimensionalRepresentation2D::NearP1;
  }
  else if (p2State == svtkHandleRepresentation::Nearby)
  {
    this->InteractionState = svtkBiDimensionalRepresentation2D::NearP2;
  }
  else if (p3State == svtkHandleRepresentation::Nearby)
  {
    this->InteractionState = svtkBiDimensionalRepresentation2D::NearP3;
  }
  else if (p4State == svtkHandleRepresentation::Nearby)
  {
    this->InteractionState = svtkBiDimensionalRepresentation2D::NearP4;
  }
  else
  {
    this->InteractionState = svtkBiDimensionalRepresentation2D::Outside;
  }

  // Okay if we're near a handle return, otherwise test edges.
  if (this->InteractionState != svtkBiDimensionalRepresentation2D::Outside)
  {
  ...
  svtkLine::Intersection(p1, p2, p3, p4, uIntersect, vIntersect);

  // Check if we are on edges
  int onL1 = (svtkLine::DistanceToLine(xyz, p1, p2, t, closest) <= tol2);
  int onL2 = (svtkLine::DistanceToLine(xyz, p3, p4, t, closest) <= tol2);

  double xyzParam;

  if (onL1 && onL2)
  {
    this->InteractionState = svtkBiDimensionalRepresentation2D::OnCenter;
  }
  else if (onL1)
  {
    if (p1[0] != p2[0])
    {
      ...
      if (xyzParam < uIntersect)
      {
        ...
        {
          this->InteractionState = svtkBiDimensionalRepresentation2D::OnL1Outer;
        }
        else
        {
          this->InteractionState = svtkBiDimensionalRepresentation2D::OnL1Inner;
        }
      }
      else
      {
  ...
  else if (onL2)
  {
    if (p3[0] != p4[0])
    {
      ...
      if (xyzParam < vIntersect)
      {
        ...
        {
          this->InteractionState = svtkBiDimensionalRepresentation2D::OnL2Outer;
        }
        else
        {
          this->InteractionState = svtkBiDimensionalRepresentation2D::OnL2Inner;
        }
      }
      else
      {
...
//----------------------------------------------------------------------
void svtkBiDimensionalRepresentation2D::StartWidgetDefinition(double e[2])
{
  double pos[3];
  pos[0] = e[0];
  pos[1] = e[1];
...
//----------------------------------------------------------------------
void svtkBiDimensionalRepresentation2D::Point2WidgetInteraction(double e[2])
{
  double pos[3], p1[3];
  pos[0] = e[0];
  pos[1] = e[1];
...
void svtkBiDimensionalRepresentation2D::Point3WidgetInteraction(double e[2])
{
  ...
  double dist = sqrt(svtkLine::DistanceToLine(pw, p1, p2, t, closest));
...
//----------------------------------------------------------------------
void svtkBiDimensionalRepresentation2D::StartWidgetManipulation(double e[2])
{
...
void svtkBiDimensionalRepresentation2D::ProjectOrthogonalPoint(
  double x[4], double y[3], double x1[3], double x2[3], double x21[3], double dir, double xP[3])
{
  ...
  dist = dir * sqrt(svtkLine::DistanceToLine(x, x1, x2, t, closest));
  ...
  svtkLine::DistanceToLine(y, x1, x2, t, closest);
...
void svtkBiDimensionalRepresentation2D::WidgetInteraction(double e[2])
{
    ...
    this->InteractionState == OnL2Outer) // rotate the representation
  {
    ...
    double r1 = svtkMath::Norm(p1c);
    double r2 = svtkMath::Norm(p2c);
    double r3 = svtkMath::Norm(p3c);
    double r4 = svtkMath::Norm(p4c);
  ...
  {
    svtkLine::DistanceToLine(pw, this->P3World, this->P4World, t, closest);
  ...
  {
    svtkLine::DistanceToLine(pw, this->P1World, this->P2World, t, closest);
    t = (t < 0.0 ? 0.0 : (t > 1.0 ? 1.0 : t));
    for (int i = 0; i < 3; i++)
    {
...
//----------------------------------------------------------------------
void svtkBiDimensionalRepresentation2D::BuildRepresentation()
{
      ...
      this->Renderer->GetSVTKWindow()->GetMTime() > this->BuildTime))
  {
    ...
    double line1Dist = sqrt(svtkMath::Distance2BetweenPoints(wp1, wp2));
    double line2Dist = 0;
    if (this->Line2Visibility)
    {
      line2Dist = sqrt(svtkMath::Distance2BetweenPoints(wp3, wp4));
    }
    ...
    int maxX = SVTK_INT_MIN, maxY = SVTK_INT_MIN;
    if (p1[1] > maxY)
    {
    ...
    int minX = SVTK_INT_MAX, minY = SVTK_INT_MAX;
    if (p1[1] < minY)
    {
...
//----------------------------------------------------------------------
char* svtkBiDimensionalRepresentation2D::GetLabelText()
{
  return this->TextMapper->GetInput();
}

//----------------------------------------------------------------------
double* svtkBiDimensionalRepresentation2D::GetLabelPosition()
{
  return this->TextActor->GetPosition();
}

//----------------------------------------------------------------------
void svtkBiDimensionalRepresentation2D::GetLabelPosition(double pos[3])
{
  this->TextActor->GetPositionCoordinate()->GetValue(pos);
}

//----------------------------------------------------------------------
void svtkBiDimensionalRepresentation2D::GetWorldLabelPosition(double pos[3])
{
...
//----------------------------------------------------------------------
void svtkBiDimensionalRepresentation2D::ReleaseGraphicsResources(svtkWindow* w)
{
...
//----------------------------------------------------------------------
int svtkBiDimensionalRepresentation2D::RenderOverlay(svtkViewport* viewport)
{
...
//----------------------------------------------------------------------
void svtkBiDimensionalRepresentation2D::Highlight(int highlightOn)
{
  if (highlightOn)
  {
...
//----------------------------------------------------------------------
void svtkBiDimensionalRepresentation2D::PrintSelf(ostream& os, svtkIndent indent)
{
