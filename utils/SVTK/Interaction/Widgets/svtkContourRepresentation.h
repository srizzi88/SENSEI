/*=========================================================================
  ...
  Module:    svtkContourRepresentation.h
 ...
 * @brief   represent the svtkContourWidget
 *
 * The svtkContourRepresentation is a superclass for various types of
 * representations for svtkContourWidget.
 *
 * @par Managing contour points:
 * The classes svtkContourRepresentationNode, svtkContourRepresentationInternals,
 * and svtkContourRepresentationPoint manage the data structure used to represent
...
#ifndef svtkContourRepresentation_h
#define svtkContourRepresentation_h
...
//----------------------------------------------------------------------
class svtkContourRepresentationPoint
{
public:
...
class svtkContourRepresentationNode
{
public:
  double WorldPosition[3];
  ...
  double NormalizedDisplayPosition[2];
  int Selected;
  std::vector<svtkContourRepresentationPoint*> Points;
...
class svtkContourRepresentationInternals
{
public:
  std::vector<svtkContourRepresentationNode*> Nodes;
  void ClearNodes()
  {
...
class SVTKINTERACTIONWIDGETS_EXPORT svtkContourRepresentation : public svtkWidgetRepresentation
{
  friend class svtkContourWidget;
...
public:
   ...
   */
  svtkTypeMacro(svtkContourRepresentation, svtkWidgetRepresentation);
   ...
   * Get the nth node.
   */
  virtual svtkContourRepresentationNode* GetNthNode(int n);
   ...
   */
  virtual int AddIntermediatePointWorldPosition(int n, double point[3], svtkIdType ptId);
   ...
   * display coordinates
   */
  svtkSetClampMacro(PixelTolerance, int, 1, 100);
  svtkGetMacro(PixelTolerance, int);
  //@}
   ...
   * world coordinates
   */
  svtkSetClampMacro(WorldTolerance, double, 0.0, SVTK_DOUBLE_MAX);
  svtkGetMacro(WorldTolerance, double);
  //@}
   ...
   */
  svtkGetMacro(CurrentOperation, int);
  svtkSetClampMacro(
    CurrentOperation, int, svtkContourRepresentation::Inactive, svtkContourRepresentation::Scale);
  void SetCurrentOperationToInactive()
  {
    this->SetCurrentOperation(svtkContourRepresentation::Inactive);
  }
  void SetCurrentOperationToTranslate()
  {
    this->SetCurrentOperation(svtkContourRepresentation::Translate);
  }
  void SetCurrentOperationToShift() { this->SetCurrentOperation(svtkContourRepresentation::Shift); }
  void SetCurrentOperationToScale() { this->SetCurrentOperation(svtkContourRepresentation::Scale); }
  //@}
  ...
  void SetPointPlacer(svtkPointPlacer*);
   ...
   * nodes.
   */
  void SetLineInterpolator(svtkContourLineInterpolator*);
  svtkGetObjectMacro(LineInterpolator, svtkContourLineInterpolator);
  //@}
   ...
   */
  void ReleaseGraphicsResources(svtkWindow* w) override = 0;
  int RenderOverlay(svtkViewport* viewport) override = 0;
  int RenderOpaqueGeometry(svtkViewport* viewport) override = 0;
  int RenderTranslucentPolygonalGeometry(svtkViewport* viewport) override = 0;
  svtkTypeBool HasTranslucentPolygonalGeometry() override = 0;
  //@}
   ...
   * forms a closed loop.
   */
  void SetClosedLoop(svtkTypeBool val);
  svtkGetMacro(ClosedLoop, svtkTypeBool);
  svtkBooleanMacro(ClosedLoop, svtkTypeBool);
  //@}
   ...
   */
  virtual void SetShowSelectedNodes(svtkTypeBool);
  svtkGetMacro(ShowSelectedNodes, svtkTypeBool);
  svtkBooleanMacro(ShowSelectedNodes, svtkTypeBool);
  //@}

  /**
   * Get the points in this contour as a svtkPolyData.
   */
  virtual svtkPolyData* GetContourRepresentationAsPolyData() = 0;

  /**
   * Get the nodes and not the intermediate points in this
   * contour as a svtkPolyData.
   */
  void GetNodePolyData(svtkPolyData* poly);

  svtkSetMacro(RebuildLocator, bool);

protected:
  svtkContourRepresentation();
  ~svtkContourRepresentation() override;
  ...
  svtkContourLineInterpolator* LineInterpolator;

  int ActiveNode;

  int CurrentOperation;
  svtkTypeBool ClosedLoop;

  // A flag to indicate whether to show the Selected nodes
  svtkTypeBool ShowSelectedNodes;

  svtkContourRepresentationInternals* Internal;
  ...
  svtkTimeStamp ContourBuildTime;
   ...
   * svtkContourWidget::Initialize. The idlist here may be used to initialize
   * a contour widget that uses a svtkPolygonalSurfacePointPlacer. This stores
   * the point id's of the nodes, since the contour is drawn on the vertices
   * of a surface mesh.
   */
  virtual void Initialize(svtkPolyData*, svtkIdList*);

  /**
   * Overloaded initialize method, that is called when the svtkIdList is nullptr
   * to maintain backwards compatibility.
   */
  virtual void Initialize(svtkPolyData*);
...
private:
  svtkContourRepresentation(const svtkContourRepresentation&) = delete;
  void operator=(const svtkContourRepresentation&) = delete;
};
