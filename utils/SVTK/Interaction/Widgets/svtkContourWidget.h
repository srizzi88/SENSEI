 ...
 *
 * The svtkContourWidget is used to select a set of points, and draw lines
 ...
 * events (that are triggered by SVTK events). The svtkContourRepresentation is
 ...
 * svtkPointPlacer and svtkContourLineInterpolator. The representation is also
 ...
 * into the svtkContourWidget's widget events:
 * <pre>
 *   svtkWidgetEvent::Select
 *        widget state is:
 *            Start or
 ...
 *   svtkCommand::WidgetValueChangedEvent (Invoked when the contour is closed
 *                                        for the first time. )
 * </pre>
 *
 * @sa
...
#ifndef svtkContourWidget_h
#define svtkContourWidget_h
...
class SVTKINTERACTIONWIDGETS_EXPORT svtkContourWidget : public svtkAbstractWidget
{
public:
  /**
   * Instantiate this class.
   */
  static svtkContourWidget* New();

  //@{
  /**
   * Standard methods for a SVTK class.
   */
  svtkTypeMacro(svtkContourWidget, svtkAbstractWidget);
   ...
   */
  void SetRepresentation(svtkContourRepresentation* r)
  {
    this->Superclass::SetWidgetRepresentation(reinterpret_cast<svtkWidgetRepresentation*>(r));
  }

  /**
   * Return the representation as a svtkContourRepresentation.
   */
  svtkContourRepresentation* GetContourRepresentation()
  {
    return reinterpret_cast<svtkContourRepresentation*>(this->WidgetRep);
  }
   ...
   */
  void SetAllowNodePicking(svtkTypeBool);
  svtkGetMacro(AllowNodePicking, svtkTypeBool);
  svtkBooleanMacro(AllowNodePicking, svtkTypeBool);
  //@}
   ...
   */
  svtkSetMacro(FollowCursor, svtkTypeBool);
  svtkGetMacro(FollowCursor, svtkTypeBool);
  svtkBooleanMacro(FollowCursor, svtkTypeBool);
  //@}
   ...
   */
  svtkSetMacro(ContinuousDraw, svtkTypeBool);
  svtkGetMacro(ContinuousDraw, svtkTypeBool);
  svtkBooleanMacro(ContinuousDraw, svtkTypeBool);
  //@}
   ...
   */
  virtual void Initialize(svtkPolyData* poly, int state = 1, svtkIdList* idList = nullptr);
...
protected:
  svtkContourWidget();
  ~svtkContourWidget() override;

  int WidgetState;
  int CurrentHandle;
  svtkTypeBool AllowNodePicking;
  svtkTypeBool FollowCursor;
  svtkTypeBool ContinuousDraw;
  int ContinuousActive;

  // Callback interface to capture events when
  // placing the widget.
  static void SelectAction(svtkAbstractWidget*);
  static void AddFinalPointAction(svtkAbstractWidget*);
  ...
  static void TranslateContourAction(svtkAbstractWidget*);
  static void ScaleContourAction(svtkAbstractWidget*);
  static void ResetAction(svtkAbstractWidget*);
...
private:
  svtkContourWidget(const svtkContourWidget&) = delete;
  void operator=(const svtkContourWidget&) = delete;
};
