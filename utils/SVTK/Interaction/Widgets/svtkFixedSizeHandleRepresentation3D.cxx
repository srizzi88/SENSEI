...
=========================================================================*/
#include "svtkFixedSizeHandleRepresentation3D.h"
...
svtkStandardNewMacro(svtkFixedSizeHandleRepresentation3D);

//----------------------------------------------------------------------
svtkFixedSizeHandleRepresentation3D::svtkFixedSizeHandleRepresentation3D()
{
...
//-----------------------------------------------------------------------------
void svtkFixedSizeHandleRepresentation3D::PrintSelf(ostream& os, svtkIndent indent)
{
...
void svtkFixedSizeHandleRepresentation3D ::WorldToDisplay(double w[4], double d[4])
{
  svtkRenderer* viewport = this->GetRenderer();
...
void svtkFixedSizeHandleRepresentation3D ::DisplayToWorld(double d[4], double w[4])
{
  d[3] = 1.0;
  svtkRenderer* viewport = this->GetRenderer();
...
//---------------------------------------------------------------------------
void svtkFixedSizeHandleRepresentation3D::BuildRepresentation()
{
  ...
  // Size in Pixels
  const double currentSizeInPixels = sqrt(svtkMath::Distance2BetweenPoints(d_p, d_c));
  ...
  if (fabs(currentSizeInPixels - displayRadius) > radiusTolerance)
  {
    ...
    const double w_r = sqrt(svtkMath::Distance2BetweenPoints(w_x, w_c));
