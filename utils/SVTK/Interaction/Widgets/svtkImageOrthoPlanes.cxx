/*=========================================================================
  ...
  Module:    svtkImageOrthoPlanes.cxx
...
=========================================================================*/
#include "svtkImageOrthoPlanes.h"
...
svtkStandardNewMacro(svtkImageOrthoPlanes);

//---------------------------------------------------------------------------
static void svtkImageOrthoPlanesInteractionCallback(
  svtkObject* obj, unsigned long, void* clientData, void* svtkNotUsed(callData))
{
  svtkImagePlaneWidget* currentImagePlane = svtkImagePlaneWidget::SafeDownCast(obj);
  svtkImageOrthoPlanes* orthoPlane = reinterpret_cast<svtkImageOrthoPlanes*>(clientData);

  orthoPlane->HandlePlaneEvent(currentImagePlane);
}

//-----------------------------------------------------------------------
svtkImageOrthoPlanes::svtkImageOrthoPlanes()
{
  this->NumberOfPlanes = 3;
  this->Planes = new svtkImagePlaneWidget*[this->NumberOfPlanes];
...
//---------------------------------------------------------------------------
void svtkImageOrthoPlanes::HandlePlaneEvent(svtkImagePlaneWidget* currentImagePlane)
{
  int i = 0;
  ...
  {
    svtkGenericWarningMacro("svtkImageOrthoPlanes: Unidentified plane " << currentImagePlane);
    return;
  }
  ...
  double xSize = svtkMath::Norm(v1);
  double ySize = svtkMath::Norm(v2);
  ...
  svtkMath::Normalize(u1);
  svtkMath::Normalize(u2);
  svtkMath::Normalize(u3);
  ...
  {
    case 0:
      dot1 = svtkMath::Dot(v1, u2);
      dot2 = svtkMath::Dot(v2, u3);
      break;
    case 1:
      dot1 = svtkMath::Dot(v1, u3);
      dot2 = svtkMath::Dot(v2, u1);
      break;
    case 2:
      dot1 = svtkMath::Dot(v1, u1);
      dot2 = svtkMath::Dot(v2, u2);
      break;
    default:
      break;
  }
  ...
  double xSizeOld = sqrt(svtkMath::Distance2BetweenPoints(q0, q1));
  double ySizeOld = sqrt(svtkMath::Distance2BetweenPoints(q0, q2));
  ...
  if (sqrt(svtkMath::Distance2BetweenPoints(newCenter, oldCenter)) > 1e-5)
  {
    double vec[3];
    ...
    if (fabs(svtkMath::Dot(v1, vec)) < 1e-5 && fabs(svtkMath::Dot(v2, vec)) < 1e-5)
    {
...
//-----------------------------------------------------------------------
void svtkImageOrthoPlanes::HandlePlanePush(
  svtkImagePlaneWidget* currentImagePlane, int indexOfModifiedPlane)
{
  ...
  {
    svtkImagePlaneWidget* planeWidget = this->Planes[j];
...
//-----------------------------------------------------------------------
void svtkImageOrthoPlanes::HandlePlaneTranslate(
  svtkImagePlaneWidget* currentImagePlane, int indexOfModifiedPlane)
{
...
//-----------------------------------------------------------------------
void svtkImageOrthoPlanes::HandlePlaneRotation(
  svtkImagePlaneWidget* currentImagePlane, int indexOfModifiedPlane)
{
  int i = 0;
  ...
  svtkTransform* rotationTransform = svtkTransform::New();
...
//-----------------------------------------------------------------------
void svtkImageOrthoPlanes::HandlePlaneScale(
  svtkImagePlaneWidget* currentImagePlane, int indexOfModifiedPlane)
{
  int i = 0;
  ...
  double xSize = sqrt(svtkMath::Distance2BetweenPoints(p0, p1));
  double ySize = sqrt(svtkMath::Distance2BetweenPoints(p0, p2));
  ...
  double xSizeOrig = sqrt(svtkMath::Distance2BetweenPoints(
    this->Origin[indexOfModifiedPlane], this->Point1[indexOfModifiedPlane]));
  double ySizeOrig = sqrt(svtkMath::Distance2BetweenPoints(
  ...
  double zScale = 1.0;
  if (sqrt(svtkMath::Distance2BetweenPoints(center, oldCenter)) <= 1e-5)
  {
...
//-----------------------------------------------------------------------
void svtkImageOrthoPlanes::SetTransformMatrix(
...
//-----------------------------------------------------------------------
void svtkImageOrthoPlanes::SetPlane(int j, svtkImagePlaneWidget* currentImagePlane)
{
  if (j > this->NumberOfPlanes)
  {
    int n = 3 * ((j + 2) / 3);

    svtkImagePlaneWidget** widgets = new svtkImagePlaneWidget*[n];
  ...
  if (j >= 0 && j < this->NumberOfPlanes)
  {
    ...
    svtkCallbackCommand* callbackCommand = svtkCallbackCommand::New();
    callbackCommand->SetClientData(this);
    callbackCommand->SetCallback(svtkImageOrthoPlanesInteractionCallback);
    this->ObserverTags[j] =
      currentImagePlane->AddObserver(svtkCommand::InteractionEvent, callbackCommand, 1.0);
  ...
  {
    svtkErrorMacro("wrong plane index");
    return;
  }
}

//-----------------------------------------------------------------------
svtkImagePlaneWidget* svtkImageOrthoPlanes::GetPlane(int i)
{
  if (i < 0 || i >= this->NumberOfPlanes)
  {
    svtkErrorMacro("requested invalid plane index");
    return nullptr;
  }
  else
  {
...
//-----------------------------------------------------------------------
void svtkImageOrthoPlanes::ResetPlanes()
{
  int i = 0;
...
//-----------------------------------------------------------------------
void svtkImageOrthoPlanes::GetBounds(double bounds[6])
{
  svtkAlgorithm* input = this->Planes[0]->GetReslice()->GetInputAlgorithm();
  ...
  svtkInformation* info = input->GetOutputInformation(0);
  int extent[6];
...
//-----------------------------------------------------------------------
void svtkImageOrthoPlanes::PrintSelf(ostream& os, svtkIndent indent)
{
