/*=========================================================================
  ...
  Module:    svtkLineRepresentation.cxx
...
svtkStandardNewMacro(svtkLineRepresentation);

svtkCxxSetObjectMacro(svtkLineRepresentation, HandleRepresentation, svtkPointHandleRepresentation3D);

//----------------------------------------------------------------------------
svtkLineRepresentation::svtkLineRepresentation()
{
  ...
  this->HandleGeometry = new svtkPolyDataAlgorithm*[2];
  for (int i = 0; i < 2; i++)
  {
  ...
  this->RepresentationState = svtkLineRepresentation::Outside;
...
//----------------------------------------------------------------------
void svtkLineRepresentation::SetDirectionalLine(bool val)
{
...
//----------------------------------------------------------------------
double svtkLineRepresentation::GetDistance()
{
  return this->Distance;
}

//----------------------------------------------------------------------
void svtkLineRepresentation::InstantiateHandleRepresentation()
{
  if (!this->Point1Representation)
  {
...
//----------------------------------------------------------------------
void svtkLineRepresentation::SetResolution(int r)
{
  this->LineSource->SetResolution(r);
}

//----------------------------------------------------------------------
int svtkLineRepresentation::GetResolution()
{
  return this->LineSource->GetResolution();
}

//----------------------------------------------------------------------
void svtkLineRepresentation::GetPolyData(svtkPolyData* pd)
{
...
// Point1
//----------------------------------------------------------------------
void svtkLineRepresentation::GetPoint1WorldPosition(double pos[3])
{
  this->Point1Representation->GetWorldPosition(pos);
}

double* svtkLineRepresentation::GetPoint1WorldPosition()
{
  return this->Point1Representation->GetWorldPosition();
}

//----------------------------------------------------------------------
void svtkLineRepresentation::GetPoint1DisplayPosition(double pos[3])
{
  this->Point1Representation->GetDisplayPosition(pos);
}

double* svtkLineRepresentation::GetPoint1DisplayPosition()
{
  return this->Point1Representation->GetDisplayPosition();
}

//----------------------------------------------------------------------
void svtkLineRepresentation::SetPoint1WorldPosition(double x[3])
{
...
//----------------------------------------------------------------------
void svtkLineRepresentation::SetPoint1DisplayPosition(double x[3])
{
...
// Point2
//----------------------------------------------------------------------
void svtkLineRepresentation::GetPoint2WorldPosition(double pos[3])
{
  this->Point2Representation->GetWorldPosition(pos);
}

double* svtkLineRepresentation::GetPoint2WorldPosition()
{
  return this->Point2Representation->GetWorldPosition();
}

//----------------------------------------------------------------------
void svtkLineRepresentation::GetPoint2DisplayPosition(double pos[3])
{
  this->Point2Representation->GetDisplayPosition(pos);
}

double* svtkLineRepresentation::GetPoint2DisplayPosition()
{
  return this->Point2Representation->GetDisplayPosition();
}

//----------------------------------------------------------------------
void svtkLineRepresentation::SetPoint2WorldPosition(double x[3])
{
...
//----------------------------------------------------------------------
void svtkLineRepresentation::SetPoint2DisplayPosition(double x[3])
{
...
//----------------------------------------------------------------------
void svtkLineRepresentation::SetRenderer(svtkRenderer* ren)
{
...
//----------------------------------------------------------------------
void svtkLineRepresentation::StartWidgetInteraction(double e[2])
{
  ...
  if (this->InteractionState == svtkLineRepresentation::Scaling)
  {
    double dp1[3], dp2[3];
...
//----------------------------------------------------------------------
void svtkLineRepresentation::WidgetInteraction(double e[2])
{
  if (this->InteractionState == svtkLineRepresentation::OnLine)
  {
  ...
  else if (this->InteractionState == svtkLineRepresentation::Scaling)
  ...
  else if (this->InteractionState == svtkLineRepresentation::TranslatingP1)
  {
    double x[3], p2[3];
    // Get the new position
  ...
  else if (this->InteractionState == svtkLineRepresentation::TranslatingP2)
  {
    double x[3], p1[3];
    // Get the new position
...
//----------------------------------------------------------------------------
void svtkLineRepresentation::PlaceWidget(double bds[6])
{
  int i;
...
//----------------------------------------------------------------------------
int svtkLineRepresentation::ComputeInteractionState(int x, int y, int svtkNotUsed(modify))
{
  ...
  {
    this->InteractionState = svtkLineRepresentation::OnP1;
    this->SetRepresentationState(svtkLineRepresentation::OnP1);
  }
  else if (p2State == svtkHandleRepresentation::Nearby)
  {
    this->InteractionState = svtkLineRepresentation::OnP2;
    this->SetRepresentationState(svtkLineRepresentation::OnP2);
  }
  else
  {
    this->InteractionState = svtkLineRepresentation::Outside;
  }

  // Okay if we're near a handle return, otherwise test the line
  if (this->InteractionState != svtkLineRepresentation::Outside)
  {
  ...
  int onLine = (svtkLine::DistanceToLine(xyz, p1, p2, t, closest) <= tol2);
  if (onLine && t < 1.0 && t > 0.0)
  {
    this->InteractionState = svtkLineRepresentation::OnLine;
    this->SetRepresentationState(svtkLineRepresentation::OnLine);
  ...
  {
    this->InteractionState = svtkLineRepresentation::Outside;
    this->SetRepresentationState(svtkLineRepresentation::Outside);
  }
...
//----------------------------------------------------------------------------
void svtkLineRepresentation::SetRepresentationState(int state)
{
  ...
  state = (state < svtkLineRepresentation::Outside
      ? svtkLineRepresentation::Outside
      : (state > svtkLineRepresentation::Scaling ? svtkLineRepresentation::Scaling : state));
  ...
  else if (state == svtkLineRepresentation::OnP1)
  {
...
//----------------------------------------------------------------------
double* svtkLineRepresentation::GetBounds()
{
  this->BuildRepresentation();
...
//----------------------------------------------------------------------------
void svtkLineRepresentation::CreateDefaultProperties()
{
  // Endpoint properties
  this->EndPointProperty = svtkProperty::New();
  this->EndPointProperty->SetColor(1, 1, 1);

  this->SelectedEndPointProperty = svtkProperty::New();
  this->SelectedEndPointProperty->SetColor(0, 1, 0);

  this->EndPoint2Property = svtkProperty::New();
  this->EndPoint2Property->SetColor(1, 1, 1);

  this->SelectedEndPoint2Property = svtkProperty::New();
...
//----------------------------------------------------------------------------
void svtkLineRepresentation::SizeHandles()
{
  // The SizeHandles() method depends on the LastPickPosition data member.
  double radius =
    this->svtkWidgetRepresentation::SizeHandlesInPixels(1.35, this->LineSource->GetPoint1());
  static_cast<svtkSphereSource*>(this->HandleGeometry[0])->SetRadius(radius);

  radius = this->svtkWidgetRepresentation::SizeHandlesInPixels(1.35, this->LineSource->GetPoint2());
  if (this->DirectionalLine)
  {
    static_cast<svtkConeSource*>(this->HandleGeometry[1])->SetRadius(radius);
    static_cast<svtkConeSource*>(this->HandleGeometry[1])->SetHeight(2.8 * radius);
  }
  else
  {
    static_cast<svtkSphereSource*>(this->HandleGeometry[1])->SetRadius(radius);
  }
}

//----------------------------------------------------------------------------
void svtkLineRepresentation::BuildRepresentation()
{
        ...
        this->Renderer->GetActiveCamera()->GetMTime() > this->BuildTime)))
  {
    ...
    static_cast<svtkSphereSource*>(this->HandleGeometry[0])->SetCenter(x1);

    this->GetPoint2WorldPosition(x2);
    this->LineSource->SetPoint2(x2);
    if (this->DirectionalLine)
    {
      static_cast<svtkConeSource*>(this->HandleGeometry[1])->SetCenter(x2);
      svtkVector3d dir(x2);
      dir = dir - svtkVector3d(x1);
      static_cast<svtkConeSource*>(this->HandleGeometry[1])->SetDirection(dir.GetData());
    }
    else
    {
      static_cast<svtkSphereSource*>(this->HandleGeometry[1])->SetCenter(x2);
    }

    this->Distance = sqrt(svtkMath::Distance2BetweenPoints(x1, x2));
...
//----------------------------------------------------------------------------
void svtkLineRepresentation::HighlightPoint(int ptId, int highlight)
{
  if (ptId == 0)
  {
    if (highlight)
...
//----------------------------------------------------------------------------
void svtkLineRepresentation::HighlightLine(int highlight)
{
  if (highlight)
  {
...
//----------------------------------------------------------------------------
void svtkLineRepresentation::SetLineColor(double r, double g, double b)
{
...
//----------------------------------------------------------------------------
void svtkLineRepresentation::ClampPosition(double x[3])
{
  for (int i = 0; i < 3; i++)
  {
...
//----------------------------------------------------------------------------
int svtkLineRepresentation::InBounds(double x[3])
{
  for (int i = 0; i < 3; i++)
  {
...
//----------------------------------------------------------------------
void svtkLineRepresentation::GetActors(svtkPropCollection* pc)
{
...
//----------------------------------------------------------------------------
void svtkLineRepresentation::ReleaseGraphicsResources(svtkWindow* w)
{
...
//----------------------------------------------------------------------------
int svtkLineRepresentation::RenderOpaqueGeometry(svtkViewport* v)
{
  int count = 0;
  this->BuildRepresentation();
...
//----------------------------------------------------------------------------
int svtkLineRepresentation::RenderTranslucentPolygonalGeometry(svtkViewport* v)
{
  int count = 0;
  this->BuildRepresentation();
...
//----------------------------------------------------------------------------
svtkTypeBool svtkLineRepresentation::HasTranslucentPolygonalGeometry()
{
  int result = 0;
  this->BuildRepresentation();
...
//----------------------------------------------------------------------------
svtkMTimeType svtkLineRepresentation::GetMTime()
{
  svtkMTimeType mTime = this->Superclass::GetMTime();
  svtkMTimeType mTime2 = this->Point1Representation->GetMTime();
...
//----------------------------------------------------------------------
void svtkLineRepresentation::SetRestrictFlag(int restrict_flag)
{
  SVTK_LEGACY_BODY(svtkLineRepresentation::SetRestricFlag, "SVTK 9");
...
//----------------------------------------------------------------------
void svtkLineRepresentation::SetDistanceAnnotationScale(double scale[3])
{
...
//----------------------------------------------------------------------
double* svtkLineRepresentation::GetDistanceAnnotationScale()
{
  return this->TextActor->GetScale();
}

//----------------------------------------------------------------------------
svtkProperty* svtkLineRepresentation::GetDistanceAnnotationProperty()
{
  return this->TextActor->GetProperty();
}

//----------------------------------------------------------------------------
void svtkLineRepresentation::PrintSelf(ostream& os, svtkIndent indent)
{
