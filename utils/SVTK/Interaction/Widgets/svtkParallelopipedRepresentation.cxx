/*=========================================================================
  ...
  Module:    svtkParallelopipedRepresentation.cxx
...
public:
  ...
  {
    svtkIdType Id[2];
    Line(svtkIdType a, svtkIdType b)
    {
      Id[0] = a;
      Id[1] = b;
    }
  } LineType;
  typedef std::vector<svtkIdType> CellType;
  ...
  void GetNeighbors(int c, svtkIdType neighborPtIds[3], int configuration = 0) const
  {
    std::set<svtkIdType> neighbors;
    ...
    int i = 0;
    for (std::set<svtkIdType>::const_iterator it = neighbors.begin(); it != neighbors.end();
  ...
  void GetNeighbors(svtkIdType node, svtkIdType neighborPtIds[3], svtkCellArray* neighborCells,
    std::vector<LineType>& lines)
  {
    GetNeighbors(8 + GetDiametricOppositeOfCorner(node), neighborPtIds, node + 1);
    svtkIdType opposingNeighborPtIds[3], opposite = GetDiametricOppositeOfCorner(node);
    GetNeighbors(opposite, opposingNeighborPtIds);

    std::vector<svtkIdType> nodes(2);
    for (int i = 0; i < 3; i++)
    {
  ...
  void FindCellsContainingNodes(
    int configuration, svtkCellArray* cellArray, const std::vector<svtkIdType>& nodes) const
  {
    svtkParallelopipedTopology::PopulateTopology(
      FindCellsContainingNodes(m_Topology[configuration], nodes), cellArray);
  }

  std::vector<CellType> FindCellsContainingNodes(
    int configuration, const std::vector<svtkIdType>& nodes)
  {
    return FindCellsContainingNodes(m_Topology[configuration], nodes);
  }

  svtkParallelopipedTopology()
  {
    ...
    for (svtkIdType i = 0; i < 8; m_Topology.push_back(GetChairClique(i++, clique)))
    {
      ;
    }
  ...
  // Populate topoplogy into a svtkCellArray.
  ...
  void PopulateTopology(int configuration, svtkCellArray* cellArray) const
  {
    svtkParallelopipedTopology::PopulateTopology(m_Topology[configuration], cellArray);
  }
...
private:
  void AddCellToClique(CliqueType& clique, svtkIdType a, svtkIdType b, svtkIdType c, svtkIdType d)
  {
    CellType v(4);
    v[0] = a;
    v[1] = b;
  ...
  static CellType RotateCell(const CellType& cell, svtkIdType endval)
  {
    CellType outputCell;
  ...
  static CellType ChairCell(svtkIdType c, const CellType& cell)
  {
  ...
  static CliqueType GetChairClique(svtkIdType c, const CliqueType& clique)
  {
  ...
  static void PopulateTopology(const CliqueType& clique, svtkCellArray* cellArray)
  {
    for (CliqueType::const_iterator clit = clique.begin(); clit != clique.end(); ++clit)
    {
      svtkIdType* ids = new svtkIdType[clit->size()];
      int i = 0;
      for (CellType::const_iterator cit = clit->begin(); cit != clit->end(); ids[i++] = *cit, ++cit)
      {
        ;
      }
      cellArray->InsertNextCell(static_cast<svtkIdType>(clit->size()), ids);
      delete[] ids;
    }
  }
  ...
  static std::vector<CellType> FindCellsContainingNodes(
    const CliqueType& clique, const std::vector<svtkIdType>& nodes)
  {
    std::vector<CellType> cells;
    for (CliqueType::const_iterator clit = clique.begin(); clit != clique.end(); ++clit)
    {
      bool found = true;
      for (std::vector<svtkIdType>::const_iterator nit = nodes.begin(); nit != nodes.end(); ++nit)
...
//----------------------------------------------------------------------------
svtkStandardNewMacro(svtkParallelopipedRepresentation);

svtkCxxSetObjectMacro(svtkParallelopipedRepresentation, HandleProperty, svtkProperty);
svtkCxxSetObjectMacro(svtkParallelopipedRepresentation, SelectedHandleProperty, svtkProperty);
svtkCxxSetObjectMacro(svtkParallelopipedRepresentation, HoveredHandleProperty, svtkProperty);

//----------------------------------------------------------------------------
svtkParallelopipedRepresentation::svtkParallelopipedRepresentation()
{
  // This contains all the connectivity information.
  this->Topology = new svtkParallelopipedTopology;
  ...
  this->HexFaceActor = svtkActor::New();
  ...
  this->HoveredHandleProperty = svtkProperty::New();
  ...
  svtkSphereHandleRepresentation* hRep = svtkSphereHandleRepresentation::New();
  ...
  // parallelopiped.
  this->ChairPointPlacer = svtkClosedSurfacePointPlacer::New();
...
//----------------------------------------------------------------------------
svtkHandleRepresentation* svtkParallelopipedRepresentation ::GetHandleRepresentation(int handleIndex)
{
...
void svtkParallelopipedRepresentation ::SetHandleRepresentation(svtkHandleRepresentation* handle)
{
  ...
  svtkSetObjectBodyMacro(HandleRepresentation, svtkHandleRepresentation, handle);
  ...
  if (this->HandleRepresentation)
  {
    ...
    {
      this->HandleRepresentations = new svtkHandleRepresentation*[8];
...
void svtkParallelopipedRepresentation::RemoveExistingChairs()
{
  ...
  if (this->HexPolyData->GetPolys()->GetNumberOfCells() == 9)
  {
    ...
    svtkCellArray* parallelopipedcells = svtkCellArray::New();
    ...
    svtkIdType neighborPtIds[3];
    svtkIdType npts = 0;
    const svtkIdType* cellPtIds = nullptr;
    ...
    svtkParallelopipedTopology::CellType nodes(3);
...
// depth of the cavity.
void svtkParallelopipedRepresentation::UpdateChairAtNode(int node)
{
  svtkIdType npts = 0;
  const svtkIdType* cellPtIds = nullptr;
  ...
  if (this->HexPolyData->GetPolys()->GetNumberOfCells() != 9)
  {
    ...
    // Hence 16 points. Look at svtkParallelopipedTopology for details.
    ...
    this->Points->SetPoint(node,
      this->Points->GetPoint(svtkParallelopipedTopology::GetDiametricOppositeOfCorner(node) + 8));
  ...
  else
  {
    ...
    svtkSmartPointer<svtkCellArray> neighborCells = svtkSmartPointer<svtkCellArray>::New();

    // Handle PointID is the diametric opposite of the chair corner on the
    // higher order parallelopiped (the chair parallelopiped).
    const svtkIdType chairHandleId =
      8 + svtkParallelopipedTopology::GetDiametricOppositeOfCorner(this->CurrentHandleIdx);

    // Get the world position of the chair handle.
    double chairPoint[3];
    this->Points->GetPoint(chairHandleId, chairPoint);

    std::vector<svtkParallelopipedTopology::LineType> lines;
    ...
    for (int i = 0; i < 3; i++)
    {
      ...
      svtkPlane::IntersectWithLine(
        chairPoint, endPoint, plane->GetNormal(), plane->GetOrigin(), t, neighborPt);
      plane->Delete();

      svtkDebugMacro(<< "ChairPoint: (" << chairPoint[0] << "," << chairPoint[1] << ","
    ...
    for (int i = 0; i < 3; i++)
    {
      std::vector<svtkIdType> nodes(3);
      svtkSmartPointer<svtkCellArray> cells = svtkSmartPointer<svtkCellArray>::New();
      nodes[0] =
        8 + svtkParallelopipedTopology::GetDiametricOppositeOfCorner(this->CurrentHandleIdx);
      nodes[1] = neighborPtIds[i];
      nodes[2] = neighborPtIds[(i + 1) % 3];
      svtkDebugMacro(<< "Looking for cells containing nodes: " << nodes[0] << "," << nodes[1] << ","
      ...
      svtkDebugMacro(<< "Parallelogram built from (nodes and points): \n"
...
int svtkParallelopipedRepresentation ::ComputeInteractionState(int X, int Y, int svtkNotUsed(modify))
{
  ...
  if (this->InteractionState == svtkParallelopipedRepresentation::RequestResizeParallelopiped ||
    this->InteractionState ==
      svtkParallelopipedRepresentation::RequestResizeParallelopipedAlongAnAxis ||
    this->InteractionState == svtkParallelopipedRepresentation::RequestChairMode)
  {
    this->CurrentHandleIdx = -1;
    ...
    for (int i = 0; i < 8; i++)
    {
      ...
      if (this->HandleRepresentations[i]->GetInteractionState() ==
        svtkHandleRepresentation::Selecting)
      {
        // The selected handle.
        ...
        {
          case svtkParallelopipedRepresentation::RequestResizeParallelopiped:
            this->InteractionState =
              (this->CurrentHandleIdx == this->ChairHandleIdx) ? ChairMode : ResizingParallelopiped;
            break;
          case svtkParallelopipedRepresentation::RequestResizeParallelopipedAlongAnAxis:
            this->InteractionState = (this->CurrentHandleIdx == this->ChairHandleIdx)
              ? ChairMode
              : ResizingParallelopipedAlongAnAxis;
            break;
          case svtkParallelopipedRepresentation::RequestChairMode:
          {
              ...
              this->HexPolyData->GetPolys()->GetNumberOfCells() == 9)
            {
              ...
              this->InteractionState = svtkParallelopipedRepresentation::Inside;
            ...
            svtkPlaneCollection* pc = svtkPlaneCollection::New();
    ...
    if (this->CurrentHandleIdx == -1)
    {
      ...
      this->InteractionState = (this->ChairPointPlacer->ComputeWorldPosition(this->Renderer,
          ...
          ? svtkParallelopipedRepresentation::Inside
          : svtkParallelopipedRepresentation::Outside);
    }

    if (this->InteractionState == svtkParallelopipedRepresentation::Inside &&
      oldInteractionState ==
        svtkParallelopipedRepresentation::RequestResizeParallelopipedAlongAnAxis)
    {
  ...
  // Handle the resizing operations (along the axis or arbitrarily).
      ...
      svtkParallelopipedRepresentation::ResizingParallelopipedAlongAnAxis ||
    this->InteractionState == svtkParallelopipedRepresentation::ResizingParallelopiped)
  {
    ...
    if (this->CurrentHandleIdx != -1)
    {
      ...
      double maxConfidence = SVTK_DOUBLE_MIN;
      ...
      for (int i = 0; i < 3; i++)
      {
        ...
        svtkInteractorObserver::ComputeWorldToDisplay(this->Renderer, neighborWorldPos[i][0],
        ...
        if (this->LastResizeAxisIdx == -1 ||
          this->InteractionState == svtkParallelopipedRepresentation::ResizingParallelopiped)
        {
          const double confidence = fabs(svtkMath::Dot2D(axis[i], motionVector));
      ...
      if (t > 0.0 // We are moving towards the other handle
        && (svtkMath::Distance2BetweenPoints(neighborWorldPos[axisIdx], newHandleWorldPos)) <
          (this->AbsoluteMinimumThickness * this->AbsoluteMinimumThickness))
      {
        // Too close. We don't want the parallelopiped collapsing, do we ?
        svtkDebugMacro(<< "AbsoluteMaximumThickness = " << this->AbsoluteMinimumThickness
                      << " This move will bring us "
                      << sqrt(svtkMath::Distance2BetweenPoints(
        ...
        t = 1.0 -
          this->AbsoluteMinimumThickness /
            sqrt(svtkMath::Distance2BetweenPoints(neighborWorldPos[axisIdx], handleWorldPos));
        ...
        svtkDebugMacro("So we are revising the value of t to "
      ...
      {
        std::vector<svtkIdType> nodes2(1);
        nodes2[0] =
          svtkParallelopipedTopology::GetDiametricOppositeOfCorner(this->ChairHandleIdx) + 8;
        const svtkParallelopipedTopology::CliqueType cells2 =
          this->Topology->FindCellsContainingNodes(this->ChairHandleIdx + 1, nodes2);

        for (svtkParallelopipedTopology::CliqueType::const_iterator clit = cells2.begin();
      ...
      // Translate this face...
      for (svtkIdType i = 0; i < npts; this->TranslatePoint(cellPtIds[i++], handleTranslation))
      {
        ;
      }
    ...
    else
    {
      ...
      this->InteractionState = svtkParallelopipedRepresentation::Outside;
    }
  }
  ...
  else if (this->InteractionState == svtkParallelopipedRepresentation::ChairMode)
  {
    // Ensure that a handle has been picked.
    if (this->CurrentHandleIdx != -1)
    {
  ...
  else
  {
    ...
    if (this->InteractionState == svtkParallelopipedRepresentation::Outside)
    {
...
//----------------------------------------------------------------------------
void svtkParallelopipedRepresentation ::TranslatePoint(int id, const double translation[3])
{
  double p[3];
...
//
void svtkParallelopipedRepresentation::GetBoundingPlanes(svtkPlaneCollection* pc)
{
...
//
void svtkParallelopipedRepresentation ::GetParallelopipedBoundingPlanes(svtkPlaneCollection* pc)
{
  svtkPlaneCollection* pc2 = svtkPlaneCollection::New();
...
void svtkParallelopipedRepresentation::DefinePlane(
  svtkPlane* plane, svtkIdType id1, svtkIdType id2, svtkIdType id3)
{
  double p[3][3];
...
void svtkParallelopipedRepresentation::DefinePlane(svtkPlane* plane, double p[3][3])
{
  plane->SetOrigin(p[0]);
...
//----------------------------------------------------------------------
void svtkParallelopipedRepresentation::GetActors(svtkPropCollection* pc)
{
  for (int i = 0; i < 8; i++)
  {
...
//----------------------------------------------------------------------
void svtkParallelopipedRepresentation::ReleaseGraphicsResources(svtkWindow* w)
{
...
//----------------------------------------------------------------------
int svtkParallelopipedRepresentation::RenderOverlay(svtkViewport* v)
{
  int count = 0;
...
//----------------------------------------------------------------------------
int svtkParallelopipedRepresentation::RenderOpaqueGeometry(svtkViewport* viewport)
{
  int count = 0;
...
//----------------------------------------------------------------------------
void svtkParallelopipedRepresentation::PositionHandles()
{
  for (int i = 0; i < 8; ++i)
  {
...
//----------------------------------------------------------------------------
void svtkParallelopipedRepresentation::HandlesOn()
{
...
//----------------------------------------------------------------------------
void svtkParallelopipedRepresentation::HandlesOff()
{
...
//----------------------------------------------------------------------------
void svtkParallelopipedRepresentation::SetHandleHighlight(int handleIdx, svtkProperty* property)
{
  if (handleIdx == -1)
  {
    // Do for all handles
    for (int i = 0; i < 8; i++)
    {
      static_cast<svtkSphereHandleRepresentation*>(this->HandleRepresentations[i])
  ...
  {
    static_cast<svtkSphereHandleRepresentation*>(this->HandleRepresentations[handleIdx])
      ->SetProperty(property);
    static_cast<svtkSphereHandleRepresentation*>(this->HandleRepresentations[handleIdx])
      ->SetSelectedProperty(property);
  }
...
//----------------------------------------------------------------------------
void svtkParallelopipedRepresentation ::SetFaceHighlight(svtkCellArray* face, svtkProperty* p)
{
  if (face)
  {
...
//----------------------------------------------------------------------------
void svtkParallelopipedRepresentation::HighlightAllFaces()
{
  svtkSmartPointer<svtkCellArray> cells = svtkSmartPointer<svtkCellArray>::New();
...
//----------------------------------------------------------------------------
void svtkParallelopipedRepresentation::UnHighlightAllFaces()
{
...
void svtkParallelopipedRepresentation::Translate(int X, int Y)
{
  ...
  svtkInteractorObserver::ComputeWorldToDisplay(this->Renderer, center[0], center[1], center[2], fp);
...
void svtkParallelopipedRepresentation::Translate(double translation[3])
{
  double* pts = static_cast<svtkDoubleArray*>(this->Points->GetData())->GetPointer(0);
  for (int i = 0; i < 16; i++)
  {
...
//----------------------------------------------------------------------------
void svtkParallelopipedRepresentation::Scale(int svtkNotUsed(X), int Y)
{
  double* pts = static_cast<svtkDoubleArray*>(this->Points->GetData())->GetPointer(0);
...
//----------------------------------------------------------------------------
void svtkParallelopipedRepresentation::PlaceWidget(double bounds[6])
{
...
//----------------------------------------------------------------------------
void svtkParallelopipedRepresentation::PlaceWidget(double corners[8][3])
{
...
//----------------------------------------------------------------------------
void svtkParallelopipedRepresentation::GetPolyData(svtkPolyData* pd)
{
...
//----------------------------------------------------------------------------
double* svtkParallelopipedRepresentation::GetBounds()
{
  return this->Points->GetBounds();
}

//----------------------------------------------------------------------------
void svtkParallelopipedRepresentation::BuildRepresentation()
{
  this->Points->Modified();
}

//----------------------------------------------------------------------------
void svtkParallelopipedRepresentation::PrintSelf(ostream& os, svtkIndent indent)
{
