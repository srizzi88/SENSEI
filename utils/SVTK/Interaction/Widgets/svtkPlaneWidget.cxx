...
svtkStandardNewMacro(svtkPlaneWidget);

svtkCxxSetObjectMacro(svtkPlaneWidget, PlaneProperty, svtkProperty);

svtkPlaneWidget::svtkPlaneWidget()
  : svtkPolyDataSourceWidget()
{
  this->State = svtkPlaneWidget::Start;
  this->EventCallbackCommand->SetCallback(svtkPlaneWidget::ProcessEvents);

  this->NormalToXAxis = 0;
  this->NormalToYAxis = 0;
  this->NormalToZAxis = 0;
  this->Representation = SVTK_PLANE_WIREFRAME;
  ...
  this->PlaneOutline = svtkPolyData::New();
  ...
  // Create the handles
  this->Handle = new svtkActor*[4];
  this->HandleMapper = new svtkPolyDataMapper*[4];
  this->HandleGeometry = new svtkSphereSource*[4];
  for (i = 0; i < 4; i++)
  {
...
void svtkPlaneWidget::SetEnabled(int enabling)
{
  if (!this->Interactor)
  {
...
//------------------------------------------------------------------------------
void svtkPlaneWidget::RegisterPickers()
{
...
void svtkPlaneWidget::ProcessEvents(
  svtkObject* svtkNotUsed(object), unsigned long event, void* clientdata, void* svtkNotUsed(calldata))
{
  svtkPlaneWidget* self = reinterpret_cast<svtkPlaneWidget*>(clientdata);
...
void svtkPlaneWidget::PrintSelf(ostream& os, svtkIndent indent)
{
  ...
  if (this->Representation == SVTK_PLANE_WIREFRAME)
  {
    os << "Wireframe\n";
  }
  else if (this->Representation == SVTK_PLANE_SURFACE)
  {
    os << "Surface\n";
  }
  else //( this->Representation == SVTK_PLANE_OUTLINE )
  {
    os << "Outline\n";
  }
...
void svtkPlaneWidget::PositionHandles()
{
  double* o = this->PlaneSource->GetOrigin();
  double* pt1 = this->PlaneSource->GetPoint1();
  double* pt2 = this->PlaneSource->GetPoint2();
  ...
  // set up the outline
  if (this->Representation == SVTK_PLANE_OUTLINE)
  {
  ...
  double d = sqrt(svtkMath::Distance2BetweenPoints(
...
int svtkPlaneWidget::HighlightHandle(svtkProp* prop)
{
...
void svtkPlaneWidget::HighlightNormal(int highlight)
{
  if (highlight)
  {
    this->ValidPick = 1;
...
void svtkPlaneWidget::HighlightPlane(int highlight)
{
  if (highlight)
  {
    this->ValidPick = 1;
...
void svtkPlaneWidget::OnStartPinch()
{
  ...
  {
    this->State = svtkPlaneWidget::Outside;
    return;
  }
  ...
  if (path != nullptr)
  {
    this->State = svtkPlaneWidget::Pinching;
...
void svtkPlaneWidget::OnPinch()
{
  if (this->State != svtkPlaneWidget::Pinching)
  {
    return;
  }
...
void svtkPlaneWidget::OnEndPinch()
{
  if (this->State != svtkPlaneWidget::Pinching)
  {
    return;
...
void svtkPlaneWidget::OnLeftButtonDown()
{
  int X = this->Interactor->GetEventPosition()[0];
  int Y = this->Interactor->GetEventPosition()[1];
  ...
  if (path != nullptr)
  {
    this->State = svtkPlaneWidget::Moving;
  ...
  else
  {
    ...
    if (path != nullptr)
    {
      ...
      {
        this->State = svtkPlaneWidget::Rotating;
        this->HighlightNormal(1);
      }
      else if (this->Interactor->GetControlKey())
      {
        this->State = svtkPlaneWidget::Spinning;
        this->HighlightNormal(1);
      }
      else
      {
...
void svtkPlaneWidget::OnLeftButtonUp()
{
  if (this->State == svtkPlaneWidget::Outside || this->State == svtkPlaneWidget::Start)
  {
    return;
  }
...
void svtkPlaneWidget::OnMiddleButtonDown()
{
  int X = this->Interactor->GetEventPosition()[0];
  int Y = this->Interactor->GetEventPosition()[1];
  ...
  if (path != nullptr)
  {
    this->State = svtkPlaneWidget::Pushing;
    this->HighlightPlane(1);
    this->HighlightNormal(1);
    this->HighlightHandle(path->GetFirstNode()->GetViewProp());
  }
...
void svtkPlaneWidget::OnMiddleButtonUp()
{
  if (this->State == svtkPlaneWidget::Outside || this->State == svtkPlaneWidget::Start)
  {
    return;
...
void svtkPlaneWidget::OnRightButtonDown()
{
  int X = this->Interactor->GetEventPosition()[0];
  int Y = this->Interactor->GetEventPosition()[1];
  ...
  if (path != nullptr)
  {
    this->State = svtkPlaneWidget::Scaling;
    this->HighlightPlane(1);
    this->HighlightHandle(path->GetFirstNode()->GetViewProp());
  }
...
void svtkPlaneWidget::OnRightButtonUp()
{
  if (this->State == svtkPlaneWidget::Outside || this->State == svtkPlaneWidget::Start)
  {
    return;
...
void svtkPlaneWidget::OnMouseMove()
{
  ...
  // Process the motion
  if (this->State == svtkPlaneWidget::Moving)
  {
    // Okay to process
    if (this->CurrentHandle)
    {
  ...
  else if (this->State == svtkPlaneWidget::Scaling)
  {
    this->Scale(prevPickPoint, pickPoint, X, Y);
  }
  else if (this->State == svtkPlaneWidget::Pushing)
  {
    this->Push(prevPickPoint, pickPoint);
  }
  else if (this->State == svtkPlaneWidget::Rotating)
  {
    camera->GetViewPlaneNormal(vpn);
    this->Rotate(X, Y, prevPickPoint, pickPoint, vpn);
  }
  else if (this->State == svtkPlaneWidget::Spinning)
  {
...
void svtkPlaneWidget::MoveOrigin(double* p1, double* p2)
{
  ...
  double vN = svtkMath::Norm(v);
  double n13 = svtkMath::Norm(p13);
  double n23 = svtkMath::Norm(p23);
  ...
  double d1 = (vN / n13) * svtkMath::Dot(v, p13) / (vN * n13);
  double d2 = (vN / n23) * svtkMath::Dot(v, p23) / (vN * n23);
...
void svtkPlaneWidget::MovePoint1(double* p1, double* p2)
{
  // Get the plane definition
  double* o = this->PlaneSource->GetOrigin();
  double* pt1 = this->PlaneSource->GetPoint1();
  ...
  double vN = svtkMath::Norm(v);
  double n02 = svtkMath::Norm(p02);
  double n32 = svtkMath::Norm(p32);
  ...
  // Project v onto these vector to determine the amount of motion
  // Scale it by the relative size of the motion to the vector length
  double d1 = (vN / n02) * svtkMath::Dot(v, p02) / (vN * n02);
  double d2 = (vN / n32) * svtkMath::Dot(v, p32) / (vN * n32);
...
void svtkPlaneWidget::MovePoint2(double* p1, double* p2)
{
  // Get the plane definition
  double* o = this->PlaneSource->GetOrigin();
  double* pt1 = this->PlaneSource->GetPoint1();
  ...
  double vN = svtkMath::Norm(v);
  double n31 = svtkMath::Norm(p31);
  double n01 = svtkMath::Norm(p01);
  ...
  // Project v onto these vector to determine the amount of motion
  // Scale it by the relative size of the motion to the vector length
  double d1 = (vN / n31) * svtkMath::Dot(v, p31) / (vN * n31);
  double d2 = (vN / n01) * svtkMath::Dot(v, p01) / (vN * n01);
...
void svtkPlaneWidget::MovePoint3(double* p1, double* p2)
{
  // Get the plane definition
  double* o = this->PlaneSource->GetOrigin();
  double* pt1 = this->PlaneSource->GetPoint1();
  ...
  double vN = svtkMath::Norm(v);
  double n10 = svtkMath::Norm(p10);
  double n20 = svtkMath::Norm(p20);
  ...
  // Project v onto these vector to determine the amount of motion
  // Scale it by the relative size of the motion to the vector length
  double d1 = (vN / n10) * svtkMath::Dot(v, p10) / (vN * n10);
  double d2 = (vN / n20) * svtkMath::Dot(v, p20) / (vN * n20);
...
void svtkPlaneWidget::Rotate(int X, int Y, double* p1, double* p2, double* vpn)
{
  double* o = this->PlaneSource->GetOrigin();
  double* pt1 = this->PlaneSource->GetPoint1();
  double* pt2 = this->PlaneSource->GetPoint2();
...
void svtkPlaneWidget::Spin(double* p1, double* p2)
{
  // Mouse motion vector in world space
  double v[3];
  v[0] = p2[0] - p1[0];
  ...
  // Spin angle
  double theta = svtkMath::DegreesFromRadians(svtkMath::Dot(v, ax_cross_rv) / rs);
...
void svtkPlaneWidget::Translate(double* p1, double* p2)
{
...
void svtkPlaneWidget::Scale(double* p1, double* p2, int svtkNotUsed(X), int Y)
{
  // Get the motion vector
  double v[3];
  v[0] = p2[0] - p1[0];
...
void svtkPlaneWidget::Push(double* p1, double* p2)
{
  // Get the motion vector
  double v[3];
  v[0] = p2[0] - p1[0];
  ...
  this->PlaneSource->Push(svtkMath::Dot(v, this->Normal));
  this->PlaneSource->Update();
  this->PositionHandles();
}

void svtkPlaneWidget::CreateDefaultProperties()
{
  // Handle properties
...
void svtkPlaneWidget::PlaceWidget(double bds[6])
{
  int i;
...
void svtkPlaneWidget::SizeHandles()
{
...
void svtkPlaneWidget::SelectRepresentation()
{
  if (!this->CurrentRenderer)
  {
    return;
  ...
  if (this->Representation == SVTK_PLANE_OFF)
  {
    this->CurrentRenderer->RemoveActor(this->PlaneActor);
  }
  else if (this->Representation == SVTK_PLANE_OUTLINE)
  {
    this->CurrentRenderer->RemoveActor(this->PlaneActor);
    this->CurrentRenderer->AddActor(this->PlaneActor);
  ...
  else //( this->Representation == SVTK_PLANE_WIREFRAME )
  {
    this->CurrentRenderer->RemoveActor(this->PlaneActor);
    this->CurrentRenderer->AddActor(this->PlaneActor);
    this->PlaneMapper->SetInputConnection(this->PlaneSource->GetOutputPort());
...
void svtkPlaneWidget::SetResolution(int r)
{
  this->PlaneSource->SetXResolution(r);
  this->PlaneSource->SetYResolution(r);
}

int svtkPlaneWidget::GetResolution()
{
...
void svtkPlaneWidget::SetOrigin(double x, double y, double z)
{
  this->PlaneSource->SetOrigin(x, y, z);
  this->PositionHandles();
}

void svtkPlaneWidget::SetOrigin(double x[3])
{
  this->SetOrigin(x[0], x[1], x[2]);
}

double* svtkPlaneWidget::GetOrigin()
{
  return this->PlaneSource->GetOrigin();
}

void svtkPlaneWidget::GetOrigin(double xyz[3])
{
...
void svtkPlaneWidget::SetPoint1(double x, double y, double z)
{
  this->PlaneSource->SetPoint1(x, y, z);
  this->PositionHandles();
}

void svtkPlaneWidget::SetPoint1(double x[3])
{
  this->SetPoint1(x[0], x[1], x[2]);
}

double* svtkPlaneWidget::GetPoint1()
{
  return this->PlaneSource->GetPoint1();
}

void svtkPlaneWidget::GetPoint1(double xyz[3])
{
...
void svtkPlaneWidget::SetPoint2(double x, double y, double z)
{
  this->PlaneSource->SetPoint2(x, y, z);
  this->PositionHandles();
}

void svtkPlaneWidget::SetPoint2(double x[3])
{
  this->SetPoint2(x[0], x[1], x[2]);
}

double* svtkPlaneWidget::GetPoint2()
{
  return this->PlaneSource->GetPoint2();
}

void svtkPlaneWidget::GetPoint2(double xyz[3])
{
...
void svtkPlaneWidget::SetCenter(double x, double y, double z)
{
...
// Description:
// Set the center of the plane.
void svtkPlaneWidget::SetCenter(double c[3])
{
...
double* svtkPlaneWidget::GetCenter()
{
  return this->PlaneSource->GetCenter();
}

void svtkPlaneWidget::GetCenter(double xyz[3])
{
...
void svtkPlaneWidget::SetNormal(double x, double y, double z)
{
...
// Description:
// Set the normal to the plane.
void svtkPlaneWidget::SetNormal(double n[3])
{
...
double* svtkPlaneWidget::GetNormal()
{
  return this->PlaneSource->GetNormal();
}

void svtkPlaneWidget::GetNormal(double xyz[3])
{
  this->PlaneSource->GetNormal(xyz);
}

void svtkPlaneWidget::GetPolyData(svtkPolyData* pd)
{
  pd->ShallowCopy(this->PlaneSource->GetOutput());
}

svtkPolyDataAlgorithm* svtkPlaneWidget::GetPolyDataAlgorithm()
{
  return this->PlaneSource;
}

void svtkPlaneWidget::GetPlane(svtkPlane* plane)
{
  if (plane == nullptr)
  {
    return;
...
void svtkPlaneWidget::UpdatePlacement()
{
  this->PlaneSource->Update();
  this->PositionHandles();
}
