 ...
 *
 * Most widgets in SVTK have a need to translate of 2D display coordinates (as
 ...
 * <p>1) svtkFocalPlanePointPlacer: This class converts 2D display positions to
 * world positions such that they lie on the focal plane.
 * <p>2) svtkPolygonalSurfacePointPlacer: Converts 2D display positions to
 * world positions such that they lie on the surface of one or more specified
 * polydatas.
 * <p>3) svtkImageActorPointPlacer: Converts 2D display positions to world
 * positions such that they lie on an ImageActor
 * <p>4) svtkBoundedPlanePointPlacer: Converts 2D display positions to world
 * positions such that they lie within a set of specified bounding planes.
 * <p>5) svtkTerrainDataPointPlacer: Converts 2D display positions to world
...
#ifndef svtkPointPlacer_h
#define svtkPointPlacer_h
...
class SVTKINTERACTIONWIDGETS_EXPORT svtkPointPlacer : public svtkObject
{
public:
  /**
   * Instantiate this class.
   */
  static svtkPointPlacer* New();

  //@{
  /**
   * Standard methods for instances of this class.
   */
  svtkTypeMacro(svtkPointPlacer, svtkObject);
   ...
   */
  virtual int ComputeWorldPosition(svtkRenderer* ren, double displayPos[2], double refWorldPos[3],
   ...
   */
  virtual int ValidateDisplayPosition(svtkRenderer*, double displayPos[2]);
   ...
   */
  virtual int UpdateWorldPosition(svtkRenderer* ren, double worldPos[3], double worldOrient[9]);

  /**
   * Give the placer a chance to update the node information, if any. Most
   * placers do not maintain any cached node information.
   * svtkPolygonalSurfacePointPlacer is one that does. It stores the point
   * id (id on the surface mesh) on which its drawn. The second argument
   * may be used to pass that in.
   * Update world position
   */
  virtual int UpdateNodeWorldPosition(double worldPos[3], svtkIdType nodePointId);
...
protected:
  svtkPointPlacer();
  ~svtkPointPlacer() override;

  int PixelTolerance;
  double WorldTolerance;

private:
  svtkPointPlacer(const svtkPointPlacer&) = delete;
  void operator=(const svtkPointPlacer&) = delete;
};
