/*=========================================================================
  ...
  Module:    svtkPolyLineRepresentation.cxx
...
//----------------------------------------------------------------------------
svtkStandardNewMacro(svtkPolyLineRepresentation);

//----------------------------------------------------------------------------
svtkPolyLineRepresentation::svtkPolyLineRepresentation()
{
  ...
  this->PolyLineSource = svtkPolyLineSource::New();
...
//----------------------------------------------------------------------------
svtkDoubleArray* svtkPolyLineRepresentation::GetHandlePositions()
{
  return svtkArrayDownCast<svtkDoubleArray>(this->PolyLineSource->GetPoints()->GetData());
}

//----------------------------------------------------------------------------
void svtkPolyLineRepresentation::BuildRepresentation()
{
  this->ValidPick = 1;
  // TODO: Avoid unnecessary rebuilds.
  // Handles have changed position, re-compute the points
  svtkPoints* points = this->PolyLineSource->GetPoints();
...
//----------------------------------------------------------------------------
void svtkPolyLineRepresentation::SetNumberOfHandles(int npts)
{
  ...
  if (npts < 1)
  {
    svtkGenericWarningMacro(<< "svtkPolyLineRepresentation: minimum of 1 points required.");
    return;
  }
  ...
  svtkIdType prevNumPoints = this->PolyLineSource->GetNumberOfPoints();
  if (this->PolyLineSource->GetNumberOfPoints() != npts)
  {
    this->PolyLineSource->Resize(npts);
    for (svtkIdType i = prevNumPoints; i < npts; ++i)
    {
...
//----------------------------------------------------------------------------
void svtkPolyLineRepresentation::GetPolyData(svtkPolyData* pd)
{
  this->PolyLineSource->Update();
...
//----------------------------------------------------------------------------
double svtkPolyLineRepresentation::GetSummedLength()
{
  svtkPoints* points = this->PolyLineSource->GetOutput()->GetPoints();
...
//----------------------------------------------------------------------------
int svtkPolyLineRepresentation::InsertHandleOnLine(double* pos)
{
...
//----------------------------------------------------------------------------
void svtkPolyLineRepresentation::InitializeHandles(svtkPoints* points)
{
  if (!points)
  {
    return;
...
//----------------------------------------------------------------------------
void svtkPolyLineRepresentation::PrintSelf(ostream& os, svtkIndent indent)
{
