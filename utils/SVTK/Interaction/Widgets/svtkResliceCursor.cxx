...
//----------------------------------------------------------------------------
svtkStandardNewMacro(svtkResliceCursor);
svtkCxxSetObjectMacro(svtkResliceCursor, Image, svtkImageData);

//----------------------------------------------------------------------------
svtkResliceCursor::svtkResliceCursor()
{
  this->XAxis[0] = 1.0;
  this->XAxis[1] = 0.0;
  this->XAxis[2] = 0.0;
  ...
  this->ReslicePlanes = svtkPlaneCollection::New();
  ...
  for (int i = 0; i < 3; i++)
  {
    ...
    this->CenterlineAxis[i] = svtkPolyData::New();

    svtkSmartPointer<svtkPoints> pointsc = svtkSmartPointer<svtkPoints>::New();
    svtkSmartPointer<svtkCellArray> linesc = svtkSmartPointer<svtkCellArray>::New();
...
//----------------------------------------------------------------------------
void svtkResliceCursor::BuildCursorTopology()
{
  if (this->Hole)
  {
...
//----------------------------------------------------------------------------
void svtkResliceCursor::BuildCursorTopologyWithoutHole()
{
  svtkIdType ptIds[2];
...
//----------------------------------------------------------------------------
void svtkResliceCursor::BuildCursorTopologyWithHole()
{
  svtkIdType ptIds[2];
...
//----------------------------------------------------------------------------
svtkPlane* svtkResliceCursor::GetPlane(int i)
{
  return this->ReslicePlanes->GetItem(i);
}

//----------------------------------------------------------------------------
svtkPolyData* svtkResliceCursor::GetPolyData()
{
  this->Update();
...
//----------------------------------------------------------------------------
void svtkResliceCursor::Update()
{
  if (!this->Image)
  {
    svtkErrorMacro(<< "Image not set !");
    return;
  }
...
//----------------------------------------------------------------------------
void svtkResliceCursor::ComputeAxes()
{
  double normals[3][3];
  for (int i = 0; i < 3; i++)
  {
  ...
  svtkMath::Cross(normals[0], normals[1], this->ZAxis);
  svtkMath::Cross(normals[1], normals[2], this->XAxis);
  svtkMath::Cross(normals[2], normals[0], this->YAxis);
}

//----------------------------------------------------------------------------
void svtkResliceCursor::BuildCursorGeometryWithHole()
{
  this->ComputeAxes();
  ...
  for (int j = 0; j < 3; j++)
  {
    svtkPoints* centerlinePoints = this->CenterlineAxis[j]->GetPoints();
...
//----------------------------------------------------------------------------
void svtkResliceCursor::BuildCursorGeometryWithoutHole()
{
  this->ComputeAxes();
...
//----------------------------------------------------------------------------
void svtkResliceCursor::BuildCursorGeometry()
{
  if (this->Hole)
  {
...
//----------------------------------------------------------------------------
void svtkResliceCursor::BuildPolyData()
{
  this->ComputeAxes();
  ...
  svtkIdType facePtIds[6][4] = { { 0, 2, 4, 6 }, { 1, 7, 5, 3 }, { 1, 3, 2, 0 }, { 0, 6, 7, 1 },
    { 2, 3, 5, 4 }, { 6, 4, 5, 7 } };
  ...
  for (int j = 0; j < 3; j++)
  {
    ...
    svtkSmartPointer<svtkCellArray> slabPolys = svtkSmartPointer<svtkCellArray>::New();
    slabPolys->AllocateEstimate(6, 4);

    for (int i = 0; i < 6; i++)
    {
      svtkIdType currFacePtIds[4] = { facePtIds[i][0] + 8 * j, facePtIds[i][1] + 8 * j,
...
//----------------------------------------------------------------------------
void svtkResliceCursor::SetCenter(double _arg1, double _arg2, double _arg3)
{
...
//----------------------------------------------------------------------------
void svtkResliceCursor::SetCenter(double _arg[3])
{
  this->SetCenter(_arg[0], _arg[1], _arg[2]);
}

//----------------------------------------------------------------------------
svtkPolyData* svtkResliceCursor::GetCenterlineAxisPolyData(int axis)
{
  this->Update();
...
//----------------------------------------------------------------------------
double* svtkResliceCursor::GetAxis(int i)
{
  if (i == 0)
  {
    return this->XAxis;
...
//----------------------------------------------------------------------------
svtkMTimeType svtkResliceCursor::GetMTime()
{
  svtkMTimeType mTime = this->Superclass::GetMTime();
  for (int i = 0; i < 3; i++)
  {
    svtkMTimeType time = this->GetPlane(i)->GetMTime();
    if (time > mTime)
    {
      mTime = time;
    }
...
//----------------------------------------------------------------------------
void svtkResliceCursor::PrintSelf(ostream& os, svtkIndent indent)
{
