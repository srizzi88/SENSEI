/*=========================================================================
  ...
  Module:    svtkResliceCursorLineRepresentation.cxx
...
#include "svtkResliceCursorPicker.h"
...
svtkStandardNewMacro(svtkResliceCursorLineRepresentation);

//----------------------------------------------------------------------
svtkResliceCursorLineRepresentation::svtkResliceCursorLineRepresentation()
{
  this->ResliceCursorActor = svtkResliceCursorActor::New();

  this->Picker = svtkResliceCursorPicker::New();
  this->Picker->SetTolerance(0.025);

  this->MatrixReslice = svtkMatrix4x4::New();
  this->MatrixView = svtkMatrix4x4::New();
  this->MatrixReslicedView = svtkMatrix4x4::New();
}
...
//----------------------------------------------------------------------
svtkResliceCursor* svtkResliceCursorLineRepresentation::GetResliceCursor()
{
  return this->ResliceCursorActor->GetCursorAlgorithm()->GetResliceCursor();
}

//----------------------------------------------------------------------
int svtkResliceCursorLineRepresentation::ComputeInteractionState(int X, int Y, int modify)
{
  this->InteractionState = svtkResliceCursorLineRepresentation::Outside;
  ...
  if (pickedCenter)
  {
    this->InteractionState = svtkResliceCursorLineRepresentation::OnCenter;
  }
  else if (pickedAxis1)
  {
    this->InteractionState = svtkResliceCursorLineRepresentation::OnAxis1;
  }
  else if (pickedAxis2)
  {
    this->InteractionState = svtkResliceCursorLineRepresentation::OnAxis2;
  }
...
void svtkResliceCursorLineRepresentation ::StartWidgetInteraction(double startEventPos[2])
{
  ...
  {
    if (svtkResliceCursor* rc = this->GetResliceCursor())
    {
...
//----------------------------------------------------------------------
void svtkResliceCursorLineRepresentation::WidgetInteraction(double e[2])
{
  svtkResliceCursor* rc = this->GetResliceCursor();
  ...
  if (rc->GetThickMode() &&
    this->ManipulationMode == svtkResliceCursorRepresentation::ResizeThickness)
  {
...
//----------------------------------------------------------------------
double svtkResliceCursorLineRepresentation ::RotateAxis(double e[2], int axis)
{
  svtkResliceCursor* rc = this->GetResliceCursor();
  ...
  svtkMath::Normalize(lastVector);
  svtkMath::Normalize(currVector);

  // compute the angle betweem both vectors. This is the amount to
  // rotate by.
  double angle = acos(svtkMath::Dot(lastVector, currVector));
  double crossVector[3];
  svtkMath::Cross(lastVector, currVector, crossVector);

  double aboutAxis[3];
  const int rcPlaneIdx = this->ResliceCursorActor->GetCursorAlgorithm()->GetReslicePlaneNormal();
  svtkPlane* normalPlane = rc->GetPlane(rcPlaneIdx);
  normalPlane->GetNormal(aboutAxis);
  const double align = svtkMath::Dot(aboutAxis, crossVector);
...
svtkResliceCursorPolyDataAlgorithm* svtkResliceCursorLineRepresentation::GetCursorAlgorithm()
{
  return this->ResliceCursorActor->GetCursorAlgorithm();
}

//----------------------------------------------------------------------
void svtkResliceCursorLineRepresentation ::RotateAxis(int axis, double angle)
{
  svtkResliceCursor* rc = this->GetResliceCursor();
  svtkPlane* planeToBeRotated = rc->GetPlane(axis);
...
//----------------------------------------------------------------------
void svtkResliceCursorLineRepresentation ::RotateVectorAboutVector(double vectorToBeRotated[3],
...
//----------------------------------------------------------------------
int svtkResliceCursorLineRepresentation ::DisplayToReslicePlaneIntersection(
  double displayPos[2], double intersectionPos[3])
{
  ...
  const int rcPlaneIdx = this->ResliceCursorActor->GetCursorAlgorithm()->GetReslicePlaneNormal();
  svtkPlane* normalPlane = this->GetResliceCursor()->GetPlane(rcPlaneIdx);
...
//----------------------------------------------------------------------
void svtkResliceCursorLineRepresentation::BuildRepresentation()
{
...
//----------------------------------------------------------------------
void svtkResliceCursorLineRepresentation::ReleaseGraphicsResources(svtkWindow* w)
{
...
//----------------------------------------------------------------------
int svtkResliceCursorLineRepresentation::RenderOverlay(svtkViewport* viewport)
{
  int count = 0;
...
//----------------------------------------------------------------------
void svtkResliceCursorLineRepresentation::SetUserMatrix(svtkMatrix4x4* m)
{
...
//----------------------------------------------------------------------
int svtkResliceCursorLineRepresentation ::RenderOpaqueGeometry(svtkViewport* viewport)
{
  ...
  const double d = sqrt(svtkMath::Distance2BetweenPoints(cp, fp));
...
double* svtkResliceCursorLineRepresentation::GetBounds()
{
  svtkMath::UninitializeBounds(this->InitialBounds);

  if (svtkResliceCursor* r = this->GetResliceCursor())
  {
...
//-----------------------------------------------------------------------------
int svtkResliceCursorLineRepresentation::RenderTranslucentPolygonalGeometry(svtkViewport* viewport)
{
  int count = 0;
  if (this->TexturePlaneActor->GetVisibility() && !this->UseImageActor)
  {
...
//-----------------------------------------------------------------------------
svtkTypeBool svtkResliceCursorLineRepresentation::HasTranslucentPolygonalGeometry()
{
...
//----------------------------------------------------------------------
void svtkResliceCursorLineRepresentation::Highlight(int) {}

//----------------------------------------------------------------------
void svtkResliceCursorLineRepresentation::PrintSelf(ostream& os, svtkIndent indent)
{
