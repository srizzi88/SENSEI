/*=========================================================================
  ...
  Module:    svtkResliceCursorPicker.cxx
...
svtkStandardNewMacro(svtkResliceCursorPicker);
svtkCxxSetObjectMacro(
  svtkResliceCursorPicker, ResliceCursorAlgorithm, svtkResliceCursorPolyDataAlgorithm);
svtkCxxSetObjectMacro(svtkResliceCursorPicker, TransformMatrix, svtkMatrix4x4);

//----------------------------------------------------------------------------
svtkResliceCursorPicker::svtkResliceCursorPicker()
{
...
//----------------------------------------------------------------------------
int svtkResliceCursorPicker::Pick(
  double selectionX, double selectionY, double selectionZ, svtkRenderer* renderer)
{
  ...
  svtkResliceCursor* rc = this->ResliceCursorAlgorithm->GetResliceCursor();
...
//
void svtkResliceCursorPicker::Pick(double displayPos[2], double world[3], svtkRenderer* ren)
{
...
// another.
//
static bool svtkResliceCursorPickerIsDifferentSanityCheck(const double a[3], const double b[3])
{
...
//
void svtkResliceCursorPicker::TransformPlane()
{
  svtkResliceCursor* rc = this->ResliceCursorAlgorithm->GetResliceCursor();
  const int axis3 = this->ResliceCursorAlgorithm->GetReslicePlaneNormal();
  ...
  if (!this->TransformMatrix)
  {
    ...
    // Sanity check
    if (svtkResliceCursorPickerIsDifferentSanityCheck(origin, center))
    {
      svtkErrorMacro("Reslice cursor center of ("
  ...
  // Sanity check
  if (svtkResliceCursorPickerIsDifferentSanityCheck(origin, originT))
  {
    svtkErrorMacro("Reslice cursor after transformation ("
  ...
  double normalT[3];
  svtkMath::Subtract(normalPointT, originT, normalT);
  svtkMath::Normalize(normalT);
...
//----------------------------------------------------------------------------
void svtkResliceCursorPicker::TransformPoint(double pIn[4], double pOut[4])
{
  this->TransformMatrix->MultiplyPoint(pIn, pOut);
}

//----------------------------------------------------------------------------
void svtkResliceCursorPicker::InverseTransformPoint(double pIn[4], double pOut[4])
{
  if (!this->TransformMatrix)
  {
    for (int i = 0; i < 3; i++)
...
//----------------------------------------------------------------------------
int svtkResliceCursorPicker::IntersectPolyDataWithLine(
  double p1[3], double p2[3], svtkPolyData* data, double tol)
{
...
//----------------------------------------------------------------------------
int svtkResliceCursorPicker::IntersectPointWithLine(
...
//----------------------------------------------------------------------------
void svtkResliceCursorPicker::PrintSelf(ostream& os, svtkIndent indent)
{
