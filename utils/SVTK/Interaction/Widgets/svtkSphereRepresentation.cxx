/*=========================================================================
  ...
  Module:    svtkSphereRepresentation.cxx
...
svtkStandardNewMacro(svtkSphereRepresentation);

//----------------------------------------------------------------------------
svtkSphereRepresentation::svtkSphereRepresentation()
{
  // The initial state
  this->InteractionState = svtkSphereRepresentation::Outside;
  ...
  this->HandleText = 1;
  this->HandleTextMapper = svtkTextMapper::New();
  this->HandleTextMapper->SetTextProperty(this->HandleTextProperty);
  this->HandleTextActor = svtkActor2D::New();
  ...
  this->RadialLine = 1;
  this->RadialLineSource = svtkLineSource::New();
  this->RadialLineSource->SetResolution(1);
  this->RadialLineMapper = svtkPolyDataMapper::New();
  this->RadialLineMapper->SetInputConnection(this->RadialLineSource->GetOutputPort());
  this->RadialLineActor = svtkActor::New();
  ...
  this->SpherePicker = svtkCellPicker::New();
...
//----------------------------------------------------------------------
void svtkSphereRepresentation::GetPolyData(svtkPolyData* pd)
{
...
//----------------------------------------------------------------------
void svtkSphereRepresentation::GetSphere(svtkSphere* sphere)
{
...
//----------------------------------------------------------------------
void svtkSphereRepresentation::HighlightSphere(int highlight)
{
  if (highlight)
  {
    this->ValidPick = 1;
...
//----------------------------------------------------------------------
void svtkSphereRepresentation::HighlightHandle(int highlight)
{
  if (highlight)
  {
    this->ValidPick = 1;
...
//------------------------------------------------------------------------------
void svtkSphereRepresentation::RegisterPickers()
{
...
//----------------------------------------------------------------------
void svtkSphereRepresentation::Scale(const double* p1, const double* p2, int svtkNotUsed(X), int Y)
{
  ...
  double sf = svtkMath::Norm(v) / radius;
...
//----------------------------------------------------------------------
void svtkSphereRepresentation::StartWidgetInteraction(double e[2])
{
...
//----------------------------------------------------------------------
void svtkSphereRepresentation::WidgetInteraction(double e[2])
{
  ...
  // Process the motion
  if (this->InteractionState == svtkSphereRepresentation::Translating)
  {
    this->Translate(prevPickPoint, pickPoint);
  }

  else if (this->InteractionState == svtkSphereRepresentation::Scaling)
  {
    this->Scale(prevPickPoint, pickPoint, static_cast<int>(e[0]), static_cast<int>(e[1]));
  }
  else if (this->InteractionState == svtkSphereRepresentation::MovingHandle)
  {
    svtkAssemblyPath* path = this->GetAssemblyPath(e[0], e[1], 0., this->SpherePicker);
...
void svtkSphereRepresentation::Translate(const double* p1, const double* p2)
{
  // Get the motion vector
  double v[3] = { 0, 0, 0 };
...
//----------------------------------------------------------------------------
void svtkSphereRepresentation::CreateDefaultProperties()
{
  if (!this->SphereProperty)
  {
  ...
  {
    this->HandleTextProperty = svtkTextProperty::New();
  ...
  {
    this->RadialLineProperty = svtkProperty::New();
    this->RadialLineProperty->SetColor(1, 0, 0);
  }
}

//----------------------------------------------------------------------------
void svtkSphereRepresentation::PlaceWidget(double center[3], double handle[3])
{
  double r = sqrt(svtkMath::Distance2BetweenPoints(center, handle));
...
//----------------------------------------------------------------------------
void svtkSphereRepresentation::SetCenter(double center[3])
{
  double c[3];
  this->SphereSource->GetCenter(c);
  if (c[0] != center[0] || c[1] != center[1] || c[2] != center[2])
  {
    ...
    if (this->GetHandleVisibility())
    {
      ...
      this->HandleDirection[1] = handle[1] - center[1];
      this->HandleDirection[2] = handle[2] - center[2];
      double r = sqrt(static_cast<double>(svtkMath::Distance2BetweenPoints(handle, center)));
      this->SphereSource->SetRadius(r);
    }
...
//----------------------------------------------------------------------------
void svtkSphereRepresentation::SetRadius(double r)
{
...
void svtkSphereRepresentation::SetHandlePosition(double handle[3])
{
  double h[3];
  ...
  if (h[0] != handle[0] || h[1] != handle[1] || h[2] != handle[2])
  {
    ...
    double r = static_cast<double>(svtkMath::Distance2BetweenPoints(handle, c));
...
void svtkSphereRepresentation::SetHandleDirection(double dir[3])
{
  double* d = this->HandleDirection;
  double dirMag = svtkMath::Norm(dir);
...
//----------------------------------------------------------------------------
void svtkSphereRepresentation::PlaceWidget(double bds[6])
{
...
//----------------------------------------------------------------------------
void svtkSphereRepresentation::PlaceHandle(const double* center, double radius)
{
...
//----------------------------------------------------------------------------
int svtkSphereRepresentation::ComputeInteractionState(int X, int Y, int svtkNotUsed(modify))
{
  ...
  if (this->HandleVisibility || this->HandleText || this->RadialLine)
  {
    ...
    {
      this->ValidPick = 1;
      this->InteractionState = svtkSphereRepresentation::MovingHandle;
  ...
  if (!handlePicked)
  {
    ...
    {
      this->ValidPick = 1;
      this->InteractionState = svtkSphereRepresentation::OnSphere;
...
//----------------------------------------------------------------------
void svtkSphereRepresentation::AdaptCenterCursorBounds()
{
...
//----------------------------------------------------------------------
void svtkSphereRepresentation::SetInteractionState(int state)
{
  // Clamp to allowable values
  state = (state < svtkSphereRepresentation::Outside
      ? svtkSphereRepresentation::Outside
      : (state > svtkSphereRepresentation::Scaling ? svtkSphereRepresentation::Scaling : state));
...
//----------------------------------------------------------------------
double* svtkSphereRepresentation::GetBounds()
{
  this->BuildRepresentation();
...
void svtkSphereRepresentation::BuildRepresentation()
{
  ...
  if (this->HandleText && this->Renderer)
  {
    ...
    double r = sqrt(static_cast<double>(svtkMath::Distance2BetweenPoints(c, hc)));
    r = (r <= 0.0 ? 1.0 : r);
    double theta = svtkMath::DegreesFromRadians(atan2((hc[1] - c[1]), (hc[0] - c[0])));
    double phi = svtkMath::DegreesFromRadians(acos((hc[2] - c[2]) / r));
    snprintf(str, sizeof(str), "(%0.2g, %1.1f, %1.1f)", r, theta, phi);
    this->HandleTextMapper->SetInput(str);
    svtkInteractorObserver::ComputeWorldToDisplay(this->Renderer, hc[0], hc[1], hc[2], tc);
...
//----------------------------------------------------------------------------
void svtkSphereRepresentation::ReleaseGraphicsResources(svtkWindow* w)
{
...
//----------------------------------------------------------------------------
int svtkSphereRepresentation::RenderOpaqueGeometry(svtkViewport* v)
{
  int count = 0;
  this->BuildRepresentation();
...
//----------------------------------------------------------------------------
int svtkSphereRepresentation::RenderTranslucentPolygonalGeometry(svtkViewport* v)
{
  int count = 0;
...
//----------------------------------------------------------------------------
int svtkSphereRepresentation::RenderOverlay(svtkViewport* v)
{
  int count = 0;
...
//----------------------------------------------------------------------------
svtkTypeBool svtkSphereRepresentation::HasTranslucentPolygonalGeometry()
{
  int result = 0;
  this->BuildRepresentation();
...
//----------------------------------------------------------------------------
void svtkSphereRepresentation::SizeHandles()
{
  double radius = this->svtkWidgetRepresentation::SizeHandlesInPixels(
    1.5, this->HandleSource->GetOutput()->GetCenter());
  this->HandleSource->SetRadius(radius);
}

//----------------------------------------------------------------------------
void svtkSphereRepresentation::PrintSelf(ostream& os, svtkIndent indent)
{
