/*=========================================================================
  ...
  Module:    svtkMultiProcessController.h
 ...
 *
 * svtkMultiProcessController is used to control multiple processes
 ...
 * svtkMultiProcessController must use the same mode for triggering RMI.
 *
 * @sa
 * svtkMPIController
 * svtkCommunicator svtkMPICommunicator
 */

#ifndef svtkMultiProcessController_h
#define svtkMultiProcessController_h

#include "svtkObject.h"
#include "svtkParallelCoreModule.h" // For export macro

#include "svtkCommunicator.h" // Needed for direct access to communicator
...
typedef void (*svtkProcessFunctionType)(svtkMultiProcessController* controller, void* userData);

// The type of function that gets called when an RMI is triggered.
typedef void (*svtkRMIFunctionType)(
  void* localArg, void* remoteArg, int remoteArgLength, int remoteProcessId);

class SVTKPARALLELCORE_EXPORT svtkMultiProcessController : public svtkObject
{
public:
  svtkTypeMacro(svtkMultiProcessController, svtkObject);
   ...
   */
  virtual void Initialize(int* svtkNotUsed(argc), char*** svtkNotUsed(argv)) = 0;

  /**
   * This method is for setting up the processes.
   * If a subclass needs to initialize process communication (i.e. MPI)
   * it would over ride this method.  Provided for initialization outside svtk.
   */
  virtual void Initialize(
    int* svtkNotUsed(argc), char*** svtkNotUsed(argv), int initializedExternally) = 0;
   ...
   * This method is for cleaning up.
   * If a subclass needs to clean up process communication (i.e. MPI)
   * it would over ride this method.  Provided for finalization outside svtk.
   */
   ...
   */
  void SetSingleMethod(svtkProcessFunctionType, void* data);

  /**
   * Object-oriented flavor of SetSingleMethod(). Instead of passing
   * some function pointer and user data, a svtkProcess object is passed
   * where the method to execute is Execute() and the data the object itself.
   */
  void SetSingleProcessObject(svtkProcess* p);
   ...
   */
  void SetMultipleMethod(int index, svtkProcessFunctionType, void* data);
   ...
   */
  static svtkMultiProcessController* GetGlobalController();
   ...
   */
  virtual svtkMultiProcessController* CreateSubController(svtkProcessGroup* group);
   ...
   */
  virtual svtkMultiProcessController* PartitionController(int localColor, int localKey);
   ...
   * The svtkRMIFunctionType has several arguments: localArg (same as passed in),
   ...
   */
  virtual unsigned long AddRMI(svtkRMIFunctionType, void* localArg, int tag);
   ...
   * Take an RMI away.
   */
  virtual void RemoveRMI(svtkRMIFunctionType f, void* arg, int tag)
  {
    (void)f;
    (void)arg;
    (void)tag;
    svtkErrorMacro("RemoveRMI Not Implemented Yet");
  }
   ...
   */
  virtual unsigned long AddRMICallback(svtkRMIFunctionType, void* localArg, int tag);
   ...
   * If reportErrors is false, no svtkErrorMacro is called.
   ...
   * This also causes svtkUpStreamPorts to return from
   * their WaitForUpdate loops.
   */
  svtkSetMacro(BreakFlag, int);
  svtkGetMacro(BreakFlag, int);
  //@}
   ...
   * satellites.
   */
  svtkSetMacro(BroadcastTriggerRMI, bool);
  svtkGetMacro(BroadcastTriggerRMI, bool);
  svtkBooleanMacro(BroadcastTriggerRMI, bool);
  //@}
  ...
  static void SetGlobalController(svtkMultiProcessController* controller);
   ...
   * svtkMultiProcessController has reserved tags between 1 and 4.
   * svtkCommunicator has reserved tags between 10 and 16.
   */
  int Send(const int* data, svtkIdType length, int remoteProcessId, int tag);
  int Send(const short* data, svtkIdType length, int remoteProcessId, int tag);
  int Send(const unsigned short* data, svtkIdType length, int remoteProcessId, int tag);
  int Send(const unsigned int* data, svtkIdType length, int remoteProcessId, int tag);
  int Send(const unsigned long* data, svtkIdType length, int remoteProcessId, int tag);
  int Send(const long* data, svtkIdType length, int remoteProcessId, int tag);
  int Send(const signed char* data, svtkIdType length, int remoteProcessId, int tag);
  int Send(const char* data, svtkIdType length, int remoteProcessId, int tag);
  int Send(const unsigned char* data, svtkIdType length, int remoteProcessId, int tag);
  int Send(const float* data, svtkIdType length, int remoteProcessId, int tag);
  int Send(const double* data, svtkIdType length, int remoteProcessId, int tag);
  int Send(const long long* data, svtkIdType length, int remoteProcessId, int tag);
  int Send(const unsigned long long* data, svtkIdType length, int remoteProcessId, int tag);
  int Send(svtkDataObject* data, int remoteId, int tag);
  int Send(svtkDataArray* data, int remoteId, int tag);
  //@}

  /**
   * Send a stream to another process. svtkMultiProcessStream makes it possible
   ...
   */
  int Receive(int* data, svtkIdType maxlength, int remoteProcessId, int tag);
  int Receive(unsigned int* data, svtkIdType maxlength, int remoteProcessId, int tag);
  int Receive(short* data, svtkIdType maxlength, int remoteProcessId, int tag);
  int Receive(unsigned short* data, svtkIdType maxlength, int remoteProcessId, int tag);
  int Receive(long* data, svtkIdType maxlength, int remoteProcessId, int tag);
  int Receive(unsigned long* data, svtkIdType maxlength, int remoteProcessId, int tag);
  int Receive(char* data, svtkIdType maxlength, int remoteProcessId, int tag);
  int Receive(unsigned char* data, svtkIdType maxlength, int remoteProcessId, int tag);
  int Receive(signed char* data, svtkIdType maxlength, int remoteProcessId, int tag);
  int Receive(float* data, svtkIdType maxlength, int remoteProcessId, int tag);
  int Receive(double* data, svtkIdType maxlength, int remoteProcessId, int tag);
  int Receive(long long* data, svtkIdType maxLength, int remoteProcessId, int tag);
  int Receive(unsigned long long* data, svtkIdType maxLength, int remoteProcessId, int tag);
  int Receive(svtkDataObject* data, int remoteId, int tag);
  int Receive(svtkDataArray* data, int remoteId, int tag);
  //@}
  ...
  svtkDataObject* ReceiveDataObject(int remoteId, int tag);
   ...
   */
  int Reduce(const int* sendBuffer, int* recvBuffer, svtkIdType length,
    svtkCommunicator::Operation* operation, int destProcessId)
  {
    return this->Communicator->Reduce(sendBuffer, recvBuffer, length, operation, destProcessId);
  }
  int Reduce(const unsigned int* sendBuffer, unsigned int* recvBuffer, svtkIdType length,
  ...
  int Reduce(const short* sendBuffer, short* recvBuffer, svtkIdType length,
    svtkCommunicator::Operation* operation, int destProcessId)
  {
    return this->Communicator->Reduce(sendBuffer, recvBuffer, length, operation, destProcessId);
  }
  ...
  int Reduce(const long* sendBuffer, long* recvBuffer, svtkIdType length,
    svtkCommunicator::Operation* operation, int destProcessId)
  {
    return this->Communicator->Reduce(sendBuffer, recvBuffer, length, operation, destProcessId);
  }
  ...
  int Reduce(const char* sendBuffer, char* recvBuffer, svtkIdType length,
    svtkCommunicator::Operation* operation, int destProcessId)
  {
    return this->Communicator->Reduce(sendBuffer, recvBuffer, length, operation, destProcessId);
  }
  ...
  int Reduce(const float* sendBuffer, float* recvBuffer, svtkIdType length,
    svtkCommunicator::Operation* operation, int destProcessId)
  {
    return this->Communicator->Reduce(sendBuffer, recvBuffer, length, operation, destProcessId);
  }
  int Reduce(const double* sendBuffer, double* recvBuffer, svtkIdType length,
    svtkCommunicator::Operation* operation, int destProcessId)
  {
    return this->Communicator->Reduce(sendBuffer, recvBuffer, length, operation, destProcessId);
  }
  ...
  int Reduce(svtkDataArray* sendBuffer, svtkDataArray* recvBuffer,
    svtkCommunicator::Operation* operation, int destProcessId)
  {
    return this->Communicator->Reduce(sendBuffer, recvBuffer, operation, destProcessId);
  }
  ...
  int AllReduce(
    const int* sendBuffer, int* recvBuffer, svtkIdType length, svtkCommunicator::Operation* operation)
  {
    return this->Communicator->AllReduce(sendBuffer, recvBuffer, length, operation);
  }
  int AllReduce(const unsigned int* sendBuffer, unsigned int* recvBuffer, svtkIdType length,
    svtkCommunicator::Operation* operation)
  {
    return this->Communicator->AllReduce(sendBuffer, recvBuffer, length, operation);
  }
  int AllReduce(const short* sendBuffer, short* recvBuffer, svtkIdType length,
    svtkCommunicator::Operation* operation)
  {
    return this->Communicator->AllReduce(sendBuffer, recvBuffer, length, operation);
  }
  ...
  int AllReduce(const long* sendBuffer, long* recvBuffer, svtkIdType length,
    svtkCommunicator::Operation* operation)
  {
    return this->Communicator->AllReduce(sendBuffer, recvBuffer, length, operation);
  }
  ...
  int AllReduce(const char* sendBuffer, char* recvBuffer, svtkIdType length,
    svtkCommunicator::Operation* operation)
  {
    return this->Communicator->AllReduce(sendBuffer, recvBuffer, length, operation);
  }
  int AllReduce(const signed char* sendBuffer, signed char* recvBuffer, svtkIdType length,
    svtkCommunicator::Operation* operation)
  {
    return this->Communicator->AllReduce(sendBuffer, recvBuffer, length, operation);
  }
  int AllReduce(const float* sendBuffer, float* recvBuffer, svtkIdType length,
    svtkCommunicator::Operation* operation)
  {
    return this->Communicator->AllReduce(sendBuffer, recvBuffer, length, operation);
  }
  int AllReduce(const double* sendBuffer, double* recvBuffer, svtkIdType length,
    svtkCommunicator::Operation* operation)
  {
    return this->Communicator->AllReduce(sendBuffer, recvBuffer, length, operation);
  }
  int AllReduce(const long long* sendBuffer, long long* recvBuffer, svtkIdType length,
    svtkCommunicator::Operation* operation)
  {
    return this->Communicator->AllReduce(sendBuffer, recvBuffer, length, operation);
  }
  int AllReduce(const unsigned long long* sendBuffer, unsigned long long* recvBuffer,
    svtkIdType length, svtkCommunicator::Operation* operation)
  {
    return this->Communicator->AllReduce(sendBuffer, recvBuffer, length, operation);
  }
  int AllReduce(
    svtkDataArray* sendBuffer, svtkDataArray* recvBuffer, svtkCommunicator::Operation* operation)
  {
    return this->Communicator->AllReduce(sendBuffer, recvBuffer, operation);
  }
   ...
   */
  int Reduce(const svtkBoundingBox& sendBuffer, svtkBoundingBox& recvBuffer, int destProcessId);
  int AllReduce(const svtkBoundingBox& sendBuffer, svtkBoundingBox& recvBuffer);
  //@}
...
protected:
  ...
  ~svtkMultiProcessController() override;
  ...
  svtkProcessFunctionType SingleMethod;
  void* SingleData;

  void GetMultipleMethod(int index, svtkProcessFunctionType& func, void*& data);
  ...
  virtual svtkMultiProcessController* GetLocalController();
  ...
  // depending on the type of controller, the subclasses are
  // responsible of deleting them.
  svtkCommunicator* RMICommunicator;

private:
  svtkMultiProcessController(const svtkMultiProcessController&) = delete;
  void operator=(const svtkMultiProcessController&) = delete;
...
inline int svtkMultiProcessController::Send(svtkDataObject* data, int remoteProcessId, int tag)
{
...
inline int svtkMultiProcessController::Send(svtkDataArray* data, int remoteProcessId, int tag)
{
  if (this->Communicator)
  {
    return this->Communicator->Send(data, remoteProcessId, tag);
...
inline int svtkMultiProcessController::Send(
  const int* data, svtkIdType length, int remoteProcessId, int tag)
{
  if (this->Communicator)
  {
...
inline int svtkMultiProcessController::Send(
  const short* data, svtkIdType length, int remoteProcessId, int tag)
{
  if (this->Communicator)
  {
    return this->Communicator->Send(data, length, remoteProcessId, tag);
...
inline int svtkMultiProcessController::Send(
  const unsigned short* data, svtkIdType length, int remoteProcessId, int tag)
{
  if (this->Communicator)
  {
    return this->Communicator->Send(data, length, remoteProcessId, tag);
...
inline int svtkMultiProcessController::Send(
  const unsigned int* data, svtkIdType length, int remoteProcessId, int tag)
{
  if (this->Communicator)
  {
    return this->Communicator->Send(data, length, remoteProcessId, tag);
...
inline int svtkMultiProcessController::Send(
  const unsigned long* data, svtkIdType length, int remoteProcessId, int tag)
{
  if (this->Communicator)
  {
    return this->Communicator->Send(data, length, remoteProcessId, tag);
...
inline int svtkMultiProcessController::Send(
  const long* data, svtkIdType length, int remoteProcessId, int tag)
{
  if (this->Communicator)
  {
    return this->Communicator->Send(data, length, remoteProcessId, tag);
...
inline int svtkMultiProcessController::Send(
  const signed char* data, svtkIdType length, int remoteProcessId, int tag)
{
  if (this->Communicator)
  {
    return this->Communicator->Send(data, length, remoteProcessId, tag);
...
inline int svtkMultiProcessController::Send(
  const char* data, svtkIdType length, int remoteProcessId, int tag)
{
  if (this->Communicator)
  {
    return this->Communicator->Send(data, length, remoteProcessId, tag);
...
inline int svtkMultiProcessController::Send(
  const unsigned char* data, svtkIdType length, int remoteProcessId, int tag)
{
  if (this->Communicator)
  {
    return this->Communicator->Send(data, length, remoteProcessId, tag);
...
inline int svtkMultiProcessController::Send(
  const float* data, svtkIdType length, int remoteProcessId, int tag)
{
  if (this->Communicator)
  {
    return this->Communicator->Send(data, length, remoteProcessId, tag);
...
inline int svtkMultiProcessController::Send(
  const double* data, svtkIdType length, int remoteProcessId, int tag)
{
  if (this->Communicator)
  {
    return this->Communicator->Send(data, length, remoteProcessId, tag);
...
inline int svtkMultiProcessController::Send(
  const long long* data, svtkIdType length, int remoteProcessId, int tag)
{
  if (this->Communicator)
  {
    return this->Communicator->Send(data, length, remoteProcessId, tag);
...
inline int svtkMultiProcessController::Send(
  const unsigned long long* data, svtkIdType length, int remoteProcessId, int tag)
{
  if (this->Communicator)
  {
    return this->Communicator->Send(data, length, remoteProcessId, tag);
...
inline int svtkMultiProcessController::Send(
  const svtkMultiProcessStream& stream, int remoteId, int tag)
{
  if (this->Communicator)
  {
...
inline int svtkMultiProcessController::Receive(svtkDataObject* data, int remoteProcessId, int tag)
{
  if (this->Communicator)
  {
...
inline svtkDataObject* svtkMultiProcessController::ReceiveDataObject(int remoteProcessId, int tag)
{
  if (this->Communicator)
  {
...
inline int svtkMultiProcessController::Receive(svtkDataArray* data, int remoteProcessId, int tag)
{
  if (this->Communicator)
  {
    return this->Communicator->Receive(data, remoteProcessId, tag);
...
inline int svtkMultiProcessController::Receive(
  int* data, svtkIdType length, int remoteProcessId, int tag)
{
  if (this->Communicator)
  {
...
inline int svtkMultiProcessController::Receive(
  unsigned int* data, svtkIdType length, int remoteProcessId, int tag)
{
  if (this->Communicator)
  {
    return this->Communicator->Receive(data, length, remoteProcessId, tag);
...
inline int svtkMultiProcessController::Receive(
  short* data, svtkIdType length, int remoteProcessId, int tag)
{
  if (this->Communicator)
  {
    return this->Communicator->Receive(data, length, remoteProcessId, tag);
...
inline int svtkMultiProcessController::Receive(
  unsigned short* data, svtkIdType length, int remoteProcessId, int tag)
{
  if (this->Communicator)
  {
    return this->Communicator->Receive(data, length, remoteProcessId, tag);
...
inline int svtkMultiProcessController::Receive(
  long* data, svtkIdType length, int remoteProcessId, int tag)
{
  if (this->Communicator)
  {
    return this->Communicator->Receive(data, length, remoteProcessId, tag);
...
inline int svtkMultiProcessController::Receive(
  unsigned long* data, svtkIdType length, int remoteProcessId, int tag)
{
  if (this->Communicator)
  {
    return this->Communicator->Receive(data, length, remoteProcessId, tag);
...
inline int svtkMultiProcessController::Receive(
  char* data, svtkIdType length, int remoteProcessId, int tag)
{
  if (this->Communicator)
  {
    return this->Communicator->Receive(data, length, remoteProcessId, tag);
...
inline int svtkMultiProcessController::Receive(
  unsigned char* data, svtkIdType length, int remoteProcessId, int tag)
{
  if (this->Communicator)
  {
    return this->Communicator->Receive(data, length, remoteProcessId, tag);
...
inline int svtkMultiProcessController::Receive(
  signed char* data, svtkIdType length, int remoteProcessId, int tag)
{
  if (this->Communicator)
  {
    return this->Communicator->Receive(data, length, remoteProcessId, tag);
...
inline int svtkMultiProcessController::Receive(
  float* data, svtkIdType length, int remoteProcessId, int tag)
{
  if (this->Communicator)
  {
    return this->Communicator->Receive(data, length, remoteProcessId, tag);
...
inline int svtkMultiProcessController::Receive(
  double* data, svtkIdType length, int remoteProcessId, int tag)
{
  if (this->Communicator)
  {
    return this->Communicator->Receive(data, length, remoteProcessId, tag);
...
inline int svtkMultiProcessController::Receive(
  long long* data, svtkIdType length, int remoteProcessId, int tag)
{
  if (this->Communicator)
  {
    return this->Communicator->Receive(data, length, remoteProcessId, tag);
...
inline int svtkMultiProcessController::Receive(
  unsigned long long* data, svtkIdType length, int remoteProcessId, int tag)
{
  if (this->Communicator)
  {
    return this->Communicator->Receive(data, length, remoteProcessId, tag);
...
inline int svtkMultiProcessController::Receive(svtkMultiProcessStream& stream, int remoteId, int tag)
{
  if (this->Communicator)
  {
...
inline void svtkMultiProcessController::Barrier()
{
  if (this->Communicator)
  {
...
inline svtkIdType svtkMultiProcessController::GetCount()
{
  if (this->Communicator)
  {
