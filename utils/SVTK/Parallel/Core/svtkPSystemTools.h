 ...
 * broadcasting the results to other processes. It is built on SVTK's
 * SystemTools class and uses the global controller for communication.
 * It uses blocking collective communication operations.
 */

#ifndef svtkPSystemTools_h
#define svtkPSystemTools_h

#include "svtkObject.h"
#include "svtkParallelCoreModule.h" // For export macro
#include <string>                  // for string functions in SystemTools

class SVTKPARALLELCORE_EXPORT svtkPSystemTools : public svtkObject
{
public:
  static svtkPSystemTools* New();
  svtkTypeMacro(svtkPSystemTools, svtkObject);
...
protected:
  svtkPSystemTools() {}
  ~svtkPSystemTools() override {}

private:
  svtkPSystemTools(const svtkPSystemTools&) = delete;
  void operator=(const svtkPSystemTools&) = delete;
};
