...
svtkStandardNewMacro(svtkSubGroup);

svtkSubGroup::svtkSubGroup()
{
...
int svtkSubGroup::Initialize(
  int p0 = 0, int p1 = 0, int me = 0, int itag = 0, svtkCommunicator* c = nullptr)
{
  int i, ii;
...
int svtkSubGroup::computeFanInTargets()
{
  int i;
  this->nTo = 0;
  this->nFrom = 0;
...
void svtkSubGroup::setUpRoot(int root)
{
  if (root == 0)
  {
    return;
...
void svtkSubGroup::setGatherPattern(int root, int length)
{
  int i;
...
int svtkSubGroup::getLocalRank(int processId)
{
...
#define REDUCE(Type, name, op)                                                                     \
  int svtkSubGroup::Reduce##name(Type* data, Type* to, int size, int root)                          \
  {                                                                                                \
    int i, p;                                                                                      \
    if (this->nmembers == 1)                                                                       \
    {                                                                                              \
...
#define BROADCAST(Type)                                                                            \
  int svtkSubGroup::Broadcast(Type* data, int length, int root)                                     \
  {                                                                                                \
    int i;                                                                                         \
    if (this->nmembers == 1)                                                                       \
    {                                                                                              \
...
BROADCAST(svtkIdType)
#endif

#define GATHER(Type)                                                                               \
  int svtkSubGroup::Gather(Type* data, Type* to, int length, int root)                              \
  {                                                                                                \
    int i;                                                                                         \
    Type* recvBuf;                                                                                 \
    if (this->nmembers == 1)                                                                       \
...
int svtkSubGroup::AllReduceUniqueList(int* list, int len, int** newList)
{
  int transferLen, myListLen, lastListLen, nextListLen;

  int* myList = nullptr;
  myListLen = svtkSubGroup::MakeSortedUnique(list, len, &myList);
  ...
  for (int i = 0; i < this->nFrom; i++)
  {
    ...
    nextListLen =
      svtkSubGroup::MergeSortedUnique(lastList, lastListLen, buf, transferLen, &nextList);
...
int svtkSubGroup::MergeSortedUnique(int* list1, int len1, int* list2, int len2, int** newList)
{
  int newLen = 0;
  int i1 = 0;
  int i2 = 0;
...
int svtkSubGroup::MakeSortedUnique(int* list, int len, int** newList)
{
  int i, newlen;
  int* newl;
...
void svtkSubGroup::PrintSubGroup() const
{
  int i;
...
void svtkSubGroup::PrintSelf(ostream& os, svtkIndent indent)
{
  int i;
