/*=========================================================================
  ...
  Module:    svtkThreadedTaskQueue.h
...
=========================================================================*/
/**
 * @class svtkThreadedTaskQueue
 * @brief simple threaded task queue
 *
 * svtkThreadedTaskQueue provides a simple task queue that can use threads to
 ...
 *
 * svtkThreadedTaskQueue's API is intended to called from the same main thread.
 ...
 * `svtkMultiThreader::GetGlobalDefaultNumberOfThreads()`.
 *
...
#ifndef svtkThreadedTaskQueue_h
#define svtkThreadedTaskQueue_h
...
{
public:
  svtkThreadedTaskQueue(std::function<R(Args...)> worker, bool strict_ordering = true,
    int buffer_size = -1, int max_concurrent_tasks = -1);
  ~svtkThreadedTaskQueue();
...
private:
  svtkThreadedTaskQueue(const svtkThreadedTaskQueue&) = delete;
  void operator=(const svtkThreadedTaskQueue&) = delete;

  std::function<R(Args...)> Worker;

  std::unique_ptr<svtkThreadedTaskQueueInternals::TaskQueue<R> > Tasks;
  std::unique_ptr<svtkThreadedTaskQueueInternals::ResultQueue<R> > Results;
...
class svtkThreadedTaskQueue<void, Args...>
{
public:
  svtkThreadedTaskQueue(std::function<void(Args...)> worker, bool strict_ordering = true,
    int buffer_size = -1, int max_concurrent_tasks = -1);
  ~svtkThreadedTaskQueue();
...
private:
  ...
  std::unique_ptr<svtkThreadedTaskQueueInternals::TaskQueue<void> > Tasks;
...
#include "svtkThreadedTaskQueue.txx"

#endif // !defined(__WRAP__)

#endif
// SVTK-HeaderTest-Exclude: svtkThreadedTaskQueue.h
