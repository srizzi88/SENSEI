/*=========================================================================
  ...
  Module:    svtkThreadedTaskQueue.txx
...
//=============================================================================
namespace svtkThreadedTaskQueueInternals
{
...
public:
  ...
  void Push(std::function<R()>&& task)
  {
    ...
    else
    {
      ...
      // svtkLogF(INFO, "pushing-task %d", (int)this->NextTaskId);
  ...
  bool Pop(std::uint64_t& task_id, std::function<R()>& task)
  {
    ...
    if (this->Tasks.size() > 0)
    {
      ...
      // svtkLogF(TRACE, "popping-task %d", (int)task_pair.first);
      this->Tasks.pop();
      lk.unlock();
...
svtkThreadedTaskQueue<R, Args...>::svtkThreadedTaskQueue(
  std::function<R(Args...)> worker, bool strict_ordering, int buffer_size, int max_concurrent_tasks)
  : Worker(worker)
  , Tasks(new svtkThreadedTaskQueueInternals::TaskQueue<R>(
      std::max(0, strict_ordering ? 0 : buffer_size)))
  , Results(new svtkThreadedTaskQueueInternals::ResultQueue<R>(strict_ordering))
  , NumberOfThreads(max_concurrent_tasks <= 0 ? svtkMultiThreader::GetGlobalDefaultNumberOfThreads()
                                              : max_concurrent_tasks)
  , Threads{ new std::thread[this->NumberOfThreads] }
{
  auto f = [this](int thread_id) {
    svtkLogger::SetThreadName("ttq::worker" + std::to_string(thread_id));
    while (true)
    {
      std::function<R()> task;
...
//-----------------------------------------------------------------------------
template <typename R, typename... Args>
void svtkThreadedTaskQueue<R, Args...>::Push(Args&&... args)
{
...
//-----------------------------------------------------------------------------
template <typename R, typename... Args>
bool svtkThreadedTaskQueue<R, Args...>::TryPop(R& result)
{
...
//-----------------------------------------------------------------------------
template <typename R, typename... Args>
bool svtkThreadedTaskQueue<R, Args...>::Pop(R& result)
{
  if (this->IsEmpty())
  {
    return false;
...
//-----------------------------------------------------------------------------
template <typename R, typename... Args>
bool svtkThreadedTaskQueue<R, Args...>::IsEmpty() const
{
...
//-----------------------------------------------------------------------------
template <typename R, typename... Args>
void svtkThreadedTaskQueue<R, Args...>::Flush()
{
  R tmp;
  while (!this->IsEmpty())
  {
...
svtkThreadedTaskQueue<void, Args...>::svtkThreadedTaskQueue(std::function<void(Args...)> worker,
  bool strict_ordering, int buffer_size, int max_concurrent_tasks)
  : Worker(worker)
  , Tasks(new svtkThreadedTaskQueueInternals::TaskQueue<void>(
      std::max(0, strict_ordering ? 0 : buffer_size)))
  , NextResultId(0)
  , NumberOfThreads(max_concurrent_tasks <= 0 ? svtkMultiThreader::GetGlobalDefaultNumberOfThreads()
                                              : max_concurrent_tasks)
...
//-----------------------------------------------------------------------------
template <typename... Args>
svtkThreadedTaskQueue<void, Args...>::~svtkThreadedTaskQueue()
{
  this->Tasks->MarkDone();
  for (int cc = 0; cc < this->NumberOfThreads; ++cc)
  {
...
//-----------------------------------------------------------------------------
template <typename... Args>
void svtkThreadedTaskQueue<void, Args...>::Push(Args&&... args)
{
  this->Tasks->Push(std::bind(this->Worker, args...));
}
...
//-----------------------------------------------------------------------------
template <typename... Args>
bool svtkThreadedTaskQueue<void, Args...>::IsEmpty() const
{
...
//-----------------------------------------------------------------------------
template <typename... Args>
void svtkThreadedTaskQueue<void, Args...>::Flush()
{
  if (this->IsEmpty())
  {
    return;
