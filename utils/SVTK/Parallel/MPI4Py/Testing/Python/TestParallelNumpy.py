...
parallel algorithms in svtk.numpy_interface.algorithms."""
...
import svtk.numpy_interface.algorithms as algs
from mpi4py import MPI

c = svtk.svtkMPIController()
...
ext = list(org_ext)
wext = w.GetOutputInformation(0).Get(svtk.svtkStreamingDemandDrivenPipeline.WHOLE_EXTENT())
for i in range(3):
    if ext[2*i] != wext[2*i]:
        ext[2*i] = ext[2*i] + 2
...
assert algs.sum(rtData / rtData, controller=svtk.svtkDummyController()) != total_npts
...
rtData3 = dsa.SVTKCompositeDataArray([rtData, dsa.NoneArray])
grad3 = dsa.SVTKCompositeDataArray([dsa.NoneArray, grad])
...
if size > 1:
    if rank == 1:
        rtData3 = dsa.SVTKCompositeDataArray([rtData2])
        grad3 = dsa.SVTKCompositeDataArray([grad2])
    else:
        rtData3 = dsa.NoneArray
        grad3 = dsa.NoneArray
...
rtData3 = dsa.SVTKCompositeDataArray([datasets[0].PointData['RTData'], datasets[1].PointData['RTData']])
grad3 = dsa.SVTKCompositeDataArray([datasets[0].PointData['gradient'], datasets[1].PointData['gradient']])
...
for i in range(start, end):
    a = svtk.svtkImageData()
    a.ShallowCopy(w.GetOutput())
    c.SetBlock(i, a)

if rank == 0:
    c.SetBlock(NUM_BLOCKS - 1, svtk.svtkPolyData())
...
res = True
dummy = svtk.svtkDummyController()
for axis in [None, 0]:
