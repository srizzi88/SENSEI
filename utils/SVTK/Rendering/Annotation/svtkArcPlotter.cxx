...
svtkStandardNewMacro(svtkArcPlotter);

svtkCxxSetObjectMacro(svtkArcPlotter, Camera, svtkCamera);

svtkArcPlotter::svtkArcPlotter()
{
  this->Camera = nullptr;
  this->PlotMode = SVTK_PLOT_SCALARS;
...
int svtkArcPlotter::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  // Initialize
  //
  svtkDebugMacro(<< "Plotting along arc");
  ...
  if (this->Camera && !this->UseDefaultNormal)
  {
    ...
    {
      inPts->GetPoint(i, x);
      svtkPlane::ProjectPoint(x, point, normal, xProj);
  ...
  lineNormals = svtkFloatArray::New();
...
int svtkArcPlotter::ProcessComponents(svtkIdType numPts, svtkPointData* pd)
{
  svtkIdType i;
  int j;
  double* range;
  ...
  {
    case SVTK_PLOT_SCALARS:
      if (pd->GetScalars())
      {
        this->Data = pd->GetScalars();
      }
      break;
    case SVTK_PLOT_VECTORS:
      if (pd->GetVectors())
      {
        this->Data = pd->GetVectors();
      }
      break;
    case SVTK_PLOT_NORMALS:
      if (pd->GetNormals())
      {
        this->Data = pd->GetNormals();
      }
      break;
    case SVTK_PLOT_TCOORDS:
      if (pd->GetTCoords())
      {
        this->Data = pd->GetTCoords();
      }
      break;
    case SVTK_PLOT_TENSORS:
      if (pd->GetTensors())
      {
        this->Data = pd->GetTensors();
      }
      break;
    case SVTK_PLOT_FIELD_DATA:
  ...
  {
    svtkErrorMacro(<< "Need input data to plot");
    return 0;
  }
...
svtkIdType svtkArcPlotter::OffsetPoint(svtkIdType ptId, svtkPoints* inPts, double n[3],
  svtkPoints* newPts, double offset, double* range, double v)
{
  double x[3], xNew[3];
  int i;
...
svtkMTimeType svtkArcPlotter::GetMTime()
{
  svtkMTimeType mTime = this->Superclass::GetMTime();
  svtkMTimeType cameraMTime;
...
void svtkArcPlotter::PrintSelf(ostream& os, svtkIndent indent)
{
  ...
  os << indent << "Plot Mode: ";
  if (this->PlotMode == SVTK_PLOT_SCALARS)
  {
    os << "Plot Scalars\n";
  }
  else if (this->PlotMode == SVTK_PLOT_VECTORS)
  {
    os << "Plot Vectors\n";
  }
  else if (this->PlotMode == SVTK_PLOT_NORMALS)
  {
    os << "Plot Normals\n";
  }
  else if (this->PlotMode == SVTK_PLOT_TCOORDS)
  {
    os << "Plot TCoords\n";
  }
  else if (this->PlotMode == SVTK_PLOT_TENSORS)
  {
    os << "Plot Tensors\n";
  }
  else
