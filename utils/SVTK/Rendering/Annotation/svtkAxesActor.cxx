...
svtkStandardNewMacro(svtkAxesActor);

svtkCxxSetObjectMacro(svtkAxesActor, UserDefinedTip, svtkPolyData);
svtkCxxSetObjectMacro(svtkAxesActor, UserDefinedShaft, svtkPolyData);

//----------------------------------------------------------------------------
svtkAxesActor::svtkAxesActor()
{
  this->AxisLabels = 1;
  ...
  svtkPolyDataMapper* shaftMapper = svtkPolyDataMapper::New();
  ...
  svtkPolyDataMapper* tipMapper = svtkPolyDataMapper::New();
  ...
  this->ShaftType = svtkAxesActor::LINE_SHAFT;
  this->TipType = svtkAxesActor::CONE_TIP;

  this->UserDefinedTip = nullptr;
  this->UserDefinedShaft = nullptr;

  this->XAxisLabel = svtkCaptionActor2D::New();
  this->YAxisLabel = svtkCaptionActor2D::New();
  this->ZAxisLabel = svtkCaptionActor2D::New();
...
void svtkAxesActor::ShallowCopy(svtkProp* prop)
{
  svtkAxesActor* a = svtkAxesActor::SafeDownCast(prop);
  if (a != nullptr)
  {
...
//----------------------------------------------------------------------------
void svtkAxesActor::GetActors(svtkPropCollection* ac)
{
...
//----------------------------------------------------------------------------
int svtkAxesActor::RenderOpaqueGeometry(svtkViewport* vp)
{
...
//-----------------------------------------------------------------------------
int svtkAxesActor::RenderTranslucentPolygonalGeometry(svtkViewport* vp)
{
  int renderedSomething = 0;
...
svtkTypeBool svtkAxesActor::HasTranslucentPolygonalGeometry()
{
  int result = 0;
...
//-----------------------------------------------------------------------------
int svtkAxesActor::RenderOverlay(svtkViewport* vp)
{
  int renderedSomething = 0;
...
//----------------------------------------------------------------------------
void svtkAxesActor::ReleaseGraphicsResources(svtkWindow* win)
{
...
//----------------------------------------------------------------------------
void svtkAxesActor::GetBounds(double bounds[6])
{
...
double* svtkAxesActor::GetBounds()
{
  double bounds[6];
  int i;
...
//----------------------------------------------------------------------------
svtkMTimeType svtkAxesActor::GetMTime()
{
...
//----------------------------------------------------------------------------
svtkMTimeType svtkAxesActor::GetRedrawMTime()
{
...
//----------------------------------------------------------------------------
void svtkAxesActor::SetTotalLength(double x, double y, double z)
{
  if (this->TotalLength[0] != x || this->TotalLength[1] != y || this->TotalLength[2] != z)
  {
    ...
    {
      svtkGenericWarningMacro("One or more axes lengths are < 0 \
...
//----------------------------------------------------------------------------
void svtkAxesActor::SetNormalizedShaftLength(double x, double y, double z)
{
    ...
    this->NormalizedShaftLength[2] != z)
  {
    ...
    {
      svtkGenericWarningMacro("One or more normalized shaft lengths \
...
//----------------------------------------------------------------------------
void svtkAxesActor::SetNormalizedTipLength(double x, double y, double z)
{
    ...
    this->NormalizedTipLength[2] != z)
  {
    ...
    if (x < 0.0 || x > 1.0 || y < 0.0 || y > 1.0 || z < 0.0 || z > 1.0)
    {
      svtkGenericWarningMacro("One or more normalized tip lengths \
      are < 0 or > 1 and may produce unexpected results.");
    }
...
//----------------------------------------------------------------------------
void svtkAxesActor::SetNormalizedLabelPosition(double x, double y, double z)
{
    ...
    this->NormalizedLabelPosition[2] != z)
  {
    ...
    if (x < 0.0 || y < 0.0 || z < 0.0)
    {
      svtkGenericWarningMacro("One or more label positions are < 0 \
                        and may produce unexpected results.");
    }
...
//----------------------------------------------------------------------------
void svtkAxesActor::SetShaftType(int type)
{
  if (this->ShaftType != type)
  {
    if (type < svtkAxesActor::CYLINDER_SHAFT || type > svtkAxesActor::USER_DEFINED_SHAFT)
    {
      svtkErrorMacro("Undefined axes shaft type.");
      return;
    }

    if (type == svtkAxesActor::USER_DEFINED_SHAFT && this->UserDefinedShaft == nullptr)
    {
      svtkErrorMacro("Set the user defined shaft before changing the type.");
      return;
    }
...
//----------------------------------------------------------------------------
void svtkAxesActor::SetTipType(int type)
{
  if (this->TipType != type)
  {
    if (type < svtkAxesActor::CONE_TIP || type > svtkAxesActor::USER_DEFINED_TIP)
    {
      svtkErrorMacro("Undefined axes tip type.");
      return;
    }

    if (type == svtkAxesActor::USER_DEFINED_TIP && this->UserDefinedTip == nullptr)
    {
      svtkErrorMacro("Set the user defined tip before changing the type.");
      return;
    }
...
//----------------------------------------------------------------------------
void svtkAxesActor::UpdateProps()
{
  ...
  {
    case svtkAxesActor::CYLINDER_SHAFT:
      (svtkPolyDataMapper::SafeDownCast(this->XAxisShaft->GetMapper()))
        ->SetInputConnection(this->CylinderSource->GetOutputPort());
      break;
    case svtkAxesActor::LINE_SHAFT:
      (svtkPolyDataMapper::SafeDownCast(this->XAxisShaft->GetMapper()))
        ->SetInputConnection(this->LineSource->GetOutputPort());
      break;
    case svtkAxesActor::USER_DEFINED_SHAFT:
      (svtkPolyDataMapper::SafeDownCast(this->XAxisShaft->GetMapper()))
  ...
  switch (this->TipType)
  {
    case svtkAxesActor::CONE_TIP:
      (svtkPolyDataMapper::SafeDownCast(this->XAxisTip->GetMapper()))
        ->SetInputConnection(this->ConeSource->GetOutputPort());
      break;
    case svtkAxesActor::SPHERE_TIP:
      (svtkPolyDataMapper::SafeDownCast(this->XAxisTip->GetMapper()))
        ->SetInputConnection(this->SphereSource->GetOutputPort());
      break;
    case svtkAxesActor::USER_DEFINED_TIP:
      (svtkPolyDataMapper::SafeDownCast(this->XAxisTip->GetMapper()))
        ->SetInputData(this->UserDefinedTip);
  }

  svtkPolyDataMapper::SafeDownCast(this->XAxisTip->GetMapper())->GetInputAlgorithm()->Update();
  svtkPolyDataMapper::SafeDownCast(this->XAxisShaft->GetMapper())->GetInputAlgorithm()->Update();
  ...
  (svtkPolyDataMapper::SafeDownCast(this->XAxisShaft->GetMapper()))->GetInput()->GetBounds(bounds);
  ...
  svtkTransform* xTransform = svtkTransform::New();
  svtkTransform* yTransform = svtkTransform::New();
  svtkTransform* zTransform = svtkTransform::New();
  ...
  (svtkPolyDataMapper::SafeDownCast(this->XAxisTip->GetMapper()))->GetInput()->GetBounds(bounds);
  ...
  this->YAxisLabel->SetAttachmentPoint(
    (bounds[0] + bounds[1]) / 2, bounds[2] + offset, bounds[5] + (bounds[5] - bounds[4]) / 2.0);

  this->ZAxisShaft->GetBounds(bounds);
  offset = this->NormalizedLabelPosition[2] * (bounds[5] - bounds[4]);
  this->ZAxisLabel->SetAttachmentPoint(
    bounds[0], bounds[2] - (bounds[3] - bounds[2]) * 2.0, bounds[4] + offset);

  svtkLinearTransform* transform = this->GetUserTransform();
  if (transform)
  {
...
//----------------------------------------------------------------------------
svtkProperty* svtkAxesActor::GetXAxisTipProperty()
{
  return this->XAxisTip->GetProperty();
}

//----------------------------------------------------------------------------
svtkProperty* svtkAxesActor::GetYAxisTipProperty()
{
  return this->YAxisTip->GetProperty();
}

//----------------------------------------------------------------------------
svtkProperty* svtkAxesActor::GetZAxisTipProperty()
{
  return this->ZAxisTip->GetProperty();
}

//----------------------------------------------------------------------------
svtkProperty* svtkAxesActor::GetXAxisShaftProperty()
{
  return this->XAxisShaft->GetProperty();
}

//----------------------------------------------------------------------------
svtkProperty* svtkAxesActor::GetYAxisShaftProperty()
{
  return this->YAxisShaft->GetProperty();
}

//----------------------------------------------------------------------------
svtkProperty* svtkAxesActor::GetZAxisShaftProperty()
{
  return this->ZAxisShaft->GetProperty();
}

//----------------------------------------------------------------------------
void svtkAxesActor::PrintSelf(ostream& os, svtkIndent indent)
{
