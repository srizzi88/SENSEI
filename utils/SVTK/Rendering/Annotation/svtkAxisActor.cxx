...
#define SVTK_MAX_TICKS 1000

#include <sstream>

svtkStandardNewMacro(svtkAxisActor);
svtkCxxSetObjectMacro(svtkAxisActor, Camera, svtkCamera);
svtkCxxSetObjectMacro(svtkAxisActor, LabelTextProperty, svtkTextProperty);
svtkCxxSetObjectMacro(svtkAxisActor, TitleTextProperty, svtkTextProperty);

//-----------------------------------------------------------------------------
// Instantiate this object.
//-----------------------------------------------------------------------------

svtkAxisActor::svtkAxisActor()
{
  ...
  this->TitleAlignLocation = SVTK_ALIGN_BOTTOM;
  this->ExponentLocation = SVTK_ALIGN_POINT2;
  ...
  this->ExponentVector = svtkVectorText::New();
  this->ExponentMapper = svtkPolyDataMapper::New();
  this->ExponentMapper->SetInputConnection(this->ExponentVector->GetOutputPort());
  this->ExponentActor = svtkAxisFollower::New();
  this->ExponentActor->SetAxis(this);
  this->ExponentActor->SetMapper(this->ExponentMapper);
  this->ExponentActor->SetEnableDistanceLOD(0);
  this->ExponentActor2D = svtkTextActor::New();

  this->ExponentProp3D = svtkProp3DAxisFollower::New();
  this->ExponentProp3D->SetAxis(this);
  this->ExponentProp3D->SetEnableDistanceLOD(0);
  this->ExponentActor3D = svtkTextActor3D::New();
  this->ExponentProp3D->SetProp3D(this->ExponentActor3D);

  // Main line of axis
  this->AxisLines = svtkPolyData::New();
  this->AxisLinesMapper = svtkPolyDataMapper::New();
  this->AxisLinesMapper->SetInputData(this->AxisLines);
  this->AxisLinesActor = svtkActor::New();
  this->AxisLinesActor->SetMapper(this->AxisLinesMapper);

  // Major ticks
  this->AxisMajorTicks = svtkPolyData::New();
  this->AxisMajorTicksMapper = svtkPolyDataMapper::New();
  this->AxisMajorTicksMapper->SetInputData(this->AxisMajorTicks);
  this->AxisMajorTicksActor = svtkActor::New();
  this->AxisMajorTicksActor->SetMapper(this->AxisMajorTicksMapper);

  // Minor ticks
  this->AxisMinorTicks = svtkPolyData::New();
  this->AxisMinorTicksMapper = svtkPolyDataMapper::New();
  this->AxisMinorTicksMapper->SetInputData(this->AxisMinorTicks);
  this->AxisMinorTicksActor = svtkActor::New();
  this->AxisMinorTicksActor->SetMapper(this->AxisMinorTicksMapper);

  this->Gridlines = svtkPolyData::New();
  this->GridlinesMapper = svtkPolyDataMapper::New();
  this->GridlinesMapper->SetInputData(this->Gridlines);
  this->GridlinesActor = svtkActor::New();
  this->GridlinesActor->SetMapper(this->GridlinesMapper);
  this->InnerGridlines = svtkPolyData::New();
  this->InnerGridlinesMapper = svtkPolyDataMapper::New();
  this->InnerGridlinesMapper->SetInputData(this->InnerGridlines);
  this->InnerGridlinesActor = svtkActor::New();
  this->InnerGridlinesActor->SetMapper(this->InnerGridlinesMapper);
  this->Gridpolys = svtkPolyData::New();
  this->GridpolysMapper = svtkPolyDataMapper::New();
  this->GridpolysMapper->SetInputData(this->Gridpolys);
  this->GridpolysActor = svtkActor::New();
  ...
  this->AxisType = SVTK_AXIS_TYPE_X;
  this->Log = false;
  //
  ...
  //
  this->AxisPosition = SVTK_AXIS_POS_MINMIN;
  ...
  this->MinorTickPts = svtkPoints::New();
  this->MajorTickPts = svtkPoints::New();
  this->GridlinePts = svtkPoints::New();
  this->InnerGridlinePts = svtkPoints::New();
  this->GridpolyPts = svtkPoints::New();
...
//-----------------------------------------------------------------------------
void svtkAxisActor::ReleaseGraphicsResources(svtkWindow* win)
{
...
//-----------------------------------------------------------------------------
int svtkAxisActor::RenderOpaqueGeometry(svtkViewport* viewport)
{
...
//-----------------------------------------------------------------------------
int svtkAxisActor::RenderTranslucentGeometry(svtkViewport* viewport)
{
...
// Build the translucent poly actors and render.
//-----------------------------------------------------------------------------
int svtkAxisActor::RenderTranslucentPolygonalGeometry(svtkViewport* viewport)
{
...
//-----------------------------------------------------------------------------
int svtkAxisActor::RenderOverlay(svtkViewport* viewport)
{
  int renderedSomething = 0;
...
//-----------------------------------------------------------------------------*
svtkTypeBool svtkAxisActor::HasTranslucentPolygonalGeometry()
{
...
//-----------------------------------------------------------------------------*
void svtkAxisActor::BuildAxis(svtkViewport* viewport, bool force)
{
  ...
  {
    svtkDebugMacro(<< "Axis has zero length, not building.");
  ...
  if (this->Log)
  {
    if (this->Range[0] <= 0.0)
    {
      svtkWarningMacro(<< "Range value undefined for log scale enabled. "
  ...
  svtkDebugMacro(<< "Rebuilding axis");
...
//-----------------------------------------------------------------------------
void svtkAxisActor::BuildLabels(svtkViewport* viewport, bool force)
{
...
static const int svtkAxisActorMultiplierTable1[4] = { -1, -1, 1, 1 };
static const int svtkAxisActorMultiplierTable2[4] = { -1, 1, 1, -1 };

//-----------------------------------------------------------------------------
// Determine and set scale factor and position for labels.
//-----------------------------------------------------------------------------
void svtkAxisActor::SetLabelPositions(svtkViewport* viewport, bool force)
{
  ...
  double labelAngle = svtkMath::RadiansFromDegrees(this->LabelTextProperty->GetOrientation());
  double labelCos = fabs(cos(labelAngle));
  double labelSin = fabs(sin(labelAngle));
  svtkAxisFollower* pAxisFollower = nullptr;
       ...
       i++, ptIdx += 4)
  {
    ...
    // labels
    // svtkProp3D::GetBounds() include previous transform (scale, orientation, translation)
    ...
    {
      svtkWarningMacro("CalculateLabelOffset flag is now deprecated and has no effect");
    }
...
//-----------------------------------------------------------------------------
void svtkAxisActor::BuildLabels2D(svtkViewport* viewport, bool force)
{
  if (!force && (!this->LabelVisibility || this->NumberOfLabelsBuilt == 0))
  {
    return;
...
//-----------------------------------------------------------------------------
void svtkAxisActor::SetLabelPositions2D(svtkViewport* viewport, bool force)
{
  if (!force && (!this->LabelVisibility || this->NumberOfLabelsBuilt == 0))
  {
    return;
  ...
  {
    case SVTK_AXIS_TYPE_X:
      xmult = 0;
      ymult = svtkAxisActorMultiplierTable1[this->AxisPosition];
      xcoeff = 0.5;
      ycoeff = 1.0;
      break;
    case SVTK_AXIS_TYPE_Y:
      xmult = svtkAxisActorMultiplierTable1[this->AxisPosition];
      ymult = 0;
      xcoeff = 1.0;
      ycoeff = 0.5;
      break;
  ...
  if (!tren)
  {
    svtkErrorMacro(<< "Unable to obtain the svtkTextRenderer instance!");
    return;
  }
       ...
       i++, ptIdx += 4)
  {
    ...
    {
      svtkErrorMacro(<< "Unable to calculate bounding box for label "
...
void svtkAxisActor::InitTitle()
{
  ...
  // Text 3D
  // svtkTextActor3D::TitleActor3D _ svtkProp3DAxisFollower::TitleProp3D
...
//-----------------------------------------------------------------------------
void svtkAxisActor::InitExponent()
{
...
//-----------------------------------------------------------------------------
void svtkAxisActor::BuildTitle(bool force)
{
  ...
  if (this->LabelVisibility && this->TitleAlignLocation != SVTK_ALIGN_TOP)
  {
  ...
  {
    svtkWarningMacro("CalculateTitleOffset flag is now deprecated and has no effect");
  }
  ...
  if (this->TickVisibility &&
    (this->TickLocation == SVTK_TICKS_BOTH ||
      (this->TickLocation == SVTK_TICKS_INSIDE && this->TitleAlignLocation == SVTK_ALIGN_TOP) ||
      (this->TickLocation == SVTK_TICKS_OUTSIDE && this->TitleAlignLocation != SVTK_ALIGN_TOP)))
  {
    for (int i = 0; i < 3; i++)
    {
...
//-----------------------------------------------------------------------------
void svtkAxisActor::BuildExponent(bool force)
{
  ...
  // find max height label (with the label text property considered)
  // only when title is on bottom
  if (this->LabelVisibility && this->ExponentLocation != SVTK_ALIGN_TOP)
  {
    double labelMaxHeight, labHeight;
    labelMaxHeight = labHeight = 0;
    for (int i = 0; i < this->NumberOfLabelsBuilt; i++)
  ...
  if (this->TickVisibility &&
    (this->TickLocation == SVTK_TICKS_BOTH ||
      (this->TickLocation == SVTK_TICKS_INSIDE && this->ExponentLocation == SVTK_ALIGN_TOP) ||
      (this->TickLocation == SVTK_TICKS_OUTSIDE && this->ExponentLocation != SVTK_ALIGN_TOP)))
  {
    for (int i = 0; i < 3; i++)
    {
...
//  title can only be centered with respect to its axis.
//-----------------------------------------------------------------------------
void svtkAxisActor::BuildTitle2D(svtkViewport* viewport, bool force)
{
  ...
  if (this->AxisType == SVTK_AXIS_TYPE_Y)
  {
    if (strlen(this->TitleActor2D->GetInput()) > 2)
    {
      // warning : orientation have to be set on svtkTextActor and not on the svtkTextActor's
      // svtkTextProperty
  ...
  int offsetSign = 1;
  if (this->TitleAlignLocation == SVTK_ALIGN_TOP)
  {
    offsetSign = -1;
  }

  if (this->AxisType == SVTK_AXIS_TYPE_X)
  {
    transpos[1] += offsetSign * this->VerticalOffsetXTitle2D;
  }
  else if (this->AxisType == SVTK_AXIS_TYPE_Y)
  {
...
//-----------------------------------------------------------------------------
void svtkAxisActor::BuildExponent2D(svtkViewport* viewport, bool force)
{
  ...
  int offsetSign = 1;
  if (this->ExponentLocation == SVTK_ALIGN_TOP)
  {
    offsetSign = -1;
  }
...
//-----------------------------------------------------------------------------
void svtkAxisActor::TransformBounds(svtkViewport* viewport, double bnds[6])
{
...
//-----------------------------------------------------------------------------
void svtkAxisActor::PrintSelf(ostream& os, svtkIndent indent)
{
...
//-----------------------------------------------------------------------------
void svtkAxisActor::SetLabels(svtkStringArray* labels)
{
  //
  ...
  if (numLabels < 0)
  {
    svtkErrorMacro(<< "Number of labels " << numLabels << " is invalid");
    return;
  }
  if (this->NumberOfLabelsBuilt != numLabels)
  {
    ...
    this->LabelVectors = new svtkVectorText*[numLabels];
    this->LabelMappers = new svtkPolyDataMapper*[numLabels];
    this->LabelActors = new svtkAxisFollower*[numLabels];
    this->LabelProps3D = new svtkProp3DAxisFollower*[numLabels];
    this->LabelActors3D = new svtkTextActor3D*[numLabels];
    this->LabelActors2D = new svtkTextActor*[numLabels];

    for (int i = 0; i < numLabels; i++)
    {
      this->LabelVectors[i] = svtkVectorText::New();
      this->LabelMappers[i] = svtkPolyDataMapper::New();
      this->LabelMappers[i]->SetInputConnection(this->LabelVectors[i]->GetOutputPort());
      this->LabelActors[i] = svtkAxisFollower::New();
      ...
      this->LabelActors[i]->GetProperty()->SetColor(this->LabelTextProperty->GetColor());
      this->LabelActors[i]->GetProperty()->SetOpacity(this->LabelTextProperty->GetOpacity());
      this->LabelProps3D[i] = svtkProp3DAxisFollower::New();
      this->LabelProps3D[i]->SetAxis(this);
      this->LabelProps3D[i]->SetEnableDistanceLOD(0);
      this->LabelActors3D[i] = svtkTextActor3D::New();
      this->LabelProps3D[i]->SetProp3D(this->LabelActors3D[i]);
      this->LabelActors2D[i] = svtkTextActor::New();
    }
  }
...
//-----------------------------------------------------------------------------*
void svtkAxisActor::SetAxisPointsAndLines()
{
  svtkPoints* mainLinePts = svtkPoints::New();
  svtkPoints* axisMajorTicksPts = svtkPoints::New();
  svtkPoints* axisMinorTicksPts = svtkPoints::New();

  svtkCellArray* mainLine = svtkCellArray::New();
  svtkCellArray* axisMajorTicksLines = svtkCellArray::New();
  svtkCellArray* axisMinorTicksLines = svtkCellArray::New();

  svtkCellArray* gridlines = svtkCellArray::New();
  svtkCellArray* innerGridlines = svtkCellArray::New();
...
//-----------------------------------------------------------------------------**
bool svtkAxisActor::TickVisibilityChanged()
{
...
//-----------------------------------------------------------------------------**
void svtkAxisActor::SetBounds(const double b[6])
{
...
// Retrieves the bounds of this actor.
//-----------------------------------------------------------------------------**
double* svtkAxisActor::GetBounds()
{
  return this->Bounds;
}
...
void svtkAxisActor::GetBounds(double b[6])
{
  for (int i = 0; i < 6; i++)
  {
    b[i] = this->Bounds[i];
...
//-----------------------------------------------------------------------------**
// Method:  svtkAxisActor::ComputeMaxLabelLength
//-----------------------------------------------------------------------------**
double svtkAxisActor::ComputeMaxLabelLength(const double svtkNotUsed(center)[3])
{
  double bounds[6];
  double xsize, ysize;
  svtkProperty* newProp = this->NewLabelProperty();
  double maxXSize = 0;
  double maxYSize = 0;
  for (int i = 0; i < this->NumberOfLabelsBuilt; i++)
  {
...
//-----------------------------------------------------------------------------**
// Method:  svtkAxisActor::ComputeTitleLength
//-----------------------------------------------------------------------------**
double svtkAxisActor::ComputeTitleLength(const double svtkNotUsed(center)[3])
{
  double bounds[6];
  double xsize, ysize;
  double length;
  ...
  else
  {
    this->TitleVector->SetText(this->Title);
    this->TitleActor->SetCamera(this->Camera);
    svtkProperty* newProp = this->NewTitleProperty();
...
//-----------------------------------------------------------------------------**
void svtkAxisActor::SetLabelScale(const double s)
{
  for (int i = 0; i < this->NumberOfLabelsBuilt; i++)
  {
...
//-----------------------------------------------------------------------------**
void svtkAxisActor::SetLabelScale(int label, const double s)
{
...
//-----------------------------------------------------------------------------**
void svtkAxisActor::SetTitleScale(const double s)
{
...
//-----------------------------------------------------------------------------**
void svtkAxisActor::SetTitle(const char* t)
{
...
//-----------------------------------------------------------------------------**
void svtkAxisActor::SetTitleAlignLocation(int location)
{
...
//-----------------------------------------------------------------------------**
void svtkAxisActor::SetExponent(const char* t)
{
...
//-----------------------------------------------------------------------------**
void svtkAxisActor::SetExponentLocation(int location)
{
...
//-----------------------------------------------------------------------------
void svtkAxisActor::SetAxisLinesProperty(svtkProperty* prop)
{
...
//-----------------------------------------------------------------------------
svtkProperty* svtkAxisActor::GetAxisLinesProperty()
{
  return this->AxisLinesActor->GetProperty();
}

//-----------------------------------------------------------------------------
void svtkAxisActor::SetAxisMainLineProperty(svtkProperty* prop)
{
  this->AxisLinesActor->SetProperty(prop);
  this->Modified();
}

svtkProperty* svtkAxisActor::GetAxisMainLineProperty()
{
  return this->GetAxisLinesProperty();
}

//-----------------------------------------------------------------------------
void svtkAxisActor::SetAxisMajorTicksProperty(svtkProperty* prop)
{
...
//-----------------------------------------------------------------------------
svtkProperty* svtkAxisActor::GetAxisMajorTicksProperty()
{
  return this->AxisMajorTicksActor->GetProperty();
}

//-----------------------------------------------------------------------------
void svtkAxisActor::SetAxisMinorTicksProperty(svtkProperty* prop)
{
...
//-----------------------------------------------------------------------------
svtkProperty* svtkAxisActor::GetAxisMinorTicksProperty()
{
  return this->AxisMinorTicksActor->GetProperty();
}

//-----------------------------------------------------------------------------
void svtkAxisActor::SetGridlinesProperty(svtkProperty* prop)
{
...
//-----------------------------------------------------------------------------
svtkProperty* svtkAxisActor::GetGridlinesProperty()
{
  return this->GridlinesActor->GetProperty();
}

//-----------------------------------------------------------------------------
void svtkAxisActor::SetInnerGridlinesProperty(svtkProperty* prop)
{
...
//-----------------------------------------------------------------------------
svtkProperty* svtkAxisActor::GetInnerGridlinesProperty()
{
  return this->InnerGridlinesActor->GetProperty();
}

//-----------------------------------------------------------------------------
void svtkAxisActor::SetGridpolysProperty(svtkProperty* prop)
{
...
//-----------------------------------------------------------------------------
svtkProperty* svtkAxisActor::GetGridpolysProperty()
{
  return this->GridpolysActor->GetProperty();
}

//-----------------------------------------------------------------------------
svtkProperty* svtkAxisActor::NewTitleProperty()
{
  svtkProperty* newProp = svtkProperty::New();
...
//-----------------------------------------------------------------------------
svtkProperty* svtkAxisActor::NewLabelProperty()
{
  svtkProperty* newProp = svtkProperty::New();
  newProp->DeepCopy(this->GetProperty());
...
//-----------------------------------------------------------------------------
double svtkAxisActor::GetDeltaMajor(int axis)
{
  return (axis >= 0 && axis <= 2) ? this->DeltaMajor[axis] : 0;
}

//-----------------------------------------------------------------------------
void svtkAxisActor::SetDeltaMajor(int axis, double value)
{
  if (axis >= 0 && axis <= 2)
  {
...
//-----------------------------------------------------------------------------
double svtkAxisActor::GetMajorStart(int axis)
{
  return (axis >= 0 && axis <= 2) ? this->MajorStart[axis] : 0;
}

//-----------------------------------------------------------------------------
void svtkAxisActor::SetMajorStart(int axis, double value)
{
  if (axis >= 0 && axis <= 2)
  {
...
//-----------------------------------------------------------------------------
bool svtkAxisActor::BoundsDisplayCoordinateChanged(svtkViewport* viewport)
{
...
//-----------------------------------------------------------------------------
svtkCoordinate* svtkAxisActor::GetPoint1Coordinate()
{
  svtkDebugMacro(<< this->GetClassName() << " (" << this << "): returning Point1 Coordinate address "
                << this->Point1Coordinate);
  return this->Point1Coordinate;
}

//---------------------------------------------------------------------------
svtkCoordinate* svtkAxisActor::GetPoint2Coordinate()
{
  svtkDebugMacro(<< this->GetClassName() << " (" << this << "): returning Point2 Coordinate address "
                << this->Point2Coordinate);
  return this->Point2Coordinate;
}

//---------------------------------------------------------------------------
void svtkAxisActor::SetPoint1(double x, double y, double z)
{
  this->Point1Coordinate->SetValue(x, y, z);
}

//---------------------------------------------------------------------------
void svtkAxisActor::SetPoint2(double x, double y, double z)
{
  this->Point2Coordinate->SetValue(x, y, z);
}

//---------------------------------------------------------------------------
double* svtkAxisActor::GetPoint1()
{
  return this->Point1Coordinate->GetValue();
}

//---------------------------------------------------------------------------
double* svtkAxisActor::GetPoint2()
{
...
// for a generic axis.
//-----------------------------------------------------------------------------
bool svtkAxisActor::BuildTickPoints(double p1[3], double p2[3], bool force)
{
...
//-----------------------------------------------------------------------------
void svtkAxisActor::BuildAxisGridLines(double p1[3], double p2[3], double localCoordSys[3][3])
{
  ...
  int uMult = svtkAxisActorMultiplierTable1[this->AxisPosition];
  int vMult = svtkAxisActorMultiplierTable2[this->AxisPosition];
  ...
  double nbIterationAsDouble = (axisLength - axisShift) / svtkMath::Norm(deltaVector);
  int nbIteration = svtkMath::Floor(nbIterationAsDouble + 2 * FLT_EPSILON) + 1;
  nbIteration = (nbIteration < SVTK_MAX_TICKS) ? nbIteration : SVTK_MAX_TICKS;
...
//-----------------------------------------------------------------------------
void svtkAxisActor::BuildMinorTicks(double p1[3], double p2[3], double localCoordSys[3][3])
{
  // (p2 - p1) vector
  double deltaVector[3];
  ...
  if (this->TickLocation == SVTK_TICKS_OUTSIDE || this->TickLocation == SVTK_TICKS_BOTH)
  {
    for (int i = 0; i < 3; i++)
    {
  ...
  if (this->TickLocation == SVTK_TICKS_INSIDE || this->TickLocation == SVTK_TICKS_BOTH)
  {
    for (int i = 0; i < 3; i++)
    {
...
//-----------------------------------------------------------------------------
void svtkAxisActor::BuildMajorTicks(double p1[3], double p2[3], double localCoordSys[3][3])
{
  double deltaVector[3];
  double* axisVector = localCoordSys[0];
  double* uVector = localCoordSys[1];
...
//-----------------------------------------------------------------------------
void svtkAxisActor::BuildMinorTicksLog(double p1[3], double p2[3], double localCoordSys[3][3])
{
  double deltaVector[3];
  ...
  for (step = lowBound; step < upBound; step *= base)
  {
    ...
    for (index = 2.0; index < base; index += 1.0)
    {
      ...
      // svtkMath::Add(minorTickOnAxis, p1, minorTickOnAxis); // handled later

      // u inside point
      svtkMath::Add(minorTickOnAxis, uPointInside, minorTickPoint);
      this->MinorTickPts->InsertNextPoint(minorTickPoint);

      // u outside point
      svtkMath::Add(minorTickOnAxis, uPointOutside, minorTickPoint);
      this->MinorTickPts->InsertNextPoint(minorTickPoint);

      if (this->Use2DMode == 0)
      {
        // v inside point
        svtkMath::Add(minorTickOnAxis, vPointInside, minorTickPoint);
        this->MinorTickPts->InsertNextPoint(minorTickPoint);

        // v outside point
        svtkMath::Add(minorTickOnAxis, vPointOutside, minorTickPoint);
        this->MinorTickPts->InsertNextPoint(minorTickPoint);
      }
    }
  }
...
//-----------------------------------------------------------------------------
void svtkAxisActor::BuildMajorTicksLog(double p1[3], double p2[3], double localCoordSys[3][3])
{
  double deltaVector[3];
  ...
  for (indexTickRangeValue = lowBound; indexTickRangeValue <= upBound; indexTickRangeValue *= base)
  {
    ...
    // u inside point
    svtkMath::Add(majorTickOnAxis, uPointInside, majorTickPoint);
    this->MajorTickPts->InsertNextPoint(majorTickPoint);

    // u outside point
    svtkMath::Add(majorTickOnAxis, uPointOutside, majorTickPoint);
    this->MajorTickPts->InsertNextPoint(majorTickPoint);

    // v inside point
    svtkMath::Add(majorTickOnAxis, vPointInside, majorTickPoint);
    this->MajorTickPts->InsertNextPoint(majorTickPoint);

    // v outside point
    svtkMath::Add(majorTickOnAxis, vPointOutside, majorTickPoint);
    this->MajorTickPts->InsertNextPoint(majorTickPoint);
  }
}

//-----------------------------------------------------------------------------
void svtkAxisActor::RotateActor2DFromAxisProjection(svtkTextActor* pActor2D)
{
  double* p1 = this->Point1Coordinate->GetValue();
  double* p2 = this->Point2Coordinate->GetValue();

  svtkMatrix4x4* matModelView = this->Camera->GetModelViewTransformMatrix();
  ...
  double dotProd = svtkMath::Dot2D(x, axisOnScreen);

  double orient = 0.0;
  if (svtkMath::Norm2D(axisOnScreen) == 0.0)
  {
  ...
  {
    orient = acos(dotProd / svtkMath::Norm2D(axisOnScreen));
    orient = svtkMath::DegreesFromRadians(orient);
  }

  // adjust angle
  if (svtkMath::Dot2D(y, axisOnScreen) < 0.0)
  {
    orient *= -1.0;
  }

  if (svtkMath::Dot2D(x, axisOnScreen) < 0.0)
  {
    orient += 180.0;
  }
