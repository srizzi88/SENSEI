...
svtkStandardNewMacro(svtkAxisActor2D);

svtkCxxSetObjectMacro(svtkAxisActor2D, LabelTextProperty, svtkTextProperty);
svtkCxxSetObjectMacro(svtkAxisActor2D, TitleTextProperty, svtkTextProperty);

//----------------------------------------------------------------------------
// Instantiate this object.
svtkAxisActor2D::svtkAxisActor2D()
{
  ...
  this->LabelMappers = new svtkTextMapper*[SVTK_MAX_LABELS];
  this->LabelActors = new svtkActor2D*[SVTK_MAX_LABELS];
  for (int i = 0; i < SVTK_MAX_LABELS; i++)
  {
  ...
  this->Axis = svtkPolyData::New();
  this->AxisMapper = svtkPolyDataMapper2D::New();
  this->AxisMapper->SetInputData(this->Axis);
  this->AxisActor = svtkActor2D::New();
...
int svtkAxisActor2D::RenderOpaqueGeometry(svtkViewport* viewport)
{
...
int svtkAxisActor2D::RenderOverlay(svtkViewport* viewport)
{
  int i, renderedSomething = 0;
...
svtkTypeBool svtkAxisActor2D::HasTranslucentPolygonalGeometry()
{
  return 0;
}
...
void svtkAxisActor2D::ReleaseGraphicsResources(svtkWindow* win)
{
...
//----------------------------------------------------------------------------
void svtkAxisActor2D::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
void svtkAxisActor2D::BuildAxis(svtkViewport* viewport)
{
  ...
  {
    svtkErrorMacro(<< "Need title text property to render axis actor");
    return;
  }

  if (this->LabelVisibility && !this->LabelTextProperty)
  {
    svtkErrorMacro(<< "Need label text property to render axis actor");
    return;
  }
  ...
  if (!viewport->GetSVTKWindow() ||
    (!positionsHaveChanged && !viewportSizeHasChanged && viewport->GetMTime() < this->BuildTime &&
      viewport->GetSVTKWindow()->GetMTime() < this->BuildTime &&
  ...
  if (this->RulerMode)
  {
    ...
    wp21[1] = wp2[1] - wp1[1];
    wp21[2] = wp2[2] - wp1[2];
    const double worldLength = svtkMath::Norm(wp21);
  ...
  if (this->LabelVisibility)
  {
    ...
    svtkMTimeType labeltime = this->AdjustedRangeBuildTime;
      ...
      this->LabelTextProperty->GetMTime() > this->BuildTime || labeltime > this->BuildTime)
    {
      if (!this->SizeFontRelativeToAxis)
      {
        svtkTextMapper::SetMultipleRelativeFontSize(viewport, this->LabelMappers,
...
//----------------------------------------------------------------------------
void svtkAxisActor2D::UpdateAdjustedRange()
{
  ...
  // changed. But since svtkActor2D::GetMTime() includes the check for
  // both Position and Position2 coordinates, we will have to bypass
  // it.

  if (this->svtkActor2D::Superclass::GetMTime() <= this->AdjustedRangeBuildTime)
  {
    return;
  }
...
static int svtkAxisActor2DComputeTicks(double sRange[2], double& interval, double& root)
{
...
// reasonable interval
void svtkAxisActor2D::ComputeRange(double inRange[2], double outRange[2], int svtkNotUsed(inNumTicks),
  int& numTicks, double& interval)
{
  ...
  double root;
  numTicks = svtkAxisActor2DComputeTicks(sRange, interval, root);
...
void svtkAxisActor2D::SetOffsetPosition(
  double xTick[3], double theta, int stringWidth, int stringHeight, int offset, svtkActor2D* actor)
{
  double x, y, center[2];
  int pos[2];
...
//----------------------------------------------------------------------------
double svtkAxisActor2D::ComputeStringOffset(double width, double height, double theta)
{
...
//----------------------------------------------------------------------------
void svtkAxisActor2D::ShallowCopy(svtkProp* prop)
{
  svtkAxisActor2D* a = svtkAxisActor2D::SafeDownCast(prop);
  if (a != nullptr)
  {
