...
svtkStandardNewMacro(svtkBarChartActor);

svtkCxxSetObjectMacro(svtkBarChartActor, Input, svtkDataObject);
svtkCxxSetObjectMacro(svtkBarChartActor, LabelTextProperty, svtkTextProperty);
svtkCxxSetObjectMacro(svtkBarChartActor, TitleTextProperty, svtkTextProperty);

// PIMPL'd list of labels
class svtkBarLabelArray : public std::vector<std::string>
{
};

//----------------------------------------------------------------------------
// Instantiate object
svtkBarChartActor::svtkBarChartActor()
{
  // Actor2D positions
  ...
  this->Title = nullptr;
  this->Labels = new svtkBarLabelArray;
  ...
  this->MinHeight = SVTK_FLOAT_MAX;
  this->MaxHeight = -SVTK_FLOAT_MAX;
...
void svtkBarChartActor::Initialize()
{
  if (this->BarActors)
  {
...
int svtkBarChartActor::RenderOverlay(svtkViewport* viewport)
{
...
//----------------------------------------------------------------------------
// Plot scalar data for each input dataset.
int svtkBarChartActor::RenderOpaqueGeometry(svtkViewport* viewport)
{
  int renderedSomething = 0;
...
svtkTypeBool svtkBarChartActor::HasTranslucentPolygonalGeometry()
{
  return 0;
}

//-----------------------------------------------------------------------------
int svtkBarChartActor::BuildPlot(svtkViewport* viewport)
{
  // Initialize
...
//----------------------------------------------------------------------------
int svtkBarChartActor::PlaceAxes(svtkViewport* viewport, const int* svtkNotUsed(size))
{
  svtkIdType i;
  ...
  svtkCellArray* xaxis = svtkCellArray::New();
  ...
  if (this->LabelVisibility)
  {
    this->BarActors = new svtkActor2D*[this->N];
    this->BarMappers = new svtkTextMapper*[this->N];
    for (i = 0; i < this->N; i++)
    {
      this->BarMappers[i] = svtkTextMapper::New();
      if ((str = this->GetBarLabel(i)) != nullptr)
      {
      ...
      this->BarActors[i] = svtkActor2D::New();
...
void svtkBarChartActor::ReleaseGraphicsResources(svtkWindow* win)
{
...
//----------------------------------------------------------------------------
void svtkBarChartActor::SetBarLabel(const int i, const char* label)
{
  if (i < 0)
  {
    return;
...
//----------------------------------------------------------------------------
const char* svtkBarChartActor::GetBarLabel(int i)
{
...
//----------------------------------------------------------------------------
void svtkBarChartActor::SetBarColor(int i, double r, double g, double b)
{
  this->LegendActor->SetEntryColor(i, r, g, b);
}

//----------------------------------------------------------------------------
double* svtkBarChartActor::GetBarColor(int i)
{
  return this->LegendActor->GetEntryColor(i);
}

//----------------------------------------------------------------------------
void svtkBarChartActor::PrintSelf(ostream& os, svtkIndent indent)
{
