/*=========================================================================
  ...
  Module:    svtkCornerAnnotation.cxx
...
//----------------------------------------------------------------------------
svtkStandardNewMacro(svtkCornerAnnotation);

svtkSetObjectImplementationMacro(svtkCornerAnnotation, ImageActor, svtkImageActor);
svtkSetObjectImplementationMacro(svtkCornerAnnotation, WindowLevel, svtkImageMapToWindowLevelColors);
svtkCxxSetObjectMacro(svtkCornerAnnotation, TextProperty, svtkTextProperty);

//----------------------------------------------------------------------------
svtkCornerAnnotation::svtkCornerAnnotation()
{
  ...
  for (int i = 0; i < NumTextPositions; i++)
  {
    ...
    this->TextMapper[i] = svtkTextMapper::New();
    this->TextActor[i] = svtkActor2D::New();
...
void svtkCornerAnnotation::ReleaseGraphicsResources(svtkWindow* win)
{
...
//----------------------------------------------------------------------------
void svtkCornerAnnotation::TextReplace(svtkImageActor* ia, svtkImageMapToWindowLevelColors* wl)
{
  int i;
  char *text, *text2;
  ...
  svtkImageData *wl_input = nullptr, *ia_input = nullptr;
  ...
  if (wl)
  {
    ...
    wl_input = svtkImageData::SafeDownCast(wl->GetInput());
    if (wl_input)
    {
      input_type_is_float =
        (wl_input->GetScalarType() == SVTK_FLOAT || wl_input->GetScalarType() == SVTK_DOUBLE);
    }
  }
  if (ia)
  {
    ...
    {
      input_type_is_float =
        (ia_input->GetScalarType() == SVTK_FLOAT || ia_input->GetScalarType() == SVTK_DOUBLE);
    }
  }
...
//----------------------------------------------------------------------------
int svtkCornerAnnotation::RenderOverlay(svtkViewport* viewport)
{
...
namespace
{
// Ported from old svtkTextMapper implementation
...
//----------------------------------------------------------------------------
int svtkCornerAnnotation::RenderOpaqueGeometry(svtkViewport* viewport)
{
  int fontSize;
  ...
  svtkImageMapToWindowLevelColors* wl = this->WindowLevel;
  svtkImageActor* ia = nullptr;
  if (this->ImageActor)
  {
    ia = this->ImageActor;
  }
  ...
  {
    svtkPropCollection* pc = viewport->GetViewProps();
    int numProps = pc->GetNumberOfItems();
    for (int i = 0; i < numProps; i++)
    {
      ia = svtkImageActor::SafeDownCast(pc->GetItemAsObject(i));
      if (ia)
      {
        if (ia->GetInput() && !wl)
        {
          wl = svtkImageMapToWindowLevelColors::SafeDownCast(ia->GetMapper()->GetInputAlgorithm());
        }
        break;
      }
    }
    ...
    (wl && wl->GetMTime() > this->BuildTime))
  {
    ...
    if (viewport_size_has_changed || tprop_has_changed || this->GetMTime() > this->BuildTime)
    {
      ...
      if (tprop_has_changed)
      {
        for (int i = 0; i < NumTextPositions; i++)
        {
          svtkTextProperty* tprop = this->TextMapper[i]->GetTextProperty();
...
svtkTypeBool svtkCornerAnnotation::HasTranslucentPolygonalGeometry()
{
  return 0;
}

//----------------------------------------------------------------------------
void svtkCornerAnnotation::SetTextActorsPosition(const int vsize[2])
{
...
//----------------------------------------------------------------------------
void svtkCornerAnnotation::SetTextActorsJustification()
{
  svtkTextProperty* tprop = this->TextMapper[LowerLeft]->GetTextProperty();
...
//----------------------------------------------------------------------------
void svtkCornerAnnotation::SetText(int i, const char* text)
{
...
//----------------------------------------------------------------------------
const char* svtkCornerAnnotation::GetText(int i)
{
  if (i < 0 || i >= NumTextPositions)
  {
    return nullptr;
...
//----------------------------------------------------------------------------
void svtkCornerAnnotation::ClearAllTexts()
{
  for (int i = 0; i < NumTextPositions; i++)
  {
    this->SetText(i, "");
...
//----------------------------------------------------------------------------
void svtkCornerAnnotation::CopyAllTextsFrom(svtkCornerAnnotation* ca)
{
  for (int i = 0; i < NumTextPositions; i++)
  {
...
//----------------------------------------------------------------------------
void svtkCornerAnnotation::PrintSelf(ostream& os, svtkIndent indent)
{
