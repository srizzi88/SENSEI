...
svtkStandardNewMacro(svtkCubeAxesActor);
svtkCxxSetObjectMacro(svtkCubeAxesActor, Camera, svtkCamera);
// *************************************************************************
// Instantiate this object.
// *************************************************************************
svtkCubeAxesActor::svtkCubeAxesActor()
  : svtkActor()
{
  this->Bounds[0] = -1.0;
  this->Bounds[1] = 1.0;
  ...
  this->FlyMode = SVTK_FLY_CLOSEST_TRIAD;
  this->GridLineLocation = SVTK_GRID_LINES_ALL;
  ...
  for (int i = 0; i < 3; i++)
  {
    this->TitleTextProperty[i] = svtkTextProperty::New();
    ...
    this->LabelTextProperty[i] = svtkTextProperty::New();
  ...
  // Axis lines
  this->XAxesLinesProperty = svtkProperty::New();
  this->YAxesLinesProperty = svtkProperty::New();
  this->ZAxesLinesProperty = svtkProperty::New();

  // Outer grid lines
  this->XAxesGridlinesProperty = svtkProperty::New();
  this->YAxesGridlinesProperty = svtkProperty::New();
  this->ZAxesGridlinesProperty = svtkProperty::New();

  // Inner grid lines
  this->XAxesInnerGridlinesProperty = svtkProperty::New();
  this->YAxesInnerGridlinesProperty = svtkProperty::New();
  this->ZAxesInnerGridlinesProperty = svtkProperty::New();
  ...
  this->XAxesGridpolysProperty = svtkProperty::New();
  this->YAxesGridpolysProperty = svtkProperty::New();
  this->ZAxesGridpolysProperty = svtkProperty::New();
  ...
  {
    this->XAxes[i] = svtkAxisActor::New();
    ...
    this->YAxes[i] = svtkAxisActor::New();
    ...
    this->ZAxes[i] = svtkAxisActor::New();
    ...
    svtkAxisFollower* follower = this->XAxes[i]->GetTitleActor();
    ...
    svtkProp3DAxisFollower* axisFollower = this->XAxes[i]->GetTitleProp3D();
  ...
  this->TickLocation = SVTK_TICKS_INSIDE;
  ...
  this->LastXRange[0] = SVTK_FLOAT_MAX;
  this->LastXRange[1] = SVTK_FLOAT_MAX;
  this->LastYRange[0] = SVTK_FLOAT_MAX;
  this->LastYRange[1] = SVTK_FLOAT_MAX;
  this->LastZRange[0] = SVTK_FLOAT_MAX;
  this->LastZRange[1] = SVTK_FLOAT_MAX;

  this->LastBounds[0] = SVTK_DOUBLE_MAX;
  this->LastBounds[1] = SVTK_DOUBLE_MAX;
  this->LastBounds[2] = SVTK_DOUBLE_MAX;
  this->LastBounds[3] = SVTK_DOUBLE_MAX;
  this->LastBounds[4] = SVTK_DOUBLE_MAX;
  this->LastBounds[5] = SVTK_DOUBLE_MAX;
  ...
  this->XAxisRange[0] = SVTK_DOUBLE_MAX;
  this->XAxisRange[1] = SVTK_DOUBLE_MAX;
  this->YAxisRange[0] = SVTK_DOUBLE_MAX;
  this->YAxisRange[1] = SVTK_DOUBLE_MAX;
  this->ZAxisRange[0] = SVTK_DOUBLE_MAX;
  this->ZAxisRange[1] = SVTK_DOUBLE_MAX;
...
// *************************************************************************
void svtkCubeAxesActor::SetUseTextActor3D(int val)
{
  for (int i = 0; i < NUMBER_OF_ALIGNED_AXIS; ++i)
  {
...
int svtkCubeAxesActor::GetUse2DMode()
{
  // It is assumed that all axes have the same value
  return this->XAxes[0]->GetUse2DMode();
}

void svtkCubeAxesActor::SetSaveTitlePosition(int val)
{
  // For 2D mode only :
...
// *************************************************************************
int svtkCubeAxesActor::RenderOpaqueGeometry(svtkViewport* viewport)
{
  static bool initialRender = true;
  return this->RenderGeometry(initialRender, viewport, true, &svtkAxisActor::RenderOpaqueGeometry);
}
...
// with the boundary of the viewport (minus borders).
// *************************************************************************
int svtkCubeAxesActor::RenderTranslucentGeometry(svtkViewport* viewport)
{
  static bool initialRender = true;
  return this->RenderGeometry(
    initialRender, viewport, true, &svtkAxisActor::RenderTranslucentGeometry);
}
...
// with the boundary of the viewport (minus borders).
// *************************************************************************
int svtkCubeAxesActor::RenderTranslucentPolygonalGeometry(svtkViewport* viewport)
{
  static bool initialRender = true;
  return this->RenderGeometry(
    initialRender, viewport, true, &svtkAxisActor::RenderTranslucentPolygonalGeometry);
}

// *************************************************************************
// RenderOverlay : render 2D annotations.
// *************************************************************************
int svtkCubeAxesActor::RenderOverlay(svtkViewport* viewport)
{
  static bool initialRender = true;
  return this->RenderGeometry(initialRender, viewport, false, &svtkAxisActor::RenderOverlay);
}

// --------------------------------------------------------------------------
svtkTypeBool svtkCubeAxesActor::HasTranslucentPolygonalGeometry()
{
...
void svtkCubeAxesActor::AdjustAxes(double bounds[6], double xCoords[NUMBER_OF_ALIGNED_AXIS][6],
...
{
  xRange[0] = (this->XAxisRange[0] == SVTK_DOUBLE_MAX ? bounds[0] : this->XAxisRange[0]);
  xRange[1] = (this->XAxisRange[1] == SVTK_DOUBLE_MAX ? bounds[1] : this->XAxisRange[1]);
  yRange[0] = (this->YAxisRange[0] == SVTK_DOUBLE_MAX ? bounds[2] : this->YAxisRange[0]);
  yRange[1] = (this->YAxisRange[1] == SVTK_DOUBLE_MAX ? bounds[3] : this->YAxisRange[1]);
  zRange[0] = (this->ZAxisRange[0] == SVTK_DOUBLE_MAX ? bounds[4] : this->ZAxisRange[0]);
  zRange[1] = (this->ZAxisRange[1] == SVTK_DOUBLE_MAX ? bounds[5] : this->ZAxisRange[1]);
...
// *************************************************************************
void svtkCubeAxesActor::SetScreenSize(double screenSize)
{
...
// *************************************************************************
void svtkCubeAxesActor::SetLabelOffset(double offset)
{
...
// *************************************************************************
void svtkCubeAxesActor::SetTitleOffset(double offset)
{
...
// *************************************************************************
void svtkCubeAxesActor::ReleaseGraphicsResources(svtkWindow* win)
{
  for (int i = 0; i < NUMBER_OF_ALIGNED_AXIS; i++)
  {
...
// ******************************************************************
void svtkCubeAxesActor::PrintSelf(ostream& os, svtkIndent indent)
{
  ...
  if (this->FlyMode == SVTK_FLY_CLOSEST_TRIAD)
  {
    os << indent << "Fly Mode: CLOSEST_TRIAD\n";
  }
  else if (this->FlyMode == SVTK_FLY_FURTHEST_TRIAD)
  {
    os << indent << "Fly Mode: FURTHEST_TRIAD\n";
  }
  else if (this->FlyMode == SVTK_FLY_STATIC_TRIAD)
  {
    os << indent << "Fly Mode: STATIC_TRIAD\n";
  }
  else if (this->FlyMode == SVTK_FLY_STATIC_EDGES)
  {
  ...
  {
    case SVTK_GRID_LINES_ALL:
      os << indent << "GridLineLocation: SVTK_GRID_LINES_ALL (0)" << endl;
      break;
    case SVTK_GRID_LINES_CLOSEST:
      os << indent << "GridLineLocation: SVTK_GRID_LINES_CLOSEST (1)" << endl;
      break;
    case SVTK_GRID_LINES_FURTHEST:
      os << indent << "GridLineLocation: SVTK_GRID_LINES_FURTHEST (2)" << endl;
      break;
  }
...
// --------------------------------------------------------------------------
void svtkCubeAxesActor::TransformBounds(
  svtkViewport* viewport, const double bounds[6], double pts[8][3])
{
  ...
  {
    svtkCubeAxesActor::GetBoundsPoint(idx, bounds, x);
...
// ***********************************************************************
bool svtkCubeAxesActor::ComputeTickSize(double bounds[6])
{
  ...
  this->LastXRange[0] = (this->XAxisRange[0] == SVTK_DOUBLE_MAX ? bounds[0] : this->XAxisRange[0]);
  this->LastXRange[1] = (this->XAxisRange[1] == SVTK_DOUBLE_MAX ? bounds[1] : this->XAxisRange[1]);
  this->LastYRange[0] = (this->YAxisRange[0] == SVTK_DOUBLE_MAX ? bounds[2] : this->YAxisRange[0]);
  this->LastYRange[1] = (this->YAxisRange[1] == SVTK_DOUBLE_MAX ? bounds[3] : this->YAxisRange[1]);
  this->LastZRange[0] = (this->ZAxisRange[0] == SVTK_DOUBLE_MAX ? bounds[4] : this->ZAxisRange[0]);
  this->LastZRange[1] = (this->ZAxisRange[1] == SVTK_DOUBLE_MAX ? bounds[5] : this->ZAxisRange[1]);
  for (int i = 0; i < 6; i++)
  {
...
// ****************************************************************************
//  Method: svtkCubeAxesActor::AdjustValues
//
//  Purpose:
...
//
// ****************************************************************************
void svtkCubeAxesActor::AdjustValues(
...
// ****************************************************************************
//  Method: svtkCubeAxesActor::AdjustRange
//
//  Purpose:
...
// ****************************************************************************
void svtkCubeAxesActor::AdjustRange(const double ranges[6])
{
...
// ****************************************************************************
int svtkCubeAxesActor::Digits(double min, double max)
{
...
int svtkCubeAxesActor::LabelExponent(double min, double max)
{
  if (min == max)
  {
    return 0;
...
// *************************************************************************
void svtkCubeAxesActor::BuildAxes(svtkViewport* viewport)
{
  ...
  {
    this->XAxisRange[0] = this->XAxisRange[1] = SVTK_DOUBLE_MAX;
    this->YAxisRange[0] = this->YAxisRange[1] = SVTK_DOUBLE_MAX;
    this->ZAxisRange[0] = this->ZAxisRange[1] = SVTK_DOUBLE_MAX;
...
// *************************************************************************
//  Sends attributes to each svtkAxisActor.  Only sets those that are
//  not dependent upon viewport changes, and thus do not need to be set
//  very often.
// *************************************************************************
void svtkCubeAxesActor::SetNonDependentAttributes()
{
  svtkProperty* prop = this->GetProperty();
  prop->SetAmbient(1.0);
  prop->SetDiffuse(0.0);

  // Make sure our Axis Base is normalized
  svtkMath::Normalize(this->AxisBaseForX);
  svtkMath::Normalize(this->AxisBaseForY);
  svtkMath::Normalize(this->AxisBaseForZ);

  // Manage custom grid visibility location if FLY and STATIC axis
  int gridLocationBasedOnAxis =
    (this->GridLineLocation == SVTK_GRID_LINES_ALL) ? SVTK_GRID_LINES_ALL : SVTK_GRID_LINES_CLOSEST;
...
//
static int svtkCubeAxesActorTriads[8][3] = { { mm, mm, mm }, { mm, Xm, Xm }, { Xm, mm, mX },
  { Xm, Xm, XX }, { mX, mX, mm }, { mX, XX, Xm }, { XX, mX, mX }, { XX, XX, XX } };
static int svtkCubeAxesActorConn[8][3] = { { 1, 2, 4 }, { 0, 3, 5 }, { 3, 0, 6 }, { 2, 1, 7 },
...
// *************************************************************************
void svtkCubeAxesActor::DetermineRenderAxes(svtkViewport* viewport)
{
  double bounds[6];
  double pts[8][3];
  ...
  // on the viewpoint
  if (!(this->GridLineLocation == SVTK_GRID_LINES_ALL &&
        (this->FlyMode == SVTK_FLY_STATIC_EDGES || this->FlyMode == SVTK_FLY_STATIC_TRIAD)))
  {
    // determine the bounds to use (input, prop, or user-defined)
    this->GetBounds(bounds);
  ...
  if (this->GridLineLocation == SVTK_GRID_LINES_CLOSEST || this->FlyMode == SVTK_FLY_CLOSEST_TRIAD)
  {
    closestIdx = this->FindClosestAxisIndex(pts);
  }

  // Check furtherst point if needed
  if (this->GridLineLocation == SVTK_GRID_LINES_FURTHEST || this->FlyMode == SVTK_FLY_FURTHEST_TRIAD)
  {
    furtherstIdx = this->FindFurtherstAxisIndex(pts);
  }

  // Manage fast static axis visibility
  if (this->FlyMode == SVTK_FLY_STATIC_EDGES || this->FlyMode == SVTK_FLY_STATIC_TRIAD)
  {
    if (this->FlyMode == SVTK_FLY_STATIC_EDGES)
    {
    ...
    this->UpdateGridLineVisibility(
      (this->GridLineLocation == SVTK_GRID_LINES_CLOSEST) ? closestIdx : furtherstIdx);
    return;
  }
  ...
  if (this->RenderCount++ == 0 || !(this->RenderCount % this->Inertia))
  {
    if (this->FlyMode == SVTK_FLY_CLOSEST_TRIAD)
    {
      xloc = svtkCubeAxesActorTriads[closestIdx][0];
      yloc = svtkCubeAxesActorTriads[closestIdx][1];
      zloc = svtkCubeAxesActorTriads[closestIdx][2];
    }
    else if (this->FlyMode == SVTK_FLY_FURTHEST_TRIAD)
    {
      xloc = svtkCubeAxesActorTriads[furtherstIdx][0];
      yloc = svtkCubeAxesActorTriads[furtherstIdx][1];
      zloc = svtkCubeAxesActorTriads[furtherstIdx][2];
    }
...
// --------------------------------------------------------------------------
double svtkCubeAxesActor::MaxOf(double a, double b)
{
  return (a > b ? a : b);
}

// --------------------------------------------------------------------------
double svtkCubeAxesActor::MaxOf(double a, double b, double c, double d)
{
  return this->MaxOf(this->MaxOf(a, b), this->MaxOf(c, d));
}

// --------------------------------------------------------------------------
inline double svtkCubeAxesActor::FFix(double value)
{
  int ivalue = static_cast<int>(value);
  return ivalue;
}

inline int svtkCubeAxesActor::FRound(double value)
{
  return value <= 0.5 ? static_cast<int>(this->FFix(value))
                      : static_cast<int>(this->FFix(value) + 1);
}

inline int svtkCubeAxesActor::GetNumTicks(double range, double fxt)
{
...
// --------------------------------------------------------------------------
inline double svtkCubeAxesActor::FSign(double value, double sign)
{
  value = fabs(value);
  if (sign < 0.)
  {
...
// *******************************************************************
// Method: svtkCubeAxesActor::AdjustTicksComputeRange
//
...
void svtkCubeAxesActor::AdjustTicksComputeRange(
  svtkAxisActor* axes[NUMBER_OF_ALIGNED_AXIS], double boundsMin, double boundsMax)
{
  ...
  svtkStringArray* customizedLabels = nullptr;
  ...
  {
    case svtkAxisActor::SVTK_AXIS_TYPE_X:
      axis = 0;
      break;
    case svtkAxisActor::SVTK_AXIS_TYPE_Y:
      axis = 1;
      break;
    case svtkAxisActor::SVTK_AXIS_TYPE_Z:
      axis = 2;
      break;
  }
...
// ****************************************************************
void svtkCubeAxesActor::AutoScale(svtkViewport* viewport)
{
...
// ****************************************************************
void svtkCubeAxesActor::AutoScale(svtkViewport* viewport, svtkAxisActor* axis[NUMBER_OF_ALIGNED_AXIS])
{
  for (int i = 0; i < NUMBER_OF_ALIGNED_AXIS; ++i)
  {
    ...
    // Now labels.
    svtkAxisFollower** labelActors = axis[i]->GetLabelActors();
...
// ****************************************************************
double svtkCubeAxesActor::AutoScale(svtkViewport* viewport, double screenSize, double position[3])
{
  double factor = 1;
  if (viewport->GetSize()[1] > 0)
  {
    factor = 2.0 * screenSize *
      tan(svtkMath::RadiansFromDegrees(this->Camera->GetViewAngle() / 2.0)) / viewport->GetSize()[1];
  }

  double dist = sqrt(svtkMath::Distance2BetweenPoints(position, this->Camera->GetPosition()));
...
// ****************************************************************
void svtkCubeAxesActor::BuildLabels(svtkAxisActor* axes[NUMBER_OF_ALIGNED_AXIS])
{
  char label[64];
  int labelCount = 0;
  ...
  double axisLength = svtkMath::Norm(axis);
  ...
  else
  {
    ...
    labelCount = svtkMath::Floor(labelCountAsDouble + 2 * FLT_EPSILON) + 1;
  }
  ...
  else
  {
    if (labelCount > 0)
    {
      ...
      for (int i = 0; i < labelCount; ++i)
      {
        labels->SetValue(i, customizedLabels->GetValue(static_cast<svtkIdType>(i * delta)));
      }
    }
  }
  for (int i = 0; i < NUMBER_OF_ALIGNED_AXIS; i++)
...
svtkStringArray* svtkCubeAxesActor::GetAxisLabels(int axis)
{
  return (axis >= 0 && axis < 3) ? this->AxisLabels[axis] : nullptr;
}

void svtkCubeAxesActor::SetAxisLabels(int axis, svtkStringArray* value)
{
  if (axis >= 0 && axis < 3 && value != this->AxisLabels[axis])
  {
    svtkStringArray* previous = this->AxisLabels[axis];
    if (value != nullptr)
    {
      value->Register(this);
    }
...
// ****************************************************************************
void svtkCubeAxesActor::SetLabelScaling(bool autoscale, int upowX, int upowY, int upowZ)
{
...
svtkTextProperty* svtkCubeAxesActor::GetTitleTextProperty(int axis)
{
...
svtkTextProperty* svtkCubeAxesActor::GetLabelTextProperty(int axis)
{
...
// ****************************************************************************
void svtkCubeAxesActor::UpdateLabels(svtkAxisActor** axis, int svtkNotUsed(index))
{
  for (int i = 0; i < NUMBER_OF_ALIGNED_AXIS; i++)
  {
    int numberOfLabelsBuild = axis[i]->GetNumberOfLabelsBuilt();
    svtkAxisFollower** labelActors = axis[i]->GetLabelActors();
    svtkProp3DAxisFollower** labelProps = axis[i]->GetLabelProps3D();
...
// ****************************************************************************
void svtkCubeAxesActor::SetXAxesLinesProperty(svtkProperty* prop)
{
  this->XAxesLinesProperty->DeepCopy(prop);
  this->Modified();
}
void svtkCubeAxesActor::SetYAxesLinesProperty(svtkProperty* prop)
{
  this->YAxesLinesProperty->DeepCopy(prop);
  this->Modified();
}
void svtkCubeAxesActor::SetZAxesLinesProperty(svtkProperty* prop)
{
...
// ****************************************************************************
svtkProperty* svtkCubeAxesActor::GetXAxesLinesProperty()
{
  return this->XAxesLinesProperty;
}
svtkProperty* svtkCubeAxesActor::GetYAxesLinesProperty()
{
  return this->YAxesLinesProperty;
}
svtkProperty* svtkCubeAxesActor::GetZAxesLinesProperty()
{
  return this->ZAxesLinesProperty;
}

// ****************************************************************************
void svtkCubeAxesActor::SetXAxesGridlinesProperty(svtkProperty* prop)
{
  this->XAxesGridlinesProperty->DeepCopy(prop);
  this->Modified();
}
void svtkCubeAxesActor::SetYAxesGridlinesProperty(svtkProperty* prop)
{
  this->YAxesGridlinesProperty->DeepCopy(prop);
  this->Modified();
}
void svtkCubeAxesActor::SetZAxesGridlinesProperty(svtkProperty* prop)
{
...
// ****************************************************************************
svtkProperty* svtkCubeAxesActor::GetXAxesGridlinesProperty()
{
  return this->XAxesGridlinesProperty;
}
svtkProperty* svtkCubeAxesActor::GetYAxesGridlinesProperty()
{
  return this->YAxesGridlinesProperty;
}
svtkProperty* svtkCubeAxesActor::GetZAxesGridlinesProperty()
{
  return this->ZAxesGridlinesProperty;
}

// ****************************************************************************
void svtkCubeAxesActor::SetXAxesInnerGridlinesProperty(svtkProperty* prop)
{
  this->XAxesInnerGridlinesProperty->DeepCopy(prop);
  this->Modified();
}
void svtkCubeAxesActor::SetYAxesInnerGridlinesProperty(svtkProperty* prop)
{
  this->YAxesInnerGridlinesProperty->DeepCopy(prop);
  this->Modified();
}
void svtkCubeAxesActor::SetZAxesInnerGridlinesProperty(svtkProperty* prop)
{
...
// ****************************************************************************
svtkProperty* svtkCubeAxesActor::GetXAxesInnerGridlinesProperty()
{
  return this->XAxesInnerGridlinesProperty;
}
svtkProperty* svtkCubeAxesActor::GetYAxesInnerGridlinesProperty()
{
  return this->YAxesInnerGridlinesProperty;
}
svtkProperty* svtkCubeAxesActor::GetZAxesInnerGridlinesProperty()
{
  return this->ZAxesInnerGridlinesProperty;
}

// ****************************************************************************
void svtkCubeAxesActor::SetXAxesGridpolysProperty(svtkProperty* prop)
{
  this->XAxesGridpolysProperty->DeepCopy(prop);
  this->Modified();
}
void svtkCubeAxesActor::SetYAxesGridpolysProperty(svtkProperty* prop)
{
  this->YAxesGridpolysProperty->DeepCopy(prop);
  this->Modified();
}
void svtkCubeAxesActor::SetZAxesGridpolysProperty(svtkProperty* prop)
{
...
// ****************************************************************************
svtkProperty* svtkCubeAxesActor::GetXAxesGridpolysProperty()
{
  return this->XAxesGridpolysProperty;
}
svtkProperty* svtkCubeAxesActor::GetYAxesGridpolysProperty()
{
  return this->YAxesGridpolysProperty;
}
svtkProperty* svtkCubeAxesActor::GetZAxesGridpolysProperty()
{
  return this->ZAxesGridpolysProperty;
}
// --------------------------------------------------------------------------
void svtkCubeAxesActor::UpdateGridLineVisibility(int idx)
{
  if (this->GridLineLocation != SVTK_GRID_LINES_ALL &&
    (this->DrawXGridlines || this->DrawYGridlines || this->DrawZGridlines))
  {
    ...
    this->XAxes[svtkCubeAxesActorTriads[idx][0]]->SetDrawGridlines(this->DrawXGridlines);
    this->YAxes[svtkCubeAxesActorTriads[idx][1]]->SetDrawGridlines(this->DrawYGridlines);
    this->ZAxes[svtkCubeAxesActorTriads[idx][2]]->SetDrawGridlines(this->DrawZGridlines);

    // Update axis render list
    int id = 0;
    if (this->NumberOfAxesX == 1)
    {
      id = this->RenderAxesX[this->NumberOfAxesX] = svtkCubeAxesActorTriads[idx][0];
    ...
    {
      id = this->RenderAxesY[this->NumberOfAxesY] = svtkCubeAxesActorTriads[idx][1];
    ...
    {
      id = this->RenderAxesZ[this->NumberOfAxesZ] = svtkCubeAxesActorTriads[idx][2];
...
// --------------------------------------------------------------------------
int svtkCubeAxesActor::FindClosestAxisIndex(double pts[8][3])
{
  // Loop over points and find the closest point to the camera
  double min = SVTK_FLOAT_MAX;
  int idx = 0;
  for (int i = 0; i < 8; i++)
  {
    if (pts[i][2] < min)
...
// --------------------------------------------------------------------------
int svtkCubeAxesActor::FindFurtherstAxisIndex(double pts[8][3])
{
  // Loop over points and find the furthest point from the camera
  double max = -SVTK_FLOAT_MAX;
  int idx = 0;
  for (int i = 0; i < 8; i++)
  {
    if (pts[i][2] > max)
...
// --------------------------------------------------------------------------
void svtkCubeAxesActor::FindBoundaryEdge(int& xloc, int& yloc, int& zloc, double pts[8][3])
{
  ...
  minSlope = SVTK_FLOAT_MAX;
  for (xIdx = 0, i = 0; i < 3; i++)
  {
    num = (pts[svtkCubeAxesActorConn[idx][i]][1] - pts[idx][1]);
    den = (pts[svtkCubeAxesActorConn[idx][i]][0] - pts[idx][0]);
    if (den != 0.0)
    {
      slope = num / den;
    }
    ...
    {
      xIdx = svtkCubeAxesActorConn[idx][i];
      yIdx = svtkCubeAxesActorConn[idx][(i + 1) % 3];
      zIdx = svtkCubeAxesActorConn[idx][(i + 2) % 3];
      xAxes = i;
      minSlope = slope;
    }
  }
  ...
  svtkMath::Normalize(e3);

  if (svtkMath::Dot(e1, e2) < svtkMath::Dot(e1, e3))
  {
    yAxes = (xAxes + 1) % 3;
  }
  else
  ...
  if (pts[svtkCubeAxesActorConn[xIdx][zAxes]][2] < pts[svtkCubeAxesActorConn[yIdx][zAxes]][2])
  {
    zIdx = xIdx;
    zIdx2 = svtkCubeAxesActorConn[xIdx][zAxes];
  }
  else
  {
    zIdx = yIdx;
    zIdx2 = svtkCubeAxesActorConn[yIdx][zAxes];
  }
  ...
  {
    case 0:
      xloc = svtkCubeAxesActorTriads[mini][0];
      break;
    case 1:
      yloc = svtkCubeAxesActorTriads[mini][1];
      break;
    case 2:
      zloc = svtkCubeAxesActorTriads[mini][2];
      break;
  }
...
// --------------------------------------------------------------------------
int svtkCubeAxesActor::RenderGeometry(bool& initialRender, svtkViewport* viewport,
  bool checkAxisVisibility, int (svtkAxisActor::*renderMethod)(svtkViewport*))
{
  ...
  svtkInformation* propKeys = this->GetPropertyKeys();
...
// --------------------------------------------------------------------------
void svtkCubeAxesActor::ComputeStickyAxesBoundingSphere(svtkViewport* viewport,
  const double originalBounds[6], double sphereCenter[3], double& sphereRadius)
{
  double aspect[2];
  viewport->GetAspect(aspect);
  svtkPlanes* frustumPlanes = svtkPlanes::New();
  ...
  svtkFrustumSource* frustumSource = svtkFrustumSource::New();
  frustumSource->SetPlanes(frustumPlanes);
  frustumPlanes->Delete();
  frustumSource->Update();

  svtkPoints* points = frustumSource->GetOutput()->GetPoints();
  ...
  double view[3];
  svtkMath::Subtract(p0, q0, view);
  double d = svtkMath::Norm(view);

  double v0[3], v1[3];
  svtkMath::Subtract(p1, q1, v0);
  svtkMath::Subtract(q2, q1, v1);
  double l = 0.5 * svtkMath::Norm(v1);
  double alpha = atan(svtkMath::Dot(v0, v1) / (d * svtkMath::Norm(v1)));
  double halfWidth = l * tan((svtkMath::Pi() - 2.0 * alpha) / 4.0);

  svtkMath::Subtract(q3, q1, v1);
  l = 0.5 * svtkMath::Norm(v1);
  alpha = atan(svtkMath::Dot(v0, v1) / (d * svtkMath::Norm(v1)));
  double halfHeight = l * tan((svtkMath::Pi() - 2.0 * alpha) / 4.0);

  sphereRadius = std::min(halfWidth, halfHeight);

  svtkMath::Normalize(view);
  ...
  double f = svtkMath::Dot(q0, sidePlane) + sidePlane[3];

  svtkBoundingBox bb(originalBounds);
  double bbCenter[3];
  bb.GetCenter(bbCenter);
  double* backPlane = frustumPlanesArray + 16;
  double g = svtkMath::Dot(bbCenter, backPlane) + backPlane[3];
  ...
  if (viewport->GetSize()[0] < viewport->GetSize()[1])
  {
    svtkMath::Subtract(q1, q3, shiftDirection); // up vector
  ...
  {
    svtkMath::Subtract(q1, q2, shiftDirection); // right vector
  ...
  svtkMath::Normalize(shiftDirection);

  double v[3], shift[3], newCenter[3];
  svtkMath::Subtract(bbCenter, sphereCenter, v);
  svtkMath::ProjectVector(v, shiftDirection, shift);
  svtkMath::Add(sphereCenter, shift, newCenter);
  ...
  if (svtkMath::Dot(minusSide, newCenter) + minusSide[3] < 0.0)
  {
    // Intersection with shifted bottom side
    double t = -(svtkMath::Dot(minusSide, newCenter) + minusSide[3]) /
      (svtkMath::Dot(minusSide, shiftDirection));
  ...
  if (svtkMath::Dot(plusSide, newCenter) + plusSide[3] < 0.0)
  {
    // Intersection with shifted top side
    double t =
      -(svtkMath::Dot(plusSide, newCenter) + plusSide[3]) / (svtkMath::Dot(plusSide, shiftDirection));
    sphereCenter[0] = newCenter[0] + t * shiftDirection[0];
    sphereCenter[1] = newCenter[1] + t * shiftDirection[1];
    sphereCenter[2] = newCenter[2] + t * shiftDirection[2];
  }
...
// --------------------------------------------------------------------------
void svtkCubeAxesActor::GetViewportLimitedBounds(svtkViewport* viewport, double bounds[6])
{
  double originalBounds[6];
  this->GetBounds(originalBounds);
  svtkBoundingBox originalBB(originalBounds);
...
// --------------------------------------------------------------------------
void svtkCubeAxesActor::GetBoundsPointBits(
...
// --------------------------------------------------------------------------
void svtkCubeAxesActor::GetBoundsPoint(
  unsigned int pointIndex, const double bounds[6], double point[3])
{
  ...
  svtkCubeAxesActor::GetBoundsPointBits(pointIndex, xBit, yBit, zBit);
...
// --------------------------------------------------------------------------
void svtkCubeAxesActor::GetRenderedBounds(double* b)
{
  svtkBoundingBox bbox(this->GetBounds()); // Data bounds
...
// --------------------------------------------------------------------------
double* svtkCubeAxesActor::GetRenderedBounds()
{
