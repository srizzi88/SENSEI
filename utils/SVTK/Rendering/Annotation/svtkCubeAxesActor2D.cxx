/*=========================================================================
  ...
  Module:    svtkCubeAxesActor2D.cxx
...
svtkStandardNewMacro(svtkCubeAxesActor2D);

svtkCxxSetObjectMacro(svtkCubeAxesActor2D, Camera, svtkCamera);
svtkCxxSetObjectMacro(svtkCubeAxesActor2D, ViewProp, svtkProp);
svtkCxxSetObjectMacro(svtkCubeAxesActor2D, AxisLabelTextProperty, svtkTextProperty);
svtkCxxSetObjectMacro(svtkCubeAxesActor2D, AxisTitleTextProperty, svtkTextProperty);

class svtkCubeAxesActor2DConnection : public svtkAlgorithm
{
public:
  static svtkCubeAxesActor2DConnection* New();
  svtkTypeMacro(svtkCubeAxesActor2DConnection, svtkAlgorithm);

  svtkCubeAxesActor2DConnection() { this->SetNumberOfInputPorts(1); }
};

svtkStandardNewMacro(svtkCubeAxesActor2DConnection);

//----------------------------------------------------------------------------
// Instantiate this object.
svtkCubeAxesActor2D::svtkCubeAxesActor2D()
{
  this->ConnectionHolder = svtkCubeAxesActor2DConnection::New();
  ...
  this->ZAxis = svtkAxisActor2D::New();
  ...
  this->XOrigin = SVTK_DOUBLE_MAX;
  this->YOrigin = SVTK_DOUBLE_MAX;
  this->ZOrigin = SVTK_DOUBLE_MAX;
}

//----------------------------------------------------------------------------
// Shallow copy of an actor.
void svtkCubeAxesActor2D::ShallowCopy(svtkCubeAxesActor2D* actor)
{
  this->svtkActor2D::ShallowCopy(actor);
...
//----------------------------------------------------------------------------
void svtkCubeAxesActor2D::SetInputConnection(svtkAlgorithmOutput* ao)
{
  this->ConnectionHolder->SetInputConnection(ao);
}

//----------------------------------------------------------------------------
void svtkCubeAxesActor2D::SetInputData(svtkDataSet* ds)
{
...
//----------------------------------------------------------------------------
svtkDataSet* svtkCubeAxesActor2D::GetInput()
{
  return svtkDataSet::SafeDownCast(this->ConnectionHolder->GetInputDataObject(0, 0));
}
...
int svtkCubeAxesActor2D::RenderOverlay(svtkViewport* viewport)
{
...
// cube. Determine which parts of the edges are visible via intersection
// with the boundary of the viewport (minus borders).
int svtkCubeAxesActor2D::RenderOpaqueGeometry(svtkViewport* viewport)
{
  double bounds[6];
  ...
  if (this->XOrigin != SVTK_DOUBLE_MAX)
  {
    bounds[0] = this->XOrigin;
  }
  if (this->YOrigin != SVTK_DOUBLE_MAX)
  {
    bounds[2] = this->YOrigin;
  }
  if (this->ZOrigin != SVTK_DOUBLE_MAX)
  {
  ...
  if (this->RenderCount++ == 0 || !(this->RenderCount % this->Inertia))
  {
    ...
    else if (this->FlyMode == SVTK_FLY_CLOSEST_TRIAD)
    {
    ...
    else if (this->FlyMode == SVTK_FLY_OUTER_EDGES)
    {
      double e1[2], e2[2], e3[2];

      // Find distance to origin
      d2Min = SVTK_DOUBLE_MAX;
      for (i = 0; i < 8; i++)
      {
      ...
      svtkMath::Normalize2D(e1);
      svtkMath::Normalize2D(e2);
      svtkMath::Normalize2D(e3);

      if (svtkMath::Dot2D(e1, e2) < svtkMath::Dot2D(e1, e3))
      {
        yAxes = (xAxes + 1) % 3;
      }
      else
...
svtkTypeBool svtkCubeAxesActor2D::HasTranslucentPolygonalGeometry()
{
  return 0;
}
...
void svtkCubeAxesActor2D::AdjustAxes(double pts[8][3], double bounds[6], int idx, int xIdx, int yIdx,
...
void svtkCubeAxesActor2D::ReleaseGraphicsResources(svtkWindow* win)
{
...
//----------------------------------------------------------------------------
// Return the ranges
void svtkCubeAxesActor2D::GetRanges(double ranges[6])
{
  int i;
  for (i = 0; i < 6; i++)
  {
...
//----------------------------------------------------------------------------
// Compute the ranges
void svtkCubeAxesActor2D::GetRanges(
...
//----------------------------------------------------------------------------
// Compute the bounds
double* svtkCubeAxesActor2D::GetRanges()
{
  double ranges[6];
  this->GetRanges(ranges);
  return this->Ranges;
...
//----------------------------------------------------------------------------
// Compute the bounds
void svtkCubeAxesActor2D::GetBounds(double bounds[6])
{
...
//----------------------------------------------------------------------------
// Compute the bounds
void svtkCubeAxesActor2D::GetBounds(
  double& xmin, double& xmax, double& ymin, double& ymax, double& zmin, double& zmax)
{
  double bounds[6];
  this->GetBounds(bounds);
...
//----------------------------------------------------------------------------
// Compute the bounds
double* svtkCubeAxesActor2D::GetBounds()
{
...
//----------------------------------------------------------------------------
void svtkCubeAxesActor2D::PrintSelf(ostream& os, svtkIndent indent)
{
  ...
  else if (this->FlyMode == SVTK_FLY_NONE)
  {
...
#define SVTK_DIVS 10
int svtkCubeAxesActor2D::ClipBounds(svtkViewport* viewport, double pts[8][3], double bounds[6])
{
  int i, j, k, numIters;
  double planes[24];
  double x[3];
  ...
  delX = (bounds[1] - bounds[0]) / (SVTK_DIVS - 1);
  delY = (bounds[3] - bounds[2]) / (SVTK_DIVS - 1);
  delZ = (bounds[5] - bounds[4]) / (SVTK_DIVS - 1);
  ...
  {
    origin[0] = anchor[0] - delX * (SVTK_DIVS - 1) / 2.0;
    origin[1] = anchor[1] - delY * (SVTK_DIVS - 1) / 2.0;
    origin[2] = anchor[2] - delZ * (SVTK_DIVS - 1) / 2.0;

    for (maxVal = 0.0, k = 0; k < SVTK_DIVS; k++)
    {
      x[2] = origin[2] + k * delZ;
      for (j = 0; j < SVTK_DIVS; j++)
      {
        x[1] = origin[1] + j * delY;
        for (i = 0; i < SVTK_DIVS; i++)
        {
          x[0] = origin[0] + i * delX;
    ...
    delX /= (SVTK_DIVS - 1) * 1.414;
    delY /= (SVTK_DIVS - 1) * 1.414;
    delZ /= (SVTK_DIVS - 1) * 1.414;
...
//----------------------------------------------------------------------------
void svtkCubeAxesActor2D::TransformBounds(svtkViewport* viewport, double bounds[6], double pts[8][3])
{
  int i, j, k, idx;
  double x[3];
...
double svtkCubeAxesActor2D::EvaluatePoint(double planes[24], double x[3])
{
  int kk;
  double* plane;
  double val, minPlanesValue;

  for (minPlanesValue = SVTK_DOUBLE_MAX, kk = 0; kk < 6; kk++)
  {
    plane = planes + kk * 4;
...
// frustum planes.
double svtkCubeAxesActor2D::EvaluateBounds(double planes[24], double bounds[6])
{
  double val, minVal, x[3];
  int i, j, k;

  for (minVal = SVTK_DOUBLE_MAX, k = 0; k < 2; k++)
  {
    x[2] = bounds[4 + k];
    for (j = 0; j < 2; j++)
    {
