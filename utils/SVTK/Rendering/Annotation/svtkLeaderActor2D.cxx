...
svtkStandardNewMacro(svtkLeaderActor2D);

svtkCxxSetObjectMacro(svtkLeaderActor2D, LabelTextProperty, svtkTextProperty);

//----------------------------------------------------------------------------
// Instantiate this object.
svtkLeaderActor2D::svtkLeaderActor2D()
{
  ...
  this->ArrowPlacement = svtkLeaderActor2D::SVTK_ARROW_BOTH;
  this->ArrowStyle = svtkLeaderActor2D::SVTK_ARROW_FILLED;
  ...
  this->LabelMapper = svtkTextMapper::New();
...
//----------------------------------------------------------------------------
void svtkLeaderActor2D::BuildLeader(svtkViewport* viewport)
{
  ...
  // arrow head(s) if any.
  svtkDebugMacro(<< "Rebuilding leader"); //---------------------------------
  ...
  ray[1] = p2[1] - p1[1];
  ray[2] = 0.0;
  double rayLength = svtkMath::Norm(ray);
  if (rayLength <= 0.0)
  {
    return;
  }
  ...
  this->Length = sqrt(svtkMath::Distance2BetweenPoints(x1, x2));
  ...
  // Build the arrows---------------------------------------
  if (this->ArrowPlacement == svtkLeaderActor2D::SVTK_ARROW_NONE)
  {
    ; // do nothin
  }
  else // we are creating arrows
  {
    ...
    if (this->ArrowPlacement == svtkLeaderActor2D::SVTK_ARROW_POINT1 ||
      this->ArrowPlacement == svtkLeaderActor2D::SVTK_ARROW_BOTH)
    {
      ...
      if (this->ArrowStyle == svtkLeaderActor2D::SVTK_ARROW_FILLED)
      {
      ...
      else if (this->ArrowStyle == svtkLeaderActor2D::SVTK_ARROW_OPEN)
      {
      ...
      else // if ( this->ArrowStyle == svtkLeaderActor2D::SVTK_ARROW_HOLLOW )
      {
    ...
    if (this->ArrowPlacement == svtkLeaderActor2D::SVTK_ARROW_POINT2 ||
      this->ArrowPlacement == svtkLeaderActor2D::SVTK_ARROW_BOTH)
    {
...
//----------------------------------------------------------------------------
#define SVTK_LA2D_FACTOR 0.015
int svtkLeaderActor2D::SetFontSize(svtkViewport* viewport, svtkTextMapper* textMapper,
  const int* targetSize, double factor, int* stringSize)
{
  ...
  targetHeight = static_cast<int>(
    SVTK_LA2D_FACTOR * factor * targetSize[0] + SVTK_LA2D_FACTOR * factor * targetSize[1]);
...
#undef SVTK_LA2D_FACTOR

//----------------------------------------------------------------------------
int svtkLeaderActor2D::ClipLeader(
...
//----------------------------------------------------------------------------
void svtkLeaderActor2D::BuildCurvedLeader(double p1[3], double p2[3], double ray[3],
  double rayLength, double theta, svtkViewport* viewport, int viewportChanged)
{
  ...
  if ((theta1 >= 0.0 && theta1 <= svtkMath::Pi() && theta2 >= 0.0 && theta2 <= svtkMath::Pi()) ||
    (theta1 <= 0.0 && theta1 >= -svtkMath::Pi() && theta2 <= 0.0 && theta2 >= -svtkMath::Pi()))
  {
    ; // do nothin angles are fine
  }
  else if (theta1 >= 0.0 && theta2 <= 0.0)
  {
    if ((theta1 - theta2) >= svtkMath::Pi())
    {
      theta2 = theta2 + 2.0 * svtkMath::Pi();
    }
  }
  else // if ( theta1 <= 0.0 && theta2 >= 0.0 )
  {
    if ((theta2 - theta1) >= svtkMath::Pi())
    {
      theta1 = theta1 + 2.0 * svtkMath::Pi();
    }
  }
  ...
  this->Angle = svtkMath::DegreesFromRadians(theta1 - theta2);
  if (this->AutoLabel || (this->Label != nullptr && this->Label[0] != 0))
  {
    int stringSize[2];
    if (this->AutoLabel)
...
int svtkLeaderActor2D::InStringBox(double center[3], int stringSize[2], double x[3])
{
...
void svtkLeaderActor2D::ReleaseGraphicsResources(svtkWindow* win)
{
...
int svtkLeaderActor2D::RenderOpaqueGeometry(svtkViewport* viewport)
{
...
int svtkLeaderActor2D::RenderOverlay(svtkViewport* viewport)
{
  int renderedSomething = 0;
  this->BuildLeader(viewport);
...
svtkTypeBool svtkLeaderActor2D::HasTranslucentPolygonalGeometry()
{
  return 0;
}

//----------------------------------------------------------------------------
void svtkLeaderActor2D::ShallowCopy(svtkProp* prop)
{
  svtkLeaderActor2D* a = svtkLeaderActor2D::SafeDownCast(prop);
  if (a != nullptr)
  {
...
//----------------------------------------------------------------------------
void svtkLeaderActor2D::PrintSelf(ostream& os, svtkIndent indent)
{
  ...
  if (this->ArrowStyle == SVTK_ARROW_FILLED)
  {
    os << "Filled\n";
  }
  else if (this->ArrowStyle == SVTK_ARROW_OPEN)
  {
    os << "Open\n";
  }
  else // if ( this->ArrowStyle == SVTK_ARROW_HOLLOW )
  {
    os << "Hollow\n";
  }
  ...
  if (this->ArrowPlacement == SVTK_ARROW_NONE)
  {
    os << "No Arrows\n";
  }
  else if (this->ArrowPlacement == SVTK_ARROW_POINT1)
  {
    os << "Arrow on first point\n";
  }
  else if (this->ArrowPlacement == SVTK_ARROW_POINT2)
  {
