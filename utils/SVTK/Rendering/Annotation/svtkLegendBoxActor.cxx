...
svtkStandardNewMacro(svtkLegendBoxActor);

svtkCxxSetObjectMacro(svtkLegendBoxActor, EntryTextProperty, svtkTextProperty);

//----------------------------------------------------------------------------
svtkLegendBoxActor::svtkLegendBoxActor()
{
  ...
  this->EntryTextProperty = svtkTextProperty::New();
  this->EntryTextProperty->SetBold(0);
  this->EntryTextProperty->SetItalic(0);
  this->EntryTextProperty->SetShadow(0);
  this->EntryTextProperty->SetFontFamily(SVTK_ARIAL);
  this->EntryTextProperty->SetJustification(SVTK_TEXT_LEFT);
  this->EntryTextProperty->SetVerticalJustification(SVTK_TEXT_CENTERED);
  ...
  // Construct the box
  this->BoxPolyData = svtkPolyData::New();
  ...
  this->Background = svtkPlaneSource::New();
  this->BackgroundActor = svtkTexturedActor2D::New();
...
//----------------------------------------------------------------------------
void svtkLegendBoxActor::InitializeEntries()
{
  int i;
...
//----------------------------------------------------------------------------
void svtkLegendBoxActor::SetNumberOfEntries(int num)
{
  ...
  else // allocate space
  {
    ...
    svtkDoubleArray* colors = svtkDoubleArray::New();
    colors->SetNumberOfComponents(3);
    colors->SetNumberOfTuples(num);
    svtkTextMapper** textMapper = new svtkTextMapper*[num];
    svtkActor2D** textActor = new svtkActor2D*[num];

    // Symbol
    svtkPolyData** symbol = new svtkPolyData*[num];
    svtkTransform** transform = new svtkTransform*[num];
    svtkTransformPolyDataFilter** symbolTransform = new svtkTransformPolyDataFilter*[num];
    svtkPolyDataMapper2D** symbolMapper = new svtkPolyDataMapper2D*[num];
    svtkActor2D** symbolActor = new svtkActor2D*[num];

    // Icon.
    svtkPlaneSource** icon = new svtkPlaneSource*[num];
    svtkTransform** iconTransform = new svtkTransform*[num];
    svtkTransformPolyDataFilter** iconTransformFilter = new svtkTransformPolyDataFilter*[num];
    svtkPolyDataMapper2D** iconMapper = new svtkPolyDataMapper2D*[num];
    svtkTexturedActor2D** iconActor = new svtkTexturedActor2D*[num];
    svtkImageData** iconImage = new svtkImageData*[num];
...
//----------------------------------------------------------------------------
void svtkLegendBoxActor::SetEntry(int i, svtkPolyData* symbol, const char* string, double color[3])
{
...
//----------------------------------------------------------------------------
void svtkLegendBoxActor::SetEntry(int i, svtkImageData* icon, const char* string, double color[3])
{
  if (i >= 0 && i < this->NumberOfEntries)
  {
...
//----------------------------------------------------------------------------
void svtkLegendBoxActor::SetEntry(
...
//----------------------------------------------------------------------------
void svtkLegendBoxActor::SetEntrySymbol(int i, svtkPolyData* symbol)
{
  if (i >= 0 && i < this->NumberOfEntries)
  {
...
//----------------------------------------------------------------------------
void svtkLegendBoxActor::SetEntryIcon(int i, svtkImageData* icon)
{
  if (i >= 0 && i < this->NumberOfEntries)
  {
...
//----------------------------------------------------------------------------
void svtkLegendBoxActor::SetEntryString(int i, const char* string)
{
  if (i >= 0 && i < this->NumberOfEntries)
  {
...
//----------------------------------------------------------------------------
void svtkLegendBoxActor::SetEntryColor(int i, double color[3])
{
  if (i >= 0 && i < this->NumberOfEntries)
  {
    double oldColor[3];
...
//----------------------------------------------------------------------------
void svtkLegendBoxActor::SetEntryColor(int i, double r, double g, double b)
{
  double rgb[3];
  rgb[0] = r;
  rgb[1] = g;
...
//----------------------------------------------------------------------------
svtkPolyData* svtkLegendBoxActor::GetEntrySymbol(int i)
{
  if (i < 0 || i >= this->NumberOfEntries)
  {
    return nullptr;
...
//----------------------------------------------------------------------------
svtkImageData* svtkLegendBoxActor::GetEntryIcon(int i)
{
  if (i < 0 || i >= this->NumberOfEntries)
  {
    return nullptr;
...
//----------------------------------------------------------------------------
const char* svtkLegendBoxActor::GetEntryString(int i)
{
  if (i < 0 || i >= this->NumberOfEntries)
  {
    return nullptr;
...
//----------------------------------------------------------------------------
double* svtkLegendBoxActor::GetEntryColor(int i)
{
  if (i < 0 || i >= this->NumberOfEntries)
  {
    return nullptr;
  ...
  {
    return svtkArrayDownCast<svtkDoubleArray>(this->Colors)->GetPointer(i * 3);
  }
}
...
void svtkLegendBoxActor::ReleaseGraphicsResources(svtkWindow* win)
{
  if (this->BackgroundActor)
  {
...
//----------------------------------------------------------------------------
int svtkLegendBoxActor::RenderOverlay(svtkViewport* viewport)
{
...
//----------------------------------------------------------------------------
int svtkLegendBoxActor::RenderOpaqueGeometry(svtkViewport* viewport)
{
  int i;
  double symbolSize;
  ...
  {
    svtkErrorMacro(<< "Need entry text property to render legend box actor");
    return 0;
  }
  ...
  {
    svtkDebugMacro(<< "Rebuilding text");
    ...
    for (i = 0; i < this->NumberOfEntries; i++)
    {
      ...
      this->TextMapper[i]->GetTextProperty()->SetVerticalJustification(SVTK_TEXT_CENTERED);
      this->TextMapper[i]->GetTextProperty()->SetJustification(SVTK_TEXT_LEFT);
...
svtkTypeBool svtkLegendBoxActor::HasTranslucentPolygonalGeometry()
{
  return 0;
}

//----------------------------------------------------------------------------
void svtkLegendBoxActor::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
void svtkLegendBoxActor::ShallowCopy(svtkProp* prop)
{
  svtkLegendBoxActor* a = svtkLegendBoxActor::SafeDownCast(prop);
  if (a != nullptr)
  {
