/*=========================================================================
  ...
  Module:    svtkParallelCoordinatesActor.cxx
...
=========================================================================*/
#include "svtkParallelCoordinatesActor.h"
...
svtkStandardNewMacro(svtkParallelCoordinatesActor);

svtkCxxSetObjectMacro(svtkParallelCoordinatesActor, LabelTextProperty, svtkTextProperty);
svtkCxxSetObjectMacro(svtkParallelCoordinatesActor, TitleTextProperty, svtkTextProperty);

class svtkParallelCoordinatesActorConnection : public svtkAlgorithm
{
public:
  static svtkParallelCoordinatesActorConnection* New();
  svtkTypeMacro(svtkParallelCoordinatesActorConnection, svtkAlgorithm);

  svtkParallelCoordinatesActorConnection() { this->SetNumberOfInputPorts(1); }
};

svtkStandardNewMacro(svtkParallelCoordinatesActorConnection);

//----------------------------------------------------------------------------
// Instantiate object
svtkParallelCoordinatesActor::svtkParallelCoordinatesActor()
{
  ...
  this->ConnectionHolder = svtkParallelCoordinatesActorConnection::New();
...
void svtkParallelCoordinatesActor::Initialize()
{
  if (this->Axes)
  {
...
//----------------------------------------------------------------------------
void svtkParallelCoordinatesActor::SetInputConnection(svtkAlgorithmOutput* ao)
{
  this->ConnectionHolder->SetInputConnection(ao);
}

//----------------------------------------------------------------------------
void svtkParallelCoordinatesActor::SetInputData(svtkDataObject* dobj)
{
...
//----------------------------------------------------------------------------
svtkDataObject* svtkParallelCoordinatesActor::GetInput()
{
...
int svtkParallelCoordinatesActor::RenderOverlay(svtkViewport* viewport)
{
...
//----------------------------------------------------------------------------
int svtkParallelCoordinatesActor::RenderOpaqueGeometry(svtkViewport* viewport)
{
  int renderedSomething = 0;

  // Initialize

  svtkDebugMacro(<< "Plotting parallel coordinates");
...
svtkTypeBool svtkParallelCoordinatesActor::HasTranslucentPolygonalGeometry()
{
  return 0;
}

//----------------------------------------------------------------------------
static inline int svtkParallelCoordinatesActorGetComponent(
...
//----------------------------------------------------------------------------
int svtkParallelCoordinatesActor::PlaceAxes(svtkViewport* viewport, const int* svtkNotUsed(size))
{
  svtkIdType i, j, k, ptId;
  ...
  if (this->IndependentVariables == SVTK_IV_COLUMN)
  {
    ...
    for (j = 0; j < numComponents; j++)
    {
      ...
      for (i = 0; i < numRows; i++)
      {
        ...
        ::svtkParallelCoordinatesActorGetComponent(field, i, j, &v);
        if (v < this->Mins[k])
        {
          this->Mins[k] = v;
        }
  ...
  else // row
  {
    for (j = 0; j < numRows; j++)
    {
      for (i = 0; i < numComponents; i++)
      {
        ...
        if (::svtkParallelCoordinatesActorGetComponent(field, j, i, &v) == 0)
        {
  ...
  this->Axes = new svtkAxisActor2D*[this->N];
  for (i = 0; i < this->N; i++)
  {
    this->Axes[i] = svtkAxisActor2D::New();
    ...
    // in that class and we know that svtkAxisActor2D use ShallowCopy internally
  ...
  if (this->IndependentVariables == SVTK_IV_COLUMN)
  {
    ...
    for (j = 0; j < numRows; j++)
    {
      ...
      {
        // v = field->GetComponent(j,i);
        if (::svtkParallelCoordinatesActorGetComponent(field, j, k, &v) == 0)
        {
...
void svtkParallelCoordinatesActor::ReleaseGraphicsResources(svtkWindow* win)
{
...
//----------------------------------------------------------------------------
void svtkParallelCoordinatesActor::PrintSelf(ostream& os, svtkIndent indent)
{
