...
#define SVTK_EXPONENT_AXES_ACTOR_RTOL (1. - 10. * SVTK_DBL_EPSILON)

svtkStandardNewMacro(svtkPolarAxesActor);
svtkCxxSetObjectMacro(svtkPolarAxesActor, Camera, svtkCamera);
svtkCxxSetObjectMacro(svtkPolarAxesActor, PolarAxisLabelTextProperty, svtkTextProperty);
svtkCxxSetObjectMacro(svtkPolarAxesActor, PolarAxisTitleTextProperty, svtkTextProperty);
svtkCxxSetObjectMacro(svtkPolarAxesActor, LastRadialAxisTextProperty, svtkTextProperty);
svtkCxxSetObjectMacro(svtkPolarAxesActor, SecondaryRadialAxesTextProperty, svtkTextProperty);
svtkCxxSetObjectMacro(svtkPolarAxesActor, LastRadialAxisProperty, svtkProperty);
svtkCxxSetObjectMacro(svtkPolarAxesActor, SecondaryRadialAxesProperty, svtkProperty);

//-----------------------------------------------------------------------------
void svtkPolarAxesActor::PrintSelf(ostream& os, svtkIndent indent)
{
  ...
  if (this->PolarAxisTitleLocation == SVTK_TITLE_BOTTOM)
  {
    os << indent << "Polar Title Location: BOTTOM" << endl;
  }
  else if (this->PolarAxisTitleLocation == SVTK_TITLE_EXTERN)
  {
  ...
  if (this->ExponentLocation == SVTK_EXPONENT_BOTTOM)
  {
    os << " next to the polar axis title." << endl;
  }
  else if (this->ExponentLocation == SVTK_EXPONENT_EXTERN)
  {
    os << " outer side." << endl;
  }
  else
  ...
  os << indent << "Radial Title Visibility: " << (this->RadialTitleVisibility ? "On" : "Off")
     << endl;
  if (this->RadialAxisTitleLocation == SVTK_TITLE_BOTTOM)
  {
    os << indent << "Radial Title Location: BOTTOM" << endl;
  }
  else if (this->RadialAxisTitleLocation == SVTK_TITLE_EXTERN)
  {
...
//-----------------------------------------------------------------------------
svtkPolarAxesActor::svtkPolarAxesActor()
  : svtkActor()
{
  // Default bounds
  this->Bounds[0] = -1.0;
  ...
  this->PolarAxisProperty = svtkProperty::New();
  this->PolarAxisProperty->SetColor(0., 0., 0.);
  this->PolarAxisTitleTextProperty = svtkTextProperty::New();
  ...
  this->PolarAxisLabelTextProperty = svtkTextProperty::New();
  ...
  this->PolarAxis = svtkAxisActor::New();
  ...
  this->LastRadialAxisProperty = svtkProperty::New();
  ...
  this->LastRadialAxisTextProperty = svtkTextProperty::New();
  ...
  this->SecondaryRadialAxesProperty = svtkProperty::New();
  ...
  this->SecondaryRadialAxesTextProperty = svtkTextProperty::New();
  ...
  this->PolarArcs = svtkPolyData::New();
  this->PolarArcsMapper = svtkPolyDataMapper::New();
  this->PolarArcsMapper->SetInputData(this->PolarArcs);
  this->PolarArcsActor = svtkActor::New();
  ...
  this->SecondaryPolarArcs = svtkPolyData::New();
  this->SecondaryPolarArcsMapper = svtkPolyDataMapper::New();
  this->SecondaryPolarArcsMapper->SetInputData(this->SecondaryPolarArcs);
  this->SecondaryPolarArcsActor = svtkActor::New();
  ...
  // Create the svtk Object for arc ticks
  this->ArcMajorTickPts = svtkPoints::New();
  this->ArcMinorTickPts = svtkPoints::New();

  this->ArcTickPolyData = svtkPolyData::New();
  this->ArcMinorTickPolyData = svtkPolyData::New();

  this->ArcTickPolyDataMapper = svtkPolyDataMapper::New();
  this->ArcTickPolyDataMapper->SetInputData(this->ArcTickPolyData);

  this->ArcMinorTickPolyDataMapper = svtkPolyDataMapper::New();
  this->ArcMinorTickPolyDataMapper->SetInputData(this->ArcMinorTickPolyData);

  this->ArcTickActor = svtkActor::New();
  this->ArcTickActor->SetMapper(this->ArcTickPolyDataMapper);

  this->ArcMinorTickActor = svtkActor::New();
  ...
  this->ExponentLocation = SVTK_EXPONENT_LABELS;

  this->RadialAngleFormat = new char[8];
  snprintf(this->RadialAngleFormat, 8, "%s", "%-#3.1f");

  this->RadialAxisTitleLocation = SVTK_TITLE_BOTTOM;
  this->PolarAxisTitleLocation = SVTK_TITLE_BOTTOM;
  ...
  this->TickLocation = svtkAxisActor::SVTK_TICKS_BOTH;
...
//-----------------------------------------------------------------------------
int svtkPolarAxesActor::RenderOpaqueGeometry(svtkViewport* viewport)
{
  // Initialization
  ...
  if (this->RadialAxesVisibility)
  {
    ...
    for (int i = 0; i < this->NumberOfRadialAxes; ++i)
    {
      isInnerAxis = (i != this->NumberOfRadialAxes - 1) ||
        (svtkMathUtilities::FuzzyCompare(MaximumAngle, MinimumAngle));
...
int svtkPolarAxesActor::RenderOverlay(svtkViewport* viewport)
{
  int renderedSomething = 0;
...
//-----------------------------------------------------------------------------
void svtkPolarAxesActor::ReleaseGraphicsResources(svtkWindow* win)
{
...
//-----------------------------------------------------------------------------
void svtkPolarAxesActor::CalculateBounds()
{
  ...
  double thetaMin = svtkMath::RadiansFromDegrees(minAngle);
  double cosThetaMin = cos(thetaMin);
  double sinThetaMin = sin(thetaMin);
  double thetaMax = svtkMath::RadiansFromDegrees(maxAngle);
...
//-----------------------------------------------------------------------------
void svtkPolarAxesActor::GetBounds(double bounds[6])
{
  for (int i = 0; i < 6; i++)
  {
...
//-----------------------------------------------------------------------------
void svtkPolarAxesActor::GetBounds(
...
//-----------------------------------------------------------------------------
double* svtkPolarAxesActor::GetBounds()
{
  return this->Bounds;
}

bool svtkPolarAxesActor::CheckMembersConsistency()
{
  if (this->MaximumAngle > 360.0 || this->MinimumAngle > 360.0)
  {
    // Incorrect MaximumRadius input
    svtkWarningMacro(<< "Cannot draw polar axis, Angle > 360.0: "
  ...
  // Min/Max Radius
  if (svtkMathUtilities::FuzzyCompare(this->MaximumRadius, this->MinimumRadius))
  {
    // MaximumRadius and this->MinimumRadius are too close
    svtkWarningMacro(<< "Maximum and Minimum Radius cannot be distinct: "
  ...
  {
    // Incorrect MaximumRadius input
    svtkWarningMacro(<< "Cannot draw polar axis, Negative Radius value set: "
                    << "MinimumRadius : " << this->MinimumRadius
                    << " _ MaximumRadius: " << this->MaximumRadius);
    return false;
  }
  ...
  // Min/Max Range
  if (svtkMathUtilities::FuzzyCompare(this->Range[0], this->Range[1]))
  {
    // MaximumRadius and this->MinimumRadius are too close
    svtkWarningMacro(<< "Maximum and Minimum Range cannot be distinct: "
  ...
  {
    svtkWarningMacro(<< "Scale Set to Linear. Range value undefined for log scale enabled. "
  ...
  {
    svtkWarningMacro(<< "Arc Delta Angle: "
  ...
  {
    svtkWarningMacro(<< "Delta Angle for radial axes: "
  ...
  if (this->PolarAxisTickRatioThickness < (1.0 / SVTK_MAXIMUM_RATIO) ||
    this->PolarAxisTickRatioThickness > SVTK_MAXIMUM_RATIO ||
    this->LastAxisTickRatioThickness < (1.0 / SVTK_MAXIMUM_RATIO) ||
    this->LastAxisTickRatioThickness > SVTK_MAXIMUM_RATIO ||
    this->ArcTickRatioThickness < (1.0 / SVTK_MAXIMUM_RATIO) ||
    this->ArcTickRatioThickness > SVTK_MAXIMUM_RATIO ||
    this->PolarAxisTickRatioSize < (1.0 / SVTK_MAXIMUM_RATIO) ||
    this->PolarAxisTickRatioSize > SVTK_MAXIMUM_RATIO ||
    this->LastAxisTickRatioSize < (1.0 / SVTK_MAXIMUM_RATIO) ||
    this->LastAxisTickRatioSize > SVTK_MAXIMUM_RATIO ||
    this->ArcTickRatioSize < (1.0 / SVTK_MAXIMUM_RATIO) ||
    this->ArcTickRatioSize > SVTK_MAXIMUM_RATIO)
  {
    svtkWarningMacro(<< "A size/thickness ratio between major and minor ticks is way too large: "
...
//-----------------------------------------------------------------------------
void svtkPolarAxesActor::BuildAxes(svtkViewport* viewport)
{
  ...
  svtkAxisActor* axis = this->PolarAxis;
  ...
  if (this->PolarAxisTitleLocation == SVTK_TITLE_BOTTOM)
  {
    this->PolarAxis->SetTitleAlignLocation(svtkAxisActor::SVTK_ALIGN_BOTTOM);
  }
  else
  {
    this->PolarAxis->SetTitleAlignLocation(svtkAxisActor::SVTK_ALIGN_POINT2);
  }
  ...
  // color copy
  svtkProperty* prop = this->PolarArcsActor->GetProperty();
  double color[3];
  prop->GetColor(color);
  ...
  svtkAxisFollower* follower = axis->GetTitleActor();
  ...
  // Update axis title follower
  svtkAxisFollower* expFollower = this->PolarAxis->GetExponentActor();
  ...
  svtkAxisFollower** labelActors = axis->GetLabelActors();
...
void svtkPolarAxesActor::AutoComputeTicksProperties()
{
...
//-----------------------------------------------------------------------------
void svtkPolarAxesActor::SetCommonAxisAttributes(svtkAxisActor* axis)
{
...
void svtkPolarAxesActor::SetPolarAxisAttributes(svtkAxisActor* axis)
{
  // Set polar axis lines
  axis->SetAxisVisibility(this->PolarAxisVisibility);

  // #### Warning #### : Set this property BEFORE apply the ticks thickness of the svtkAxisActor
  // instances
  ...
  if (this->ExponentLocation == SVTK_EXPONENT_BOTTOM)
  {
    axis->SetExponentLocation(svtkAxisActor::SVTK_ALIGN_BOTTOM);
    axis->SetExponentVisibility(true);
  }
  else if (this->ExponentLocation == SVTK_EXPONENT_EXTERN)
  {
    axis->SetExponentLocation(svtkAxisActor::SVTK_ALIGN_POINT2);
    axis->SetExponentVisibility(true);
  }
  else
  {
...
//-----------------------------------------------------------------------------
inline double svtkPolarAxesActor::FFix(double value)
{
...
//-----------------------------------------------------------------------------
inline double svtkPolarAxesActor::FSign(double value, double sign)
{
  value = fabs(value);
  if (sign < 0.)
  {
...
//-----------------------------------------------------------------------------
void svtkPolarAxesActor::CreateRadialAxes(int axisCount)
{
  ...
  this->RadialAxes = new svtkAxisActor*[this->NumberOfRadialAxes];
  for (int i = 0; i < this->NumberOfRadialAxes; ++i)
  {
    // Create axis of type X
    this->RadialAxes[i] = svtkAxisActor::New();
    svtkAxisActor* axis = this->RadialAxes[i];
    axis->SetAxisTypeToX();
    axis->SetCalculateTitleOffset(0);
    axis->SetCalculateLabelOffset(0);
...
//-----------------------------------------------------------------------------
void svtkPolarAxesActor::BuildRadialAxes()
{
  ...
  if (svtkMathUtilities::FuzzyCompare(this->MaximumAngle, this->MinimumAngle) ||
    angleSection == 360.0)
  {
    angleSection = 360.0;
  }
       ...
       alphaDeg += dAlpha, i++)
  {
    ...
    actualAngle = svtkMath::DegreesFromRadians(atan2(yEnd - this->Pole[1], xEnd - this->Pole[0]));
    ...
    if (this->RadialAxisTitleLocation == SVTK_TITLE_BOTTOM)
    {
      axis->SetTitleAlignLocation(svtkAxisActor::SVTK_ALIGN_BOTTOM);
    }
    else
    {
      axis->SetTitleAlignLocation(svtkAxisActor::SVTK_ALIGN_POINT2);
    }
...
//-----------------------------------------------------------------------------
void svtkPolarAxesActor::BuildArcTicks()
{
  ...
  // set svtk object to draw the ticks
  svtkNew<svtkPoints> majorPts;
  svtkNew<svtkPoints> minorPts;
  svtkNew<svtkCellArray> majorLines;
  svtkNew<svtkCellArray> minorLines;
  svtkIdType ptIds[2];
...
void svtkPolarAxesActor::StoreTicksPtsFromParamEllipse(
  double a, double angleEllipseRad, double tickSize, svtkPoints* tickPts)
{
  ...
  if (this->TickLocation == svtkAxisActor::SVTK_TICKS_INSIDE ||
    this->TickLocation == svtkAxisActor::SVTK_TICKS_BOTH)
  {
    for (i = 0; i < 3; i++)
    {
  ...
  if (this->TickLocation == svtkAxisActor::SVTK_TICKS_OUTSIDE ||
    this->TickLocation == svtkAxisActor::SVTK_TICKS_BOTH)
  {
    for (i = 0; i < 3; i++)
    {
  ...
  svtkIdType nPoints = tickPts->GetNumberOfPoints();
...
//-----------------------------------------------------------------------------
void svtkPolarAxesActor::BuildPolarAxisLabelsArcs()
{
  double angleSection = (this->MaximumAngle > this->MinimumAngle)
    ? this->MaximumAngle - this->MinimumAngle
    : 360.0 - fabs(this->MaximumAngle - this->MinimumAngle);
  ...
  // Prepare trigonometric quantities
  svtkIdType arcResolution =
    static_cast<svtkIdType>(angleSection * (SVTK_POLAR_ARC_RESOLUTION_PER_DEG / this->Ratio));

  // Principal Arc points
  svtkNew<svtkPoints> polarArcsPoints;
  this->PolarArcs->SetPoints(polarArcsPoints);

  // Principal Arc lines
  svtkNew<svtkCellArray> polarArcsLines;
  this->PolarArcs->SetLines(polarArcsLines);

  // Secondary Arc points
  svtkNew<svtkPoints> secondaryPolarArcsPoints;
  this->SecondaryPolarArcs->SetPoints(secondaryPolarArcsPoints);

  // Secondary Arc lines
  svtkNew<svtkCellArray> secondaryPolarArcsLines;
  ...
  while (currentValue < axis->GetRange()[1])
  {
    ...
    if (deltaArc + this->MinimumRadius > 0. && isArcVisible)
    {
      ...
      svtkNew<svtkEllipseArcSource> arc;
  ...
  // set up svtk collection to store labels
  svtkNew<svtkStringArray> labels;

  if (this->ExponentLocation != SVTK_EXPONENT_LABELS)
  {
  ...
  else
  {
    ...
    labels->SetNumberOfValues(static_cast<svtkIdType>(labelValList.size()));
...
//-----------------------------------------------------------------------------
void svtkPolarAxesActor::BuildPolarArcsLog()
{
  double angleSection = (this->MaximumAngle > this->MinimumAngle)
    ? this->MaximumAngle - this->MinimumAngle
    : 360.0 - fabs(this->MaximumAngle - this->MinimumAngle);
  ...
  svtkMath::Subtract(axis->GetPoint2(), axis->GetPoint1(), deltaVector);
  svtkMath::Subtract(axis->GetPoint2(), axis->GetPoint1(), polarAxisUnitVect);
  svtkMath::Normalize(polarAxisUnitVect);
  ...
  // reuse deltaVector
  svtkMath::Subtract(axis->GetPoint1(), this->Pole, deltaVector);
  double distanceAxisPoint1FromPole = svtkMath::Norm(deltaVector);
  ...
  for (indexTickRangeValue = lowBound; indexTickRangeValue <= upBound; indexTickRangeValue *= base)
  {
    ...
    if (svtkMath::Norm(deltaVector) == 0.0)
    {
      continue;
    }

    // epsilon is a very low value. svtkMathUtilities::FuzzyCompare is not fuzzy enough ...
    if (fabs(fabs(miniAngleEllipseRad) - svtkMath::Pi() / 2.0) < epsilon)
    {
...
//-----------------------------------------------------------------------------
void svtkPolarAxesActor::BuildLabelsLog()
{
  // Prepare storage for polar axis labels
  std::list<double> labelValList;
...
//-----------------------------------------------------------------------------
void svtkPolarAxesActor::BuildPolarAxisLabelsArcsLog()
{
  this->BuildPolarArcsLog();
  ...
  // Update axis title follower
  svtkAxisFollower* follower = this->PolarAxis->GetTitleActor();
  ...
  // Update axis label followers
  svtkAxisFollower** labelActors = this->PolarAxis->GetLabelActors();
...
//-----------------------------------------------------------------------------
std::string svtkPolarAxesActor::FindExponentAndAdjustValues(std::list<double>& valuesList)
{
...
//-----------------------------------------------------------------------------
void svtkPolarAxesActor::GetSignificantPartFromValues(
  svtkStringArray* valuesStr, std::list<double>& valuesList)
{
  ...
  valuesStr->SetNumberOfValues(static_cast<svtkIdType>(valuesList.size()));
...
//-----------------------------------------------------------------------------
void svtkPolarAxesActor::AutoScale(svtkViewport* viewport)
{
  // Scale polar axis title
  svtkAxisActor* axis = this->PolarAxis;
  double newTitleScale = svtkAxisFollower::AutoScale(
  ...
  for (int i = 0; i < this->NumberOfRadialAxes; ++i)
  {
    ...
    // Scale title
    newTitleScale = svtkAxisFollower::AutoScale(
      viewport, this->Camera, this->ScreenSize, axis->GetTitleActor()->GetPosition());
    axis->SetTitleScale(newTitleScale);
  }
}
...
//-----------------------------------------------------------------------------
void svtkPolarAxesActor::SetPole(double p[3])
{
  this->Pole[0] = p[0];
  this->Pole[1] = p[1];
  this->Pole[2] = p[2];
...
//-----------------------------------------------------------------------------
void svtkPolarAxesActor::SetPole(double x, double y, double z)
{
  this->Pole[0] = x;
  this->Pole[1] = y;
  this->Pole[2] = z;
...
//-----------------------------------------------------------------------------
void svtkPolarAxesActor::SetMinimumRadius(double r)
{
...
//-----------------------------------------------------------------------------
void svtkPolarAxesActor::SetMaximumRadius(double r)
{
...
//-----------------------------------------------------------------------------
void svtkPolarAxesActor::SetMinimumAngle(double a)
{
  if (a > 360.)
  {
...
//-----------------------------------------------------------------------------
void svtkPolarAxesActor::SetMaximumAngle(double a)
{
  if (a > 360.)
  {
...
//-----------------------------------------------------------------------------
void svtkPolarAxesActor::SetUse2DMode(int val)
{
  for (int i = 0; i < this->NumberOfRadialAxes; ++i)
  {
...
//-----------------------------------------------------------------------------
int svtkPolarAxesActor::GetUse2DMode()
{
  return this->PolarAxis->GetUse2DMode();
}

//-----------------------------------------------------------------------------
void svtkPolarAxesActor::SetPolarAxisProperty(svtkProperty* prop)
{
...
//-----------------------------------------------------------------------------
void svtkPolarAxesActor::SetPolarArcsProperty(svtkProperty* prop)
{
...
//-----------------------------------------------------------------------------
svtkProperty* svtkPolarAxesActor::GetPolarArcsProperty()
{
  return this->PolarArcsActor->GetProperty();
}

//-----------------------------------------------------------------------------
void svtkPolarAxesActor::SetSecondaryPolarArcsProperty(svtkProperty* prop)
{
...
//-----------------------------------------------------------------------------
svtkProperty* svtkPolarAxesActor::GetSecondaryPolarArcsProperty()
{
  return this->SecondaryPolarArcsActor->GetProperty();
}

//-----------------------------------------------------------------------------
void svtkPolarAxesActor::SetNumberOfPolarAxisTicks(int tickCountRequired)
{
  double rangeLength = fabs(this->Range[1] - this->Range[0]);
  double step = this->ComputeIdealStep(
    tickCountRequired - 1, rangeLength, SVTK_MAXIMUM_NUMBER_OF_POLAR_AXIS_TICKS - 1);
...
//-----------------------------------------------------------------------------
void svtkPolarAxesActor::ComputeDeltaAngleRadialAxes(svtkIdType n)
{
  if (n <= 1)
  {
...
//-----------------------------------------------------------------------------
double svtkPolarAxesActor::ComputeIdealStep(int subDivsRequired, double rangeLength, int maxSubDivs)
{
...
//-----------------------------------------------------------------------------
int svtkPolarAxesActor::GetNumberOfPolarAxisTicks()
{
  double rangeLength = fabs(this->Range[1] - this->Range[0]);
  return static_cast<int>((rangeLength / this->DeltaRangeMajor) + 1);
}

double svtkPolarAxesActor::ComputeEllipseAngle(double angleInDegrees, double ratio)
{
  double miniAngleEllipse;
  double minimumAngleRad = svtkMath::RadiansFromDegrees(angleInDegrees);
  minimumAngleRad = std::fmod(minimumAngleRad, 2.0 * svtkMath::Pi());

  // result range: -pi / 2, pi / 2
  miniAngleEllipse = atan(tan(minimumAngleRad) / ratio);

  // ellipse range: 0, 2 * pi
  if (minimumAngleRad > svtkMath::Pi() / 2 && minimumAngleRad <= svtkMath::Pi())
  {
    miniAngleEllipse += svtkMath::Pi();
  }
  else if (minimumAngleRad > svtkMath::Pi() && minimumAngleRad <= 1.5 * svtkMath::Pi())
  {
    miniAngleEllipse -= svtkMath::Pi();
  }
