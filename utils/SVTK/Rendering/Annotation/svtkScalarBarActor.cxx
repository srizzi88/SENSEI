...
#include "svtkScalarBarActorInternal.h"
...
#undef SVTK_DBG_LAYOUT

svtkStandardNewMacro(svtkScalarBarActor);

svtkCxxSetObjectMacro(svtkScalarBarActor, LookupTable, svtkScalarsToColors);
svtkCxxSetObjectMacro(svtkScalarBarActor, AnnotationTextProperty, svtkTextProperty);
svtkCxxSetObjectMacro(svtkScalarBarActor, LabelTextProperty, svtkTextProperty);
svtkCxxSetObjectMacro(svtkScalarBarActor, TitleTextProperty, svtkTextProperty);
svtkCxxSetObjectMacro(svtkScalarBarActor, BackgroundProperty, svtkProperty2D);
svtkCxxSetObjectMacro(svtkScalarBarActor, FrameProperty, svtkProperty2D);

//----------------------------------------------------------------------------
// Instantiate object with 64 maximum colors; 5 labels; %%-#6.3g label
// format, no title, and vertical orientation. The initial scalar bar
// size is (0.05 x 0.8) of the viewport size.
svtkScalarBarActor::svtkScalarBarActor()
{
  this->P = new svtkScalarBarActorInternal;
  ...
  this->Orientation = SVTK_ORIENT_VERTICAL;
  this->Title = nullptr;
  ...
  this->AnnotationTextProperty = svtkTextProperty::New();
  ...
  this->TitleActor = svtkTextActor::New();
  this->TitleActor->GetPositionCoordinate()->SetReferenceCoordinate(this->PositionCoordinate);

  this->ScalarBar = svtkPolyData::New();
  this->ScalarBarMapper = svtkPolyDataMapper2D::New();
  this->ScalarBarMapper->SetInputData(this->ScalarBar);
  this->ScalarBarActor = svtkActor2D::New();
  ...
  this->P->NanSwatch = svtkPolyData::New();
  this->P->NanSwatchMapper = svtkPolyDataMapper2D::New();
  this->P->NanSwatchActor = svtkActor2D::New();
  ...
  this->P->BelowRangeSwatch = svtkPolyData::New();
  this->P->BelowRangeSwatchMapper = svtkPolyDataMapper2D::New();
  this->P->BelowRangeSwatchActor = svtkActor2D::New();
  ...
  this->P->AboveRangeSwatch = svtkPolyData::New();
  this->P->AboveRangeSwatchMapper = svtkPolyDataMapper2D::New();
  this->P->AboveRangeSwatchActor = svtkActor2D::New();
  ...
  this->P->AnnotationBoxes = svtkPolyData::New();
  this->P->AnnotationBoxesMapper = svtkPolyDataMapper2D::New();
  this->P->AnnotationBoxesActor = svtkActor2D::New();
  ...
  this->P->AnnotationBoxesActor->GetPositionCoordinate()->SetReferenceCoordinate(
    this->PositionCoordinate);
  this->P->AnnotationLeaders = svtkPolyData::New();
  this->P->AnnotationLeadersMapper = svtkPolyDataMapper2D::New();
  this->P->AnnotationLeadersActor = svtkActor2D::New();
  ...
  svtkPolyDataMapper2D* textureMapper = svtkPolyDataMapper2D::New();
  ...
  svtkCellArray* polys2 = svtkCellArray::New();
  ...
  svtkProperty2D* imageProperty = svtkProperty2D::New();
  ...
  this->MaximumWidthInPixels = SVTK_INT_MAX;
  this->MaximumHeightInPixels = SVTK_INT_MAX;

  this->BackgroundProperty = svtkProperty2D::New();
  this->FrameProperty = svtkProperty2D::New();

  this->DrawBackground = 0;
  this->Background = svtkPolyData::New();
  this->BackgroundMapper = svtkPolyDataMapper2D::New();
  this->BackgroundMapper->SetInputData(this->Background);
  this->BackgroundActor = svtkActor2D::New();
...
#ifdef SVTK_DBG_LAYOUT
  this->DrawFrame = 1;
#else  // SVTK_DBG_LAYOUT
  this->DrawFrame = 0;
#endif // SVTK_DBG_LAYOUT
  this->Frame = svtkPolyData::New();
...
void svtkScalarBarActor::ReleaseGraphicsResources(svtkWindow* win)
{
  this->TitleActor->ReleaseGraphicsResources(win);
  if (!this->P->TextActors.empty())
  {
    svtkScalarBarActorInternal::ActorVector::iterator it;
...
//----------------------------------------------------------------------------
void svtkScalarBarActor::GetScalarBarRect(int rect[4], svtkViewport* viewport)
{
  svtkCoordinate* origin = this->ScalarBarActor->GetPositionCoordinate();
...
//----------------------------------------------------------------------------
int svtkScalarBarActor::RenderOverlay(svtkViewport* viewport)
{
...
int svtkScalarBarActor::RebuildLayoutIfNeeded(svtkViewport* viewport)
{
  if (!this->LookupTable)
  {
    svtkWarningMacro(<< "Need a mapper to render a scalar bar");
    return 0;
  }

  if (!this->TitleTextProperty)
  {
    svtkErrorMacro(<< "Need title text property to render a scalar bar");
    return 0;
  }

  if (!this->LabelTextProperty)
  {
    svtkErrorMacro(<< "Need label text property to render a scalar bar");
    return 0;
  }

  if (!this->AnnotationTextProperty)
  {
    svtkErrorMacro(<< "Need annotation text property to render a scalar bar");
    return 0;
  }
...
//----------------------------------------------------------------------------
int svtkScalarBarActor::RenderOpaqueGeometry(svtkViewport* viewport)
{
  if (!this->RebuildLayoutIfNeeded(viewport))
  {
    return 0;
  ...
  if (!this->LookupTable->GetIndexedLookup())
  {
    ...
    svtkScalarBarActorInternal::ActorVector::iterator ait;
...
svtkTypeBool svtkScalarBarActor::HasTranslucentPolygonalGeometry()
...
//----------------------------------------------------------------------------
void svtkScalarBarActor::PrintSelf(ostream& os, svtkIndent indent)
{
  ...
  if (this->Orientation == SVTK_ORIENT_HORIZONTAL)
  {
    os << "Horizontal\n";
  }
  else
  ...
  if (this->TextPosition == svtkScalarBarActor::PrecedeScalarBar)
  {
...
//----------------------------------------------------------------------------
void svtkScalarBarActor::ShallowCopy(svtkProp* prop)
{
  svtkScalarBarActor* a = svtkScalarBarActor::SafeDownCast(prop);
  if (a != nullptr)
  {
...
//----------------------------------------------------------------------------
void svtkScalarBarActor::RebuildLayout(svtkViewport* viewport)
{
  svtkDebugMacro(<< "Rebuilding subobjects");
  ...
  if (this->Orientation == SVTK_ORIENT_VERTICAL)
  {
    this->P->TL[0] = 0;
    this->P->TL[1] = 1;
  }
  ...
  // Now generate/configure the SVTK datasets and actors that
...
namespace
{
void AddBox(svtkPoints* pts, svtkCellArray* lines, svtkScalarBarBox& box, int tl[2])
{
  svtkIdType pid[5];
...
void svtkScalarBarActor::DrawBoxes()
{
  svtkPoints* pts = this->Frame->GetPoints();
  svtkCellArray* lines = this->Frame->GetLines();
...
//----------------------------------------------------------------------------
void svtkScalarBarActor::ComputeFrame()
{
...
//----------------------------------------------------------------------------
void svtkScalarBarActor::ComputeScalarBarThickness()
{
...
//----------------------------------------------------------------------------
void svtkScalarBarActor::ComputeSwatchPad()
{
  if (this->P->NumNotes)
  {
...
//----------------------------------------------------------------------------
void svtkScalarBarActor::LayoutNanSwatch()
{
...
//----------------------------------------------------------------------------
void svtkScalarBarActor::LayoutBelowRangeSwatch()
{
  // We don't have ScalarBarBox.Size[1] set yet; use the frame width instead.
...
//----------------------------------------------------------------------------
void svtkScalarBarActor::LayoutAboveRangeSwatch()
{
  // We don't have ScalarBarBox.Size[1] set yet; use the frame width instead.
...
//----------------------------------------------------------------------------
void svtkScalarBarActor::LayoutAboveRangeSwatchPosn()
{
  if (this->Orientation == SVTK_ORIENT_VERTICAL)
  {
...
//----------------------------------------------------------------------------
void svtkScalarBarActor::PrepareTitleText()
{
  ...
  if (this->TitleTextProperty->GetMTime() > this->BuildTime)
  {
    ...
    this->TitleActor->GetTextProperty()->SetVerticalJustification(
      this->TextPosition == PrecedeScalarBar ? SVTK_TEXT_BOTTOM : SVTK_TEXT_TOP);
  }
}

//----------------------------------------------------------------------------
void svtkScalarBarActor::LayoutTitle()
{
  if (this->Title == nullptr || !strlen(this->Title))
  {
    this->P->TitleBox.Posn = svtkTuple<int, 2>(0.);
    this->P->TitleBox.Size = svtkTuple<int, 2>(0.);
    return;
  }
  ...
  targetHeight = static_cast<int>(
    (this->Orientation == SVTK_ORIENT_VERTICAL || this->LookupTable->GetIndexedLookup())
  ...
  if (this->Orientation == SVTK_ORIENT_VERTICAL ||
    this->TextPosition == svtkScalarBarActor::SucceedScalarBar)
  {
...
//-----------------------------------------------------------------------------
void svtkScalarBarActor::ComputeScalarBarLength()
{
  this->P->ScalarBarBox.Size[1] = this->Orientation == SVTK_ORIENT_VERTICAL
...
//-----------------------------------------------------------------------------
void svtkScalarBarActor::LayoutTicks()
{
  if (this->LookupTable->GetIndexedLookup())
  ...
  {
    this->P->TextActors[i].TakeReference(svtkTextActor::New());
  ...
  if (this->NumberOfLabels)
  {
    ...
    if (!this->UnconstrainedFontSize)
    {
      svtkTextActor::SetMultipleConstrainedFontSize(this->P->Viewport, targetWidth, targetHeight,
...
//-----------------------------------------------------------------------------
void svtkScalarBarActor::LayoutForUnconstrainedFont()
{
  if (this->UnconstrainedFontSize)
  {
...
//----------------------------------------------------------------------------
void svtkScalarBarActor::LayoutAnnotations()
{
  if (this->DrawAnnotations)
  {
    const double* range = this->LookupTable->GetRange();
...
//----------------------------------------------------------------------------
void svtkScalarBarActor::ConfigureFrame()
{
  // set frame structure
  svtkPoints* frPts = svtkPoints::New();
  frPts->SetNumberOfPoints(5);
  svtkCellArray* frLines = svtkCellArray::New();
  ...
  svtkPoints* bgPts = svtkPoints::New();
  bgPts->SetNumberOfPoints(4);
  svtkCellArray* bgPolys = svtkCellArray::New();
  ...
  svtkIdType bgIds[5] = { 0, 1, 2, 3, 4 };
  bgPolys->InsertNextCell(4, bgIds);
  frLines->InsertNextCell(5, bgIds);
}

//----------------------------------------------------------------------------
void svtkScalarBarActor::ConfigureScalarBar()
{
  svtkScalarsToColors* lut = this->LookupTable;
  ...
  this->P->SwatchPts = svtkPoints::New();
  this->P->SwatchPts->SetNumberOfPoints(numPts);
  this->P->Polys = svtkCellArray::New();
  this->P->Polys->AllocateEstimate(this->P->NumSwatches, 4);
  this->P->SwatchColors = svtkUnsignedCharArray::New();
  ...
  // scalar bar polydata.
  svtkPoints* texturePoints = svtkPoints::New();
  ...
  svtkDataArray* tc = this->TexturePolyData->GetPointData()->GetTCoords();
...
//----------------------------------------------------------------------------
void svtkScalarBarActor::ConfigureTitle()
{
  this->TitleActor->SetPosition(
    this->P->TitleBox.Posn[0] + this->P->TitleBox.Size[this->P->TL[0]] / 2,
    this->TitleActor->GetTextProperty()->GetVerticalJustification() == SVTK_TEXT_BOTTOM
      ? this->P->TitleBox.Posn[1]
      : this->P->TitleBox.Posn[1] + this->P->TitleBox.Size[this->P->TL[1]]);
}

//-----------------------------------------------------------------------------
void svtkScalarBarActor::ConfigureTicks()
{
  double val;
  ...
  for (int i = 0; i < this->NumberOfLabelsBuilt; ++i)
  {
    ...
    { // VERTICAL
      this->P->TextActors[i]->GetTextProperty()->SetJustification(
        this->TextPosition == PrecedeScalarBar ? SVTK_TEXT_RIGHT : SVTK_TEXT_LEFT);
      this->P->TextActors[i]->GetTextProperty()->SetVerticalJustificationToBottom();
      this->P->TextActors[i]->SetPosition(this->TextPosition == svtkScalarBarActor::PrecedeScalarBar
    ...
    { // HORIZONTAL
      ...
      this->P->TextActors[i]->GetTextProperty()->SetVerticalJustification(
        this->TextPosition == PrecedeScalarBar ? SVTK_TEXT_TOP : SVTK_TEXT_BOTTOM);
...
//-----------------------------------------------------------------------------
void svtkScalarBarActor::ConfigureNanSwatch()
{
  if (!this->DrawNanAnnotation)
  {
    return;
...
//----------------------------------------------------------------------------
void svtkScalarBarActor::ConfigureAboveBelowRangeSwatch(bool above)
{
  // Check above/below
  svtkPolyData* swatch;
  svtkScalarBarBox* box;
  if (above)
  {
    if (!this->DrawAboveRangeSwatch)
    {
  ...
  svtkIdType ptIds[4] = { 0, 1, 2, 3 };
  polys->InsertNextCell(4, ptIds);
  this->LookupTable->GetIndexedColor(-1, rgba);

  svtkLookupTable* lt = svtkLookupTable::SafeDownCast(this->LookupTable);
  svtkColorTransferFunction* ctf = svtkColorTransferFunction::SafeDownCast(this->LookupTable);
...
//----------------------------------------------------------------------------
void svtkScalarBarActor::ConfigureAnnotations()
{
  ...
  int i = 0;
  std::map<double, svtkStdString>::iterator it;
  for (it = this->P->Labels.begin(); it != this->P->Labels.end(); ++it, ++i)
  {
    ...
    this->P->AnnotationLabels[i] = svtkSmartPointer<svtkTextActor>::New();
  ...
  if (this->LookupTable->GetIndexedLookup())
  {
    ...
    // Must reset the color on the leader lines since svtkTextProperty
    // doesn't inherit svtkProperty.
    ...
    // Since labels are user-provided, we render with svtkTextActor
    // to allow fancy-ness.
    ...
    if (this->Orientation == SVTK_ORIENT_VERTICAL)
    {
      this->PlaceAnnotationsVertically(
        this->TextPosition == svtkScalarBarActor::SucceedScalarBar ? swatchC0 : swatchC1,
  ...
  else
  {
    if (this->Orientation == SVTK_ORIENT_VERTICAL)
    {
      this->PlaceAnnotationsVertically(this->TextPosition == svtkScalarBarActor::SucceedScalarBar
          ? this->P->ScalarBarBox.Posn[this->P->TL[0]]
...
//----------------------------------------------------------------------------
void svtkScalarBarActor::FreeLayoutStorage()
{
  // Delete previously constructed objects
  if (this->P->Viewport && this->P->Viewport->GetSVTKWindow())
  {
    svtkWindow* win = this->P->Viewport->GetSVTKWindow();
    if (!this->P->TextActors.empty())
    {
      svtkScalarBarActorInternal::ActorVector::iterator it;
      for (it = this->P->TextActors.begin(); it != this->P->TextActors.end(); ++it)
...
//----------------------------------------------------------------------------
void svtkScalarBarActor::SizeTitle(double* titleSize, int* size, svtkViewport* viewport)
{
...
//----------------------------------------------------------------------------
int svtkScalarBarActor::MapAnnotationLabels(
  svtkScalarsToColors* lkup, double start, double delta, const double* range)
{
  ...
  bool vertical = (this->Orientation == SVTK_ORIENT_VERTICAL);
  svtkColor4d fltCol;
  ...
  if (numNotes > 0)
  {
    ...
    {
      svtkStdString label = lkup->GetAnnotation(i);
      ...
      if (canPositionLabel)
      {
        ...
        {
          svtkVariant pos = lkup->GetAnnotatedValue(i);
          ...
          x = canPositionLabel ? (start + (x - range[0]) * delta / drange) : svtkMath::Nan();
        }
      }
      if (canPositionLabel)
      {
...
//----------------------------------------------------------------------------
int svtkScalarBarActor::PlaceAnnotationsVertically(double barX, double barY,
  double svtkNotUsed(barWidth), double barHeight, double svtkNotUsed(delta), double pad)
{
  if (!this->LookupTable)
  {
    return 0;
...
#define SVTK_ANN_VLAYOUT(j, dir, delt)                                                              \
  ...
  this->P->AnnotationLabels[j]->GetTextProperty()->SetJustification(                               \
    this->TextPosition == PrecedeScalarBar ? SVTK_TEXT_LEFT : SVTK_TEXT_RIGHT);                      \
  ...
  svtkPoints* lpts = svtkPoints::New();
  svtkCellArray* llines = svtkCellArray::New();
  svtkUnsignedCharArray* llcolors = svtkUnsignedCharArray::New();
  ...
  svtkIdType ll[2]; // leader-line endpoint IDs
  if (2 * ic == numNotes)
  {
    dn = ic - 1;
    up = ic;
...
//-----------------------------------------------------------------------------
struct svtkScalarBarHLabelInfo
{
...
struct svtkScalarBarHLabelPlacer
{
  std::vector<svtkScalarBarHLabelInfo> Places;
  unsigned Ctr;
  double Y0;
  double XBounds[2];
  int NumPlaced;
  ...
  {
    svtkScalarBarHLabelInfo& placement(this->Places[i]);
    ...
    { // center label
      ...
      placement.Justification = SVTK_TEXT_CENTERED;
    ...
    else // placing *a lateral* (as opposed to *the medial*) label.
    {
      ...
      if (posRelToCenter == 1)
      {
        ...
        medNeighbor = i - 1;
        placement.Justification = SVTK_TEXT_RIGHT;
        placement.X[1] = xBest;
      ...
      else // posRelToCenter == -1
      {
        ...
        placement.Justification = SVTK_TEXT_LEFT;
        placement.X[0] = xBest;
  ...
  void BreakLeader(svtkScalarBarHLabelInfo& label, double& curY, int j, svtkPoints* pts,
    svtkCellArray* lines, svtkUnsignedCharArray* colors, const svtkColor3ub& color)
  {
    svtkScalarBarHLabelInfo& other(this->Places[j]);
    svtkIdType pt;
  ...
  void AddBrokenLeader(int lidx, svtkPoints* pts, svtkCellArray* lines, svtkUnsignedCharArray* colors,
    const svtkColor3ub& color)
  {
    svtkIdType pt;
    svtkScalarBarHLabelInfo& label(this->Places[lidx]);
 ...
 */
int svtkScalarBarActor::PlaceAnnotationsHorizontally(
  double barX, double barY, double barWidth, double barHeight, double svtkNotUsed(delta), double pad)
{
  if (!this->LookupTable)
  {
    return 0;
...
#define SVTK_ANN_HLAYOUT(j, placer)                                                                 \
  ...
  int numNotes = static_cast<int>(this->P->AnnotationLabels.size());
  bool precede = this->TextPosition == svtkScalarBarActor::PrecedeScalarBar;
  svtkScalarBarHLabelPlacer placer(numNotes, precede ? barY : barY - barHeight, precede ? 1 : -1,
