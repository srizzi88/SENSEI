/*=========================================================================
  ...
  Module:    svtkSpiderPlotActor.cxx
...
svtkStandardNewMacro(svtkSpiderPlotActor);

svtkCxxSetObjectMacro(svtkSpiderPlotActor, LabelTextProperty, svtkTextProperty);
svtkCxxSetObjectMacro(svtkSpiderPlotActor, TitleTextProperty, svtkTextProperty);

// PIMPL'd list of labels
class svtkAxisLabelArray : public std::vector<std::string>
{
};

// PIMPL'd list of ranges
struct svtkAxisRange
{
  double Min;
  double Max;
  svtkAxisRange()
  ...
  svtkAxisRange(double min, double max)
    : Min(min)
    , Max(max)
  {
  }
};
class svtkAxisRanges : public std::vector<svtkAxisRange>
{
};

class svtkSpiderPlotActorConnection : public svtkAlgorithm
{
public:
  static svtkSpiderPlotActorConnection* New();
  svtkTypeMacro(svtkSpiderPlotActorConnection, svtkAlgorithm);

  svtkSpiderPlotActorConnection() { this->SetNumberOfInputPorts(1); }
};

svtkStandardNewMacro(svtkSpiderPlotActorConnection);

//----------------------------------------------------------------------------
// Instantiate object
svtkSpiderPlotActor::svtkSpiderPlotActor()
{
  // Actor2D positions
  ...
  this->ConnectionHolder = svtkSpiderPlotActorConnection::New();

  this->IndependentVariables = SVTK_IV_COLUMN;
  this->TitleVisibility = 1;
  this->Title = nullptr;
  this->Labels = new svtkAxisLabelArray;
  this->Ranges = new svtkAxisRanges;
...
//----------------------------------------------------------------------------
void svtkSpiderPlotActor::SetInputConnection(svtkAlgorithmOutput* ao)
{
  this->ConnectionHolder->SetInputConnection(ao);
}

//----------------------------------------------------------------------------
void svtkSpiderPlotActor::SetInputData(svtkDataObject* dobj)
{
...
//----------------------------------------------------------------------------
svtkDataObject* svtkSpiderPlotActor::GetInput()
{
...
void svtkSpiderPlotActor::Initialize()
{
  if (this->LabelActors)
  {
...
int svtkSpiderPlotActor::RenderOverlay(svtkViewport* viewport)
{
...
//----------------------------------------------------------------------------
// Plot scalar data for each input dataset.
int svtkSpiderPlotActor::RenderOpaqueGeometry(svtkViewport* viewport)
{
  int renderedSomething = 0;
...
svtkTypeBool svtkSpiderPlotActor::HasTranslucentPolygonalGeometry()
{
  return 0;
}

//-----------------------------------------------------------------------------
int svtkSpiderPlotActor::BuildPlot(svtkViewport* viewport)
{
  // Initialize
  svtkDebugMacro(<< "Building spider plot");
...
//----------------------------------------------------------------------------
static inline int svtkSpiderPlotActorGetComponent(
...
#define SVTK_RING_PTS 64
//----------------------------------------------------------------------------
int svtkSpiderPlotActor::PlaceAxes(svtkViewport* viewport, const int* svtkNotUsed(size))
{
  svtkIdType i, j, k;
  ...
  svtkIdType numRows = SVTK_ID_MAX; // figure out number of rows
  ...
  if (this->IndependentVariables == SVTK_IV_COLUMN)
  {
    this->N = numColumns;
  }
  else // row
  ...
  { // ranges not specified
    if (this->IndependentVariables == SVTK_IV_COLUMN)
    {
      ...
      for (j = 0; j < numComponents; j++)
      {
        ...
        for (i = 0; i < numRows; i++)
        {
          ...
          ::svtkSpiderPlotActorGetComponent(field, i, j, &v);
          if (v < this->Mins[k])
          {
            this->Mins[k] = v;
          }
    ...
    else // row
    {
      for (j = 0; j < numRows; j++)
      {
        for (i = 0; i < numComponents; i++)
        {
          ...
          if (::svtkSpiderPlotActorGetComponent(field, j, i, &v) == 0)
          {
    ...
    for (i = 0; i < this->N; i++)
    {
      this->Ranges->push_back(svtkAxisRange(this->Mins[i], this->Maxs[i]));
    }
  ...
  webPts->Allocate(this->NumberOfRings * SVTK_RING_PTS);
  svtkCellArray* webLines = svtkCellArray::New();
  webLines->AllocateEstimate(this->N + this->NumberOfRings, SVTK_RING_PTS);
  this->WebData->SetPoints(webPts);
  this->WebData->SetLines(webLines);
  svtkIdType ptId, pIds[SVTK_RING_PTS + 1];
  double currentTheta, x[3];
  x[2] = 0.0;

  // Specify the positions for the axes
  this->Theta = 2.0 * svtkMath::Pi() / this->N;
  ...
  if (this->NumberOfRings > 0)
  {
    ...
    for (i = 0; i < this->NumberOfRings; i++)
    {
      ...
      pIds[SVTK_RING_PTS] = pIds[0];
      webLines->InsertNextCell(SVTK_RING_PTS + 1, pIds);
    }
  }
  ...
  if (this->LabelVisibility)
  {
    this->LabelActors = new svtkActor2D*[this->N];
    this->LabelMappers = new svtkTextMapper*[this->N];
    char label[1024];
    const char* str;
    for (i = 0; i < this->N; i++)
    {
      ...
      this->LabelMappers[i] = svtkTextMapper::New();
      ...
      this->LabelActors[i] = svtkActor2D::New();
  ...
  if (this->IndependentVariables == SVTK_IV_COLUMN)
  {
    ...
    for (j = 0; j < numRows; j++)
    {
      ...
      {
        if (::svtkSpiderPlotActorGetComponent(field, j, k, &v) == 0)
        {
...
void svtkSpiderPlotActor::ReleaseGraphicsResources(svtkWindow* win)
{
...
//----------------------------------------------------------------------------
void svtkSpiderPlotActor::SetAxisLabel(const int i, const char* label)
{
  if (i < 0)
  {
    return;
...
//----------------------------------------------------------------------------
const char* svtkSpiderPlotActor::GetAxisLabel(int i)
{
  if (i < 0)
  {
    return nullptr;
...
//----------------------------------------------------------------------------
void svtkSpiderPlotActor::SetAxisRange(int i, double min, double max)
{
  if (i < 0)
  {
    return;
  ...
  (*this->Ranges)[i] = svtkAxisRange(min, max);
  this->Modified();
}

//----------------------------------------------------------------------------
void svtkSpiderPlotActor::SetAxisRange(int i, double range[2])
{
  this->SetAxisRange(i, range[0], range[1]);
}

//----------------------------------------------------------------------------
void svtkSpiderPlotActor::GetAxisRange(int i, double range[2])
{
  if (i < 0)
  {
    return;
  ...
  svtkAxisRange arange = this->Ranges->at(i);
  range[0] = arange.Min;
  range[1] = arange.Max;
}

//----------------------------------------------------------------------------
void svtkSpiderPlotActor::SetPlotColor(int i, double r, double g, double b)
{
  this->LegendActor->SetEntryColor(i, r, g, b);
}

//----------------------------------------------------------------------------
double* svtkSpiderPlotActor::GetPlotColor(int i)
{
  return this->LegendActor->GetEntryColor(i);
}

//----------------------------------------------------------------------------
void svtkSpiderPlotActor::PrintSelf(ostream& os, svtkIndent indent)
{
