...
/**
 * @class   svtkBrush
 * @brief   provides a brush that fills shapes drawn by svtkContext2D.
 *
 *
 * The svtkBrush defines the fill (or pattern) of shapes that are drawn by
 * svtkContext2D. The color is stored as four unsigned chars (RGBA), where the
...
class SVTKRENDERINGCONTEXT2D_EXPORT svtkBrush : public svtkObject
{
public:
  svtkTypeMacro(svtkBrush, svtkObject);
  void PrintSelf(ostream& os, svtkIndent indent) override;

  static svtkBrush* New();
   ...
   */
  void SetTexture(svtkImageData* image);

  //@{
  /**
   * Get the texture that is used to fill polygons
   */
  svtkGetObjectMacro(Texture, svtkImageData);
  //@}
   ...
   */
  svtkSetMacro(TextureProperties, int);
  //@}
   ...
   */
  svtkGetMacro(TextureProperties, int);
  //@}

  /**
   * Make a deep copy of the supplied brush.
   */
  void DeepCopy(svtkBrush* brush);
...
protected:
  ...
  unsigned char* Color;
  svtkColor4ub BrushColor;
  svtkImageData* Texture;
  int TextureProperties;

private:
  svtkBrush(const svtkBrush&) = delete;
  void operator=(const svtkBrush&) = delete;
};
