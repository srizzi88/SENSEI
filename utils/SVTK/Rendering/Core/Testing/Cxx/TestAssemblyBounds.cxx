...
{
  svtkSmartPointer<svtkSphereSource> source1 = svtkSmartPointer<svtkSphereSource>::New();
  ...
  svtkSmartPointer<svtkClipPolyData> clipper1 = svtkSmartPointer<svtkClipPolyData>::New();
  clipper1->SetInputConnection(source1->GetOutputPort());
  clipper1->SetClipFunction(plane);
  clipper1->SetValue(0);

  svtkSmartPointer<svtkDataSetMapper> mapper1 = svtkSmartPointer<svtkDataSetMapper>::New();
  mapper1->SetInputConnection(clipper1->GetOutputPort());

  svtkSmartPointer<svtkActor> actor1 = svtkSmartPointer<svtkActor>::New();
  actor1->SetMapper(mapper1);

  svtkSmartPointer<svtkSphereSource> source2 = svtkSmartPointer<svtkSphereSource>::New();
  source2->SetCenter(8, 8, 8);
  source2->SetRadius(2);

  svtkSmartPointer<svtkClipPolyData> clipper2 = svtkSmartPointer<svtkClipPolyData>::New();
  ...
  // Create an assembly
  svtkSmartPointer<svtkAssembly> assembly = svtkSmartPointer<svtkAssembly>::New();
