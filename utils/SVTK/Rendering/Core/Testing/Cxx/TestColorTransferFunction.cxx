...
bool TestColorSpace()
{
  ...
  {
    cerr << "ERROR: ColorSpace == SVTK_CTF_RGB failed!" << endl;
    return false;
  }
  ...
  {
    cerr << "ERROR: ColorSpace == SVTK_CTF_LAB_CIEDE2000 failed!" << endl;
    return false;
  }
  ...
  {
    cerr << "ERROR: ColorSpace == SVTK_CTF_STEP failed!" << endl;
    return false;
  }
  return true;
}

int TestColorTransferFunction(int svtkNotUsed(argc), char* svtkNotUsed(argv)[])
{
  svtkNew<svtkColorTransferFunction> ctf;
  ...
  ctf->GetTable(svtkMath::Nan(), 1., 256, table);
