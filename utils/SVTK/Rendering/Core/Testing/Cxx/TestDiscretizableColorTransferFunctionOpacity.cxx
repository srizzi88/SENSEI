...
int TestDiscretizableColorTransferFunctionOpacity(int, char*[])
{
  ...
  svtkSmartPointer<svtkDiscretizableColorTransferFunction> dctf =
  ...
  svtkSmartPointer<svtkPiecewiseFunction> pf = svtkSmartPointer<svtkPiecewiseFunction>::New();
  for (int i = 0; i < 2; ++i)
  {
  ...
  svtkSmartPointer<svtkUnsignedCharArray> mapScalarsOutput;

  //--------------------------------------------------------------------------
  //  Colors mapping only. Output format = SVTK_RGB
  //--------------------------------------------------------------------------

  // Map void* array to opacity using first entry point
  dctf->MapScalarsThroughTable(
    inputScalars, mapScalarsThroughTableOutput, SVTK_DOUBLE, 3, 1, SVTK_RGB);
  // Map data array to opacity using second entry point
  mapScalarsOutput.TakeReference(dctf->MapScalars(da, SVTK_COLOR_MODE_DEFAULT, -1));
  ...
  //--------------------------------------------------------------------------
  //  Colors and opacity mapping. Output format = SVTK_RGBA
  //--------------------------------------------------------------------------

  // Map void* array to opacity using first entry point
  dctf->MapScalarsThroughTable(
    inputScalars, mapScalarsThroughTableOutput, SVTK_DOUBLE, 3, 1, SVTK_RGBA);
  // Map data array to opacity using second entry point
  mapScalarsOutput.TakeReference(dctf->MapScalars(da, SVTK_COLOR_MODE_MAP_SCALARS, -1));
