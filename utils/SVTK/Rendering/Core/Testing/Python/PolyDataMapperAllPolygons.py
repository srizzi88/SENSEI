...
triangleStripPointScalars = svtk.svtkFloatArray()
...
triangleStripCellScalars = svtk.svtkFloatArray()
triangleStripCellScalars.SetNumberOfTuples(1)
triangleStripCellScalars.InsertValue(0, 1)

triangleStripPointNormals = svtk.svtkFloatArray()
...
triangleStripCellNormals = svtk.svtkFloatArray()
...
for type in types:
    for mask in masks:
        idx = str(i)
        exec("grid" + idx + " = svtk.svtkUnstructuredGrid()")
        ...
        exec("geometry" + idx + " = svtk.svtkGeometryFilter()")
        eval("geometry" + idx).SetInputData(eval("grid" + idx))

        exec("triangles" + idx + " = svtk.svtkTriangleFilter()")
