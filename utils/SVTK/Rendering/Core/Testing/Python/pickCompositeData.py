...
# This script tests picking props, points and cells rendered with a svtkCompositePolyDataMapper2.
#
import svtk
import math
import time
...
# Create a svtkMultiBlockDataSet with 3
# svtkPolyData blocks, rendered using a svtkCompositePolyDataMapper2
# and picked using svtkPicker, svtkPointPicker and svtkCellPicker.
...
cda=svtk.svtkCompositeDataDisplayAttributes()
m=svtk.svtkCompositePolyDataMapper2()
m.SetInputDataObject(mbd)
m.SetCompositeDataDisplayAttributes(cda)

for blk in range(0,3):
  mbd.SetBlock(blk,svtk.svtkPolyData())
...
# Define the pickers
propPicker = svtk.svtkPicker()
cellPicker = svtk.svtkCellPicker()
pointPicker = svtk.svtkPointPicker()
...
def pick(xy,propPickData,pointPickData,cellPickData):
    ...
    points in z direction at position xy, and pick with svtkPicker,
    svtkPointPicker and svtkCellPicker.
