 ...
 *
 * svtkAbstractPicker is an abstract superclass that defines a minimal API
 ...
 * method.
 *
 * svtkPicker fires events during the picking process.  These
 ...
 * PickEvent of svtkProp (and its subclasses such as svtkActor) is fired
 * prior to the PickEvent of svtkPicker.
 *
 * @warning
 * svtkAbstractPicker and its subclasses will not pick props that are
 * "unpickable" (see svtkProp) or are fully transparent (if transparency
 * is a property of the svtkProp).
 *
 * @warning
 ...
 * include svtkPicker, svtkCellPicker, and svtkPointPicker. Examples of hardware
 * pickers include svtkWorldPointPicker and svtkPropPicker.
 *
 * @sa
 * svtkPropPicker uses hardware acceleration to pick an instance of svtkProp.
 * (This means that 2D and 3D props can be picked, and it's relatively fast.)
 * If you need to pick cells or points, you might wish to use svtkCellPicker
 * or svtkPointPicker. svtkWorldPointPicker is the fastest picker, returning
 * an x-y-z coordinate value using the hardware z-buffer. svtkPicker can be
 * used to pick the bounding box of 3D props.
 */

#ifndef svtkAbstractPicker_h
#define svtkAbstractPicker_h
...
class SVTKRENDERINGCORE_EXPORT svtkAbstractPicker : public svtkObject
{
public:
  svtkTypeMacro(svtkAbstractPicker, svtkObject);
   ...
   */
  svtkGetVectorMacro(SelectionPoint, double, 3);
  //@}
   ...
   */
  svtkGetVectorMacro(PickPosition, double, 3);
  //@}
   ...
   */
  virtual int Pick(
    double selectionX, double selectionY, double selectionZ, svtkRenderer* renderer) = 0;
   ...
   */
  virtual int Pick3DPoint(double /* selectionPt */[3], svtkRenderer* /*ren*/)
  {
    svtkErrorMacro("Pick3DPoint called without implementation");
    return 0;
  }
   ...
   * Return non-zero if something was successfully picked.
   */
  virtual int Pick3DRay(double /* selectionPt */[3], double /* orient */[4], svtkRenderer* /*ren*/)
  {
    svtkErrorMacro("Pick3DRay called without implementation");
    return 0;
  }
   ...
   */
  svtkSetMacro(PickFromList, svtkTypeBool);
  svtkGetMacro(PickFromList, svtkTypeBool);
  svtkBooleanMacro(PickFromList, svtkTypeBool);
  //@}
   ...
   */
  void AddPickList(svtkProp*);

  /**
   * Delete an actor from the pick list.
   */
  void DeletePickList(svtkProp*);

  /**
   * Return the list of actors in the PickList.
   */
  svtkPropCollection* GetPickList() { return this->PickList; }

protected:
  svtkAbstractPicker();
  ~svtkAbstractPicker() override;

  virtual void Initialize();

  svtkRenderer* Renderer;    // pick occurred in this renderer's viewport
  ...
  svtkTypeBool PickFromList;
  svtkPropCollection* PickList;

private:
  svtkAbstractPicker(const svtkAbstractPicker&) = delete;
  void operator=(const svtkAbstractPicker&) = delete;
};
