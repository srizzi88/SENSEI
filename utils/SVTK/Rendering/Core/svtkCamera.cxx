...
//----------------------------------------------------------------------------
// Needed when we don't use the svtkStandardNewMacro.
svtkObjectFactoryNewMacro(svtkCamera);

svtkCxxSetObjectMacro(svtkCamera, Information, svtkInformation);
svtkCxxSetObjectMacro(svtkCamera, EyeTransformMatrix, svtkMatrix4x4);
svtkCxxSetObjectMacro(svtkCamera, ModelTransformMatrix, svtkMatrix4x4);
svtkCxxSetObjectMacro(svtkCamera, ExplicitProjectionTransformMatrix, svtkMatrix4x4);

//-----------------------------------------------------------------------------
class svtkCameraCallbackCommand : public svtkCommand
{
public:
  static svtkCameraCallbackCommand* New() { return new svtkCameraCallbackCommand; }
  svtkCamera* Self;
...
protected:
  svtkCameraCallbackCommand() { this->Self = nullptr; }
  ~svtkCameraCallbackCommand() override = default;
};
...
svtkCamera::svtkCamera()
{
  ...
  this->WorldToScreenMatrix = svtkMatrix4x4::New();
  this->WorldToScreenMatrix->Identity();

  this->EyeTransformMatrix = svtkMatrix4x4::New();
  this->EyeTransformMatrix->Identity();

  this->ModelTransformMatrix = svtkMatrix4x4::New();
  ...
  this->Transform = svtkPerspectiveTransform::New();
  this->ViewTransform = svtkTransform::New();
  this->ProjectionTransform = svtkPerspectiveTransform::New();
  this->CameraLightTransform = svtkTransform::New();
  this->ModelViewTransform = svtkTransform::New();
...
//----------------------------------------------------------------------------
void svtkCamera::SetScissorRect(svtkRecti scissorRect)
{
  this->ScissorRect = scissorRect;
}

//----------------------------------------------------------------------------
void svtkCamera::GetScissorRect(svtkRecti& scissorRect)
{
...
//----------------------------------------------------------------------------
void svtkCamera::SetPosition(double x, double y, double z)
{
  ...
  svtkDebugMacro(<< " Position set to ( " << this->Position[0] << ", " << this->Position[1] << ", "
                << this->Position[2] << ")");
...
//----------------------------------------------------------------------------
void svtkCamera::SetUserTransform(svtkHomogeneousTransform* transform)
{
...
//----------------------------------------------------------------------------
void svtkCamera::SetUserViewTransform(svtkHomogeneousTransform* transform)
{
  ...
  if (transform)
  {
    ...
    if (!this->UserViewTransformCallbackCommand)
    {
      this->UserViewTransformCallbackCommand = svtkCameraCallbackCommand::New();
      this->UserViewTransformCallbackCommand->Self = this;
    }
    this->UserViewTransform->AddObserver(
      svtkCommand::ModifiedEvent, this->UserViewTransformCallbackCommand);
  }
  this->Modified();
  this->ComputeViewTransform();
  this->ComputeDistance();
...
//----------------------------------------------------------------------------
void svtkCamera::SetFocalPoint(double x, double y, double z)
{
  ...
  svtkDebugMacro(<< " FocalPoint set to ( " << this->FocalPoint[0] << ", " << this->FocalPoint[1]
                << ", " << this->FocalPoint[2] << ")");
...
//----------------------------------------------------------------------------
void svtkCamera::SetViewUp(double x, double y, double z)
{
  ...
  svtkDebugMacro(<< " ViewUp set to ( " << this->ViewUp[0] << ", " << this->ViewUp[1] << ", "
                << this->ViewUp[2] << ")");
...
void svtkCamera::ComputeViewTransform()
{
...
//----------------------------------------------------------------------------
void svtkCamera::ComputeCameraLightTransform()
{
  svtkTransform* t;
  double d;
...
//----------------------------------------------------------------------------
void svtkCamera::ComputeWorldToScreenMatrix()
{
  ...
  if (this->WorldToScreenMatrixMTime.GetMTime() < this->GetMTime())
  {
    ...
    svtkMath::Normalize(xAxis);
    svtkMath::Normalize(yAxis);
    svtkMath::Cross(xAxis, yAxis, zAxis);
    svtkMath::Normalize(zAxis);
...
//----------------------------------------------------------------------------
void svtkCamera::ComputeOffAxisProjectionFrustum()
{
  ...
  // svtkMatrix::MultiplyPoint expect homogeneous coordinate.
  ...
  svtkMatrix4x4::Multiply4x4(this->ProjectionTransform->GetMatrix(), this->WorldToScreenMatrix,
    this->ProjectionTransform->GetMatrix());
}

//----------------------------------------------------------------------------
void svtkCamera::ComputeModelViewMatrix()
{
  ...
  {
    svtkMatrix4x4::Multiply4x4(this->ViewTransform->GetMatrix(), this->ModelTransformMatrix,
      this->ModelViewTransform->GetMatrix());
  }
}

//----------------------------------------------------------------------------
void svtkCamera::OrthogonalizeViewUp()
{
  // the orthogonalized ViewUp is just the second row of the view matrix
  svtkMatrix4x4* matrix = this->ViewTransform->GetMatrix();
...
void svtkCamera::SetDistance(double d)
{
  if (this->Distance == d)
  {
    return;
  ...
  if (this->Distance < 0.0002)
  {
    ...
    svtkDebugMacro(<< " Distance is set to minimum.");
  }
  ...
  svtkDebugMacro(<< " Distance set to ( " << this->Distance << ")");
...
void svtkCamera::ComputeDistance()
{
...
void svtkCamera::Dolly(double amount)
{
  if (amount <= 0.0)
  {
    return;
...
void svtkCamera::SetRoll(double roll)
{
  // roll is a rotation of camera view up about the direction of projection
  svtkDebugMacro(<< " Setting Roll to " << roll << "");
...
void svtkCamera::Roll(double angle)
{
  double newViewUp[3];
  this->Transform->Identity();
...
// position.
void svtkCamera::Yaw(double angle)
{
...
void svtkCamera::Pitch(double angle)
{
...
void svtkCamera::Azimuth(double angle)
{
...
void svtkCamera::Elevation(double angle)
{
...
void svtkCamera::ApplyTransform(svtkTransform* t)
{
...
//----------------------------------------------------------------------------
void svtkCamera::SetParallelProjection(svtkTypeBool flag)
{
...
//----------------------------------------------------------------------------
void svtkCamera::SetViewAngle(double angle)
{
...
//----------------------------------------------------------------------------
void svtkCamera::SetUseHorizontalViewAngle(svtkTypeBool flag)
{
...
//----------------------------------------------------------------------------
void svtkCamera::SetParallelScale(double scale)
{
...
void svtkCamera::Zoom(double amount)
{
  if (amount <= 0.0)
  {
    return;
...
//----------------------------------------------------------------------------
void svtkCamera::SetClippingRange(double nearz, double farz)
{
  double thickness;
  ...
  if (nearz > farz)
  {
    svtkDebugMacro(<< " Front and back clipping range reversed");
    double temp = nearz;
    nearz = farz;
    farz = temp;
  }
  ...
  {
    thickness = 1e-20;
    svtkDebugMacro(<< " ClippingRange thickness is set to minimum.");
  ...
  svtkDebugMacro(<< " ClippingRange set to ( " << this->ClippingRange[0] << ", "
...
void svtkCamera::SetThickness(double s)
{
  if (this->Thickness == s)
  {
    return;
...
//----------------------------------------------------------------------------
void svtkCamera::SetWindowCenter(double x, double y)
{
...
//----------------------------------------------------------------------------
void svtkCamera::SetObliqueAngles(double alpha, double beta)
{
  alpha = svtkMath::RadiansFromDegrees(alpha);
  beta = svtkMath::RadiansFromDegrees(beta);
...
//
void svtkCamera::SetViewShear(double dxdz, double dydz, double center)
{
...
void svtkCamera::SetViewShear(double d[3])
{
...
void svtkCamera::ComputeProjectionTransform(double aspect, double nearz, double farz)
{
  ...
  else
  {
    ...
    double tmp = tan(svtkMath::RadiansFromDegrees(this->ViewAngle) / 2.);
    double width;
    double height;
...
svtkMatrix4x4* svtkCamera::GetProjectionTransformMatrix(svtkRenderer* ren)
{
  double aspect[2];
  int lowerLeft[2];
  int usize, vsize;
...
//----------------------------------------------------------------------------
// Return the projection transform matrix. See ComputeProjectionTransform.
svtkMatrix4x4* svtkCamera::GetProjectionTransformMatrix(double aspect, double nearz, double farz)
{
...
svtkPerspectiveTransform* svtkCamera::GetProjectionTransformObject(
...
//----------------------------------------------------------------------------
// Return the projection transform matrix. See ComputeProjectionTransform.
svtkMatrix4x4* svtkCamera::GetCompositeProjectionTransformMatrix(
  double aspect, double nearz, double farz)
{
...
svtkMatrix4x4* svtkCamera::GetCameraLightTransformMatrix()
{
  // return the transform
...
//----------------------------------------------------------------------------
void svtkCamera::ComputeViewPlaneNormal()
{
  if (this->ViewShear[0] != 0.0 || this->ViewShear[1] != 0.0)
  {
...
// the view frustum.
void svtkCamera::GetFrustumPlanes(double aspect, double planes[24])
{
  int i;
  ...
  svtkMatrix4x4::DeepCopy(*matrix, this->GetCompositeProjectionTransformMatrix(aspect, -1, +1));
  ...
  for (i = 0; i < 6; i++)
  {
    svtkMatrix4x4::MultiplyPoint(*matrix, normals[i], normals[i]);
...
//----------------------------------------------------------------------------
svtkMTimeType svtkCamera::GetViewingRaysMTime()
{
  return this->ViewingRaysMTime.GetMTime();
}

//----------------------------------------------------------------------------
void svtkCamera::ViewingRaysModified()
{
...
void svtkCamera::ShallowCopy(svtkCamera* source)
{
...
// \pre source_exists!=0
// \pre not_this: source!=this
void svtkCamera::DeepCopy(svtkCamera* source)
{
  assert("pre: source_exists" && source != nullptr);
  assert("pre: not_this" && source != this);
  ...
  else
  {
    ...
    {
      this->UserTransform =
        static_cast<svtkHomogeneousTransform*>(source->UserTransform->MakeTransform());
    }
  ...
  else
  {
    if (this->UserViewTransform == nullptr)
    {
      this->UserViewTransform =
        static_cast<svtkHomogeneousTransform*>(source->UserViewTransform->MakeTransform());
    }
  ...
  else
  {
    if (this->ViewTransform == nullptr)
    {
      this->ViewTransform = static_cast<svtkTransform*>(source->ViewTransform->MakeTransform());
    }
  ...
  else
  {
    if (this->ProjectionTransform == nullptr)
    {
      this->ProjectionTransform =
        static_cast<svtkPerspectiveTransform*>(source->ProjectionTransform->MakeTransform());
    }
  ...
  else
  {
    if (this->Transform == nullptr)
    {
      this->Transform = static_cast<svtkPerspectiveTransform*>(source->Transform->MakeTransform());
    }
  ...
  else
  {
    if (this->CameraLightTransform == nullptr)
    {
      this->CameraLightTransform =
        static_cast<svtkTransform*>(source->CameraLightTransform->MakeTransform());
    }
  ...
  else
  {
    if (this->ModelViewTransform == nullptr)
    {
      this->ModelViewTransform =
        static_cast<svtkTransform*>(source->ModelViewTransform->MakeTransform());
    }
  ...
  else
  {
    if (this->ModelTransformMatrix == nullptr)
    {
      this->ModelTransformMatrix =
        static_cast<svtkMatrix4x4*>(source->ModelTransformMatrix->NewInstance());
    }
  ...
  else
  {
    if (this->EyeTransformMatrix == nullptr)
    {
      this->EyeTransformMatrix =
        static_cast<svtkMatrix4x4*>(source->EyeTransformMatrix->NewInstance());
    }
  ...
  else
  {
    if (this->WorldToScreenMatrix == nullptr)
    {
      this->WorldToScreenMatrix =
        static_cast<svtkMatrix4x4*>(source->WorldToScreenMatrix->NewInstance());
    }
...
// \pre source_exists!=0
// \pre not_this: source!=this
void svtkCamera::PartialCopy(svtkCamera* source)
{
  assert("pre: source_exists" && source != nullptr);
  assert("pre: not_this" && source != this);
...
//----------------------------------------------------------------------------
void svtkCamera::PrintSelf(ostream& os, svtkIndent indent)
{
...
//-----------------------------------------------------------------------------
void svtkCamera::SetEyePosition(double eyePosition[3])
{
  if (!eyePosition)
  {
    svtkErrorMacro(<< "ERROR: Invalid or nullptr eye position\n");
    return;
  }
...
//-----------------------------------------------------------------------------
void svtkCamera::GetEyePosition(double eyePosition[3])
{
  if (!eyePosition)
  {
    svtkErrorMacro(<< "ERROR: Invalid or nullptr eye position\n");
...
//-----------------------------------------------------------------------------
void svtkCamera::GetEyePlaneNormal(double normal[3])
{
  if (!normal)
  {
  ...
  // Just to be sure.
  svtkMath::Normalize(localNormal);
...
//-----------------------------------------------------------------------------
svtkMatrix4x4* svtkCamera::GetModelViewTransformMatrix()
{
...
//-----------------------------------------------------------------------------
svtkTransform* svtkCamera::GetModelViewTransformObject()
{
  this->ComputeModelViewMatrix();
...
//-----------------------------------------------------------------------------
svtkMatrix4x4* svtkCamera::GetViewTransformMatrix()
{
  return this->GetModelViewTransformMatrix();
}

//-----------------------------------------------------------------------------
svtkTransform* svtkCamera::GetViewTransformObject()
{
  return this->GetModelViewTransformObject();
}

//-----------------------------------------------------------------------------
double* svtkCamera::GetOrientation()
{
  return this->ViewTransform->GetOrientation();
}

//-----------------------------------------------------------------------------
double* svtkCamera::GetOrientationWXYZ()
{
  return this->ViewTransform->GetOrientationWXYZ();
}

// ----------------------------------------------------------------------------
void svtkCamera::SetEyeTransformMatrix(const double elements[16])
{
...
// ----------------------------------------------------------------------------
void svtkCamera::SetModelTransformMatrix(const double elements[16])
{
