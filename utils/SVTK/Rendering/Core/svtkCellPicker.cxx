...
svtkStandardNewMacro(svtkCellPicker);

//----------------------------------------------------------------------------
svtkCellPicker::svtkCellPicker()
{
  // List of locators for accelerating polydata picking
  this->Locators = svtkCollection::New();
  ...
  this->Gradients = svtkDoubleArray::New();
...
//----------------------------------------------------------------------------
void svtkCellPicker::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
void svtkCellPicker::Initialize()
{
  this->ResetPickInfo();
...
//----------------------------------------------------------------------------
void svtkCellPicker::ResetPickInfo()
{
  // First, reset information from the superclass, since
  // svtkPicker does not have a ResetPickInfo method
...
//----------------------------------------------------------------------------
void svtkCellPicker::ResetCellPickerInfo()
{
...
//----------------------------------------------------------------------------
void svtkCellPicker::AddLocator(svtkAbstractCellLocator* locator)
{
...
//----------------------------------------------------------------------------
void svtkCellPicker::RemoveLocator(svtkAbstractCellLocator* locator)
{
  this->Locators->RemoveItem(locator);
}

//----------------------------------------------------------------------------
void svtkCellPicker::RemoveAllLocators()
{
  this->Locators->RemoveAllItems();
}

//----------------------------------------------------------------------------
int svtkCellPicker::Pick(
  double selectionX, double selectionY, double selectionZ, svtkRenderer* renderer)
{
  ...
  if ((pickResult = this->Superclass::Pick(selectionX, selectionY, selectionZ, renderer)) == 0)
  {
    ...
    svtkMath::Normalize(this->PickNormal);
  }
...
//----------------------------------------------------------------------------
int svtkCellPicker::Pick3DRay(double pos[3], double orient[4], svtkRenderer* renderer)
{
  int pickResult = 0;
...
#define SVTKCELLPICKER_PLANE_TOL 1e-14

double svtkCellPicker::IntersectWithLine(const double p1[3], const double p2[3], double tol,
  svtkAssemblyPath* path, svtkProp3D* prop, svtkAbstractMapper3D* m)
{
  svtkMapper* mapper = nullptr;
  svtkAbstractVolumeMapper* volumeMapper = nullptr;
  svtkImageMapper3D* imageMapper = nullptr;
  svtkAbstractHyperTreeGridMapper* htgMapper = nullptr;
  ...
  // HyperTreeGrid
  else if ((htgMapper = svtkAbstractHyperTreeGridMapper::SafeDownCast(m)))
  {
    tMin = this->IntersectHyperTreeGridWithLine(p1, p2, t1, t2, htgMapper);
  }

  // Volume
  else if ((volumeMapper = svtkAbstractVolumeMapper::SafeDownCast(m)))
  {
    tMin = this->IntersectVolumeWithLine(p1, p2, t1, t2, prop, volumeMapper);
  }

  // Image
  else if ((imageMapper = svtkImageMapper3D::SafeDownCast(m)))
  {
    tMin = this->IntersectImageWithLine(p1, p2, t1, t2, prop, imageMapper);
  }

  // Actor
  else if ((mapper = svtkMapper::SafeDownCast(m)))
  {
  ...
  if (tMin < this->GlobalTMin)
  {
    ...
    if (fabs(tMin - t1) < SVTKCELLPICKER_PLANE_TOL && clippingPlaneId >= 0)
    {
...
//----------------------------------------------------------------------------
double svtkCellPicker::IntersectActorWithLine(const double p1[3], const double p2[3], double t1,
  double t2, double tol, svtkProp3D* prop, svtkMapper* mapper)
{
  ...
  double minPCoords[3];
  double pDistMin = SVTK_DOUBLE_MAX;
  svtkIdType minCellId = -1;
  int minSubId = -1;
  double minXYZ[3];
  minXYZ[0] = minXYZ[1] = minXYZ[2] = 0.0;
  svtkAbstractCellLocator* locator = nullptr;
  svtkIdType flatIndex = -1;

  svtkDataSet* data = mapper->GetInput();
  if (data)
  {
  ...
  else
  {
    ...
    if (composite)
    {
      ...
      for (auto node : svtk::Range(composite, Opts::SkipEmptyNodes))
      {
        svtkDataSet* ds = svtkDataSet::SafeDownCast(node.GetDataObject());
        if (!ds)
        {
          svtkDebugMacro(<< "Skipping " << node.GetDataObject()->GetClassName() << " block at index "
        ...
        if (!svtkBox::IntersectBox(bounds, const_cast<double*>(p1), ray, xyzDummy, tDummy))
        {
  ...
  if (minCellId >= 0 && tMin < this->GlobalTMin)
  {
    ...
    svtkGenericCell* cell = this->Cell;
    ...
    svtkLODProp3D* lodActor = nullptr;
    if ((actor = svtkActor::SafeDownCast(prop)))
    {
      this->Texture = actor->GetTexture();
    }
    else if ((lodActor = svtkLODProp3D::SafeDownCast(prop)))
    {
    ...
    if (this->PickTextureData && this->Texture)
    {
      ...
      svtkImageData* image = this->Texture->GetInput();
      this->DataSet = image;
    ...
    if (!this->ComputeSurfaceNormal(data, cell, weights, this->MapperNormal))
    {
      ...
      svtkMath::Normalize(this->MapperNormal);
    }
...
bool svtkCellPicker::IntersectDataSetWithLine(svtkDataSet* dataSet, const double p1[3],
  const double p2[3], double t1, double t2, double tol, svtkAbstractCellLocator*& locator,
  svtkIdType& minCellId, int& minSubId, double& tMin, double& pDistMin, double minXYZ[3],
  double minPCoords[3])
{
  bool cellWasPicked = false;
  // Polydata has no 3D cells
  svtkTypeBool isPolyData = dataSet->IsA("svtkPolyData");
  ...
  while (
    (locator = static_cast<svtkAbstractCellLocator*>(this->Locators->GetNextItemAsObject(iter))))
  {
  ...
  {
    svtkIdList* pointIds = this->PointIds;
...
//----------------------------------------------------------------------------
double svtkCellPicker::IntersectHyperTreeGridWithLine(const double p1[3], const double p2[3],
  double t1, double t2, svtkAbstractHyperTreeGridMapper* mapper)
{
  // Retrieve input grid
  svtkUniformHyperTreeGrid* grid = svtkUniformHyperTreeGrid::SafeDownCast(mapper->GetDataSetInput());
  if (grid == nullptr)
  {
  ...
  normal[3] += svtkMath::Dot(origin, normal);
  double norm = svtkMath::Norm(normal);
  ...
  if (tMin < this->GlobalTMin)
  {
    ...
    // when we do make start to exercise this we can add a new dataobject to svtkPicker::DataObject
...
//----------------------------------------------------------------------------
bool svtkCellPicker::RecursivelyProcessTree(
  svtkHyperTreeGridNonOrientedGeometryCursor* cursor, int level)
{
  ...
  int numChildren = svtkHyperTreeGrid::SafeDownCast(this->DataSet)->GetNumberOfChildren();
...
//----------------------------------------------------------------------------
// Intersect a svtkVolume with a line by ray casting.
...
#define SVTKCELLPICKER_VOXEL_TOL 1e-6

double svtkCellPicker::IntersectVolumeWithLine(const double p1[3], const double p2[3], double t1,
  double t2, svtkProp3D* prop, svtkAbstractVolumeMapper* mapper)
{
  ...
  svtkVolumeProperty* property = nullptr;
  svtkVolume* volume = nullptr;
  svtkLODProp3D* lodVolume = nullptr;
  if ((volume = svtkVolume::SafeDownCast(prop)))
  {
    property = volume->GetProperty();
  }
  else if ((lodVolume = svtkLODProp3D::SafeDownCast(prop)))
  {
  ...
  double rayLength = sqrt(svtkMath::Distance2BetweenPoints(x1, x2)) * (t2 - t1);

  // This is the minimum increment that will be allowed
  double tTol = SVTKCELLPICKER_VOXEL_TOL / rayLength * (t2 - t1);
  ...
  svtkDataArray* scalars = svtkDataArray::CreateDataArray(data->GetScalarType());
  scalars->SetNumberOfComponents(numComponents);
  svtkIdType scalarArraySize = numComponents * data->GetNumberOfPoints();
  ...
  {
    svtkPiecewiseFunction* scalarOpacity =
    ...
    svtkPiecewiseFunction* gradientOpacity = nullptr;
    ...
    while (t <= t2)
    {
      ...
      for (int k = 0; k < 3; k++)
      {
        ...
        if (fabs(x2[k] - x1[k]) > SVTKCELLPICKER_VOXEL_TOL * rayLength)
        {
          ...
          if (x2[k] > x1[k])
          {
            nextX = svtkMath::Floor(lastX + SVTKCELLPICKER_VOXEL_TOL) + 1;
          }
          else
          {
            nextX = svtkMath::Ceil(lastX - SVTKCELLPICKER_VOXEL_TOL) - 1;
          }
    ...
    if (opacity > opacityThreshold)
    {
      ...
      if (t < tMin && t < this->GlobalTMin)
      {
        ...
        else
        {
          ...
          int* ci = this->CellIJK;
          double weights[8];
          svtkVoxel::InterpolationFunctions(this->PCoords, weights);
...
//----------------------------------------------------------------------------
double svtkCellPicker::IntersectImageWithLine(const double p1[3], const double p2[3], double t1,
  double t2, svtkProp3D* prop, svtkImageMapper3D* imageMapper)
{
  // Get the image information
  svtkImageData* data = imageMapper->GetInput();
  int extent[6];
  data->GetExtent(extent);
  ...
  double tMin, tMax;
  if (!svtkBox::IntersectWithLine(bounds, x1, x2, tMin, tMax, nullptr, nullptr, planeId, plane2Id))
  {
    return SVTK_DOUBLE_MAX;
  }
  ...
  if (tMin != tMax)
  {
    ...
    double w1 = svtkMath::Dot(x1, xnormal) + xnormal[3];
    double w2 = svtkMath::Dot(x2, xnormal) + xnormal[3];
    if (w1 * w2 > SVTKCELLPICKER_VOXEL_TOL)
    {
      return SVTK_DOUBLE_MAX;
    }
    if (w1 * w2 < 0)
  ...
  if (tMin < this->GlobalTMin)
  {
    ...
    for (int j = 0; j < 3; j++)
    {
      ...
      if (x[j] < bounds[2 * j])
      {
        if (x[j] < bounds[2 * j] - SVTKCELLPICKER_VOXEL_TOL)
        {
          return SVTK_DOUBLE_MAX;
        }
        x[j] = bounds[2 * j];
      ...
      {
        if (x[j] > bounds[2 * j + 1] + SVTKCELLPICKER_VOXEL_TOL)
        {
          return SVTK_DOUBLE_MAX;
        }
        x[j] = bounds[2 * j + 1];
...
//----------------------------------------------------------------------------
// This is a catch-all for Prop3D types that svtkCellPicker does not
// recognize.  It can be overridden in subclasses to provide support
// for picking new Prop3D types.

double svtkCellPicker::IntersectProp3DWithLine(
  const double*, const double*, double, double, double, svtkProp3D*, svtkAbstractMapper3D*)
{
  return SVTK_DOUBLE_MAX;
}
...
int svtkCellPicker::ClipLineWithPlanes(svtkAbstractMapper3D* mapper, svtkMatrix4x4* mat,
...
int svtkCellPicker::ClipLineWithExtent(
  const int extent[6], const double x1[3], const double x2[3], double& t1, double& t2, int& planeId)
{
  ...
  int p2;
  return svtkBox::IntersectWithLine(bounds, x1, x2, t1, t2, nullptr, nullptr, planeId, p2);
}
...
int svtkCellPicker::ComputeSurfaceNormal(
  svtkDataSet* data, svtkCell* cell, const double* weights, double normal[3])
{
  ...
  if (normals)
  {
    ...
    svtkIdType numPoints = cell->GetNumberOfPoints();
    for (svtkIdType k = 0; k < numPoints; k++)
    {
  ...
  {
    svtkPolygon::ComputeNormal(cell->Points, normal);
  }
  else
  {
    return 0;
...
int svtkCellPicker::ComputeSurfaceTCoord(
...
void svtkCellPicker::SubCellFromCell(svtkGenericCell* cell, int subId)
{
  switch (cell->GetCellType())
  {
    ...
    case SVTK_POLY_VERTEX:
    {
      ...
      svtkIdType pointId = cell->PointIds->GetId(subId);
...
//----------------------------------------------------------------------------
int svtkCellPicker::HasSubCells(int cellType)
{
  switch (cellType)
  {
    case SVTK_TRIANGLE_STRIP:
...
int svtkCellPicker::GetNumberOfSubCells(svtkIdList* pointIds, int cellType)
{
  switch (cellType)
  {
    case SVTK_TRIANGLE_STRIP:
...
// requires a svtkIdList that contains the pointIds for the cell.
void svtkCellPicker::GetSubCell(
  svtkDataSet* data, svtkIdList* ptIds, int subId, int cellType, svtkGenericCell* cell)
{
  switch (cellType)
  {
    case SVTK_TRIANGLE_STRIP:
    ...
    case SVTK_POLY_VERTEX:
    {
      ...
      svtkIdType pointId = ptIds->GetId(subId);
...
void svtkCellPicker::SetImageDataPickInfo(const double x[3], const int extent[6])
{
  for (int j = 0; j < 3; j++)
  {
    double xj = x[j];
    ...
    this->CellIJK[j] = svtkMath::Floor(xj);
  ...
  this->PointId = svtkStructuredData::ComputePointIdForExtent(ext, this->PointIJK);

  this->CellId = svtkStructuredData::ComputeCellIdForExtent(ext, this->CellIJK);
...
double svtkCellPicker::ComputeVolumeOpacity(const int xi[3], const double pcoords[3],
  svtkImageData* data, svtkDataArray* scalars, svtkPiecewiseFunction* scalarOpacity,
  svtkPiecewiseFunction* gradientOpacity)
{
  ...
  double weights[8];
  svtkVoxel::InterpolationFunctions(const_cast<double*>(pcoords), weights);
  ...
  svtkIdType xInc = 1;
  svtkIdType yInc = extent[1] - extent[0] + 1;
  svtkIdType zInc = yInc * (extent[3] - extent[2] + 1);
  if (xi[0] == extent[1])
  {
    xInc = 0;
  }
  ...
  svtkIdType ptId = data->ComputePointId(const_cast<int*>(xi));
  double val = 0.0;
  for (int j = 0; j < 8; j++)
  {
    svtkIdType ptInc = (j & 1) * xInc + ((j >> 1) & 1) * yInc + ((j >> 2) & 1) * zInc;
  ...
  else if (scalarType == SVTK_FLOAT || scalarType == SVTK_DOUBLE)
  {
    opacity *= val;
  }
  else
