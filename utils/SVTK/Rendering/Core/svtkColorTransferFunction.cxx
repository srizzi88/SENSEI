/*=========================================================================
  ...
  Module:    svtkColorTransferFunction.cxx
...
svtkStandardNewMacro(svtkColorTransferFunction);
...
class svtkCTFCompareNodes
{
public:
  bool operator()(const svtkCTFNode* node1, const svtkCTFNode* node2) { return node1->X < node2->X; }
};

class svtkCTFFindNodeEqual
{
public:
  double X;
  bool operator()(const svtkCTFNode* node) { return node->X == this->X; }
};

class svtkCTFFindNodeInRange
{
public:
  double X1;
  double X2;
  bool operator()(const svtkCTFNode* node) { return (node->X >= this->X1 && node->X <= this->X2); }
};

class svtkCTFFindNodeOutOfRange
{
public:
  double X1;
  double X2;
...
class svtkColorTransferFunctionInternals
{
public:
  std::vector<svtkCTFNode*> Nodes;
  svtkCTFCompareNodes CompareNodes;
  svtkCTFFindNodeEqual FindNodeEqual;
  svtkCTFFindNodeInRange FindNodeInRange;
  svtkCTFFindNodeOutOfRange FindNodeOutOfRange;
};

//=============================================================================
// Convert to and from a special polar version of CIELAB (useful for creating
// continuous diverging color maps).
inline void svtkColorTransferFunctionLabToMsh(const double lab[3], double msh[3])
{
...
inline void svtkColorTransferFunctionMshToLab(const double msh[3], double lab[3])
{
...
inline double svtkColorTransferFunctionAngleDiff(double a1, double a2)
{
  double adiff = a1 - a2;
  if (adiff < 0.0)
    adiff = -adiff;
  while (adiff >= 2.0 * svtkMath::Pi())
    adiff -= (2.0 * svtkMath::Pi());
  if (adiff > svtkMath::Pi())
    adiff = (2.0 * svtkMath::Pi()) - adiff;
  return adiff;
}

// For the case when interpolating from a saturated color to an unsaturated
// color, find a hue for the unsaturated color that makes sense.
inline double svtkColorTransferFunctionAdjustHue(const double msh[3], double unsatM)
{
  if (msh[0] >= unsatM - 0.1)
  {
...
inline void svtkColorTransferFunctionInterpolateDiverging(
  double s, const double rgb1[3], const double rgb2[3], double result[3])
{
  double lab1[3], lab2[3];
  svtkMath::RGBToLab(rgb1, lab1);
  svtkMath::RGBToLab(rgb2, lab2);

  double msh1[3], msh2[3];
  svtkColorTransferFunctionLabToMsh(lab1, msh1);
  svtkColorTransferFunctionLabToMsh(lab2, msh2);

  // If the endpoints are distinct saturated colors, then place white in between
  // them.
  if ((msh1[1] > 0.05) && (msh2[1] > 0.05) &&
    (svtkColorTransferFunctionAngleDiff(msh1[2], msh2[2]) > 0.33 * svtkMath::Pi()))
  {
  ...
  if ((msh1[1] < 0.05) && (msh2[1] > 0.05))
  {
    msh1[2] = svtkColorTransferFunctionAdjustHue(msh2, msh1[0]);
  }
  else if ((msh2[1] < 0.05) && (msh1[1] > 0.05))
  {
    msh2[2] = svtkColorTransferFunctionAdjustHue(msh1, msh2[0]);
  }
  ...
  double labTmp[3];
  svtkColorTransferFunctionMshToLab(mshTmp, labTmp);
  svtkMath::LabToRGB(labTmp, result);
}

// Interpolate a LAB/CIEDE2000 color map.
inline void svtkColorTransferFunctionInterpolateLABCIEDE2000(double s, const double rgb1[3],
  const double rgb2[3], double result[3], svtkSmartPointer<svtkColorTransferFunction>& cachedPathCTF,
  bool forceExactSupportColors = false)
{
    ...
    (val[1] != rgb2[0] || val[2] != rgb2[1] || val[3] != rgb2[2]))
  {
    ...
    {
      cachedPathCTF = svtkSmartPointer<svtkColorTransferFunction>::New();
    }
    else
    {
...
//----------------------------------------------------------------------------
// Construct a new svtkColorTransferFunction with default values
svtkColorTransferFunction::svtkColorTransferFunction()
{
  ...
  this->Clamping = 1;
  this->ColorSpace = SVTK_CTF_RGB;
  this->HSVWrap = 1; // By default HSV will be wrap

  this->Scale = SVTK_CTF_LINEAR;
  ...
  this->Internal = new svtkColorTransferFunctionInternals;
}

//----------------------------------------------------------------------------
// Destruct a svtkColorTransferFunction
svtkColorTransferFunction::~svtkColorTransferFunction()
{
  delete[] this->Table;
...
int svtkColorTransferFunction::GetSize()
{
...
double* svtkColorTransferFunction::GetDataPointer()
{
...
int svtkColorTransferFunction::AddRGBPoint(double x, double r, double g, double b)
{
...
//----------------------------------------------------------------------------
// Add a point defined in RGB
int svtkColorTransferFunction::AddRGBPoint(
  double x, double r, double g, double b, double midpoint, double sharpness)
{
  // Error check
  if (midpoint < 0.0 || midpoint > 1.0)
  {
    svtkErrorMacro("Midpoint outside range [0.0, 1.0]");
    return -1;
  }

  if (sharpness < 0.0 || sharpness > 1.0)
  {
    svtkErrorMacro("Sharpness outside range [0.0, 1.0]");
    return -1;
  }
  ...
  // Create the new node
  svtkCTFNode* node = new svtkCTFNode;
  node->X = x;
  node->R = r;
  node->G = g;
  node->B = b;
...
int svtkColorTransferFunction::AddHSVPoint(double x, double h, double s, double v)
{
  double r, b, g;
...
//----------------------------------------------------------------------------
// Add a point defined in HSV
int svtkColorTransferFunction::AddHSVPoint(
...
// the Range
void svtkColorTransferFunction::SortAndUpdateRange()
{
  std::sort(
...
//----------------------------------------------------------------------------
bool svtkColorTransferFunction::UpdateRange()
{
  double oldRange[2];
...
//----------------------------------------------------------------------------
// Remove a point
int svtkColorTransferFunction::RemovePoint(double x)
{
  ...
  std::vector<svtkCTFNode*>::iterator iter = std::find_if(
...
//----------------------------------------------------------------------------
void svtkColorTransferFunction::MovePoint(double oldX, double newX)
{
  if (oldX == newX)
  {
    // Nothing to do.
...
//----------------------------------------------------------------------------
// Remove all points
void svtkColorTransferFunction::RemoveAllPoints()
{
  for (unsigned int i = 0; i < this->Internal->Nodes.size(); i++)
  {
    delete this->Internal->Nodes[i];
...
void svtkColorTransferFunction::AddRGBSegment(
...
void svtkColorTransferFunction::AddHSVSegment(
  double x1, double h1, double s1, double v1, double x2, double h2, double s2, double v2)
{
  double r1, r2, b1, b2, g1, g2;

  svtkMath::HSVToRGB(h1, s1, v1, &r1, &g1, &b1);
  svtkMath::HSVToRGB(h2, s2, v2, &r2, &g2, &b2);
  this->AddRGBSegment(x1, r1, g1, b1, x2, r2, g2, b2);
}

//----------------------------------------------------------------------------
// Returns the RGBA color evaluated at the specified location
const unsigned char* svtkColorTransferFunction::MapValue(double x)
{
  double rgb[3];
  this->GetColor(x, rgb);
...
void svtkColorTransferFunction::GetColor(double x, double rgb[3])
{
  if (this->IndexedLookup)
  {
    int numNodes = this->GetSize();
    svtkVariant xv(x);
    svtkIdType idx = this->GetAnnotatedValueIndexInternal(xv);
    if (idx < 0 || numNodes == 0)
    {
...
double svtkColorTransferFunction::GetRedValue(double x)
{
  double rgb[3];
  this->GetColor(x, rgb);
...
double svtkColorTransferFunction::GetGreenValue(double x)
{
  double rgb[3];
  this->GetColor(x, rgb);
...
double svtkColorTransferFunction::GetBlueValue(double x)
{
  double rgb[3];
  this->GetColor(x, rgb);
...
void svtkColorTransferFunction::GetTable(double xStart, double xEnd, int size, double* table)
{
  int i, j;
  ...
  // the NaN color.
  if (svtkMath::IsNan(xStart) || svtkMath::IsNan(xEnd))
  {
  ...
  bool usingLogScale = this->Scale == SVTK_CTF_LOG10;
  if (usingLogScale)
  {
  ...
  svtkSmartPointer<svtkColorTransferFunction> ciede2000Helper = nullptr;
  ...
  for (i = 0; i < size; i++)
  {
    ...
    else if (x < this->Range[0] || (svtkMath::IsInf(x) && x < 0))
    {
      tptr[0] = 0.0;
      tptr[1] = 0.0;
      tptr[2] = 0.0;
    ...
    else
    {
      ...
      if (sharpness < 0.01)
      {
        ...
        if (this->ColorSpace == SVTK_CTF_RGB)
        {
        ...
        else if (this->ColorSpace == SVTK_CTF_STEP)
        {
          tptr[0] = rgb2[0];
          tptr[1] = rgb2[1];
          tptr[2] = rgb2[2];
        }
        else if (this->ColorSpace == SVTK_CTF_HSV)
        {
          double hsv1[3], hsv2[3];
          svtkMath::RGBToHSV(rgb1, hsv1);
          svtkMath::RGBToHSV(rgb2, hsv2);
          ...
          svtkMath::HSVToRGB(hsvTmp, tptr);
        }
        else if (this->ColorSpace == SVTK_CTF_LAB)
        {
          double lab1[3], lab2[3];
          svtkMath::RGBToLab(rgb1, lab1);
          svtkMath::RGBToLab(rgb2, lab2);
          ...
          // Now convert back to RGB
          svtkMath::LabToRGB(labTmp, tptr);
        }
        else if (this->ColorSpace == SVTK_CTF_DIVERGING)
        {
          svtkColorTransferFunctionInterpolateDiverging(s, rgb1, rgb2, tptr);
        }
        else if (this->ColorSpace == SVTK_CTF_LAB_CIEDE2000)
        {
          svtkColorTransferFunctionInterpolateLABCIEDE2000(s, rgb1, rgb2, tptr, ciede2000Helper);
        }
        else
        {
          svtkErrorMacro("ColorSpace set to invalid value.");
        }
        continue;
      }
      ...
      else if (this->ColorSpace == SVTK_CTF_LAB_CIEDE2000)
      {
        ...
        svtkColorTransferFunctionInterpolateLABCIEDE2000(val, rgb1, rgb2, tptr, ciede2000Helper);
      }
      else if (this->ColorSpace == SVTK_CTF_DIVERGING)
      {
...
//----------------------------------------------------------------------------
void svtkColorTransferFunction::GetTable(double xStart, double xEnd, int size, float* table)
{
...
//----------------------------------------------------------------------------
const unsigned char* svtkColorTransferFunction::GetTable(double xStart, double xEnd, int size)
{
  ...
  {
    svtkErrorMacro("Attempting to lookup a value with no points in the function");
    return this->Table;
  }
...
//----------------------------------------------------------------------------
void svtkColorTransferFunction::BuildFunctionFromTable(
...
int svtkColorTransferFunction::GetNodeValue(int index, double val[6])
{
  int size = static_cast<int>(this->Internal->Nodes.size());
...
//----------------------------------------------------------------------------
// For a specified index value, get the node parameters
int svtkColorTransferFunction::SetNodeValue(int index, double val[6])
{
  int size = static_cast<int>(this->Internal->Nodes.size());
...
//----------------------------------------------------------------------------
void svtkColorTransferFunction::DeepCopy(svtkScalarsToColors* o)
{
  svtkColorTransferFunction* f = nullptr;
  if (o)
  {
    this->Superclass::DeepCopy(o);
    f = svtkColorTransferFunction::SafeDownCast(o);
  }
...
//----------------------------------------------------------------------------
void svtkColorTransferFunction::ShallowCopy(svtkColorTransferFunction* f)
{
  if (f != nullptr)
  {
...
// and unsigned short.
template <class T>
void svtkColorTransferFunctionMapData(svtkColorTransferFunction* self, T* input,
  unsigned char* output, int length, int inIncr, int outFormat, long)
{
  ...
  if (self->GetSize() == 0)
  {
    svtkGenericWarningMacro("Transfer Function Has No Points!");
    return;
  }
  ...
  while (--i >= 0)
  {
    ...
    if (outFormat == SVTK_RGB || outFormat == SVTK_RGBA)
    {
    ...
    if (outFormat == SVTK_RGBA || outFormat == SVTK_LUMINANCE_ALPHA)
    {
      *(optr++) = alpha;
    }
    iptr += inIncr;
...
static void svtkColorTransferFunctionMapData(svtkColorTransferFunction* self, unsigned char* input,
...
static void svtkColorTransferFunctionMapData(svtkColorTransferFunction* self, unsigned short* input,
  unsigned char* output, int length, int inIncr, int outFormat, int)
{
  int x;
  int i = length;
...
//----------------------------------------------------------------------------
template <class T>
void svtkColorTransferFunctionIndexedMapData(svtkColorTransferFunction* self, T* input,
  unsigned char* output, int length, int inIncr, int outFormat, long)
{
  int i = length;
  double nodeVal[6];
  ...
  {
    if (outFormat == SVTK_RGBA)
    {
      while (--i >= 0)
      {
        vin = *input;
        svtkIdType idx = self->GetAnnotatedValueIndexInternal(vin);
        if (idx < 0 || numNodes == 0)
    ...
    else if (outFormat == SVTK_LUMINANCE_ALPHA)
    {
      while (--i >= 0)
      {
        vin = *input;
...
//----------------------------------------------------------------------------
void svtkColorTransferFunction::MapScalarsThroughTable2(void* input, unsigned char* output,
  int inputDataType, int numberOfValues, int inputIncrement, int outputFormat)
{
  if (this->GetSize() == 0)
  {
    svtkDebugMacro("Transfer Function Has No Points!");
    return;
  }
  if (this->IndexedLookup)
  {
    ...
    {
      // Use svtkExtendedTemplateMacro to cover case of SVTK_STRING input
      svtkExtendedTemplateMacro(svtkColorTransferFunctionIndexedMapData(this,
        static_cast<SVTK_TT*>(input), output, numberOfValues, inputIncrement, outputFormat, 1));

      default:
        svtkErrorMacro(<< "MapImageThroughTable: Unknown input ScalarType");
        return;
    }
  }
  else
  {
    switch (inputDataType)
    {
      svtkTemplateMacro(svtkColorTransferFunctionMapData(this, static_cast<SVTK_TT*>(input), output,
...
//----------------------------------------------------------------------------
svtkIdType svtkColorTransferFunction::GetNumberOfAvailableColors()
{
  ...
  if (this->Table)
  {
    ...
    // has been called.
    return static_cast<svtkIdType>(this->TableSize);
  }
  return 16777216; // 2^24
}

//----------------------------------------------------------------------------
void svtkColorTransferFunction::GetIndexedColor(svtkIdType idx, double rgba[4])
{
  svtkIdType n = this->GetSize();
  if (n > 0 && idx >= 0)
  {
    double nodeValue[6];
...
//----------------------------------------------------------------------------
void svtkColorTransferFunction::FillFromDataPointer(int nb, double* ptr)
{
  if (nb <= 0 || !ptr)
  {
    return;
...
//----------------------------------------------------------------------------
int svtkColorTransferFunction::AdjustRange(double range[2])
{
  if (!range)
  {
    return 0;
  ...
  while (!done)
  {
    ...
    std::vector<svtkCTFNode*>::iterator iter = std::find_if(this->Internal->Nodes.begin(),
...
//--------------------------------------------------------------------------
int svtkColorTransferFunction::EstimateMinNumberOfSamples(double const& x1, double const& x2)
{
...
//----------------------------------------------------------------------------
double svtkColorTransferFunction::FindMinimumXDistance()
{
  std::vector<svtkCTFNode*> const& nodes = this->Internal->Nodes;
...
//----------------------------------------------------------------------------
// Print method for svtkColorTransferFunction
void svtkColorTransferFunction::PrintSelf(ostream& os, svtkIndent indent)
{
  ...
  else if (this->ColorSpace == SVTK_CTF_HSV && this->HSVWrap)
  {
