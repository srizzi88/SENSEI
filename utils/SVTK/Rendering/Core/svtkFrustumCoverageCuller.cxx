/*=========================================================================
  ...
  Module:    svtkFrustumCoverageCuller.cxx
...
svtkStandardNewMacro(svtkFrustumCoverageCuller);

// Create a frustum coverage culler with default values
svtkFrustumCoverageCuller::svtkFrustumCoverageCuller()
{
  this->MinimumCoverage = 0.0;
  this->MaximumCoverage = 1.0;
  this->SortingStyle = SVTK_CULLER_SORT_NONE;
}
...
double svtkFrustumCoverageCuller::Cull(
  svtkRenderer* ren, svtkProp** propList, int& listLength, int& initialized)
{
  svtkProp* prop;
  double total_time;
  double center[3];
  ...
  for (propLoop = 0; propLoop < listLength; propLoop++)
  {
    ...
    {
      distanceList[propLoop] = -SVTK_DOUBLE_MAX;
      coverage = 0.001;
    }
  ...
  if (this->SortingStyle == SVTK_CULLER_SORT_FRONT_TO_BACK)
  {
  ...
  if (this->SortingStyle == SVTK_CULLER_SORT_BACK_TO_FRONT)
  {
    for (propLoop = 1; propLoop < listLength; propLoop++)
    {
      index1 = propLoop;
...
const char* svtkFrustumCoverageCuller::GetSortingStyleAsString()
{
  if (this->SortingStyle == SVTK_CULLER_SORT_NONE)
  {
    return "None";
  }
  if (this->SortingStyle == SVTK_CULLER_SORT_FRONT_TO_BACK)
...
void svtkFrustumCoverageCuller::PrintSelf(ostream& os, svtkIndent indent)
{
