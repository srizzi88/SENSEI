/*=========================================================================
  ...
  Module:    svtkHardwareSelector.cxx
...
{
public:
  bool operator()(const svtkHardwareSelector::PixelInformation& a,
    const svtkHardwareSelector::PixelInformation& b) const
  {
    if (a.Valid != b.Valid)
    {
...
class svtkHardwareSelector::svtkInternals
{
public:
  // Ids for props that were hit.
  std::set<int> HitProps;
  std::map<int, svtkSmartPointer<svtkProp> > Props;
  ...
  typedef std::map<PixelInformation, std::set<svtkIdType>, PixelInformationComparator>
    MapOfAttributeIds;

  typedef std::map<PixelInformation, svtkIdType, PixelInformationComparator> PixelCountType;

  //-----------------------------------------------------------------------------
  svtkSelection* ConvertSelection(
    int fieldassociation, const MapOfAttributeIds& dataMap, const PixelCountType& pixelCounts)
  {
    ...
    for (iter = dataMap.begin(); iter != dataMap.end(); ++iter)
    {
      ...
      const std::set<svtkIdType>& id_values = iter->second;
      svtkSelectionNode* child = svtkSelectionNode::New();
      child->SetContentType(svtkSelectionNode::INDICES);
      switch (fieldassociation)
      {
        case svtkDataObject::FIELD_ASSOCIATION_CELLS:
          child->SetFieldType(svtkSelectionNode::CELL);
          break;

        case svtkDataObject::FIELD_ASSOCIATION_POINTS:
          child->SetFieldType(svtkSelectionNode::POINT);
          break;
      }
      child->GetProperties()->Set(svtkSelectionNode::PROP_ID(), key.PropID);
      child->GetProperties()->Set(svtkSelectionNode::PROP(), key.Prop);

      if (this->ZValues.find(key.PropID) != this->ZValues.end())
      {
        child->GetProperties()->Set(svtkSelectionNode::ZBUFFER_VALUE(), this->ZValues[key.PropID]);
      }

      PixelCountType::const_iterator pit = pixelCounts.find(key);
      child->GetProperties()->Set(svtkSelectionNode::PIXEL_COUNT(), pit->second);
      if (key.ProcessID >= 0)
      {
        child->GetProperties()->Set(svtkSelectionNode::PROCESS_ID(), key.ProcessID);
      }

      child->GetProperties()->Set(svtkSelectionNode::COMPOSITE_INDEX(), key.CompositeID);

      svtkIdTypeArray* ids = svtkIdTypeArray::New();
      ids->SetName("SelectedIds");
      ids->SetNumberOfComponents(1);
      ids->SetNumberOfTuples(static_cast<svtkIdType>(iter->second.size()));
      svtkIdType* ptr = ids->GetPointer(0);
      std::set<svtkIdType>::const_iterator idIter;
      svtkIdType cc = 0;
  ...
  //-----------------------------------------------------------------------------
  bool PixelInsidePolygon(float x, float y, int* polygonPoints, svtkIdType count)
  {
    // http://en.wikipedia.org/wiki/Point_in_polygon
    // RayCasting method shooting the ray along the x axis, using float
    bool inside = false;
    float xintersection;
    for (svtkIdType i = 0; i < count; i += 2)
    {
...
//----------------------------------------------------------------------------
svtkAbstractObjectFactoryNewMacro(svtkHardwareSelector);

//----------------------------------------------------------------------------
svtkCxxSetObjectMacro(svtkHardwareSelector, Renderer, svtkRenderer);

//----------------------------------------------------------------------------
svtkHardwareSelector::svtkHardwareSelector()
{
  this->Internals = new svtkInternals();
  this->Renderer = nullptr;
  this->Area[0] = this->Area[1] = this->Area[2] = this->Area[3] = 0;
  this->FieldAssociation = svtkDataObject::FIELD_ASSOCIATION_CELLS;
...
//----------------------------------------------------------------------------
void svtkHardwareSelector::ReleasePixBuffers()
{
  for (int cc = 0; cc < 10; cc++)
  {
...
//----------------------------------------------------------------------------
void svtkHardwareSelector::BeginSelection()
{
  this->MaximumPointId = 0;
  this->MaximumCellId = 0;
  this->Renderer->Clear();
...
//----------------------------------------------------------------------------
void svtkHardwareSelector::EndSelection()
{
  this->Internals->HitProps.clear();
...
//----------------------------------------------------------------------------
svtkSelection* svtkHardwareSelector::Select()
{
  svtkSelection* sel = nullptr;
...
//----------------------------------------------------------------------------
bool svtkHardwareSelector::CaptureBuffers()
{
  if (!this->Renderer)
  {
    svtkErrorMacro("Renderer must be set before calling Select.");
    return false;
  }
  ...
  {
    svtkErrorMacro("Color buffer depth must be at least 8 bit. "
                  "Currently: "
...
void svtkHardwareSelector::SetPropColorValue(svtkIdType val)
{
  float color[3];
  svtkHardwareSelector::Convert(val + 1, color);
  this->SetPropColorValue(color);
}

//----------------------------------------------------------------------------
bool svtkHardwareSelector::PassRequired(int pass)
{
...
//----------------------------------------------------------------------------
void svtkHardwareSelector::SavePixelBuffer(int passNo)
{
...
//----------------------------------------------------------------------------
void svtkHardwareSelector::ProcessPixelBuffers()
{
  ...
  {
    svtkProp* prop = this->GetPropFromID(ai);
    if (prop)
    {
...
void svtkHardwareSelector::BuildPropHitList(unsigned char* pixelbuffer)
{
...
double svtkHardwareSelector::GetZValue(int val)
{
...
//----------------------------------------------------------------------------
void svtkHardwareSelector::BeginRenderProp()
{
  this->InPropRender++;
...
//----------------------------------------------------------------------------
void svtkHardwareSelector::EndRenderProp()
{
...
//----------------------------------------------------------------------------
void svtkHardwareSelector::RenderCompositeIndex(unsigned int index)
{
...
//----------------------------------------------------------------------------
void svtkHardwareSelector::UpdateMaximumPointId(svtkIdType attribid)
{
  if (attribid < 0)
  {
...
//----------------------------------------------------------------------------
void svtkHardwareSelector::UpdateMaximumCellId(svtkIdType attribid)
{
  if (attribid < 0)
  {
    // negative attribid is valid. It happens when rendering higher order
...
//----------------------------------------------------------------------------
void svtkHardwareSelector::RenderProcessId(unsigned int processid)
{
...
//----------------------------------------------------------------------------
int svtkHardwareSelector::Render(svtkRenderer* renderer, svtkProp** propArray, int propArrayCount)
{
  if (this->Renderer != renderer)
  {
    svtkErrorMacro("Usage error.");
    return 0;
  }
...
//----------------------------------------------------------------------------
svtkProp* svtkHardwareSelector::GetPropFromID(int id)
{
  std::map<int, svtkSmartPointer<svtkProp> >::iterator iter = this->Internals->Props.find(id);
...
//----------------------------------------------------------------------------
std::string svtkHardwareSelector::PassTypeToString(PassTypes type)
{
  switch (type)
  {
    case svtkHardwareSelector::PROCESS_PASS:
      return "PROCESS_PASS";
    case svtkHardwareSelector::ACTOR_PASS:
      return "ACTOR_PASS";
    case svtkHardwareSelector::COMPOSITE_INDEX_PASS:
...
//----------------------------------------------------------------------------
bool svtkHardwareSelector::IsPropHit(int id)
{
...
//----------------------------------------------------------------------------
svtkHardwareSelector::PixelInformation svtkHardwareSelector::GetPixelInformation(
  const unsigned int in_display_position[2], int maxDistance, unsigned int out_selected_position[2])
{
  ...
  if (maxDist == 0)
  {
    ...
    if (this->GetFieldAssociation() == svtkDataObject::FIELD_ASSOCIATION_POINTS)
    {
...
//----------------------------------------------------------------------------
svtkSelection* svtkHardwareSelector::GenerateSelection(
  unsigned int x1, unsigned int y1, unsigned int x2, unsigned int y2)
{
  svtkInternals::MapOfAttributeIds dataMap;
  svtkInternals::PixelCountType pixelCounts;
...
//----------------------------------------------------------------------------
svtkSelection* svtkHardwareSelector::GeneratePolygonSelection(int* polygonPoints, svtkIdType count)
{
  ...
  int x1 = SVTK_INT_MAX, x2 = SVTK_INT_MIN, y1 = SVTK_INT_MAX, y2 = SVTK_INT_MIN;
...
//----------------------------------------------------------------------------
void svtkHardwareSelector::PrintSelf(ostream& os, svtkIndent indent)
{
