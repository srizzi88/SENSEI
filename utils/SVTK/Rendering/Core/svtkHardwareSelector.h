 ...
 *
 * svtkHardwareSelector is a helper that orchestrates color buffer based
 * selection. This relies on OpenGL.
 * svtkHardwareSelector can be used to select visible cells or points within a
 ...
 * Select will cause the attached svtkRenderer to render in a special color mode,
 ...
 * svtkSelection instance with the cells/points selected.
 *
 * Limitations:
 ...
 * values to the appropriate SVTK values.
 *
...
#ifndef svtkHardwareSelector_h
#define svtkHardwareSelector_h
...
class SVTKRENDERINGCORE_EXPORT svtkHardwareSelector : public svtkObject
{
public:
  //@{
  /**
...
public:
  static svtkHardwareSelector* New();
  svtkTypeMacro(svtkHardwareSelector, svtkObject);
   ...
   */
  svtkSetVector4Macro(Area, unsigned int);
  svtkGetVector4Macro(Area, unsigned int);
  //@}

  //@{
  /**
   * Set the field type to select. Valid values are
   * \li svtkDataObject::FIELD_ASSOCIATION_POINTS
   * \li svtkDataObject::FIELD_ASSOCIATION_CELLS
   * \li svtkDataObject::FIELD_ASSOCIATION_VERTICES
   * \li svtkDataObject::FIELD_ASSOCIATION_EDGES
   * \li svtkDataObject::FIELD_ASSOCIATION_ROWS
   * Currently only FIELD_ASSOCIATION_POINTS and FIELD_ASSOCIATION_CELLS are
   * supported.
   */
  svtkSetMacro(FieldAssociation, int);
   ...
   */
  svtkSetMacro(UseProcessIdFromData, bool);
  svtkGetMacro(UseProcessIdFromData, bool);
  //@}

  /**
   * Perform the selection. Returns a new instance of svtkSelection containing
   * the selection on success.
   */
  svtkSelection* Select();

  //@{
  /**
   * It is possible to use the svtkHardwareSelector for a custom picking. (Look
   * at svtkScenePicker). In that case instead of Select() on can use
  ...
  //@{
  /**
   * Called by any svtkMapper or svtkProp subclass to indicate the
   * maximum cell or point attribute ID it uses. These values are
   * used for determining if the POINT_ID_HIGH or CELL_ID_HIGH
   * passes are required.
   */
  virtual void UpdateMaximumCellId(svtkIdType attribid);
  virtual void UpdateMaximumPointId(svtkIdType attribid);
  //@}
  ...
  /**
   * Called by svtkRenderer to render the selection pass.
   * Returns the number of props rendered.
   */
  int Render(svtkRenderer* renderer, svtkProp** propArray, int propArrayCount);
   ...
   */
  svtkGetMacro(ActorPassOnly, bool);
  svtkSetMacro(ActorPassOnly, bool);
  //@}
   ...
   */
  svtkGetMacro(CaptureZValues, bool);
  svtkSetMacro(CaptureZValues, bool);
  //@}
   ...
   */
  svtkSetMacro(ProcessID, int);
  svtkGetMacro(ProcessID, int);
  //@}
   ...
   */
  svtkGetVector3Macro(PropColorValue, float);
  svtkSetVector3Macro(PropColorValue, float);
  void SetPropColorValue(svtkIdType val);
  //@}
  ...
  /**
   * Generates the svtkSelection from pixel buffers.
   ...
   */
  virtual svtkSelection* GenerateSelection() { return GenerateSelection(this->Area); }
  virtual svtkSelection* GenerateSelection(unsigned int r[4])
  {
    return GenerateSelection(r[0], r[1], r[2], r[3]);
  }
  virtual svtkSelection* GenerateSelection(
   ...
   */
  virtual svtkSelection* GeneratePolygonSelection(int* polygonPoints, svtkIdType count);

  /**
   * returns the prop associated with a ID. This is valid only until
   * ReleasePixBuffers() gets called.
   */
  svtkProp* GetPropFromID(int id);
  ...
  static void Convert(svtkIdType id, float tcoord[3])
  {
...
protected:
  ...
  virtual void BeginRenderProp(svtkRenderWindow*) = 0;
  virtual void EndRenderProp(svtkRenderWindow*) = 0;
  ...
  svtkIdType GetID(int low24, int mid24, int high16)
  {
    svtkIdType val = 0;
    val |= high16;
    val = val << 24;
   ...
   */
  virtual int GetPropID(int idx, svtkProp* svtkNotUsed(prop)) { return idx; }
  ...
  svtkIdType MaximumPointId;
  svtkIdType MaximumCellId;
  //@}
...
private:
  svtkHardwareSelector(const svtkHardwareSelector&) = delete;
  void operator=(const svtkHardwareSelector&) = delete;
