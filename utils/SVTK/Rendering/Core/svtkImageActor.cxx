...
svtkStandardNewMacro(svtkImageActor);

//----------------------------------------------------------------------------
svtkImageActor::svtkImageActor()
{
  ...
  svtkMath::UninitializeBounds(this->DisplayBounds);
  ...
  svtkImageSliceMapper* mapper = svtkImageSliceMapper::New();
  this->Mapper = mapper;
  mapper->BorderOff();
...
//----------------------------------------------------------------------------
void svtkImageActor::SetInputData(svtkImageData* input)
{
...
//----------------------------------------------------------------------------
svtkAlgorithm* svtkImageActor::GetInputAlgorithm()
{
  if (!this->Mapper)
  {
    return nullptr;
...
//----------------------------------------------------------------------------
svtkImageData* svtkImageActor::GetInput()
{
  if (!this->Mapper)
  {
    return nullptr;
...
//----------------------------------------------------------------------------
void svtkImageActor::SetInterpolate(svtkTypeBool i)
{
  if (this->Property)
  {
    if (i)
    {
      if (this->Property->GetInterpolationType() != SVTK_LINEAR_INTERPOLATION)
      {
        this->Property->SetInterpolationTypeToLinear();
        this->Modified();
      }
    ...
    {
      if (this->Property->GetInterpolationType() != SVTK_NEAREST_INTERPOLATION)
      {
...
//----------------------------------------------------------------------------
svtkTypeBool svtkImageActor::GetInterpolate()
{
  if (this->Property && this->Property->GetInterpolationType() != SVTK_NEAREST_INTERPOLATION)
  {
    return 1;
  }
...
//----------------------------------------------------------------------------
void svtkImageActor::SetOpacity(double o)
{
...
//----------------------------------------------------------------------------
double svtkImageActor::GetOpacity()
{
  if (this->Property)
  {
...
//----------------------------------------------------------------------------
int svtkImageActor::GetSliceNumber()
{
  if (!this->Mapper || !this->Mapper->IsA("svtkImageSliceMapper"))
  {
    return 0;
  }

  return static_cast<svtkImageSliceMapper*>(this->Mapper)->GetSliceNumber();
}

//----------------------------------------------------------------------------
int svtkImageActor::GetSliceNumberMax()
{
  if (!this->Mapper || !this->Mapper->IsA("svtkImageSliceMapper"))
  {
    return 0;
  ...
  return static_cast<svtkImageSliceMapper*>(this->Mapper)->GetSliceNumberMaxValue();
}

//----------------------------------------------------------------------------
int svtkImageActor::GetSliceNumberMin()
{
  if (!this->Mapper || !this->Mapper->IsA("svtkImageSliceMapper"))
  {
    return 0;
  ...
  return static_cast<svtkImageSliceMapper*>(this->Mapper)->GetSliceNumberMinValue();
}

//----------------------------------------------------------------------------
void svtkImageActor::SetDisplayExtent(const int extent[6])
{
  int idx, modified = 0;
  ...
  if (modified)
  {
    if (this->Mapper && this->Mapper->IsA("svtkImageSliceMapper"))
    {
      ...
      {
        static_cast<svtkImageSliceMapper*>(this->Mapper)->CroppingOn();
        static_cast<svtkImageSliceMapper*>(this->Mapper)->SetCroppingRegion(this->DisplayExtent);
        static_cast<svtkImageSliceMapper*>(this->Mapper)
          ->SetOrientation(this->GetOrientationFromExtent(this->DisplayExtent));
      }
      else
      {
        static_cast<svtkImageSliceMapper*>(this->Mapper)->CroppingOff();
        static_cast<svtkImageSliceMapper*>(this->Mapper)->SetOrientationToZ();
      }
    }
    this->Modified();
  }
...
//----------------------------------------------------------------------------
void svtkImageActor::SetDisplayExtent(int minX, int maxX, int minY, int maxY, int minZ, int maxZ)
{
...
//----------------------------------------------------------------------------
void svtkImageActor::GetDisplayExtent(int extent[6])
{
...
double* svtkImageActor::GetDisplayBounds()
{
  svtkAlgorithm* inputAlg = nullptr;
  ...
  int extent[6];
  svtkInformation* inputInfo = this->Mapper->GetInputInformation();
  inputInfo->Get(svtkStreamingDemandDrivenPipeline::WHOLE_EXTENT(), extent);
  double spacing[3] = { 1, 1, 1 };
  if (inputInfo->Has(svtkDataObject::SPACING()))
  {
    inputInfo->Get(svtkDataObject::SPACING(), spacing);
  }
  double origin[3] = { 0, 0, 0 };
  if (inputInfo->Has(svtkDataObject::ORIGIN()))
  {
    inputInfo->Get(svtkDataObject::ORIGIN(), origin);
  }
...
void svtkImageActor::GetDisplayBounds(double bounds[6])
{
...
double* svtkImageActor::GetBounds()
{
  int i, n;
...
//----------------------------------------------------------------------------
int svtkImageActor::GetOrientationFromExtent(const int extent[6])
{
  int orientation = 2;
...
//----------------------------------------------------------------------------
void svtkImageActor::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
int svtkImageActor::GetWholeZMin()
{
  ...
  int* extent =
    this->Mapper->GetInputInformation()->Get(svtkStreamingDemandDrivenPipeline::WHOLE_EXTENT());
  return extent[4];
}

//----------------------------------------------------------------------------
int svtkImageActor::GetWholeZMax()
{
  if (!this->GetInputAlgorithm())
  {
    return 0;
...
svtkTypeBool svtkImageActor::HasTranslucentPolygonalGeometry()
{
  if (this->ForceOpaque)
  {
    return 0;
  ...
  // backwards compatibility, note that the newer svtkImageSlice class
  ...
  svtkAlgorithm* inputAlg = this->Mapper->GetInputAlgorithm();
  if (!inputAlg)
  {
    return 0;
  }

  // This MTime check is the same as done in svtkTexture
  ...
  svtkInformation* scalarInfo = svtkDataObject::GetActiveFieldInformation(
    inputInfo, svtkDataObject::FIELD_ASSOCIATION_POINTS, svtkDataSetAttributes::SCALARS);
  ...
  this->TranslucentCachedResult = (scalarType == SVTK_UNSIGNED_CHAR && numComp % 2 == 0);
