 ...
 *
 * svtkImageActor is used to render an image in a 3D scene.  The image
 ...
 * the svtkImageSlice class, which provides more functionality than
 * svtkImageActor.
 *
 * @sa
 * svtkImageData svtkImageSliceMapper svtkImageProperty
 */

#ifndef svtkImageActor_h
#define svtkImageActor_h
...
class SVTKRENDERINGCORE_EXPORT svtkImageActor : public svtkImageSlice
{
public:
  svtkTypeMacro(svtkImageActor, svtkImageSlice);
  void PrintSelf(ostream& os, svtkIndent indent) override;

  /**
   * Instantiate the image actor.
   */
  static svtkImageActor* New();
   ...
   */
  virtual void SetInterpolate(svtkTypeBool);
  virtual svtkTypeBool GetInterpolate();
  ...
  int* GetDisplayExtent() SVTK_SIZEHINT(6) { return this->DisplayExtent; }
  //@}
...
private:
  svtkImageActor(const svtkImageActor&) = delete;
  void operator=(const svtkImageActor&) = delete;
};
