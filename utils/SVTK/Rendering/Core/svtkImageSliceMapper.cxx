/*=========================================================================
  ...
  Module:    svtkImageSliceMapper.cxx
...
svtkCxxSetObjectMacro(svtkImageSliceMapper, Points, svtkPoints);

//----------------------------------------------------------------------------
// Return nullptr if no override is supplied.
svtkAbstractObjectFactoryNewMacro(svtkImageSliceMapper);

//----------------------------------------------------------------------------
svtkImageSliceMapper::svtkImageSliceMapper()
{
  this->SliceNumber = 0;
...
//----------------------------------------------------------------------------
void svtkImageSliceMapper::ReleaseGraphicsResources(svtkWindow*)
{
  // see OpenGL subclass for implementation
}

//----------------------------------------------------------------------------
void svtkImageSliceMapper::Render(svtkRenderer*, svtkImageSlice*)
{
  // see OpenGL subclass for implementation
}

//----------------------------------------------------------------------------
svtkTypeBool svtkImageSliceMapper::ProcessRequest(
  svtkInformation* request, svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  ...
  if (request->Has(svtkStreamingDemandDrivenPipeline::REQUEST_INFORMATION()))
  {
    ...
    {
      inInfo->Get(svtkDataObject::DIRECTION(), dir);
    }
    else
    {
      svtkMatrix3x3::Identity(dir);
    }
    ...
    if (matrix)
    {
      svtkMatrix4x4::Multiply4x4(matrix->GetData(), i2p, i2p);
    }
    ...
    svtkMatrix4x4::MultiplyPoint(i2p, point, point);
    point[0] /= point[3];
    point[1] /= point[3];
    point[2] /= point[3];
    svtkMatrix4x4::Invert(i2p, i2p);
    svtkMatrix4x4::Transpose(i2p, i2p);
    svtkMatrix4x4::MultiplyPoint(i2p, normal, normal);
  ...
  if (request->Has(svtkStreamingDemandDrivenPipeline::REQUEST_UPDATE_EXTENT()))
  {
    ...
    if (this->Streaming)
    {
      ...
      inInfo->Set(svtkStreamingDemandDrivenPipeline::UPDATE_EXTENT(), this->DisplayExtent, 6);
    }
    else
    {
      int ext[6];
...
//----------------------------------------------------------------------------
void svtkImageSliceMapper::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
svtkMTimeType svtkImageSliceMapper::GetMTime()
{
  ...
  if (this->SliceFacesCamera || this->SliceAtFocalPoint)
  {
    ...
    {
      svtkCamera* camera = ren->GetActiveCamera();
      svtkMTimeType mTime2 = prop->GetMTime();
      if (mTime2 > mTime)
      {
        mTime = mTime2;
      }
...
//----------------------------------------------------------------------------
void svtkImageSliceMapper::SetSliceNumber(int i)
{
...
//----------------------------------------------------------------------------
int svtkImageSliceMapper::GetSliceNumber()
{
  return this->SliceNumber;
}

//----------------------------------------------------------------------------
int svtkImageSliceMapper::GetSliceNumberMinValue()
{
...
//----------------------------------------------------------------------------
int svtkImageSliceMapper::GetSliceNumberMaxValue()
{
  this->UpdateInformation();
...
//----------------------------------------------------------------------------
void svtkImageSliceMapper::GetIndexBounds(double extent[6])
{
  if (!this->GetInput())
  {
    return;
...
//----------------------------------------------------------------------------
double* svtkImageSliceMapper::GetBounds()
{
  if (!this->GetInput())
  {
...
//----------------------------------------------------------------------------
int svtkImageSliceMapper::GetOrientationFromCamera(double const* propMatrix, svtkCamera* camera)
{
  double normal[4] = { 0, 0, -1, 0 };
  camera->GetDirectionOfProjection(normal);

  double mat[16];
  svtkMatrix4x4::Invert(propMatrix, mat);
  svtkMatrix4x4::Transpose(mat, mat);
  ...
  for (int c = 0; c < 3; ++c)
  {
    ...
    double dot = svtkMath::Dot(vec, normal);
...
//----------------------------------------------------------------------------
int svtkImageSliceMapper::GetSliceFromCamera(double const* propMatrix, svtkCamera* camera)
{
  ...
  double mat[16];
  svtkMatrix4x4::Invert(propMatrix, mat);
  svtkMatrix4x4::MultiplyPoint(mat, p, p);
  double slicepos = p[orientation] / p[3];

  // round to get integer, add a tolerance to prefer rounding up
  return svtkMath::Floor(slicepos + (0.5 + 7.62939453125e-06));
}

//----------------------------------------------------------------------------
void svtkImageSliceMapper::GetSlicePlaneInDataCoords(
  svtkMatrix4x4* svtkNotUsed(propMatrix), double normal[4])
{
  int orientation = this->Orientation % 3;
  ...
  double scale = svtkMath::Normalize(normal);
...
//----------------------------------------------------------------------------
void svtkImageSliceMapper::GetDimensionIndices(int orientation, int& xdim, int& ydim)
{
