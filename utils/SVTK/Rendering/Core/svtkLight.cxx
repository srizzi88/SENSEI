...
svtkCxxSetObjectMacro(svtkLight, Information, svtkInformation);
svtkCxxSetObjectMacro(svtkLight, TransformMatrix, svtkMatrix4x4);

svtkObjectFactoryNewMacro(svtkLight);
...
svtkLight::svtkLight()
{
  ...
  this->LightType = SVTK_LIGHT_TYPE_SCENE_LIGHT;
...
// ----------------------------------------------------------------------------
svtkLight* svtkLight::ShallowClone()
{
  svtkLight* result = svtkLight::New();
...
// ----------------------------------------------------------------------------
void svtkLight::SetDirectionAngle(double elevation, double azimuth)
{
  elevation = svtkMath::RadiansFromDegrees(elevation);
  azimuth = svtkMath::RadiansFromDegrees(azimuth);
...
// Preserve SVTK's old way of setting light color
// which affected only diffuse and specular components.
void svtkLight::SetColor(double R, double G, double B)
{
  this->SetDiffuseColor(R, G, B);
  this->SetSpecularColor(R, G, B);
}

int svtkLight::LightTypeIsHeadlight()
{
  return this->LightType == SVTK_LIGHT_TYPE_HEADLIGHT;
}

int svtkLight::LightTypeIsCameraLight()
{
  return this->LightType == SVTK_LIGHT_TYPE_CAMERA_LIGHT;
}

int svtkLight::LightTypeIsSceneLight()
{
  return this->LightType == SVTK_LIGHT_TYPE_SCENE_LIGHT;
}

void svtkLight::GetTransformedPosition(double a[3])
{
...
void svtkLight::GetTransformedPosition(double& x, double& y, double& z)
{
  double a[3];
...
void svtkLight::GetTransformedFocalPoint(double& x, double& y, double& z)
{
  double a[3];
...
void svtkLight::TransformVector(double a[3], double b[3])
{
  if (this->TransformMatrix)
  {
    double ha[4];
...
void svtkLight::DeepCopy(svtkLight* light)
{
  ...
  {
    svtkNew<svtkMatrix4x4> matrix4x4;
...
void svtkLight::SetLightType(int type)
{
  svtkDebugMacro(<< this->GetClassName() << " (" << this << "): setting LightType to " << type);
...
void svtkLight::PrintSelf(ostream& os, svtkIndent indent)
{
  ...
  os << indent << "LightType: ";
  if (this->LightType == SVTK_LIGHT_TYPE_HEADLIGHT)
  {
    os << "Headlight\n";
  }
  else if (this->LightType == SVTK_LIGHT_TYPE_CAMERA_LIGHT)
  {
    os << "CameraLight\n";
  }
  else if (this->LightType == SVTK_LIGHT_TYPE_SCENE_LIGHT)
  {
    os << "SceneLight\n";
  }
  else
