...
svtkStandardNewMacro(svtkLightKit);

static const char* svtkLightKitTypeStrings[] = { "KeyLight", "FillLight", "BackLight", "HeadLight",
  nullptr };

static const char* svtkLightKitSubTypeStrings[] = { "Warmth", "Intensity", "Elevation", "Azimuth",
  "K:F Ratio", "K:B Ratio", "K:H Ratio", nullptr };

// These are the same as svtkLightKitSubTypeStrings but shorter
// useful for a GUI with minimum space
static const char* svtkLightKitSubTypeShortStrings[] = { "War.", "Int. ", "Ele.", "Azi.", "K:F",
  "K:B", "K:H", nullptr };

//----------------------------------------------------------------------------
svtkLightKit::svtkLightKit()
{
  // create members
  this->KeyLight = svtkLight::New();
  this->FillLight = svtkLight::New();
  this->HeadLight = svtkLight::New();
  this->BackLight0 = svtkLight::New();
  this->BackLight1 = svtkLight::New();

  for (int i = 0; i < 4; i++)
  {
    this->WarmthFunction[i] = svtkPiecewiseFunction::New();
  }
...
//----------------------------------------------------------------------------
void svtkLightKit::SetKeyLightAngle(double elevation, double azimuth)
{
...
//----------------------------------------------------------------------------
void svtkLightKit::SetFillLightAngle(double elevation, double azimuth)
{
...
//----------------------------------------------------------------------------
void svtkLightKit::SetBackLightAngle(double elevation, double azimuth)
{
...
//----------------------------------------------------------------------------
void svtkLightKit::WarmthToRGB(double w, double rgb[3])
{
...
//----------------------------------------------------------------------------
double svtkLightKit::WarmthToIntensity(double w)
{
  return this->WarmthFunction[3]->GetValue(w);
}

//----------------------------------------------------------------------------
void svtkLightKit::WarmthToRGBI(double w, double rgb[3], double& i)
{
  rgb[0] = this->WarmthFunction[0]->GetValue(w);
  rgb[1] = this->WarmthFunction[1]->GetValue(w);
  rgb[2] = this->WarmthFunction[2]->GetValue(w);
...
//----------------------------------------------------------------------------
void svtkLightKit::AddLightsToRenderer(svtkRenderer* renderer)
{
  if (renderer != nullptr)
  {
...
//----------------------------------------------------------------------------
void svtkLightKit::RemoveLightsFromRenderer(svtkRenderer* renderer)
{
  if (renderer != nullptr)
  {
...
//----------------------------------------------------------------------------
void svtkLightKit::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
void svtkLightKit::DeepCopy(svtkLightKit* k)
{
...
//----------------------------------------------------------------------------
void svtkLightKit::InitializeWarmthFunctions()
{
...
//----------------------------------------------------------------------------
const char* svtkLightKit::GetStringFromType(int type)
{
  static const int n = sizeof(svtkLightKitTypeStrings) / sizeof(char*);
  if (type < n)
  {
    return svtkLightKitTypeStrings[type];
  }
  else
  {
    return nullptr;
...
//----------------------------------------------------------------------------
const char* svtkLightKit::GetStringFromSubType(int subtype)
{
  static const int n = sizeof(svtkLightKitSubTypeStrings) / sizeof(char*);
  if (subtype < n)
  {
    return svtkLightKitSubTypeStrings[subtype];
  }
  else
  {
    return nullptr;
...
//----------------------------------------------------------------------------
const char* svtkLightKit::GetShortStringFromSubType(int subtype)
{
  static const int n = sizeof(svtkLightKitSubTypeShortStrings) / sizeof(char*);
  if (subtype < n)
  {
    return svtkLightKitSubTypeShortStrings[subtype];
  }
  else
  {
    return nullptr;
...
//----------------------------------------------------------------------------
svtkLightKit::LightKitSubType svtkLightKit::GetSubType(svtkLightKit::LightKitType type, int i)
{
  // return subtype
