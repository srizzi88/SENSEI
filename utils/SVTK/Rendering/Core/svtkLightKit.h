 ...
 *
 * svtkLightKit is designed to make general purpose lighting of svtk
 ...
 * svtk light, which is a headlight located at the camera. (HeadLights
 ...
 * side.
 *
 * svtkLightKit limits the colors that can be assigned to any light to
 ...
 * is set, svtkLightKit will attempt to compensate for these perceptual
 * differences by increasing the brightness of more saturated colors.
 *
 * A LightKit is not explicitly part of the svtk pipeline.  Rather, it
 * is a composite object that controls the behavior of lights using a
 * unified user interface.  Every time a parameter of svtkLightKit is
 * adjusted, the properties of its lights are modified.
 *
 * @par Credits:
 * svtkLightKit was originally written and contributed to svtk by
 * Michael Halle (mhalle@bwh.harvard.edu) at the Surgical Planning
 * Lab, Brigham and Women's Hospital.
 */

#ifndef svtkLightKit_h
#define svtkLightKit_h
...
class SVTKRENDERINGCORE_EXPORT svtkLightKit : public svtkObject
{
public:
  static svtkLightKit* New();
  svtkTypeMacro(svtkLightKit, svtkObject);
   ...
   */
  svtkSetMacro(KeyLightIntensity, double);
  svtkGetMacro(KeyLightIntensity, double);
  //@}
   ...
   */
  svtkSetClampMacro(KeyToFillRatio, double, 0.5, SVTK_DOUBLE_MAX);
  svtkGetMacro(KeyToFillRatio, double);
  //@}
   ...
   */
  svtkSetClampMacro(KeyToHeadRatio, double, 0.5, SVTK_DOUBLE_MAX);
  svtkGetMacro(KeyToHeadRatio, double);
  //@}
   ...
   */
  svtkSetClampMacro(KeyToBackRatio, double, 0.5, SVTK_DOUBLE_MAX);
  svtkGetMacro(KeyToBackRatio, double);
  //@}
   ...
   */
  svtkSetMacro(KeyLightWarmth, double);
  svtkGetMacro(KeyLightWarmth, double);
  //@}

  svtkSetMacro(FillLightWarmth, double);
  svtkGetMacro(FillLightWarmth, double);

  svtkSetMacro(HeadLightWarmth, double);
  svtkGetMacro(HeadLightWarmth, double);

  svtkSetMacro(BackLightWarmth, double);
  svtkGetMacro(BackLightWarmth, double);

  //@{
  /**
   * Returns the floating-point RGB values of each of the light's color.
   */
  svtkGetVectorMacro(KeyLightColor, double, 3);
  svtkGetVectorMacro(FillLightColor, double, 3);
  svtkGetVectorMacro(HeadLightColor, double, 3);
  svtkGetVectorMacro(BackLightColor, double, 3);
  //@}
   ...
   */
  svtkBooleanMacro(MaintainLuminance, svtkTypeBool);
  svtkGetMacro(MaintainLuminance, svtkTypeBool);
  svtkSetMacro(MaintainLuminance, svtkTypeBool);
  //@}
  ...
  svtkGetVectorMacro(KeyLightAngle, double, 2);
  ...
  svtkGetVectorMacro(FillLightAngle, double, 2);
  ...
  svtkGetVectorMacro(BackLightAngle, double, 2);
   ...
   */
  void AddLightsToRenderer(svtkRenderer* renderer);
  void RemoveLightsFromRenderer(svtkRenderer* renderer);
  //@}

  void DeepCopy(svtkLightKit* kit);
...
protected:
  ...
  svtkLight* BackLight0;
  svtkLight* BackLight1;
  ...
  svtkTypeBool MaintainLuminance;

  svtkPiecewiseFunction* WarmthFunction[4]; // r, g, b, perceptual length

private:
  svtkLightKit(const svtkLightKit&) = delete;
  void operator=(const svtkLightKit&) = delete;
};
