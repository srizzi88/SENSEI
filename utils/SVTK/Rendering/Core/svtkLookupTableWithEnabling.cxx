/*=========================================================================
  ...
  Module:    svtkLookupTableWithEnabling.cxx
...
svtkStandardNewMacro(svtkLookupTableWithEnabling);

svtkCxxSetObjectMacro(svtkLookupTableWithEnabling, EnabledArray, svtkDataArray);

// Construct with range=(0,1); and hsv ranges set up for rainbow color table
// (from red to blue).
svtkLookupTableWithEnabling::svtkLookupTableWithEnabling(int sze, int ext)
...
//----------------------------------------------------------------------------
svtkLookupTableWithEnabling::~svtkLookupTableWithEnabling()
{
...
void svtkLookupTableWithEnabling::DisableColor(unsigned char r, unsigned char g, unsigned char b,
...
static void svtkLookupTableWithEnablingLogRange(double range[2], double logRange[2])
{
  double rmin = range[0];
  double rmax = range[1];
...
inline double svtkApplyLogScale(double v, double range[2], double logRange[2])
{
...
inline unsigned char* svtkLinearLookup(
...
// of 8-bit chunks
template <class T>
void svtkLookupTableWithEnablingMapData(svtkLookupTableWithEnabling* self, T* input,
  unsigned char* output, int length, int inIncr, int outFormat)
{
  ...
  {
    if (self->GetScale() == SVTK_SCALE_LOG10)
    {
      double val;
      double logRange[2];
      svtkLookupTableWithEnablingLogRange(range, logRange);
      shift = -logRange[0];
      if (logRange[1] <= logRange[0])
      {
        scale = SVTK_DOUBLE_MAX;
      }
      else
      {
      ...
      if (outFormat == SVTK_RGBA)
      {
        while (--i >= 0)
        {
          val = svtkApplyLogScale(*input, range, logRange);
          cptr = svtkLinearLookup(val, table, maxIndex, shift, scale);
    ...
    else // not log scale
    {
      ...
      if (outFormat == SVTK_RGBA)
      {
        while (--i >= 0)
        {
          cptr = svtkLinearLookup(*input, table, maxIndex, shift, scale);
          if (hasEnabledArray && !self->GetEnabledArray()->GetTuple1(length - i - 1))
          {
            self->DisableColor(cptr[0], cptr[1], cptr[2], &r, &g, &b);
            *output++ = r;
...
//----------------------------------------------------------------------------
void svtkLookupTableWithEnabling::MapScalarsThroughTable2(void* input, unsigned char* output,
  int inputDataType, int numberOfValues, int inputIncrement, int outputFormat)
{
  switch (inputDataType)
  {
    ...
    {
      svtkIdType i, id;
      svtkBitArray* bitArray = svtkBitArray::New();
      bitArray->SetVoidArray(input, numberOfValues, 1);
      svtkUnsignedCharArray* newInput = svtkUnsignedCharArray::New();
      ...
      svtkLookupTableWithEnablingMapData(this, static_cast<unsigned char*>(newInput->GetPointer(0)),
    ...
    break;
      ...
      svtkTemplateMacro(svtkLookupTableWithEnablingMapData(
...
//----------------------------------------------------------------------------
void svtkLookupTableWithEnabling::PrintSelf(ostream& os, svtkIndent indent)
{
