 ...
 *
 * svtkMapper is an abstract class to specify interface between data and
 * graphics primitives. Subclasses of svtkMapper map data through a
 ...
 *
 * @sa
 * svtkDataSetMapper svtkPolyDataMapper
 */
...
#include "svtkSystemIncludes.h"      // For SVTK_COLOR_MODE_DEFAULT and _MAP_SCALARS
#include <vector>                   // for method args

#define SVTK_RESOLVE_OFF 0
#define SVTK_RESOLVE_POLYGON_OFFSET 1
#define SVTK_RESOLVE_SHIFT_ZBUFFER 2

#define SVTK_GET_ARRAY_BY_ID 0
#define SVTK_GET_ARRAY_BY_NAME 1

#define SVTK_MATERIALMODE_DEFAULT 0
#define SVTK_MATERIALMODE_AMBIENT 1
#define SVTK_MATERIALMODE_DIFFUSE 2
#define SVTK_MATERIALMODE_AMBIENT_AND_DIFFUSE 3
...
class SVTKRENDERINGCORE_EXPORT svtkMapper : public svtkAbstractMapper3D
{
public:
  svtkTypeMacro(svtkMapper, svtkAbstractMapper3D);
   ...
   */
  virtual void Render(svtkRenderer* ren, svtkActor* a) = 0;
   ...
   */
  svtkSetMacro(Static, svtkTypeBool);
  svtkGetMacro(Static, svtkTypeBool);
  svtkBooleanMacro(Static, svtkTypeBool);
  //@}
   ...
   */
  svtkSetMacro(InterpolateScalarsBeforeMapping, svtkTypeBool);
  svtkGetMacro(InterpolateScalarsBeforeMapping, svtkTypeBool);
  svtkBooleanMacro(InterpolateScalarsBeforeMapping, svtkTypeBool);
  //@}
  ...
  void SetScalarModeToUseFieldData() { this->SetScalarMode(SVTK_SCALAR_MODE_USE_FIELD_DATA); }
  ...
  svtkSetMacro(FieldDataTupleId, svtkIdType);
  svtkGetMacro(FieldDataTupleId, svtkIdType);
  ...
  svtkGetMacro(ArrayAccessMode, int);
  ...
  static void SetResolveCoincidentTopologyToOff() { SetResolveCoincidentTopology(SVTK_RESOLVE_OFF); }
  ...
  {
    SetResolveCoincidentTopology(SVTK_RESOLVE_SHIFT_ZBUFFER);
  }
  //@}
  ...
  /**
   * This instance variable is used by svtkLODActor to determine which
  ...
  /**
   * Get the input as a svtkDataSet.  This method is overridden in
   * the specialized mapper classes to return more specific data types.
   */
  svtkDataSet* GetInput();

  /**
   * Get the input to this mapper as a svtkDataSet, instead of as a
   ...
   */
  svtkDataSet* GetInputAsDataSet() { return this->GetInput(); }
   ...
   */
  virtual svtkUnsignedCharArray* MapScalars(double alpha);
  virtual svtkUnsignedCharArray* MapScalars(double alpha, int& cellFlag);
  virtual svtkUnsignedCharArray* MapScalars(svtkDataSet* input, double alpha);
  virtual svtkUnsignedCharArray* MapScalars(svtkDataSet* input, double alpha, int& cellFlag);
  //@}
   ...
   */
  virtual void ProcessSelectorPixelBuffers(svtkHardwareSelector* /* sel */,
    std::vector<unsigned int>& /* pixeloffsets */, svtkProp* /* prop */)
  {
  }
   ...
   */
  virtual int CanUseTextureMapForColoring(svtkDataObject* input);
   ...
   */
  svtkUnsignedCharArray* GetColorMapColors();

  /**
   * Provide read access to the color texture coordinate array
   */
  svtkFloatArray* GetColorCoordinates();

  /**
   * Provide read access to the color texture array
   */
  svtkImageData* GetColorTextureMap();
...
protected:
  ...
  svtkTypeBool InterpolateScalarsBeforeMapping;
  // Coordinate for each point.
  svtkFloatArray* ColorCoordinates;
  // 1D ColorMap used for the texture image.
  svtkImageData* ColorTextureMap;
  void MapScalarsToTexture(svtkAbstractArray* scalars, double alpha);
...
private:
  svtkMapper(const svtkMapper&) = delete;
  void operator=(const svtkMapper&) = delete;
};
