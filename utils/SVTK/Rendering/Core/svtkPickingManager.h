 ...
 * when the manager is picked (e.g. svtkPickingManager::Pick()),
 ...
 * selected.
 * @warning
 * Every time a svtkWidget and/or a svtkWidgetRepresentation is instantiated,
 ...
 * \sa Enabled using svtkPickingManager::EnabledOn(), EnabledOff(),
 * SetEnabled(bool).
 * @par Important:
...
#ifndef svtkPickingManager_h
#define svtkPickingManager_h
...
class SVTKRENDERINGCORE_EXPORT svtkPickingManager : public svtkObject
{
public:
  static svtkPickingManager* New();
  svtkTypeMacro(svtkPickingManager, svtkObject);
  ...
  svtkGetMacro(OptimizeOnInteractorEvents, bool);
  //@}
  ...
  svtkGetMacro(Interactor, svtkRenderWindowInteractor*);
  //@}
   ...
   */
  void AddPicker(svtkAbstractPicker* picker, svtkObject* object = nullptr);

  /**
   * Unregister the \a picker from the picking manager.
   * If \a object is non null, only the pair (\a picker, \a object) is removed.
   */
  void RemovePicker(svtkAbstractPicker* picker, svtkObject* object = nullptr);
   ...
   */
  void RemoveObject(svtkObject* object);
   ...
   * information.
   */
  bool Pick(svtkAbstractPicker* picker, svtkObject* object);
   ...
   * This is an overloaded function.
   */
  bool Pick(svtkAbstractPicker* picker);
  ...
  svtkAssemblyPath* GetAssemblyPath(double X, double Y, double Z, svtkAbstractPropPicker* picker,
    svtkRenderer* renderer, svtkObject* obj);
   ...
   */
  int GetNumberOfObjectsLinked(svtkAbstractPicker* picker);

protected:
  svtkPickingManager();
  ~svtkPickingManager() override;
...
private:
  svtkPickingManager(const svtkPickingManager&) = delete;
  void operator=(const svtkPickingManager&) = delete;
