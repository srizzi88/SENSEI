...
//----------------------------------------------------------------------------
svtkMTimeType svtkProp3D::GetMTime()
{
...
//----------------------------------------------------------------------------
svtkMTimeType svtkProp3D::GetUserTransformMatrixMTime()
{
...
void svtkProp3D::AddPosition(double deltaX, double deltaY, double deltaZ)
{
  double position[3];
...
//----------------------------------------------------------------------------
void svtkProp3D::AddPosition(double deltaPosition[3])
{
...
// finally RotateY.
void svtkProp3D::SetOrientation(double x, double y, double z)
{
  ...
  svtkDebugMacro(<< " Orientation set to ( " << this->Orientation[0] << ", " << this->Orientation[1]
                << ", " << this->Orientation[2] << ")\n");
...
//----------------------------------------------------------------------------
void svtkProp3D::SetOrientation(double a[3])
{
...
double* svtkProp3D::GetOrientation()
{
  // return the orientation of the transformation matrix
  this->Transform->GetOrientation(this->Orientation);

  svtkDebugMacro(<< " Returning Orientation of ( " << this->Orientation[0] << ", "
...
//----------------------------------------------------------------------------
void svtkProp3D::GetOrientation(double o[3])
{
  // return the orientation of the transformation matrix
  this->Transform->GetOrientation(o);
  svtkDebugMacro(<< " Returning Orientation of ( " << o[0] << ", " << o[1] << ", " << o[2] << ")\n");
}

//----------------------------------------------------------------------------
// Returns the WXYZ orientation of the Prop3D.
double* svtkProp3D::GetOrientationWXYZ()
{
...
void svtkProp3D::AddOrientation(double a1, double a2, double a3)
{
  double orient[3];
...
//----------------------------------------------------------------------------
void svtkProp3D::AddOrientation(double a[3])
{
...
void svtkProp3D::RotateX(double angle)
{
  this->IsIdentity = 0;
  this->Transform->PreMultiply();
...
// transformation matrix.
void svtkProp3D::RotateY(double angle)
{
  this->IsIdentity = 0;
  this->Transform->PreMultiply();
...
void svtkProp3D::RotateZ(double angle)
{
  this->IsIdentity = 0;
  this->Transform->PreMultiply();
...
void svtkProp3D::RotateWXYZ(double degree, double x, double y, double z)
{
  this->IsIdentity = 0;
...
//----------------------------------------------------------------------------
void svtkProp3D::SetUserTransform(svtkLinearTransform* transform)
{
  this->IsIdentity = 0;
...
//----------------------------------------------------------------------------
void svtkProp3D::SetUserMatrix(svtkMatrix4x4* matrix)
{
  this->IsIdentity = 0;
...
//----------------------------------------------------------------------------
void svtkProp3D::GetMatrix(svtkMatrix4x4* result)
{
  double mine[16];
  this->GetMatrix(mine);
  int idx = 0;
...
//----------------------------------------------------------------------------
void svtkProp3D::GetMatrix(double result[16])
{
  this->ComputeMatrix();
  svtkMatrix4x4::DeepCopy(result, this->Matrix);
}

//----------------------------------------------------------------------------
void svtkProp3D::ComputeMatrix()
{
  if (this->IsIdentity)
  {
    return;
...
void svtkProp3D::GetBounds(double bounds[6])
{
  this->GetBounds();
  for (int i = 0; i < 6; i++)
  {
...
//----------------------------------------------------------------------------
// Shallow copy of svtkProp3D.
void svtkProp3D::ShallowCopy(svtkProp* prop)
{
  int i;
  svtkProp3D* p = svtkProp3D::SafeDownCast(prop);
...
//----------------------------------------------------------------------------
// Backdoor allows temporary replacement of matrix in svtkProp3D
void svtkProp3D::PokeMatrix(svtkMatrix4x4* matrix)
{
  ...
  if (matrix != nullptr) // set a new transformation
  {
    ...
    {
      this->CachedProp3D = svtkActor::New();
    }
...
//----------------------------------------------------------------------------
void svtkProp3D::InitPathTraversal()
{
  if (this->Paths)
  {
    this->Paths->Delete();
...
//----------------------------------------------------------------------------
svtkMatrix4x4* svtkProp3D::GetUserMatrix()
{
  if (this->UserTransform)
  {
...
//----------------------------------------------------------------------------
void svtkProp3D::PrintSelf(ostream& os, svtkIndent indent)
{
