 ...
 *
 * svtkProp3D is an abstract class used to represent an entity in a rendering
 * scene (i.e., svtkProp3D is a svtkProp with an associated transformation
 ...
 * Rot(y) Rot(x) Rot (z) Trans(origin) Trans(position). Both svtkActor and
 * svtkVolume are specializations of class svtkProp.  The constructor defaults
 * to: origin(0,0,0) position=(0,0,0) orientation=(0,0,0), no user defined
 * matrix or transform, and no texture map.
 *
 * @sa
 * svtkProp svtkActor svtkAssembly svtkVolume
 */
...
class SVTKRENDERINGCORE_EXPORT svtkProp3D : public svtkProp
{
public:
  svtkTypeMacro(svtkProp3D, svtkProp);
  void PrintSelf(ostream& os, svtkIndent indent) override;

  /**
   * Shallow copy of this svtkProp3D.
   */
  ...
  {
    svtkDebugMacro(<< this->GetClassName() << " (" << this << "): setting Position to (" << x << ","
                  << y << "," << z << ")");
  ...
  {
    svtkDebugMacro(<< this->GetClassName() << " (" << this << "): setting Origin to (" << x << ","
                  << y << "," << z << ")");
  ...
  {
    svtkDebugMacro(<< this->GetClassName() << " (" << this << "): setting Scale to (" << x << ","
                  << y << "," << z << ")");
   ...
   * is present.
   */
  void SetUserTransform(svtkLinearTransform* transform);
  svtkGetObjectMacro(UserTransform, svtkLinearTransform);
  //@}
   ...
   */
  void SetUserMatrix(svtkMatrix4x4* matrix);
  svtkMatrix4x4* GetUserMatrix();
  //@}
   ...
   */
  virtual void GetMatrix(svtkMatrix4x4* m);
  ...
  double* GetBounds() SVTK_SIZEHINT(6) override = 0;
  //@}
   ...
   */
  double* GetXRange() SVTK_SIZEHINT(2);

  /**
   * Get the Prop3D's y range in world coordinates.
   */
  double* GetYRange() SVTK_SIZEHINT(2);

  /**
   * Get the Prop3D's z range in world coordinates.
   */
  double* GetZRange() SVTK_SIZEHINT(2);
  ...
  /**
   * This method modifies the svtkProp3D so that its transformation
   ...
   * matrix into the UserMatrix of this svtkProp3D. If the method is
   * called again with a NULL matrix, then the original state of the
   * svtkProp3D will be restored. This method is used to support
   * picking and assembly structures.
   */
  void PokeMatrix(svtkMatrix4x4* matrix) override;
   ...
   */
  svtkMTimeType GetUserTransformMatrixMTime();
   ...
   */
  svtkMatrix4x4* GetMatrix() override
  {
    this->ComputeMatrix();
    return this->Matrix;
  }
...
protected:
  ...
  svtkLinearTransform* UserTransform;
  svtkMatrix4x4* UserMatrix;
  svtkMatrix4x4* Matrix;
  svtkTimeStamp MatrixMTime;
  double Origin[3];
  double Position[3];
  ...
  double Bounds[6];
  svtkProp3D* CachedProp3D; // support the PokeMatrix() method
  int IsIdentity;

private:
  svtkProp3D(const svtkProp3D&) = delete;
  void operator=(const svtkProp3D&) = delete;
};
