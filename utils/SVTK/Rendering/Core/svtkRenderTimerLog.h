...
#ifndef svtkRenderTimerLog_h
#define svtkRenderTimerLog_h
 ...
 */
#define SVTK_SCOPED_RENDER_EVENT(eventName, timer) SVTK_SCOPED_RENDER_EVENT2(eventName, timer, _event)
 ...
 */
#define SVTK_SCOPED_RENDER_EVENT2(eventName, timer, identifier)                                     \
  svtkRenderTimerLog::ScopedEventLogger identifier;                                                 \
  do                                                                                               \
  {                                                                                                \
...
class SVTKRENDERINGCORE_EXPORT svtkRenderTimerLog : public svtkObject
{
public:
  struct Frame;

  /** Container for a single timed event. */
  struct SVTKRENDERINGCORE_EXPORT Event
  {
    /** Event name. */
    std::string Name;
    ...
    svtkTypeUInt64 ElapsedTimeNanoseconds() const { return this->EndTime - this->StartTime; }
     ...
     */
    void Print(std::ostream& os, float threshMs = 0.f, svtkIndent indent = svtkIndent())
    {
      this->Print(os, 0.f, threshMs, indent);
    }

    friend struct svtkRenderTimerLog::Frame;

  protected:
    void Print(std::ostream& os, float parentTime, float threshMs, svtkIndent indent);
  };

  /** Container for a frame's events. */
  struct SVTKRENDERINGCORE_EXPORT Frame
  {
    std::vector<Event> Events;
   ...
   * svtkRenderTimerLog::StartScopedEvent(name) is called, and end when the
   * returned object is destroyed, or ScopedEventLogger::Stop() is called.
   */
  struct SVTKRENDERINGCORE_EXPORT ScopedEventLogger
  {
    ScopedEventLogger()
      : Log(nullptr)
    {
    ...
    void Stop();
    friend class svtkRenderTimerLog;

  protected:
    ScopedEventLogger(svtkRenderTimerLog* log)
      : Log(log)
    {
    }
  ...
  private:
    ...
    svtkRenderTimerLog* Log;
  };

  static svtkRenderTimerLog* New();
  svtkTypeMacro(svtkRenderTimerLog, svtkObject);
  ...
  svtkSetMacro(LoggingEnabled, bool);
  svtkGetMacro(LoggingEnabled, bool);
  svtkBooleanMacro(LoggingEnabled, bool);
  /**@}*/
   ...
   */
  svtkSetMacro(FrameLimit, unsigned int);
  svtkGetMacro(FrameLimit, unsigned int);
  /**@}*/
...
protected:
  svtkRenderTimerLog();
  ~svtkRenderTimerLog() override;

  bool LoggingEnabled;
  unsigned int FrameLimit;

private:
  svtkRenderTimerLog(const svtkRenderTimerLog&) = delete;
  void operator=(const svtkRenderTimerLog&) = delete;
};

#endif // svtkRenderTimerLog_h
