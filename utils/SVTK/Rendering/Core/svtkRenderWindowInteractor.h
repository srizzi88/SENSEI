/*=========================================================================
  ...
  Module:    svtkRenderWindowInteractor.h
 ...
 *
 *
 * svtkRenderWindowInteractor provides a platform-independent interaction
 ...
 * messages to svtkInteractorObserver and its subclasses. svtkRenderWindowInteractor
 * also provides controls for picking, rendering frame rate, and headlights.
 *
 * svtkRenderWindowInteractor has changed from previous implementations and
 * now serves only as a shell to hold user preferences and route messages to
 * svtkInteractorStyle. Callbacks are available for many events.  Platform
 ...
 *
 * @warning
 * svtkRenderWindowInteractor routes events through SVTK's command/observer
 * design pattern. That is, when svtkRenderWindowInteractor (actually, one of
 * its subclasses) sees a platform-dependent event, it translates this into
 * a SVTK event using the InvokeEvent() method. Then any svtkInteractorObservers
...
#ifndef svtkRenderWindowInteractor_h
#define svtkRenderWindowInteractor_h

#include "svtkCommand.h" // for method sig
#include "svtkObject.h"
#include "svtkRenderingCoreModule.h" // For export macro

class svtkTimerIdMap;

// Timer flags for win32/X compatibility
#define SVTKI_TIMER_FIRST 0
#define SVTKI_TIMER_UPDATE 1

// maximum pointers active at once
// for example in multitouch
#define SVTKI_MAX_POINTERS 5

class svtkAbstractPicker;
...
class SVTKRENDERINGCORE_EXPORT svtkRenderWindowInteractor : public svtkObject
{

  friend class svtkInteractorEventRecorder;

public:
  static svtkRenderWindowInteractor* New();
  svtkTypeMacro(svtkRenderWindowInteractor, svtkObject);
   ...
   */
  svtkGetMacro(Done, bool);
  svtkSetMacro(Done, bool);
  ...
  svtkGetMacro(Enabled, int);

  //@{
  /**
   * Enable/Disable whether svtkRenderWindowInteractor::Render() calls
   * this->RenderWindow->Render().
   */
  svtkBooleanMacro(EnableRender, bool);
  svtkSetMacro(EnableRender, bool);
  svtkGetMacro(EnableRender, bool);
  //@}
   ...
   */
  void SetRenderWindow(svtkRenderWindow* aren);
   ...
   */
  void SetHardwareWindow(svtkHardwareWindow* aren);
  svtkGetObjectMacro(HardwareWindow, svtkHardwareWindow);
  //@}
   ...
   * (timerType==SVTKI_TIMER_FIRST) or whether it is being reset
   * (timerType==SVTKI_TIMER_UPDATE). (In initial implementations of SVTK this
  ...
  /**
   * Get the SVTK timer ID that corresponds to the supplied platform ID.
   */
  virtual int GetSVTKTimerId(int platformTimerId);
   ...
   * See svtkGenericRenderWindowInteractor's InternalCreateTimer and
   * InternalDestroyTimer for an example.
   */
  svtkSetMacro(TimerEventId, int);
  svtkGetMacro(TimerEventId, int);
  svtkSetMacro(TimerEventType, int);
  svtkGetMacro(TimerEventType, int);
  svtkSetMacro(TimerEventDuration, int);
  svtkGetMacro(TimerEventDuration, int);
  svtkSetMacro(TimerEventPlatformId, int);
  svtkGetMacro(TimerEventPlatformId, int);
  //@}
   ...
   * is a svtkInteractorStyleSwitch object.
   */
  virtual void SetInteractorStyle(svtkInteractorObserver*);
  svtkGetObjectMacro(InteractorStyle, svtkInteractorObserver);
  //@}
  ...
  //@{
  /**
   * Set/Get the desired update rate. This is used by svtkLODActor's to tell
   ...
   * The default is 15.
   */
  svtkSetClampMacro(DesiredUpdateRate, double, 0.0001, SVTK_FLOAT_MAX);
   ...
   */
  svtkSetClampMacro(StillUpdateRate, double, 0.0001, SVTK_FLOAT_MAX);
  svtkGetMacro(StillUpdateRate, double);
  //@}
   ...
   * Default is 0.
   */
  svtkGetMacro(Initialized, int);
  //@}

  //@{
  /**
   * Set/Get the object used to perform pick operations. In order to
   * pick instances of svtkProp, the picker must be a subclass of
   * svtkAbstractPropPicker, meaning that it can identify a particular
   * instance of svtkProp.
   */
  virtual void SetPicker(svtkAbstractPicker*);
  svtkGetObjectMacro(Picker, svtkAbstractPicker);
  //@}

  /**
   * Create default picker. Used to create one when none is specified.
   * Default is an instance of svtkPropPicker.
   */
  virtual svtkAbstractPropPicker* CreateDefaultPicker();
   ...
   */
  virtual void SetPickingManager(svtkPickingManager*);
  svtkGetObjectMacro(PickingManager, svtkPickingManager);
  //@}
   ...
   */
  void FlyTo(svtkRenderer* ren, double x, double y, double z);
  void FlyTo(svtkRenderer* ren, double* x) { this->FlyTo(ren, x[0], x[1], x[2]); }
  void FlyToImage(svtkRenderer* ren, double x, double y);
  void FlyToImage(svtkRenderer* ren, double* x) { this->FlyToImage(ren, x[0], x[1]); }
  //@}
   ...
   */
  svtkSetClampMacro(NumberOfFlyFrames, int, 1, SVTK_INT_MAX);
  svtkGetMacro(NumberOfFlyFrames, int);
  //@}
   ...
   */
  svtkSetMacro(Dolly, double);
  svtkGetMacro(Dolly, double);
  //@}
   ...
   */
  svtkGetVector2Macro(EventPosition, int);
  svtkGetVector2Macro(LastEventPosition, int);
  svtkSetVector2Macro(LastEventPosition, int);
  virtual void SetEventPosition(int x, int y)
  {
    svtkDebugMacro(<< this->GetClassName() << " (" << this << "): setting EventPosition to (" << x
                  << "," << y << ")");
   ...
   */
  svtkSetMacro(AltKey, int);
  svtkGetMacro(AltKey, int);
  //@}
   ...
   */
  svtkSetMacro(ControlKey, int);
  svtkGetMacro(ControlKey, int);
  //@}
   ...
   */
  svtkSetMacro(PointerIndex, int);
  svtkGetMacro(PointerIndex, int);
  //@}
  ...
  svtkGetMacro(LastRotation, double);
  //@}
  ...
  svtkGetVector2Macro(Translation, double);
  svtkGetVector2Macro(LastTranslation, double);
  //@}
  ...
  svtkSetVector2Macro(EventSize, int);
  svtkGetVector2Macro(EventSize, int);
  //@}
   ...
   * renderers.
   */
  virtual svtkRenderer* FindPokedRenderer(int, int);

  /**
   * Return the object used to mediate between svtkInteractorObservers
   ...
   */
  svtkObserverMediator* GetObserverMediator();

  //@{
  /**
   * Use a 3DConnexion device. Initial value is false.
   * If SVTK is not build with the TDx option, this is no-op.
   * If SVTK is build with the TDx option, and a device is not connected,
   * a warning is emitted.
   * It is must be called before the first Render to be effective, otherwise
   * it is ignored.
   */
  svtkSetMacro(UseTDx, bool);
  svtkGetMacro(UseTDx, bool);
  //@}
   ...
   * will be converted into gestures by SVTK. If turned off the
   * raw multitouch events will be passed down.
   */
  svtkSetMacro(RecognizeGestures, bool);
  svtkGetMacro(RecognizeGestures, bool);
  //@}
   ...
   */
  svtkGetMacro(PointersDownCount, int);
  //@}
...
protected:
  ...
  ~svtkRenderWindowInteractor() override;

  svtkRenderWindow* RenderWindow;
  svtkHardwareWindow* HardwareWindow;
  svtkInteractorObserver* InteractorStyle;

  // Used as a helper object to pick instances of svtkProp
   ...
   * Default is an instance of svtkPickingManager.
   */
  virtual svtkPickingManager* CreateDefaultPickingManager();
  ...
  int EventPositions[SVTKI_MAX_POINTERS][2];
  int LastEventPositions[SVTKI_MAX_POINTERS][2];
  int PointerIndex;

  size_t PointerIndexLookup[SVTKI_MAX_POINTERS];
   ...
   * the supplied svtkCommands have registered are invoked. (This method is
   * typically used by widgets, i.e., subclasses of svtkInteractorObserver, to
   * grab events once an event sequence begins.) Note that the friend
   * declaration is done here to avoid doing so in the superclass svtkObject.
   */
  friend class svtkInteractorObserver;
  ...
  friend struct svtkTimerStruct;
  svtkTimerIdMap* TimerMap;     // An internal, PIMPLd map of timers and associated attributes
  ...
  // when recognizing gestures SVTK will take multitouch events
  // if it receives them and convert them to gestures
  bool RecognizeGestures;
  int PointersDownCount;
  int PointersDown[SVTKI_MAX_POINTERS];
  virtual void RecognizeGesture(svtkCommand::EventIds);
  int StartingEventPositions[SVTKI_MAX_POINTERS][2];
  svtkCommand::EventIds CurrentGesture;

private:
  svtkRenderWindowInteractor(const svtkRenderWindowInteractor&) = delete;
  void operator=(const svtkRenderWindowInteractor&) = delete;
};
