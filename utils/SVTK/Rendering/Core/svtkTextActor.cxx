...
svtkObjectFactoryNewMacro(svtkTextActor);

// ----------------------------------------------------------------------------
svtkTextActor::svtkTextActor()
{
  // To remain compatible with code using svtkActor2D, we must set
  ...
  this->Rectangle = svtkPolyData::New();
  this->RectanglePoints = svtkPoints::New();
  ...
  this->ScaledTextProperty = svtkTextProperty::New();
...
// ----------------------------------------------------------------------------
void svtkTextActor::GetBoundingBox(svtkViewport* vport, double bbox[4])
{
  ...
  {
    svtkErrorMacro("UpdateRectangle failed to do so");
  }
}

//----------------------------------------------------------------------------
void svtkTextActor::GetSize(svtkViewport* vport, double size[2])
{
  double bds[4];
...
//----------------------------------------------------------------------------
int svtkTextActor::SetConstrainedFontSize(svtkViewport* viewport, int targetWidth, int targetHeight)
{
  return this->SetConstrainedFontSize(this, viewport, targetWidth, targetHeight);
}

//----------------------------------------------------------------------------
int svtkTextActor::SetConstrainedFontSize(
  svtkTextActor* tactor, svtkViewport* viewport, int targetWidth, int targetHeight)
{
  ...
  svtkTextProperty* tprop = tactor->GetTextProperty();
  if (!tprop)
  {
...
//----------------------------------------------------------------------------
int svtkTextActor::SetMultipleConstrainedFontSize(svtkViewport* viewport, int targetWidth,
  int targetHeight, svtkTextActor** actors, int nbOfActors, int* maxResultingSize)
{
...
// ----------------------------------------------------------------------------
void svtkTextActor::SetNonLinearFontScale(double exp, int tgt)
{
...
// ----------------------------------------------------------------------------
bool svtkTextActor::RenderImage(svtkTextProperty* tprop, svtkViewport* vp)
{
  svtkStdString text;
  ...
  svtkWindow* win = vp->GetSVTKWindow();
  if (!win)
  {
...
// ----------------------------------------------------------------------------
bool svtkTextActor::GetImageBoundingBox(svtkTextProperty* tprop, svtkViewport* vp, int bbox[4])
{
  svtkStdString text;
  if (this->Input && this->Input[0])
  {
...
// ----------------------------------------------------------------------------
void svtkTextActor::SetInput(const char* str)
{
  if (!str)
  {
    str = "";
...
// ----------------------------------------------------------------------------
char* svtkTextActor::GetInput()
{
  return this->Input;
}

// ----------------------------------------------------------------------------
void svtkTextActor::SetTextProperty(svtkTextProperty* p)
{
...
// ----------------------------------------------------------------------------
void svtkTextActor::ShallowCopy(svtkProp* prop)
{
  svtkTextActor* a = svtkTextActor::SafeDownCast(prop);
  if (a != nullptr)
  {
...
void svtkTextActor::ReleaseGraphicsResources(svtkWindow* win)
{
  this->Superclass::ReleaseGraphicsResources(win);
}

// ----------------------------------------------------------------------------
int svtkTextActor::RenderOverlay(svtkViewport* viewport)
{
...
// ----------------------------------------------------------------------------
int svtkTextActor::RenderOpaqueGeometry(svtkViewport* viewport)
{
  if (!this->Visibility)
  {
    return 0;
...
svtkTypeBool svtkTextActor::HasTranslucentPolygonalGeometry()
{
  return 0;
}

// ----------------------------------------------------------------------------
void svtkTextActor::SetOrientation(float orientation)
{
...
// ----------------------------------------------------------------------------
int svtkTextActor::GetAlignmentPoint()
{
  int alignmentCode = 0;
  ...
  switch (this->TextProperty->GetJustification())
  {
    ...
    case SVTK_TEXT_RIGHT:
      ...
      break;
    default:
      svtkErrorMacro(<< "Unknown justifaction code.");
  }
...
// ----------------------------------------------------------------------------
void svtkTextActor::SetAlignmentPoint(int val)
{
  svtkWarningMacro(<< "Alignment point is being deprecated.  You should use "
...
//-----------------------------------------------------------------------------
float svtkTextActor::GetFontScale(svtkViewport* viewport)
{
...
//-----------------------------------------------------------------------------
void svtkTextActor::ComputeScaledFont(svtkViewport* viewport)
{
  ...
  if (this->TextScaleMode == TEXT_SCALE_MODE_VIEWPORT)
  {
      ...
      (this->TextProperty && (this->TextProperty->GetMTime() > this->BuildTime)))
    {
      ...
      double scale = static_cast<double>(svtkTextActor::GetFontScale(viewport));
  ...
  if (this->TextScaleMode == TEXT_SCALE_MODE_PROP)
  {
      ...
      (this->TextProperty && (this->TextProperty->GetMTime() > this->BuildTime)))
    {
        ...
        orientationHasChanged)
      {
        ...
        if (fsize == -1)
        {
          svtkWarningMacro(<< "Could not determine constrained font size for "
                             "string:\n\t'"
                          << this->Input
                          << "'\n. Resetting to "
                             "20pt.");
  ...
  svtkWarningMacro(<< "Unknown text scaling mode: " << this->TextScaleMode);
}

// ----------------------------------------------------------------------------
void svtkTextActor::ComputeRectangle(svtkViewport* viewport)
{
  int dims[2] = { 0, 0 };
  ...
  if (this->ImageData)
  {
    ...
    {
      svtkErrorMacro("Cannot compute bounding box.");
      return;
    }
    ...
    // compute TCoords.
    svtkFloatArray* tc =
      svtkArrayDownCast<svtkFloatArray>(this->Rectangle->GetPointData()->GetTCoords());
    float tcXMax, tcYMax;
  ...
  if ((this->TextScaleMode == TEXT_SCALE_MODE_PROP) || this->UseBorderAlign)
  {
    ...
    switch (this->GetAlignmentPoint())
    {
      ...
      case 8:
        ...
        break;
      default:
        svtkErrorMacro(<< "Bad alignment point value.");
    }
  }
  else
  {
...
// ----------------------------------------------------------------------------
int svtkTextActor::UpdateRectangle(svtkViewport* viewport)
{
...
// ----------------------------------------------------------------------------
void svtkTextActor::SpecifiedToDisplay(double* pos, svtkViewport* vport, int specified)
{
  if (!vport)
  {
    return;
...
// ----------------------------------------------------------------------------
void svtkTextActor::DisplayToSpecified(double* pos, svtkViewport* vport, int specified)
{
  switch (specified)
  {
    case SVTK_WORLD:
...
// ----------------------------------------------------------------------------
void svtkTextActor::PrintSelf(ostream& os, svtkIndent indent)
{
