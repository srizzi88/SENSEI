/*=========================================================================
  ...
  Module:    svtkTransformInterpolator.cxx
...
=========================================================================*/
#include "svtkTransformInterpolator.h"
#include "svtkMath.h"
...
svtkStandardNewMacro(svtkTransformInterpolator);
...
struct svtkQTransform
{
  ...
  svtkQTransform(double t, svtkTransform* xform)
  {
    this->Time = t;
    if (xform)
    {
      ...
      q[0] = svtkMath::RadiansFromDegrees(q[0]);
...
class svtkTransformList : public std::list<svtkQTransform>
{
};
typedef svtkTransformList::iterator TransformListIterator;

//----------------------------------------------------------------------------
svtkTransformInterpolator::svtkTransformInterpolator()
{
  ...
  this->ScaleInterpolator = svtkTupleInterpolator::New();
  this->RotationInterpolator = svtkQuaternionInterpolator::New();

  // Quaternion interpolation
  this->TransformList = new svtkTransformList;
  this->Initialized = 0;
}
...
//----------------------------------------------------------------------------
svtkMTimeType svtkTransformInterpolator::GetMTime()
{
  svtkMTimeType mTime = this->Superclass::GetMTime();
  svtkMTimeType posMTime, scaleMTime, rotMTime;
...
//----------------------------------------------------------------------------
int svtkTransformInterpolator::GetNumberOfTransforms()
{
  return static_cast<int>(this->TransformList->size());
}

//----------------------------------------------------------------------------
double svtkTransformInterpolator::GetMinimumT()
{
...
//----------------------------------------------------------------------------
double svtkTransformInterpolator::GetMaximumT()
{
  if (this->TransformList->empty())
  {
    return SVTK_FLOAT_MAX;
...
//----------------------------------------------------------------------------
void svtkTransformInterpolator::Initialize()
{
  this->TransformList->clear();
}

//----------------------------------------------------------------------------
void svtkTransformInterpolator::AddTransform(double t, svtkTransform* xform)
{
  ...
  {
    this->TransformList->push_front(svtkQTransform(t, xform));
    return;
  }
  else if (t > this->TransformList->back().Time)
  {
    this->TransformList->push_back(svtkQTransform(t, xform));
    return;
  }
  else if (size == 1 && t == this->TransformList->back().Time)
  {
    this->TransformList->front() = svtkQTransform(t, xform);
    return;
  }
  ...
  for (int i = 0; i < (size - 1); i++, ++iter, ++nextIter)
  {
    if (t == iter->Time)
    {
      (*iter) = svtkQTransform(t, xform);
    }
    else if (t > iter->Time && t < nextIter->Time)
    {
      this->TransformList->insert(nextIter, svtkQTransform(t, xform));
    }
  }
...
//----------------------------------------------------------------------------
void svtkTransformInterpolator::AddTransform(double t, svtkMatrix4x4* matrix)
{
...
//----------------------------------------------------------------------------
void svtkTransformInterpolator::AddTransform(double t, svtkProp3D* prop3D)
{
  this->AddTransform(t, prop3D->GetMatrix());
}

//----------------------------------------------------------------------------
void svtkTransformInterpolator::RemoveTransform(double t)
{
...
//----------------------------------------------------------------------------
void svtkTransformInterpolator::SetPositionInterpolator(svtkTupleInterpolator* pi)
{
...
//----------------------------------------------------------------------------
void svtkTransformInterpolator::SetScaleInterpolator(svtkTupleInterpolator* si)
{
...
//----------------------------------------------------------------------------
void svtkTransformInterpolator::SetRotationInterpolator(svtkQuaternionInterpolator* ri)
{
...
//----------------------------------------------------------------------------
void svtkTransformInterpolator::InitializeInterpolation()
{
  if (this->TransformList->empty())
  {
    return;
...
//----------------------------------------------------------------------------
void svtkTransformInterpolator::InterpolateTransform(double t, svtkTransform* xform)
{
  if (this->TransformList->empty())
  {
    return;
  ...
  Q[0] = svtkMath::DegreesFromRadians(q.GetRotationAngleAndAxis(Q + 1));
...
//----------------------------------------------------------------------------
void svtkTransformInterpolator::PrintSelf(ostream& os, svtkIndent indent)
{
