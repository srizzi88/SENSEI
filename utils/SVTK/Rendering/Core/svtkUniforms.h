 ...
 * the implementation is found in graphics api specific derived classes (e.g.: svtkOpenGLUniforms).
 *
 * @sa
 * svtkOpenGLUniforms svtkShaderProperty
 *
 * @par Thanks:
 * Developed by Simon Drouin (sdrouin2@bwh.harvard.edu) at Brigham and Women's Hospital.
 */

#ifndef svtkUniforms_h
#define svtkUniforms_h
...
class SVTKRENDERINGCORE_EXPORT svtkUniforms : public svtkObject
{
public:
  static svtkUniforms* New();
  svtkTypeMacro(svtkUniforms, svtkObject);
  void PrintSelf(ostream& os, svtkIndent indent) override;

  virtual svtkMTimeType GetUniformListMTime() = 0;
  ...
  virtual void SetUniform(const char* name, svtkUniforms::TupleType tt, int nbComponents,
  ...
  virtual void SetUniformMatrix(const char* name, svtkMatrix3x3* v) = 0;
  virtual void SetUniformMatrix(const char* name, svtkMatrix4x4* v) = 0;
  //@}
  ...
  virtual bool GetUniformMatrix(const char* name, svtkMatrix3x3* v) = 0;
  virtual bool GetUniformMatrix(const char* name, svtkMatrix4x4* v) = 0;
  //@}
  ...
  virtual const char* GetNthUniformName(svtkIdType uniformIndex) = 0;
...
protected:
  svtkUniforms() {}
  ~svtkUniforms() override {}

private:
  svtkUniforms(const svtkUniforms&) = delete;
  void operator=(const svtkUniforms&) = delete;
};
