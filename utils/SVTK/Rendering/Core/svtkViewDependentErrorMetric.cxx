/*=========================================================================
  ...
  Module:    svtkViewDependentErrorMetric.cxx
...
svtkStandardNewMacro(svtkViewDependentErrorMetric);

//-----------------------------------------------------------------------------
svtkViewDependentErrorMetric::svtkViewDependentErrorMetric()
{
...
void svtkViewDependentErrorMetric::SetPixelTolerance(double value)
{
...
void svtkViewDependentErrorMetric::SetViewport(svtkViewport* viewport)
{
...
//-----------------------------------------------------------------------------
int svtkViewDependentErrorMetric::RequiresEdgeSubdivision(
...
double svtkViewDependentErrorMetric::GetError(
  double* leftPoint, double* midPoint, double* rightPoint, double svtkNotUsed(alpha))
{
  assert("pre: leftPoint_exists" && leftPoint != nullptr);
  assert("pre: midPoint_exists" && midPoint != nullptr);
...
double svtkViewDependentErrorMetric::Distance2LinePoint(double x[2], double y[2], double z[2])
{
  double u[2];
  double v[2];
  double w[2];
  ...
  svtkMath::Normalize2D(u);

  v[0] = z[0] - x[0];
  v[1] = z[1] - x[1];

  double dot = svtkMath::Dot2D(u, v);

  w[0] = v[0] - dot * u[0];
  w[1] = v[1] - dot * u[1];

  return svtkMath::Dot2D(w, w);
}

//-----------------------------------------------------------------------------
void svtkViewDependentErrorMetric::PrintSelf(ostream& os, svtkIndent indent)
{
