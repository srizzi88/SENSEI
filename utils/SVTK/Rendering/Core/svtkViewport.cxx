...
svtkViewport::svtkViewport()
{
  ...
  this->Props = svtkPropCollection::New();
  this->Actors2D = svtkActor2DCollection::New();
}
...
//----------------------------------------------------------------------------
void svtkViewport::AddActor2D(svtkProp* p)
{
  this->AddViewProp(p);
}

//----------------------------------------------------------------------------
void svtkViewport::RemoveActor2D(svtkProp* p)
{
...
//----------------------------------------------------------------------------
int svtkViewport::HasViewProp(svtkProp* p)
{
  return (p && this->Props->IsItemPresent(p));
}

//----------------------------------------------------------------------------
void svtkViewport::AddViewProp(svtkProp* p)
{
...
//----------------------------------------------------------------------------
void svtkViewport::RemoveViewProp(svtkProp* p)
{
  if (p && this->HasViewProp(p))
  {
    if (this->SVTKWindow)
    {
      this->SVTKWindow->MakeCurrent();
    }
    p->ReleaseGraphicsResources(this->SVTKWindow);
...
svtkActor2DCollection* svtkViewport::GetActors2D()
{
  svtkProp* aProp;
...
void svtkViewport::DisplayToView()
{
  if (this->SVTKWindow)
  {
    double vx, vy, vz;
    ...
    const int* size = this->SVTKWindow->GetSize();
    if (size == nullptr)
    {
      return;
    }
...
void svtkViewport::ViewToDisplay()
{
  if (this->SVTKWindow)
  {
    double dx, dy;
...
int* svtkViewport::GetOrigin()
{
  if (this->SVTKWindow)
  {
    const int* winSize = this->SVTKWindow->GetSize();
...
double* svtkViewport::GetCenter()
{
  if (this->SVTKWindow)
  {
    // get physical window dimensions
    const int* size = this->GetSVTKWindow()->GetSize();
    if (size)
    {
...
int svtkViewport::IsInViewport(int x, int y)
{
  if (this->SVTKWindow)
  {
    // get physical window dimensions
...
//----------------------------------------------------------------------------
void svtkViewport::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
void svtkViewport::LocalDisplayToDisplay(double& svtkNotUsed(u), double& v)
{
  if (this->SVTKWindow)
  {
    /* get physical window dimensions */
...
//----------------------------------------------------------------------------
void svtkViewport::DisplayToLocalDisplay(double& svtkNotUsed(u), double& v)
{
  if (this->SVTKWindow)
  {
    /* get physical window dimensions */
...
//----------------------------------------------------------------------------
void svtkViewport::DisplayToNormalizedDisplay(double& u, double& v)
{
  if (this->SVTKWindow)
  {
    /* get physical window dimensions */
...
//----------------------------------------------------------------------------
void svtkViewport::NormalizedDisplayToViewport(double& u, double& v)
{
  if (this->SVTKWindow)
  {
...
//----------------------------------------------------------------------------
void svtkViewport::ViewportToNormalizedViewport(double& u, double& v)
{
  if (this->SVTKWindow)
  {
    /* get physical window dimensions */
...
//----------------------------------------------------------------------------
void svtkViewport::NormalizedViewportToView(double& x, double& y, double& svtkNotUsed(z))
{
  if (this->SVTKWindow)
  {
    // for tiling we must consider the tiledViewport
    double* tvport = this->SVTKWindow->GetTileViewport();
...
//----------------------------------------------------------------------------
void svtkViewport::NormalizedDisplayToDisplay(double& u, double& v)
{
  if (this->SVTKWindow)
  {
    /* get physical window dimensions */
...
//----------------------------------------------------------------------------
void svtkViewport::ViewportToNormalizedDisplay(double& u, double& v)
{
  if (this->SVTKWindow)
  {
    // get the pixel value for the viewport origin
...
//----------------------------------------------------------------------------
void svtkViewport::NormalizedViewportToViewport(double& u, double& v)
{
  if (this->SVTKWindow)
  {
    /* get physical window dimensions */
...
//----------------------------------------------------------------------------
void svtkViewport::ViewToNormalizedViewport(double& x, double& y, double& svtkNotUsed(z))
{
  if (this->SVTKWindow)
  {
    // for tiling we must consider the tiledViewport
...
void svtkViewport::ComputeAspect()
{
  if (this->SVTKWindow)
  {
    ...
    {
      // it happens if the svtkWindow is attached to the svtkViewport but
      // the svtkWindow is not initialized yet, so size[0]==0 and size[1]==0
...
//----------------------------------------------------------------------------
svtkAssemblyPath* svtkViewport::PickPropFrom(
  double selectionX, double selectionY, svtkPropCollection* pickfrom)
{
...
//----------------------------------------------------------------------------
svtkAssemblyPath* svtkViewport::PickPropFrom(double selectionX1, double selectionY1,
  double selectionX2, double selectionY2, svtkPropCollection* pickfrom)
{
  this->PickFromProps = pickfrom;
...
void svtkViewport::GetTiledSize(int* usize, int* vsize)
{
  int llx, lly;
...
//----------------------------------------------------------------------------
void svtkViewport::GetTiledSizeAndOrigin(int* usize, int* vsize, int* lowerLeftU, int* lowerLeftV)
{
  double* vport;
  ...
  if (this->GetSVTKWindow())
  {
    this->GetSVTKWindow()->GetTileViewport(tileViewPort);
  }
  else
  {
    tileViewPort[0] = 0;
  ...
  double vpu = svtkMath::ClampValue(vport[0] - tileViewPort[0], 0.0, 1.0);
  double vpv = svtkMath::ClampValue(vport[1] - tileViewPort[1], 0.0, 1.0);
  ...
  *lowerLeftV = static_cast<int>(vpv + 0.5);
  ...
  // lower left boundary of this tile
  double vpu2 = svtkMath::ClampValue(vport[2] - tileViewPort[0], 0.0, 1.0);
  double vpv2 = svtkMath::ClampValue(vport[3] - tileViewPort[1], 0.0, 1.0);
