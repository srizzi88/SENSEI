 ...
 *
 *
 * svtkVolume is used to represent a volumetric entity in a rendering scene.
 * It inherits functions related to the volume's position, orientation and
 * origin from svtkProp3D. The volume maintains a reference to the
 ...
 *
 * @sa
 * svtkAbstractVolumeMapper svtkVolumeProperty svtkProp3D
 */
...
class SVTKRENDERINGCORE_EXPORT svtkVolume : public svtkProp3D
{
public:
  svtkTypeMacro(svtkVolume, svtkProp3D);
   ...
   * orientation=(0,0,0).
   */
  static svtkVolume* New();

  //@{
  /**
   * Set/Get the volume mapper.
   */
  void SetMapper(svtkAbstractVolumeMapper* mapper);
  svtkGetObjectMacro(Mapper, svtkAbstractVolumeMapper);
  //@}
   ...
   */
  virtual void SetProperty(svtkVolumeProperty* property);
  virtual svtkVolumeProperty* GetProperty();
  //@}
   ...
   */
  void GetVolumes(svtkPropCollection* vc) override;
  ...
  /**
   * Shallow copy of this svtkVolume. Overloads the virtual svtkProp method.
   */
   ...
   * DO NOT USE THIS METHOD OUTSIDE OF THE RENDERING PROCESS
   */
  void UpdateScalarOpacityforSampleSize(svtkRenderer* ren, float sample_distance);
...
protected:
  ...
  svtkAbstractVolumeMapper* Mapper;
  ...
  float* RGBArray[SVTK_MAX_VRCOMP];
  svtkTimeStamp RGBArrayMTime[SVTK_MAX_VRCOMP];

  // The gray transfer function array - for unsigned char data this
  // is 256 elements, for short or unsigned short it is 65536 elements
  // This is a sample at each scalar value of the gray transfer
  // function.  A time stamp is kept to know when it needs rebuilding
  float* GrayArray[SVTK_MAX_VRCOMP];
  svtkTimeStamp GrayArrayMTime[SVTK_MAX_VRCOMP];
  ...
  // function.  A time stamp is kept to know when it needs rebuilding
  float* ScalarOpacityArray[SVTK_MAX_VRCOMP];
  svtkTimeStamp ScalarOpacityArrayMTime[SVTK_MAX_VRCOMP];
  ...
  float* CorrectedScalarOpacityArray[SVTK_MAX_VRCOMP];
  svtkTimeStamp CorrectedScalarOpacityArrayMTime[SVTK_MAX_VRCOMP];
  ...
  float GradientOpacityArray[SVTK_MAX_VRCOMP][256];
  float GradientOpacityConstant[SVTK_MAX_VRCOMP];
  svtkTimeStamp GradientOpacityArrayMTime[SVTK_MAX_VRCOMP];

  // Function to compute screen coverage of this volume
  double ComputeScreenCoverage(svtkViewport* vp);

private:
  svtkVolume(const svtkVolume&) = delete;
  void operator=(const svtkVolume&) = delete;
};
