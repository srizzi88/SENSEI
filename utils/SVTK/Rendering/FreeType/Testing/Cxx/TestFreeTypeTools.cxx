...
//----------------------------------------------------------------------------
int CheckIfIDExists(svtkTextProperty* property, std::set<size_t>& ids)
{
  size_t id;
  svtkFreeTypeTools::GetInstance()->MapTextPropertyToId(property, &id);
  if (!ids.empty() && ids.find(id) != ids.end())
  {
    std::cout << "ID " << id << " already exists for other svtkTextProperty settings\n";
    return EXIT_FAILURE;
  }
  ids.insert(id);
...
int TestFreeTypeTools(int, char*[])
{
  ...
  svtkSmartPointer<svtkTextProperty> property = svtkSmartPointer<svtkTextProperty>::New();
