...
{
template <typename T>
void configureTextArray(svtkNew<T> objs[2][2][3], const std::string& prefix)
{
  ...
  for (size_t t = 0; t < 2; ++t)
  {
    for (size_t e = 0; e < 2; ++e)
    {
      ...
      {
        T* obj = objs[t][e][b];
        svtkTextProperty* tprop = obj->GetTextProperty();
...
// svtkTextProperty::ForceOpaqueTextures is handled by the opaque render pass.
int TestTextOpaquePass(int, char*[])
{
  ...
  svtkNew<svtkTextActor> textActor[2][2][3];

  svtkNew<svtkTextActor3D> textActor3D[2][2][3];

  svtkNew<svtkTextMapper> textMapper[2][2][3];
  svtkNew<svtkActor2D> textMapperActor[2][2][3];

  svtkNew<svtkBillboardTextActor3D> billboardActor[2][2][3];

  configureTextArray(textActor, "svtkTextActor");
  configureTextArray(textActor3D, "svtkTextActor3D");
  configureTextArray(textMapper, "svtkTextMapper");
  configureTextArray(billboardActor, "svtkBillboardTextActor3D");

  const int width = 600;
  const int height = 600;

  // Disable everything but opaque and overlay:
  svtkNew<svtkRenderStepsPass> pass;
