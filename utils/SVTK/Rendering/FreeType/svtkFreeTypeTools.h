 ...
 *
 * svtkFreeTypeTools provides a low-level interface to the FreeType2 library,
...
#ifndef svtkFreeTypeTools_h
#define svtkFreeTypeTools_h

#include "svtkObject.h"
#include "svtkRenderingFreeTypeModule.h" // For export macro
#include "svtkSmartPointer.h"            // For smart pointer
#include "svtkTextRenderer.h"            // For Metrics struct
#include "svtkUnicodeString.h"           // For svtkUnicodeStringValueType
...
// FreeType
#include "svtk_freetype.h" //since ft2build.h could be in the path
...
// PIMPL class for FTC_FaceID->svtkTextProperty lookup
class svtkTextPropertyLookup;

//----------------------------------------------------------------------------
// Singleton cleanup
class SVTKRENDERINGFREETYPE_EXPORT svtkFreeTypeToolsCleanup
{
public:
  svtkFreeTypeToolsCleanup();
  ~svtkFreeTypeToolsCleanup();

private:
  svtkFreeTypeToolsCleanup(const svtkFreeTypeToolsCleanup&) = delete;
  svtkFreeTypeToolsCleanup& operator=(const svtkFreeTypeToolsCleanup&) = delete;
};

//----------------------------------------------------------------------------
// Singleton font cache
class SVTKRENDERINGFREETYPE_EXPORT svtkFreeTypeTools : public svtkObject
{
public:
  struct FaceMetrics
  {
  ...
  {
    int HorizAdvance;
    svtkSmartPointer<svtkPath> Path;
  };

  svtkTypeMacro(svtkFreeTypeTools, svtkObject);
  void PrintSelf(ostream& os, svtkIndent indent) override;

  /**
   * Return the singleton instance with no reference counting.
   */
  static svtkFreeTypeTools* GetInstance();

  /**
   * Supply a user defined instance. Call Delete() on the supplied
   * instance after setting it to fix the reference count.
   */
  static void SetInstance(svtkFreeTypeTools* instance);
   ...
   */
  svtkSetMacro(DebugTextures, bool);
  svtkGetMacro(DebugTextures, bool);
  svtkBooleanMacro(DebugTextures, bool);
  //@}
   ...
   * square.
   */
  FaceMetrics GetFaceMetrics(svtkTextProperty* tprop);
   ...
   */
  GlyphOutline GetUnscaledGlyphOutline(svtkTextProperty* tprop, svtkUnicodeStringValueType charId);
   ...
   * the EM square.
   */
  std::array<int, 2> GetUnscaledKerning(svtkTextProperty* tprop, svtkUnicodeStringValueType leftChar,
    svtkUnicodeStringValueType rightChar);
   ...
   */
  svtkSetClampMacro(MaximumNumberOfFaces, unsigned int, 1, SVTK_UNSIGNED_INT_MAX);
  svtkGetMacro(MaximumNumberOfFaces, unsigned int);
  svtkSetClampMacro(MaximumNumberOfSizes, unsigned int, 1, SVTK_UNSIGNED_INT_MAX);
  svtkGetMacro(MaximumNumberOfSizes, unsigned int);
  svtkSetClampMacro(MaximumNumberOfBytes, unsigned long, 1, SVTK_UNSIGNED_LONG_MAX);
  svtkGetMacro(MaximumNumberOfBytes, unsigned long);
  //@}
   ...
   * @sa GetMetrics
   */
  bool GetBoundingBox(svtkTextProperty* tprop, const svtkStdString& str, int dpi, int bbox[4]);
  bool GetBoundingBox(svtkTextProperty* tprop, const svtkUnicodeString& str, int dpi, int bbox[4]);
  //@}
  ...
  bool GetMetrics(svtkTextProperty* tprop, const svtkUnicodeString& str, int dpi,
   ...
   * options.
   */
  bool RenderString(svtkTextProperty* tprop, const svtkStdString& str, int dpi, svtkImageData* data,
    int textDims[2] = nullptr);
  bool RenderString(svtkTextProperty* tprop, const svtkUnicodeString& str, int dpi,
    svtkImageData* data, int textDims[2] = nullptr);
  //@}
   ...
   */
  bool StringToPath(svtkTextProperty* tprop, const svtkStdString& str, int dpi, svtkPath* path);
  bool StringToPath(svtkTextProperty* tprop, const svtkUnicodeString& str, int dpi, svtkPath* path);
  //@}
  ...
  int GetConstrainedFontSize(const svtkUnicodeString& str, svtkTextProperty* tprop, int dpi,
   ...
   */
  static svtkTypeUInt16 HashString(const char* str);

  /**
   * Hash a string of a given length. This function hashes n chars and does
   * not depend on a terminating null character.
   */
  static svtkTypeUInt32 HashBuffer(const void* str, size_t n, svtkTypeUInt32 hash = 0);
   ...
   * stored).
   */
  void MapTextPropertyToId(svtkTextProperty* tprop, size_t* tprop_cache_id);
  void MapIdToTextProperty(size_t tprop_cache_id, svtkTextProperty* tprop);
  //@}
   ...
   * hardware.
   */
  svtkSetMacro(ScaleToPowerTwo, bool);
  svtkGetMacro(ScaleToPowerTwo, bool);
  svtkBooleanMacro(ScaleToPowerTwo, bool);
  //@}

  //@{
  /**
   * Force use of the fonts compiled into SVTK, ignoring any FontConfig or
   * embedded fonts. Useful for generating test images consistently across
   * platforms. This flag is on by default.
   */
  svtkSetMacro(ForceCompiledFonts, bool);
  svtkGetMacro(ForceCompiledFonts, bool);
  svtkBooleanMacro(ForceCompiledFonts, bool);
  //@}
   ...
   */
  static bool LookupFace(svtkTextProperty* tprop, FT_Library lib, FT_Face* face);
...
protected:
  ...
  class MetaData;
  class ImageMetaData;
  bool PrepareMetaData(svtkTextProperty* tprop, int dpi, MetaData& metaData);
  bool PrepareImageMetaData(svtkTextProperty* tprop, svtkImageData* image, ImageMetaData& metaData);
  //@}
   ...
   */
  void PrepareImageData(svtkImageData* data, int bbox[4]);

  /**
   * Draw the background quad on the image.
   */
  void RenderBackground(svtkTextProperty* tprop, svtkImageData* image, ImageMetaData& metaData);
   ...
   */
  bool GetSize(svtkTextProperty* tprop, FT_Size* size);
   ...
   * Return true on success, false otherwise
   */
  bool GetFace(svtkTextProperty* tprop, FT_Face* face);
   ...
   * Return true on success, false otherwise
   */
  bool GetGlyphIndex(svtkTextProperty* tprop, FT_UInt32 c, FT_UInt* gindex);
  ...
  bool GetGlyph(
    svtkTextProperty* tprop, FT_UInt32 c, FT_Glyph* glyph, int request = GLYPH_REQUEST_DEFAULT);
  ...
  svtkFreeTypeTools();
  ~svtkFreeTypeTools() override;

  /**
   * Attempt to get the typeface of the specified font.
   */
  bool GetFace(svtkTextProperty* prop, size_t& prop_cache_id, FT_Face& face, bool& face_has_kerning);
   ...
   */
  static svtkFreeTypeTools* Instance;

  /**
   * Lookup table that maps free type font cache face ids to svtkTextProperties
   */
  svtkTextPropertyLookup* TextPropertyLookup;
...
private:
  svtkFreeTypeTools(const svtkFreeTypeTools&) = delete;
  void operator=(const svtkFreeTypeTools&) = delete;
   ...
   */
  template <typename StringType>
  bool StringToPathInternal(svtkTextProperty* tprop, const StringType& str, int dpi, svtkPath* path);
   ...
   */
  bool CalculateBoundingBox(const svtkStdString& str, MetaData& metaData);
  bool CalculateBoundingBox(const svtkUnicodeString& str, MetaData& metaData);
  template <typename T>
  ...
  bool RenderCharacter(CharType character, int& x, int& y, FT_UInt& previousGlyphIndex,
    svtkImageData* image, MetaData& metaData);
  template <typename CharType>
  bool RenderCharacter(CharType character, int& x, int& y, FT_UInt& previousGlyphIndex,
    svtkPath* path, MetaData& metaData);
  //@}

  void OutlineToPath(int x, int y, FT_Outline* outline, svtkPath* path);
...
static svtkFreeTypeToolsCleanup svtkFreeTypeToolsCleanupInstance;
