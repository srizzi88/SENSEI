/*=========================================================================
  ...
  Module:    svtkOpenGLGL2PSHelperImpl.cxx
...
#include "svtkOpenGLGL2PSHelperImpl.h"
...
svtkStandardNewMacro(svtkOpenGLGL2PSHelperImpl);

namespace
{

bool GetMetrics(
  svtkRenderWindow* renwin, svtkTextProperty* tprop, const char* str, svtkTextRenderer::Metrics& m)
{
  ...
  svtkNew<svtkTextProperty> tpropTmp;
...
void GetSpaceStr(const char* str, svtkStdString* spaceStr)
{
  *spaceStr = str;
...
bool ComputeBottomLeft(
  svtkTextProperty* tprop, svtkTuple<int, 4> bbox, double pos[3], bool textAsPath, double blpos[3])
{
  ...
  if (gl2psGetFileFormat() == GL2PS_PDF && !textAsPath &&
    (tprop->GetJustification() != SVTK_TEXT_LEFT ||
      tprop->GetVerticalJustification() != SVTK_TEXT_BOTTOM))
  {
...
//------------------------------------------------------------------------------
void svtkOpenGLGL2PSHelperImpl::PrintSelf(std::ostream& os, svtkIndent indent)
{
  this->Superclass::PrintSelf(os, indent);
}

//------------------------------------------------------------------------------
void svtkOpenGLGL2PSHelperImpl::ProcessTransformFeedback(
  svtkTransformFeedback* tfc, svtkRenderer* ren, svtkActor* act)
{
...
//------------------------------------------------------------------------------
void svtkOpenGLGL2PSHelperImpl::ProcessTransformFeedback(
  svtkTransformFeedback* tfc, svtkRenderer* ren, unsigned char col[4])
{
...
//------------------------------------------------------------------------------
void svtkOpenGLGL2PSHelperImpl::ProcessTransformFeedback(
  svtkTransformFeedback* tfc, svtkRenderer* ren, float col[4])
{
  ...
  // Captured data:
  typedef std::vector<svtkTransformFeedback::VaryingMetaData> VarVector;
  const VarVector& vars = tfc->GetVaryings();
  unsigned char* data = static_cast<unsigned char*>(tfc->GetBufferData());

  if (!data)
  {
    svtkErrorMacro("TransformFeedback buffer is nullptr.");
    return;
  }
  ...
  while (data < dataEnd)
  {
    ...
    for (VarVector::const_iterator it = vars.begin(), itEnd = vars.end(); it != itEnd; ++it)
    {
      ...
      switch (it->Role)
      {
        case svtkTransformFeedback::Vertex_ClipCoordinate_F:
        {
          posSet = true;
        ...
        case svtkTransformFeedback::Color_RGBA_F:
        {
          float* fdata = reinterpret_cast<float*>(data);
        ...
        case svtkTransformFeedback::Normal_F:
        case svtkTransformFeedback::Next_Buffer:
        default:
          svtkWarningMacro("Unhandled data role: " << it->Role);
      }
    ...
    if (!posSet)
    {
      ...
      svtkErrorMacro("Position info missing from capture.");
    }
    ...
    switch (primitiveMode)
    {
      ...
      default:
        svtkWarningMacro("Unhandled primitive mode: " << primitiveMode);
        break;
    }
  }
...
//------------------------------------------------------------------------------
void svtkOpenGLGL2PSHelperImpl::DrawString(const std::string& str, svtkTextProperty* tprop,
  double pos[3], double backgroundDepth, svtkRenderer* ren)
{
  if (str.empty())
  {
    return;
  ...
  if (tren == nullptr)
  {
    svtkErrorMacro("svtkTextRenderer unavailable.");
    return;
  }
  ...
  // Is this mathtext?
  bool isMath = tren->DetectBackend(str) == svtkTextRenderer::MathText;
...
//------------------------------------------------------------------------------
void svtkOpenGLGL2PSHelperImpl::DrawPath(svtkPath* path, double rasterPos[3], double windowPos[2],
...
//------------------------------------------------------------------------------
void svtkOpenGLGL2PSHelperImpl::Draw3DPath(svtkPath* path, svtkMatrix4x4* actorMatrix,
  double rasterPos[3], unsigned char actorColor[4], svtkRenderer* ren, const char* label)
{
  double translation[2] = { 0.0, 0.0 };
  svtkNew<svtkPath> projPath;
...
//------------------------------------------------------------------------------
void svtkOpenGLGL2PSHelperImpl::DrawImage(svtkImageData* input, double pos[3])
{
  // Must be RGB/RGBA:
  GLenum format = 0;
  switch (input->GetNumberOfScalarComponents())
  {
    ...
    case 4:
      ...
      break;
    default:
      svtkErrorMacro("Invalid image format: Must be RGB or RGBA.");
      return;
  }

  if (input->GetDimensions()[2] != 1)
  {
    svtkErrorMacro("Invalid image format: 3D ImageData are not supported.");
    return;
  }
  ...
  if (!svtkDataTypesCompare(inScalars->GetDataType(), SVTK_FLOAT))
  {
    svtkErrorMacro("Invalid image format: Expected float scalars.");
    return;
  }
...
//------------------------------------------------------------------------------
svtkOpenGLGL2PSHelperImpl::svtkOpenGLGL2PSHelperImpl() = default;

//------------------------------------------------------------------------------
svtkOpenGLGL2PSHelperImpl::~svtkOpenGLGL2PSHelperImpl() = default;

//------------------------------------------------------------------------------
const char* svtkOpenGLGL2PSHelperImpl::TextPropertyToPSFontName(svtkTextProperty* tprop)
{
...
//------------------------------------------------------------------------------
int svtkOpenGLGL2PSHelperImpl::TextPropertyToGL2PSAlignment(svtkTextProperty* tprop)
{
  switch (tprop->GetJustification())
  {
    case SVTK_TEXT_LEFT:
...
//------------------------------------------------------------------------------
void svtkOpenGLGL2PSHelperImpl::GetTransformParameters(svtkRenderer* ren, svtkMatrix4x4* actorMatrix,
  svtkMatrix4x4* xform, double vpOrigin[2], double halfSize[2], double zFact[2])
{
  ...
  if (actorMatrix)
  {
    svtkMatrix4x4::Multiply4x4(xform, actorMatrix, xform);
  }
...
//------------------------------------------------------------------------------
inline void svtkOpenGLGL2PSHelperImpl::ProjectPoint(
  double point[3], svtkRenderer* ren, svtkMatrix4x4* actorMatrix)
{
  svtkNew<svtkMatrix4x4> xform;
  double vpOrigin[2];
  double halfSize[2];
  double zFact[2];
  svtkOpenGLGL2PSHelperImpl::GetTransformParameters(
    ren, actorMatrix, xform, vpOrigin, halfSize, zFact);

  double tmp[4] = { point[0], point[1], point[2], 1. };
  svtkOpenGLGL2PSHelperImpl::ProjectPoint(
    tmp, xform, vpOrigin, halfSize[0], halfSize[1], zFact[0], zFact[1]);
}

//------------------------------------------------------------------------------
inline void svtkOpenGLGL2PSHelperImpl::ProjectPoint(double point[4], svtkMatrix4x4* transformMatrix,
...
//------------------------------------------------------------------------------
inline void svtkOpenGLGL2PSHelperImpl::ProjectPoints(
  svtkPoints* points, svtkRenderer* ren, svtkMatrix4x4* actorMatrix)
{
  svtkNew<svtkMatrix4x4> xform;
  double vpOrigin[2];
  double halfSize[2];
...
//------------------------------------------------------------------------------
inline void svtkOpenGLGL2PSHelperImpl::UnprojectPoint(double point[4],
  svtkMatrix4x4* invTransformMatrix, double viewportOrigin[2], double halfWidth, double halfHeight,
...
//------------------------------------------------------------------------------
inline void svtkOpenGLGL2PSHelperImpl::UnprojectPoints(
  double* points3D, svtkIdType numPoints, svtkRenderer* ren, svtkMatrix4x4* actorMatrix)
{
  svtkNew<svtkMatrix4x4> xform;
  double vpOrigin[2];
  double halfSize[2];
  ...
  for (svtkIdType i = 0; i < numPoints; ++i)
  {
    ...
    point[3] = 1.0;
    svtkOpenGLGL2PSHelperImpl::UnprojectPoint(
      point, xform, vpOrigin, halfSize[0], halfSize[1], zFact[0], zFact[1]);
...
//------------------------------------------------------------------------------
void svtkOpenGLGL2PSHelperImpl::DrawPathPS(svtkPath* path, double rasterPos[3], double windowPos[2],
  ...
  const std::string& label)
{
  ...
  while (code < codeEnd)
  {
    ...
    switch (static_cast<svtkPath::ControlPointType>(*code))
    {
...
//------------------------------------------------------------------------------
void svtkOpenGLGL2PSHelperImpl::DrawPathPDF(svtkPath* path, double rasterPos[3], double windowPos[2],
  unsigned char rgba[4], double scale[2], double rotateAngle, float strokeWidth,
  const std::string& /*label*/)
{
  svtkFloatArray* points = svtkArrayDownCast<svtkFloatArray>(path->GetPoints()->GetData());
  ...
  out << 1.f << " " << 0.f << " " << 0.f << " " << 1.f << " " << windowPos[0] << " " << windowPos[1]
      << " cm" << endl;
  // rotate
  float sT = sin(svtkMath::RadiansFromDegrees(rotateAngle));
  float cT = cos(svtkMath::RadiansFromDegrees(rotateAngle));
...
//------------------------------------------------------------------------------
void svtkOpenGLGL2PSHelperImpl::DrawPathSVG(svtkPath* path, double rasterPos[3], double windowPos[2],
  unsigned char rgba[4], double scale[2], double rotateAngle, float strokeWidth,
  const std::string& label)
{
  svtkFloatArray* points = svtkArrayDownCast<svtkFloatArray>(path->GetPoints()->GetData());
