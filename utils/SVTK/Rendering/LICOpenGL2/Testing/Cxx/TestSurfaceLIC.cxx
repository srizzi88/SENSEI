...
#include "svtkSurfaceLICTestDriver.h"
...
int TestSurfaceLIC(int argc, char* argv[])
{
  ...
  arg.AddArgument("--data", argT::EQUAL_ARGUMENT, &filename,
    "(required) Enter dataset to load (currently only *.[svtk|vtp] files are supported");
  ...
  svtkSmartPointer<svtkDataObject> dataObj;
  std::string ext = svtksys::SystemTools::GetFilenameExtension(filename);
  if (ext == ".svtk")
  {
  ...
  {
    svtkXMLPolyDataReader* reader = svtkXMLPolyDataReader::New();
    reader->SetFileName(filename.c_str());
    reader->Update();
  ...
  {
    svtkXMLMultiBlockDataReader* reader = svtkXMLMultiBlockDataReader::New();
    reader->SetFileName(filename.c_str());
  ...
  if (!dataObj) // || dataObj->GetNumberOfElements(svtkDataObject::POINT) == 0)
  {
  ...
  int status = svtkSurfaceLICTestDriver(argc, argv, dataObj, num_steps, step_size, enhanced_lic,
