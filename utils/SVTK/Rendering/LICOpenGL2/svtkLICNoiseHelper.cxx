...
//-----------------------------------------------------------------------------
void svtkLICRandomNoise2D::GetValidDimensionAndGrainSize(int type, int& sideLen, int& grainSize)
{
...
//-----------------------------------------------------------------------------
int svtkLICRandomNoise2D::ShouldGenerateValue(double prob)
{
...
//-----------------------------------------------------------------------------
float* svtkLICRandomNoise2D::Generate(int type, int& sideLen, int& grainSize, float minNoiseVal,
...
//-----------------------------------------------------------------------------
float* svtkLICRandomNoise2D::GenerateUniform(int sideLen, int grainSize, float minNoiseVal,
  float maxNoiseVal, int nLevels, double impulseProb, float impulseBgNoiseVal, int seed)
{
...
//-----------------------------------------------------------------------------
float* svtkLICRandomNoise2D::GenerateGaussian(int sideLen, int grainSize, float minNoiseVal,
  float maxNoiseVal, int nLevels, double impulseProb, float impulseBgNoiseVal, int seed)
{
...
//-----------------------------------------------------------------------------
float* svtkLICRandomNoise2D::GeneratePerlin(int sideLen, int grainSize, float minNoiseVal,
  float maxNoiseVal, int nLevels, double impulseProb, float impulseBgNoiseVal, int seed)
{
...
*/
svtkImageData* svtkLICRandomNoise2D::GetNoiseResource()
{
  std::string base64string;
  for (unsigned int cc = 0; cc < file_noise200x200_svtk_nb_sections; cc++)
  {
    base64string += reinterpret_cast<const char*>(file_noise200x200_svtk_sections[cc]);
  }

  unsigned char* binaryInput = new unsigned char[file_noise200x200_svtk_decoded_length + 10];

  unsigned long binarylength = static_cast<unsigned long>(
    svtkBase64Utilities::DecodeSafely(reinterpret_cast<const unsigned char*>(base64string.c_str()),
      base64string.length(), binaryInput, file_noise200x200_svtk_decoded_length + 10));

  assert("check valid_length" && (binarylength == file_noise200x200_svtk_decoded_length));
  ...
  reader->Update();
  svtkImageData* data = svtkImageData::New();
