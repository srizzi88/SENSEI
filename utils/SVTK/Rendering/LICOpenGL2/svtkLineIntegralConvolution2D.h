/*=========================================================================
  ...
  Module:    svtkLineIntegralConvolution2D.h
 ...
 *  as done in svtkSurfaceLICPainter).
 *
 ...
 *  svtkLineIntegralConvolution2D provides an option for creating enhanced LIC
 ...
 *
 *  svtkLineIntegralConvolution2D applies masking to zero-vector fragments so
 *  that un-filtered white noise areas are made totally transparent by class
 *  svtkSurfaceLICPainter to show the underlying geometry surface.
 *
 ...
 *
 * @sa
 *  svtkImageDataLIC2D svtkStructuredGridLIC2D
 */

#ifndef svtkLineIntegralConvolution2D_h
#define svtkLineIntegralConvolution2D_h
...
class SVTKRENDERINGLICOPENGL2_EXPORT svtkLineIntegralConvolution2D : public svtkObject
{
public:
  static svtkLineIntegralConvolution2D* New();
  svtkTypeMacro(svtkLineIntegralConvolution2D, svtkObject);
   ...
   */
  void SetContext(svtkOpenGLRenderWindow* context);
   ...
   */
  svtkSetClampMacro(EnhancedLIC, int, 0, 1);
  ...
  svtkSetClampMacro(EnhanceContrast, int, 0, 2);
  svtkGetMacro(EnhanceContrast, int);
  svtkBooleanMacro(EnhanceContrast, int);
  //@}
   ...
   * the min and max.
   */
  svtkSetClampMacro(LowContrastEnhancementFactor, double, 0.0, 1.0);
  svtkGetMacro(LowContrastEnhancementFactor, double);
  svtkSetClampMacro(HighContrastEnhancementFactor, double, 0.0, 1.0);
  svtkGetMacro(HighContrastEnhancementFactor, double);
  //@}
   ...
   * sufficient.
   */
  svtkSetClampMacro(AntiAlias, int, 0, SVTK_INT_MAX);
   ...
   */
  svtkSetClampMacro(NumberOfSteps, int, 0, SVTK_INT_MAX);
   ...
   * interface is double as SVTK interface is, but GPU only supports float.
   * Thus it will be converted to float in the execution of the algorithm.
   */
  svtkSetClampMacro(StepSize, double, 0.0, SVTK_FLOAT_MAX);
  ...
  svtkGetVector2Macro(ComponentIds, int);
  //@}
   ...
   */
  svtkSetClampMacro(MaxNoiseValue, double, 0.0, 1.0);
  ...
  svtkGetMacro(TransformVectors, int);
  //@}
   ...
   * svtkSetVector2Macro(GridSpacings, double);
   * svtkGetVector2Macro(GridSpacings, double);
   */
   ...
   * units.
   */
  svtkSetClampMacro(MaskThreshold, double, -1.0, SVTK_FLOAT_MAX);
   ...
   */
  svtkTextureObject* Execute(svtkTextureObject* vectorTex, svtkTextureObject* noiseTex);

  /**
   * Compute the lic on the indicated subset of the vector field
   * texture.
   */
  svtkTextureObject* Execute(
    const int extent[4], svtkTextureObject* vectorTex, svtkTextureObject* noiseTex);
   ...
   */
  svtkTextureObject* Execute(const svtkPixelExtent& inputTexExtent,
    const std::deque<svtkPixelExtent>& vectorExtent, const std::deque<svtkPixelExtent>& licExtent,
   ...
   */
  static void SetVectorTexParameters(svtkTextureObject* vectors);

  static void SetNoiseTexParameters(svtkTextureObject* noise);
  ...
  virtual svtkPainterCommunicator* GetCommunicator();
...
protected:
  ...
  ~svtkLineIntegralConvolution2D() override;

  svtkPainterCommunicator* Comm;

  void SetVTShader(svtkShaderProgram* prog);
  void SetLIC0Shader(svtkShaderProgram* prog);
  void SetLICIShader(svtkShaderProgram* prog);
  void SetLICNShader(svtkShaderProgram* prog);
  void SetEEShader(svtkShaderProgram* prog);
  void SetCEShader(svtkShaderProgram* prog);
  void SetAAHShader(svtkShaderProgram* prog);
  void SetAAVShader(svtkShaderProgram* prog);

  void BuildShaders();

  void RenderQuad(float computeBounds[4], svtkPixelExtent computeExtent);

  svtkTextureObject* AllocateBuffer(unsigned int texSize[2]);

  /**
   * Convenience functions to ensure that the input textures are
   * configured correctly.
   */
  void SetNoise2TexParameters(svtkTextureObject* noise);

  /**
   * Methods used for parallel benchmarks. Use cmake to define
   * svtkSurfaceLICPainterTIME to enable benchmarks. During each
   * update timing information is stored, it can be written to
   * disk by calling WriteLog (defined in svtkSurfaceLICPainter).
   */
...
protected:
  ...
  svtkOpenGLHelper* FinalBlendProgram;
  svtkOpenGLHelper* IntermediateBlendProgram;
  svtkOpenGLHelper* VTShader;
  svtkOpenGLHelper* LIC0Shader;
  svtkOpenGLHelper* LICIShader;
  svtkOpenGLHelper* LICNShader;
  svtkOpenGLHelper* EEShader;
  svtkOpenGLHelper* CEShader;
  svtkOpenGLHelper* AAHShader;
  svtkOpenGLHelper* AAVShader;
...
private:
  svtkLineIntegralConvolution2D(const svtkLineIntegralConvolution2D&) = delete;
  void operator=(const svtkLineIntegralConvolution2D&) = delete;
};
