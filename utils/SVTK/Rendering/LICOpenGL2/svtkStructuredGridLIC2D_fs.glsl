...
//  Module:    svtkStructuredGridLIC2D_fs.glsl
//
...
// Filename: svtkStructuredGridLIC2D_fs.glsl
...
mat3 svtkInverse(mat3 mm, float det)
{
  mat3 m=transpose(mm);
...
mat3 svtkJacobian(ivec3 ijk, vec3 dims, sampler2D tex)
{
...
void main(void)
{
  ...
  mat3 J = svtkJacobian(ijk, uDimensions, texPoints);

  // compute inverse of J.
  vec3 vector = getVector(ijk, uDimensions, texVectorField);
  float detJ = svtkDeterminant(J);
  mat3 invJ = svtkInverse(J, detJ);
