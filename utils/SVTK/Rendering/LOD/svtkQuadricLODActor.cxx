/*=========================================================================
  ...
  Module:    svtkQuadricLODActor.cxx
...
svtkStandardNewMacro(svtkQuadricLODActor);

//---------------------------------------------------------------------------
// Specify the quadric clustering algorithm for decimating the geometry.
svtkCxxSetObjectMacro(svtkQuadricLODActor, LODFilter, svtkQuadricClustering);

//-------------------------------------------------------------------------
svtkQuadricLODActor::svtkQuadricLODActor()
{
  // Configure the decimation (quadric clustering) filter
  this->LODFilter = svtkQuadricClustering::New();
  ...
  this->LODActor = svtkActor::New();

  // mapper for LOD actor
  this->LODMapper = svtkPolyDataMapper::New();
...
//----------------------------------------------------------------------------
int svtkQuadricLODActor::RenderOpaqueGeometry(svtkViewport* vp)
{
...
//----------------------------------------------------------------------------
void svtkQuadricLODActor::Render(svtkRenderer* ren, svtkMapper* svtkNotUsed(m))
{
  if (!this->Mapper)
  {
      ...
      (this->CachedInteractiveFrameRate > 1.1 * frameRate)))
  {
    ...
    // and dim=40.  This causes svtkQuadricClustering::AddTriangle()'s computations
    // to overflow.  If you set dim=35 there's no overflow, if you set it to 36 there is.

    // Construct the LOD
    svtkPolyData* pd = svtkPolyData::SafeDownCast(this->Mapper->GetInput());
    ...
    svtkDebugMacro("QC bin size: " << dim);
  ...
  // across renders.
  svtkMapper* bestMapper = this->Mapper;
#ifndef NDEBUG
...
#endif
    svtkDebugMacro("LOD render (best,allowed): " << bestTime << "," << allowedTime);
  }
  else
  { // use full resolution
    ...
    matrix = this->LODActor->GetUserMatrix();
    this->GetMatrix(matrix);
    svtkDebugMacro("----Full render (best,allowed): " << bestTime << "," << allowedTime);
  }
...
//----------------------------------------------------------------------------
void svtkQuadricLODActor::ReleaseGraphicsResources(svtkWindow* renWin)
{
...
//----------------------------------------------------------------------------
void svtkQuadricLODActor::ShallowCopy(svtkProp* prop)
{
  // Now do superclass
...
//----------------------------------------------------------------------------
void svtkQuadricLODActor::SetCamera(svtkCamera* camera)
{
  svtkFollower* follower = svtkFollower::SafeDownCast(this->LODActor);
  if (follower)
  {
...
//----------------------------------------------------------------------------
void svtkQuadricLODActor::PrintSelf(ostream& os, svtkIndent indent)
{
