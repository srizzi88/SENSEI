...
=========================================================================*/
/**
 * @class   svtkQuadricLODActor
 * @brief   a specific level-of-detail strategy using the
 * quadric clustering decimation algorithm
 *
 * svtkQuadricLODActor implements a specific strategy for level-of-detail
 * using the svtkQuadricClustering decimation algorithm. It supports only two
 ...
 * svtkRenderWindowInteractor's DesiredUpdateRate). It also makes use of
 * display lists for performance, and adjusts the svtkQuadricClustering
 ...
 * (this works with the svtkMapper::SetStatic() method). `Enabling Static results
 ...
 *
 * @warning
 * This class can be used as a direct replacement for svtkActor. It may also be
 * used as a replacement for svtkFollower's (the ability to track a camera is
 * provided).
 *
 * @sa
 * svtkLODActor svtkQuadricClustering
 */

#ifndef svtkQuadricLODActor_h
#define svtkQuadricLODActor_h
...
class SVTKRENDERINGLOD_EXPORT svtkQuadricLODActor : public svtkActor
{
public:
  /**
   * Creates a svtkQuadricLODActor.
   */
  static svtkQuadricLODActor* New();

  //@{
  /**
   * Standard class methods.
   */
  svtkTypeMacro(svtkQuadricLODActor, svtkActor);
   ...
   */
  svtkSetMacro(DeferLODConstruction, svtkTypeBool);
  svtkGetMacro(DeferLODConstruction, svtkTypeBool);
  svtkBooleanMacro(DeferLODConstruction, svtkTypeBool);
  //@}
   ...
   */
  svtkSetClampMacro(DataConfiguration, int, UNKNOWN, XYZVOLUME);
  svtkGetMacro(DataConfiguration, int);
   ...
   */
  svtkSetClampMacro(CollapseDimensionRatio, double, 0.0, 1.0);
  svtkGetMacro(CollapseDimensionRatio, double);
  //@}

  //@{
  /**
   * This class will create a svtkQuadricClustering algorithm automatically.
   ...
   */
  void SetLODFilter(svtkQuadricClustering* lodFilter);
  svtkGetObjectMacro(LODFilter, svtkQuadricClustering);
  //@}
   ...
   * By default, the prop type is a svtkActor.
   */
  svtkSetClampMacro(PropType, int, FOLLOWER, ACTOR);
  svtkGetMacro(PropType, int);
   ...
   * prop type is set to a svtkFollower.
   */
...
protected:
  ...
  svtkTypeBool DeferLODConstruction;

  // Keep track of building
  svtkTimeStamp BuildTime;

private:
  svtkQuadricLODActor(const svtkQuadricLODActor&) = delete;
  void operator=(const svtkQuadricLODActor&) = delete;
};
