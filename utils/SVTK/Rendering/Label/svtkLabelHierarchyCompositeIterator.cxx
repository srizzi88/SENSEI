/*=========================================================================
  ...
  Module:    svtkLabelHierarchyCompositeIterator.cxx
...
svtkStandardNewMacro(svtkLabelHierarchyCompositeIterator);

class svtkLabelHierarchyCompositeIterator::Internal
{
public:
  typedef std::vector<std::pair<svtkSmartPointer<svtkLabelHierarchyIterator>, int> > IteratorVector;
...
svtkLabelHierarchyCompositeIterator::svtkLabelHierarchyCompositeIterator()
{
...
void svtkLabelHierarchyCompositeIterator::AddIterator(svtkLabelHierarchyIterator* it, int count)
{
  this->Implementation->Iterators.push_back(
    std::make_pair(svtkSmartPointer<svtkLabelHierarchyIterator>(it), count));
}

void svtkLabelHierarchyCompositeIterator::ClearIterators()
{
  this->Implementation->Iterators.clear();
}

void svtkLabelHierarchyCompositeIterator::Begin(svtkIdTypeArray* list)
{
  this->Implementation->CurrentIterator = 0;
  this->Implementation->CurrentCount = 0;
  this->Implementation->InitialTraversal = 0;
...
void svtkLabelHierarchyCompositeIterator::Next()
{
  ...
  svtkLabelHierarchyIterator* iter =
...
bool svtkLabelHierarchyCompositeIterator::IsAtEnd()
{
  return this->Implementation->CurrentIterator >= this->Implementation->Iterators.size();
}

svtkIdType svtkLabelHierarchyCompositeIterator::GetLabelId()
{
...
svtkLabelHierarchy* svtkLabelHierarchyCompositeIterator::GetHierarchy()
{
  if (this->Implementation->CurrentIterator < this->Implementation->Iterators.size())
  {
    return this->Implementation->Iterators[this->Implementation->CurrentIterator]
...
void svtkLabelHierarchyCompositeIterator::GetNodeGeometry(double ctr[3], double& size)
{
  if (this->Implementation->CurrentIterator < this->Implementation->Iterators.size())
  {
...
void svtkLabelHierarchyCompositeIterator::PrintSelf(ostream& os, svtkIndent indent)
{
