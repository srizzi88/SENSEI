/*=========================================================================

  Program:   Visualization Toolkit
  Module:    svtkLabelHierarchyIterator.cxx

  Copyright (c) Ken Martin, Will Schroeder, Bill Lorensen
  All rights reserved.
  See Copyright.txt or http://www.kitware.com/Copyright.htm for details.

     This software is distributed WITHOUT ANY WARRANTY; without even
     the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR
     PURPOSE.  See the above copyright notice for more information.

=========================================================================*/
/*-------------------------------------------------------------------------
  Copyright 2008 Sandia Corporation.
  Under the terms of Contract DE-AC04-94AL85000 with Sandia Corporation,
  the U.S. Government retains certain rights in this software.
-------------------------------------------------------------------------*/

#include "svtkLabelHierarchyIterator.h"

#include "svtkCamera.h"
#include "svtkCellType.h"
#include "svtkCoordinate.h"
#include "svtkDataArray.h"
#include "svtkExtractSelectedFrustum.h"
#include "svtkIntArray.h"
#include "svtkLabelHierarchy.h"
#include "svtkLabelHierarchyPrivate.h"
#include "svtkMath.h"
#include "svtkObjectFactory.h"
#include "svtkPlanes.h"
#include "svtkPointData.h"
#include "svtkPoints.h"
#include "svtkPolyData.h"

svtkCxxSetObjectMacro(svtkLabelHierarchyIterator, Hierarchy, svtkLabelHierarchy);
svtkCxxSetObjectMacro(svtkLabelHierarchyIterator, TraversedBounds, svtkPolyData);

svtkLabelHierarchyIterator::svtkLabelHierarchyIterator()
{
  this->Hierarchy = nullptr;
  this->TraversedBounds = nullptr;
  this->BoundsFactor = 1.0;
  this->AllBounds = 0;
  this->AllBoundsRecorded = 0;
}

svtkLabelHierarchyIterator::~svtkLabelHierarchyIterator()
{
  if (this->Hierarchy)
  {
    this->Hierarchy->Delete();
  }
  if (this->TraversedBounds)
  {
    this->TraversedBounds->Delete();
  }
}

void svtkLabelHierarchyIterator::PrintSelf(ostream& os, svtkIndent indent)
{
  this->Superclass::PrintSelf(os, indent);
  os << indent << "Hierarchy: " << this->Hierarchy << "\n";
  os << indent << "BoundsFactor: " << this->BoundsFactor << "\n";
  os << indent << "TraversedBounds: " << this->TraversedBounds << "\n";
  os << indent << "AllBounds: " << this->AllBounds << "\n";
  os << indent << "AllBoundsRecorded: " << this->AllBoundsRecorded << "\n";
}

void svtkLabelHierarchyIterator::GetPoint(double x[3])
{
  this->GetHierarchy()->GetPoints()->GetPoint(this->GetLabelId(), x);
}

void svtkLabelHierarchyIterator::GetSize(double sz[2])
{
  if (!this->GetHierarchy())
  {
    sz[0] = sz[1] = 0.;
    return;
  }
  svtkDataArray* labelSizeArr = this->GetHierarchy()->GetSizes();
  if (!labelSizeArr)
  {
    sz[0] = sz[1] = 0.;
    return;
  }
  svtkIdType lid = this->GetLabelId();
  double* ls = labelSizeArr->GetTuple(lid);
  sz[0] = ls[0];
  sz[1] = ls[1];
}

int svtkLabelHierarchyIterator::GetType()
{
  if (!this->GetHierarchy())
  {
    return -1;
  }
  svtkDataArray* labelTypeArr = this->GetHierarchy()->GetPointData()->GetArray("Type");
  if (!labelTypeArr)
  {
    return -1;
  }
  svtkIntArray* labelTypeIArr = svtkArrayDownCast<svtkIntArray>(labelTypeArr);
  if (!labelTypeIArr)
  {
    return -1;
  }
  if (labelTypeIArr->GetNumberOfTuples() == 0)
  {
    return -1;
  }
  svtkIdType lid = this->GetLabelId();
  return labelTypeIArr->GetValue(lid);
}

svtkStdString svtkLabelHierarchyIterator::GetLabel()
{
  if (!this->GetHierarchy())
  {
    return svtkStdString();
  }
  svtkAbstractArray* labelArr = this->GetHierarchy()->GetLabels();
  if (!labelArr)
  {
    return "";
  }
  return labelArr->GetVariantValue(this->GetLabelId()).ToString();
}

svtkUnicodeString svtkLabelHierarchyIterator::GetUnicodeLabel()
{
  if (!this->GetHierarchy())
  {
    return svtkUnicodeString();
  }
  svtkAbstractArray* labelArr = this->GetHierarchy()->GetLabels();
  if (!labelArr)
  {
    return svtkUnicodeString();
  }
  return labelArr->GetVariantValue(this->GetLabelId()).ToUnicodeString();
}

double svtkLabelHierarchyIterator::GetOrientation()
{
  if (!this->GetHierarchy())
  {
    return 0.0;
  }
  svtkDataArray* arr = this->GetHierarchy()->GetOrientations();
  if (!arr)
  {
    return 0.0;
  }
  return arr->GetTuple1(this->GetLabelId());
}

void svtkLabelHierarchyIterator::GetBoundedSize(double sz[2])
{
  if (!this->GetHierarchy())
  {
    sz[0] = sz[1] = 0.;
    return;
  }
  svtkDataArray* boundedSizeArr = this->GetHierarchy()->GetBoundedSizes();
  if (!boundedSizeArr)
  {
    sz[0] = sz[1] = 0.;
    return;
  }
  svtkIdType lid = this->GetLabelId();
  double* ls = boundedSizeArr->GetTuple(lid);
  sz[0] = ls[0];
  sz[1] = ls[1];
}

void svtkLabelHierarchyIterator::BoxNode()
{
  if (!this->TraversedBounds || this->IsAtEnd())
  {
    return;
  }

  if (this->AllBounds)
  {
    if (!this->AllBoundsRecorded)
    {
      this->AllBoundsRecorded = 1;
      this->BoxAllNodes(this->TraversedBounds);
    }
    return;
  }

  double ctr[3];
  double sz;
  this->GetNodeGeometry(ctr, sz);
  double tf = this->BoundsFactor;

  if (this->Hierarchy->GetImplementation()->Hierarchy3)
  {
    this->BoxNodeInternal3(ctr, tf * sz);
  }
  else if (this->Hierarchy->GetImplementation()->Hierarchy2)
  {
    this->BoxNodeInternal2(ctr, tf * sz);
  }
}

void svtkLabelHierarchyIterator::BoxAllNodes(svtkPolyData* boxes)
{
  if (!boxes)
  {
    return;
  }

  svtkPolyData* tmp = this->TraversedBounds;
  this->TraversedBounds = boxes;
  double tf = this->BoundsFactor;

  if (this->Hierarchy->GetImplementation()->Hierarchy3)
  {
    svtkLabelHierarchy::Implementation::HierarchyIterator3 iter;
    for (iter = this->Hierarchy->GetImplementation()->Hierarchy3->begin(true);
         iter != this->Hierarchy->GetImplementation()->Hierarchy3->end(true); ++iter)
    {
      this->BoxNodeInternal3(iter->value().GetCenter(), iter->value().GetSize() / 2. * tf);
    }
  }
  else if (this->Hierarchy->GetImplementation()->Hierarchy2)
  {
    svtkLabelHierarchy::Implementation::HierarchyIterator2 iter;
    double ctr[3];
    double zvalfoo = this->Hierarchy->GetImplementation()->Z2;
    for (iter = this->Hierarchy->GetImplementation()->Hierarchy2->begin(true);
         iter != this->Hierarchy->GetImplementation()->Hierarchy2->end(true); ++iter)
    {
      ctr[0] = iter->value().GetCenter()[0];
      ctr[1] = iter->value().GetCenter()[1];
      ctr[2] = zvalfoo;
      this->BoxNodeInternal2(ctr, iter->value().GetSize() / 2. * tf);
    }
  }

  this->TraversedBounds = tmp;
}

static const int svtkLabelHierarchyIteratorEdgeIds[12][2] = {
  { 0, 1 },
  { 1, 2 },
  { 2, 3 },
  { 3, 0 },

  { 4, 5 },
  { 5, 6 },
  { 6, 7 },
  { 7, 4 },

  { 0, 4 },
  { 1, 5 },
  { 2, 6 },
  { 3, 7 },
};

void svtkLabelHierarchyIterator::BoxNodeInternal3(const double* ctr, double sz)
{
  svtkIdType conn[8];
  svtkPoints* pts = this->TraversedBounds->GetPoints();
  conn[0] = pts->InsertNextPoint(ctr[0] - sz, ctr[1] - sz, ctr[2] - sz);
  conn[1] = pts->InsertNextPoint(ctr[0] + sz, ctr[1] - sz, ctr[2] - sz);
  conn[2] = pts->InsertNextPoint(ctr[0] + sz, ctr[1] + sz, ctr[2] - sz);
  conn[3] = pts->InsertNextPoint(ctr[0] - sz, ctr[1] + sz, ctr[2] - sz);
  conn[4] = pts->InsertNextPoint(ctr[0] - sz, ctr[1] - sz, ctr[2] + sz);
  conn[5] = pts->InsertNextPoint(ctr[0] + sz, ctr[1] - sz, ctr[2] + sz);
  conn[6] = pts->InsertNextPoint(ctr[0] + sz, ctr[1] + sz, ctr[2] + sz);
  conn[7] = pts->InsertNextPoint(ctr[0] - sz, ctr[1] + sz, ctr[2] + sz);
  svtkIdType econn[2];
  for (int i = 0; i < 12; ++i)
  {
    econn[0] = conn[svtkLabelHierarchyIteratorEdgeIds[i][0]];
    econn[1] = conn[svtkLabelHierarchyIteratorEdgeIds[i][1]];
    this->TraversedBounds->InsertNextCell(SVTK_LINE, 2, econn);
  }
}

void svtkLabelHierarchyIterator::BoxNodeInternal2(const double* ctr, double sz)
{
  svtkIdType conn[4];
  svtkPoints* pts = this->TraversedBounds->GetPoints();
  conn[0] = pts->InsertNextPoint(ctr[0] - sz, ctr[1] - sz, ctr[2]);
  conn[1] = pts->InsertNextPoint(ctr[0] + sz, ctr[1] - sz, ctr[2]);
  conn[2] = pts->InsertNextPoint(ctr[0] + sz, ctr[1] + sz, ctr[2]);
  conn[3] = pts->InsertNextPoint(ctr[0] - sz, ctr[1] + sz, ctr[2]);
  svtkIdType econn[2];
  for (int i = 0; i < 4; ++i)
  {
    econn[0] = conn[svtkLabelHierarchyIteratorEdgeIds[i][0]];
    econn[1] = conn[svtkLabelHierarchyIteratorEdgeIds[i][1]];
    this->TraversedBounds->InsertNextCell(SVTK_LINE, 2, econn);
  }
}
