/*=========================================================================
  ...
  Module:    svtkLabelPlacementMapper.cxx
...
#include "svtkLabelHierarchyCompositeIterator.h"
...
public:
  ...
  void Render(
    svtkRenderer* ren, int shape, int style, double margin, double color[3], double opacity) const
  {
    if (shape == svtkLabelPlacementMapper::NONE)
    {
      return;
    }
    ...
    switch (shape)
    {
      case svtkLabelPlacementMapper::ROUNDED_RECT:
      {
        double roundedFactor = svtkMath::Pi() / 4;
        double rx[2];
        double ry[2];
      ...
      case svtkLabelPlacementMapper::RECT:
      default:
      {
    ...
    poly->SetPoints(pts);
    if (style == svtkLabelPlacementMapper::OUTLINE)
    {
      poly->SetLines(cells);
    }
    else
...
class svtkLabelPlacementMapper::Internal
{
public:
...
svtkStandardNewMacro(svtkLabelPlacementMapper);
svtkCxxSetObjectMacro(svtkLabelPlacementMapper, AnchorTransform, svtkCoordinate);
svtkCxxSetObjectMacro(svtkLabelPlacementMapper, RenderStrategy, svtkLabelRenderStrategy);

//----------------------------------------------------------------------------
svtkLabelPlacementMapper::svtkLabelPlacementMapper()
{
  ...
  svtkSmartPointer<svtkFreeTypeLabelRenderStrategy> s =
...
//----------------------------------------------------------------------------
int svtkLabelPlacementMapper::FillInputPortInformation(int svtkNotUsed(port), svtkInformation* info)
{
...
//----------------------------------------------------------------------------
void svtkLabelPlacementMapper::RenderOverlay(svtkViewport* viewport, svtkActor2D* svtkNotUsed(actor))
{
  ...
  svtkLabelHierarchy::GetAnchorFrustumPlanes(frustumPlanes, ren, this->AnchorTransform);
  ...
  svtkSmartPointer<svtkLabelHierarchyCompositeIterator> inIter =
    svtkSmartPointer<svtkLabelHierarchyCompositeIterator>::New();

  svtkSmartPointer<svtkPolyData> boundsPoly = svtkSmartPointer<svtkPolyData>::New();
  ...
  {
    svtkLabelHierarchy* inData = svtkLabelHierarchy::SafeDownCast(this->GetInputDataObject(0, i));
    svtkLabelHierarchyIterator* it = inData->NewIterator(
  ...
  svtkSmartPointer<svtkTextProperty> tpropCopy = svtkSmartPointer<svtkTextProperty>::New();
  ...
  for (; !inIter->IsAtEnd(); inIter->Next())
  {
    ...
    svtkTextProperty* tprop = inIter->GetHierarchy()->GetTextProperty();
    ...
    if (this->RenderStrategy->SupportsBoundedSize() && inIter->GetHierarchy()->GetBoundedSizes())
    {
      ...
      int width = static_cast<int>(sqrt(svtkMath::Distance2BetweenPoints(p, pWidth)));
      if (width < 20)
      {
        continue;
      }
      ...
      int height = static_cast<int>(sqrt(svtkMath::Distance2BetweenPoints(p, pHeight)));
    ...
    double orientRad = svtkMath::RadiansFromDegrees(orient);
  ...
  if (this->OutputTraversedBounds)
  {
    // For some reason I cannot use svtkPolyDataMapper, I need to use
    // svtkPolyDataMapper2D. This causes lines behind the camera to be sometimes
    // transformed on-screen. Since this is for debugging, I'm going to punt
    // on this one.
    svtkSmartPointer<svtkTransformCoordinateSystems> trans =
      svtkSmartPointer<svtkTransformCoordinateSystems>::New();
    svtkSmartPointer<svtkPolyDataMapper2D> boundsMapper = svtkSmartPointer<svtkPolyDataMapper2D>::New();
    svtkSmartPointer<svtkActor2D> boundsActor = svtkSmartPointer<svtkActor2D>::New();
  ...
  svtkDebugMacro("------");
  svtkDebugMacro("Placed: " << placed);
  svtkDebugMacro("Labels Occluded: " << occluded);
...
//----------------------------------------------------------------------------
void svtkLabelPlacementMapper::ReleaseGraphicsResources(svtkWindow* win)
{
  this->RenderStrategy->ReleaseGraphicsResources(win);
}

//----------------------------------------------------------------------------
void svtkLabelPlacementMapper::PrintSelf(ostream& os, svtkIndent indent)
{
