...
svtkStandardNewMacro(svtkLabelPlacer);
svtkCxxSetObjectMacro(svtkLabelPlacer, AnchorTransform, svtkCoordinate);

class svtkLabelPlacer::Internal
{
public:
  ...
  float TileSize[2];
  int NumTiles[2];
  svtkSmartPointer<svtkIdTypeArray> NewLabelsPlaced;
  svtkSmartPointer<svtkIdTypeArray> LastLabelsPlaced;
  static int DumpPlaced;

  Internal(float viewport[4], float tilesize[2])
  {
    this->NewLabelsPlaced = svtkSmartPointer<svtkIdTypeArray>::New();
    this->LastLabelsPlaced = svtkSmartPointer<svtkIdTypeArray>::New();
  ...
  void Reset(float viewport[4], float tileSize[2])
  {
    ...
    svtkSmartPointer<svtkIdTypeArray> tmp = this->LastLabelsPlaced;
...
int svtkLabelPlacer::Internal::DumpPlaced = 0;

svtkLabelPlacer::svtkLabelPlacer()
{
  this->Renderer = nullptr;
  this->Gravity = CenterCenter;
  this->AnchorTransform = svtkCoordinate::New();
  ...
  // this->IteratorType = svtkLabelHierarchy::DEPTH_FIRST;
  // this->IteratorType = svtkLabelHierarchy::FULL_SORT;
  this->IteratorType = svtkLabelHierarchy::QUEUE;
  this->VisiblePoints = svtkSelectVisiblePoints::New();
  ...
  this->OutputCoordinateSystem = svtkLabelPlacer::WORLD;
...
void svtkLabelPlacer::SetRenderer(svtkRenderer* ren)
{
...
void svtkLabelPlacer::PrintSelf(ostream& os, svtkIndent indent)
{
 ...
 * yet.
 */
void svtkLabelPlacer::SetGravity(int gravity)
{
  ...
  {
    svtkWarningMacro("Ignoring gravity " << gravity << " with no horizontal bit set");
    return;
  }

  if (!(gravity & VerticalBitMask))
  {
    svtkWarningMacro("Ignoring gravity " << gravity << " with no vertical bit set");
    return;
  }
...
svtkMTimeType svtkLabelPlacer::GetMTime()
{
...
int svtkLabelPlacer::FillInputPortInformation(int svtkNotUsed(port), svtkInformation* info)
{
  info->Set(svtkAlgorithm::INPUT_REQUIRED_DATA_TYPE(), "svtkLabelHierarchy");
  return 1;
}

int svtkLabelPlacer::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** inputVector, svtkInformationVector* outputVector)
{
  if (!this->Renderer)
  {
    svtkErrorMacro("No renderer -- can't determine screen space size.");
    return 0;
  }
  ...
  svtkInformation* outInfo2 = outputVector->GetInformationObject(2);
  svtkInformation* outInfo3 = outputVector->GetInformationObject(3);

  svtkLabelHierarchy* inData =
    svtkLabelHierarchy::SafeDownCast(inInfo->Get(svtkDataObject::DATA_OBJECT()));
  svtkPolyData* ouData0 = svtkPolyData::SafeDownCast(outInfo0->Get(svtkDataObject::DATA_OBJECT()));
  svtkPolyData* ouData1 = svtkPolyData::SafeDownCast(outInfo1->Get(svtkDataObject::DATA_OBJECT()));
  svtkPolyData* ouData2 = svtkPolyData::SafeDownCast(outInfo2->Get(svtkDataObject::DATA_OBJECT()));
  svtkPolyData* ouData3 = svtkPolyData::SafeDownCast(outInfo3->Get(svtkDataObject::DATA_OBJECT()));

  svtkStringArray* nameArr0 = svtkStringArray::New();
  svtkUnicodeStringArray* nameUArr0 = svtkUnicodeStringArray::New();
  ...
  svtkDoubleArray* opArr0 = svtkDoubleArray::New();
  opArr0->SetName("Opacity");
  ouData0->GetPointData()->AddArray(opArr0);
  opArr0->Delete();

  svtkIntArray* iconIndexArr1 = svtkIntArray::New();
  iconIndexArr1->SetName("IconIndex");
  ouData1->GetPointData()->AddArray(iconIndexArr1);
  iconIndexArr1->Delete();

  svtkIntArray* idArr0 = svtkIntArray::New();
  idArr0->SetName("ID");
  ouData0->GetPointData()->AddArray(idArr0);
  idArr0->Delete();

  svtkStringArray* nameArr = svtkArrayDownCast<svtkStringArray>(inData->GetLabels());
  svtkUnicodeStringArray* nameUArr = svtkArrayDownCast<svtkUnicodeStringArray>(inData->GetLabels());
  svtkIntArray* iconIndexArr = svtkArrayDownCast<svtkIntArray>(inData->GetIconIndices());
  ...
  svtkPoints* ipts = inData->GetPoints();
  if (!ipts)
  {
    return 1;
  }
  svtkDataArray* isz = inData->GetPointData()->GetArray("LabelSize");
  if (!isz) //|| isz->GetNumberOfComponents() > 2 )
  {
    svtkWarningMacro("Missing or improper label size point array -- output will be empty.");
    return 1;
  }
  ...
  svtkPoints* opts0 = ouData0->GetPoints();
  if (!opts0)
  {
    opts0 = svtkPoints::New();
    ouData0->SetPoints(opts0);
    opts0->FastDelete();
  }
  ouData0->AllocateExact(1024, 1024);

  svtkPoints* opts1 = ouData1->GetPoints();
  if (!opts1)
  {
    opts1 = svtkPoints::New();
    ouData1->SetPoints(opts1);
    opts1->FastDelete();
  }
  ouData1->AllocateExact(1024, 1024);

  svtkPoints* opts2 = ouData2->GetPoints();
  if (!opts2)
  {
    opts2 = svtkPoints::New();
    ouData2->SetPoints(opts2);
    opts2->FastDelete();
  }
  ouData2->AllocateExact(1024, 1024);

  svtkPoints* opts3 = ouData3->GetPoints();
  svtkCellArray* ocells3 = ouData3->GetLines();
  if (!opts3)
  {
    opts3 = svtkPoints::New();
    ouData3->SetPoints(opts3);
    opts3->FastDelete();
  }
  if (!ocells3)
  {
    ocells3 = svtkCellArray::New();
  ...
  svtkLabelHierarchy::GetAnchorFrustumPlanes(frustumPlanes, this->Renderer, this->AnchorTransform);
  ...
  svtkLabelHierarchyIterator* inIter = inData->NewIterator(
  ...
  timer->StopTimer();
  svtkDebugMacro("Iterator initialization time: " << timer->GetElapsedTime());
  timer->StartTimer();
  for (; !inIter->IsAtEnd(); inIter->Next())
  {
    // Ignore labels that don't have text or an icon.
    svtkIdType labelType = inIter->GetType();
    ...
    {
      svtkDebugMacro("Arf. Bad label type " << labelType);
      continue;
    }

    inIter->GetPoint(x);

    if (this->AnchorTransform->GetCoordinateSystem() == SVTK_WORLD)
    {
      // Cull points behind the camera. Cannot rely on hither-yon planes because the camera
      // position gets changed during svtkInteractorStyle::Dolly() and RequestData() called from
    ...
    if (this->Debug)
    {
      svtkDebugMacro("Try: " << inIter->GetLabelId() << " (" << ll[0] << ", " << ll[1] << "  "
                            << ur[0] << "," << ur[1] << ")");
      if (labelType == 0)
      {
        if (this->UseUnicodeStrings)
        {
          svtkDebugMacro("Area: " << renderedLabelArea << "  /  " << allowableLabelArea << " \""
    ...
    {
      svtkDebugMacro("Early exit due to large iterated label area");
      break;
    }
#endif
    ...
    if (this->Buckets->PlaceLabel(opacity, ll[0], ur[0], ll[1], ur[1]))
    {
      ...
      {
        svtkDebugMacro("Early exit due to large rendered label area");
        break;
      }
#endif // 0
      svtkIdType conn[4];
      ...
      { // label is text
        if (this->Buckets->DumpPlaced)
        {
          if (this->UseUnicodeStrings)
          {
            svtkDebugMacro(<< ll[0] << " -- " << ur[0] << ", " << ll[1] << " -- " << ur[1] << ": "
                          << nameUArr->GetValue(inIter->GetLabelId()).utf8_str());
          }
          else
          {
        ...
        ouData0->InsertNextCell(SVTK_VERTEX, 1, conn);
        if (this->UseUnicodeStrings)
        {
      ...
      { // label is an icon
        if (this->Buckets->DumpPlaced)
        {
          svtkDebugMacro(<< ll[0] << " -- " << ur[0] << ", " << ll[1] << " -- " << ur[1] << ": Icon "
        ...
        svtkIdType cid = ouData1->InsertNextCell(SVTK_VERTEX, 1, conn);
        (void)cid;
        svtkDebugMacro("     Point: " << conn[0] << " (" << x[0] << "," << x[1] << "," << x[2]
                                     << ") Vertex: " << cid);
      ...
      svtkDebugMacro("Placed: " << inIter->GetLabelId() << " (" << ll[0] << ", " << ll[1] << "  "
                               << ur[0] << "," << ur[1] << ") " << labelType);
      placed++;
    }
  }
  ...
  timer->StopTimer();
  svtkDebugMacro("Iteration time: " << timer->GetElapsedTime());
