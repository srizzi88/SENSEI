/*=========================================================================
  ...
  Module:    svtkLabeledTreeMapDataMapper.cxx
...
svtkStandardNewMacro(svtkLabeledTreeMapDataMapper);

svtkLabeledTreeMapDataMapper::svtkLabeledTreeMapDataMapper()
  ...
  , DynamicLevel(0)
{
  ...
  this->VCoord = svtkCoordinate::New();
  this->VertexList = svtkIdList::New();
  this->VertexList->SetNumberOfIds(this->NumberOfLabelsAllocated);
  this->TextPoints = svtkPoints::New();
  this->TextPoints->Allocate(this->NumberOfLabelsAllocated);
  this->VerticalLabelProperty = svtkTextProperty::New();
...
//----------------------------------------------------------------------------
void svtkLabeledTreeMapDataMapper::SetRectanglesArrayName(const char* name)
{
...
void svtkLabeledTreeMapDataMapper::ReleaseGraphicsResources(svtkWindow* win)
{
...
svtkTree* svtkLabeledTreeMapDataMapper::GetInputTree()
{
  return svtkTree::SafeDownCast(this->GetExecutive()->GetInputData(0, 0));
}

void svtkLabeledTreeMapDataMapper::UpdateFontSizes()
{
  char test[2];
  test[1] = '\0';
  int tSize[2], i;
  ...
  {
    this->TextMappers[0] = svtkTextMapper::New();
...
void svtkLabeledTreeMapDataMapper::PrintSelf(ostream& os, svtkIndent indent)
{
...
int svtkLabeledTreeMapDataMapper::UpdateWindowInfo(svtkViewport* viewport)
{
...
void svtkLabeledTreeMapDataMapper::GetVertexLabel(svtkIdType vertex, svtkDataArray* numericData,
  svtkStringArray* stringData, int activeComp, int numComp, char* string, size_t stringSize)
{
  char format[1024];
  double val;
  int j;
  if (numericData)
  {
    if (numComp == 1)
    {
      if (numericData->GetDataType() == SVTK_CHAR)
      {
        if (strcmp(this->LabelFormat, "%c") != 0)
        {
          svtkErrorMacro(<< "Label format must be %c to use with char");
          string[0] = '\0';
          return;
        }
  ...
  else if (stringData) // rendering string data
  {
    ...
    {
      svtkErrorMacro(<< "Label format must be %s to use with strings");
      string[0] = '\0';
      return;
    }
...
//----------------------------------------------------------------------------
void svtkLabeledTreeMapDataMapper::RenderOverlay(svtkViewport* viewport, svtkActor2D* actor)
{
  int i;
  double x[3];
...
//----------------------------------------------------------------------------
void svtkLabeledTreeMapDataMapper::RenderOpaqueGeometry(svtkViewport* viewport, svtkActor2D* actor)
{
  int i, numComp = 0, pointIdLabels, activeComp = 0;
  int numVertices;
  double x[3];
  svtkAbstractArray* abstractData;
  svtkDataArray *numericData, *tempData;
  svtkStringArray* stringData;
  svtkTree* input = this->GetInputTree();
  if (!input)
  {
    svtkErrorMacro(<< "Need input tree to render labels (2)");
    return;
  }
  ...
  svtkDataSetAttributes* pd = input->GetVertexData();
  // Get the treeMap info
  tempData = this->GetInputArrayToProcess(0, input);
  if (!tempData)
  {
    svtkErrorMacro(<< "Input Tree does not have box information.");
    return;
  }
  svtkFloatArray* boxInfo = svtkArrayDownCast<svtkFloatArray>(tempData);
    ...
    tprop->GetMTime() > this->BuildTime)
  {
    ...
    {
      svtkErrorMacro(<< "Need input data to render labels (3)");
      return;
    }
    ...
    if (numVertices > this->NumberOfLabelsAllocated)
    {
      ...
      this->TextMappers = new svtkTextMapper*[numVertices];
...
void svtkLabeledTreeMapDataMapper::LabelTree(svtkTree* tree, svtkFloatArray* boxInfo,
  svtkDataArray* numericData, svtkStringArray* stringData, int activeComp, int numComps)
{

  float blimits[4], blimitsDC[4], textPosWC[3];
  char string[1024];
  int results;
  svtkTextProperty* tprop = nullptr;
  svtkIdType vertex, level, root = tree->GetRoot();
  if (root < 0)
  {
    svtkErrorMacro(<< "Input Tree does not have a root.");
    return;
  }
  ...
  while (dfs->HasNext())
  {
    ...
    if (!results)
    {
      ...
      {
        this->TextMappers[this->NumberOfLabels] = svtkTextMapper::New();
      }
...
int svtkLabeledTreeMapDataMapper::GetStringSize(char* string, int level)
{
...
int svtkLabeledTreeMapDataMapper::ConvertToDC(float* binfo, float* newBinfo)
{
...
void svtkLabeledTreeMapDataMapper::SetFontSizeRange(int maxSize, int minSize, int delta)
{
  if (maxSize < minSize)
  {
    svtkErrorMacro(<< "maxSize is smaller than minSize");
    return;
  }
  if (delta < 1)
  {
    svtkErrorMacro(<< "delta must be positive");
    return;
  }
  ...
  if (this->MaxFontLevel != nLevels)
  {
    ...
    this->HLabelProperties = new svtkTextProperty*[this->MaxFontLevel + 1];
    for (i = 0; i <= this->MaxFontLevel; i++)
    {
      this->FontWidths[i] = new int[95];
      this->HLabelProperties[i] = svtkTextProperty::New();
...
void svtkLabeledTreeMapDataMapper::GetFontSizeRange(int range[3])
{
...
int svtkLabeledTreeMapDataMapper::AnalyseLabel(
  char* string, int level, float* blimitsDC, float* textPosWC, svtkTextProperty** tprop)
{
  ...
  // Horizontal label.
  // (Vertical labels don't work due to issues with svtkTextActor.)
  oDir = 0;
...
int svtkLabeledTreeMapDataMapper::ApplyMasks(int level, float flimits[4], float blimits[4])
{
...
void svtkLabeledTreeMapDataMapper::SetLevelRange(int start, int end)
{
  if (((end != -1) && (start > end)) || (start < 0))
  {
    svtkErrorMacro(<< "Invalid level range specified.");
    return;
  }
...
void svtkLabeledTreeMapDataMapper::GetLevelRange(int range[2])
{
