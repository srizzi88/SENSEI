...
class svtkScalarKeyboardCommand : public svtkCommand
{
public:
  static svtkScalarKeyboardCommand* New() { return new svtkScalarKeyboardCommand; }
  virtual void Execute(
    svtkObject* caller, unsigned long svtkNotUsed(eventId), void* svtkNotUsed(callData))
  {
    svtkRenderWindowInteractor* ri = svtkRenderWindowInteractor::SafeDownCast(caller);
    if (this->Lookup && this->RenderWindow && ri)
    {
      ...
      else if (ri->GetKeySym()[0] == 'k')
      {
        this->ScalarBar->SetTextPosition(
          this->ScalarBar->GetTextPosition() == svtkScalarBarActor::PrecedeScalarBar
            ? svtkScalarBarActor::SucceedScalarBar
            : svtkScalarBarActor::PrecedeScalarBar);
        cout << "Text position "
             << (this->ScalarBar->GetTextPosition() == svtkScalarBarActor::PrecedeScalarBar
                    ? "PRECEDE"
                    : "SUCCEED")
      ...
      {
        svtkIdType idx = this->Lookup->GetAnnotatedValueIndex(4.00);
        if (idx >= 0)
        {
          svtkStdString prev = this->Lookup->GetAnnotation(idx);
  ...
  svtkLookupTable* Lookup;
  svtkRenderWindow* RenderWindow;
  svtkScalarBarActor* ScalarBar;
};
...
int TestIndexedLookupScalarBar(int argc, char* argv[])
{
  ...
  svtkSmartPointer<svtkBandedPolyDataContourFilter> bands =
  ...
  svtkSmartPointer<svtkScalarBarWidget> scalarWidget = svtkSmartPointer<svtkScalarBarWidget>::New();
  ...
  svtkSmartPointer<svtkLookupTable> lutBC = svtkSmartPointer<svtkLookupTable>::New();
  svtkSmartPointer<svtkScalarBarWidget> scalarWidgetB = svtkSmartPointer<svtkScalarBarWidget>::New();
  ...
  svtkScalarBarRepresentation* srep =
    svtkScalarBarRepresentation::SafeDownCast(scalarWidgetB->GetRepresentation());
  ...
  svtkSmartPointer<svtkScalarBarWidget> scalarWidgetC = svtkSmartPointer<svtkScalarBarWidget>::New();
  ...
  srep = svtkScalarBarRepresentation::SafeDownCast(scalarWidgetC->GetRepresentation());
  ...
  svtkSmartPointer<svtkScalarKeyboardCommand> cb = svtkSmartPointer<svtkScalarKeyboardCommand>::New();
  svtkSmartPointer<svtkColorSeries> pal = svtkSmartPointer<svtkColorSeries>::New();
  pal->SetColorSchemeByName("Brewer Sequential Blue-Green (5)");
  cb->Lookup = svtkLookupTable::SafeDownCast(scalarWidget->GetScalarBarActor()->GetLookupTable());
  ...
  iren->AddObserver(svtkCommand::KeyPressEvent, cb);
  recorder->Play();
  // recorder->EnabledOn();
...
#if 0 // Use this to get positions for statically-placed scalar bars that test an even number of
      // annotations.
  srep = svtkScalarBarRepresentation::SafeDownCast( scalarWidget->GetRepresentation() );
  double* foo;
