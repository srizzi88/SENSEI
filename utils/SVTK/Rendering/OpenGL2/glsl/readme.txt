SVTK uses shaders to perform its OpenGL rendering.  SVTK supports many different
...
are just strings of code. For svtkOpenGLPolyDataMapper we make use of heavy
...
//SVTK::SomeConcept::SomeAction

For example

//SVTK::Normal::Dec  - declaration any uniforms/varying needed for normals
//SVTK::Normal::Impl - Implementation of shader code for handling normals

All shaders should start with the following line

//SVTK::System::Dec

Which svtkOpenGLShaderCache will replace with a #version and some other values to
match the system and OpenGL Context it has. The other line you need (only in
your fragment shader) is

//SVTK::Output::Dec

which SVTK uses to map shader outputs to the framebuffer.
...
vertex shader.  If a geometry shader is present SVTK will rename the fragment
