...
Module:    svtkCocoaRenderWindow.h
 ...
 *
 *
 * svtkCocoaRenderWindow is a concrete implementation of the abstract
 * class svtkOpenGLRenderWindow. It is only available on Mac OS X.
 * To use this class, build SVTK with SVTK_USE_COCOA turned ON (this is
 * the default).
 ...
 * svtkCocoaRenderWindow uses Objective-C++, and the OpenGL and
 * Cocoa APIs. This class's default behaviour is to create an NSWindow and
 * a svtkCocoaGLView which are used together to draw all SVTK content.
 * If you already have an NSWindow and svtkCocoaGLView and you want this
 ...
 *
 * @sa
 * svtkOpenGLRenderWindow svtkCocoaGLView
 *
 * @warning
...
#ifndef svtkCocoaRenderWindow_h
#define svtkCocoaRenderWindow_h

#include "svtkOpenGLRenderWindow.h"
#include "svtkRenderingOpenGL2Module.h" // For export macro
#include <stack>                       // for ivar

class SVTKRENDERINGOPENGL2_EXPORT svtkCocoaRenderWindow : public svtkOpenGLRenderWindow
{
public:
  static svtkCocoaRenderWindow* New();
  svtkTypeMacro(svtkCocoaRenderWindow, svtkOpenGLRenderWindow);
   ...
   * implemented for the svtkCocoaRenderWindow.  It can only be called
  ...
  /**
   * Remap the window.  This is not implemented for the svtkCocoaRenderWindow.
   */
  void WindowRemap() override;

  /**
   * Set the preferred window size to full screen.  This is not implemented
   * for the svtkCocoaRenderWindow.
   */
  ...
  {
    svtkDebugMacro("SetNextWindowInfo not implemented (WindowRemap not implemented).");
  }
  void* GetGenericDrawable() override
  {
    svtkDebugMacro("Method not implemented.");
    return nullptr;
  }
  ...
  {
    svtkDebugMacro("SetNextWindowId not implemented (WindowRemap not implemented).");
  }
   ...
   * svtkCocoaGLView (NSView subclass) which are used together to draw
   * all svtk stuff into. If you already have an NSWindow and NSView and
   ...
   * This class' default behaviour, that is, if you never call
   * SetWindowId()/SetRootWindow() is to create an NSWindow and a
   * svtkCocoaGLView (NSView subclass) which are used together to draw all
   * svtk stuff into. If you already have an NSWindow and NSView and you
  ...
  /**
   * Set the NSView* for the svtkRenderWindow to be parented within.  The
   * Position and Size of the RenderWindow will set the rectangle of the
   * NSView that the svtkRenderWindow will create within this parent.
   * If you set the WindowId, then this ParentId will be ignored.
   */
  void SetParentId(void* nsview) override;

  /**
   * Get the parent NSView* for this svtkRenderWindow.  This method will
   ...
   * with the NSView. You might want this is you are rendering svtk content into a
...
protected:
  svtkCocoaRenderWindow();
  ~svtkCocoaRenderWindow() override;
  ...
  svtkTypeBool OnScreenInitialized;
  ...
  void SetCocoaServer(void* server); // Really a svtkCocoaServer*
  void* GetCocoaServer();

private:
  svtkCocoaRenderWindow(const svtkCocoaRenderWindow&) = delete;
  void operator=(const svtkCocoaRenderWindow&) = delete;
...
private:
  ...
  svtkTypeBool WindowCreated;
  svtkTypeBool ViewCreated;
