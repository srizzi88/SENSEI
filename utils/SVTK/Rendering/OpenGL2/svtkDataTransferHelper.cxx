/*=========================================================================
  ...
  Module:    svtkDataTransferHelper.cxx
...
static void svtkGetDimensions(int extents[6], int dims[3])
{
...
svtkStandardNewMacro(svtkDataTransferHelper);
svtkCxxSetObjectMacro(svtkDataTransferHelper, Texture, svtkTextureObject);
svtkCxxSetObjectMacro(svtkDataTransferHelper, Array, svtkDataArray);
//----------------------------------------------------------------------------
svtkDataTransferHelper::svtkDataTransferHelper()
{
...
bool svtkDataTransferHelper::GetExtentIsValid(int* extent)
{
...
bool svtkDataTransferHelper::GetCPUExtentIsValid()
{
...
bool svtkDataTransferHelper::GetGPUExtentIsValid()
{
...
bool svtkDataTransferHelper::GetTextureExtentIsValid()
{
...
bool svtkDataTransferHelper::IsSupported(svtkRenderWindow* renWin)
{
  svtkOpenGLRenderWindow* rw = svtkOpenGLRenderWindow::SafeDownCast(renWin);
  return (svtkPixelBufferObject::IsSupported(renWin) && svtkTextureObject::IsSupported(rw));
}

//----------------------------------------------------------------------------
svtkRenderWindow* svtkDataTransferHelper::GetContext()
{
  return this->Context;
}

//----------------------------------------------------------------------------
void svtkDataTransferHelper::SetContext(svtkRenderWindow* renWin)
{
  ...
  svtkOpenGLRenderWindow* openGLRenWin = svtkOpenGLRenderWindow::SafeDownCast(renWin);
...
//----------------------------------------------------------------------------
// Description:
// Upload GPUExtent from CPU svtkDataArray to GPU texture.
...
// componentList!=0)
bool svtkDataTransferHelper::Upload(int components, int* componentList)
{
  ...
  if (!this->Context)
  {
    svtkErrorMacro("Cannot upload to GPU without context.");
    return false;
  }
  ...
  int gpudims[3];
  int texturedims[3];
  svtkGetDimensions(this->CPUExtent, cpudims);
  svtkGetDimensions(this->GPUExtent, gpudims);
  svtkDebugMacro(<< "CPUDims: " << cpudims[0] << ", " << cpudims[1] << ", " << cpudims[2] << endl);
  svtkDebugMacro(<< "GPUDims: " << gpudims[0] << ", " << gpudims[1] << ", " << gpudims[2] << endl);
  ...
  {
    svtkGetDimensions(this->TextureExtent, texturedims);
  }

  int numComps = this->Array->GetNumberOfComponents();

  // * Upload data to svtkPixelBufferObject.
  svtkSmartPointer<svtkPixelBufferObject> pbo = this->GetPBO();

  // We need to get the ContinuousIncrements as computed by the svtkImageData.
  // For that we create a dummy image data object,
  svtkSmartPointer<svtkImageData> tempImg = svtkSmartPointer<svtkImageData>::New();
  tempImg->SetDimensions(1, 1, 1);
  // scalars are needed for ComputeIncrements().
  tempImg->AllocateScalars(SVTK_DOUBLE, 1);
  ...
  svtkIdType ptId = svtkStructuredData::ComputePointId(cpudims, pt);
  if (!pbo->Upload3D(this->Array->GetDataType(), this->Array->GetVoidPointer(ptId * numComps),
        ...
        componentList))
  {
    svtkErrorMacro("Failed to load data to pixel buffer.");
    return false;
  }
  ...
  if (!this->Texture)
  {
    ...
    tex->SetContext(svtkOpenGLRenderWindow::SafeDownCast(this->Context));
    this->SetTexture(tex);
    tex->Delete();
  }

  int tempdims[3] = { 0, 0, 0 };
  int dataDescription = svtkStructuredData::SetDimensions(texturedims, tempdims);
  int dimension = svtkStructuredData::GetDataDimension(dataDescription);
  ...
  if (!uploaded)
  {
    svtkErrorMacro("Failed to upload data to texture.");
    return false;
  }
  return true;
}
...
// Description:
// new comment:
// Download GPUExtent from GPU texture to CPU svtkDataArray.
...
// if GPUExtent does not cover all GPUExtent, some of the svtkDataArray will
// be uninitialized.
// Reminder: A=>B <=> !A||B
...
bool svtkDataTransferHelper::Download()
{
  return (this->DownloadAsync1() && this->DownloadAsync2());
}

//----------------------------------------------------------------------------
bool svtkDataTransferHelper::DownloadAsync1()
{
  if (!this->Context)
  {
    svtkErrorMacro("Cannot download from GPU without context.");
    return false;
  }
  ...
  // * Download data to svtkPixelBufferObject.
  svtkSmartPointer<svtkPixelBufferObject> pbo;
  pbo.TakeReference(this->Texture->Download());

  if (!pbo)
  {
    svtkErrorMacro("Failed to download texture to a Pixel Buffer object.");
    return false;
  }

  if (pbo->GetSize() < static_cast<unsigned int>(gpudims[0] * gpudims[1] * gpudims[2] * numComps))
  {
    svtkErrorMacro("GPU data size is smaller than GPUExtent.");
    return false;
  }
...
//----------------------------------------------------------------------------
bool svtkDataTransferHelper::DownloadAsync2()
{
  if (!this->AsyncDownloadPBO)
  {
    svtkErrorMacro("DownloadAsync1() must be called successfully "
  ...
  if (!this->Array)
  {
    svtkDataArray* array = svtkDataArray::CreateDataArray(this->Texture->GetSVTKDataType());
    this->SetArray(array);
    array->Delete();
...
//----------------------------------------------------------------------------
bool svtkDataTransferHelper::GetShaderSupportsTextureInt()
{
  return this->ShaderSupportsTextureInt;
}

//----------------------------------------------------------------------------
void svtkDataTransferHelper::SetShaderSupportsTextureInt(bool value)
{
  this->ShaderSupportsTextureInt = value;
}

//----------------------------------------------------------------------------
svtkPixelBufferObject* svtkDataTransferHelper::GetPBO()
{
  if (!this->PBO)
  {
    this->PBO = svtkSmartPointer<svtkPixelBufferObject>::New();
...
//----------------------------------------------------------------------------
void svtkDataTransferHelper::PrintSelf(ostream& os, svtkIndent indent)
{
