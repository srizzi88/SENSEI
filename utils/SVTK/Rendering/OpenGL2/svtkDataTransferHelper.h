/*=========================================================================
  ...
  Module:    svtkDataTransferHelper.h
...
=========================================================================*/
/**
 * @class   svtkDataTransferHelper
 * @brief   is a helper class that aids in transferring
 *  data between CPU memory and GPU memory.
 *
 *
 *  svtkDataTransferHelper is a helper class that aids in transferring data
 *  between the CPU memory and the GPU memory. The data in GPU memory is
 *  stored as textures which that in CPU memory is stored as svtkDataArray.
 *  svtkDataTransferHelper provides API to transfer only a sub-extent of CPU
 *  structured data to/from the GPU.
 *
 * @sa
 *  svtkPixelBufferObject svtkTextureObject svtkOpenGLExtensionManager
 */

#ifndef svtkDataTransferHelper_h
#define svtkDataTransferHelper_h
...
class SVTKRENDERINGOPENGL2_EXPORT svtkDataTransferHelper : public svtkObject
{
public:
  static svtkDataTransferHelper* New();
  svtkTypeMacro(svtkDataTransferHelper, svtkObject);
  ...
  //@{
  /**
   * Set the CPU data extent. The extent matches the svtkDataArray size.
   * If the svtkDataArray comes from an svtkImageData and it is part of the
   * point data, it is usually the svtkImageData extent.
   * It can be on cell data too, but in this case it does not match the
   * svtkImageData extent.
   * If the svtkDataArray comes from a svtkDataSet, just
   * set it to a one-dimenstional extent equal to the number of tuples.
   * Initial value is (0,0,0,0,0,0), a valid one tuple array.
   */
  svtkSetVector6Macro(CPUExtent, int);
  svtkGetVector6Macro(CPUExtent, int);
  //@}
   ...
   * Initial value is (0,0,0,0,0,0), a valid one tuple array.
   */
  svtkSetVector6Macro(GPUExtent, int);
  svtkGetVector6Macro(GPUExtent, int);
  //@}
   ...
   */
  svtkSetVector6Macro(TextureExtent, int);
  svtkGetVector6Macro(TextureExtent, int);
  //@}
   ...
   */
  svtkSetMacro(MinTextureDimension, int);
  svtkGetMacro(MinTextureDimension, int);
  //@}
   ...
   */
  svtkGetObjectMacro(Array, svtkDataArray);
  void SetArray(svtkDataArray* array);
  //@}
   ...
   */
  svtkGetObjectMacro(Texture, svtkTextureObject);
  void SetTexture(svtkTextureObject* texture);
  //@}
...
protected:
  ...
  ~svtkDataTransferHelper() override;
  ...
  svtkSmartPointer<svtkPixelBufferObject> AsyncDownloadPBO;

  svtkPixelBufferObject* GetPBO();

  // We try to reuse the PBO if possible.
  svtkSmartPointer<svtkPixelBufferObject> PBO;

private:
  svtkDataTransferHelper(const svtkDataTransferHelper&) = delete;
  void operator=(const svtkDataTransferHelper&) = delete;
};
