/*=========================================================================
  ...
  Module:    svtkDualDepthPeelingPass.h
...
/**
 * @class   svtkDualDepthPeelingPass
 ...
 * appropriate RenderPass (usually svtkVolumetricPass).
 *
...
#ifndef svtkDualDepthPeelingPass_h
#define svtkDualDepthPeelingPass_h
...
class SVTKRENDERINGOPENGL2_EXPORT svtkDualDepthPeelingPass : public svtkDepthPeelingPass
{
public:
  static svtkDualDepthPeelingPass* New();
  svtkTypeMacro(svtkDualDepthPeelingPass, svtkDepthPeelingPass);
   ...
   * It is usually set to a svtkVolumetricPass.
   * Initial value is a NULL pointer.
   */
  svtkGetObjectMacro(VolumetricPass, svtkRenderPass) virtual void SetVolumetricPass(
...
protected:
  ...
  bool PreReplaceVolumetricShaderValues(std::string& vertexShader, std::string& geometryShader,
    std::string& fragmentShader, svtkAbstractMapper* mapper, svtkProp* prop);
  bool SetTranslucentShaderParameters(svtkShaderProgram* program, svtkAbstractMapper* mapper,
    svtkProp* prop, svtkOpenGLVertexArrayObject* VAO);
  bool SetVolumetricShaderParameters(svtkShaderProgram* program, svtkAbstractMapper* mapper,
    svtkProp* prop, svtkOpenGLVertexArrayObject* VAO);
  ...
  ~svtkDualDepthPeelingPass() override;

  void SetCurrentStage(ShaderStage stage);
  svtkSetMacro(CurrentPeelType, PeelType);
   ...
   */
  void Initialize(const svtkRenderState* state);

  //@{
  /**
   * Initialize helpers.
   */
  void InitColorTexture(svtkTextureObject* tex, const svtkRenderState* s);
  void InitDepthTexture(svtkTextureObject* tex, const svtkRenderState* s);
  void InitOpaqueDepthTexture(svtkTextureObject* tex, const svtkRenderState* s);
  void InitFramebuffer(const svtkRenderState* s);
  //@}
  ...
  svtkRenderTimerLog* Timer;
  svtkRenderPass* VolumetricPass;
  const svtkRenderState* RenderState;

  svtkOpenGLQuadHelper* CopyColorHelper;
  svtkOpenGLQuadHelper* CopyDepthHelper;
  svtkOpenGLQuadHelper* BackBlendHelper;
  svtkOpenGLQuadHelper* BlendHelper;

  svtkTextureObject* Textures[NumberOfTextures];
  ...
  svtkTimeStamp CurrentStageTimeStamp;
...
private:
  svtkDualDepthPeelingPass(const svtkDualDepthPeelingPass&) = delete;
  void operator=(const svtkDualDepthPeelingPass&) = delete;
};

#endif // svtkDualDepthPeelingPass_h
