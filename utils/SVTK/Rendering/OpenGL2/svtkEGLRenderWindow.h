...
=========================================================================*/
/**
 * @class   svtkEGLRenderWindow
 * @brief   OpenGL rendering window
 *
 * svtkEGLRenderWindow is a concrete implementation of the abstract class
 * svtkRenderWindow. This class creates a window on Android platform and
 * for client API OpenGL ES and an offscreen pbuffer for OpenGL.
 * svtkOpenGLRenderer interfaces to the OpenGL graphics library.
 * Application programmers should normally use svtkRenderWindow instead of
 * the OpenGL specific version.
 *
 * If the SVTK_DEFAULT_EGL_DEVICE_INDEX environment variable is present at
 * the time of construction, it's value will be used to initialize the
 * DeviceIndex, falling back to the SVTK_DEFAULT_EGL_DEVICE_INDEX
 * preprocessor definition otherwise.
 */

#ifndef svtkEGLRenderWindow_h
#define svtkEGLRenderWindow_h
...
class SVTKRENDERINGOPENGL2_EXPORT svtkEGLRenderWindow : public svtkOpenGLRenderWindow
{
public:
  static svtkEGLRenderWindow* New();
  svtkTypeMacro(svtkEGLRenderWindow, svtkOpenGLRenderWindow);
   ...
   */
  svtkTypeBool IsDirect() override { return 1; }
   ...
   */
  svtkTypeBool GetEventPending() override { return 0; }

  svtkTypeBool GetOwnWindow() { return this->OwnWindow; }
...
private:
  svtkEGLRenderWindow(const svtkEGLRenderWindow&) = delete;
  void operator=(const svtkEGLRenderWindow&) = delete;
