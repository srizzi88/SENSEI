 ...
 *
 *
 * svtkIOSRenderWindow is a concrete implementation of the abstract
 * class svtkOpenGLRenderWindow. It is only available on iOS.
 * To use this class, build SVTK with SVTK_USE_IOS turned ON (this is
 * the default).
 ...
 * svtkIOSRenderWindow uses Objective-C++, and the OpenGL and
 * IOS APIs. This class's default behaviour is to create an NSWindow and
 * a svtkIOSGLView which are used together to draw all SVTK content.
 * If you already have an NSWindow and svtkIOSGLView and you want this
 ...
 *
 * @sa
 * svtkOpenGLRenderWindow svtkIOSGLView
 *
 * @warning
...
#ifndef svtkIOSRenderWindow_h
#define svtkIOSRenderWindow_h

#include "svtkOpenGLRenderWindow.h"
#include "svtkRenderingOpenGL2Module.h" // For export macro

class SVTKRENDERINGOPENGL2_EXPORT svtkIOSRenderWindow : public svtkOpenGLRenderWindow
{
public:
  static svtkIOSRenderWindow* New();
  svtkTypeMacro(svtkIOSRenderWindow, svtkOpenGLRenderWindow);
   ...
   * implemented for the svtkIOSRenderWindow.  It can only be called
  ...
  /**
   * Remap the window.  This is not implemented for the svtkIOSRenderWindow.
   */
  void WindowRemap() override;

  /**
   * Set the preferred window size to full screen.  This is not implemented
   * for the svtkIOSRenderWindow.
   */
  ...
  {
    svtkWarningMacro("SetNextWindowInfo not implemented (WindowRemap not implemented).");
  }
  void* GetGenericDrawable() override
  {
    svtkWarningMacro("Method not implemented.");
    return 0;
  }
  void SetDisplayId(void*) override { svtkWarningMacro("Method not implemented."); }
  ...
  {
    svtkWarningMacro("SetNextWindowId not implemented (WindowRemap not implemented).");
  }
   ...
   * svtkIOSGLView (UIView subclass) which are used together to draw
   * all svtk stuff into. If you already have an NSWindow and UIView and
   ...
   * SetWindowId()/SetRootWindow() is to create an NSWindow and a
   * svtkIOSGLView (UIView subclass) which are used together to draw all
   * svtk stuff into. If you already have an NSWindow and UIView and you
  ...
  /**
   * Set the UIView* for the svtkRenderWindow to be parented within.  The
   * Position and Size of the RenderWindow will set the rectangle of the
   * UIView that the svtkRenderWindow will create within this parent.
   * If you set the WindowId, then this ParentId will be ignored.
   */
  void SetParentId(void* UIView) override;

  /**
   * Get the parent UIView* for this svtkRenderWindow.  This method will
...
protected:
  ...
  svtkTypeBool OffScreenInitialized;
  svtkTypeBool OnScreenInitialized;

  // IOS seems to have issues with getting RGB data
  int ReadPixels(
    const svtkRecti& rect, int front, int glFormat, int glType, void* data, int right = 0) override;

private:
  svtkIOSRenderWindow(const svtkIOSRenderWindow&) = delete;
  void operator=(const svtkIOSRenderWindow&) = delete;
