/*=========================================================================
  ...
  Module:    svtkOpenGLFramebufferObject.h
 ...
 * svtkTextureObject (both 2D or a slice of a 3D texture) or
 * a svtkRenderbuffer. In some cases a renderbuffer may be faster
 ...
 * fbo->SaveCurrentBindingsAndBuffers();
 * fbo->Bind();
 * fbo->AddColorAttachment(0, svtkTextureObj);
 ...
 *
 * This class replaces both svtkFrameBufferObject and svtkFrameBufferObject2
 ...
 *
 * @sa
 * svtkTextureObject, svtkRenderbufferObject
 */

#ifndef svtkOpenGLFramebufferObject_h
#define svtkOpenGLFramebufferObject_h
 ...
 */
#define SVTK_UPDATED_FRAMEBUFFER

/**
 * A variant of svtkErrorMacro that is used to verify framebuffer
 ...
 */
#ifdef NDEBUG
#define svtkCheckFrameBufferStatusMacro(mode)
#define svtkStaticCheckFrameBufferStatusMacro(mode)
#else
#define svtkCheckFrameBufferStatusMacroImpl(macro, mode)                                            \
  {                                                                                                \
    const char* eStr;                                                                              \
    bool ok = svtkOpenGLFramebufferObject::GetFrameBufferStatus(mode, eStr);                        \
    if (!ok)                                                                                       \
    {                                                                                              \
      macro(<< "OpenGL ERROR. The FBO is incomplete : " << eStr);                                  \
    }                                                                                              \
  }
#define svtkCheckFrameBufferStatusMacro(mode) svtkCheckFrameBufferStatusMacroImpl(svtkErrorMacro, mode)
#define svtkStaticCheckFrameBufferStatusMacro(mode)                                                 \
  svtkCheckFrameBufferStatusMacroImpl(svtkGenericWarningMacro, mode)
#endif
...
class SVTKRENDERINGOPENGL2_EXPORT svtkOpenGLFramebufferObject : public svtkFrameBufferObjectBase
{
public:
  static svtkOpenGLFramebufferObject* New();
  svtkTypeMacro(svtkOpenGLFramebufferObject, svtkFrameBufferObjectBase);
  ...
  svtkGetMacro(ActiveReadBuffer, unsigned int);
   ...
   */
  void RenderQuad(int minX, int maxX, int minY, int maxY, svtkShaderProgram* program,
   ...
   */
  void AddColorAttachment(unsigned int attId, svtkTextureObject* tex, unsigned int zslice = 0,
    unsigned int format = 0, unsigned int mipmapLevel = 0);
  void AddColorAttachment(unsigned int attId, svtkRenderbuffer* tex);
  ...
  void AddDepthAttachment(svtkTextureObject* tex);
  void AddDepthAttachment(svtkRenderbuffer* tex);
  ...
  {
    svtkDebugMacro(<< this->GetClassName() << " (" << this << "): returning LastSize pointer "
                  << this->LastSize);
  ...
  void GetLastSize(int& _arg1, int& _arg2) override
  {
    ...
    svtkDebugMacro(<< this->GetClassName() << " (" << this << "): returning LastSize (" << _arg1
                  << "," << _arg2 << ")");
  }
   ...
   * prints detected errors to svtkErrorMacro.
   */
  ...
  svtkGetMacro(FBOIndex, unsigned int);
   ...
   */
  svtkPixelBufferObject* DownloadColor1(int extent[4], int svtkType, int channel);

  svtkPixelBufferObject* DownloadColor3(int extent[4], int svtkType);

  svtkPixelBufferObject* DownloadColor4(int extent[4], int svtkType);
   ...
   */
  svtkPixelBufferObject* DownloadDepth(int extent[4], int svtkType);
...
protected:
   ...
   */
  static bool LoadRequiredExtensions(svtkOpenGLRenderWindow*) { return true; }
  ...
  void DestroyDepthBuffer(svtkWindow* win);
  void DestroyColorBuffers(svtkWindow* win);
  ...
  /**
   * Given a svtk type get a compatible open gl type.
   */
  int GetOpenGLType(int svtkType);

  svtkOpenGLFramebufferObject();
  ~svtkOpenGLFramebufferObject() override;
  ...
  svtkFOInfo* DepthBuffer;
  std::map<unsigned int, svtkFOInfo*> ColorBuffers;

private:
  svtkOpenGLFramebufferObject(const svtkOpenGLFramebufferObject&) = delete;
  void operator=(const svtkOpenGLFramebufferObject&) = delete;
};
