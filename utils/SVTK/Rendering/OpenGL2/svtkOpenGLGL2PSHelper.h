/*=========================================================================
  ...
  Module:    svtkOpenGLGL2PSHelper.h
 ...
 * GL2PS context. Link to svtkRenderingGL2PSOpenGL2 to bring in the
 * svtkOpenGLGL2PSHelperImpl class, the object factory override that implements
 * this interface.
 */

#ifndef svtkOpenGLGL2PSHelper_h
#define svtkOpenGLGL2PSHelper_h
...
class SVTKRENDERINGOPENGL2_EXPORT svtkOpenGLGL2PSHelper : public svtkObject
{
public:
  static svtkOpenGLGL2PSHelper* New();
  svtkAbstractTypeMacro(svtkOpenGLGL2PSHelper, svtkObject);
   ...
   */
  static svtkOpenGLGL2PSHelper* GetInstance();
  static void SetInstance(svtkOpenGLGL2PSHelper*);
  //@}
   ...
   */
  svtkGetMacro(RenderWindow, svtkRenderWindow*);
  //@}
   ...
   * and use the svtkOpenGLGL2PSHelper API to draw themselves during the capture
   * pass.
   */
  svtkGetMacro(ActiveState, State);
  //@}
   ...
   * 0xffff.
   */
  svtkSetMacro(LineStipple, unsigned short);
  svtkGetMacro(LineStipple, unsigned short);
  //@}
  ...
  virtual void ProcessTransformFeedback(
    svtkTransformFeedback* tfc, svtkRenderer* ren, svtkActor* act) = 0;
  virtual void ProcessTransformFeedback(
    svtkTransformFeedback* tfc, svtkRenderer* ren, unsigned char col[4]) = 0;
  virtual void ProcessTransformFeedback(
    svtkTransformFeedback* tfc, svtkRenderer* ren, float col[4]) = 0;
  //@}
   ...
   * @sa TextAsPath
   */
  virtual void DrawString(const std::string& str, svtkTextProperty* tprop, double pos[3],
    double backgroundDepth, svtkRenderer* ren) = 0;

  /**
   * Generate PS, EPS, or SVG markup from a svtkPath object, and then inject it
   ...
   */
  virtual void DrawPath(svtkPath* path, double rasterPos[3], double windowPos[2],
   ...
   */
  virtual void Draw3DPath(svtkPath* path, svtkMatrix4x4* actorMatrix, double rasterPos[3],
    unsigned char actorColor[4], svtkRenderer* ren, const char* label = nullptr) = 0;

  /**
   * Draw the image at pos.
   * Image must be RGB or RGBA with float scalars.
   */
  virtual void DrawImage(svtkImageData* image, double pos[3]) = 0;

protected:
  friend class svtkOpenGLGL2PSExporter;

  svtkOpenGLGL2PSHelper();
  ~svtkOpenGLGL2PSHelper() override;

  svtkSetMacro(ActiveState, State);
  svtkSetMacro(TextAsPath, bool);
  svtkSetMacro(RenderWindow, svtkRenderWindow*); // Doesn't ref count, not needed.
  svtkSetMacro(PointSizeFactor, float);
  svtkSetMacro(LineWidthFactor, float);

  static svtkOpenGLGL2PSHelper* Instance;
...
private:
  svtkOpenGLGL2PSHelper(const svtkOpenGLGL2PSHelper&) = delete;
  void operator=(const svtkOpenGLGL2PSHelper&) = delete;
};

#endif // svtkOpenGLGL2PSHelper_h
