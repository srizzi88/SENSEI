/*=========================================================================
  ...
  Module:    svtkOpenGLImageMapper.cxx
...
=========================================================================*/
#include "svtkOpenGLImageMapper.h"
...
svtkStandardNewMacro(svtkOpenGLImageMapper);

svtkOpenGLImageMapper::svtkOpenGLImageMapper()
{
  this->Actor = svtkTexturedActor2D::New();
...
void svtkOpenGLImageMapper::ReleaseGraphicsResources(svtkWindow* renWin)
{
...
#define svtkClampToUnsignedChar(x, y)                                                               \
  {                                                                                                \
    val = (y);                                                                                     \
    if (val < 0)                                                                                   \
    {                                                                                              \
...
#define svtkClampIntToUnsignedChar(x, y, shift)                                                     \
  {                                                                                                \
    val = (y);                                                                                     \
    if (val < 0)                                                                                   \
    {                                                                                              \
...
inline int svtkPadToFour(int n)
{
  return ((n + 3) / 4) * 4;
}
...
template <class T>
void svtkOpenGLImageMapperRenderDouble(svtkOpenGLImageMapper* self, svtkImageData* data, T* dataPtr,
  double shift, double scale, svtkViewport* viewport)
{
  ...
  svtkIdType tempIncs[3];
  data->GetIncrements(tempIncs);
  svtkIdType inInc1 = tempIncs[1];
  ...
  if (bpp < 4)
  {
    newPtr = new unsigned char[svtkPadToFour(3 * width * height)];
  }
  else
  {
  ...
  while (--j >= 0)
  {
    ...
    switch (bpp)
    {
      case 1:
        while (--i >= 0)
        {
          svtkClampToUnsignedChar(tmp, ((*inPtr++ + shift) * scale));
          *ptr++ = tmp;
          *ptr++ = tmp;
          *ptr++ = tmp;
        }
      ...
      case 2:
        while (--i >= 0)
        {
          svtkClampToUnsignedChar(tmp, ((*inPtr++ + shift) * scale));
          *ptr++ = tmp;
          svtkClampToUnsignedChar(*ptr++, ((*inPtr++ + shift) * scale));
          *ptr++ = tmp;
        }
        break;
  ...
  svtkOpenGLStaticCheckErrorMacro("failed after ImageMapperRenderDouble");
}
...
template <class T>
void svtkOpenGLImageMapperRenderShort(svtkOpenGLImageMapper* self, svtkImageData* data, T* dataPtr,
  double shift, double scale, svtkViewport* viewport)
{
  svtkOpenGLClearErrorMacro();
  ...
  */
  ...
  while (--j >= 0)
  {
    ...
    switch (bpp)
    {
      case 1:
        while (--i >= 0)
        {
          svtkClampIntToUnsignedChar(tmp, (*inPtr++ * sscale + sshift), bitShift);
          *ptr++ = tmp;
          *ptr++ = tmp;
          *ptr++ = tmp;
        }
      ...
      case 2:
        while (--i >= 0)
        {
          svtkClampIntToUnsignedChar(tmp, (*inPtr++ * sscale + sshift), bitShift);
          *ptr++ = tmp;
          svtkClampIntToUnsignedChar(*ptr++, (*inPtr++ * sscale + sshift), bitShift);
          *ptr++ = tmp;
        }
        break;
  ...
  svtkOpenGLStaticCheckErrorMacro("failed after ImageMapperRenderShort");
}
...
template <class T>
void svtkOpenGLImageMapperRenderChar(
  svtkOpenGLImageMapper* self, svtkImageData* data, T* dataPtr, svtkViewport* viewport)
{
  svtkOpenGLClearErrorMacro();
  ...
  { // feed through other bytes without reformatting
    ...
    if (nC == 3)
    {
      newPtr = new unsigned char[svtkPadToFour(nC * width * height)];
    }
    else
    {
  ...
  svtkOpenGLStaticCheckErrorMacro("failed after ImageMapperRenderChar");
}
...
// match.
template <class T>
void svtkOpenGLImageMapperRender(svtkOpenGLImageMapper* self, svtkImageData* data, T* dataPtr,
  double shift, double scale, svtkViewport* viewport)
{
  svtkOpenGLImageMapperRenderDouble(self, data, dataPtr, shift, scale, viewport);
}

static void svtkOpenGLImageMapperRender(svtkOpenGLImageMapper* self, svtkImageData* data,
  char* dataPtr, double shift, double scale, svtkViewport* viewport)
{
  if (shift == 0.0 && scale == 1.0)
  {
    svtkOpenGLImageMapperRenderChar(self, data, dataPtr, viewport);
  }
  else
  {
    svtkOpenGLImageMapperRenderShort(self, data, dataPtr, shift, scale, viewport);
  }
}

static void svtkOpenGLImageMapperRender(svtkOpenGLImageMapper* self, svtkImageData* data,
  unsigned char* dataPtr, double shift, double scale, svtkViewport* viewport)
{
  if (shift == 0.0 && scale == 1.0)
  {
    svtkOpenGLImageMapperRenderChar(self, data, dataPtr, viewport);
...
static void svtkOpenGLImageMapperRender(svtkOpenGLImageMapper* self, svtkImageData* data,
  signed char* dataPtr, double shift, double scale, svtkViewport* viewport)
{
  if (shift == 0.0 && scale == 1.0)
  {
    svtkOpenGLImageMapperRenderChar(self, data, dataPtr, viewport);
...
static void svtkOpenGLImageMapperRender(svtkOpenGLImageMapper* self, svtkImageData* data,
  short* dataPtr, double shift, double scale, svtkViewport* viewport)
{
  svtkOpenGLImageMapperRenderShort(self, data, dataPtr, shift, scale, viewport);
}

static void svtkOpenGLImageMapperRender(svtkOpenGLImageMapper* self, svtkImageData* data,
  unsigned short* dataPtr, double shift, double scale, svtkViewport* viewport)
{
  svtkOpenGLImageMapperRenderShort(self, data, dataPtr, shift, scale, viewport);
}
...
void svtkOpenGLImageMapper::RenderData(svtkViewport* viewport, svtkImageData* data, svtkActor2D* actor)
{
  void* ptr0;
  double shift, scale;

  svtkWindow* window = static_cast<svtkWindow*>(viewport->GetSVTKWindow());
  if (!window)
  {
    svtkErrorMacro(<< "svtkOpenGLImageMapper::RenderData - no window set for viewport");
    return;
  }
  ...
  {
    svtkTemplateMacro(
      svtkOpenGLImageMapperRender(this, data, static_cast<SVTK_TT*>(ptr0), shift, scale, viewport));
    default:
      svtkErrorMacro(<< "Unsupported image type: " << data->GetScalarType());
  }

  svtkOpenGLCheckErrorMacro("failed after RenderData");
}

void svtkOpenGLImageMapper::DrawPixels(
  svtkViewport* viewport, int width, int height, int numComponents, void* data)
{
  ...
  svtkPolyData* pd = svtkPolyDataMapper2D::SafeDownCast(this->Actor->GetMapper())->GetInput();
  ...
  svtkDataArray* tcoords = pd->GetPointData()->GetTCoords();
  float tmp[2];
  tmp[0] = 0;
  tmp[1] = 0;
...
void svtkOpenGLImageMapper::PrintSelf(ostream& os, svtkIndent indent)
{
