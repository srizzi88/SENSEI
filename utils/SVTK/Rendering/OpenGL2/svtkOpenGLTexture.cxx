...
// ---------------------------------------------------------------------------
svtkStandardNewMacro(svtkOpenGLTexture);

// ---------------------------------------------------------------------------
svtkOpenGLTexture::svtkOpenGLTexture()
{
...
void svtkOpenGLTexture::ReleaseGraphicsResources(svtkWindow* win)
{
...
// ---------------------------------------------------------------------------
void svtkOpenGLTexture::SetTextureObject(svtkTextureObject* textureObject)
{
  svtkDebugMacro(<< this->GetClassName() << " (" << this << "): setting TextureObject to "
                << textureObject);
  if (this->TextureObject != textureObject)
  {
    svtkTextureObject* temp = this->TextureObject;
...
// ---------------------------------------------------------------------------
int svtkOpenGLTexture::GetTextureUnit()
{
  if (this->TextureObject)
  {
...
// ---------------------------------------------------------------------------
void svtkOpenGLTexture::CopyTexImage(int x, int y, int width, int height)
{
...
void svtkOpenGLTexture::Render(svtkRenderer* ren)
{
...
// ----------------------------------------------------------------------------
// Implement base class method.
void svtkOpenGLTexture::Load(svtkRenderer* ren)
{
  ...
  if (!this->ExternalTextureObject)
  {
    ...
    int numIns = 1;
    svtkMTimeType inputTime = input->GetMTime();

    if (this->CubeMap)
    {
      for (int i = 1; i < this->GetNumberOfInputPorts(); ++i)
      {
        svtkImageData* in = svtkImageData::SafeDownCast(this->GetInputDataObject(i, 0));
        if (!in)
        {
          break;
        }
      ...
      if (numIns < 6)
      {
        svtkErrorMacro("Cube Maps require 6 inputs");
        return;
      }
    }
    ...
    {
      svtkErrorMacro("A render window was deleted without releasing graphics resources");
    }
      ...
      renWin->GetContextCreationTime() > this->LoadTime)
    {
      ...
      svtkDataArray* scalars = this->GetInputArrayToProcess(0, input);
      ...
      for (int i = 0; i < numIns && i < 6; ++i)
      {
        ...
        svtkDataArray* inscalars = this->GetInputArrayToProcess(i, in);

        // decide whether the texture needs to be resampled
        GLint maxDimGL;
        ostate->svtkglGetIntegerv(GL_MAX_TEXTURE_SIZE, &maxDimGL);
        svtkOpenGLCheckErrorMacro("failed at glGetIntegerv");
        ...
        bool directColors = (this->ColorMode != SVTK_COLOR_MODE_MAP_SCALARS &&
                              inscalars->GetDataType() == SVTK_UNSIGNED_CHAR) ||
          (this->ColorMode == SVTK_COLOR_MODE_DIRECT_SCALARS &&
            (inscalars->GetDataType() == SVTK_UNSIGNED_SHORT ||
              inscalars->GetDataType() == SVTK_FLOAT));
        ...
        if (resampleNeeded)
        {
          svtkDebugMacro("Texture too big for gl, maximum is " << maxDimGL);
          svtkDebugMacro(<< "Resampling texture to power of two for OpenGL");
      ...
      if (this->IsDepthTexture)
      {
        this->TextureObject->CreateDepthFromRaw(
          xsize, ysize, svtkTextureObject::Float32, dataType, nullptr);
      }
      else
      {
        if (numIns == 6)
      ...
      if (this->Interpolate)
      {
        ...
        int majorV;
        int minorV;
        static_cast<svtkOpenGLRenderWindow*>(ren->GetSVTKWindow())->GetOpenGLVersion(majorV, minorV);
        int levels = floor(log2(svtkMath::Max(size[0], size[1]))) + 1;
      ...
      {
        this->TextureObject->SetMinificationFilter(svtkTextureObject::Nearest);
        this->TextureObject->SetMagnificationFilter(svtkTextureObject::Nearest);
      }
      if (this->Repeat)
      {
        this->TextureObject->SetWrapS(svtkTextureObject::Repeat);
        this->TextureObject->SetWrapT(svtkTextureObject::Repeat);
        this->TextureObject->SetWrapR(svtkTextureObject::Repeat);
      }
      else
      {
  ...
  svtkOpenGLCheckErrorMacro("failed after Load");
}

// ----------------------------------------------------------------------------
void svtkOpenGLTexture::PostRender(svtkRenderer* ren)
{
  if (this->TextureObject)
  {
  ...
  if (this->GetInput() && this->PremultipliedAlpha)
  {
    ...
    renWin->GetState()->svtkglBlendFuncSeparate(this->PrevBlendParams[0], this->PrevBlendParams[1],
...
{
  int size = svtkMath::NearestPowerOfTwo(i);
...
// ----------------------------------------------------------------------------
void svtkOpenGLTexture::PrintSelf(ostream& os, svtkIndent indent)
{
  this->Superclass::PrintSelf(os, indent);
}

// ----------------------------------------------------------------------------
int svtkOpenGLTexture::IsTranslucent()
{
