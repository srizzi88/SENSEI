 ...
 *
 * This class implements all SetUniform* functions supported by svtkShaderProgram but instead of
...
#ifndef svtkOpenGLUniforms_h
#define svtkOpenGLUniforms_h

#include "svtkRenderingOpenGL2Module.h" // For export macro
#include "svtkUniforms.h"
#include <string> // For member functions

class svtkUniformInternals;
class svtkShaderProgram;

class SVTKRENDERINGOPENGL2_EXPORT svtkOpenGLUniforms : public svtkUniforms
{
public:
  static svtkOpenGLUniforms* New();
  svtkTypeMacro(svtkOpenGLUniforms, svtkUniforms);
  ...
  svtkMTimeType GetUniformListMTime() override;
  ...
  void SetUniform(const char* name, svtkUniforms::TupleType tt, int nbComponents,
  ...
  void SetUniformMatrix(const char* name, svtkMatrix3x3* v) override;
  void SetUniformMatrix(const char* name, svtkMatrix4x4* v) override;
  //@}
  ...
  bool GetUniformMatrix(const char* name, svtkMatrix3x3* v) override;
  bool GetUniformMatrix(const char* name, svtkMatrix4x4* v) override;
  //@}
  ...
  const char* GetNthUniformName(svtkIdType uniformIndex) override;
...
protected:
  svtkOpenGLUniforms();
  ~svtkOpenGLUniforms() override;

private:
  svtkOpenGLUniforms(const svtkOpenGLUniforms&) = delete;
  void operator=(const svtkOpenGLUniforms&) = delete;

  svtkUniformInternals* Internals;
};
