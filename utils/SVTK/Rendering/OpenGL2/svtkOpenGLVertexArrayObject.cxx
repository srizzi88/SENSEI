...
svtkStandardNewMacro(svtkOpenGLVertexArrayObject);

namespace
{
// Copied from svtkglShaderProgram, time to move into a common header?
...
class svtkOpenGLVertexArrayObject::Private
{
public:
  Private()
  {
...
svtkOpenGLVertexArrayObject::svtkOpenGLVertexArrayObject()
{
  this->Internal = new svtkOpenGLVertexArrayObject::Private;
}
...
void svtkOpenGLVertexArrayObject::SetForceEmulation(bool val)
{
  this->Internal->ForceEmulation = val;
}

void svtkOpenGLVertexArrayObject::Bind()
{
...
void svtkOpenGLVertexArrayObject::Release()
{
  if (this->Internal->IsReady() && this->Internal->Supported)
  {
    glBindVertexArray(0);
...
void svtkOpenGLVertexArrayObject::ShaderProgramChanged()
{
  this->Release();
...
void svtkOpenGLVertexArrayObject::ReleaseGraphicsResources()
{
  this->ShaderProgramChanged();
  this->Internal->ReleaseGraphicsResources();
}

bool svtkOpenGLVertexArrayObject::AddAttributeArray(svtkShaderProgram* program,
  svtkOpenGLVertexBufferObject* buffer, const std::string& name, int offset, bool normalize)
{
...
bool svtkOpenGLVertexArrayObject::AddAttributeArrayWithDivisor(svtkShaderProgram* program,
  svtkOpenGLBufferObject* buffer, const std::string& name, int offset, size_t stride,
  int elementType, int elementTupleSize, bool normalize, int divisor, bool isMatrix)
{
  if (!program)
  {
    svtkErrorMacro("attempt to add attribute without a program for attribute " << name);
    return false;
  }
  ...
  if (!program->isBound())
  {
    svtkErrorMacro("attempt to add attribute without a bound program for attribute " << name);
    return false;
  }

  if (buffer->GetHandle() == 0)
  {
    svtkErrorMacro("attempt to add attribute without a handleless buffer for attribute " << name);
    return false;
  }

  if (buffer->GetType() != svtkOpenGLBufferObject::ArrayBuffer)
  {
    svtkErrorMacro("attempt to add attribute without an array buffer for attribute " << name);
    return false;
  }
  ...
  if (!this->Internal->IsReady() ||
    this->Internal->HandleProgram != static_cast<GLuint>(program->GetHandle()))
  {
    svtkErrorMacro("attempt to add attribute when not ready for attribute " << name);
    return false;
  }
  ...
  if (attribs.Index == -1)
  {
    svtkErrorMacro("attempt to add attribute not found in program for attribute " << name);
    return false;
  }
...
bool svtkOpenGLVertexArrayObject::AddAttributeMatrixWithDivisor(svtkShaderProgram* program,
  svtkOpenGLBufferObject* buffer, const std::string& name, int offset, size_t stride,
...
bool svtkOpenGLVertexArrayObject::RemoveAttributeArray(const std::string& name)
{
...
//-----------------------------------------------------------------------------
void svtkOpenGLVertexArrayObject::PrintSelf(ostream& os, svtkIndent indent)
{
