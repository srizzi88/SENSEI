...
=========================================================================*/
#ifndef svtkOpenGLVertexBufferObject_h
#define svtkOpenGLVertexBufferObject_h
...
union svtkFourByteUnion {
  unsigned char c[4];
  short s[2];
  float f;
};

class SVTKRENDERINGOPENGL2_EXPORT svtkOpenGLVertexBufferObject : public svtkOpenGLBufferObject
{
public:
  static svtkOpenGLVertexBufferObject* New();
  svtkTypeMacro(svtkOpenGLVertexBufferObject, svtkOpenGLBufferObject);
  ...
  void UploadDataArray(svtkDataArray* array);

  // append a data array to this VBO, always
  // copies the data from the data array
  void AppendDataArray(svtkDataArray* array);

  // Get the mtime when this VBO was loaded
  svtkGetMacro(UploadTime, svtkTimeStamp);
  ...
  svtkGetMacro(CoordShiftAndScaleEnabled, bool);
  svtkGetMacro(CoordShiftAndScaleMethod, ShiftScaleMethod);
  ...
  svtkGetMacro(DataTypeSize, unsigned int);

  // How many tuples in the VBO
  svtkGetMacro(NumberOfTuples, unsigned int);

  // How many components in the VBO
  svtkGetMacro(NumberOfComponents, unsigned int);

  // Set/Get the VBO stride in bytes
  svtkSetMacro(Stride, unsigned int);
  svtkGetMacro(Stride, unsigned int);
  ...
  void SetCache(svtkOpenGLVertexBufferObjectCache* cache);

protected:
  svtkOpenGLVertexBufferObject();
  ~svtkOpenGLVertexBufferObject() override;
  ...
  svtkOpenGLVertexBufferObjectCache* Cache;

private:
  svtkOpenGLVertexBufferObject(const svtkOpenGLVertexBufferObject&) = delete;
  void operator=(const svtkOpenGLVertexBufferObject&) = delete;
};
