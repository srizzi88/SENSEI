/*=========================================================================
  ...
  Module:    svtkOpenGLVertexBufferObjectGroup.cxx
...
// ----------------------------------------------------------------------------
svtkStandardNewMacro(svtkOpenGLVertexBufferObjectGroup);

typedef std::map<std::string, svtkOpenGLVertexBufferObject*>::iterator vboIter;

// typedef std::map<const std::string, svtkDataArray *>::iterator arrayIter;
typedef std::map<std::string, std::vector<svtkDataArray*> >::iterator arrayIter;

// ----------------------------------------------------------------------------
svtkOpenGLVertexBufferObjectGroup::svtkOpenGLVertexBufferObjectGroup() = default;

// ----------------------------------------------------------------------------
svtkOpenGLVertexBufferObjectGroup::~svtkOpenGLVertexBufferObjectGroup()
{
...
int svtkOpenGLVertexBufferObjectGroup::GetNumberOfComponents(const char* attribute)
{
...
int svtkOpenGLVertexBufferObjectGroup::GetNumberOfTuples(const char* attribute)
{
  vboIter i = this->UsedVBOs.find(attribute);
  if (i != this->UsedVBOs.end())
  {
...
svtkOpenGLVertexBufferObject* svtkOpenGLVertexBufferObjectGroup::GetVBO(const char* attribute)
{
  vboIter i = this->UsedVBOs.find(attribute);
  if (i != this->UsedVBOs.end())
  {
...
void svtkOpenGLVertexBufferObjectGroup::RemoveAttribute(const char* attribute)
{
  ...
  if (diter != this->UsedDataArrays.end())
  {
    ...
    std::vector<svtkDataArray*>& vec = diter->second;
    ...
    std::vector<svtkDataArray*>& arrays = this->UsedDataArrays[attribute];
    svtkIdType totalOffset = 0;
    for (svtkDataArray* arr : arrays)
    {
...
void svtkOpenGLVertexBufferObjectGroup::CacheDataArray(
  const char* attribute, svtkDataArray* da, svtkViewport* vp, int destType)
{
  svtkOpenGLRenderWindow* renWin = svtkOpenGLRenderWindow::SafeDownCast(vp->GetSVTKWindow());
  ...
  const char* attribute, svtkDataArray* da, svtkOpenGLVertexBufferObjectCache* cache, int destType)
{
  ...
  da->Register(this);
  std::vector<svtkDataArray*>& vec = this->UsedDataArrays[attribute];
  vec.push_back(da);
  this->UsedDataArrayMaps[attribute][da] = 0;
  this->UsedDataArraySizes[attribute] = da->GetNumberOfTuples();

  // get the VBO for this DA
  svtkOpenGLVertexBufferObject* vbo = cache->GetVBO(da, destType);
  vboIter viter = this->UsedVBOs.find(attribute);
...
//
bool svtkOpenGLVertexBufferObjectGroup::ArrayExists(
...
void svtkOpenGLVertexBufferObjectGroup::AppendDataArray(
  const char* attribute, svtkDataArray* da, int destType)
{
  if (!da)
  {
    return;
...
void svtkOpenGLVertexBufferObjectGroup::ReleaseGraphicsResources(svtkWindow*)
{
  for (vboIter i = this->UsedVBOs.begin(); i != this->UsedVBOs.end(); ++i)
  {
    i->second->Delete();
...
void svtkOpenGLVertexBufferObjectGroup::AddAllAttributesToVAO(
  svtkShaderProgram* program, svtkOpenGLVertexArrayObject* vao)
{
  for (vboIter i = this->UsedVBOs.begin(); i != this->UsedVBOs.end(); ++i)
  {
    ...
    {
      svtkOpenGLVertexBufferObject* vbo = i->second;
      if (!vao->AddAttributeArray(program, vbo, dataShaderName,
            0,                                          // offset see assert later in this file
            (vbo->GetDataType() == SVTK_UNSIGNED_CHAR))) // TODO: fix tweak. true for colors.
      {
        svtkErrorMacro(<< "Error setting '" << dataShaderName << "' in shader VAO.");
      }
    }
  }
}

void svtkOpenGLVertexBufferObjectGroup::ClearAllDataArrays()
{
  for (arrayIter i = this->UsedDataArrays.begin(); i != this->UsedDataArrays.end(); ++i)
  {
    std::vector<svtkDataArray*>& vec = i->second;
    for (size_t j = 0; j < vec.size(); j++)
    {
      if (vec[j])
      {
...
void svtkOpenGLVertexBufferObjectGroup::ClearAllVBOs()
{
  for (vboIter i = this->UsedVBOs.begin(); i != this->UsedVBOs.end(); ++i)
  {
    i->second->Delete();
...
void svtkOpenGLVertexBufferObjectGroup::BuildAllVBOs(svtkViewport* vp)
{
  svtkOpenGLRenderWindow* renWin = svtkOpenGLRenderWindow::SafeDownCast(vp->GetSVTKWindow());
  svtkOpenGLVertexBufferObjectCache* cache = renWin->GetVBOCache();
...
// ----------------------------------------------------------------------------
void svtkOpenGLVertexBufferObjectGroup::BuildAllVBOs(svtkOpenGLVertexBufferObjectCache*)
{
  ...
  for (arrayIter i = this->UsedDataArrays.begin(); i != this->UsedDataArrays.end(); ++i)
  {
    ...
    std::vector<svtkDataArray*>& vec = i->second;
    svtkOpenGLVertexBufferObject* vbo = this->UsedVBOs[attribute];
...
//----------------------------------------------------------------------------
svtkMTimeType svtkOpenGLVertexBufferObjectGroup::GetMTime()
{
  svtkMTimeType mTime = this->Superclass::GetMTime();

  for (vboIter i = this->UsedVBOs.begin(); i != this->UsedVBOs.end(); ++i)
  {
    svtkMTimeType time = i->second->GetMTime();
    if (time > mTime)
    {
      mTime = time;
    }
...
// ----------------------------------------------------------------------------
void svtkOpenGLVertexBufferObjectGroup::PrintSelf(ostream& os, svtkIndent indent)
{
