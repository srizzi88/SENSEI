/*=========================================================================
  ...
  Module:    svtkPanoramicProjectionPass.cxx
...
svtkStandardNewMacro(svtkPanoramicProjectionPass);

// ----------------------------------------------------------------------------
void svtkPanoramicProjectionPass::PrintSelf(ostream& os, svtkIndent indent)
{
...
// ----------------------------------------------------------------------------
void svtkPanoramicProjectionPass::Render(const svtkRenderState* s)
{
  ...
  {
    svtkWarningMacro("no delegate in svtkPanoramicProjectionPass.");
    return;
  }
  ...
  ostate->svtkglViewport(0, 0, this->CubeResolution, this->CubeResolution);
  ostate->svtkglScissor(0, 0, this->CubeResolution, this->CubeResolution);
...
// ----------------------------------------------------------------------------
void svtkPanoramicProjectionPass::InitOpenGLResources(svtkOpenGLRenderWindow* renWin)
{
  ...
  if (this->CubeMapTexture == nullptr)
  {
    ...
    this->CubeMapTexture = svtkTextureObject::New();
    this->CubeMapTexture->SetContext(renWin);
    if (this->Interpolate)
    {
      this->CubeMapTexture->SetMinificationFilter(svtkTextureObject::Linear);
      this->CubeMapTexture->SetMagnificationFilter(svtkTextureObject::Linear);
    }
    this->CubeMapTexture->SetWrapS(svtkTextureObject::ClampToEdge);
    this->CubeMapTexture->SetWrapT(svtkTextureObject::ClampToEdge);
    this->CubeMapTexture->SetWrapR(svtkTextureObject::ClampToEdge);
    this->CubeMapTexture->CreateCubeFromRaw(
      this->CubeResolution, this->CubeResolution, 4, SVTK_UNSIGNED_CHAR, nullptr);
  }
...
// ----------------------------------------------------------------------------
void svtkPanoramicProjectionPass::Project(svtkOpenGLRenderWindow* renWin)
{
  ...
  if (!this->QuadHelper)
  {
    ...
    switch (this->ProjectionType)
    {
      ...
      case Azimuthal:
        ...
        break;
      default:
        svtkErrorMacro("Projection type unknown");
        break;
    }

    svtkShaderProgram::Substitute(FSSource, "//SVTK::FSQ::Impl", ss.str());
  ...
  this->QuadHelper->Program->SetUniformf("angle", svtkMath::RadiansFromDegrees(this->Angle));
...
// ----------------------------------------------------------------------------
void svtkPanoramicProjectionPass::RenderOnFace(const svtkRenderState* s, int faceIndex)
{
  ...
  svtkSmartPointer<svtkCamera> oldCamera = r->GetActiveCamera();
  svtkNew<svtkCamera> newCamera;
  ...
  if (r->GetRenderWindow()->GetStereoRender())
  {
    ...
    svtkMath::Cross(newCamera->GetDirectionOfProjection(), newCamera->GetViewUp(), right);
  ...
  svtkNew<svtkTransform> lightsTransform;
  ...
  svtkNew<svtkPerspectiveTransform> perspectiveTransform;
...
// ----------------------------------------------------------------------------
void svtkPanoramicProjectionPass::ReleaseGraphicsResources(svtkWindow* w)
{
