...
#include "svtkSSAAPassFS.h"
#include "svtkTextureObjectVS.h" // a pass through shader

svtkStandardNewMacro(svtkSSAAPass);

svtkCxxSetObjectMacro(svtkSSAAPass, DelegatePass, svtkRenderPass);

// ----------------------------------------------------------------------------
svtkSSAAPass::svtkSSAAPass()
{
...
// ----------------------------------------------------------------------------
void svtkSSAAPass::PrintSelf(ostream& os, svtkIndent indent)
{
...
// \pre s_exists: s!=0
void svtkSSAAPass::Render(const svtkRenderState* s)
{
  ...
  ostate->svtkglViewport(0, 0, w, h);
  ostate->svtkglScissor(0, 0, w, h);
  ...
  {
    this->Pass2->Create2D(
      static_cast<unsigned int>(width), static_cast<unsigned int>(h), 4, SVTK_UNSIGNED_CHAR, false);
  }
  ...
  if (!this->SSAAProgram)
  {
    this->SSAAProgram = new svtkOpenGLHelper;
    // build the shader source code
    //    std::string VSSource = svtkSSAAPassVS;
    std::string VSSource = svtkTextureObjectVS;
    std::string FSSource = svtkSSAAPassFS;
    std::string GSSource;
...
// resources.
// \pre w_exists: w!=0
void svtkSSAAPass::ReleaseGraphicsResources(svtkWindow* w)
{
