...
inline GLenum convertTypeToGL(int type)
{
  switch (type)
  {
    ...
    case SVTK_DOUBLE:
      ...
      return GL_DOUBLE;
#else
      svtkGenericWarningMacro(<< "Attempt to use GL_DOUBLE when not supported");
      return 0;
#endif
    default:
      return 0;
...
typedef std::map<const char*, int, svtkShaderProgram::cmp_str>::iterator IterT;

svtkStandardNewMacro(svtkShaderProgram);

svtkCxxSetObjectMacro(svtkShaderProgram, VertexShader, svtkShader);
svtkCxxSetObjectMacro(svtkShaderProgram, FragmentShader, svtkShader);
svtkCxxSetObjectMacro(svtkShaderProgram, GeometryShader, svtkShader);
svtkCxxSetObjectMacro(svtkShaderProgram, TransformFeedback, svtkTransformFeedback);

svtkShaderProgram::svtkShaderProgram()
{
  this->VertexShader = svtkShader::New();
  this->VertexShader->SetType(svtkShader::Vertex);
  this->FragmentShader = svtkShader::New();
  this->FragmentShader->SetType(svtkShader::Fragment);
  this->GeometryShader = svtkShader::New();
  this->GeometryShader->SetType(svtkShader::Geometry);
...
bool svtkShaderProgram::Substitute(
...
// Process the string, and return a version with replacements.
bool svtkShaderProgram::Substitute(
  svtkShader* shader, const std::string& search, const std::string& replace, bool all)
{
  if (!shader)
  {
    return false;
  ...
  bool replaced = svtkShaderProgram::Substitute(source, search, replace, all);
  if (!replaced)
  {
    return false;
  }
...
template <class T>
bool svtkShaderProgram::SetAttributeArray(
  const char* name, const T& array, int tupleSize, NormalizeOption normalize)
{
  ...
  int type = svtkTypeTraits<typename T::value_type>::SVTKTypeID();
  return this->SetAttributeArrayInternal(name, &array[0], type, tupleSize, normalize);
}

bool svtkShaderProgram::AttachShader(const svtkShader* shader)
{
  ...
  if (shader->GetType() == svtkShader::Unknown)
  {
  ...
  if (shader->GetType() == svtkShader::Vertex)
  {
  ...
  else if (shader->GetType() == svtkShader::Fragment)
  {
  ...
  else if (shader->GetType() == svtkShader::Geometry)
  {
...
bool svtkShaderProgram::DetachShader(const svtkShader* shader)
{
  if (shader->GetHandle() == 0)
  {
    this->Error = "Shader object was not initialized, cannot attach it.";
...
#endif
    case svtkShader::Unknown:
    default:
      return false;
  }
}

void svtkShaderProgram::ClearMaps()
{
...
void svtkShaderProgram::SetUniformGroupUpdateTime(int gid, svtkMTimeType tm)
{
  this->UniformGroupMTimes[gid] = tm;
}

svtkMTimeType svtkShaderProgram::GetUniformGroupUpdateTime(int gid)
{
...
bool svtkShaderProgram::Bind()
{
  if (this->FileNamePrefixForDebugging != nullptr && this->FileNamePrefixForDebugging[0] != 0)
  {
    const char* exts[3] = { "VS.glsl", "FS.glsl", "GS.glsl" };
    svtkShader* shaders[3] = { this->VertexShader, this->FragmentShader, this->GeometryShader };
    for (int cc = 0; cc < 3; cc++)
    {
      std::string fname = this->FileNamePrefixForDebugging;
      fname += exts[cc];
      if (svtksys::SystemTools::FileExists(fname))
      {
        svtksys::ifstream ifp(fname.c_str());
        assert(ifp);
      ...
      {
        svtksys::ofstream ofp(fname.c_str());
...
int svtkShaderProgram::CompileShader()
{
  if (!this->GetVertexShader()->Compile())
  {
    ...
    svtkErrorMacro(<< sstm.str());
    svtkErrorMacro(<< this->GetVertexShader()->GetError());
    return 0;
  }
  if (!this->GetFragmentShader()->Compile())
  {
    ...
    svtkErrorMacro(<< sstm.str());
    svtkErrorMacro(<< this->GetFragmentShader()->GetError());
    return 0;
  }
#ifdef GL_GEOMETRY_SHADER
  ...
  if (!this->Link())
  {
    svtkErrorMacro(<< "Links failed: " << this->GetError());
    return 0;
  }
...
void svtkShaderProgram::Release()
{
  glUseProgram(0);
  this->Bound = false;
}

void svtkShaderProgram::ReleaseGraphicsResources(svtkWindow* win)
{
  this->Release();
  ...
  svtkOpenGLRenderWindow* renWin = svtkOpenGLRenderWindow::SafeDownCast(win);
...
bool svtkShaderProgram::EnableAttributeArray(const char* name)
{
...
bool svtkShaderProgram::DisableAttributeArray(const char* name)
{
  GLint location = static_cast<GLint>(this->FindAttributeArray(name));
  if (location == -1)
  {
...
bool svtkShaderProgram::UseAttributeArray(const char* name, int offset, size_t stride,
...
bool svtkShaderProgram::SetUniformi(const char* name, int i)
{
...
bool svtkShaderProgram::SetUniformf(const char* name, float f)
{
  GLint location = static_cast<GLint>(this->FindUniform(name));
  if (location == -1)
  {
...
bool svtkShaderProgram::SetUniformMatrix(const char* name, svtkMatrix4x4* matrix)
{
  GLint location = static_cast<GLint>(this->FindUniform(name));
  if (location == -1)
  {
...
bool svtkShaderProgram::SetUniformMatrix3x3(const char* name, float* matrix)
{
  GLint location = static_cast<GLint>(this->FindUniform(name));
  if (location == -1)
  {
...
bool svtkShaderProgram::SetUniformMatrix4x4(const char* name, float* matrix)
{
  return this->SetUniformMatrix4x4v(name, 1, matrix);
}

bool svtkShaderProgram::SetUniformMatrix4x4v(const char* name, const int count, float* matrix)
{
  GLint location = static_cast<GLint>(this->FindUniform(name));
  if (location == -1)
  {
...
bool svtkShaderProgram::SetUniformMatrix(const char* name, svtkMatrix3x3* matrix)
{
  GLint location = static_cast<GLint>(this->FindUniform(name));
  if (location == -1)
  {
...
bool svtkShaderProgram::SetUniform1fv(const char* name, const int count, const float* v)
{
  GLint location = static_cast<GLint>(this->FindUniform(name));
  if (location == -1)
  {
...
bool svtkShaderProgram::SetUniform1iv(const char* name, const int count, const int* v)
{
  GLint location = static_cast<GLint>(this->FindUniform(name));
  if (location == -1)
  {
...
bool svtkShaderProgram::SetUniform3fv(const char* name, const int count, const float* f)
{
  GLint location = static_cast<GLint>(this->FindUniform(name));
  if (location == -1)
  {
...
bool svtkShaderProgram::SetUniform3fv(const char* name, const int count, const float (*v)[3])
{
  GLint location = static_cast<GLint>(this->FindUniform(name));
  if (location == -1)
  {
...
bool svtkShaderProgram::SetUniform4fv(const char* name, const int count, const float* f)
{
  GLint location = static_cast<GLint>(this->FindUniform(name));
  if (location == -1)
  {
...
bool svtkShaderProgram::SetUniform4fv(const char* name, const int count, const float (*v)[4])
{
  GLint location = static_cast<GLint>(this->FindUniform(name));
  if (location == -1)
  {
...
bool svtkShaderProgram::SetUniform2f(const char* name, const float v[2])
{
  GLint location = static_cast<GLint>(this->FindUniform(name));
  if (location == -1)
  {
...
bool svtkShaderProgram::SetUniform2fv(const char* name, const int count, const float* f)
{
  GLint location = static_cast<GLint>(this->FindUniform(name));
  if (location == -1)
  {
...
bool svtkShaderProgram::SetUniform2fv(const char* name, const int count, const float (*f)[2])
{
  GLint location = static_cast<GLint>(this->FindUniform(name));
  if (location == -1)
  {
...
bool svtkShaderProgram::SetUniform3f(const char* name, const float v[3])
{
  GLint location = static_cast<GLint>(this->FindUniform(name));
  if (location == -1)
  {
...
bool svtkShaderProgram::SetUniform3f(const char* name, const double v[3])
{
  GLint location = static_cast<GLint>(this->FindUniform(name));
  if (location == -1)
  {
...
bool svtkShaderProgram::SetUniform4f(const char* name, const float v[4])
{
  GLint location = static_cast<GLint>(this->FindUniform(name));
  if (location == -1)
  {
...
bool svtkShaderProgram::SetUniform2i(const char* name, const int v[2])
{
  GLint location = static_cast<GLint>(this->FindUniform(name));
  if (location == -1)
  {
...
bool svtkShaderProgram::SetUniform3uc(const char* name, const unsigned char v[3])
{
  GLint location = static_cast<GLint>(this->FindUniform(name));
  if (location == -1)
  {
...
bool svtkShaderProgram::SetUniform4uc(const char* name, const unsigned char v[4])
{
  GLint location = static_cast<GLint>(this->FindUniform(name));
  if (location == -1)
  {
...
bool svtkShaderProgram::SetAttributeArrayInternal(const char* name, void* buffer, int type,
  int tupleSize, svtkShaderProgram::NormalizeOption normalize)
{
  if (type == -1)
  {
...
int svtkShaderProgram::FindAttributeArray(const char* cname)
{
...
int svtkShaderProgram::FindUniform(const char* cname)
{
  if (cname == nullptr || !this->Linked)
  {
    return -1;
...
bool svtkShaderProgram::IsUniformUsed(const char* cname)
{
  int result = this->FindUniform(cname);

  if (result == -1 && !this->Linked)
  {
    svtkErrorMacro("attempt to find uniform when the shader program is not linked");
  }
  return (result != -1);
}

// ----------------------------------------------------------------------------
bool svtkShaderProgram::IsAttributeUsed(const char* cname)
{
  int result = this->FindAttributeArray(cname);

  if (result == -1 && !this->Linked)
  {
    svtkErrorMacro("attempt to find attribute when the shader program is not linked");
  }
  return (result != -1);
}

// ----------------------------------------------------------------------------
void svtkShaderProgram::PrintSelf(ostream& os, svtkIndent indent)
{
