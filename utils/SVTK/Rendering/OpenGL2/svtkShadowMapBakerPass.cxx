/*=========================================================================
  ...
  Module:    svtkShadowMapBakerPass.cxx
...
// only for svtkShadowMapBakerPass developers.
//#define SVTK_SHADOW_MAP_BAKER_PASS_DEBUG
//#define DONT_DUPLICATE_LIGHTS

svtkStandardNewMacro(svtkShadowMapBakerPass);
svtkCxxSetObjectMacro(svtkShadowMapBakerPass, OpaqueSequence, svtkRenderPass);
svtkCxxSetObjectMacro(svtkShadowMapBakerPass, CompositeZPass, svtkRenderPass);
...
void svtkShadowMapBakerPass::PointNearFar(
  double* v, double* pt, double* dir, double& mNear, double& mFar, bool initialized)
{
  ...
  diff[1] = v[1] - pt[1];
  diff[2] = v[2] - pt[2];
  double dot = svtkMath::Dot(diff, dir);
  if (initialized)
  {
    if (dot < mNear)
    {
...
void svtkShadowMapBakerPass::BoxNearFar(
...
// ----------------------------------------------------------------------------
svtkShadowMapBakerPass::svtkShadowMapBakerPass()
{
  svtkNew<svtkCameraPass> camP;
...
svtkShadowMapBakerPass::~svtkShadowMapBakerPass()
{
  ...
  {
    svtkErrorMacro(<< "ShadowMaps should have been deleted in ReleaseGraphicsResources().");
  }
  if (this->LightCameras != nullptr)
  {
    svtkErrorMacro(<< "LightCameras should have been deleted in ReleaseGraphicsResources().");
  }
}

// ----------------------------------------------------------------------------
void svtkShadowMapBakerPass::PrintSelf(ostream& os, svtkIndent indent)
{
...
// ----------------------------------------------------------------------------
bool svtkShadowMapBakerPass::GetHasShadows()
{
  return this->HasShadows;
}

// ----------------------------------------------------------------------------
bool svtkShadowMapBakerPass::LightCreatesShadow(svtkLight* l)
{
...
// ----------------------------------------------------------------------------
std::vector<svtkSmartPointer<svtkTextureObject> >* svtkShadowMapBakerPass::GetShadowMaps()
{
  return this->ShadowMaps;
}

// ----------------------------------------------------------------------------
std::vector<svtkSmartPointer<svtkCamera> >* svtkShadowMapBakerPass::GetLightCameras()
{
  return this->LightCameras;
}

// ----------------------------------------------------------------------------
bool svtkShadowMapBakerPass::GetNeedUpdate()
{
  return this->NeedUpdate;
}

// ----------------------------------------------------------------------------
void svtkShadowMapBakerPass::SetUpToDate()
{
...
// \pre s_exists: s!=0
void svtkShadowMapBakerPass::Render(const svtkRenderState* s)
{
  ...
  if (this->OpaqueSequence != nullptr)
  {
    ...
    svtkOpenGLState::ScopedglEnableDisable ssaver(ostate, GL_SCISSOR_TEST);
    ...
    svtkProp** propArray = nullptr;
    svtkMTimeType latestPropTime = 0;

    if (hasLight)
    {
      // at least one receiver?
      svtkCollectionSimpleIterator pit;
      svtkPropCollection* props = r->GetViewProps();
      props->InitTraversal(pit);
      svtkProp* p = props->GetNextProp(pit);
      propArray = new svtkProp*[props->GetNumberOfItems()];
      while (p != nullptr)
      {
        svtkMTimeType mTime = p->GetMTime();
    ...
    svtkCamera* realCamera = r->GetActiveCamera();
    svtkRenderState s2(r);
    if (this->NeedUpdate) // create or re-create the shadow maps.
    {
#ifdef SVTK_SHADOW_MAP_BAKER_PASS_DEBUG
      ...
      if (this->ShadowMaps == nullptr)
      {
        this->ShadowMaps = new std::vector<svtkSmartPointer<svtkTextureObject> >();
      ...
      if (this->LightCameras == nullptr)
      {
        this->LightCameras = new std::vector<svtkSmartPointer<svtkCamera> >();
      ...
      while (l != nullptr)
      {
        if (l->GetSwitch() && this->LightCreatesShadow(l))
        {
          svtkTextureObject* map = (*this->ShadowMaps)[this->CurrentLightIndex];
          if (map == nullptr)
          {
            map = svtkTextureObject::New();
            (*this->ShadowMaps)[this->CurrentLightIndex] = map;
            map->SetMagnificationFilter(svtkTextureObject::Linear);
            map->SetMinificationFilter(svtkTextureObject::Linear);
            map->SetWrapS(svtkTextureObject::ClampToEdge);
            map->SetWrapT(svtkTextureObject::ClampToEdge);
            map->SetWrapR(svtkTextureObject::ClampToEdge);
            map->SetContext(context);
            map->Allocate2D(this->Resolution, this->Resolution, 1, SVTK_FLOAT);
            map->Delete();
          }
          svtkCamera* lightCamera = (*this->LightCameras)[this->CurrentLightIndex];
          if (lightCamera == nullptr)
          {
            lightCamera = svtkOpenGLCamera::New();
...
//------------------------------------------------------------------------------
bool svtkShadowMapBakerPass::SetShaderParameters(svtkShaderProgram* program, svtkAbstractMapper*,
...
//------------------------------------------------------------------------------
bool svtkShadowMapBakerPass::PreReplaceShaderValues(
  std::string&, std::string&, std::string& fragmentShader, svtkAbstractMapper*, svtkProp*)
{
  svtkShaderProgram::Substitute(fragmentShader, "//SVTK::Light::Dec",
    "//SVTK::Light::Dec\n"
    "uniform float nearZ;\n"
    "uniform float farZ;\n"
...
void svtkShadowMapBakerPass::BuildCameraLight(svtkLight* light, double* bb, svtkCamera* lcamera)
{
  ...
  double vx[3], vup[3];
  svtkMath::Perpendiculars(dir, vx, vup, 0);
  double mNear, mFar;
...
// resources.
// \pre w_exists: w!=0
void svtkShadowMapBakerPass::ReleaseGraphicsResources(svtkWindow* w)
{
