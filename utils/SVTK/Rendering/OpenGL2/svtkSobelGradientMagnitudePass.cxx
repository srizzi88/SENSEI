/*=========================================================================
  ...
  Module:    svtkSobelGradientMagnitudePass.cxx
...
// only for svtkSobelGradientMagnitudePass developers.
//#define SVTK_SOBEL_PASS_DEBUG

#ifdef SVTK_SOBEL_BLUR_PASS_DEBUG
...
#include "svtkSobelGradientMagnitudePass1FS.h"
#include "svtkSobelGradientMagnitudePass2FS.h"
#include "svtkTextureObjectVS.h" // a pass through shader

svtkStandardNewMacro(svtkSobelGradientMagnitudePass);

// ----------------------------------------------------------------------------
svtkSobelGradientMagnitudePass::svtkSobelGradientMagnitudePass()
{
...
svtkSobelGradientMagnitudePass::~svtkSobelGradientMagnitudePass()
{
  ...
  if (this->Gx1 != nullptr)
  {
    svtkErrorMacro(<< "Gx1 should have been deleted in ReleaseGraphicsResources().");
  }
  if (this->Gy1 != nullptr)
  {
    svtkErrorMacro(<< "Gx1 should have been deleted in ReleaseGraphicsResources().");
...
// ----------------------------------------------------------------------------
void svtkSobelGradientMagnitudePass::PrintSelf(ostream& os, svtkIndent indent)
{
...
// \pre s_exists: s!=0
void svtkSobelGradientMagnitudePass::Render(const svtkRenderState* s)
{
  ...
  svtkRenderer* renderer = s->GetRenderer();
  ...
  bool fbo_support = svtkOpenGLFramebufferObject::IsSupported(context) != 0;
  bool texture_support = svtkTextureObject::IsSupported(context) != 0;
...
#ifdef SVTK_SOBEL_PASS_DEBUG
  // Save first pass in file for debugging.
  svtkPixelBufferObject* pbo = this->Pass1->Download();
  ...
  incs[0] = 0;
  incs[1] = 0;
  bool status = pbo->Download2D(SVTK_UNSIGNED_CHAR, openglRawData, dims, 4, incs);
  assert("check" && status);
  pbo->Delete();
  ...
  if (this->Gx1 == nullptr)
  {
    this->Gx1 = svtkTextureObject::New();
  ...
  {
    this->Gx1->Create2D(w, h, 4, SVTK_UNSIGNED_CHAR, false);
  }

  if (this->Gy1 == nullptr)
  {
    this->Gy1 = svtkTextureObject::New();
  ...
  {
    this->Gy1->Create2D(w, h, 4, SVTK_UNSIGNED_CHAR, false);
  }
#ifdef SVTK_SOBEL_PASS_DEBUG
  ...
  if (!this->Program1)
  {
    this->Program1 = new svtkOpenGLHelper;

    // build the shader source code
    std::string VSSource = svtkTextureObjectVS;
    std::string FSSource = svtkSobelGradientMagnitudePass1FS;
    std::string GSSource;

    // compile and bind it if needed
    svtkShaderProgram* newShader = context->GetShaderCache()->ReadyShaderProgram(
  ...
  {
    svtkErrorMacro("Couldn't build the shader program. At this point , it can be an error in a "
  ...
  status = pbo->Download2D(SVTK_UNSIGNED_CHAR, openglRawData, dims, 4, incs);
  ...
  rgbaToRgb = svtkImageExtractComponents::New();
  rgbaToRgb->SetInputConnection(importer->GetOutputPort());
  rgbaToRgb->SetComponents(0, 1, 2);
  ...
  if (!this->Program2)
  {
    this->Program2 = new svtkOpenGLHelper;

    // build the shader source code
    std::string VSSource = svtkTextureObjectVS;
    std::string FSSource = svtkSobelGradientMagnitudePass2FS;
    std::string GSSource;
...
// resources.
// \pre w_exists: w!=0
void svtkSobelGradientMagnitudePass::ReleaseGraphicsResources(svtkWindow* w)
{
