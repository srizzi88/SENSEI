...
=========================================================================*/
/**
 * @class   svtkStateStorage
 * @brief   Class to make storing and comparing state quick and easy
 *
 * svtkStateStorage is just a thin wrapper around std::vector<unsigned char>
...
#ifndef svtkStateStorage_h
#define svtkStateStorage_h
...
class SVTKRENDERINGOPENGL2_EXPORT svtkStateStorage
{
public:
  svtkStateStorage() {}
  ...
  bool operator!=(const svtkStateStorage& b) const
  {
  ...
  svtkStateStorage& operator=(const svtkStateStorage& b)
  {
...
private:
  svtkStateStorage(const svtkStateStorage&) = delete;
};

template <class T>
inline void svtkStateStorage::Append(const T& value, const char* name)
{
...
public:
  ...
  bool operator!=(const svtkStateStorage& b) const { return this->Storage != b.Storage; }
...
template <class T>
inline void svtkStateStorage::Append(const T& value, const char*)
{
  const char* start = reinterpret_cast<const char*>(&value);
  this->Storage.insert(this->Storage.end(), start, start + sizeof(T));
}
...
#endif // svtkStateStorage_h

// SVTK-HeaderTest-Exclude: svtkStateStorage.h
