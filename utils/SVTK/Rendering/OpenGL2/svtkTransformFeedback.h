/*=========================================================================
  ...
  Module:    svtkTransformFeedback.h
 ...
 * SVTK to capture vertex information during GL2PS export when using the OpenGL2
 * backend as a replacement for the deprecated OpenGL feedback buffer.
 */

#ifndef svtkTransformFeedback_h
#define svtkTransformFeedback_h
...
class SVTKRENDERINGOPENGL2_EXPORT svtkTransformFeedback : public svtkObject
{
public:
  static svtkTransformFeedback* New();
  svtkTypeMacro(svtkTransformFeedback, svtkObject);
   ...
   */
  svtkSetMacro(NumberOfVertices, size_t);
  void SetNumberOfVertices(int drawMode, size_t inputVerts);
  svtkGetMacro(NumberOfVertices, size_t);
  //@}
   ...
   * svtkSetMacro(BufferMode, int);
   * svtkGetMacro(BufferMode, int);
   */
   ...
   */
  void BindVaryings(svtkShaderProgram* prog);

  /**
   * Get the transform buffer object. Only valid after calling BindBuffer.
   */
  svtkOpenGLBufferObject* GetBuffer(int index);
   ...
   * BindBuffer.
   */
  svtkSetMacro(PrimitiveMode, int);
  svtkGetMacro(PrimitiveMode, int);
  //@}
...
protected:
  ...
  ~svtkTransformFeedback() override;

private:
  svtkTransformFeedback(const svtkTransformFeedback&) = delete;
  void operator=(const svtkTransformFeedback&) = delete;
  ...
  std::vector<svtkOpenGLBufferObject*> Buffers;
  int PrimitiveMode;
...
//------------------------------------------------------------------------------
inline size_t svtkTransformFeedback::GetBytesPerVertex(svtkTransformFeedback::VaryingRole role)
{
  switch (role)
  {
  ...
  svtkGenericWarningMacro("Unknown role enum value: " << role);
  return 0;
}

#endif // svtkTransformFeedback_h
