...
Module:    svtkWin32OpenGLRenderWindow.cxx
...
svtkStandardNewMacro(svtkWin32OpenGLRenderWindow);

svtkWin32OpenGLRenderWindow::svtkWin32OpenGLRenderWindow()
{
  ...
  this->StereoType = SVTK_STEREO_CRYSTAL_EYES;
  this->CursorHidden = 0;
...
//------------------------------------------------------------------------------
void svtkWin32OpenGLRenderWindow::Clean()
{
  ...
  if (this->OwnContext && this->ContextId)
  {
    ...
    {
      svtkErrorMacro("wglMakeCurrent failed in Clean(), error: " << GetLastError());
    }
    if (wglDeleteContext(this->ContextId) != TRUE)
    {
      svtkErrorMacro("wglDeleteContext failed in Clean(), error: " << GetLastError());
    }
  }
...
//------------------------------------------------------------------------------
void svtkWin32OpenGLRenderWindow::CleanUpRenderers()
{
...
//------------------------------------------------------------------------------
LRESULT APIENTRY svtkWin32OpenGLRenderWindow::WndProc(
  HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
  LRESULT res;

  svtkWin32OpenGLRenderWindow* me =
    (svtkWin32OpenGLRenderWindow*)svtkGetWindowLong(hWnd, sizeof(svtkLONG));
...
//------------------------------------------------------------------------------
void svtkWin32OpenGLRenderWindow::SetWindowName(const char* _arg)
{
...
//------------------------------------------------------------------------------
svtkTypeBool svtkWin32OpenGLRenderWindow::GetEventPending()
{
  MSG msg;
...
// ----------------------------------------------------------------------------
bool svtkWin32OpenGLRenderWindow::InitializeFromCurrentContext()
{
...
// ----------------------------------------------------------------------------
void svtkWin32OpenGLRenderWindow::MakeCurrent()
{
  ...
  if (this->ContextId != current)
  {
    if (wglMakeCurrent(this->DeviceContext, this->ContextId) != TRUE)
    {
      ...
      if (lpMsgBuf)
      {
        ...
        svtkErrorMacro(<< wcscat(wtemp, wmsg));
        delete[] wmsg;
        delete[] wtemp;
#else
        svtkErrorMacro("wglMakeCurrent failed in MakeCurrent(), error: " << (LPCTSTR)lpMsgBuf);
#endif
        ::LocalFree(lpMsgBuf);
      }
    }
...
//------------------------------------------------------------------------------
void svtkWin32OpenGLRenderWindow::PushContext()
{
  HGLRC current = wglGetCurrentContext();
...
//------------------------------------------------------------------------------
void svtkWin32OpenGLRenderWindow::PopContext()
{
  HGLRC current = wglGetCurrentContext();
...
bool svtkWin32OpenGLRenderWindow::IsCurrent()
{
  return this->ContextId != 0 && this->ContextId == wglGetCurrentContext();
}

//------------------------------------------------------------------------------
bool svtkWin32OpenGLRenderWindow::SetSwapControl(int i)
{
...
// ----------------------------------------------------------------------------
void svtkWin32OpenGLRenderWindow::SetSize(int width, int height)
{
...
//------------------------------------------------------------------------------
void svtkWin32OpenGLRenderWindow::SetPosition(int x, int y)
{
  static bool resizing = false;
...
void svtkWin32OpenGLRenderWindow::Frame(void)
{
  this->MakeCurrent();
  ...
  if (!this->AbortRender && this->DoubleBuffer && this->SwapBuffers)
  {
    ...
    if (this->DeviceContext && !this->UseOffScreenBuffers)
    {
      ...
      svtkDebugMacro(<< " SwapBuffers\n");
    }
  }
}

//------------------------------------------------------------------------------
void svtkWin32OpenGLRenderWindow::SVTKRegisterClass()
{
...
#endif
  {
    ...
    wndClass.lpfnWndProc = svtkWin32OpenGLRenderWindow::WndProc;
...
//------------------------------------------------------------------------------
svtkTypeBool svtkWin32OpenGLRenderWindow::IsDirect()
{
  this->MakeCurrent();
...
//------------------------------------------------------------------------------
const char* svtkWin32OpenGLRenderWindow::ReportCapabilities()
{
  this->MakeCurrent();
...
//------------------------------------------------------------------------------
void svtkWin32OpenGLRenderWindow::SetupPixelFormatPaletteAndContext(
  HDC hDC, DWORD dwFlags, int debug, int bpp, int zbpp)
{
  // Create a dummy window, needed for calling wglGetProcAddress.
#ifdef UNICODE
  HWND tempId = CreateWindow(L"svtkOpenGL", 0, 0, 0, 0, 1, 1, 0, 0, this->ApplicationInstance, 0);
#else
  HWND tempId = CreateWindow("svtkOpenGL", 0, 0, 0, 0, 1, 1, 0, 0, this->ApplicationInstance, 0);
#endif
  ...
  if (!m_valid)
  {
    svtkErrorMacro("GLEW could not be initialized.");
    return;
  }
  ...
  if (wglChoosePixelFormatARB)
  {
    ...
    else
    {
      ...
      {
        svtkGenericWarningMacro("No Stereo Available!");
  ...
  {
    svtkErrorMacro("failed to get wglChoosePixelFormatARB");
  }
  ...
  if (pixelFormat)
  {
    ...
    if (wglCreateContextAttribsARB)
    {
      ...
      if (this->ContextId)
      {
        ...
        if (iContextAttribs[1] == 3 && iContextAttribs[3] == 1)
        {
          ...
          {
            svtkErrorMacro("We have determined that your graphics system is"
                          " an Intel SandyBridge based system. These systems only partially "
                          " support SVTK. If you encounter any issues please make sure"
    ...
    {
      svtkErrorMacro("wglCreateContext failed in CreateAWindow(), error: " << GetLastError());
    }
  }
  ...
  if (!pixelFormat)
  {
    svtkErrorMacro("failed to get valid pixel format.");
  }
...
//------------------------------------------------------------------------------
void svtkWin32OpenGLRenderWindow::SetupPalette(HDC hDC)
{
...
//------------------------------------------------------------------------------
LRESULT svtkWin32OpenGLRenderWindow::MessageProc(
  HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
  switch (message)
  {
    ...
    case WM_SETCURSOR:
      ...
      break;
    default:
      this->InvokeEvent(svtkCommand::RenderWindowMessageEvent, &message);
      break;
  }
...
//------------------------------------------------------------------------------
void svtkWin32OpenGLRenderWindow::InitializeApplication()
{
...
//------------------------------------------------------------------------------
void svtkWin32OpenGLRenderWindow::SetShowWindow(bool val)
{
...
//------------------------------------------------------------------------------
void svtkWin32OpenGLRenderWindow::CreateAWindow()
{
  this->SVTKRegisterClass();
  ...
  if (this->WindowIdReferenceCount == 0)
  {
    ...
    if (!this->WindowId)
    {
...
#ifdef UNICODE
        this->WindowId =
          CreateWindow(L"svtkOpenGL", wname, WS_CHILD | WS_CLIPCHILDREN /*| WS_CLIPSIBLINGS*/, x, y,
            width, height, this->ParentId, nullptr, this->ApplicationInstance, nullptr);
#else
        this->WindowId = CreateWindow("svtkOpenGL", this->WindowName,
...
#else
        this->WindowId = CreateWindow("svtkOpenGL", this->WindowName, style, x, y, r.right - r.left,
          r.bottom - r.top, nullptr, nullptr, this->ApplicationInstance, nullptr);
#endif
      }
#ifdef UNICODE
      ...
      if (!this->WindowId)
      {
        svtkErrorMacro("Could not create window, error:  " << GetLastError());
        return;
      }
...
void svtkWin32OpenGLRenderWindow::WindowInitialize()
{
...
void svtkWin32OpenGLRenderWindow::Initialize(void)
{
  ...
  {
    svtkWin32OpenGLRenderWindow* renWin =
      svtkWin32OpenGLRenderWindow::SafeDownCast(this->SharedRenderWindow);
...
//------------------------------------------------------------------------------
void svtkWin32OpenGLRenderWindow::Finalize(void)
{
...
//------------------------------------------------------------------------------
void svtkWin32OpenGLRenderWindow::DestroyWindow()
{
  if (this->WindowIdReferenceCount > 0)
  {
    --this->WindowIdReferenceCount;
    if (this->WindowIdReferenceCount == 0)
    {
      ...
      if (this->WindowId)
      {
        ...
        svtkSetWindowLong(this->WindowId, sizeof(svtkLONG), (svtkLONG)0);
        if (this->OwnWindow)
        {
...
int* svtkWin32OpenGLRenderWindow::GetSize(void)
{
...
int* svtkWin32OpenGLRenderWindow::GetScreenSize(void)
{
...
int* svtkWin32OpenGLRenderWindow::GetPosition(void)
{
...
void svtkWin32OpenGLRenderWindow::SetFullScreen(svtkTypeBool arg)
{
...
//
void svtkWin32OpenGLRenderWindow::SetStereoCapableWindow(svtkTypeBool capable)
{
  if (this->ContextId == 0)
  {
...
void svtkWin32OpenGLRenderWindow::PrefFullScreen()
{
...
//------------------------------------------------------------------------------
// Remap the window.
void svtkWin32OpenGLRenderWindow::WindowRemap()
{
...
//------------------------------------------------------------------------------
void svtkWin32OpenGLRenderWindow::PrintSelf(ostream& os, svtkIndent indent)
{
...
//------------------------------------------------------------------------------
// Get the window id.
HWND svtkWin32OpenGLRenderWindow::GetWindowId()
{
  svtkDebugMacro(<< "Returning WindowId of " << this->WindowId << "\n");
...
void svtkWin32OpenGLRenderWindow::SetWindowId(HWND arg)
{
  svtkDebugMacro(<< "Setting WindowId to " << arg << "\n");
...
void svtkWin32OpenGLRenderWindow::SetWindowInfo(const char* info)
{
  int tmp;
  ...
  svtkDebugMacro(<< "Setting WindowId to " << this->WindowId << "\n");
}

//------------------------------------------------------------------------------
void svtkWin32OpenGLRenderWindow::SetNextWindowInfo(const char* info)
{
  int tmp;
...
//------------------------------------------------------------------------------
void svtkWin32OpenGLRenderWindow::SetDisplayId(void* arg)
{
  this->DeviceContext = (HDC)arg;
}

//------------------------------------------------------------------------------
void svtkWin32OpenGLRenderWindow::SetContextId(HGLRC arg)
{
  this->ContextId = arg;
}

//------------------------------------------------------------------------------
void svtkWin32OpenGLRenderWindow::SetDeviceContext(HDC arg)
{
...
void svtkWin32OpenGLRenderWindow::SetParentInfo(const char* info)
{
  int tmp;
  ...
  svtkDebugMacro(<< "Setting ParentId to " << this->ParentId << "\n");
}

//------------------------------------------------------------------------------
// Set the window id to a pre-existing window.
void svtkWin32OpenGLRenderWindow::SetParentId(HWND arg)
{
  svtkDebugMacro(<< "Setting ParentId to " << arg << "\n");
...
void svtkWin32OpenGLRenderWindow::SetNextWindowId(HWND arg)
{
  svtkDebugMacro(<< "Setting NextWindowId to " << arg << "\n");

  this->NextWindowId = arg;
}

//------------------------------------------------------------------------------
void svtkWin32OpenGLRenderWindow::SetNextWindowId(void* arg)
{
  this->SetNextWindowId((HWND)arg);
}

//----------------------------------------------------------------------------
void svtkWin32OpenGLRenderWindow::HideCursor()
{
  if (this->CursorHidden)
  {
    return;
...
//----------------------------------------------------------------------------
void svtkWin32OpenGLRenderWindow::ShowCursor()
{
  if (!this->CursorHidden)
  {
    return;
...
//----------------------------------------------------------------------------
void svtkWin32OpenGLRenderWindow::SetCursorPosition(int x, int y)
{
...
//----------------------------------------------------------------------------
void svtkWin32OpenGLRenderWindow::SetCurrentCursor(int shape)
{
...
//----------------------------------------------------------------------------
bool svtkWin32OpenGLRenderWindow::DetectDPI()
{
