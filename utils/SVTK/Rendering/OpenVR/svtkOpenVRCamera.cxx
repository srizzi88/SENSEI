...
svtkStandardNewMacro(svtkOpenVRCamera);

svtkOpenVRCamera::svtkOpenVRCamera()
{
  this->LeftEyeProjection = 0;
  this->RightEyeProjection = 0;

  this->LeftEyeTCDCMatrix = svtkMatrix4x4::New();
  this->RightEyeTCDCMatrix = svtkMatrix4x4::New();
...
void svtkOpenVRCamera::GetHMDEyePoses(svtkRenderer* ren)
{
  svtkOpenVRRenderWindow* win = svtkOpenVRRenderWindow::SafeDownCast(ren->GetRenderWindow());
...
void svtkOpenVRCamera::GetHMDEyeProjections(svtkRenderer* ren)
{
  svtkOpenVRRenderWindow* win = svtkOpenVRRenderWindow::SafeDownCast(ren->GetRenderWindow());
...
void svtkOpenVRCamera::ApplyEyePose(svtkOpenVRRenderWindow* win, bool left, double factor)
{
...
void svtkOpenVRCamera::Render(svtkRenderer* ren)
{
  ...
  if (!this->LeftEyeProjection)
  {
    this->LeftEyeProjection = svtkMatrix4x4::New();
    this->RightEyeProjection = svtkMatrix4x4::New();
  ...
  if (this->LeftEye)
  {
    ...
    {
      ostate->svtkglEnable(GL_MULTISAMPLE);
    }
  ...
  ostate->svtkglViewport(0, 0, renSize[0], renSize[1]);
  ostate->svtkglScissor(0, 0, renSize[0], renSize[1]);
  ren->Clear();
...
void svtkOpenVRCamera::GetKeyMatrices(svtkRenderer* ren, svtkMatrix4x4*& wcvc, svtkMatrix3x3*& normMat,
  svtkMatrix4x4*& vcdc, svtkMatrix4x4*& wcdc)
{
  ...
  {
    svtkMatrix4x4* w2v = this->GetModelViewTransformMatrix();
    ...
    if (this->LeftEye)
    {
      ...
      svtkMatrix4x4::Multiply4x4(this->WCVCMatrix, this->LeftEyeProjection, this->WCDCMatrix);
      ...
      svtkMatrix4x4::Multiply4x4(this->LeftEyeTCDCMatrix, this->WCDCMatrix, this->LeftEyeTCDCMatrix);
    }
    else
    {
      svtkMatrix4x4::Multiply4x4(this->WCVCMatrix, this->RightEyeProjection, this->WCDCMatrix);
...
void svtkOpenVRCamera::GetTrackingToDCMatrix(svtkMatrix4x4*& tcdc)
{
  if (this->LeftEye)
  {
...
void svtkOpenVRCamera::PrintSelf(ostream& os, svtkIndent indent)
{
