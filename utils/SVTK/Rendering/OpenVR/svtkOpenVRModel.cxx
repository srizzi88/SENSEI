...
svtkOpenVRModel
=========================================================================*/
svtkStandardNewMacro(svtkOpenVRModel);

svtkOpenVRModel::svtkOpenVRModel()
{
  this->RawModel = nullptr;
  this->RawTexture = nullptr;
  this->Loaded = false;
  this->ModelVBO = svtkOpenGLVertexBufferObject::New();
...
//----------------------------------------------------------------------------
void svtkOpenVRModel::PrintSelf(ostream& os, svtkIndent indent)
{
...
void svtkOpenVRModel::ReleaseGraphicsResources(svtkWindow* win)
{
...
bool svtkOpenVRModel::Build(svtkOpenVRRenderWindow* win)
{
  this->ModelVBO->Upload(
    this->RawModel->rVertexData, this->RawModel->unVertexCount, svtkOpenGLBufferObject::ArrayBuffer);

  this->ModelHelper.IBO->Upload(this->RawModel->rIndexData, this->RawModel->unTriangleCount * 3,
    svtkOpenGLBufferObject::ElementArrayBuffer);
  ...
  svtkShaderProgram* program = this->ModelHelper.Program;
  this->ModelHelper.VAO->Bind();
  if (!this->ModelHelper.VAO->AddAttributeArray(program, this->ModelVBO, "position",
        offsetof(vr::RenderModel_Vertex_t, vPosition), sizeof(vr::RenderModel_Vertex_t), SVTK_FLOAT,
        3, false))
  {
  ...
  if (!this->ModelHelper.VAO->AddAttributeArray(program, this->ModelVBO, "v2TexCoordsIn",
        ...
        SVTK_FLOAT, 2, false))
  {
    svtkErrorMacro(<< "Error setting tcoords in shader VAO.");
  }
...
void svtkOpenVRModel::Render(svtkOpenVRRenderWindow* win, const vr::TrackedDevicePose_t& pose)
{
  ...
  if (this->RawModel && !this->RawTexture)
  {
    ...
    {
      svtkErrorMacro(<< "Unable to load render texture for render model " << this->GetName());
    }

    // if this call succeeded and we have the texture
    // then build the SVTK structures
    if (this->RawTexture)
    {
      if (!this->Build(win))
      {
        svtkErrorMacro("Unable to create GL model from render model " << this->GetName());
      }
...
void svtkOpenVRModel::SetShowRay(bool v)
{
  this->Ray->SetShow(v);
}

void svtkOpenVRModel::SetRayLength(double length)
{
  this->Ray->SetLength(length);
}

void svtkOpenVRModel::SetRayColor(double r, double g, double b)
{
  float color[] = { r, g, b };
