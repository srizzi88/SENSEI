/*=========================================================================
  ...
  Module:    svtkCompressCompositer.cxx
...
svtkStandardNewMacro(svtkCompressCompositer);
...
//-------------------------------------------------------------------------
svtkCompressCompositer::svtkCompressCompositer()
{
...
template <class P>
int svtkCompressCompositerCompress(float* zIn, P* pIn, float* zOut, P* pOut, int numPixels)
{
  float* endZ;
  int length = 0;
  int compressCount;
...
// We could easily compress inplace, but it works out better for buffer
// management if we do not.  zIn == zOut is allowed....
void svtkCompressCompositer::Compress(
  svtkFloatArray* zIn, svtkDataArray* pIn, svtkFloatArray* zOut, svtkDataArray* pOut)
{
  ...
  svtkTimerLog::MarkStartEvent("Compress");

  // This is just a complex switch statement
  // to call the correct templated function.
  if (pIn->GetDataType() == SVTK_UNSIGNED_CHAR)
  {
    if (pIn->GetNumberOfComponents() == 3)
    {
      length = svtkCompressCompositerCompress(pzf1, reinterpret_cast<svtkCharRGBType*>(ppv1), pzf2,
        reinterpret_cast<svtkCharRGBType*>(ppv2), totalPixels);
    }
    else if (pIn->GetNumberOfComponents() == 4)
    {
      length = svtkCompressCompositerCompress(pzf1, reinterpret_cast<svtkCharRGBAType*>(ppv1), pzf2,
        reinterpret_cast<svtkCharRGBAType*>(ppv2), totalPixels);
    }
    else
    {
      svtkGenericWarningMacro("Pixels have unexpected number of components.");
      return;
    }
  }
  else if (pIn->GetDataType() == SVTK_FLOAT && pIn->GetNumberOfComponents() == 4)
  {
    length = svtkCompressCompositerCompress(pzf1, reinterpret_cast<svtkFloatRGBAType*>(ppv1), pzf2,
      reinterpret_cast<svtkFloatRGBAType*>(ppv2), totalPixels);
  }
  else
  {
    svtkGenericWarningMacro("Unexpected pixel type.");
    return;
  }
  ...
  svtkTimerLog::MarkEndEvent("Compress");
}
...
template <class P>
void svtkCompressCompositerUncompress(float* zIn, P* pIn, float* zOut, P* pOut, int lengthIn)
{
  float* endZ;
  int count;
  P background;
...
// We could easily compress inplace, but it works out better for buffer
// management if we do not.  zIn == zOut is allowed....
void svtkCompressCompositer::Uncompress(
  svtkFloatArray* zIn, svtkDataArray* pIn, svtkFloatArray* zOut, svtkDataArray* pOut, int lengthOut)
{
  float* pzf1 = zIn->GetPointer(0);
  float* pzf2 = zOut->GetPointer(0);
  void* ppv1 = pIn->GetVoidPointer(0);
  ...
  svtkTimerLog::MarkStartEvent("Uncompress");
  ...
  if (pIn->GetDataType() == SVTK_UNSIGNED_CHAR)
  {
    if (pIn->GetNumberOfComponents() == 3)
    {
      svtkCompressCompositerUncompress(pzf1, reinterpret_cast<svtkCharRGBType*>(ppv1), pzf2,
        reinterpret_cast<svtkCharRGBType*>(ppv2), lengthIn);
    }
    else if (pIn->GetNumberOfComponents() == 4)
    {
      svtkCompressCompositerUncompress(pzf1, reinterpret_cast<svtkCharRGBAType*>(ppv1), pzf2,
        reinterpret_cast<svtkCharRGBAType*>(ppv2), lengthIn);
    }
    else
    {
      svtkGenericWarningMacro("Pixels have unexpected number of components.");
  ...
  {
    svtkCompressCompositerUncompress(pzf1, reinterpret_cast<svtkFloatRGBAType*>(ppv1), pzf2,
      reinterpret_cast<svtkFloatRGBAType*>(ppv2), lengthIn);
  }
  else
  {
    svtkGenericWarningMacro("Unexpected pixel type.");
  ...
  svtkTimerLog::MarkEndEvent("Uncompress");
}
...
// z values above 1.0 mean: Repeat background for that many pixels.
template <class P>
int svtkCompressCompositerCompositePair(
...
// Can handle compositing compressed buffers.
// z values above 1.0 mean: Repeat background for that many pixels.
void svtkCompressCompositer::CompositeImagePair(svtkFloatArray* localZ, svtkDataArray* localP,
  svtkFloatArray* remoteZ, svtkDataArray* remoteP, svtkFloatArray* outZ, svtkDataArray* outP)
{
  ...
  // svtkTimerLog::MarkStartEvent("Coomposite Image Pair");

  // This is just a complex switch statement
  // to call the correct templated function.
  if (localP->GetDataType() == SVTK_UNSIGNED_CHAR)
  {
    if (localP->GetNumberOfComponents() == 3)
    {
      l3 = svtkCompressCompositerCompositePair(z1, reinterpret_cast<svtkCharRGBType*>(p1), z2,
        reinterpret_cast<svtkCharRGBType*>(p2), z3, reinterpret_cast<svtkCharRGBType*>(p3), length1);
    }
    else if (localP->GetNumberOfComponents() == 4)
    {
      l3 = svtkCompressCompositerCompositePair(z1, reinterpret_cast<svtkCharRGBAType*>(p1), z2,
        reinterpret_cast<svtkCharRGBAType*>(p2), z3, reinterpret_cast<svtkCharRGBAType*>(p3),
        length1);
    }
    else
    {
  ...
  else if (localP->GetDataType() == SVTK_FLOAT && localP->GetNumberOfComponents() == 4)
  {
    l3 = svtkCompressCompositerCompositePair(z1, reinterpret_cast<svtkFloatRGBAType*>(p1), z2,
      reinterpret_cast<svtkFloatRGBAType*>(p2), z3, reinterpret_cast<svtkFloatRGBAType*>(p3),
      length1);
  }
  else
  {
  ...
  // svtkTimerLog::MarkEndEvent("Coomposite Image Pair");
}

#define svtkTCPow2(j) (1 << (j))

//----------------------------------------------------------------------------
inline int svtkTCLog2(int j, int& exact)
{
  int counter = 0;
  exact = 1;
  while (j)
...
//----------------------------------------------------------------------------
void svtkCompressCompositer::CompositeBuffer(
  svtkDataArray* pBuf, svtkFloatArray* zBuf, svtkDataArray* pTmp, svtkFloatArray* zTmp)
{
  ...
  int i, id;
  int exactLog;
  int logProcs = svtkTCLog2(numProcs, exactLog);
  int uncompressedLength = zBuf->GetNumberOfTuples();
  int bufSize = 0;
  int numComps = pBuf->GetNumberOfComponents();
  svtkDataArray *p1, *p2, *p3;
  svtkFloatArray *z1, *z2, *z3;
    ...
    this->InternalPData->GetSize() < pBuf->GetSize())
  {
    if (this->InternalPData)
    {
      svtkCompositer::DeleteArray(this->InternalPData);
      this->InternalPData = nullptr;
    }
    if (pBuf->GetDataType() == SVTK_UNSIGNED_CHAR)
    {
      this->InternalPData = svtkUnsignedCharArray::New();
      svtkCompositer::ResizeUnsignedCharArray(
        static_cast<svtkUnsignedCharArray*>(this->InternalPData), numComps, pBuf->GetSize());
    }
    else
    {
      this->InternalPData = svtkFloatArray::New();
      svtkCompositer::ResizeFloatArray(
        static_cast<svtkFloatArray*>(this->InternalPData), numComps, pBuf->GetSize());
    }
  }
  // Now float array.
  if (this->InternalZData == nullptr || this->InternalZData->GetSize() < zBuf->GetSize())
  {
    if (this->InternalZData)
    {
      svtkCompositer::DeleteArray(this->InternalZData);
      this->InternalZData = nullptr;
    }
    this->InternalZData = svtkFloatArray::New();
    svtkCompositer::ResizeFloatArray(
      static_cast<svtkFloatArray*>(this->InternalZData), 1, zBuf->GetSize());
  }
...
#ifdef MPIPROALLOC
  svtkCommunicator::SetUseCopy(0);
#endif
  for (i = 0; i < logProcs; i++)
  {
    if ((myId % (int)svtkTCPow2(i)) == 0)
    { // Find participants
      if ((myId % (int)svtkTCPow2(i + 1)) < svtkTCPow2(i))
      {
        // receivers
        id = myId + svtkTCPow2(i);
        ...
        if (id < numProcs)
        {
          ...
          this->Controller->Receive(&bufSize, 1, id, 98);
          if (pTmp->GetDataType() == SVTK_UNSIGNED_CHAR)
          {
      ...
      { // The current data is always in buffer 1.
        ...
        if (id < numProcs)
        {
          ...
          this->Controller->Send(&bufSize, 1, id, 98);
          if (p1->GetDataType() == SVTK_UNSIGNED_CHAR)
          {
...
//----------------------------------------------------------------------------
void svtkCompressCompositer::PrintSelf(ostream& os, svtkIndent indent)
{
