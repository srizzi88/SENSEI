/*=========================================================================
  ...
  Module:    svtkSynchronizedRenderers.cxx
...
//----------------------------------------------------------------------------
class svtkSynchronizedRenderers::svtkObserver : public svtkCommand
{
public:
  ...
  svtkSynchronizedRenderers* Target;
};

svtkStandardNewMacro(svtkSynchronizedRenderers);
svtkCxxSetObjectMacro(svtkSynchronizedRenderers, ParallelController, svtkMultiProcessController);
svtkCxxSetObjectMacro(svtkSynchronizedRenderers, CaptureDelegate, svtkSynchronizedRenderers);
//----------------------------------------------------------------------------
svtkSynchronizedRenderers::svtkSynchronizedRenderers()
  ...
  , FixBackground(false)
{
  this->Observer = svtkSynchronizedRenderers::svtkObserver::New();
...
svtkSynchronizedRenderers::~svtkSynchronizedRenderers()
{
  ...
  // svtkOpenGLFXAAFilter is only available on opengl2:
  if (this->FXAAFilter)
  {
...
//----------------------------------------------------------------------------
void svtkSynchronizedRenderers::SetRenderer(svtkRenderer* renderer)
{
  if (this->Renderer != renderer)
  {
    ...
    {
      svtkErrorMacro("Received non OpenGL renderer");
      assert(false);
    }

    svtkSetObjectBodyMacro(Renderer, svtkOpenGLRenderer, glRenderer);
    if (this->Renderer)
    {
      this->Renderer->AddObserver(svtkCommand::StartEvent, this->Observer);
      this->Renderer->AddObserver(svtkCommand::EndEvent, this->Observer);
      // this->Renderer->AddObserver(svtkCommand::AbortCheckEvent, this->Observer);
    }
  }
}

//----------------------------------------------------------------------------
void svtkSynchronizedRenderers::HandleStartRender()
{
...
//----------------------------------------------------------------------------
void svtkSynchronizedRenderers::MasterStartRender()
{
...
//----------------------------------------------------------------------------
void svtkSynchronizedRenderers::SlaveStartRender()
{
  svtkMultiProcessStream stream;
  this->ParallelController->Broadcast(stream, this->RootProcessId);
...
//----------------------------------------------------------------------------
void svtkSynchronizedRenderers::HandleEndRender()
{
  if (this->CaptureDelegate && this->CaptureDelegate->GetAutomaticEventHandling() == false)
  {
...
//----------------------------------------------------------------------------
void svtkSynchronizedRenderers::MasterEndRender() {}

//----------------------------------------------------------------------------
void svtkSynchronizedRenderers::SlaveEndRender() {}

//----------------------------------------------------------------------------
svtkSynchronizedRenderers::svtkRawImage& svtkSynchronizedRenderers::CaptureRenderedImage()
{
...
//----------------------------------------------------------------------------
void svtkSynchronizedRenderers::PushImageToScreen()
{
  svtkRawImage& rawImage = this->Image;
  if (!rawImage.IsValid())
  {
  ...
  svtkHardwareSelector* sel = this->Renderer->GetSelector();
  if (sel)
  {
...
////----------------------------------------------------------------------------
// void svtkSynchronizedRenderers::ResetCamera()
//{
//  if (!this->ParallelController)
//    {
//    svtkErrorMacro("No controller set.");
//    return;
//    }
//
//  if (this->ParallelController->GetLocalProcessId() == this->RootProcessId)
...
//----------------------------------------------------------------------------
void svtkSynchronizedRenderers::CollectiveExpandForVisiblePropBounds(double bounds[6])
{
  // get local bounds.
  ...
  if (this->ParallelController->IsA("svtkMPIController"))
  {
    ...
    this->ParallelController->AllReduce(min_bounds, min_result, 3, svtkCommunicator::MIN_OP);
    this->ParallelController->AllReduce(max_bounds, max_result, 3, svtkCommunicator::MAX_OP);
  ...
  {
    // since svtkSocketController does not support such reduction operation, we
...
//----------------------------------------------------------------------------
void svtkSynchronizedRenderers::PrintSelf(ostream& os, svtkIndent indent)
{
...
// ** INFO OBJECT METHODS ***
//----------------------------------------------------------------------------
void svtkSynchronizedRenderers::RendererInfo::Save(svtkMultiProcessStream& stream)
{
...
//----------------------------------------------------------------------------
bool svtkSynchronizedRenderers::RendererInfo::Restore(svtkMultiProcessStream& stream)
{
  int tag;
  stream >> tag;
  if (tag != 1023)
...
//----------------------------------------------------------------------------
void svtkSynchronizedRenderers::RendererInfo::CopyFrom(svtkRenderer* ren)
{
  ...
  svtkMatrix4x4* eyeTransformationMatrix = cam->GetEyeTransformMatrix();
  svtkMatrix4x4* modelTransformationMatrix = cam->GetModelTransformMatrix();
  for (int i = 0; i < 4; ++i)
  {
    for (int j = 0; j < 4; ++j)
    {
...
//----------------------------------------------------------------------------
void svtkSynchronizedRenderers::RendererInfo::CopyTo(svtkRenderer* ren)
{
  svtkCamera* cam = ren->GetActiveCamera();
  ...
  // We reuse svtkMatrix4x4 objects present on the camera and then use
  // svtkMatrix4x4::SetElement(). This avoids modifying the mtime of the
...
//****************************************************************************
// svtkSynchronizedRenderers::svtkRawImage Methods
//****************************************************************************

//----------------------------------------------------------------------------
void svtkSynchronizedRenderers::svtkRawImage::Initialize(int dx, int dy, svtkUnsignedCharArray* data)
{
  this->Data = data;
  this->Size[0] = dx;
  this->Size[1] = dy;
...
//----------------------------------------------------------------------------
void svtkSynchronizedRenderers::svtkRawImage::Allocate(int dx, int dy, int numcomps)
{
...
//----------------------------------------------------------------------------
void svtkSynchronizedRenderers::svtkRawImage::SaveAsPNG(const char* filename)
{
  if (!this->IsValid())
  {
    svtkGenericWarningMacro("Image is not valid. Cannot save PNG.");
    return;
  }

  svtkImageData* img = svtkImageData::New();
  img->SetDimensions(this->Size[0], this->Size[1], 1);
  img->AllocateScalars(SVTK_UNSIGNED_CHAR, this->Data->GetNumberOfComponents());
...
//----------------------------------------------------------------------------
bool svtkSynchronizedRenderers::svtkRawImage::PushToViewport(svtkRenderer* ren)
{
  if (!this->IsValid())
  {
    svtkGenericWarningMacro("Image not valid. Cannot push to screen.");
    return false;
  }

  int size[2], low_point[2];
  ren->GetTiledSizeAndOrigin(&size[0], &size[1], &low_point[0], &low_point[1]);
  svtkLogF(TRACE, "GetTiledSizeAndOrigin(w=%d, h=%d, x=%d, y=%d)", size[0], size[1], low_point[0],
    low_point[1]);
  if (size[0] <= 0 || size[1] <= 0)
  {
    svtkGenericWarningMacro("Viewport empty. Cannot push to screen.");
    return false;
  }

  svtkOpenGLState* ostate = static_cast<svtkOpenGLRenderWindow*>(ren->GetSVTKWindow())->GetState();
  ostate->svtkglEnable(GL_SCISSOR_TEST);
  ostate->svtkglViewport(low_point[0], low_point[1], size[0], size[1]);
  ostate->svtkglScissor(low_point[0], low_point[1], size[0], size[1]);
...
//----------------------------------------------------------------------------
bool svtkSynchronizedRenderers::svtkRawImage::PushToFrameBuffer(svtkRenderer* ren)
{
  if (!this->IsValid())
  {
    svtkGenericWarningMacro("Image not valid. Cannot push to screen.");
  ...
  svtkOpenGLRenderUtilities::MarkDebugEvent("svtkRawImage::PushToViewport begin");
  ...
  ostate->svtkglBlendFuncSeparate(GL_ONE, GL_ONE_MINUS_SRC_ALPHA, GL_ONE, GL_ONE_MINUS_SRC_ALPHA);
  ...
  renWin->DrawPixels(low_point[0], low_point[1], low_point[0] + size[0] - 1,
    ...
    this->GetHeight(), this->Data->GetNumberOfComponents(), SVTK_UNSIGNED_CHAR,
    this->GetRawPtr()->GetVoidPointer(0));

  svtkOpenGLStaticCheckErrorMacro("failed after PushToFrameBuffer");
  svtkOpenGLRenderUtilities::MarkDebugEvent("svtkRawImage::PushToViewport end");
  return true;
}

//----------------------------------------------------------------------------
bool svtkSynchronizedRenderers::svtkRawImage::Capture(svtkRenderer* ren)
{
  double viewport[4];
  ...
  svtkHardwareSelector* sel = ren->GetSelector();
  if (sel)
  {
...
svtkRenderer* svtkSynchronizedRenderers::GetRenderer()
{
  svtkDebugMacro(<< this->GetClassName() << " (" << this << "): returning Render of "
                << this->Renderer);
