...
svtkStandardNewMacro(svtkTreeCompositer);

//-------------------------------------------------------------------------
svtkTreeCompositer::svtkTreeCompositer() {}
...
void svtkCompositeImagePair(
  svtkFloatArray* localZ, svtkDataArray* localP, svtkFloatArray* remoteZ, svtkDataArray* remoteP)
{
  int i, j;
  int pixel_data_size;
  float* pEnd;
...
static inline int svtkTCLog2(int j, int& exact)
{
  int counter = 0;
  exact = 1;
  while (j)
...
void svtkTreeCompositer::CompositeBuffer(
    ...
    { // Find participants
      if ((myId % (int)svtkTCPow2(i + 1)) < svtkTCPow2(i))
      {
        ...
        if (id < numProcs)
        {
          ...
          svtkCompositeImagePair(zBuf, pBuf, zTmp, pTmp);
        }
      }
      else
      {
...
void svtkTreeCompositer::PrintSelf(ostream& os, svtkIndent indent)
{
