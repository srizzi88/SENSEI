...
Module:    svtkQtTreeRingLabelMapper.cxx
...
svtkStandardNewMacro(svtkQtTreeRingLabelMapper);

svtkCxxSetObjectMacro(svtkQtTreeRingLabelMapper, LabelTextProperty, svtkTextProperty);

svtkQtTreeRingLabelMapper::svtkQtTreeRingLabelMapper()
{
  this->Input = nullptr;
  ...
  this->TextureMapToPlane = svtkTextureMapToPlane::New();
  this->polyDataMapper = svtkPolyDataMapper2D::New();

  this->QtImageSource = svtkQImageToImageSource::New();
  this->LabelTexture = svtkTexture::New();
...
//----------------------------------------------------------------------------
void svtkQtTreeRingLabelMapper::RenderOverlay(svtkViewport* viewport, svtkActor2D* actor)
{
...
//----------------------------------------------------------------------------
void svtkQtTreeRingLabelMapper::RenderOpaqueGeometry(svtkViewport* viewport, svtkActor2D* actor)
{
  if (!QApplication::instance())
  {
    svtkErrorMacro("This class requires a QApplication instance.");
    return;
  }

  svtkTextProperty* tprop = this->LabelTextProperty;
  if (!tprop)
  {
  ...
  svtkDataArray *numericData, *sectorInfo;
  ...
  if (!sectorInfo)
  {
    svtkErrorMacro(<< "Input Tree does not have sector information.");
    return;
  }

  svtkRenderer* renderer = svtkRenderer::SafeDownCast(viewport);
  ...
  SVTK_CREATE(svtkQImageToImageSource, qis);
...
void svtkQtTreeRingLabelMapper::LabelTree(svtkTree* tree, svtkDataArray* sectorInfo,
  svtkDataArray* numericData, svtkStringArray* stringData, svtkUnicodeStringArray* uStringData,
  int activeComp, int numComps, svtkViewport* viewport)
{
  delete this->QtImage;
  this->QtImage = new QImage(this->WindowSize[0], this->WindowSize[1], QImage::Format_ARGB32);

  char string[1024];
  svtkIdType i, root = tree->GetRoot();
  if (root < 0)
  {
  ...
  svtkDataArray* TextRotationArray = tree->GetVertexData()->GetArray(this->TextRotationArrayName);
  ...
  for (i = 0; i < tree->GetNumberOfVertices(); i++)
  {
    ...
    // svtk has been using the text baseline as the starting point, so
...
bool svtkQtTreeRingLabelMapper::PointInWindow(
  double* sinfo, double* newDim, double* textPosDC, svtkViewport* viewport)
{
  double r = (0.5 * (sinfo[3] - sinfo[2])) + sinfo[2];
  double theta = sinfo[0] + (0.5 * (sinfo[1] - sinfo[0]));
  double x = r * cos(svtkMath::RadiansFromDegrees(theta));
  double y = r * sin(svtkMath::RadiansFromDegrees(theta));
  ...
  double xlc = sinfo[2] * cos(svtkMath::RadiansFromDegrees(sinfo[0]));
  double xuc = sinfo[3] * cos(svtkMath::RadiansFromDegrees(sinfo[0]));
  double ylc = sinfo[2] * sin(svtkMath::RadiansFromDegrees(sinfo[0]));
  double yuc = sinfo[3] * sin(svtkMath::RadiansFromDegrees(sinfo[0]));
  ...
  double widthWC = r * svtkMath::RadiansFromDegrees(sinfo[1] - sinfo[0]);
...
//----------------------------------------------------------------------------
void svtkQtTreeRingLabelMapper::SetSectorsArrayName(const char* name)
{
  this->SetInputArrayToProcess(0, 0, 0, svtkDataObject::FIELD_ASSOCIATION_VERTICES, name);
}

svtkTree* svtkQtTreeRingLabelMapper::GetInputTree()
{
  return svtkTree::SafeDownCast(this->GetExecutive()->GetInputData(0, 0));
}

void svtkQtTreeRingLabelMapper::PrintSelf(ostream& os, svtkIndent indent)
{
...
void svtkQtTreeRingLabelMapper::GetVertexLabel(svtkIdType vertex, svtkDataArray* numericData,
  svtkStringArray* stringData, svtkUnicodeStringArray* uStringData, int activeComp, int numComp,
...
svtkMTimeType svtkQtTreeRingLabelMapper::GetMTime()
{
  svtkMTimeType filterMTime = this->MTime.GetMTime();
  if (this->Renderer)
  {
    svtkRenderWindow* rw = this->Renderer->GetRenderWindow();
    if (rw)
    {
      svtkMTimeType renWindMTime = rw->GetMTime();
