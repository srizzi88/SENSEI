...
int TestPathTracerMaterials(int argc, char* argv[])
{
  ...
  svtkOSPRayRendererNode::SetSamplesPerPixel(1, renderer);
  ...
  svtkSmartPointer<svtkSuperquadricSource> polysource = svtkSmartPointer<svtkSuperquadricSource>::New();
  ...
  int i, j = 0;
  svtkSmartPointer<svtkActor> actor;
  svtkProperty* prop;
  svtkSmartPointer<svtkPolyDataMapper> mapper;
  ...
  // color mapping, this is SVTK after all
  j++;
  {
  ...
  */
  ...
  // now finally draw
  renWin->Render();                           // let svtk pick a decent camera
