/*=========================================================================
   ...
   Module:    svtkOSPRayMoleculeMapperNode.cxx
...
//============================================================================
svtkStandardNewMacro(svtkOSPRayMoleculeMapperNode);

//----------------------------------------------------------------------------
svtkOSPRayMoleculeMapperNode::svtkOSPRayMoleculeMapperNode() {}
...
//----------------------------------------------------------------------------
void svtkOSPRayMoleculeMapperNode::PrintSelf(ostream& os, svtkIndent indent)
{
  this->Superclass::PrintSelf(os, indent);
}

//----------------------------------------------------------------------------
void svtkOSPRayMoleculeMapperNode::Render(bool prepass)
{
  if (!prepass)
  {
    return;
  ...
  svtkMoleculeMapper* mapper = svtkMoleculeMapper::SafeDownCast(this->GetRenderable());
  svtkMolecule* molecule = mapper->GetInput();
  svtkScalarsToColors* lut = mapper->GetLookupTable();
    ...
    molecule->GetMTime() > this->BuildTime)
  {
    ...
    for (int i = 0; i < mapper->GetPeriodicTable()->GetNumberOfElements(); i++)
    {
      ...
      // svtkOSPRayPolyDataMapperNode::CellMaterials() and lookup custom materials from discrete LUT.
      ...
      auto oMaterial =
        svtkOSPRayMaterialHelpers::NewMaterial(orn, orn->GetORenderer(), "OBJMaterial");
    ...
    if (mapper->GetRenderAtoms())
    {
      ...
      int* idata = (int*)mdata;
      svtkUnsignedShortArray* atomicNumbers = molecule->GetAtomicNumberArray();
    ...
    if (mapper->GetRenderBonds())
    {
      ...
      if (mapper->GetBondColorMode() == svtkMoleculeMapper::DiscreteByAtom)
      {
