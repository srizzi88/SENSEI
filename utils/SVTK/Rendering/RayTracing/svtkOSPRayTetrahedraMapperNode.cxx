/*=========================================================================
  ...
  Module:    svtkOSPRayTetrahedraMapperNode.cxx
...
svtkStandardNewMacro(svtkOSPRayTetrahedraMapperNode);

//-----------------------------------------------------------------------------
svtkOSPRayTetrahedraMapperNode::svtkOSPRayTetrahedraMapperNode()
{
...
void svtkOSPRayTetrahedraMapperNode::PrintSelf(ostream& os, svtkIndent indent)
{
  this->Superclass::PrintSelf(os, indent);
}

//----------------------------------------------------------------------------
void svtkOSPRayTetrahedraMapperNode::Render(bool prepass)
{
  if (prepass)
  {
    svtkUnstructuredGridVolumeMapper* mapper =
      svtkUnstructuredGridVolumeMapper::SafeDownCast(this->GetRenderable());
    if (!mapper)
    {
    ...
    svtkDataSet* dataSet = mapper->GetDataSetInput();
    if (!dataSet)
    {
      return;
    }
    ...
    svtkDataArray* array =
      svtkDataArray::SafeDownCast(this->GetArrayToProcess(dataSet, fieldAssociation));
    if (!array)
    {
    ...
    if (mapper->GetDataSetInput()->GetMTime() > this->BuildTime)
    {
      ...
      else
      {
        ...
        for (int i = 0; i < numberOfCells; i++)
        {
          ...
          if (cell->GetCellType() == SVTK_TETRA)
          {
            for (int j = 0; j < 4; j++)
            {
          ...
          else if (cell->GetCellType() == SVTK_HEXAHEDRON)
          {
            for (int j = 0; j < 8; j++)
            {
              this->Cells.push_back(cell->GetPointId(j));
          ...
          else if (cell->GetCellType() == SVTK_WEDGE)
          {
            for (int j = 0; j < 2; ++j)
            {
              this->Cells.push_back(-1);
          ...
          {
            svtkWarningMacro("Unsupported cell type encountered: "
