/*=========================================================================
  ...
  Module:    svtkPolyDataMapperNode.cxx
...
//============================================================================
svtkStandardNewMacro(svtkPolyDataMapperNode);

//----------------------------------------------------------------------------
svtkPolyDataMapperNode::svtkPolyDataMapperNode() {}

//----------------------------------------------------------------------------
svtkPolyDataMapperNode::~svtkPolyDataMapperNode() {}

//----------------------------------------------------------------------------
void svtkPolyDataMapperNode::PrintSelf(ostream& os, svtkIndent indent)
{
  this->Superclass::PrintSelf(os, indent);
}

//----------------------------------------------------------------------------
void svtkPolyDataMapperNode::TransformPoints(
  svtkActor* act, svtkPolyData* poly, std::vector<double>& _vertices)
{
...
// The CreateXIndexBuffer's were adapted from svtkOpenGLIndexBufferObject.
// Apply rule of three if made again somewhere in SVTK.
...
void CreatePointIndexBuffer(svtkCellArray* cells, std::vector<unsigned int>& indexArray,
...
// At same time creates a reverse cell index array for obtaining cell quantities for points
void CreateLineIndexBuffer(svtkCellArray* cells, std::vector<unsigned int>& indexArray,
  std::vector<unsigned int>& reverseArray)
{
  // TODO: restore the preallocate and append to offset features I omitted
  const svtkIdType* indices(nullptr);
...
// back to first.
void CreateTriangleLineIndexBuffer(svtkCellArray* cells, std::vector<unsigned int>& indexArray,
  std::vector<unsigned int>& reverseArray)
{
  // TODO: restore the preallocate and append to offset features I omitted
  const svtkIdType* indices(nullptr);
...
// At same time creates a reverse cell index array for obtaining cell quantities for points
void CreateTriangleIndexBuffer(svtkCellArray* cells, svtkPoints* points,
  std::vector<unsigned int>& indexArray, std::vector<unsigned int>& reverseArray)
{
  ...
  svtkPolygon* polygon = nullptr;
  svtkIdList* tris = nullptr;
  svtkPoints* triPoints = nullptr;
    ...
    if (npts > 3)
    {
      ...
      else // 7 sided polygon or higher, do a full smart triangulation
      {
        if (!polygon)
        {
          ...
          tris = svtkIdList::New();
          triPoints = svtkPoints::New();
        }

        svtkIdType* triIndices = new svtkIdType[npts];
...
// At same time creates a reverse cell index array for obtaining cell quantities for points
void CreateStripIndexBuffer(svtkCellArray* cells, std::vector<unsigned int>& indexArray,
...
//----------------------------------------------------------------------------
void svtkPolyDataMapperNode::MakeConnectivity(
  svtkPolyData* poly, int representation, svtkPDConnectivity& conn)
{
