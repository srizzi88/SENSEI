...
class TestTextActor3DViewer(Testing.svtkTest):
    ...
    def SetUp(self):
        ...
        def AddSphere(ren):
            objSource = svtk.svtkSphereSource()

            objMapper = svtk.svtkPolyDataMapper()
            objMapper.SetInputConnection(objSource.GetOutputPort())

            objActor = svtk.svtkActor()
        ...
        def AddOneTextActor(baseTextProp):
            name = "ia"
            self.textActors[name] = svtk.svtkTextActor3D()
        ...
        def AddManyTextActors(baseTextProp):
            lut = svtk.svtkColorTransferFunction()
        ...
        # Base text property
        baseTextProp = svtk.svtkTextProperty()
