/*=========================================================================
  ...
  Module:    svtkWin32RenderWindowInteractor.cxx
...
// sees it in the friend decl in svtkWin32RenderWindowInteractor, but
// GCC needs to see the declaration beforehand. It has to do with the
// CALLBACK attribute.
SVTKRENDERINGUI_EXPORT LRESULT CALLBACK svtkHandleMessage(HWND, UINT, WPARAM, LPARAM);
SVTKRENDERINGUI_EXPORT LRESULT CALLBACK svtkHandleMessage2(
  HWND, UINT, WPARAM, LPARAM, class svtkWin32RenderWindowInteractor*);
...
svtkStandardNewMacro(svtkWin32RenderWindowInteractor);

void (*svtkWin32RenderWindowInteractor::ClassExitMethod)(void*) = (void (*)(void*))nullptr;
void* svtkWin32RenderWindowInteractor::ClassExitMethodArg = (void*)nullptr;
void (*svtkWin32RenderWindowInteractor::ClassExitMethodArgDelete)(void*) = (void (*)(void*))nullptr;

//----------------------------------------------------------------------------
// Construct object so that light follows camera motion.
svtkWin32RenderWindowInteractor::svtkWin32RenderWindowInteractor()
{
  this->WindowId = 0;
...
svtkWin32RenderWindowInteractor::~svtkWin32RenderWindowInteractor()
{
  svtkRenderWindow* tmp;

  // we need to release any hold we have on a windows event loop
  if (this->WindowId && this->Enabled && this->InstallMessageProc)
  {
    svtkRenderWindow* ren = this->RenderWindow;
    tmp = (svtkRenderWindow*)(svtkGetWindowLong(this->WindowId, sizeof(svtkLONG)));
    ...
    {
      svtkSetWindowLong(this->WindowId, svtkGWL_WNDPROC, (intptr_t)this->OldProc);
    }
    this->Enabled = 0;
  }
#ifdef SVTK_USE_TDX
...
//----------------------------------------------------------------------------
void svtkWin32RenderWindowInteractor::ProcessEvents()
{
...
//----------------------------------------------------------------------------
void svtkWin32RenderWindowInteractor::StartEventLoop()
{
  // No need to do anything if this is a 'mapped' interactor
  if (!this->Enabled || !this->InstallMessageProc)
  {
...
void svtkWin32RenderWindowInteractor::Initialize()
{
  svtkRenderWindow* ren;
  int* size;
...
//----------------------------------------------------------------------------
void svtkWin32RenderWindowInteractor::Enable()
{
  svtkRenderWindow* ren;
  svtkRenderWindow* tmp;
  if (this->Enabled)
  ...
  if (this->InstallMessageProc)
  {
    // add our callback
    ren = this->RenderWindow;
    this->OldProc = (WNDPROC)svtkGetWindowLong(this->WindowId, svtkGWL_WNDPROC);
    tmp = (svtkRenderWindow*)svtkGetWindowLong(this->WindowId, sizeof(svtkLONG));
    // watch for odd conditions
    if (tmp != ren)
    {
      ...
      // event handler so that it can properly
      // call this event handler if required
      CallWindowProc(this->OldProc, this->WindowId, WM_USER + 12, 24, (intptr_t)svtkHandleMessage);
    }
    else
    {
      svtkSetWindowLong(this->WindowId, svtkGWL_WNDPROC, (intptr_t)svtkHandleMessage);
    }
...
//----------------------------------------------------------------------------
void svtkWin32RenderWindowInteractor::Disable()
{
  svtkRenderWindow* tmp;
  if (!this->Enabled)
  {
...
//----------------------------------------------------------------------------
void svtkWin32RenderWindowInteractor::TerminateApp(void)
{
  this->Done = true;
...
//----------------------------------------------------------------------------
int svtkWin32RenderWindowInteractor::InternalCreateTimer(
...
//----------------------------------------------------------------------------
int svtkWin32RenderWindowInteractor::InternalDestroyTimer(int platformTimerId)
{
...
// Event loop handlers
//-------------------------------------------------------------
int svtkWin32RenderWindowInteractor::OnMouseMove(HWND hWnd, UINT nFlags, int X, int Y)
{
  if (!this->Enabled)
  {
    return 0;
  ...
  return this->InvokeEvent(svtkCommand::MouseMoveEvent, nullptr);
}

//----------------------------------------------------------------------------
int svtkWin32RenderWindowInteractor::OnNCMouseMove(HWND, UINT nFlags, int X, int Y)
{
  ...
  this->SetAltKey(GetKeyState(VK_MENU) & (~1));
  const int ret = this->InvokeEvent(svtkCommand::LeaveEvent, nullptr);
  this->MouseInWindow = 0;
  return ret;
}

//----------------------------------------------------------------------------
int svtkWin32RenderWindowInteractor::OnMouseWheelForward(HWND, UINT nFlags, int X, int Y)
{
  if (!this->Enabled)
  {
    return 0;
  ...
  this->SetEventInformationFlipY(X, Y, nFlags & MK_CONTROL, nFlags & MK_SHIFT);
  this->SetAltKey(GetKeyState(VK_MENU) & (~1));
  return this->InvokeEvent(svtkCommand::MouseWheelForwardEvent, nullptr);
}

//----------------------------------------------------------------------------
int svtkWin32RenderWindowInteractor::OnMouseWheelBackward(HWND, UINT nFlags, int X, int Y)
{
  if (!this->Enabled)
  {
    return 0;
  ...
  this->SetEventInformationFlipY(X, Y, nFlags & MK_CONTROL, nFlags & MK_SHIFT);
  this->SetAltKey(GetKeyState(VK_MENU) & (~1));
  return this->InvokeEvent(svtkCommand::MouseWheelBackwardEvent, nullptr);
}

//----------------------------------------------------------------------------
int svtkWin32RenderWindowInteractor::OnLButtonDown(HWND wnd, UINT nFlags, int X, int Y, int repeat)
{
  if (!this->Enabled)
  {
    return 0;
  ...
  this->SetAltKey(GetKeyState(VK_MENU) & (~1));
  return this->InvokeEvent(svtkCommand::LeftButtonPressEvent, nullptr);
}

//----------------------------------------------------------------------------
int svtkWin32RenderWindowInteractor::OnLButtonUp(HWND, UINT nFlags, int X, int Y)
{
  if (!this->Enabled)
  {
    return 0;
  ...
  this->SetEventInformationFlipY(X, Y, nFlags & MK_CONTROL, nFlags & MK_SHIFT);
  this->SetAltKey(GetKeyState(VK_MENU) & (~1));
  const int ret = this->InvokeEvent(svtkCommand::LeftButtonReleaseEvent, nullptr);
  ReleaseCapture();
  return ret;
}

//----------------------------------------------------------------------------
int svtkWin32RenderWindowInteractor::OnMButtonDown(HWND wnd, UINT nFlags, int X, int Y, int repeat)
{
  if (!this->Enabled)
  {
    return 0;
  ...
  this->SetEventInformationFlipY(X, Y, nFlags & MK_CONTROL, nFlags & MK_SHIFT, 0, repeat);
  this->SetAltKey(GetKeyState(VK_MENU) & (~1));
  return this->InvokeEvent(svtkCommand::MiddleButtonPressEvent, nullptr);
}

//----------------------------------------------------------------------------
int svtkWin32RenderWindowInteractor::OnMButtonUp(HWND, UINT nFlags, int X, int Y)
{
  if (!this->Enabled)
  {
    return 0;
  ...
  this->SetEventInformationFlipY(X, Y, nFlags & MK_CONTROL, nFlags & MK_SHIFT);
  this->SetAltKey(GetKeyState(VK_MENU) & (~1));
  const int ret = this->InvokeEvent(svtkCommand::MiddleButtonReleaseEvent, nullptr);
  ReleaseCapture();
  return ret;
}

//----------------------------------------------------------------------------
int svtkWin32RenderWindowInteractor::OnRButtonDown(HWND wnd, UINT nFlags, int X, int Y, int repeat)
{
  if (!this->Enabled)
  {
    return 0;
  ...
  this->SetEventInformationFlipY(X, Y, nFlags & MK_CONTROL, nFlags & MK_SHIFT, 0, repeat);
  this->SetAltKey(GetKeyState(VK_MENU) & (~1));
  return this->InvokeEvent(svtkCommand::RightButtonPressEvent, nullptr);
}

//----------------------------------------------------------------------------
int svtkWin32RenderWindowInteractor::OnRButtonUp(HWND, UINT nFlags, int X, int Y)
{
  if (!this->Enabled)
  {
    return 0;
  ...
  this->SetEventInformationFlipY(X, Y, nFlags & MK_CONTROL, nFlags & MK_SHIFT);
  this->SetAltKey(GetKeyState(VK_MENU) & (~1));
  const int ret = this->InvokeEvent(svtkCommand::RightButtonReleaseEvent, nullptr);
  ReleaseCapture();
  return ret;
}

//----------------------------------------------------------------------------
int svtkWin32RenderWindowInteractor::OnSize(HWND, UINT, int X, int Y)
{
  this->UpdateSize(X, Y);
  if (this->Enabled)
  {
    return this->InvokeEvent(svtkCommand::ConfigureEvent, nullptr);
  }
  return 0;
}

//----------------------------------------------------------------------------
int svtkWin32RenderWindowInteractor::OnTimer(HWND, UINT timerId)
{
  if (!this->Enabled)
  {
    return 0;
  ...
  const int ret = this->InvokeEvent(svtkCommand::TimerEvent, (void*)&tid);
...
//----------------------------------------------------------------------------
int svtkWin32RenderWindowInteractor::OnKeyDown(HWND, UINT vCode, UINT nRepCnt, UINT nFlags)
{
  if (!this->Enabled)
  {
    return 0;
  ...
  this->SetAltKey(alt);
  return this->InvokeEvent(svtkCommand::KeyPressEvent, nullptr);
}

//----------------------------------------------------------------------------
int svtkWin32RenderWindowInteractor::OnKeyUp(HWND, UINT vCode, UINT nRepCnt, UINT nFlags)
{
  if (!this->Enabled)
  {
    return 0;
  ...
  this->SetKeyEventInformation(ctrl, shift, nChar, nRepCnt, keysym);
  this->SetAltKey(alt);
  return this->InvokeEvent(svtkCommand::KeyReleaseEvent, nullptr);
}

//----------------------------------------------------------------------------
int svtkWin32RenderWindowInteractor::OnChar(HWND, UINT nChar, UINT nRepCnt, UINT)
{
  if (!this->Enabled)
  {
    return 0;
  ...
  this->SetAltKey(alt);
  return this->InvokeEvent(svtkCommand::CharEvent, nullptr);
}

//----------------------------------------------------------------------------
int svtkWin32RenderWindowInteractor::OnFocus(HWND, UINT)
{
  if (!this->Enabled)
  {
    return 0;
...
//----------------------------------------------------------------------------
int svtkWin32RenderWindowInteractor::OnKillFocus(HWND, UINT)
{
  if (!this->Enabled)
  {
    return 0;
...
//----------------------------------------------------------------------------
int svtkWin32RenderWindowInteractor::OnTouch(HWND hWnd, UINT wParam, UINT lParam)
{
  if (!this->Enabled)
  {
    return 0;
  ...
  if (pInputs)
  {
    ...
    if (GTII((HTOUCHINPUT)lParam, cInputs, pInputs, sizeof(TOUCHINPUT)))
    {
      ...
      for (UINT i = 0; i < cInputs; i++)
      {
        ...
        if (ti.dwID != 0 && index < SVTKI_MAX_POINTERS)
        {
      ...
      if (!didUpOrDown)
      {
        ret = this->InvokeEvent(svtkCommand::MouseMoveEvent, nullptr);
      }
      else
      {
        ret = 1;
...
//----------------------------------------------------------------------------
int svtkWin32RenderWindowInteractor::OnDropFiles(HWND, WPARAM wParam)
{
  if (!this->Enabled)
  {
    return 0;
  ...
  if (DragQueryPoint(hdrop, &pt))
  {
    ...
    this->InvokeEvent(svtkCommand::UpdateDropLocationEvent, location);
  }
  ...
  if (cFiles > 0)
  {
    ...
    this->InvokeEvent(svtkCommand::DropFilesEvent, filePaths);
  }
...
LRESULT CALLBACK svtkHandleMessage(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  LRESULT res = 0;
  svtkRenderWindow* ren;
  svtkWin32RenderWindowInteractor* me = 0;

  ren = (svtkRenderWindow*)svtkGetWindowLong(hWnd, sizeof(svtkLONG));

  if (ren)
  {
    me = (svtkWin32RenderWindowInteractor*)ren->GetInteractor();
  }

  if (me && me->GetReferenceCount() > 0)
  {
    me->Register(me);
    res = svtkHandleMessage2(hWnd, uMsg, wParam, lParam, me);
    me->UnRegister(me);
  }
...
//----------------------------------------------------------------------------
LRESULT CALLBACK svtkHandleMessage2(
  HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, svtkWin32RenderWindowInteractor* me)
{
  ...
  switch (uMsg)
  {
    case WM_PAINT:
    {
      ...
      me->InvokeEvent(svtkCommand::RenderEvent, nullptr);
      return ret;
    }
    ...
    case WM_MOUSELEAVE:
      me->InvokeEvent(svtkCommand::LeaveEvent, nullptr);
      me->MouseInWindow = 0;
      break;
...
void svtkWin32RenderWindowInteractor::SetClassExitMethod(void (*f)(void*), void* arg)
{
  if (f != svtkWin32RenderWindowInteractor::ClassExitMethod ||
    arg != svtkWin32RenderWindowInteractor::ClassExitMethodArg)
  {
    // delete the current arg if there is a delete method
    if ((svtkWin32RenderWindowInteractor::ClassExitMethodArg) &&
      (svtkWin32RenderWindowInteractor::ClassExitMethodArgDelete))
    {
      (*svtkWin32RenderWindowInteractor::ClassExitMethodArgDelete)(
        svtkWin32RenderWindowInteractor::ClassExitMethodArg);
    }
    svtkWin32RenderWindowInteractor::ClassExitMethod = f;
    svtkWin32RenderWindowInteractor::ClassExitMethodArg = arg;
...
void svtkWin32RenderWindowInteractor::SetClassExitMethodArgDelete(void (*f)(void*))
{
  if (f != svtkWin32RenderWindowInteractor::ClassExitMethodArgDelete)
  {
    svtkWin32RenderWindowInteractor::ClassExitMethodArgDelete = f;
...
//----------------------------------------------------------------------------
void svtkWin32RenderWindowInteractor::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
void svtkWin32RenderWindowInteractor::ExitCallback()
{
