/*=========================================================================
  ...
  Module:    svtkXRenderWindowInteractor.cxx
...
=========================================================================*/
#include "svtkXRenderWindowInteractor.h"
...
svtkStandardNewMacro(svtkXRenderWindowInteractor);
...
struct svtkXRenderWindowInteractorTimer
{
...
// is separate from the TimerMap in the svtkRenderWindowInteractor
...
{
public:
  svtkXRenderWindowInteractorInternals() { this->TimerIdCount = 1; }
  ~svtkXRenderWindowInteractorInternals() {}
  ...
  void FireTimers(svtkXRenderWindowInteractor* rwi)
  {
    if (this->LocalToTimer.size())
    {
      timeval ctv;
      ...
      for (auto& timer : this->LocalToTimer)
      {
        ...
        {
          int timerId = rwi->GetSVTKTimerId(timer.first);
          rwi->InvokeEvent(svtkCommand::TimerEvent, &timerId);
...
private:
  int TimerIdCount;
  std::map<int, svtkXRenderWindowInteractorTimer> LocalToTimer;
};

int svtkXRenderWindowInteractor::BreakLoopFlag = 1;

// for some reason the X11 def of KeySym is getting messed up
typedef XID svtkKeySym;

//-------------------------------------------------------------------------
svtkXRenderWindowInteractor::svtkXRenderWindowInteractor()
{
  this->Internal = new svtkXRenderWindowInteractorInternals;
...
void svtkXRenderWindowInteractor::TerminateApp()
{
  ...
  // Send a SVTK_BreakXtLoop ClientMessage event to be sure we pop out of the
  ...
  client.message_type = XInternAtom(this->DisplayId, "SVTK_BreakXtLoop", False);
...
void svtkXRenderWindowInteractor::SetBreakLoopFlag(int f)
{
  if (f)
  {
...
void svtkXRenderWindowInteractor::BreakLoopFlagOff()
{
  this->BreakLoopFlag = 0;
  this->Modified();
}

void svtkXRenderWindowInteractor::BreakLoopFlagOn()
{
  this->TerminateApp();
  this->Modified();
}

void svtkXRenderWindowInteractor::ProcessEvents()
{
  XEvent event;
...
// loop is exited.
void svtkXRenderWindowInteractor::StartEventLoop()
{
...
void svtkXRenderWindowInteractor::Initialize()
{
  if (this->Initialized)
  {
    return;
  ...
  if (!this->DisplayId)
  {
    svtkDebugMacro("opening display");
    this->DisplayId = XOpenDisplay(nullptr);
    svtkDebugMacro("opened display");
...
//-------------------------------------------------------------------------
void svtkXRenderWindowInteractor::Enable()
{
...
//-------------------------------------------------------------------------
void svtkXRenderWindowInteractor::Disable()
{
  if (!this->Enabled)
  {
    return;
...
//-------------------------------------------------------------------------
void svtkXRenderWindowInteractor::PrintSelf(ostream& os, svtkIndent indent)
{
...
//-------------------------------------------------------------------------
void svtkXRenderWindowInteractor::UpdateSize(int x, int y)
{
...
//-------------------------------------------------------------------------
void svtkXRenderWindowInteractor::UpdateSizeNoXResize(int x, int y)
{
  // if the size changed send this on to the RenderWindow
  if ((x != this->Size[0]) || (y != this->Size[1]))
  {
    this->Size[0] = x;
    this->Size[1] = y;
    // static_cast<svtkXOpenGLRenderWindow*>(this->RenderWindow)->SetSizeNoXResize(x, y);
    this->RenderWindow->SetSize(x, y);
  }
}

//-------------------------------------------------------------------------
void svtkXRenderWindowInteractor::FireTimers()
{
  if (this->GetEnabled())
  {
...
int svtkXRenderWindowInteractor::InternalCreateTimer(
  int svtkNotUsed(timerId), int svtkNotUsed(timerType), unsigned long duration)
{
...
//-------------------------------------------------------------------------
int svtkXRenderWindowInteractor::InternalDestroyTimer(int platformTimerId)
{
...
//-------------------------------------------------------------------------
void svtkXRenderWindowInteractor::DispatchEvent(XEvent* event)
{
  int xp, yp;
  ...
  switch (event->type)
  {
    ...
    case ClientMessage:
    {
      if (event->xclient.message_type == this->XdndPositionAtom)
      {
        ...
        // Convert it to SVTK compatible location
        double location[2];
...
//-------------------------------------------------------------------------
void svtkXRenderWindowInteractor::GetMousePosition(int* x, int* y)
{
  Window root, child;
  int root_x, root_y;
  unsigned int keys;
...
//-------------------------------------------------------------------------
// void svtkXRenderWindowInteractor::Timer(XtPointer client_data, XtIntervalId* id)
// {
//   svtkXRenderWindowInteractorTimer(client_data, id);
// }

//-------------------------------------------------------------------------
// void svtkXRenderWindowInteractor::Callback(
//   Widget w, XtPointer client_data, XEvent* event, Boolean* ctd)
// {
//   svtkXRenderWindowInteractorCallback(w, client_data, event, ctd);
// }
