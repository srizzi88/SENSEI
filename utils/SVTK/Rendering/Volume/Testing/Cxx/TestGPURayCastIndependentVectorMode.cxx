...
/** \description
 * Tests vector rendering mode in svtkVolumeSmartMapper. VectorMode builds
 ...
 * vector's magnitude, an additional data array is computed through svtkImageMagnitude.
...
int TestGPURayCastIndependentVectorMode(int argc, char* argv[])
{
  ...
  svtkNew<svtkSmartVolumeMapper> mapper;
  ...
  svtkNew<svtkSmartVolumeMapper> mapperMag;
  ...
  svtkNew<svtkVolumeProperty> propertyMag;
  propertyMag->SetColor(0, ctf2);
  propertyMag->SetScalarOpacity(0, pf2);

  svtkNew<svtkVolume> volumeMag;
  ...
  svtkNew<svtkSmartVolumeMapper> mapperMagCells;
  ...
  svtkNew<svtkVolume> volumeMagCells;
  ...
  // are generated
  mapper->SetVectorMode(svtkSmartVolumeMapper::COMPONENT);
  mapper->SetVectorComponent(0);
  mapperMag->SetVectorMode(svtkSmartVolumeMapper::MAGNITUDE);
  renWin->Render();

  mapper->SetVectorMode(svtkSmartVolumeMapper::MAGNITUDE);
  mapperMag->SetVectorMode(svtkSmartVolumeMapper::COMPONENT);
  ...
  mapperMagCells->SetVectorMode(svtkSmartVolumeMapper::COMPONENT);
  mapperMagCells->SetVectorComponent(2);
  renWin->Render();

  mapperMagCells->SetVectorMode(svtkSmartVolumeMapper::MAGNITUDE);
  renWin->Render();
