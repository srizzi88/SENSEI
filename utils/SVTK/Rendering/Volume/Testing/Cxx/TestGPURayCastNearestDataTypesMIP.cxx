...
int TestGPURayCastNearestDataTypesMIP(int argc, char* argv[])
{
  ...
  svtkImageShiftScale* shiftScale[4][2];
  svtkColorTransferFunction* color[4][2];
  svtkPiecewiseFunction* opacity[4][2];

  // unsigned char
  shiftScale[0][0] = svtkImageShiftScale::New();
  shiftScale[0][0]->SetInputConnection(reader->GetOutputPort());

  color[0][0] = svtkColorTransferFunction::New();
  color[0][0]->AddRGBPoint(0, 0, 0, 1, 0.5, 1.0);
  color[0][0]->AddRGBPoint(255, 0, 1, 0);

  opacity[0][0] = svtkPiecewiseFunction::New();
  opacity[0][0]->AddPoint(0, 0);
  opacity[0][0]->AddPoint(255, 1);

  // signed char
  shiftScale[0][1] = svtkImageShiftScale::New();
  ...
  color[2][0] = svtkColorTransferFunction::New();
  color[2][0]->AddRGBPoint(0, 0, 0, 1, 0.5, 1.0);
  color[2][0]->AddRGBPoint(SVTK_UNSIGNED_INT_MAX, 0, 1, 0);

  opacity[2][0] = svtkPiecewiseFunction::New();
  opacity[2][0]->AddPoint(0, 0);
  opacity[2][0]->AddPoint(SVTK_UNSIGNED_INT_MAX, 1);

  // int
  shiftScale[2][1] = svtkImageShiftScale::New();
  shiftScale[2][1]->SetInputConnection(shiftScale[2][0]->GetOutputPort());
  shiftScale[2][1]->SetShift(SVTK_INT_MIN);
  shiftScale[2][1]->SetOutputScalarTypeToInt();

  color[2][1] = svtkColorTransferFunction::New();
  color[2][1]->AddRGBPoint(SVTK_INT_MIN, 0, 0, 1, 0.5, 1.0);
  color[2][1]->AddRGBPoint(SVTK_INT_MAX, 0, 1, 0);

  opacity[2][1] = svtkPiecewiseFunction::New();

  opacity[2][1]->AddPoint(SVTK_INT_MIN, 0);
  opacity[2][1]->AddPoint(SVTK_INT_MAX, 1);

  // float [-1 1]
  svtkImageShiftScale* shiftScale_3_0_pre = svtkImageShiftScale::New();
  ...
  shiftScale[3][0] = svtkImageShiftScale::New();
  ...
  color[3][0] = svtkColorTransferFunction::New();
  color[3][0]->AddRGBPoint(-1.0, 0, 0, 1, 0.5, 1.0);
  color[3][0]->AddRGBPoint(1.0, 0, 1, 0);

  opacity[3][0] = svtkPiecewiseFunction::New();
  opacity[3][0]->AddPoint(-1.0, 0);
  opacity[3][0]->AddPoint(1.0, 1);

  // double [-1000 3000]
  svtkImageShiftScale* shiftScale_3_1_pre = svtkImageShiftScale::New();
  ...
  shiftScale[3][1] = svtkImageShiftScale::New();
  ...
  color[3][1] = svtkColorTransferFunction::New();
  color[3][1]->AddRGBPoint(-1000, 0, 0, 1, 0.5, 1.0);
  color[3][1]->AddRGBPoint(3000, 0, 1, 0);

  opacity[3][1] = svtkPiecewiseFunction::New();
  ...
  svtkGPUVolumeRayCastMapper* volumeMapper[4][2];
  svtkVolumeProperty* volumeProperty[4][2];
  svtkVolume* volume[4][2];
  svtkTransform* userMatrix[4][2];
  ...
  while (i < 4)
  {
    ...
    while (j < 2)
    {
      ...
      volumeProperty[i][j] = svtkVolumeProperty::New();
      volumeProperty[i][j]->SetColor(color[i][j]);
      volumeProperty[i][j]->SetScalarOpacity(opacity[i][j]);
      volumeProperty[i][j]->SetInterpolationType(SVTK_NEAREST_INTERPOLATION);

      volume[i][j] = svtkVolume::New();
      volume[i][j]->SetMapper(volumeMapper[i][j]);
      volume[i][j]->SetProperty(volumeProperty[i][j]);

      userMatrix[i][j] = svtkTransform::New();
