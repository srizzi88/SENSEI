...
class FixedPointRayCasterNearest(object):
    ...
    def __init__(self, ren, renWin, iren):
        ...
        # Create a gaussian
        gs = svtk.svtkImageGaussianSource()
        ...
        d = svtk.svtkImageContinuousDilate3D()
        ...
        gridR = svtk.svtkImageGridSource()
        ...
        dR = svtk.svtkImageContinuousDilate3D()
        dR.SetInputConnection(gridR.GetOutputPort())
        dR.SetKernelSize(2, 2, 2)

        gridG = svtk.svtkImageGridSource()
        ...
        dG = svtk.svtkImageContinuousDilate3D()
        dG.SetInputConnection(gridG.GetOutputPort())
        dG.SetKernelSize(2, 2, 2)

        gridB = svtk.svtkImageGridSource()
        ...
        rainbow = svtk.svtkColorTransferFunction()
        ...
        for i in range(0, 5):
            for j in range(0, 5):
                ...
                self.volumeProperty[i][j] = svtk.svtkVolumeProperty()
                self.volumeMapper[i][j] = svtk.svtkFixedPointVolumeRayCastMapper()
                self.volumeMapper[i][j].SetSampleDistance(0.25)
                self.volumeMapper[i][j].SetNumberOfThreads(1)

                volume[i][j] = svtk.svtkVolume()
...
class TestFixedPointRayCasterNearest(svtk.test.Testing.svtkTest):
...
if __name__ == "__main__":
     svtk.test.Testing.main([(TestFixedPointRayCasterNearest, 'test')])
