/*=========================================================================
  ...
  Module:    svtkEncodedGradientEstimator.cxx
...
#include "svtkRecursiveSphereDirectionEncoder.h"
#include "svtkTimerLog.h"

#include <cmath>

svtkCxxSetObjectMacro(svtkEncodedGradientEstimator, InputData, svtkImageData);

// Construct a svtkEncodedGradientEstimator with initial values of nullptr for
...
svtkEncodedGradientEstimator::svtkEncodedGradientEstimator()
{
  ...
  this->DirectionEncoder = svtkRecursiveSphereDirectionEncoder::New();
...
// Destruct a svtkEncodedGradientEstimator - free up any memory used
svtkEncodedGradientEstimator::~svtkEncodedGradientEstimator()
{
...
void svtkEncodedGradientEstimator::SetZeroNormalThreshold(float v)
{
  if (this->ZeroNormalThreshold != v)
  {
    if (v < 0.0)
    {
      svtkErrorMacro(<< "The ZeroNormalThreshold must be a value >= 0.0");
      return;
    }
...
void svtkEncodedGradientEstimator::SetDirectionEncoder(svtkDirectionEncoder* direnc)
{
...
int svtkEncodedGradientEstimator::GetEncodedNormalIndex(svtkIdType xyzIndex)
{
  this->Update();
  return *(this->EncodedNormals + xyzIndex);
}

int svtkEncodedGradientEstimator::GetEncodedNormalIndex(int xIndex, int yIndex, int zIndex)
{
  svtkIdType ystep, zstep;
...
unsigned short* svtkEncodedGradientEstimator::GetEncodedNormals()
{
  this->Update();
...
unsigned char* svtkEncodedGradientEstimator::GetGradientMagnitudes()
{
  this->Update();
...
void svtkEncodedGradientEstimator::Update()
{
  ...
  if (!this->InputData)
  {
    svtkErrorMacro(<< "No input in gradient estimator.");
    return;
  }
    ...
    this->InputData->GetMTime() > this->BuildTime || !this->EncodedNormals)
  {
    ...
    startSeconds = svtkTimerLog::GetUniversalTime();
    startCPUSeconds = svtkTimerLog::GetCPUTime();
    ...
    svtkIdType encodedSize = scalarInputSize[0];
    ...
    endSeconds = svtkTimerLog::GetUniversalTime();
    endCPUSeconds = svtkTimerLog::GetCPUTime();
...
void svtkEncodedGradientEstimator::ComputeCircleLimits(int size)
{
  int *ptr, y;
...
// Print the svtkEncodedGradientEstimator
void svtkEncodedGradientEstimator::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
void svtkEncodedGradientEstimator::ReportReferences(svtkGarbageCollector* collector)
{
  this->Superclass::ReportReferences(collector);
  svtkGarbageCollectorReport(collector, this->InputData, "Input");
}
