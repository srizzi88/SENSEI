/*=========================================================================
  ...
  Module:    svtkEncodedGradientShader.cxx
...
svtkStandardNewMacro(svtkEncodedGradientShader);

svtkEncodedGradientShader::svtkEncodedGradientShader()
{
  int i, j;

  for (j = 0; j < SVTK_MAX_SHADING_TABLES; j++)
  {
...
float* svtkEncodedGradientShader::GetRedDiffuseShadingTable(svtkVolume* vol)
{
  int index;

  for (index = 0; index < SVTK_MAX_SHADING_TABLES; index++)
  {
  ...
  if (index == SVTK_MAX_SHADING_TABLES)
  {
    svtkErrorMacro(<< "No shading table found for that volume!");
    return nullptr;
  }
...
float* svtkEncodedGradientShader::GetGreenDiffuseShadingTable(svtkVolume* vol)
{
  int index;
...
float* svtkEncodedGradientShader::GetBlueDiffuseShadingTable(svtkVolume* vol)
{
  int index;
...
float* svtkEncodedGradientShader::GetRedSpecularShadingTable(svtkVolume* vol)
{
  int index;
...
float* svtkEncodedGradientShader::GetGreenSpecularShadingTable(svtkVolume* vol)
{
  int index;
...
float* svtkEncodedGradientShader::GetBlueSpecularShadingTable(svtkVolume* vol)
{
  int index;
...
void svtkEncodedGradientShader::UpdateShadingTable(
  svtkRenderer* ren, svtkVolume* vol, svtkEncodedGradientEstimator* gradest)
{
  ...
  svtkLightCollection* lightCollection;
  svtkLight* light;
  double norm;
  int update_flag;
  ...
  if (index == SVTK_MAX_SHADING_TABLES)
  {
    svtkErrorMacro(<< "Too many shading tables!\n"
                  << "Increase limit SVTK_MAX_SHADING_TABLES and recompile!");
    return;
  }
  ...
  svtkLight* artificialLight = nullptr;
  if (lightCollection->GetNumberOfItems() == 0)
  {
    artificialLight = svtkLight::New();
...
void svtkEncodedGradientShader::BuildShadingTable(int index, double lightDirection[3],
  ...
  svtkEncodedGradientEstimator* gradest, int updateFlag)
{
  double lx, ly, lz;
  double n_dot_l;
  double n_dot_v;
...
void svtkEncodedGradientShader::PrintSelf(ostream& os, svtkIndent indent)
{
