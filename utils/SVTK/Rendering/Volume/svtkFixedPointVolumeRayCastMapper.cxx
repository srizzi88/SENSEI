/*=========================================================================
  ...
  Module:    svtkFixedPointVolumeRayCastMapper.cxx
...
#include "svtkEncodedGradientShader.h"
...
#include "svtkVolumeRayCastSpaceLeapingImageFilter.h"

#include <cmath>
#include <exception>

svtkStandardNewMacro(svtkFixedPointVolumeRayCastMapper);
svtkCxxSetObjectMacro(svtkFixedPointVolumeRayCastMapper, RayCastImage, svtkFixedPointRayCastImage);

#define svtkVRCMultiplyPointMacro(A, B, M)                                                          \
...
#define svtkVRCMultiplyNormalMacro(A, B, M)                                                         \
...
//----------------------------------------------------------------------------
template <class T>
void svtkFixedPointVolumeRayCastMapperComputeCS1CGradients(T* dataPtr, int dim[3], double spacing[3],
  double scalarRange[2], unsigned short** gradientNormal, unsigned char** gradientMagnitude,
  svtkDirectionEncoder* directionEncoder, int thread_id, int thread_count,
  svtkFixedPointVolumeRayCastMapper* me)
{
  int x, y, z;
  svtkIdType yinc, zinc;
  int x_start, x_limit;
  int y_start, y_limit;
  int z_start, z_limit;
  T* dptr;
  ...
  if (thread_id == 0)
  {
    me->InvokeEvent(svtkCommand::VolumeMapperComputeGradientsStartEvent, nullptr);
  }
  ...
  yinc = static_cast<svtkIdType>(dim[0]);
  zinc = yinc * static_cast<svtkIdType>(dim[1]);
  ...
  for (z = z_start; z < z_limit; z++)
  {
    ...
    for (y = y_start; y < y_limit; y++)
    {
      ...
      for (x = xlow; x < xhigh; x++)
      {
        ...
        // Scaling in the svtkVolume is isotropic, so this is the
    ...
    if ((z / thread_count) % 8 == 7 && thread_id == 0)
    {
      ...
      me->InvokeEvent(svtkCommand::VolumeMapperComputeGradientsProgressEvent, args);
    }
  }
  ...
  if (thread_id == 0)
  {
    me->InvokeEvent(svtkCommand::VolumeMapperComputeGradientsEndEvent, nullptr);
  }
}

SVTK_THREAD_RETURN_TYPE svtkFPVRCMSwitchOnDataType(void* arg)
{
  svtkFixedPointVolumeRayCastMapper* mapper;
  int thread_count;
  int thread_id;

  thread_id = ((svtkMultiThreader::ThreadInfo*)(arg))->ThreadID;
  thread_count = ((svtkMultiThreader::ThreadInfo*)(arg))->NumberOfThreads;
  mapper = (svtkFixedPointVolumeRayCastMapper*)(((svtkMultiThreader::ThreadInfo*)(arg))->UserData);

  svtkImageData* input = mapper->GetInput();
  ...
  {
    svtkFixedPointVolumeRayCastMapperComputeCS1CGradients((unsigned char*)(dataPtr), dim, spacing,
      scalarRange, mapper->GradientNormal, mapper->GradientMagnitude, mapper->DirectionEncoder,
      thread_id, thread_count, mapper);
  }
  else if (scalarType == SVTK_UNSIGNED_SHORT)
  {
    svtkFixedPointVolumeRayCastMapperComputeCS1CGradients((unsigned short*)(dataPtr), dim, spacing,
      scalarRange, mapper->GradientNormal, mapper->GradientMagnitude, mapper->DirectionEncoder,
      thread_id, thread_count, mapper);
  }
  else if (scalarType == SVTK_CHAR)
  {
    svtkFixedPointVolumeRayCastMapperComputeCS1CGradients((char*)(dataPtr), dim, spacing,
      scalarRange, mapper->GradientNormal, mapper->GradientMagnitude, mapper->DirectionEncoder,
      thread_id, thread_count, mapper);
  }
  else if (scalarType == SVTK_SHORT)
  {
    svtkFixedPointVolumeRayCastMapperComputeCS1CGradients((short*)(dataPtr), dim, spacing,
      scalarRange, mapper->GradientNormal, mapper->GradientMagnitude, mapper->DirectionEncoder,
      thread_id, thread_count, mapper);
  }
...
template <class T>
void svtkFixedPointVolumeRayCastMapperComputeGradients(T* dataPtr, int dim[3], double spacing[3],
  int components, int independent, double scalarRange[4][2], unsigned short** gradientNormal,
  unsigned char** gradientMagnitude, svtkDirectionEncoder* directionEncoder,
  svtkFixedPointVolumeRayCastMapper* me)
{
  int x, y, z, c;
  svtkIdType yinc, zinc;
...
//----------------------------------------------------------------------------
// Construct a new svtkFixedPointVolumeRayCastMapper with default values
svtkFixedPointVolumeRayCastMapper::svtkFixedPointVolumeRayCastMapper()
{
  ...
  this->ViewToVoxelsMatrix = svtkMatrix4x4::New();
  this->VoxelsToViewMatrix = svtkMatrix4x4::New();
  this->WorldToVoxelsMatrix = svtkMatrix4x4::New();
  this->VoxelsToWorldMatrix = svtkMatrix4x4::New();

  this->VolumeMatrix = svtkMatrix4x4::New();

  this->PerspectiveTransform = svtkTransform::New();
  this->VoxelsTransform = svtkTransform::New();
  this->VoxelsToViewTransform = svtkTransform::New();

  this->Threader = svtkMultiThreader::New();
  this->ThreadWarning = true;
  this->RayCastImage = svtkFixedPointRayCastImage::New();
  ...
  this->MIPHelper = svtkFixedPointVolumeRayCastMIPHelper::New();
  this->CompositeHelper = svtkFixedPointVolumeRayCastCompositeHelper::New();
  this->CompositeGOHelper = svtkFixedPointVolumeRayCastCompositeGOHelper::New();
  this->CompositeShadeHelper = svtkFixedPointVolumeRayCastCompositeShadeHelper::New();
  this->CompositeGOShadeHelper = svtkFixedPointVolumeRayCastCompositeGOShadeHelper::New();
  ...
  this->DirectionEncoder = svtkSphericalDirectionEncoder::New();
  this->GradientShader = svtkEncodedGradientShader::New();
  this->GradientEstimator = svtkFiniteDifferenceGradientEstimator::New();
  ...
  this->SpaceLeapFilter = svtkVolumeRayCastSpaceLeapingImageFilter::New();
  ...
  this->MinMaxVolumeCache = svtkImageData::New();
}

//----------------------------------------------------------------------------
// Destruct a svtkFixedPointVolumeRayCastMapper - clean up any memory used
svtkFixedPointVolumeRayCastMapper::~svtkFixedPointVolumeRayCastMapper()
{
...
float svtkFixedPointVolumeRayCastMapper::ComputeRequiredImageSampleDistance(
  float desiredTime, svtkRenderer* ren)
{
  return this->ComputeRequiredImageSampleDistance(desiredTime, ren, nullptr);
}

float svtkFixedPointVolumeRayCastMapper::ComputeRequiredImageSampleDistance(
  float desiredTime, svtkRenderer* ren, svtkVolume* vol)
{
  float result;
...
float svtkFixedPointVolumeRayCastMapper::RetrieveRenderTime(svtkRenderer* ren, svtkVolume* vol)
{
  int i;
...
float svtkFixedPointVolumeRayCastMapper::RetrieveRenderTime(svtkRenderer* ren)
{
  int i;
...
void svtkFixedPointVolumeRayCastMapper::StoreRenderTime(svtkRenderer* ren, svtkVolume* vol, float time)
{
  int i;
  for (i = 0; i < this->RenderTableEntries; i++)
  {
...
void svtkFixedPointVolumeRayCastMapper::SetNumberOfThreads(int num)
{
  this->Threader->SetNumberOfThreads(num);
}

int svtkFixedPointVolumeRayCastMapper::GetNumberOfThreads()
{
  if (this->Threader)
  {
...
void svtkFixedPointVolumeRayCastMapper::UpdateMinMaxVolume(svtkVolume* vol)
{
  ...
  // svtkVolumeRayCastSpaceLeapingImageFilter::WriteMinMaxVolume(
...
//----------------------------------------------------------------------------
void svtkFixedPointVolumeRayCastMapper::UpdateCroppingRegions()
{
...
int svtkFixedPointVolumeRayCastMapper::PerImageInitialization(svtkRenderer* ren, svtkVolume* vol,
...
void svtkFixedPointVolumeRayCastMapper::PerVolumeInitialization(svtkRenderer* ren, svtkVolume* vol)
{
  ...
  if (usingCellColors)
  {
    svtkErrorMacro("Cell Scalars not supported");
    return;
  }
...
void svtkFixedPointVolumeRayCastMapper::PerSubVolumeInitialization(
  svtkRenderer* ren, svtkVolume* vol, int multiRender)
{
  this->UpdateCroppingRegions();
...
void svtkFixedPointVolumeRayCastMapper::RenderSubVolume()
{
...
void svtkFixedPointVolumeRayCastMapper::DisplayRenderedImage(svtkRenderer* ren, svtkVolume* vol)
{
  float depth;
...
void svtkFixedPointVolumeRayCastMapper::ApplyFinalColorWindowLevel()
{
...
void svtkFixedPointVolumeRayCastMapper::AbortRender()
{
  // Restore values
...
void svtkFixedPointVolumeRayCastMapper::CaptureZBuffer(svtkRenderer* ren)
{
...
void svtkFixedPointVolumeRayCastMapper::Render(svtkRenderer* ren, svtkVolume* vol)
{
  //  if (this->Threader->GetNumberOfThreads() > 1 && this->ThreadWarning)
  //    {
  //    svtkWarningMacro("Number of threads "
  ...
  if (this->GetBlendMode() != svtkVolumeMapper::COMPOSITE_BLEND &&
    this->GetBlendMode() != svtkVolumeMapper::MAXIMUM_INTENSITY_BLEND &&
    this->GetBlendMode() != svtkVolumeMapper::MINIMUM_INTENSITY_BLEND &&
    this->GetBlendMode() != svtkVolumeMapper::ADDITIVE_BLEND)
  {
...
SVTK_THREAD_RETURN_TYPE FixedPointVolumeRayCastMapper_CastRays(void* arg)
{
  ...
  int threadID = ((svtkMultiThreader::ThreadInfo*)(arg))->ThreadID;
  int threadCount = ((svtkMultiThreader::ThreadInfo*)(arg))->NumberOfThreads;

  svtkFixedPointVolumeRayCastMapper* me =
    (svtkFixedPointVolumeRayCastMapper*)(((svtkMultiThreader::ThreadInfo*)arg)->UserData);

  if (!me)
  {
    svtkGenericWarningMacro("Irrecoverable error: no mapper specified");
    return SVTK_THREAD_RETURN_VALUE;
  }

  svtkVolume* vol = me->GetVolume();

  if (me->GetBlendMode() == svtkVolumeMapper::MAXIMUM_INTENSITY_BLEND ||
    me->GetBlendMode() == svtkVolumeMapper::MINIMUM_INTENSITY_BLEND)
  {
...
// Create an image into the svtkImageData argmument. Used generally for
// creating thumbnail images
void svtkFixedPointVolumeRayCastMapper::CreateCanonicalView(
  svtkVolume* vol, svtkImageData* image, int blend_mode, double direction[3], double viewUp[3])
{
  // Make sure we have as long as we'd like so that the
  // image sample distance will be 1.0
  vol->SetAllocatedRenderTime(SVTK_DOUBLE_MAX, nullptr);
  ...
  // Now copy the image into the svtkImageData
...
void svtkFixedPointVolumeRayCastMapper::ComputeRayInfo(
  int x, int y, unsigned int pos[3], unsigned int dir[3], unsigned int* numSteps)
{
  ...
  viewRay[2] = 0.0;
  svtkVRCMultiplyPointMacro(viewRay, rayStart, this->ViewToVoxelsArray);

  viewRay[2] = this->RayCastImage->GetZBufferValue(x, y);

  svtkVRCMultiplyPointMacro(viewRay, rayEnd, this->ViewToVoxelsArray);
  ...
  *numSteps = 0;
        ...
        rayStart, rayEnd, this->NumTransformedClippingPlanes, this->TransformedClippingPlanes)))
  {
    ...
    double worldLength = svtkMath::Normalize(worldRayDirection) / this->SampleDistance;
...
void svtkFixedPointVolumeRayCastMapper::InitializeRayInfo(svtkVolume* vol)
{
  if (!vol)
  {
    return;
  ...
  if (this->NumTransformedClippingPlanes > 0)
  {
    ...
    for (i = 0; i < this->NumTransformedClippingPlanes; i++)
    {
      ...
      svtkPlane* onePlane = (svtkPlane*)this->ClippingPlanes->GetItemAsObject(i);
      onePlane->GetNormal(worldNormal);
      onePlane->GetOrigin(worldOrigin);
      float* planePtr = this->TransformedClippingPlanes + 4 * i;
      svtkVRCMultiplyNormalMacro(worldNormal, planePtr, this->VoxelsToWorldArray);
      svtkVRCMultiplyPointMacro(worldOrigin, inputOrigin, this->WorldToVoxelsArray);
...
int svtkFixedPointVolumeRayCastMapper::ComputeRowBounds(
  svtkRenderer* ren, int imageFlag, int rowBoundsFlag, int inputExtent[6])
{
  float voxelPoint[3];
  float viewPoint[8][4];
  int i, j, k;
    ...
    for (k = 0; k < 2; k++)
    {
      ...
      for (j = 0; j < 2; j++)
      {
        ...
        for (i = 0; i < 2; i++)
        {
          ...
          svtkVRCMultiplyPointMacro(voxelPoint, viewPoint[idx], voxelsToViewMatrix);
...
void svtkFixedPointVolumeRayCastMapper::ComputeMatrices(double inputOrigin[3],
  double inputSpacing[3], int inputExtent[6], svtkRenderer* ren, svtkVolume* vol)
{
...
int svtkFixedPointVolumeRayCastMapper::ClipRayAgainstClippingPlanes(
...
int svtkFixedPointVolumeRayCastMapper::ClipRayAgainstVolume(
...
void svtkFixedPointVolumeRayCastMapper::ComputeGradients(svtkVolume* vol)
{
  svtkImageData* input = this->GetInput();
  ...
  svtkIdType sliceSize = (static_cast<svtkIdType>(dim[0]) * static_cast<svtkIdType>(dim[1]) *
    ((independent) ? (components) : (1)));
  svtkIdType numSlices = dim[2];
  ...
  if (components == 1 &&
    (scalarType == SVTK_UNSIGNED_CHAR || scalarType == SVTK_CHAR ||
      scalarType == SVTK_UNSIGNED_SHORT || scalarType == SVTK_SHORT))
  {
    this->Threader->SetSingleMethod(svtkFPVRCMSwitchOnDataType, (svtkObject*)this);
    this->Threader->SingleMethodExecute();
  }
  ...
  else
  {
    switch (scalarType)
    {
      svtkTemplateMacro(svtkFixedPointVolumeRayCastMapperComputeGradients((SVTK_TT*)(dataPtr), dim,
...
int svtkFixedPointVolumeRayCastMapper::UpdateShadingTable(svtkRenderer* ren, svtkVolume* vol)
{
  ...
  for (c = 0; c < ((vol->GetProperty()->GetIndependentComponents()) ? (components) : (1)); c++)
  {
    ...
    {
      *(tablePtr++) = static_cast<unsigned short>((*(rptr++)) * SVTKKW_FP_SCALE + 0.5);
      *(tablePtr++) = static_cast<unsigned short>((*(gptr++)) * SVTKKW_FP_SCALE + 0.5);
      *(tablePtr++) = static_cast<unsigned short>((*(bptr++)) * SVTKKW_FP_SCALE + 0.5);
    }
...
int svtkFixedPointVolumeRayCastMapper::UpdateGradients(svtkVolume* vol)
{
  int needToUpdate = 0;
  ...
  {
    svtkPiecewiseFunction* f = vol->GetProperty()->GetGradientOpacity(c);
...
int svtkFixedPointVolumeRayCastMapper::UpdateColorTable(svtkVolume* vol)
{
  int needToUpdate = 0;
  ...
  if (blendMode != this->SavedBlendMode)
  {
    ...
    if (this->GetBlendMode() == svtkVolumeMapper::MINIMUM_INTENSITY_BLEND)
    {
  ...
  svtkColorTransferFunction* rgbFunc[4];
  svtkPiecewiseFunction* grayFunc[4];
  svtkPiecewiseFunction* scalarOpacityFunc[4];
  svtkPiecewiseFunction* gradientOpacityFunc[4];
  int colorChannels[4];
    ...
    for (c = 0; c < components; c++)
    {
      ...
      for (i = 0; i < this->TableSize[c]; i++)
      {
        this->ColorTable[c][3 * i] =
          static_cast<unsigned short>(tmpArray[3 * i] * SVTKKW_FP_SCALE + 0.5);
        this->ColorTable[c][3 * i + 1] =
          static_cast<unsigned short>(tmpArray[3 * i + 1] * SVTKKW_FP_SCALE + 0.5);
        this->ColorTable[c][3 * i + 2] =
          static_cast<unsigned short>(tmpArray[3 * i + 2] * SVTKKW_FP_SCALE + 0.5);
      }
      ...
      for (i = 0; i < this->TableSize[c]; i++)
      {
        this->ScalarOpacityTable[c][i] =
          static_cast<unsigned short>(tmpArray[i] * SVTKKW_FP_SCALE + 0.5);
      }
...
int svtkFixedPointVolumeRayCastMapper::ShouldUseNearestNeighborInterpolation(svtkVolume* vol)
{
  //  return ( this->UseShortCuts ||
  //           vol->GetProperty()->GetInterpolationType() == SVTK_NEAREST_INTERPOLATION );
  return (vol->GetProperty()->GetInterpolationType() == SVTK_NEAREST_INTERPOLATION);
}

// Print method for svtkFixedPointVolumeRayCastMapper
void svtkFixedPointVolumeRayCastMapper::PrintSelf(ostream& os, svtkIndent indent)
{
...
void svtkFixedPointVolumeRayCastMapper::ReleaseGraphicsResources(svtkWindow* win)
{
