...
svtkMultiVolume::svtkMultiVolume()
  : Superclass()
  , TexToBBox(svtkSmartPointer<svtkMatrix4x4>::New())
{
  svtkMath::UninitializeBounds(this->Bounds);
  svtkMath::UninitializeBounds(this->DataBounds.data());
}
...
svtkStandardNewMacro(svtkMultiVolume);

void svtkMultiVolume::SetVolume(svtkVolume* vol, int port)
{
...
svtkVolume* svtkMultiVolume::GetVolume(int port)
{
  auto vol = this->FindVolume(port);
  if (!vol)
  {
    svtkErrorMacro(<< "Failed to query svtkVolume instance for port " << port);
    return nullptr;
  }
...
svtkVolume* svtkMultiVolume::FindVolume(int port)
{
  svtkVolume* vol = nullptr;
...
double* svtkMultiVolume::GetBounds()
{
  if (!this->VolumesChanged() && svtkMath::AreBoundsInitialized(this->Bounds))
  {
    return this->Bounds;
  }
  ...
  // total bounds (T_total = T_vol * T_tex (svtkImageData))
  for (auto& item : this->Volumes)
  {
    // Transform to world coordinates (ensure the matrix is
    // up-to-date).
    const int port = item.first;
    auto mapper = svtkGPUVolumeRayCastMapper::SafeDownCast(this->Mapper);
    if (!mapper)
    {
      svtkErrorMacro(<< "svtkMultiVolume is currently only supported by"
                       " svtkGPUVolumeRayCastMapper.");
      return this->Bounds;
    }
    double* bnd = mapper->GetBoundsFromPort(port);

    svtkVolume* vol = item.second;
    vol->ComputeMatrix();
  ...
  svtkVector3d minPoint(this->Bounds[0], this->Bounds[2], this->Bounds[4]);
  svtkVector3d maxPoint(this->Bounds[1], this->Bounds[3], this->Bounds[5]);
...
std::array<double, 6> svtkMultiVolume::ComputeAABounds(double bounds[6], svtkMatrix4x4* T) const
{
  using Point = svtkVector4d;
  using PointVec = std::vector<Point>;

  // Create all corner poiints of the bounding box
  svtkVector3d dim(bounds[1] - bounds[0], bounds[3] - bounds[2], bounds[5] - bounds[4]);
...
bool svtkMultiVolume::VolumesChanged()
{
  auto mapper = svtkGPUVolumeRayCastMapper::SafeDownCast(this->Mapper);
  if (!mapper)
  {
...
svtkMTimeType svtkMultiVolume::GetMTime()
{
  auto mTime = this->svtkObject::GetMTime();
...
void svtkMultiVolume::PrintSelf(ostream& os, svtkIndent indent)
{
...
void svtkMultiVolume::ShallowCopy(svtkProp* prop)
{
  auto multiVol = svtkMultiVolume::SafeDownCast(prop);
  if (multiVol)
  {
...
int svtkMultiVolume::RenderVolumetricGeometry(svtkViewport* vp)
{
  this->Update();
...
void svtkMultiVolume::SetProperty(svtkVolumeProperty* svtkNotUsed(property))
{
  svtkWarningMacro(<< "This svtkVolumeProperty will not be used during"
                  << " rendering. Volume properties should be specified through registered"
                  << " svtkVolume instances.");
}

svtkVolumeProperty* svtkMultiVolume::GetProperty()
{
