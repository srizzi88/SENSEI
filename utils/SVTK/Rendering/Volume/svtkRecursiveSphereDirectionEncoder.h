/*=========================================================================
  ...
  Module:    svtkRecursiveSphereDirectionEncoder.h
...
/**
 * @class   svtkRecursiveSphereDirectionEncoder
 * @brief   A direction encoder based on the recursive subdivision of an octahedron
 *
 * svtkRecursiveSphereDirectionEncoder is a direction encoder which uses the
...
#ifndef svtkRecursiveSphereDirectionEncoder_h
#define svtkRecursiveSphereDirectionEncoder_h

#include "svtkDirectionEncoder.h"
#include "svtkRenderingVolumeModule.h" // For export macro

class SVTKRENDERINGVOLUME_EXPORT svtkRecursiveSphereDirectionEncoder : public svtkDirectionEncoder
{
public:
  svtkTypeMacro(svtkRecursiveSphereDirectionEncoder, svtkDirectionEncoder);
   ...
   * sphere.
   */
  static svtkRecursiveSphereDirectionEncoder* New();
   ...
   * zero normal.
   */
  svtkSetClampMacro(RecursionDepth, int, 0, 6);
  svtkGetMacro(RecursionDepth, int);
  //@}

protected:
  svtkRecursiveSphereDirectionEncoder();
  ~svtkRecursiveSphereDirectionEncoder() override;
...
private:
  svtkRecursiveSphereDirectionEncoder(const svtkRecursiveSphereDirectionEncoder&) = delete;
  void operator=(const svtkRecursiveSphereDirectionEncoder&) = delete;
};
