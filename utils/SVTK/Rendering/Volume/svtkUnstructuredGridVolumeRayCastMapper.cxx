/*=========================================================================
  ...
  Module:    svtkUnstructuredGridVolumeRayCastMapper.cxx
...
SVTK_THREAD_RETURN_TYPE UnstructuredGridVolumeRayCastMapper_CastRays(void* arg);

svtkStandardNewMacro(svtkUnstructuredGridVolumeRayCastMapper);

svtkCxxSetObjectMacro(svtkUnstructuredGridVolumeRayCastMapper, RayCastFunction,
  svtkUnstructuredGridVolumeRayCastFunction);
svtkCxxSetObjectMacro(
  svtkUnstructuredGridVolumeRayCastMapper, RayIntegrator, svtkUnstructuredGridVolumeRayIntegrator);

// Construct a new svtkUnstructuredGridVolumeRayCastMapper with default values
svtkUnstructuredGridVolumeRayCastMapper::svtkUnstructuredGridVolumeRayCastMapper()
{
  ...
  this->RayCastFunction = svtkUnstructuredGridBunykRayCastFunction::New();
  this->RayIntegrator = nullptr;
  this->RealRayIntegrator = nullptr;
}

// Destruct a svtkUnstructuredGridVolumeRayCastMapper - clean up any memory used
svtkUnstructuredGridVolumeRayCastMapper::~svtkUnstructuredGridVolumeRayCastMapper()
{
...
float svtkUnstructuredGridVolumeRayCastMapper::RetrieveRenderTime(svtkRenderer* ren, svtkVolume* vol)
{
  int i;
...
void svtkUnstructuredGridVolumeRayCastMapper::StoreRenderTime(
...
void svtkUnstructuredGridVolumeRayCastMapper::ReleaseGraphicsResources(svtkWindow*) {}

void svtkUnstructuredGridVolumeRayCastMapper::Render(svtkRenderer* ren, svtkVolume* vol)
{
  int i;
  ...
  {
    svtkErrorMacro("Can't use the ray cast mapper without scalars!");
    return;
  }
...
#undef ESTABLISH_INTEGRATOR
  ...
  this->RayCastIterators = new svtkUnstructuredGridVolumeRayCastIterator*[this->NumberOfThreads];
  this->IntersectedCellsBuffer = new svtkIdList*[this->NumberOfThreads];
  this->IntersectionLengthsBuffer = new svtkDoubleArray*[this->NumberOfThreads];
  this->NearIntersectionsBuffer = new svtkDataArray*[this->NumberOfThreads];
  this->FarIntersectionsBuffer = new svtkDataArray*[this->NumberOfThreads];
  for (i = 0; i < this->NumberOfThreads; i++)
  {
    this->RayCastIterators[i] = this->RayCastFunction->NewIterator();
    this->IntersectionLengthsBuffer[i] = svtkDoubleArray::New();
    this->IntersectionLengthsBuffer[i]->Allocate(
      this->RayCastIterators[i]->GetMaxNumberOfIntersections());
    this->NearIntersectionsBuffer[i] = svtkDataArray::CreateDataArray(this->Scalars->GetDataType());
    this->NearIntersectionsBuffer[i]->Allocate(
      this->RayCastIterators[i]->GetMaxNumberOfIntersections());
    if (this->CellScalars)
    {
      this->IntersectedCellsBuffer[i] = svtkIdList::New();
    ...
    else
    {
      ...
      this->FarIntersectionsBuffer[i] = svtkDataArray::CreateDataArray(this->Scalars->GetDataType());
...
SVTK_THREAD_RETURN_TYPE UnstructuredGridVolumeRayCastMapper_CastRays(void* arg)
{
  ...
  svtkUnstructuredGridVolumeRayCastMapper* me =
    (svtkUnstructuredGridVolumeRayCastMapper*)((svtkMultiThreader::ThreadInfo*)arg)->UserData;

  if (!me)
  {
    svtkGenericWarningMacro("The volume does not have a ray cast mapper!");
...
template <class T>
inline void svtkUGVRCMLookupCopy(
  const T* src, T* dest, svtkIdType* lookup, int numcomponents, int numtuples)
{
...
void svtkUnstructuredGridVolumeRayCastMapper::CastRays(int threadID, int threadCount)
{
  int i, j;
  unsigned char* ucptr;

  svtkRenderWindow* renWin = this->CurrentRenderer->GetRenderWindow();
  svtkUnstructuredGridVolumeRayCastIterator* iterator = this->RayCastIterators[threadID];

  svtkIdList* intersectedCells = this->IntersectedCellsBuffer[threadID];
  svtkDoubleArray* intersectionLengths = this->IntersectionLengthsBuffer[threadID];
  svtkDataArray* nearIntersections = this->NearIntersectionsBuffer[threadID];
  svtkDataArray* farIntersections = this->FarIntersectionsBuffer[threadID];
    ...
    for (i = 0; i < this->ImageInUseSize[0]; i++)
    {
      ...
      do
      {
        if (this->CellScalars)
        {
          ...
          {
            svtkTemplateMacro(svtkUGVRCMLookupCopy((const SVTK_TT*)this->Scalars->GetVoidPointer(0),
              (SVTK_TT*)nearIntersections->GetVoidPointer(0), intersectedCells->GetPointer(0),
...
double svtkUnstructuredGridVolumeRayCastMapper::GetMinimumBoundsDepth(
...
double svtkUnstructuredGridVolumeRayCastMapper::GetZBufferValue(int x, int y)
{
  int xPos, yPos;
...
// Print method for svtkUnstructuredGridVolumeRayCastMapper
void svtkUnstructuredGridVolumeRayCastMapper::PrintSelf(ostream& os, svtkIndent indent)
{
