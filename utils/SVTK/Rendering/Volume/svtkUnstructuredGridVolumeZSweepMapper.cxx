/*=========================================================================
  ...
  Module:    svtkUnstructuredGridVolumeZSweepMapper.cxx
...
=========================================================================*/
#include "svtkUnstructuredGridVolumeZSweepMapper.h"
...
#include "svtkUnstructuredGridHomogeneousRayIntegrator.h"
#include "svtkUnstructuredGridPartialPreIntegration.h"
#include "svtkUnstructuredGridPreIntegration.h"
...
enum
{
  SVTK_VALUES_X_INDEX = 0, //  world coordinate
  SVTK_VALUES_Y_INDEX,     //  world coordinate
  SVTK_VALUES_Z_INDEX,     //  world coordinate
  SVTK_VALUES_SCALAR_INDEX,
  SVTK_VALUES_SIZE // size of a value array
};

// Internal classes
//-----------------------------------------------------------------------------
// Store the result of the scan conversion at some pixel.
class svtkPixelListEntry
{
public:
  svtkPixelListEntry() = default;

  void Init(double values[SVTK_VALUES_SIZE], double zView, bool exitFace)
  {
    this->Zview = zView;
    int i = 0;
    while (i < SVTK_VALUES_SIZE)
    {
  ...
  svtkPixelListEntry* GetPrevious() { return this->Previous; }
  svtkPixelListEntry* GetNext() { return this->Next; }

  void SetPrevious(svtkPixelListEntry* e) { this->Previous = e; }
  void SetNext(svtkPixelListEntry* e) { this->Next = e; }

protected:
  double Values[SVTK_VALUES_SIZE];
  double Zview;
  bool ExitFace;

  // List structure: both for the free block list (one-way) and any
  // pixel list (two-way)
  svtkPixelListEntry* Next;
  // List structure: only for the pixel list (two-way)
  svtkPixelListEntry* Previous;

private:
  svtkPixelListEntry(const svtkPixelListEntry& other) = delete;
  svtkPixelListEntry& operator=(const svtkPixelListEntry& other) = delete;
};
...
{
public:
  svtkVertexEntry() = default;

  svtkVertexEntry(int screenX, int screenY, double xWorld, double yWorld, double zWorld,
    ...
    , InvW(invW)
  {
    this->Values[SVTK_VALUES_X_INDEX] = xWorld;
    this->Values[SVTK_VALUES_Y_INDEX] = yWorld;
    this->Values[SVTK_VALUES_Z_INDEX] = zWorld;
    this->Values[SVTK_VALUES_SCALAR_INDEX] = scalar;
  }
  ...
  svtkVertexEntry& operator=(const svtkVertexEntry& other)
  {
    ScreenX = other.ScreenX;
    ScreenY = other.ScreenY;
    memcpy(Values, other.Values, sizeof(double) * SVTK_VALUES_SIZE);
    Zview = other.Zview;
    InvW = other.InvW;
    return *this;
  }
  svtkVertexEntry(const svtkVertexEntry& other)
  {
    if (this != &other)
    {
      *this = other;
...
protected:
  // Destructor.
  virtual ~svtkScreenEdge() = default;
};
...
enum
{
  SVTK_CASE_VERTICAL = 0,
  SVTK_CASE_MOSTLY_VERTICAL,
  SVTK_CASE_DIAGONAL,
  SVTK_CASE_HORIZONTAL_BEGIN,
  SVTK_CASE_HORIZONTAL_END,
  SVTK_CASE_HORIZONTAL_MS,      // most significant pixel
  SVTK_CASE_VERTICAL_IN_TO_OUT, // with edge equation
  SVTK_CASE_VERTICAL_OUT_TO_IN,
  SVTK_CASE_HORIZONTAL_IN_TO_OUT,
  SVTK_CASE_HORIZONTAL_OUT_TO_IN,
  SVTK_CASE_UNDEFINED
};
...
class svtkSimpleScreenEdge : public svtkScreenEdge
{
public:
  svtkSimpleScreenEdge() { this->Case = SVTK_CASE_UNDEFINED; }
  ...
  void Init(svtkVertexEntry* v0, svtkVertexEntry* v2, int dx20, int dy20, int onRight)
  {
    ...
    double pv0[SVTK_VALUES_SIZE];
    double pv2[SVTK_VALUES_SIZE];
    int i = 0;
    while (i < SVTK_VALUES_SIZE)
    {
    ...
    else
    {
      if (dx20 > 0)
      {
        ...
        if (dx20 > dy20)
        {
          ...
          if (onRight)
          {
            this->Case = SVTK_CASE_HORIZONTAL_IN_TO_OUT;
          }
          else
          {
            this->Case = SVTK_CASE_HORIZONTAL_OUT_TO_IN;
          }
          this->Error = 0;
          this->SDy = dy20;
          ...
          this->Case = SVTK_CASE_HORIZONTAL_MS;
          this->XStep = dx20 / dy20; // integral division
          this->Dy = dy20;
          this->Dy2 = dy20 << 1; // *2
          this->Error = 0;
          ...
          if (!onRight)
          {
            this->Case = SVTK_CASE_HORIZONTAL_BEGIN;
            this->First = 1;
            this->Dy2 = dy20 << 1; // *2
            this->Dx2 = dx20 << 1; // *2
            this->Error = dx20;
          ...
          {
            this->Case = SVTK_CASE_HORIZONTAL_END;
        ...
        else
        {
          ...
          else
          {
            ...
            if (onRight)
            {
              this->Case = SVTK_CASE_VERTICAL_IN_TO_OUT;
            }
            else
            {
              this->Case = SVTK_CASE_VERTICAL_OUT_TO_IN;
            }
            this->Error = 0;
            this->SDy = dy20;
            this->Dx = dx20;
            ...
            this->Dz = (z2 - z0) * invDy20;
#else
            this->Case = SVTK_CASE_MOSTLY_VERTICAL;
            this->Dx2 = dx20 << 1; // *2
            this->Dy2 = dy20 << 1; // *2
            this->Error = dy20;
  ...
  void NextLine(int y) override
  {
    ...
    switch (this->Case)
    {
      ...
      case SVTK_CASE_DIAGONAL:
        ...
        this->Zview += this->Dz;
        break;
      case SVTK_CASE_MOSTLY_VERTICAL:
        // X
        this->Error += this->Dx2;
        ...
        this->Zview += this->Dz;
        break;
      case SVTK_CASE_VERTICAL_OUT_TO_IN:
        this->Error -= this->Dx;
        if (this->SDy > 0)
        {
...
//              assert("check: negative_equation" && this->Error>0);
        ...
        this->Zview += this->Dz;
        break;
      case SVTK_CASE_VERTICAL_IN_TO_OUT:
      ...
      case SVTK_CASE_HORIZONTAL_OUT_TO_IN:
        this->Error -= this->Dx;
        this->X += this->XStep;
        if (this->SDy > 0)
        {
      ...
      case SVTK_CASE_HORIZONTAL_IN_TO_OUT:
        this->Error += this->SDy - this->Dx;
        this->X += this->XStep;
        if (this->SDy < 0)
        {
...
//            assert("check: negative_equation" && this->Error<0);
      ...
      case SVTK_CASE_HORIZONTAL_BEGIN:
        if (this->First)
        {
          this->First = 0;
        }
      ...
      case SVTK_CASE_HORIZONTAL_MS:
        ...
        break;
      default:
        svtkGenericWarningMacro(<< "Undefined edge case");
    }
  }
...
protected:
  ...
  int Dx2;       // 2*dx
  int Dy2;       // 2*dy
  int First;     // use only with SVTK_CASE_HORIZONTAL_BEGIN case
  int XStep;     // dx/dy
  ...
  double Dpv[SVTK_VALUES_SIZE];

  // Current projected values
  double PValues[SVTK_VALUES_SIZE];
  // Dpv*XStep
  double PValuesStep[SVTK_VALUES_SIZE];
  // Values at the end vertex.
  double PValues2[SVTK_VALUES_SIZE];

  int Dy;  // SVTK_HORIZONTAL_MS
  int SDy; // SVTK_VERTICAL_LEFT/RIGHT
  int Dx;  // SVTK_VERTICAL_LEFT/RIGHT
};
...
class svtkDoubleScreenEdge : public svtkScreenEdge
{
public:
  void Init(
    svtkVertexEntry* v0, svtkVertexEntry* v1, svtkVertexEntry* v2, int dx10, int dy10, int onRight)
  {
...
protected:
  svtkSimpleScreenEdge Top;
  svtkSimpleScreenEdge Bottom;
  svtkScreenEdge* Current;
};
...
public:
  ...
  void Init(int x0, double invW0,
    double pValues0[SVTK_VALUES_SIZE], // projected values
    double zView0, int x1, double invW1,
    double pValues1[SVTK_VALUES_SIZE], // projected values
    double zView1)
  {
  ...
  svtkTypeBool IsAtEnd() { return this->X > this->X1; }
...
// typedef std::list<svtkPixelListEntry *> svtkPixelList;

class svtkPixelListEntryBlock
{
public:
  svtkPixelListEntryBlock(svtkIdType size)
  {
    assert("pre: positive_size" && size > 0);
    this->Size = size;
    this->Next = nullptr;
    this->Array = new svtkPixelListEntry[size];
    this->Last = this->Array + size - 1;
    // link each entry to the next one
    svtkPixelListEntry* p;
    svtkPixelListEntry* q;
    p = this->Array;
    q = p + 1;
    svtkIdType i = 1;
    while (i < size)
  ...
  ~svtkPixelListEntryBlock() { delete[] this->Array; }
  svtkIdType GetSize() { return this->Size; }
  svtkPixelListEntryBlock* GetNext() { return this->Next; }
  svtkPixelListEntry* GetFirst() { return this->Array; }
  svtkPixelListEntry* GetLast() { return this->Last; }
  void SetNext(svtkPixelListEntryBlock* other) { this->Next = other; }

protected:
  svtkIdType Size;
  svtkPixelListEntryBlock* Next;
  svtkPixelListEntry* Array;
  svtkPixelListEntry* Last;
};

const svtkIdType SVTK_PIXEL_BLOCK_SIZE = 64;

class svtkPixelListEntryMemory
{
public:
  svtkPixelListEntryMemory()
  {
    this->FirstBlock = new svtkPixelListEntryBlock(SVTK_PIXEL_BLOCK_SIZE);
    this->FirstFreeElement = this->FirstBlock->GetFirst();
    this->Size = SVTK_PIXEL_BLOCK_SIZE;
  }
  ~svtkPixelListEntryMemory()
  {
    svtkPixelListEntryBlock* p = this->FirstBlock;
    svtkPixelListEntryBlock* q;
    while (p != nullptr)
    {
      q = p->GetNext();
      delete p;
  ...
  svtkPixelListEntry* AllocateEntry()
  {
    ...
    svtkPixelListEntry* result = this->FirstFreeElement;
    this->FirstFreeElement = result->GetNext();
    assert("post: result_exists" && result != nullptr);
    return result;
  }
  void FreeEntry(svtkPixelListEntry* e)
  {
  ...
  void FreeSubList(svtkPixelListEntry* first, svtkPixelListEntry* last)
  {
...
protected:
  void AllocateBlock(svtkIdType size)
  {
    assert("pre: positive_size" && size > 0);
    svtkPixelListEntryBlock* b = new svtkPixelListEntryBlock(size);
    this->Size += size;
  ...
  svtkPixelListEntryBlock* FirstBlock;
  svtkPixelListEntry* FirstFreeElement;
  svtkIdType Size; // overall size, in number of elements, not in bytes
};

class svtkPixelList
{
public:
  svtkPixelList() { this->Size = 0; }
  svtkPixelListEntry* GetFirst()
  {
  ...
  void AddAndSort(svtkPixelListEntry* p)
  {
    ...
    {
      svtkPixelListEntry* it = this->Last;
      int sorted = 0;
      double z = p->GetZview();
      ...
      else
      {
        ...
        {
          svtkPixelListEntry* q = it->GetNext();
          q->SetPrevious(p);
          p->SetNext(q);
          p->SetPrevious(it);
          it->SetNext(p);
  ...
  void RemoveFirst(svtkPixelListEntryMemory* mm)
  {
    assert("pre: not_empty" && this->Size > 0);
    assert("pre: mm_exists" && mm != nullptr);

    svtkPixelListEntry* p = this->First;
    if (this->Size > 1)
    {
  ...
  void Clear(svtkPixelListEntryMemory* mm)
  {
    assert("pre: mm_exists" && mm != nullptr);
    if (this->Size > 0)
    {
...
{
public:
  typedef std::vector<svtkPixelList> VectorType;

  svtkPixelListFrame(int size)
    : Vector(size)
  {
  }

  // Return width*height
  svtkIdType GetSize() { return static_cast<svtkIdType>(this->Vector.size()); }

  // Return the size of the list at pixel `i'.
  svtkIdType GetListSize(int i)
  {
  ...
  void AddAndSort(int i, svtkPixelListEntry* pixelEntry)
  {
    assert("pre: valid_i" && i >= 0 && i < this->GetSize());
  ...
  svtkPixelListEntry* GetFront(int i)
  {
    assert("pre: valid_i" && i >= 0 && i < this->GetSize());
  ...
  void PopFront(int i, svtkPixelListEntryMemory* mm)
  {
    assert("pre: valid_i" && i >= 0 && i < this->GetSize());
    assert("pre: not_empty" && this->GetListSize(i) > 0);
    assert("pre: mm_exists" && mm != nullptr);
  ...
  svtkPixelListEntry* GetFirst(int i)
  {
    assert("pre: valid_i" && i >= 0 && i < this->GetSize());
    return this->Vector[i].GetFirst();
  }
  ...
  std::list<svtkPixelListEntry *>::iterator GetEndIterator(int i)
  {
      assert("pre: valid_i" && i>=0 && i<this->GetSize());
  ...
  void Clean(svtkPixelListEntryMemory* mm)
  {
    assert("pre: mm_exists" && mm != nullptr);
    svtkIdType i = 0;
    svtkIdType c = static_cast<svtkIdType>(this->Vector.size());
    while (i < c)
    {
      svtkPixelList* l = &(Vector[i]);
      l->Clear(mm);
      ++i;
    }
  }
...
protected:
  ...
  //  std::vector<svtkIdType> Sizes;

  //  std::list<svtkPixelListEntry *>::iterator It;
  //  std::list<svtkPixelListEntry *>::iterator PreviousIt;
  //  std::list<svtkPixelListEntry *>::iterator ItEnd;
};
...
public:
  ...
  svtkFace(svtkIdType faceIds[3], int externalSide)
  {
...
private:
  svtkFace() = delete;
  svtkFace(const svtkFace& other) = delete;
  svtkFace& operator=(const svtkFace& other) = delete;
};
...
{
public:
  typedef std::vector<std::list<svtkFace*>*> VectorType;
  VectorType Vector;

  std::list<svtkFace*> AllFaces; // to set up rendering to false.
  ...
  void AddFace(svtkIdType faceIds[3], svtkDataArray* scalars, svtkIdType cellIdx,
    int orientationChanged, bool external)
  {
    ...
    svtkFace* f = this->GetFace(faceIds);
    if (f == nullptr)
    {
      int externalSide;
      if (external)
      {
        ...
        {
          externalSide = svtkFace::BACK_FACE;
        }
        else
        {
          externalSide = svtkFace::FRONT_FACE;
        }
      }
      else
      {
        externalSide = svtkFace::NOT_EXTERNAL;
      }
      f = new svtkFace(faceIds, externalSide);
      ...
      while (i < 3)
      {
        std::list<svtkFace*>* p = this->Vector[faceIds[i]];
        if (p == nullptr)
        {
          p = new std::list<svtkFace*>;
  ...
  void SetNotRendered()
  {
    std::list<svtkFace*>::iterator it;
    std::list<svtkFace*>::iterator end;
...
protected:
  ...
  svtkFace* GetFace(svtkIdType faceIds[3])
  {
    std::list<svtkFace*>* useSet = this->Vector[faceIds[0]];
    svtkFace* result = nullptr;
  ...
  // Used in GetFace()
  std::list<svtkFace*>::iterator It;
  std::list<svtkFace*>::iterator ItEnd;
};
...
{
public:
  typedef std::vector<svtkVertexEntry> VectorType;
  VectorType Vector;

  // Initialize with the number of vertices.
  svtkVertices(int size)
    : Vector(size)
  {
  }
};
...
using namespace svtkUnstructuredGridVolumeZSweepMapperNamespace;

//-----------------------------------------------------------------------------
// Implementation of the public class.

svtkStandardNewMacro(svtkUnstructuredGridVolumeZSweepMapper);

svtkCxxSetObjectMacro(
  svtkUnstructuredGridVolumeZSweepMapper, RayIntegrator, svtkUnstructuredGridVolumeRayIntegrator);

//-----------------------------------------------------------------------------
// Description:
// Set MaxPixelListSize to 32.
svtkUnstructuredGridVolumeZSweepMapper::svtkUnstructuredGridVolumeZSweepMapper()
{
  ...
  this->ImageDisplayHelper = svtkRayCastImageDisplayHelper::New();

  this->PixelListFrame = nullptr;

  this->Cell = svtkGenericCell::New();

  this->EventList = svtkPriorityQueue::New();

  this->UseSet = nullptr;
  this->Vertices = nullptr;

  this->PerspectiveTransform = svtkTransform::New();
  this->PerspectiveMatrix = svtkMatrix4x4::New();

  this->SimpleEdge = new svtkSimpleScreenEdge;
  this->DoubleEdge = new svtkDoubleScreenEdge;

  this->Span = new svtkSpan;

  this->RayIntegrator = nullptr;
  this->RealRayIntegrator = nullptr;

  this->IntersectionLengths = svtkDoubleArray::New();
  this->IntersectionLengths->SetNumberOfValues(1);
  this->NearIntersections = svtkDoubleArray::New();
  this->NearIntersections->SetNumberOfValues(1);
  this->FarIntersections = svtkDoubleArray::New();
...
//-----------------------------------------------------------------------------
float svtkUnstructuredGridVolumeZSweepMapper::RetrieveRenderTime(svtkRenderer* ren, svtkVolume* vol)
{
  int i;
...
//-----------------------------------------------------------------------------
void svtkUnstructuredGridVolumeZSweepMapper::StoreRenderTime(
  svtkRenderer* ren, svtkVolume* vol, float time)
{
  int i;
  for (i = 0; i < this->RenderTableEntries; i++)
  {
  ...
  if (this->RenderTableEntries >= this->RenderTableSize)
  {
    ...
    svtkVolume** oldVolumePtr = this->RenderVolumeTable;
    svtkRenderer** oldRendererPtr = this->RenderRendererTable;

    this->RenderTimeTable = new float[this->RenderTableSize];
    this->RenderVolumeTable = new svtkVolume*[this->RenderTableSize];
    this->RenderRendererTable = new svtkRenderer*[this->RenderTableSize];
...
//-----------------------------------------------------------------------------
void svtkUnstructuredGridVolumeZSweepMapper::PrintSelf(ostream& os, svtkIndent indent)
{
...
int svtkUnstructuredGridVolumeZSweepMapper::GetMaxPixelListSize()
{
...
// parameter.
void svtkUnstructuredGridVolumeZSweepMapper::SetMaxPixelListSize(int size)
{
...
// Render the volume
void svtkUnstructuredGridVolumeZSweepMapper::Render(svtkRenderer* ren, svtkVolume* vol)
{
  svtkDebugMacro(<< "Render");
  ...
  {
    svtkErrorMacro("Can't use the ZSweep mapper without scalars!");
    return;
  }
  ...
  else
  {
    if (this->CellScalars)
    {
      ESTABLISH_INTEGRATOR(svtkUnstructuredGridHomogeneousRayIntegrator);
    }
    else
    {
      if (vol->GetProperty()->GetIndependentComponents())
      {
        ESTABLISH_INTEGRATOR(svtkUnstructuredGridPreIntegration);
      }
      else
      {
        ESTABLISH_INTEGRATOR(svtkUnstructuredGridPartialPreIntegration);
      }
    }
  }
  ...
  svtkDebugMacro(<< "BuildUseSets: start");
  this->BuildUseSets();
  svtkDebugMacro(<< "BuildUseSets: done");
  ...
  svtkDebugMacro(<< "ProjectAndSortVertices: start");
  this->ProjectAndSortVertices(ren, vol);
  svtkDebugMacro(<< "ProjectAndSortVertices: done");

  // 3. Create an empty "pixel list" (two way linked list) for each pixel of
  //    the screen.
  svtkDebugMacro(<< "CreateAndCleanPixelList: start");
  this->CreateAndCleanPixelList();
  svtkDebugMacro(<< "CreateAndCleanPixelList: done");

  // 4. Main loop
  // (section 2 paragraph 11)
  svtkDebugMacro(<< "MainLoop: start");
  this->MainLoop(ren->GetRenderWindow());
  svtkDebugMacro(<< "MainLoop: done");
...
//-----------------------------------------------------------------------------
void svtkUnstructuredGridVolumeZSweepMapper::AllocateUseSet(svtkIdType size)
{
  if (this->UseSet != nullptr)
  {
    if (size > static_cast<svtkIdType>(this->UseSet->Vector.size()))
    {
      delete this->UseSet;
      this->UseSet = new svtkUseSet(size);
    }
    else
    {
      this->UseSet->Clear();
...
//-----------------------------------------------------------------------------
void svtkUnstructuredGridVolumeZSweepMapper::AllocateVertices(svtkIdType size)
{
  if (this->Vertices != nullptr)
  {
    if (size > static_cast<svtkIdType>(this->Vertices->Vector.size()))
    {
      delete this->Vertices;
      this->Vertices = new svtkVertices(size);
    }
  }
  else
  {
...
//-----------------------------------------------------------------------------
void svtkUnstructuredGridVolumeZSweepMapper::BuildUseSets()
{
  int needsUpdate = 0;
  ...
  svtkIdList* cellNeighbors = svtkIdList::New();
  ...
  for (cellIter->InitTraversal(); !cellIter->IsDoneWithTraversal(); cellIter->GoToNextCell())
  {
    ...
    svtkIdType faces = this->Cell->GetNumberOfFaces();
    if (faces > 0)
    {
      svtkIdType faceidx = 0;
      svtkCell* face;
      svtkIdType faceIds[3];
      svtkIdType orderedFaceIds[3];
      // for each face
      while (faceidx < faces)
      {
...
// has changed.
int svtkUnstructuredGridVolumeZSweepMapper::ReorderTriangle(svtkIdType v[3], svtkIdType w[3])
{
  if (v[0] > v[1])
  {
    if (v[1] > v[2])
...
//-----------------------------------------------------------------------------
void svtkUnstructuredGridVolumeZSweepMapper::ProjectAndSortVertices(svtkRenderer* ren, svtkVolume* vol)
{
  ...
  svtkVertexEntry* vertex = nullptr;
...
//-----------------------------------------------------------------------------
void svtkUnstructuredGridVolumeZSweepMapper::CreateAndCleanPixelList()
{
  // paper: a "pixel list" is a double linked list. We put that in a queue.
  svtkIdType size = this->ImageInUseSize[0] * this->ImageInUseSize[1];
  ...
  if (this->PixelListFrame == nullptr)
  {
    this->PixelListFrame = new svtkPixelListFrame(size);
  }
}

//-----------------------------------------------------------------------------
void svtkUnstructuredGridVolumeZSweepMapper::MainLoop(svtkRenderWindow* renWin)
{
  ...
  svtkIdType vertex = this->EventList->Peek(0, previousZTarget);
  ...
  svtkIdType progressCount = 0;
  svtkIdType sum = this->EventList->GetNumberOfItems();

  if (this->MemoryManager == nullptr)
  {
    this->MemoryManager = new svtkPixelListEntryMemory;
  }
    ...
    { // otherwise the vertex is not useful, basically this is the
      ...
      if (previousZTarget == currentZ)
      {
        ...
        while (it != itEnd)
        {
          ...
          svtkIdType* vids = face->GetFaceIds();
          svtkIdType i = 0;
          while (i < 3)
          {
...
//        face->SetRendered(1);
  ...
  if (!aborded)
  {
    ...
    svtkDebugMacro(<< "Flush Compositing");
//   this->SavePixelListFrame();
  ...
  //  svtkDebugMacro(<<"MaxRecordedPixelListSize="<<this->MaxRecordedPixelListSize);

  assert("post: empty_list" && this->EventList->GetNumberOfItems() == 0);
}

//-----------------------------------------------------------------------------
void svtkUnstructuredGridVolumeZSweepMapper::SavePixelListFrame()
{
  svtkPolyData* dataset = svtkPolyData::New();

  svtkIdType height = this->ImageInUseSize[1];
  svtkIdType width = this->ImageInUseSize[0];
  svtkPixelListEntry* current;
  svtkIdType i;

  svtkPoints* pts = svtkPoints::New();
  pts->SetDataTypeToDouble();

  svtkDoubleArray* dataArray = svtkDoubleArray::New();
...
void svtkUnstructuredGridVolumeZSweepMapper::RasterizeFace(svtkIdType faceIds[3], int externalSide)
{
  ...
  svtkVertexEntry* v0 = &(this->Vertices->Vector[faceIds[0]]);
  svtkVertexEntry* v1 = &(this->Vertices->Vector[faceIds[1]]);
  svtkVertexEntry* v2 = &(this->Vertices->Vector[faceIds[2]]);

  bool exitFace = false;

  // Find the orientation of the triangle on the screen to get the right
  // scalar
  if ((externalSide != svtkFace::NOT_EXTERNAL) || this->CellScalars)
  {
    ...
    // space.
    if (externalSide == svtkFace::FRONT_FACE)
    {
#ifdef BACK_TO_FRONT
      exitFace = (zcross >= 0);
#else
    ...
    else if (externalSide == svtkFace::BACK_FACE)
    {
#ifdef BACK_TO_FRONT
      exitFace = (zcross <= 0);
#else
...
// Description:
// Perform a scan conversion of a triangle, interpolating z and the scalar.
void svtkUnstructuredGridVolumeZSweepMapper::RasterizeTriangle(
  svtkVertexEntry* ve0, svtkVertexEntry* ve1, svtkVertexEntry* ve2, bool externalFace)
{
  assert("pre: ve0_exists" && ve0 != nullptr);
  assert("pre: ve1_exists" && ve1 != nullptr);
  assert("pre: ve2_exists" && ve2 != nullptr);

  svtkVertexEntry* v0 = ve0;
  svtkVertexEntry* v1 = ve1;
  svtkVertexEntry* v2 = ve2;
  ...
  svtkScreenEdge* leftEdge = nullptr;
  svtkScreenEdge* rightEdge = nullptr;
  ...
  if (det == 0) // v0v1v2 aligned or v0=v1=v2
  {
      ...
      v0->GetScreenY() == v1->GetScreenY() && v0->GetScreenY() == v2->GetScreenY())
    {
      ...
      {
        svtkIdType i = y * this->ImageInUseSize[0] + x;
        // Write the pixel
        svtkPixelListEntry* p0 = this->MemoryManager->AllocateEntry();
        p0->Init(v0->GetValues(), v0->GetZview(), externalFace);
        if (this->CellScalars)
        {
          p0->GetValues()[SVTK_VALUES_SCALAR_INDEX] = this->FaceScalars[this->FaceSide];
        }
        this->PixelListFrame->AddAndSort(i, p0);

        svtkPixelListEntry* p1 = this->MemoryManager->AllocateEntry();
        p1->Init(v1->GetValues(), v1->GetZview(), externalFace);
        if (this->CellScalars)
        {
          p1->GetValues()[SVTK_VALUES_SCALAR_INDEX] = this->FaceScalars[this->FaceSide];
        }
        this->PixelListFrame->AddAndSort(i, p1);

        svtkPixelListEntry* p2 = this->MemoryManager->AllocateEntry();
        p2->Init(v2->GetValues(), v2->GetZview(), externalFace);
        if (this->CellScalars)
        {
          p2->GetValues()[SVTK_VALUES_SCALAR_INDEX] = this->FaceScalars[this->FaceSide];
        }
...
//-----------------------------------------------------------------------------
void svtkUnstructuredGridVolumeZSweepMapper::RasterizeSpan(
  int y, svtkScreenEdge* left, svtkScreenEdge* right, bool exitFace)
{
  assert("pre: left_exists" && left != nullptr);
  assert("pre: right_exists" && right != nullptr);

  svtkIdType i = y * this->ImageInUseSize[0];
  ...
  while (!this->Span->IsAtEnd())
  {
    ...
    if (x >= 0 && x < this->ImageInUseSize[0]) // clipping
    {
      svtkIdType j = i + x;
      // Write the pixel
      svtkPixelListEntry* p = this->MemoryManager->AllocateEntry();
      p->Init(this->Span->GetValues(), this->Span->GetZview(), exitFace);

      if (this->CellScalars)
      {
        p->GetValues()[SVTK_VALUES_SCALAR_INDEX] = this->FaceScalars[this->FaceSide];
      }
...
enum
{
  SVTK_LINE_CONSTANT = 0,
  SVTK_LINE_BRESENHAM,
  SVTK_LINE_DIAGONAL
};

//-----------------------------------------------------------------------------
void svtkUnstructuredGridVolumeZSweepMapper::RasterizeLine(
  svtkVertexEntry* v0, svtkVertexEntry* v1, bool exitFace)
{
  ...
  if (xIncrement)
  {
    if (dy == 0)
    {
      lineCase = SVTK_LINE_CONSTANT;
    }
    else
    {
      lineCase = SVTK_LINE_BRESENHAM;
      dx2 = dx << 1;
      dy2 = dy << 1;
      e = dx;
    }
  ...
  else
  {
    if (dx == 0)
    {
      if (dy == 0)
      {
        ...
        {
          svtkIdType j = y * this->ImageInUseSize[0] + x; // mult==bad!!
          // Write the pixel
          svtkPixelListEntry* p0 = this->MemoryManager->AllocateEntry();
  ...
  while (!done)
  {
    ...
    switch (lineCase)
    {
      case SVTK_LINE_CONSTANT:
        ...
        // values, invw, zview
        break;
      case SVTK_LINE_DIAGONAL:
        ++y;
        x += xSign;
        done = y > v1->GetScreenY();
        // values, invw, zview
        break;
      case SVTK_LINE_BRESENHAM:
        if (xIncrement)
        {
          x += xSign;
          e += dy2;
...
//-----------------------------------------------------------------------------
void svtkUnstructuredGridVolumeZSweepMapper::CompositeFunction(double zTarget)
{
  int y = this->YBounds[0];
  svtkIdType i = y * this->ImageInUseSize[0] + this->XBounds[0];

  svtkIdType index = (y * this->ImageMemorySize[0] + this->XBounds[0]) << 2; // *4
  svtkIdType indexStep = this->ImageMemorySize[0] << 2;                      // *4
        ...
        while (!done)
        {
          ...
          if (doIntegration)
          {
            if (current->GetZview() != next->GetZview())
            {
              ...
              length =
                sqrt(svtkMath::Distance2BetweenPoints(current->GetValues(), next->GetValues()));
              if (length != 0)
              //              if(length>=0.4)
              {
                ...
                if (this->CellScalars)
                {
                  ...
                  this->NearIntersections->SetValue(
                    0, current->GetValues()[SVTK_VALUES_SCALAR_INDEX]);
                ...
                else
                {
                  this->NearIntersections->SetValue(
                    0, current->GetValues()[SVTK_VALUES_SCALAR_INDEX]);
                  this->FarIntersections->SetValue(0, next->GetValues()[SVTK_VALUES_SCALAR_INDEX]);
                }
#ifdef BACK_TO_FRONT
...
unsigned char svtkUnstructuredGridVolumeZSweepMapper::ColorComponentRealToByte(float color)
{
...
//-----------------------------------------------------------------------------
double svtkUnstructuredGridVolumeZSweepMapper::GetZBufferValue(int x, int y)
{
  int xPos, yPos;
...
//-----------------------------------------------------------------------------
double svtkUnstructuredGridVolumeZSweepMapper::GetMinimumBoundsDepth(
