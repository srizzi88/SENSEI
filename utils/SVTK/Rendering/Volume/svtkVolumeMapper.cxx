...
// Construct a svtkVolumeMapper with empty scalar input and clipping off.
svtkVolumeMapper::svtkVolumeMapper()
{
  int i;

  this->BlendMode = svtkVolumeMapper::COMPOSITE_BLEND;
  this->AverageIPScalarRange[0] = SVTK_FLOAT_MIN;
  this->AverageIPScalarRange[1] = SVTK_FLOAT_MAX;
  ...
  this->CroppingRegionFlags = SVTK_CROP_SUBVOLUME;
}

svtkVolumeMapper::~svtkVolumeMapper() = default;

void svtkVolumeMapper::ConvertCroppingRegionPlanesToVoxels()
{
...
void svtkVolumeMapper::SetInputData(svtkDataSet* genericInput)
{
  svtkImageData* input = svtkImageData::SafeDownCast(genericInput);
  ...
  {
    svtkErrorMacro("The SetInput method of this mapper requires svtkImageData as input");
  }
}

void svtkVolumeMapper::SetInputData(svtkImageData* input)
{
  this->SetInputDataInternal(0, input);
}

svtkImageData* svtkVolumeMapper::GetInput()
{
...
svtkImageData* svtkVolumeMapper::GetInput(const int port)
{
  if (this->GetNumberOfInputConnections(0) < 1)
  {
    return nullptr;
  ...
  return svtkImageData::SafeDownCast(this->GetExecutive()->GetInputData(port, 0));
}

// Print the svtkVolumeMapper
void svtkVolumeMapper::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
int svtkVolumeMapper::FillInputPortInformation(int port, svtkInformation* info)
{
...
//----------------------------------------------------------------------------
double svtkVolumeMapper::SpacingAdjustedSampleDistance(double inputSpacing[3], int inputExtent[6])
{
