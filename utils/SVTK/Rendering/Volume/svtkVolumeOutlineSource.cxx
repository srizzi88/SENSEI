/*=========================================================================
  ...
  Module:    svtkVolumeOutlineSource.cxx
...
=========================================================================*/
#include "svtkVolumeOutlineSource.h"
...
svtkStandardNewMacro(svtkVolumeOutlineSource);

svtkCxxSetObjectMacro(svtkVolumeOutlineSource, VolumeMapper, svtkVolumeMapper);

//----------------------------------------------------------------------------
svtkVolumeOutlineSource::svtkVolumeOutlineSource()
{
...
//----------------------------------------------------------------------------
void svtkVolumeOutlineSource::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
int svtkVolumeOutlineSource::ComputeCubePlanes(
...
//----------------------------------------------------------------------------
int svtkVolumeOutlineSource::ComputePipelineMTime(svtkInformation* svtkNotUsed(request),
  ...
  int svtkNotUsed(requestFromOutputPort), svtkMTimeType* mtime)
{
  ...
  if (this->VolumeMapper)
  {
    svtkMTimeType mapperMTime = this->VolumeMapper->GetMTime();
    if (mapperMTime > mTime)
    {
      mTime = mapperMTime;
    }
    svtkDemandDrivenPipeline* input =
      svtkDemandDrivenPipeline::SafeDownCast(this->VolumeMapper->GetInputExecutive());
    if (input)
    {
      // Need to do this because we are not formally connected
      // to the Mapper's pipeline
      input->UpdateInformation();
      svtkMTimeType pipelineMTime = input->GetPipelineMTime();
...
//----------------------------------------------------------------------------
int svtkVolumeOutlineSource::RequestInformation(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** svtkNotUsed(inputVector), svtkInformationVector* svtkNotUsed(outputVector))
{
  ...
  if (!this->VolumeMapper)
  {
    svtkWarningMacro("No VolumeMapper has been set.");
    return 1;
  }

  svtkInformation* mapInfo = this->VolumeMapper->GetInputInformation();

  if (!mapInfo)
  {
    svtkWarningMacro("The VolumeMapper does not have an input set.");
    return 1;
  }
  ...
  mapInfo->Get(svtkDataObject::SPACING(), spacing);
  mapInfo->Get(svtkDataObject::ORIGIN(), origin);
  mapInfo->Get(svtkStreamingDemandDrivenPipeline::WHOLE_EXTENT(), extent);
...
//----------------------------------------------------------------------------
int svtkVolumeOutlineSource::RequestData(svtkInformation* svtkNotUsed(request),
  svtkInformationVector** svtkNotUsed(inputVector), svtkInformationVector* outputVector)
{
  ...
  svtkDebugMacro(<< "Creating cropping region outline");
  ...
  svtkUnsignedCharArray* scalars = nullptr;

  if (this->GenerateScalars)
  {
    scalars = svtkUnsignedCharArray::New();
    scalars->SetNumberOfComponents(3);
  }

  // Generate all the lines for the outline.
  svtkCellArray* lines = nullptr;
...
//----------------------------------------------------------------------------
void svtkVolumeOutlineSource::GeneratePolys(svtkCellArray* polys, svtkUnsignedCharArray* scalars,
...
//----------------------------------------------------------------------------
void svtkVolumeOutlineSource::GenerateLines(svtkCellArray* lines, svtkUnsignedCharArray* scalars,
  unsigned char colors[2][3], int activePlane, int flags, int tolPtId[3][4])
{
...
//----------------------------------------------------------------------------
void svtkVolumeOutlineSource::GeneratePoints(
      ...
      for (int k = 0; k < 4; k++)
      {
        ...
        if ((pointBits & 1))
        {
          ...
          for (int arrayId = 0; arrayId < 2; arrayId++)
          {
            ...
            svtkCellArray* cells = cellArrays[arrayId];
            if (cells)
            {
...
//----------------------------------------------------------------------------
void svtkVolumeOutlineSource::NudgeCropPlanesToBounds(
...
//----------------------------------------------------------------------------
void svtkVolumeOutlineSource::CreateColorValues(
