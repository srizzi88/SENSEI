...
svtkStandardNewMacro(svtkVolumePicker);

//----------------------------------------------------------------------------
svtkVolumePicker::svtkVolumePicker()
{
...
//----------------------------------------------------------------------------
svtkVolumePicker::~svtkVolumePicker() = default;

//----------------------------------------------------------------------------
void svtkVolumePicker::PrintSelf(ostream& os, svtkIndent indent)
{
...
//----------------------------------------------------------------------------
void svtkVolumePicker::ResetPickInfo()
{
...
//----------------------------------------------------------------------------
// Intersect a svtkVolume with a line by ray casting.  Compared to the
// same method in the superclass, this method will look for cropping planes.

double svtkVolumePicker::IntersectVolumeWithLine(const double p1[3], const double p2[3], double t1,
  double t2, svtkProp3D* prop, svtkAbstractVolumeMapper* mapper)
{
  ...
  svtkVolumeMapper* vmapper = svtkVolumeMapper::SafeDownCast(mapper);
  ...
  double s1 = s1List[0] = SVTK_DOUBLE_MAX;
  planeIdList[0] = -1;
  ...
  else
  {
    ...
    for (int segment = 0; segment < numSegments; segment++)
    {
      if ((tMin = this->Superclass::IntersectVolumeWithLine(
             p1, p2, t1List[segment], t2List[segment], prop, mapper)) < SVTK_DOUBLE_MAX)
      {
        s1 = s1List[segment];
...
int svtkVolumePicker::ClipLineWithCroppingRegion(const double bounds[6], const int extent[6],
  ...
  int& numSegments, double* t1List, double* t2List, double* s1List, int* planeIdList)
{
  ...
  if (!svtkVolumePicker::ClipLineWithExtent(extent, x1, x2, s1, s2, extentPlaneId))
  {
    return 0;
  }
  ...
  for (;;)
  {
    if (!svtkBox::IntersectWithLine(blockBounds, x1, x2, s1, s2, nullptr, nullptr, plane1, plane2))
    {
