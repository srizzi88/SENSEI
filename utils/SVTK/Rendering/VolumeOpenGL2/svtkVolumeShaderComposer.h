/*=========================================================================
  ...
  Module:    svtkVolumeShaderComposer.h
...
#ifndef svtkVolumeShaderComposer_h
#define svtkVolumeShaderComposer_h
...
namespace
{
bool HasGradientOpacity(svtkOpenGLGPUVolumeRayCastMapper::VolumeInputMap& inputs)
{
  for (auto& item : inputs)
  {
    svtkVolumeProperty* volProp = item.second.Volume->GetProperty();
...
bool HasLighting(svtkOpenGLGPUVolumeRayCastMapper::VolumeInputMap& inputs)
{
  for (auto& item : inputs)
  {
    svtkVolumeProperty* volProp = item.second.Volume->GetProperty();
...
bool UseClippedVoxelIntensity(svtkOpenGLGPUVolumeRayCastMapper::VolumeInputMap& inputs)
{
  for (auto& item : inputs)
  {
    svtkVolumeProperty* volProp = item.second.Volume->GetProperty();
...
namespace svtkvolume
{
//--------------------------------------------------------------------------
std::string ComputeClipPositionImplementation(
  svtkRenderer* svtkNotUsed(ren), svtkVolumeMapper* svtkNotUsed(mapper), svtkVolume* svtkNotUsed(vol))
{
  return std::string(
...
//--------------------------------------------------------------------------
std::string BaseDeclarationVertex(svtkRenderer* svtkNotUsed(ren), svtkVolumeMapper* mapper,
  svtkVolume* svtkNotUsed(vol), bool multipleInputs)
{
  auto gpuMapper = svtkGPUVolumeRayCastMapper::SafeDownCast(mapper);
...
//--------------------------------------------------------------------------
std::string BaseDeclarationFragment(svtkRenderer* svtkNotUsed(ren), svtkVolumeMapper* mapper,
  svtkOpenGLGPUVolumeRayCastMapper::VolumeInputMap& inputs, int svtkNotUsed(numberOfLights),
  int lightingComplexity, int noOfComponents, int independentComponents)
{
  ...
  svtkOpenGLGPUVolumeRayCastMapper* glMapper = svtkOpenGLGPUVolumeRayCastMapper::SafeDownCast(mapper);
  if (glMapper->GetCurrentPass() != svtkOpenGLGPUVolumeRayCastMapper::DepthPass &&
    glMapper->GetUseDepthPass())
  {
    toShaderStr << "uniform sampler2D in_depthPassSampler;\n";
  }

  if (glMapper->GetBlendMode() == svtkVolumeMapper::ISOSURFACE_BLEND)
  {
  ...
  else if (glMapper->GetBlendMode() == svtkVolumeMapper::SLICE_BLEND)
  {
    svtkVolume* vol = inputs.begin()->second.Volume;
    svtkImplicitFunction* func = vol->GetProperty()->GetSliceFunction();

    if (func && func->IsA("svtkPlane"))
    {
      toShaderStr
...
//--------------------------------------------------------------------------
std::string BaseInit(svtkRenderer* svtkNotUsed(ren), svtkVolumeMapper* mapper,
  svtkOpenGLGPUVolumeRayCastMapper::VolumeInputMap& inputs, int lightingComplexity)
{
  svtkOpenGLGPUVolumeRayCastMapper* glMapper = svtkOpenGLGPUVolumeRayCastMapper::SafeDownCast(mapper);
  svtkVolume* vol = inputs.begin()->second.Volume;
  ...
  if (glMapper->GetCurrentPass() != svtkOpenGLGPUVolumeRayCastMapper::DepthPass &&
    glMapper->GetUseDepthPass() && glMapper->GetBlendMode() == svtkVolumeMapper::COMPOSITE_BLEND)
  {
  ...
  if (glMapper->GetBlendMode() != svtkVolumeMapper::SLICE_BLEND)
  {
...
std::string BaseImplementation(
  svtkRenderer* svtkNotUsed(ren), svtkVolumeMapper* mapper, svtkVolume* svtkNotUsed(vol))
{
  svtkOpenGLGPUVolumeRayCastMapper* glMapper = svtkOpenGLGPUVolumeRayCastMapper::SafeDownCast(mapper);
...
//--------------------------------------------------------------------------
std::string ComputeGradientOpacity1DDecl(svtkVolume* vol, int noOfComponents,
...
std::string ComputeGradientDeclaration(
  svtkOpenGLGPUVolumeRayCastMapper* mapper, svtkOpenGLGPUVolumeRayCastMapper::VolumeInputMap& inputs)
{
...
//--------------------------------------------------------------------------
std::string ComputeLightingDeclaration(svtkRenderer* svtkNotUsed(ren), svtkVolumeMapper* mapper,
  svtkVolume* vol, int noOfComponents, int independentComponents, int svtkNotUsed(numberOfLights),
  int lightingComplexity)
{
  ...
  int const shadeReqd = volProperty->GetShade() &&
    (mapper->GetBlendMode() == svtkVolumeMapper::COMPOSITE_BLEND ||
      mapper->GetBlendMode() == svtkVolumeMapper::ISOSURFACE_BLEND ||
      mapper->GetBlendMode() == svtkVolumeMapper::SLICE_BLEND);
  ...
  auto glMapper = svtkOpenGLGPUVolumeRayCastMapper::SafeDownCast(mapper);
  ...
  // is defined.
  if (transferMode == svtkVolumeProperty::TF_1D && glMapper->GetInputCount() == 1)
  {
    if (noOfComponents == 1 || !independentComponents)
    {
...
//--------------------------------------------------------------------------
std::string ComputeRayDirectionDeclaration(svtkRenderer* ren, svtkVolumeMapper* svtkNotUsed(mapper),
  svtkVolume* svtkNotUsed(vol), int svtkNotUsed(noOfComponents))
{
...
//--------------------------------------------------------------------------
std::string ComputeColorDeclaration(svtkRenderer* svtkNotUsed(ren),
  svtkVolumeMapper* svtkNotUsed(mapper), svtkVolume* svtkNotUsed(vol), int noOfComponents,
...
//--------------------------------------------------------------------------
std::string ComputeColorMultiDeclaration(svtkOpenGLGPUVolumeRayCastMapper::VolumeInputMap& inputs)
{
  std::ostringstream ss;
  int i = 0;
  for (auto& item : inputs)
  {
    ...
    if (prop->GetTransferFunctionMode() != svtkVolumeProperty::TF_1D)
      continue;
...
//--------------------------------------------------------------------------
std::string ComputeOpacityMultiDeclaration(svtkOpenGLGPUVolumeRayCastMapper::VolumeInputMap& inputs)
{
  std::ostringstream ss;
  int i = 0;
  for (auto& item : inputs)
...
std::string ComputeGradientOpacityMulti1DDecl(
  svtkOpenGLGPUVolumeRayCastMapper::VolumeInputMap& inputs)
{
  std::ostringstream ss;
  ...
  {
    auto prop = item.second.Volume->GetProperty();
    if (prop->GetTransferFunctionMode() != svtkVolumeProperty::TF_1D || !prop->HasGradientOpacity())
      continue;
...
//--------------------------------------------------------------------------
std::string ComputeOpacityDeclaration(svtkRenderer* svtkNotUsed(ren),
  svtkVolumeMapper* svtkNotUsed(mapper), svtkVolume* svtkNotUsed(vol), int noOfComponents,
...
//--------------------------------------------------------------------------
std::string ComputeColor2DDeclaration(svtkRenderer* svtkNotUsed(ren),
  svtkVolumeMapper* svtkNotUsed(mapper), svtkVolume* svtkNotUsed(vol), int noOfComponents,
  int independentComponents, std::map<int, std::string> colorTableMap)
{
  if (noOfComponents == 1)
...
std::string Transfer2DDeclaration(svtkOpenGLGPUVolumeRayCastMapper::VolumeInputMap& inputs)
{
  std::ostringstream ss;
  int i = 0;
  for (auto& item : inputs)
  {
    auto prop = item.second.Volume->GetProperty();
    if (prop->GetTransferFunctionMode() != svtkVolumeProperty::TF_2D)
      continue;
...
//--------------------------------------------------------------------------
std::string ComputeOpacity2DDeclaration(svtkRenderer* svtkNotUsed(ren),
  svtkVolumeMapper* svtkNotUsed(mapper), svtkVolume* svtkNotUsed(vol), int noOfComponents,
  int independentComponents, std::map<int, std::string> opacityTableMap)
{
  std::ostringstream toString;
...
std::string ShadingDeclarationFragment(
  svtkRenderer* svtkNotUsed(ren), svtkVolumeMapper* mapper, svtkVolume* svtkNotUsed(vol))
{
  if (mapper->GetBlendMode() == svtkVolumeMapper::MAXIMUM_INTENSITY_BLEND)
  {
    return std::string("\
        \n bool l_firstValue;\
        \n vec4 l_maxValue;");
  }
  else if (mapper->GetBlendMode() == svtkVolumeMapper::MINIMUM_INTENSITY_BLEND)
  {
    return std::string("\
        \n bool l_firstValue;\
        \n vec4 l_minValue;");
  }
  else if (mapper->GetBlendMode() == svtkVolumeMapper::AVERAGE_INTENSITY_BLEND)
  {
    return std::string("\
        \n  uvec4 l_numSamples;\
        \n  vec4 l_avgValue;");
  }
  else if (mapper->GetBlendMode() == svtkVolumeMapper::ADDITIVE_BLEND)
  {
    return std::string("\
        \n  vec4 l_sumValue;");
  }
  else if (mapper->GetBlendMode() == svtkVolumeMapper::ISOSURFACE_BLEND)
  {
    return std::string("\
...
//--------------------------------------------------------------------------
std::string GradientCacheDec(svtkRenderer* svtkNotUsed(ren), svtkVolume* svtkNotUsed(vol),
  svtkOpenGLGPUVolumeRayCastMapper::VolumeInputMap& inputs, int independentComponents = 0)
{
  const int numInputs = static_cast<int>(inputs.size());
...
//--------------------------------------------------------------------------
std::string PreComputeGradientsImpl(svtkRenderer* svtkNotUsed(ren), svtkVolume* svtkNotUsed(vol),
...
std::string ShadingMultipleInputs(
  svtkVolumeMapper* mapper, svtkOpenGLGPUVolumeRayCastMapper::VolumeInputMap& inputs)
{
  std::ostringstream toShaderStr;
  ...
  {
    case svtkVolumeMapper::COMPOSITE_BLEND:
    default:
    {
      int i = 0;
      for (auto& item : inputs)
      {
        ...
        if (property->GetTransferFunctionMode() == svtkVolumeProperty::TF_1D)
        {
        ...
        else if (property->GetTransferFunctionMode() == svtkVolumeProperty::TF_2D)
        {
          const auto& grad = input.GradientCacheName;
          toShaderStr <<
...
//--------------------------------------------------------------------------
std::string ShadingSingleInput(svtkRenderer* svtkNotUsed(ren), svtkVolumeMapper* mapper,
  svtkVolume* svtkNotUsed(vol), svtkImageData* maskInput, svtkVolumeTexture* mask, int maskType,
  int noOfComponents, int independentComponents = 0)
{
  ...
  else if (mapper->GetBlendMode() == svtkVolumeMapper::SLICE_BLEND)
  {
    shaderStr += std::string("\
  ...
  else if (mapper->GetBlendMode() == svtkVolumeMapper::COMPOSITE_BLEND)
  {
    if (noOfComponents > 1 && independentComponents)
    {
      shaderStr += std::string("\
      ...
      if (glMapper->GetUseDepthPass() &&
        glMapper->GetCurrentPass() == svtkOpenGLGPUVolumeRayCastMapper::DepthPass)
      {
        shaderStr += std::string("\
      ...
      else if (!mask || !maskInput || maskType != svtkGPUVolumeRayCastMapper::LabelMapMaskType)
      {
        shaderStr += std::string("\
          \n        // Data fetching from the red channel of volume texture\
    ...
    {
      if (!mask || !maskInput || maskType != svtkGPUVolumeRayCastMapper::LabelMapMaskType)
      {
        shaderStr += std::string("\
             \n      g_srcColor = vec4(0.0);\
             \n      g_srcColor.a = computeOpacity(scalar);\
  ...
  svtkRenderer* svtkNotUsed(ren), svtkVolumeMapper* svtkNotUsed(mapper), svtkVolume* svtkNotUsed(vol))
{
  return std::string("\
    ...
    \n  // pass svtkHardwareSelection::ACTOR_PASS.\
  ...
  svtkRenderer* svtkNotUsed(ren), svtkVolumeMapper* svtkNotUsed(mapper), svtkVolume* svtkNotUsed(vol))
{
  ...
  \n  // Special coloring mode which renders the voxel index in fragments that\
  \n  // have accumulated certain level of opacity. Used during the selection\
  \n  // pass svtkHardwareSelection::ID_MID24.\
  \n  if (g_fragColor.a > 3.0/ 255.0)\
  \n    {\
  \n    uvec3 volumeDim = uvec3(in_textureExtentsMax - in_textureExtentsMin);\
  \n    uvec3 voxelCoords = uvec3(volumeDim * g_dataPos);\
...
//--------------------------------------------------------------------------
std::string ShadingExit(svtkRenderer* svtkNotUsed(ren), svtkVolumeMapper* mapper,
  svtkVolume* svtkNotUsed(vol), int noOfComponents, int independentComponents = 0)
{
  svtkOpenGLGPUVolumeRayCastMapper* glMapper = svtkOpenGLGPUVolumeRayCastMapper::SafeDownCast(mapper);
  ...
  else if (mapper->GetBlendMode() == svtkVolumeMapper::MAXIMUM_INTENSITY_BLEND)
  {
    if (noOfComponents > 1 && independentComponents)
    {
      return std::string("\
...
//--------------------------------------------------------------------------
std::string TerminationInit(svtkRenderer* svtkNotUsed(ren), svtkVolumeMapper* mapper, svtkVolume* vol)
{
  std::string shaderStr;
  shaderStr += std::string("\
  ...
  if (mapper->GetBlendMode() == svtkVolumeMapper::SLICE_BLEND)
  {
    svtkImplicitFunction* sliceFunc = vol->GetProperty()->GetSliceFunction();
    if (sliceFunc)
    {
      if (sliceFunc->IsA("svtkPlane"))
      {
        shaderStr += std::string("\
          \n\
...
//--------------------------------------------------------------------------
std::string ClippingInit(svtkRenderer* ren, svtkVolumeMapper* mapper, svtkVolume* svtkNotUsed(vol))
{
  if (!mapper->GetClippingPlanes())
  {
    return std::string();
...
//--------------------------------------------------------------------------
std::string BinaryMaskDeclaration(svtkRenderer* svtkNotUsed(ren), svtkVolumeMapper* svtkNotUsed(mapper),
  svtkVolume* svtkNotUsed(vol), svtkImageData* maskInput, svtkVolumeTexture* mask,
  int svtkNotUsed(maskType))
{
  if (!mask || !maskInput)
  {
    return std::string();
...
//--------------------------------------------------------------------------
std::string BinaryMaskImplementation(svtkRenderer* svtkNotUsed(ren),
  svtkVolumeMapper* svtkNotUsed(mapper), svtkVolume* svtkNotUsed(vol), svtkImageData* maskInput,
  svtkVolumeTexture* mask, int maskType)
{
  if (!mask || !maskInput || maskType == svtkGPUVolumeRayCastMapper::LabelMapMaskType)
  {
    return std::string();
...
//--------------------------------------------------------------------------
std::string CompositeMaskDeclarationFragment(svtkRenderer* svtkNotUsed(ren),
  svtkVolumeMapper* svtkNotUsed(mapper), svtkVolume* svtkNotUsed(vol), svtkImageData* maskInput,
  svtkVolumeTexture* mask, int maskType)
{
  if (!mask || !maskInput || maskType != svtkGPUVolumeRayCastMapper::LabelMapMaskType)
...
//--------------------------------------------------------------------------
std::string CompositeMaskImplementation(svtkRenderer* svtkNotUsed(ren),
  svtkVolumeMapper* svtkNotUsed(mapper), svtkVolume* svtkNotUsed(vol), svtkImageData* maskInput,
  svtkVolumeTexture* mask, int maskType, int noOfComponents)
{
  if (!mask || !maskInput || maskType != svtkGPUVolumeRayCastMapper::LabelMapMaskType)
  {
    return std::string();
...
#endif // svtkVolumeShaderComposer_h
// SVTK-HeaderTest-Exclude: svtkVolumeShaderComposer.h
