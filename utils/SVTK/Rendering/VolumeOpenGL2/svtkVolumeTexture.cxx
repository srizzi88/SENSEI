...
svtkVolumeTexture::svtkVolumeTexture()
  : HandleLargeDataTypes(false)
  , InterpolationType(svtkTextureObject::Linear)
  , Texture(nullptr)
  , CurrentBlockIdx(0)
  , StreamBlocks(false)
  , Scalars(nullptr)
...
//-----------------------------------------------------------------------------
svtkStandardNewMacro(svtkVolumeTexture);

//-----------------------------------------------------------------------------
bool svtkVolumeTexture::LoadVolume(svtkRenderer* ren, svtkImageData* data, svtkDataArray* scalars,
  int const isCell, int const interpolation)
{
  ...
  else // Single block
  {
    ...
    svtkImageData* singleBlock = svtkImageData::New();
    singleBlock->ShallowCopy(data);
    singleBlock->SetExtent(this->FullExtent.GetData());
    this->ImageDataBlocks.push_back(singleBlock);
  }

  // Get default formats from svtkTextureObject
  if (!this->Texture)
  {
    this->Texture = svtkSmartPointer<svtkTextureObject>::New();
...
//-----------------------------------------------------------------------------
void svtkVolumeTexture::SetInterpolation(int const interpolation)
{
  this->InterpolationType = interpolation;
...
//-----------------------------------------------------------------------------
svtkVolumeTexture::VolumeBlock* svtkVolumeTexture::GetNextBlock()
{
...
//-----------------------------------------------------------------------------
svtkVolumeTexture::VolumeBlock* svtkVolumeTexture::GetCurrentBlock()
{
  return this->SortedVolumeBlocks[this->CurrentBlockIdx];
}

//-----------------------------------------------------------------------------
void svtkVolumeTexture::CreateBlocks(
  unsigned int const format, unsigned int const internalFormat, int const type)
{
  ...
  {
    svtkImageData* imData = this->ImageDataBlocks.at(i);
...
//-----------------------------------------------------------------------------
void svtkVolumeTexture::AdjustExtentForCell(Size6& extent)
{
  int i = 1;
  while (i < 6)
  {
...
//-----------------------------------------------------------------------------
svtkVolumeTexture::Size3 svtkVolumeTexture::ComputeBlockSize(int* extent)
{
  int i = 0;
  Size3 texSize;
  while (i < 3)
...
//-----------------------------------------------------------------------------
bool svtkVolumeTexture::LoadTexture(int const interpolation, VolumeBlock* volBlock)
{
  int const noOfComponents = this->Scalars->GetNumberOfComponents();
  int scalarType = this->Scalars->GetDataType();

  svtkSmartPointer<svtkImageData> block = volBlock->ImageData;
  int blockExt[6];
  block->GetExtent(blockExt);
  Size3 const& blockSize = volBlock->TextureSize;
  svtkTextureObject* texture = volBlock->TextureObject;
  svtkIdType const& tupleIdx = volBlock->TupleIndex;
  ...
  if (!this->HandleLargeDataTypes)
  {
    ...
    svtkIdType const dataIdx = tupleIdx * noOfComponents;
    ...
    texture->Activate();
    texture->SetWrapS(svtkTextureObject::ClampToEdge);
    texture->SetWrapT(svtkTextureObject::ClampToEdge);
    texture->SetWrapR(svtkTextureObject::ClampToEdge);
  ...
  else // Handle 64-bit types
  {
    ...
    svtkFloatArray* sliceArray = svtkFloatArray::New();
    sliceArray->SetNumberOfComponents(noOfComponents);
    sliceArray->SetNumberOfTuples(blockSize[0] * blockSize[1]);

    int k = 0;
    svtkIdType const kInc = this->FullSize[0] * this->FullSize[1];
    svtkIdType kOffset = tupleIdx;

    float* tupPtr = new float[noOfComponents];
    while (k < blockSize[2])
    {
      int j = 0;
      svtkIdType jOffset = 0;
      svtkIdType jDestOffset = 0;
      while (j < blockSize[1])
      {
        int i = 0;
        while (i < blockSize[0])
...
//-----------------------------------------------------------------------------
void svtkVolumeTexture::ReleaseGraphicsResources(svtkWindow* win)
{
  if (this->Texture)
  {
...
//-----------------------------------------------------------------------------
void svtkVolumeTexture::ClearBlocks()
{
...
//-----------------------------------------------------------------------------
void svtkVolumeTexture::SplitVolume(svtkImageData* imageData, Size3 const& part)
{
  ...
  this->ImageDataBlocks = std::vector<svtkImageData*>();
  ...
  for (int k = 0; k < static_cast<int>(numBlocks_z); k++)
  {
    for (int j = 0; j < static_cast<int>(numBlocks_y); j++)
    {
      for (int i = 0; i < static_cast<int>(numBlocks_x); i++)
      {
        ...
        // Create a proxy svtkImageData object for each block
        svtkImageData* block = svtkImageData::New();
...
//-----------------------------------------------------------------------------
void svtkVolumeTexture::GetScaleAndBias(
  const int scalarType, float* scalarRange, float& scale, float& bias)
{
  ...
  switch (scalarType)
  {
    case SVTK_UNSIGNED_CHAR:
      glScale = 1.0 / (SVTK_UNSIGNED_CHAR_MAX + 1);
      glBias = 0.0;
      break;
    case SVTK_SIGNED_CHAR:
      glScale = 2.0 / (SVTK_UNSIGNED_CHAR_MAX + 1);
      glBias = -1.0 - SVTK_SIGNED_CHAR_MIN * glScale;
      break;
    case SVTK_SHORT:
      glScale = 2.0 / (SVTK_UNSIGNED_SHORT_MAX + 1);
      glBias = -1.0 - SVTK_SHORT_MIN * glScale;
      break;
    case SVTK_UNSIGNED_SHORT:
      glScale = 1.0 / (SVTK_UNSIGNED_SHORT_MAX + 1);
      glBias = 0.0;
      break;
    case SVTK_CHAR:
    case SVTK_BIT:
...
//-----------------------------------------------------------------------------
void svtkVolumeTexture::SelectTextureFormat(unsigned int& format, unsigned int& internalFormat,
  int& type, int const scalarType, int const noOfComponents)
{
  ...
  int const components = svtkMath::Min(noOfComponents, 4);
...
//----------------------------------------------------------------------------
void svtkVolumeTexture::UpdateVolume(svtkVolumeProperty* property)
{
...
//----------------------------------------------------------------------------
void svtkVolumeTexture::UpdateInterpolationType(int const interpolation)
{
  if (interpolation == SVTK_LINEAR_INTERPOLATION &&
    this->InterpolationType != svtkTextureObject::Linear)
  {
    this->SetInterpolation(svtkTextureObject::Linear);
  }
  else if (interpolation == SVTK_NEAREST_INTERPOLATION &&
    this->InterpolationType != svtkTextureObject::Nearest)
  {
    this->SetInterpolation(svtkTextureObject::Nearest);
  }
  else if (interpolation != SVTK_LINEAR_INTERPOLATION && interpolation != SVTK_NEAREST_INTERPOLATION)
  {
...
//----------------------------------------------------------------------------
void svtkVolumeTexture::SortBlocksBackToFront(svtkRenderer* ren, svtkMatrix4x4* volumeMat)
{
  if (this->ImageDataBlocks.size() > 1)
  {
    svtkBlockSortHelper::BackToFront<svtkImageData> sortBlocks(ren, volumeMat);
...
//-----------------------------------------------------------------------------
void svtkVolumeTexture::SetPartitions(int const x, int const y, int const z)
{
  if (x > 0 && y > 0 && z > 0)
  {
    if (x > 1 || y > 1 || z > 1)
...
//-----------------------------------------------------------------------------
const svtkVolumeTexture::Size3& svtkVolumeTexture::GetPartitions()
{
  return this->Partitions;
}

//-----------------------------------------------------------------------------
void svtkVolumeTexture::PrintSelf(ostream& os, svtkIndent indent)
{
...
//-----------------------------------------------------------------------------
bool svtkVolumeTexture::AreDimensionsValid(
...
//-----------------------------------------------------------------------------
bool svtkVolumeTexture::SafeLoadTexture(svtkTextureObject* texture, int const width, int const height,
  int const depth, int numComps, int dataType, void* dataPtr)
{
  if (!AreDimensionsValid(texture, width, height, depth))
  {
    svtkErrorMacro(<< "Invalid texture dimensions [" << width << ", " << height << ", " << depth
                  << "]");
    return false;
  }
  ...
  {
    svtkErrorMacro(<< "Capabilities check via proxy texture 3D allocation "
                     "failed!");
    return false;
  }
  ...
  {
    svtkErrorMacro(<< "Texture 3D allocation failed! \n");
    return false;
  }
...
//----------------------------------------------------------------------------
void svtkVolumeTexture::ComputeBounds(VolumeBlock* block)
{
  svtkImageData* input = block->ImageData;
  double spacing[3];
...
//----------------------------------------------------------------------------
void svtkVolumeTexture::UpdateTextureToDataMatrix(VolumeBlock* block)
{
  ...
  svtkMatrix4x4* matrix = block->TextureToDataset;
  matrix->Identity();
  ...
  svtkImageData::TransformContinuousIndexToPhysicalPoint(block->Extents[0], block->Extents[2],
...
//-----------------------------------------------------------------------------
void svtkVolumeTexture::ComputeCellToPointMatrix(int extents[6])
{
  this->CellToPointMatrix->Identity();
  this->AdjustedTexMin[0] = this->AdjustedTexMin[1] = this->AdjustedTexMin[2] = 0.0f;
  this->AdjustedTexMin[3] = 1.0f;
...
//----------------------------------------------------------------------------
svtkDataArray* svtkVolumeTexture::GetLoadedScalars()
{
  return this->Scalars;
}
