...
## =========================================================================*/
## .NAME HeaderTesting - a SVTK style and validity checking utility
## .SECTION Description
## HeaderTesting is a script which checks the list of header files for
## validity based on SVTK coding standard. It checks for proper super
...
## .SECTION See Also
## http://www.svtk.org https://www.cdash.org/
## http://www.svtk.org/contribute.php#coding-standards
...
class TestSVTKFiles:
    ...
    def CheckExclude(self):
        prefix = '// SVTK-HeaderTest-Exclude:'
        prefix_c = '/* SVTK-HeaderTest-Exclude:'
        suffix_c = ' */'
        exclude = 0
        for l in self.FileLines:
            if l.startswith(prefix):
                ...
                if e == os.path.basename(self.FileName):
                    ...
                    self.Error("Wrong exclusion: "+l.rstrip())
        if exclude > 1:
            self.Error("Multiple SVTK-HeaderTest-Exclude lines")
        return exclude > 0
    ...
    def CheckParent(self):
        classre = "^class(\s+SVTK_DEPRECATED)?(\s+[^\s]*_EXPORT)?\s+(svtk[A-Z0-9_][^ :\n]*)\s*:\s*public\s+(svtk[^ \n\{]*)"
        cname = ""
        pname = ""
        classlines = []
    ...
    def CheckTypeMacro(self):
        ...
        lines = []
        oldlines = []
        typere = "^\s*svtk(Abstract|Base)?Type(Revision)*Macro\s*\(\s*(svtk[^ ,]+)\s*,\s*(svtk[^ \)]+)\s*\)\s*"
        typesplitre = "^\s*svtk(Abstract|Base)?Type(Revision)*Macro\s*\("
    ...
    def CheckPrintSelf(self):
        if not self.ClassName:
            return
        typere = "^\s*void\s*PrintSelf\s*\(\s*ostream\s*&\s*os*\s*,\s*svtkIndent\s*indent\s*\)"
        newtypere = "^\s*virtual\s*void\s*PrintSelf\s*\(\s*ostream\s*&\s*os*\s*,\s*svtkIndent\s*indent\s*\)"
...
if len(sys.argv) > 2:
  export = sys.argv[2]
  if export[:3] == "SVTK" and export[len(export)-len("EXPORT"):] == "EXPORT":
