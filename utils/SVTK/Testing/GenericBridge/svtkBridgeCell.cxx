...
svtkStandardNewMacro(svtkBridgeCell);

//-----------------------------------------------------------------------------
void svtkBridgeCell::PrintSelf(ostream& os, svtkIndent indent)
{
...
svtkIdType svtkBridgeCell::GetId()
{
  return this->Id;
}
...
int svtkBridgeCell::IsInDataSet()
{
...
// \post (result==SVTK_HIGHER_ORDER_EDGE)||
//       (result==SVTK_HIGHER_ORDER_TRIANGLE)||
//       (result==SVTK_HIGHER_ORDER_TETRAHEDRON)
int svtkBridgeCell::GetType()
{
  int result = 0;
  switch (this->Cell->GetCellType())
  {
    ...
    case SVTK_QUADRATIC_HEXAHEDRON:
      result = SVTK_HIGHER_ORDER_HEXAHEDRON;
      break;
    case SVTK_WEDGE:
    case SVTK_QUADRATIC_WEDGE:
      result = SVTK_HIGHER_ORDER_WEDGE;
      break;
    case SVTK_PYRAMID:
    case SVTK_QUADRATIC_PYRAMID:
      result = SVTK_HIGHER_ORDER_PYRAMID;
      break;
...
int svtkBridgeCell::GetDimension()
{
...
int svtkBridgeCell::GetGeometryOrder()
{
  int result;
  ...
  {
    result = 2; // GetOrder() is missing in svtkCell...
  }
...
int svtkBridgeCell::IsGeometryLinear()
{
...
// \pre a_exists: a!=0
// \post positive_result: result>=0
int svtkBridgeCell::GetAttributeOrder(svtkGenericAttribute* svtkNotUsed(a))
{
...
svtkTypeBool svtkBridgeCell::IsAttributeLinear(svtkGenericAttribute* a)
{
...
// \post positive_result: result>=0
int svtkBridgeCell::GetNumberOfPoints()
{
...
// \post positive_result: result>=0
int svtkBridgeCell::GetNumberOfBoundaries(int dim)
{
...
int svtkBridgeCell::GetNumberOfDOFNodes()
{
...
// \pre it_exists: it!=0
void svtkBridgeCell::GetPointIterator(svtkGenericPointIterator* it)
{
  assert("pre: it_exists" && it != nullptr);
  static_cast<svtkBridgePointIterator*>(it)->InitWithCell(this);
}
...
svtkGenericCellIterator* svtkBridgeCell::NewCellIterator()
{
  svtkGenericCellIterator* result = svtkBridgeCellIterator::New();
...
void svtkBridgeCell::GetBoundaryIterator(svtkGenericCellIterator* boundaries, int dim)
{
  assert("pre: valid_dim_range" && ((dim == -1) || ((dim >= 0) && (dim < GetDimension()))));
  assert("pre: boundaries_exist" && boundaries != nullptr);
  static_cast<svtkBridgeCellIterator*>(boundaries)->InitWithCellBoundaries(this, dim);
}
...
// \post positive_result: result>=0
int svtkBridgeCell::CountNeighbors(svtkGenericAdaptorCell* boundary)
{
  ...
  svtkBridgeCell* b = static_cast<svtkBridgeCell*>(boundary);
  svtkIdList* pts = b->Cell->GetPointIds();
...
void svtkBridgeCell::CountEdgeNeighbors(int* sharing)
{
  assert("pre: large_enough" && this->GetDimension() >= 2);

  svtkIdType c = this->Cell->GetNumberOfEdges();
  svtkIdList* cells = svtkIdList::New();
  svtkIdType i = 0;
  svtkCell* edge;
  svtkIdList* pts;
...
void svtkBridgeCell::GetNeighbors(svtkGenericAdaptorCell* boundary, svtkGenericCellIterator* neighbors)
{
  assert("pre: boundary_exists" && boundary != nullptr);
  assert("pre: real_boundary" && !boundary->IsInDataSet());
  assert("pre: cell_of_the_dataset" && IsInDataSet());
  ...
  svtkIdList* cells = svtkIdList::New();
  svtkIdList* pts = static_cast<svtkBridgeCell*>(boundary)->Cell->GetPointIds();
  this->DataSet->Implementation->GetCellNeighbors(this->Id, pts, cells);

  static_cast<svtkBridgeCellIterator*>(neighbors)->InitWithCells(cells, this->DataSet);
...
int svtkBridgeCell::FindClosestBoundary(
  int subId, double pcoords[3], svtkGenericCellIterator*& boundary)
{
  assert("pre: positive_subId" && subId >= 0);

  svtkIdList* pts = svtkIdList::New();
  int result = this->Cell->CellBoundary(subId, pcoords, pts);
  static_cast<svtkBridgeCellIterator*>(boundary)->InitWithPoints(
    this->Cell->Points, pts, this->GetDimension() - 1, 0); // id of the boundary always 0?
  pts->Delete();
...
//                                               dist2>=0)
int svtkBridgeCell::EvaluatePosition(
  const double x[3], double* closestPoint, int& subId, double pcoords[3], double& dist2)
{
  this->AllocateWeights();
  int result = this->Cell->EvaluatePosition(x, closestPoint, subId, pcoords, dist2, this->Weights);
  ...
  assert("post: positive_distance" &&
    (!(result != -1) || (!(closestPoint != nullptr) || dist2 >= 0))); // A=>B: !A || B
  return result;
}
...
void svtkBridgeCell::EvaluateLocation(int subId, double pcoords[3], double x[3])
{
  assert("pre: positive_subId" && subId >= 0);
  assert("pre: clamped_pcoords" && (0 <= pcoords[0]) && (pcoords[0] <= 1) && (0 <= pcoords[1]) &&
    (pcoords[1] <= 1) && (0 <= pcoords[2]) && (pcoords[2] <= 1));
...
// `val'.
// \pre a_exists: a!=0
// \pre a_is_point_centered: a->GetCentering()==svtkPointCentered
...
void svtkBridgeCell::InterpolateTuple(svtkGenericAttribute* a, double pcoords[3], double* val)
{
  assert("pre: a_exists" && a != nullptr);
  assert("pre: a_is_point_centered" && a->GetCentering() == svtkPointCentered);
  assert("pre: clamped_point" &&
    (pcoords[0] >= 0 && pcoords[0] <= 1 && pcoords[1] >= 0 && pcoords[1] <= 1 && pcoords[2] >= 0 &&
      pcoords[2] <= 1));
  assert("pre: val_exists" && val != nullptr);

  svtkBridgeAttribute* ba = static_cast<svtkBridgeAttribute*>(a);
...
void svtkBridgeCell::InterpolateTuple(
  svtkGenericAttributeCollection* c, double pcoords[3], double* val)
{
  ...
  while (i < count)
  {
    if (c->GetAttribute(i)->GetCentering() == svtkPointCentered)
    {
...
int svtkBridgeCell::IntersectWithLine(
...
void svtkBridgeCell::Derivatives(
  int subId, double pcoords[3], svtkGenericAttribute* attribute, double* derivs)
{
  ...
  this->Cell->Derivatives(subId, pcoords, tuples, attribute->GetNumberOfComponents(), derivs);
  delete[] tuples;
}
...
// coordinates.
// THREAD SAFE
void svtkBridgeCell::GetBounds(double bounds[6])
{
...
double* svtkBridgeCell::GetBounds()
{
...
// \post positive_result: result>=0
double svtkBridgeCell::GetLength2()
{
...
int svtkBridgeCell::GetParametricCenter(double pcoords[3])
{
...
// cell.)
// \post positive_result: result>=0
double svtkBridgeCell::GetParametricDistance(const double pcoords[3])
{
...
double* svtkBridgeCell::GetParametricCoords()
{
...
//----------------------------------------------------------------------------
int svtkBridgeCell::IsFaceOnBoundary(svtkIdType faceId)
{
  assert("pre: 3d" && this->GetDimension() == 3);

  // result=CountNeighbors(boundary(faceId))==0;

  svtkCell* face = this->Cell->GetFace(faceId);
  svtkIdList* cells = svtkIdList::New(); // expensive
...
#if 0
  if(this->GetType()==SVTK_QUADRATIC_TETRA)
  {
    if(result)
    {
      cout<<"************************************************ boundary"<<endl;
...
//----------------------------------------------------------------------------
int svtkBridgeCell::IsOnBoundary()
{
...
void svtkBridgeCell::GetPointIds(svtkIdType* id)
{
  svtkIdType i = 0;
  svtkIdList* l = this->Cell->GetPointIds();
  svtkIdType c = this->GetNumberOfBoundaries(0);
  while (i < c)
  {
    id[i] = l->GetId(i);
    ++i;
...
const svtkIdType* svtkBridgeCell::GetFaceArray(svtkIdType faceId)
{
  assert("pre: is_3d" && this->GetDimension() == 3);
  assert("pre: valid_faceId_range" && faceId >= 0 && faceId < this->GetNumberOfBoundaries(2));

  const svtkIdType* result = nullptr;

  switch (this->GetType())
  {
    case SVTK_HIGHER_ORDER_TETRAHEDRON:
      result = svtkTetra::GetFaceArray(faceId);
      break;
    case SVTK_HIGHER_ORDER_HEXAHEDRON:
      if (this->Cell->GetCellType() == SVTK_VOXEL)
      {
        result = svtkVoxel::GetFaceArray(faceId);
      }
      else
      {
        result = svtkHexahedron::GetFaceArray(faceId);
      }
      break;
    case SVTK_HIGHER_ORDER_WEDGE:
      result = svtkWedge::GetFaceArray(faceId);
      break;
    case SVTK_HIGHER_ORDER_PYRAMID:
      result = svtkPyramid::GetFaceArray(faceId);
      break;
    case SVTK_PENTAGONAL_PRISM:
      assert("check: TODO" && 0);
      break;
...
// \pre valid_faceId_range: faceId>=0 && faceId<this->GetNumberOfBoundaries(2)
// \post positive_result: && result>0
int svtkBridgeCell::GetNumberOfVerticesOnFace(int faceId)
{
  assert("pre: is_3d" && this->GetDimension() == 3);
  assert("pre: valid_faceId_range" && faceId >= 0 && faceId < this->GetNumberOfBoundaries(2));
...
// copy/paste of svtkTriangle.cxx
static constexpr svtkIdType triangleEdges[3][2] = { { 0, 1 }, { 1, 2 }, { 2, 0 } };
static constexpr svtkIdType quadEdges[4][2] = { { 0, 1 }, { 1, 2 }, { 3, 2 }, { 0, 3 } };
...
const svtkIdType* svtkBridgeCell::GetEdgeArray(svtkIdType edgeId)
{
  ...
  switch (this->GetType())
  {
    ...
    case SVTK_HIGHER_ORDER_QUAD:
      ...
      break;
    case SVTK_HIGHER_ORDER_TETRAHEDRON:
      result = svtkTetra::GetEdgeArray(edgeId);
      break;
    case SVTK_HIGHER_ORDER_HEXAHEDRON:
      if (this->Cell->GetCellType() == SVTK_VOXEL)
      {
        result = svtkVoxel::GetEdgeArray(edgeId);
      }
      else
      {
        result = svtkHexahedron::GetEdgeArray(edgeId);
      }
      break;
    case SVTK_HIGHER_ORDER_WEDGE:
      result = svtkWedge::GetEdgeArray(edgeId);
      break;
    case SVTK_HIGHER_ORDER_PYRAMID:
      result = svtkPyramid::GetEdgeArray(edgeId);
      break;
    case SVTK_PENTAGONAL_PRISM:
      assert("check: TODO" && 0);
      break;
...
void svtkBridgeCell::Init(svtkBridgeDataSet* ds, svtkIdType cellid)
{
  assert("pre: ds_exists" && ds != nullptr);
  assert("pre: valid_cellid" && (cellid >= 0) && (cellid < ds->GetNumberOfCells()));

  svtkSetObjectBodyMacro(DataSet, svtkBridgeDataSet, ds);
  svtkCell* tmp = ds->Implementation->GetCell(cellid);
  svtkSetObjectBodyMacro(Cell, svtkCell, tmp);
  this->Id = cellid;
  this->BoolIsInDataSet = 1;
  if (this->InternalIterator == nullptr)
  {
    this->InternalIterator = svtkBridgeCellIterator::New();
  }
...
// \pre c_exists: c!=0
void svtkBridgeCell::InitWithCell(svtkCell* c, svtkIdType id)
{
  assert("pre: c_exists" && c != nullptr);
  ...
  // warning: do directly svtkSetObjectBodyMacro(Cell,svtkCell,c->NewInstance())
...
void svtkBridgeCell::DeepCopy(svtkBridgeCell* other)
{
  ...
  {
    svtkSetObjectBodyMacro(DataSet, svtkBridgeDataSet, other->DataSet);
...
{
  svtkSetObjectBodyMacro(DataSet, svtkBridgeDataSet, 0);
  svtkSetObjectBodyMacro(InternalIterator, svtkBridgeCellIterator, 0);
  svtkSetObjectBodyMacro(Cell, svtkCell, 0);
...
void svtkBridgeCell::AllocateWeights()
{
  if (this->Weights != nullptr && this->WeightsCapacity < this->GetNumberOfPoints())
  {
    delete[] this->Weights;
    this->Weights = nullptr;
  }
...
void svtkBridgeCell::InterpolationFunctions(const double pcoords[3], double* weights)
{
