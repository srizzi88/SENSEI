 ...
 * @brief   Implementation of svtkGenericAdaptorCell
 *
...
#ifndef svtkBridgeCell_h
#define svtkBridgeCell_h
...
class SVTKTESTINGGENERICBRIDGE_EXPORT svtkBridgeCell : public svtkGenericAdaptorCell
{
public:
  static svtkBridgeCell* New();
  svtkTypeMacro(svtkBridgeCell, svtkGenericAdaptorCell);
   ...
   * \post positive_result: result>=0
   */
  int GetAttributeOrder(svtkGenericAttribute* a) override;
   ...
   */
  svtkTypeBool IsAttributeLinear(svtkGenericAttribute* a);
   ...
   * \pre it_exists: it!=0
   */
  void GetPointIterator(svtkGenericPointIterator* it) override;

  /**
   * Create an empty cell iterator.
   * \post result_exists: result!=0
   */
  svtkGenericCellIterator* NewCellIterator() override;
   ...
   */
  void GetBoundaryIterator(svtkGenericCellIterator* boundaries, int dim = -1) override;
   ...
   * \post positive_result: result>=0
   */
  int CountNeighbors(svtkGenericAdaptorCell* boundary) override;
   ...
   */
  void GetNeighbors(svtkGenericAdaptorCell* boundary, svtkGenericCellIterator* neighbors) override;
   ...
   */
  int FindClosestBoundary(int subId, double pcoords[3], svtkGenericCellIterator*& boundary) override;
   ...
   */
  void InterpolateTuple(svtkGenericAttribute* a, double pcoords[3], double* val) override;
   ...
   */
  void InterpolateTuple(svtkGenericAttributeCollection* c, double pcoords[3], double* val) override;
#if 0
  /**
   ...
   * NOTE: `svtkGenericAttributeCollection *attributes' will be replaced by a
   * `svtkInformation'.
   ...
   */
  virtual void Contour(svtkContourValues *values,
                       svtkImplicitFunction *f,
                       svtkGenericAttributeCollection *attributes,
  ...
  virtual void Clip(double value,
                    svtkImplicitFunction *f,
                    svtkGenericAttributeCollection *attributes,
                    svtkGenericCellTessellator *tess,
                    int insideOut,
                    svtkPointLocator *locator,
   ...
   */
  void Derivatives(
    int subId, double pcoords[3], svtkGenericAttribute* attribute, double* derivs) override;
   ...
   * \pre cd_exists: cd!=0
   */
  virtual void Tessellate(svtkGenericAttributeCollection *attributes,
                          svtkPoints *points, svtkCellArray* cellArray,
                          svtkPointData *pd, svtkCellData* cd);
#endif
  // For the internals of the tessellation algorithm (the hash table in particular)
  int IsFaceOnBoundary(svtkIdType faceId) override;
   ...
   */
  void GetPointIds(svtkIdType* id) override;
#if 0
  virtual void TriangulateFace(svtkGenericAttributeCollection *attributes,
                               svtkGenericCellTessellator *tess,
                               int index,
                               svtkPoints *pts, svtkCellArray *cellArray,
                               svtkPointData *pd,
                               svtkCellData *cd );
#endif
  //@}
   ...
   */
  const svtkIdType* GetFaceArray(svtkIdType faceId) override;
   ...
   */
  const svtkIdType* GetEdgeArray(svtkIdType edgeId) override;
   ...
   */
  void Init(svtkBridgeDataSet* ds, svtkIdType cellid);
   ...
   * \pre c_exists: c!=0
   */
  void InitWithCell(svtkCell* c, svtkIdType id);
   ...
   */
  void DeepCopy(svtkBridgeCell* other);

protected:
  svtkBridgeCell();
  ~svtkBridgeCell() override;
  ...
  friend class svtkBridgeDataSet;
  friend class svtkBridgeAttribute;
  friend class svtkBridgeCellIterator;
  friend class svtkBridgeCellIteratorOnDataSet;
  friend class svtkBridgeCellIteratorOne;
  friend class svtkBridgeCellIteratorOnCellBoundaries;
  friend class svtkBridgePointIteratorOnCell;

  svtkCell* Cell;
  svtkBridgeDataSet* DataSet;
  svtkIdType Id; // what does it mean for boundary cells?
  int BoolIsInDataSet;
  svtkBridgeCellIterator* InternalIterator; // used in Contour

  double* Weights; // interpolation functions
  int WeightsCapacity;

private:
  svtkBridgeCell(const svtkBridgeCell&) = delete;
  void operator=(const svtkBridgeCell&) = delete;
};
