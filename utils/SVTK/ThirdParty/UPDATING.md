...
itself (e.g., the `zlib/svtkzlib` directory for zlib), should go through the
...
a `for/foo` branch, like `for/svtk` for example, but may be `master` or any
...
```sh
$ cd twisted/
$ git checkout for/svtk
$ git fetch origin
$ git rebase --onto twisted-17.5.0 twisted-17.1.0
$ git push
```

Now import into SVTK

```sh
$ cd svtk/ThirdParty/twisted
...
created where patches for the `foo` project will be applied (i.e., `for/svtk`
for SVTK's patches to the project). Usually, changes to the build system, the
...
$ git checkout -b for/svtk
$ git push --set-upstream kitware for/svtk
```
...
circumstances. Please post a message on the [SVTK Discourse][] forum asking
