...
#
function(svtkm_extract_real_library library real_library)
  if(NOT UNIX)
...
if(SVTKm_ENABLE_TBB AND NOT TARGET svtkm::tbb)
  ...
  if (CMAKE_VERSION VERSION_LESS 3.10)
    add_library(svtkm::tbb SHARED IMPORTED GLOBAL)
  else()
    add_library(svtkm::tbb UNKNOWN IMPORTED GLOBAL)
  endif()

  set_target_properties(svtkm::tbb PROPERTIES
      INTERFACE_INCLUDE_DIRECTORIES "${TBB_INCLUDE_DIRS}")

  if(EXISTS "${TBB_LIBRARY_RELEASE}")
    svtkm_extract_real_library("${TBB_LIBRARY_RELEASE}" real_path)
    set_property(TARGET svtkm::tbb APPEND PROPERTY IMPORTED_CONFIGURATIONS RELEASE)
    set_target_properties(svtkm::tbb PROPERTIES
      IMPORTED_LINK_INTERFACE_LANGUAGES "CXX"
      IMPORTED_LOCATION_RELEASE "${real_path}"
      )
  elseif(EXISTS "${TBB_LIBRARY}")
    #When SVTK-m is mixed with OSPray we could use the OSPray FindTBB file
    #which doesn't define TBB_LIBRARY_RELEASE but instead defined only
    #TBB_LIBRARY
    svtkm_extract_real_library("${TBB_LIBRARY}" real_path)
    set_property(TARGET svtkm::tbb APPEND PROPERTY IMPORTED_CONFIGURATIONS RELEASE)
    set_target_properties(svtkm::tbb PROPERTIES
      IMPORTED_LINK_INTERFACE_LANGUAGES "CXX"
      IMPORTED_LOCATION_RELEASE "${real_path}"
  ...
  if(EXISTS "${TBB_LIBRARY_DEBUG}")
    svtkm_extract_real_library("${TBB_LIBRARY_DEBUG}" real_path)
    set_property(TARGET svtkm::tbb APPEND PROPERTY IMPORTED_CONFIGURATIONS DEBUG)
    set_target_properties(svtkm::tbb PROPERTIES
      IMPORTED_LINK_INTERFACE_LANGUAGES "CXX"
...
if(SVTKm_ENABLE_OPENMP AND NOT TARGET svtkm::openmp)
  cmake_minimum_required(VERSION 3.12...3.15 FATAL_ERROR)
  find_package(OpenMP 4.0 REQUIRED COMPONENTS CXX QUIET)

  add_library(svtkm::openmp INTERFACE IMPORTED GLOBAL)
  if(OpenMP_CXX_FLAGS)
    set_property(TARGET svtkm::openmp
  ...
  if(OpenMP_CXX_LIBRARIES)
    set_target_properties(svtkm::openmp PROPERTIES
...
if(SVTKm_ENABLE_CUDA)
  ...
  if(CMAKE_CUDA_COMPILER_ID STREQUAL "NVIDIA" AND
    ...
    message(FATAL_ERROR "SVTK-m CUDA support requires version 9.2+")
  endif()

 if (NOT TARGET svtkm::cuda)
    add_library(svtkm_cuda INTERFACE)
    add_library(svtkm::cuda ALIAS svtkm_cuda)
    set_target_properties(svtkm_cuda PROPERTIES EXPORT_NAME svtkm::cuda)

    install(TARGETS svtkm_cuda EXPORT ${SVTKm_EXPORT_NAME})
    # Reserve `requires_static_builds` to potential work around issues
    # where SVTK-m doesn't work when building shared as virtual functions fail
    # inside device code. We don't want to force BUILD_SHARED_LIBS to a specific
    # value as that could impact other projects that embed SVTK-m. Instead what
    # we do is make sure that libraries built by svtkm_library() are static
    # if they use CUDA
    #
    ...
    set_target_properties(svtkm_cuda PROPERTIES
    ...
    set(SVTKm_CUDA_Architecture "native" CACHE STRING "Which GPU Architecture(s) to compile for")
    set_property(CACHE SVTKm_CUDA_Architecture PROPERTY STRINGS native fermi kepler maxwell pascal volta turing all none)

    #detect what the property is set too
    if(SVTKm_CUDA_Architecture STREQUAL "native")

      if(SVTKM_CUDA_NATIVE_EXE_PROCESS_RAN_OUTPUT)
        #Use the cached value
        set(arch_flags ${SVTKM_CUDA_NATIVE_EXE_PROCESS_RAN_OUTPUT})
      else()

        #run execute_process to do auto_detection
        if(CMAKE_GENERATOR MATCHES "Visual Studio")
          set(args "-ccbin" "${CMAKE_CXX_COMPILER}" "--run" "${SVTKm_CMAKE_MODULE_PATH}/SVTKmDetectCUDAVersion.cu")
        elseif(CUDA_HOST_COMPILER)
          set(args "-ccbin" "${CUDA_HOST_COMPILER}" "--run" "${SVTKm_CMAKE_MODULE_PATH}/SVTKmDetectCUDAVersion.cu")
        else()
          set(args "--run" "${SVTKm_CMAKE_MODULE_PATH}/SVTKmDetectCUDAVersion.cu")
        endif()
        ...
        if(ran_properly EQUAL 0)
          ...
          set(SVTKM_CUDA_NATIVE_EXE_PROCESS_RAN_OUTPUT ${run_output} CACHE INTERNAL
                  "device type(s) for cuda[native]")
        else()
          message(FATAL_ERROR "Error detecting architecture flags for CUDA. Please set SVTKm_CUDA_Architecture manually.")
        endif()
      endif()
    endif()

    if(SVTKm_CUDA_Architecture STREQUAL "fermi")
      set(arch_flags --generate-code=arch=compute_20,code=sm_20)
    elseif(SVTKm_CUDA_Architecture STREQUAL "kepler")
      set(arch_flags --generate-code=arch=compute_30,code=sm_30
                     --generate-code=arch=compute_35,code=sm_35)
    elseif(SVTKm_CUDA_Architecture STREQUAL "maxwell")
      set(arch_flags --generate-code=arch=compute_50,code=sm_50)
    elseif(SVTKm_CUDA_Architecture STREQUAL "pascal")
      set(arch_flags --generate-code=arch=compute_60,code=sm_60)
    elseif(SVTKm_CUDA_Architecture STREQUAL "volta")
      set(arch_flags --generate-code=arch=compute_70,code=sm_70)
    elseif(SVTKm_CUDA_Architecture STREQUAL "turing")
      set(arch_flags --generate-code=arch=compute_75,code=sm_75)
    elseif(SVTKm_CUDA_Architecture STREQUAL "all")
      set(arch_flags --generate-code=arch=compute_30,code=sm_30
                     --generate-code=arch=compute_35,code=sm_35
    ...
    # target property to have this automatically exported for us
    set_target_properties(svtkm_cuda PROPERTIES cuda_architecture_flags "${arch_flags}")
    set(SVTKm_CUDA_Architecture_Flags "${arch_flags}")
  endif()
endif()
