# Contributing to SVTK-m #

This page documents how to develop SVTK-m through [Git](http://git-scm.com).
...
document procedures used by the SVTK-m development community. In the
...
2.  [Fork SVTK-m] into your user's namespace on GitLab.

3.  Use Git to create a local clone of the main SVTK repository:

        $ git clone https://gitlab.kitware.com/svtk/svtk-m.git
        $ cd svtk-m

    The main repository will be configured as your `origin` remote.

4.  Run the developer setup script to prepare your SVTK-m work tree and
...
5. (Optional but highly recommended.) 
    [Register with the SVTK-m dashboard] on Kitware's CDash instance to
...
6.  (Optional but highly recommended.) 
    [Sign up for the SVTK-m mailing list] to communicate with other
    developers and users.

[GitLab Access]: https://gitlab.kitware.com/users/sign_in
[Fork SVTK-m]: https://gitlab.kitware.com/svtk/svtk-m/forks/new
[Register with the SVTK-m dashboard]: https://open.cdash.org/register.php
[Sign up for the SVTK-m mailing list]: http://svtk.org/mailman/listinfo/svtkm


## Workflow ##

SVTK-m development uses a [branchy workflow] based on topic branches. Our
...
3.  Edit files and create commits (repeat as needed):
    ...
    Caveats:
      * Data files must be placed under a folder explicitly named 'data'.
        This is required as SVTK-m uses Git-LFS to efficiently support data
        files.
...
maintainable. SVTK-m requires that your message start with a single subject
...
main [SVTK-m GitLab] repository page and using the "Fork" button in the
upper right.

[SVTK-m GitLab]: https://gitlab.kitware.com/svtk/svtk-m
...
with a url of the form
    ...
    https://gitlab.kitware.com/<username>/svtk-m/merge_requests/new
...
1.  In the "**Source branch**" box select the `<username>/svtk-m` repository
    and the `my-topic` branch.

2.  In the "**Target branch**" box select the `svtk/svtk-m` repository and
...
the form:
    ...
    $ git fetch https://gitlab.kitware.com/$username/svtk-m.git $branch
...
SVTK-m has a [buildbot](http://buildbot.net) instance watching for merge
requests to test. Each time a merge request is updated the buildbot user
(@buildbot) will automatically trigger a new build on all SVTK-m buildbot
...
version of SVTK-m. Even if you a revising your topic branch for some other
...
familiar with contributing to SVTK-m, you may get your repository in a bad
...
The SVTK-m contribution workflow assumes that your `origin` remote is
attached to the main SVTK-m GitLab repository. If it is not, that will cause
...
If you have a line like
    ...
    origin  https://gitlab.kitware.com/svtk/svtk-m.git (fetch)

or

    origin  git@gitlab.kitware.com:svtk/svtk-m.git (fetch)
...
it by simply changing the remote's URL:
    ...
    $ git remote set-url origin https://gitlab.kitware.com/svtk/svtk-m.git
...
should be the main SVTK-m repository). This should be set up if you
correctly cloned the main SVTK-m repository, but can get accidentally
changed.
