...
//This demo shows off using svtk-m in multiple threads in two different ways.
//
//At a high level we have 2 primary threads, an IO thread and a Worker thread
//The IO thread will generate all data using the svtk-m serial device, and
//will post this data to a worker queue as a svtk-m partitioned dataset.
//The Worker thread will pull down these svtk-m data and run a
//svtk-m filter on the partitions.
//The svtk-m filter it runs will itself have a worker pool which it will
...
//
//Unfortunately due to some thread unsafe logic in SVTK-m it is currently not
...
void partition_processing(TaskQueue<svtkm::cont::PartitionedDataSet>& queue);
int main(int argc, char** argv)
{
  ...
  TaskQueue<svtkm::cont::PartitionedDataSet> queue;
...
void partition_processing(TaskQueue<svtkm::cont::PartitionedDataSet>& queue)
{
  ...
  while (queue.hasTasks())
  {
    ...
    svtkm::cont::PartitionedDataSet pds = queue.pop();
    ...
    svtkm::cont::PartitionedDataSet result = gradient.Execute(pds);
    ...
    for (auto&& partition : result)
    {
      ...
      if (!partition.HasField("Gradients", svtkm::cont::Field::Association::POINTS))
      {
