...
void process_partition_tbb(RuntimeTaskQueue& queue)
{
  ...
  //This makes sure that any svtkm::filters used by our
...
void process_partition_openMP(RuntimeTaskQueue& queue)
{
  ...
  //is actually thread-local, so we can use that.
  //
  svtkm::cont::GetRuntimeDeviceTracker().ForceDevice(svtkm::cont::DeviceAdapterTagOpenMP{});
...
void process_partition_cuda(RuntimeTaskQueue& queue, int gpuId)
{
  ...
  //is actually thread-local, so we can use that.
  //
  svtkm::cont::GetRuntimeDeviceTracker().ForceDevice(svtkm::cont::DeviceAdapterTagCuda{});
  (void)gpuId;
...
//-----------------------------------------------------------------------------
SVTKM_CONT MultiDeviceGradient::MultiDeviceGradient()
  ...
  , Workers()
{
  ...
  const bool runOnCuda = tracker.CanRunOn(svtkm::cont::DeviceAdapterTagCuda{});
  const bool runOnOpenMP = tracker.CanRunOn(svtkm::cont::DeviceAdapterTagOpenMP{});
  const bool runOnTbb = tracker.CanRunOn(svtkm::cont::DeviceAdapterTagTBB{});
...
inline SVTKM_CONT svtkm::cont::PartitionedDataSet MultiDeviceGradient::PrepareForExecution(
  ...
  const svtkm::filter::PolicyBase<DerivedPolicy>& policy)
{
  ...
  output.AppendPartitions(
    std::vector<svtkm::cont::DataSet>(static_cast<size_t>(pds.GetNumberOfPartitions())));
  svtkm::cont::PartitionedDataSet* outPtr = &output;
  ...
  {
    svtkm::cont::DataSet input = *partition;
    this->Queue.push( //build a lambda that is the work to do
      [=]() {
        svtkm::filter::Gradient perThreadGrad = gradient;

        svtkm::cont::DataSet result = perThreadGrad.Execute(input, policy);
