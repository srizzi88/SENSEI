...
void read_oscillators(std::string filePath, svtkm::source::Oscillator& source)
{
...
void writeData(std::string& basePath, int timestep, int iSize, int jSize, int kSize, double* values)
{
  ...
  else
  {
    metaFilePathPointer << "{\n"
                        ...
                        << "  \"type\": [\"tonic-query-data-model\", \"svtk-volume\"],\n"
...
void printUsage(const std::string& svtkm_options)
{
  std::cout << "Usage: Oscillator [options]\n\n"
            ...
            << "General SVTK-m Options:\n\n"
            << svtkm_options << std::endl;
}
...
int main(int argc, char** argv)
{
  ...
  auto initializeResults = svtkm::cont::Initialize(argc, argv, opts);
  // Process args
  ...
  svtkm::source::Oscillator source(svtkm::Id3{ sizeX, sizeY, sizeZ });
  ...
  while (currentTime < endTime)
  {
    ...
    svtkm::cont::DataSet rdata = source.Execute();
    if (generateOutput)
    {
      svtkm::cont::ArrayHandle<svtkm::Float64> tmp;
      rdata.GetField("scalars", svtkm::cont::Field::Association::POINTS).GetData().CopyTo(tmp);
