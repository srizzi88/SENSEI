...
#ifndef svtk_m_Geometry_h
#define svtk_m_Geometry_h
...
template <typename CoordType = svtkm::FloatDefault, int Dim = 3, bool IsTwoSided = false>
struct Ray
{
  static constexpr int Dimension = Dim;
  using Vector = svtkm::Vec<CoordType, Dim>;
  ...
  SVTKM_EXEC_CONT bool Intersect(const Ray<CoordType, Dim, OtherTwoSided>& other,
                                Vector& point,
                                CoordType tol = 0.f);
};

/// Represent a finite line segment with a pair of points
template <typename CoordType = svtkm::FloatDefault, int Dim = 3>
struct LineSegment
{
  static constexpr int Dimension = Dim;
  using Vector = svtkm::Vec<CoordType, Dim>;
  ...
  template <int Dim_ = Dim, typename std::enable_if<Dim_ == 3, int>::type = 0>
  SVTKM_EXEC_CONT Plane<CoordType> PerpendicularBisector() const;

  /// Construct a perpendicular bisector to this line segment (only when Dimension is 2).
  template <int Dim_ = Dim, typename std::enable_if<Dim_ == 2, int>::type = 0>
  SVTKM_EXEC_CONT Ray<CoordType, Dim, true> PerpendicularBisector() const;
  ...
  /// Call svtkm::Normal() on the return value if you want a normalized result.
  SVTKM_EXEC_CONT
  ...
  /// reported as valid.
  template <int Dim_ = Dim, typename std::enable_if<Dim_ == 2, int>::type = 0>
  SVTKM_EXEC_CONT bool IntersectInfinite(const LineSegment<CoordType, Dim>& other,
                                        Vector& point,
                                        CoordType tol = 0.f);
};

/// Represent a plane with a base point (origin) and normal vector.
template <typename CoordType = svtkm::FloatDefault>
struct Plane
{
  using Vector = svtkm::Vec<CoordType, 3>;
  Vector Origin;
  Vector Normal;
  ...
  SVTKM_EXEC_CONT
  bool IsValid() const { return !svtkm::IsInf(this->Normal[0]); }
  ...
  SVTKM_EXEC_CONT bool Intersect(const Ray<CoordType, 3, IsTwoSided>& ray,
                                CoordType& parameter,
                                Vector& point,
                                bool& lineInPlane,
...
struct Sphere
{
  ...
  SVTKM_EXEC_CONT Sphere(const Vector& center, CoordType radius);
...
using Ray2d = Ray2<svtkm::FloatDefault>;
using Ray3d = Ray3<svtkm::FloatDefault>;
using Line2d = Line2<svtkm::FloatDefault>;
using Line3d = Line3<svtkm::FloatDefault>;
using LineSegment2d = LineSegment2<svtkm::FloatDefault>;
using LineSegment3d = LineSegment3<svtkm::FloatDefault>;
using Plane3d = Plane<svtkm::FloatDefault>;
using Circle2d = Circle<svtkm::FloatDefault>;
using Sphere2d = Sphere2<svtkm::FloatDefault>;
using Sphere3d = Sphere3<svtkm::FloatDefault>;
...
SVTKM_EXEC_CONT svtkm::Plane<CoordType> make_PlaneFromPointAndLine(
  const svtkm::Vec<CoordType, 3>& point,
  const svtkm::Ray<CoordType, 3, IsTwoSided>& ray,
  CoordType tol2 = static_cast<CoordType>(1e-8f));

template <typename CoordType>
SVTKM_EXEC_CONT svtkm::Plane<CoordType> make_PlaneFromPointAndLineSegment(
  const svtkm::Vec<CoordType, 3>& point,
...
template <typename CoordType>
SVTKM_EXEC_CONT svtkm::Circle<CoordType> make_CircleFrom3Points(
  const typename svtkm::Vec<CoordType, 2>& p0,
...
template <typename CoordType>
SVTKM_EXEC_CONT svtkm::Sphere<CoordType, 3> make_SphereFrom4Points(
  const svtkm::Vec<CoordType, 3>& a0,
...
#include <svtkm/Geometry.hxx>

#endif // svtk_m_Geometry_h
