...
Ray<CoordType, Dim, IsTwoSided>::Ray(const LineSegment<CoordType, Dim>& segment)
  ...
  , Direction(svtkm::Normal(segment.Direction()))
{
}
...
Ray<CoordType, Dim, IsTwoSided>::Ray(const Vector& point, const Vector& direction)
  : Origin(point)
  , Direction(svtkm::Normal(direction))
{
}
...
// compiler error when calling svtkm::IsInf() here. But the fix
...
#else
  return !svtkm::IsInf(this->Direction[0]);
#endif
}
                                                      ...
                                                      Vector& projectedPoint) const
{
  ...
  auto mag2 = svtkm::MagnitudeSquared(dir);
...
{
  return svtkm::MagnitudeSquared(this->Direction()) < tol2;
}
                                                  ...
                                                  Vector& projectedPoint) const
{
  ...
  if (mag2 <= static_cast<CoordType>(0.0f))
  {
    ...
    param = static_cast<CoordType>(0.0f);
    return svtkm::Magnitude(point - this->Endpoints[0]);
  }

  // Find the closest point on the line, then clamp to the line segment
  param = svtkm::Clamp(svtkm::Dot(point - this->Endpoints[0], dir) / mag2,
...
Plane<CoordType>::Plane(const Vector& origin, const Vector& normal, CoordType tol2)
  : Origin(origin)
  , Normal(svtkm::Normal(normal))
{
  if (tol2 > 0.0f && svtkm::MagnitudeSquared(normal) < tol2)
  {
    auto inf = svtkm::Infinity<CoordType>();
    Normal = svtkm::Vec<CoordType, 3>(inf, inf, inf);
  }
}
...
{
  auto dist = svtkm::Dot(point - this->Origin, this->Normal);
  return dist;
}
...
{
  auto vop = svtkm::Project(point - this->Origin, this->Normal);
                                 ...
                                 CoordType tol) const
{
  ...
  CoordType dirDot = svtkm::Dot(this->Normal, ray.Direction);
  // If the ray/line lies parallel to the plane, the intersection is degenerate:
  if (svtkm::Abs(dirDot) < tol)
  {
    lineInPlane = (svtkm::Abs(d0) < tol);
    return false;
  }
  lineInPlane = false;
  parameter = -d0 / dirDot;
                                 ...
                                 bool& lineInPlane) const
{
  ...
  //
  // Note that the svtkm::SignBit comments below cause an internal compiler
  // error on cuda 9.1, ubuntu 17.10 or they would be used:
  bool c0 = d0 < 0; // !!svtkm::SignBit(d0);
  bool c1 = d1 < 0; // !!svtkm::SignBit(d1);
  CoordType a0 = svtkm::Abs(d0);
  CoordType a1 = svtkm::Abs(d1);
  if (c0 == c1)
  {
...
{
  auto dir = svtkm::Cross(this->Normal, other.Normal);
  auto mag2 = svtkm::MagnitudeSquared(dir);
  if (mag2 < tol2)
  ...
  auto nn = svtkm::Normal(dir);
  auto moveDir01 = svtkm::Cross(this->Normal, nn);
  auto moveDir02 = svtkm::Cross(other.Normal, nn);
  Ray<CoordType, 3, true> bra(this->Origin, moveDir01);
  Ray<CoordType, 3, true> brb(other.Origin, moveDir02);
  svtkm::Vec<CoordType, 3> p0a;
  svtkm::Vec<CoordType, 3> p0b;
  CoordType pDummyA, pDummyB;
  bool bDummyA, bDummyB;
  auto tol = svtkm::Sqrt(tol2);
  this->Intersect(brb, pDummyA, p0a, bDummyA, tol);
  other.Intersect(bra, pDummyB, p0b, bDummyB, tol);
  ray = svtkm::Ray<CoordType, 3, true>((p0a + p0b) * 0.5f, nn);
  return true;
}
...
int Sphere<CoordType, Dim>::Classify(const Vector& point, CoordType tol2) const
{
  ...
  auto d2 = svtkm::MagnitudeSquared(point - this->Center);
...
svtkm::Plane<CoordType> make_PlaneFromPointAndLine(const svtkm::Vec<CoordType, 3>& point,
                                                  const svtkm::Ray<CoordType, 3, IsTwoSided>& ray,
                                                  CoordType tol2)
{
  auto tmpDir = point - ray.Origin;
  return svtkm::Plane<CoordType>(point, svtkm::Cross(ray.Direction, tmpDir), tol2);
}

template <typename CoordType>
svtkm::Plane<CoordType> make_PlaneFromPointAndLineSegment(
  const svtkm::Vec<CoordType, 3>& point,
  const svtkm::LineSegment3<CoordType>& segment,
  CoordType tol2)
{
  auto tmpDir = point - segment.Endpoints[0];
  return svtkm::Plane<CoordType>(point, svtkm::Cross(segment.Direction(), tmpDir), tol2);
}

template <typename CoordType>
svtkm::Circle<CoordType> make_CircleFrom3Points(const typename svtkm::Vec<CoordType, 2>& p0,
                                               const typename svtkm::Vec<CoordType, 2>& p1,
                                               const typename svtkm::Vec<CoordType, 2>& p2,
                                               CoordType tol)
{
  constexpr int Dim = 2;
  using Vector = typename svtkm::Circle<CoordType>::Vector;

  svtkm::LineSegment<CoordType, Dim> l01(p0, p1);
  svtkm::LineSegment<CoordType, Dim> l02(p0, p2);
  ...
  if (!pb01.IsValid() || !pb02.IsValid())
  {
    ...
    radius = -1.f;
    return svtkm::Circle<CoordType>(center, radius);
  }
  auto isValid = pb01.Intersect(pb02, center, tol);
  radius = isValid ? svtkm::Magnitude(center - p0) : static_cast<CoordType>(-1.0f);
  if (!isValid)
  { // Initialize center to something.
    center = Vector(svtkm::Nan<CoordType>(), svtkm::Nan<CoordType>());
  }
  return svtkm::Circle<CoordType>(center, radius);
}

template <typename CoordType>
svtkm::Sphere<CoordType, 3> make_SphereFrom4Points(const svtkm::Vec<CoordType, 3>& a0,
                                                  const svtkm::Vec<CoordType, 3>& a1,
                                                  const svtkm::Vec<CoordType, 3>& a2,
                                                  const svtkm::Vec<CoordType, 3>& a3,
                                                  CoordType tol)
{
  ...
  CoordType d0 = svtkm::MagnitudeSquared(a1 - a0);
  CoordType d1 = svtkm::MagnitudeSquared(a2 - a0);
  CoordType d2 = svtkm::MagnitudeSquared(a3 - a0);
  CoordType d3 = svtkm::MagnitudeSquared(a2 - a1);
  CoordType d4 = svtkm::MagnitudeSquared(a3 - a1);
  CoordType d5 = svtkm::MagnitudeSquared(a3 - a2);
  CoordType sel0 = svtkm::Min(d0, svtkm::Min(d1, d2));
  CoordType sel1 = svtkm::Min(d0, svtkm::Min(d3, d4));
  CoordType sel2 = svtkm::Min(d1, svtkm::Min(d3, d5));
  CoordType sel3 = svtkm::Min(d2, svtkm::Min(d4, d5));
  CoordType selm = svtkm::Max(svtkm::Max(sel0, sel1), svtkm::Max(sel2, sel3));

  svtkm::Vec<CoordType, 3> p0 = a0;
  svtkm::Vec<CoordType, 3> p1 = a1;
  svtkm::Vec<CoordType, 3> p2 = a2;
  svtkm::Vec<CoordType, 3> p3 = a3;
  if (sel0 == selm)
  {
    p3 = a0;
    p0 = a3;
  ...
  svtkm::Vec<svtkm::Vec<CoordType, 3>, 3> axes;
  axes[1] = p1 - p0;
  axes[2] = p2 - p0;
  axes[0] = svtkm::Cross(axes[1], axes[2]);
  svtkm::Vec<svtkm::Vec<CoordType, 3>, 3> basis;
  int rank = svtkm::Orthonormalize(axes, basis, tol);
  if (rank < 3)
  {
    svtkm::Sphere<CoordType, 3> invalid;
    invalid.Radius = -1.0f;
    return invalid;
  }

  // Project points to plane and fit a circle.
  auto p0_p = svtkm::Vec<CoordType, 2>{ 0.f }; // This is p0's new coordinate...
  auto p1_p = svtkm::Vec<CoordType, 2>(svtkm::ProjectedDistance(axes[1], basis[1]),
                                      svtkm::ProjectedDistance(axes[1], basis[2]));
  auto p2_p = svtkm::Vec<CoordType, 2>(svtkm::ProjectedDistance(axes[2], basis[1]),
                                      svtkm::ProjectedDistance(axes[2], basis[2]));
  ...
  svtkm::Vec<CoordType, 3> circleCenterWorld =
    p0 + circle.Center[0] * basis[1] + circle.Center[1] * basis[2];

  svtkm::Line3<CoordType> centerRay(circleCenterWorld, basis[0]);
  // If our remaining unused point (p3) lines on centerRay,
  // use one of the other points to locate the sphere's center:
  svtkm::Vec<CoordType, 3> circlePointInPlaneOfP3;
  if (svtkm::Abs(centerRay.DistanceTo(p3)) < tol)
  {
  ...
  else
  {
    ...
    circlePointInPlaneOfP3 =
      circleCenterWorld + svtkm::Normal(pp3.ClosestPoint(p3) - circleCenterWorld) * circle.Radius;
  }

  auto bisectorPlane =
    svtkm::LineSegment3<CoordType>(circlePointInPlaneOfP3, p3).PerpendicularBisector();
  svtkm::Vec<CoordType, 3> sphereCenter;
  CoordType param;
  bool lineInPlane;
  ...
  auto sphereRadius = svtkm::Magnitude(sphereCenter - p3);
  svtkm::Sphere<CoordType, 3> result(sphereCenter, sphereRadius);
  ;
  return result;
}
