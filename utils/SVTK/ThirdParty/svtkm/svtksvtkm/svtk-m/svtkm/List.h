...
//============================================================================
#ifndef svtk_m_List_h
#define svtk_m_List_h
...
struct IsListImpl
{
  ...
  using type = std::is_base_of<svtkm::detail::ListRoot, T>;
};

template <typename... Ts>
struct IsListImpl<svtkm::List<Ts...>>
{
  using type = std::true_type;
};

template <typename T>
using IsList = typename svtkm::internal::IsListImpl<T>::type;
...
///
#define SVTKM_IS_LIST(type)                                                                         \
  SVTKM_STATIC_ASSERT_MSG((::svtkm::internal::IsList<type>::value),                                  \
                         "Provided type is not a valid SVTK-m list type.")
...
template <typename... Ts>
struct AsListImpl<svtkm::List<Ts...>>
{
  using type = svtkm::List<Ts...>;
};
...
using ListUniversal = svtkm::List<detail::UniversalTypeTag>;
...
struct ListApplyImpl<svtkm::List<Ts...>, Target>
{
  using type = Target<Ts...>;
};
// Cannot apply the universal list.
template <template <typename...> class Target>
struct ListApplyImpl<svtkm::ListUniversal, Target>;
...
using ListSize =
  std::integral_constant<svtkm::IdComponent,
                         svtkm::IdComponent{ brigand::size<internal::AsList<List>>::value }>;

/// \brief Finds the type at the given index.
///
/// This becomes the type of the list at the given index.
///
template <typename List, svtkm::IdComponent Index>
using ListAt =
  brigand::at<internal::AsList<List>, std::integral_constant<svtkm::IdComponent, Index>>;
...
template <svtkm::IdComponent NumSearched, typename Target, typename... Remaining>
struct FindFirstOfType;

// Not found
template <svtkm::IdComponent NumSearched, typename Target>
struct FindFirstOfType<NumSearched, Target> : std::integral_constant<svtkm::IdComponent, -1>
{
};

// Basic search next one
template <bool NextIsTarget, svtkm::IdComponent NumSearched, typename Target, typename... Remaining>
struct FindFirstOfCheckHead;

template <svtkm::IdComponent NumSearched, typename Target, typename... Ts>
struct FindFirstOfCheckHead<true, NumSearched, Target, Ts...>
  : std::integral_constant<svtkm::IdComponent, NumSearched>
{
};

template <svtkm::IdComponent NumSearched, typename Target, typename Next, typename... Remaining>
...
struct FindFirstOfCheckHead<false, NumSearched, Target>
  : std::integral_constant<svtkm::IdComponent, -1>
{
};
...
template <bool OneInFirst4Matches, svtkm::IdComponent NumSearched, typename Target, typename... Ts>
struct FindFirstOfSplit4;

template <svtkm::IdComponent NumSearched,
          typename Target,
          typename T0,
          typename T1,
          typename T2,
...
template <bool OneInFirst8Matches, svtkm::IdComponent NumSearched, typename Target, typename... Ts>
...
struct ListIndexOfImpl<svtkm::List<Ts...>, Target>
{
  using type = std::integral_constant<svtkm::IdComponent, FindFirstOfType<0, Target, Ts...>::value>;
};
template <typename Target>
struct ListIndexOfImpl<svtkm::ListUniversal, Target>
{
  SVTKM_STATIC_ASSERT_MSG((std::is_same<Target, void>::value && std::is_same<Target, int>::value),
...
struct ListHasImpl
{
  using type = std::integral_constant<bool, (svtkm::ListIndexOf<List, T>::value >= 0)>;
};

template <typename T>
struct ListHasImpl<svtkm::ListUniversal, T>
{
  using type = std::true_type;
};
...
#if defined(SVTKM_MSVC) && (_MSC_VER < 1911)
...
///
/// Note that this does not work correctly with `svtkm::ListUniversal`.
template <typename... Lists>
...
struct ListIntersectTags
  : ListIntersectTagsChoose<svtkm::ListHas<List, Element>::value, State, Element>
{
};
...
struct ListIntersectImpl<List1, svtkm::ListUniversal>
{
  SVTKM_IS_LIST(List1);
...
struct ListIntersectImpl<svtkm::ListUniversal, List2>
{
  SVTKM_IS_LIST(List2);
...
template <>
struct ListIntersectImpl<svtkm::ListUniversal, svtkm::ListUniversal>
{
  using type = svtkm::ListUniversal;
};
...
template <typename... Ts, template <typename> class Target>
struct ListTransformImpl<svtkm::List<Ts...>, Target>
{
  using type = svtkm::List<Target<Ts>...>;
};
// Cannot transform the universal list.
template <template <typename> class Target>
struct ListTransformImpl<svtkm::ListUniversal, Target>;
...
///
/// ```cpp
/// svtkm::ListRemoveIf<svtkm::List<int, float, long long, double>, std::is_integral>
/// ```
///
/// resolves to a `List` that is equivalent to `svtkm::List<float, double>` because
...
SVTKM_EXEC_CONT inline bool ListForEachCallThrough(Functor&& f, Args&&... args)
{
...
SVTKM_EXEC_CONT void ListForEachImpl(Functor&& f, svtkm::List<Ts...>, Args&&... args)
{
...
template <typename Functor, typename... Args>
SVTKM_EXEC_CONT void ListForEachImpl(Functor&&, svtkm::ListEmpty, Args&&...)
{
...
SVTKM_EXEC_CONT void ListForEach(Functor&& f, List, Args&&... args)
{
...
struct ListCrossImpl
{
  ...
  using type = brigand::reverse_fold<
    svtkm::List<List1, List2>,
    svtkm::List<svtkm::List<>>,
...
///
/// The resulting list has the form of `svtkm::List<svtkm::List<A1,B1>, svtkm::List<A1,B2>,...>`
///
template <typename List1, typename List2>
using ListCross =
