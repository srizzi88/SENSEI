...
//============================================================================
#ifndef svtk_m_NewtonsMethod_h
#define svtk_m_NewtonsMethod_h
...
template <typename ScalarType, svtkm::IdComponent Size>
struct NewtonsMethodResult
{
  bool Valid;
  bool Converged;
  svtkm::Vec<ScalarType, Size> Solution;
};
...
template <typename ScalarType,
          svtkm::IdComponent Size,
          typename JacobianFunctor,
          typename FunctionFunctor>
SVTKM_EXEC_CONT NewtonsMethodResult<ScalarType, Size> NewtonsMethod(
  JacobianFunctor jacobianEvaluator,
  FunctionFunctor functionEvaluator,
  svtkm::Vec<ScalarType, Size> desiredFunctionOutput,
  svtkm::Vec<ScalarType, Size> initialGuess = svtkm::Vec<ScalarType, Size>(ScalarType(0)),
  ScalarType convergeDifference = ScalarType(1e-3),
  svtkm::IdComponent maxIterations = 10)
{
  using VectorType = svtkm::Vec<ScalarType, Size>;
  using MatrixType = svtkm::Matrix<ScalarType, Size, Size>;

  VectorType x = initialGuess;

  bool valid = false;
  bool converged = false;
  for (svtkm::IdComponent iteration = 0; !converged && (iteration < maxIterations); iteration++)
  {
    ...
    VectorType deltaX =
      svtkm::SolveLinearSystem(jacobian, currentFunctionOutput - desiredFunctionOutput, valid);
    if (!valid)
    {
      break;
    }
    ...
    {
      converged &= (svtkm::Abs(deltaX[index]) < convergeDifference);
    }
  }
