...
#define svtk_m_cont_ArrayHandle_hxx
...
template <typename T, typename S>
svtkm::Id ArrayHandle<T, S>::GetNumberOfValues(LockType& lock) const
{
  if (this->Internals->IsControlArrayValid(lock))
  {
...
template <typename T, typename S>
void ArrayHandle<T, S>::Shrink(svtkm::Id numberOfValues)
{
  ...
  if (numberOfValues > 0)
  {
    ...
    svtkm::Id originalNumberOfValues = this->GetNumberOfValues(lock);
    ...
    SVTKM_ASSERT(this->GetNumberOfValues(lock) == numberOfValues);
  }
...
ArrayHandle<T, S>::PrepareForOutput(svtkm::Id numberOfValues, DeviceAdapterTag device)
{
  SVTKM_IS_DEVICE_ADAPTER_TAG(DeviceAdapterTag);
...
inline void SVTKM_CONT StorageSerialization(svtkmdiy::BinaryBuffer& bb,
                                           const ArrayHandle& obj,
                                           std::false_type)
{
  svtkm::Id count = obj.GetNumberOfValues();
  svtkmdiy::save(bb, count);

  svtkmdiy::save(bb, svtkm::Id(0)); //not a basic storage
  auto portal = obj.GetPortalConstControl();
  for (svtkm::Id i = 0; i < count; ++i)
  {
    svtkmdiy::save(bb, portal.Get(i));
  }
}
                                           ...
                                           std::true_type)
{
  ...
  svtkmdiy::save(bb, svtkm::Id(1)); //is basic storage
  svtkmdiy::save(bb, obj.GetStorage().GetArray(), static_cast<std::size_t>(count));
}
}

template <typename T, typename S>
inline void SVTKM_CONT ArrayHandleDefaultSerialization(svtkmdiy::BinaryBuffer& bb,
                                                      const svtkm::cont::ArrayHandle<T, S>& obj)
{
  using is_basic = typename std::is_same<S, svtkm::cont::StorageTagBasic>::type;
...
template <typename T>
SVTKM_CONT void Serialization<svtkm::cont::ArrayHandle<T>>::load(BinaryBuffer& bb,
                                                               svtkm::cont::ArrayHandle<T>& obj)
{
  svtkm::Id count = 0;
  svtkmdiy::load(bb, count);
  obj.Allocate(count);

  svtkm::Id input_was_basic_storage = 0;
  svtkmdiy::load(bb, input_was_basic_storage);
  if (input_was_basic_storage)
  {
    svtkmdiy::load(bb, obj.GetStorage().GetArray(), static_cast<std::size_t>(count));
  }
  else
  {
...
#endif //svtk_m_cont_ArrayHandle_hxx
