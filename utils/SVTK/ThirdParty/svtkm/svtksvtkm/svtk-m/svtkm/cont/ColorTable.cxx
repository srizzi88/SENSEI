...
#include <svtkm/cont/ColorTablePrivate.hxx>
...
bool LoadColorTablePreset(svtkm::cont::ColorTable::Preset preset, svtkm::cont::ColorTable& table);
bool LoadColorTablePreset(std::string name, svtkm::cont::ColorTable& table);
} // namespace internal

//----------------------------------------------------------------------------
ColorTable::ColorTable(svtkm::cont::ColorTable::Preset preset)
...
//----------------------------------------------------------------------------
ColorTable::ColorTable(const svtkm::Range& range,
...
ColorTable::ColorTable(const svtkm::Range& range,
  ...
  : Impl(std::make_shared<detail::ColorTableInternals>())
{
  svtkm::Vec<float, 3> rgb1(rgba1[0], rgba1[1], rgba1[2]);
  svtkm::Vec<float, 3> rgb2(rgba2[0], rgba2[1], rgba2[2]);
  this->AddSegment(range.Min, rgb1, range.Max, rgb2);
  ...
  : Impl(std::make_shared<detail::ColorTableInternals>())
{
  ...
  this->FillColorTableFromDataPointer(static_cast<svtkm::Int32>(rgbPoints.size()), rgbPoints.data());
  this->FillOpacityTableFromDataPointer(static_cast<svtkm::Int32>(alphaPoints.size()),
                                        alphaPoints.data());
}
...
//----------------------------------------------------------------------------
bool ColorTable::LoadPreset(svtkm::cont::ColorTable::Preset preset)
{
...
void ColorTable::SetColorSpace(ColorSpace space)
{
  ...
  if (this->Impl->CSpace != space || this->Impl->HostSideCache.get() == nullptr)
  {
    ...
    switch (space)
    {
      ...
      {
        auto* hostPortal = new svtkm::exec::ColorTableRGB();
        this->Impl->HostSideCache.reset(hostPortal);
        break;
      }
      case svtkm::cont::ColorSpace::HSV:
      {
        auto* hostPortal = new svtkm::exec::ColorTableHSV();
        this->Impl->HostSideCache.reset(hostPortal);
        break;
      }
      case svtkm::cont::ColorSpace::HSV_WRAP:
      {
        auto* hostPortal = new svtkm::exec::ColorTableHSVWrap();
        this->Impl->HostSideCache.reset(hostPortal);
        break;
      }
      case svtkm::cont::ColorSpace::LAB:
      {
        auto* hostPortal = new svtkm::exec::ColorTableLab();
        this->Impl->HostSideCache.reset(hostPortal);
        break;
      }
      case svtkm::cont::ColorSpace::DIVERGING:
      {
        auto* hostPortal = new svtkm::exec::ColorTableDiverging();
        this->Impl->HostSideCache.reset(hostPortal);
        break;
      }
      default:
        throw svtkm::cont::ErrorBadType("unknown svtkm::cont::ColorType requested");
    }
  }
}
...
//----------------------------------------------------------------------------
void ColorTable::SetBelowRangeColor(const svtkm::Vec<float, 3>& c)
{
...
//----------------------------------------------------------------------------
const svtkm::Vec<float, 3>& ColorTable::GetBelowRangeColor() const
{
  return this->Impl->HostSideCache->BelowRangeColor;
}

//----------------------------------------------------------------------------
void ColorTable::SetAboveRangeColor(const svtkm::Vec<float, 3>& c)
{
...
//----------------------------------------------------------------------------
const svtkm::Vec<float, 3>& ColorTable::GetAboveRangeColor() const
{
  return this->Impl->HostSideCache->AboveRangeColor;
}

//----------------------------------------------------------------------------
void ColorTable::SetNaNColor(const svtkm::Vec<float, 3>& c)
{
...
//----------------------------------------------------------------------------
const svtkm::Vec<float, 3>& ColorTable::GetNaNColor() const
{
...
//---------------------------------------------------------------------------
const svtkm::Range& ColorTable::GetRange() const
{
  return this->Impl->TableRange;
}

//---------------------------------------------------------------------------
void ColorTable::RescaleToRange(const svtkm::Range& r)
{
  if (r == this->GetRange())
  {
    return;
  ...
  SVTKM_ASSERT(oldScale > 0);
  SVTKM_ASSERT(newScale > 0);
...
//---------------------------------------------------------------------------
svtkm::Int32 ColorTable::AddPoint(double x, const svtkm::Vec<float, 3>& rgb)
{
  ...
  this->Impl->ColorArraysChanged = true;
  return static_cast<svtkm::Int32>(index);
}

//---------------------------------------------------------------------------
svtkm::Int32 ColorTable::AddPointHSV(double x, const svtkm::Vec<float, 3>& hsv)
{
  return this->AddPoint(x, hsvTorgb(hsv));
}

//---------------------------------------------------------------------------
svtkm::Int32 ColorTable::AddSegment(double x1,
                                   const svtkm::Vec<float, 3>& rgb1,
                                   double x2,
                                   const svtkm::Vec<float, 3>& rgb2)
{
  ...
  svtkm::Int32 pos = this->AddPoint(x1, rgb1);
  this->AddPoint(x2, rgb2);
  return pos;
}

//---------------------------------------------------------------------------
svtkm::Int32 ColorTable::AddSegmentHSV(double x1,
...
//---------------------------------------------------------------------------
bool ColorTable::GetPoint(svtkm::Int32 index, svtkm::Vec<double, 4>& data) const
{
...
//---------------------------------------------------------------------------
svtkm::Int32 ColorTable::UpdatePoint(svtkm::Int32 index, const svtkm::Vec<double, 4>& data)
{
...
bool ColorTable::RemovePoint(double x)
{
  auto begin = this->Impl->ColorNodePos.begin();
  auto pos = std::lower_bound(begin, this->Impl->ColorNodePos.end(), x);
  return this->RemovePoint(static_cast<svtkm::Int32>(std::distance(begin, pos)));
}

//---------------------------------------------------------------------------
bool ColorTable::RemovePoint(svtkm::Int32 index)
{
  std::size_t i = static_cast<std::size_t>(index);
  const std::size_t size = this->Impl->ColorNodePos.size();
  if (index < 0 || i >= size)
...
//---------------------------------------------------------------------------
svtkm::Int32 ColorTable::GetNumberOfPoints() const
{
  return static_cast<svtkm::Int32>(this->Impl->ColorNodePos.size());
}

//---------------------------------------------------------------------------
svtkm::Int32 ColorTable::AddPointAlpha(double x, float alpha, float midpoint, float sharpness)
{
  ...
  const svtkm::Vec<float, 2> midsharp(midpoint, sharpness);
  std::size_t index = 0;
...
//---------------------------------------------------------------------------
svtkm::Int32 ColorTable::AddSegmentAlpha(double x1,
                                        float alpha1,
                                        double x2,
                                        float alpha2,
                                        ...
                                        const svtkm::Vec<float, 2>& mid_sharp2)
{
  ...
  svtkm::Int32 pos = this->AddPointAlpha(x1, alpha1, mid_sharp1[0], mid_sharp1[1]);
  this->AddPointAlpha(x2, alpha2, mid_sharp2[0], mid_sharp2[1]);
  return pos;
}

//---------------------------------------------------------------------------
bool ColorTable::GetPointAlpha(svtkm::Int32 index, svtkm::Vec<double, 4>& data) const
{
  std::size_t i = static_cast<std::size_t>(index);
...
//---------------------------------------------------------------------------
svtkm::Int32 ColorTable::UpdatePointAlpha(svtkm::Int32 index, const svtkm::Vec<double, 4>& data)
{
  //skip data[0] as we don't care about position
  if (outside_range(data[1], data[2], data[3]))
  {
...
bool ColorTable::RemovePointAlpha(double x)
{
  auto begin = this->Impl->OpacityNodePos.begin();
  auto pos = std::lower_bound(begin, this->Impl->OpacityNodePos.end(), x);
  return this->RemovePointAlpha(static_cast<svtkm::Int32>(std::distance(begin, pos)));
}

//---------------------------------------------------------------------------
bool ColorTable::RemovePointAlpha(svtkm::Int32 index)
{
  std::size_t i = static_cast<std::size_t>(index);
  const std::size_t size = this->Impl->OpacityNodePos.size();
  if (index < 0 || i >= size)
...
//---------------------------------------------------------------------------
svtkm::Int32 ColorTable::GetNumberOfPointsAlpha() const
{
  return static_cast<svtkm::Int32>(this->Impl->OpacityNodePos.size());
}

//---------------------------------------------------------------------------
bool ColorTable::FillColorTableFromDataPointer(svtkm::Int32 n, const double* ptr)
{
  if (n <= 0 || ptr == nullptr)
  {
    return false;
...
//---------------------------------------------------------------------------
bool ColorTable::FillColorTableFromDataPointer(svtkm::Int32 n, const float* ptr)
{
  if (n <= 0 || ptr == nullptr)
  {
    return false;
  ...
  for (std::size_t i = 0; i < size; ++i)
  { //allows us to support unsorted arrays
    svtkm::Vec<float, 3> rgb(ptr[1], ptr[2], ptr[3]);
    this->AddPoint(ptr[0], rgb);
    ptr += 4;
  }
  this->Impl->ColorArraysChanged = true;
...
//---------------------------------------------------------------------------
bool ColorTable::FillOpacityTableFromDataPointer(svtkm::Int32 n, const double* ptr)
{
  if (n <= 0 || ptr == nullptr)
  {
    return false;
...
//---------------------------------------------------------------------------
bool ColorTable::FillOpacityTableFromDataPointer(svtkm::Int32 n, const float* ptr)
{
  if (n <= 0 || ptr == nullptr)
  {
    return false;
...
//---------------------------------------------------------------------------
svtkm::Id ColorTable::GetModifiedCount() const
{
...
void ColorTable::UpdateExecutionColorTable(
  svtkm::cont::VirtualObjectHandle<svtkm::exec::ColorTableBase>* handle) const
{
...
ColorTable::TransferState ColorTable::GetExecutionDataForTransfer() const
{
  ...
  {
    this->Impl->ColorPosHandle = svtkm::cont::make_ArrayHandle(this->Impl->ColorNodePos);
    this->Impl->ColorRGBHandle = svtkm::cont::make_ArrayHandle(this->Impl->ColorRGB);
  }

  if (this->Impl->OpacityArraysChanged)
  {
    this->Impl->OpacityPosHandle = svtkm::cont::make_ArrayHandle(this->Impl->OpacityNodePos);
    this->Impl->OpacityAlphaHandle = svtkm::cont::make_ArrayHandle(this->Impl->OpacityAlpha);
    this->Impl->OpacityMidSharpHandle = svtkm::cont::make_ArrayHandle(this->Impl->OpacityMidSharp);
  }
...
//----------------------------------------------------------------------------
svtkm::exec::ColorTableBase* ColorTable::GetControlRepresentation() const
{
  return this->Impl->HostSideCache.get();
}

//----------------------------------------------------------------------------
svtkm::cont::VirtualObjectHandle<svtkm::exec::ColorTableBase> const* ColorTable::GetExecutionHandle()
  const
{
