...
struct ColorTableInternals
{
  ...
  svtkm::Range TableRange = { 1.0, 0.0 };
  ...
  std::unique_ptr<svtkm::exec::ColorTableBase> HostSideCache;
  //Execution side version of the ColorTableBase.
  std::unique_ptr<svtkm::cont::VirtualObjectHandle<svtkm::exec::ColorTableBase>> ExecHandle;

  std::vector<double> ColorNodePos;
  std::vector<svtkm::Vec<float, 3>> ColorRGB;

  std::vector<double> OpacityNodePos;
  std::vector<float> OpacityAlpha;
  std::vector<svtkm::Vec<float, 2>> OpacityMidSharp;

  svtkm::cont::ArrayHandle<double> ColorPosHandle;
  svtkm::cont::ArrayHandle<svtkm::Vec<float, 3>> ColorRGBHandle;
  svtkm::cont::ArrayHandle<double> OpacityPosHandle;
  svtkm::cont::ArrayHandle<float> OpacityAlphaHandle;
  svtkm::cont::ArrayHandle<svtkm::Vec<float, 2>> OpacityMidSharpHandle;
...
{
  static constexpr svtkm::Int64 value = 2048L;
};
...
{
  static constexpr svtkm::Int64 value = 4503599627370496L;
};

inline bool rangeAlmostEqual(const svtkm::Range& r)
{
  svtkm::Int64 irange[2];
  // needs to be a memcpy to avoid strict aliasing issues, doing a count
  // of 2*sizeof(T) to couple both values at the same time
  std::memcpy(irange, &r.Min, sizeof(svtkm::Int64));
  std::memcpy(irange + 1, &r.Max, sizeof(svtkm::Int64));
  // determine the absolute delta between these two numbers.
  const svtkm::Int64 delta = std::abs(irange[1] - irange[0]);
...
{
  constexpr bool is_float32_type = std::is_same<T, svtkm::Float32>::value;
  using IRange = typename std::conditional<is_float32_type, svtkm::Int32, svtkm::Int64>::type;
  IRange irange[2];
  // needs to be a memcpy to avoid strict aliasing issues, doing a count
  // of 2*sizeof(T) to couple both values at the same time
...
inline svtkm::Range adjustRange(const svtkm::Range& r)
{
...
inline bool outside_vrange(const svtkm::Vec<double, 2>& x)
  { return x[0] < 0.0 || x[0] > 1.0 || x[1] < 0.0 || x[1] > 1.0; }
inline bool outside_vrange(const svtkm::Vec<double, 3>& x)
  { return x[0] < 0.0 || x[0] > 1.0 || x[1] < 0.0 || x[1] > 1.0 || x[2] < 0.0 || x[2] > 1.0; }
inline bool outside_vrange(float x) { return x < 0.0f || x > 1.0f; }
inline bool outside_vrange(const svtkm::Vec<float, 2>& x)
  { return x[0] < 0.0f || x[0] > 1.0f || x[1] < 0.0f || x[1] > 1.0f; }
inline bool outside_vrange(const svtkm::Vec<float, 3>& x)
