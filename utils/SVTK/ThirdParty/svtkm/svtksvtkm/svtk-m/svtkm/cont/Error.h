...
//============================================================================
#ifndef svtk_m_cont_Error_h
#define svtk_m_cont_Error_h

// Note that this class and (most likely) all of its subclasses are not
// templated.  If there is any reason to create a SVTKm control library,
...
/// The superclass of all exceptions thrown by any SVTKm function or method.
///
class SVTKM_ALWAYS_EXPORT Error : public std::exception
{
public:
...
//To get around this we make our own A/W variants on windows.
  ...
  /// are not device independent, `svtkm::TryExecute`, for example, may try
...
protected:
  ...
  Error(const std::string& message, bool is_device_independent = false)
    : Message(message)
    , StackTrace(svtkm::cont::GetStackTrace(1))
    , What(Message + "\n" + StackTrace)
