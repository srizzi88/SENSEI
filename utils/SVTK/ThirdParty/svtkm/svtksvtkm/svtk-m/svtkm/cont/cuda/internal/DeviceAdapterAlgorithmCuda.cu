...
static svtkm::cont::cuda::ScheduleParameters (
...
SVTKM_CONT_EXPORT void InitScheduleParameters(
  svtkm::cont::cuda::ScheduleParameters (*function)(const char*, int, int, int, int, int))
{
...
struct SVTKM_CONT_EXPORT ScheduleParameterBuilder
{
  ...
  std::map<GPU_STRATA, svtkm::cont::cuda::ScheduleParameters> Presets;
  std::function<svtkm::cont::cuda::ScheduleParameters(const char*, int, int, int, int, int)> Compute;
  ...
  ScheduleParameterBuilder()
    : Presets{
      ...
      { GPU_STRATA::OLDER,
                                { 32, 128,  8, { 16, 16, 1 }, 32, { 8, 8, 4 } } }, //SVTK-m default for less than pascal
      { GPU_STRATA::PASCAL,     { 32, 128,  8, { 16, 16, 1 }, 32, { 8, 8, 4 } } }, //SVTK-m default for pascal
      { GPU_STRATA::VOLTA,      { 32, 128,  8, { 16, 16, 1 }, 32, { 8, 8, 4 } } }, //SVTK-m default for volta
      { GPU_STRATA::PASCAL_HPC, { 32, 256, 16, { 16, 16, 1 }, 64, { 8, 8, 4 } } }, //P100
      { GPU_STRATA::VOLTA_HPC,  { 32, 256, 16, { 16, 16, 1 }, 64, { 8, 8, 4 } } }, //V100
    }
    , Compute(nullptr)
  {
    if (svtkm::cont::cuda::ComputeFromEnv != nullptr)
    {
      this->Compute = svtkm::cont::cuda::ComputeFromEnv;
    }
    else
    {
  ...
  svtkm::cont::cuda::ScheduleParameters ComputeFromPreset(const char* name,
                                                         int major,
                                                         int minor,
                                                         int numSMs,
                                                         int maxThreadsPerSM,
...
SVTKM_CONT_EXPORT void SetupKernelSchedulingParameters()
{
  //check flag
  ...
  std::call_once(lookupBuiltFlag, []() {
    ...
    int count = 0;
    SVTKM_CUDA_CALL(cudaGetDeviceCount(&count));
...
auto DeviceAdapterAlgorithm<svtkm::cont::DeviceAdapterTagCuda>::GetPinnedErrorArray()
  -> const PinnedErrorArray&
{
  constexpr svtkm::Id ERROR_ARRAY_SIZE = 1024;
  static thread_local PinnedErrorArray local;

  if (!local.HostPtr)
  {
    SVTKM_CUDA_CALL(cudaMallocHost((void**)&local.HostPtr, ERROR_ARRAY_SIZE, cudaHostAllocMapped));
    SVTKM_CUDA_CALL(cudaHostGetDevicePointer(&local.DevicePtr, local.HostPtr, 0));
...
void DeviceAdapterAlgorithm<svtkm::cont::DeviceAdapterTagCuda>::SetupErrorBuffer(
  svtkm::exec::cuda::internal::TaskStrided& functor)
{
  auto pinnedArray = GetPinnedErrorArray();
  svtkm::exec::internal::ErrorMessageBuffer errorMessage(pinnedArray.DevicePtr, pinnedArray.Size);
  functor.SetErrorMessageBuffer(errorMessage);
}

void DeviceAdapterAlgorithm<svtkm::cont::DeviceAdapterTagCuda>::CheckForErrors()
{
  auto pinnedArray = GetPinnedErrorArray();
  if (pinnedArray.HostPtr[0] != '\0')
  {
    SVTKM_CUDA_CALL(cudaStreamSynchronize(cudaStreamPerThread));
    auto excep = svtkm::cont::ErrorExecution(pinnedArray.HostPtr);
    pinnedArray.HostPtr[0] = '\0'; // clear
    throw excep;
  }
}

void DeviceAdapterAlgorithm<svtkm::cont::DeviceAdapterTagCuda>::GetBlocksAndThreads(
  svtkm::UInt32& blocks,
  svtkm::UInt32& threadsPerBlock,
  svtkm::Id size)
{
  (void)size;
  svtkm::cont::cuda::internal::SetupKernelSchedulingParameters();

  int deviceId;
  SVTKM_CUDA_CALL(cudaGetDevice(&deviceId)); //get deviceid from cuda
...
void DeviceAdapterAlgorithm<svtkm::cont::DeviceAdapterTagCuda>::LogKernelLaunch(
  ...
  svtkm::Id)
{
  ...
  std::string name = svtkm::cont::TypeToString(worklet_info);
  SVTKM_LOG_F(svtkm::cont::LogLevel::KernelLaunches,
