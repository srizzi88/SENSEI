...
{
  const svtkm::UInt64 numBytes = static_cast<svtkm::UInt64>(numberOfValues) * sizeOfValue;
  ...
  {
    const svtkm::UInt64 managedCapacity =
      static_cast<svtkm::UInt64>(static_cast<const char*>(execArray.ArrayControlCapacity) -
  ...
  {
    const svtkm::UInt64 cap = static_cast<svtkm::UInt64>(static_cast<char*>(execArray.ArrayCapacity) -
  ...
  SVTKM_ASSERT(execArray.Array == nullptr);
  ...
  svtkm::UInt64 numBytes) const
{
  ...
  SVTKM_LOG_F(svtkm::cont::LogLevel::MemTransfer,
             ...
             svtkm::cont::GetHumanReadableSize(numBytes).c_str(),
             numBytes);

  SVTKM_CUDA_CALL(cudaMemcpyAsync(executionPtr,
                                 controlPtr,
                                                                       ...
                                                                       svtkm::UInt64 numBytes) const
{
  ...
  else
  {
    ...
    SVTKM_CUDA_CALL(cudaMemcpyAsync(controlPtr,
                                   executionPtr,
                                   static_cast<std::size_t>(numBytes),
  ...
  svtkm::cont::DeviceAdapterAlgorithm<DeviceAdapterTagCuda>::Synchronize();
}

void ExecutionArrayInterfaceBasic<DeviceAdapterTagCuda>::UsingForRead(
  const void* svtkmNotUsed(controlPtr),
