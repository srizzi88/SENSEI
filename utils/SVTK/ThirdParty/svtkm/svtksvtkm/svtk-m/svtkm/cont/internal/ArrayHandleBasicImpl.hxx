...
#define svtk_m_cont_internal_ArrayHandleBasicImpl_hxx
...
SVTKM_CONT bool ArrayHandle<T, StorageTagBasic>::operator==(const ArrayHandle<VT, ST>&) const
{
...
template <typename T>
svtkm::Id ArrayHandle<T, StorageTagBasic>::GetNumberOfValues() const
{
  LockType lock = this->GetLock();
...
template <typename T>
void ArrayHandle<T, StorageTagBasic>::Allocate(svtkm::Id numberOfValues)
{
  LockType lock = this->GetLock();
...
template <typename T>
void ArrayHandle<T, StorageTagBasic>::Shrink(svtkm::Id numberOfValues)
{
  LockType lock = this->GetLock();
...
ArrayHandle<T, StorageTagBasic>::PrepareForOutput(svtkm::Id numVals, DeviceAdapterTag device)
{
  SVTKM_IS_DEVICE_ADAPTER_TAG(DeviceAdapterTag);
  LockType lock = this->GetLock();
  this->PrepareForDevice(lock, device);
...
#endif // not svtk_m_cont_internal_ArrayHandleBasicImpl_hxx
