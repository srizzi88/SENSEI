...
struct TemplatedTests
{
  using ArrayManagerType =
    svtkm::cont::internal::ArrayManagerExecutionShareWithControl<T, svtkm::cont::StorageTagBasic>;
  ...
  {
    svtkm::cont::ArrayPortalToIterators<typename StorageType::PortalType> iterators(
      array.GetPortal());
  ...
  void InputData()
  {
    ...
    SVTKM_TEST_ASSERT(svtkm::cont::ArrayPortalToIteratorBegin(storage.GetPortalConst()) ==
                       svtkm::cont::ArrayPortalToIteratorBegin(executionArray.PrepareForInput(true)),
                     "Execution array manager not holding control array iterators.");

    SVTKM_TEST_ASSERT(CheckManager(executionArray, INPUT_VALUE), "Did not get correct array back.");
  }
  ...
  void InPlaceData()
  {
    ...
    SVTKM_TEST_ASSERT(
      svtkm::cont::ArrayPortalToIteratorBegin(storage.GetPortal()) ==
        svtkm::cont::ArrayPortalToIteratorBegin(executionArray.PrepareForInPlace(true)),
      "Execution array manager not holding control array iterators.");
    SVTKM_TEST_ASSERT(
      svtkm::cont::ArrayPortalToIteratorBegin(storage.GetPortalConst()) ==
        svtkm::cont::ArrayPortalToIteratorBegin(executionArray.PrepareForInput(false)),
      "Execution array manager not holding control array iterators.");
  ...
  void OutputData()
  {
    ...
    svtkm::cont::ArrayPortalToIterators<typename ArrayManagerType::PortalType> iterators(
      executionArray.PrepareForOutput(ARRAY_SIZE));
    std::fill(iterators.GetBegin(), iterators.GetEnd(), OUTPUT_VALUE);

    SVTKM_TEST_ASSERT(CheckManager(executionArray, OUTPUT_VALUE), "Did not get correct array back.");

    executionArray.RetrieveOutputData(&storage);

    SVTKM_TEST_ASSERT(CheckStorage(storage, OUTPUT_VALUE),
...
{
  return svtkm::cont::testing::Testing::Run(TestArrayManagerShare, argc, argv);
}
