...
struct Adder : public ScanBody
{
  ...
  struct NodeWrapper : public NodeImpl
  {
    ...
    static constexpr size_t DataSize =
      sizeof(NodeImpl) + sizeof(svtkm::Id2) + 3 * sizeof(NodeWrapper*) + sizeof(bool);
  ...
  ValueType Execute(const svtkm::Id2& range)
  {
...
#pragma GCC diagnostic pop
  ...
  bool IsLeaf(const svtkm::Id2& range) const { return (range[1] - range[0]) <= this->LeafSize; }

  // Use to split ranges. Ensures that the first range is always a multiple of
  // LeafSize, when possible.
  svtkm::Id ComputeMidpoint(const svtkm::Id2& range) const
  {
  ...
  void Prepare(const svtkm::Id2& range)
  {
    ...
    svtkm::Id numVals = range[1] - range[0];
  ...
  void Scan(Node* node)
  {
    ...
    { // split range:
      svtkm::Id midpoint = this->ComputeMidpoint(node->Range);

      Node* right = this->AllocNode();
      right->Parent = node;
      node->Right = right;
      right->Range = svtkm::Id2(midpoint, node->Range[1]);
      ...
      {
        SVTKM_OPENMP_DIRECTIVE(task) { explicitThis->Scan(right); } // end right task

        Node* left = this->AllocNode();
        left->Parent = node;
        node->Left = left;
        left->Range = svtkm::Id2(node->Range[0], midpoint);
  ...
  void UpdateOutput(Node* node)
  {
    if (node->Left != nullptr)
    {
      ...
      else // Otherwise descent into both:
      {
        ...
        // section.
        SVTKM_OPENMP_DIRECTIVE(task) { explicitThis->UpdateOutput(node->Right); } // end task
...
struct ScanExclusiveBody
{
  ...
  struct Node
  {
    ...
    ValueType Sum{ svtkm::TypeTraits<ValueType>::ZeroInitialization() };

    // The sum of all elements prior to this node's range
    ValueType Carry{ svtkm::TypeTraits<ValueType>::ZeroInitialization() };
  };
  ...
  void ComputeSummary(Node* node, const svtkm::Id2& range, bool leftEdge)
  {
    auto input = svtkm::cont::ArrayPortalToIteratorBegin(this->InPortal);
    ...
    {
      for (svtkm::Id i = range[0] + 1; i < range[1]; ++i)
      {
  ...
  void UpdateOutput(const Node* node, const svtkm::Id2& range, bool leftEdge)
  {
    this->UpdateOutputImpl(node, range, leftEdge, false);
  }

  ValueType UpdateOutputImpl(const Node* node, const svtkm::Id2& range, bool skip, bool useInit)
  {
    if (skip)
    {
    ...
    auto input = svtkm::cont::ArrayPortalToIteratorBegin(this->InPortal);
    auto output = svtkm::cont::ArrayPortalToIteratorBegin(this->OutPortal);
    ...
    svtkm::Id end = range[1];

    for (svtkm::Id i = range[0]; i < end; ++i)
    {
...
struct ScanInclusiveBody
{
  ...
  ValueType UpdateOutputImpl(const Node* node, const svtkm::Id2& range, bool skip, bool useCarry)
  {
    if (skip)
    {
      // Do nothing; this was already done in ComputeSummary.
