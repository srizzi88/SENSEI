...
using Vector3 = svtkm::Vec3f;

const svtkm::Id3 DIMENSIONS(16, 18, 5);
const svtkm::Id NUM_POINTS = 1440;
...
void TestArrayHandleUniformPointCoordinates()
{
  ...
  svtkm::cont::ArrayHandleUniformPointCoordinates arrayHandle(DIMENSIONS, ORIGIN, SPACING);
  SVTKM_TEST_ASSERT(arrayHandle.GetNumberOfValues() == NUM_POINTS,
                   "Array computed wrong number of points.");

  std::cout << "Getting array portal." << std::endl;
  svtkm::internal::ArrayPortalUniformPointCoordinates portal = arrayHandle.GetPortalConstControl();
  SVTKM_TEST_ASSERT(portal.GetNumberOfValues() == NUM_POINTS, "Portal has wrong number of points.");
  SVTKM_TEST_ASSERT(portal.GetRange3() == DIMENSIONS, "Portal range is wrong.");

  std::cout << "Checking computed values of portal." << std::endl;
  Vector3 expectedValue;
  svtkm::Id flatIndex = 0;
  svtkm::Id3 blockIndex;
  ...
  for (blockIndex[2] = 0; blockIndex[2] < DIMENSIONS[2]; blockIndex[2]++)
  {
    ...
    for (blockIndex[1] = 0; blockIndex[1] < DIMENSIONS[1]; blockIndex[1]++)
    {
      ...
      {
        SVTKM_TEST_ASSERT(test_equal(expectedValue, portal.Get(flatIndex)),
                         "Got wrong value for flat index.");

        SVTKM_TEST_ASSERT(test_equal(expectedValue, portal.Get(blockIndex)),
...
{
  return svtkm::cont::testing::Testing::Run(TestArrayHandleUniformPointCoordinates, argc, argv);
}
