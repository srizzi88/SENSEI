...
void CheckArgs(int argc, char* argv[], T&&... args)
{
  ...
  for (std::size_t i = 0; i < numArgs; ++i)
  {
    SVTKM_TEST_ASSERT(expectedArgs[i] == argv[i], "Arg ", i, " wrong");
  }
...
void InitializeNoOptions()
{
  ...
  char** argv;
  MakeArgs(argc, argv);
  svtkm::cont::InitializeResult result = svtkm::cont::Initialize(argc, argv);
  CheckArgs(argc, argv);
...
{
  std::cout << "Initialize with options both for SVTK-m and some that are not." << std::endl;

  int argc;
  char** argv;
  MakeArgs(argc, argv, "--foo", "--device", "Any", "--bar", "baz");
  svtkm::cont::Initialize(argc, argv, svtkm::cont::InitializeOptions::AddHelp);
...
void InitializeWithHelp()
{
  ...
  SVTKM_TEST_FAIL("Help argument did not exit as expected.");
}

void DoInitializeTests()
{
  // Technically, by the time we get here, we have already called Initialize once.
  std::cout << "Note: This test calls svtkm::cont::Initialize multiple times to test" << std::endl
...
{
  return svtkm::cont::testing::Testing::Run(DoInitializeTests, argc, argv);
}
