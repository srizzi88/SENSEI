...
//============================================================================
#ifndef svtk_m_exec_ColorTable_hxx
#define svtk_m_exec_ColorTable_hxx
...
SVTKM_EXEC
inline void RGBToHSV(const svtkm::Vec<float, 3>& rgb, svtkm::Vec<float, 3>& hsv)
{
  constexpr float onethird = 1.0f / 3.0f;
  constexpr float onesixth = 1.0f / 6.0f;
  constexpr float twothird = 2.0f / 3.0f;

  const float cmax = svtkm::Max(rgb[0], svtkm::Max(rgb[1], rgb[2]));
  const float cmin = svtkm::Min(rgb[0], svtkm::Min(rgb[1], rgb[2]));
...
SVTKM_EXEC
inline svtkm::Vec<float, 3> HSVToRGB(const svtkm::Vec<float, 3>& hsv)
{
...
SVTKM_EXEC
inline void RGBToLab(const svtkm::Vec<float, 3>& rgb, svtkm::Vec<float, 3>& lab)
{
  // clang-format off
  ...
  float g = rgb[1];
  float b = rgb[2];
  if ( r > 0.04045f ) r = svtkm::Pow(( r + 0.055f ) / 1.055f, 2.4f);
  else                r = r / 12.92f;
  if ( g > 0.04045f ) g = svtkm::Pow(( g + 0.055f ) / 1.055f, 2.4f);
  else                g = g / 12.92f;
  if ( b > 0.04045f ) b = svtkm::Pow(( b + 0.055f ) / 1.055f, 2.4f);
  else                b = b / 12.92f;
  ...
  if ( var_X > 0.008856f ) var_X = svtkm::Pow(var_X, onethird);
  else                     var_X = ( 7.787f * var_X ) + sixteen_onesixteen;
  if ( var_Y > 0.008856f ) var_Y = svtkm::Pow(var_Y, onethird);
  else                     var_Y = ( 7.787f * var_Y ) + sixteen_onesixteen;
  if ( var_Z > 0.008856f ) var_Z = svtkm::Pow(var_Z, onethird);
...
SVTKM_EXEC
inline svtkm::Vec<float, 3> LabToRGB(const svtkm::Vec<float, 3>& lab)
{
  // clang-format off
  svtkm::Vec<float, 3> rgb;
  ...
  if ( svtkm::Pow(rgb[0],3) > 0.008856f ) rgb[0] = svtkm::Pow(rgb[0],3);
  else rgb[0] = ( rgb[0] - sixteen_onesixteen ) / 7.787f;

  if ( svtkm::Pow(rgb[1],3) > 0.008856f ) rgb[1] = svtkm::Pow(rgb[1],3);
  else rgb[1] = ( rgb[1] - sixteen_onesixteen ) / 7.787f;

  if ( svtkm::Pow(rgb[2],3) > 0.008856f ) rgb[2] = svtkm::Pow(rgb[2],3);
  ...
  if (r > 0.0031308f) r = 1.055f * (svtkm::Pow(r, one_twopointfour)) - 0.055f;
  else r = 12.92f * r;
  if (g > 0.0031308f) g = 1.055f * (svtkm::Pow(g ,one_twopointfour)) - 0.055f;
  else  g = 12.92f * (g);
  if (b > 0.0031308f) b = 1.055f * (svtkm::Pow(b, one_twopointfour)) - 0.055f;
  else b = 12.92f * (b);

  // Clip colors. ideally we would do something that is perceptually closest
  // (since we can see colors outside of the display gamut), but this seems to
  // work well enough.
  const float maxVal = svtkm::Max(r, svtkm::Max(g,b));
  if (maxVal > 1.0f)
  {
    r /= maxVal;
    g /= maxVal;
...
SVTKM_EXEC
inline void LabToMsh(const svtkm::Vec<float, 3>& lab, svtkm::Vec<float, 3>& msh)
{
  const float& L = lab[0];
  const float& a = lab[1];
  const float& b = lab[2];
  ...
  M = svtkm::Sqrt(L * L + a * a + b * b);
  s = (M > 0.001f) ? svtkm::ACos(L / M) : 0.0f;
  h = (s > 0.001f) ? svtkm::ATan2(b, a) : 0.0f;
}

// Convert from a special polar version of CIELAB (useful for creating
// continuous diverging color maps).
SVTKM_EXEC
inline svtkm::Vec<float, 3> MshToLab(const svtkm::Vec<float, 3>& msh)
{
  const float& M = msh[0];
  const float& s = msh[1];
  const float& h = msh[2];
  svtkm::Vec<float, 3> r(
    M * svtkm::Cos(s), M * svtkm::Sin(s) * svtkm::Cos(h), M * svtkm::Sin(s) * svtkm::Sin(h));
  return r;
}
...
{
  constexpr float f_pi = svtkm::Pif();
  constexpr float f_two_pi = svtkm::TwoPif();
  float adiff = a1 - a2;
  if (adiff < 0.0f)
    adiff = -adiff;
...
SVTKM_EXEC
inline float DivergingAdjustHue(const svtkm::Vec<float, 3>& msh, float unsatM)
{
  const float sinS = svtkm::Sin(msh[1]);
  ...
  else
  {
    ...
    float hueSpin = msh[1] * svtkm::Sqrt(unsatM * unsatM - msh[0] * msh[0]) / (msh[0] * sinS);

    constexpr float one_third_pi = svtkm::Pi_3f();
...
//---------------------------------------------------------------------------
SVTKM_EXEC
svtkm::Vec<float, 3> ColorTableBase::MapThroughColorSpace(double value) const
{
  svtkm::Vec<float, 3> rgb1, rgb2;
  float weight = 0;
...
void ColorTableBase::FindColors(double value,
                                svtkm::Vec<float, 3>& rgb1,
                                svtkm::Vec<float, 3>& rgb2,
                                float& weight) const
{
...
float ColorTableBase::MapThroughOpacitySpace(double value) const
{
  ...
  else
  {
    ...
    else if (midsharp[1] == 0.0f)
    {
      ...
      return svtkm::Lerp(alpha1, alpha2, weight);
    }
    else
    {
      ...
      if (weight < 0.5f)
      {
        weight = 0.5f * svtkm::Pow(weight * 2.0f, 1.0f + 10.0f * midsharp[1]);
      }
      else if (weight > .5)
      {
        weight = 1.0f - 0.5f * svtkm::Pow((1.0f - weight) * 2.0f, 1.0f + 10.0f * midsharp[1]);
      }
      ...
      // the Y range
      result = svtkm::Max(result, svtkm::Min(alpha1, alpha2));
      result = svtkm::Min(result, svtkm::Max(alpha1, alpha2));
      return result;
    }
  }
}
//---------------------------------------------------------------------------
SVTKM_EXEC
svtkm::Vec<float, 3> ColorTableRGB::MapThroughColorSpace(const svtkm::Vec<float, 3>& rgb1,
                                                        const svtkm::Vec<float, 3>& rgb2,
                                                        float weight) const
{
  return svtkm::Lerp(rgb1, rgb2, weight);
}

//---------------------------------------------------------------------------
SVTKM_EXEC
svtkm::Vec<float, 3> ColorTableHSV::MapThroughColorSpace(const svtkm::Vec<float, 3>& rgb1,
                                                        const svtkm::Vec<float, 3>& rgb2,
                                                        float weight) const
{
  svtkm::Vec<float, 3> hsv1, hsv2;
  detail::RGBToHSV(rgb1, hsv1);
  detail::RGBToHSV(rgb2, hsv2);

  auto tmp = svtkm::Lerp(hsv1, hsv2, weight);
  if (tmp[0] < 0.0f)
  {
    tmp[0] += 1.0f;
  }
...
//---------------------------------------------------------------------------
SVTKM_EXEC
svtkm::Vec<float, 3> ColorTableHSVWrap::MapThroughColorSpace(const svtkm::Vec<float, 3>& rgb1,
                                                            const svtkm::Vec<float, 3>& rgb2,
                                                            float weight) const
{
  svtkm::Vec<float, 3> hsv1, hsv2;
...
//---------------------------------------------------------------------------
SVTKM_EXEC
svtkm::Vec<float, 3> ColorTableLab::MapThroughColorSpace(const svtkm::Vec<float, 3>& rgb1,
                                                        const svtkm::Vec<float, 3>& rgb2,
                                                        float weight) const
{
  svtkm::Vec<float, 3> lab1, lab2;
  detail::RGBToLab(rgb1, lab1);
  detail::RGBToLab(rgb2, lab2);

  auto tmp = svtkm::Lerp(lab1, lab2, weight);
...
//---------------------------------------------------------------------------
SVTKM_EXEC
svtkm::Vec<float, 3> ColorTableDiverging::MapThroughColorSpace(const svtkm::Vec<float, 3>& rgb1,
                                                              const svtkm::Vec<float, 3>& rgb2,
                                                              float weight) const
{
  svtkm::Vec<float, 3> lab1, lab2;
      ...
      (detail::DivergingAngleDiff(msh1[2], msh2[2]) > one_third_pi))
  {
    ...
    // scalar value.
    float Mmid = svtkm::Max(msh1[0], msh2[0]);
    Mmid = svtkm::Max(88.0f, Mmid);
    if (weight < 0.5f)
    {
      msh2[0] = Mmid;
      msh2[1] = 0.0f;
  ...
  auto tmp = svtkm::Lerp(msh1, msh2, weight);
...
SVTKM_EXPLICITLY_INSTANTIATE_TRANSFER(svtkm::exec::ColorTableRGB);
SVTKM_EXPLICITLY_INSTANTIATE_TRANSFER(svtkm::exec::ColorTableHSV);
SVTKM_EXPLICITLY_INSTANTIATE_TRANSFER(svtkm::exec::ColorTableHSVWrap);
SVTKM_EXPLICITLY_INSTANTIATE_TRANSFER(svtkm::exec::ColorTableLab);
SVTKM_EXPLICITLY_INSTANTIATE_TRANSFER(svtkm::exec::ColorTableDiverging);
#endif
