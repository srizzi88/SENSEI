...
//============================================================================
#ifndef svtk_m_exec_cuda_internal_ExecutionPolicy_h
#define svtk_m_exec_cuda_internal_ExecutionPolicy_h
...
struct svtkm_cuda_policy : thrust::device_execution_policy<svtkm_cuda_policy>
{
};
...
__host__ __device__ void sort(
  const svtkm_cuda_policy& exec,
  T* first,
  T* last,
  svtkm::exec::cuda::internal::WrappedBinaryPredicate<T, svtkm::SortLess> comp)
...
__host__ __device__ void sort(
  ...
  T* first,
  T* last,
  svtkm::exec::cuda::internal::WrappedBinaryPredicate<T, ::thrust::less<T>> comp)
{ //sort for concrete pointers and less than op
  //this makes sure that we invoke the thrust radix sort and not merge sort
  return thrust::sort(ThrustCudaPolicyPerThread, first, last, thrust::less<T>());
}
...
__host__ __device__ void sort(
  ...
  T* first,
  T* last,
  svtkm::exec::cuda::internal::WrappedBinaryPredicate<T, svtkm::SortGreater> comp)
...
__host__ __device__ void sort(
  ...
  T* first,
  T* last,
  svtkm::exec::cuda::internal::WrappedBinaryPredicate<T, ::thrust::greater<T>> comp)
{ //sort for concrete pointers and greater than op
  //this makes sure that we invoke the thrust radix sort and not merge sort
  return thrust::sort(ThrustCudaPolicyPerThread, first, last, thrust::greater<T>());
}
...
__host__ __device__ void sort(const svtkm_cuda_policy& exec,
...
__host__ __device__ void sort_by_key(const svtkm_cuda_policy& exec,
...
{
#if defined(SVTKM_CUDA_VERSION_MAJOR) && (SVTKM_CUDA_VERSION_MAJOR == 7) &&                          \
  (SVTKM_CUDA_VERSION_MINOR >= 5)
