...
void ValidateLCSFilterResult(const svtkm::cont::ArrayHandle<svtkm::FloatDefault>& svtkmOutput,
                             const svtkm::cont::ArrayHandle<svtkm::FloatDefault>& visitOutput,
                             const svtkm::cont::ArrayHandle<svtkm::FloatDefault>& expDiff)
{
  SVTKM_TEST_ASSERT(svtkmOutput.GetNumberOfValues() == visitOutput.GetNumberOfValues(),
                   "Wrong number of values");

  const svtkm::FloatDefault tolerance = static_cast<svtkm::FloatDefault>(1e-3);

  auto svtkmPortal = svtkmOutput.GetPortalConstControl();
  auto visitPortal = visitOutput.GetPortalConstControl();
  auto diffPortal = expDiff.GetPortalConstControl();

  for (svtkm::Id index = 0; index < svtkmOutput.GetNumberOfValues(); index++)
  {
    auto svtkm = svtkmPortal.Get(index);
    auto visit = visitPortal.Get(index);
    auto exp = diffPortal.Get(index);
    auto diff = svtkm::Abs(svtkm - visit);
    std::stringstream message;
    message << "Calculated o/p is not correct, \n"
            << "SVTKM : " << svtkm << ", VISIT : " << visit << ", RES : " << svtkm::Abs(diff - exp);
    SVTKM_TEST_ASSERT(svtkm::Abs(diff - exp) <= tolerance || diff < tolerance, message.str());
  }
}

svtkm::FloatDefault vecData2D[200 * 3] = {
...
svtkm::FloatDefault vecData3D[125 * 3] = {
...
svtkm::FloatDefault visitData2D[200] = {
...
svtkm::FloatDefault visitData3D[125] = {
...
svtkm::FloatDefault diffData2D[200] = {
...
svtkm::FloatDefault diffData3D[125] = {
...
void PopulateData(std::vector<svtkm::Vec3f>& fieldVec,
                  std::vector<svtkm::FloatDefault>& visitVec,
                  std::vector<svtkm::FloatDefault>& diffVec,
                  const Option option)
{
  switch (option)
  {
    case Option::_2D:
    {
      svtkm::Id elements = 200;
      for (svtkm::Id index = 0; index < elements; index++)
      {
        svtkm::Id flatidx = index * 3;
        svtkm::Vec3f field;
        ...
        svtkm::FloatDefault diff = auxiliary::diffData2D[index];
        diffVec.push_back(diff);
        svtkm::FloatDefault visit = auxiliary::visitData2D[index];
    ...
    case Option::_3D:
    {
      ...
      for (svtkm::Id index = 0; index < elements; index++)
      {
        ...
        fieldVec.push_back(field);
        svtkm::FloatDefault diff = auxiliary::diffData3D[index];
        diffVec.push_back(diff);
        svtkm::FloatDefault visit = auxiliary::visitData3D[index];
        visitVec.push_back(visit);
      }
      break;
    }
...
void Test2DLCS()
{
  ...
  svtkm::cont::DataSet inputData;
  svtkm::Id2 dims(20, 10);
  svtkm::Vec2f origin(0.0f, 0.0f);
  svtkm::Vec2f spacing;
  spacing[0] = 2.0f / static_cast<svtkm::FloatDefault>(dims[0] - 1);
  spacing[1] = 1.0f / static_cast<svtkm::FloatDefault>(dims[1] - 1);
  svtkm::cont::DataSetBuilderUniform dataBuilder;
  inputData = dataBuilder.Create(dims, origin, spacing);

  std::vector<svtkm::FloatDefault> diffVec;
  std::vector<svtkm::FloatDefault> visitVec;
  std::vector<svtkm::Vec3f> fieldVec;

  auxiliary::PopulateData(fieldVec, visitVec, diffVec, auxiliary::Option::_2D);

  svtkm::cont::ArrayHandle<svtkm::FloatDefault> diffHandle = svtkm::cont::make_ArrayHandle(diffVec);
  svtkm::cont::ArrayHandle<svtkm::FloatDefault> visitHandle = svtkm::cont::make_ArrayHandle(visitVec);
  svtkm::cont::ArrayHandle<svtkm::Vec3f> fieldHandle = svtkm::cont::make_ArrayHandle(fieldVec);
  svtkm::cont::DataSetFieldAdd fieldAdder;
  fieldAdder.AddPointField(inputData, "velocity", fieldHandle);

  svtkm::filter::LagrangianStructures lagrangianStructures;
  ...
  svtkm::cont::DataSet outputData = lagrangianStructures.Execute(inputData);

  svtkm::cont::ArrayHandle<svtkm::FloatDefault> FTLEField;
...
void Test3DLCS()
{
  ...
  svtkm::Vec3f origin(0.0f, 0.0f, 0.0f);
  svtkm::Vec3f spacing;
  spacing[0] = 10.0f / static_cast<svtkm::FloatDefault>(dims[0] - 1);
  spacing[1] = 10.0f / static_cast<svtkm::FloatDefault>(dims[1] - 1);
  spacing[2] = 10.0f / static_cast<svtkm::FloatDefault>(dims[2] - 1);
  svtkm::cont::DataSetBuilderUniform dataBuilder;
  inputData = dataBuilder.Create(dims, origin, spacing);
...
{
  return svtkm::cont::testing::Testing::Run(TestLagrangianStructures, argc, argv);
}
