...
inline svtkm::cont::DataSet MakeExplicitDataSet()
{
...
bool TestMeshQualityFilter(const svtkm::cont::DataSet& input,
                           const std::vector<svtkm::FloatDefault>& expectedVals,
                           ...
                           svtkm::filter::MeshQuality& filter)
{
  ...
  if (portal1.GetNumberOfValues() != (svtkm::Id)expectedVals.size())
  {
...
int TestMeshQuality()
{
  using FloatVec = std::vector<svtkm::FloatDefault>;

  //Test variables
  svtkm::cont::DataSet input = MakeExplicitDataSet();

  int numFailures = 0;
  bool testFailed = false;

  std::vector<FloatVec> expectedValues;
  std::vector<svtkm::filter::CellMetric> metrics;
  ...
  metrics.push_back(svtkm::filter::CellMetric::ASPECT_RATIO);
  ...
  metrics.push_back(svtkm::filter::CellMetric::ASPECT_GAMMA);
  ...
  metrics.push_back(svtkm::filter::CellMetric::CONDITION);
  ...
  metrics.push_back(svtkm::filter::CellMetric::MIN_ANGLE);
  ...
  metrics.push_back(svtkm::filter::CellMetric::MAX_ANGLE);
  ...
  metrics.push_back(svtkm::filter::CellMetric::MIN_DIAGONAL);
  ...
  metrics.push_back(svtkm::filter::CellMetric::MAX_DIAGONAL);
  ...
  metrics.push_back(svtkm::filter::CellMetric::JACOBIAN);
  ...
  metrics.push_back(svtkm::filter::CellMetric::SCALED_JACOBIAN);
  ...
  metrics.push_back(svtkm::filter::CellMetric::ODDY);
  ...
  metrics.push_back(svtkm::filter::CellMetric::DIAGONAL_RATIO);
  ...
  metrics.push_back(svtkm::filter::CellMetric::SHAPE);
  ...
  metrics.push_back(svtkm::filter::CellMetric::SHEAR);
  ...
  metrics.push_back(svtkm::filter::CellMetric::SKEW);
  ...
  metrics.push_back(svtkm::filter::CellMetric::DIMENSION);
  ...
  metrics.push_back(svtkm::filter::CellMetric::RELATIVE_SIZE_SQUARED);
  ...
  metrics.push_back(svtkm::filter::CellMetric::SHAPE_AND_SIZE);
  ...
  for (unsigned long i = 0; i < numTests; i++)
  {
    ...
    svtkm::filter::MeshQuality filter(metrics[i]);
  ...
  if (numFailures > 0)
  {
    ...
    SVTKM_TEST_ASSERT(see_previous_messages, "Failure occurred during test");
  }
  return 0;
}
...
{
  return svtkm::cont::testing::Testing::Run(TestMeshQuality, argc, argv);
}
