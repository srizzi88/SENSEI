...
#include <svtkm/filter/NDEntropy.h>
...
void RunTest()
{
  ...
  svtkm::filter::NDEntropy ndEntropyFilter;
  ...
  //It is stored in "Entropy" (type svtkm::Float64) field in the return dataset
  //The field has only one element, the entropy
  svtkm::cont::DataSet outputData = ndEntropyFilter.Execute(ds);

  svtkm::cont::ArrayHandle<svtkm::Float64> entropyHandle;
  outputData.GetField("Entropy").GetData().CopyTo(entropyHandle);
  svtkm::Float64 e = entropyHandle.GetPortalControl().Get(0);

  SVTKM_TEST_ASSERT(fabs(e - 7.457857) < 0.001,
