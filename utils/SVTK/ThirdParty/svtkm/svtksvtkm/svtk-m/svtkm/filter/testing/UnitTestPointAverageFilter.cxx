...
void TestPointAverageRegular3D()
{
  ...
  SVTKM_TEST_ASSERT(result.HasPointField("cellvar"), "Field missing.");
  svtkm::cont::Field Result = result.GetPointField("cellvar");
  svtkm::cont::ArrayHandle<svtkm::Float32> resultArrayHandle;
  Result.GetData().CopyTo(resultArrayHandle);

  svtkm::Float32 expected[18] = { 0.f, 0.5f, 1.f, 0.f, 0.5f, 1.f, 1.f, 1.5f, 2.f,
...
void TestPointAverageExplicit1()
{
  ...
  svtkm::Float32 expected[5] = { 100.1f, 100.15f, 100.15f, 100.2f, 100.2f };
  for (int i = 0; i < 5; ++i)
  {
    SVTKM_TEST_ASSERT(test_equal(resultArrayHandle.GetPortalConstControl().Get(i), expected[i]),
                     "Wrong result for PointAverage worklet on 3D regular data");
...
void TestPointAverageExplicit2()
{
  ...
  svtkm::cont::ArrayHandle<svtkm::Float32> resultArrayHandle;
  result.GetPointField("cellvar").GetData().CopyTo(resultArrayHandle);
  svtkm::Float32 expected[11] = { 100.1f, 105.05f, 105.05f, 100.1f, 115.3f, 115.2f,
...
{
  return svtkm::cont::testing::Testing::Run(TestPointAverage, argc, argv);
}
