...
{
  std::vector<svtkm::Float32> pvec(16), cvec(9);
  for (std::size_t i = 0; i < 16; ++i)
  {
    pvec[i] = static_cast<svtkm::Float32>(i) * 0.3f;
  }
  for (std::size_t i = 0; i < 9; ++i)
  {
    cvec[i] = static_cast<svtkm::Float32>(i) * 0.7f;
  }

  auto input = svtkm::cont::DataSetBuilderUniform::Create(
    svtkm::Id2(4, 4), svtkm::make_Vec(0.0f, 0.0f), svtkm::make_Vec(1.0f, 1.0f));
  svtkm::cont::DataSetFieldAdd::AddPointField(input, "pointdata", pvec);
  svtkm::cont::DataSetFieldAdd::AddCellField(input, "celldata", cvec);
  return input;
}
...
private:
  ...
  using HiddenArrayType = svtkm::cont::ArrayHandle<svtkm::UInt8>;
  ...
  static void ExplicitToUnifrom()
  {
    ...
    TestResultArray(svtkm::cont::Cast<FieldArrayType>(output.GetField("pointdata").GetData()),
                    GetExpectedPointData());
    TestResultArray(svtkm::cont::Cast<FieldArrayType>(output.GetField("celldata").GetData()),
                    GetExpectedCellData());
    TestResultArray(svtkm::cont::Cast<HiddenArrayType>(output.GetPointField("HIDDEN").GetData()),
                    GetExpectedHiddenPoints());
    TestResultArray(svtkm::cont::Cast<HiddenArrayType>(output.GetCellField("HIDDEN").GetData()),
