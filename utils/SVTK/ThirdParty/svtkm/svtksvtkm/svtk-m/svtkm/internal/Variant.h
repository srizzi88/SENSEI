...
#define svtk_m_internal_Variant_h

#include <svtkm/internal/VariantDetail.h>

#include <svtkm/Deprecated.h>
#include <svtkm/List.h>

#if defined(SVTKM_USING_GLIBCXX_4)
...
{
  static constexpr std::size_t alignment_value = svtkmstd::max_size<alignof(Types)...>::value;

  using type =
    svtkmstd::aligned_data_block<alignment_value, svtkmstd::max_size<Len, sizeof(Types)...>::value>;
};
...
#else // NOT SVTKM_USING_GLIBCXX_4
namespace svtkmstd
{
...
struct VariantCopyFunctor
{
  ...
  template <typename T>
  SVTKM_EXEC_CONT void operator()(const T& src, void* destPointer) const noexcept
  {
...
struct VariantDestroyFunctor
{
  SVTKM_SUPPRESS_EXEC_WARNINGS
  template <typename T>
  SVTKM_EXEC_CONT void operator()(T& src) const noexcept
  {
    src.~T();
  }
};
...
struct AllTriviallyCopyable<T0>
  : std::integral_constant<bool, (svtkmstd::is_trivially_copyable<T0>::value)>
{
};
...
struct AllTriviallyCopyable<T0, T1>
  : std::integral_constant<bool,
                           (svtkmstd::is_trivially_copyable<T0>::value &&
                            svtkmstd::is_trivially_copyable<T1>::value)>
{
};
...
struct AllTriviallyCopyable<T0, T1, T2>
  : std::integral_constant<bool,
                           (svtkmstd::is_trivially_copyable<T0>::value &&
                            svtkmstd::is_trivially_copyable<T1>::value &&
                            svtkmstd::is_trivially_copyable<T2>::value)>
{
};
...
struct AllTriviallyCopyable<T0, T1, T2, T3>
  : std::integral_constant<bool,
                           (svtkmstd::is_trivially_copyable<T0>::value &&
                            svtkmstd::is_trivially_copyable<T1>::value &&
                            svtkmstd::is_trivially_copyable<T2>::value &&
                            svtkmstd::is_trivially_copyable<T3>::value)>
{
};
...
struct AllTriviallyCopyable<T0, T1, T2, T3, T4, Ts...>
  : std::integral_constant<bool,
                           (svtkmstd::is_trivially_copyable<T0>::value &&
                            ...
                            svtkmstd::is_trivially_copyable<T2>::value &&
                            svtkmstd::is_trivially_copyable<T3>::value &&
                            svtkmstd::is_trivially_copyable<T4>::value &&
...
template <typename... Ts>
struct VariantTriviallyCopyable<svtkm::internal::Variant<Ts...>> : AllTriviallyCopyable<Ts...>
{
};
...
{
  typename svtkmstd::aligned_union<0, Ts...>::type Storage;

  svtkm::IdComponent Index = -1;

  template <svtkm::IdComponent Index>
  using TypeAt = typename svtkm::ListAt<svtkm::List<Ts...>, Index>;

  SVTKM_EXEC_CONT void* GetPointer() { return reinterpret_cast<void*>(&this->Storage); }
  SVTKM_EXEC_CONT const void* GetPointer() const
  {
    return reinterpret_cast<const void*>(&this->Storage);
  }

  SVTKM_EXEC_CONT svtkm::IdComponent GetIndex() const noexcept { return this->Index; }
  SVTKM_EXEC_CONT bool IsValid() const noexcept { return this->GetIndex() >= 0; }

  SVTKM_EXEC_CONT void Reset() noexcept
  {
    if (this->IsValid())
    {
  ...
  SVTKM_EXEC_CONT auto CastAndCall(Functor&& f, Args&&... args) const
  ...
  template <typename Functor, typename... Args>
  SVTKM_EXEC_CONT auto CastAndCall(Functor&& f, Args&&... args) noexcept(
...
template <typename... Ts>
struct VariantConstructorImpl<svtkm::internal::Variant<Ts...>, std::true_type>
...
template <typename... Ts>
struct VariantConstructorImpl<svtkm::internal::Variant<Ts...>, std::false_type>
  : VariantStorageImpl<Ts...>
{
  VariantConstructorImpl() = default;

  SVTKM_EXEC_CONT ~VariantConstructorImpl() { this->Reset(); }

  SVTKM_EXEC_CONT VariantConstructorImpl(const VariantConstructorImpl& src) noexcept
  {
    src.CastAndCall(VariantCopyFunctor{}, this->GetPointer());
    this->Index = src.Index;
  }

  SVTKM_EXEC_CONT VariantConstructorImpl(VariantConstructorImpl&& rhs) noexcept
  {
  ...
  SVTKM_EXEC_CONT VariantConstructorImpl& operator=(const VariantConstructorImpl& src) noexcept
  {
    this->Reset();
  ...
  SVTKM_EXEC_CONT VariantConstructorImpl& operator=(VariantConstructorImpl&& rhs) noexcept
  {
    this->Reset();
    this->Storage = std::move(rhs.Storage);
    this->Index = std::move(rhs.Index);
...
public:
  ...
  ///
  SVTKM_EXEC_CONT svtkm::IdComponent GetIndex() const noexcept
  {
  ...
  ///
  static constexpr svtkm::IdComponent NumberOfTypes = svtkm::IdComponent{ sizeof...(Ts) };
  ...
  template <typename T>
  SVTKM_EXEC_CONT Variant(const T& src) noexcept
  {
    constexpr svtkm::IdComponent index = GetIndexOf<T>();
    // Might be a way to use an enable_if to enforce a proper type.
    SVTKM_STATIC_ASSERT_MSG(index >= 0, "Attempting to put invalid type into a Variant");
  ...
  SVTKM_EXEC_CONT Variant(const T&& src) noexcept
  {
    constexpr svtkm::IdComponent index = IndexOf<T>::value;
    // Might be a way to use an enable_if to enforce a proper type.
    SVTKM_STATIC_ASSERT_MSG(index >= 0, "Attempting to put invalid type into a Variant");
  ...
  SVTKM_EXEC_CONT T& Emplace(Args&&... args)
  {
    constexpr svtkm::IdComponent I = GetIndexOf<T>();
    SVTKM_STATIC_ASSERT_MSG(I >= 0, "Variant::Emplace called with invalid type.");
    return this->EmplaceImpl<T, I>(std::forward<Args>(args)...);
  }

  template <typename T, typename U, typename... Args>
  SVTKM_EXEC_CONT T& Emplace(std::initializer_list<U> il, Args&&... args)
  {
    constexpr svtkm::IdComponent I = GetIndexOf<T>();
    SVTKM_STATIC_ASSERT_MSG(I >= 0, "Variant::Emplace called with invalid type.");
  ...
  template <svtkm::IdComponent I, typename... Args>
  SVTKM_EXEC_CONT TypeAt<I>& Emplace(Args&&... args)
  {
    SVTKM_STATIC_ASSERT_MSG((I >= 0) && (I < NumberOfTypes),
                           "Variant::Emplace called with invalid index");
    return this->EmplaceImpl<TypeAt<I>, I>(std::forward<Args>(args)...);
  }

  template <svtkm::IdComponent I, typename U, typename... Args>
  SVTKM_EXEC_CONT TypeAt<I>& Emplace(std::initializer_list<U> il, Args&&... args)
  {
    SVTKM_STATIC_ASSERT_MSG((I >= 0) && (I < NumberOfTypes),
                           "Variant::Emplace called with invalid index");
...
private:
  template <typename T, svtkm::IdComponent I, typename... Args>
  SVTKM_EXEC_CONT T& EmplaceImpl(Args&&... args)
  {
    this->Reset();
  ...
  template <typename T, svtkm::IdComponent I, typename U, typename... Args>
  SVTKM_EXEC_CONT T& EmplaceImpl(std::initializer_list<U> il, Args&&... args)
  {
    this->Reset();
...
public:
  ...
  ///
  template <svtkm::IdComponent I>
  SVTKM_EXEC_CONT TypeAt<I>& Get() noexcept
  {
    SVTKM_ASSERT(I == this->GetIndex());
    return *reinterpret_cast<TypeAt<I>*>(this->GetPointer());
  }

  template <svtkm::IdComponent I>
  SVTKM_EXEC_CONT const TypeAt<I>& Get() const noexcept
  {
    SVTKM_ASSERT(I == this->GetIndex());
  ...
  ///
  template <typename T>
  SVTKM_EXEC_CONT T& Get() noexcept
  {
    SVTKM_ASSERT(this->GetIndexOf<T>() == this->GetIndex());
    return *reinterpret_cast<T*>(this->GetPointer());
  }

  template <typename T>
  SVTKM_EXEC_CONT const T& Get() const noexcept
  {
    SVTKM_ASSERT(this->GetIndexOf<T>() == this->GetIndex());
  ...
  ///
  SVTKM_EXEC_CONT void Reset() noexcept { this->Superclass::Reset(); }
};
...
using ListTagAsVariant SVTKM_DEPRECATED(
  1.6,
  "svtkm::ListTag is no longer supported. Use svtkm::List instead.") =
  svtkm::ListApply<ListTag, svtkm::internal::Variant>;

/// \brief Convert a `List` to a `Variant`.
///
template <typename List>
using ListAsVariant = svtkm::ListApply<List, svtkm::internal::Variant>;
}
} // namespace svtkm::internal

#endif //svtk_m_internal_Variant_h
