...
//============================================================================
#ifndef svtk_m_io_reader_BOVDataSetReader_h
#define svtk_m_io_reader_BOVDataSetReader_h
...
private:
  ...
  void LoadFile()
  {
    ...
    if (stream.fail())
      throw svtkm::io::ErrorIO("Failed to open file: " + this->FileName);

    DataFormat dataFormat = ByteData;
    std::string bovFile, line, token, options, variableName;
    svtkm::Id numComponents = 1;
    svtkm::Id3 dim;
    svtkm::Vec3f origin(0, 0, 0);
    svtkm::Vec3f spacing(1, 1, 1);
    ...
    while (stream.good())
    {
      ...
      if (pos == std::string::npos)
        throw svtkm::io::ErrorIO("Unsupported option: " + line);
      ...
      else if (token.find("DATA") != std::string::npos && token.find("FORMAT") != std::string::npos)
      {
        ...
        if (opt.find("FLOAT") != std::string::npos || opt.find("REAL") != std::string::npos)
          ...
          throw svtkm::io::ErrorIO("Unsupported data type: " + token);
      }
               ...
               token.find("COMPONENTS") != std::string::npos)
      {
        ...
        if (numComponents != 1 && numComponents != 3)
          throw svtkm::io::ErrorIO("Unsupported number of components");
      }
...
*/
    ...
    if (spacingSet)
    {
      spacing[0] = (spacing[0]) / static_cast<svtkm::FloatDefault>(dim[0] - 1);
      spacing[1] = (spacing[1]) / static_cast<svtkm::FloatDefault>(dim[1] - 1);
      spacing[2] = (spacing[2]) / static_cast<svtkm::FloatDefault>(dim[2] - 1);
    }
...
*/
    ...
    else if (numComponents == 3)
    {
      if (dataFormat == FloatData)
      {
        svtkm::cont::ArrayHandle<svtkm::Vec3f_32> var;
        ReadVector(fullPathDataFile, numTuples, var);
        dsf.AddPointField(this->DataSet, variableName, var);
      }
      else if (dataFormat == DoubleData)
      {
        svtkm::cont::ArrayHandle<svtkm::Vec3f_64> var;
        ReadVector(fullPathDataFile, numTuples, var);
        dsf.AddPointField(this->DataSet, variableName, var);
      }
    }
  ...
  template <typename T>
  void ReadBuffer(const std::string& fName, const svtkm::Id& sz, std::vector<T>& buff)
  {
    FILE* fp = fopen(fName.c_str(), "rb");
    size_t readSize = static_cast<size_t>(sz);
    if (fp == nullptr)
      throw svtkm::io::ErrorIO("Unable to open data file: " + fName);
    buff.resize(readSize);
    size_t nread = fread(&buff[0], sizeof(T), readSize, fp);
    if (nread != readSize)
      throw svtkm::io::ErrorIO("Data file read failed: " + fName);
    fclose(fp);
  }

  template <typename T>
  void ReadScalar(const std::string& fName,
                  const svtkm::Id& nTuples,
                  svtkm::cont::ArrayHandle<T>& var)
  {
    std::vector<T> buff;
    ReadBuffer(fName, nTuples, buff);
    var.Allocate(nTuples);
    for (svtkm::Id i = 0; i < nTuples; i++)
  ...
  void ReadVector(const std::string& fName,
                  const svtkm::Id& nTuples,
                  svtkm::cont::ArrayHandle<svtkm::Vec<T, 3>>& var)
  {
    std::vector<T> buff;
...
#endif // svtk_m_io_reader_BOVReader_h
