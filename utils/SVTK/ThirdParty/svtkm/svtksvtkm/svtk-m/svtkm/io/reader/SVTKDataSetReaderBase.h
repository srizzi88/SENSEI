...
//============================================================================
#ifndef svtk_m_io_reader_SVTKDataSetReaderBase_h
#define svtk_m_io_reader_SVTKDataSetReaderBase_h

#include <svtkm/io/internal/Endian.h>
#include <svtkm/io/internal/SVTKDataSetCells.h>
#include <svtkm/io/internal/SVTKDataSetStructures.h>
...
struct SVTKDataSetFile
{
  std::string FileName;
  svtkm::Id2 Version;
  std::string Title;
  bool IsBinary;
  svtkm::io::internal::DataSetStructure Structure;
  std::ifstream Stream;
};

inline void PrintSVTKDataFileSummary(const SVTKDataSetFile& df, std::ostream& out)
{
  ...
  out << "\tDataSet type: " << svtkm::io::internal::DataSetStructureString(df.Structure)
      << std::endl;
}
...
inline void parseAssert(bool condition)
{
  if (!condition)
  {
    throw svtkm::io::ErrorIO("Parse Error");
  }
}
...
template <>
struct StreamIOType<svtkm::Int8>
{
  using Type = svtkm::Int16;
};
template <>
struct StreamIOType<svtkm::UInt8>
{
  using Type = svtkm::UInt16;
};
...
template <>
struct ClosestCommonType<svtkm::Int8>
{
  using Type = svtkm::Int32;
};
template <>
struct ClosestCommonType<svtkm::UInt8>
{
  using Type = svtkm::Int32;
};
template <>
struct ClosestCommonType<svtkm::Int16>
{
  using Type = svtkm::Int32;
};
template <>
struct ClosestCommonType<svtkm::UInt16>
{
  using Type = svtkm::Int32;
};
template <>
struct ClosestCommonType<svtkm::UInt32>
{
  using Type = svtkm::Int64;
};
template <>
struct ClosestCommonType<svtkm::UInt64>
{
  using Type = svtkm::Int64;
};
...
template <>
struct ClosestFloat<svtkm::Int8>
{
  using Type = svtkm::Float32;
};
template <>
struct ClosestFloat<svtkm::UInt8>
{
  using Type = svtkm::Float32;
};
template <>
struct ClosestFloat<svtkm::Int16>
{
  using Type = svtkm::Float32;
};
template <>
struct ClosestFloat<svtkm::UInt16>
{
  using Type = svtkm::Float32;
};
template <>
struct ClosestFloat<svtkm::Int32>
{
  using Type = svtkm::Float64;
};
template <>
struct ClosestFloat<svtkm::UInt32>
{
  using Type = svtkm::Float64;
};
template <>
struct ClosestFloat<svtkm::Int64>
{
  using Type = svtkm::Float64;
};
template <>
struct ClosestFloat<svtkm::UInt64>
{
  using Type = svtkm::Float64;
};

template <typename T>
svtkm::cont::VariantArrayHandle CreateVariantArrayHandle(const std::vector<T>& vec)
{
  switch (svtkm::VecTraits<T>::NUM_COMPONENTS)
  {
    case 1:
    {
      ...
      if (not_same)
      {
        SVTKM_LOG_S(svtkm::cont::LogLevel::Info,
                   "Type " << svtkm::io::internal::DataTypeName<T>::Name()
                           << " is currently unsupported. Converting to "
                           << svtkm::io::internal::DataTypeName<CommonType>::Name()
                           << ".");
      }

      svtkm::cont::ArrayHandle<CommonType> output;
      output.Allocate(static_cast<svtkm::Id>(vec.size()));
      auto portal = output.GetPortalControl();
      for (svtkm::Id i = 0; i < output.GetNumberOfValues(); ++i)
      {
        portal.Set(i, static_cast<CommonType>(vec[static_cast<std::size_t>(i)]));
      }

      return svtkm::cont::VariantArrayHandle(output);
    }
    case 2:
    case 3:
    case 9:
    {
      constexpr auto numComps = svtkm::VecTraits<T>::NUM_COMPONENTS;

      using InComponentType = typename svtkm::VecTraits<T>::ComponentType;
      using OutComponentType = typename ClosestFloat<InComponentType>::Type;
      using CommonType = svtkm::Vec<OutComponentType, numComps>;
      constexpr bool not_same = !std::is_same<T, CommonType>::value;
      if (not_same)
      {
        SVTKM_LOG_S(svtkm::cont::LogLevel::Info,
                   "Type " << svtkm::io::internal::DataTypeName<InComponentType>::Name() << "["
                           << svtkm::VecTraits<T>::GetNumberOfComponents(T())
                           << "] "
                           << "is currently unsupported. Converting to "
                           << svtkm::io::internal::DataTypeName<OutComponentType>::Name()
                           << "["
                           << numComps
                           << "].");
      }
      ...
      for (svtkm::Id i = 0; i < output.GetNumberOfValues(); ++i)
      {
        ...
        for (svtkm::IdComponent j = 0; j < numComps; ++j)
        {
          outval[j] = static_cast<OutComponentType>(
            svtkm::VecTraits<T>::GetComponent(vec[static_cast<std::size_t>(i)], j));
        }
        portal.Set(i, outval);
      }
    ...
    default:
    {
      SVTKM_LOG_S(svtkm::cont::LogLevel::Warn, "Only 1, 2, 3, or 9 components supported. Skipping.");
      return svtkm::cont::VariantArrayHandle(svtkm::cont::ArrayHandle<svtkm::Float32>());
    }
  }
}

inline svtkm::cont::DynamicCellSet CreateCellSetStructured(const svtkm::Id3& dim)
{
  if (dim[0] > 1 && dim[1] > 1 && dim[2] > 1)
  {
    svtkm::cont::CellSetStructured<3> cs;
    cs.SetPointDimensions(svtkm::make_Vec(dim[0], dim[1], dim[2]));
    return cs;
  }
  else if (dim[0] > 1 && dim[1] > 1 && dim[2] <= 1)
  {
    svtkm::cont::CellSetStructured<2> cs;
    cs.SetPointDimensions(svtkm::make_Vec(dim[0], dim[1]));
    return cs;
  }
  else if (dim[0] > 1 && dim[1] <= 1 && dim[2] <= 1)
  {
    svtkm::cont::CellSetStructured<1> cs;
...
class SVTKDataSetReaderBase
{
public:
  explicit SVTKDataSetReaderBase(const char* fileName)
    : DataFile(new internal::SVTKDataSetFile)
    , DataSet()
    , Loaded(false)
  {
  ...
  explicit SVTKDataSetReaderBase(const std::string& fileName)
    : DataFile(new internal::SVTKDataSetFile)
    , DataSet()
    , Loaded(false)
  {
  ...
  virtual ~SVTKDataSetReaderBase() {}

  const svtkm::cont::DataSet& ReadDataSet()
  {
    if (!this->Loaded)
    {
      try
      ...
      catch (std::ifstream::failure& e)
      {
        ...
        throw svtkm::io::ErrorIO(message + e.what());
      }
    }
  ...
  const svtkm::cont::DataSet& GetDataSet() const { return this->DataSet; }

  virtual void PrintSummary(std::ostream& out) const
  {
    out << "SVTKDataSetReader" << std::endl;
    PrintSVTKDataFileSummary(*this->DataFile.get(), out);
...
protected:
  void ReadPoints()
  {
    ...
    svtkm::cont::VariantArrayHandle points =
      this->DoReadArrayVariant(svtkm::cont::Field::Association::POINTS, dataType, numPoints, 3);

    this->DataSet.AddCoordinateSystem(svtkm::cont::CoordinateSystem("coordinates", points));
  }

  void ReadCells(svtkm::cont::ArrayHandle<svtkm::Id>& connectivity,
                 svtkm::cont::ArrayHandle<svtkm::IdComponent>& numIndices)
  {
    svtkm::Id numCells, numInts;
    this->DataFile->Stream >> numCells >> numInts >> std::ws;

    connectivity.Allocate(numInts - numCells);
    numIndices.Allocate(numCells);

    std::vector<svtkm::Int32> buffer(static_cast<std::size_t>(numInts));
    this->ReadArray(buffer);

    svtkm::Int32* buffp = &buffer[0];
    auto connectivityPortal = connectivity.GetPortalControl();
    auto numIndicesPortal = numIndices.GetPortalControl();
    for (svtkm::Id i = 0, connInd = 0; i < numCells; ++i)
    {
      svtkm::IdComponent numInds = static_cast<svtkm::IdComponent>(*buffp++);
      numIndicesPortal.Set(i, numInds);
      for (svtkm::IdComponent j = 0; j < numInds; ++j, ++connInd)
      {
        connectivityPortal.Set(connInd, static_cast<svtkm::Id>(*buffp++));
      }
    }
  }

  void ReadShapes(svtkm::cont::ArrayHandle<svtkm::UInt8>& shapes)
  {
    std::string tag;
    svtkm::Id numCells;
    ...
    std::vector<svtkm::Int32> buffer(static_cast<std::size_t>(numCells));
    this->ReadArray(buffer);
    ...
    {
      shapesPortal.Set(i, static_cast<svtkm::UInt8>(*buffp++));
    }
  }
  ...
  void ReadAttributes()
  {
    ...
    while (!this->DataFile->Stream.eof())
    {
      if (tag == "POINT_DATA")
      {
        association = svtkm::cont::Field::Association::POINTS;
      }
      else if (tag == "CELL_DATA")
      {
        association = svtkm::cont::Field::Association::CELL_SET;
      }
      else
      {
  ...
  void SetCellsPermutation(const svtkm::cont::ArrayHandle<svtkm::Id>& permutation)
  {
    this->CellsPermutation = permutation;
  }

  svtkm::cont::ArrayHandle<svtkm::Id> GetCellsPermutation() const { return this->CellsPermutation; }

  void TransferDataFile(SVTKDataSetReaderBase& reader)
  {
...
private:
  void OpenFile()
  {
    ...
    catch (std::ifstream::failure&)
    {
      ...
      throw svtkm::io::ErrorIO(message);
    }
  }

  void ReadHeader()
  {
    char vstring[] = "# svtk DataFile Version";
    ...
    {
      throw svtkm::io::ErrorIO("Incorrect file format.");
    }
    ...
    {
      SVTKM_LOG_S(svtkm::cont::LogLevel::Warn,
    ...
    {
      throw svtkm::io::ErrorIO("Unsupported Format.");
    }
    ...
    this->DataFile->Structure = svtkm::io::internal::DataSetStructureId(structStr);
    if (this->DataFile->Structure == svtkm::io::internal::DATASET_UNKNOWN)
    {
                ...
                svtkm::cont::VariantArrayHandle& data)
  {
    if (data.GetNumberOfValues() > 0)
    {
      switch (association)
      {
        ...
        case svtkm::cont::Field::Association::WHOLE_MESH:
          this->DataSet.AddField(svtkm::cont::Field(name, association, data));
          break;
  ...
  void ReadScalars(svtkm::cont::Field::Association association, std::size_t numElements)
  {
    std::string dataName, dataType, lookupTableName;
    svtkm::IdComponent numComponents = 1;
  ...
  void ReadColorScalars(svtkm::cont::Field::Association association, std::size_t numElements)
  {
    SVTKM_LOG_S(svtkm::cont::LogLevel::Warn,
  ...
  void ReadLookupTable()
  {
    ...
    this->SkipArray(numEntries, svtkm::Vec<svtkm::io::internal::ColorChannel8, 4>());
  }

  void ReadTextureCoordinates(svtkm::cont::Field::Association association, std::size_t numElements)
  {
    std::string dataName;
    svtkm::IdComponent numComponents;
    std::string dataType;
  ...
  void ReadVectors(svtkm::cont::Field::Association association, std::size_t numElements)
  {
    std::string dataName;
    std::string dataType;
  ...
  void ReadTensors(svtkm::cont::Field::Association association, std::size_t numElements)
  {
    std::string dataName;
    std::string dataType;
    this->DataFile->Stream >> dataName >> dataType >> std::ws;
  ...
  void ReadFields(svtkm::cont::Field::Association association, std::size_t expectedNumElements)
  {
    std::string dataName;
    svtkm::Id numArrays;
    this->DataFile->Stream >> dataName >> numArrays >> std::ws;
    for (svtkm::Id i = 0; i < numArrays; ++i)
    {
...
protected:
  void ReadGlobalFields(std::vector<svtkm::Float32>* visitBounds = nullptr)
  {
    std::string dataName;
    svtkm::Id numArrays;
    this->DataFile->Stream >> dataName >> numArrays >> std::ws;
...
private:
  ...
  {
  public:
    SkipArrayVariant(SVTKDataSetReaderBase* reader, std::size_t numElements)
      : Reader(reader)
    ...
    template <typename T>
    void operator()(svtkm::IdComponent numComponents, T) const
    {
      this->Reader->SkipArray(this->NumElements * static_cast<std::size_t>(numComponents), T());
    }

  protected:
    SVTKDataSetReaderBase* Reader;
  ...
  {
  public:
    ReadArrayVariant(SVTKDataSetReaderBase* reader,
                     svtkm::cont::Field::Association association,
                     std::size_t numElements,
                     svtkm::cont::VariantArrayHandle& data)
    ...
    void operator()(T) const
    {
      ...
      if ((this->Association != svtkm::cont::Field::Association::CELL_SET) ||
      ...
      else
      {
        ...
        // data due to differences between SVTK and SVTK-m cell shapes.
        auto permutation = this->Reader->GetCellsPermutation().GetPortalConstControl();
        svtkm::Id outSize = permutation.GetNumberOfValues();
        std::vector<T> permutedBuffer(static_cast<std::size_t>(outSize));
        for (svtkm::Id outIndex = 0; outIndex < outSize; outIndex++)
        {
...
protected:
                          ...
                          svtkm::IdComponent numComponents)
  {
    ...
    if (dataType == "string")
    {
      const svtkm::Id stringCount = numComponents * static_cast<svtkm::Id>(numElements);
      for (svtkm::Id i = 0; i < stringCount; ++i)
      {
        std::string trash;
    ...
    {
      svtkm::io::internal::DataType typeId = svtkm::io::internal::DataTypeId(dataType);
      svtkm::io::internal::SelectTypeAndCall(
        typeId, numComponents, SkipArrayVariant(this, numElements));
    }
  }

  svtkm::cont::VariantArrayHandle DoReadArrayVariant(svtkm::cont::Field::Association association,
                                                    std::string dataType,
                                                    std::size_t numElements,
                                                    svtkm::IdComponent numComponents)
  {
    // Create empty data to start so that the return can check if data were actually read
    svtkm::cont::ArrayHandle<svtkm::Float32> empty;
    svtkm::cont::VariantArrayHandle data(empty);
  ...
  void ReadArray(std::vector<T>& buffer)
  {
    ...
    constexpr svtkm::IdComponent numComponents = svtkm::VecTraits<T>::NUM_COMPONENTS;
    ...
    if (this->DataFile->IsBinary)
    {
      ...
      if (svtkm::io::internal::IsLittleEndian())
      {
        svtkm::io::internal::FlipEndianness(buffer);
      }
    }
    else
    {
      ...
      {
        for (svtkm::IdComponent j = 0; j < numComponents; ++j)
        {
          typename internal::StreamIOType<ComponentType>::Type val;
          this->DataFile->Stream >> val;
          svtkm::VecTraits<T>::SetComponent(buffer[i], j, static_cast<ComponentType>(val));
        }
      }
    }
  ...
  void ReadArray(std::vector<svtkm::Vec<svtkm::io::internal::DummyBitType, NumComponents>>& buffer)
  {
    SVTKM_LOG_S(svtkm::cont::LogLevel::Warn,
               "Support for data type 'bit' is not implemented. Skipping.");
    this->SkipArray(buffer.size(), svtkm::Vec<svtkm::io::internal::DummyBitType, NumComponents>());
    buffer.clear();
  }

  void ReadArray(std::vector<svtkm::io::internal::DummyBitType>& buffer)
  {
    SVTKM_LOG_S(svtkm::cont::LogLevel::Warn,
               "Support for data type 'bit' is not implemented. Skipping.");
    this->SkipArray(buffer.size(), svtkm::io::internal::DummyBitType());
    buffer.clear();
  }
  ...
  void SkipArray(std::size_t numElements,
                 svtkm::Vec<svtkm::io::internal::DummyBitType, NumComponents>)
  {
    this->SkipArray(numElements * static_cast<std::size_t>(NumComponents),
                    svtkm::io::internal::DummyBitType(),
                    NumComponents);
  }
  ...
  void SkipArrayMetaData(svtkm::IdComponent numComponents)
  {
    ...
    SVTKM_LOG_S(svtkm::cont::LogLevel::Warn, "METADATA is not supported. Attempting to Skip.");
...
protected:
  std::unique_ptr<internal::SVTKDataSetFile> DataFile;
  svtkm::cont::DataSet DataSet;

private:
  bool Loaded;
  svtkm::cont::ArrayHandle<svtkm::Id> CellsPermutation;

  friend class SVTKDataSetReader;
};
...
SVTKM_BASIC_TYPE_VECTOR(svtkm::io::internal::ColorChannel8)
SVTKM_BASIC_TYPE_VECTOR(svtkm::io::internal::DummyBitType)

#endif // svtk_m_io_reader_SVTKDataSetReaderBase_h
