...
void AxisAnnotation3D::SetLabelFontScale(Float64 s)
{
  ...
  {
    this->Labels[i]->SetScale(svtkm::Float32(s));
  }
}
                              ...
                              Canvas& canvas)
{
  ...
  while (this->Labels.size() < nmajor)
  {
    this->Labels.push_back(
      std::unique_ptr<TextAnnotationBillboard>(new svtkm::rendering::TextAnnotationBillboard(
        "test", this->Color, svtkm::Float32(this->FontScale), svtkm::Vec3f_32(0, 0, 0), 0)));
  }

  std::stringstream numberToString;
  for (unsigned int i = 0; i < nmajor; ++i)
  {
    svtkm::Vec3f_64 tickPos = proportions[i] * (this->Point1 - this->Point0) + this->Point0;
    for (int pass = 0; pass <= 1; pass++)
    {
      svtkm::Vec3f_64 tickSize(0);
      if (pass == 0)
      {
        switch (this->Axis)
        {
      ...
      svtkm::Vec3f_64 start = tickPos - tickSize * this->TickMajorOffset;
      svtkm::Vec3f_64 end = tickPos - tickSize * (1.0 - this->TickMajorOffset);

      worldAnnotator.AddLine(start, end, this->LineWidth, this->Color, infront);
    }

    svtkm::Vec3f_32 tickSize(0);
    svtkm::Float32 s = 0.4f * this->FontOffset;
    switch (this->Axis)
    {
      case 0:
        tickSize[1] = s;
    ...
    this->Labels[i]->SetPosition(svtkm::Float32(tickPos[0] - tickSize[0]),
                                 svtkm::Float32(tickPos[1] - tickSize[1]),
                                 svtkm::Float32(tickPos[2] - tickSize[2]));
    svtkm::Vec3f_32 pp(svtkm::Float32(tickPos[0] - tickSize[0]),
                      svtkm::Float32(tickPos[1] - tickSize[1]),
                      svtkm::Float32(tickPos[2] - tickSize[2]));
  ...
  for (unsigned int i = 0; i < nminor; ++i)
  {
    ...
    for (int pass = 0; pass <= 1; pass++)
    {
      ...
      tickSize = tickSize * this->Invert;
      svtkm::Vec3f_64 start = tickPos - tickSize * this->TickMinorOffset;
      svtkm::Vec3f_64 end = tickPos - tickSize * (1.0 - this->TickMinorOffset);
