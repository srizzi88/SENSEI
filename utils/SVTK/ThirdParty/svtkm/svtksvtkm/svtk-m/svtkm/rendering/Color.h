...
//============================================================================
#ifndef svtk_m_rendering_Color_h
#define svtk_m_rendering_Color_h
...
public:
  ...
  SVTKM_EXEC_CONT
  Color(svtkm::Float32 r_, svtkm::Float32 g_, svtkm::Float32 b_, svtkm::Float32 a_ = 1.f)
    : Components(r_, g_, b_, a_)
  {
  }

  SVTKM_EXEC_CONT
  Color(const svtkm::Vec4f_32& components)
    : Components(components)
  {
  }

  SVTKM_EXEC_CONT
  void SetComponentFromByte(svtkm::Int32 i, svtkm::UInt8 v)
  {
    ...
    Components[i] = static_cast<svtkm::Float32>(v) / 255.f;
    // clamp?
    if (Components[i] < 0)
      Components[i] = 0;
    if (Components[i] > 1)
  ...
  SVTKM_EXEC_CONT
  svtkm::UInt8 GetComponentAsByte(int i)
  {
    ...
    int tv = svtkm::Int32(Components[i] * 256.f);
    // Converting even from valid values (i.e 1.0)
    // can give a result outside the range (i.e. 256),
    // but we have to clamp anyway.
    return svtkm::UInt8((tv < 0) ? 0 : (tv > 255) ? 255 : tv);
  }

  SVTKM_EXEC_CONT
  void GetRGBA(svtkm::UInt8& r, svtkm::UInt8& g, svtkm::UInt8& b, svtkm::UInt8& a)
  {
  ...
  SVTKM_EXEC_CONT
  svtkm::Float64 RawBrightness() { return (Components[0] + Components[1] + Components[2]) / 3.; }
  ...
  static SVTKM_RENDERING_EXPORT Color white, black;
  static SVTKM_RENDERING_EXPORT Color red, green, blue;
  static SVTKM_RENDERING_EXPORT Color cyan, magenta, yellow;
  static SVTKM_RENDERING_EXPORT Color gray10, gray20, gray30, gray40, gray50, gray60, gray70, gray80,
    gray90;
};
}
} //namespace svtkm::rendering
#endif //svtk_m_rendering_Color_h
