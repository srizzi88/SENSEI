...
//============================================================================
#ifndef svtk_m_rendering_raytracing_Cell_Intersector_h
#define svtk_m_rendering_raytracing_Cell_Intersector_h
...
//
template <typename T>
SVTKM_EXEC_CONT inline void IntersectZoo(T xpoints[8],
                                        T ypoints[8],
                                        T zpoints[8],
                                        const svtkm::Vec<T, 3>& dir,
                                        const svtkm::Vec<T, 3>& origin,
                                        T distances[6],
                                        const svtkm::Int32& shapeType)
{
  ...
  CellTables tables;
  const svtkm::Int32 tableOffset = tables.ZooLookUp(tables.CellTypeLookUp(shapeType), 0);
  const svtkm::Int32 numTriangles = tables.ZooLookUp(tables.CellTypeLookUp(shapeType), 1);
  // Decompose each face into two triangles
  for (int i = 0; i < 6; ++i)
    distances[i] = -1.;
  for (int i = 0; i < numTriangles; ++i)
  {
    const svtkm::Int32 offset = tableOffset + i;
    svtkm::Vec<T, 3> a, c, b;
    ...
    const svtkm::Int32 faceId = tables.ZooTable(offset, 0);
    T distance = -1.f;
    ...
    if (distance != -1.f)
    {
      if (distances[faceId] != -1.f)
        distances[faceId] = svtkm::Min(distance, distances[faceId]);
      else
...
template <typename T>
SVTKM_EXEC_CONT inline void IntersectHex(T xpoints[8],
                                        T ypoints[8],
                                        T zpoints[8],
                                        const svtkm::Vec<T, 3>& dir,
                                        const svtkm::Vec<T, 3>& origin,
                                        T distances[6])
{
  ...
  for (int i = 0; i < 6; ++i)
  {
    ...
    {
      if (distances[i] != -1.f)
        distances[i] = svtkm::Min(distance, distances[i]);
      else
        distances[i] = distance;
    }
  }
...
template <typename T>
SVTKM_EXEC_CONT inline void IntersectTet(T xpoints[8],
                                        T ypoints[8],
                                        T zpoints[8],
                                        const svtkm::Vec<T, 3>& dir,
                                        const svtkm::Vec<T, 3>& origin,
                                        T distances[6])
{
  ...
  CellTables tables;
  const svtkm::Int32 tableOffset = tables.FaceLookUp(tables.CellTypeLookUp(CELL_SHAPE_TETRA), 0);
...
//
template <typename T>
SVTKM_EXEC_CONT inline void IntersectWedge(T xpoints[8],
                                          T ypoints[8],
                                          T zpoints[8],
                                          const svtkm::Vec<T, 3>& dir,
                                          const svtkm::Vec<T, 3>& origin,
                                          T distances[6])
{
  ...
  CellTables tables;
  const svtkm::Int32 tableOffset = tables.FaceLookUp(tables.CellTypeLookUp(CELL_SHAPE_WEDGE), 0);
  // Decompose each face into two triangles
  for (int i = 0; i < 5; ++i)
  {
    svtkm::Vec<T, 3> a, c, b, d;
...
public:
  template <typename T>
  SVTKM_EXEC_CONT inline void IntersectCell(T* svtkmNotUsed(xpoints),
                                           T* svtkmNotUsed(ypoints),
                                           T* svtkmNotUsed(zpoints),
                                           const svtkm::Vec<T, 3>& svtkmNotUsed(dir),
                                           const svtkm::Vec<T, 3>& svtkmNotUsed(origin),
                                           T* svtkmNotUsed(distances),
                                           const svtkm::UInt8 svtkmNotUsed(cellShape = 12));
};
...
public:
  template <typename T>
  SVTKM_EXEC_CONT inline void IntersectCell(T* xpoints,
                                           T* ypoints,
                                           T* zpoints,
                                           const svtkm::Vec<T, 3>& dir,
                                           const svtkm::Vec<T, 3>& origin,
                                           T* distances,
                                           const svtkm::UInt8 cellShape = 12) const
  {
    if (cellShape == 12)
    {
...
public:
  ...
  SVTKM_EXEC_CONT inline void IntersectCell(T* xpoints,
                                           ...
                                           const svtkm::Vec<T, 3>& origin,
                                           T distances[6],
                                           const svtkm::UInt8 cellShape = 0) const
  {
    IntersectZoo(xpoints, ypoints, zpoints, dir, origin, distances, cellShape);
  }
};
