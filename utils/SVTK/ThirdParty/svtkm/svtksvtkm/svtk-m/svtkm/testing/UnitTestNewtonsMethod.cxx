...
{
  using Vector3 = svtkm::Vec<T, 3>;
  using Matrix3x3 = svtkm::Matrix<T, 3, 3>;
  ...
  for (initialGuess[0] = 0.25f; initialGuess[0] <= 1; initialGuess[0] += 0.25f)
  {
    for (initialGuess[1] = 0.25f; initialGuess[1] <= 1; initialGuess[1] += 0.25f)
    {
      for (initialGuess[2] = 0.25f; initialGuess[2] <= 1; initialGuess[2] += 0.25f)
      {
        ...
        auto result = svtkm::NewtonsMethod(
          EvaluateJacobian<T>(), EvaluateFunctions<T>(), desiredOutput, initialGuess, T(1e-6));

        SVTKM_TEST_ASSERT(test_equal(result.Solution, expected1) ||
...
void TestNewtonsMethod()
{
  ...
  TestNewtonsMethodTemplate<svtkm::Float32>();
  std::cout << "*** Float64 *************************" << std::endl;
  TestNewtonsMethodTemplate<svtkm::Float64>();
}
...
{
  return svtkm::testing::Testing::Run(TestNewtonsMethod, argc, argv);
}
