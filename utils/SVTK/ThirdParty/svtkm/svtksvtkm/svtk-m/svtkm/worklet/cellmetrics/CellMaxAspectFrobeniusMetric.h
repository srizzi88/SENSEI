...
//  this software.
//============================================================================
#ifndef svtk_m_worklet_cellmetrics_CellMaxAspectFrobeniusMetric_h
#define svtk_m_worklet_cellmetrics_CellMaxAspectFrobeniusMetric_h
...
* SVTK implementation, but are provided here.
...
#include "svtkm/worklet/cellmetrics/CellAspectFrobeniusMetric.h"
...
SVTKM_EXEC OutType ComputeTetCondition(const VecType edges[])
{
  ...
  OutType det = (OutType)svtkm::Dot(edges[0], svtkm::Cross(edges[1], edges[2]));

  if (det <= svtkm::NegativeInfinity<OutType>())
    return svtkm::Infinity<OutType>();

  OutType term1 =
    svtkm::Dot(edges[0], edges[0]) + svtkm::Dot(edges[1], edges[1]) + svtkm::Dot(edges[2], edges[2]);

  VecType crosses[3] = { svtkm::Cross(edges[0], edges[1]),
                         svtkm::Cross(edges[1], edges[2]),
                         svtkm::Cross(edges[2], edges[0]) };

  OutType term2 = svtkm::Dot(crosses[0], crosses[0]) + svtkm::Dot(crosses[1], crosses[1]) +
    svtkm::Dot(crosses[2], crosses[2]);

  return svtkm::Sqrt(term1 * term2) / det;
}
...
SVTKM_EXEC OutType CellMaxAspectFrobeniusMetric(const svtkm::IdComponent& numPts,
...
{
  if (numPts == 3)
    return svtkm::worklet::cellmetrics::CellAspectFrobeniusMetric<OutType>(
      numPts, pts, svtkm::CellShapeTagTriangle(), worklet);
  if (numPts == 4)
    return CellMaxAspectFrobeniusMetric<OutType>(numPts, pts, svtkm::CellShapeTagQuad(), worklet);
  else
  {
    worklet.RaiseError(
                                               ...
                                               const svtkm::exec::FunctorBase& worklet)
{
  ...
  FloatType a2 = svtkm::MagnitudeSquared(QuadEdges[0]);
  FloatType b2 = svtkm::MagnitudeSquared(QuadEdges[1]);
  FloatType c2 = svtkm::MagnitudeSquared(QuadEdges[2]);
  FloatType d2 = svtkm::MagnitudeSquared(QuadEdges[3]);
  ...
  FloatType ab = svtkm::Magnitude(svtkm::Cross(QuadEdges[0], QuadEdges[1]));
  FloatType bc = svtkm::Magnitude(svtkm::Cross(QuadEdges[1], QuadEdges[2]));
  FloatType cd = svtkm::Magnitude(svtkm::Cross(QuadEdges[2], QuadEdges[3]));
  FloatType da = svtkm::Magnitude(svtkm::Cross(QuadEdges[3], QuadEdges[0]));

  if (ab < svtkm::NegativeInfinity<OutType>() || bc < svtkm::NegativeInfinity<OutType>() ||
      cd < svtkm::NegativeInfinity<OutType>() || da < svtkm::NegativeInfinity<OutType>())
    return svtkm::Infinity<OutType>();
  ...
  if (max_aspect_frobenius > 0)
    svtkm::Min(max_aspect_frobenius, svtkm::Infinity<OutType>());

  return svtkm::Max(max_aspect_frobenius, svtkm::NegativeInfinity<OutType>());
}
                                               ...
                                               const svtkm::exec::FunctorBase& worklet)
{
  ...
  return svtkm::worklet::cellmetrics::CellAspectFrobeniusMetric<OutType, PointCoordVecType>(
                                               ...
                                               const svtkm::exec::FunctorBase& worklet)
{
  ...
  if (max_aspect_frobenius > 0)
    return svtkm::Min(max_aspect_frobenius, svtkm::Infinity<OutType>());

  return svtkm::Max(max_aspect_frobenius, OutType(0.0));
}
                                               ...
                                               const svtkm::exec::FunctorBase& worklet)
{
  ...
  const svtkm::Vec<Vector, 4> tetras[6] = {
    svtkm::Vec<Vector, 4>(pts[0], pts[1], pts[2], pts[3]), //tet 0
    svtkm::Vec<Vector, 4>(pts[1], pts[2], pts[0], pts[4]), //tet 1
    svtkm::Vec<Vector, 4>(pts[2], pts[0], pts[1], pts[5]), //tet 2
    svtkm::Vec<Vector, 4>(pts[3], pts[5], pts[4], pts[0]), //tet 3
    svtkm::Vec<Vector, 4>(pts[4], pts[3], pts[5], pts[1]), //tet 4
    svtkm::Vec<Vector, 4>(pts[5], pts[4], pts[3], pts[2])  //tet 5
  };
  ...
  const svtkm::IdComponent tetPts = 4;
  OutType max_aspect_frobenius = svtkm::worklet::cellmetrics::CellAspectFrobeniusMetric<OutType>(
    tetPts, tetras[0], svtkm::CellShapeTagTetra(), worklet);
  OutType curr;
  for (svtkm::Id i = 1; i < 6; i++)
  {
    curr = svtkm::worklet::cellmetrics::CellAspectFrobeniusMetric<OutType>(
      tetPts, tetras[i], svtkm::CellShapeTagTetra(), worklet);
    if (curr > max_aspect_frobenius)
      max_aspect_frobenius = curr;
  }
...
#endif // svtk_m_worklet_cellmetrics_CellMaxAspectFrobeniusMetric_h
