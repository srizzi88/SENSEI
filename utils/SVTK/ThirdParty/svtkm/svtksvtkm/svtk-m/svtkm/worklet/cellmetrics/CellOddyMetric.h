...
//  this software.
//============================================================================
#ifndef svtk_m_worklet_cellmetrics_Oddy_h
#define svtk_m_worklet_cellmetrics_Oddy_h
...
SVTKM_EXEC OutType CellOddyMetric(const svtkm::IdComponent& numPts,
...
SVTKM_EXEC Scalar GetQuadOddyQi(const Vector& Li, const Vector& LiPlus1, const Vector& NiPlus1)
{
  const Scalar two(2.0);
  const Scalar four(4.0);
  const Scalar liMagnitudeSquared = svtkm::MagnitudeSquared(Li);
  const Scalar liPlus1MagnitudeSquared = svtkm::MagnitudeSquared(LiPlus1);
  const Scalar niPlus1MagnitudeSquared = svtkm::MagnitudeSquared(NiPlus1);
  const Scalar q = (((liMagnitudeSquared - liPlus1MagnitudeSquared) *
                     (liMagnitudeSquared - liPlus1MagnitudeSquared)) +
                    (four * (svtkm::Dot(Li, LiPlus1) * svtkm::Dot(Li, LiPlus1)))) /
                                 ...
                                 const svtkm::exec::FunctorBase& worklet)
{
  ...
  if (svtkm::MagnitudeSquared(N0) <= Scalar(0.0) || svtkm::MagnitudeSquared(N1) <= Scalar(0.0) ||
      svtkm::MagnitudeSquared(N2) <= Scalar(0.0) || svtkm::MagnitudeSquared(N3) <= Scalar(0.0))
  {
  ...
  const Scalar q = svtkm::Max(q0, svtkm::Max(q1, svtkm::Max(q2, q3)));
  return q;
}
                                 ...
                                 const svtkm::exec::FunctorBase& worklet)
{
  ...
  OutType negativeInfinity = svtkm::NegativeInfinity<OutType>();
  ...
  for (svtkm::IdComponent matrixNumber = 0; matrixNumber < 9; matrixNumber++)
  {
    /*
    ...
    */
    tempMatrix1_1 =
      svtkm::Dot(hexJacobianMatrices[matrixNumber][0], hexJacobianMatrices[matrixNumber][0]);
    tempMatrix1_2 =
      svtkm::Dot(hexJacobianMatrices[matrixNumber][0], hexJacobianMatrices[matrixNumber][1]);
    tempMatrix1_3 =
      svtkm::Dot(hexJacobianMatrices[matrixNumber][0], hexJacobianMatrices[matrixNumber][2]);
    tempMatrix2_2 =
      svtkm::Dot(hexJacobianMatrices[matrixNumber][1], hexJacobianMatrices[matrixNumber][1]);
    tempMatrix2_3 =
      svtkm::Dot(hexJacobianMatrices[matrixNumber][1], hexJacobianMatrices[matrixNumber][2]);
    tempMatrix3_3 =
      svtkm::Dot(hexJacobianMatrices[matrixNumber][2], hexJacobianMatrices[matrixNumber][2]);
    determinant = svtkm::Dot(
      hexJacobianMatrices[matrixNumber][0],
      svtkm::Cross(hexJacobianMatrices[matrixNumber][1], hexJacobianMatrices[matrixNumber][2]));
    ...
    else
    {
      ...
      currentOddy = (firstPartOfNumerator - secondPartOfNumerator) /
        (svtkm::Pow(determinant, OutType(4.0 * third)));
      maxOddy = currentOddy > maxOddy ? currentOddy : maxOddy;
    }
  }
  if (maxOddy > 0)
  {
    return svtkm::Min(maxOddy, svtkm::Infinity<OutType>()); //normal case
  }

  return svtkm::Max(maxOddy, negativeInfinity);
}
} // namespace cellmetrics
} // namespace worklet
} // namespace svtkm
#endif // svtk_m_worklet_cellmetrics_Oddy_h
