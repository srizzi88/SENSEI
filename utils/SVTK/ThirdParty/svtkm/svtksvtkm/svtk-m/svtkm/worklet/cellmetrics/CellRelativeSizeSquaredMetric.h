...
//  this software.
//============================================================================
#ifndef svtk_m_worklet_cellmetrics_CellRelativeSizeSquaredMetric_h
#define svtk_m_worklet_cellmetrics_CellRelativeSizeSquaredMetric_h
...
SVTKM_EXEC OutType CellRelativeSizeSquaredMetric(const svtkm::IdComponent& numPts,
                                                ...
                                                const svtkm::exec::FunctorBase& worklet)
{
  ...
  OutType A = svtkm::exec::CellMeasure<OutType>(numPts, pts, tag, worklet);
  OutType R = A / avgArea;
  if (R == OutType(0.))
    return OutType(0.);
  OutType q = svtkm::Pow(svtkm::Min(R, OutType(1.) / R), OutType(2.));
  return OutType(q);
}
                                                ...
                                                const svtkm::exec::FunctorBase& worklet)
{
  ...
  OutType V = svtkm::exec::CellMeasure<OutType>(numPts, pts, tag, worklet);
                                                ...
                                                const svtkm::exec::FunctorBase& worklet)
{
  ...
  OutType X3z = (pts[4][2] - pts[0][2]) + (pts[5][2] - pts[1][2]) + (pts[6][2] - pts[2][2]) +
    (pts[7][2] - pts[3][2]);
  svtkm::Matrix<OutType, 3, 3> A8;
  svtkm::MatrixSetRow(A8, 0, svtkm::Vec<OutType, 3>(X1x, X1y, X1z));
  svtkm::MatrixSetRow(A8, 1, svtkm::Vec<OutType, 3>(X2x, X2y, X2z));
  svtkm::MatrixSetRow(A8, 2, svtkm::Vec<OutType, 3>(X3x, X3y, X3z));
  OutType D = svtkm::MatrixDeterminant(A8);
  D = D / (OutType(64.) * avgVolume);
  if (D == OutType(0.))
    return OutType(0.);
  OutType q = svtkm::Pow(svtkm::Min(D, OutType(1.) / D), OutType(2.));
  return OutType(q);
}
...
#endif // svtk_m_exec_cellmetrics_CellRelativeSizeSquaredMetric.h
