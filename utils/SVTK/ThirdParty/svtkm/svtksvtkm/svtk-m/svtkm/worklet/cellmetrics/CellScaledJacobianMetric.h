...
//  this software.
//============================================================================
#ifndef svtk_m_worklet_cellmetrics_ScaledJacobian_h
#define svtk_m_worklet_cellmetrics_ScaledJacobian_h
...
SVTKM_EXEC OutType CellScaledJacobianMetric(const svtkm::IdComponent& numPts,
                                           ...
                                           const svtkm::exec::FunctorBase& worklet)
{
  ...
  const Scalar modifier = (Scalar)(2 * svtkm::Sqrt(3) / 3);
  ...
  const Scalar productMax = svtkm::Max(l0l1_product, svtkm::Max(l0l2_product, l1l2_product));
  if (productMax <= Scalar(0.0))
  {
    return Scalar(0.0);
  }
  // compute jacobian of triangle
  Vector TriCross = svtkm::Cross(l2, l1);
  Scalar scaledJacobian = svtkm::Magnitude(TriCross);

  //add all pieces together
  //TODO change
  Vector normalVector = (Scalar(1.0) / Scalar(3.0)) * (l0 + l1 + l2);
  Vector surfaceNormalAtCenter = svtkm::TriangleNormal(normalVector, normalVector, normalVector);
  if (svtkm::Dot(surfaceNormalAtCenter, TriCross) < 0)
  {
    scaledJacobian *= -1;
  }
                                           ...
                                           const svtkm::exec::FunctorBase& worklet)
{
  ...
  const Scalar negativeInfinity = svtkm::NegativeInfinity<Scalar>();
...
*/
  ...
  const Scalar q =
    svtkm::Min(alpha0Scaled, svtkm::Min(alpha1Scaled, svtkm::Min(alpha2Scaled, alpha3Scaled)));
  return q;
}
                                           ...
                                           const svtkm::exec::FunctorBase& worklet)
{
  ...
  OutType currDeterminant, minDeterminant = svtkm::Infinity<OutType>();
  FloatType lenSquared1, lenSquared2, lenSquared3, minLengthSquared = svtkm::Infinity<FloatType>();
  svtkm::IdComponent matrixIndex;
  for (matrixIndex = 0; matrixIndex < 9; matrixIndex++)
  {
    lenSquared1 = (FloatType)svtkm::MagnitudeSquared(hexMatrices[matrixIndex][0]);
    minLengthSquared = lenSquared1 < minLengthSquared ? lenSquared1 : minLengthSquared;
    lenSquared2 = (FloatType)svtkm::MagnitudeSquared(hexMatrices[matrixIndex][1]);
    minLengthSquared = lenSquared2 < minLengthSquared ? lenSquared2 : minLengthSquared;
    lenSquared3 = (FloatType)svtkm::MagnitudeSquared(hexMatrices[matrixIndex][2]);
    minLengthSquared = lenSquared3 < minLengthSquared ? lenSquared3 : minLengthSquared;

    svtkm::Normalize(hexMatrices[matrixIndex][0]);
    svtkm::Normalize(hexMatrices[matrixIndex][1]);
    svtkm::Normalize(hexMatrices[matrixIndex][2]);
    currDeterminant =
      (OutType)svtkm::Dot(hexMatrices[matrixIndex][0],
                         svtkm::Cross(hexMatrices[matrixIndex][1], hexMatrices[matrixIndex][2]));
    if (currDeterminant < minDeterminant)
    {
      minDeterminant = currDeterminant;
    }
  }
  if (minLengthSquared < svtkm::NegativeInfinity<FloatType>())
  {
    return svtkm::Infinity<OutType>();
  }
  OutType toReturn = minDeterminant;
  if (toReturn > 0)
    return svtkm::Min(toReturn, svtkm::Infinity<OutType>()); //normal case

  return svtkm::Max(toReturn, svtkm::NegativeInfinity<OutType>());
}
                                           ...
                                           const svtkm::exec::FunctorBase& worklet)
{
  ...
  OutType jacobian = svtkm::Dot(svtkm::Cross(Edges[2], Edges[0]), Edges[3]);
  // compute the scaled jacobian
  OutType currSide, maxSide = svtkm::NegativeInfinity<OutType>();
  svtkm::IdComponent edgeIndex, sideIndex;
  for (edgeIndex = 0; edgeIndex < 6; edgeIndex++)
  {
    EdgesSquared[edgeIndex] = svtkm::MagnitudeSquared(Edges[edgeIndex]);
  }
  ...
  maxSide = svtkm::Sqrt(maxSide);
  OutType toUseInCalculation = jacobian > maxSide ? jacobian : maxSide;
  if (toUseInCalculation < svtkm::NegativeInfinity<OutType>())
  {
    return svtkm::Infinity<OutType>();
  }
  return (svtkm::Sqrt<OutType>(2) * jacobian) / toUseInCalculation;
}
} // namespace cellmetrics
} // namespace worklet
} // namespace svtkm
