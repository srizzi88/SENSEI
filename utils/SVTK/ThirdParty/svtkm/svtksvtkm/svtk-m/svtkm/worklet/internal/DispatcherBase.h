...
//============================================================================
#ifndef svtk_m_worklet_internal_DispatcherBase_h
#define svtk_m_worklet_internal_DispatcherBase_h
...
inline auto scheduling_range(const svtkm::worklet::Keys<KeyType>& inputDomain)
...
template <typename KeyType>
inline auto scheduling_range(const svtkm::worklet::Keys<KeyType>* const inputDomain)
  -> decltype(inputDomain->GetInputRange())
{
  return inputDomain->GetInputRange();
}
...
inline void PrintFailureMessage(int index)
{
  ...
  throw svtkm::cont::ErrorBadType(message.str());
}
...
struct DetermineIfHasDynamicParameter
{
  ...
  using DynamicTag = typename svtkm::cont::internal::DynamicTransformTraits<T>::DynamicTag;
  using isDynamic =
    typename std::is_same<DynamicTag, svtkm::cont::internal::DynamicTransformTagCastAndCall>::type;
...
struct DetermineHasCorrectParameters
{
  ...
  struct Functor
  {
    ...
    static constexpr bool isCorrect = svtkm::cont::arg::TypeCheck<TypeCheckTag, T>::value;
    ...
    SVTKM_READ_THE_SOURCE_CODE_FOR_HELP(isCorrect);
...
{
  template <typename ControlSignatureTag, svtkm::IdComponent Index>
  struct ReturnType
  {
    ...
    SVTKM_IS_CONTROL_SIGNATURE_TAG(ControlSignatureTag);
...
{
  template <typename ExecutionSignatureTag, svtkm::IdComponent Index>
  struct ReturnType
  {
    ...
    SVTKM_IS_EXECUTION_SIGNATURE_TAG(ExecutionSignatureTag);
...
template <typename ControlInterface, svtkm::IdComponent Index>
...
SVTKM_CONT
inline svtkm::Id FlatRange(svtkm::Id range)
{
  return range;
}

SVTKM_CONT
inline svtkm::Id FlatRange(const svtkm::Id3& range)
{
...
struct DispatcherBaseTransportFunctor
{
  ...
  SVTKM_CONT DispatcherBaseTransportFunctor(const InputDomainType& inputDomain,
  ...
  template <typename ControlParameter, svtkm::IdComponent Index>
  struct ReturnType
  {
    using TransportTag =
    ...
    using TransportType = typename svtkm::cont::arg::Transport<TransportTag, T, Device>;
    using type = typename TransportType::ExecObjectType;
  };

  template <typename ControlParameter, svtkm::IdComponent Index>
  SVTKM_CONT typename ReturnType<ControlParameter, Index>::type operator()(
    ...
    svtkm::internal::IndexTag<Index>) const
  {
    using TransportTag =
      typename DispatcherBaseTransportInvokeTypes<ControlInterface, Index>::TransportTag;
    using T = remove_pointer_and_decay<ControlParameter>;
    svtkm::cont::arg::Transport<TransportTag, T, Device> transport;
...
struct convert_arg_wrapper
{
  ...
  void operator()(T&& t, Args&&... args) const
  {
    ...
    using valid =
      std::integral_constant<bool, svtkm::cont::arg::TypeCheck<TypeCheckTag, Type>::value>;
  ...
  {
    svtkm::worklet::internal::detail::PrintFailureMessage(LeftToProcess);
  }
};
...
template <std::size_t LeftToProcess,
          ...
          typename Trampoline,
          typename... Args>
inline void convert_arg(svtkm::cont::internal::DynamicTransformTagStatic,
                        T&& t,
                        const ContParams&,
...
template <std::size_t LeftToProcess,
          ...
          typename Trampoline,
          typename... Args>
inline void convert_arg(svtkm::cont::internal::DynamicTransformTagCastAndCall,
                        T&& t,
                        const ContParams&,
                        const Trampoline& trampoline,
                        Args&&... args)
{ //This is something dynamic so cast and call
  ...
  svtkm::cont::CastAndCall(as_ref(t),
...
struct for_each_dynamic_arg
{
  ...
  void operator()(const Trampoline& trampoline, ContParams&& sig, T&& t, Args&&... args) const
  {
    ...
    using tag = typename svtkm::cont::internal::DynamicTransformTraits<Type>::DynamicTag;
...
template <svtkm::IdComponent MaxIndexAllowed>
struct PlaceholderValidator
{
  ...
  template <int N>
  void operator()(brigand::type_<svtkm::placeholders::Arg<N>>) const
  {
...
protected:
  using ControlInterface =
    svtkm::internal::FunctionInterface<typename WorkletType::ControlSignature>;

  // We go through the GetExecSig as that generates a default ExecutionSignature
  // if one doesn't exist on the worklet
  using ExecutionSignature =
    typename svtkm::placeholders::GetExecSig<WorkletType>::ExecutionSignature;
  using ExecutionInterface = svtkm::internal::FunctionInterface<ExecutionSignature>;

  static constexpr svtkm::IdComponent NUM_INVOKE_PARAMS = ControlInterface::ARITY;
...
private:
  ...
  template <typename... Args>
  SVTKM_CONT void StartInvoke(Args&&... args) const
  {
    using ParameterInterface = svtkm::internal::FunctionInterface<void(remove_cvref<Args>...)>;

    SVTKM_STATIC_ASSERT_MSG(ParameterInterface::ARITY == NUM_INVOKE_PARAMS,
  ...
  template <typename... Args>
  SVTKM_CONT void StartInvokeDynamic(std::true_type, Args&&... args) const
  {
    ...
    using ContParamsInfo =
      svtkm::internal::detail::FunctionSigInfo<typename WorkletType::ControlSignature>;
    typename ContParamsInfo::Parameters parameters;
    detail::deduce(*this, parameters, std::forward<Args>(args)...);
  }

  template <typename... Args>
  SVTKM_CONT void StartInvokeDynamic(std::false_type, Args&&... args) const
  {
    using ParameterInterface = svtkm::internal::FunctionInterface<void(remove_cvref<Args>...)>;
    ...
    using ParamTypes = typename ParameterInterface::ParameterSig;
    using ContSigTypes = typename svtkm::internal::detail::FunctionSigInfo<
    ...
    auto fi = svtkm::internal::make_FunctionInterface<void, remove_cvref<Args>...>(args...);
    auto ivc = svtkm::internal::Invocation<ParameterInterface,
                                          ControlInterface,
                                          ExecutionInterface,
                                          WorkletType::InputDomain::INDEX,
                                          svtkm::internal::NullType,
                                          svtkm::internal::NullType>(
      fi, svtkm::internal::NullType{}, svtkm::internal::NullType{});
...
public:
  ...
  ///
  SVTKM_CONT
  void SetDevice(svtkm::cont::DeviceAdapterId device) { this->Device = device; }
  ...
  template <typename... Args>
  SVTKM_CONT void Invoke(Args&&... args) const
  {
    SVTKM_LOG_SCOPE(svtkm::cont::LogLevel::Perf,
                   "Invoking Worklet: '%s'",
                   svtkm::cont::TypeToString<WorkletType>().c_str());
...
protected:
  ...
  SVTKM_CONT void BasicInvoke(Invocation& invocation, svtkm::Id numInstances) const
  {
    bool success =
      svtkm::cont::TryExecuteOnDevice(this->Device,
  ...
  template <typename Invocation>
  SVTKM_CONT void BasicInvoke(Invocation& invocation, svtkm::Id2 dimensions) const
  {
    this->BasicInvoke(invocation, svtkm::Id3(dimensions[0], dimensions[1], 1));
  }

  template <typename Invocation>
  SVTKM_CONT void BasicInvoke(Invocation& invocation, svtkm::Id3 dimensions) const
  {
    bool success =
      svtkm::cont::TryExecuteOnDevice(this->Device,
                                     internal::detail::DispatcherBaseTryExecuteFunctor(),
...
private:
                                           ...
                                           DeviceAdapter device) const
  {
    ...
    svtkm::internal::Invocation<ExecObjectParameters,
  ...
  SVTKM_CONT void InvokeSchedule(const Invocation& invocation, RangeType range, DeviceAdapter) const
  {
...
#endif //svtk_m_worklet_internal_DispatcherBase_h
