...
//============================================================================
#ifndef svtk_m_worklet_internal_Placeholders_h
#define svtk_m_worklet_internal_Placeholders_h
...
struct Arg : svtkm::exec::arg::BasicArg<ControlSignatureIndex>
{
};
...
struct GetExecSig
{
  ...
  static svtkmstd::tuple<std::true_type, typename U::ExecutionSignature> get_exec_sig(int);

  template <typename U>
  static svtkmstd::tuple<std::false_type, std::false_type> get_exec_sig(...);

  using cont_sig = typename WorkletType::ControlSignature;
  using cont_sig_info = svtkm::placeholders::FunctionSigArity<cont_sig>;

  using result = decltype(get_exec_sig<WorkletType>(0));
  using has_explicit_exec_sig = typename svtkmstd::tuple_element<0, result>::type;

  using ExecutionSignature = typename std::conditional<
    has_explicit_exec_sig::value,
    typename svtkmstd::tuple_element<1, result>::type,
    typename svtkm::placeholders::DefaultExecSig<cont_sig_info::value>::type>::type;
};
}
}
