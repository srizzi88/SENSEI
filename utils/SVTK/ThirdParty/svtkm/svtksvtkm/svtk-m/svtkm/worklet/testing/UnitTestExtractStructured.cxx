...
public:
  void TestUniform2D() const
  {
    ...
    SVTKM_TEST_ASSERT(test_equal(outCellSet.GetNumberOfPoints(), 9),
                     "Wrong result for ExtractStructured worklet");
    SVTKM_TEST_ASSERT(test_equal(outCellSet.GetNumberOfCells(), 4),
                     "Wrong result for ExtractStructured worklet");
  }
  ...
  void TestUniform3D() const
  {
    ...
    svtkm::worklet::ExtractStructured worklet;
    svtkm::worklet::ExtractStructured::DynamicCellSetStructured outCellSet;

    // RangeId3 within dataset
    svtkm::RangeId3 range0(1, 4, 1, 4, 1, 4);
    svtkm::Id3 sample(1, 1, 1);
    bool includeBoundary = false;
    bool includeOffset = false;

    outCellSet = worklet.Run(cellSet, range0, sample, includeBoundary, includeOffset);
    SVTKM_TEST_ASSERT(test_equal(outCellSet.GetNumberOfPoints(), 27),
                     "Wrong result for ExtractStructured worklet");
    SVTKM_TEST_ASSERT(test_equal(outCellSet.GetNumberOfCells(), 8),
                     "Wrong result for ExtractStructured worklet");

    // RangeId3 surrounds dataset
    svtkm::RangeId3 range1(-1, 8, -1, 8, -1, 8);
    outCellSet = worklet.Run(cellSet, range1, sample, includeBoundary, includeOffset);
    SVTKM_TEST_ASSERT(test_equal(outCellSet.GetNumberOfPoints(), 125),
                     "Wrong result for ExtractStructured worklet");
    SVTKM_TEST_ASSERT(test_equal(outCellSet.GetNumberOfCells(), 64),
                     "Wrong result for ExtractStructured worklet");

    // RangeId3 intersects dataset on near boundary
    svtkm::RangeId3 range2(-1, 3, -1, 3, -1, 3);
    ...
    svtkm::RangeId3 range5(2, 8, 1, 2, 1, 4);
  ...
  void TestUniform3D1() const
  {
    ...
    svtkm::RangeId3 range0(0, 5, 0, 5, 1, 4);
    svtkm::Id3 sample0(2, 2, 1);
    ...
    // RangeId3 and subsample
    svtkm::RangeId3 range1(0, 5, 0, 5, 1, 4);
    svtkm::Id3 sample1(3, 3, 2);
    bool includeBoundary1 = false;

    outCellSet = worklet.Run(cellSet, range1, sample1, includeBoundary1, includeOffset);

    SVTKM_TEST_ASSERT(test_equal(outCellSet.GetNumberOfPoints(), 8),
                     "Wrong result for ExtractStructured worklet");
    SVTKM_TEST_ASSERT(test_equal(outCellSet.GetNumberOfCells(), 1),
                     "Wrong result for ExtractStructured worklet");

    // RangeId3 and subsample
    svtkm::RangeId3 range2(0, 5, 0, 5, 1, 4);
    svtkm::Id3 sample2(3, 3, 2);
    bool includeBoundary2 = true;

    outCellSet = worklet.Run(cellSet, range2, sample2, includeBoundary2, includeOffset);

    SVTKM_TEST_ASSERT(test_equal(outCellSet.GetNumberOfPoints(), 18),
                     "Wrong result for ExtractStructured worklet");
    SVTKM_TEST_ASSERT(test_equal(outCellSet.GetNumberOfCells(), 4),
                     "Wrong result for ExtractStructured worklet");
  }
  ...
  void TestRectilinear2D() const
  {
    ...
    SVTKM_TEST_ASSERT(test_equal(outCellSet.GetNumberOfPoints(), 4),
                     "Wrong result for ExtractStructured worklet");
    SVTKM_TEST_ASSERT(test_equal(outCellSet.GetNumberOfCells(), 1),
                     "Wrong result for ExtractStructured worklet");
  }
  ...
  void TestOffset3D1() const
  {
    ...
    // RangeID3 and subsample
    svtkm::RangeId3 range(5, 15, 0, 10, 0, 10);
    svtkm::Id3 sample(1, 1, 1);
    svtkm::Id3 test_offset(10, 0, 0);
    svtkm::Id3 no_offset(0, 0, 0);
    svtkm::Id3 new_dims(5, 10, 10);
    bool includeBoundary = false;
    bool includeOffset = false;
    cellSet.SetPointDimensions(svtkm::make_Vec(10, 10, 10));

    svtkm::worklet::ExtractStructured worklet;
    auto outCellSet = worklet.Run(cellSet, range, sample, includeBoundary, includeOffset);

    SVTKM_TEST_ASSERT(test_equal(cellSet.GetGlobalPointIndexStart(), no_offset));
    svtkm::Id3 cellDims =
      outCellSet.Cast<CellSetType>().GetSchedulingRange(svtkm::TopologyElementTagCell());

    includeOffset = true;
    cellSet.SetGlobalPointIndexStart(test_offset);
    outCellSet = worklet.Run(cellSet, range, sample, includeBoundary, includeOffset);
    cellDims = outCellSet.Cast<CellSetType>().GetSchedulingRange(svtkm::TopologyElementTagCell());
    CellSetType cs = outCellSet.Cast<CellSetType>();
    cellDims = cs.GetPointDimensions();
    SVTKM_TEST_ASSERT(test_equal(cellDims, new_dims));
    SVTKM_TEST_ASSERT(test_equal(cellSet.GetGlobalPointIndexStart(), test_offset));
  }
  ...
  void TestOffset3D2() const
  {
    ...
    using CellSetType = svtkm::cont::CellSetStructured<3>;
    CellSetType cellSet;
    svtkm::RangeId3 range(15, 20, 0, 10, 0, 10);
    svtkm::Id3 sample(1, 1, 1);
    svtkm::Id3 test_dims(5, 10, 10);
    svtkm::Id3 gpis(10, 0, 0);
    svtkm::Id3 test_offset(15, 0, 0);
    bool includeBoundary = false;
    ...
    auto outCellSet = worklet.Run(cellSet, range, sample, includeBoundary, includeOffset);
    CellSetType cs = outCellSet.Cast<CellSetType>();
    svtkm::Id3 cellDims = cs.GetPointDimensions();
    SVTKM_TEST_ASSERT(test_equal(cellDims, test_dims));
    SVTKM_TEST_ASSERT(test_equal(cs.GetGlobalPointIndexStart(), test_offset));
  }
  ...
  void TestOffset3D3() const
  {
    ...
    using CellSetType = svtkm::cont::CellSetStructured<3>;
    CellSetType cellSet;
    svtkm::RangeId3 range(100, 110, 0, 10, 0, 10);
    svtkm::Id3 sample(1, 1, 1);
    svtkm::Id3 test_dims(0, 0, 0);
    bool includeBoundary = false;
    bool includeOffset = true;
    cellSet.SetPointDimensions(svtkm::make_Vec(10, 10, 10));
    svtkm::worklet::ExtractStructured worklet;
    ...
    auto outCellSet = worklet.Run(cellSet, range, sample, includeBoundary, includeOffset);
    CellSetType cs = outCellSet.Cast<CellSetType>();
    SVTKM_TEST_ASSERT(test_equal(cs.GetPointDimensions(), test_dims));
  }
  void TestOffset2D() const
  {
    ...
    CellSetType cellSet;
    // RangeID3 and subsample
    svtkm::RangeId3 range(5, 15, 0, 10, 0, 1);
    svtkm::Id3 sample(1, 1, 1);
    svtkm::Id2 test_offset(10, 0);
    svtkm::Id2 no_offset(0, 0);
    svtkm::Id2 new_dims(5, 10);
    bool includeBoundary = false;
    bool includeOffset = false;
    cellSet.SetPointDimensions(svtkm::make_Vec(10, 10));
    svtkm::worklet::ExtractStructured worklet;
    auto outCellSet = worklet.Run(cellSet, range, sample, includeBoundary, includeOffset);
    SVTKM_TEST_ASSERT(test_equal(cellSet.GetGlobalPointIndexStart(), no_offset));
    svtkm::Id2 cellDims =
