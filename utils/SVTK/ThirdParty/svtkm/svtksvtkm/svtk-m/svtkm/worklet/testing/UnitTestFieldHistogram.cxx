...
//
void PrintHistogram(svtkm::cont::ArrayHandle<svtkm::Id> bins,
                    svtkm::Id numberOfBins,
                    const svtkm::Range& range,
                    svtkm::Float32 delta)
{
  svtkm::cont::ArrayHandle<svtkm::Id>::PortalConstControl binPortal = bins.GetPortalConstControl();

  svtkm::Id sum = 0;
  for (svtkm::Id i = 0; i < numberOfBins; i++)
  {
    svtkm::Float64 lo = range.Min + (static_cast<svtkm::Float64>(i) * delta);
    svtkm::Float64 hi = lo + delta;
    sum += binPortal.Get(i);
    std::cout << "  BIN[" << i << "] Range[" << lo << ", " << hi << "] = " << binPortal.Get(i)
              << std::endl;
  }
  SVTKM_TEST_ASSERT(test_equal(sum, 1000), "Histogram not full");
}
...
{
  return svtkm::cont::testing::Testing::Run(TestFieldHistogram, argc, argv);
}
