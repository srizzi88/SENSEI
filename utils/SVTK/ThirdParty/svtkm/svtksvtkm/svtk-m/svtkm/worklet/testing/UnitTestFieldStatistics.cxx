...
//
svtkm::cont::DataSet Make2DUniformStatDataSet0()
{
  ...
  svtkm::cont::ArrayHandleUniformPointCoordinates coordinates(svtkm::Id3(xVerts, yVerts, 1));
  dataSet.AddCoordinateSystem(svtkm::cont::CoordinateSystem("coordinates", coordinates));

  // Create cell scalar
  svtkm::Float32 data[nVerts] = { 4, 1, 10, 6, 8, 2, 9, 3, 5, 7 };
  dataSet.AddField(svtkm::cont::make_Field(
    "data", svtkm::cont::Field::Association::CELL_SET, data, nCells, svtkm::CopyFlag::On));

  svtkm::cont::CellSetStructured<dimension> cellSet;

  //Set uniform structure
  cellSet.SetPointDimensions(svtkm::make_Vec(xVerts, yVerts));
...
//
svtkm::cont::DataSet Make2DUniformStatDataSet1()
{
  svtkm::cont::DataSet dataSet;
  ...
  svtkm::Float32 chiSquare[nVerts] = {
  ...
  // Set point scalars
  dataSet.AddField(svtkm::cont::make_Field(
    "p_poisson", svtkm::cont::Field::Association::POINTS, poisson, nVerts, svtkm::CopyFlag::On));
  dataSet.AddField(svtkm::cont::make_Field(
    "p_normal", svtkm::cont::Field::Association::POINTS, normal, nVerts, svtkm::CopyFlag::On));
  dataSet.AddField(svtkm::cont::make_Field(
    "p_chiSquare", svtkm::cont::Field::Association::POINTS, chiSquare, nVerts, svtkm::CopyFlag::On));
  dataSet.AddField(svtkm::cont::make_Field(
    "p_uniform", svtkm::cont::Field::Association::POINTS, uniform, nVerts, svtkm::CopyFlag::On));

  // Set cell scalars
  dataSet.AddField(svtkm::cont::make_Field(
    "c_poisson", svtkm::cont::Field::Association::CELL_SET, poisson, nCells, svtkm::CopyFlag::On));
  dataSet.AddField(svtkm::cont::make_Field(
    "c_normal", svtkm::cont::Field::Association::CELL_SET, normal, nCells, svtkm::CopyFlag::On));
  dataSet.AddField(svtkm::cont::make_Field("c_chiSquare",
                                          svtkm::cont::Field::Association::CELL_SET,
                                          chiSquare,
                                          nCells,
                                          svtkm::CopyFlag::On));
  dataSet.AddField(svtkm::cont::make_Field(
    "c_uniform", svtkm::cont::Field::Association::CELL_SET, poisson, nCells, svtkm::CopyFlag::On));
...
//
void PrintStatInfo(svtkm::worklet::FieldStatistics<svtkm::Float32>::StatInfo statinfo)
{
...
void TestFieldSimple()
{
  ...
  svtkm::worklet::FieldStatistics<svtkm::Float32>::StatInfo statinfo;

  // Data attached is [1:10]
  svtkm::cont::DataSet ds = Make2DUniformStatDataSet0();

  // Cell data
  svtkm::cont::ArrayHandle<svtkm::Float32> data;
  ds.GetField("data").GetData().CopyTo(data);

  // Run
  svtkm::worklet::FieldStatistics<svtkm::Float32>().Run(data, statinfo);
  std::cout << "Statistics for CELL data:" << std::endl;
  PrintStatInfo(statinfo);

  svtkm::Float32 expected[8] = { 6.0f, 1.0f, 10.0f, 5.5f, 8.25f, 2.87228f, 0.0f, 1.77576f };
  SVTKM_TEST_ASSERT(test_equal(statinfo.median, expected[0]), "Error in median");
  SVTKM_TEST_ASSERT(test_equal(statinfo.minimum, expected[1]), "Error in minimum");
  SVTKM_TEST_ASSERT(test_equal(statinfo.maximum, expected[2]), "Error in maximum");
  SVTKM_TEST_ASSERT(test_equal(statinfo.mean, expected[3]), "Error in mean");
  SVTKM_TEST_ASSERT(test_equal(statinfo.variance, expected[4]), "Error in variance");
  SVTKM_TEST_ASSERT(test_equal(statinfo.stddev, expected[5]), "Error in stddev");
  SVTKM_TEST_ASSERT(test_equal(statinfo.skewness, expected[6]), "Error in skewness");
  SVTKM_TEST_ASSERT(test_equal(statinfo.kurtosis, expected[7]), "Error in kurtosis");
}
...
void TestFieldStandardDistributions()
{
  ...
  svtkm::cont::DataSet ds = Make2DUniformStatDataSet1();

  // Point data
  svtkm::cont::ArrayHandle<svtkm::Float32> p_poisson;
  ds.GetField("p_poisson").GetData().CopyTo(p_poisson);
  svtkm::cont::ArrayHandle<svtkm::Float32> p_normal;
  ds.GetField("p_normal").GetData().CopyTo(p_normal);
  svtkm::cont::ArrayHandle<svtkm::Float32> p_chiSquare;
  ds.GetField("p_chiSquare").GetData().CopyTo(p_chiSquare);
  svtkm::cont::ArrayHandle<svtkm::Float32> p_uniform;
  ds.GetField("p_uniform").GetData().CopyTo(p_uniform);

  // Run Poisson data
  svtkm::worklet::FieldStatistics<svtkm::Float32>().Run(p_poisson, statinfo);
  std::cout << "Poisson distributed POINT data:" << std::endl;
  PrintStatInfo(statinfo);

  // Run Normal data
  svtkm::worklet::FieldStatistics<svtkm::Float32>().Run(p_normal, statinfo);
  std::cout << "Normal distributed POINT data:" << std::endl;
  PrintStatInfo(statinfo);

  // Run Chi Square data
  svtkm::worklet::FieldStatistics<svtkm::Float32>().Run(p_chiSquare, statinfo);
  std::cout << "Chi Square distributed POINT data:" << std::endl;
  PrintStatInfo(statinfo);

  // Run Uniform data
  svtkm::worklet::FieldStatistics<svtkm::Float32>().Run(p_uniform, statinfo);
...
{
  return svtkm::cont::testing::Testing::Run(TestFieldStatistics, argc, argv);
}
