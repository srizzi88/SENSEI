...
void TestPointGradientUniform3DWithVectorField2()
{
  ...
  SVTKM_TEST_ASSERT((extraOutput.Divergence.GetNumberOfValues() == nVerts),
                   "Divergence field should be generated");
  SVTKM_TEST_ASSERT((extraOutput.Vorticity.GetNumberOfValues() == nVerts),
                   "Vorticity field should be generated");
  SVTKM_TEST_ASSERT((extraOutput.QCriterion.GetNumberOfValues() == nVerts),
  ...
  for (int i = 0; i < 4; ++i)
  {
    ...
    const svtkm::Vec3f_64 es(eg[1][2] + eg[2][1], eg[2][0] + eg[0][2], eg[0][1] + eg[1][0]);
    const svtkm::Vec3f_64 ed(eg[0][0], eg[1][1], eg[2][2]);

    //compute QCriterion
    svtkm::Float64 qcriterion =
      ((svtkm::Dot(ev, ev) / 2.0f) - (svtkm::Dot(ed, ed) + (svtkm::Dot(es, es) / 2.0f))) / 2.0f;
...
void TestPointGradientExplicit3D()
{
  ...
  svtkm::cont::testing::MakeTestDataSet testDataSet;
  svtkm::cont::DataSet dataSet = testDataSet.Make3DExplicitDataSet5();
  ...
  //svtkm::cont::printSummary_ArrayHandle(result, std::cout, true);
  const int nVerts = 11;
  svtkm::Vec3f_32 expected[nVerts] = {
...
void TestPointGradientExplicit2D()
{
  ...
  svtkm::cont::testing::MakeTestDataSet testDataSet;
  svtkm::cont::DataSet dataSet = testDataSet.Make2DExplicitDataSet0();
...
{
  return svtkm::cont::testing::Testing::Run(TestPointGradient, argc, argv);
}
