...
int TestSplatKernels()
{
  ...
  for (int i = 0; i < 100; ++i)
  {
    ...
    s = IntegralOfKernel(svtkm::worklet::splatkernels::Gaussian<3>(smoothinglength));
    SVTKM_TEST_ASSERT(fabs(s - 1.0) < eps, "Gaussian 3D integration failure");
  }
  ...
  for (int i = 0; i < 100; ++i)
  {
    smoothinglength = 0.01 + i * (10.0 / 100.0);
    s =
      IntegralOfKernel(svtkm::worklet::splatkernels::Gaussian<3>(smoothinglength), smoothinglength);
    SVTKM_TEST_ASSERT(fabs(s - 1.0) < eps, "Gaussian 3D integration failure");
  }

  //  s  = IntegralOfKernel(svtkm::worklet::splatkernels::Gaussian<2>(smoothinglength));
  //  SVTKM_TEST_ASSERT ( fabs(s - 1.0) < eps, "Gaussian 2D integration failure");

  std::cout << "Testing Spline3rdOrder 3D kernel integration \n";
  for (int i = 0; i < 100; ++i)
  {
    smoothinglength = 0.01 + i * (10.0 / 100.0);
    s = IntegralOfKernel(svtkm::worklet::splatkernels::Spline3rdOrder<3>(smoothinglength));
    SVTKM_TEST_ASSERT(fabs(s - 1.0) < eps, "Spline3rdOrder 3D integration failure");
  }

  //  s  = IntegralOfKernel(svtkm::worklet::splatkernels::Spline3rdOrder<2>(smoothinglength));
  //  SVTKM_TEST_ASSERT ( fabs(s - 1.0) < eps, "Spline3rdOrder 2D integration failure");
...
{
  return svtkm::cont::testing::Testing::Run(TestSplatKernels, argc, argv);
}
