...
#include <svtkm/worklet/StreamLineUniformGrid.h>
...
template <typename T>
SVTKM_EXEC_CONT svtkm::Vec<T, 3> Normalize(svtkm::Vec<T, 3> v)
{
  T magnitude = static_cast<T>(sqrt(svtkm::Dot(v, v)));
  T zero = static_cast<T>(0.0);
  T one = static_cast<T>(1.0);
  if (magnitude == zero)
    return svtkm::make_Vec(zero, zero, zero);
  else
...
void TestStreamLineUniformGrid()
{
  ...
  svtkm::Id numSeeds = 5;
  svtkm::Id maxSteps = 50;
  svtkm::Float32 timeStep = 0.5f;

  // Size of the dataset
  const svtkm::Id3 vdims(5, 5, 5);

  // Read vector data at each point of the uniform grid and store
  svtkm::Id nElements = vdims[0] * vdims[1] * vdims[2] * 3;

  std::vector<svtkm::Vec3f_32> field;
  for (svtkm::Id i = 0; i < nElements; i++)
  {
    svtkm::Float32 x = data[i];
    svtkm::Float32 y = data[++i];
    svtkm::Float32 z = data[++i];
    svtkm::Vec3f_32 vecData(x, y, z);
    field.push_back(Normalize(vecData));
  }
  svtkm::cont::ArrayHandle<svtkm::Vec3f_32> fieldArray;
  fieldArray = svtkm::cont::make_ArrayHandle(&field[0], nElements);

  // Construct the input dataset (uniform) to hold the input and set vector data
  svtkm::cont::DataSet inDataSet;
  svtkm::cont::ArrayHandleUniformPointCoordinates coordinates(vdims);
  inDataSet.AddCoordinateSystem(svtkm::cont::CoordinateSystem("coordinates", coordinates));
  inDataSet.AddField(
    svtkm::cont::Field("vecData", svtkm::cont::Field::Association::POINTS, fieldArray));

  svtkm::cont::CellSetStructured<3> inCellSet;
  inCellSet.SetPointDimensions(svtkm::make_Vec(vdims[0], vdims[1], vdims[2]));
  inDataSet.SetCellSet(inCellSet);

  // Create and run the filter
  svtkm::worklet::StreamLineFilterUniformGrid<svtkm::Float32> streamLines;
  auto outDataSet =
    streamLines.Run(inDataSet, svtkm::worklet::streamline::BOTH, numSeeds, maxSteps, timeStep);
  ...
  svtkm::Id numberOfCells = outCellSet.GetNumberOfCells();
  svtkm::Id numberOfPoints = coordArray.GetNumberOfValues();
  std::cout << "Number of polylines " << numberOfCells << std::endl;
  std::cout << "Number of coordinates " << numberOfPoints << std::endl;

  SVTKM_TEST_ASSERT(test_equal(numberOfCells, numSeeds * 2),
                   "Wrong number of cells for stream lines");
}

int UnitTestStreamLineUniformGrid(int argc, char* argv[])
{
  return svtkm::cont::testing::Testing::Run(TestStreamLineUniformGrid, argc, argv);
}
