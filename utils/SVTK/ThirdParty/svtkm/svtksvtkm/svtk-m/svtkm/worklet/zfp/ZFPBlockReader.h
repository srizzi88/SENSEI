...
//============================================================================
#ifndef svtk_m_worklet_zfp_block_reader_h
#define svtk_m_worklet_zfp_block_reader_h
...
template <svtkm::Int32 block_size, typename WordsPortalType>
struct BlockReader
{
  const WordsPortalType& Words;
  const svtkm::Int32 m_maxbits;
  int m_block_idx;
  ...
  {
    Index = svtkm::Id(static_cast<size_t>(block_idx * maxbits) / (sizeof(Word) * 8));
    m_buffer = static_cast<Word>(Words.Get(Index));
    m_current_bit = (block_idx * maxbits) % svtkm::Int32((sizeof(Word) * 8));

    m_buffer >>= m_current_bit;
    m_block_idx = block_idx;
  }

  inline SVTKM_EXEC unsigned int read_bit()
  {
    svtkm::UInt32 bit = svtkm::UInt32(m_buffer) & 1u;
    ++m_current_bit;
    m_buffer >>= 1;
    // handle moving into next word
    if (m_current_bit >= svtkm::Int32(sizeof(Word) * 8))
    {
      m_current_bit = 0;
      ++Index;
      if (Index > MaxIndex)
  ...
  inline SVTKM_EXEC svtkm::UInt64 read_bits(const int& n_bits)
  {
    svtkm::UInt64 bits;
    // rem bits will always be positive
    svtkm::Int32 rem_bits = svtkm::Int32(sizeof(Word) * 8) - m_current_bit;

    svtkm::Int32 first_read = svtkm::Min(rem_bits, n_bits);
    // first mask
    Word mask = ((Word)1 << ((first_read))) - 1;
    bits = m_buffer & mask;
    m_buffer >>= n_bits;
    m_current_bit += first_read;
    svtkm::Int32 next_read = 0;
    if (n_bits >= rem_bits)
    {
      m_current_bit = 0;
      // just read in 0s if someone asks for more bits past the end of the array.
      // not sure what the best way to deal with this i
      Index = svtkm::Min(MaxIndex, Index + 1);
      m_buffer = static_cast<Word>(Words.Get(Index));
...
#endif //  svtk_m_worklet_zfp_block_reader_h
