/*=========================================================================
  ...
  Module:    svtkContextInteractorStyle.h
...
#ifndef svtkContextInteractorStyle_h
#define svtkContextInteractorStyle_h
...
class SVTKVIEWSCONTEXT2D_EXPORT svtkContextInteractorStyle : public svtkInteractorStyle
{
public:
  static svtkContextInteractorStyle* New();
  svtkTypeMacro(svtkContextInteractorStyle, svtkInteractorStyle);
   ...
   * The scene is observed (svtkCommand::ModifiedEvent) and a refresh on the
...
protected:
  svtkContextInteractorStyle();
  ~svtkContextInteractorStyle() override;
  ...
  svtkWeakPointer<svtkContextScene> Scene;
  svtkNew<svtkCallbackCommand> SceneCallbackCommand;
  svtkNew<svtkCallbackCommand> InteractorCallbackCommand;
  int ProcessingEvents;
  svtkMTimeType LastSceneRepaintMTime;

  int SceneTimerId;
  bool TimerCallbackInitialized;

private:
  svtkContextInteractorStyle(const svtkContextInteractorStyle&) = delete;
  void operator=(const svtkContextInteractorStyle&) = delete;

  void ConstructMouseEvent(svtkContextMouseEvent& event, int button);
  bool ProcessMousePress(const svtkContextMouseEvent& event);
};
