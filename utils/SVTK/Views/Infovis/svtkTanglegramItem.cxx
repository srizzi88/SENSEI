...
svtkStandardNewMacro(svtkTanglegramItem);

//-----------------------------------------------------------------------------
svtkTanglegramItem::svtkTanglegramItem()
{
  this->Dendrogram1 = svtkSmartPointer<svtkDendrogramItem>::New();
  this->Dendrogram1->ExtendLeafNodesOn();
  this->AddItem(this->Dendrogram1);

  this->Dendrogram2 = svtkSmartPointer<svtkDendrogramItem>::New();
  ...
  this->LookupTable = svtkSmartPointer<svtkLookupTable>::New();
...
//-----------------------------------------------------------------------------
void svtkTanglegramItem::SetTree1(svtkTree* tree)
{
...
//-----------------------------------------------------------------------------
void svtkTanglegramItem::SetTree2(svtkTree* tree)
{
...
//-----------------------------------------------------------------------------
svtkTable* svtkTanglegramItem::GetTable()
{
  return this->Table;
}

//-----------------------------------------------------------------------------
void svtkTanglegramItem::SetTable(svtkTable* table)
{
  if (table == nullptr)
  {
    return;
  ...
  this->SourceNames = svtkArrayDownCast<svtkStringArray>(this->Table->GetColumn(0));
  this->GenerateLookupTable();
  this->TreeReordered = false;
}

//-----------------------------------------------------------------------------
bool svtkTanglegramItem::Paint(svtkContext2D* painter)
{
...
//-----------------------------------------------------------------------------
void svtkTanglegramItem::RefreshBuffers(svtkContext2D* painter)
{
...
//-----------------------------------------------------------------------------
void svtkTanglegramItem::PaintCorrespondenceLines(svtkContext2D* painter)
{
  ...
  for (svtkIdType row = 0; row < this->Table->GetNumberOfRows(); ++row)
  {
    ...
    svtkIdType tree1Index = this->Tree1Names->LookupValue(source);
    if (tree1Index == -1)
    {
      continue;
    }
    ...
    for (svtkIdType col = 1; col < this->Table->GetNumberOfColumns(); ++col)
    {
      ...
      svtkIdType tree2Index = this->Tree2Names->LookupValue(target);
      if (tree2Index == -1)
      {
        continue;
      }
      ...
      int fontSize = painter->ComputeFontSizeForBoundedString("Igq", SVTK_FLOAT_MAX, this->Spacing);
      ...
      painter->GetPen()->SetLineType(svtkPen::DOT_LINE);
...
//-----------------------------------------------------------------------------
void svtkTanglegramItem::PaintTreeLabels(svtkContext2D* painter)
{
...
//-----------------------------------------------------------------------------
void svtkTanglegramItem::ReorderTree()
{
  ...
  svtkTree* tree = this->Dendrogram2->GetTree();

  this->Tree2Names =
    svtkArrayDownCast<svtkStringArray>(tree->GetVertexData()->GetAbstractArray("node name"));
  ...
  {
    svtkIdType vertex = bfsIterator->Next();
...
//-----------------------------------------------------------------------------
void svtkTanglegramItem::ReorderTreeAtVertex(svtkIdType parent, svtkTree* tree)
{
  ...
  for (svtkIdType i = 0; i < tree->GetNumberOfChildren(parent); ++i)
  {
    svtkIdType child = tree->GetChild(parent, i);
    double score = this->GetPositionScoreForVertex(child, tree);
    svtkDendrogramItem::WeightedVertex wv = { child, score };
    queue.push(wv);
  }

  svtkNew<svtkIdTypeArray> newChildOrder;
  while (!queue.empty())
  {
    svtkDendrogramItem::WeightedVertex wv = queue.top();
    queue.pop();
...
//-----------------------------------------------------------------------------
double svtkTanglegramItem::GetPositionScoreForVertex(svtkIdType vertex, svtkTree* tree)
{
  ...
  while (dfsIterator->HasNext())
  {
    ...
    svtkDoubleArray* column =
      svtkArrayDownCast<svtkDoubleArray>(this->Table->GetColumnByName(tree2Name.c_str()));
    ...
    for (svtkIdType row = 0; row < column->GetNumberOfTuples(); ++row)
    {
...
//-----------------------------------------------------------------------------
void svtkTanglegramItem::SetOrientation(int orientation)
{
...
//-----------------------------------------------------------------------------
int svtkTanglegramItem::GetOrientation()
{
  return this->Orientation;
}

//-----------------------------------------------------------------------------
void svtkTanglegramItem::GenerateLookupTable()
{
  this->LookupTable->SetNumberOfTableValues(255);
  this->LookupTable->Build();

  svtkNew<svtkColorSeries> colorSeries;
  colorSeries->SetColorScheme(svtkColorSeries::BREWER_QUALITATIVE_ACCENT);
...
//-----------------------------------------------------------------------------
bool svtkTanglegramItem::MouseDoubleClickEvent(const svtkContextMouseEvent& event)
{
...
//-----------------------------------------------------------------------------
float svtkTanglegramItem::GetTreeLineWidth()
{
  return this->Dendrogram1->GetLineWidth();
}

//-----------------------------------------------------------------------------
void svtkTanglegramItem::SetTreeLineWidth(float width)
{
...
//-----------------------------------------------------------------------------
bool svtkTanglegramItem::Hit(const svtkContextMouseEvent& svtkNotUsed(mouse))
{
...
//-----------------------------------------------------------------------------
void svtkTanglegramItem::PrintSelf(ostream& os, svtkIndent indent)
{
