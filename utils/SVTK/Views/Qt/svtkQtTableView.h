 ...
 * @brief   A SVTK view based on a Qt Table view.
 *
 *
 * svtkQtTableView is a SVTK view using an underlying QTableView.
 *
 * @par Thanks:
...
#ifndef svtkQtTableView_h
#define svtkQtTableView_h
...
class SVTKVIEWSQT_EXPORT svtkQtTableView : public svtkQtView
{
  Q_OBJECT

public:
  static svtkQtTableView* New();
  svtkTypeMacro(svtkQtTableView, svtkQtView);
   ...
   * FIXME: This should be replaced with an Add/Remove column API.
   */
  svtkSetStringMacro(ColumnName);
  svtkGetStringMacro(ColumnName);
  //@}
  ...
  svtkGetMacro(SortSelectionToTop, bool);
  //@}
  ...
  svtkGetMacro(ApplyRowColors, bool);
  //@}
   ...
   */
  virtual void GetSelectedItems(svtkIdTypeArray* arr);

protected:
  svtkQtTableView();
  ~svtkQtTableView() override;
...
private:
  ...
  svtkSmartPointer<svtkAddMembershipArray> AddSelectedColumn;
  svtkSmartPointer<svtkDataObjectToTable> DataObjectToTable;
  svtkSmartPointer<svtkApplyColors> ApplyColors;

  svtkQtTableView(const svtkQtTableView&) = delete;
  void operator=(const svtkQtTableView&) = delete;
};
