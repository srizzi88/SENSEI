r"""protocols is a module that contains a set of SVTK Web related
...
from svtk.svtkWebCore import svtkWebApplication, svtkWebInteractionEvent
...
# =============================================================================
#
# Base class for any SVTK Web based protocol
#
# =============================================================================

class svtkWebProtocol(LinkProtocol):
    ...
    def mapIdToObject(self, id):
        """
        Maps global-id for a svtkObject to the svtkObject instance. May return None if the
        id is not valid.
        """
        id = int(id)
        if id <= 0:
            return None
        return self.getApplication().GetObjectIdMap().GetSVTKObject(id)

    def getGlobalId(self, obj):
        """
        Return the id for a given svtkObject
        """
    ...
    def setActiveView(self, view):
        """
        Set a svtkRenderWindow to be the active one
        """
...
class svtkWebMouseHandler(svtkWebProtocol):
    ...
    def mouseInteraction(self, event):
        ...
        buttons = 0
        if event["buttonLeft"]:
            buttons |= svtkWebInteractionEvent.LEFT_BUTTON;
        if event["buttonMiddle"]:
            buttons |= svtkWebInteractionEvent.MIDDLE_BUTTON;
        if event["buttonRight"]:
            buttons |= svtkWebInteractionEvent.RIGHT_BUTTON;

        modifiers = 0
        if event["shiftKey"]:
            modifiers |= svtkWebInteractionEvent.SHIFT_KEY
        if event["ctrlKey"]:
            modifiers |= svtkWebInteractionEvent.CTRL_KEY
        if event["altKey"]:
            modifiers |= svtkWebInteractionEvent.ALT_KEY
        if event["metaKey"]:
            modifiers |= svtkWebInteractionEvent.META_KEY

        pvevent = svtkWebInteractionEvent()
...
class svtkWebViewPort(svtkWebProtocol):
...
class svtkWebViewPortImageDelivery(svtkWebProtocol):
...
class svtkWebPublishImageDelivery(svtkWebProtocol):
    def __init__(self, decode=True):
        super(svtkWebPublishImageDelivery, self).__init__()
        self.trackingViews = {}
    ...
    def stillRender(self, options):
        ...
        if self.decode:
            ...
            # Convert the svtkUnsignedCharArray into a bytes object, required by Autobahn websockets
...
class svtkWebViewPortGeometryDelivery(svtkWebProtocol):
...
class svtkWebFileBrowser(svtkWebProtocol):
