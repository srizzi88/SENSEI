...
from svtk.svtkCommonCore import svtkTypeUInt32Array
from svtk.svtkFiltersGeometry import svtkCompositeDataGeometryFilter
from svtk.svtkFiltersGeometry import svtkGeometryFilter
from svtk.svtkRenderingCore import svtkColorTransferFunction
...
class SynchronizationContext():
    ...
    def getCachedDataArray(self, pMd5, binary=False, compression=False):
        ...
        if binary:
            # Convert the svtkUnsignedCharArray into a bytes object, required by
            # Autobahn websockets
...
def initializeSerializers():
    # Actors/viewProps
    registerInstanceSerializer('svtkOpenGLActor', genericActorSerializer)
    registerInstanceSerializer('svtkPVLODActor', genericActorSerializer)

    # Mappers
    registerInstanceSerializer(
        'svtkOpenGLPolyDataMapper', genericMapperSerializer)
    registerInstanceSerializer(
        'svtkCompositePolyDataMapper2', genericMapperSerializer)
    registerInstanceSerializer('svtkDataSetMapper', genericMapperSerializer)

    # LookupTables/TransferFunctions
    registerInstanceSerializer('svtkLookupTable', lookupTableSerializer2)
    registerInstanceSerializer(
        'svtkPVDiscretizableColorTransferFunction', colorTransferFunctionSerializer)

    # Textures
    registerInstanceSerializer('svtkOpenGLTexture', textureSerializer)

    # Property
    registerInstanceSerializer('svtkOpenGLProperty', propertySerializer)

    # Datasets
    registerInstanceSerializer('svtkPolyData', polydataSerializer)
    registerInstanceSerializer('svtkImageData', imagedataSerializer)
    registerInstanceSerializer(
        'svtkUnstructuredGrid', mergeToPolydataSerializer)
    registerInstanceSerializer(
        'svtkMultiBlockDataSet', mergeToPolydataSerializer)

    # RenderWindows
    registerInstanceSerializer('svtkCocoaRenderWindow', renderWindowSerializer)
    registerInstanceSerializer(
        'svtkXOpenGLRenderWindow', renderWindowSerializer)
    registerInstanceSerializer(
        'svtkWin32OpenGLRenderWindow', renderWindowSerializer)
    registerInstanceSerializer('svtkEGLRenderWindow', renderWindowSerializer)
    registerInstanceSerializer('svtkOpenVRRenderWindow', renderWindowSerializer)
    registerInstanceSerializer(
        'svtkGenericOpenGLRenderWindow', renderWindowSerializer)
    registerInstanceSerializer(
        'svtkOSOpenGLRenderWindow', renderWindowSerializer)
    registerInstanceSerializer('svtkOpenGLRenderWindow', renderWindowSerializer)
    registerInstanceSerializer('svtkIOSRenderWindow', renderWindowSerializer)
    registerInstanceSerializer(
        'svtkExternalOpenGLRenderWindow', renderWindowSerializer)

    # Renderers
    registerInstanceSerializer('svtkOpenGLRenderer', rendererSerializer)

    # Cameras
    registerInstanceSerializer('svtkOpenGLCamera', cameraSerializer)

    # Lights
    registerInstanceSerializer('svtkPVLight', lightSerializer)
    registerInstanceSerializer('svtkOpenGLLight', lightSerializer)
...
def extractRequiredFields(extractedFields, parent, dataset, context, requestedFields=['Normals', 'TCoords']):
    ...
    if parent.IsA('svtkTexture') and dataset.GetPointData().GetScalars():
        arrayMeta = getArrayDescription(
            dataset.GetPointData().GetScalars(), context)
        arrayMeta['location'] = 'pointData'
        arrayMeta['registration'] = 'setScalars'
...
def mergeToPolydataSerializer(parent, dataObject, dataObjectId, context, depth):
    ...
    if dataObject.IsA('svtkCompositeDataSet'):
        gf = svtkCompositeDataGeometryFilter()
        gf.SetInputData(dataObject)
        gf.Update()
        dataset = gf.GetOutput()
    elif dataObject.IsA('svtkUnstructuredGrid'):
        gf = svtkGeometryFilter()
        gf.SetInputData(dataObject)
        gf.Update()
        dataset = gf.GetOutput()
    else:
    ...
    if len(dependencies) > 1:
        return {
            ...
            'properties': {
                ...
                # These commented properties do not yet have real setters in svtk.js
...
def lightTypeToString(value):
    """
    #define SVTK_LIGHT_TYPE_HEADLIGHT    1
    #define SVTK_LIGHT_TYPE_CAMERA_LIGHT 2
    #define SVTK_LIGHT_TYPE_SCENE_LIGHT  3
