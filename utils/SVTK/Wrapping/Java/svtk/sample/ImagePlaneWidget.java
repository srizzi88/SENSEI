...
import svtk.svtkImagePlaneWidget;
import svtk.svtkNativeLibrary;
import svtk.svtkOutlineFilter;
import svtk.svtkPolyDataMapper;
import svtk.svtkVolume16Reader;
import svtk.svtkAlgorithmOutput;

/**
 * Example of complex 3D widget in use.
 */
public class ImagePlaneWidget extends svtkCanvas {
  ...
  public ImagePlaneWidget(String path) {
    ...
    svtkVolume16Reader v16 = new svtkVolume16Reader();
  ...
  public void setImageData(svtkImageData id, svtkAlgorithmOutput out) {
    // The shared picker enables us to use 3 planes at one time
    // and gets the picking order right
    svtkCellPicker picker = new svtkCellPicker();
    picker.SetTolerance(0.005);

    // The 3 image plane widgets are used to probe the dataset.
    svtkImagePlaneWidget planeWidgetX = new svtkImagePlaneWidget();
    ...
    svtkImagePlaneWidget planeWidgetY = new svtkImagePlaneWidget();
    ...
    svtkImagePlaneWidget planeWidgetZ = new svtkImagePlaneWidget();
    ...
    svtkOutlineFilter outline = new svtkOutlineFilter();
    outline.SetInputConnection(out);

    svtkPolyDataMapper outlineMapper = new svtkPolyDataMapper();
    outlineMapper.SetInputConnection(outline.GetOutputPort());

    svtkActor outlineActor = new svtkActor();
  ...
  static public void printUsage(String err) {
    ...
    System.err.println("      path is location of your SVTKData directory");
    System.exit(-1);
  }

  public static void main(String[] argv) {
    int argSize = argv.length;
    String pathToSVTKData = "";
    int argCurrent = 0;
    ...
    try {
      while (argSize > argCurrent) {
        if (argv[argCurrent].equals("-D")) {
          ++argCurrent;
          pathToSVTKData = argv[argCurrent];
          ++argCurrent;
        } else {
    ...
    if (pathToSVTKData.equals(""))
      ImagePlaneWidget.printUsage("");

    final File f = new File(pathToSVTKData + "/Data/headsq");
