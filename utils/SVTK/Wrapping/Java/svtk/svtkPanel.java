...
/**
 * Java AWT component that encapsulate svtkRenderWindow, svtkRenderer, svtkCamera,
 * svtkLight.
 *
 * If a svtkInteractor is needed, use svtkCanvas instead. This is necessary when
 * Widget and Picker are used.
 *
 * @author Kitware
 */
public class svtkPanel extends Canvas implements MouseListener, MouseMotionListener, MouseWheelListener, KeyListener {
  private static final long serialVersionUID = 1L;
  protected svtkRenderWindow rw = new svtkRenderWindow();
  protected svtkRenderer ren = new svtkRenderer();
  protected svtkCamera cam = null;
  protected svtkLight lgt = new svtkLight();
  protected int lastX;
  protected int lastY;
  ...
  public void Delete() {
    ...
    // Free internal SVTK objects
    ren = null;
    cam = null;
    lgt = null;
    // On linux we prefer to have a memory leak instead of a crash
    if(!rw.GetClassName().equals("svtkXOpenGLRenderWindow")) {
      rw = null;
    } else {
  ...
  protected native int RenderCreate(svtkRenderWindow id0);
  ...
  public svtkPanel(svtkRenderWindow renwin) {
    rw = renwin;
    rw.AddRenderer(ren);
    addMouseListener(this);
    addMouseMotionListener(this);
  ...
  public void HardCopy(String filename, int mag) {
    ...
    svtkWindowToImageFilter w2if = new svtkWindowToImageFilter();
    w2if.SetInput(rw);

    w2if.SetScale(mag);
    w2if.Update();

    svtkTIFFWriter writer = new svtkTIFFWriter();
  ...
  public void pickActor(int x, int y) {
    ...
    svtkPropPicker picker = new svtkPropPicker();
