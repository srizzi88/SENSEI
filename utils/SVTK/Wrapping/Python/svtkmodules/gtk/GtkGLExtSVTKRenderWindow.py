...
Description:
  ...
  This provides a SVTK widget for pyGtk.  This embeds a svtkRenderWindow
  inside a GTK widget.  This is based on GtkSVTKRenderWindow.py.
...
Credits:
  ...
  this code based on SVTK's GtkSVTKRenderWindow.py and extended it to
...
import gtk.gtkgl
from gtk import gdk
from svtkmodules.svtkRenderingCore import svtkActor, svtkCellPicker, svtkProperty, svtkRenderWindow


class GtkGLExtSVTKRenderWindowBase(gtk.gtkgl.DrawingArea):
...
class GtkGLExtSVTKRenderWindow(GtkGLExtSVTKRenderWindowBase):

    """ An example of a fully functional GtkGLExtSVTKRenderWindow that
    is based on the svtkRenderWidget.py provided with the SVTK
    sources."""

    def __init__(self, *args):
        GtkGLExtSVTKRenderWindowBase.__init__(self)
    ...
    def Render(self):
        ...
        GtkGLExtSVTKRenderWindowBase.Render(self)
...
def main():
    ...
    # The main window
    window = gtk.Window()
    window.set_title("A GtkGLExtSVTKRenderWindow Demo!")
    window.connect("destroy", gtk.mainquit)
    window.connect("delete_event", gtk.mainquit)
    window.set_border_width(10)

    svtkgtk = GtkGLExtSVTKRenderWindow()
    svtkgtk.show()

    vbox = gtk.VBox(spacing=3)
    vbox.show()
    vbox.pack_start(svtkgtk)
    ...
    ren = svtkRenderer()
    svtkgtk.GetRenderWindow().AddRenderer(ren)
