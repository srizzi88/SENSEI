...
Description:
  ...
  Provides a pyGtk svtkRenderWindowInteractor widget.  This embeds a
  svtkRenderWindow inside a GTK widget and uses the
  svtkGenericRenderWindowInteractor for the event handling.  This is
  similar to GtkSVTKRenderWindowInteractor.py.
...
class GtkGLExtSVTKRenderWindowInteractor(gtk.gtkgl.DrawingArea):

    """ Embeds a svtkRenderWindow into a pyGTK widget and uses
    svtkGenericRenderWindowInteractor for the event handling.  This
...
def main():
    ...
    gsvtk = GtkGLExtSVTKRenderWindowInteractor()
    #gsvtk.SetDesiredUpdateRate(1000)
    gsvtk.set_size_request(400, 400)
    vbox.pack_start(gsvtk)
    gsvtk.show()
    gsvtk.Initialize()
    gsvtk.Start()
    # prevents 'q' from exiting the app.
    gsvtk.AddObserver("ExitEvent", lambda o,e,x=None: x)
