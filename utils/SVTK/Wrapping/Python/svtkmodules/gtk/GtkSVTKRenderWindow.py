...
Description:
  ...
  Provides a simple SVTK widget for pyGtk.  This embeds a
  svtkRenderWindow inside a GTK widget.  This is based on
  svtkTkRenderWidget.py.  The GtkSVTKRenderWindowBase class provides the
  abstraction necessary for someone to use their own interaction
  behaviour.  The method names are similar to those in
  svtkInteractorStyle.h.
...
Bugs:
  ...
  GtkSVTKRenderWindow I grab the focus but I don't know what to do when
...
class GtkSVTKRenderWindowBase(gtkgl.GtkGLArea):

    """ A base class that enables one to embed a svtkRenderWindow into
    ...
    svtkInteractorStyle class included with SVTK. """
    ...
    def OnEnter(self, wid, event):
        """Entering the svtkRenderWindow."""
        return gtk.TRUE

    def OnLeave(self, wid, event):
        """Leaving the svtkRenderWindow."""
        return gtk.TRUE
...
class GtkSVTKRenderWindow(GtkSVTKRenderWindowBase):

    """ An example of a fully functional GtkSVTKRenderWindow that is
    based on the svtkRenderWidget.py provided with the SVTK sources."""

    def __init__(self, *args):
        l = list(args)
        l.insert(0, self)
        apply(GtkSVTKRenderWindowBase.__init__, l)
    ...
    def Render(self):
        ...
        GtkSVTKRenderWindowBase.Render(self)
...
def main():
    ...
    window.set_title("A GtkSVTKRenderWindow Demo!")
    ...
    # The GtkSVTKRenderWindow
    gsvtk = GtkSVTKRenderWindow()
    #gsvtk.SetDesiredUpdateRate(1000)
    gsvtk.set_usize(400, 400)
    vbox.pack_start(gsvtk)
    gsvtk.show()
    ...
    #coneActor = svtkLODActor()
    coneActor = svtkActor()
    coneActor.SetMapper(coneMapper)
    coneActor.GetProperty().SetColor(0.5, 0.5, 1.0)
    ren = svtkRenderer()
    gsvtk.GetRenderWindow().AddRenderer(ren)
