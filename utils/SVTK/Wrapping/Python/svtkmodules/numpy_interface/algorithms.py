...
data is partitioned according to SVTK data parallel execution
...
import itertools
try:
    from ..svtkParallelCore import svtkMultiProcessController
    from ..svtkParallelMPI4Py import svtkMPI4PyCommunicator
except ImportError:
    svtkMultiProcessController = None
    svtkMPI4PyCommunicator = None
...
def _apply_func2(func, array, args):
    """Apply a function to each member of a SVTKCompositeDataArray.
...
def apply_ufunc(func, array, args=()):
    """Apply a function to each member of a SVTKCompositeDataArray.
    SVTKArray and numpy arrays are also supported."""
    if array is dsa.NoneArray:
        return dsa.NoneArray
    elif type(array) == dsa.SVTKCompositeDataArray:
        return dsa.SVTKCompositeDataArray(_apply_func2(func, array, args), dataset = array.DataSet)
    else:
        return func(array)

def _make_ufunc(ufunc):
    """ Given a ufunc, creates a closure that applies it to each member
    of a SVTKCompositeDataArray.
...
def apply_dfunc(dfunc, array1, val2):
    """Apply a two argument function to each member of a SVTKCompositeDataArray
    and another argument The second argument can be a SVTKCompositeDataArray, in
    ...
    SVTKArray and numpy arrays are also supported."""
    if type(array1) == dsa.SVTKCompositeDataArray and type(val2) == dsa.SVTKCompositeDataArray:
        res = []
        ...
        return dsa.SVTKCompositeDataArray(res, dataset = array1.DataSet)
    elif type(array1) == dsa.SVTKCompositeDataArray:
        res = []
...
def _make_dfunc(dfunc):
    ...
    applies it to each member of a SVTKCompositeDataArray.
...
def _make_dsfunc(dsfunc):
    ...
    creates a closure that applies it to each member of a SVTKCompositeDataArray.
    Note that this function is mainly for internal use by this module."""
    def new_dsfunc(array, ds=None):
        if type(array) == dsa.SVTKCompositeDataArray:
            res = []
            for a in array.Arrays:
                if a is dsa.NoneArray:
                    res.append(dsa.NoneArray)
            ...
            return dsa.SVTKCompositeDataArray(res, dataset = array.DataSet)
...
def _make_dsfunc2(dsfunc):
    ...
    def new_dsfunc2(ds):
        if type(ds) == dsa.CompositeDataSet:
            ...
            return dsa.SVTKCompositeDataArray(res, dataset = ds)
        else:
            return dsfunc(ds)
    return new_dsfunc2
...
def _global_func(impl, array, axis, controller):
    ...
    if axis is None or axis == 0:
        if controller is None and svtkMultiProcessController is not None:
            controller = svtkMultiProcessController.GetGlobalController()
        if controller and controller.IsA("svtkMPIController") and controller.GetNumberOfProcesses() > 1:
            try:
                from mpi4py import MPI
            except ImportError:
                raise RuntimeError('MPI4Py is required to perform multi-rank operations')
            comm = svtkMPI4PyCommunicator.ConvertToPython(controller.GetCommunicator())
...
def bitwise_or(array1, array2):
    ...
    if type(array1) == dsa.SVTKCompositeDataArray and type(array2) == dsa.SVTKCompositeDataArray:
        res = []
...
def make_point_mask_from_NaNs(dataset, array):
    ...
    have a corresponding value of svtkmodules.svtkCommonDataModel.svtkDataSetAttributes.HIDDENPOINT.
    These values are also combined with any ghost values that the
    dataset may have."""
    from ..svtkCommonDataModel import svtkDataSetAttributes
    ghosts = dataset.PointData[svtkDataSetAttributes.GhostArrayName()]
    return make_mask_from_NaNs(array, ghosts)

def make_cell_mask_from_NaNs(dataset, array):
    """This method will create a ghost array corresponding to an
    input with NaN values. For each NaN value, the output array will
    have a corresponding value of svtkmodules.svtkCommonDataModel.svtkDataSetAttributes.HIDDENCELL.
    These values are also combined with any ghost values that the
    dataset may have."""
    from ..svtkCommonDataModel import svtkDataSetAttributes
    ghosts = dataset.CellData[svtkDataSetAttributes.GhostArrayName()]
    return make_mask_from_NaNs(array, ghosts, True)

def make_mask_from_NaNs(array, ghost_array=dsa.NoneArray, is_cell=False):
    """This method will create a ghost array corresponding to an
    input with NaN values. For each NaN value, the output array will
    have a corresponding value of svtkmodules.svtkCommonDataModel.svtkDataSetAttributes.HIDDENPOINT or
    ...
    from ..svtkCommonDataModel import svtkDataSetAttributes
    if is_cell:
        mask_value = svtkDataSetAttributes.HIDDENCELL
    else:
        mask_value = svtkDataSetAttributes.HIDDENPOINT
...
def sum(array, axis=None, controller=None):
    ...
    * axis=1: Sum values of all components of each tuple and return an
    ...
    sum(array, controller=svtkmodules.svtkParallelCore.svtkDummyController()).
    """
    class SumImpl:
        def op(self):
            return algs.sum
...
def max(array, axis=None, controller=None):
    ...
    * axis=1: Return the max values of all components of each tuple
    ...
    max(array, controller=svtkmodules.svtkParallelCore.svtkDummyController()).
    """
    class MaxImpl:
        def op(self):
            return algs.max
...
def min(array, axis=None, controller=None):
    ...
    * axis=1: Return the min values of all components of each tuple and
    ...
    min(array, controller=svtkmodules.svtkParallelCore.svtkDummyController()).
    """
    class MinImpl:
        def op(self):
            return algs.min
...
def _global_per_block(impl, array, axis=None, controller=None):
    ...
    t = type(array)
    if t == dsa.SVTKArray or t == numpy.ndarray:
        from ..svtkCommonDataModel import svtkMultiBlockDataSet
        array = dsa.SVTKCompositeDataArray([array])
        ds = svtkMultiBlockDataSet()
        ds.SetBlock(0, dataset.SVTKObject)
        dataset = ds
    ...
    if controller is None and svtkMultiProcessController is not None:
        controller = svtkMultiProcessController.GetGlobalController()
    if controller and controller.IsA("svtkMPIController"):
        from mpi4py import MPI
        comm = svtkMPI4PyCommunicator.ConvertToPython(controller.GetCommunicator())
        ...
        if numpy.all(id_count <= 1):
            return dsa.SVTKCompositeDataArray(results, dataset=dataset)
        ...
        for i in range(to_reduce):
            ...
            if success:
                if size == 1:
                    results[loc] = dsa.SVTKArray(rresults[i])
                else:
...
def sum_per_block(array, axis=None, controller=None):
    ...
    each block of an SVTKCompositeDataArray.
    ...
    * axis=1: Sum values of all components of each tuple and return an
    ...
    sum_per_block(array, controller=svtkmodules.svtkParallelCore.svtkDummyController()).
    """
...
def count_per_block(array, axis=None, controller=None):
    """Return the number of elements of each block in a SVTKCompositeDataArray
    along an axis.
...
def mean_per_block(array, axis=None, controller=None):
    ...
    for each block of a SVTKCompositeDataArray.
    ...
    * axis=1: Return the mean values of all components of each tuple and
    ...
    mean(array, controller=svtkmodules.svtkParallelCore.svtkDummyController()).
    """
    if axis is None or axis == 0:
...
def max_per_block(array, axis=None, controller=None):
    ...
    * axis=1: Return the max values of all components of each tuple and return
    ...
    max_per_block(array, controller=svtkmodules.svtkParallelCore.svtkDummyController()).
    """
...
def min_per_block(array, axis=None, controller=None):
    ...
    * axis=1: Return the min values of all components of each tuple and
    ...
    min_per_block(array, controller=svtkmodules.svtkParallelCore.svtkDummyController()).
    """
...
def var(array, axis=None, controller=None):
    ...
    * axis=1: Return the variance values of all components of each tuple and
    ...
    var(array, controller=svtkmodules.svtkParallelCore.svtkDummyController()).
    """
...
def std(array, axis=None, controller=None):
    ...
    * axis=1: Return the standard deviation values of all components of
    ...
    std(array, controller=svtkmodules.svtkParallelCore.svtkDummyController()).
    """
...
def make_vector(arrayx, arrayy, arrayz=None):
    ...
    if type(arrayx) == dsa.SVTKCompositeDataArray and type(arrayy) == dsa.SVTKCompositeDataArray and (type(arrayz) == dsa.SVTKCompositeDataArray or arrayz is None):
        res = []
        if arrayz is None:
            ...
            for ax, ay, az in izip(arrayx.Arrays, arrayy.Arrays, arrayz.Arrays):
                if ax is not dsa.NoneArray and ay is not dsa.NoneArray and az is not dsa.NoneArray:
                    ...
                    res.append(dsa.NoneArray)
        return dsa.SVTKCompositeDataArray(res, dataset = arrayx.DataSet)
    else:
        return algs.make_vector(arrayx, arrayy, arrayz)

def unstructured_from_composite_arrays(points, arrays, controller=None):
    """Given a set of SVTKCompositeDataArrays, creates a svtkUnstructuredGrid.
    ...
    from ..svtkCommonDataModel import svtkUnstructuredGrid
    from ..svtkCommonCore import svtkDoubleArray, svtkPoints
    ugrid = svtkUnstructuredGrid()
    da = svtkDoubleArray()
...
position in the array shape. In SVTK, this function's main use is to
...
SVTKArray([[ 1.,  1.,  1.],
       ...
       [ 1.,  1.,  1.]])
>>> algs.mag(v)
SVTKArray([ 1.73205081,  1.73205081,  1.73205081,  1.73205081,  1.73205081])
>>> v / algs.mag(v)
...
SVTKArray([[ 0.57735027,  0.57735027,  0.57735027],
