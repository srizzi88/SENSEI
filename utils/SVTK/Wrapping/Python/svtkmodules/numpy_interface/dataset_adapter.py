...
to SVTK datasets and arrays. This is best described with some examples.

To normalize a SVTK array:

from svtkmodules.svtkImagingCore svtkRTAnalyticSource
import svtkmodules.numpy_interface.dataset_adapter as dsa
import svtkmodules.numpy_interface.algorithms as algs

rt = svtkRTAnalyticSource()
rt.Update()
...
>>> grad[0:10]
SVTKArray([[ 0.10729134,  0.03763443,  0.03136338],
...
>>> grad[:, 0]
SVTKArray([ 0.10729134,  0.02754352,  0.02248248, ..., -0.02748174,
...
We have implemented a SVTKCompositeDataArray class that handles many
...
from svtkmodules.svtkCommonDataModel import svtkMultiBlockDataSet
mb = svtkMultiBlockDataSet()
mb.SetBlock(0, image.SVTKObject)
mb.SetBlock(1e, image.SVTKObject)
...
import operator
import sys
from ..svtkCommonCore import buffer_shared
from ..util import numpy_support
from ..svtkCommonDataModel import svtkDataObject
from ..svtkCommonCore import svtkWeakReference
import weakref
...
class ArrayAssociation :
    """Easy access to svtkDataObject.AttributeTypes"""
    POINT = svtkDataObject.POINT
    CELL  = svtkDataObject.CELL
    FIELD = svtkDataObject.FIELD
    ROW = svtkDataObject.ROW

class SVTKObjectWrapper(object):
    """Superclass for classes that wrap SVTK objects with Python objects.
    This class holds a reference to the wrapped SVTK object. It also
    forwards unresolved methods to the underlying object by overloading
    __get__attr."""
    def __init__(self, svtkobject):
        self.SVTKObject = svtkobject

    def __getattr__(self, name):
        "Forwards unknown attribute requests to SVTK object."
        return getattr(self.SVTKObject, name)

def svtkDataArrayToSVTKArray(array, dataset=None):
    "Given a svtkDataArray and a dataset owning it, returns a SVTKArray."
    narray = numpy_support.svtk_to_numpy(array)

    # Make arrays of 9 components into matrices. Also transpose
    # as SVTK store matrices in Fortran order
    shape = narray.shape
    if len(shape) == 2 and shape[1] == 9:
        narray = narray.reshape((shape[0], 3, 3)).transpose(0, 2, 1)

    return SVTKArray(narray, array=array, dataset=dataset)

def numpyTosvtkDataArray(array, name="numpy_array", array_type=None):
    """Given a numpy array or a SVTKArray and a name, returns a svtkDataArray.
    The resulting svtkDataArray will store a reference to the numpy array:
    the numpy array is released only when the svtkDataArray is destroyed."""
    svtkarray = numpy_support.numpy_to_svtk(array, array_type=array_type)
    svtkarray.SetName(name)
    return svtkarray
    ...
    def __new__(mcs, name, parent, attr):
        ...
        def add_numeric_op(attr_name):
            ...
            def closure(self, other):
                return SVTKArray._numeric_op(self, other, attr_name)
        ...
        def add_reverse_numeric_op(attr_name):
            ...
            def closure(self, other):
                return SVTKArray._reverse_numeric_op(self, other, attr_name)
            closure.__name__ = attr_name
            attr[attr_name] = closure
...
@_metaclass(SVTKArrayMetaClass)
class SVTKArray(numpy.ndarray):
    """This is a sub-class of numpy ndarray that stores a
    reference to a svtk array as well as the owning dataset.
    The numpy array and svtk array should point to the same
    memory location."""
    ...
    def __new__(cls, input_array, array=None, dataset=None):
        ...
        obj.SVTKObject = array
        if dataset:
            obj._dataset = svtkWeakReference()
            obj._dataset.Set(dataset.SVTKObject)
        # Finally, we must return the newly created object:
        return obj

    def __array_finalize__(self,obj):
        # Copy the SVTK array only if the two share data
        slf = _make_tensor_array_contiguous(self)
        obj2 = _make_tensor_array_contiguous(obj)

        self.SVTKObject = None
        try:
            ...
            if buffer_shared(slf, obj2):
                self.SVTKObject = getattr(obj, 'SVTKObject', None)
        except TypeError:
            pass
    ...
    def __getattr__(self, name):
        "Forwards unknown attribute requests to SVTK array."
        try:
            o = self.__dict__["SVTKObject"]
        except KeyError:
            o = None
        if o is None:
    ...
    def DataSet(self):
        ...
        dataset is held through a svtkWeakReference to ensure it doesn't prevent
    ...
    def DataSet(self, dataset):
        ...
        through a svtkWeakReference.
        """
        # Do we have dataset to store
        if dataset and dataset.SVTKObject:
            # Do we need to create a svtkWeakReference
            if not hasattr(self, '_dataset') or self._dataset is None:
                self._dataset = svtkWeakReference()

            self._dataset.Set(dataset.SVTKObject)
        else:
            self._dataset = None

class SVTKNoneArrayMetaClass(type):
    def __new__(mcs, name, parent, attr):
        ...
        def _add_op(attr_name, op):
            ...
            _numeric_op(self, other, op)."""
            def closure(self, other):
                return SVTKNoneArray._op(self, other, op)
            closure.__name__ = attr_name
            attr[attr_name] = closure
...
@_metaclass(SVTKNoneArrayMetaClass)
class SVTKNoneArray(object):
    """SVTKNoneArray is used to represent a "void" array. An instance
...
NoneArray = SVTKNoneArray()

class SVTKCompositeDataArrayMetaClass(type):
    def __new__(mcs, name, parent, attr):
        """Simplify the implementation of the numeric/logical sequence API."""
        def add_numeric_op(attr_name, op):
            ...
            _numeric_op(self, other, op)."""
            def closure(self, other):
                return SVTKCompositeDataArray._numeric_op(self, other, op)
            closure.__name__ = attr_name
            attr[attr_name] = closure
        ...
        def add_reverse_numeric_op(attr_name, op):
            ...
            _reverse_numeric_op(self, other, op)."""
            def closure(self, other):
                return SVTKCompositeDataArray._reverse_numeric_op(self, other, op)
            closure.__name__ = attr_name
            attr[attr_name] = closure
...
@_metaclass(SVTKCompositeDataArrayMetaClass)
class SVTKCompositeDataArray(object):
    ...
    nothing but a collection of svtkDataArrays. A SVTKCompositeDataArray
    ...
    SVTK modules should be used to process composite arrays.
    """
    ...
    def GetArrays(self):
        """Returns the internal container of SVTKArrays. If necessary,
    ...
    def __getitem__(self, index):
        """Overwritten to refer indexing to underlying SVTKArrays.
        ...
        self.__init_from_composite()
        res = []
        if type(index) == SVTKCompositeDataArray:
            ...
            for a in self._Arrays:
                if a is not NoneArray:
                    ...
                    res.append(NoneArray)
        return SVTKCompositeDataArray(res, dataset=self.DataSet)
    ...
    def _numeric_op(self, other, op):
        ...
        self.__init_from_composite()
        res = []
        if type(other) == SVTKCompositeDataArray:
...
class DataSetAttributes(SVTKObjectWrapper):
    """This is a python friendly wrapper of svtkDataSetAttributes. It
    returns SVTKArrays. It also provides the dictionary interface."""

    def __init__(self, svtkobject, dataset, association):
        super(DataSetAttributes, self).__init__(svtkobject)
        # import weakref
    ...
    def GetArray(self, idx):
        "Given an index or name, returns a SVTKArray."
        if isinstance(idx, int) and idx >= self.SVTKObject.GetNumberOfArrays():
            raise IndexError("array index out of range")
        svtkarray = self.SVTKObject.GetArray(idx)
        if not svtkarray:
            svtkarray = self.SVTKObject.GetAbstractArray(idx)
            if svtkarray:
                return svtkarray
            return NoneArray
        array = svtkDataArrayToSVTKArray(svtkarray, self.DataSet)
    ...
    def keys(self):
        ...
        kys = []
        narrays = self.SVTKObject.GetNumberOfArrays()
        for i in range(narrays):
            name = self.SVTKObject.GetAbstractArray(i).GetName()
            if name:
                kys.append(name)
        return kys
    ...
    def values(self):
        ...
        narrays = self.SVTKObject.GetNumberOfArrays()
        for i in range(narrays):
            a = self.SVTKObject.GetAbstractArray(i)
            if a.GetName():
                vals.append(a)
        return vals

    def PassData(self, other):
        "A wrapper for svtkDataSet.PassData."
        try:
            self.SVTKObject.PassData(other)
        except TypeError:
            self.SVTKObject.PassData(other.SVTKObject)
    ...
    def append(self, narray, name):
        ...
        if len(shape) == 3:
            ...
            # If column order (c order), transpose. SVTK wants row order (fortran
        ...
        copy = SVTKArray(narray)
        try:
            copy.SVTKObject = narray.SVTKObject
        except AttributeError: pass
        arr = numpyTosvtkDataArray(copy, name)
        self.SVTKObject.AddArray(arr)


class CompositeDataSetAttributes():
    """This is a python friendly wrapper for svtkDataSetAttributes for composite
    ...
    def append(self, narray, name):
        ...
        added = False
        if not isinstance(narray, SVTKCompositeDataArray): # Scalar input
            for ds in self.DataSet:
...
class CompositeDataIterator(object):
    """Wrapper for a svtkCompositeDataIterator class to satisfy
       the python iterator protocol. This iterator iterates
       over non-empty leaf nodes. To iterate over empty or
       non-leaf nodes, use the svtkCompositeDataIterator directly.
       """
    ...
    def __getattr__(self, name):
        """Returns attributes from the svtkCompositeDataIterator."""
...
class DataObject(SVTKObjectWrapper):
    """A wrapper for svtkDataObject that makes it easier to access FielData
    arrays as SVTKArrays
    """
    ...
    def GetAttributes(self, type):
        ...
        if type == ArrayAssociation.FIELD:
            return DataSetAttributes(self.SVTKObject.GetFieldData(), self, type)
        return DataSetAttributes(self.SVTKObject.GetAttributes(type), self, type)

    def GetFieldData(self):
        "Returns the field data as a DataSetAttributes instance."
        return DataSetAttributes(self.SVTKObject.GetFieldData(), self, ArrayAssociation.FIELD)

    FieldData = property(GetFieldData, None, None, "This property returns the field data of a data object.")

class Table(DataObject):
    """A wrapper for svtkFielData that makes it easier to access RowData array as
    SVTKArrays
    """
    def GetRowData(self):
...
class CompositeDataSet(DataObject):
    """A wrapper for svtkCompositeData and subclasses that makes it easier
    to access Point/Cell/Field data as SVTKCompositeDataArrays. It also
    provides a Python type iterator."""

    def __init__(self, svtkobject):
        DataObject.__init__(self, svtkobject)
    ...
    def GetPoints(self):
        "Returns the points as a SVTKCompositeDataArray instance."
        if self._Points is None or self._Points() is None:
            ...
            if len(pts) == 0 or all([a is NoneArray for a in pts]):
                ...
                cpts = SVTKCompositeDataArray(pts, dataset=self)
...
class DataSet(DataObject):
    """This is a python friendly wrapper of a svtkDataSet that defines
...
class PointSet(DataSet):
    """This is a python friendly wrapper of a svtkPointSet that defines
    a few useful properties."""
    def GetPoints(self):
        """Returns the points as a SVTKArray instance. Returns None if the
        dataset has implicit points."""
        if not self.SVTKObject.GetPoints():
            return None
        array = svtkDataArrayToSVTKArray(
            self.SVTKObject.GetPoints().GetData(), self)
        array.Association = ArrayAssociation.POINT
        return array

    def SetPoints(self, pts):
        """Given a SVTKArray instance, sets the points of the dataset."""
        from ..svtkCommonCore import svtkPoints
        if isinstance(pts, svtkPoints):
            p = pts
        else:
            pts = numpyTosvtkDataArray(pts)
            p = svtkPoints()
            p.SetData(pts)
        self.SVTKObject.SetPoints(p)

    Points = property(GetPoints, SetPoints, None, "This property returns the point coordinates of dataset.")

class PolyData(PointSet):
    """This is a python friendly wrapper of a svtkPolyData that defines
    a few useful properties."""

    def GetPolygons(self):
        """Returns the polys as a SVTKArray instance."""
        if not self.SVTKObject.GetPolys():
            return None
        return svtkDataArrayToSVTKArray(
            self.SVTKObject.GetPolys().GetData(), self)
...
class UnstructuredGrid(PointSet):
    """This is a python friendly wrapper of a svtkUnstructuredGrid that defines
    a few useful properties."""

    def GetCellTypes(self):
        """Returns the cell types as a SVTKArray instance."""
        if not self.SVTKObject.GetCellTypesArray():
            return None
        return svtkDataArrayToSVTKArray(
            self.SVTKObject.GetCellTypesArray(), self)

    def GetCellLocations(self):
        """Returns the cell locations as a SVTKArray instance."""
        if not self.SVTKObject.GetCellLocationsArray():
            return None
        return svtkDataArrayToSVTKArray(
            self.SVTKObject.GetCellLocationsArray(), self)

    def GetCells(self):
        """Returns the cells as a SVTKArray instance."""
        if not self.SVTKObject.GetCells():
            return None
        return svtkDataArrayToSVTKArray(
            self.SVTKObject.GetCells().GetData(), self)

    def SetCells(self, cellTypes, cellLocations, cells):
        """Given cellTypes, cellLocations, cells as SVTKArrays,
        populates the unstructured grid data structures."""
        from ..util.svtkConstants import SVTK_ID_TYPE
        from ..svtkCommonDataModel import svtkCellArray
        cellTypes = numpyTosvtkDataArray(cellTypes)
        cellLocations = numpyTosvtkDataArray(cellLocations, array_type=SVTK_ID_TYPE)
        cells = numpyTosvtkDataArray(cells, array_type=SVTK_ID_TYPE)
        ca = svtkCellArray()
        ca.SetCells(cellTypes.GetNumberOfTuples(), cells)
        self.SVTKObject.SetCells(cellTypes, cellLocations, ca)
...
class Graph(DataObject):
    """This is a python friendly wrapper of a svtkGraph that defines
    a few useful properties."""
...
class Molecule(DataObject):
    """This is a python friendly wrapper of a svtkMolecule that defines
    a few useful properties."""
...
def WrapDataObject(ds):
    """Returns a Numpy friendly wrapper of a svtkDataObject."""
    if ds.IsA("svtkPolyData"):
        return PolyData(ds)
    elif ds.IsA("svtkUnstructuredGrid"):
        return UnstructuredGrid(ds)
    elif ds.IsA("svtkPointSet"):
        return PointSet(ds)
    elif ds.IsA("svtkDataSet"):
        return DataSet(ds)
    elif ds.IsA("svtkCompositeDataSet"):
        return CompositeDataSet(ds)
    elif ds.IsA("svtkTable"):
        return Table(ds)
    elif ds.IsA("svtkMolecule"):
        return Molecule(ds)
    elif ds.IsA("svtkGraph"):
        return Table(ds)
    elif ds.IsA("svtkDataObject"):
