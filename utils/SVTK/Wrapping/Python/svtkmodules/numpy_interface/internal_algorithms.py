...
import numpy
from svtkmodules.util import numpy_support
from svtkmodules.svtkCommonDataModel import svtkImageData
from svtkmodules.svtkFiltersCore import svtkCellDataToPointData, svtkPolyDataNormals
from svtkmodules.svtkFiltersGeneral import svtkCellDerivatives
from svtkmodules.svtkFiltersVerdict import svtkCellSizeFilter, svtkCellQuality, svtkMatrixMathFilter
...
def _cell_derivatives (narray, dataset, attribute_type, filter):
    ...
    # numpy_to_svtk converts only contiguous arrays
    if not narray.flags.contiguous : narray = narray.copy()
    varray = numpy_support.numpy_to_svtk(narray)
    ...
    ds.UnRegister(None)
    ds.CopyStructure(dataset.SVTKObject)
...
def _cell_quality (dataset, quality) :
    ...
    ans = dsa.svtkDataArrayToSVTKArray(varray, dataset)

    # The association information has been lost over the svtk filter
...
def _matrix_math_filter (narray, operation) :
    ...
    # Reshape is necessary because numpy_support.numpy_to_svtk only works with 2D or
    # less arrays.
    ...
    ans = dsa.svtkDataArrayToSVTKArray(varray)
...
def curl (narray, dataset=None):
    ...
    ans = dsa.svtkDataArrayToSVTKArray(retVal, dataset)
...
def dot (a1, a2):
    ...
    m = a1*a2
    va = dsa.SVTKArray(numpy.add.reduce(m, 1))
...
def make_vector(ax, ay, az=None):
    ...
    v = numpy.vstack([ax, ay, az]).transpose().view(dsa.SVTKArray)
