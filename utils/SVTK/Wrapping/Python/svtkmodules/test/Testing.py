""" This module attempts to make it easy to create SVTK-Python
...
This SVTK-Python test module supports image based tests with multiple
...
compareImageWithSavedImage(src_img, img_fname, threshold=0.15):
   Compares given source image (in the form of a svtkImageData) with
...
from svtkmodules.svtkCommonCore import svtkCommand, svtkDebugLeaks
from svtkmodules.svtkCommonSystem import svtkTimerLog
from svtkmodules.svtkIOImage import svtkPNGReader, svtkPNGWriter
from svtkmodules.svtkImagingCore import svtkImageDifference, svtkImageShiftScale
from svtkmodules.svtkRenderingCore import svtkWindowToImageFilter
from . import BlackBox

# location of the SVTK data files.  Set via command line args or
...
# location of the SVTK baseline images.  Set via command line args or
# environment variable.
SVTK_BASELINE_ROOT = ""

# location of the SVTK difference images for failed tests.  Set via
...
class svtkTest(unittest.TestCase):
    """A simple default SVTK test class that defines a few useful
    ...
    generating SVTK pipelines you should create the pipeline in the
    ...
    # Avoid the exit-with-error in svtkDebugLeaks.
    dl = svtkDebugLeaks()
    dl.SetExitError(0)
    dl = None
    ...
    def _testParse(self, obj):
        ...
        its various methods using svtkMethodParser.  This is a useful
        test because it gets all the methods of the svtkObject, parses
    ...
    def pathToData(self, filename):
        ...
        """
        global SVTK_DATA_PATHS
        if not filename:
            return SVTK_DATA_PATHS
        for path in SVTK_DATA_PATHS:
    ...
    def pathToValidatedOutput(self, filename):
        ...
        the JUST_VALID option for your test in CMake.
        """
        global SVTK_BASELINE_PATHS
        if not filename:
            return SVTK_BASELINE_PATHS
        for path in SVTK_BASELINE_PATHS:
            if filename == os.path.split(path)[-1]:
                return path
        return filename
    ...
    def prepareTestImage(self, interactor, **kwargs):
        ...
        interactor.AddObserver(svtkCommand.KeyPressEvent, onKeyPress)
        interactor.AddObserver(svtkCommand.LeftButtonPressEvent, onButton)
        interactor.AddObserver(svtkCommand.LeftButtonReleaseEvent, onButton)
        interactor.AddObserver(svtkCommand.MouseMoveEvent, onMovement)
        interactor.Start()
        rw = interactor.GetRenderWindow()
        baseline = 'baselineFilename'
        if 'filename' in kwargs:
            # Render an image and save it to the given filename
            w2if = svtkWindowToImageFilter()
        ...
        snippet above) referencing the svtkRenderWindow and svtkCamera, respectively.
...
def getAbsImagePath(img_basename):
    ...
    name."""
    global SVTK_BASELINE_ROOT
    return os.path.join(SVTK_BASELINE_ROOT, img_basename)

def _getTempImagePath(img_fname):
    x = os.path.join(SVTK_TEMP_DIR, os.path.split(img_fname)[1])
    return os.path.abspath(x)

def compareImageWithSavedImage(src_img, img_fname, threshold=0.15):
    """Compares a source image (src_img, which is a svtkImageData) with
...
def compareImage(renwin, img_fname, threshold=0.15):
    """Compares renwin's (a svtkRenderWindow) contents with the image
...
def main(cases):
    ...
    main ([(svtkTestClass, 'test'), (svtkTestClass1, 'test')])
    """
...
def test(cases):
    ...
    Example:
      ...
      test = test_suite([(svtkTestClass, 'test'),
                        (svtkTestClass1, 'test')])
    """
...
def usage():
    ...
    -D /path/to/SVTKData
    --data-dir /path/to/SVTKData

          Directory containing SVTK Data use for tests.  If this option
          is not set via the command line the environment variable
          SVTK_DATA_ROOT is used.  If the environment variable is not
          set the value defaults to '../../../../../SVTKData'.
    ...
    --baseline-root /path/to/valid/image_dir/
          ...
          line the environment variable SVTK_BASELINE_ROOT is used.  If
    ...
    --temp-dir /path/to/valid/temporary_dir/
          ...
          the environment variable SVTK_TEMP_DIR is used.  If the
...
def processCmdLine():
    ...
    global SVTK_DATA_ROOT, SVTK_BASELINE_ROOT, SVTK_TEMP_DIR, SVTK_BASELINE_PATHS
    global _VERBOSE, _NO_IMAGE, _INTERACT

    # setup defaults
    try:
        SVTK_DATA_ROOT = os.environ['SVTK_DATA_ROOT']
    except KeyError:
        SVTK_DATA_ROOT = os.path.normpath("../../../../../SVTKData")

    try:
        SVTK_BASELINE_ROOT = os.environ['SVTK_BASELINE_ROOT']
    except KeyError:
        pass

    try:
        SVTK_TEMP_DIR = os.environ['SVTK_TEMP_DIR']
    except KeyError:
        SVTK_TEMP_DIR = os.path.normpath("../../../../Testing/Temporary")

    for o, a in opts:
        if o in ('-D', '--data-dir'):
            oa = os.path.abspath(a)
            if os.path.isfile(oa):
                SVTK_DATA_PATHS.append(oa)
            else:
                SVTK_DATA_ROOT = oa
        if o in ('-B', '--baseline-root'):
            SVTK_BASELINE_ROOT = os.path.abspath(a)
        if o in ('-T', '--temp-dir'):
            SVTK_TEMP_DIR = os.path.abspath(a)
        if o in ('-V', '--validated-output'):
            SVTK_BASELINE_PATHS.append(os.path.abspath(a))
        if o in ('-n', '--no-image'):
            _NO_IMAGE = 1
        if o in ('-I', '--interact'):
            _INTERACT = 1
    ...
    if not SVTK_BASELINE_ROOT: # default value.
        SVTK_BASELINE_ROOT = SVTK_DATA_ROOT
...
if __name__ == "__main__":
    ...
    class SampleTest(svtkTest):
        from svtkmodules.svtkRenderingCore import svtkActor
        obj = svtkActor()
        def testParse(self):
