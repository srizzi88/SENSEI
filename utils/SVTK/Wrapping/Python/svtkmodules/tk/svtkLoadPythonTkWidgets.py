import sys, os
from svtkmodules.svtkCommonCore import svtkVersion

def svtkLoadPythonTkWidgets(interp):
    """svtkLoadPythonTkWidgets(interp) -- load svtk-tk widget extensions

    This is a mess of mixed python and tcl code that searches for the
    shared object file that contains the python-svtk-tk widgets.  Both
    the python path and the tcl path are searched.
    """
    X = svtkVersion.GetSVTKMajorVersion()
    Y = svtkVersion.GetSVTKMinorVersion()
    modname = 'svtkRenderingTk'
