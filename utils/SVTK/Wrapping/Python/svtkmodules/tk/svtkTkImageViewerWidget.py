"""
A svtkTkImageViewerWidget for python, which is based on the
svtkTkImageWindowWidget.
...
from svtkmodules.svtkInteractionImage import svtkImageViewer
from svtkmodules.svtkRenderingCore import svtkActor2D, svtkTextMapper
...
class svtkTkImageViewerWidget(tkinter.Widget):
    """
    A svtkTkImageViewerWidget for Python.

    Use GetImageViewer() to get the svtkImageViewer.
    ...
    def __init__(self, master, cnf={}, **kw):
        ...
        try: # use specified svtkImageViewer
            imageViewer = kw['iv']
        except KeyError: # or create one if none specified
            imageViewer = svtkImageViewer()
        ...
        kw['iv'] = imageViewer.GetAddressAsString("svtkImageViewer")
        tkinter.Widget.__init__(self, master, 'svtkTkImageViewerWidget',
                                cnf, kw)
        if doubleBuffer:
    ...
    def __getattr__(self,attr):
        # because the tk part of svtkTkImageViewerWidget must have
        ...
        if attr == '_ImageViewer':
            ...
            return svtkImageViewer('_%s_svtkImageViewer_p' % addr)
    ...
    def ResetTkImageViewer(self):
        ...
        ext = info.Get(svtkStreamingDemandDrivenPipeline.WHOLE_EXTENT())
        ext[4] = z
        ext[5] = z
...
if __name__ == "__main__":
    from svtkmodules.svtkImagingSources import svtkImageCanvasSource2D

    canvas = svtkImageCanvasSource2D()
    ...
    widget = svtkTkImageViewerWidget(frame,width=512,height=512,double=1)
