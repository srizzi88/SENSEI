"""
A simple svtkTkRenderWidget for tkinter.
...
May 11, 1999  - Major rewrite by David Gobbi to make the
                ...
                bindings.
July 14, 1999 - Added modification by Ken Martin for SVTK 2.4, to
                use svtk widgets instead of Togl.
Aug 29, 1999  - Renamed file to svtkRenderWidget.py
...
doing SVTK rendering in Python.

In SVTK (and C++) there is a very important distinction between
public ivars (attributes in pythonspeak), protected ivars, and
private ivars.  When you write a python class that you want
to 'look and feel' like a SVTK class, you should follow these rules.
...
1) Attributes should never be public.  Attributes should always be
   ...
   attributes through public Set/Get methods (same as SVTK).
...
'NULL' - the python/svtk wrappers guarantee their equivalence.
"""

from __future__ import absolute_import
import math, os, sys
from svtkmodules.svtkRenderingCore import svtkCellPicker, svtkProperty, svtkRenderWindow
...
from .svtkLoadPythonTkWidgets import svtkLoadPythonTkWidgets

class svtkTkRenderWidget(tkinter.Widget):
    """
    A svtkTkRenderWidget for Python.
    ...
    def __init__(self, master, cnf={}, **kw):
        ...
        svtkLoadPythonTkWidgets(master.tk)

        try: # check to see if a render window was specified
            renderWindow = kw['rw']
        except KeyError:
            renderWindow = svtkRenderWindow()
        ...
        kw['rw'] = renderWindow.GetAddressAsString("svtkRenderWindow")
        tkinter.Widget.__init__(self, master, 'svtkTkRenderWidget', cnf, kw)
        ...
        self._Picker = svtkCellPicker()
        self._PickedAssembly = None
        self._PickedProperty = svtkProperty()
    ...
    def __getattr__(self,attr):
        # because the tk part of svtkTkRenderWidget must have
    ...
    def GetRenderWindow(self):
        ...
        return svtkRenderWindow('_%s_svtkRenderWindow_p' % addr)
    ...
    def Expose(self):
        if (not self.__InExpose):
            self.__InExpose = 1
            if (not self._RenderWindow.IsA('svtkCocoaRenderWindow')):
                self.update()
...
#----------------------------------------------------------------------------
def svtkRenderWidgetConeExample():
    ...
    # create svtkTkRenderWidget
    pane = svtkTkRenderWidget(root,width=300,height=300)
