...
class VtkDirMethodParser:
    """ Parses the methods from dir(svtk_obj). """
    def initialize_methods (self, svtk_obj):
        debug ("VtkDirMethodParser:: initialize_methods ()")

        self.methods = dir (svtk_obj)[:]
    ...
    def parse_methods (self, svtk_obj):
        debug ("VtkDirMethodParser:: parse_methods()")
        self.initialize_methods (svtk_obj)
        ...
        self.clean_up_methods (svtk_obj)

    def clean_up_methods (self, svtk_obj):
        self.clean_get_set (svtk_obj)
        self.clean_state_methods (svtk_obj)
        self.clean_get_methods (svtk_obj)

    def clean_get_set (self, svtk_obj):
        ...
        for method in self.get_set_meths[:]:
            try:
                eval ("svtk_obj.Get%s ().GetClassName ()"%method)
                ...
                continue
            try:
                val = eval ("svtk_obj.Get%s ()"%method)
            except (TypeError, AttributeError):
                self.get_set_meths.remove (method)
            else:
                if val is None:
    ...
    def clean_state_methods (self, svtk_obj):
...
class VtkPrintMethodParser:
    ...
    """ This class finds the methods for a given svtkObject.  It uses
    the output from svtkObject->Print() (or in Python str(svtkObject))
    ...
    def parse_methods (self, svtk_obj):
        ...
        if self._initialize_methods (svtk_obj):
            # if David Gobbi's improvements are in this version of SVTK
        ...
        for method in self.methods:
            ...
            if (method[1] == "On") or (method[1] == "Off"):
                try:
                    val = eval ("svtk_obj.Get%s ()"%method[0])
                    if val == 1:
                        eval ("svtk_obj.%sOn ()"%method[0])
                    elif val == 0:
                        eval ("svtk_obj.%sOff ()"%method[0])
                except AttributeError:
                    pass
                else:
                ...
                if found:
                    ...
                    try:
                        t = eval ("svtk_obj.Get%sAsString ()"%method[0])
                    except AttributeError:
                        pass
                    else:
                    ...
                    try:
                        t = eval ("svtk_obj.Get%s ().GetClassName ()"%
                                  method[0])
                    except AttributeError:
                        pass
                    else:
                    ...
                    try:
                        ...
                        try:
                            f = eval ("svtk_obj.Set%s"%method[0])
                        except AttributeError:
                            self.get_meths.append ("Get"+method[0])
                        else:
                            try:
        ...
        self._clean_up_methods (svtk_obj)

    def _get_str_obj (self, svtk_obj):
        debug ("VtkPrintMethodParser:: _get_str_obj()")
        self.methods = str (svtk_obj)
        self.methods = self.methods.split ("\n")
        del self.methods[0]

    def _initialize_methods (self, svtk_obj):
        "Do the basic parsing and setting up"
        debug ("VtkPrintMethodParser:: _initialize_methods()")
        dir_p = VtkDirMethodParser ()
        dir_p.parse_methods (svtk_obj)

        # testing if this version of svtk has David Gobbi's cool
        # stuff. If it does then no need to do other things.
        try:
            junk = svtk_obj.__class__
        except AttributeError:
            pass
        else:
    ...
    def _clean_up_methods (self, svtk_obj):
