...
static void* buffer_pointer_and_size(PyObject* o, Py_ssize_t* size)
{
  ...
  // New buffer protocol
  Py_buffer view = SVTK_PYBUFFER_INITIALIZER;
...
//--------------------------------------------------------------------
static PyObject* PySVTKExtras_buffer_shared(PyObject*, PyObject* args)
{
...
//--------------------------------------------------------------------
static PyMethodDef PySVTKExtras_Methods[] = {
  { "buffer_shared", PySVTKExtras_buffer_shared, METH_VARARGS,
...
//--------------------------------------------------------------------
// Exported method called by svtkCommonCorePythonInit
void PySVTKAddFile_PySVTKExtras(PyObject* dict)
{
  // It is necessary to call PyType_Ready() on all subclasses
  PyType_Ready(&PySVTKNumberReference_Type);
  PyType_Ready(&PySVTKStringReference_Type);
  PyType_Ready(&PySVTKTupleReference_Type);

  // Add the "mutable" object (used for C++ pass-by-reference)
  PyObject* o = (PyObject*)&PySVTKReference_Type;
  PyDict_SetItemString(dict, "reference", o); // new name (as of SVTK 8.1)
  PyDict_SetItemString(dict, "mutable", o);   // old name

  for (PyMethodDef* meth = PySVTKExtras_Methods; meth->ml_name != nullptr; meth++)
  {
