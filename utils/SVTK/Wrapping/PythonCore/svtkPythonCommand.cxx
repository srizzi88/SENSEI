...
svtkPythonCommand::svtkPythonCommand()
{
  this->obj = nullptr;
  this->ThreadState = nullptr;
  svtkPythonUtil::RegisterPythonCommand(this);
}

svtkPythonCommand::~svtkPythonCommand()
{
  svtkPythonUtil::UnRegisterPythonCommand(this);
...
void svtkPythonCommand::SetObject(PyObject* o)
{
  svtkPythonScopeGilEnsurer gilEnsurer;
  Py_INCREF(o);
  this->obj = o;
...
void svtkPythonCommand::SetThreadState(PyThreadState* ts)
{
  this->ThreadState = ts;
}
...
void svtkPythonCommand::Execute(svtkObject* ptr, unsigned long eventtype, void* callData)
{
  if (!this->obj)
  {
    return;
  ...
  // thread state.
  //
  // If a threadstate has been set using svtkPythonCommand::SetThreadState,
  // then swap it in here.  See the email to svtk-developers@svtk.org from
  // June 18, 2009 with subject "Py_NewInterpreter and svtkPythonCallback issue"
  ...
  if (eventtype != svtkCommand::DeleteEvent && ptr && ptr->GetReferenceCount() > 0)
  {
    obj2 = svtkPythonUtil::GetObjectFromPointer(ptr);
  }
  else
  {
    Py_INCREF(Py_None);
  ...
  //   affecting any existing SVTK Python code
  ...
  if (callData && callDataTypeObj)
  {
    if (PyInt_Check(callDataTypeObj))
    {
      ...
      if (!invalid)
      {
        if (callDataTypeLong == SVTK_STRING)
        {
        ...
        else if (callDataTypeLong == SVTK_OBJECT)
        {
          // this means the user wants the callData cast as a svtkObject
          PyObject* callDataAsSVTKObject =
            svtkPythonUtil::GetObjectFromPointer(reinterpret_cast<svtkObject*>(callData));
          arglist = BuildCallDataArgList(obj2, eventname, callDataAsSVTKObject);
        }
        else if (callDataTypeLong == SVTK_INT)
        {
        ...
        else if (callDataTypeLong == SVTK_LONG)
        {
        ...
        else if (callDataTypeLong == SVTK_DOUBLE)
        {
        ...
        else if (callDataTypeLong == SVTK_FLOAT)
        {
