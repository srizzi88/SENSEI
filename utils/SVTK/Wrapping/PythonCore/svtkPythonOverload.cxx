...
=========================================================================*/
/**
 * Created in June 2010 by David Gobbi, originally in svtkPythonUtil.
 *
 ...
 * see svtkWrapPython_ArgCheckString() in svtkWrapPython.c.
 */
...
//--------------------------------------------------------------------
// Enums for svtkPythonOverload::CheckArg().
// Values between SVTK_PYTHON_GOOD_MATCH and SVTK_PYTHON_NEEDS_CONVERSION
// are reserved for checking how many generations a svtkObject arg is from
// the requested arg type.

enum svtkPythonArgPenalties
{
  SVTK_PYTHON_EXACT_MATCH = 0,
  SVTK_PYTHON_GOOD_MATCH = 1,
  SVTK_PYTHON_NEEDS_CONVERSION = 65534,
  SVTK_PYTHON_INCOMPATIBLE = 65535
};

//--------------------------------------------------------------------
// A helper struct for CallMethod
class svtkPythonOverloadHelper
{
public:
  svtkPythonOverloadHelper()
    : m_format(nullptr)
    , m_classname(nullptr)
    , m_penalty(0)
    , m_optional(false)
  ...
  bool good() { return (m_penalty < SVTK_PYTHON_INCOMPATIBLE); }
  void addpenalty(int p);
  bool betterthan(const svtkPythonOverloadHelper* other);
...
// Construct the object with a penalty of SVTK_PYTHON_EXACT_MATCH
void svtkPythonOverloadHelper::initialize(bool selfIsClass, const char* format)
{
  ...
  // remove the first arg check if "self" is not a PySVTKClass
  ...
  this->m_penalty = SVTK_PYTHON_EXACT_MATCH;
...
bool svtkPythonOverloadHelper::next(const char** format, const char** classname)
{
  if (*m_format == '|')
  {
    m_optional = true;
...
void svtkPythonOverloadHelper::addpenalty(int p)
{
  if (p > m_penalty)
  {
...
bool svtkPythonOverloadHelper::betterthan(const svtkPythonOverloadHelper* other)
{
...
//--------------------------------------------------------------------
// If tmpi > SVTK_INT_MAX, then penalize types of int size or smaller

static int svtkPythonIntPenalty(PY_LONG_LONG tmpi, int penalty, char format)
{
  if (tmpi > SVTK_INT_MAX || tmpi < SVTK_INT_MIN)
  {
    if (format != 'k')
    {
      if (penalty < SVTK_PYTHON_GOOD_MATCH)
      {
        penalty = SVTK_PYTHON_GOOD_MATCH;
#if SVTK_SIZEOF_LONG == SVTK_SIZEOF_INT
        if (format != 'i')
        {
          penalty++;
        }
...
static int svtkPythonStringPenalty(PyObject* u, char format, int penalty)
{
...
static bool svtkPythonCanConvertToInt(PyObject* arg)
{
...
// svtkWrapPython_ArgCheckString() in svtkWrapPythonOverload.c.
//
// The "level" parameter limits possible recursion of this method,
// it is incremented every time recursion occurs.

int svtkPythonOverload::CheckArg(PyObject* arg, const char* format, const char* name, int level)
{
  int penalty = SVTK_PYTHON_EXACT_MATCH;
  bool badref = false;
  ...
  if (PySVTKReference_Check(arg))
  {
    arg = PySVTKReference_GetValue(arg);
  }
  ...
  switch (*format)
  {
    ...
    case 'q':
      ...
      if (!PyBool_Check(arg))
      {
        penalty = SVTK_PYTHON_NEEDS_CONVERSION;
        int tmpi = PyObject_IsTrue(arg);
        if (tmpi == -1 || PyErr_Occurred())
        {
          PyErr_Clear();
          penalty = SVTK_PYTHON_INCOMPATIBLE;
        }
      }
      break;
    ...
    case 'K':
...
#else
        penalty = svtkPythonIntPenalty(PyInt_AsLong(arg), penalty, *format);
#endif
      }
#endif /* SVTK_PY3K */
      else if (PyLong_Check(arg))
      {
        ...
        penalty = svtkPythonIntPenalty(tmpi, penalty, *format);
      }
      else // not PyInt or PyLong
      {
        if (level == 0)
        {
          penalty = SVTK_PYTHON_NEEDS_CONVERSION;
#ifdef SVTK_PY3K
          if (!svtkPythonCanConvertToInt(arg))
          {
            penalty = SVTK_PYTHON_INCOMPATIBLE;
          }
#else
    ...
    case 'z':
      ...
      {
#ifdef SVTK_PY3K
        penalty = svtkPythonStringPenalty(arg, *format, penalty);
#else
        penalty = SVTK_PYTHON_NEEDS_CONVERSION;
#endif
      }
    ...
    case 'V':
      // SVTK object pointer (instance of svtkObjectBase or a subclass)
      if (classname[0] == '*')
      {
        classname++;
        ...
        else if (PySVTKObject_Check(arg))
        {
          PySVTKClass* info = svtkPythonUtil::FindClass(classname);
    ...
    case 'W':
      // SVTK special type (non reference counted)
        ...
        if (Py_TYPE(arg) != pytype)
        {
          ...
          if (!basetype)
          {
            ...
            else
            {
              ...
              if (!svtkPythonOverload::FindConversionMethod(info->svtk_constructors, arg))
              {
                penalty = SVTK_PYTHON_INCOMPATIBLE;
              }
            }
    ...
    case 'E':
      ...
      if (classname[0] != '*')
      {
        ...
        if (PyInt_Check(arg))
        {
          PyTypeObject* pytype = svtkPythonUtil::FindEnum(classname);
          if (pytype && PyObject_TypeCheck(arg, pytype))
          {
            penalty = SVTK_PYTHON_EXACT_MATCH;
          }
          else
          {
            penalty = SVTK_PYTHON_NEEDS_CONVERSION;
    ...
    default:
      svtkGenericWarningMacro("Unrecognized arg format character " << format[0]);
      penalty = SVTK_PYTHON_INCOMPATIBLE;
  }
...
PyObject* svtkPythonOverload::CallMethod(PyMethodDef* methods, PyObject* self, PyObject* args)
{
  ...
  {
    svtkPythonOverloadHelper helperStorage[16];
    svtkPythonOverloadHelper* helperArray = helperStorage;
    svtkPythonOverloadHelper* helper;
    ...
    for (sig = 0; methods[sig].ml_meth != nullptr; sig++)
    {
      ...
      if ((sig & 15) == 0 && sig != 0)
      {
        ...
        svtkPythonOverloadHelper* tmp = helperArray;
        helperArray = new svtkPythonOverloadHelper[sig + 16];
        for (int k = 0; k < sig; k++)
        {
          helperArray[k] = tmp[k];
        }
    ...
    for (Py_ssize_t i = 0; i < n; i++)
    {
      ...
      for (sig = 0; sig < nsig; sig++)
      {
        ...
        {
          int argpenalty = svtkPythonOverload::CheckArg(arg, format, classname);
          helper->addpenalty(argpenalty);
        }
        else
        {
          helper->addpenalty(SVTK_PYTHON_INCOMPATIBLE);
        }
      }
    }

    // Loop through methods and identify the best match
    svtkPythonOverloadHelper* bestmatch = nullptr;
    meth = nullptr;
    matchCount = 0;
    for (sig = 0; sig < nsig; sig++)
    {
...
PyMethodDef* svtkPythonOverload::FindConversionMethod(PyMethodDef* methods, PyObject* arg)
{
  svtkPythonOverloadHelper helper;
  ...
  int minPenalty = SVTK_PYTHON_NEEDS_CONVERSION;
  int matchCount = 0;
  ...
  for (PyMethodDef* meth = methods; meth->ml_meth != nullptr; meth++)
  {
    ...
    if (meth->ml_doc[0] != '-')
    {
      ...
      if (helper.next(&format, &classname) && !helper.next(&dummy1, &dummy2))
      {
        ...
        int penalty = svtkPythonOverload::CheckArg(arg, format, classname, 1);
