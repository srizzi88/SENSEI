...
/* Lexical tokens defined in svtkParse.l */
...
member_access_specifier:
    PUBLIC { access_level = SVTK_ACCESS_PUBLIC; }
  | PRIVATE { access_level = SVTK_ACCESS_PRIVATE; }
  | PROTECTED { access_level = SVTK_ACCESS_PROTECTED; }
...
opt_virtual:
    { $<integer>$ = 0; }
  | VIRTUAL { $<integer>$ = SVTK_PARSE_VIRTUAL; }
...
access_specifier:
    PUBLIC { $<integer>$ = SVTK_ACCESS_PUBLIC; }
  | PRIVATE { $<integer>$ = SVTK_ACCESS_PRIVATE; }
  | PROTECTED { $<integer>$ = SVTK_ACCESS_PROTECTED; }
...
function_direct_declarator:
    ...
    { $<integer>$ = (SVTK_PARSE_FUNCTION | $<integer>1); popFunction(); }
...
using_id:
  ...
  | scope_operator_sig nested_name_specifier operator_function_id
    { $<str>$ = svtkstrcat3($<str>1, $<str>2, $<str>3); }
...
opt_ellipsis:
    { $<integer>$ = 0; }
  | ELLIPSIS { postSig("..."); $<integer>$ = SVTK_PARSE_PACK; }
...
operator_function_id:
    ...
    { chopSig(); $<str>$ = svtkstrcat(copySig(), $<str>2); postSig($<str>2); }
...
direct_abstract_declarator:
    ...
    {
      if ($<integer>3 == SVTK_PARSE_FUNCTION)
      {
        $<integer>$ = (SVTK_PARSE_FUNCTION_PTR | $<integer>1);
      }
      else
      {
        $<integer>$ = $<integer>1;
  ...
  | lp_or_la ref_attribute_specifier_seq abstract_declarator ')'
    { postSig(")"); } opt_array_or_parameters
    {
      ...
      if ($<integer>6 == SVTK_PARSE_FUNCTION)
      {
        if (scope) { scope = svtkstrndup(scope, strlen(scope) - 2); }
        getFunction()->Class = scope;
        $<integer>$ = (parens | SVTK_PARSE_FUNCTION);
      }
      else if ($<integer>6 == SVTK_PARSE_ARRAY)
      {
...
direct_declarator:
  ...
  | lp_or_la declarator ')' { postSig(")"); }
    opt_array_or_parameters
    {
      ...
      if ($<integer>5 == SVTK_PARSE_FUNCTION)
      {
        if (scope) { scope = svtkstrndup(scope, strlen(scope) - 2); }
        getFunction()->Class = scope;
        $<integer>$ = (parens | SVTK_PARSE_FUNCTION);
      }
      else if ($<integer>5 == SVTK_PARSE_ARRAY)
      {
        $<integer>$ = add_indirection_to_array(parens);
      }
    }
...
opt_array_or_parameters:
  ...
  | '(' { pushFunction(); postSig("("); } parameter_declaration_clause ')'
    ...
    {
      $<integer>$ = SVTK_PARSE_FUNCTION;
      popFunction();
    }
  | array_decorator_seq { $<integer>$ = SVTK_PARSE_ARRAY; }
...
unqualified_id:
  ...
  | template_id
  | decltype_specifier
  | tilde_sig class_name { $<str>$ = svtkstrcat("~", $<str>2); }
  | tilde_sig decltype_specifier { $<str>$ = svtkstrcat("~", $<str>2); }
...
nested_name_specifier:
  ...
  | nested_name_specifier TEMPLATE { postSig("template "); }
    template_id scope_operator_sig
    { $<str>$ = svtkstrcat4($<str>1, "template ", $<str>4, $<str>5); }
...
decl_specifier2:
  ...
  | TYPEDEF { setTypeMod(SVTK_PARSE_TYPEDEF); }
  | FRIEND { setTypeMod(SVTK_PARSE_FRIEND); }
...
storage_class_specifier:
    MUTABLE { postSig("mutable "); $<integer>$ = SVTK_PARSE_MUTABLE; }
  | EXTERN { $<integer>$ = 0; }
  | EXTERN STRING_LITERAL { $<integer>$ = 0; }
  | STATIC { postSig("static "); $<integer>$ = SVTK_PARSE_STATIC; }
  | THREAD_LOCAL
    { postSig("thread_local "); $<integer>$ = SVTK_PARSE_THREAD_LOCAL; }

function_specifier:
    INLINE { $<integer>$ = 0; }
  | VIRTUAL { postSig("virtual "); $<integer>$ = SVTK_PARSE_VIRTUAL; }
  | EXPLICIT { postSig("explicit "); $<integer>$ = SVTK_PARSE_EXPLICIT; }

cv_qualifier:
    CONST { postSig("const "); $<integer>$ = SVTK_PARSE_CONST; }
  | VOLATILE { postSig("volatile "); $<integer>$ = SVTK_PARSE_VOLATILE; }
...
type_name:
    StdString { typeSig($<str>1); $<integer>$ = SVTK_PARSE_STRING; }
  | UnicodeString { typeSig($<str>1); $<integer>$ = SVTK_PARSE_UNICODE_STRING;}
  | OSTREAM { typeSig($<str>1); $<integer>$ = SVTK_PARSE_OSTREAM; }
  | ISTREAM { typeSig($<str>1); $<integer>$ = SVTK_PARSE_ISTREAM; }
  | ID { typeSig($<str>1); $<integer>$ = SVTK_PARSE_UNKNOWN; }
  | SVTK_ID { typeSig($<str>1); $<integer>$ = SVTK_PARSE_OBJECT; }
  | QT_ID { typeSig($<str>1); $<integer>$ = SVTK_PARSE_QOBJECT; }
  | NULLPTR_T { typeSig($<str>1); $<integer>$ = SVTK_PARSE_NULLPTR_T; }
  | SSIZE_T { typeSig($<str>1); $<integer>$ = SVTK_PARSE_SSIZE_T; }
  | SIZE_T { typeSig($<str>1); $<integer>$ = SVTK_PARSE_SIZE_T; }

primitive_type:
    AUTO   { postSig("auto "); $<integer>$ = 0; }
  | VOID   { postSig("void "); $<integer>$ = SVTK_PARSE_VOID; }
  | BOOL { postSig("bool "); $<integer>$ = SVTK_PARSE_BOOL; }
  | FLOAT  { postSig("float "); $<integer>$ = SVTK_PARSE_FLOAT; }
  | DOUBLE { postSig("double "); $<integer>$ = SVTK_PARSE_DOUBLE; }
  | CHAR   { postSig("char "); $<integer>$ = SVTK_PARSE_CHAR; }
  | CHAR16_T { postSig("char16_t "); $<integer>$ = SVTK_PARSE_CHAR16_T; }
  | CHAR32_T   { postSig("char32_t "); $<integer>$ = SVTK_PARSE_CHAR32_T; }
  | WCHAR_T { postSig("wchar_t "); $<integer>$ = SVTK_PARSE_WCHAR_T; }
  | INT    { postSig("int "); $<integer>$ = SVTK_PARSE_INT; }
  | SHORT  { postSig("short "); $<integer>$ = SVTK_PARSE_SHORT; }
  | LONG   { postSig("long "); $<integer>$ = SVTK_PARSE_LONG; }
  | INT64__ { postSig("__int64 "); $<integer>$ = SVTK_PARSE___INT64; }
  | SIGNED { postSig("signed "); $<integer>$ = SVTK_PARSE_INT; }
  | UNSIGNED { postSig("unsigned "); $<integer>$ = SVTK_PARSE_UNSIGNED_INT; }
...
/* &          is SVTK_PARSE_REF
   *          is SVTK_PARSE_POINTER
   *&         is SVTK_PARSE_POINTER_REF
   **         is SVTK_PARSE_POINTER_POINTER
   **&        is SVTK_PARSE_POINTER_POINTER_REF
   *const     is SVTK_PARSE_CONST_POINTER
   *const&    is SVTK_PARSE_CONST_POINTER_REF
   *const*    is SVTK_PARSE_POINTER_CONST_POINTER
   everything else is SVTK_PARSE_BAD_INDIRECT,
   unless the SVTK_PARSE_INDIRECT bitfield is expanded.
   */
...
reference:
    ...
    { postSig("&"); $<integer>$ = SVTK_PARSE_REF; }

rvalue_reference:
    OP_LOGIC_AND ref_attribute_specifier_seq
    { postSig("&&"); $<integer>$ = (SVTK_PARSE_RVALUE | SVTK_PARSE_REF); }
...
ptr_cv_qualifier_seq:
    { $<integer>$ = SVTK_PARSE_POINTER; }
  | cv_qualifier_seq
    {
      if (($<integer>1 & SVTK_PARSE_CONST) != 0)
      {
        $<integer>$ = SVTK_PARSE_CONST_POINTER;
      }
      if (($<integer>1 & SVTK_PARSE_VOLATILE) != 0)
      {
        $<integer>$ = SVTK_PARSE_BAD_INDIRECT;
      }
    }

/* "SVTK_BAD_INDIRECT" occurs when the bitfield fills up */
...
attribute_using_prefix:
    USING using_id ':'
    { setAttributePrefix(svtkstrcat($<str>2, "::")); }
...
attribute_pack:
    { $<integer>$ = 0; }
  | ELLIPSIS { $<integer>$ = SVTK_PARSE_PACK; }
...
operator_id:
  ...
  | STRING_LITERAL ID { $<str>$ = svtkstrcat("\"\" ", $<str>2); }
