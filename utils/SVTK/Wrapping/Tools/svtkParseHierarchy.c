...
/* forward declaration */
static int svtkParseHierarchy_ReadFileIntoInfo(HierarchyInfo* info, const char* filename);

/* Find an entry with a binary search */
HierarchyEntry* svtkParseHierarchy_FindEntry(const HierarchyInfo* info, const char* classname)
{
  HierarchyEntry key;
  ...
  n = svtkParse_UnscopedNameLength(classname);
  m = svtkParse_IdentifierLength(classname);
  while (classname[n] == ':' && classname[n + 1] == ':')
  {
    i = n + 2;
    n += 2 + svtkParse_UnscopedNameLength(&classname[i]);
    m += 2 + svtkParse_IdentifierLength(&classname[i]);
  }
  ...
  if (n != m)
  {
    ...
    while (classname[n] == ':' && classname[n + 1] == ':')
    {
      i = n + 2;
      n += 2 + svtkParse_UnscopedNameLength(&classname[i]);
      l = 2 + svtkParse_IdentifierLength(&classname[i]);
...
HierarchyEntry* svtkParseHierarchy_FindEntryEx(
  const HierarchyInfo* info, const char* classname, const char* scope)
{
  ...
  while (entry == 0 && scope != 0)
  {
    ...
    entry = svtkParseHierarchy_FindEntry(info, cp);
    ...
    if (entry == 0)
    {
      scope_entry = svtkParseHierarchy_FindEntry(info, scope);
      scope = 0;
      scope_needs_free = 0;
      if (scope_entry && scope_entry->NumberOfSuperClasses)
      {
        for (i = 0; i + 1 < scope_entry->NumberOfSuperClasses; i++)
        {
          ...
          scope = svtkParseHierarchy_ExpandTypedefsInName(info, scope_entry->SuperClasses[i], NULL);
          scope_needs_free = (scope != scope_entry->SuperClasses[i]);

          /* recurse if more than one superclass */
          entry = svtkParseHierarchy_FindEntryEx(info, classname, scope);
          if (entry)
          {
            if (scope_needs_free)
            {
  ...
  if (entry == 0)
  {
    entry = svtkParseHierarchy_FindEntry(info, classname);
  }
 ...
 * XXX DEPRECATED; use svtkParseHierarchy_ReadFiles
 */
HierarchyInfo* svtkParseHierarchy_ReadFile(const char* filename)
{
  char* fn = (char*)filename;
  return svtkParseHierarchy_ReadFiles(1, &fn);
}

/* read hierarchy files into a HierarchyInfo struct, or return NULL */
HierarchyInfo* svtkParseHierarchy_ReadFiles(int n, char** filenames)
{
  HierarchyInfo* info;
  int currentFile = 0;
  ...
  svtkParse_InitStringCache(info->Strings);

  for (currentFile = 0; currentFile < n; currentFile++)
  {
    if (!svtkParseHierarchy_ReadFileIntoInfo(info, filenames[currentFile]))
    {
      svtkParseHierarchy_Free(info);
      exit(1);
    }
  }
...
static int svtkParseHierarchy_ReadFileIntoInfo(HierarchyInfo* info, const char* filename)
{
  ...
  for (lineno = 1; fgets(line, (int)maxlen, fp); lineno++)
  {
    ...
    entry->Name = svtkParse_CacheString(info->Strings, &line[i], m);
    i += m;
    ...
    if (line[i] == '<')
    {
      ...
      for (j = 0; line[i] != '>' && line[i] != '\0'; j++)
      {
        ...
        entry->TemplateParameters[j] = svtkParse_CacheString(info->Strings, &line[i], m);
        i += m;
        i += skip_space(&line[i]);
        ...
        if (line[i] == '=')
        {
          ...
          entry->TemplateDefaults[j] = svtkParse_CacheString(info->Strings, &line[i], m);
          i += m;
          i += skip_space(&line[i]);
        }
      ...
      {
        i += 2;
        m = svtkParse_NameLength(&line[i]);
        n = strlen(entry->Name);
        cp = svtkParse_NewString(info->Strings, n + m + 2);
    ...
    if (line[i] == ':')
    {
      ...
      /* else check for superclasses */
        ...
        for (j = 0;; j++)
        {
          ...
          i += skip_space(&line[i]);
          n = svtkParse_NameLength(&line[i]);
          entry->SuperClasses[j] = svtkParse_CacheString(info->Strings, &line[i], n);
    ...
    else if (line[i] == '=')
    {
      ...
      svtkParse_InitValue(entry->Typedef);
      ...
      if (line[i] == '&')
      {
        i++;
        i += skip_space(&line[i]);
        entry->Typedef->Type |= SVTK_PARSE_REF;
      }
      ...
      while (line[i] == '[')
      {
        ...
        ccp = svtkParse_CacheString(info->Strings, &line[i], n);
      ...
      while (line[i] == '*' || strncmp(&line[i], "const*", 6) == 0)
      {
        ...
        if (line[i] == '*')
        {
          bits = (bits | SVTK_PARSE_POINTER);
        }
        else
        {
          bits = (bits | SVTK_PARSE_CONST_POINTER);
          i += 5;
        }
        bits = (bits & SVTK_PARSE_POINTER_MASK);
        i++;
        i += skip_space(&line[i]);
      }
      ...
      {
        pointers = (pointers << 2);
        pointers = (pointers | (bits & SVTK_PARSE_POINTER_LOWMASK));
        bits = ((bits >> 2) & SVTK_PARSE_POINTER_MASK);
      }

      /* add pointer indirection to correspond to first array dimension */
      if (entry->Typedef->NumberOfDimensions > 1)
      {
        pointers = ((pointers << 2) | SVTK_PARSE_ARRAY);
      }
      else if (entry->Typedef->NumberOfDimensions == 1)
      {
        pointers = ((pointers << 2) | SVTK_PARSE_POINTER);
      }

      /* include the pointers in the type */
      entry->Typedef->Type |= (pointers & SVTK_PARSE_POINTER_MASK);

      /* read the base type (and const) */
      bits = 0;
      i += svtkParse_BasicTypeFromString(&line[i], &bits, &ccp, &n);
      entry->Typedef->Class = svtkParse_CacheString(info->Strings, ccp, n);
    ...
    if (line[i] == ';')
    {
      ...
      entry->HeaderFile = svtkParse_CacheString(info->Strings, &line[i], n);
      ...
      if (line[i] == ';')
      {
        ...
        entry->Module = svtkParse_CacheString(info->Strings, &line[i], n);
      ...
      while (line[i] == ';')
      {
        ...
        if (n && skip_space(&line[i]) != n)
        {
          entry->Properties[entry->NumberOfProperties++] =
            svtkParse_CacheString(info->Strings, &line[i], n);
        }
        i += n;
      }
    }
...
void svtkParseHierarchy_Free(HierarchyInfo* info)
{
  HierarchyEntry* entry;
  int i;
...
int svtkParseHierarchy_IsTypeOfTemplated(const HierarchyInfo* info, const HierarchyEntry* entry,
  ...
  while (iterating)
  {
    ...
    if (entry->NumberOfTemplateParameters)
    {
      ...
      if (classname[m] == '<')
      {
        ...
        svtkParse_DecomposeTemplatedType(classname, &name, nargs, &args, entry->TemplateDefaults);
      }
    }
    ...
    for (j = 0; j < entry->NumberOfSuperClasses && rval == 0; j++)
    {
      ...
      if (templated)
      {
        ...
        supername = svtkParse_StringReplace(
      ...
      if (i == -1)
      {
        ...
        tmph = svtkParseHierarchy_FindEntry(info, supername);
        while (tmph && tmph->IsTypedef)
        {
          if (tmph->Typedef->Class)
          {
            tmph = svtkParseHierarchy_FindEntry(info, tmph->Typedef->Class);
            continue;
          }
          break;
        }
      ...
      if (i >= 0)
      {
        ...
        {
          rval = svtkParseHierarchy_IsTypeOfTemplated(
...
int svtkParseHierarchy_IsTypeOf(
  const HierarchyInfo* info, const HierarchyEntry* entry, const char* baseclass)
{
  return svtkParseHierarchy_IsTypeOfTemplated(info, entry, entry->Name, baseclass, NULL);
}

/* Free args returned by IsTypeOfTemplated */
void svtkParseHierarchy_FreeTemplateArgs(int n, const char* args[])
{
  int i;
...
const char* svtkParseHierarchy_TemplatedSuperClass(
  const HierarchyEntry* entry, const char* classname, int i)
{
  ...
  if (i < entry->NumberOfSuperClasses)
  {
    ...
    j = svtkParse_IdentifierLength(classname);

    if (classname[j] == '<')
    {
      svtkParse_DecomposeTemplatedType(
      ...
      svtkParse_FreeTemplateDecomposition(name, entry->NumberOfTemplateParameters, args);
    }
...
const char* svtkParseHierarchy_GetProperty(const HierarchyEntry* entry, const char* property)
{
  int i;
  size_t k;
  ...
  if (entry)
  {
    for (i = 0; i < entry->NumberOfProperties; i++)
    {
      ...
      k = svtkParse_NameLength(entry->Properties[i]);
...
int svtkParseHierarchy_IsPrimary(const HierarchyEntry* entry)
{
...
const char* svtkParseHierarchy_ExpandTypedefsInTemplateArgs(
  const HierarchyInfo* info, const char* name, StringCache* cache, const char* scope)
{
  ...
  do
  {
    ...
    i++;
    i += svtkParse_ValueInfoFromString(&val, cache, &name[i]);
    svtkParseHierarchy_ExpandTypedefsInValue(info, &val, cache, scope);
    l++;
    n = svtkParse_ValueInfoToString(&val, NULL, SVTK_PARSE_EVERYTHING);
    if (l + n >= m)
    {
    ...
    l += svtkParse_ValueInfoToString(&val, &text[l], SVTK_PARSE_EVERYTHING);
    text[l] = ',';
  } while (name[i] == ',');
  ...
  return svtkParse_CacheString(cache, text, l);
}

/* Expand all unrecognized types in a ValueInfo struct by
 * using the typedefs in the HierarchyInfo struct. */
int svtkParseHierarchy_ExpandTypedefsInValue(
  const HierarchyInfo* info, ValueInfo* val, StringCache* cache, const char* scope)
{
  ...
  while (((val->Type & SVTK_PARSE_BASE_TYPE) == SVTK_PARSE_OBJECT ||
           (val->Type & SVTK_PARSE_BASE_TYPE) == SVTK_PARSE_UNKNOWN) &&
    val->Class != 0)
  {
    if (strncmp(val->Class, "std::", 5) == 0)
    {
      /* check for template args, expand if necessary */
      val->Class = svtkParseHierarchy_ExpandTypedefsInTemplateArgs(info, val->Class, cache, scope);
      result = 1;
      break;
    }

    entry = svtkParseHierarchy_FindEntryEx(info, val->Class, scope);

    if (entry && entry->IsTypedef)
    {
      svtkParse_ExpandTypedef(val, entry->Typedef);
      ...
      scope = 0;
      l = svtkParse_UnscopedNameLength(entry->Name);
      if (entry->Name[l] == ':' && entry->Name[l + 1] == ':')
      {
        do
        {
          n = l;
          l = n + 2 + svtkParse_UnscopedNameLength(&entry->Name[n + 2]);
      ...
      n = svtkParse_UnscopedNameLength(val->Class);
      if (val->Class[n] == ':' && val->Class[n + 1] == ':')
      {
        /* try to expand the scope, since it might itself be a typedef */
        ValueInfo prefix;
        svtkParse_InitValue(&prefix);
        prefix.Type = SVTK_PARSE_UNKNOWN;
        prefix.Class = svtkParse_CacheString(cache, val->Class, n);
        svtkParseHierarchy_ExpandTypedefsInValue(info, &prefix, cache, scope);
        ...
        if (l != n || strncmp(prefix.Class, val->Class, l) != 0)
        {
          ...
          val->Class = svtkParse_CacheString(cache, cp, m);
          free(cp);
        }
      }
    }
    ...
    {
      newclass = svtkParseHierarchy_ExpandTypedefsInName(info, val->Class, scope);
      if (newclass != val->Class)
      {
        val->Class = svtkParse_CacheString(cache, newclass, strlen(newclass));
        free((char*)newclass);
      }
      result = 1;
      break;
...
const char* svtkParseHierarchy_ExpandTypedefsInName(
  const HierarchyInfo* info, const char* name, const char* scope)
{
  ...
  m = svtkParse_IdentifierLength(name);
  if (name[m] != '\0')
  {
    return name;
  }
  ...
  if (!entry)
  {
    entry = svtkParseHierarchy_FindEntry(info, name);
  }
...
/* -------------------------------------------------------------------- */
const char* svtkParseHierarchy_QualifiedEnumName(
  ...
  if (data)
  {
    ...
    for (j = 0; j < data->NumberOfEnums; j++)
    {
      ...
      if (name && info->Name && strcmp(name, info->Name) == 0)
      {
        ...
        scoped_name = svtkParse_NewString(cache, scoped_len);
