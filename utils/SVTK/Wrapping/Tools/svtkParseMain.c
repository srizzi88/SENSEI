...
static void parse_print_help(FILE* fp, const char* cmd, int multi)
{
  fprintf(fp,
    ...
    "  --version         print the SVTK version\n"
...
static int read_option_file(StringCache* strings, const char* filename, int* argn, char*** args)
{
  ...
  while (fgets(line, (int)maxlen, fp))
  {
    ...
    argstring = svtkParse_NewString(strings, n);
    arg = argstring;
    i = 0;
...
static int parse_check_options(int argc, char* argv[], int multi)
{
  ...
  for (i = 1; i < argc; i++)
  {
    ...
    {
      const char* ver = SVTK_PARSE_VERSION;
    ...
    else if (strcmp(argv[i], "-imacros") == 0)
    {
      ...
      cp = argv[i];
      svtkParse_IncludeMacros(cp);
    }
    else if (strcmp(argv[i], "-undef") == 0)
    {
      svtkParse_UndefinePlatformMacros();
    }
    ...
    else if (argv[i][0] == '-' && isalpha(argv[i][1]))
    {
      ...
      else if (c == 'D')
      {
        ...
        svtkParse_DefineMacro(cp, &cp[j]);
      }
      else if (c == 'U')
      {
        svtkParse_UndefineMacro(cp);
      }
    }
...
OptionInfo* svtkParse_GetCommandLineOptions(void)
{
  return &options;
}

/* Command-line argument handler for wrapper tools */
FileInfo* svtkParse_Main(int argc, char* argv[])
{
  int argi;
  FILE* ifile;
  FILE* hfile = 0;
  ...
  svtkParse_SetCommandName(parse_exename(argv[0]));

  /* pre-define the __SVTK_WRAP__ macro */
  svtkParse_DefineMacro("__SVTK_WRAP__", 0);

  /* expand any "@file" args */
  svtkParse_InitStringCache(&strings);
  ...
  {
    svtkParse_DumpMacros(options.OutputFileName);
  }
  ...
  data = svtkParse_ParseFile(options.InputFileName, ifile, stderr);
  ...
  for (ihfiles = 0; ihfiles < nhfiles; ihfiles++)
  {
    ...
    if (hfilename && hfilename[0] != '\0')
    {
      if (!(hfile = fopen(hfilename, "r")))
      {
        ...
        fclose(ifile);
        svtkParse_FreeFile(data);
        exit(1);
      }

      /* fill in some blanks by using the hints file */
      svtkParse_ReadHints(data, hfile, stderr);
    }
  }
  ...
  if (data->MainClass)
  {
    ...
    for (ifunc = 0; ifunc < nfunc; ifunc++)
    {
      ...
      if (func && func->Access == SVTK_ACCESS_PUBLIC && func->Name &&
...
/* Command-line argument handler for wrapper tools */
StringCache* svtkParse_MainMulti(int argc, char* argv[])
{
  int argi;
  int argn;
  char** args;
