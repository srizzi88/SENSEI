...
int svtkParseMerge_PushClass(MergeInfo* info, const char* classname)
{
...
int svtkParseMerge_PushFunction(MergeInfo* info, int depth)
{
...
int svtkParseMerge_PushOverride(MergeInfo* info, int i, int depth)
{
...
MergeInfo* svtkParseMerge_CreateMergeInfo(ClassInfo* classInfo)
{
  ...
  svtkParseMerge_PushClass(info, classInfo->Name);
  n = classInfo->NumberOfFunctions;
  for (i = 0; i < n; i++)
  {
    svtkParseMerge_PushFunction(info, 0);
  }
...
static void merge_function(FileInfo* finfo, FunctionInfo* merge, const FunctionInfo* func)
{
  ...
  if (merge->NumberOfPreconds == 0)
  {
    for (i = 0; i < func->NumberOfPreconds; i++)
    {
      ...
      svtkParse_InitTokenizer(&t, func->Preconds[i], WS_DEFAULT);
      do
      {
        int matched = 0;
      ...
      svtkParse_AddStringToArray(
        &merge->Preconds, &merge->NumberOfPreconds, svtkParse_CacheString(finfo->Strings, text, l));
    }
  }
  ...
  for (i = -1; i < j; i++)
  {
    ...
    if (arg && arg2)
    {
      ...
      /* attribute flags */
      arg->Type |= (arg2->Type & SVTK_PARSE_ATTRIBUTES);
    }
  }
...
void svtkParseMerge_MergeUsing(
  FileInfo* finfo, MergeInfo* info, ClassInfo* merge, const ClassInfo* super, int depth)
{
  ...
  for (i = 0; i < n; i++)
  {
    ...
    for (j = 0; j < m; j++)
    {
      ...
      {
        if (svtkParse_CompareFunctionSignature(func, f2) != 0)
        {
          match = 1;
          break;
        }
    ...
    if (!match)
    {
      ...
      if (is_constructor)
      {
        ...
        for (j = func->NumberOfParameters; j > 0; j--)
        {
          ...
          svtkParse_InitFunction(f2);
          f2->Access = u->Access;
          f2->Name = merge->Name;
          f2->Class = merge->Name;
          f2->Comment = func->Comment;
          f2->IsExplicit = func->IsExplicit;
          l = svtkParse_FunctionInfoToString(f2, NULL, SVTK_PARSE_EVERYTHING);
          cp = svtkParse_NewString(finfo->Strings, l);
          svtkParse_FunctionInfoToString(f2, cp, SVTK_PARSE_EVERYTHING);
          f2->Signature = cp;
          lastval = NULL;
          for (k = 0; k < j; k++)
          {
            param = (ValueInfo*)malloc(sizeof(ValueInfo));
            svtkParse_CopyValue(param, func->Parameters[k]);
            lastval = param->Value;
            param->Value = NULL; /* clear default parameter value */
            svtkParse_AddParameterToFunction(f2, param);
          }
          svtkParse_AddFunctionToClass(merge, f2);
          if (info)
          {
            svtkParseMerge_PushFunction(info, depth);
          }
          if (lastval == NULL)
          {
      ...
      else
      {
        ...
        f2 = (FunctionInfo*)malloc(sizeof(FunctionInfo));
        svtkParse_CopyFunction(f2, func);
        f2->Access = u->Access;
        f2->Class = merge->Name;
        svtkParse_AddFunctionToClass(merge, f2);
        if (info)
...
int svtkParseMerge_Merge(FileInfo* finfo, MergeInfo* info, ClassInfo* merge, ClassInfo* super)
{
  ...
  depth = svtkParseMerge_PushClass(info, super->Name);

  svtkParseMerge_MergeUsing(finfo, info, merge, super, depth);
      ...
      if (f1 && f1->Name && strcmp(f1->Name, func->Name) == 0)
      {
        if (match)
        {
          ...
          for (j = 0; j < m; j++)
          {
            ...
            {
              if (svtkParse_CompareFunctionSignature(f1, f2) != 0)
              {
                merge_function(finfo, f2, f1);
                svtkParseMerge_PushOverride(info, j, depth);
              }
            }
          }
        }
        ...
        {
          /* copy into the merge */
          svtkParse_AddFunctionToClass(merge, f1);
          svtkParseMerge_PushFunction(info, depth);
          m++;
        }
  ...
  MergeInfo* info, ClassInfo* merge)
{
  ...
  {
    entry = svtkParseHierarchy_FindEntry(hinfo, &classname[2]);
  }
  else
  {
    entry = svtkParseHierarchy_FindEntryEx(hinfo, classname, nspacename);
  }
  ...
  if (n > 0 && !cinfo)
  {
    ...
    filename = svtkParse_FindIncludeFile(header);
    if (!filename)
    {
    ...
    finfo = svtkParse_ParseFile(filename, fp, stderr);
    fclose(fp);
      ...
      for (ihintfiles = 0; ihintfiles < nhintfiles; ihintfiles++)
      {
        ...
        if (hintfilename && hintfilename[0] != '\0')
        {
          if (!(hintfile = fopen(hintfilename, "r")))
          {
            ...
            svtkParse_FreeFile(finfo);
            exit(1);
          }

          svtkParse_ReadHints(finfo, hintfile, stderr);
          fclose(hintfile);
        }
      }
    }
  ...
  if (cinfo)
  {
    ...
    svtkParse_CopyClass(new_cinfo, cinfo);
    if (template_args)
    {
    ...
    if (info)
    {
      svtkParseMerge_Merge(finfo, info, merge, cinfo);
      recurse = 1;
    }
    else
    {
      svtkParseMerge_MergeUsing(finfo, info, merge, cinfo, 0);
    ...
    svtkParse_FreeClass(cinfo);
  }

  if (template_arg_count > 0)
  {
    svtkParse_FreeTemplateDecomposition(classname, template_arg_count, template_args);
  }
}
  ...
  FileInfo* finfo, NamespaceInfo* data, ClassInfo* classInfo)
{
  ...
  MergeInfo* info = NULL;
  OptionInfo* oinfo = svtkParse_GetCommandLineOptions();
  int i, n;

  if (oinfo->HierarchyFileNames)
  {
    hinfo =
      svtkParseHierarchy_ReadFiles(oinfo->NumberOfHierarchyFileNames, oinfo->HierarchyFileNames);

    info = svtkParseMerge_CreateMergeInfo(classInfo);

    n = classInfo->NumberOfSuperClasses;
    for (i = 0; i < n; i++)
    {
      svtkParseMerge_MergeHelper(finfo, data, hinfo, classInfo->SuperClasses[i],
