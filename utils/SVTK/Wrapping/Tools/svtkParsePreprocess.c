...
static int preproc_identical(const char* text1, const char* text2)
{
  ...
  if (text1 != text2)
  {
    ...
    if (text1 && text2)
    {
      ...
      svtkParse_InitTokenizer(&t1, text1, WS_PREPROC);
      svtkParse_InitTokenizer(&t2, text2, WS_PREPROC);
      ...
      do
      {
        ...
        svtkParse_NextToken(&t1);
        svtkParse_NextToken(&t2);
      } while (t1.tok && t2.tok);
...
static MacroInfo* preproc_new_macro(PreprocessInfo* info, const char* name, const char* definition)
{
  ...
  svtkParsePreprocess_InitMacro(macro);
  macro->Ordinal = ++info->MacroCounter;

  if (name)
  {
    size_t n = svtkParse_SkipId(name);
    macro->Name = svtkParse_CacheString(info->Strings, name, n);
  }
  ...
  if (definition)
  {
    ...
    svtkParse_InitTokenizer(&tokens, cp, WS_PREPROC);

    do
    {
      cp = tokens.text + tokens.len;
    } while (svtkParse_NextToken(&tokens));

    n = cp - definition;
    macro->Definition = svtkParse_CacheString(info->Strings, definition, n);
  }
  ...
  PreprocessInfo* info, const char* name, const char* definition)
{
  ...
  svtkParse_InitTokenizer(&token, name, WS_PREPROC);
...
static int preproc_skip_parentheses(StringTokenizer* tokens)
{
  ...
  if (tokens->tok == '(')
  {
    ...
    while (depth > 0 && svtkParse_NextToken(tokens))
    {
      if (tokens->tok == '(')
      {
        depth++;
  ...
  if (tokens->tok == ')')
  {
    svtkParse_NextToken(tokens);
    return SVTK_PARSE_OK;
  }

#if PREPROC_DEBUG
  fprintf(stderr, "syntax error %d\n", __LINE__);
#endif
  return SVTK_PARSE_SYNTAX_ERROR;
}
...
static int preproc_evaluate_char(const char* cp, preproc_int_t* val, int* is_unsigned)
{
  ...
  if (*cp == '\'')
  {
    ...
    if (*cp != '\\')
    {
      code = svtkParse_DecodeUtf8(&cp, NULL);
    }
    else if (*cp != '\'' && *cp != '\n' && *cp != '\0')
    {
      cp++;
      ...
      else if (*cp == 'x')
      {
        ...
        {
          cp++;
        } while (svtkParse_CharType(*cp, CPRE_HEX));
      }
      else if (*cp == 'u')
      {
        code = string_to_preproc_int(cp + 1, 16);
        do
        {
          cp++;
          i++;
        } while (i < 5 && svtkParse_CharType(*cp, CPRE_HEX));
        if (i != 5)
        {
          cp -= i;
        }
      ...
      else if (*cp == 'U')
      {
        ...
        do
        {
          cp++;
          i++;
        } while (i < 9 && svtkParse_CharType(*cp, CPRE_HEX));
        if (i != 9)
        {
          cp -= i;
        }
...
static int preproc_evaluate_integer(const char* cp, preproc_int_t* val, int* is_unsigned)
{
  ...
  if (cp[0] == '0' && (cp[1] == 'x' || cp[1] == 'X'))
  {
    ...
    *is_unsigned = 1;
    ep = cp;
    while (svtkParse_CharType(*ep, CPRE_HEX))
    {
      ep++;
      if (*ep == '\'' && svtkParse_CharType(ep[1], CPRE_HEX))
      {
        apos++;
        ep += 2;
      }
  ...
  else if (cp[0] == '0' && svtkParse_CharType(cp[1], CPRE_DIGIT))
  {
    cp += 1;
    base = 8;
    *is_unsigned = 1;
  ...
  else
  {
    base = 10;
    *is_unsigned = 0;
    while (svtkParse_CharType(*ep, CPRE_DIGIT))
    {
      ep++;
      if (*ep == '\'' && svtkParse_CharType(ep[1], CPRE_DIGIT))
      {
        apos++;
        ep += 2;
      }
  ...
  if (*ep == '.' ||
    ((ep[0] == 'e' || ep[0] == 'E') &&
      (svtkParse_CharType(ep[1], CPRE_SIGN) || svtkParse_CharType(ep[1], CPRE_DIGIT))))
  {
    *val = 0;
    return SVTK_PARSE_PREPROC_DOUBLE;
  }
  ...
  PreprocessInfo* info, StringTokenizer* tokens, preproc_int_t* val, int* is_unsigned)
{
  ...
  while (tokens->tok == TOK_ID)
  {
    ...
    else
    {
      ...
      *is_unsigned = 0;
      ...
      {
        return SVTK_PARSE_MACRO_UNDEFINED;
      }
      else if (macro->IsFunction)
      {
        /* expand function macros using the arguments */
        args = tokens->text;
        if (tokens->tok != '(' || preproc_skip_parentheses(tokens) != SVTK_PARSE_OK)
        {
#if PREPROC_DEBUG
          fprintf(stderr, "syntax error %d\n", __LINE__);
#endif
      ...
      expansion = svtkParsePreprocess_ExpandMacro(info, macro, args);
      if (expansion == NULL)
      {
#if PREPROC_DEBUG
        fprintf(stderr, "syntax error %d\n", __LINE__);
#endif
        return (args ? SVTK_PARSE_MACRO_NUMARGS : SVTK_PARSE_SYNTAX_ERROR);
      }
      cp = expansion;
      cp += svtkParse_SkipWhitespace(cp, WS_PREPROC);
      if (*cp != '\0')
      {
        macro->IsExcluded = 1;
        result = svtkParsePreprocess_EvaluateExpression(info, expansion, val, is_unsigned);
        macro->IsExcluded = 0;
        svtkParsePreprocess_FreeMacroExpansion(info, macro, expansion);
        return result;
      }
      svtkParsePreprocess_FreeMacroExpansion(info, macro, expansion);
    }
  ...
  if (tokens->tok == '(')
  {
    ...
    if ((result & SVTK_PARSE_FATAL_ERROR) == 0)
    {
      if (tokens->tok == ')')
      {
        svtkParse_NextToken(tokens);
  ...
  else if (tokens->tok == TOK_NUMBER)
  {
    ...
    {
      result = SVTK_PARSE_PREPROC_FLOAT;
    }
    svtkParse_NextToken(tokens);
    return result;
  }
  ...
  else if (tokens->tok == TOK_STRING)
  {
    ...
    *is_unsigned = 0;
    ...
    return SVTK_PARSE_PREPROC_STRING;
  }
  ...
  PreprocessInfo* info, StringTokenizer* tokens, preproc_int_t* val, int* is_unsigned)
{
  ...
  result = preproc_evaluate_unary(info, tokens, val, is_unsigned);
  while ((result & SVTK_PARSE_FATAL_ERROR) == 0)
  {
    op = tokens->tok;
    if (op != '*' && op != '/' && op != '%')
    {
  ...
  while ((result & SVTK_PARSE_FATAL_ERROR) == 0)
  {
    ...
    if (*val == 0)
    {
        ...
        tokens->tok != ',' && tokens->tok != TOK_OR)
      {
        if (tokens->tok == '(')
        {
          if (preproc_skip_parentheses(tokens) != SVTK_PARSE_OK)
          {
#if PREPROC_DEBUG
            fprintf(stderr, "syntax error %d\n", __LINE__);
#endif
            result = SVTK_PARSE_SYNTAX_ERROR;
          }
        }
        else
        {
 ...
 * Returns SVTK_PARSE_OK if the expression is true,
 * or SVTK_PARSE_SKIP of the expression is false. */
int preproc_evaluate_conditional(PreprocessInfo* info, StringTokenizer* tokens)
{
  ...
  if ((result & SVTK_PARSE_FATAL_ERROR) == 0)
  {
    ...
    return (rval == 0 ? SVTK_PARSE_SKIP : SVTK_PARSE_OK);
  }
 ...
 * A return value of SVTK_PARSE_SKIP means that
  ...
  if (tokens->hash == HASH_IF || tokens->hash == HASH_IFDEF || tokens->hash == HASH_IFNDEF)
  {
    if (info->ConditionalDepth == 0)
    {
      ...
      else
      {
        ...
        svtkParse_NextToken(tokens);
        result = ((v1 ^ v2) ? SVTK_PARSE_SKIP : SVTK_PARSE_OK);
      }

      if (result != SVTK_PARSE_SKIP)
      {
  ...
  if (tokens->hash == HASH_DEFINE)
  {
    ...
    if (name[namelen] == '(')
    {
      ...
      while (tokens->tok != 0 && tokens->tok != ')')
      {
        ...
        params[n++] = svtkParse_CacheString(info->Strings, param, l);
...
#endif
      return SVTK_PARSE_MACRO_REDEFINED;
    }
...
static int preproc_add_include_file(PreprocessInfo* info, const char* name)
{
  ...
  info->IncludeFiles[info->NumberOfIncludeFiles++] =
    svtkParse_CacheString(info->Strings, name, strlen(name));
  ...
  PreprocessInfo* info, const char* filename, preproc_search_t order, int cache_only)
{
  ...
  if (order == SVTK_PARSE_SOURCE_INCLUDE)
  {
    extra = 1;
  }
  else if (order == SVTK_PARSE_CURDIR_INCLUDE)
  {
  ...
  j = 0;
  while (svtkParse_CharType(filename[j], CPRE_XID))
  {
    j++;
  }
  ...
  for (count = 0; count < (2 - cache_only); count++)
  {
    ...
    for (i = 0; i < (n + extra); i++)
    {
...
#endif
      {
        ...
        info->IncludeFiles[info->NumberOfIncludeFiles++] =
          svtkParse_CacheString(info->Strings, output, strlen(output));
        free(output);
 ...
 * function for svtkParsePreprocess_IncludeFile() to allow raw strings to
 ...
 */
int svtkParsePreprocess_IncludeFile(
  PreprocessInfo* info, const char* filename, preproc_search_t order)
{
  ...
  if (path == NULL)
  {
    ...
    if (filename[k] == '>')
      ...
      fprintf(stderr, "couldn't find file \"%*.*s\"\n", k, k, filename);
#endif
    return SVTK_PARSE_FILE_NOT_FOUND;
  }
...
#endif
    return SVTK_PARSE_FILE_OPEN_ERROR;
  }
  ...
  do
  {
      ...
      else
      {
        ...
        while ((n = fread(&tbuf[r], 1, tbuflen - r, fp)) == 0 && ferror(fp))
        {
          if (errno != EINTR)
          {
            ...
            return SVTK_PARSE_FILE_READ_ERROR;
          }
          errno = 0;
          clearerr(fp);
        }
    ...
    while (i < n)
    {
      ...
      while (j + 4 > linelen)
      {
        ...
        {
          free(oldline);
          return SVTK_PARSE_OUT_OF_MEMORY;
        }
      }
      ...
      else if (tbuf[i] == '\"' || tbuf[i] == '\'')
      {
        ...
        if (state == '\"' && j > 0 && line[j - 1] == 'R' &&
          ((j > 2 && (line[j - 3] == 'u' || line[j - 2] == '8') &&
             (j == 3 || !svtkParse_CharType(line[j - 4], CPRE_XID | CPRE_QUOTE))) ||
            (j > 1 && (line[j - 2] == 'u' || line[j - 2] == 'U' || line[j - 2] == 'L') &&
              (j == 2 || !svtkParse_CharType(line[j - 3], CPRE_XID | CPRE_QUOTE))) ||
            (j == 1 || !svtkParse_CharType(line[j - 2], CPRE_XID | CPRE_QUOTE))))
        {
          state = '(';
          d = j + 1;
        }
    ...
    if (i < n || n == 0)
    {
      ...
      if (*cp == '#')
      {
        svtkParsePreprocess_HandleDirective(info, line);
      }
    }
  } while (n > 0);
...
static int preproc_evaluate_include(PreprocessInfo* info, StringTokenizer* tokens)
{
  ...
  if (tokens->hash == HASH_INCLUDE)
  {
    ...
    {
      filename = cp + 1;
      cp += svtkParse_SkipQuotes(cp);
      if (cp <= filename + 1 || *(cp - 1) != '\"')
      {
        return SVTK_PARSE_SYNTAX_ERROR;
      }

      return svtkParsePreprocess_IncludeFile(info, filename, SVTK_PARSE_SOURCE_INCLUDE);
    }
    else if (*cp == '<')
    {
      cp++;
      ...
      return svtkParsePreprocess_IncludeFile(info, filename, SVTK_PARSE_SYSTEM_INCLUDE);
    }
  }
 ...
 */
int svtkParsePreprocess_HandleDirective(PreprocessInfo* info, const char* directive)
{
  int result = SVTK_PARSE_OK;
  StringTokenizer tokens;

  svtkParse_InitTokenizer(&tokens, directive, WS_PREPROC);
 ...
 * If no errors occurred, the result will be SVTK_PARSE_OK.
 */
int svtkParsePreprocess_EvaluateExpression(
  PreprocessInfo* info, const char* text, preproc_int_t* val, int* is_unsigned)
{
  StringTokenizer tokens;
  svtkParse_InitTokenizer(&tokens, text, WS_PREPROC);
 ...
 */
void svtkParsePreprocess_AddStandardMacros(PreprocessInfo* info, preproc_platform_t platform)
{
 ...
 */
int svtkParsePreprocess_AddMacro(PreprocessInfo* info, const char* name, const char* definition)
{
  StringTokenizer token;
  MacroInfo** macro_p;
  MacroInfo* macro;
 ...
 */
MacroInfo* svtkParsePreprocess_GetMacro(PreprocessInfo* info, const char* name)
{
  StringTokenizer token;
  MacroInfo* macro;
 ...
 */
int svtkParsePreprocess_RemoveMacro(PreprocessInfo* info, const char* name)
{
  StringTokenizer token;
 ...
 */
MacroInfo* svtkParsePreprocess_NextMacro(PreprocessInfo* info, MacroInfo* macro)
{
  MacroInfo** mptr;
  int i, n;
  int ordinal = 0;
 ...
 */
const char* svtkParsePreprocess_ExpandMacro(
  PreprocessInfo* info, MacroInfo* macro, const char* argstring)
{
  ...
  if (macro->IsFunction)
  {
    ...
    if (macro->NumberOfParameters == 0 && n == 1)
    {
      ...
      tp += svtkParse_SkipWhitespace(tp, WS_PREPROC);
      if (tp + 1 >= values[1])
      {
        n = 0;
      }
  ...
  while (*cp != '\0')
  {
    ...
    while (!svtkParse_CharType(*cp, CPRE_ID) && *cp != '\0')
    {
      dp = cp;
      cp += svtkParse_SkipWhitespace(cp, WS_PREPROC);
      if (cp > dp)
      ...
      else if (svtkParse_CharType(*cp, CPRE_QUOTE))
      {
        cp += svtkParse_SkipQuotes(cp);
        dp = cp;
        wp = cp;
      ...
      else if (svtkParse_CharType(*cp, CPRE_DIGIT))
      {
        cp += svtkParse_SkipNumber(cp);
        dp = cp;
        wp = cp;
        noexpand = 0;
      }
    ...
    if (l > 0)
    {
      for (j = 0; j < macro->NumberOfParameters; j++)
      {
        ...
        if (strncmp(pp, macro->Parameters[j], l) == 0 && macro->Parameters[j][l] == '\0')
        {
          ...
          c = *pp;
          while (svtkParse_CharType(c, CPRE_WHITE))
          {
            c = *(++pp);
            l--;
          }
          ...
          wp = cp;
          wp += svtkParse_SkipWhitespace(wp, WS_PREPROC);
          if (wp[0] == '#' && wp[1] == '#')
          {
            noexpand = 1;
          }
      ...
      else if (empty_variadic && j == macro->NumberOfParameters - 1)
      {
        ...
        if (k > 0)
        {
          ...
          {
            c = rp[--k];
          } while (k > 0 && svtkParse_CharType(c, CPRE_WHITE));
          if (rp[k] == ',')
          {
            i = k;
          }
      ...
      else
      {
        ...
        strncpy(&rp[i], pp, l);
        rp[i + l] = '\0';
        text = svtkParsePreprocess_ProcessString(info, &rp[i]);
        if (text)
        {
          l = strlen(text);
          if (text != &rp[i])
          {
            ...
            /* memmove as we intentionally are not copying /0 */
            memmove(&rp[i], text, l);
            svtkParsePreprocess_FreeProcessedString(info, text);
            if (tp && tp != stack_rp)
            {
              free(tp);
            }
 ...
 * Process a string
 */
const char* svtkParsePreprocess_ProcessString(PreprocessInfo* info, const char* text)
{
  char stack_rp[128];
  char* rp;
  char* ep;
  ...
  while (tokens.tok)
  {
    ...
    if (tokens.tok == TOK_ID)
    {
      ...
      if (macro && !macro->IsExcluded)
      {
        ...
        if (expand)
        {
          ...
          macro->IsExcluded = 1;
          processed = svtkParsePreprocess_ProcessString(info, expansion);
          macro->IsExcluded = 0;
          if (processed == NULL)
          {
            svtkParsePreprocess_FreeMacroExpansion(info, macro, expansion);
          ...
          {
            svtkParsePreprocess_FreeProcessedString(info, processed);
          }
          svtkParsePreprocess_FreeMacroExpansion(info, macro, expansion);
        }
      }
    ...
    l = tokens.len;
    cp = tokens.text;
    if (svtkParse_NextToken(&tokens) && tokens.text > cp + l)
    {
      rp[i++] = ' ';
    }
  }
  ...
  else
  {
    ...
    const char* tp = svtkParsePreprocess_ProcessString(info, rp);
    if (rp != tp)
    {
      if (rp != stack_rp)
      {
 ...
 */
void svtkParsePreprocess_FreeMacroExpansion(PreprocessInfo* info, MacroInfo* macro, const char* text)
{
 ...
 */
void svtkParsePreprocess_FreeProcessedString(PreprocessInfo* info, const char* text)
{
  if (info)
  {
    free((char*)text);
 ...
 */
void svtkParsePreprocess_IncludeDirectory(PreprocessInfo* info, const char* name)
{
  int i, n;
 ...
 */
const char* svtkParsePreprocess_FindIncludeFile(
 ...
 */
void svtkParsePreprocess_InitMacro(MacroInfo* macro)
{
  macro->Name = NULL;
 ...
 * Free a preprocessor macro struct
 */
void svtkParsePreprocess_FreeMacro(MacroInfo* macro)
{
 ...
 */
void svtkParsePreprocess_Init(PreprocessInfo* info, const char* filename)
{
  info->FileName = NULL;
 ...
 */
void svtkParsePreprocess_Free(PreprocessInfo* info)
{
  int i, n;
  MacroInfo** mptr;
  ...
  if (info->MacroHashTable)
  {
    ...
    for (i = 0; i < n; i++)
    {
      ...
      if (mptr)
      {
        while (*mptr)
        {
          svtkParsePreprocess_FreeMacro(*mptr++);
        }
      }
