...
/**
  ...
  svtkParsePreprocess_HandleDirective() function, which will
...
#ifndef svtkParsePreprocess_h
#define svtkParsePreprocess_h
...
{
  SVTK_PARSE_CURDIR_INCLUDE, /* look in current directory first */
  SVTK_PARSE_SOURCE_INCLUDE, /* look in source directory first */
  SVTK_PARSE_SYSTEM_INCLUDE  /* search system directories first */
} preproc_search_t;
...
{
  SVTK_PARSE_OK = 0,
  SVTK_PARSE_SKIP = 1,            /* skip next block */
  SVTK_PARSE_PREPROC_DOUBLE = 2,  /* encountered a double */
  SVTK_PARSE_PREPROC_FLOAT = 3,   /* encountered a float */
  SVTK_PARSE_PREPROC_STRING = 4,  /* encountered a string */
  SVTK_PARSE_MACRO_UNDEFINED = 5, /* macro lookup failed */
  SVTK_PARSE_MACRO_REDEFINED = 6, /* attempt to redefine a macro */
  SVTK_PARSE_FILE_NOT_FOUND = 7,  /* include file not found */
  SVTK_PARSE_FILE_OPEN_ERROR = 8, /* include file not readable */
  SVTK_PARSE_FILE_READ_ERROR = 9, /* error during read */
  SVTK_PARSE_MACRO_NUMARGS = 10,  /* wrong number of args to func macro */
  SVTK_PARSE_SYNTAX_ERROR = 11,   /* any and all syntax errors */
  SVTK_PARSE_OUT_OF_MEMORY = 12   /* out-of-memory */
};

/**
 * Bitfield for fatal errors.
 */
#define SVTK_PARSE_FATAL_ERROR 0xF8
...
extern "C"
{
  ...
  /**
   * Handle a preprocessor directive.  Return value SVTK_PARSE_OK
   * means that no errors occurred, while SVTK_PARSE_SKIP means that
   ...
   * value of SVTK_PARSE_OK means that no errors occurred, while
   * SVTK_PREPROC_DOUBLE, SVTK_PREPROC_FLOAT, and SVTK_PREPROC_STRING
   * indicate that the preprocessor encountered a non-integer value.
   * Error return values are SVTK_PARSE_MACRO_UNDEFINED and
   * SVTK_PARSE_SYNTAX_ERRORS.  Undefined macros evaluate to zero.
   */
  SVTKWRAPPINGTOOLS_EXPORT
  ...
  /**
   * Add all standard preprocessor symbols. Use SVTK_PARSE_NATIVE for
   * the platform to add the same macros as the native compiler.  For
   * cross-compiling, use SVTK_PARSE_UNDEF and then define the macros
   ...
   * values are SVTK_PARSE_OK and SVTK_PARSE_MACRO_REDEFINED.
   */
  SVTKWRAPPINGTOOLS_EXPORT
  int svtkParsePreprocess_AddMacro(PreprocessInfo* info, const char* name, const char* definition);

  /**
   * Remove a preprocessor symbol.  Return values are SVTK_PARSE_OK
   * and SVTK_PARSE_MACRO_UNDEFINED.
   */
  SVTKWRAPPINGTOOLS_EXPORT
   ...
   */
  SVTKWRAPPINGTOOLS_EXPORT
  void svtkParsePreprocess_FreeMacroExpansion(
  ...
  /**
   * Find an include file in the path.  If order is SVTK_PARSE_SYSTEM_INCLUDE,
   ...
   * will be SVTK_PARSE_OK if no errors occurred.
   */
  SVTKWRAPPINGTOOLS_EXPORT
   ...
   */
  SVTKWRAPPINGTOOLS_EXPORT
  void svtkParsePreprocess_InitMacro(MacroInfo* symbol);
...
#endif
/* SVTK-HeaderTest-Exclude: svtkParsePreprocess.h */
