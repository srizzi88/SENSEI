...
int svtkParse_CharType(char c, int bits)
{
  return parse_chartype(c, bits);
}

/** Skip over a comment. */
size_t svtkParse_SkipComment(const char* text)
{
  const char* cp = text;
...
size_t svtkParse_SkipWhitespace(const char* text, parse_space_t spacetype)
{
  const char* cp = text;
...
size_t svtkParse_SkipQuotes(const char* text)
{
  const char* cp = text;
  const char qc = *cp;
...
/** Skip over a number. */
size_t svtkParse_SkipNumber(const char* text)
{
  const char* cp = text;
...
/** Skip over a name. */
size_t svtkParse_SkipId(const char* text)
{
  const char* cp = text;
...
{
  size_t l = svtkParse_SkipQuotes(cp);
  if (l && cp[l] == '_')
  {
    l += svtkParse_SkipId(cp + l);
  }
  return l;
}
...
int svtkParse_NextToken(StringTokenizer* tokens)
{
  ...
  if (*cp == '/' || *cp == '\\')
  {
    cp += svtkParse_SkipWhitespace(cp, tokens->ws);
  }
  ...
  else if (parse_chartype(*cp, CPRE_DIGIT) || (cp[0] == '.' && parse_chartype(cp[1], CPRE_DIGIT)))
  {
    ...
    tokens->hash = 0;
    tokens->text = cp;
    tokens->len = svtkParse_SkipNumber(cp);
  }
  else if (cp[0] == '/' && (cp[1] == '/' || cp[1] == '*'))
  {
    ...
    tokens->hash = 0;
    tokens->text = cp;
    tokens->len = svtkParse_SkipComment(cp);
  }
  else
  {
    int t = cp[0];
...
const char* svtkParse_CacheString(StringCache* cache, const char* in, size_t n)
{
  char* res = svtkParse_NewString(cache, n);
  strncpy(res, in, n);
  res[n] = '\0';
