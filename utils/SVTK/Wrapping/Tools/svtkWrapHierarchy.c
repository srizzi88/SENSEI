...
/**
 The svtkWrapHierarchy program builds a text file that describes the
 class hierarchy.
...
static char** append_unique_line(char** lines, char* line, size_t* np)
{
  ...
  n = *np;
  m = svtkParse_NameLength(line);

  /* check to make sure this line isn't a duplicate */
  for (l = 0; l < n; l++)
  {
    if (svtkParse_NameLength(lines[l]) == m && strncmp(line, lines[l], m) == 0)
    {
      break;
    }
  }
...
static char* append_typedef_to_line(char* line, size_t* m, size_t* maxlen, ValueInfo* typedef_info)
{
  ...
  type = (type & SVTK_PARSE_POINTER_MASK);
  if (ndims > 0 && (type & SVTK_PARSE_POINTER_LOWMASK) == SVTK_PARSE_ARRAY)
  {
    type = ((type >> 2) & SVTK_PARSE_POINTER_MASK);
  }
  else if (ndims == 1)
  {
    type = ((type >> 2) & SVTK_PARSE_POINTER_MASK);
  ...
  while (type)
  {
    unsigned int bits = (type & SVTK_PARSE_POINTER_LOWMASK);
    type = ((type >> 2) & SVTK_PARSE_POINTER_MASK);

    if (bits == SVTK_PARSE_POINTER)
    {
      line = append_to_line(line, "*", m, maxlen);
    }
    else if (bits == SVTK_PARSE_CONST_POINTER)
    {
  ...
  const char* header_file, const char* module_name)
{
  ...
  for (i = 0; i < data->NumberOfItems; i++)
  {
    ...
    if (data->Items[i].Type == SVTK_CLASS_INFO || data->Items[i].Type == SVTK_STRUCT_INFO)
    {
    ...
    else if (data->Items[i].Type == SVTK_ENUM_INFO)
    {
      line = append_scope_to_line(line, &m, &maxlen, scope);
      line = append_enum_to_line(line, &m, &maxlen, data->Enums[data->Items[i].Index]);
    }
    else if (data->Items[i].Type == SVTK_TYPEDEF_INFO)
    {
      line = append_scope_to_line(line, &m, &maxlen, scope);
  ...
  const char* scope, const char* header_file, const char* module_name, const char* flags)
{
  ...
  for (i = 0; i < data->NumberOfItems; i++)
  {
    ...
    else if (data->Items[i].Type != SVTK_NAMESPACE_INFO)
    {
      /* unhandled file element */
      continue;
    }

    if (data->Items[i].Type != SVTK_NAMESPACE_INFO)
    {
      /* append filename and flags */
      line = append_trailer(line, &m, &maxlen, header_file, module_name, tmpflags);
    ...
    if (data->Items[i].Type == SVTK_NAMESPACE_INFO && data->Namespaces[data->Items[i].Index]->Name)
    {
...
/**
 * Read a header file with svtkParse.tab.c
 *
 * If "lines" is provided, the file contents
 * will be appended to them.
 */
static char** svtkWrapHierarchy_ParseHeaderFile(
  FILE* fp, const char* filename, const char* module_name, const char* flags, char** lines)
{
  ...
  data = svtkParse_ParseFile(filename, fp, stderr);
 ...
 */
static char** svtkWrapHierarchy_ReadHierarchyFile(FILE* fp, char** lines)
{
  char* line;
  size_t maxlen = 15;
  size_t i, n;
 ...
 */
static int svtkWrapHierarchy_CompareHierarchyFile(FILE* fp, char* lines[])
{
 ...
 */
static int svtkWrapHierarchy_WriteHierarchyFile(FILE* fp, char* lines[])
{
  size_t i;
 ...
 */
static char** svtkWrapHierarchy_TryParseHeaderFile(
  const char* file_name, const char* module_name, const char* flags, char** lines)
{
  ...
  if (!input_file)
  {
    fprintf(stderr, "svtkWrapHierarchy: couldn't open file %s\n", file_name);
    exit(1);
  }

  lines = svtkWrapHierarchy_ParseHeaderFile(input_file, file_name, module_name, flags, lines);
 ...
 */
static char** svtkWrapHierarchy_TryReadHierarchyFile(const char* file_name, char** lines)
{
  FILE* input_file;
  ...
  lines = svtkWrapHierarchy_ReadHierarchyFile(input_file, lines);
  if (!lines)
  {
    fclose(input_file);
    fprintf(stderr, "svtkWrapHierarchy: error reading file %s\n", file_name);
    exit(1);
  }
  fclose(input_file);
 ...
 */
static int svtkWrapHierarchy_TryWriteHierarchyFile(const char* file_name, char* lines[])
{
  FILE* output_file;
  int matched = 0;

  output_file = fopen(file_name, "r");
  if (output_file && svtkWrapHierarchy_CompareHierarchyFile(output_file, lines))
  {
    matched = 1;
  }
  if (output_file)
  ...
  if (!matched)
  {
    ...
    {
      /* There are two CMAKE_CUSTOM_COMMANDS for svtkWrapHierarchy,
    ...
    if (!output_file)
    {
      fprintf(stderr, "svtkWrapHierarchy: tried %i times to write %s\n", tries, file_name);
      exit(1);
    }
    if (!svtkWrapHierarchy_WriteHierarchyFile(output_file, lines))
    {
      fclose(output_file);
      fprintf(stderr, "svtkWrapHierarchy: error writing file %s\n", file_name);
      exit(1);
    }
    fclose(output_file);
  }
...
int main(int argc, char* argv[])
{
  ...
  svtkParse_DefineMacro("__SVTK_WRAP_HIERARCHY__", 0);

  /* parse command-line options */
  StringCache* string_cache = svtkParse_MainMulti(argc, argv);
  ...
  /* read the data file */
  files = svtkWrapHierarchy_TryReadHierarchyFile(options->InputFileName, files);

  /* read in all the prior files */
  for (i = 1; i < options->NumberOfFiles; i++)
  {
    lines = svtkWrapHierarchy_TryReadHierarchyFile(options->Files[i], lines);
  }
  ...
  for (i = 0; files[i] != NULL; i++)
  {
    ...
    lines = svtkWrapHierarchy_TryParseHeaderFile(files[i], module_name, flags, lines);
  }
  ...
  svtkWrapHierarchy_TryWriteHierarchyFile(options->OutputFileName, lines);
  ...
  svtkParse_FreeStringCache(string_cache);
  free(string_cache);
  free(files);
  free(lines);
  return 0;
