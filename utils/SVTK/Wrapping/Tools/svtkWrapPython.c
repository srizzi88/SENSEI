...
/* -------------------------------------------------------------------- */
/* the main entry method, called by svtkParse.y */
void svtkParseOutput(FILE* fp, FileInfo* data);

/* -------------------------------------------------------------------- */
/* prototypes for the methods used by the python wrappers */

/* get the header file for the named SVTK class */
static const char* svtkWrapPython_ClassHeader(HierarchyInfo* hinfo, const char* classname);

/* get the module for the named SVTK class */
static const char* svtkWrapPython_ClassModule(HierarchyInfo* hinfo, const char* classname);

/* print out headers for any special types used by methods */
static void svtkWrapPython_GenerateSpecialHeaders(
  ...
  FILE* fp, FileInfo* file_info, HierarchyInfo* hinfo)
{
  ...
  /* always include svtkVariant, it is often used as a template arg
  ...
  for (ii = 0; ii < nn; ii++)
  {
    ...
    for (i = 0; i < n; i++)
    {
      ...
      if (currentFunction->Access == SVTK_ACCESS_PUBLIC && !currentFunction->IsExcluded &&
        strcmp(currentFunction->Class, data->Name) == 0)
      {
        classname = "void";
        aType = SVTK_PARSE_VOID;
        ...
        for (j = -1; j < m; j++)
        {
          ...
          {
            if ((aType & SVTK_PARSE_BASE_TYPE) == SVTK_PARSE_STRING)
            {
              classname = "svtkStdString";
            }
            else if ((aType & SVTK_PARSE_BASE_TYPE) == SVTK_PARSE_UNICODE_STRING)
            {
              classname = "svtkUnicodeString";
            }
            else if ((aType & SVTK_PARSE_BASE_TYPE) != SVTK_PARSE_OBJECT)
            {
              classname = 0;
            }
            else if ((aType & SVTK_PARSE_REF) != 0)
            {
              classname = 0;
            }
          }
  ...
  if (data)
  {
    ownincfile = svtkWrapPython_ClassHeader(hinfo, data->Name);
  }
  ...
  {
    const char* incfile;
    incfile = svtkWrapPython_ClassHeader(hinfo, types[i]);
  ...
  /* special case for the way svtkGenericDataArray template is used */
  if (data && strcmp(data->Name, "svtkGenericDataArray") == 0)
  {
    fprintf(fp,
 ...
 * it will print the svtkXXPython.c file contents to "fp".  */
...
int main(int argc, char* argv[])
{
  ...
  unsigned char wrapAsSVTKObject[MAX_WRAPPED_CLASSES];
  ClassInfo* data = NULL;
  ...
  FileInfo* file_info;
  FILE* fp;
  const char* module = "svtkCommonCore";
  const char* name;
  ...
  svtkParse_DefineMacro("__SVTK_WRAP_PYTHON__", 0);
  ...
  if (hinfo)
  {
    ...
    {
      svtkWrap_MergeSuperClasses(contents->Classes[i], file_info, hinfo);
    }
    for (i = 0; i < contents->NumberOfClasses; i++)
    {
      svtkWrap_ExpandTypedefs(contents->Classes[i], file_info, hinfo);
    }
  }

  /* the SVTK_WRAPPING_CXX tells header files where they're included from */
  fprintf(fp,
    "// python wrapper for %s\n//\n"
    "#define SVTK_WRAPPING_CXX\n",
    name);

  /* unless this is svtkObjectBase.h, define SVTK_STREAMS_FWD_ONLY */
  if (strcmp("svtkObjectBase", name) != 0)
  {
    /* Block inclusion of full streams.  */
    fprintf(fp, "#define SVTK_STREAMS_FWD_ONLY\n");
  }

  /* lots of important utility functions are defined in svtkPythonArgs.h */
  fprintf(fp,
  ...
  /* svtkPythonCommand is needed to wrap svtkObject.h */
  if (strcmp("svtkObject", name) == 0)
  {
    fprintf(fp, "#include \"svtkPythonCommand.h\"\n");
  }

  /* generate includes for any special types that are used */
  svtkWrapPython_GenerateSpecialHeaders(fp, file_info, hinfo);
  ...
  fprintf(
    fp, "extern \"C\" { %s void PySVTKAddFile_%s(PyObject *dict); }\n", "SVTK_ABI_EXPORT", name);
  ...
  if (data && hinfo)
  {
    module = svtkWrapPython_ClassModule(hinfo, data->Name);
  }

  /* Identify all enum types that are used by methods */
  svtkWrapPython_MarkAllEnums(file_info->Contents, hinfo);

  /* Wrap any enum types defined in the global namespace */
  for (i = 0; i < contents->NumberOfEnums; i++)
  {
    svtkWrapPython_GenerateEnumType(fp, module, NULL, contents->Enums[i]);
  }
  ...
  for (i = 0; i < contents->NumberOfNamespaces; i++)
  {
    ...
    {
      svtkWrapPython_WrapNamespace(fp, module, contents->Namespaces[i]);
  ...
  for (i = 0; i < contents->NumberOfClasses; i++)
  {
    ...
    /* guess whether type is a svtkobject */
    is_svtkobject = (data == file_info->MainClass ? 1 : 0);
    if (hinfo)
    {
      is_svtkobject = svtkWrap_IsTypeOf(hinfo, data->Name, "svtkObjectBase");
    }
    ...
    wrapAsSVTKObject[i] = (is_svtkobject ? 1 : 0);
  }
  ...
  for (i = 0; i < contents->NumberOfClasses; i++)
  {
    ...
    is_svtkobject = wrapAsSVTKObject[i];

    /* if "hinfo" is present, wrap everything, else just the main class */
    if (hinfo || data == file_info->MainClass)
    {
      if (svtkWrapPython_WrapOneClass(fp, module, data->Name, data, file_info, hinfo, is_svtkobject))
      {
        /* re-index wrapAsSVTKObject for wrapped classes */
        wrapAsSVTKObject[numberOfWrappedClasses] = (is_svtkobject ? 1 : 0);
  ...
  for (i = 0; i < numberOfWrappedClasses; i++)
  {
    ...
    if (data->Template)
    {
      ...
      fprintf(fp,
        ...
        "      if (nt)\n"
        "      {\n"
        "        nt = svtkPythonUtil::StripModule(nt);\n"
    ...
    else if (is_svtkobject)
    {
      /* Class is derived from svtkObjectBase */
      fprintf(fp,
        "  o = Py%s_ClassNew();\n"
        "\n",
        data->Name);
    ...
    {
      /* Classes that are not derived from svtkObjectBase */
      fprintf(fp,
        "  o = Py%s_TypeNew();\n"
        "\n",
        data->Name);
  ...
  svtkWrapPython_AddPublicEnumTypes(fp, "  ", "dict", "o", contents);

  /* add any constants defined in the file */
  svtkWrapPython_AddPublicConstants(fp, "  ", "dict", "o", contents);
