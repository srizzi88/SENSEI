/*=========================================================================
  ...
  Module:    svtkWrapPythonConstant.c
...
/* The scope, attrib, and valstring parameters are optional and can be
   ...
   val->Value is used.
*/
void svtkWrapPython_AddConstantHelper(FILE* fp, const char* indent, const char* dictvar,
  ...
  const char* attribval, ValueInfo* val)
{
  ...
  valtype = (val->Type & SVTK_PARSE_UNQUALIFIED_TYPE);
  valname = val->Name;
  valstring = attribval;
  if (valstring == 0)
  {
  ...
  FILE* fp, const char* indent, const char* dictvar, const char* objvar, NamespaceInfo* data)
{
  ...
  if (scope)
  {
    ...
    else
    {
      ...
      svtkWrapText_PythonName(scope, text);
      pythonscope = text;
    }
  }
  ...
  while (j < data->NumberOfConstants)
  {
    ...
    if (val->Access != SVTK_ACCESS_PUBLIC)
    {
      j++;
      continue;
    }
    ...
    if (j + 1 == data->NumberOfConstants || val->Type != data->Constants[j + 1]->Type ||
      !svtkWrap_IsScalar(val) || (!val->IsEnum && !svtkWrap_IsNumeric(val)))
    {
      svtkWrapPython_AddConstant(fp, indent, dictvar, objvar, scope, val);
      j++;
      continue;
    }
    ...
    valtype = val->Type;
    typeName = (val->IsEnum ? val->Class : svtkWrap_GetTypeName(val));
    ...
    for (k = j; k < data->NumberOfConstants; k++)
    {
      ...
      if (val->Access == SVTK_ACCESS_PUBLIC)
      {
        tname = (val->IsEnum ? val->Class : svtkWrap_GetTypeName(val));
    ...
    /* check to make sure there won't be a name conflict between an
       ...
       for svtkImplicitBoolean which has a variable and enum type both
    ...
    svtkWrapPython_AddConstantHelper(fp, nextindent, dictvar, objvar, scope, pythonscope,
...
{
  svtkWrapPython_AddConstantHelper(fp, indent, dictvar, objvar, scope, scope, NULL, NULL, val);
}
