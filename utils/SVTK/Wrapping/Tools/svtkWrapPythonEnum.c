...
int svtkWrapPython_IsEnumWrapped(HierarchyInfo* hinfo, const char* enumname)
{
  ...
  if (hinfo && enumname)
  {
    entry = svtkParseHierarchy_FindEntry(hinfo, enumname);
...
void svtkWrapPython_MarkAllEnums(NamespaceInfo* contents, HierarchyInfo* hinfo)
{
  ...
  for (ii = 0; ii < nn; ii++)
  {
    ...
    for (i = 0; i < n; i++)
    {
      ...
      if (!currentFunction->IsExcluded && currentFunction->Access == SVTK_ACCESS_PUBLIC)
      {
        ...
        for (j = (val ? -1 : 0); j < m; j++)
        {
          ...
          if (svtkWrap_IsEnumMember(data, val) || svtkWrapPython_IsEnumWrapped(hinfo, val->Class))
          {
            val->IsEnum = 1;
          }
        }
  ...
  const char* objvar, const char* scope, EnumInfo* cls)
{
  ...
  fprintf(fp,
    "%sPySVTKEnum_Add(&Py%s%s%s_Type, \"%s%s%s\");\n"
    "\n",
  ...
  FILE* fp, const char* module, const char* classname, EnumInfo* data)
{
  ...
  fprintf(fp,
    ...
    "{\n"
    "  return PySVTKEnum_New(&Py%s_Type, static_cast<int>(val));\n"
    "}\n"
    "\n",
    enumname, enumname);
}
  ...
  FILE* fp, const char* indent, const char* dictvar, const char* objvar, NamespaceInfo* data)
{
  ...
  if (data->Name)
  {
    ...
    svtkWrapText_PythonName(data->Name, text);
    pythonname = text;
  }
  ...
  for (i = 0; i < data->NumberOfEnums; i++)
  {
    ...
    {
      svtkWrapPython_AddEnumType(fp, indent, dictvar, objvar, pythonname, data->Enums[i]);
      fprintf(fp, "\n");
    }
  }
}
