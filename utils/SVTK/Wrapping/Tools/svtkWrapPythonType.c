 ...
 * i.e. types that are not derived from svtkObjectBase */
/* -------------------------------------------------------------------- */

/* -------------------------------------------------------------------- */
/* check if class has a wrapped constructor, and return its name if so */
static const char* svtkWrapPython_WrappedConstructor(
  ClassInfo* data, HierarchyInfo* hinfo, size_t* np)
{
  ...
  m = svtkParse_UnscopedNameLength(constructor);
  ...
  for (i = 0; i < data->NumberOfFunctions; i++)
  {
    ...
    if (theFunc->Name && strncmp(theFunc->Name, constructor, n) == 0 && theFunc->Name[n] == '\0' &&
      !theFunc->Template && svtkWrapPython_MethodCheck(data, theFunc, hinfo))
    {
      *np = n;
      return constructor;
    }
...
static void svtkWrapPython_NewDeleteProtocol(
  FILE* fp, const char* classname, ClassInfo* data, HierarchyInfo* hinfo)
{
  ...
  if (!data->IsAbstract)
  {
    constructor = svtkWrapPython_WrappedConstructor(data, hinfo, &n);
  }
...
/* -------------------------------------------------------------------- */
/* generate function for printing a special object */
static void svtkWrapPython_PrintProtocol(
  FILE* fp, const char* classname, ClassInfo* data, FileInfo* finfo, SpecialTypeInfo* info)
{
  ...
  for (i = 0; i < finfo->Contents->NumberOfFunctions; i++)
  {
      ...
      strcmp(func->Name, "operator<<") == 0)
    {
      if (func->NumberOfParameters == 2 &&
        (func->Parameters[0]->Type & SVTK_PARSE_UNQUALIFIED_TYPE) == SVTK_PARSE_OSTREAM_REF &&
        (func->Parameters[1]->Type & SVTK_PARSE_BASE_TYPE) == SVTK_PARSE_OBJECT &&
        (func->Parameters[1]->Type & SVTK_PARSE_POINTER_MASK) == 0 &&
        !svtkWrap_IsNonConstRef(func->Parameters[1]) &&
  ...
  if (info->has_print)
  {
    fprintf(fp,
      ...
      "  if (obj->svtk_ptr)\n"
      "  {\n"
      "    os << *static_cast<const %s *>(obj->svtk_ptr);\n"
      "  }\n"
...
static void svtkWrapPython_RichCompareProtocol(
  FILE* fp, const char* classname, ClassInfo* data, FileInfo* finfo, SpecialTypeInfo* info)
{
  ...
  for (i = 0; i < n; i++)
  {
    if (i < data->NumberOfFunctions)
    {
      ...
      if (func->NumberOfParameters != 1 ||
        (func->Parameters[0]->Type & SVTK_PARSE_BASE_TYPE) != SVTK_PARSE_OBJECT ||
        (func->Parameters[0]->Type & SVTK_PARSE_POINTER_MASK) != 0 ||
  ...
  if (compare_ops != 0)
  {
    ...
    for (i = 1; i <= 2; i++)
    {
      ...
      fprintf(fp,
        ...
        "  {\n"
        "    PySVTKSpecialObject *s%d = (PySVTKSpecialObject *)o%d;\n"
        "    so%d = static_cast<const %s *>(s%d->svtk_ptr);\n"
        "  }\n"
        "  else\n"
        "  {\n"
        "    so%d = static_cast<const %s *>(\n"
        "      svtkPythonUtil::GetPointerFromSpecialObject(\n"
        "        o%d, \"%s\", &n%d));\n"
        "    if (so%d == nullptr)\n"
        "    {\n"
        "      PyErr_Clear();\n"
...
static void svtkWrapPython_SequenceProtocol(
  FILE* fp, const char* classname, ClassInfo* data, HierarchyInfo* hinfo, SpecialTypeInfo* info)
{
  ...
  for (i = 0; i < data->NumberOfFunctions; i++)
  {
    ...
    if (func->Name && func->IsOperator && !func->IsDeleted && !func->IsExcluded &&
      strcmp(func->Name, "operator[]") == 0 && svtkWrapPython_MethodCheck(data, func, hinfo))
    {
      if (func->NumberOfParameters == 1 && func->ReturnValue &&
        svtkWrap_IsInteger(func->Parameters[0]))
      {
        if (!setItemFunc && svtkWrap_IsNonConstRef(func->ReturnValue))
        {
          setItemFunc = func;
        }
  ...
  if (getItemFunc && getItemFunc->SizeHint)
  {
    ...
    fprintf(fp,
      ...
      "{\n"
      "  void *vp = svtkPythonArgs::GetSelfSpecialPointer(self);\n"
    ...
    svtkWrapPython_DeclareVariables(fp, data, getItemFunc);
    ...
    fprintf(fp,
      ...
      "  else\n"
      "  {\n",
      svtkWrap_GetTypeName(getItemFunc->Parameters[0]), getItemFunc->SizeHint);

    fprintf(fp, "  ");
    svtkWrap_DeclareVariable(
      fp, data, getItemFunc->ReturnValue, "tempr", -1, SVTK_WRAP_RETURN | SVTK_WRAP_NOSEMI);

    fprintf(fp,
      " = %s(*op)[temp0];\n"
      "\n",
      (svtkWrap_IsRef(getItemFunc->ReturnValue) ? "&" : ""));

    svtkWrapPython_ReturnValue(fp, data, getItemFunc->ReturnValue, 1);
    ...
    if (setItemFunc)
    {
      ...
      svtkWrap_DeclareVariable(fp, data, setItemFunc->Parameters[0], "temp", 0, SVTK_WRAP_ARG);
      svtkWrap_DeclareVariable(fp, data, setItemFunc->ReturnValue, "temp", 1, SVTK_WRAP_ARG);
      ...
      fprintf(fp,
        ...
        "  }\n"
        "  else if (",
        svtkWrap_GetTypeName(setItemFunc->Parameters[0]), getItemFunc->SizeHint);

      svtkWrapPython_GetSingleArgument(fp, data, 1, setItemFunc->ReturnValue, 1);
      ...
      fprintf(fp,
        ...
        "    (*op)[temp0] = %stemp1;\n"
        "\n",
        ((svtkWrap_IsRef(getItemFunc->ReturnValue) && svtkWrap_IsObject(getItemFunc->ReturnValue))
            ? "*"
            : ""));
...
static void svtkWrapPython_HashProtocol(FILE* fp, const char* classname, ClassInfo* data)
{
  /* the hash function, defined only for specific types */
  fprintf(fp, "static Py_hash_t Py%s_Hash(PyObject *self)\n", classname);

  if (strcmp(data->Name, "svtkTimeStamp") == 0)
  {
    /* hash for svtkTimeStamp is just the timestamp itself */
    fprintf(fp,
      "{\n"
      "  PySVTKSpecialObject *obj = (PySVTKSpecialObject *)self;\n"
      "  const svtkTimeStamp *op = static_cast<const svtkTimeStamp *>(obj->svtk_ptr);\n"
      "  svtkMTimeType mtime = *op;\n"
      "  long h = (long)mtime;\n"
      "  if (h != -1) { return h; }\n"
      "  return -2;\n"
      "}\n"
  ...
  else if (strcmp(data->Name, "svtkVariant") == 0)
  {
    /* hash for svtkVariant is cached to avoid recomputation, this is
     * safe because svtkVariant is an immutable object, and is necessary
     * because computing the hash for svtkVariant is very expensive */
    fprintf(fp,
      "{\n"
      "  PySVTKSpecialObject *obj = (PySVTKSpecialObject *)self;\n"
      "  const svtkVariant *op = static_cast<const svtkVariant *>(obj->svtk_ptr);\n"
      "  long h = obj->svtk_hash;\n"
      "  if (h != -1)\n"
      "  {\n"
      "    return h;\n"
      "  }\n"
      "  h = svtkPythonUtil::VariantHash(op);\n"
      "  obj->svtk_hash = h;\n"
      "  return h;\n"
      "}\n"
      "\n");
...
static void svtkWrapPython_SpecialTypeProtocols(FILE* fp, const char* classname, ClassInfo* data,
  FileInfo* finfo, HierarchyInfo* hinfo, SpecialTypeInfo* info)
{
  ...
  svtkWrapPython_NewDeleteProtocol(fp, classname, data, hinfo);
  svtkWrapPython_PrintProtocol(fp, classname, data, finfo, info);
  svtkWrapPython_RichCompareProtocol(fp, classname, data, finfo, info);
  svtkWrapPython_SequenceProtocol(fp, classname, data, hinfo, info);
  svtkWrapPython_HashProtocol(fp, classname, data);
}

/* -------------------------------------------------------------------- */
/* For classes that aren't derived from svtkObjectBase, check to see if
 * they are wrappable */
  ...
  ClassInfo* data, FileInfo* finfo, HierarchyInfo* hinfo)
{
  ...
  /* get the superclass */
  supermodule = svtkWrapPython_HasWrappedSuperClass(hinfo, data->Name, &is_external);
  if (supermodule)
  {
    has_superclass = 1;
    name = svtkWrapPython_GetSuperClass(data, hinfo);
    svtkWrapText_PythonName(name, supername);
  }

  /* generate all constructor methods */
  if (constructor)
  {
    svtkWrapPython_GenerateMethods(fp, classname, data, finfo, hinfo, 0, 1);
  }

  /* generate all functions and protocols needed for the type */
  svtkWrapPython_SpecialTypeProtocols(fp, classname, data, finfo, hinfo, &info);
  ...
  fprintf(fp,
    ...
    "  sizeof(PySVTKSpecialObject), // tp_basicsize\n"
    "  0, // tp_itemsize\n"
    ...
    "  nullptr, // tp_setattr\n"
    "  nullptr, // tp_compare\n"
    "  PySVTKSpecialObject_Repr, // tp_repr\n",
  ...
  {
    fprintf(fp, "  PySVTKSpecialObject_SequenceString, // tp_str\n");
  }
  else
  {
  ...
  if (constructor && svtkWrap_HasPublicCopyConstructor(data))
  {
    has_copycons = 1;
  ...
  /* the method for adding the SVTK extras to the type,
  ...
  {
    fprintf(fp,
      "  PyTypeObject *pytype = PySVTKSpecialType_Add(\n"
      "    &Py%s_Type,\n"
      "    Py%s_Methods,\n"
      "    Py%s_%*.*s_Methods,\n"
      "    &Py%s_CCopy);\n"
  ...
  if (has_superclass)
  {
    ...
    {
      fprintf(fp,
        "  pytype->tp_base = svtkPythonUtil::FindSpecialTypeObject("
        "\"%s\");\n\n",
        supername);
    }
  }
